/*
 * XREFs of NtUserPostMessage @ 0x1C004F200
 * Callers:
 *     <none>
 * Callees:
 *     ?IsMessageAllowedAcrossILByReceiver@@YAHPEAUtagPROCESSINFO@@0PEAUtagWND@@I_K_JH@Z @ 0x1C001A3EC (-IsMessageAllowedAcrossILByReceiver@@YAHPEAUtagPROCESSINFO@@0PEAUtagWND@@I_K_JH@Z.c)
 *     MSGSQMAddMessage @ 0x1C001A558 (MSGSQMAddMessage.c)
 *     ?IsMessageAlwaysAllowedAcrossIL@@YAHI@Z @ 0x1C001A5DC (-IsMessageAlwaysAllowedAcrossIL@@YAHI@Z.c)
 *     ?_PostTransformableMessageExtended@@YAPEAUtagQMSG@@PEAUtagWND@@I_K_JPEAUtagINPUT_MESSAGE_SOURCE@@H@Z @ 0x1C004F5A0 (-_PostTransformableMessageExtended@@YAPEAUtagQMSG@@PEAUtagWND@@I_K_JPEAUtagINPUT_MESSAGE_SOURCE@.c)
 *     IAMThreadAccessGranted @ 0x1C008F634 (IAMThreadAccessGranted.c)
 *     UserSetLastError @ 0x1C00D60E0 (UserSetLastError.c)
 *     ForwardTouchMessage @ 0x1C01DBEFC (ForwardTouchMessage.c)
 *     ValidateDDEConvPair @ 0x1C021C6A0 (ValidateDDEConvPair.c)
 */

// write access to const memory has been detected, the output may be wrong!
__int64 __fastcall NtUserPostMessage(unsigned __int64 a1, unsigned int a2, unsigned __int64 a3, volatile void *a4)
{
  __int64 v4; // rbx
  unsigned __int64 v8; // rcx
  __int64 v9; // r8
  __int64 v10; // rdx
  int v11; // edi
  __int64 v12; // rsi
  BOOL v13; // ebp
  __int64 v14; // rbp
  struct tagPROCESSINFO *CurrentProcessWin32Process; // rax
  struct tagPROCESSINFO *v16; // r14
  struct tagQMSG *v17; // rax
  unsigned __int8 v19; // al
  int v20; // r13d
  char v21; // al
  __int16 v22; // cx
  __int64 v23; // rcx
  __int64 v25; // rdx
  __int64 v26; // [rsp+50h] [rbp-38h] BYREF

  v4 = a2;
  EnterCrit(0LL, 1LL);
  gbValidateHandleForIL = 0;
  if ( (v4 & 0xFFFE0000) != 0 )
  {
    UserSetLastError(87LL);
    v11 = 0;
    goto LABEL_21;
  }
  v10 = 0x2000LL;
  if ( (unsigned int)v4 < 0x400 )
  {
    v8 = (unsigned __int16)MessageTable[v4];
    if ( (v8 & 0x2000) != 0 )
    {
      v11 = 0;
      v23 = 5LL;
LABEL_60:
      UserSetLastError(v23);
      goto LABEL_21;
    }
  }
  v11 = 0;
  if ( a1 >= 0x10000 && a1 != -1LL )
    goto LABEL_5;
  if ( !a1 )
  {
    v12 = 0LL;
    goto LABEL_6;
  }
  if ( a1 == 0xFFFF || a1 == -1LL )
  {
    v12 = -1LL;
  }
  else
  {
LABEL_5:
    v12 = ValidateHwnd(a1);
    if ( !v12 )
    {
      if ( (_DWORD)v4 == 993 )
        v11 = 1;
      goto LABEL_21;
    }
  }
LABEL_6:
  if ( (_DWORD)v4 == 576 )
  {
    if ( (unsigned __int64)(v12 - 1) > 0xFFFFFFFFFFFFFFFDuLL )
    {
      v23 = 1400LL;
      goto LABEL_60;
    }
    v11 = ForwardTouchMessage(gptiCurrent, a4, 1, 0LL, 0, 0LL);
  }
  else
  {
    if ( (((_DWORD)v4 - 281) & 0xFFFFFFFD) == 0
      || (unsigned int)(v4 - 577) <= 3
      || (unsigned int)(v4 - 581) <= 0x12 && (_DWORD)v4 != 589 )
    {
      v23 = 1002LL;
      goto LABEL_60;
    }
    v13 = 0;
    if ( (unsigned int)(v4 - 992) <= 8 )
      v13 = ValidateDDEConvPair(a3, v12, v9) != 0;
    if ( (_DWORD)v4 == 274 && a3 == 61488 )
    {
      if ( (unsigned int)IAMThreadAccessGranted(gptiCurrent) )
        v13 = 1;
    }
    if ( (unsigned __int64)(v12 - 1) <= 0xFFFFFFFFFFFFFFFDuLL && !v13 )
    {
      v14 = *(_QWORD *)(*(_QWORD *)(v12 + 16) + 424LL);
      CurrentProcessWin32Process = (struct tagPROCESSINFO *)PsGetCurrentProcessWin32Process(v8, v10, v9);
      v16 = CurrentProcessWin32Process;
      if ( (struct tagPROCESSINFO *)v14 != CurrentProcessWin32Process )
      {
        if ( (_DWORD)v4 == 717 )
          goto LABEL_21;
        if ( !(unsigned int)IsMessageAllowedAcrossILByReceiver(
                              CurrentProcessWin32Process,
                              (void *const **)v14,
                              (struct tagWND *)v12,
                              v4,
                              a3,
                              (__int64)a4,
                              0) )
        {
          if ( (unsigned int)IsMessageAlwaysAllowedAcrossIL(v4) )
          {
            MSGSQMAddMessage(v16, (struct tagPROCESSINFO *)v14, v4, a3, (unsigned __int64)a4, 0, 2);
          }
          else if ( (_DWORD)v4 != 274 || ((a3 - 61472) & 0xFFFFFFFFFFFFFEBFuLL) != 0 || a3 == 61792 )
          {
            if ( *(_QWORD *)v14 == gpepCSRSS )
              v26 = 0x2000LL;
            else
              v26 = *(_QWORD *)(v14 + 880);
            v19 = CheckAccess((char *)v16 + 880, &v26);
            v20 = v19;
            if ( v19 )
            {
              v21 = Enforced();
              v22 = 1;
              if ( v21 )
                v22 = 5;
              MSGSQMAddMessage(v16, (struct tagPROCESSINFO *)v14, v4, a3, (unsigned __int64)a4, 0, v22);
            }
            if ( !v20 )
            {
              if ( (_DWORD)v4 != 793
                || (v25 = *(_QWORD *)(v12 + 16), *(_QWORD *)(v25 + 1392) != v12)
                || *(_QWORD *)(v25 + 432) != *(_QWORD *)(gptiCurrent + 432LL) )
              {
                EtwTraceUIPIMsgError(v16, v14, (unsigned int)v4, a3, a4);
                UserSetLastError(5LL);
                MSGSQMAddMessage(v16, (struct tagPROCESSINFO *)v14, v4, a3, (unsigned __int64)a4, 0, 0);
                goto LABEL_21;
              }
            }
          }
          else
          {
            MSGSQMAddMessage(v16, (struct tagPROCESSINFO *)v14, 0x112u, a3, (unsigned __int64)a4, 0, 2);
          }
        }
      }
    }
    v17 = _PostTransformableMessageExtended((struct tagWND *)v12, v4, a3, (__int64)a4, 0LL, 1);
    v8 = (unsigned __int64)MmSystemRangeStart;
    if ( v17 >= MmSystemRangeStart )
    {
      if ( v17 )
        v11 = 1;
      LODWORD(v17) = v11;
    }
    v11 = (int)v17;
  }
LABEL_21:
  UserSessionSwitchLeaveCrit(v8);
  return v11;
}
