/*
 * XREFs of ?vDIBSolidBlt@@YAXPEAVSURFACE@@PEAU_RECTL@@PEAU_CLIPOBJ@@KH@Z @ 0x1C00898E8
 * Callers:
...
void __fastcall vDIBSolidBlt(struct SURFACE *a1, struct _RECTL *a2, struct _CLIPOBJ *a3, int a4, int a5)
{
  ...
  LONG bottom; // eax
  LONG v28; // r8d
  unsigned int v29; // [rsp+40h] [rbp-1E8h]
  ...
  if ( (unsigned int)(v10 - 1) > 5 && bUMPDSecurityGateEx() )
  {
    if ( gfUMPDDebug )
      DbgPrint(
