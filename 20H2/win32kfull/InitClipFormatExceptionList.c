/*
 * XREFs of InitClipFormatExceptionList @ 0x1C0124C60
 * Callers:
 *     <none>
 * Callees:
  ...
  *(_OWORD *)&ObjectAttributes.SecurityDescriptor = 0LL;
  ...
  if ( result >= 0 )
  {
    ...
    if ( ZwQueryKey(KeyHandle, KeyFullInformation, v0, Length, &Length) >= 0
      && (v6 = v0[8]) != 0
      && (HIDWORD(WPP_MAIN_CB.DeviceQueue.DeviceListHead.Flink) = v0[8], v6 <= 0x1FFF)
      ...
      && (v1 = (_DWORD *)Win32AllocPool(v5 + 16, 1650684757LL)) != 0LL )
    {
      ...
      if ( HIDWORD(WPP_MAIN_CB.DeviceQueue.DeviceListHead.Flink) )
      {
        while ( ZwEnumerateValueKey(KeyHandle, v8, KeyValuePartialInformation, v1, v0[10] + 16, &Length) >= 0 )
        {
          ...
          if ( ++v8 >= HIDWORD(WPP_MAIN_CB.DeviceQueue.DeviceListHead.Flink) )
            goto LABEL_15;
        }
        HIDWORD(WPP_MAIN_CB.DeviceQueue.DeviceListHead.Flink) = v8;
      }
    }
    else
    {
