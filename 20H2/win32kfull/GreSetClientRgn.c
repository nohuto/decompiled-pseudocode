/*
 * XREFs of GreSetClientRgn @ 0x1C0286F4C
 * Callers:
 *     InvalidateGDIWindows @ 0x1C01E81B0 (InvalidateGDIWindows.c)
 * Callees:
 *     ?vUnlock@SEMOBJ@@QEAAXXZ @ 0x1C002120C (-vUnlock@SEMOBJ@@QEAAXXZ.c)
 *     ??1RGNMEMOBJTMP@@QEAA@XZ @ 0x1C00690E4 (--1RGNMEMOBJTMP@@QEAA@XZ.c)
 *     ??1RGNOBJAPI@@QEAA@XZ @ 0x1C009EEA8 (--1RGNOBJAPI@@QEAA@XZ.c)
 *     ?bOffsetAdd@ERECTL@@QEAAHAEBU_POINTL@@H@Z @ 0x1C014BAB8 (-bOffsetAdd@ERECTL@@QEAAHAEBU_POINTL@@H@Z.c)
 *     _guard_dispatch_icall_nop @ 0x1C015F580 (_guard_dispatch_icall_nop.c)
 *     ?bValid@EWNDOBJ@@QEBAHXZ @ 0x1C0286008 (-bValid@EWNDOBJ@@QEBAHXZ.c)
 *     ?vSetClip@EWNDOBJ@@QEAAXPEAVREGION@@VERECTL@@@Z @ 0x1C02864F8 (-vSetClip@EWNDOBJ@@QEAAXPEAVREGION@@VERECTL@@@Z.c)
 *     ?vUpdateDrvDelta@TRACKOBJ@@QEAAXPEAVEWNDOBJ@@K@Z @ 0x1C0286754 (-vUpdateDrvDelta@TRACKOBJ@@QEAAXPEAVEWNDOBJ@@K@Z.c)
 */

void __fastcall GreSetClientRgn(struct EWNDOBJ *a1, HRGN a2, unsigned int *a3)
{
  __int64 v6; // rdx
  bool v7; // zf
  struct REGION **v8; // r14
  unsigned int v9; // edi
  unsigned int v10; // r12d
  unsigned int v11; // r15d
  unsigned int v12; // esi
  __int64 v13; // rcx
  __int64 v14; // rax
  int v15; // edx
  int v16; // ecx
  BOOL v17; // edi
  struct REGION *v18; // rdx
  signed __int32 v19; // ecx
  struct REGION *v20; // r8
  signed __int32 v21; // ecx
  __int128 v22; // xmm0
  struct REGION *v23; // rdx
  int v24; // eax
  __int128 v25; // [rsp+20h] [rbp-79h] BYREF
  _QWORD v26[3]; // [rsp+30h] [rbp-69h] BYREF
  _QWORD v27[2]; // [rsp+48h] [rbp-51h] BYREF
  _QWORD v28[2]; // [rsp+58h] [rbp-41h] BYREF
  _QWORD v29[2]; // [rsp+68h] [rbp-31h] BYREF
  _QWORD v30[2]; // [rsp+78h] [rbp-21h] BYREF
  __int64 v31; // [rsp+88h] [rbp-11h] BYREF
  __int64 v32; // [rsp+90h] [rbp-9h] BYREF
  __int128 v33; // [rsp+A0h] [rbp+7h] BYREF
  _OWORD v34[4]; // [rsp+B0h] [rbp+17h] BYREF
  struct _POINTL v35; // [rsp+100h] [rbp+67h] BYREF
  __int64 v36; // [rsp+108h] [rbp+6Fh] BYREF
  __int64 v37; // [rsp+118h] [rbp+7Fh] BYREF

  if ( a2 )
  {
    if ( EWNDOBJ::bValid(a1) )
    {
      GreSetRegionOwner(v6, 0LL);
      RGNOBJAPI::RGNOBJAPI((RGNOBJAPI *)v26, a2, 0);
      if ( v26[0] )
      {
        v7 = (*((_DWORD *)a1 + 46) & 0x100) == 0;
        v8 = (struct REGION **)((char *)a1 + 56);
        v9 = *a3;
        v10 = a3[1];
        v11 = a3[2];
        v12 = a3[3];
        v37 = *((_QWORD *)a1 + 7);
        *(_QWORD *)&v25 = __PAIR64__(v10, v9);
        *((_QWORD *)&v25 + 1) = __PAIR64__(v12, v11);
        if ( v7 )
        {
          v13 = *(_QWORD *)(*((_QWORD *)a1 + 21) + 32LL);
          v14 = *(_QWORD *)(v13 + 48);
          if ( v14 )
          {
            if ( v13 == *(_QWORD *)(v14 + 2552) )
            {
              v15 = *(_DWORD *)(v14 + 2584);
              v35.x = -v15;
              v16 = *(_DWORD *)(v14 + 2588);
              v35.y = -v16;
              if ( v15 || v16 )
              {
                RGNOBJ::bOffset((RGNOBJ *)v26, &v35);
                ERECTL::bOffsetAdd((ERECTL *)&v25, &v35, 0);
                v12 = HIDWORD(v25);
                v11 = DWORD2(v25);
                v10 = DWORD1(v25);
                v9 = v25;
              }
            }
          }
          RGNMEMOBJ::RGNMEMOBJ((RGNMEMOBJ *)v27);
          RGNMEMOBJ::vPushThreadGuardedObject((RGNMEMOBJ *)v27);
          RGNMEMOBJ::RGNMEMOBJ((RGNMEMOBJ *)v28);
          RGNMEMOBJ::vPushThreadGuardedObject((RGNMEMOBJ *)v28);
          if ( v27[0] )
          {
            if ( v28[0] )
            {
              RGNOBJ::vSet((RGNOBJ *)v28, (struct _RECTL *)(*((_QWORD *)a1 + 21) + 52LL));
              RGNOBJ::bCopy((RGNOBJ *)v27, (struct RGNOBJ *)v26);
              RGNOBJAPI::iCombine((RGNOBJAPI *)v26, (struct RGNOBJ *)v27, (struct RGNOBJ *)v28, 1);
              if ( RGNOBJ::iCombine((RGNOBJ *)v27, (struct RGNOBJ *)v26, (struct RGNOBJ *)v28, 1) )
                RGNOBJAPI::bSwap((RGNOBJAPI *)v26, (struct RGNOBJ *)v27);
            }
          }
          RGNMEMOBJTMP::~RGNMEMOBJTMP((RGNMEMOBJTMP *)v28);
          RGNMEMOBJTMP::~RGNMEMOBJTMP((RGNMEMOBJTMP *)v27);
        }
        if ( RGNOBJ::bEqual((RGNOBJ *)&v37, (struct RGNOBJ *)v26)
          && *((_DWORD *)a1 + 8) == v9
          && *((_DWORD *)a1 + 10) == v11
          && *((_DWORD *)a1 + 9) == v10
          && *((_DWORD *)a1 + 11) == v12 )
        {
          RGNOBJAPI::bDeleteRGNOBJAPI((RGNOBJAPI *)v26);
        }
        else
        {
          RGNMEMOBJ::RGNMEMOBJ((RGNMEMOBJ *)v30);
          RGNMEMOBJ::vPushThreadGuardedObject((RGNMEMOBJ *)v30);
          v17 = 0;
          if ( (*((_DWORD *)a1 + 46) & 0x200) != 0 && v30[0] )
            v17 = RGNOBJ::bCopy((RGNOBJ *)v30, (struct RGNOBJ *)v26) != 0;
          v32 = ghsemWndobj;
          GreAcquireSemaphore(ghsemWndobj);
          v31 = *((_QWORD *)a1 + 25);
          GreAcquireSemaphore(v31);
          if ( v17 )
          {
            v36 = *((_QWORD *)a1 + 24);
            if ( v36 )
            {
              RGNOBJ::vSwap((RGNOBJ *)v30, (struct RGNOBJ *)&v36);
              *((_QWORD *)a1 + 24) = v36;
            }
          }
          if ( (*((_DWORD *)a1 + 46) & 1) != 0 )
          {
            RGNMEMOBJ::RGNMEMOBJ((RGNMEMOBJ *)v29, 0, 1);
            RGNMEMOBJ::vPushThreadGuardedObject((RGNMEMOBJ *)v29);
            if ( v29[0] && RGNOBJ::iCombine((RGNOBJ *)v29, (struct RGNOBJ *)v26, (struct RGNOBJ *)&v37, 4) )
            {
              RGNOBJ::vSwap((struct EWNDOBJ *)((char *)a1 + 56), (struct RGNOBJ *)v29);
              v18 = *v8;
              v19 = _InterlockedExchangeAdd(*(volatile signed __int32 **)&REGION::ulUniqueREGION, 1u);
              v33 = v25;
              *((_DWORD *)v18 + 7) = v19 + 1;
              EWNDOBJ::vSetClip((__int64)a1, *v8, &v33);
              TRACKOBJ::vUpdateDrvDelta(*((TRACKOBJ **)a1 + 21), a1);
            }
            RGNMEMOBJTMP::~RGNMEMOBJTMP((RGNMEMOBJTMP *)v29);
          }
          RGNOBJAPI::bSwap((RGNOBJAPI *)v26, (struct EWNDOBJ *)((char *)a1 + 56));
          v20 = *v8;
          v21 = _InterlockedExchangeAdd(*(volatile signed __int32 **)&REGION::ulUniqueREGION, 1u);
          v22 = v25;
          *((_DWORD *)v20 + 7) = v21 + 1;
          v23 = *v8;
          v34[0] = v22;
          EWNDOBJ::vSetClip((__int64)a1, v23, v34);
          RGNOBJAPI::bDeleteRGNOBJAPI((RGNOBJAPI *)v26);
          v24 = *((_DWORD *)a1 + 46);
          if ( (v24 & 2) != 0 )
          {
            (*(void (__fastcall **)(struct EWNDOBJ *))(*((_QWORD *)a1 + 21) + 40LL))(a1);
            v24 = *((_DWORD *)a1 + 46);
          }
          *((_DWORD *)a1 + 46) = v24 | 0x80000000;
          *(_DWORD *)(*((_QWORD *)a1 + 21) + 48LL) |= 0x80000000;
          SEMOBJ::vUnlock((SEMOBJ *)&v31);
          SEMOBJ::vUnlock((SEMOBJ *)&v32);
          RGNMEMOBJTMP::~RGNMEMOBJTMP((RGNMEMOBJTMP *)v30);
        }
      }
      RGNOBJAPI::~RGNOBJAPI((RGNOBJAPI *)v26);
    }
    else
    {
      bDeleteRegion(v6);
    }
  }
}
