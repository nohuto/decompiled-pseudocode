/*
 * XREFs of zzzWakeInputIdle @ 0x1C007ED48
 * Callers:
 *     xxxDesktopThreadWaiter @ 0x1C007E9C8 (xxxDesktopThreadWaiter.c)
 * Callees:
 *     zzzCalcStartCursorHide @ 0x1C00CC4C0 (zzzCalcStartCursorHide.c)
 */

__int64 __fastcall zzzWakeInputIdle(__int64 a1, __int64 a2, __int64 a3)
{
  __int64 CurrentProcessWin32Process; // rax
  __int64 v5; // rdi
  __int64 v6; // rax
  struct _KEVENT *v7; // rcx
  __int64 result; // rax
  __int64 v9; // rdx
  struct _KEVENT *v10; // rcx

  CurrentProcessWin32Process = PsGetCurrentProcessWin32Process(a1, a2, a3);
  *(_DWORD *)(a1 + 488) &= ~0x2000u;
  v5 = CurrentProcessWin32Process;
  if ( (*(_DWORD *)(a1 + 488) & 0x1000) != 0 )
  {
    if ( *(_QWORD *)(*(_QWORD *)(a1 + 616) + 24LL) )
    {
      EtwTraceWakeInputIdle(0LL, a1);
      v9 = *(_QWORD *)(*(_QWORD *)(a1 + 616) + 24LL);
      v10 = *(struct _KEVENT **)(v9 + 32);
      if ( v10 )
      {
        if ( v10 != (struct _KEVENT *)-1LL )
        {
          KeSetEvent(v10, 1, 0);
          ObfDereferenceObject(*(PVOID *)(*(_QWORD *)(*(_QWORD *)(a1 + 616) + 24LL) + 32LL));
          *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a1 + 616) + 24LL) + 32LL) = -1LL;
        }
      }
      else
      {
        *(_QWORD *)(v9 + 32) = -1LL;
      }
    }
  }
  else
  {
    v6 = *(_QWORD *)(a1 + 424);
    if ( !*(_QWORD *)(v6 + 328) )
    {
      *(_QWORD *)(v6 + 328) = a1;
      v6 = *(_QWORD *)(a1 + 424);
    }
    if ( *(_QWORD *)(v6 + 328) == a1 )
    {
      EtwTraceWakeInputIdle(0LL, a1);
      v7 = *(struct _KEVENT **)(v5 + 16);
      if ( v7 )
      {
        if ( v7 == (struct _KEVENT *)-1LL )
          goto LABEL_7;
        KeSetEvent(v7, 1, 0);
        ObfDereferenceObject(*(PVOID *)(v5 + 16));
      }
      *(_QWORD *)(v5 + 16) = -1LL;
    }
  }
LABEL_7:
  result = *(unsigned int *)(v5 + 12);
  if ( (result & 4) != 0 )
  {
    *(_DWORD *)(v5 + 12) = result & 0xFFFFFFFB;
    return zzzCalcStartCursorHide(0LL, 0LL);
  }
  return result;
}
