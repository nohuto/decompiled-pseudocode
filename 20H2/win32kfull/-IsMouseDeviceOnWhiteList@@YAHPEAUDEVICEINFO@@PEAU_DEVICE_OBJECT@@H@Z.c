/*
 * XREFs of ?IsMouseDeviceOnWhiteList@@YAHPEAUDEVICEINFO@@PEAU_DEVICE_OBJECT@@H@Z @ 0x1C0220800
 * Callers:
 *     IsLegacyTouchPadDevice @ 0x1C0117D00 (IsLegacyTouchPadDevice.c)
 * Callees:
 *     ?IsDeviceOnRegistryList@@YAHU_UNICODE_STRING@@0H@Z @ 0x1C011832C (-IsDeviceOnRegistryList@@YAHU_UNICODE_STRING@@0H@Z.c)
 *     ?GetMouseDeviceHardwareId@@YAJPEAUDEVICEINFO@@PEAU_DEVICE_OBJECT@@PEAU_UNICODE_STRING@@@Z @ 0x1C0118458 (-GetMouseDeviceHardwareId@@YAJPEAUDEVICEINFO@@PEAU_DEVICE_OBJECT@@PEAU_UNICODE_STRING@@@Z.c)
 *     __security_check_cookie @ 0x1C015A0C0 (__security_check_cookie.c)
 */

__int64 __fastcall IsMouseDeviceOnWhiteList(struct DEVICEINFO *a1, struct _DEVICE_OBJECT *a2, int a3, __int64 a4)
{
  unsigned int v5; // ebx
  struct _UNICODE_STRING v7; // [rsp+20h] [rbp-E0h] BYREF
  struct _UNICODE_STRING DestinationString; // [rsp+30h] [rbp-D0h] BYREF
  struct _UNICODE_STRING v9; // [rsp+40h] [rbp-C0h] BYREF
  struct _UNICODE_STRING v10; // [rsp+50h] [rbp-B0h] BYREF
  char v11; // [rsp+60h] [rbp-A0h] BYREF

  *(_QWORD *)&v7.Length = 13107200LL;
  v7.Buffer = (PWSTR)&v11;
  DestinationString = 0LL;
  v5 = 0;
  if ( (int)GetMouseDeviceHardwareId(a1, a2, &v7, a4) >= 0 )
  {
    RtlInitUnicodeString(
      &DestinationString,
      L"\\Registry\\Machine\\Software\\Microsoft\\Windows\\CurrentVersion\\PrecisionTouchPad\\LegacyDevices");
    v9 = v7;
    v10 = DestinationString;
    return IsDeviceOnRegistryList(&v10, &v9, a3);
  }
  return v5;
}
