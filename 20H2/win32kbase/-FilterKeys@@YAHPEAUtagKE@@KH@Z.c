/*
 * XREFs of ?FilterKeys@@YAHPEAUtagKE@@KH@Z @ 0x1C0181430
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall FilterKeys(struct tagKE *a1, __int64 a2, int a3)
{
  ...
  if ( v4 == -95 )
  {
    ...
    else if ( gPhysModifierState == 2 && (xmmword_1C0249BCC & 4) != 0 && !gtmridFKActivation && gFilterKeysState != 8 )
    {
      gFilterKeysState = 1;
  ...
  if ( !v5 )
  {
    if ( v4 != gLastVkDown )
    {
      ...
      gFKNextProcIndex = a3;
      gFKKeyEvent = v10;
      xmmword_1C0251EB0 = *((_OWORD *)a1 + 1);
      if ( !DWORD1(xmmword_1C0249BCC) )
      {
        v12 = HIDWORD(xmmword_1C0249BCC);
        if ( HIDWORD(xmmword_1C0249BCC) )
        {
          if ( DWORD2(xmmword_1C0249BCC) )
            v12 = DWORD2(xmmword_1C0249BCC);
          gtmridFKResponse = ApiSetEditionSetAccessibilityTimer(0LL, v12, xxxFKRepeatRateTimer);
        }
        return 1LL;
      }
      v11 = ApiSetEditionSetAccessibilityTimer(0LL, DWORD1(xmmword_1C0249BCC), xxxFKAcceptanceDelayTimer);
      gdwPUDFlags &= ~0x800u;
