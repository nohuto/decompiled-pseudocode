/*
 * XREFs of ?EmitUpdate@CCompiledEffectTemplateMarshaler@DirectComposition@@AEAA_NPEAPEAVCBatch@2@@Z @ 0x1C007F6CC
 * Callers:
 *     ?EmitUpdateCommands@CCompiledEffectTemplateMarshaler@DirectComposition@@MEAA_NPEAPEAVCBatch@2@@Z @ 0x1C007F7E0 (-EmitUpdateCommands@CCompiledEffectTemplateMarshaler@DirectComposition@@MEAA_NPEAPEAVCBatch@2@@Z.c)
 * Callees:
 *     OpenDwmHandle @ 0x1C0066E3C (OpenDwmHandle.c)
 *     ??_GCEvent@DirectComposition@@QEAAPEAXI@Z @ 0x1C0067304 (--_GCEvent@DirectComposition@@QEAAPEAXI@Z.c)
 *     ?EnsureBatchBuffer@CBatch@DirectComposition@@SA_NPEAPEAV12@_KPEAPEAX@Z @ 0x1C007C3E8 (-EnsureBatchBuffer@CBatch@DirectComposition@@SA_NPEAPEAV12@_KPEAPEAX@Z.c)
 */

char __fastcall DirectComposition::CCompiledEffectTemplateMarshaler::EmitUpdate(
        DirectComposition::CCompiledEffectTemplateMarshaler *this,
        struct DirectComposition::CBatch **a2)
{
  __int64 v4; // r9
  _DWORD *v5; // rcx
  __int64 v6; // rax
  __int64 v7; // rax
  _QWORD *v8; // rsi
  DirectComposition::CEvent *v9; // rcx
  _QWORD *v10; // rax
  __int64 v11; // rdx
  int v13; // [rsp+20h] [rbp-18h]
  _DWORD *v14; // [rsp+40h] [rbp+8h] BYREF

  if ( (*((_DWORD *)this + 4) & 0x40) != 0 )
    return 1;
  v14 = 0LL;
  if ( DirectComposition::CBatch::EnsureBatchBuffer(a2, 0x24uLL, (void **)&v14) )
  {
    v5 = v14;
    *v14 = 36;
    *(_OWORD *)(v5 + 1) = 0LL;
    *(_OWORD *)(v5 + 5) = 0LL;
    v5[1] = 59;
    v5[2] = *((_DWORD *)this + 6);
    v6 = *((_QWORD *)this + 8);
    if ( v6 )
      LODWORD(v6) = *(_DWORD *)(v6 + 24);
    v5[3] = v6;
    v5[4] = *((_DWORD *)this + 18);
    v5[5] = *((_DWORD *)this + 19);
    v7 = *((_QWORD *)this + 11);
    if ( v7 )
    {
      v8 = v5 + 7;
      if ( (int)OpenDwmHandle(*(PVOID *)(v7 + 8), (POBJECT_TYPE)ExEventObjectType, 2u, v4, v13, (PHANDLE)(v5 + 7)) < 0 )
      {
        *v8 = 0LL;
        KeSetEvent(*(PRKEVENT *)(*((_QWORD *)this + 11) + 8LL), 1, 0);
      }
      v9 = (DirectComposition::CEvent *)*((_QWORD *)this + 11);
      if ( v9 )
        DirectComposition::CEvent::`scalar deleting destructor'(v9);
      *((_QWORD *)this + 11) = 0LL;
    }
    v10 = (_QWORD *)*((_QWORD *)this + 12);
    if ( v10 )
    {
      v11 = *((_QWORD *)*a2 + 1);
      *v10 = *(_QWORD *)(v11 + 208);
      *(_QWORD *)(v11 + 208) = v10;
      *((_QWORD *)this + 12) = 0LL;
    }
    *((_DWORD *)this + 4) |= 0x40u;
    return 1;
  }
  return 0;
}
