/*
 * XREFs of ?GetInstance@CStreamConnectionManagerImpl@@SA?AV?$shared_ptr@VCStreamConnectionManager@@@std@@XZ @ 0x180061424
 * Callers:
...
std::_Ref_count_base **__fastcall CStreamConnectionManagerImpl::GetInstance(std::_Ref_count_base **a1)
{
  ...
  *(_OWORD *)v12 = 0LL;
  ...
  else
  {
    ...
    if ( v6 && (int)CMonitorManager::Initialize(v6, 0x12u, 9uLL, &dword_18015C920) >= 0 )
    {
