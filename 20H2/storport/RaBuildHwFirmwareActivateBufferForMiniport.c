/*
 * XREFs of RaBuildHwFirmwareActivateBufferForMiniport @ 0x1C004384C
 * Callers:
 *     RaidAdapterHwFirmwareActivateIoctl @ 0x1C002CD08 (RaidAdapterHwFirmwareActivateIoctl.c)
 * Callees:
 *     RaidAllocatePool @ 0x1C000BD70 (RaidAllocatePool.c)
 *     memset @ 0x1C001F8C0 (memset.c)
 */

__int64 __fastcall RaBuildHwFirmwareActivateBufferForMiniport(__int64 a1, __int64 a2, _QWORD *a3, _DWORD *a4)
{
  __int64 v4; // rdi
  unsigned int v5; // esi
  __int64 v6; // r14
  __int64 v9; // rbp
  char *Pool; // rax
  char *v11; // rbx
  int v12; // eax
  __int64 result; // rax

  v4 = *(_QWORD *)(a1 + 64);
  v5 = 0;
  v6 = *(_QWORD *)(a2 + 24);
  if ( *(_DWORD *)v4 == 1 )
    v9 = *(_QWORD *)(v4 + 24);
  else
    v9 = *(_QWORD *)(a1 + 64);
  if ( *(_DWORD *)v4 != 1 )
    v4 = 0LL;
  Pool = (char *)RaidAllocatePool(NonPagedPoolNx, 0x44uLL, 0x72536152u, a1);
  v11 = Pool;
  if ( Pool )
  {
    memset(Pool, 0, 0x44uLL);
    *(_DWORD *)v11 = 28;
    *((_DWORD *)v11 + 4) = 1771392;
    *(_QWORD *)(v11 + 4) = 0x455241574D524946LL;
    if ( v4 )
      v12 = *(_DWORD *)(v4 + 1264);
    else
      v12 = *(_DWORD *)(v9 + 4268);
    *((_DWORD *)v11 + 3) = v12;
    *((_DWORD *)v11 + 6) = 40;
    *((_DWORD *)v11 + 7) = 1;
    *((_DWORD *)v11 + 10) = v4 == 0;
    *((_DWORD *)v11 + 12) = 12;
    *((_DWORD *)v11 + 8) = 24;
    *((_DWORD *)v11 + 9) = 3;
    *((_DWORD *)v11 + 11) = 56;
    *((_DWORD *)v11 + 14) = 1;
    *((_DWORD *)v11 + 15) = 12;
    v11[64] = *(_BYTE *)(v6 + 12);
    if ( *(int *)(v6 + 8) < 0 )
      *((_DWORD *)v11 + 10) |= 0x80000000;
  }
  else
  {
    v5 = -1073741670;
  }
  result = v5;
  *a3 = v11;
  *a4 = 68;
  return result;
}
