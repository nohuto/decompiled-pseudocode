/*
 * XREFs of IvtCreateDevice @ 0x1404E5EF0
 * Callers:
 *     <none>
 * Callees:
 *     ExtEnvAllocateMemory @ 0x1404D3720 (ExtEnvAllocateMemory.c)
 *     ExtEnvFreeMemory @ 0x1404D39EC (ExtEnvFreeMemory.c)
 *     HalpIvtCheckIdPathMatch @ 0x1404E5790 (HalpIvtCheckIdPathMatch.c)
 *     IvtpGetNextInternalDeviceScope @ 0x1404E6084 (IvtpGetNextInternalDeviceScope.c)
 */

__int64 __fastcall IvtCreateDevice(__int64 a1, __int64 a2, char a3, unsigned __int64 *a4)
{
  __int64 v6; // rdi
  char *v7; // rdx
  char *NextInternalDeviceScope; // rax
  char *v9; // rbx
  int v10; // ebx
  int Memory; // eax
  __int64 v13; // rcx
  unsigned __int64 v14; // rdi
  _DWORD **v15; // r14
  __int64 *v16; // r8
  unsigned __int64 v17; // rdx
  _OWORD *v18; // [rsp+40h] [rbp+8h] BYREF

  v18 = 0LL;
  v6 = a1;
  if ( *(_DWORD *)a2 != 1 || *(_DWORD *)(a1 + 212) != *(unsigned __int16 *)(a2 + 8) )
    return (unsigned int)-1073741275;
  if ( (a3 & 1) == 0 || !*(_BYTE *)(a1 + 228) )
  {
    v7 = 0LL;
    while ( 1 )
    {
      NextInternalDeviceScope = (char *)IvtpGetNextInternalDeviceScope(a1, v7);
      v9 = NextInternalDeviceScope;
      if ( !NextInternalDeviceScope )
        return (unsigned int)-1073741275;
      if ( HalpIvtCheckIdPathMatch(a2, NextInternalDeviceScope) )
        break;
      v7 = v9;
      a1 = v6;
    }
  }
  Memory = ExtEnvAllocateMemory(a1, 0x28u, (__int64 *)&v18);
  v14 = (unsigned __int64)v18;
  v10 = Memory;
  if ( Memory < 0
    || (v15 = (_DWORD **)(v18 + 2),
        v16 = (__int64 *)(v18 + 2),
        *v18 = 0LL,
        *(_OWORD *)(v14 + 16) = 0LL,
        *(_QWORD *)(v14 + 32) = 0LL,
        v10 = ExtEnvAllocateMemory(v13, 4u, v16),
        v10 < 0) )
  {
    if ( v14 )
    {
      v17 = *(_QWORD *)(v14 + 32);
      if ( v17 )
        ExtEnvFreeMemory(v13, v17);
      ExtEnvFreeMemory(v13, v14);
    }
  }
  else
  {
    v10 = 0;
    **v15 = 0;
    **v15 = *(unsigned __int16 *)(a2 + 12);
    *a4 = v14;
  }
  return (unsigned int)v10;
}
