/*
 * XREFs of Phase1InitializationDiscard @ 0x140A3DB24
 * Callers:
 *     Phase1Initialization @ 0x140793E90 (Phase1Initialization.c)
 * Callees:
 *     RtlInitUnicodeString @ 0x140210D50 (RtlInitUnicodeString.c)
 *     PsGetCurrentServerSiloGlobals @ 0x14023CD40 (PsGetCurrentServerSiloGlobals.c)
 *     RtlInitAnsiString @ 0x14023DB90 (RtlInitAnsiString.c)
 *     RtlStringCbCopyA @ 0x140253AB0 (RtlStringCbCopyA.c)
 *     KeQueryPerformanceCounter @ 0x140266AF0 (KeQueryPerformanceCounter.c)
 *     KeSetPriorityThread @ 0x1402DAB40 (KeSetPriorityThread.c)
 *     ExLocalTimeToSystemTime @ 0x140312120 (ExLocalTimeToSystemTime.c)
 *     RtlTimeFieldsToTime @ 0x1403124F0 (RtlTimeFieldsToTime.c)
 *     ExSystemTimeToLocalTime @ 0x140330FF0 (ExSystemTimeToLocalTime.c)
 *     RtlTimeToTimeFields @ 0x14036E6A0 (RtlTimeToTimeFields.c)
 *     PoNotifySystemTimeSet @ 0x1403A6F44 (PoNotifySystemTimeSet.c)
 *     KeSetSystemTime @ 0x1403A7188 (KeSetSystemTime.c)
 *     InbvDisplayString @ 0x1403A7A30 (InbvDisplayString.c)
 *     RtlStringCbPrintfA @ 0x1403A7A54 (RtlStringCbPrintfA.c)
 *     DisplayBootBitmap @ 0x1403B3710 (DisplayBootBitmap.c)
 *     MakeGdtReadOnly @ 0x1403B503C (MakeGdtReadOnly.c)
 *     FsRtlSendModernAppTermination @ 0x1403BFC60 (FsRtlSendModernAppTermination.c)
 *     KeSetAffinityProcess @ 0x1403CA8FC (KeSetAffinityProcess.c)
 *     HvlDebuggerSupportInitialize @ 0x1403CF220 (HvlDebuggerSupportInitialize.c)
 *     HvlWnfUpdateStartupState @ 0x1403CF980 (HvlWnfUpdateStartupState.c)
 *     InbvEnableDisplayString @ 0x1403CFA10 (InbvEnableDisplayString.c)
 *     InbvInstallDisplayStringFilter @ 0x1403CFA90 (InbvInstallDisplayStringFilter.c)
 *     __security_check_cookie @ 0x1403CFAF0 (__security_check_cookie.c)
 *     _strupr @ 0x1403D06B0 (_strupr.c)
 *     strtoul @ 0x1403D0B90 (strtoul.c)
 *     strncmp @ 0x1403D0BD0 (strncmp.c)
 *     _strtoui64 @ 0x1403D0EE0 (_strtoui64.c)
 *     strstr @ 0x1403D0F10 (strstr.c)
 *     atol @ 0x1403D0FB0 (atol.c)
 *     ZwClose @ 0x1403F8350 (ZwClose.c)
 *     ZwCreateSection @ 0x1403F8AB0 (ZwCreateSection.c)
 *     ZwQueryLicenseValue @ 0x1403FABF0 (ZwQueryLicenseValue.c)
 *     ZwSetSystemTime @ 0x1403FB710 (ZwSetSystemTime.c)
 *     ZwUpdateWnfStateData @ 0x1403FBB50 (ZwUpdateWnfStateData.c)
 *     KeBugCheck @ 0x1403FBC80 (KeBugCheck.c)
 *     KeBugCheckEx @ 0x1403FBCA0 (KeBugCheckEx.c)
 *     memmove @ 0x140411040 (memmove.c)
 *     memset @ 0x140411300 (memset.c)
 *     RtlStringCbPrintfExA @ 0x1404B47C4 (RtlStringCbPrintfExA.c)
 *     RtlInitializeCompression @ 0x1405839D4 (RtlInitializeCompression.c)
 *     ExpWriteTimeZoneBias @ 0x1405CF98C (ExpWriteTimeZoneBias.c)
 *     ObReferenceObjectByHandle @ 0x1406118C0 (ObReferenceObjectByHandle.c)
 *     RtlFindMessage @ 0x1406AA660 (RtlFindMessage.c)
 *     MmMapViewInSystemSpace @ 0x1406DF780 (MmMapViewInSystemSpace.c)
 *     ExAcquireTimeRefreshLock @ 0x1406F5A38 (ExAcquireTimeRefreshLock.c)
 *     ExReleaseTimeRefreshLock @ 0x1406F73F0 (ExReleaseTimeRefreshLock.c)
 *     ExInitializeLeapSecondData @ 0x14079EDEC (ExInitializeLeapSecondData.c)
 *     ExpRefreshTimeZoneInformation @ 0x14079F494 (ExpRefreshTimeZoneInformation.c)
 *     RtlInitNlsTables @ 0x1407C8554 (RtlInitNlsTables.c)
 *     RtlResetRtlTranslations @ 0x1407CBAE4 (RtlResetRtlTranslations.c)
 *     ExInitializeUtcTimeZoneBias @ 0x1407D0710 (ExInitializeUtcTimeZoneBias.c)
 *     ExInitLicenseCallback @ 0x1407D0A78 (ExInitLicenseCallback.c)
 *     IopInitializeBootLogging @ 0x140894150 (IopInitializeBootLogging.c)
 *     HalInitSystem @ 0x14099E730 (HalInitSystem.c)
 *     ExAllocatePoolWithTag @ 0x1409B7010 (ExAllocatePoolWithTag.c)
 *     ExFreePoolWithTag @ 0x1409B70B0 (ExFreePoolWithTag.c)
 *     KdInitSystem @ 0x1409B8160 (KdInitSystem.c)
 *     BgGetDisplayContext @ 0x1409F4028 (BgGetDisplayContext.c)
 *     HalReportResourceUsage @ 0x140A3F2D0 (HalReportResourceUsage.c)
 *     CcInitializeCacheManager @ 0x140A3F638 (CcInitializeCacheManager.c)
 *     IoCreateObjectTypes @ 0x140A3FB58 (IoCreateObjectTypes.c)
 *     ObInitSystem @ 0x140A409B8 (ObInitSystem.c)
 *     PoInitSystem @ 0x140A41DE0 (PoInitSystem.c)
 *     EmInitSystem @ 0x140A4633C (EmInitSystem.c)
 *     KeInitializeClock @ 0x140A48B4C (KeInitializeClock.c)
 *     ExInitSystemPhase2 @ 0x140A48DBC (ExInitSystemPhase2.c)
 *     PsInitSystem @ 0x140A4ABB8 (PsInitSystem.c)
 *     KeInitSystem @ 0x140A4ABFC (KeInitSystem.c)
 *     KePerformGroupConfiguration @ 0x140A4BE38 (KePerformGroupConfiguration.c)
 *     KeStartAllProcessors @ 0x140A4BFC8 (KeStartAllProcessors.c)
 *     MmInitSystem @ 0x140A52764 (MmInitSystem.c)
 *     CmInitSystem1 @ 0x140A58954 (CmInitSystem1.c)
 *     ExInitializeTimeRefresh @ 0x140A59BC0 (ExInitializeTimeRefresh.c)
 *     SeInitSystem @ 0x140A6553C (SeInitSystem.c)
 *     VmInitSystem @ 0x140A6576C (VmInitSystem.c)
 *     ExInitSystem @ 0x140A6B50C (ExInitSystem.c)
 *     PfInitializeSuperfetch @ 0x140A6D32C (PfInitializeSuperfetch.c)
 *     ExpComputeCyclesPerYield @ 0x140A6DD60 (ExpComputeCyclesPerYield.c)
 *     SmInitSystem @ 0x140A6E354 (SmInitSystem.c)
 *     InbvDriverInitialize @ 0x140A6E608 (InbvDriverInitialize.c)
 *     FsRtlInitSystem @ 0x140A6EA60 (FsRtlInitSystem.c)
 *     DbgkInitialize @ 0x140A7052C (DbgkInitialize.c)
 *     CreateSystemRootLink @ 0x140A7272C (CreateSystemRootLink.c)
 *     VerifierInitSystem @ 0x140A72B04 (VerifierInitSystem.c)
 *     LpcInitSystem @ 0x140A72E5C (LpcInitSystem.c)
 *     SeCodeIntegrityInitializePolicy @ 0x140A737D8 (SeCodeIntegrityInitializePolicy.c)
 *     GetBootSystemTime @ 0x140A748AC (GetBootSystemTime.c)
 *     InitSkuSessionParameters @ 0x140A74D84 (InitSkuSessionParameters.c)
 *     PpInitSystem @ 0x140A75884 (PpInitSystem.c)
 *     BcdInitializeBcdSyncMutant @ 0x140A76888 (BcdInitializeBcdSyncMutant.c)
 *     MfgInitSystem @ 0x140A769A4 (MfgInitSystem.c)
 *     ExInitializeExternalBootSupport @ 0x140A76C6C (ExInitializeExternalBootSupport.c)
 *     ExInitializeNls @ 0x140A76F44 (ExInitializeNls.c)
 *     HalAllProcessorsStarted @ 0x140A77230 (HalAllProcessorsStarted.c)
 *     VslGetKsrContext @ 0x140A77348 (VslGetKsrContext.c)
 *     CreateMiniNtBootKey @ 0x140A8F250 (CreateMiniNtBootKey.c)
 *     RtlInitializeRangeListPackage @ 0x140A9687C (RtlInitializeRangeListPackage.c)
 *     BgkDisplayProgressIndicator @ 0x140A98F4C (BgkDisplayProgressIndicator.c)
 *     BgkDisplayBackgroundUpdate @ 0x140A99B44 (BgkDisplayBackgroundUpdate.c)
 */

char __fastcall Phase1InitializationDiscard(ULONG_PTR BugCheckParameter3)
{
  __int64 v2; // r12
  char *PoolWithTag; // r13
  struct _KTHREAD *CurrentThread; // rcx
  char *v5; // rcx
  char *v6; // rdi
  char *v7; // rax
  char *v8; // rax
  char *v9; // rax
  char *v10; // rbx
  __int16 v11; // ax
  int v12; // edx
  char v13; // al
  char *v14; // rax
  char *v15; // rax
  const char *v16; // rbx
  __int64 v17; // rsi
  char v18; // al
  int Message; // eax
  NTSTRSAFE_PSTR v20; // rbx
  int v21; // r15d
  NTSTATUS v22; // eax
  size_t v23; // rsi
  char *v24; // rbx
  NTSTATUS v25; // eax
  NTSTATUS v26; // eax
  NTSTATUS v27; // eax
  int v28; // esi
  _OWORD *v29; // rax
  char *v30; // rcx
  __int64 v31; // rdx
  __int128 v32; // xmm1
  int v33; // eax
  char *v34; // rax
  char *v35; // rax
  __int16 v36; // bx
  char *v37; // rax
  char *v38; // rax
  char *v39; // rax
  char *v40; // rax
  char *v41; // rax
  char *v42; // rax
  char *v43; // rax
  char *v44; // rax
  char *v45; // rax
  char *v46; // rax
  __int64 v47; // rcx
  const char *v48; // rbx
  unsigned int v49; // r9d
  int v50; // eax
  __int64 v51; // r9
  __int64 v52; // rdx
  _QWORD *v53; // rcx
  NTSTATUS v54; // eax
  __int64 v55; // r8
  __int64 v56; // r9
  int inited; // eax
  int v58; // eax
  int SystemRootLink; // eax
  NTSTATUS v60; // eax
  NTSTATUS v61; // eax
  ULONG_PTR v62; // rbx
  NTSTATUS v63; // eax
  PVOID v64; // rbx
  char *v65; // r8
  USHORT *v66; // rdx
  USHORT *v67; // rcx
  int v68; // eax
  __int128 *DisplayContext; // rax
  ULONG_PTR v70; // rbx
  int v71; // eax
  int v72; // eax
  int v73; // eax
  __int64 v74; // rdx
  __int64 v75; // rcx
  __int64 v76; // r8
  int v77; // ebx
  __int64 v78; // rdx
  __int64 v79; // rcx
  __int64 v80; // r8
  __int64 v81; // rdx
  __int64 v82; // rcx
  __int64 v83; // r8
  __int64 v84; // r9
  __int64 v85; // rdx
  __int64 v86; // rcx
  __int64 v87; // r8
  __int64 v88; // rdx
  __int64 v89; // rcx
  __int64 v90; // r8
  __int64 v91; // rdx
  __int64 v92; // rcx
  __int64 v93; // r8
  char *v94; // rbx
  char *v95; // r12
  __int64 v96; // rdi
  const char *v97; // rbx
  int v98; // eax
  unsigned int v99; // r9d
  __int64 v100; // rdx
  __int64 v101; // rcx
  __int64 v102; // r8
  __int64 v103; // rdx
  __int64 v104; // rcx
  __int64 v105; // r8
  int v106; // eax
  __int64 v107; // rdx
  __int64 v108; // rcx
  __int64 v109; // r8
  int v110; // eax
  __int64 v111; // rdx
  __int64 v112; // rcx
  __int64 v113; // r8
  int v114; // eax
  __int64 v115; // r8
  ULONG dwFlags[2]; // [rsp+20h] [rbp-E0h]
  char v118; // [rsp+40h] [rbp-C0h]
  LARGE_INTEGER Time; // [rsp+48h] [rbp-B8h] BYREF
  int v120; // [rsp+50h] [rbp-B0h]
  ULONG_PTR BugCheckParameter4; // [rsp+58h] [rbp-A8h] BYREF
  int v122; // [rsp+60h] [rbp-A0h]
  int v123; // [rsp+64h] [rbp-9Ch] BYREF
  LARGE_INTEGER LocalTime; // [rsp+68h] [rbp-98h] BYREF
  int v125; // [rsp+70h] [rbp-90h] BYREF
  char *EndPtr; // [rsp+78h] [rbp-88h] BYREF
  size_t pcbRemaining; // [rsp+80h] [rbp-80h] BYREF
  NTSTRSAFE_PSTR ppszDestEnd; // [rsp+88h] [rbp-78h] BYREF
  HANDLE SectionHandle; // [rsp+90h] [rbp-70h] BYREF
  __int64 v130; // [rsp+98h] [rbp-68h] BYREF
  unsigned __int16 *v131; // [rsp+A0h] [rbp-60h] BYREF
  LARGE_INTEGER MaximumSize; // [rsp+A8h] [rbp-58h] BYREF
  PVOID Object; // [rsp+B0h] [rbp-50h] BYREF
  PVOID MappedBase; // [rsp+B8h] [rbp-48h] BYREF
  unsigned __int16 *v135; // [rsp+C0h] [rbp-40h] BYREF
  STRING v136; // [rsp+C8h] [rbp-38h] BYREF
  __int64 v137; // [rsp+D8h] [rbp-28h]
  ULONG_PTR ViewSize; // [rsp+E0h] [rbp-20h] BYREF
  TIME_FIELDS TimeFields; // [rsp+E8h] [rbp-18h] BYREF
  UNICODE_STRING DestinationString; // [rsp+F8h] [rbp-8h] BYREF
  _QWORD v141[10]; // [rsp+110h] [rbp+10h] BYREF
  char pszDest[24]; // [rsp+160h] [rbp+60h] BYREF

  Time.QuadPart = 0LL;
  LocalTime.QuadPart = 0LL;
  v130 = 0LL;
  v136 = 0LL;
  SectionHandle = 0LL;
  MaximumSize.QuadPart = 0LL;
  BugCheckParameter4 = 0LL;
  v131 = 0LL;
  TimeFields = 0LL;
  LOBYTE(v120) = 0;
  v122 = 0;
  v123 = 0;
  DestinationString = 0LL;
  v2 = *((_QWORD *)PsGetCurrentServerSiloGlobals() + 133);
  v137 = v2;
  PoolWithTag = (char *)ExAllocatePoolWithTag(NonPagedPoolNx, 0x200uLL, 0x74696E49u);
  if ( !PoolWithTag )
    KeBugCheck(0x31u);
  CurrentThread = KeGetCurrentThread();
  LODWORD(InitializationPhase) = 1;
  v118 = 0;
  KeSetPriorityThread(CurrentThread, 31);
  v5 = *(char **)(BugCheckParameter3 + 216);
  if ( v5 )
    v6 = strupr(v5);
  else
    v6 = 0LL;
  if ( v6 )
  {
    v7 = strstr(v6, " HYPERVISORROOTPROC=");
    if ( v7 )
    {
      v8 = strstr(v7, "=");
      if ( v8 )
        KeRootProcSpecified = atol(v8 + 1);
    }
    v9 = strstr(v6, " HYPERVISORROOTPROCNUMANODES=");
    if ( v9 )
    {
      v10 = strstr(v9, "=");
      if ( v10 )
      {
        do
        {
          if ( (unsigned int)KeRootProcNumaNodesSpecified >= 0x40 )
            break;
          v11 = atol(++v10);
          v12 = KeRootProcNumaNodesSpecified;
          KeRootProcNumaNodes[KeRootProcNumaNodesSpecified] = v11;
          KeRootProcNumaNodesSpecified = v12 + 1;
          while ( 1 )
          {
            v13 = *v10;
            if ( *v10 == 44 )
              break;
            if ( v13 == 32 )
              goto LABEL_19;
            if ( !v13 )
              break;
            ++v10;
          }
        }
        while ( v13 != 32 && v13 );
      }
    }
LABEL_19:
    v14 = strstr(v6, " HYPERVISORROOTPROCNUMANODELPS=");
    if ( v14 )
    {
      v15 = strstr(v14, "=");
      KeRootProcNumaNodeLpsSpecified = 1;
      v16 = v15;
      KeRootProcNumaNodesSpecified = 0;
      KeRootProcSpecified = 0;
      if ( v15 )
      {
        do
        {
          ++v16;
          EndPtr = 0LL;
          v17 = strtoul(v16, &EndPtr, 10);
          if ( v16 != EndPtr && *EndPtr == 61 && (unsigned int)v17 < 0x40 )
          {
            v16 = EndPtr + 1;
            KeRootProcNumaNodeLps[v17] = strtoui64(EndPtr + 1, &EndPtr, 16);
          }
          while ( 1 )
          {
            v18 = *v16;
            if ( *v16 == 44 )
              break;
            if ( v18 == 32 )
              goto LABEL_31;
            if ( !v18 )
              break;
            ++v16;
          }
        }
        while ( v18 != 32 && v18 );
LABEL_31:
        v2 = v137;
      }
    }
  }
  KePerformGroupConfiguration(BugCheckParameter3);
  if ( !(unsigned __int8)HalInitSystem((unsigned int)InitializationPhase, BugCheckParameter3) )
    goto LABEL_178;
  KeInitializeClock((unsigned int)InitializationPhase);
  if ( !v6 || !strstr(v6, "NOGUIBOOT") )
  {
    InbvEnableDisplayString(0);
    BgkDisplayProgressIndicator();
    BgkDisplayBackgroundUpdate();
    InbvInstallDisplayStringFilter((__int64 (__fastcall *)(_QWORD))DisplayFilter);
    InbvDriverInitialize(1LL, BugCheckParameter3, 7LL);
    DisplayBootBitmap(0);
  }
  if ( v6 && strstr(v6, "MININT") )
  {
    InitIsWinPEMode = 1;
    if ( strstr(v6, "INRAM") )
      InitWinPEModeType |= 0x80000000;
    else
      InitWinPEModeType |= 1u;
  }
  Message = RtlFindMessage(0x140000000LL, 0xBu, 0, 0x4000007Eu, (unsigned __int16 **)&BugCheckParameter4);
  v20 = PoolWithTag;
  ppszDestEnd = PoolWithTag;
  v21 = Message;
  pcbRemaining = 256LL;
  if ( CmCSDVersionString.Length )
  {
    v22 = RtlStringCbPrintfExA(PoolWithTag, 0xFFuLL, &ppszDestEnd, &pcbRemaining, 0, ": %wZ", &CmCSDVersionString);
    if ( v22 < 0 )
      KeBugCheckEx(0x32u, v22, 7uLL, 0LL, 0LL);
    v20 = ppszDestEnd;
    v23 = pcbRemaining;
  }
  else
  {
    v23 = 255LL;
    pcbRemaining = 255LL;
  }
  *v20 = 0;
  v24 = v20 + 1;
  ppszDestEnd = v24;
  v25 = RtlStringCbPrintfA(pszDest, 0x18uLL, "%u.%u", 10, 0);
  if ( v25 < 0 )
    KeBugCheckEx(0x32u, v25, 7uLL, 1uLL, 0LL);
  if ( v21 < 0 )
  {
    v27 = RtlStringCbCopyA(v24, v23, "MICROSOFT (R) WINDOWS (TM)\n");
    if ( v27 < 0 )
      KeBugCheckEx(0x32u, v27, 7uLL, 3uLL, 0LL);
  }
  else
  {
    dwFlags[0] = (unsigned __int16)NtBuildNumber;
    v26 = RtlStringCbPrintfA(
            v24,
            v23,
            (NTSTRSAFE_PCSTR)(BugCheckParameter4 + 4),
            pszDest,
            *(_QWORD *)dwFlags,
            PoolWithTag);
    if ( v26 < 0 )
      KeBugCheckEx(0x32u, v26, 7uLL, 2uLL, 0LL);
  }
  InbvDisplayString((__int64)v24);
  v28 = 2;
  v29 = PoolWithTag + 256;
  v30 = PoolWithTag;
  v31 = 2LL;
  do
  {
    *v29 = *(_OWORD *)v30;
    v29[1] = *((_OWORD *)v30 + 1);
    v29[2] = *((_OWORD *)v30 + 2);
    v29[3] = *((_OWORD *)v30 + 3);
    v29[4] = *((_OWORD *)v30 + 4);
    v29[5] = *((_OWORD *)v30 + 5);
    v29[6] = *((_OWORD *)v30 + 6);
    v29 += 8;
    v32 = *((_OWORD *)v30 + 7);
    v30 += 128;
    *(v29 - 1) = v32;
    --v31;
  }
  while ( v31 );
  if ( !(unsigned __int8)PoInitSystem(0LL, BugCheckParameter3, 128LL) )
LABEL_211:
    KeBugCheck(0xA0u);
  if ( !ExpRealTimeIsUniversal )
  {
    v33 = *(_DWORD *)(v2 + 436);
    if ( v33 == -1 )
    {
      v33 = ExpAltTimeZoneBias;
      *(_DWORD *)(v2 + 436) = ExpAltTimeZoneBias;
      v118 = 1;
    }
    *(_QWORD *)(v2 + 440) = 600000000LL * v33;
    MEMORY[0xFFFFF7800000025C] = 0;
    ExpWriteTimeZoneBias((_DWORD *)(v2 + 440));
  }
  GetBootSystemTime(*(_QWORD *)(BugCheckParameter3 + 240), &Time);
  if ( v6 )
  {
    v34 = strstr(v6, "YEAR");
    if ( v34 )
    {
      v35 = strstr(v34, "=");
      if ( v35 )
      {
        v36 = atol(v35 + 1);
        RtlTimeToTimeFields(&Time, &TimeFields);
        TimeFields.Year = v36;
        RtlTimeFieldsToTime(&TimeFields, &Time);
      }
    }
  }
  if ( ExpRealTimeIsUniversal )
    LocalTime = Time;
  else
    ExSystemTimeToLocalTime(&Time, &LocalTime);
  KeSetSystemTime((__int64)&Time, (__int64)&v130, 4);
  PoNotifySystemTimeSet((__int64 *)&Time, &v130);
  RtlInitUnicodeString(&DestinationString, L"Kernel-RegisteredProcessors");
  if ( (int)ZwQueryLicenseValue((__int64)&DestinationString, (__int64)&v123) < 0 || v122 != 4 || v123 != 4 )
    KeRegisteredProcessors = 1;
  if ( v6 )
  {
    v37 = strstr(v6, " BOOTPROC=");
    if ( v37 )
    {
      v38 = strstr(v37, "=");
      if ( v38 )
        KeBootprocSpecified = atol(v38 + 1);
    }
    v39 = strstr(v6, " NUMPROC=");
    if ( v39 )
    {
      v40 = strstr(v39, "=");
      if ( v40 )
        KeNumprocSpecified = atol(v40 + 1);
    }
    v41 = strstr(v6, " HYPERVISORNUMPROC=");
    if ( v41 )
    {
      v42 = strstr(v41, "=");
      if ( v42 )
        KeHypervisorNumprocSpecified = atol(v42 + 1);
    }
    if ( !KeRootProcNumaNodeLpsSpecified )
    {
      v43 = strstr(v6, " HYPERVISORROOTPROCPERNODE=");
      if ( v43 )
      {
        v44 = strstr(v43, "=");
        if ( v44 )
          KeRootProcPerNodeSpecified = atol(v44 + 1);
      }
      v45 = strstr(v6, " HYPERVISORROOTPROCPERCORE=");
      if ( v45 )
      {
        v46 = strstr(v45, "=");
        if ( v46 )
          KeRootProcPerCoreSpecified = atol(v46 + 1);
      }
    }
    if ( strstr(v6, " MAXPROC") )
      KeMaximumProcessors = 1280;
  }
  qword_140D587B8 = KeQueryPerformanceCounter(0LL).QuadPart;
  KeStartAllProcessors(v47);
  qword_140D587C0 = KeQueryPerformanceCounter(0LL).QuadPart;
  KeSetAffinityProcess((__int64)KeGetCurrentThread()->ApcState.Process, 0, (unsigned __int16 *)KeActiveProcessors);
  MakeGdtReadOnly();
  v48 = (int)RtlFindMessage(0x140000000LL, 0xBu, 0, 0x40000089u, &v131) < 0
      ? "MultiProcessor Kernel\r\n"
      : (const char *)(v131 + 2);
  if ( !(unsigned __int8)HalAllProcessorsStarted() )
LABEL_178:
    KeBugCheck(0x61u);
  RtlInitAnsiString(&v136, v48);
  if ( v136.Length >= 2u )
    v136.Length -= 2;
  v49 = 1073741960;
  if ( (unsigned int)KeNumberProcessors_0 > 1 )
    v49 = 1073741981;
  v50 = RtlFindMessage(0x140000000LL, 0xBu, 0, v49, (unsigned __int16 **)&BugCheckParameter4);
  v51 = 0LL;
  v52 = *(unsigned int *)MmPhysicalMemoryBlock;
  if ( (_DWORD)v52 )
  {
    v53 = (char *)MmPhysicalMemoryBlock + 24;
    do
    {
      v51 += *v53;
      v53 += 2;
      --v52;
    }
    while ( v52 );
  }
  if ( v50 < 0 )
    v54 = RtlStringCbPrintfA(
            PoolWithTag,
            0x100uLL,
            "%u System Processor [%u MB Memory] %Z\n",
            (unsigned int)KeNumberProcessors_0,
            (unsigned __int64)(v51 + 255) >> 8,
            &v136);
  else
    v54 = RtlStringCbPrintfA(
            PoolWithTag,
            0x100uLL,
            (NTSTRSAFE_PCSTR)(BugCheckParameter4 + 4),
            (unsigned int)KeNumberProcessors_0,
            (unsigned __int64)(v51 + 255) >> 8,
            &v136);
  if ( v54 < 0 )
    KeBugCheckEx(0x32u, v54, 7uLL, 4uLL, 0LL);
  InbvDisplayString((__int64)PoolWithTag);
  ExFreePoolWithTag(PoolWithTag, 0);
  if ( !(unsigned __int8)ObInitSystem() )
    KeBugCheck(0x62u);
  if ( !(unsigned __int8)ExInitSystem() )
    KeBugCheckEx(0x32u, 0xFFFFFFFFC0000001uLL, 0LL, 1uLL, 0LL);
  HalReportResourceUsage(0xFFFFFFFFLL);
  if ( !(unsigned __int8)IoCreateObjectTypes() )
    KeBugCheckEx(0x32u, 0xFFFFFFFFC0000001uLL, 0LL, 4uLL, 0LL);
  if ( !(unsigned __int8)KeInitSystem(1LL) )
    KeBugCheckEx(0x32u, 0xFFFFFFFFC0000001uLL, 0LL, 2uLL, 0LL);
  if ( !KdInitSystem(InitializationPhase, 0LL, v55, v56) )
    KeBugCheckEx(0x32u, 0xFFFFFFFFC0000001uLL, 0LL, 3uLL, 0LL);
  inited = TmInitSystem(
             &TmResourceManagerObjectType,
             &TmEnlistmentObjectType,
             &TmTransactionManagerObjectType,
             &TmTransactionObjectType);
  if ( (int)(inited + 0x80000000) >= 0 && inited != -1073741637 )
    KeBugCheckEx(0x32u, 0LL, 0LL, 0LL, 0LL);
  v58 = DbgkInitialize();
  if ( v58 < 0 )
    KeBugCheckEx(0x32u, v58, 0LL, 0LL, 0LL);
  BcdInitializeBcdSyncMutant();
  VerifierInitSystem(0LL);
  if ( !(unsigned __int8)SeInitSystem() )
    KeBugCheck(0x63u);
  PsInitSystem(1LL, BugCheckParameter3);
  ExInitLicenseCallback();
  SystemRootLink = CreateSystemRootLink(BugCheckParameter3);
  if ( SystemRootLink < 0 )
    KeBugCheckEx(0x64u, SystemRootLink, 0LL, 0LL, 0LL);
  qword_140D587D8 = KeQueryPerformanceCounter(0LL).QuadPart;
  if ( !(unsigned __int8)MmInitSystem(1LL, BugCheckParameter3) )
    KeBugCheck(0x65u);
  qword_140D587E0 = KeQueryPerformanceCounter(0LL).QuadPart;
  if ( InitNlsTableSize )
  {
    MaximumSize.QuadPart = InitNlsTableSize;
    v60 = ZwCreateSection(&SectionHandle, 0xF001Fu, 0LL, &MaximumSize, 4u, 0x8000000u, 0LL);
    if ( v60 < 0 )
      KeBugCheckEx(0x32u, v60, 1uLL, 0LL, 0LL);
    Object = 0LL;
    v61 = ObReferenceObjectByHandle(SectionHandle, 0xF001Fu, MmSectionObjectType, 0, &Object, 0LL);
    InitNlsSectionPointer = Object;
    v62 = v61;
    ZwClose(SectionHandle);
    if ( (v62 & 0x80000000) != 0LL )
      KeBugCheckEx(0x32u, v62, 2uLL, 0LL, 0LL);
    MappedBase = 0LL;
    ViewSize = 0LL;
    v63 = MmMapViewInSystemSpace(InitNlsSectionPointer, &MappedBase, &ViewSize);
    if ( v63 < 0 )
      KeBugCheckEx(0x32u, v63, 3uLL, 0LL, 0LL);
    v64 = MappedBase;
    memmove(MappedBase, InitNlsTableBase, InitNlsTableSize);
    InitNlsTableBase = v64;
  }
  if ( !InitNlsTableBase )
  {
    v65 = 0LL;
LABEL_120:
    v66 = 0LL;
    v67 = 0LL;
    goto LABEL_124;
  }
  if ( !InitUnicodeCaseTableDataOffset )
  {
    v65 = (char *)InitNlsTableBase;
    goto LABEL_120;
  }
  v65 = (char *)InitNlsTableBase + (unsigned int)InitUnicodeCaseTableDataOffset;
  v66 = (USHORT *)((char *)InitNlsTableBase + (unsigned int)InitOemCodePageDataOffset);
  v67 = (USHORT *)((char *)InitNlsTableBase + (unsigned int)InitAnsiCodePageDataOffset);
LABEL_124:
  RtlInitNlsTables(v67, v66, (__int64)v65);
  RtlResetRtlTranslations();
  if ( !(unsigned __int8)CcInitializeCacheManager() )
    KeBugCheck(0x66u);
  if ( !(unsigned __int8)CmInitSystem1(BugCheckParameter3) )
    KeBugCheck(0x67u);
  v68 = ExInitializeLeapSecondData();
  if ( v68 < 0 )
    KeBugCheckEx(0x32u, v68, 0xCuLL, 0LL, 0LL);
  InitSkuSessionParameters();
  qword_140D587A8 = KeQueryPerformanceCounter(0LL).QuadPart;
  memset(v141, 0, 0x48uLL);
  LODWORD(v141[0]) = 72;
  v141[1] = MmMapLockedRestartPages;
  v141[2] = MmUnmapLockedRestartPages;
  v141[3] = KeRemoveEnclavePage;
  v141[4] = KdPullRemoteFileEx;
  VslGetKsrContext();
  DisplayContext = BgGetDisplayContext();
  v70 = (int)KsrInitSystem(BugCheckParameter3, v141, DisplayContext);
  qword_140D587B0 = KeQueryPerformanceCounter(0LL).QuadPart;
  if ( (int)(v70 + 0x80000000) >= 0 && (_DWORD)v70 != -1073741637 )
    KeBugCheckEx(0x32u, v70, 0LL, 1uLL, 0LL);
  ExKsrInterface = *(_OWORD *)&v141[5];
  *(_OWORD *)&xmmword_140C195D0 = *(_OWORD *)&v141[7];
  v71 = EmInitSystem(0LL, BugCheckParameter3);
  if ( v71 < 0 )
    KeBugCheckEx(0x32u, v71, 8uLL, 0LL, 0LL);
  v72 = MfgInitSystem(BugCheckParameter3);
  if ( v72 < 0 )
    KeBugCheckEx(0x32u, v72, 9uLL, 0LL, 0LL);
  PfInitializeSuperfetch();
  SmInitSystem(0LL);
  v73 = VmInitSystem(0LL);
  if ( v73 < 0 )
    KeBugCheckEx(0x32u, v73, 0xAuLL, 0LL, 0LL);
  if ( (*(_BYTE *)(*(_QWORD *)(BugCheckParameter3 + 240) + 2624LL) & 2) == 0 || strstr(v6, "FORCETIMESYNC") )
    ZwUpdateWnfStateData((__int64)&WNF_BOOT_INVALID_TIME_SOURCE, 0LL);
  HvlWnfUpdateStartupState();
  v125 = 0;
  FsRtlSendModernAppTermination(&v125, 1u, 1);
  ExInitializeTimeRefresh(v75, v74, v76);
  ExAcquireTimeRefreshLock(1u);
  ExInitializeUtcTimeZoneBias(&LocalTime);
  v77 = *(_DWORD *)(v2 + 436);
  ExpRefreshTimeZoneInformation(0);
  ExReleaseTimeRefreshLock();
  if ( v118 )
  {
    ExLocalTimeToSystemTime(&LocalTime, &Time);
    KeSetSystemTime((__int64)&Time, (__int64)&v130, 4);
  }
  else if ( v77 != *(_DWORD *)(v2 + 436) )
  {
    ZwSetSystemTime(0LL, 0LL);
  }
  if ( !(unsigned __int8)FsRtlInitSystem(v79, v78, v80) )
    KeBugCheck(0x68u);
  RtlInitializeCompression();
  RtlInitializeRangeListPackage(v82, v81, v83);
  HvlDebuggerSupportInitialize(BugCheckParameter3);
  HalReportResourceUsage(0LL);
  KdInitialize(1LL, BugCheckParameter3, &KdpContext, v84);
  if ( !(unsigned __int8)PpInitSystem(v86, v85, v87) )
    KeBugCheck(0x90u);
  if ( !(unsigned __int8)LpcInitSystem(v89, v88, v90) )
    KeBugCheck(0x6Au);
  if ( v6 )
    v94 = strstr(v6, "SAFEBOOT:");
  else
    v94 = 0LL;
  v95 = v94;
  if ( v94 )
  {
    v96 = 9LL;
    v97 = v94 + 9;
    if ( !strncmp(v97, "MINIMAL", 7uLL) )
    {
      LODWORD(InitSafeBootMode) = 1;
      v96 = 16LL;
    }
    else
    {
      if ( !strncmp(v97, "NETWORK", 7uLL) )
      {
        v96 = 16LL;
      }
      else if ( !strncmp(v97, "DSREPAIR", 8uLL) )
      {
        v28 = 3;
        v96 = 17LL;
      }
      else
      {
        v28 = 0;
      }
      LODWORD(InitSafeBootMode) = v28;
    }
    v92 = (__int64)&v95[v96];
    if ( v95[v96] )
    {
      v98 = strncmp((const char *)v92, "(ALTERNATESHELL)", 0x10uLL);
      v92 = (unsigned __int8)v120;
      if ( !v98 )
        v92 = 1LL;
      v120 = v92;
    }
    if ( (_DWORD)InitSafeBootMode )
    {
      v135 = 0LL;
      v99 = 0;
      switch ( (_DWORD)InitSafeBootMode )
      {
        case 1:
          v99 = 168;
          break;
        case 2:
          v99 = 169;
          break;
        case 3:
          v99 = 170;
          break;
      }
      if ( (int)RtlFindMessage(0x140000000LL, 0xBu, 0, v99, &v135) >= 0 )
        InbvDisplayString((__int64)(v135 + 2));
    }
  }
  if ( (*(_DWORD *)(*(_QWORD *)(BugCheckParameter3 + 240) + 132LL) & 0x800) != 0 )
  {
    if ( (int)RtlFindMessage(0x140000000LL, 0xBu, 0, 0xB7u, (unsigned __int16 **)&BugCheckParameter4) >= 0 )
      InbvDisplayString(BugCheckParameter4 + 4);
    IopInitializeBootLogging(BugCheckParameter3, (__int64)(PoolWithTag + 256));
  }
  ExInitSystemPhase2(v92, v91, v93);
  MEMORY[0xFFFFF780000002D6] = ExpComputeCyclesPerYield(v101, v100, v102);
  if ( InitIsWinPEMode )
    CreateMiniNtBootKey(v104, v103, v105);
  v106 = SeCodeIntegrityInitializePolicy(BugCheckParameter3);
  if ( v106 < 0 )
    KeBugCheckEx(0x32u, v106, 0x69436553uLL, 0LL, 0LL);
  KdpTimeSlipPending = 0;
  v110 = ExInitializeNls(v108, v107, v109);
  if ( v110 < 0 )
    KeBugCheckEx(0x32u, v110, 7uLL, 0LL, 0LL);
  v114 = ExInitializeExternalBootSupport(v112, v111, v113);
  if ( v114 < 0 )
    KeBugCheckEx(0x32u, v114, 8uLL, 0LL, 0LL);
  if ( !(unsigned __int8)PoInitSystem(1LL, BugCheckParameter3, v115) )
    goto LABEL_211;
  return v120;
}
