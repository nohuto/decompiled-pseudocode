/*
 * XREFs of MiGetUltraMdlContext @ 0x14055D898
 * Callers:
...
PSLIST_ENTRY MiGetUltraMdlContext()
{
  ...
  {
    LODWORD(v1) = v1 & 7;
    result = RtlpInterlockedPopEntrySList((PSLIST_HEADER)(qword_140C4EAB8 + ((v2 + (unsigned int)v1) << 6)));
    if ( result )
      return result;
    LOBYTE(v1) = v1 + 1;
    ++v0;
  ...
  while ( v0 < 8 );
  if ( (unsigned int)dword_140C4EAB0 >= 0x80 )
    return 0LL;
