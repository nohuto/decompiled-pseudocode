/*
 * XREFs of VfTargetDriversGetVerifierData @ 0x1409D9E50
 * Callers:
 *     VfUtilGetDifPluginDriverData @ 0x14059ED30 (VfUtilGetDifPluginDriverData.c)
 *     VfUtilCheckRuleEnforcement @ 0x1409C95EC (VfUtilCheckRuleEnforcement.c)
 *     VfUtilGetDriverName @ 0x1409C9780 (VfUtilGetDriverName.c)
 *     VfUtilGetSigningLevel @ 0x1409C97B0 (VfUtilGetSigningLevel.c)
 *     VfUtilIsBootDriver @ 0x1409C97D0 (VfUtilIsBootDriver.c)
 *     VeAllocatePoolWithTagPriority @ 0x1409D75E0 (VeAllocatePoolWithTagPriority.c)
 *     VfTargetDriversIsEnabled @ 0x1409D9F40 (VfTargetDriversIsEnabled.c)
 * Callees:
 *     VfTargetDriversGetNode @ 0x1405A036C (VfTargetDriversGetNode.c)
 *     VfDriverIsKernelImageAddress @ 0x1409CB8D0 (VfDriverIsKernelImageAddress.c)
 */

__int64 __fastcall VfTargetDriversGetVerifierData(unsigned __int64 a1)
{
  unsigned __int64 v1; // rcx
  _QWORD *Node; // rax

  if ( VfDriverIsKernelImageAddress(a1) )
  {
    if ( !KernelVerifier )
      return 0LL;
  }
  else if ( KernelVerifier )
  {
    if ( dword_140C1CF48 < 2 )
      return 0LL;
  }
  else if ( !dword_140C1CF48 )
  {
    return 0LL;
  }
  Node = VfTargetDriversGetNode(v1);
  if ( !Node )
    return 0LL;
  return Node[7];
}
