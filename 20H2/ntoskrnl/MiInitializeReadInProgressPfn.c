/*
 * XREFs of MiInitializeReadInProgressPfn @ 0x14024D760
 * Callers:
 *     MiPrivateFixup @ 0x140239840 (MiPrivateFixup.c)
 *     MiResolvePageFileFault @ 0x140239F98 (MiResolvePageFileFault.c)
 *     MiPfPutPagesInTransition @ 0x14024CE50 (MiPfPutPagesInTransition.c)
 * Callees:
 *     MiLocateAddress @ 0x14022ED30 (MiLocateAddress.c)
 *     MiIsPrototypePteVadLookup @ 0x140236D50 (MiIsPrototypePteVadLookup.c)
 *     MiChangePageAttribute @ 0x14023DCD0 (MiChangePageAttribute.c)
 *     MiSwizzleInvalidPte @ 0x1402AD3A0 (MiSwizzleInvalidPte.c)
 *     MI_READ_PTE_LOCK_FREE @ 0x1402B12D0 (MI_READ_PTE_LOCK_FREE.c)
 *     KeYieldProcessorEx @ 0x1402DE380 (KeYieldProcessorEx.c)
 *     MiWritePteShadow @ 0x140313F2C (MiWritePteShadow.c)
 *     MiPteHasShadow @ 0x140313F8C (MiPteHasShadow.c)
 *     KiRemoveSystemWorkPriorityKick @ 0x1403F1DD4 (KiRemoveSystemWorkPriorityKick.c)
 */

unsigned __int64 __fastcall MiInitializeReadInProgressPfn(
        __int64 Process,
        _QWORD *a2,
        int a3,
        __int64 SchedulerAssist,
        __int64 a5,
        char a6)
{
  unsigned __int64 v6; // r11
  _QWORD *v7; // r14
  unsigned __int64 v8; // r15
  __int64 v9; // rbp
  __int64 v10; // rdi
  unsigned __int64 result; // rax
  unsigned __int64 v12; // r10
  __int64 v13; // r14
  unsigned __int64 v14; // rbx
  __int64 v15; // rdx
  __int64 v16; // rax
  __int64 v17; // rdx
  unsigned __int64 v18; // rax
  __int64 v19; // r9
  unsigned __int64 v20; // r8
  unsigned __int64 v21; // rax
  __int64 v22; // rcx
  unsigned __int64 v23; // rax
  unsigned __int64 v24; // rdx
  __int64 Address; // rax
  char v26; // dl
  unsigned __int64 v27; // r10
  __int64 v28; // rax
  char v29; // r10
  unsigned int v30; // r13d
  unsigned __int64 v31; // rdx
  unsigned __int64 v32; // r10
  unsigned __int64 v33; // rdi
  struct _LIST_ENTRY *Flink; // rax
  __int64 v35; // rax
  struct _LIST_ENTRY *v36; // rdx
  __int64 v37; // rax
  __int64 v38; // rdx
  __int64 v39; // r15
  unsigned __int8 CurrentIrql; // r12
  __int64 v41; // rax
  __int64 v42; // rax
  unsigned __int8 v43; // al
  struct _KPRCB *CurrentPrcb; // r10
  int v45; // eax
  bool v46; // zf
  __int64 v47; // rdx
  int v48; // [rsp+20h] [rbp-78h]
  int v49; // [rsp+24h] [rbp-74h] BYREF
  int v50; // [rsp+28h] [rbp-70h] BYREF
  __int64 v51; // [rsp+30h] [rbp-68h] BYREF
  unsigned __int64 v52; // [rsp+38h] [rbp-60h] BYREF
  unsigned __int64 v53; // [rsp+40h] [rbp-58h]
  unsigned __int64 v54; // [rsp+A0h] [rbp+8h]
  _QWORD *v55; // [rsp+A8h] [rbp+10h]
  unsigned __int64 v56; // [rsp+B8h] [rbp+20h]

  v56 = SchedulerAssist;
  v55 = a2;
  v54 = Process;
  v6 = SchedulerAssist;
  v7 = a2;
  v8 = Process;
  v9 = 0LL;
  v10 = 0xFFFFFFFFFLL;
  result = (unsigned __int64)&a2[a3];
  v53 = result;
  if ( (unsigned __int64)a2 < result )
  {
    v12 = 0xFFFFF6FB7DBED7F8uLL;
    while ( 1 )
    {
      result = 0xFFFFFA8000000000uLL;
      v13 = 48LL * *v7 - 0x58000000000LL;
      if ( v13 != qword_140C4EBF0 )
        break;
LABEL_129:
      v6 += 8LL;
      v7 = v55 + 1;
      v56 = v6;
      v55 = v7;
      if ( (unsigned __int64)v7 >= v53 )
        return result;
    }
    v14 = *(_QWORD *)v6;
    if ( v6 >= 0xFFFFF6FB7DBED000uLL
      && v6 <= 0xFFFFF6FB7DBED7F8uLL
      && (unsigned int)MiPteHasShadow(Process, 0xFFFFF6FB7DBED000uLL, 0xFFFFFA8000000000uLL, SchedulerAssist)
      && (v14 & 1) != 0
      && ((v14 & 0x20) == 0 || (v14 & 0x42) == 0) )
    {
      Process = (__int64)KeGetCurrentThread()->ApcState.Process;
      v15 = *(_QWORD *)(Process + 1928);
      if ( v15 )
      {
        v16 = *(_QWORD *)(v15 + 8 * ((v6 >> 3) & 0x1FF));
        v17 = v14 | 0x20;
        Process = (unsigned __int8)v16;
        LOBYTE(Process) = v16 & 0x20;
        if ( (v16 & 0x20) == 0 )
          v17 = v14;
        v14 = v17;
        if ( (v16 & 0x42) != 0 )
          v14 = v17 | 0x42;
      }
    }
    v19 = (unsigned __int8)a6;
    v51 = v14;
    v18 = v14;
    LOBYTE(v19) = (a6 & 0x40) != 0;
    LOBYTE(v48) = 0;
    v20 = 0LL;
    if ( (a6 & 0x40) != 0 )
    {
      if ( MiIsPrototypePteVadLookup(v14) )
      {
        v21 = v14;
      }
      else
      {
        v22 = v14;
        if ( qword_140C4DDC0 && (v14 & 0x10) == 0 )
          v22 = v14 & ~qword_140C4DDC0;
        v51 = MI_READ_PTE_LOCK_FREE(v22 >> 16);
        v23 = MI_READ_PTE_LOCK_FREE(&v51);
        v21 = MI_READ_PTE_LOCK_FREE(48 * ((v23 >> 12) & 0xFFFFFFFFFLL) - 0x57FFFFFFFF0LL);
      }
      v48 = MmMakeProtectNotWriteCopy[(v21 >> 5) & 0x1F];
      v18 = MiSwizzleInvalidPte(32LL * (v48 & 0x1F));
    }
    else
    {
      v24 = v14;
      if ( (v14 & 0x400) != 0 || (v14 & 0x800) == 0 )
      {
LABEL_30:
        *(_QWORD *)(v13 + 16) = v18;
        if ( (_BYTE)v19 )
        {
          LODWORD(Address) = v20;
        }
        else
        {
          if ( (a6 & 0x10) != 0
            && (*(_QWORD *)(v13 + 40) |= 0x8000000000000000uLL,
                Process = (__int64)&v51,
                (unsigned __int64)&v51 >= 0xFFFFF6FB7DBED000uLL)
            && (Process = (__int64)&v51, (unsigned __int64)&v51 <= v12) )
          {
            if ( (unsigned int)MiPteHasShadow(&v51, v24, v20, v19)
              && (v26 & 1) != 0
              && ((v26 & 0x20) == 0 || (v26 & 0x42) == 0) )
            {
              Process = (__int64)KeGetCurrentThread()->ApcState.Process;
              v20 = *(_QWORD *)(Process + 1928);
              if ( v20 )
              {
                v28 = *(_QWORD *)(v20 + 8 * (((unsigned __int64)&v51 >> 3) & 0x1FF));
                if ( (v28 & 0x20) != 0 )
                  v27 |= 0x20uLL;
                if ( (v28 & 0x42) != 0 )
                  v27 |= 0x42uLL;
              }
              else
              {
                v27 = v51;
              }
            }
          }
          else
          {
            v27 = v24;
          }
          v48 = (v27 >> 5) & 0x1F;
          Address = (v27 >> 5) & 0x1F;
          if ( v48 == 24 )
          {
            Process = 0x7FFFFFFEFFFFLL;
            LOBYTE(v48) = 24;
            if ( v8 <= 0x7FFFFFFEFFFFLL )
            {
              Address = (__int64)MiLocateAddress(v8);
              LOBYTE(v48) = v29;
              LODWORD(Address) = (*(_DWORD *)(Address + 48) >> 7) & 0x1F;
            }
          }
        }
        if ( (_DWORD)Address )
        {
          if ( (_DWORD)Address == 31 )
          {
            v30 = 1;
          }
          else
          {
            Process = (unsigned int)Address >> 3;
            if ( (_DWORD)Process == 3 && (Address & 7) != 0 )
              v30 = 2;
            else
              v30 = Process != 1;
          }
        }
        else
        {
          v30 = 3;
        }
        SchedulerAssist = 0xFFFFFFFFFLL;
        if ( v10 == 0xFFFFFFFFFLL )
        {
          v31 = ((v6 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL;
          v32 = 0xFFFFF6FB7DBED000uLL;
          v33 = *(_QWORD *)v31;
          v20 = 0xFFFFF6FB7DBED7F8uLL;
          if ( v31 >= 0xFFFFF6FB7DBED000uLL
            && v31 <= 0xFFFFF6FB7DBED7F8uLL
            && (unsigned int)MiPteHasShadow(Process, v31, 0xFFFFF6FB7DBED7F8uLL, 0xFFFFFFFFFLL)
            && (v33 & 1) != 0
            && ((v33 & 0x20) == 0 || (v33 & 0x42) == 0) )
          {
            Flink = KeGetCurrentThread()->ApcState.Process[1].ProcessListEntry.Flink;
            if ( Flink )
            {
              v35 = *((_QWORD *)&Flink->Flink + ((v31 >> 3) & 0x1FF));
              v31 = v33 | 0x20;
              if ( (v35 & 0x20) == 0 )
                v31 = v33;
              v33 = v31;
              if ( (v35 & 0x42) != 0 )
                v33 = v31 | 0x42;
            }
          }
          v52 = v33;
          if ( (unsigned __int64)&v52 >= v32
            && (unsigned __int64)&v52 <= v20
            && (unsigned int)MiPteHasShadow(&v52, v31, v20, SchedulerAssist)
            && (v33 & 1) != 0
            && ((v33 & 0x20) == 0 || (v33 & 0x42) == 0) )
          {
            v36 = KeGetCurrentThread()->ApcState.Process[1].ProcessListEntry.Flink;
            if ( v36 )
            {
              v37 = *((_QWORD *)&v36->Flink + (((unsigned __int64)&v52 >> 3) & 0x1FF));
              v38 = v33 | 0x20;
              if ( (v37 & 0x20) == 0 )
                v38 = v33;
              v33 = v38;
              if ( (v37 & 0x42) != 0 )
                v33 = v38 | 0x42;
            }
          }
          v10 = SchedulerAssist & (v33 >> 12);
          v9 = 48 * v10 - 0x58000000000LL;
        }
        v39 = v6;
        if ( (a6 & 0x20) != 0 )
          v39 = v6 & 0x7FFFFFFFFFFFFFFFLL;
        CurrentIrql = KeGetCurrentIrql();
        __writecr8(2uLL);
        if ( KiIrqlFlags && (KiIrqlFlags & 1) != 0 && CurrentIrql <= 0xFu )
        {
          SchedulerAssist = (__int64)KeGetCurrentPrcb()->SchedulerAssist;
          v20 = (-1 << (CurrentIrql + 1)) & 4u | *(_DWORD *)(SchedulerAssist + 20);
          *(_DWORD *)(SchedulerAssist + 20) = v20;
        }
        v49 = 0;
        while ( _interlockedbittestandset64((volatile signed __int32 *)(v13 + 24), 0x3FuLL) )
        {
          do
            KeYieldProcessorEx(&v49);
          while ( *(__int64 *)(v13 + 24) < 0 );
        }
        *(_BYTE *)(v13 + 34) |= 0x20u;
        v41 = a5;
        if ( a5 )
          v41 = a5 + 32;
        *(_QWORD *)v13 = v41;
        Process = *(unsigned __int8 *)(v13 + 34);
        if ( *(unsigned __int8 *)(v13 + 34) >> 6 != v30 )
        {
          MiChangePageAttribute(v13, v30, 1);
          Process = *(unsigned __int8 *)(v13 + 34);
        }
        *(_QWORD *)(v13 + 24) &= 0xC000000000000000uLL;
        *(_WORD *)(v13 + 32) = 1;
        if ( (a6 & 8) != 0 )
          *(_BYTE *)(v13 + 35) ^= (*(_BYTE *)(v13 + 35) ^ a6) & 7;
        v42 = v10 ^ *(_QWORD *)(v13 + 40);
        *(_QWORD *)(v13 + 8) = v39;
        LOBYTE(Process) = Process & 0xF8 | 2;
        *(_QWORD *)(v13 + 40) ^= v42 & 0xFFFFFFFFFLL;
        *(_BYTE *)(v13 + 34) = Process;
        if ( a6 < 0 )
          *(_BYTE *)(v13 + 35) |= 0x20u;
        _InterlockedAnd64((volatile signed __int64 *)(v13 + 24), 0x7FFFFFFFFFFFFFFFuLL);
        if ( KiIrqlFlags )
        {
          if ( (KiIrqlFlags & 1) != 0 )
          {
            v43 = KeGetCurrentIrql();
            if ( v43 <= 0xFu && CurrentIrql <= 0xFu && v43 >= 2u )
            {
              CurrentPrcb = KeGetCurrentPrcb();
              Process = (unsigned int)CurrentIrql + 1;
              SchedulerAssist = (__int64)CurrentPrcb->SchedulerAssist;
              v45 = ~(unsigned __int16)(-1LL << (CurrentIrql + 1));
              v46 = (v45 & *(_DWORD *)(SchedulerAssist + 20)) == 0;
              v20 = (unsigned int)v45 & *(_DWORD *)(SchedulerAssist + 20);
              *(_DWORD *)(SchedulerAssist + 20) = v20;
              if ( v46 )
                KiRemoveSystemWorkPriorityKick(CurrentPrcb);
            }
          }
        }
        result = CurrentIrql;
        __writecr8(CurrentIrql);
        if ( (v14 & 0x400) == 0 && (v14 & 0x800) != 0 )
        {
          v6 = v56;
        }
        else
        {
          v47 = 32 * (((*v55 & 0xFFFFFFFFFLL) << 7) | v48 & 0x1F | 0x40);
          if ( qword_140C4DDC0 )
          {
            if ( (qword_140C4DDC0 & v47) != 0 )
              v47 |= 0x10uLL;
            else
              v47 |= qword_140C4DDC0;
          }
          result = 0xFFFFF6FB7DBED000uLL;
          v6 = v56;
          if ( v56 >= 0xFFFFF6FB7DBED000uLL )
          {
            result = 0xFFFFF6FB7DBED7F8uLL;
            if ( v56 <= 0xFFFFF6FB7DBED7F8uLL )
            {
              if ( (unsigned int)MiPteHasShadow(Process, v47, v20, SchedulerAssist) )
              {
                if ( !HIBYTE(word_140C4DE88) && (v47 & 1) != 0 )
                  v47 |= 0x8000000000000000uLL;
                *(_QWORD *)v6 = v47;
                result = MiWritePteShadow(v6, v47);
                goto LABEL_123;
              }
              result = (unsigned __int64)KeGetCurrentThread();
              Process = *(_QWORD *)(result + 184);
              if ( (*(_DWORD *)(Process + 2172) & 0x1000) != 0 && (v47 & 1) != 0 )
              {
                result = 0x8000000000000000uLL;
                v47 |= 0x8000000000000000uLL;
              }
            }
          }
          *(_QWORD *)v6 = v47;
        }
LABEL_123:
        if ( (a6 & 0x40) == 0 )
        {
          v50 = 0;
          while ( _interlockedbittestandset64((volatile signed __int32 *)(v9 + 24), 0x3FuLL) )
          {
            do
              KeYieldProcessorEx(&v50);
            while ( *(__int64 *)(v9 + 24) < 0 );
          }
          result = *(_QWORD *)(v9 + 24);
          Process = result ^ (result ^ (result + 1)) & 0x3FFFFFFFFFFFFFFFLL;
          *(_QWORD *)(v9 + 24) = Process;
          _InterlockedAnd64((volatile signed __int64 *)(v9 + 24), 0x7FFFFFFFFFFFFFFFuLL);
          v6 = v56;
        }
        v8 = v54;
        v12 = 0xFFFFF6FB7DBED7F8uLL;
        goto LABEL_129;
      }
      if ( qword_140C4DDC0 && (v14 & 0x10) == 0 )
        v18 = v14 & ~qword_140C4DDC0;
      Process = 6 * ((v18 >> 12) & 0xFFFFFFFFFLL);
      v18 = *(_QWORD *)(48 * ((v18 >> 12) & 0xFFFFFFFFFLL) - 0x58000000000LL + 16);
    }
    v24 = v18;
    v51 = v18;
    goto LABEL_30;
  }
  return result;
}
