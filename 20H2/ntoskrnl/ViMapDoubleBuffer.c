/*
 * XREFs of ViMapDoubleBuffer @ 0x1409D20A4
 * Callers:
...
__int64 __fastcall ViMapDoubleBuffer(__int64 a1, PMDL MemoryDescriptorList, ULONG_PTR a3, unsigned int a4, char a5)
{
  ...
  if ( !a4 )
  {
    ViHalPreprocessOptions(byte_140C12D04, "Driver is attempting to map a 0-length transfer.", 33LL, a1, 0LL, 0LL);
    Priority = byte_140C12D04;
    v9 = 0LL;
...
LABEL_10:
  ...
  if ( (unsigned int)(a3 - MemoryDescriptorList->ByteOffset - LODWORD(MemoryDescriptorList->StartVa)) >= MemoryDescriptorList->ByteCount )
  {
    ViHalPreprocessOptions(
      ...
      a3,
      (__int64)MemoryDescriptorList);
    VfReportIssueWithOptions(0xE6u, 0x1FuLL, 2uLL, a3, (ULONG_PTR)MemoryDescriptorList, &dword_140C12D0C);
    return 0LL;
  }
