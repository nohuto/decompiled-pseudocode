/*
 * XREFs of PopDiagTraceSystemIdleAssessment @ 0x1408EE004
 * Callers:
 *     PopIsSystemIdle @ 0x1408F3704 (PopIsSystemIdle.c)
 * Callees:
 *     EtwWrite @ 0x14020E030 (EtwWrite.c)
 *     EtwEventEnabled @ 0x14027E3D0 (EtwEventEnabled.c)
 *     __security_check_cookie @ 0x1403CFAF0 (__security_check_cookie.c)
 */

char __fastcall PopDiagTraceSystemIdleAssessment(int a1, int a2, unsigned __int8 a3)
{
  _UNKNOWN **v3; // rax
  int v4; // edi
  REGHANDLE v5; // rbx
  int v7; // [rsp+30h] [rbp-40h] BYREF
  struct _EVENT_DATA_DESCRIPTOR UserData; // [rsp+38h] [rbp-38h] BYREF
  int *v9; // [rsp+48h] [rbp-28h]
  int v10; // [rsp+50h] [rbp-20h]
  int v11; // [rsp+54h] [rbp-1Ch]
  int *v12; // [rsp+58h] [rbp-18h]
  int v13; // [rsp+60h] [rbp-10h]
  int v14; // [rsp+64h] [rbp-Ch]
  _UNKNOWN *retaddr; // [rsp+78h] [rbp+8h] BYREF
  int v16; // [rsp+80h] [rbp+10h] BYREF
  int v17; // [rsp+88h] [rbp+18h] BYREF

  v3 = &retaddr;
  v17 = a2;
  v16 = a1;
  v4 = a3;
  if ( PopDiagHandleRegistered )
  {
    v5 = PopDiagHandle;
    LOBYTE(v3) = EtwEventEnabled(PopDiagHandle, &POP_ETW_EVENT_SYSTEM_IDLE_ASSESSMENT);
    if ( (_BYTE)v3 )
    {
      UserData.Reserved = 0;
      v11 = 0;
      v14 = 0;
      UserData.Ptr = (ULONGLONG)&v16;
      UserData.Size = 4;
      v9 = &v17;
      v12 = &v7;
      v10 = 4;
      v13 = 4;
      v7 = v4;
      LOBYTE(v3) = EtwWrite(v5, &POP_ETW_EVENT_SYSTEM_IDLE_ASSESSMENT, 0LL, 3u, &UserData);
    }
  }
  return (char)v3;
}
