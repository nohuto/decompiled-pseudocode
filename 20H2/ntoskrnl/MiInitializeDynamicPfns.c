/*
 * XREFs of MiInitializeDynamicPfns @ 0x14052CAA0
 * Callers:
 *     MiMapNewPfns @ 0x1408C8514 (MiMapNewPfns.c)
 *     MiRemovePhysicalMemory @ 0x1408C87CC (MiRemovePhysicalMemory.c)
 * Callees:
 *     MiReferenceControlAreaPfn @ 0x140232A28 (MiReferenceControlAreaPfn.c)
 *     MmAccessFault @ 0x14026E3B0 (MmAccessFault.c)
 *     ExReleaseSpinLockExclusiveFromDpcLevel @ 0x14027E490 (ExReleaseSpinLockExclusiveFromDpcLevel.c)
 *     MiSearchNumaNodeTable @ 0x1402AEBA0 (MiSearchNumaNodeTable.c)
 *     MiUnlockProtoPoolPage @ 0x1402C8010 (MiUnlockProtoPoolPage.c)
 *     MiLockProtoPoolPage @ 0x1402CA9B0 (MiLockProtoPoolPage.c)
 *     ExAcquireSpinLockExclusive @ 0x1402D4460 (ExAcquireSpinLockExclusive.c)
 *     MiAbortCombineScan @ 0x140349794 (MiAbortCombineScan.c)
 *     MiUpdateLargePageBitMap @ 0x14034A334 (MiUpdateLargePageBitMap.c)
 *     MiLockPageInline @ 0x14034E290 (MiLockPageInline.c)
 *     MiInitializeUnusablePfns @ 0x1403AB0E0 (MiInitializeUnusablePfns.c)
 *     MiRestrictRangeToNode @ 0x1403AB5C0 (MiRestrictRangeToNode.c)
 *     __security_check_cookie @ 0x1403CFAF0 (__security_check_cookie.c)
 *     KiRemoveSystemWorkPriorityKick @ 0x1403F1DD4 (KiRemoveSystemWorkPriorityKick.c)
 *     MiInitializeAllResidentPageBasePfns @ 0x1403F542C (MiInitializeAllResidentPageBasePfns.c)
 *     MiDetermineNewPfnHeatState @ 0x1403F65E4 (MiDetermineNewPfnHeatState.c)
 *     memset @ 0x140411300 (memset.c)
 *     MiSetPfnRemovalRequested @ 0x14054D928 (MiSetPfnRemovalRequested.c)
 *     MiInitializeDpcGang @ 0x14055EA80 (MiInitializeDpcGang.c)
 *     MiStartDpcGang @ 0x14055EBDC (MiStartDpcGang.c)
 */

void __fastcall MiInitializeDynamicPfns(
        ULONG_PTR BugCheckParameter2,
        ULONG_PTR a2,
        __int16 *a3,
        unsigned int a4,
        __int64 a5,
        __int64 a6)
{
  __int64 v10; // rdx
  __int64 v11; // r8
  _DWORD *SchedulerAssist; // r9
  unsigned __int64 v13; // rbx
  char v14; // si
  int v15; // eax
  KIRQL v16; // al
  unsigned __int64 v17; // rsi
  unsigned __int8 CurrentIrql; // al
  struct _KPRCB *CurrentPrcb; // r10
  int v20; // eax
  bool v21; // zf
  unsigned int v22; // r14d
  int v23; // eax
  int v24; // r9d
  unsigned __int64 v25; // rsi
  unsigned __int8 v26; // r13
  __int64 v27; // r15
  ULONG_PTR v28; // rax
  unsigned __int64 v29; // rbx
  _QWORD *v30; // rax
  int v31; // eax
  char v32; // al
  unsigned __int64 v33; // rsi
  int v34; // r15d
  unsigned __int8 v35; // al
  __int128 v36; // xmm1
  __int64 v37; // rax
  unsigned __int64 v38; // rcx
  __int128 v39; // xmm0
  __int128 v40; // xmm1
  __int128 v41; // xmm0
  unsigned __int8 v42; // al
  unsigned __int8 v43; // di
  struct _KPRCB *v44; // r10
  int v45; // eax
  unsigned __int8 v46[2]; // [rsp+40h] [rbp-C0h] BYREF
  __int16 v47; // [rsp+42h] [rbp-BEh]
  int v48; // [rsp+44h] [rbp-BCh]
  __int64 v49; // [rsp+48h] [rbp-B8h]
  __int64 v50; // [rsp+50h] [rbp-B0h]
  __int16 *v51; // [rsp+58h] [rbp-A8h]
  __int128 v52; // [rsp+60h] [rbp-A0h]
  __int128 v53; // [rsp+70h] [rbp-90h]
  __int128 v54; // [rsp+80h] [rbp-80h]
  __int128 v55; // [rsp+90h] [rbp-70h]
  __int128 v56; // [rsp+A0h] [rbp-60h]
  __int128 v57; // [rsp+B0h] [rbp-50h]
  _QWORD v58[48]; // [rsp+C0h] [rbp-40h] BYREF

  v50 = a5;
  v51 = a3;
  v49 = a6;
  v55 = 0LL;
  v56 = 0LL;
  v57 = 0LL;
  v52 = 0LL;
  v53 = 0LL;
  v54 = 0LL;
  memset(v58, 0, sizeof(v58));
  v47 = *a3;
  v13 = 48 * BugCheckParameter2 - 0x58000000000LL;
  v14 = 5;
  v15 = a4 & 6;
  v48 = v15;
  if ( v15 == 4 )
  {
    v16 = ExAcquireSpinLockExclusive(&dword_140C50CC8);
    qword_140C52710 += a2;
    v17 = v16;
    byte_140C50C0C = 1;
    ExReleaseSpinLockExclusiveFromDpcLevel(&dword_140C50CC8);
    if ( KiIrqlFlags )
    {
      if ( (KiIrqlFlags & 1) != 0 )
      {
        CurrentIrql = KeGetCurrentIrql();
        if ( CurrentIrql <= 0xFu && (unsigned __int8)v17 <= 0xFu && CurrentIrql >= 2u )
        {
          CurrentPrcb = KeGetCurrentPrcb();
          v10 = -1LL << ((unsigned __int8)v17 + 1);
          SchedulerAssist = CurrentPrcb->SchedulerAssist;
          v20 = ~(unsigned __int16)v10;
          v21 = (v20 & SchedulerAssist[5]) == 0;
          v11 = (unsigned int)v20 & SchedulerAssist[5];
          SchedulerAssist[5] = v11;
          if ( v21 )
            KiRemoveSystemWorkPriorityKick((__int64)CurrentPrcb);
        }
      }
    }
    __writecr8(v17);
    v15 = v48;
    v14 = 6;
  }
  if ( (a4 & 1) != 0 )
  {
    v22 = a4 | 0x40;
    if ( v15 != 4 )
      v22 = a4;
    if ( (v22 & 0x1800) != 0 )
    {
      v23 = MiDetermineNewPfnHeatState(0, (v22 >> 11) & 1);
      MiInitializeAllResidentPageBasePfns(
        (unsigned __int16 *)&MiSystemPartition,
        BugCheckParameter2,
        a2,
        v24,
        1,
        v23 == 0,
        0);
    }
    if ( (v22 & 2) != 0 )
    {
      v25 = *(_QWORD *)(a5 + 8);
      MiReferenceControlAreaPfn(**(_QWORD **)a5, *(_QWORD *)a5, a2);
    }
    else
    {
      v25 = 0LL;
    }
    v26 = 17;
    v27 = 0LL;
    v46[0] = 17;
    while ( a2 )
    {
      v28 = MiRestrictRangeToNode(BugCheckParameter2, a2);
      v29 = v28;
      if ( v50 )
      {
        if ( v28 > (unsigned __int64)(4096 - (unsigned int)(v25 & 0xFFF)) >> 3 )
          v29 = (unsigned __int64)(4096 - (unsigned int)(v25 & 0xFFF)) >> 3;
        while ( 1 )
        {
          v27 = MiLockProtoPoolPage(v25, v46);
          if ( v27 )
            break;
          MmAccessFault(2uLL, v25, 0, 0LL);
        }
        MiInitializeUnusablePfns(
          (__m128i *)(48 * BugCheckParameter2 - 0x58000000000LL),
          v29,
          v47,
          v22,
          *(_QWORD *)v50,
          v25,
          *(_DWORD *)(v50 + 16));
        v26 = v46[0];
      }
      else
      {
        v30 = MiSearchNumaNodeTable(BugCheckParameter2);
        MiInitializeDpcGang(v58, *((unsigned int *)v30 + 2), 1LL);
        v58[21] = v51;
        v31 = LODWORD(v58[23]) | 4;
        HIDWORD(v58[22]) = v22;
        v58[0] = BugCheckParameter2;
        v58[1] = v29;
        v58[20] = v49;
        if ( v49 )
          v31 = LODWORD(v58[23]) | 0xC;
        LODWORD(v58[23]) = v31 | 2;
        MiStartDpcGang(v58);
      }
      if ( v25 )
      {
        MiUnlockProtoPoolPage(v27, v26);
        v25 += 8 * v29;
      }
      a2 -= v29;
      BugCheckParameter2 += v29;
      v49 += v29 << 12;
    }
  }
  else
  {
    if ( (a4 & 2) == 0 )
      MiUpdateLargePageBitMap((__int64)&MiSystemPartition, BugCheckParameter2, a2, 0, 0);
    *((_QWORD *)&v56 + 1) |= 0x8000000000000000uLL;
    *((_QWORD *)&v53 + 1) |= 0x8000000000000000uLL;
    v32 = v14 | BYTE2(v54) & 0xF8;
    v33 = v13 + 48 * a2;
    BYTE2(v54) = v32;
    if ( v13 < v33 )
    {
      v34 = a4 & 0x10;
      do
      {
        v35 = MiLockPageInline(v13, v10, v11, SchedulerAssist);
        v21 = (*(_BYTE *)(v13 + 35) & 0x40) == 0;
        v46[0] = v35;
        if ( !v21 )
          MiSetPfnRemovalRequested(v13, 0LL);
        v36 = v53;
        v37 = *((_QWORD *)&v54 + 1);
        v38 = *((_QWORD *)&v54 + 1) ^ ((unsigned __int64)(unsigned __int16)(*(_QWORD *)(v13 + 40) >> 39) << 39);
        *(_OWORD *)v13 = v52;
        *((_QWORD *)&v54 + 1) = v38 & 0x1FF8000000000LL ^ v37;
        v39 = v54;
        *(_OWORD *)(v13 + 16) = v36;
        *(_OWORD *)(v13 + 32) = v39;
        MiAbortCombineScan(v13);
        if ( v34 )
        {
          v40 = v56;
          *(_OWORD *)v13 = v55;
          v41 = v57;
          *(_OWORD *)(v13 + 16) = v40;
          *(_OWORD *)(v13 + 32) = v41;
        }
        _InterlockedAnd64((volatile signed __int64 *)(v13 + 24), 0x7FFFFFFFFFFFFFFFuLL);
        if ( KiIrqlFlags && (KiIrqlFlags & 1) != 0 && (v42 = KeGetCurrentIrql(), v42 <= 0xFu) )
        {
          v43 = v46[0];
          if ( v46[0] <= 0xFu && v42 >= 2u )
          {
            v44 = KeGetCurrentPrcb();
            SchedulerAssist = v44->SchedulerAssist;
            v43 = v46[0];
            v10 = -1LL << (v46[0] + 1);
            v45 = ~(unsigned __int16)v10;
            v21 = (v45 & SchedulerAssist[5]) == 0;
            v11 = (unsigned int)v45 & SchedulerAssist[5];
            SchedulerAssist[5] = v11;
            if ( v21 )
              KiRemoveSystemWorkPriorityKick((__int64)v44);
          }
        }
        else
        {
          v43 = v46[0];
        }
        __writecr8(v43);
        v13 += 48LL;
      }
      while ( v13 < v33 );
    }
  }
}
