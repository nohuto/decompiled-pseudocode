/*
 * XREFs of IopLoadDriver @ 0x140737DD0
 * Callers:
 *     PipCallDriverAddDeviceQueryRoutine @ 0x140736D64 (PipCallDriverAddDeviceQueryRoutine.c)
 *     IopLoadUnloadDriver @ 0x14077CA40 (IopLoadUnloadDriver.c)
 *     IopInitializeSystemDrivers @ 0x140A5CBE8 (IopInitializeSystemDrivers.c)
 * Callees:
 *     IopVerifierExAllocatePool @ 0x14020AC94 (IopVerifierExAllocatePool.c)
 *     RtlInitUnicodeString @ 0x140210D50 (RtlInitUnicodeString.c)
 *     HalPutDmaAdapter @ 0x1402211F0 (HalPutDmaAdapter.c)
 *     ExReleaseResourceLite @ 0x1402214C0 (ExReleaseResourceLite.c)
 *     ExAcquireResourceExclusiveLite @ 0x140221C70 (ExAcquireResourceExclusiveLite.c)
 *     RtlAppendUnicodeToString @ 0x140267DE0 (RtlAppendUnicodeToString.c)
 *     RtlImageNtHeader @ 0x140297240 (RtlImageNtHeader.c)
 *     DbgPrint @ 0x1403647B0 (DbgPrint.c)
 *     PnpDiagnosticTraceObject @ 0x140364A08 (PnpDiagnosticTraceObject.c)
 *     PnpDiagnosticTraceObjectWithStatus @ 0x140364B84 (PnpDiagnosticTraceObjectWithStatus.c)
 *     HeadlessKernelAddLogEntry @ 0x14036D9A8 (HeadlessKernelAddLogEntry.c)
 *     PnpDiagnosticTraceDriverFullInfo @ 0x14037C050 (PnpDiagnosticTraceDriverFullInfo.c)
 *     VfDifCaptureIoCallbacks @ 0x14037DBB0 (VfDifCaptureIoCallbacks.c)
 *     VfDifCaptureDriverEntry @ 0x14037DBD4 (VfDifCaptureDriverEntry.c)
 *     IopResurrectDriver @ 0x1403A6970 (IopResurrectDriver.c)
 *     __security_check_cookie @ 0x1403CFAF0 (__security_check_cookie.c)
 *     ZwClose @ 0x1403F8350 (ZwClose.c)
 *     KeBugCheckEx @ 0x1403FBCA0 (KeBugCheckEx.c)
 *     memmove @ 0x140411040 (memmove.c)
 *     memset @ 0x140411300 (memset.c)
 *     ObMakeTemporaryObject @ 0x1405D6DB0 (ObMakeTemporaryObject.c)
 *     ObOpenObjectByName @ 0x14060CA70 (ObOpenObjectByName.c)
 *     ObReferenceObjectByHandle @ 0x1406118C0 (ObReferenceObjectByHandle.c)
 *     NtQueryValueKey @ 0x140618560 (NtQueryValueKey.c)
 *     NtQueryKey @ 0x140618FF0 (NtQueryKey.c)
 *     ObCloseHandle @ 0x140627D70 (ObCloseHandle.c)
 *     NtQueryObject @ 0x140634BE0 (NtQueryObject.c)
 *     MmReleaseLoadLock @ 0x1406592C0 (MmReleaseLoadLock.c)
 *     MmAcquireLoadLock @ 0x140659320 (MmAcquireLoadLock.c)
 *     ObInsertObject @ 0x1407096B0 (ObInsertObject.c)
 *     ObCreateObject @ 0x14070ACF0 (ObCreateObject.c)
 *     IopGetDriverNameFromKeyNode @ 0x14073714C (IopGetDriverNameFromKeyNode.c)
 *     IopGetRegistryValue @ 0x1407372E4 (IopGetRegistryValue.c)
 *     IopBuildFullDriverPath @ 0x140738824 (IopBuildFullDriverPath.c)
 *     PnpPrepareDriverLoading @ 0x14073B76C (PnpPrepareDriverLoading.c)
 *     KseShimDriverIoCallbacks @ 0x140751754 (KseShimDriverIoCallbacks.c)
 *     MmLoadSystemImage @ 0x140754BE0 (MmLoadSystemImage.c)
 *     MiFreeDriverInitialization @ 0x140757CF4 (MiFreeDriverInitialization.c)
 *     PnpCallDriverEntry @ 0x14076C0A8 (PnpCallDriverEntry.c)
 *     IopReadyDeviceObjects @ 0x14076D23C (IopReadyDeviceObjects.c)
 *     IopBootLog @ 0x14076D764 (IopBootLog.c)
 *     MmUnloadSystemImage @ 0x14076DF90 (MmUnloadSystemImage.c)
 *     EtwTiLogDriverObjectLoad @ 0x14076E000 (EtwTiLogDriverObjectLoad.c)
 *     PnpDriverLoadingFailed @ 0x1407CE860 (PnpDriverLoadingFailed.c)
 *     IopCheckIfNotNativeDriver @ 0x140893D20 (IopCheckIfNotNativeDriver.c)
 *     IopLogBlockedDriverEvent @ 0x140894564 (IopLogBlockedDriverEvent.c)
 *     IopSafebootDriverLoad @ 0x140894964 (IopSafebootDriverLoad.c)
 *     ExFreePoolWithTag @ 0x1409B70B0 (ExFreePoolWithTag.c)
 */

__int64 __fastcall IopLoadDriver(HANDLE Handle, char a2, unsigned __int8 a3, int *a4)
{
  ULONG_PTR v4; // rsi
  unsigned int v5; // ebx
  _WORD *v7; // r12
  int v8; // eax
  unsigned __int16 *v9; // rdi
  int DriverNameFromKeyNode; // r14d
  __int64 v11; // rcx
  PVOID Pool; // rax
  int SystemImage; // eax
  __int64 v14; // rax
  _QWORD *v15; // rbx
  __int64 v16; // rdx
  __int64 v17; // rax
  HANDLE v18; // rdi
  KPROCESSOR_MODE PreviousMode; // r9
  NTSTATUS v20; // eax
  char *v21; // r15
  PVOID v22; // rax
  PVOID v23; // rdi
  unsigned int v24; // ebx
  __int64 v25; // rax
  __int64 v26; // rdx
  unsigned int i; // eax
  __int64 v28; // rdi
  struct _KTHREAD *Lock; // rbx
  int ValueKey; // eax
  __int64 v32; // rdx
  ULONG_PTR v33; // r9
  PVOID *NewObject; // [rsp+20h] [rbp-E0h]
  UNICODE_STRING Destination; // [rsp+58h] [rbp-A8h] BYREF
  __int64 v37; // [rsp+68h] [rbp-98h] BYREF
  __int64 v38; // [rsp+70h] [rbp-90h] BYREF
  PVOID v39; // [rsp+78h] [rbp-88h]
  char v40[4]; // [rsp+80h] [rbp-80h]
  UNICODE_STRING v41; // [rsp+88h] [rbp-78h] BYREF
  __int64 v42; // [rsp+98h] [rbp-68h] BYREF
  int v43; // [rsp+A0h] [rbp-60h] BYREF
  HANDLE v44; // [rsp+A8h] [rbp-58h] BYREF
  ULONG_PTR BugCheckParameter2; // [rsp+B0h] [rbp-50h] BYREF
  int v46; // [rsp+B8h] [rbp-48h] BYREF
  ULONG v47; // [rsp+BCh] [rbp-44h] BYREF
  PVOID v48; // [rsp+C0h] [rbp-40h] BYREF
  PVOID v49; // [rsp+C8h] [rbp-38h] BYREF
  UNICODE_STRING DestinationString; // [rsp+D0h] [rbp-30h] BYREF
  PVOID Object; // [rsp+E0h] [rbp-20h] BYREF
  PVOID v52; // [rsp+E8h] [rbp-18h] BYREF
  PVOID P; // [rsp+F0h] [rbp-10h]
  int *v54; // [rsp+F8h] [rbp-8h]
  _DWORD v55[2]; // [rsp+100h] [rbp+0h] BYREF
  __int64 v56; // [rsp+108h] [rbp+8h]
  UNICODE_STRING *v57; // [rsp+110h] [rbp+10h]
  int v58; // [rsp+118h] [rbp+18h]
  int v59; // [rsp+11Ch] [rbp+1Ch]
  __int128 v60; // [rsp+120h] [rbp+20h]
  _WORD v61[40]; // [rsp+130h] [rbp+30h] BYREF

  v4 = 0LL;
  v54 = a4;
  *a4 = 0;
  v5 = a3;
  P = 0LL;
  LODWORD(v37) = 0;
  *(_QWORD *)&Destination.Length = 0LL;
  v7 = 0LL;
  v38 = 0LL;
  v39 = 0LL;
  v55[1] = 0;
  v59 = 0;
  BugCheckParameter2 = 0LL;
  *(_QWORD *)&v41.Length = 0LL;
  Object = 0LL;
  v42 = 0LL;
  *(_DWORD *)v40 = 0;
  v44 = 0LL;
  v41.Buffer = 0LL;
  Destination.Buffer = 0LL;
  v43 = 0;
  v8 = NtQueryKey(Handle, 0, 0LL, 0, &v37);
  if ( v8 != -1073741789 && v8 != -2147483643 )
  {
    DriverNameFromKeyNode = -1073741472;
    goto LABEL_35;
  }
  if ( (int)v37 + 8 < (unsigned int)v37 )
  {
    DriverNameFromKeyNode = -1073741675;
    goto LABEL_35;
  }
  P = IopVerifierExAllocatePool(NonPagedPoolNx, (unsigned int)(v37 + 8));
  v9 = (unsigned __int16 *)P;
  if ( !P )
  {
    DriverNameFromKeyNode = -1073741670;
    goto LABEL_35;
  }
  DriverNameFromKeyNode = NtQueryKey(Handle, 0, (unsigned __int64)P, v37, &v37);
  if ( DriverNameFromKeyNode >= 0 )
  {
    v11 = v9[6];
    Destination.Length = v11;
    Destination.MaximumLength = v11 + 8;
    Destination.Buffer = v9 + 8;
    Pool = IopVerifierExAllocatePool(PagedPool, v11 + 2);
    v39 = Pool;
    v7 = Pool;
    if ( Pool )
    {
      LOWORD(v38) = Destination.Length;
      WORD1(v38) = Destination.Length + 2;
      memmove(Pool, Destination.Buffer, Destination.Length);
      v7[(unsigned __int64)Destination.Length >> 1] = 0;
      RtlAppendUnicodeToString(&Destination, L".SYS");
      HeadlessKernelAddLogEntry();
      PnpDiagnosticTraceObject(&KMPnPEvt_DriverLoad_Start, (unsigned __int16 *)&v38);
      if ( (_DWORD)InitSafeBootMode )
      {
        if ( a2 )
        {
          v46 = 0;
          DestinationString = 0LL;
          RtlInitUnicodeString(&DestinationString, L"Group");
          memset(v61, 0, 0x4CuLL);
          LODWORD(NewObject) = 76;
          ValueKey = NtQueryValueKey(
                       Handle,
                       (unsigned __int64)&DestinationString,
                       2u,
                       (char *)v61,
                       (size_t)NewObject,
                       (unsigned __int64)&v46);
          if ( ValueKey < 0
            || (DestinationString.Length = v61[4] - 2,
                DestinationString.MaximumLength = v61[4] - 2,
                DestinationString.Buffer = &v61[6],
                !(unsigned __int8)IopSafebootDriverLoad(&DestinationString)) )
          {
            if ( !(unsigned __int8)IopSafebootDriverLoad(&Destination) )
            {
              IopBootLog(&Destination);
              DbgPrint("SAFEBOOT: skipping device = %wZ(%wZ)\n", &Destination, &DestinationString);
              HeadlessKernelAddLogEntry();
              ObCloseHandle(Handle, 0);
              return 3221226335LL;
            }
          }
        }
      }
      DriverNameFromKeyNode = IopBuildFullDriverPath(&v38, Handle, &Destination);
      if ( DriverNameFromKeyNode >= 0 )
      {
        DriverNameFromKeyNode = IopGetDriverNameFromKeyNode(Handle, &v41);
        if ( DriverNameFromKeyNode >= 0 )
        {
          v55[0] = 48;
          v56 = 0LL;
          v57 = &v41;
          v58 = IopCaseInsensitive != 0 ? 592 : 528;
          v60 = 0LL;
          ExAcquireResourceExclusiveLite(&IopDriverLoadResource, 1u);
          SystemImage = MmLoadSystemImage(
                          (unsigned int)&Destination,
                          0,
                          0,
                          0,
                          (__int64)&BugCheckParameter2,
                          (__int64)&v42);
          DriverNameFromKeyNode = SystemImage;
          if ( SystemImage < 0 )
          {
            if ( SystemImage == -1073741554 )
              goto LABEL_53;
            if ( (unsigned __int8)IopCheckIfNotNativeDriver((unsigned int)SystemImage, &Destination) == 1 )
            {
              DriverNameFromKeyNode = ((_BYTE)v5 != 0) - 1073740949;
              IopLogBlockedDriverEvent(&Destination, v32, (unsigned int)DriverNameFromKeyNode);
            }
            if ( DriverNameFromKeyNode == -1073741554 )
            {
LABEL_53:
              DriverNameFromKeyNode = ObOpenObjectByName(
                                        (__int64)v55,
                                        (__int64)IoDriverObjectType,
                                        0,
                                        0LL,
                                        0,
                                        0LL,
                                        (__int64)&v44);
              if ( DriverNameFromKeyNode < 0 )
              {
                ExReleaseResourceLite(&IopDriverLoadResource);
                IopBootLog(&Destination);
                if ( DriverNameFromKeyNode == -1073741772 )
                  DriverNameFromKeyNode = -1073740914;
                goto LABEL_35;
              }
              v48 = 0LL;
              DriverNameFromKeyNode = ObReferenceObjectByHandle(v44, 0, IoDriverObjectType, 0, &v48, 0LL);
              ZwClose(v44);
              if ( DriverNameFromKeyNode >= 0 )
              {
                DriverNameFromKeyNode = IopResurrectDriver((__int64)v48);
                HalPutDmaAdapter((PADAPTER_OBJECT)v48);
              }
            }
            ExReleaseResourceLite(&IopDriverLoadResource);
            IopBootLog(&Destination);
          }
          else
          {
            v14 = RtlImageNtHeader(v42);
            *(_WORD *)v40 = *(_WORD *)(v14 + 70);
            *(_WORD *)&v40[2] = *(_WORD *)(v14 + 68);
            DriverNameFromKeyNode = PnpPrepareDriverLoading(&v38, Handle, v42, v5, &v43);
            if ( DriverNameFromKeyNode < 0
              || (DriverNameFromKeyNode = ObCreateObject(
                                            KeGetCurrentThread()->$6A88714AB977AAA8032D9F5E2A96BA31::gap0[10],
                                            IoDriverObjectType,
                                            (int)v55,
                                            0,
                                            0,
                                            416,
                                            0,
                                            0,
                                            &Object),
                  DriverNameFromKeyNode < 0) )
            {
              MmUnloadSystemImage(BugCheckParameter2);
              ExReleaseResourceLite(&IopDriverLoadResource);
            }
            else
            {
              v15 = Object;
              memset(Object, 0, 0x1A0uLL);
              v15[6] = v15 + 42;
              v15[42] = v15;
              v47 = 28;
              memset64(v15 + 14, (unsigned __int64)IopInvalidDeviceRequest, 0x1CuLL);
              *(_DWORD *)v15 = 22020100;
              v16 = RtlImageNtHeader(v42);
              *(_WORD *)v40 = *(_WORD *)(v16 + 70);
              *(_WORD *)&v40[2] = *(_WORD *)(v16 + 68);
              v17 = v42 + *(unsigned int *)(v16 + 40);
              if ( (*(_WORD *)(v16 + 94) & 0x2000) == 0 )
                *((_DWORD *)v15 + 4) |= 2u;
              v15[11] = v17;
              v15[5] = BugCheckParameter2;
              v15[3] = v42;
              *((_DWORD *)v15 + 8) = *(_DWORD *)(v16 + 80);
              DriverNameFromKeyNode = ObInsertObject(v15, 0LL, 1u, 0, 0LL, &v44);
              ExReleaseResourceLite(&IopDriverLoadResource);
              if ( DriverNameFromKeyNode >= 0 )
              {
                v18 = v44;
                PreviousMode = KeGetCurrentThread()->PreviousMode;
                v49 = 0LL;
                v20 = ObReferenceObjectByHandle(v44, 0, IoDriverObjectType, PreviousMode, &v49, 0LL);
                if ( v20 )
                  KeBugCheckEx(0x11Fu, (ULONG_PTR)v18, v20, (ULONG_PTR)v49, 0LL);
                ZwClose(v18);
                v21 = (char *)v49;
                *((_QWORD *)v49 + 9) = &CmRegistryMachineHardwareDescriptionSystemName;
                v22 = IopVerifierExAllocatePool(NonPagedPoolNx, v41.MaximumLength);
                *((_QWORD *)v21 + 8) = v22;
                if ( v22 )
                {
                  *((_WORD *)v21 + 29) = v41.MaximumLength;
                  *((_WORD *)v21 + 28) = v41.Length;
                  memmove(*((void **)v21 + 8), v41.Buffer, v41.MaximumLength);
                }
                v23 = IopVerifierExAllocatePool(NonPagedPoolNx, 0x1000uLL);
                if ( v23 )
                {
                  DriverNameFromKeyNode = NtQueryObject(Handle, ObjectNameInformation, v23, 0x1000u, &v47);
                  if ( DriverNameFromKeyNode < 0 )
                  {
                    ObMakeTemporaryObject(v21);
                    HalPutDmaAdapter((PADAPTER_OBJECT)v21);
                    ExFreePoolWithTag(v23, 0);
                    v7 = v39;
                  }
                  else
                  {
                    v7 = v39;
                    if ( v39 )
                    {
                      v24 = WORD1(v38);
                      *(_QWORD *)(*((_QWORD *)v21 + 6) + 32LL) = IopVerifierExAllocatePool(NonPagedPoolNx, WORD1(v38));
                      v25 = *((_QWORD *)v21 + 6);
                      if ( *(_QWORD *)(v25 + 32) )
                      {
                        *(_WORD *)(v25 + 26) = v24;
                        *(_WORD *)(*((_QWORD *)v21 + 6) + 24LL) = v38;
                        memmove(*(void **)(*((_QWORD *)v21 + 6) + 32LL), v7, v24);
                      }
                    }
                    if ( (v43 & 1) != 0 )
                      *((_DWORD *)v21 + 4) |= 0x100u;
                    PnpDiagnosticTraceObject(&KMPnPEvt_DriverInit_Start, (unsigned __int16 *)v23);
                    VfDifCaptureDriverEntry((__int64)v21);
                    DriverNameFromKeyNode = PnpCallDriverEntry(v21, v23);
                    if ( DriverNameFromKeyNode >= 0 )
                    {
                      VfDifCaptureIoCallbacks((__int64)v21);
                      KseShimDriverIoCallbacks(v21, v26, &Destination);
                    }
                    PnpDiagnosticTraceObjectWithStatus(
                      &KMPnPEvt_DriverInit_Stop,
                      (unsigned __int16 *)v23,
                      DriverNameFromKeyNode);
                    *v54 = DriverNameFromKeyNode;
                    if ( DriverNameFromKeyNode < 0 )
                      DriverNameFromKeyNode = -1073740955;
                    for ( i = 0; i <= 0x1B; ++i )
                    {
                      if ( !*(_QWORD *)&v21[8 * i + 112] )
                        *(_QWORD *)&v21[8 * i + 112] = IopInvalidDeviceRequest;
                    }
                    ExFreePoolWithTag(v23, 0);
                    if ( DriverNameFromKeyNode < 0 )
                    {
                      ObMakeTemporaryObject(v21);
                      HalPutDmaAdapter((PADAPTER_OBJECT)v21);
                    }
                    else
                    {
                      IopBootLog(&Destination);
                      v28 = *((_QWORD *)v21 + 5);
                      Lock = MmAcquireLoadLock();
                      MiFreeDriverInitialization(v28);
                      MmReleaseLoadLock((__int64)Lock);
                      IopReadyDeviceObjects(v21);
                      EtwTiLogDriverObjectLoad(v21 + 56);
                    }
                  }
                }
                else
                {
                  ObMakeTemporaryObject(v21);
                  HalPutDmaAdapter((PADAPTER_OBJECT)v21);
                  v7 = v39;
                  DriverNameFromKeyNode = -1073741670;
                }
                goto LABEL_35;
              }
            }
            IopBootLog(&Destination);
            v7 = v39;
          }
        }
        goto LABEL_35;
      }
    }
    else
    {
      DriverNameFromKeyNode = -1073741670;
    }
    Destination.Buffer = 0LL;
  }
LABEL_35:
  HeadlessKernelAddLogEntry();
  if ( (int)(DriverNameFromKeyNode + 0x80000000) >= 0 && DriverNameFromKeyNode != -1073741218 )
  {
    v52 = 0LL;
    if ( DriverNameFromKeyNode != -1073741554 )
    {
      PnpDriverLoadingFailed(Handle);
      if ( DriverNameFromKeyNode != -1073740955 && IopGetRegistryValue(Handle, L"ErrorControl", 0, &v52) >= 0 )
      {
        if ( *((_DWORD *)v52 + 3)
          && (_BYTE)CmFirstTime == 1
          && (unsigned int)(*(_DWORD *)((char *)v52 + *((unsigned int *)v52 + 2)) - 2) <= 1 )
        {
          if ( ((unsigned __int64)&Destination & -(__int64)(Destination.Buffer != 0LL)) != 0 )
            v33 = *(_QWORD *)(((unsigned __int64)&Destination & -(__int64)(Destination.Buffer != 0LL)) + 8);
          else
            v33 = 0LL;
          if ( ((unsigned __int64)&v41 & -(__int64)(v41.Buffer != 0LL)) != 0 )
            v4 = *(_QWORD *)(((unsigned __int64)&v41 & -(__int64)(v41.Buffer != 0LL)) + 8);
          KeBugCheckEx(0x5Au, 1uLL, v4, v33, DriverNameFromKeyNode);
        }
        ExFreePoolWithTag(v52, 0);
      }
    }
  }
  if ( P )
    ExFreePoolWithTag(P, 0);
  if ( v7 )
  {
    PnpDiagnosticTraceDriverFullInfo(
      &KMPnPEvt_DriverLoad_Stop,
      (unsigned __int16 *)&v38,
      DriverNameFromKeyNode,
      (__int64)&v41,
      v40[0]);
    ExFreePoolWithTag(v7, 0);
  }
  if ( v41.Buffer )
    ExFreePoolWithTag(v41.Buffer, 0);
  if ( Destination.Buffer )
    ExFreePoolWithTag(Destination.Buffer, 0);
  ObCloseHandle(Handle, 0);
  return (unsigned int)DriverNameFromKeyNode;
}
