/*
 * XREFs of MiInitializeEnclaveMetadataPage @ 0x140A958EC
 * Callers:
 *     MiCreateEnclaveRegions @ 0x140A538D0 (MiCreateEnclaveRegions.c)
 * Callees:
 *     MiPteInShadowRange @ 0x14021EA50 (MiPteInShadowRange.c)
 *     MiAllocatePool @ 0x14022D320 (MiAllocatePool.c)
 *     MiReservePtes @ 0x140285DC0 (MiReservePtes.c)
 *     MiReleasePtes @ 0x140294430 (MiReleasePtes.c)
 *     MiMakeValidPte @ 0x1402B1B40 (MiMakeValidPte.c)
 *     MiWritePteShadow @ 0x140313F2C (MiWritePteShadow.c)
 *     MiPteHasShadow @ 0x140313F8C (MiPteHasShadow.c)
 *     KeCreateEnclaveMetadataPage @ 0x1405137AC (KeCreateEnclaveMetadataPage.c)
 *     MiGetEnclavePage @ 0x140548BD8 (MiGetEnclavePage.c)
 *     MiReturnEnclavePage @ 0x140549920 (MiReturnEnclavePage.c)
 *     ExFreePoolWithTag @ 0x1409B70B0 (ExFreePoolWithTag.c)
 */

__int64 MiInitializeEnclaveMetadataPage()
{
  ULONG_PTR v0; // rdi
  _DWORD *Pool; // rax
  _QWORD *v2; // rsi
  _DWORD *v3; // rax
  __int64 EnclavePage; // rax
  __int64 v5; // r9
  ULONG_PTR v6; // rbp
  unsigned __int64 ValidPte; // rbx
  int v8; // r14d
  __int64 v9; // r8
  bool v10; // zf
  __int64 result; // rax
  __int64 v12; // rdx
  __int64 v13; // r8
  _DWORD *v14; // r9

  v0 = MiReservePtes((__int64)&qword_140C4EDC0, 1u);
  if ( !v0 )
    return 0LL;
  Pool = MiAllocatePool(64, 0x50uLL, 0x4D424D45u);
  v2 = Pool;
  if ( Pool )
  {
    *Pool = 512;
    v3 = Pool + 4;
    v2[1] = v3;
    *v3 |= 1u;
    EnclavePage = MiGetEnclavePage(&MiSystemPartition, 0);
    v6 = EnclavePage;
    if ( EnclavePage == -1 )
    {
LABEL_16:
      ExFreePoolWithTag(v2, 0);
      if ( v6 != -1LL )
        MiReturnEnclavePage(v6, v12, v13, v14);
      goto LABEL_18;
    }
    ValidPte = MiMakeValidPte(v0, EnclavePage, 3221225476LL, v5);
    v8 = 0;
    if ( MiPteInShadowRange(v0) )
    {
      if ( (unsigned int)MiPteHasShadow() )
      {
        v8 = 1;
        if ( HIBYTE(word_140C4DE88) )
          goto LABEL_12;
        v10 = (ValidPte & 1) == 0;
      }
      else
      {
        if ( (HIDWORD(KeGetCurrentThread()->ApcState.Process[2].Header.WaitListHead.Flink) & 0x1000) == 0 )
          goto LABEL_12;
        v10 = (ValidPte & 1) == 0;
      }
      if ( !v10 )
        ValidPte |= 0x8000000000000000uLL;
    }
LABEL_12:
    *(_QWORD *)v0 = ValidPte;
    if ( v8 )
      MiWritePteShadow(v0, ValidPte, v9);
    if ( (int)KeCreateEnclaveMetadataPage() >= 0 )
    {
      qword_140C4ED18 = 0LL;
      result = 1LL;
      qword_140C4ED08 = (__int64)(v0 << 25) >> 16;
      qword_140C4ED10 = (__int64)v2;
      dword_140C4ED20 = 0;
      return result;
    }
    goto LABEL_16;
  }
LABEL_18:
  MiReleasePtes((__int64)&qword_140C4EDC0, (_QWORD *)v0, 1u);
  return 0LL;
}
