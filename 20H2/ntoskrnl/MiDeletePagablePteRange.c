/*
 * XREFs of MiDeletePagablePteRange @ 0x1402BA840
 * Callers:
 *     MiDeleteVirtualAddresses @ 0x1402508A0 (MiDeleteVirtualAddresses.c)
 *     MiDeleteVad @ 0x14027E4D0 (MiDeleteVad.c)
 *     MiDeleteSessionAddressSpace @ 0x140388E8C (MiDeleteSessionAddressSpace.c)
 *     MiDeleteSessionPdes @ 0x14038C378 (MiDeleteSessionPdes.c)
 *     MiDeleteVadBitmap @ 0x1406602C0 (MiDeleteVadBitmap.c)
 * Callees:
 *     KiLeaveGuardedRegionUnsafe @ 0x140220E40 (KiLeaveGuardedRegionUnsafe.c)
 *     KeReleaseInStackQueuedSpinLockFromDpcLevel @ 0x1402237F0 (KeReleaseInStackQueuedSpinLockFromDpcLevel.c)
 *     KxAcquireQueuedSpinLock @ 0x140226AC0 (KxAcquireQueuedSpinLock.c)
 *     MiWalkPageTables @ 0x14026A5E0 (MiWalkPageTables.c)
 *     MiFastLockLeafPageTable @ 0x14026F9F0 (MiFastLockLeafPageTable.c)
 *     MiCheckProcessShadow @ 0x140270BA0 (MiCheckProcessShadow.c)
 *     MiGetSharedVm @ 0x14027C730 (MiGetSharedVm.c)
 *     MiUnlockWorkingSetExclusive @ 0x14027F090 (MiUnlockWorkingSetExclusive.c)
 *     MiTbFlushType @ 0x1402BA458 (MiTbFlushType.c)
 *     MiDeleteVa @ 0x1402BAF60 (MiDeleteVa.c)
 *     MiDeleteVaTail @ 0x1402BF6B0 (MiDeleteVaTail.c)
 *     MiGetUsedPtesHandle @ 0x1402CC840 (MiGetUsedPtesHandle.c)
 *     ExAcquireSpinLockShared @ 0x1402D2DC0 (ExAcquireSpinLockShared.c)
 *     ExAcquireSpinLockExclusive @ 0x1402D4460 (ExAcquireSpinLockExclusive.c)
 *     ExReleaseAutoExpandPushLockExclusive @ 0x1403052B0 (ExReleaseAutoExpandPushLockExclusive.c)
 *     ExAcquireAutoExpandPushLockExclusive @ 0x1403060F0 (ExAcquireAutoExpandPushLockExclusive.c)
 *     MiPreUnlockWorkingSetShared @ 0x14031ACE0 (MiPreUnlockWorkingSetShared.c)
 *     MiUnlockPageTableInternal @ 0x14035B910 (MiUnlockPageTableInternal.c)
 *     __security_check_cookie @ 0x1403CFAF0 (__security_check_cookie.c)
 *     KiRemoveSystemWorkPriorityKick @ 0x1403F1DD4 (KiRemoveSystemWorkPriorityKick.c)
 *     MiFreePhysicalPageChain @ 0x14054A0E8 (MiFreePhysicalPageChain.c)
 *     MiFreeLargePages @ 0x14055C9C8 (MiFreeLargePages.c)
 *     ExpReleaseSpinLockSharedFromDpcLevelInstrumented @ 0x1405B43A4 (ExpReleaseSpinLockSharedFromDpcLevelInstrumented.c)
 */

void __fastcall MiDeletePagablePteRange(
        __int64 a1,
        unsigned __int8 a2,
        unsigned __int64 a3,
        unsigned __int64 a4,
        char a5,
        int a6,
        _QWORD *a7)
{
  unsigned __int64 v7; // rbx
  int v8; // r12d
  unsigned __int64 v10; // r15
  unsigned __int8 v11; // al
  unsigned __int8 CurrentIrql; // si
  char v13; // di
  int v14; // edx
  int v15; // r8d
  __int64 v16; // r9
  int v17; // r10d
  int v18; // r11d
  char v19; // al
  __int16 v20; // ax
  char v21; // cl
  char v22; // r12
  __int64 v23; // rcx
  struct _KTHREAD *CurrentThread; // rax
  __int16 v25; // ax
  unsigned __int8 v26; // al
  _DWORD *SchedulerAssist; // r9
  LONG *v28; // rbx
  KIRQL v29; // al
  LONG *SharedVm; // rbx
  LONG *v31; // rax
  int v32; // eax
  unsigned __int64 i; // rdi
  __int64 v34; // rax
  __int64 v35; // rdx
  bool v36; // zf
  char v37; // al
  int v38; // eax
  struct _KPRCB *CurrentPrcb; // rcx
  _DWORD *v40; // rdx
  int v41; // eax
  unsigned __int8 v42; // al
  struct _KPRCB *v43; // r10
  _DWORD *v44; // r9
  int v45; // eax
  __int64 v46; // rcx
  __int64 v47; // rcx
  int v49; // [rsp+24h] [rbp-DCh]
  __int64 v51; // [rsp+48h] [rbp-B8h] BYREF
  unsigned __int64 v52; // [rsp+50h] [rbp-B0h]
  struct _KLOCK_QUEUE_HANDLE LockHandle; // [rsp+60h] [rbp-A0h] BYREF
  unsigned __int64 v54; // [rsp+78h] [rbp-88h]
  unsigned __int64 v55; // [rsp+80h] [rbp-80h]
  unsigned __int64 v56; // [rsp+88h] [rbp-78h]
  __int64 v57; // [rsp+90h] [rbp-70h]
  __int128 v58; // [rsp+A0h] [rbp-60h] BYREF
  int *v59; // [rsp+B0h] [rbp-50h]
  __int64 v60; // [rsp+B8h] [rbp-48h]
  unsigned __int64 v61; // [rsp+C0h] [rbp-40h]
  __int64 v62; // [rsp+C8h] [rbp-38h]
  __int128 v63; // [rsp+D0h] [rbp-30h]
  __int128 v64; // [rsp+E0h] [rbp-20h]
  __int128 v65; // [rsp+F0h] [rbp-10h]
  __int128 v66; // [rsp+100h] [rbp+0h]
  __int128 v67; // [rsp+110h] [rbp+10h]
  __int128 v68; // [rsp+120h] [rbp+20h]
  __int64 v69; // [rsp+130h] [rbp+30h]
  __int64 (__fastcall *v70)(_QWORD, _QWORD, _QWORD); // [rsp+138h] [rbp+38h]
  __int64 (__fastcall *v71)(_QWORD); // [rsp+140h] [rbp+40h]
  _OWORD *v72; // [rsp+148h] [rbp+48h]
  _OWORD v73[6]; // [rsp+150h] [rbp+50h] BYREF
  __int128 v74; // [rsp+1B0h] [rbp+B0h]
  __int128 v75; // [rsp+1C0h] [rbp+C0h]
  __int128 v76; // [rsp+1D0h] [rbp+D0h]
  __int64 v77; // [rsp+1E0h] [rbp+E0h]
  int v78; // [rsp+1F0h] [rbp+F0h] BYREF
  __int16 v79; // [rsp+1F4h] [rbp+F4h]
  __int16 v80; // [rsp+1F6h] [rbp+F6h]
  __int64 v81; // [rsp+1F8h] [rbp+F8h]
  __int64 v82; // [rsp+200h] [rbp+100h]
  __int64 v83; // [rsp+208h] [rbp+108h]
  __int128 v84; // [rsp+210h] [rbp+110h]
  __int128 v85; // [rsp+220h] [rbp+120h]
  __int128 v86; // [rsp+230h] [rbp+130h]
  __int128 v87; // [rsp+240h] [rbp+140h]
  __int128 v88; // [rsp+250h] [rbp+150h]
  __int128 v89; // [rsp+260h] [rbp+160h]
  __int128 v90; // [rsp+270h] [rbp+170h]
  __int128 v91; // [rsp+280h] [rbp+180h]
  __int128 v92; // [rsp+290h] [rbp+190h]
  __int64 v93; // [rsp+2A0h] [rbp+1A0h]
  void *retaddr; // [rsp+308h] [rbp+208h]
  char v95; // [rsp+330h] [rbp+230h]

  v8 = a6;
  v52 = a3;
  v80 = 0;
  v10 = a3;
  v93 = 0LL;
  v11 = a2;
  v95 = 4 * (a5 & 7);
  LOBYTE(v7) = 0;
  v84 = 0LL;
  v85 = 0LL;
  v86 = 0LL;
  v87 = 0LL;
  v88 = 0LL;
  v89 = 0LL;
  v90 = 0LL;
  v91 = 0LL;
  v92 = 0LL;
  memset(&LockHandle, 0, sizeof(LockHandle));
  while ( 1 )
  {
    v49 = v7;
    CurrentIrql = v11;
    v77 = 2LL;
    v13 = 0;
    v69 = 0LL;
    v74 = 0LL;
    v73[1] = (unsigned __int64)a7;
    v73[0] = 0LL;
    DWORD2(v74) = v8;
    memset(&v73[2], 0, 64);
    v75 = 0LL;
    v76 = 0LL;
    v58 = 0LL;
    v63 = 0LL;
    v64 = 0LL;
    v65 = 0LL;
    v66 = 0LL;
    v67 = 0LL;
    v68 = 0LL;
    v78 = MiTbFlushType(a1);
    v59 = &v78;
    v19 = BYTE12(v74);
    v79 = 0;
    v82 = 0LL;
    v83 = 0LL;
    if ( (BYTE4(PerfGlobalGroupMask) & 1) != 0 )
      v19 = 1;
    BYTE12(v74) = v19;
    v81 = 20LL;
    BYTE2(v58) = v95 | _mm_cvtsi128_si32(_mm_srli_si128((__m128i)0LL, 2)) & 0xE3;
    v72 = v73;
    v20 = 39;
    LOWORD(v58) = 39;
    if ( v15 )
    {
      v20 = 1059;
      LOWORD(v58) = 1059;
    }
    if ( v14 )
    {
      v20 |= 0x800u;
      LOWORD(v58) = v20;
    }
    v21 = BYTE5(v58);
    v61 = v10;
    if ( v18 )
      v21 = 63;
    v62 = v16;
    BYTE5(v58) = v21;
    v22 = 0;
    v60 = a1;
    v70 = MiDeleteVa;
    v71 = MiDeleteVaTail;
    if ( v17 )
    {
      v23 = *(_QWORD *)(a7[4] + 32LL);
      CurrentThread = KeGetCurrentThread();
      --CurrentThread->SpecialApcDisable;
      ExAcquireAutoExpandPushLockExclusive(v23 + 56, 0LL);
      v20 = v58;
    }
    if ( a2 != 17 )
    {
      v22 = 1;
      v25 = v20 & 0xFFFD;
      LOWORD(v58) = v25;
      goto LABEL_27;
    }
    if ( (v58 & 4) != 0 )
    {
      v26 = *(_BYTE *)(a1 + 184) & 7;
      if ( v26 >= 6u )
      {
        CurrentIrql = KeGetCurrentIrql();
        __writecr8(2uLL);
        if ( KiIrqlFlags && (KiIrqlFlags & 1) != 0 && CurrentIrql <= 0xFu )
        {
          SchedulerAssist = KeGetCurrentPrcb()->SchedulerAssist;
          SchedulerAssist[5] |= (-1 << (CurrentIrql + 1)) & 4;
        }
        goto LABEL_26;
      }
      v28 = &dword_140C4F600;
      if ( v26 != 2 )
        v28 = (LONG *)(a1 + 192);
      v29 = ExAcquireSpinLockShared(v28);
      if ( v28[1] )
        _InterlockedExchange(v28 + 1, 0);
    }
    else
    {
      SharedVm = MiGetSharedVm(a1);
      v29 = ExAcquireSpinLockExclusive(SharedVm);
      SharedVm[1] = 0;
    }
    LODWORD(v7) = v49;
    CurrentIrql = v29;
LABEL_26:
    v25 = v58;
LABEL_27:
    BYTE6(v58) = CurrentIrql;
    if ( (a6 & 0x100) != 0 )
    {
      if ( (a6 & 0x200) == 0 )
        v25 = v25 & 0xEFFB | 0x1000;
      LOWORD(v58) = v25 & 0xFFFD;
      v31 = MiGetSharedVm(a1);
      LockHandle.LockQueue.Next = 0LL;
      LockHandle.LockQueue.Lock = (unsigned __int64 *volatile)(v31 + 16);
      KxAcquireQueuedSpinLock((__int64)&LockHandle, (volatile __int64 *)v31 + 8);
    }
    if ( (a6 & 0x80) == 0 )
      goto LABEL_42;
    if ( (_BYTE)v7 )
      goto LABEL_42;
    if ( ((a4 ^ v10) & 0xFFFFFFE00000LL) != 0 )
      goto LABEL_42;
    v32 = MiFastLockLeafPageTable(a1, v10, 0);
    if ( !v32 )
      goto LABEL_42;
    v7 = ((v10 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL;
    v54 = v7;
    v55 = ((v7 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL;
    v56 = ((v55 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL;
    v57 = ((v56 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL;
    if ( v32 == 1 )
    {
      for ( i = ((a4 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL; v7 <= i; v7 += 8 * v34 )
      {
        MiDeleteVa(&v58, v7, 0LL);
        v34 = (unsigned int)BYTE3(v58) + 1;
        BYTE3(v58) = 0;
      }
      MiDeleteVaTail(&v58);
      v36 = (*(_DWORD *)(MiGetUsedPtesHandle(v52, v35) + 16) & 0x3FF0000) == 0;
      v37 = v49;
      if ( v36 )
        v37 = 1;
      v13 = 1;
      LOBYTE(v49) = v37;
    }
    MiUnlockPageTableInternal(a1);
    if ( !v13 )
LABEL_42:
      MiWalkPageTables((__int64)&v58);
    if ( (a6 & 0x100) != 0 )
    {
      KeReleaseInStackQueuedSpinLockFromDpcLevel(&LockHandle);
      if ( (a6 & 0x200) == 0 )
        LOWORD(v58) = v58 | 4;
    }
    if ( !v22 )
    {
      if ( (v58 & 4) != 0 )
      {
        if ( (*(_BYTE *)(a1 + 184) & 7u) < 6 )
        {
          v38 = *(_DWORD *)(a1 + 184);
          if ( (v38 & 0x8000000) != 0 || (v38 & 0x4000000) != 0 || (v38 & 0x10000000) != 0 )
          {
            MiPreUnlockWorkingSetShared(a1, CurrentIrql);
            LOBYTE(v38) = *(_BYTE *)(a1 + 184);
          }
          v7 = (unsigned __int64)&dword_140C4F600;
          if ( (v38 & 7) != 2 )
            v7 = a1 + 192;
          MiCheckProcessShadow(a1, 1u);
          if ( (BYTE6(PerfGlobalGroupMask) & 1) != 0 )
          {
            ExpReleaseSpinLockSharedFromDpcLevelInstrumented(v7, retaddr);
          }
          else
          {
            _InterlockedAnd((volatile signed __int32 *)v7, 0xBFFFFFFF);
            _InterlockedDecrement((volatile signed __int32 *)v7);
          }
          CurrentPrcb = KeGetCurrentPrcb();
          v40 = CurrentPrcb->SchedulerAssist;
          if ( v40 )
          {
            if ( CurrentPrcb->NestingLevel <= 1u )
            {
              v41 = v40[6] - 1;
              v40[6] = v41;
              if ( !v41 )
                KiRemoveSystemWorkPriorityKick(CurrentPrcb);
            }
          }
        }
        if ( KiIrqlFlags )
        {
          if ( (KiIrqlFlags & 1) != 0 )
          {
            v42 = KeGetCurrentIrql();
            if ( v42 <= 0xFu && CurrentIrql <= 0xFu && v42 >= 2u )
            {
              v43 = KeGetCurrentPrcb();
              v44 = v43->SchedulerAssist;
              v45 = ~(unsigned __int16)(-1LL << (CurrentIrql + 1));
              v36 = (v45 & v44[5]) == 0;
              v44[5] &= v45;
              if ( v36 )
                KiRemoveSystemWorkPriorityKick(v43);
            }
          }
        }
        __writecr8(CurrentIrql);
      }
      else
      {
        MiUnlockWorkingSetExclusive(a1, CurrentIrql);
      }
    }
    v46 = a7[5];
    if ( v46 < 0 )
    {
      v51 = 0LL;
      a7[1] = MiFreeLargePages(v46, &v51);
      a7[5] = v51;
    }
    v47 = a7[4];
    if ( v47 && (_QWORD)v75 )
      MiFreePhysicalPageChain(v47, v75, 1LL);
    if ( (a6 & 0x10) != 0 )
    {
      v7 = (unsigned __int64)KeGetCurrentThread();
      ExReleaseAutoExpandPushLockExclusive(*(_QWORD *)(a7[4] + 32LL) + 56LL, 0LL);
      KiLeaveGuardedRegionUnsafe(v7);
    }
    if ( (_BYTE)v49 != 1 )
      break;
    v10 = v52;
    LOBYTE(v7) = 2;
    v11 = a2;
    v8 = a6;
  }
}
