/*
 * XREFs of PopInsertPowerRequestObject @ 0x14071CC40
 * Callers:
 *     PopCreateUserPowerRequest @ 0x14071CC80 (PopCreateUserPowerRequest.c)
 *     PopCreateKernelPowerRequest @ 0x14076E658 (PopCreateKernelPowerRequest.c)
 * Callees:
 *     <none>
 */

_QWORD *__fastcall PopInsertPowerRequestObject(_QWORD *a1, char a2)
{
  _QWORD *result; // rax

  if ( !a2 )
  {
    result = (_QWORD *)qword_140C24E98;
    if ( *(__int64 **)qword_140C24E98 == &PopPowerRequestObjectList )
    {
      ++PopPowerRequestObjectCount;
      *a1 = &PopPowerRequestObjectList;
      a1[1] = result;
      *result = a1;
      qword_140C24E98 = (__int64)a1;
      return result;
    }
FatalListEntryError_113:
    __fastfail(3u);
  }
  result = (_QWORD *)qword_140C24EA8;
  if ( *(__int64 **)qword_140C24EA8 != &PopSpecialPowerRequestObjectList )
    goto FatalListEntryError_113;
  ++PopSpecialPowerRequestObjectCount;
  *a1 = &PopSpecialPowerRequestObjectList;
  a1[1] = result;
  *result = a1;
  qword_140C24EA8 = (__int64)a1;
  return result;
}
