/*
 * XREFs of ??1?$vector@V?$unique_ptr@VCCheckMPOCache@@U?$default_delete@VCCheckMPOCache@@@std@@@std@@V?$allocator@V?$unique_ptr@VCCheckMPOCache@@U?$default_delete@VCCheckMPOCache@@@std@@@std@@@2@@std@@QEAA@XZ @ 0x1800EBA68
 * Callers:
...
void __fastcall std::vector<std::unique_ptr<CCheckMPOCache>>::~vector<std::unique_ptr<CCheckMPOCache>>(__int64 a1)
{
  char ***v1; // rdi
  ...
  if ( *(_QWORD *)a1 )
  {
    std::_Destroy_range<std::allocator<std::unique_ptr<CCheckMPOCache>>>(*(char ****)a1, *(char ****)(a1 + 8));
