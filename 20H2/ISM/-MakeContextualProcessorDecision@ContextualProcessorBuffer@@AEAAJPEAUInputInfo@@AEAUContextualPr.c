/*
 * XREFs of ?MakeContextualProcessorDecision@ContextualProcessorBuffer@@AEAAJPEAUInputInfo@@AEAUContextualProcessorResponse@@AEAUContextualProcessorMetadata@1@AEAV?$unordered_map@KV?$ComPtr@UIInputTarget@@@WRL@Microsoft@@U?$hash@K@std@@U?$equal_to@K@5@V?$allocator@U?$pair@$$CBKV?$ComPtr@UIInputTarget@@@WRL@Microsoft@@@std@@@5@@std@@AEAV?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@6@@Z @ 0x18014C404
 * Callers:
 *     ?DeliverInput@ContextualProcessorBuffer@@UEAAJPEAUInputInfo@@@Z @ 0x18014B210 (-DeliverInput@ContextualProcessorBuffer@@UEAAJPEAUInputInfo@@@Z.c)
 * Callees:
 *     ??1?$com_ptr_t@VAsyncHRESULTPrincipal@@Uerr_exception_policy@wil@@@wil@@QEAA@XZ @ 0x1800107DC (--1-$com_ptr_t@VAsyncHRESULTPrincipal@@Uerr_exception_policy@wil@@@wil@@QEAA@XZ.c)
 *     ??$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z @ 0x1800117B4 (--$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z.c)
 *     _tlgKeywordOn @ 0x1800190D0 (_tlgKeywordOn.c)
 *     ?get@?$static_lazy@VInputETW@@@details@wil@@QEAAPEAVInputETW@@P6AXXZ@Z @ 0x180029640 (-get@-$static_lazy@VInputETW@@@details@wil@@QEAAPEAVInputETW@@P6AXXZ@Z.c)
 *     ??3@YAXPEAXAEBUnothrow_t@std@@@Z @ 0x18003C740 (--3@YAXPEAXAEBUnothrow_t@std@@@Z.c)
 *     ?_Tidy@?$vector@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBEUTargetingInfo@KeyboardProcessor@@@std@@@std@@@std@@@std@@V?$allocator@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBEUTargetingInfo@KeyboardProcessor@@@std@@@std@@@std@@@std@@@2@@std@@AEAAXXZ @ 0x180045414 (-_Tidy@-$vector@V-$_List_unchecked_iterator@V-$_List_val@U-$_List_simple_types@U-$pair@$$CBEUTar.c)
 *     _guard_dispatch_icall_nop @ 0x18004A660 (_guard_dispatch_icall_nop.c)
 *     ??$_Free_non_head@V?$allocator@U?$_List_node@U?$pair@$$CBKW4ContextualProcessorState@@@std@@PEAX@std@@@std@@@?$_List_node@U?$pair@$$CBKW4ContextualProcessorState@@@std@@PEAX@std@@SAXAEAV?$allocator@U?$_List_node@U?$pair@$$CBKW4ContextualProcessorState@@@std@@PEAX@std@@@1@PEAU01@@Z @ 0x180096C74 (--$_Free_non_head@V-$allocator@U-$_List_node@U-$pair@$$CBKW4ContextualProcessorState@@@std@@PEAX.c)
 *     ??0?$VariableSizedPayloadStorage@UInputInfo@@@@QEAA@PEBUInputInfo@@@Z @ 0x1800A29E0 (--0-$VariableSizedPayloadStorage@UInputInfo@@@@QEAA@PEBUInputInfo@@@Z.c)
 *     ??0?$unordered_map@KW4ContextualProcessorState@@U?$hash@K@std@@U?$equal_to@K@3@V?$allocator@U?$pair@$$CBKW4ContextualProcessorState@@@std@@@3@@std@@QEAA@XZ @ 0x1800AE1E4 (--0-$unordered_map@KW4ContextualProcessorState@@U-$hash@K@std@@U-$equal_to@K@3@V-$allocator@U-$p.c)
 *     ??$_Insert_unverified@$$CBKU_Not_a_node_tag@std@@@?$_Hash@V?$_Uset_traits@KV?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@K@2@$0A@@std@@@std@@IEAA?AU?$pair@V?$_List_const_iterator@V?$_List_val@U?$_List_simple_types@K@std@@@std@@@std@@_N@1@$$QEBKU_Not_a_node_tag@1@@Z @ 0x1800F68D4 (--$_Insert_unverified@$$CBKU_Not_a_node_tag@std@@@-$_Hash@V-$_Uset_traits@KV-$_Uhash_compare@KU-.c)
 *     ??$?0V?$allocator@U?$_List_node@KPEAX@std@@@std@@@?$_Hash@V?$_Uset_traits@KV?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@K@2@$0A@@std@@@std@@QEAA@AEBV01@AEBV?$allocator@U?$_List_node@KPEAX@std@@@1@@Z @ 0x1801497EC (--$-0V-$allocator@U-$_List_node@KPEAX@std@@@std@@@-$_Hash@V-$_Uset_traits@KV-$_Uhash_compare@KU-.c)
 *     ??$Write@U?$_tlgWrapSz@G@@U?$_tlgWrapperByVal@$07@@U?$_tlgWrapperByVal@$03@@@?$_tlgWriteTemplate@$$A6AJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2IPEAU_EVENT_DATA_DESCRIPTOR@@@Z$1?_tlgWriteTransfer_EventWriteTransfer@@YAJ0122I3@ZPEBU2@PEBU2@@@SAJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2AEBU?$_tlgWrapSz@G@@AEBU?$_tlgWrapperByVal@$07@@AEBU?$_tlgWrapperByVal@$03@@@Z @ 0x1801499C4 (--$Write@U-$_tlgWrapSz@G@@U-$_tlgWrapperByVal@$07@@U-$_tlgWrapperByVal@$03@@@-$_tlgWriteTemplate.c)
 *     ??4?$ComPtr@UIUnknown@@@WRL@Microsoft@@QEAAAEAV012@AEBV012@@Z @ 0x18014B054 (--4-$ComPtr@UIUnknown@@@WRL@Microsoft@@QEAAAEAV012@AEBV012@@Z.c)
 *     ??R?$_Func_class@XPEAUInputInfo@@AEAV?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@std@@PEAU1@@std@@QEBAXPEAUInputInfo@@AEAV?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@1@0@Z @ 0x18014B150 (--R-$_Func_class@XPEAUInputInfo@@AEAV-$unordered_set@KU-$hash@K@std@@U-$equal_to@K@2@V-$allocato.c)
 *     ?OnInput@ContextualProcessing@InputTraceLogging@@SAXPEBUInputInfo@@PEBGW4ContextualProcessorDecision@@PEBUIInputTarget@@J@Z @ 0x18014C9F0 (-OnInput@ContextualProcessing@InputTraceLogging@@SAXPEBUInputInfo@@PEBGW4ContextualProcessorDeci.c)
 *     ?ProcessContextualProcessorDecision@ContextualProcessorBuffer@@AEAAXPEAUInputInfo@@V?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@std@@AEAUContextualProcessorResponse@@AEAUContextualProcessorMetadata@1@AEAV?$unordered_map@KV?$ComPtr@UIInputTarget@@@WRL@Microsoft@@U?$hash@K@std@@U?$equal_to@K@5@V?$allocator@U?$pair@$$CBKV?$ComPtr@UIInputTarget@@@WRL@Microsoft@@@std@@@5@@4@AEAV34@@Z @ 0x18014CB34 (-ProcessContextualProcessorDecision@ContextualProcessorBuffer@@AEAAXPEAUInputInfo@@V-$unordered_.c)
 *     ?_Copy_assign@?$_Hash@V?$_Umap_traits@KW4ContextualProcessorDecision@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKW4ContextualProcessorDecision@@@std@@@3@$0A@@std@@@std@@AEAAXAEBV12@U?$integral_constant@_N$0A@@2@@Z @ 0x18014D790 (-_Copy_assign@-$_Hash@V-$_Umap_traits@KW4ContextualProcessorDecision@@V-$_Uhash_compare@KU-$hash.c)
 *     ?clear@?$_Hash@V?$_Umap_traits@KW4ContextualProcessorDecision@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKW4ContextualProcessorDecision@@@std@@@3@$0A@@std@@@std@@QEAAXXZ @ 0x18014DA7C (-clear@-$_Hash@V-$_Umap_traits@KW4ContextualProcessorDecision@@V-$_Uhash_compare@KU-$hash@K@std@.c)
 *     ?Get@InfoMetadata@@SAPEAV1@XZ @ 0x1801A4924 (-Get@InfoMetadata@@SAPEAV1@XZ.c)
 *     ?GetInput@InfoMetadata@@QEAAPEBUInputInfoMetadata@1@W4InputType@@@Z @ 0x1801A4964 (-GetInput@InfoMetadata@@QEAAPEBUInputInfoMetadata@1@W4InputType@@@Z.c)
 */

// Hidden C++ exception states: #wind=4
__int64 __fastcall ContextualProcessorBuffer::MakeContextualProcessorDecision(
        __int64 a1,
        int *a2,
        __int64 a3,
        _QWORD *a4,
        __int64 a5,
        __int64 a6)
{
  int v9; // r12d
  __int64 v10; // rbx
  __int64 v11; // rdi
  __int64 v12; // rdx
  __int64 v13; // r8
  __int64 v14; // rcx
  _QWORD *v15; // rsi
  __int64 v16; // rcx
  unsigned __int8 *v17; // r8
  const char *v18; // r9
  __int64 **v19; // rax
  __int64 **i; // rdi
  bool v21; // zf
  int *v22; // r12
  int *v23; // rsi
  struct InfoMetadata *v24; // rax
  __int64 Input; // rax
  __int64 v26; // r8
  const char *v27; // r9
  __int64 v28; // rdi
  void *v29; // r12
  __int64 v30; // rdi
  __int64 v31; // rsi
  __int64 v32; // rcx
  LPVOID v33; // rax
  __int64 v34; // r8
  __int64 v35; // r9
  __int64 v36; // rdi
  unsigned int v37; // esi
  __int64 v38; // rax
  const struct std::nothrow_t *v39; // rdx
  __int64 v40; // rcx
  void *v42; // [rsp+40h] [rbp-A9h] BYREF
  __int64 v43; // [rsp+48h] [rbp-A1h] BYREF
  __int64 v44; // [rsp+50h] [rbp-99h] BYREF
  void *v45; // [rsp+58h] [rbp-91h] BYREF
  _BYTE v46[8]; // [rsp+70h] [rbp-79h] BYREF
  _QWORD **v47; // [rsp+78h] [rbp-71h]
  unsigned __int64 v48; // [rsp+80h] [rbp-69h]
  _BYTE v49[40]; // [rsp+88h] [rbp-61h] BYREF
  _BYTE v50[128]; // [rsp+B0h] [rbp-39h] BYREF
  __int64 v51; // [rsp+140h] [rbp+57h] BYREF
  int *v52; // [rsp+148h] [rbp+5Fh]
  __int64 v53; // [rsp+150h] [rbp+67h] BYREF
  void *v54; // [rsp+158h] [rbp+6Fh] BYREF

  v52 = a2;
  v9 = 0;
  *(_DWORD *)(a3 + 8) = 0;
  std::_Hash<std::_Umap_traits<unsigned long,enum ContextualProcessorDecision,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,enum ContextualProcessorDecision>>,0>>::clear(a3 + 16);
  Microsoft::WRL::ComPtr<IUnknown>::operator=((__int64 *)a3, a4 + 18);
  if ( a1 )
  {
    v10 = a1 + 24;
    v11 = a1 + 24;
  }
  else
  {
    v10 = 0LL;
    v11 = 0LL;
  }
  v53 = v10;
  wil::com_ptr_t<AsyncHRESULTPrincipal,wil::err_exception_policy>::~com_ptr_t<AsyncHRESULTPrincipal,wil::err_exception_policy>(&v53);
  if ( *(_QWORD *)(a3 + 152) != v11 )
  {
    v51 = v10;
    wil::com_ptr_t<AsyncHRESULTPrincipal,wil::err_exception_policy>::~com_ptr_t<AsyncHRESULTPrincipal,wil::err_exception_policy>(&v51);
    v14 = *(_QWORD *)(a3 + 152);
    *(_QWORD *)(a3 + 152) = v10;
    if ( v14 )
      (*(void (__fastcall **)(__int64))(*(_QWORD *)v14 + 16LL))(v14);
  }
  *(_DWORD *)(*(_QWORD *)(a1 + 160) + 104LL) = *((_DWORD *)a4 + 18);
  v15 = a4 + 10;
  v16 = *(_QWORD *)(a1 + 160) + 112LL;
  if ( (_QWORD *)v16 != a4 + 10 )
    std::_Hash<std::_Umap_traits<unsigned long,enum ContextualProcessorDecision,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,enum ContextualProcessorDecision>>,0>>::_Copy_assign(
      v16,
      a4 + 10);
  std::unordered_map<unsigned long,enum ContextualProcessorState>::unordered_map<unsigned long,enum ContextualProcessorState>(
    (__int64)v46,
    v12,
    v13);
  v19 = (__int64 **)a4[2];
  v51 = (__int64)v19;
  for ( i = (__int64 **)*v19; i != v19; i = (__int64 **)*i )
  {
    v17 = (unsigned __int8 *)(i + 2);
    if ( *((_DWORD *)i + 5) )
    {
      std::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>::_Insert_unverified<unsigned long const,std::_Not_a_node_tag>(
        (__int64)v46,
        (__int64)&v45,
        v17);
      v19 = (__int64 **)v51;
    }
  }
  if ( v48 < a4[3] || (v21 = *(_QWORD *)(a5 + 16) == 0LL, LOBYTE(v51) = 0, !v21) )
    LOBYTE(v51) = 1;
  if ( v48 )
  {
    v22 = v52;
    VariableSizedPayloadStorage<InputInfo>::VariableSizedPayloadStorage<InputInfo>(&v42, v52, (__int64)v17, v18);
    v23 = (int *)v42;
    v24 = InfoMetadata::Get();
    Input = InfoMetadata::GetInput(v24, (unsigned int)*v22);
    v28 = Input;
    if ( Input && (_BYTE)v51 )
      std::_Func_class<void,InputInfo *,std::unordered_set<unsigned long> &,InputInfo *>::operator()(
        Input + 64,
        (__int64)v22,
        (__int64)v46,
        v23);
    VariableSizedPayloadStorage<InputInfo>::VariableSizedPayloadStorage<InputInfo>(&v54, v23, v26, v27);
    v29 = v54;
    v45 = v54;
    if ( v28 )
      std::_Func_class<void,InputInfo *,std::unordered_set<unsigned long> &,InputInfo *>::operator()(
        v28 + 256,
        (__int64)v23,
        a6,
        v54);
    v9 = (*(__int64 (__fastcall **)(_QWORD, void *, _QWORD, __int64))(*(_QWORD *)*a4 + 40LL))(
           *a4,
           v29,
           *(_QWORD *)(a1 + 160),
           a3);
    v30 = *(_QWORD *)(a3 + 80);
    v31 = (*(__int64 (__fastcall **)(_QWORD))(*(_QWORD *)*a4 + 48LL))(*a4);
    v33 = wil::details::static_lazy<InputETW>::get(
            v32,
            _lambda_ffe5d831185bf77c783480ebf240a46d_::_lambda_invoker_cdecl_);
    if ( **((_DWORD **)v33 + 1) > 4u && tlgKeywordOn(*((_QWORD *)v33 + 1), 1LL) )
    {
      LODWORD(v51) = v9;
      v43 = v30;
      v44 = v31;
      _tlgWriteTemplate<long (_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),&long _tlgWriteTransfer_EventWriteTransfer(_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),_GUID const *,_GUID const *>::Write<_tlgWrapSz<unsigned short>,_tlgWrapperByVal<8>,_tlgWrapperByVal<4>>(
        v34,
        (unsigned __int8 *)dword_1801CE6F3,
        v34,
        v35,
        (const wchar_t **)&v44,
        (__int64)&v43,
        (__int64)&v51);
    }
    v36 = *(_QWORD *)(a3 + 80);
    v37 = *(_DWORD *)(a3 + 8);
    v38 = (*(__int64 (__fastcall **)(_QWORD))(*(_QWORD *)*a4 + 48LL))(*a4);
    InputTraceLogging::ContextualProcessing::OnInput(v45, v38, v37, v36, v9);
    if ( v54 )
      operator delete(v54, v39);
    if ( v42 )
      operator delete(v42, v39);
    v15 = a4 + 10;
  }
  if ( v9 >= 0 )
  {
    *((_DWORD *)a4 + 18) = *(_DWORD *)(a3 + 8);
    if ( v15 != (_QWORD *)(a3 + 16) )
      std::_Hash<std::_Umap_traits<unsigned long,enum ContextualProcessorDecision,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,enum ContextualProcessorDecision>>,0>>::_Copy_assign(
        v15,
        a3 + 16);
  }
  else
  {
    *(_DWORD *)(a3 + 8) = 0;
    std::_Hash<std::_Umap_traits<unsigned long,enum ContextualProcessorDecision,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,enum ContextualProcessorDecision>>,0>>::clear(a3 + 16);
  }
  std::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>(
    (__int64)v50,
    (__int64)v46,
    (__int64 *)v17);
  ContextualProcessorBuffer::ProcessContextualProcessorDecision(
    a1,
    (_DWORD)v52,
    (unsigned int)v50,
    a3,
    (__int64)a4,
    a5,
    a6);
  std::vector<std::_List_unchecked_iterator<std::_List_val<std::_List_simple_types<std::pair<unsigned char const,KeyboardProcessor::TargetingInfo>>>>>::_Tidy((__int64)v49);
  std::_List_node<std::pair<unsigned long const,enum ContextualProcessorState>,void *>::_Free_non_head<std::allocator<std::_List_node<std::pair<unsigned long const,enum ContextualProcessorState>,void *>>>(
    v40,
    v47);
  std::_Deallocate<16,0>(v47, (const struct std::nothrow_t *)0x18);
  if ( v10 )
    (*(void (__fastcall **)(__int64))(*(_QWORD *)v10 + 16LL))(v10);
  return (unsigned int)v9;
}
