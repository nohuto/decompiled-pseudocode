...
_QWORD *__fastcall std::list<std::pair<IInputTarget * const,std::unordered_set<unsigned long>>>::erase(
        __int64 a1,
        _QWORD *a2,
        void *a3)
{
  ...
  *(_QWORD *)(*(_QWORD *)a3 + 8LL) = *((_QWORD *)a3 + 1);
  --*(_QWORD *)(a1 + 8);
  std::vector<std::_List_unchecked_iterator<std::_List_val<std::_List_simple_types<std::pair<unsigned char const,KeyboardProcessor::TargetingInfo>>>>>::_Tidy((__int64)a3 + 48);
  std::_List_node<std::pair<unsigned long const,enum ContextualProcessorState>,void *>::_Free_non_head<std::allocator<std::_List_node<std::pair<unsigned long const,enum ContextualProcessorState>,void *>>>(
    v6,
    *((_QWORD ***)a3 + 4));
  std::_Deallocate<16,0>(*((void **)a3 + 4), (const struct std::nothrow_t *)0x18);
  std::_Deallocate<16,0>(a3, (const struct std::nothrow_t *)0x58);
  *a2 = v3;
  return a2;
}
