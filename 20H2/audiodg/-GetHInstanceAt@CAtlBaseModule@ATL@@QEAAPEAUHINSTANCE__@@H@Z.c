/*
 * XREFs of ?GetHInstanceAt@CAtlBaseModule@ATL@@QEAAPEAUHINSTANCE__@@H@Z @ 0x14005053C
 * Callers:
 *     ?AtlFindStringResourceInstance@ATL@@YAPEAUHINSTANCE__@@IG@Z @ 0x140041810 (-AtlFindStringResourceInstance@ATL@@YAPEAUHINSTANCE__@@IG@Z.c)
 * Callees:
 *     ?_AtlRaiseException@ATL@@YAXKK@Z @ 0x14003E7B8 (-_AtlRaiseException@ATL@@YAXKK@Z.c)
 */

__int64 __fastcall ATL::CAtlBaseModule::GetHInstanceAt(ATL::CAtlBaseModule *this, int a2)
{
  __int64 v2; // rbx
  __int64 v3; // rdi

  v2 = a2;
  EnterCriticalSection(&stru_14008FFE8);
  v3 = 0LL;
  if ( (int)v2 <= dword_140090018 && (int)v2 >= 0 )
  {
    if ( (_DWORD)v2 == dword_140090018 )
    {
      v3 = qword_14008FFD0;
    }
    else
    {
      if ( (int)v2 >= dword_140090018 )
      {
        ATL::_AtlRaiseException(0xC000008C);
        JUMPOUT(0x1400505B2LL);
      }
      v3 = *((_QWORD *)qword_140090010 + v2);
    }
  }
  LeaveCriticalSection(&stru_14008FFE8);
  return v3;
}
