/*
 * XREFs of _vsnwprintf @ 0x18008DC70
 * Callers:
 *     RtlStringCchPrintfW @ 0x1800445CC (RtlStringCchPrintfW.c)
 *     StringCbPrintfW @ 0x180058298 (StringCbPrintfW.c)
 *     RtlStringVPrintfWorkerW @ 0x180066594 (RtlStringVPrintfWorkerW.c)
 *     RtlStringVPrintfWorkerW_0 @ 0x180069A08 (RtlStringVPrintfWorkerW_0.c)
 *     RtlStringCbPrintfW @ 0x1800D8910 (RtlStringCbPrintfW.c)
 *     StringCchPrintfW @ 0x1800DE388 (StringCchPrintfW.c)
 * Callees:
 *     _vsnwprintf_l @ 0x18008DC8C (_vsnwprintf_l.c)
 */

int __cdecl vsnwprintf(wchar_t *Buffer, size_t BufferCount, const wchar_t *Format, va_list Args)
{
  return vsnwprintf_l(Buffer, BufferCount, Format, 0LL, Args);
}
