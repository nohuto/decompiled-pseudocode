/*
 * XREFs of McTemplateK0pqqqqqqqsssxqqqt @ 0x1C0039BF8
 * Callers:
 *     Etw_ControllerCreate @ 0x1C0038FFC (Etw_ControllerCreate.c)
 *     Etw_ControllerDelete @ 0x1C00390FC (Etw_ControllerDelete.c)
...
ULONG McTemplateK0pqqqqqqqsssxqqqt(__int64 a1, const EVENT_DESCRIPTOR *a2, __int64 a3, ...)
{
  const struct _MCGEN_TRACE_CONTEXT *v4; // rdx
  __int64 v5; // rcx
  __int64 v6; // rax
  int v7; // eax
  const struct _MCGEN_TRACE_CONTEXT *v8; // rdx
  __int64 v9; // rax
  int v10; // eax
  ...
  const struct _MCGEN_TRACE_CONTEXT *v31; // [rsp+C0h] [rbp-40h]
  int v32; // [rsp+C8h] [rbp-38h]
  int v33; // [rsp+CCh] [rbp-34h]
  const struct _MCGEN_TRACE_CONTEXT *v34; // [rsp+D0h] [rbp-30h]
  int v35; // [rsp+D8h] [rbp-28h]
  int v36; // [rsp+DCh] [rbp-24h]
  const struct _MCGEN_TRACE_CONTEXT *v37; // [rsp+E0h] [rbp-20h]
  ...
  const struct _MCGEN_TRACE_CONTEXT *v66; // [rsp+1B8h] [rbp+B8h]
  const struct _MCGEN_TRACE_CONTEXT *v67; // [rsp+1C0h] [rbp+C0h]
  const struct _MCGEN_TRACE_CONTEXT *v68; // [rsp+1C8h] [rbp+C8h]
  ...
  v66 = va_arg(va8, const struct _MCGEN_TRACE_CONTEXT *);
  ...
  if ( v66 )
  {
    ...
    do
      ++v6;
    while ( *((_BYTE *)&v66->RegistrationHandle + v6) );
    v7 = v6 + 1;
  }
  else
  {
  ...
  if ( v67 )
  {
    ...
    do
      ++v9;
    while ( *((_BYTE *)&v67->RegistrationHandle + v9) );
    v10 = v9 + 1;
  }
  else
  {
  ...
  if ( v68 )
  {
    do
      ++v5;
    while ( *((_BYTE *)&v68->RegistrationHandle + v5) );
  ...
  v47 = 4LL;
  v49 = 4LL;
  return McGenEventWrite((PMCGEN_TRACE_CONTEXT)v12, a2, 0LL, 0x11u, &EventData);
}
