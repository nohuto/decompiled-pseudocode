/*
 * XREFs of ndisReadMiniportDefaultPortAuthStates @ 0x1C00BC674
 * Callers:
 *     ndisMInitializeAdapter @ 0x1C010B314 (ndisMInitializeAdapter.c)
 * Callees:
 *     WPP_SF_q @ 0x1C003A798 (WPP_SF_q.c)
 *     WPP_SF_qD @ 0x1C003A7CC (WPP_SF_qD.c)
 *     ndisReadMiniportMediaSpecificPortAuthStates @ 0x1C00BC768 (ndisReadMiniportMediaSpecificPortAuthStates.c)
 *     ndisReadMiniportSpecificPortAuthStates @ 0x1C00BC95C (ndisReadMiniportSpecificPortAuthStates.c)
 *     NdisCloseConfiguration @ 0x1C00C48C0 (NdisCloseConfiguration.c)
 *     NdisOpenConfigurationEx @ 0x1C00C6C20 (NdisOpenConfigurationEx.c)
 */

__int64 __fastcall ndisReadMiniportDefaultPortAuthStates(__int64 a1)
{
  unsigned int MiniportMediaSpecificPortAuthStates; // edi
  unsigned int MiniportSpecificPortAuthStates; // eax
  PVOID v4; // rcx
  struct _NDIS_CONFIGURATION_OBJECT ConfigObject; // [rsp+20h] [rbp-28h] BYREF
  PVOID ConfigurationHandle; // [rsp+50h] [rbp+8h] BYREF

  if ( (unsigned __int8)byte_1C00A025C >= 4u )
    WPP_SF_q(0x36u, &WPP_b87f241b44683114d5775ca3de93698d_Traceguids, a1);
  ConfigObject.Flags = 0;
  ConfigObject.Header = (_NDIS_OBJECT_HEADER)1573289;
  ConfigObject.NdisHandle = (void *)a1;
  MiniportMediaSpecificPortAuthStates = NdisOpenConfigurationEx(&ConfigObject, &ConfigurationHandle);
  if ( !MiniportMediaSpecificPortAuthStates )
  {
    MiniportSpecificPortAuthStates = ndisReadMiniportSpecificPortAuthStates(a1, ConfigurationHandle);
    v4 = ConfigurationHandle;
    MiniportMediaSpecificPortAuthStates = MiniportSpecificPortAuthStates;
    if ( !MiniportSpecificPortAuthStates )
    {
      *(_DWORD *)(a1 + 2700) |= 8u;
      NdisCloseConfiguration(v4);
      goto LABEL_9;
    }
    NdisCloseConfiguration(ConfigurationHandle);
    goto LABEL_6;
  }
  if ( *(_BYTE *)(a1 + 32) < 6u )
LABEL_6:
    MiniportMediaSpecificPortAuthStates = ndisReadMiniportMediaSpecificPortAuthStates(a1);
  if ( MiniportMediaSpecificPortAuthStates )
  {
    *(_DWORD *)(a1 + 3352) = ndisGlobalPortSendAuthorizationState;
    *(_DWORD *)(a1 + 3356) = ndisGlobalPortReceiveAuthorizationState;
    *(_DWORD *)(a1 + 3344) = ndisGlobalPortSendControlState;
    *(_DWORD *)(a1 + 3348) = ndisGlobalPortReceiveControlState;
    *(_DWORD *)(a1 + 3336) = ndisGlobalPortSendAuthorizationState;
    *(_DWORD *)(a1 + 3340) = ndisGlobalPortReceiveAuthorizationState;
    *(_DWORD *)(a1 + 3328) = ndisGlobalPortSendControlState;
    *(_DWORD *)(a1 + 3332) = ndisGlobalPortReceiveControlState;
  }
LABEL_9:
  if ( (unsigned __int8)byte_1C00A025C >= 4u )
    WPP_SF_qD(0x37u, &WPP_b87f241b44683114d5775ca3de93698d_Traceguids, a1, MiniportMediaSpecificPortAuthStates);
  return MiniportMediaSpecificPortAuthStates;
}
