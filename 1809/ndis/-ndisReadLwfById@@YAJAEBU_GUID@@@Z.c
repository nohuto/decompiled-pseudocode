/*
 * XREFs of ?ndisReadLwfById@@YAJAEBU_GUID@@@Z @ 0x1C01063C0
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall ndisReadLwfById(const struct _GUID *a1)
{
  ...
  v8 = v1;
  Handle[0] = 0LL;
  netsetupBuildObjectPath(3, &a1->Data1, 0, 1, v16);
  FilterAltitude = KRegKey::Open((KRegKey *)Handle, 1u, v16, 0LL);
  if ( !FilterAltitude )
  {
    v6 = Handle;
    FilterAltitude = ndisBlowStringListIntoAtoms(&v6, (const struct _NETSETUPPROPKEY *)&unk_1C0091550, (__int64)&v9);
    if ( !FilterAltitude )
    {
      FilterAltitude = ndisBlowStringListIntoAtoms(&v6, (const struct _NETSETUPPROPKEY *)&unk_1C0091568, (__int64)&v11);
      if ( !FilterAltitude )
      {
        FilterAltitude = ndisGetFilterAltitude((struct NetSetupPropertyBag *)&v6, &v13);
        if ( !FilterAltitude )
        {
          FilterAltitude = NetSetupPropertyBag::ReadBoolean(&v6, (const struct _NETSETUPPROPKEY *)&unk_1C0091598, &v14);
          if ( !FilterAltitude )
          {
            NetSetupPropertyBag::ReadBoolean(&v6, (const struct _NETSETUPPROPKEY *)&unk_1C0091580, &v5);
