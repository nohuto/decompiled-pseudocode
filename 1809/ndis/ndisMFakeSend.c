/*
 * XREFs of ndisMFakeSend @ 0x1C0062790
 * Callers:
 *     <none>
 * Callees:
 *     WPP_SF_q @ 0x1C003A798 (WPP_SF_q.c)
 */

__int64 __fastcall ndisMFakeSend(__int64 a1)
{
  __int64 v1; // rdi
  unsigned int v3; // ebx

  v1 = *(_QWORD *)(a1 + 16);
  if ( (unsigned __int8)byte_1C00A0252 >= 4u )
    WPP_SF_q(0x59u, &WPP_f453a6740ca1326a168191528118e90e_Traceguids, v1);
  v3 = -1073741823;
  if ( v1 )
    v3 = *(_DWORD *)(v1 + 540);
  if ( *(_BYTE *)(*(_QWORD *)(a1 + 24) + 56LL) < 6u && (v3 & 0xC0230000) == 0xC0230000 )
    v3 = (unsigned __int16)v3 | 0xC0010000;
  if ( (unsigned __int8)byte_1C00A0252 >= 4u )
    WPP_SF_q(0x5Au, &WPP_f453a6740ca1326a168191528118e90e_Traceguids, v1);
  return v3;
}
