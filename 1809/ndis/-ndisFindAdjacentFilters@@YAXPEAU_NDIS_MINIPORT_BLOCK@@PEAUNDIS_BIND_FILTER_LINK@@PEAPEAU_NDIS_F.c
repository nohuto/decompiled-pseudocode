/*
 * XREFs of ?ndisFindAdjacentFilters@@YAXPEAU_NDIS_MINIPORT_BLOCK@@PEAUNDIS_BIND_FILTER_LINK@@PEAPEAU_NDIS_FILTER_BLOCK@@2@Z @ 0x1C00C09BC
 * Callers:
...
LABEL_15:
        __fastfail(5u);
      Myptr = a1->Bindings.Filters._p[v10]._Myptr;
      if ( Myptr == a2 )
        break;
    ...
    if ( v15 < (unsigned int)m_numElements )
    {
      while ( 1 )
      {
        if ( v15 >= (unsigned int)v11 )
          goto LABEL_15;
        if ( Ndis::BindState::GetBindContext(&a1->Bindings.Filters._p[v15]._Myptr->BindState) )
          break;
