/*
 * XREFs of RtlpHpSegMgrRelease @ 0x180066270
 * Callers:
 *     RtlpHpSegMgrAllocate @ 0x18005D600 (RtlpHpSegMgrAllocate.c)
 *     RtlpHpSegSegmentFree @ 0x1800661D0 (RtlpHpSegSegmentFree.c)
 * Callees:
 *     RtlpHpSegMgrCommit @ 0x180060A68 (RtlpHpSegMgrCommit.c)
 *     RtlpHpFreeVA @ 0x1800611FC (RtlpHpFreeVA.c)
 *     RtlpHpQueryVA @ 0x180066388 (RtlpHpQueryVA.c)
 *     RtlpHpSegMgrVaCtxFree @ 0x18010DBE0 (RtlpHpSegMgrVaCtxFree.c)
 */

char __fastcall RtlpHpSegMgrRelease(__int64 a1, __int64 a2, int a3)
{
  unsigned __int64 v4; // rax
  int v5; // edx
  unsigned __int64 v6; // rdx
  _WORD *v8; // rbx
  unsigned int v9; // esi
  unsigned __int64 v10; // r14
  __int128 v12; // [rsp+40h] [rbp-10h] BYREF
  __int64 v13; // [rsp+80h] [rbp+30h] BYREF
  unsigned __int64 v14; // [rsp+88h] [rbp+38h] BYREF
  _WORD *v15; // [rsp+98h] [rbp+48h] BYREF

  LOBYTE(v4) = *(_BYTE *)(a1 + 13) & 7;
  v5 = ~*(_DWORD *)a1;
  v13 = a2;
  v6 = (unsigned int)(v5 + 1);
  v14 = v6;
  if ( !(_BYTE)v4 )
  {
LABEL_2:
    if ( a2 )
    {
      v12 = *(_OWORD *)(a1 + 40);
      LOBYTE(v4) = RtlpHpFreeVA((unsigned __int64 *)&v13, &v14, 0x8000, &v12);
    }
    return v4;
  }
  if ( v6 >= 0x200000 )
    goto LABEL_9;
  if ( a3 > 0 )
    RtlpHpSegMgrCommit(a1, a2, 0, (unsigned int)v6 >> 12, -a3, 0x4000, 0);
  v4 = RtlpHpSegMgrVaCtxFree(a1, a2, &v14);
  v13 = v4;
  a2 = v4;
  if ( v4 )
  {
LABEL_9:
    RtlpHpQueryVA(a2, v6, &v15, 0LL);
    v8 = v15;
    v9 = 0;
    v4 = v14 >> 21;
    v10 = (unsigned __int64)&v15[v14 >> 21];
    if ( (unsigned __int64)v15 < v10 )
    {
      do
      {
        LOWORD(v4) = *v8;
        if ( (*v8 & 0x7FF) != 0 )
          LOBYTE(v4) = RtlpHpSegMgrCommit(a1, v13, v9, 512, -(v4 & 0x7FF), 0x4000, 0);
        ++v8;
        v9 += 512;
      }
      while ( (unsigned __int64)v8 < v10 );
    }
    a2 = v13;
    goto LABEL_2;
  }
  return v4;
}
