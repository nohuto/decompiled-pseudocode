/*
 * XREFs of LdrpCreatePendingEnclaveModule @ 0x1800CF100
 * Callers:
 *     LdrLoadEnclaveModule @ 0x1800CEEC0 (LdrLoadEnclaveModule.c)
 *     LdrpFindOrPrepareEnclaveModule @ 0x1800CF658 (LdrpFindOrPrepareEnclaveModule.c)
 * Callees:
 *     RtlEqualUnicodeString @ 0x18000A720 (RtlEqualUnicodeString.c)
 *     LdrpAllocatePlaceHolder @ 0x180026CE4 (LdrpAllocatePlaceHolder.c)
 */

__int64 __fastcall LdrpCreatePendingEnclaveModule(
        _QWORD *a1,
        const void **a2,
        int a3,
        int a4,
        __int64 a5,
        __int64 a6,
        _QWORD *a7,
        __int64 a8)
{
  __int64 result; // rax
  _QWORD *v11; // rbx
  _QWORD *v12; // rcx
  __int64 v13[3]; // [rsp+40h] [rbp-18h] BYREF

  result = LdrpAllocatePlaceHolder(a2, a5, a3, a4, a6, v13, a8);
  if ( (int)result >= 0 )
  {
    v11 = (_QWORD *)v13[0];
    *(_QWORD *)(*(_QWORD *)(v13[0] + 176) + 168LL) = a1;
    v12 = (_QWORD *)a1[12];
    if ( (_QWORD *)*v12 != a1 + 11 )
      __fastfail(3u);
    *v11 = a1 + 11;
    v11[1] = v12;
    *v12 = v11;
    a1[12] = v11;
    if ( a7 )
      *a7 = v11;
    if ( RtlEqualUnicodeString((PUNICODE_STRING)a2, (PUNICODE_STRING)&stru_180119310, 1u) )
    {
      a1[14] = v11;
    }
    else if ( RtlEqualUnicodeString((PUNICODE_STRING)a2, (PUNICODE_STRING)&stru_180119300, 1u) )
    {
      a1[15] = v11;
    }
    return 0LL;
  }
  return result;
}
