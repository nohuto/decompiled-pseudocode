/*
 * XREFs of ?Thunk_Materialize_BamoHeatGripServiceProxy_0@?$IBamoPeer_ISMBamos_AutoBamos_Receive@VBamoPeerImpl@ISMBamos_AutoBamos@BamoImpl@@@@SAJPEAXPEAPEAX@Z @ 0x180018580
 * Callers:
 *     <none>
 * Callees:
 *     ?Materialize_BamoHeatGripServiceProxy@BamoPeerImpl@ISMBamos_AutoBamos@BamoImpl@@QEAAJI@Z @ 0x180016E4C (-Materialize_BamoHeatGripServiceProxy@BamoPeerImpl@ISMBamos_AutoBamos@BamoImpl@@QEAAJI@Z.c)
 */

void __fastcall __noreturn IBamoPeer_ISMBamos_AutoBamos_Receive<BamoImpl::ISMBamos_AutoBamos::BamoPeerImpl>::Thunk_Materialize_BamoHeatGripServiceProxy_0(
        BamoImpl::ISMBamos_AutoBamos::BamoPeerImpl *a1)
{
  BamoImpl::ISMBamos_AutoBamos::BamoPeerImpl::Materialize_BamoHeatGripServiceProxy(a1);
}
