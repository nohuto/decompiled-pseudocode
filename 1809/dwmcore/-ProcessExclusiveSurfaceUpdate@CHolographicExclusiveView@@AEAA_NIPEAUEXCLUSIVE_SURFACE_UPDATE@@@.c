/*
 * XREFs of ?ProcessExclusiveSurfaceUpdate@CHolographicExclusiveView@@AEAA_NIPEAUEXCLUSIVE_SURFACE_UPDATE@@@Z @ 0x18020CB64
 * Callers:
        ...
        struct EXCLUSIVE_SURFACE_UPDATE *a3)
{
  ...
  if ( AnalogExclusiveSurfaceUpdates >= 0 )
  {
    if ( v15 )
    {
      ...
      {
        MilInstrumentationCheckHR_MaybeFailFast(v10, 0LL, 0, v9, 0x1DBu);
      }
    }
  }
  else
  {
    MilInstrumentationCheckHR_MaybeFailFast(v8, 0LL, 0, AnalogExclusiveSurfaceUpdates | 0x10000000, 0x1D1u);
  }
  return v3;
}
