/*
 * XREFs of ?CalcDeviceTransformDelta@CTreeEffectLayer@@MEAAJPEAVCDrawingContext@@PEAVCMILMatrix@@@Z @ 0x18000F0B0
 * Callers:
 *     <none>
 * Callees:
        ...
        struct CMILMatrix *a3)
{
  ...
  if ( v7 )
  {
    ...
    if ( ShadowBounds < 0 )
    {
      MilInstrumentationCheckHR_MaybeFailFast(v9, 0LL, 0, ShadowBounds, 0x299u);
      return v6;
    }
    if ( IsEmpty(&v37) )
    {
  ...
  if ( CLayerVisual::IsAutomaticBoundsToLocalSpaceEnabled(*((CLayerVisual **)this + 31)) )
  {
    ...
    if ( v28 < 0 )
    {
      MilInstrumentationCheckHR_MaybeFailFast(v29, 0LL, 0, v28, 0x2BBu);
    }
    else
    {
