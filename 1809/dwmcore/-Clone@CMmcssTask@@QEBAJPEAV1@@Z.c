/*
 * XREFs of ?Clone@CMmcssTask@@QEBAJPEAV1@@Z @ 0x18008A9BC
 * Callers:
 *     ?ProcessKernelTokens@CSurfaceManager@@AEAAJXZ @ 0x18008A894 (-ProcessKernelTokens@CSurfaceManager@@AEAAJXZ.c)
 * Callees:
 *     ?Apply@CMmcssTask@@QEAAJ_N@Z @ 0x18007AD94 (-Apply@CMmcssTask@@QEAAJ_N@Z.c)
 *     ?Set@CMmcssTask@@QEAAJPEBUDWM_MMTASK@@_N@Z @ 0x18008A598 (-Set@CMmcssTask@@QEAAJPEBUDWM_MMTASK@@_N@Z.c)
 *     ?IsActive@CMmcssTask@@QEBA_NXZ @ 0x18008AA50 (-IsActive@CMmcssTask@@QEBA_NXZ.c)
 *     ?AreEqualMmTask@CMmcssTask@@CA_NPEBUDWM_MMTASK@@0@Z @ 0x18008AA70 (-AreEqualMmTask@CMmcssTask@@CA_NPEBUDWM_MMTASK@@0@Z.c)
 *     ?Revert@CMmcssTask@@QEAAXXZ @ 0x18008ACCC (-Revert@CMmcssTask@@QEAAXXZ.c)
 *     ??1?$CGuard@VCCriticalSection@@@@QEAA@XZ @ 0x180099D60 (--1-$CGuard@VCCriticalSection@@@@QEAA@XZ.c)
 *     ?MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJI@Z @ 0x1800C7F7C (-MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJI@Z.c)
 */

__int64 __fastcall CMmcssTask::Clone(const struct DWM_MMTASK **this, const struct DWM_MMTASK **a2)
{
  unsigned int v4; // esi
  char IsActive; // al
  CMmcssTask *v6; // rcx
  char v7; // r8
  char v9; // al
  int v10; // eax
  unsigned int v11; // ecx
  CMmcssTask *v12; // [rsp+40h] [rbp+8h] BYREF

  v12 = (CMmcssTask *)this;
  v4 = 0;
  EnterCriticalSection((LPCRITICAL_SECTION)this);
  if ( CMmcssTask::AreEqualMmTask(this[5], a2[5]) )
  {
    CMmcssTask::IsActive((CMmcssTask *)a2);
    IsActive = CMmcssTask::IsActive((CMmcssTask *)this);
    if ( v7 != IsActive )
    {
      if ( CMmcssTask::IsActive(v6) )
        CMmcssTask::Apply((struct _RTL_CRITICAL_SECTION *)a2, 0);
      else
        CMmcssTask::Revert((CMmcssTask *)a2);
    }
  }
  else
  {
    v9 = CMmcssTask::IsActive((CMmcssTask *)this);
    v10 = CMmcssTask::Set(a2, this[5], v9);
    v4 = v10;
    if ( v10 < 0 )
      MilInstrumentationCheckHR_MaybeFailFast(v11, 0LL, 0, v10, 0x7Au);
  }
  CGuard<CCriticalSection>::~CGuard<CCriticalSection>(&v12);
  return v4;
}
