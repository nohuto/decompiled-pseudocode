/*
 * XREFs of ?EnsureDrawingContextFrame@CContentBounder@@AEAAJXZ @ 0x18004C508
 * Callers:
...
__int64 __fastcall CContentBounder::EnsureDrawingContextFrame(CContentBounder *this)
{
  ...
  if ( v8 < 0 )
  {
    MilInstrumentationCheckHR_MaybeFailFast(v9, 0LL, 0, v8, 0x71u);
  }
  else
  {
    ...
    if ( v10 >= 0 )
      goto LABEL_6;
    MilInstrumentationCheckHR_MaybeFailFast(v11, 0LL, 0, v10, 0x72u);
  }
...
LABEL_6:
  if ( v5 < 0 )
  {
    MilInstrumentationCheckHR_MaybeFailFast(v11, 0LL, 0, v5, 0x94u);
...
LABEL_2:
  ...
  v5 = v3;
  if ( v3 < 0 )
    MilInstrumentationCheckHR_MaybeFailFast(v4, 0LL, 0, v3, 0xA0u);
  return (unsigned int)v5;
}
