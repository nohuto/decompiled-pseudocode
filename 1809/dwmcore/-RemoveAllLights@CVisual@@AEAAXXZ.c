/*
 * XREFs of ?RemoveAllLights@CVisual@@AEAAXXZ @ 0x18004E4E8
 * Callers:
 *     ??1CVisual@@MEAA@XZ @ 0x18004D570 (--1CVisual@@MEAA@XZ.c)
 * Callees:
 *     ?Remove@CPtrArrayBase@@IEAA_N_K@Z @ 0x18005B468 (-Remove@CPtrArrayBase@@IEAA_N_K@Z.c)
 *     _guard_dispatch_icall_nop @ 0x1800F00A0 (_guard_dispatch_icall_nop.c)
 */

void __fastcall CVisual::RemoveAllLights(CVisual *this)
{
  _BYTE *v1; // rsi
  _BYTE *v3; // rdi
  __int64 v4; // rbp
  _BYTE *v5; // r14
  _BYTE *v6; // rdi
  __int64 v7; // rsi

  v1 = (_BYTE *)*((_QWORD *)this + 36);
  v3 = (_BYTE *)*((_QWORD *)this + 35);
  v4 = (v1 - v3) >> 4;
  if ( v4 )
  {
    while ( v3 != v1 )
    {
      if ( !v3[8] )
        (*(void (__fastcall **)(_QWORD, CVisual *))(**(_QWORD **)v3 + 240LL))(*(_QWORD *)v3, this);
      CPtrArrayBase::Remove((CPtrArrayBase *)(*(_QWORD *)v3 + 24LL), (unsigned __int64)this);
      v3 += 16;
    }
    *((_QWORD *)this + 36) = *((_QWORD *)this + 35);
  }
  v5 = (_BYTE *)*((_QWORD *)this + 39);
  v6 = (_BYTE *)*((_QWORD *)this + 38);
  v7 = (v5 - v6) >> 4;
  if ( v7 )
  {
    while ( v6 != v5 )
    {
      if ( !v6[8] )
        (*(void (__fastcall **)(_QWORD, CVisual *))(**(_QWORD **)v6 + 248LL))(*(_QWORD *)v6, this);
      v6 += 16;
    }
    *((_QWORD *)this + 39) = *((_QWORD *)this + 38);
  }
  if ( v7 + v4 )
    (*(void (__fastcall **)(CVisual *, __int64))(*(_QWORD *)this + 64LL))(this, 1LL);
}
