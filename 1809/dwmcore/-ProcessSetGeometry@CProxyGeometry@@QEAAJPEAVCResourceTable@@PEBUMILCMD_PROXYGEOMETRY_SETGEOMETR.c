/*
 * XREFs of ?ProcessSetGeometry@CProxyGeometry@@QEAAJPEAVCResourceTable@@PEBUMILCMD_PROXYGEOMETRY_SETGEOMETRY@@@Z @ 0x1801B1C2C
 * Callers:
...
__int64 __fastcall CProxyGeometry::ProcessSetGeometry(
        ...
        const struct MILCMD_PROXYGEOMETRY_SETGEOMETRY *a3)
{
  unsigned int v4; // ebx
  ...
  if ( ResourceWithoutType != this[17] )
  {
           ...
           59LL) )
    {
      ...
      if ( v8 < 0 )
      {
        MilInstrumentationCheckHR_MaybeFailFast(v9, 0LL, 0, v8, 0x46u);
      }
      else
      {
    ...
    {
      v4 = -2003303421;
      MilInstrumentationCheckHR_MaybeFailFast(v7, 0LL, 0, -2003303421, 0x43u);
    }
  }
  return v4;
}
