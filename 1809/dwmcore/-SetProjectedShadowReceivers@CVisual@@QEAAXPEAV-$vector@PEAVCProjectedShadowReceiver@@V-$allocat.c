/*
 * XREFs of ?SetProjectedShadowReceivers@CVisual@@QEAAXPEAV?$vector@PEAVCProjectedShadowReceiver@@V?$allocator@PEAVCProjectedShadowReceiver@@@std@@@std@@@Z @ 0x1801AF770
 * Callers:
...
__int64 __fastcall CVisual::SetProjectedShadowReceivers(__int64 a1, __int64 a2)
{
  ...
  if ( a2 )
  {
    ...
    {
      result = (__int64)CSparseAlignedStorage<8,8>::AllocatedStorage::ReserveSlot(v2, 16);
      *(_QWORD *)result = a2;
    }
  }
  else if ( (result & 0x10000) != 0 )
  {
    ...
    *v5 = result & 0xFFFEFFFF;
    result = CSparseAlignedStorage<8,8>::AllocatedStorage::FindSlot(v7, 16);
