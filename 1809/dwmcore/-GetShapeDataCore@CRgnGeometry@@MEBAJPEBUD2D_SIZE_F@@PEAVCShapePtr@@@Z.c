/*
 * XREFs of ?GetShapeDataCore@CRgnGeometry@@MEBAJPEBUD2D_SIZE_F@@PEAVCShapePtr@@@Z @ 0x18009B8E0
 * Callers:
 *     <none>
 * Callees:
 *     ??2@YAPEAX_K@Z @ 0x18002FAC8 (--2@YAPEAX_K@Z.c)
 *     ?Release@CShapePtr@@QEAAXXZ @ 0x1800659B4 (-Release@CShapePtr@@QEAAXXZ.c)
 *     ?BuildFromRects@CRegionShape@@QEAAJPEBV?$TMilRect_@HUtagRECT@@UMilPointAndSizeL@@U_CMilRectL_@RectUniqueness@@@@I@Z @ 0x18009B9D4 (-BuildFromRects@CRegionShape@@QEAAJPEBV-$TMilRect_@HUtagRECT@@UMilPointAndSizeL@@U_CMilRectL_@Re.c)
 *     ?MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJI@Z @ 0x1800C7F7C (-MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJI@Z.c)
 *     memset_0 @ 0x1800EC516 (memset_0.c)
 *     _guard_dispatch_icall_nop @ 0x1800F00A0 (_guard_dispatch_icall_nop.c)
 */

__int64 __fastcall CRgnGeometry::GetShapeDataCore(
        CRgnGeometry *this,
        const struct D2D_SIZE_F *a2,
        struct CShapePtr *a3)
{
  unsigned int v4; // ebx
  _QWORD *v6; // rax
  unsigned int v7; // ecx
  _QWORD *v8; // rdi
  __int64 v9; // rdx
  __int64 v11; // rcx

  v4 = 0;
  v6 = operator new(0x60uLL);
  v8 = v6;
  if ( v6 )
  {
    memset_0(v6, 0, 0x60uLL);
    v8[1] = 0LL;
    *v8 = &CRegionShape::`vftable';
    v8[2] = v8 + 3;
    *((_DWORD *)v8 + 6) = 0;
    v8[11] = 0LL;
  }
  else
  {
    v8 = 0LL;
  }
  if ( v8 )
  {
    v9 = *((_QWORD *)this + 18);
    if ( v9 )
    {
      CRegionShape::BuildFromRects(v8, v9);
    }
    else
    {
      *(_DWORD *)v8[2] = 0;
      v11 = v8[11];
      v8[11] = 0LL;
      if ( v11 )
        (*(void (__fastcall **)(__int64))(*(_QWORD *)v11 + 16LL))(v11);
    }
    CShapePtr::Release(a3);
    *(_QWORD *)a3 = v8;
    *((_BYTE *)a3 + 8) = 1;
  }
  else
  {
    v4 = -2147024882;
    MilInstrumentationCheckHR_MaybeFailFast(v7, 0LL, 0, -2147024882, 0x29u);
  }
  return v4;
}
