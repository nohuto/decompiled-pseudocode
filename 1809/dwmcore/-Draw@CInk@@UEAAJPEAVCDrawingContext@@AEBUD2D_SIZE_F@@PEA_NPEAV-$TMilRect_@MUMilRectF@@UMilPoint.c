/*
 * XREFs of ?Draw@CInk@@UEAAJPEAVCDrawingContext@@AEBUD2D_SIZE_F@@PEA_NPEAV?$TMilRect_@MUMilRectF@@UMilPointAndSizeF@@UNotNeeded@RectUniqueness@@@@@Z @ 0x18019B020
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall CInk::Draw(__int64 a1, struct ID2DContext **a2)
{
  ...
  if ( D2DInk < 0 )
  {
    MilInstrumentationCheckHR_MaybeFailFast(v5, 0LL, 0, D2DInk, 0xFEu);
  }
  else
  {
    ...
    v7 = v9;
    if ( v9 < 0 )
      MilInstrumentationCheckHR_MaybeFailFast(v10, 0LL, 0, v9, 0x100u);
  }
  if ( v6 )
