/*
 * XREFs of HUBREG_QueryUsbflagsValuesForDevice @ 0x1C007378C
 * Callers:
 *     HUBMISC_QueryAndCacheRegistryValuesForDevice @ 0x1C0072564 (HUBMISC_QueryAndCacheRegistryValuesForDevice.c)
 * Callees:
 *     RtlStringCchPrintfA @ 0x1C0001008 (RtlStringCchPrintfA.c)
 *     WPP_RECORDER_SF_d @ 0x1C0001A3C (WPP_RECORDER_SF_d.c)
 *     RtlUnicodeStringPrintf @ 0x1C0010EB4 (RtlUnicodeStringPrintf.c)
 *     HUBMISC_QueryKseDeviceFlags @ 0x1C002C6E8 (HUBMISC_QueryKseDeviceFlags.c)
 *     HUBREG_QueryUsbflagsAlternateSettingFilter @ 0x1C002DB44 (HUBREG_QueryUsbflagsAlternateSettingFilter.c)
 *     __security_check_cookie @ 0x1C003C520 (__security_check_cookie.c)
 *     _guard_dispatch_icall_nop @ 0x1C003C610 (_guard_dispatch_icall_nop.c)
 *     HUBREG_OpenCreateUsbflagsDeviceKey @ 0x1C0072CE8 (HUBREG_OpenCreateUsbflagsDeviceKey.c)
 */

__int64 __fastcall HUBREG_QueryUsbflagsValuesForDevice(__int64 a1, __int64 a2, __int64 a3, __int64 a4)
{
  __int64 v4; // rbx
  char v9; // r13
  __int64 v10; // rax
  int v11; // esi
  int v12; // r9d
  wchar_t *v13; // r10
  __int64 v14; // rbx
  NTSTATUS v15; // eax
  unsigned __int16 v16; // r9
  bool v17; // zf
  char v18; // al
  __int64 v20; // [rsp+28h] [rbp-D8h]
  int v21; // [rsp+80h] [rbp-80h] BYREF
  __int64 v22; // [rsp+88h] [rbp-78h] BYREF
  char pszDest[8]; // [rsp+90h] [rbp-70h] BYREF
  __int64 v24; // [rsp+98h] [rbp-68h] BYREF
  __int64 v25; // [rsp+A0h] [rbp-60h] BYREF
  __int64 v26; // [rsp+A8h] [rbp-58h] BYREF
  __int64 v27; // [rsp+B0h] [rbp-50h] BYREF
  __int64 v28; // [rsp+B8h] [rbp-48h] BYREF
  __int64 v29; // [rsp+C0h] [rbp-40h] BYREF
  __int64 v30; // [rsp+C8h] [rbp-38h] BYREF
  __int64 v31; // [rsp+D0h] [rbp-30h] BYREF
  __int64 v32; // [rsp+D8h] [rbp-28h] BYREF
  struct _UNICODE_STRING DestinationString; // [rsp+E0h] [rbp-20h] BYREF
  char v34; // [rsp+F0h] [rbp-10h] BYREF

  v4 = *(_QWORD *)a1;
  v29 = 0LL;
  v9 = *(_BYTE *)(v4 + 200);
  DestinationString.Buffer = (wchar_t *)&v34;
  v10 = *(_QWORD *)(a1 + 8);
  v22 = 0LL;
  v24 = 0LL;
  *(_DWORD *)&DestinationString.Length = 6291456;
  HUBREG_OpenCreateUsbflagsDeviceKey(a2, a3, a4, 0x20019u, &v29, &v22, 0, *(_QWORD *)(v10 + 1432));
  v11 = HUBREG_OpenCreateUsbflagsDeviceKey(
          a2,
          a3,
          a4,
          0x20019u,
          0LL,
          &v24,
          1,
          *(_QWORD *)(*(_QWORD *)(a1 + 8) + 1432LL));
  if ( v11 < 0 )
    goto LABEL_150;
  v12 = *(unsigned __int8 *)(a1 + 1992);
  v31 = 0LL;
  v27 = 0LL;
  v28 = 0LL;
  v25 = 0LL;
  v26 = 0LL;
  v30 = 0LL;
  RtlStringCchPrintfA(pszDest, 3uLL, "%02X", v12);
  if ( *(_DWORD *)(v4 + 168) == 3 && (v13 = *(wchar_t **)(v4 + 176)) != 0LL )
    HUBMISC_QueryKseDeviceFlags(
      (__int64)pszDest,
      a2,
      a3,
      a4,
      v13,
      *(_WORD **)(v4 + 184),
      *(_WORD **)(v4 + 192),
      &v31,
      &v27,
      &v28,
      &v25,
      &v26,
      &v30,
      0,
      *(_QWORD *)(*(_QWORD *)(a1 + 8) + 1432LL));
  else
    HUBMISC_QueryKseDeviceFlags(
      (__int64)pszDest,
      a2,
      a3,
      a4,
      0LL,
      0LL,
      0LL,
      &v31,
      &v27,
      &v28,
      &v25,
      &v26,
      &v30,
      0,
      *(_QWORD *)(*(_QWORD *)(a1 + 8) + 1432LL));
  v14 = v31 | v27 | v28 | v25 | v26 | v30;
  if ( v29 )
  {
    v15 = RtlUnicodeStringPrintf(
            &DestinationString,
            L"IgnoreHWSerNum%04X%04X",
            *(unsigned __int16 *)(a1 + 1996),
            *(unsigned __int16 *)(a1 + 1998));
    v11 = v15;
    if ( v15 < 0 )
    {
      v16 = 25;
LABEL_9:
      LODWORD(v20) = v15;
LABEL_149:
      WPP_RECORDER_SF_d(
        *(_QWORD *)(*(_QWORD *)(a1 + 8) + 1432LL),
        2u,
        5u,
        v16,
        (__int64)&WPP_a5d7102001b13a39eb4d855100c5c7b8_Traceguids,
        v20);
      goto LABEL_150;
    }
    v21 = 0;
    v15 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, struct _UNICODE_STRING *, __int64, int *, _QWORD, _QWORD))(WdfFunctions_01015 + 1880))(
            WdfDriverGlobals,
            v29,
            &DestinationString,
            4LL,
            &v21,
            0LL,
            0LL);
    v11 = v15;
    if ( v15 < 0 )
    {
      if ( v15 != -1073741772 )
      {
        v16 = 26;
        goto LABEL_9;
      }
    }
    else if ( v21 )
    {
      _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 1u);
    }
  }
  v21 = 0;
  v11 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, const wchar_t *, __int64, int *, _QWORD, _QWORD))(WdfFunctions_01015 + 1880))(
          WdfDriverGlobals,
          v24,
          L"\b\n",
          2LL,
          &v21,
          0LL,
          0LL);
  if ( v11 < 0 )
  {
    if ( v11 != -1073741772 )
    {
      v16 = 27;
      LODWORD(v20) = v11;
      goto LABEL_149;
    }
    if ( (v14 & 1) == 0 )
    {
      if ( (v25 & 2) != 0 || (v26 & 2) != 0 || (v27 & 2) != 0 || (v28 & 2) != 0 )
        _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 2u);
      goto LABEL_26;
    }
  }
  else if ( v21 )
  {
    *(_BYTE *)(a1 + 2052) = BYTE1(v21);
    goto LABEL_26;
  }
  _InterlockedOr((volatile signed __int32 *)(a1 + 1632), 0x80u);
LABEL_26:
  v21 = 0;
  if ( !v22 )
    goto LABEL_33;
  v15 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, void *, __int64, int *, _QWORD, _QWORD))(WdfFunctions_01015 + 1880))(
          WdfDriverGlobals,
          v22,
          &g_IgnoreHwSerialNumber,
          4LL,
          &v21,
          0LL,
          0LL);
  v11 = v15;
  if ( v15 >= 0 && v21 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 1u);
  if ( v15 == -1073741772 )
  {
LABEL_33:
    if ( (v14 & 0x40) != 0 )
      _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 1u);
  }
  else if ( v15 < 0 )
  {
    v16 = 28;
    goto LABEL_9;
  }
  v21 = 0;
  if ( !v22 )
    goto LABEL_42;
  v15 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, const wchar_t *, __int64, int *, _QWORD, _QWORD))(WdfFunctions_01015 + 1880))(
          WdfDriverGlobals,
          v22,
          L"68",
          4LL,
          &v21,
          0LL,
          0LL);
  v11 = v15;
  if ( v15 >= 0 && v21 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x200000u);
  if ( v15 == -1073741772 )
  {
LABEL_42:
    if ( (v14 & 0x80000000) != 0 )
      _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x200000u);
  }
  else if ( v15 < 0 )
  {
    v16 = 29;
    goto LABEL_9;
  }
  v21 = 0;
  if ( !v22 )
    goto LABEL_51;
  v15 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, void *, __int64, int *, _QWORD, _QWORD))(WdfFunctions_01015 + 1880))(
          WdfDriverGlobals,
          v22,
          &g_ResetOnResume,
          4LL,
          &v21,
          0LL,
          0LL);
  v11 = v15;
  if ( v15 >= 0 && v21 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 4u);
  if ( v15 == -1073741772 )
  {
LABEL_51:
    if ( (v14 & 4) != 0 )
      _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 4u);
  }
  else if ( v15 < 0 )
  {
    v16 = 30;
    goto LABEL_9;
  }
  v21 = 0;
  _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 8u);
  if ( !v22 )
    goto LABEL_60;
  v15 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, const wchar_t *, __int64, int *, _QWORD, _QWORD))(WdfFunctions_01015 + 1880))(
          WdfDriverGlobals,
          v22,
          L"&(",
          4LL,
          &v21,
          0LL,
          0LL);
  v11 = v15;
  if ( v15 >= 0 && !v21 )
    _InterlockedAnd((volatile signed __int32 *)(a1 + 1644), 0xFFFFFFF7);
  if ( v15 == -1073741772 )
  {
LABEL_60:
    if ( (v14 & 8) != 0 )
      _InterlockedAnd((volatile signed __int32 *)(a1 + 1644), 0xFFFFFFF7);
  }
  else if ( v15 < 0 )
  {
    v16 = 31;
    goto LABEL_9;
  }
  v21 = 0;
  if ( !v22 )
    goto LABEL_69;
  v15 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, const wchar_t *, __int64, int *, _QWORD, _QWORD))(WdfFunctions_01015 + 1880))(
          WdfDriverGlobals,
          v22,
          L"02",
          4LL,
          &v21,
          0LL,
          0LL);
  v11 = v15;
  if ( v15 >= 0 && v21 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x10u);
  if ( v15 == -1073741772 )
  {
LABEL_69:
    if ( (v14 & 0x10) != 0 )
      _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x10u);
  }
  else if ( v15 < 0 )
  {
    v16 = 32;
    goto LABEL_9;
  }
  v21 = 0;
  if ( !v22 )
    goto LABEL_78;
  v15 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, const wchar_t *, __int64, int *, _QWORD, _QWORD))(WdfFunctions_01015 + 1880))(
          WdfDriverGlobals,
          v22,
          L":<",
          4LL,
          &v21,
          0LL,
          0LL);
  v11 = v15;
  if ( v15 >= 0 && v21 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x800000u);
  if ( v15 == -1073741772 )
  {
LABEL_78:
    if ( (v14 & 0x1000000000LL) != 0 )
      _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x800000u);
  }
  else if ( v15 < 0 )
  {
    v16 = 33;
    goto LABEL_9;
  }
  v21 = 0;
  v15 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, const wchar_t *, __int64, int *, _QWORD, _QWORD))(WdfFunctions_01015 + 1880))(
          WdfDriverGlobals,
          v24,
          L"(*",
          4LL,
          &v21,
          0LL,
          0LL);
  v11 = v15;
  if ( v15 < 0 )
  {
    if ( v15 != -1073741772 )
    {
      v16 = 34;
      goto LABEL_9;
    }
    v17 = (v14 & 0x20) == 0;
  }
  else
  {
    v17 = v21 == 0;
  }
  if ( !v17 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x20u);
  v21 = 0;
  if ( !v22 )
    goto LABEL_93;
  v15 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, void *, __int64, int *, _QWORD, _QWORD))(WdfFunctions_01015 + 1880))(
          WdfDriverGlobals,
          v22,
          &g_DisableLpm,
          4LL,
          &v21,
          0LL,
          0LL);
  v11 = v15;
  if ( v15 >= 0 && v21 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x80u);
  if ( v15 == -1073741772 )
  {
LABEL_93:
    if ( (v14 & 0x1000) != 0 )
      _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x80u);
  }
  else if ( v15 < 0 )
  {
    v16 = 35;
    goto LABEL_9;
  }
  if ( (v14 & 0x400) != 0 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x40u);
  if ( (v14 & 0x4000) != 0 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x100u);
  if ( (v14 & 0x10000) != 0 && *(_BYTE *)(*(_QWORD *)a1 + 240LL) )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x80u);
  if ( (v14 & 0x80000) != 0 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x400u);
  if ( (v14 & 0x200000) != 0 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x800u);
  if ( (v14 & 0x800000) != 0 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x1000u);
  if ( (v14 & 0x1000000) != 0 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x2000u);
  v21 = 0;
  if ( !v22 )
    goto LABEL_117;
  v15 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, const wchar_t *, __int64, int *, _QWORD, _QWORD))(WdfFunctions_01015 + 1880))(
          WdfDriverGlobals,
          v22,
          L",.",
          4LL,
          &v21,
          0LL,
          0LL);
  v11 = v15;
  if ( v15 >= 0 && v21 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x8000u);
  if ( v15 == -1073741772 )
  {
LABEL_117:
    if ( (v14 & 0x8000000) != 0 )
      _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x8000u);
  }
  else if ( v15 < 0 )
  {
    v16 = 36;
    goto LABEL_9;
  }
  if ( (v14 & 0x2000) != 0 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x20000u);
  if ( (v14 & 0x20000) != 0 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x40000u);
  if ( (v14 & 0x40000000) != 0 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x100000u);
  if ( ((v14 & 0x400000) != 0 || (v14 & 0x4000000000LL) != 0 && v9) && (*(_DWORD *)(a1 + 1632) & 2) == 0 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x80000u);
  if ( (v14 & 0x100000000LL) != 0 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x400000u);
  if ( (v14 & 0x2000000000LL) != 0 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x1000000u);
  if ( (v14 & 0x80000000000LL) != 0 )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x4000000u);
  v32 = 0LL;
  v15 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, const wchar_t *, __int64, __int64 *, _QWORD, _QWORD))(WdfFunctions_01015 + 1880))(
          WdfDriverGlobals,
          v24,
          L".0",
          8LL,
          &v32,
          0LL,
          0LL);
  v11 = v15;
  if ( v15 < 0 )
  {
    if ( v15 != -1073741772 )
    {
      v16 = 37;
      goto LABEL_9;
    }
  }
  else
  {
    _InterlockedOr((volatile signed __int32 *)(a1 + 2468), 4u);
    v18 = BYTE6(v32);
    *(_DWORD *)(a1 + 2464) |= 4u;
    *(_BYTE *)(a1 + 2052) = v18;
    *(_QWORD *)(a1 + 2480) = v32;
  }
  if ( *(_WORD *)(a1 + 1996) == 8457 && *(_WORD *)(a1 + 1998) == 2064 && (unsigned __int8)*(_WORD *)(a1 + 2000) < 0x89u )
    _InterlockedOr((volatile signed __int32 *)(a1 + 1644), 0x10000u);
  if ( v22 )
    HUBREG_QueryUsbflagsAlternateSettingFilter(a1, v22);
  v11 = 0;
LABEL_150:
  if ( v22 )
    (*(void (__fastcall **)(PWDF_DRIVER_GLOBALS))(WdfFunctions_01015 + 1848))(WdfDriverGlobals);
  if ( v29 )
    (*(void (__fastcall **)(PWDF_DRIVER_GLOBALS))(WdfFunctions_01015 + 1848))(WdfDriverGlobals);
  if ( v24 )
    (*(void (__fastcall **)(PWDF_DRIVER_GLOBALS))(WdfFunctions_01015 + 1848))(WdfDriverGlobals);
  return (unsigned int)v11;
}
