/*
 * XREFs of NtFlipObjectRemoveContent @ 0x1C0058BC0
 * Callers:
 *     <none>
 * Callees:
 *     ?RemoveContent@FlipManagerObject@@QEAAJ_K@Z @ 0x1C0057910 (-RemoveContent@FlipManagerObject@@QEAAJ_K@Z.c)
 *     ?ResolveHandle@FlipManagerObject@@KAJPEAXKDPEAPEAU1@@Z @ 0x1C00595CC (-ResolveHandle@FlipManagerObject@@KAJPEAXKDPEAPEAU1@@Z.c)
 */

__int64 __fastcall NtFlipObjectRemoveContent(void *a1, unsigned __int64 *a2)
{
  unsigned __int64 v3; // r14
  char v4; // r8
  int v5; // ebx
  FlipManagerObject *v6; // rdi
  FlipManagerObject *v8; // [rsp+60h] [rbp+18h] BYREF

  if ( a1 && a2 )
  {
    v8 = 0LL;
    if ( a2 + 1 < a2 || (unsigned __int64)(a2 + 1) > MmUserProbeAddress )
      a2 = (unsigned __int64 *)MmUserProbeAddress;
    v3 = *a2;
    KeEnterCriticalRegion();
    v5 = FlipManagerObject::ResolveHandle(a1, 2u, v4, &v8);
    v6 = v8;
    if ( v5 >= 0 )
      v5 = FlipManagerObject::RemoveContent(v8, v3);
    if ( v6 )
      ObfDereferenceObject(v6);
    KeLeaveCriticalRegion();
  }
  else
  {
    return (unsigned int)-1073741811;
  }
  return (unsigned int)v5;
}
