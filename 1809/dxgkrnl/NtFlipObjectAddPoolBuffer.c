/*
 * XREFs of NtFlipObjectAddPoolBuffer @ 0x1C0057BC0
 * Callers:
 *     <none>
 * Callees:
 *     ??$CreateFlipPropertySetWorker@VCFlipPropertySet@@@@YAJIPEAUFlipPropertyItem@@_NPEAPEAVCFlipPropertySet@@@Z @ 0x1C0056D84 (--$CreateFlipPropertySetWorker@VCFlipPropertySet@@@@YAJIPEAUFlipPropertyItem@@_NPEAPEAVCFlipProp.c)
 *     ?AddPoolBuffer@FlipManagerObject@@QEAAJ_KPEAXAEBU_LUID@@PEAVCFlipPropertySet@@@Z @ 0x1C005744C (-AddPoolBuffer@FlipManagerObject@@QEAAJ_KPEAXAEBU_LUID@@PEAVCFlipPropertySet@@@Z.c)
 *     ?Release@CFlipPropertySetBase@@QEAAKXZ @ 0x1C00578E0 (-Release@CFlipPropertySetBase@@QEAAKXZ.c)
 *     ?ResolveHandle@FlipManagerObject@@KAJPEAXKDPEAPEAU1@@Z @ 0x1C00595CC (-ResolveHandle@FlipManagerObject@@KAJPEAXKDPEAPEAU1@@Z.c)
 */

__int64 __fastcall NtFlipObjectAddPoolBuffer(
        void *a1,
        struct FlipManagerObject **a2,
        void *a3,
        struct _LUID *a4,
        unsigned int a5,
        char *a6)
{
  struct FlipManagerObject *v8; // rdi
  struct FlipManagerObject *v9; // r14
  __int64 v10; // r8
  int v11; // ebx
  char v12; // r8
  struct CFlipPropertySet *v13; // rsi
  struct CFlipPropertySet *v15; // [rsp+30h] [rbp-48h] BYREF
  struct FlipManagerObject *v16[2]; // [rsp+38h] [rbp-40h] BYREF
  struct _LUID v17; // [rsp+48h] [rbp-30h] BYREF

  if ( a1 && a2 && a3 && a4 )
  {
    v15 = 0LL;
    v8 = 0LL;
    v16[0] = 0LL;
    if ( &a4[1] < a4 || (unsigned __int64)&a4[1] > MmUserProbeAddress )
      a4 = (struct _LUID *)MmUserProbeAddress;
    v17 = *a4;
    if ( a2 + 1 < a2 || (unsigned __int64)(a2 + 1) > MmUserProbeAddress )
      a2 = (struct FlipManagerObject **)MmUserProbeAddress;
    v9 = *a2;
    v16[1] = *a2;
    KeEnterCriticalRegion();
    v11 = CreateFlipPropertySetWorker<CFlipPropertySet>(a5, a6, v10, &v15);
    v13 = v15;
    if ( v11 >= 0 )
    {
      v11 = FlipManagerObject::ResolveHandle(a1, 2u, v12, v16);
      v8 = v16[0];
      if ( v11 >= 0 )
        v11 = FlipManagerObject::AddPoolBuffer(v16[0], (unsigned __int64)v9, a3, &v17, v13);
    }
    if ( v13 )
      CFlipPropertySetBase::Release(v13);
    if ( v8 )
      ObfDereferenceObject(v8);
    KeLeaveCriticalRegion();
  }
  else
  {
    return (unsigned int)-1073741811;
  }
  return (unsigned int)v11;
}
