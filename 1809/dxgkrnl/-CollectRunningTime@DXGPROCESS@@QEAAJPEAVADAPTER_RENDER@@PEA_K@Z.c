/*
 * XREFs of ?CollectRunningTime@DXGPROCESS@@QEAAJPEAVADAPTER_RENDER@@PEA_K@Z @ 0x1C0228B38
 * Callers:
 *     ?ApplyCsFunctionAdapterCallback@@YAJPEAVDXGADAPTER@@PEAX@Z @ 0x1C0228700 (-ApplyCsFunctionAdapterCallback@@YAJPEAVDXGADAPTER@@PEAX@Z.c)
 * Callees:
 *     __security_check_cookie @ 0x1C0020760 (__security_check_cookie.c)
 *     ?VidSchQueryProcessAdapterStatistics@VIDSCH_EXPORT@@QEAAJPEAU_VIDSCH_GLOBAL@@PEAVDXGPROCESS@@PEAU_D3DKMT_QUERYSTATISTICS_PROCESS_ADAPTER_INFORMATION@@@Z @ 0x1C0034F04 (-VidSchQueryProcessAdapterStatistics@VIDSCH_EXPORT@@QEAAJPEAU_VIDSCH_GLOBAL@@PEAVDXGPROCESS@@PEA.c)
 *     ?VidSchQueryProcessNodeStatistics@VIDSCH_EXPORT@@QEAAJPEAU_VIDSCH_GLOBAL@@IPEAVDXGPROCESS@@PEAU_D3DKMT_QUERYSTATISTICS_PROCESS_NODE_INFORMATION@@@Z @ 0x1C0034F90 (-VidSchQueryProcessNodeStatistics@VIDSCH_EXPORT@@QEAAJPEAU_VIDSCH_GLOBAL@@IPEAVDXGPROCESS@@PEAU_.c)
 */

__int64 __fastcall DXGPROCESS::CollectRunningTime(DXGPROCESS *this, struct ADAPTER_RENDER *a2, unsigned __int64 *a3)
{
  VIDSCH_EXPORT *v3; // rsi
  struct _VIDSCH_GLOBAL *v5; // r14
  __int64 result; // rax
  unsigned int v8; // ebx
  _BYTE v9[4]; // [rsp+30h] [rbp-258h] BYREF
  unsigned int v10; // [rsp+34h] [rbp-254h]
  _QWORD v11[34]; // [rsp+140h] [rbp-148h] BYREF

  v3 = (VIDSCH_EXPORT *)*((_QWORD *)a2 + 65);
  v5 = (struct _VIDSCH_GLOBAL *)*((_QWORD *)a2 + 66);
  *a3 = 0LL;
  if ( !v3 )
    return 3221225858LL;
  result = VIDSCH_EXPORT::VidSchQueryProcessAdapterStatistics(
             v3,
             v5,
             this,
             (struct _D3DKMT_QUERYSTATISTICS_PROCESS_ADAPTER_INFORMATION *)v9);
  if ( (int)result >= 0 )
  {
    v8 = 0;
    if ( v10 )
    {
      while ( 1 )
      {
        result = VIDSCH_EXPORT::VidSchQueryProcessNodeStatistics(
                   v3,
                   v5,
                   v8,
                   this,
                   (struct _D3DKMT_QUERYSTATISTICS_PROCESS_NODE_INFORMATION *)v11);
        if ( (int)result < 0 )
          break;
        ++v8;
        *a3 += v11[0];
        if ( v8 >= v10 )
          return 0LL;
      }
    }
    else
    {
      return 0LL;
    }
  }
  return result;
}
