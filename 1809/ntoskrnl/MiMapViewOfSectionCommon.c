/*
 * XREFs of MiMapViewOfSectionCommon @ 0x1406783B0
 * Callers:
 *     MiMapViewOfSectionExCommon @ 0x140677DD0 (MiMapViewOfSectionExCommon.c)
 *     NtMapViewOfSection @ 0x140678170 (NtMapViewOfSection.c)
 * Callees:
 *     ObfDereferenceObject @ 0x14004E150 (ObfDereferenceObject.c)
 *     ObfDereferenceObjectWithTag @ 0x140051510 (ObfDereferenceObjectWithTag.c)
 *     MiMakeProtectionMask @ 0x140074490 (MiMakeProtectionMask.c)
 *     memset @ 0x1401D1980 (memset.c)
 *     ObReferenceObjectByHandle @ 0x1405E9350 (ObReferenceObjectByHandle.c)
 *     ObpReferenceObjectByHandleWithTag @ 0x1405E9390 (ObpReferenceObjectByHandleWithTag.c)
 *     ExRaiseDatatypeMisalignment @ 0x1408D7880 (ExRaiseDatatypeMisalignment.c)
 */

__int64 __fastcall MiMapViewOfSectionCommon(
        ULONG_PTR BugCheckParameter1,
        void *a2,
        int a3,
        PVOID *a4,
        unsigned __int64 a5,
        unsigned __int64 a6,
        int a7,
        __int64 a8,
        char a9,
        PVOID *a10)
{
  int ProtectionMask; // eax
  PVOID *v15; // r15
  char v16; // si
  __int64 result; // rax
  NTSTATUS v18; // r12d
  __int64 v19; // rdx
  PVOID *v20; // rdx
  __int64 v21; // r8
  __int64 v22; // r8
  PVOID v23; // rax
  unsigned __int64 v24; // rdx
  PVOID Object; // [rsp+48h] [rbp-40h] BYREF

  memset(a10, 0, 0x30uLL);
  ProtectionMask = MiMakeProtectionMask(a7 & 0xBFFFFFFF);
  *((_DWORD *)a10 + 6) = ProtectionMask;
  if ( ProtectionMask == -1 )
    return 3221225541LL;
  *((_DWORD *)a10 + 6) = ProtectionMask & 7;
  v15 = a10 + 5;
  if ( a3 )
  {
    *v15 = (PVOID)BugCheckParameter1;
    a10[4] = a2;
    v16 = a9;
  }
  else
  {
    v16 = a9;
    result = ObpReferenceObjectByHandleWithTag(
               BugCheckParameter1,
               8,
               (__int64)PsProcessType,
               a9,
               0x77566D4Du,
               a10 + 5,
               0LL,
               0LL);
    if ( (int)result < 0 )
      return result;
    v18 = ObReferenceObjectByHandle(
            a2,
            MmMakeSectionAccess[*((unsigned int *)a10 + 6)],
            MmSectionObjectType,
            a9,
            &Object,
            0LL);
    a10[4] = Object;
    if ( v18 < 0 )
    {
      ObfDereferenceObjectWithTag(*v15, 0x77566D4Du);
      return (unsigned int)v18;
    }
  }
  if ( v16 )
  {
    v19 = (__int64)a4;
    if ( (unsigned __int64)a4 >= 0x7FFFFFFF0000LL )
      v19 = 0x7FFFFFFF0000LL;
    *(_QWORD *)v19 = *(_QWORD *)v19;
    v20 = (PVOID *)a5;
    v21 = a5;
    if ( a5 >= 0x7FFFFFFF0000LL )
      v21 = 0x7FFFFFFF0000LL;
    *(_QWORD *)v21 = *(_QWORD *)v21;
  }
  else
  {
    v20 = (PVOID *)a5;
  }
  *a10 = *a4;
  a10[1] = *v20;
  if ( a6 )
  {
    if ( v16 )
    {
      v22 = a6;
      if ( (a6 & 3) != 0 )
        ExRaiseDatatypeMisalignment();
      if ( a6 >= 0x7FFFFFFF0000LL )
        v22 = 0x7FFFFFFF0000LL;
      *(_BYTE *)v22 = *(_BYTE *)v22;
      *(_BYTE *)(v22 + 7) = *(_BYTE *)(v22 + 7);
    }
    a10[2] = *(PVOID *)a6;
  }
  else
  {
    a10[2] = 0LL;
  }
  v23 = *a10;
  if ( (unsigned __int64)*a10 <= 0x7FFFFFFEFFFFLL )
  {
    v24 = (unsigned __int64)a10[1];
    if ( v24 <= 0x7FFFFFFF0000LL - (__int64)v23 && (unsigned __int64)v23 + v24 <= 0xFFFFFFFFFFFFFFFFuLL >> a8 )
      return 0LL;
  }
  if ( !a3 )
  {
    ObfDereferenceObject(a10[4]);
    ObfDereferenceObjectWithTag(a10[5], 0x77566D4Du);
  }
  return 3221225485LL;
}
