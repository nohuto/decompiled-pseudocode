...
__int64 __fastcall IoGetDeviceDirectory(__int64 a1, int a2, int a3, __int64 a4, __int64 *a5)
{
  ...
  UNICODE_STRING v29; // [rsp+80h] [rbp-39h] BYREF
  ...
  UUID SourceString; // [rsp+C8h] [rbp+Fh] BYREF
  ...
  *(_QWORD *)&SourceString.Data1 = 0LL;
  ...
  if ( !a2 )
  {
    StateRootPath = PiGetStateRootPath(L"DriverState", L"\\SystemRoot\\System32\\DriverState", 1u, &DestinationString);
    if ( StateRootPath >= 0 )
    {
...
LABEL_34:
        if ( StateRootPath >= 0 )
        {
          ...
          if ( v23 == -1073741766 )
          {
            ...
            if ( StateRootPath < 0 )
              goto LABEL_45;
            StateRootPath = RtlStringFromGUIDEx(&SourceString.Data1, (__int64)&UnicodeString, 1);
            if ( StateRootPath < 0 )
              goto LABEL_45;
