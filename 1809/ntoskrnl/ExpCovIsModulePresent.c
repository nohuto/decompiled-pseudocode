/*
 * XREFs of ExpCovIsModulePresent @ 0x1408D7614
 * Callers:
 *     ExpCovIsLoadedModulePresent @ 0x1408D75DC (ExpCovIsLoadedModulePresent.c)
 *     ExpCovIsUnLoadedModulePresent @ 0x1408D76B8 (ExpCovIsUnLoadedModulePresent.c)
 * Callees:
 *     RtlCompareMemory @ 0x1401C5BB0 (RtlCompareMemory.c)
 *     RtlCompareUnicodeString @ 0x1405BFF20 (RtlCompareUnicodeString.c)
 *     RtlFreeAnsiString @ 0x140623790 (RtlFreeAnsiString.c)
 *     ExpCovReadFriendlyName @ 0x1408D7FF8 (ExpCovReadFriendlyName.c)
 */

bool __fastcall ExpCovIsModulePresent(__int64 a1, __int64 a2, int a3, const UNICODE_STRING *a4, void *Source1)
{
  char v6; // bl
  int v7; // r8d
  UNICODE_STRING String2; // [rsp+20h] [rbp-18h] BYREF

  String2.Buffer = 0LL;
  *(_DWORD *)&String2.Length = 0;
  v6 = 0;
  if ( !a3 )
    goto LABEL_6;
  v7 = a3 - 1;
  if ( !v7 )
    return RtlCompareMemory(Source1, (const void *)(a2 + 8), 0x10uLL) == 16;
  if ( v7 == 1 )
  {
    if ( (int)ExpCovReadFriendlyName(a2, a1, &String2) < 0 || RtlCompareUnicodeString(a4, &String2, 1u) )
    {
LABEL_7:
      if ( String2.Buffer )
        RtlFreeAnsiString(&String2);
      return v6;
    }
LABEL_6:
    v6 = 1;
    goto LABEL_7;
  }
  return v6;
}
