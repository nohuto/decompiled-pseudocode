/*
 * XREFs of PopDiagTracePowerAggregatorValidationEvent @ 0x140874D84
 * Callers:
...
char __fastcall PopDiagTracePowerAggregatorValidationEvent(int a1, int a2, int a3)
{
  _UNKNOWN **v3; // rax
  REGHANDLE v4; // rbx
  ...
  {
    v4 = PopDiagHandle;
    LOBYTE(v3) = EtwEventEnabled(PopDiagHandle, &POP_ETW_EVENT_POWER_AGGREGATOR_VALIDATION_EVENT);
    if ( (_BYTE)v3 )
    {
      UserData.Reserved = 0;
      v9 = 0;
      ...
      v8 = 4;
      v11 = 4;
      LOBYTE(v3) = EtwWrite(v4, &POP_ETW_EVENT_POWER_AGGREGATOR_VALIDATION_EVENT, 0LL, 3u, &UserData);
    }
  }
  return (char)v3;
}
