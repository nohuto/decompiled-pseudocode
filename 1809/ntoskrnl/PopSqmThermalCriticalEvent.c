/*
 * XREFs of PopSqmThermalCriticalEvent @ 0x14087C6D0
 * Callers:
 *     PopSqmThermalCriticalShutdown @ 0x14087C8A4 (PopSqmThermalCriticalShutdown.c)
 *     PopSqmThermalHibernate @ 0x14087C8C4 (PopSqmThermalHibernate.c)
 * Callees:
...
char __fastcall PopSqmThermalCriticalEvent(__int64 a1, unsigned int a2, int a3)
{
  ...
  if ( v8 )
  {
    DeviceAttachmentBaseRefWithTag = *(void **)(a1 + 864);
    if ( pCallbackContext.LevelPlus1 > 5 )
    {
      LOBYTE(DeviceAttachmentBaseRefWithTag) = TlgKeywordOn(&pCallbackContext, 0x800000000000uLL);
      if ( (_BYTE)DeviceAttachmentBaseRefWithTag )
      {
        ...
        LOBYTE(DeviceAttachmentBaseRefWithTag) = TlgWrite(&pCallbackContext, &unk_1403713C6, 0LL, 0LL, 0xBu, &pData);
      }
    }
  }
  if ( v7 )
