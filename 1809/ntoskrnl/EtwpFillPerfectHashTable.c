/*
 * XREFs of EtwpFillPerfectHashTable @ 0x1408C3438
 * Callers:
 *     EtwpCreatePerfectHashFunction @ 0x1408C321C (EtwpCreatePerfectHashFunction.c)
 * Callees:
 *     <none>
 */

__int64 __fastcall EtwpFillPerfectHashTable(
        _WORD *a1,
        unsigned __int16 a2,
        char a3,
        unsigned __int16 a4,
        __int64 a5,
        unsigned __int16 *a6)
{
  __int64 v7; // rdi
  __int64 v10; // rax
  __int64 v11; // r8
  unsigned __int16 v12; // bx
  unsigned __int16 v13; // r11
  __int16 v14; // r9
  unsigned __int16 v15; // r8
  unsigned __int16 v16; // dx
  __int64 v17; // rax
  unsigned __int16 v18; // r8
  unsigned __int16 v19; // dx
  unsigned int v20; // ebp
  unsigned __int16 v21; // cx
  unsigned __int16 i; // dx
  __int64 v23; // r9
  unsigned __int16 v24; // cx
  unsigned __int16 v26; // [rsp+38h] [rbp+10h]

  LOWORD(v7) = 0;
  v10 = a5;
  *a6 = 0;
  v11 = 128LL;
  v12 = a4 + 1;
  v26 = a4 + 1;
  v13 = a4 + 1;
  do
  {
    *(_WORD *)(v10 + 2) = -1;
    *(_BYTE *)v10 = -1;
    v10 += 4LL;
    --v11;
  }
  while ( v11 );
  if ( a2 )
  {
    v7 = a2;
    do
    {
      v14 = *a1;
      v15 = 0;
      v16 = a4 & __ROR2__(*a1, a3);
      while ( *(_WORD *)(a5 + 4LL * v16 + 2) != 0xFFFF )
      {
        v17 = v16;
        if ( *(_WORD *)(a5 + 4LL * v16 + 2) == v14 )
          break;
        v16 = *(unsigned __int8 *)(a5 + 4LL * v16);
        ++v15;
        if ( v16 == 255 )
        {
          v16 = v13++;
          *(_BYTE *)(a5 + 4 * v17) = v16;
        }
        if ( v15 > *a6 )
          *a6 = v15;
      }
      ++a1;
      *(_WORD *)(a5 + 4LL * v16 + 2) = v14;
      --v7;
    }
    while ( v7 );
    v12 = v26;
  }
  v18 = v13 - 1;
  v19 = v7;
  v20 = a4 + 1;
  while ( v18 > v20 && v19 < a4 )
  {
    if ( *(_BYTE *)(a5 + 4LL * v18) == 0xFF )
    {
      do
      {
        if ( *(_WORD *)(a5 + 4LL * v19 + 2) == 0xFFFF )
          break;
        ++v19;
      }
      while ( v19 < a4 );
      if ( v19 >= a4 )
        break;
      *(_WORD *)(a5 + 4LL * v19 + 2) = *(_WORD *)(a5 + 4LL * v18 + 2);
      v21 = v7;
      if ( (unsigned __int16)v7 < v13 )
      {
        while ( *(unsigned __int8 *)(a5 + 4LL * v21) != v18 )
        {
          if ( ++v21 >= v13 )
            goto LABEL_25;
        }
        *(_BYTE *)(a5 + 4LL * v21) = v19;
LABEL_25:
        LOWORD(v7) = 0;
      }
      *(_WORD *)(a5 + 4LL * v18 + 2) = -1;
      if ( v18 == v13 - 1 )
        --v13;
    }
    --v18;
    ++v19;
  }
  for ( i = v13 - 1; i > v20; ++v12 )
  {
    if ( v12 >= i )
      break;
    do
    {
      if ( *(_WORD *)(a5 + 4LL * v12 + 2) == 0xFFFF )
        break;
      ++v12;
    }
    while ( v12 < i );
    if ( v12 >= i )
      break;
    v23 = i;
    v24 = 0;
    *(_WORD *)(a5 + 4LL * v12 + 2) = *(_WORD *)(a5 + 4LL * i + 2);
    if ( v13 )
    {
      while ( *(unsigned __int8 *)(a5 + 4LL * v24) != i )
      {
        if ( ++v24 >= v13 )
          goto LABEL_40;
      }
      *(_BYTE *)(a5 + 4LL * v24) = v12;
    }
LABEL_40:
    --i;
    *(_BYTE *)(a5 + 4LL * v12) = *(_BYTE *)(a5 + 4 * v23);
    --v13;
  }
  return v13;
}
