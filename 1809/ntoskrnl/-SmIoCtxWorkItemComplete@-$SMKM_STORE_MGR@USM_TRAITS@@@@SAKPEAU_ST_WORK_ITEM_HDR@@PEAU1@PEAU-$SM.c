/*
 * XREFs of ?SmIoCtxWorkItemComplete@?$SMKM_STORE_MGR@USM_TRAITS@@@@SAKPEAU_ST_WORK_ITEM_HDR@@PEAU1@PEAU?$SMKM_STORE@USM_TRAITS@@@@J@Z @ 0x14014C800
 * Callers:
 *     ?SmWorkItemFree@?$SMKM_STORE_MGR@USM_TRAITS@@@@SAXPEAU1@PEAU?$SMKM_STORE@USM_TRAITS@@@@PEAU_ST_WORK_ITEM_HDR@@PEAJ@Z @ 0x14014C69C (-SmWorkItemFree@-$SMKM_STORE_MGR@USM_TRAITS@@@@SAXPEAU1@PEAU-$SMKM_STORE@USM_TRAITS@@@@PEAU_ST_W.c)
 * Callees:
 *     MmMapLockedPagesSpecifyCache @ 0x14005C0C0 (MmMapLockedPagesSpecifyCache.c)
 *     SmAcquireReleaseResAvailForRead @ 0x14014B1EC (SmAcquireReleaseResAvailForRead.c)
 *     ?SmFeEvictComplete@?$SMKM_STORE_MGR@USM_TRAITS@@@@SAXPEAU1@PEAT_SM_PAGE_KEY@@KK@Z @ 0x14014BF6C (-SmFeEvictComplete@-$SMKM_STORE_MGR@USM_TRAITS@@@@SAXPEAU1@PEAT_SM_PAGE_KEY@@KK@Z.c)
 *     SmIoRequestComplete @ 0x14014C97C (SmIoRequestComplete.c)
 *     SmFpFree @ 0x14014CA1C (SmFpFree.c)
 *     ?SmProcessAddCompletion@?$SMKM_STORE_MGR@USM_TRAITS@@@@SAXPEAU1@PEAU_SM_WORK_ITEM@1@KPEAU?$SMKM_STORE@USM_TRAITS@@@@J@Z @ 0x14014DAD8 (-SmProcessAddCompletion@-$SMKM_STORE_MGR@USM_TRAITS@@@@SAXPEAU1@PEAU_SM_WORK_ITEM@1@KPEAU-$SMKM_.c)
 */

__int64 __fastcall SMKM_STORE_MGR<SM_TRAITS>::SmIoCtxWorkItemComplete(int *a1, __int64 a2, __int64 a3, int a4)
{
  __int64 *v4; // rbx
  int v5; // r11d
  unsigned int v7; // esi
  __int64 v8; // r14
  int v9; // eax
  __int64 v10; // rcx
  int v11; // r10d
  __int64 v12; // rbp
  unsigned int v13; // r8d
  int v14; // eax
  int v16; // eax
  int v17; // eax
  int v18; // eax
  int v19; // eax
  int v20; // eax

  v4 = (__int64 *)*((_QWORD *)a1 + 4);
  v5 = a3;
  v7 = 1;
  if ( (unsigned __int64)v4 <= 1 )
    v8 = 0LL;
  else
    v8 = *v4;
  v9 = *a1;
  v10 = *a1 & 7;
  v11 = *(_DWORD *)(a3 + 6016) & 0x3FF;
  if ( (v10 & 0xFFFFFFFD) != 0 )
  {
    v12 = 0LL;
    v13 = 0;
  }
  else
  {
    v12 = *((_QWORD *)a1 + 2);
    v13 = *(_DWORD *)(v12 + 40) >> 12;
    if ( (_DWORD)v10 == 2 )
    {
      v4[1] = 0LL;
      v9 = *a1;
    }
  }
  v14 = v9 & 7;
  if ( v14 )
  {
    v16 = v14 - 1;
    if ( !v16 )
    {
      if ( (a1[4] & 1) == 0 )
      {
        SMKM_STORE_MGR<SM_TRAITS>::SmFeEvictComplete((__int64)&SmGlobals, (unsigned int *)a1 + 2, a1[3], v11);
        *(_DWORD *)v4 = 0;
        goto LABEL_8;
      }
      return 0;
    }
    v17 = v16 - 1;
    if ( !v17 )
    {
      v18 = a1[2];
      v10 = *((_QWORD *)a1 + 4);
      if ( (_BYTE)v18 )
      {
        *(_WORD *)(v10 + 8) = (unsigned __int8)v18;
        if ( (dword_14055B0F0 & 8) != 0 )
          a4 = 0;
      }
      *(_DWORD *)v10 = a4;
      if ( (dword_14055B0F0 & 0x10) != 0 )
        SmAcquireReleaseResAvailForRead((signed __int64)v4, v13, 1);
      goto LABEL_8;
    }
    v19 = v17 - 1;
    if ( !v19 )
    {
      *(_DWORD *)v4 = a4;
      v4[1] = (unsigned int)a1[3];
      goto LABEL_8;
    }
    v20 = v19 - 1;
    if ( v20 )
    {
      if ( v20 != 1 )
        goto LABEL_8;
    }
    else if ( (a1[2] & 7) == 0 )
    {
      return 0;
    }
    *(_DWORD *)v4 = a4;
    v4[1] = 0LL;
    goto LABEL_8;
  }
  SMKM_STORE_MGR<SM_TRAITS>::SmProcessAddCompletion((unsigned int)&SmGlobals, (_DWORD)a1, v13, v5, a4);
LABEL_8:
  if ( v12 )
  {
    if ( (*(_BYTE *)(v12 + 10) & 5) == 0 )
      MmMapLockedPagesSpecifyCache((PMDL)v12, 0, MmCached, 0LL, 0, 0x40000010u);
    if ( (dword_14055B0F0 & 2) != 0 )
      SmFpFree(&dword_14055AFC0, 5LL, v4, v12);
  }
  if ( v8 )
    SmIoRequestComplete(v10, a1, v8, v4);
  return v7;
}
