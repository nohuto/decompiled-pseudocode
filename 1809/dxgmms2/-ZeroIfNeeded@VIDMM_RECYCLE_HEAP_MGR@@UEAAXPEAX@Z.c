/*
 * XREFs of ?ZeroIfNeeded@VIDMM_RECYCLE_HEAP_MGR@@UEAAXPEAX@Z @ 0x1C005DF30
 * Callers:
 *     <none>
 * Callees:
...
void __fastcall VIDMM_RECYCLE_HEAP_MGR::ZeroIfNeeded(VIDMM_RECYCLE_HEAP_MGR *this, VIDMM_RECYCLE_MULTIRANGE *a2)
{
  ...
  _QWORD *v25; // rax
  __int64 v26; // rax
  struct _KTHREAD **v27; // [rsp+20h] [rbp-68h] BYREF
  char v28; // [rsp+28h] [rbp-60h]
  struct _KAPC_STATE v29; // [rsp+30h] [rbp-58h] BYREF
  VIDMM_PROCESS *v30; // [rsp+60h] [rbp-28h]
  ...
  {
    if ( !(unsigned __int8)ExTryAcquirePushLockExclusiveEx(v6 + 2, 0LL) )
    {
      if ( bTracingEnabled )
      {
        v16 = *((_DWORD *)v6 + 7);
        if ( v16 != -1 && (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
          McTemplateK0q((__int64)v8, &EventBlockThread, v9, v16);
      }
      _InterlockedIncrement64((volatile signed __int64 *)v6);
      ExAcquirePushLockExclusiveEx(v6 + 2, 0LL);
    }
    if ( v6[1] )
    {
