/*
 * XREFs of ?CloseAllocation@VIDMM_GLOBAL@@QEAAJPEAU_VIDMM_MULTI_ALLOC@@EPEAPEAU_VIDMM_LOCAL_ALLOC@@U_D3DDDICB_DESTROYALLOCATION2FLAGS@@PEAPEAU_KEVENT@@@Z @ 0x1C006207C
 * Callers:
 *     VidMmCloseAllocation @ 0x1C0001B50 (VidMmCloseAllocation.c)
 *     VidMmTryCloseAllocation @ 0x1C00158D0 (VidMmTryCloseAllocation.c)
 * Callees:
  ...
  if ( Current )
  {
    ...
    else
    {
      ...
      if ( !(unsigned __int8)ExTryAcquirePushLockExclusiveEx((char *)Current + 144, 0LL) )
      {
        if ( bTracingEnabled )
        {
          v16 = *((_DWORD *)Current + 40);
          if ( v16 != -1 && (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
            McTemplateK0q(v11, &EventBlockThread, v12, v16);
        }
