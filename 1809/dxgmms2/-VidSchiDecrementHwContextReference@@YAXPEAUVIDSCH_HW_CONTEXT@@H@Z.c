/*
 * XREFs of ?VidSchiDecrementHwContextReference@@YAXPEAUVIDSCH_HW_CONTEXT@@H@Z @ 0x1C00347AC
 * Callers:
...
void __fastcall VidSchiDecrementHwContextReference(unsigned int *P, __int64 a2)
{
  ...
  v3 = a2;
  if ( !(_DWORD)a2 )
    KeAcquireInStackQueuedSpinLock((PKSPIN_LOCK)(*(_QWORD *)(v2 + 24) + 1648LL), &LockHandle);
  if ( _InterlockedExchangeAdd((volatile signed __int32 *)P + 10, 0xFFFFFFFF) == 1 )
  {
    ...
    *v8 = v7;
    ...
    if ( *((_BYTE *)P + 91) )
    {
      RtlAvlRemoveNode(v2 + 1720, P + 24);
      RtlAvlRemoveNode(v2 + 1728, P + 30);
      *((_BYTE *)P + 91) = 0;
    }
