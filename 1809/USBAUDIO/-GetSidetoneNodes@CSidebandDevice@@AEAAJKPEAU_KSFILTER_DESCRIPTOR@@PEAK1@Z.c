/*
 * XREFs of ?GetSidetoneNodes@CSidebandDevice@@AEAAJKPEAU_KSFILTER_DESCRIPTOR@@PEAK1@Z @ 0x1C0008D84
 * Callers:
 ...
 *     ?RecurseToCaptureBridgePin@CSidebandDevice@@AEAAJPEAU_KSFILTER_DESCRIPTOR@@PEBUKSTOPOLOGY_CONNECTION@@PEAK@Z @ 0x1C0009194 (-RecurseToCaptureBridgePin@CSidebandDevice@@AEAAJPEAU_KSFILTER_DESCRIPTOR@@PEBUKSTOPOLOGY_CONNEC.c)
 */
        ...
        unsigned int *a5)
{
  ...
  if ( (_DWORD)ConnectionsCount )
  {
    ...
    do
    {
      if ( *(p_ToNodePin - 1) == -1 && *p_ToNodePin == v9 )
        Flink = (int)P.Flink;
      p_ToNodePin += 4;
      --v15;
    }
    ...
    if ( Flink )
    {
      ...
      {
        v18 = &Connections[v17];
        if ( v18->ToNode == -1 && v18->ToNodePin == v9 )
          break;
        v17 += LODWORD(P.Flink);
...
LABEL_16:
      if ( v11 >= 0 )
      {
        P.Blink = &P;
        P.Flink = &P;
        if ( (int)CSidebandDevice::GetNodesInPath(this, v6, v9, &P) < 0 )
          goto LABEL_37;
        v21 = P.Flink;
        if ( P.Flink == &P )
          goto LABEL_37;
        do
        {
          ...
          v22 = *(const GUID **)((char *)&a3->NodeDescriptors->Type + a3->NodeDescriptorSize * LODWORD(v21[1].Flink));
