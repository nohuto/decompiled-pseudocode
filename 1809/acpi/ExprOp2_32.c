/*
 * XREFs of ExprOp2_32 @ 0x1C0066EB0
 * Callers:
 *     ExprOp2 @ 0x1C0020D80 (ExprOp2.c)
 * Callees:
 *     ValidateArgTypes @ 0x1C00099D0 (ValidateArgTypes.c)
 *     WriteObject @ 0x1C000A8E0 (WriteObject.c)
 *     ValidateTarget @ 0x1C000AF3C (ValidateTarget.c)
 */

__int64 __fastcall ExprOp2_32(struct _SLIST_ENTRY *a1, _QWORD *a2)
{
  __int64 result; // rax
  int v5; // edi
  __int64 v6; // r9
  unsigned int v7; // ecx
  unsigned int v8; // ecx
  unsigned int v9; // ecx
  unsigned int v10; // ecx
  unsigned int v11; // ecx
  __int64 v12; // rax
  unsigned int v13; // ecx
  __int64 v14; // rax
  unsigned int v15; // ecx
  int v16; // ecx
  int v17; // edx
  unsigned int v18; // ecx
  unsigned int v19; // ecx
  unsigned int v20; // ecx
  unsigned int v21; // ecx
  __int64 v22; // rax
  unsigned int v23; // ecx
  __int64 v24; // [rsp+38h] [rbp+10h] BYREF

  result = ValidateArgTypes((__int64)a1, a2[10], 0, "II");
  v5 = 0;
  if ( (int)result >= 0 )
  {
    result = ValidateTarget((__int64)a1, a2[10] + 80LL, 135LL, (__int64)&v24);
    if ( (int)result >= 0 )
    {
      *(_WORD *)(a2[11] + 2LL) = 1;
      v7 = *(_DWORD *)(a2[7] + 8LL);
      if ( v7 > 0x7B )
      {
        v18 = v7 - 124;
        if ( !v18 )
        {
          v16 = ~(*(_DWORD *)(a2[10] + 16LL) & *(_DWORD *)(a2[10] + 56LL));
          goto LABEL_31;
        }
        v19 = v18 - 1;
        if ( v19 )
        {
          v20 = v19 - 1;
          if ( v20 )
          {
            v21 = v20 - 1;
            if ( v21 )
            {
              if ( v21 != 6 )
                return WriteObject(a1, v24, a2[11], v6);
              v22 = a2[10];
              v23 = *(_DWORD *)(v22 + 56);
              if ( !v23 )
                goto LABEL_14;
              v17 = *(_DWORD *)(v22 + 16) % v23;
            }
            else
            {
              v17 = *(_DWORD *)(a2[10] + 16LL) ^ *(_DWORD *)(a2[10] + 56LL);
            }
          }
          else
          {
            v17 = ~(*(_DWORD *)(a2[10] + 16LL) | *(_DWORD *)(a2[10] + 56LL));
          }
        }
        else
        {
          v17 = *(_DWORD *)(a2[10] + 16LL) | *(_DWORD *)(a2[10] + 56LL);
        }
      }
      else
      {
        if ( v7 == 123 )
        {
          v16 = *(_DWORD *)(a2[10] + 16LL) & *(_DWORD *)(a2[10] + 56LL);
          goto LABEL_31;
        }
        v8 = v7 - 114;
        if ( !v8 )
        {
          v16 = *(_DWORD *)(a2[10] + 16LL) + *(_DWORD *)(a2[10] + 56LL);
          goto LABEL_31;
        }
        v9 = v8 - 2;
        if ( v9 )
        {
          v10 = v9 - 3;
          if ( v10 )
          {
            v11 = v10 - 2;
            if ( v11 )
            {
              if ( v11 != 1 )
                return WriteObject(a1, v24, a2[11], v6);
              v12 = a2[10];
              v13 = *(_DWORD *)(v12 + 56);
              if ( v13 < 0x20 )
                v5 = *(_DWORD *)(v12 + 16) >> v13;
            }
            else
            {
              v14 = a2[10];
              v15 = *(_DWORD *)(v14 + 56);
              if ( v15 < 0x20 )
                v5 = *(_DWORD *)(v14 + 16) << v15;
            }
LABEL_14:
            *(_DWORD *)(a2[11] + 16LL) = v5;
            return WriteObject(a1, v24, a2[11], v6);
          }
          v16 = *(_DWORD *)(a2[10] + 16LL) * *(_DWORD *)(a2[10] + 56LL);
LABEL_31:
          *(_DWORD *)(a2[11] + 16LL) = v16;
          return WriteObject(a1, v24, a2[11], v6);
        }
        v17 = *(_DWORD *)(a2[10] + 16LL) - *(_DWORD *)(a2[10] + 56LL);
      }
      *(_DWORD *)(a2[11] + 16LL) = v17;
      return WriteObject(a1, v24, a2[11], v6);
    }
  }
  return result;
}
