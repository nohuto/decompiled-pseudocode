/*
 * XREFs of ?CheckPlacementBounds@@YAXPEAUtagRECT@@PEAUtagPOINT@@1PEAUtagMONITOR@@@Z @ 0x1C00B8F34
 * Callers:
 *     xxxSetWindowPlacement @ 0x1C00B87BC (xxxSetWindowPlacement.c)
 * Callees:
 *     GetMonitorWorkRect @ 0x1C00B7058 (GetMonitorWorkRect.c)
 */

void __fastcall CheckPlacementBounds(
        struct tagRECT *a1,
        struct tagPOINT *a2,
        struct tagPOINT *a3,
        struct tagMONITOR *a4)
{
  __m128i *MonitorWorkRect; // rax
  LONG top; // r13d
  int v9; // ebp
  __int64 v10; // r9
  unsigned __int64 v11; // r11
  __int64 v12; // rdx
  int v13; // ecx
  LONG bottom; // r10d
  int v15; // edi
  LONG left; // r14d
  int v17; // esi
  LONG right; // r8d
  LONG x; // r8d
  LONG y; // ecx
  int v21; // ebx
  int v22; // r10d
  int v23; // ecx
  int v24; // r10d
  int v25; // r8d
  int v26; // eax
  int v27; // r10d
  LONG v28; // eax
  int v29; // r8d
  LONG v30; // ecx
  __m128i v31; // [rsp+20h] [rbp-48h]
  __m128i v32; // [rsp+30h] [rbp-38h] BYREF

  MonitorWorkRect = GetMonitorWorkRect(&v32, (__int64)a4);
  top = a1->top;
  v9 = -1;
  v10 = MonitorWorkRect->m128i_i64[0];
  v11 = HIDWORD(MonitorWorkRect->m128i_i64[0]);
  v31 = *MonitorWorkRect;
  v12 = MonitorWorkRect->m128i_i64[1];
  if ( top < (int)v11 )
    v13 = -1;
  else
    v13 = top > v31.m128i_i32[3];
  bottom = a1->bottom;
  if ( bottom < (int)v11 )
    v15 = -1;
  else
    v15 = bottom > v31.m128i_i32[3];
  left = a1->left;
  if ( a1->left < (int)v10 )
    v17 = -1;
  else
    v17 = left > v31.m128i_i32[2];
  right = a1->right;
  if ( right >= (int)v10 )
    v9 = right > v31.m128i_i32[2];
  if ( v13 * v15 > 0 || v17 * v9 > 0 )
  {
    if ( v13 >= 0 )
    {
      if ( v15 > 0 )
      {
        v27 = bottom - top;
        v28 = HIDWORD(MonitorWorkRect->m128i_i64[0]);
        if ( v31.m128i_i32[3] - v27 > (int)v11 )
          v28 = v31.m128i_i32[3] - v27;
        a1->top = v28;
        a1->bottom = v27 + v28;
      }
    }
    else
    {
      a1->top = v11;
      a1->bottom = bottom - top;
    }
    if ( v17 >= 0 )
    {
      if ( v9 > 0 )
      {
        v29 = right - left;
        v30 = v10;
        if ( (int)v12 - v29 > (int)v10 )
          v30 = v12 - v29;
        a1->left = v30;
        a1->right = v30 + v29;
      }
    }
    else
    {
      a1->left = v10;
      a1->right = right - left;
    }
  }
  x = a2->x;
  if ( a2->x != -1 )
  {
    y = a2->y;
    v21 = y >= (int)v11 ? y > SHIDWORD(v12) : -1;
    v22 = y + *(_DWORD *)(gpsi + 2088LL);
    v23 = v22 >= (int)v11 ? v22 > SHIDWORD(v12) : -1;
    v24 = x >= (int)v10 ? x > (int)v12 : -1;
    v25 = *(_DWORD *)(gpsi + 2084LL) + x;
    v26 = v25 >= (int)v10 ? v25 > (int)v12 : -1;
    if ( v21 * v23 > 0 || v24 * v26 > 0 )
    {
      a2->y = -1;
      a2->x = -1;
    }
  }
  if ( a3->x != -1 && ((int)v10 + a3->x >= (int)v12 || (int)v11 + a3->y >= SHIDWORD(v12)) )
  {
    a3->x = 0;
    a3->y = 0;
  }
}
