/*
 * XREFs of GreSaveDCInternal @ 0x1C0161094
 * Callers:
 *     NtGdiSaveDC @ 0x1C0056C30 (NtGdiSaveDC.c)
 *     GreSaveDC @ 0x1C0056E34 (GreSaveDC.c)
 * Callees:
 *     ?vUnlock@SEMOBJ@@QEAAXXZ @ 0x1C004899C (-vUnlock@SEMOBJ@@QEAAXXZ.c)
 *     ??0DCOBJ@@QEAA@PEAUHDC__@@@Z @ 0x1C0056C48 (--0DCOBJ@@QEAA@PEAUHDC__@@@Z.c)
 *     ??1DCOBJ@@QEAA@XZ @ 0x1C0056E10 (--1DCOBJ@@QEAA@XZ.c)
 *     ??0SAVEOBJ@@QEAA@AEAVDCOBJ@@JP6AH0J@ZP6AX0J@Z@Z @ 0x1C0056E50 (--0SAVEOBJ@@QEAA@AEAVDCOBJ@@JP6AH0J@ZP6AX0J@Z@Z.c)
 *     _guard_dispatch_icall_nop @ 0x1C01632C0 (_guard_dispatch_icall_nop.c)
 */

__int64 __fastcall GreSaveDCInternal(HDC a1, int a2)
{
  DC *v3; // rcx
  unsigned int v4; // r14d
  __int64 v5; // rdx
  int v6; // ebx
  __int64 v7; // rcx
  int v8; // r12d
  DC **v9; // r15
  __int64 v10; // r13
  int v11; // esi
  int v12; // edi
  __int64 v13; // rbx
  int v14; // eax
  DC *v15; // rcx
  __int64 v16; // rdx
  _QWORD v18[2]; // [rsp+30h] [rbp-59h] BYREF
  int v19; // [rsp+40h] [rbp-49h]
  DC *v20[6]; // [rsp+48h] [rbp-41h] BYREF
  unsigned int v21; // [rsp+78h] [rbp-11h] BYREF
  int v22; // [rsp+7Ch] [rbp-Dh]
  __int64 v23; // [rsp+80h] [rbp-9h]
  void (__fastcall *v24)(__int64, _QWORD); // [rsp+88h] [rbp-1h]
  unsigned int v25; // [rsp+90h] [rbp+7h] BYREF
  int v26; // [rsp+94h] [rbp+Bh]
  __int64 v27; // [rsp+98h] [rbp+Fh]
  void (__fastcall *v28)(__int64, _QWORD); // [rsp+A0h] [rbp+17h]
  __int64 v29; // [rsp+100h] [rbp+77h] BYREF
  __int64 v30; // [rsp+108h] [rbp+7Fh] BYREF

  DCOBJ::DCOBJ((DCOBJ *)v20, a1);
  v3 = v20[0];
  v4 = 0;
  if ( !v20[0] )
    goto LABEL_39;
  if ( !a2 )
  {
    if ( (*(_DWORD *)(HmgPentryFromPobj(v20[0]) + 8) & 0xFFFFFFFE) != 0 )
    {
      v3 = v20[0];
      goto LABEL_5;
    }
LABEL_39:
    EngSetLastError(6u);
    goto LABEL_40;
  }
LABEL_5:
  v5 = *((_QWORD *)v3 + 122);
  v6 = *(_DWORD *)(v5 + 152);
  if ( (v6 & 0x1000) != 0 )
  {
    GreDCSelectBrush(v3, *(_QWORD *)(v5 + 160));
    v3 = v20[0];
  }
  if ( (v6 & 0x2000) != 0 )
  {
    GreDCSelectPen(v3, *(_QWORD *)(*((_QWORD *)v3 + 122) + 168LL));
    v3 = v20[0];
  }
  v7 = *((_QWORD *)v3 + 6);
  v29 = v7;
  if ( (*(_DWORD *)(v7 + 40) & 1) != 0 )
    GreLockVisRgn(v7);
  v30 = ghsemPalette;
  GreAcquireSemaphore(ghsemPalette);
  v8 = *((_DWORD *)v20[0] + 9) & 0x800;
  if ( v8 )
    DC::bMakeInfoDC(v20[0], 0);
  DCMEMOBJ::DCMEMOBJ((DCMEMOBJ *)v18, (struct DCOBJ *)v20);
  if ( v18[0] )
  {
    PDEVOBJ::vReferencePdev((PDEVOBJ *)&v29);
    v9 = v20;
    v10 = *((_QWORD *)v20[0] + 14);
    *((_QWORD *)v20[0] + 14) = *(_QWORD *)v18[0];
    SAVEOBJ::SAVEOBJ(
      (SAVEOBJ *)&v25,
      (struct DCOBJ *)v20,
      *(_DWORD *)(v18[0] + 104LL),
      (int (*)(struct DCOBJ *, int))bSavePath,
      (void (*)(struct DCOBJ *, int))vRestorePath);
    v11 = v26;
    if ( !v26 )
      goto LABEL_33;
    SAVEOBJ::SAVEOBJ(
      (SAVEOBJ *)&v21,
      (struct DCOBJ *)v20,
      *(_DWORD *)(v18[0] + 104LL),
      (int (*)(struct DCOBJ *, int))bSaveRegion,
      vRestoreRegion);
    v12 = v22;
    if ( v22 )
    {
      if ( (unsigned int)GreSetDCOwnerEx(*(_QWORD *)v18[0], 2147483666LL, 0LL, 1LL) )
      {
        v12 = 0;
        v19 = 1;
        v11 = 0;
        v22 = 0;
        v26 = 0;
        v9 = 0LL;
        PDEVOBJ::vUnreferencePdev(&v29, 0LL);
        if ( (*((_DWORD *)v20[0] + 9) & 0x4000) != 0 )
          *(_DWORD *)(v18[0] + 36LL) |= 0x4000u;
        v13 = *(_QWORD *)(v18[0] + 496LL);
        if ( v13 )
        {
          v14 = *(_DWORD *)(v13 + 112);
          if ( (v14 & 0x800) != 0 )
          {
            INC_SHARE_REF_CNT(*(_QWORD *)(v18[0] + 496LL));
          }
          else if ( v14 >= 0 )
          {
            INC_SHARE_REF_CNT(*(_QWORD *)(v18[0] + 496LL));
            ++*(_DWORD *)(v13 + 168);
          }
        }
        INC_SHARE_REF_CNT(*((_QWORD *)v20[0] + 17));
        INC_SHARE_REF_CNT(*((_QWORD *)v20[0] + 18));
        INC_SHARE_REF_CNT(*((_QWORD *)v20[0] + 19));
        INC_SHARE_REF_CNT(*((_QWORD *)v20[0] + 12));
        v15 = v20[0];
        v16 = *((_QWORD *)v20[0] + 11);
        if ( (struct PALETTE *)v16 != ppalDefault )
        {
          _InterlockedIncrement((volatile signed __int32 *)(v16 + 56));
          v15 = v20[0];
          v11 = v26;
          v12 = v22;
        }
        v4 = *((_DWORD *)v15 + 26);
        *((_DWORD *)v15 + 26) = v4 + 1;
      }
      if ( v12 )
        v24(v23, v21);
    }
    if ( v11 )
      v28(v27, v25);
    if ( v9 )
LABEL_33:
      *((_QWORD *)*v9 + 14) = v10;
  }
  else
  {
    EngSetLastError(8u);
  }
  DCMEMOBJ::~DCMEMOBJ((DCMEMOBJ *)v18);
  if ( v8 )
    DC::bMakeInfoDC(v20[0], 1);
  if ( (*(_DWORD *)(v29 + 40) & 1) != 0 )
    GreUnlockVisRgn(v29);
  SEMOBJ::vUnlock((SEMOBJ *)&v30);
LABEL_40:
  DCOBJ::~DCOBJ((DCOBJ *)v20);
  return v4;
}
