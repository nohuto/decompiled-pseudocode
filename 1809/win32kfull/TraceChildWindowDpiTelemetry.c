/*
 * XREFs of TraceChildWindowDpiTelemetry @ 0x1C0006400
 * Callers:
...
char __fastcall TraceChildWindowDpiTelemetry(__int64 a1, __int64 a2, int a3)
{
  ...
  if ( a2 )
  {
    ...
    v11 = *(_QWORD *)(ProcessImageFilename + 424);
    if ( v8 == v11 )
    {
      v4 = pwsz;
    }
  ...
  if ( hProvider > 5u )
  {
    ...
    if ( (_BYTE)ProcessImageFilename )
    {
      ...
      TlgCreateWsz(&v20, v4);
      LOBYTE(ProcessImageFilename) = TlgWrite((TraceLoggingHProvider)&hProvider, &unk_1C02D551B, 0LL, 0LL, 5u, &pData);
    }
  }
  if ( v10 )
