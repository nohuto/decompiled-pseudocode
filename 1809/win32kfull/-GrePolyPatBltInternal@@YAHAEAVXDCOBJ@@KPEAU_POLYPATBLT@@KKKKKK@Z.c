/*
 * XREFs of ?GrePolyPatBltInternal@@YAHAEAVXDCOBJ@@KPEAU_POLYPATBLT@@KKKKKK@Z @ 0x1C005604C
 * Callers:
 *     NtGdiPolyPatBlt @ 0x1C0055850 (NtGdiPolyPatBlt.c)
 *     ?NtGdiFlushUserBatchInternal@@YAXPEAX@Z @ 0x1C0057DD0 (-NtGdiFlushUserBatchInternal@@YAXPEAX@Z.c)
 *     GrePolyPatBlt @ 0x1C007B620 (GrePolyPatBlt.c)
 * Callees:
 *     bCvtPts1 @ 0x1C00477FC (bCvtPts1.c)
 *     ?GrePatBltLockedDC@@YAHAEAVXDCOBJ@@AEAVEXFORMOBJ@@PEAVERECTL@@KPEAVSURFACE@@KKKK@Z @ 0x1C0055BA4 (-GrePatBltLockedDC@@YAHAEAVXDCOBJ@@AEAVEXFORMOBJ@@PEAVERECTL@@KPEAVSURFACE@@KKKK@Z.c)
 *     ??1DEVLOCKOBJ@@QEAA@XZ @ 0x1C0057424 (--1DEVLOCKOBJ@@QEAA@XZ.c)
 *     ??0DEVLOCKOBJ@@QEAA@XZ @ 0x1C0099DC4 (--0DEVLOCKOBJ@@QEAA@XZ.c)
 *     GreMaskBlt @ 0x1C009B9B0 (GreMaskBlt.c)
 *     ?vAccumulate@XDCOBJ@@QEAAXAEAVERECTL@@@Z @ 0x1C009FFA0 (-vAccumulate@XDCOBJ@@QEAAXAEAVERECTL@@@Z.c)
 *     ?bFullScreen@XDCOBJ@@QEAAHXZ @ 0x1C00A4AD4 (-bFullScreen@XDCOBJ@@QEAAHXZ.c)
 *     __security_check_cookie @ 0x1C015AF60 (__security_check_cookie.c)
 *     ?bTightBoundsToRegion@DEVLOCKOBJ@@QEAAHXZ @ 0x1C015F5AC (-bTightBoundsToRegion@DEVLOCKOBJ@@QEAAHXZ.c)
 */

__int64 __fastcall GrePolyPatBltInternal(
        struct XDCOBJ *a1,
        unsigned int a2,
        struct _POLYPATBLT *a3,
        int a4,
        unsigned int a5,
        unsigned int a6,
        unsigned int a7,
        unsigned int a8,
        unsigned int a9)
{
  int v9; // esi
  struct _POLYPATBLT *v10; // r13
  unsigned int v12; // edi
  bool v13; // zf
  unsigned int v14; // edx
  unsigned int v15; // r15d
  __int64 v16; // rcx
  __int64 v17; // rax
  __int64 v18; // r12
  __int64 v19; // rdx
  int v20; // esi
  int v21; // r14d
  int v22; // edx
  int v23; // r8d
  __int64 v24; // rax
  int v25; // r9d
  unsigned __int64 v26; // rdx
  int v27; // r8d
  unsigned int locked; // eax
  __int64 v29; // rcx
  int v31; // eax
  unsigned int v32; // eax
  int v33; // [rsp+70h] [rbp-128h]
  int v34; // [rsp+74h] [rbp-124h]
  int v35; // [rsp+78h] [rbp-120h]
  __int64 v36; // [rsp+80h] [rbp-118h]
  unsigned __int64 v37; // [rsp+88h] [rbp-110h] BYREF
  int v38; // [rsp+90h] [rbp-108h]
  int v39; // [rsp+94h] [rbp-104h]
  int v40; // [rsp+98h] [rbp-100h]
  int v41; // [rsp+9Ch] [rbp-FCh]
  int v42; // [rsp+A0h] [rbp-F8h]
  int v43; // [rsp+A4h] [rbp-F4h]
  struct SURFACE *v44; // [rsp+A8h] [rbp-F0h]
  __int64 v45; // [rsp+B0h] [rbp-E8h]
  __int64 v46; // [rsp+B8h] [rbp-E0h]
  struct XDCOBJ *v47; // [rsp+C0h] [rbp-D8h]
  __int64 v48; // [rsp+C8h] [rbp-D0h]
  _BYTE v49[112]; // [rsp+D0h] [rbp-C8h] BYREF
  int v50; // [rsp+140h] [rbp-58h] BYREF
  int v51; // [rsp+144h] [rbp-54h]
  unsigned int v52; // [rsp+148h] [rbp-50h]
  int v53; // [rsp+14Ch] [rbp-4Ch]

  v9 = a4;
  v10 = a3;
  v47 = a1;
  v12 = 1;
  v13 = a4 == 0;
  if ( !a4 )
  {
LABEL_43:
    if ( !v13 )
      return 0;
    return v12;
  }
  if ( !a3 || a5 )
  {
    v13 = a4 == 0;
    goto LABEL_43;
  }
  v14 = HIWORD(a2);
  v15 = ((unsigned __int8)v14 << 8) | (unsigned __int8)v14;
  if ( (((unsigned __int8)v14 ^ (unsigned __int8)(4 * v14)) & 0xCC) != 0 )
    return 0;
  v16 = *(_QWORD *)a1;
  if ( !v16 )
  {
    return 0;
  }
  else
  {
    v17 = *(_QWORD *)(v16 + 976);
    v18 = *(_QWORD *)(v17 + 160);
    v46 = v18;
    if ( (*(_DWORD *)(v17 + 152) & 0x1000) != 0 )
      GreDCSelectBrush(v16, v18);
    DEVLOCKOBJ::DEVLOCKOBJ((DEVLOCKOBJ *)v49);
    if ( DEVLOCKOBJ::bLock((DEVLOCKOBJ *)v49, a1, 0) )
    {
      v19 = *(_QWORD *)a1;
      v38 = *(_DWORD *)(*(_QWORD *)(*(_QWORD *)a1 + 976LL) + 208LL);
      v39 = *(_DWORD *)(*(_QWORD *)(v19 + 976) + 108LL) & 1;
      if ( (*(_DWORD *)(*(_QWORD *)(v19 + 976) + 340LL) & 0x1E000) != 0 )
      {
        EXFORMOBJ::vInit((EXFORMOBJ *)&v37, a1, 0x204u, 0);
        v19 = *(_QWORD *)a1;
      }
      else
      {
        v37 = v19 + 320;
      }
      v44 = *(struct SURFACE **)(v19 + 496);
      while ( 1 )
      {
        v35 = v9 - 1;
        if ( !v9 )
          goto LABEL_34;
        v36 = 0LL;
        v20 = *(_DWORD *)v10;
        v40 = *(_DWORD *)v10;
        v21 = *((_DWORD *)v10 + 1);
        v41 = v21;
        v22 = *((_DWORD *)v10 + 2);
        v33 = v22;
        v42 = v22;
        v23 = *((_DWORD *)v10 + 3);
        v34 = v23;
        v43 = v23;
        v24 = *((_QWORD *)v10 + 2);
        v45 = v24;
        v48 = v24;
        if ( v24 )
        {
          v36 = GreDCSelectBrush(*(_QWORD *)a1, v24);
          v22 = v33;
          v23 = v34;
        }
        v25 = *(_DWORD *)(v37 + 32);
        if ( (v25 & 1) == 0 )
          break;
        v50 = v20;
        v51 = v21;
        v26 = (unsigned int)(v20 + v22);
        v52 = v26;
        v27 = v21 + v23;
        v53 = v27;
        if ( (v25 & 0x43) != 0x43 )
        {
          bCvtPts1(v37, &v50, 2LL);
          v27 = v53;
          v26 = v52;
          v21 = v51;
          v20 = v50;
        }
        if ( v39 )
        {
          v50 = ++v20;
          v26 = (unsigned int)(v26 + 1);
          v52 = v26;
        }
        if ( v20 > (int)v26 )
        {
          v32 = v20;
          v20 = v26;
          v50 = v26;
          v26 = v32;
          v52 = v32;
        }
        if ( v21 > v27 )
        {
          v31 = v21;
          v21 = v27;
          v51 = v27;
          v27 = v31;
          v53 = v31;
        }
        if ( v20 != (_DWORD)v26 && v21 != v27 )
        {
          if ( (*(_DWORD *)(*(_QWORD *)a1 + 36LL) & 0xE0) != 0 )
            XDCOBJ::vAccumulate(a1, (struct ERECTL *)&v50);
          if ( v44 )
          {
            locked = GrePatBltLockedDC(
                       (__int64)a1,
                       (struct EXFORMOBJ *)v26,
                       (struct ERECTL *)&v50,
                       v15,
                       v44,
                       a6,
                       a7,
                       a8,
                       a9);
LABEL_30:
            v12 = locked;
          }
        }
        if ( v45 )
          GreDCSelectBrush(*(_QWORD *)a1, v36);
        DEVLOCKOBJ::bTightBoundsToRegion((DEVLOCKOBJ *)v49);
        v10 = (struct _POLYPATBLT *)((char *)v10 + 24);
        v9 = v35;
      }
      locked = GreMaskBlt(**(HDC **)a1, v23, 0LL, 0, 0, 0LL, 0, 0, v15 << 16, 0);
      goto LABEL_30;
    }
    v12 = XDCOBJ::bFullScreen(a1);
LABEL_34:
    v29 = *(_QWORD *)(*(_QWORD *)a1 + 976LL);
    if ( *(_QWORD *)(v29 + 160) != v18 )
    {
      *(_QWORD *)(v29 + 160) = v18;
      *(_DWORD *)(*(_QWORD *)(*(_QWORD *)a1 + 976LL) + 152LL) |= 0x1000u;
    }
    DEVLOCKOBJ::~DEVLOCKOBJ((DEVLOCKOBJ *)v49);
  }
  return v12;
}
