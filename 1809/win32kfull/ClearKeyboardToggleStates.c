/*
 * XREFs of ClearKeyboardToggleStates @ 0x1C00AFA3C
 * Callers:
...
__int64 ClearKeyboardToggleStates()
{
  ...
  {
    ++v0;
    v6[v1] = gafAsyncKeyState[(unsigned __int64)byte_1C02CDEC8[v1] >> 2] & (1 << (2 * (byte_1C02CDEC8[v1] & 3) + 1));
    ++v1;
  }
  while ( v0 < 0xE );
  ...
  do
  {
    if ( v6[v5] )
    {
      result = 2 * (byte_1C02CDEC8[v5] & 3u) + 1;
      gafAsyncKeyState[(unsigned __int64)byte_1C02CDEC8[v5] >> 2] |= 1 << (2 * (byte_1C02CDEC8[v5] & 3) + 1);
    }
    ++v4;
    ++v5;
  }
