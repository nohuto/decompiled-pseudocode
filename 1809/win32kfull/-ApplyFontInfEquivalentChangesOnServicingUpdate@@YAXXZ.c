/*
 * XREFs of ?ApplyFontInfEquivalentChangesOnServicingUpdate@@YAXXZ @ 0x1C01601A4
 * Callers:
...
void ApplyFontInfEquivalentChangesOnServicingUpdate(void)
{
  ...
  ULONG v13; // esi
  _BYTE *v14; // rbx
  USHORT OemCodePage[2]; // [rsp+38h] [rbp-D0h] BYREF
  USHORT AnsiCodePage; // [rsp+3Ch] [rbp-CCh] BYREF
  ...
  _QWORD KeyValueInformation[2]; // [rsp+328h] [rbp+220h] BYREF
  ...
  *(_OWORD *)&ObjectAttributes.SecurityDescriptor = 0LL;
  ...
  {
    RtlInitUnicodeString(&DestinationString, L"UpdateFontLinks");
    v0 = 2;
    ResultLength[0] = 0;
           ...
           ResultLength) >= 0 )
    {
      ...
      if ( AnsiCodePage == 950 )
        goto LABEL_9;
      if ( OemCodePage[0] == 936 )
      {
LABEL_8:
        v0 = 1;
        goto LABEL_9;
      }
      if ( OemCodePage[0] == 950 )
      {
LABEL_9:
        RtlInitUnicodeString(
          &DestinationString,
        ...
        *(_OWORD *)&ObjectAttributes.SecurityDescriptor = 0LL;
        if ( ZwOpenKey(&Handle, 0xF003Fu, &ObjectAttributes) >= 0 )
        {
          ...
          *((_WORD *)v2 + 8) = v12;
          ...
          RtlInitUnicodeString(&DestinationString, L"SimSun-ExtG");
          ZwSetValueKey(Handle, &DestinationString, 0, 7u, v14, v13);
          ZwClose(Handle);
        }
      }
    }
