/*
 * XREFs of ?CreateUsermodeParameters@FontManagementRequest@@UEAAJPEAVUmfdTls@@PEAPEAX@Z @ 0x1C02C1730
 * Callers:
 *     <none>
 * Callees:
 *     ?CommitUMBuffer@UmfdTls@@QEAAPEAXK_N@Z @ 0x1C00927BC (-CommitUMBuffer@UmfdTls@@QEAAPEAXK_N@Z.c)
 *     ?PrepareUsermodeFontObj@FontDriverDdiRequest@@KAXPEAVUmfdTls@@PEAU_FONTOBJ@@1@Z @ 0x1C00928B4 (-PrepareUsermodeFontObj@FontDriverDdiRequest@@KAXPEAVUmfdTls@@PEAU_FONTOBJ@@1@Z.c)
 *     ?TryComputeAlignedFieldSizes@UmfdTls@@SA_NIPEAII0I0I0@Z @ 0x1C0146CD8 (-TryComputeAlignedFieldSizes@UmfdTls@@SA_NIPEAII0I0I0@Z.c)
 *     memmove @ 0x1C0163300 (memmove.c)
 */

__int64 __fastcall FontManagementRequest::CreateUsermodeParameters(
        FontManagementRequest *this,
        UmfdUMBuffer **a2,
        void **a3)
{
  __int64 v6; // rsi
  int v7; // ecx
  __int64 v8; // r14
  __int64 v9; // r15
  char *v10; // rax
  _QWORD *v11; // rdi
  char *v12; // rdx
  const void *v13; // rdx
  struct _FONTOBJ *v14; // rdx
  unsigned int v16; // [rsp+40h] [rbp-38h] BYREF
  unsigned int v17[3]; // [rsp+44h] [rbp-34h] BYREF
  unsigned int v18; // [rsp+80h] [rbp+8h] BYREF
  unsigned int v19; // [rsp+98h] [rbp+20h] BYREF

  if ( !UmfdTls::TryComputeAlignedFieldSizes(
          0x38u,
          v17,
          0x40u,
          &v16,
          *((_DWORD *)this + 15),
          &v18,
          *((_DWORD *)this + 18),
          &v19) )
    return 3221225495LL;
  v6 = v18;
  v7 = v18 + v19;
  if ( v18 + v19 < v18 )
    return 3221225495LL;
  v8 = v16;
  if ( v16 + v7 < v16 )
    return 3221225495LL;
  v9 = v17[0];
  if ( v16 + v7 + v17[0] < v17[0] )
    return 3221225495LL;
  v10 = (char *)UmfdTls::CommitUMBuffer(a2, v16 + v7 + v17[0], 1);
  v11 = v10;
  if ( !v10 )
    return 3221225495LL;
  *((_QWORD *)this + 12) = &v10[v9];
  v12 = &v10[v9 + v8];
  *((_QWORD *)this + 13) = v12;
  *((_QWORD *)this + 14) = &v12[v6];
  v13 = (const void *)*((_QWORD *)this + 8);
  if ( v13 )
    memmove(*((void **)this + 13), v13, *((unsigned int *)this + 15));
  else
    *((_QWORD *)this + 13) = 0LL;
  if ( !*((_QWORD *)this + 10) )
    *((_QWORD *)this + 14) = 0LL;
  v14 = (struct _FONTOBJ *)*((_QWORD *)this + 6);
  if ( v14 )
  {
    FontDriverDdiRequest::PrepareUsermodeFontObj((struct UmfdTls *)a2, v14, *((struct _FONTOBJ **)this + 12));
    v11[1] = *((_QWORD *)this + 12);
  }
  *v11 = 0LL;
  *((_DWORD *)v11 + 4) = *((_DWORD *)this + 14);
  *((_DWORD *)v11 + 5) = *((_DWORD *)this + 15);
  v11[3] = *((_QWORD *)this + 13);
  *((_DWORD *)v11 + 8) = *((_DWORD *)this + 18);
  v11[5] = *((_QWORD *)this + 14);
  *a3 = v11;
  return 0LL;
}
