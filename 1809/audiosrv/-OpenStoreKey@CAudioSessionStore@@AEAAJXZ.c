/*
 * XREFs of ?OpenStoreKey@CAudioSessionStore@@AEAAJXZ @ 0x18000706C
 * Callers:
 ...
 *     WPP_SF_qSSq @ 0x1800D8770 (WPP_SF_qSSq.c)
 */
...
LABEL_16:
  if ( hKey )
    RegCloseKey(hKey);
  if ( ThreadUserStringSid < 0 )
    AudSrvTraceLoggingErrorHelper("CAudioSessionStore::OpenStoreKey", 0x1858u, ThreadUserStringSid);
  return (unsigned int)ThreadUserStringSid;
}
