/*
 * XREFs of ?GetProcessingModeCharacteristicsFromPropertyStore@CEndpointCharacteristics@@AEAAJXZ @ 0x18004D234
 * Callers:
 *     ?DiscoverProcessingModeCharacteristics@CEndpointCharacteristics@@AEAAJPEA_N@Z @ 0x18004BBA8 (-DiscoverProcessingModeCharacteristics@CEndpointCharacteristics@@AEAAJPEA_N@Z.c)
 * Callees:
 *     ?ReadProcessingModeCharacteristics@CEndpointCharacteristics@@AEAAJW4__MIDL___MIDL_itf_audioengineendpoint_0000_0000_0001@@PEAPEAV?$CSimpleArray@PEAVCConnectorProcessingModeCharacteristics@@V?$CSimpleArrayEqualHelper@PEAVCConnectorProcessingModeCharacteristics@@@ATL@@@ATL@@@Z @ 0x18004CD80 (-ReadProcessingModeCharacteristics@CEndpointCharacteristics@@AEAAJW4__MIDL___MIDL_itf_audioengin.c)
 *     ??A?$CSimpleArray@PEAVCConnectorProcessingModeCharacteristics@@V?$CSimpleArrayEqualHelper@PEAVCConnectorProcessingModeCharacteristics@@@ATL@@@ATL@@QEAAAEAPEAVCConnectorProcessingModeCharacteristics@@H@Z @ 0x18004D3D0 (--A-$CSimpleArray@PEAVCConnectorProcessingModeCharacteristics@@V-$CSimpleArrayEqualHelper@PEAVCC.c)
 *     ?GetAliasedDeviceConnectorMode@CEndpointCharacteristics@@QEAA?AU_GUID@@W4__MIDL___MIDL_itf_audioengineendpoint_0000_0000_0001@@U2@W4DEVICE_INITIALIZATION@@@Z @ 0x18004D3FC (-GetAliasedDeviceConnectorMode@CEndpointCharacteristics@@QEAA-AU_GUID@@W4__MIDL___MIDL_itf_audio.c)
 *     ??4?$CSimpleArray@U_GUID@@V?$CSimpleArrayEqualHelper@U_GUID@@@ATL@@@ATL@@QEAAAEAV01@AEBV01@@Z @ 0x18004DEFC (--4-$CSimpleArray@U_GUID@@V-$CSimpleArrayEqualHelper@U_GUID@@@ATL@@@ATL@@QEAAAEAV01@AEBV01@@Z.c)
 *     ?AddMultiple@CAudioSignalProcessingModeArray@@QEAAJIPEBU_GUID@@@Z @ 0x18004E468 (-AddMultiple@CAudioSignalProcessingModeArray@@QEAAJIPEBU_GUID@@@Z.c)
 *     ??A?$CSimpleArray@U_GUID@@V?$CSimpleArrayEqualHelper@U_GUID@@@ATL@@@ATL@@QEAAAEAU_GUID@@H@Z @ 0x18004F8E0 (--A-$CSimpleArray@U_GUID@@V-$CSimpleArrayEqualHelper@U_GUID@@@ATL@@@ATL@@QEAAAEAU_GUID@@H@Z.c)
 *     ?RemoveAll@?$CSimpleArray@U_GUID@@V?$CSimpleArrayEqualHelper@U_GUID@@@ATL@@@ATL@@QEAAXXZ @ 0x18004FC98 (-RemoveAll@-$CSimpleArray@U_GUID@@V-$CSimpleArrayEqualHelper@U_GUID@@@ATL@@@ATL@@QEAAXXZ.c)
 *     __security_check_cookie @ 0x180060590 (__security_check_cookie.c)
 *     ??3@YAXPEAXAEBUnothrow_t@std@@@Z @ 0x180060C94 (--3@YAXPEAXAEBUnothrow_t@std@@@Z.c)
 */

// Hidden C++ exception states: #wind=4
__int64 __fastcall CEndpointCharacteristics::GetProcessingModeCharacteristicsFromPropertyStore(
        CEndpointCharacteristics *this)
{
  __int64 v2; // rbx
  __int64 v3; // rdi
  int ProcessingModeCharacteristics; // esi
  unsigned int v5; // r15d
  unsigned int i; // r12d
  int v7; // eax
  bool v8; // zf
  _QWORD *v9; // rax
  _QWORD *v10; // rcx
  __int64 v11; // rax
  __int64 v12; // rax
  __int64 v13; // rax
  unsigned int v15; // esi
  int v16; // r12d
  unsigned int j; // r15d
  int v18; // eax
  bool v19; // zf
  _QWORD *v20; // rax
  _QWORD *v21; // rcx
  __int64 v22; // rax
  __int64 v23; // [rsp+38h] [rbp-29h] BYREF
  int v24; // [rsp+40h] [rbp-21h]
  int v25; // [rsp+44h] [rbp-1Dh]
  __int64 v26; // [rsp+48h] [rbp-19h] BYREF
  __int64 v27; // [rsp+50h] [rbp-11h] BYREF
  __int64 v28; // [rsp+58h] [rbp-9h] BYREF
  int v29; // [rsp+60h] [rbp-1h]
  int v30; // [rsp+64h] [rbp+3h]
  __int128 v31; // [rsp+68h] [rbp+7h] BYREF
  __int64 v32; // [rsp+78h] [rbp+17h]
  __int64 v33; // [rsp+80h] [rbp+1Fh] BYREF
  __int64 v34; // [rsp+88h] [rbp+27h]

  v32 = -2LL;
  v2 = 0LL;
  v26 = 0LL;
  v3 = 0LL;
  v27 = 0LL;
  v28 = 0LL;
  v29 = 0;
  v30 = 0;
  if ( !*((_DWORD *)this + 46) )
  {
LABEL_2:
    ProcessingModeCharacteristics = CEndpointCharacteristics::ReadProcessingModeCharacteristics(
                                      (__int64)this,
                                      0,
                                      (__int64)&v27);
    if ( ProcessingModeCharacteristics >= 0 )
    {
      ATL::CSimpleArray<_GUID,ATL::CSimpleArrayEqualHelper<_GUID>>::operator=(&v28, (char *)this + 72);
      if ( v29 == *((_DWORD *)this + 20) )
      {
        ProcessingModeCharacteristics = CAudioSignalProcessingModeArray::AddMultiple(
                                          (CAudioSignalProcessingModeArray *)&v28,
                                          *((_DWORD *)this + 376),
                                          *((const struct _GUID **)this + 187));
        if ( ProcessingModeCharacteristics >= 0 )
        {
          v5 = 0;
          v3 = v27;
          if ( v29 <= 0 )
          {
LABEL_14:
            v12 = v2;
            v2 = 0LL;
            *((_QWORD *)this + 25) = v12;
            v13 = v3;
            v3 = 0LL;
            *((_QWORD *)this + 24) = v13;
          }
          else
          {
            while ( 1 )
            {
              v31 = *(_OWORD *)ATL::CSimpleArray<_GUID,ATL::CSimpleArrayEqualHelper<_GUID>>::operator[](&v28, v5);
              CEndpointCharacteristics::GetAliasedDeviceConnectorMode(this, &v33, 0LL, &v31, 0);
              for ( i = 0; ; ++i )
              {
                v7 = *(_DWORD *)(v3 + 8);
                v8 = i == v7;
                if ( (int)i >= v7 )
                  break;
                v9 = (_QWORD *)ATL::CSimpleArray<CConnectorProcessingModeCharacteristics *,ATL::CSimpleArrayEqualHelper<CConnectorProcessingModeCharacteristics *>>::operator[](
                                 v3,
                                 i);
                v10 = (_QWORD *)*v9;
                v11 = *(_QWORD *)*v9 - v33;
                if ( !v11 )
                  v11 = v10[1] - v34;
                if ( !v11 )
                {
                  v8 = i == *(_DWORD *)(v3 + 8);
                  break;
                }
              }
              if ( v8 )
                break;
              if ( (int)++v5 >= v29 )
                goto LABEL_14;
            }
            ProcessingModeCharacteristics = -2147023728;
          }
          goto LABEL_15;
        }
      }
      else
      {
        ProcessingModeCharacteristics = -2147024882;
      }
    }
    v3 = v27;
    goto LABEL_15;
  }
  v23 = 0LL;
  v24 = 0;
  v25 = 0;
  ProcessingModeCharacteristics = CEndpointCharacteristics::ReadProcessingModeCharacteristics(
                                    (__int64)this,
                                    3,
                                    (__int64)&v26);
  if ( ProcessingModeCharacteristics < 0 )
  {
LABEL_22:
    ATL::CSimpleArray<_GUID,ATL::CSimpleArrayEqualHelper<_GUID>>::RemoveAll(&v23);
    v2 = v26;
    goto LABEL_15;
  }
  ATL::CSimpleArray<_GUID,ATL::CSimpleArrayEqualHelper<_GUID>>::operator=(&v23, (char *)this + 120);
  if ( v24 != *((_DWORD *)this + 32) )
  {
    ProcessingModeCharacteristics = -2147024882;
    goto LABEL_22;
  }
  ProcessingModeCharacteristics = CAudioSignalProcessingModeArray::AddMultiple(
                                    (CAudioSignalProcessingModeArray *)&v23,
                                    *((_DWORD *)this + 388),
                                    *((const struct _GUID **)this + 193));
  if ( ProcessingModeCharacteristics < 0 )
    goto LABEL_22;
  v15 = 0;
  v2 = v26;
  v16 = v24;
  if ( v24 <= 0 )
  {
LABEL_34:
    ATL::CSimpleArray<_GUID,ATL::CSimpleArrayEqualHelper<_GUID>>::RemoveAll(&v23);
    goto LABEL_2;
  }
  while ( 1 )
  {
    v31 = *(_OWORD *)ATL::CSimpleArray<_GUID,ATL::CSimpleArrayEqualHelper<_GUID>>::operator[](&v23, v15);
    CEndpointCharacteristics::GetAliasedDeviceConnectorMode(this, &v33, 3LL, &v31, 0);
    for ( j = 0; ; ++j )
    {
      v18 = *(_DWORD *)(v2 + 8);
      v19 = j == v18;
      if ( (int)j >= v18 )
        break;
      v20 = (_QWORD *)ATL::CSimpleArray<CConnectorProcessingModeCharacteristics *,ATL::CSimpleArrayEqualHelper<CConnectorProcessingModeCharacteristics *>>::operator[](
                        v2,
                        j);
      v21 = (_QWORD *)*v20;
      v22 = *(_QWORD *)*v20 - v33;
      if ( !v22 )
        v22 = v21[1] - v34;
      if ( !v22 )
      {
        v19 = j == *(_DWORD *)(v2 + 8);
        break;
      }
    }
    if ( v19 )
      break;
    if ( (int)++v15 >= v16 )
      goto LABEL_34;
  }
  ProcessingModeCharacteristics = -2147023728;
  ATL::CSimpleArray<_GUID,ATL::CSimpleArrayEqualHelper<_GUID>>::RemoveAll(&v23);
LABEL_15:
  ATL::CSimpleArray<_GUID,ATL::CSimpleArrayEqualHelper<_GUID>>::RemoveAll(&v28);
  if ( v3 )
  {
    if ( *(_QWORD *)v3 )
    {
      free(*(void **)v3);
      *(_QWORD *)v3 = 0LL;
    }
    *(_DWORD *)(v3 + 8) = 0;
    *(_DWORD *)(v3 + 12) = 0;
    operator delete((void *)v3, (const struct std::nothrow_t *)0x10);
  }
  if ( v2 )
  {
    if ( *(_QWORD *)v2 )
    {
      free(*(void **)v2);
      *(_QWORD *)v2 = 0LL;
    }
    *(_DWORD *)(v2 + 8) = 0;
    *(_DWORD *)(v2 + 12) = 0;
    operator delete((void *)v2, (const struct std::nothrow_t *)0x10);
  }
  return (unsigned int)ProcessingModeCharacteristics;
}
