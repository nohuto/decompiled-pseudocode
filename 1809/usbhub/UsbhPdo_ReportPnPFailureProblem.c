/*
 * XREFs of UsbhPdo_ReportPnPFailureProblem @ 0x1C0059BF8
 * Callers:
 ...
 *     RtlUnalignedStringCbLengthW @ 0x1C0058368 (RtlUnalignedStringCbLengthW.c)
 */
...
void __fastcall UsbhPdo_ReportPnPFailureProblem(PDEVICE_OBJECT DeviceObject)
{
  ...
  if ( v2[705] )
  {
    LODWORD(pcbLength) = PdoExt((__int64)DeviceObject)[705];
    UsbhSetPdoRegistryParameter(DeviceObject, L"EnumerationFailureCode", 4u, &pcbLength, 4u);
    if ( RtlFindMessage(WPP_MAIN_CB.Queue.ListEntry.Blink[1].Blink, 0xBu, 0, v2[705], &v8) >= 0
