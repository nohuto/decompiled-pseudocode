/*
 * XREFs of UsbhHubResetIrqPipeWorker @ 0x1C0047BA0
 * Callers:
 *     <none>
 * Callees:
 *     Usb_Disconnected @ 0x1C00047AC (Usb_Disconnected.c)
 *     UsbhDecHubBusy @ 0x1C000E470 (UsbhDecHubBusy.c)
 *     UsbhPostInterrupt @ 0x1C000FCA0 (UsbhPostInterrupt.c)
 *     FdoExt @ 0x1C00122E0 (FdoExt.c)
 *     Log @ 0x1C0012D10 (Log.c)
 *     UsbhIncHubBusy @ 0x1C0019500 (UsbhIncHubBusy.c)
 *     WPP_RECORDER_SF_d @ 0x1C0040078 (WPP_RECORDER_SF_d.c)
 *     UsbhResetIrqPipe @ 0x1C0048168 (UsbhResetIrqPipe.c)
 *     UsbhException @ 0x1C0053928 (UsbhException.c)
 */

NTSTATUS __fastcall UsbhHubResetIrqPipeWorker(PDEVICE_OBJECT DeviceObject, unsigned int a2, __int64 a3)
{
  __int64 v4; // rsi
  _DWORD *v6; // rdi
  _QWORD *v7; // r14
  NTSTATUS result; // eax
  __int64 v9; // rbp
  __int64 v10; // rdx
  struct _KEVENT *v11; // r9
  __int64 v12; // rdx
  PDEVICE_OBJECT v13; // rcx
  unsigned __int16 v14; // r9
  __int64 v15; // [rsp+28h] [rbp-50h]
  int v16; // [rsp+48h] [rbp-30h]

  v4 = a2;
  if ( LOWORD(WPP_GLOBAL_Control->DeviceType) )
    WPP_RECORDER_SF_d(
      (__int64)WPP_GLOBAL_Control->DeviceExtension,
      0,
      1u,
      0x1Fu,
      (__int64)&WPP_059b6fb8637c348d1b3657d61bfde3a6_Traceguids,
      a2);
  Log((__int64)DeviceObject, 4, 1769107794, a3, v4);
  v6 = FdoExt((__int64)DeviceObject);
  v7 = (_QWORD *)UsbhIncHubBusy((__int64)DeviceObject, a3, (__int64)DeviceObject, 1380086643, 0);
  _InterlockedAdd(v6 + 684, 1u);
  result = v6[640];
  if ( (result & 0x1000) != 0 )
  {
    v6[640] = result & 0xFFFFEFFF;
    Log((__int64)DeviceObject, 4, 1769107794, 0LL, 0LL);
    if ( LOWORD(WPP_GLOBAL_Control->DeviceType) )
    {
      LODWORD(v15) = v4;
      WPP_RECORDER_SF_d(
        (__int64)WPP_GLOBAL_Control->DeviceExtension,
        0,
        1u,
        0x20u,
        (__int64)&WPP_059b6fb8637c348d1b3657d61bfde3a6_Traceguids,
        v15);
    }
    v9 = (int)UsbhResetIrqPipe(DeviceObject);
    Log((__int64)DeviceObject, 4, 1766999379, 0LL, v9);
    if ( (v9 & 0xC0000000) == 0xC0000000 )
    {
      if ( Usb_Disconnected(v9) )
      {
        KeSetEvent(v11, 0, 0);
        _InterlockedDecrement(v6 + 684);
        v13 = WPP_GLOBAL_Control;
        if ( !LOWORD(WPP_GLOBAL_Control->DeviceType) )
          return UsbhDecHubBusy((__int64)DeviceObject, v12, v7);
        v14 = 33;
      }
      else
      {
        _InterlockedDecrement(v6 + 684);
        KeSetEvent(v11, 0, 0);
        if ( !*((_BYTE *)v6 + 5268) )
        {
          LOBYTE(v16) = 1;
          UsbhException((int)DeviceObject, 0, 7, 0, 0, v9, 0, usbfile_bus_c, 2768, v16);
        }
        v13 = WPP_GLOBAL_Control;
        if ( !LOWORD(WPP_GLOBAL_Control->DeviceType) )
          return UsbhDecHubBusy((__int64)DeviceObject, v12, v7);
        v14 = 34;
      }
      LODWORD(v15) = v4;
      WPP_RECORDER_SF_d(
        (__int64)v13->DeviceExtension,
        0,
        1u,
        v14,
        (__int64)&WPP_059b6fb8637c348d1b3657d61bfde3a6_Traceguids,
        v15);
      return UsbhDecHubBusy((__int64)DeviceObject, v12, v7);
    }
    _InterlockedDecrement(v6 + 684);
    UsbhDecHubBusy((__int64)DeviceObject, v10, v7);
    result = UsbhPostInterrupt(DeviceObject);
    if ( LOWORD(WPP_GLOBAL_Control->DeviceType) )
    {
      LODWORD(v15) = v4;
      return WPP_RECORDER_SF_d(
               (__int64)WPP_GLOBAL_Control->DeviceExtension,
               0,
               1u,
               0x23u,
               (__int64)&WPP_059b6fb8637c348d1b3657d61bfde3a6_Traceguids,
               v15);
    }
  }
  return result;
}
