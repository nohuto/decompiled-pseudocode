/*
 * XREFs of StorPortWorkItemRoutine @ 0x1C003F4B0
 * Callers:
 *     <none>
 * Callees:
...
void __fastcall StorPortWorkItemRoutine(PDEVICE_OBJECT DeviceObject, PVOID Context)
{
  ...
  if ( Context )
  {
    ...
    if ( v4 )
    {
      v4(*((_QWORD *)DeviceExtension + 69) + 16LL, *((_QWORD *)Context + 2), Context);
      if ( _InterlockedExchange((volatile __int32 *)Context + 6, 0) == 2 )
      {
        IoFreeWorkItem(*(PIO_WORKITEM *)Context);
        *(_QWORD *)Context = 0LL;
        ExFreePoolWithTag(Context, 0x49576152u);
        _InterlockedDecrement((volatile signed __int32 *)DeviceExtension + 1262);
      }
    }
  }
  if ( DeviceExtension )
