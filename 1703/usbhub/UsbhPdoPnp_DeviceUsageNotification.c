/*
 * XREFs of UsbhPdoPnp_DeviceUsageNotification @ 0x1C0055A50
 * Callers:
 *     <none>
 * Callees:
 ...
 *     UsbhPdo_IsDeviceExternal @ 0x1C007270C (UsbhPdo_IsDeviceExternal.c)
 */
...
LABEL_7:
    ...
    if ( v10 )
    {
      ...
      if ( Status >= 0 )
      {
        ...
        {
          if ( Lock )
            _InterlockedIncrement(v6 + 678);
          else
            _InterlockedDecrement(v6 + 678);
          if ( Options == 2 )
          {
            if ( Lock )
              _InterlockedIncrement(v6 + 694);
            else
              _InterlockedDecrement(v6 + 694);
          }
          if ( v6[678] )
    ...
    if ( Options != 4 )
      goto LABEL_31;
LABEL_26:
    if ( RtlCheckRegistryKey(2u, (PWSTR)L"MiniNT") < 0 && !*((_BYTE *)v6 + 2716) )
    {
      if ( IsDeviceExternal == 1 )
      {
        v15 = UsbhRegisterBootDeviceNotification(DeviceObject, v14, v6 + 682);
