/*
 * XREFs of UsbhCompletePdoIdleIrp @ 0x1C0001280
 * Callers:
 *     UsbhPdoSetDx @ 0x1C0001100 (UsbhPdoSetDx.c)
 *     UsbhPdoSetD0 @ 0x1C0003620 (UsbhPdoSetD0.c)
 *     UsbhPdoRemoveCleanup @ 0x1C0056160 (UsbhPdoRemoveCleanup.c)
 * Callees:
 *     UsbhSetIdleIrpState @ 0x1C0001350 (UsbhSetIdleIrpState.c)
 *     UsbhGetIdleIrpState @ 0x1C00013BC (UsbhGetIdleIrpState.c)
 *     Log @ 0x1C0013A00 (Log.c)
 *     PdoExt @ 0x1C0018C00 (PdoExt.c)
 *     UsbhIdleIrp_Event @ 0x1C0056EE8 (UsbhIdleIrp_Event.c)
 */

__int64 __fastcall UsbhCompletePdoIdleIrp(int a1, __int64 a2, int a3)
{
  __int64 v6; // rsi
  struct _KEVENT *v7; // r14
  __int64 v8; // rcx
  int v9; // ecx
  int IdleIrpState; // r9d
  PIRP v11; // rsi
  __int64 result; // rax
  char v13; // [rsp+20h] [rbp-28h]
  char v14; // [rsp+60h] [rbp+18h] BYREF

  v6 = PdoExt(a2);
  v7 = (struct _KEVENT *)(v6 + 2048);
  KeWaitForSingleObject((PVOID)(v6 + 2048), Executive, 0, 0, 0LL);
  IdleIrpState = UsbhGetIdleIrpState(v8, a2, &v14);
  v13 = v14;
  if ( IdleIrpState == 2 )
  {
    *(_DWORD *)(v6 + 1432) = a3;
    IdleIrpState = 3;
  }
  UsbhSetIdleIrpState(v9, a2, 9, IdleIrpState, v13);
  v11 = IoCsqRemoveNextIrp((PIO_CSQ)(v6 + 1456), 0LL);
  KeSetEvent(v7, 0, 0);
  result = Log(a1, 16, 1668303172, a3, (__int64)v11);
  if ( v11 )
    return UsbhIdleIrp_Event(a1, a2, (_DWORD)v11, 5, a3);
  return result;
}
