/*
 * XREFs of UsbhPdo_ReportPnPFailureProblem @ 0x1C00562F0
 * Callers:
 ...
 *     RtlUnalignedStringCbLengthW @ 0x1C0054ADC (RtlUnalignedStringCbLengthW.c)
 */
...
void __fastcall UsbhPdo_ReportPnPFailureProblem(PDEVICE_OBJECT DeviceObject)
{
  ...
  if ( v2[699] )
  {
    LODWORD(pcbLength) = PdoExt((__int64)DeviceObject)[699];
    UsbhSetPdoRegistryParameter(DeviceObject, L"EnumerationFailureCode", 4u, &pcbLength, 4u);
    if ( RtlFindMessage(WPP_MAIN_CB.Queue.ListEntry.Blink[1].Blink, 0xBu, 0, v2[699], &v8) >= 0
      && RtlUnalignedStringCbLengthW((STRSAFE_PCUNZWCH)v8->Text, v8->Length - 4, &pcbLength) >= 0 )
    {
      v3 = pcbLength + 86;
      pcbLength += 86LL;
      ...
      if ( PoolWithTag )
      {
        ...
        LODWORD(MessageResourceEntry) = v2[699];
