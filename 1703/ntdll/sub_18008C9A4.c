...
__int64 __fastcall sub_18008C9A4(HANDLE Handle, PACL Dacl)
{
  ...
  v2 = 0;
  Dacla = 0LL;
  if ( ZwQuerySecurityObject(Handle, 5u, 0LL, 0, &Length) != -1073741789 )
    return 0LL;
  Heap = RtlAllocateHeap(NtCurrentPeb()->ProcessHeap, 0, Length);
  if ( !Heap )
    return 3221225626LL;
  DaclSecurityDescriptor = ZwQuerySecurityObject(Handle, 5u, Heap, Length, &Length);
    ...
    for ( i = 0; ; ++i )
    {
      ...
      if ( !*v9 && RtlEqualSid(v9 + 8, &v32) )
      {
        if ( (v9[1] & 3) == 3 )
          goto LABEL_17;
        v9[1] |= 3u;
        SelfRelativeSD = ZwSetSecurityObject(Handle, 4u, Heap);
        goto LABEL_24;
      }
    }
  }
...
LABEL_17:
    ...
    if ( v17 )
    {
      ...
      if ( v15 < 0 )
        goto LABEL_29;
      Length = BufferSize;
      v2 = ZwSetSecurityObject(Handle, 5u, v18);
      v10 = v18;
LABEL_18:
