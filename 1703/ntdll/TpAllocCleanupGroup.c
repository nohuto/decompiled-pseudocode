/*
 * XREFs of TpAllocCleanupGroup @ 0x180082B50
 * Callers:
 *     <none>
 * Callees:
...
NTSTATUS __cdecl TpAllocCleanupGroup(PTP_CLEANUP_GROUP *CleanupGroupReturn)
{
  ...
  {
    sub_1801058B8(CleanupGroupReturn);
    return -1073741811;
  }
  else
  {
    *CleanupGroupReturn = 0LL;
    Heap = RtlAllocateHeap(NtCurrentPeb()->ProcessHeap, dword_18015C000 | 8, 0x50uLL);
    ...
    if ( result < 0 )
    {
      if ( v3 )
      {
        RtlFreeHeap(NtCurrentPeb()->ProcessHeap, dword_18015C000, BaseAddress);
        return v6;
      }
    }
  }
