/*
 * XREFs of RtlWaitForWnfMetaNotification @ 0x180087110
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall RtlWaitForWnfMetaNotification(WNF_STATE_NAME a1, int a2, unsigned int a3, __int64 a4, int *a5)
{
  ...
  *a5 = 0;
  ...
  if ( (a2 & 0xFFFFFFF1) == 0 )
  {
    ...
    if ( (a2 & 8) != 0 )
    {
      v8 = ZwQueryWnfStateNameInformation(&StateName, WnfInfoIsQuiescent, 0LL, &InfoBuffer, 4u);
      if ( v8 )
        goto LABEL_16;
      if ( InfoBuffer )
        v7 = 8;
...
LABEL_9:
      ...
      if ( v8 >= 0 )
      {
        ...
        v8 = RtlRegisterForWnfMetaNotification(
               ...
               StateName.Data[0],
               a2,
               (unsigned int)sub_180089910,
               (__int64)&v14);
        if ( v8 >= 0 )
        {
          Timeout.QuadPart = -10000 * v6;
          v8 = ZwWaitForSingleObject(EventHandle, 0, &Timeout);
          if ( !v8 )
            *a5 = v14;
    ...
    v8 = ZwQueryWnfStateNameInformation(&StateName, WnfInfoSubscribersPresent, 0LL, &InfoBuffer, 4u);
    if ( !v8 )
    {
