...
int __fastcall sub_18001094C(char *BaseOfImage, __int64 a2)
{
    ...
    *(_DWORD *)(a2 + 4) = v5->OptionalHeader.CheckSum;
    ...
    if ( !v6 )
    {
      ...
      if ( (_DWORD)VirtualAddress )
      {
        ...
        {
          LODWORD(VirtualAddress) = -(int)sub_180010B8C(&DestinationString, &BaseOfImage[VirtualAddress], 16LL);
          v8 = v7 & -(__int64)((_DWORD)VirtualAddress != 0);
          if ( v8 )
          {
            VirtualAddress = sub_180010BC4(
                               &DestinationString,
                               v7 & -(__int64)((_DWORD)VirtualAddress != 0),
                               *(unsigned __int16 *)((v7 & -(__int64)((_DWORD)VirtualAddress != 0)) + 0xC));
            if ( VirtualAddress )
            {
              VirtualAddress = sub_180010C00(&DestinationString, VirtualAddress, *(unsigned __int16 *)(v8 + 14), 16LL);
              if ( VirtualAddress )
              {
                v9 = *(unsigned int *)(VirtualAddress + 4);
                LODWORD(v9) = v9 & 0x7FFFFFFF;
                LODWORD(VirtualAddress) = sub_180010B8C(&DestinationString, v8 + v9, 16LL);
                if ( (_DWORD)VirtualAddress )
                {
                  VirtualAddress = sub_180010BC4(&DestinationString, v10, *(unsigned __int16 *)(v11 + 12));
                  if ( VirtualAddress )
                  {
                    VirtualAddress = sub_180010C00(
                                       &DestinationString,
                                       VirtualAddress,
                    ...
                    if ( VirtualAddress )
                    {
                      ...
                      LODWORD(VirtualAddress) = sub_180010B8C(&DestinationString, v8 + v13, 16LL);
                      if ( (_DWORD)VirtualAddress )
                      {
                        VirtualAddress = sub_180010BC4(&DestinationString, v14, 0LL);
                        if ( VirtualAddress )
                        {
                          if ( *(_DWORD *)(VirtualAddress + 4) < 0x80000000 )
                          {
                            LODWORD(VirtualAddress) = sub_180010B8C(
                                                        &DestinationString,
                            ...
                            if ( (_DWORD)VirtualAddress )
                            {
                              if ( v15[1] >= 0x5C )
                              {
                                LODWORD(VirtualAddress) = -(int)sub_180010B8C(
                                                                  &DestinationString,
                                                                  &BaseOfImage[*v15],
                                                                  92LL);
                                ...
                                if ( v18 )
                                {
                                  ...
                                  if ( (_DWORD)VirtualAddress )
                                  {
                                    LODWORD(VirtualAddress) = sub_180010B8C(&DestinationString, v18 + 6, 32LL);
                                    if ( (_DWORD)VirtualAddress )
                                    {
                                      RtlInitUnicodeString(&DestinationString, L"VS_VERSION_INFO");
                                      LODWORD(VirtualAddress) = sub_180010C70(v18 + 6, v19, &OutHeaders);
                                      if ( (int)VirtualAddress >= 0 )
                                      {
