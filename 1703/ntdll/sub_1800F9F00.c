/*
 * XREFs of sub_1800F9F00 @ 0x1800F9F00
 * Callers:
 *     RtlTraceDatabaseFind @ 0x1800F9BC0 (RtlTraceDatabaseFind.c)
 *     sub_1800F9D5C @ 0x1800F9D5C (sub_1800F9D5C.c)
 * Callees:
 *     _guard_dispatch_icall_nop @ 0x1800A8C20 (_guard_dispatch_icall_nop.c)
 */

char __fastcall sub_1800F9F00(__int64 a1, unsigned int a2, __int64 a3, _QWORD *a4)
{
  unsigned int v8; // eax
  __int64 i; // rdx
  __int64 j; // r8

  v8 = (*(__int64 (__fastcall **)(_QWORD, __int64))(a1 + 104))(a2, a3);
  ++*(_DWORD *)(a1 + 4LL * (v8 & 0xF) + 128);
  for ( i = *(_QWORD *)(*(_QWORD *)(a1 + 96) + 8LL * (v8 % *(_DWORD *)(a1 + 88))); ; i = *(_QWORD *)(i + 40) )
  {
    if ( !i )
    {
      if ( a4 )
        *a4 = 0LL;
      return 0;
    }
    if ( a2 == *(_DWORD *)(i + 8) )
    {
      for ( j = 0LL; (unsigned int)j < a2; j = (unsigned int)(j + 1) )
      {
        if ( *(_QWORD *)(*(_QWORD *)(i + 48) + 8 * j) != *(_QWORD *)(a3 + 8 * j) )
          break;
      }
      if ( (_DWORD)j == a2 )
        break;
    }
  }
  if ( a4 )
    *a4 = i;
  return 1;
}
