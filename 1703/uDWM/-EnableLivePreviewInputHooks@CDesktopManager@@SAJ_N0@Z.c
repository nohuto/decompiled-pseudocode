/*
 * XREFs of ?EnableLivePreviewInputHooks@CDesktopManager@@SAJ_N0@Z @ 0x1800789F8
 * Callers:
...
__int64 __fastcall CDesktopManager::EnableLivePreviewInputHooks(char a1, char a2)
{
  ...
  if ( !a1 )
  {
    CIconicBitmapRegistry::OnLivePreviewDismissed(*((CIconicBitmapRegistry **)CDesktopManager::s_pDesktopManagerInstance
                                                  + 26));
    v4 = (HWND)*((_QWORD *)CDesktopManager::s_pDesktopManagerInstance + 74);
    if ( v4 )
  ...
  v5 = CDesktopManager::EnableInputHooksHelper(
         ...
         a2,
         L"LivePreview",
         (HWND *)CDesktopManager::s_pDesktopManagerInstance + 74);
  if ( a1 )
  {
    v6 = (HWND)*((_QWORD *)CDesktopManager::s_pDesktopManagerInstance + 74);
    if ( v6 )
