/*
 * XREFs of ?_SnapshotWindows@CDesktopThumbnailBase@@IEAAJXZ @ 0x18009E6DC
 * Callers:
  ...
  if ( v1 != -1 )
  {
    ...
    if ( WindowListForDesktop->Flink != WindowListForDesktop )
    {
      while ( 1 )
      {
        if ( Flink )
        {
          if ( (*(unsigned __int8 (__fastcall **)(CDesktopThumbnailBase *, _QWORD))(*(_QWORD *)this + 192LL))(
                 this,
                 LODWORD(Flink[7].Flink)) )
          {
            if ( Flink[25].Flink )
            {
              ...
              if ( (v6 & 1) != 0 && ((v6 | BYTE4(Flink[37].Flink)) & 4) == 0 )
              {
                if ( Flink[2].Blink )
                {
