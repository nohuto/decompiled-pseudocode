/*
 * XREFs of VidSchiProfilePerformanceTick @ 0x1C00093B0
 * Callers:
 *     VidSchiCheckPreemptionPolicy @ 0x1C0006030 (VidSchiCheckPreemptionPolicy.c)
 *     VidSchiSendToExecutionQueue @ 0x1C0006B50 (VidSchiSendToExecutionQueue.c)
 *     VidSchiProcessDpcCompletedPacket @ 0x1C00072F0 (VidSchiProcessDpcCompletedPacket.c)
 *     ?VidSchiProcessCompletedQueuePacketInternal@@YAPEAU_VIDSCH_QUEUE_PACKET@@PEAU1@@Z @ 0x1C0008200 (-VidSchiProcessCompletedQueuePacketInternal@@YAPEAU_VIDSCH_QUEUE_PACKET@@PEAU1@@Z.c)
 *     VidSchiScheduleCommandToRun @ 0x1C0008F20 (VidSchiScheduleCommandToRun.c)
 *     VidSchiSwitchContextWithCheck @ 0x1C00098D0 (VidSchiSwitchContextWithCheck.c)
 *     VidSchiProcessIsrCompletedPacket @ 0x1C000B190 (VidSchiProcessIsrCompletedPacket.c)
 *     VidSchiProcessIsrPreemptedPacket @ 0x1C000D838 (VidSchiProcessIsrPreemptedPacket.c)
 *     VidSchiProcessDpcPreemptedPacket @ 0x1C000DF80 (VidSchiProcessDpcPreemptedPacket.c)
 *     VidSchiProcessDpcSystemRequest @ 0x1C000E3A0 (VidSchiProcessDpcSystemRequest.c)
 *     VidSchiNeedToForcePreemptNode @ 0x1C0010794 (VidSchiNeedToForcePreemptNode.c)
 *     VidSchiProcessIsrFaultedPacket @ 0x1C0026704 (VidSchiProcessIsrFaultedPacket.c)
 *     VidSchiReadCommandFromContextQueue @ 0x1C0026C00 (VidSchiReadCommandFromContextQueue.c)
 *     VidSchiSelectContext @ 0x1C002787C (VidSchiSelectContext.c)
 *     VidSchiCompletePendingCommandInNodeHwQueue @ 0x1C002AC84 (VidSchiCompletePendingCommandInNodeHwQueue.c)
 *     VidSchiWaitForSchedulerEvents @ 0x1C006AE70 (VidSchiWaitForSchedulerEvents.c)
 *     VidSchiSubmitCommandPacketToQueue @ 0x1C006B1A0 (VidSchiSubmitCommandPacketToQueue.c)
 *     VidSchiWaitForDrainFlipQueue @ 0x1C006D098 (VidSchiWaitForDrainFlipQueue.c)
 *     VidSchiWaitForCompletePreemption @ 0x1C006DB54 (VidSchiWaitForCompletePreemption.c)
 *     VidSchiWaitForEmptyHwQueue @ 0x1C00AB6B4 (VidSchiWaitForEmptyHwQueue.c)
 * Callees:
 *     ?VidSchiAddGpuWorkEntry@@YAXPEAU_VIDSCH_CONTEXT@@_N@Z @ 0x1C0012048 (-VidSchiAddGpuWorkEntry@@YAXPEAU_VIDSCH_CONTEXT@@_N@Z.c)
 *     __security_check_cookie @ 0x1C0014430 (__security_check_cookie.c)
 *     Template_pq @ 0x1C001FA14 (Template_pq.c)
 *     Template_pqq @ 0x1C001FA90 (Template_pqq.c)
 *     Template_ppqqxx @ 0x1C00241C0 (Template_ppqqxx.c)
 *     Template_ppqxqpq @ 0x1C0024284 (Template_ppqxqpq.c)
 *     Template_pqqPR2 @ 0x1C0024578 (Template_pqqPR2.c)
 *     Template_pqqpx @ 0x1C00247F8 (Template_pqqpx.c)
 *     Template_pqqqPR3XR3 @ 0x1C00249A4 (Template_pqqqPR3XR3.c)
 *     Template_pqqtt @ 0x1C0024C64 (Template_pqqtt.c)
 *     Template_pqqxqqtp @ 0x1C0024F68 (Template_pqqxqqtp.c)
 *     Template_pqxqqqxqp @ 0x1C00251D4 (Template_pqxqqqxqp.c)
 *     Template_pqxqt @ 0x1C00252CC (Template_pqxqt.c)
 *     Template_qtpiixi @ 0x1C00256EC (Template_qtpiixi.c)
 *     ?AllocateElements@?$PagedPoolZeroedArray@_K$01@@QEAAPEA_KI@Z @ 0x1C00AA560 (-AllocateElements@-$PagedPoolZeroedArray@_K$01@@QEAAPEA_KI@Z.c)
 */

void __fastcall VidSchiProfilePerformanceTick(
        __int64 a1,
        __int64 a2,
        __int64 a3,
        __int64 a4,
        __int64 a5,
        __int64 a6,
        __int64 a7,
        __int64 a8)
{
  int v10; // ebx
  int v11; // esi
  __int64 v12; // rdx
  __int64 v13; // rdx
  __int64 v14; // rdx
  __int64 v15; // rdx
  __int64 v16; // rax
  __int64 v17; // rdx
  __int64 v18; // rdx
  __int64 v19; // rdx
  void *v20; // rdx
  __int64 v21; // r9
  __int64 v22; // rax
  __int64 v23; // rdx
  int v24; // ecx
  int v25; // r8d
  int v26; // edx
  __int64 v27; // rcx
  __int64 v28; // r9
  __int64 v29; // rdx
  __int64 v30; // rax
  __int64 v31; // rdx
  __int64 v32; // rax
  __int64 v33; // r8
  unsigned int i; // edx
  int v35; // eax
  __int64 v36; // rcx
  __int64 v37; // r9
  int v38; // edx
  int v39; // r8d
  __int64 v40; // rcx
  __int64 v41; // r9
  __int64 v42; // rcx
  __int64 v43; // r9
  __int16 v44; // ax
  int v45; // edx
  __int64 v46; // rcx
  __int64 v47; // r9
  __int64 v48; // rcx
  __int64 v49; // r9
  __int64 v50; // rax
  int v51; // r8d
  __int64 v52; // rcx
  __int64 v53; // rdx
  __int64 v54; // r9
  __int64 v55; // rcx
  int v56; // r10d
  __int64 v57; // r11
  int v58; // edx
  __int64 v59; // r8
  __int64 v60; // r9
  __int64 v61; // rdx
  __int64 v62; // r9
  __int64 v63; // rcx
  __int64 v64; // r9
  __int64 v65; // rdx
  __int64 v66; // r9
  __int64 v67; // r8
  __int64 v68; // rdx
  __int64 v69; // rdx
  __int64 v70; // rax
  int v71; // [rsp+20h] [rbp-B8h]
  int v72; // [rsp+60h] [rbp-78h]
  __int64 v73; // [rsp+60h] [rbp-78h]
  __int64 v74; // [rsp+60h] [rbp-78h]
  PVOID v75; // [rsp+68h] [rbp-70h] BYREF
  char v76; // [rsp+70h] [rbp-68h] BYREF
  int v77; // [rsp+80h] [rbp-58h]
  struct _KLOCK_QUEUE_HANDLE P; // [rsp+88h] [rbp-50h] BYREF
  int v79; // [rsp+A0h] [rbp-38h]

  v10 = a1;
  v11 = a8;
  if ( bTracingEnabled )
  {
    switch ( (int)a1 )
    {
      case 1:
        v21 = *(_QWORD *)(a5 + 56);
        if ( !v21 || (*(_DWORD *)(a5 + 112) & 0x40) != 0 )
          v21 = a5;
        if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
        {
          v71 = a8;
          v20 = &EventAttemptPreemption;
          goto LABEL_25;
        }
        break;
      case 2:
        if ( a5 )
        {
          v22 = *(_QWORD *)(a5 + 96);
          v23 = *(_QWORD *)(a5 + 56);
          v24 = *(_DWORD *)(v22 + 1688);
          v25 = *(unsigned __int16 *)(v22 + 4);
          if ( !v23 || (*(_DWORD *)(a5 + 112) & 0x40) != 0 )
            LODWORD(v23) = a5;
        }
        else
        {
          v24 = 0;
          v25 = 0;
          LODWORD(v23) = 0;
        }
        if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x80u) != 0 )
          Template_ppqqxx(
            v24,
            v23,
            v25,
            *(_QWORD *)(a2 + 16),
            v23,
            v25,
            v24,
            *(_QWORD *)(a2 + 392),
            *(_QWORD *)(a2 + 400));
        break;
      case 4:
        v26 = *(_DWORD *)(a6 + 48);
        if ( v26 )
        {
          if ( v26 == 4 )
          {
            KeAcquireInStackQueuedSpinLock((PKSPIN_LOCK)(a2 + 1880), &P);
            v40 = *(_QWORD *)(a6 + 88);
            v41 = *(_QWORD *)(v40 + 56);
            if ( !v41 || (*(_DWORD *)(v40 + 112) & 0x40) != 0 )
              v41 = *(_QWORD *)(a6 + 88);
            if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
              Template_pqqpx(
                v40,
                v38,
                v39,
                v41,
                *(_DWORD *)(a6 + 104),
                *(_DWORD *)(a6 + 264),
                *(_QWORD *)(a6 + 272),
                *(_QWORD *)(a6 + 296));
            KeReleaseInStackQueuedSpinLock(&P);
          }
          else if ( v26 == 5 )
          {
            v29 = *(unsigned int *)(a6 + 268);
            v75 = 0LL;
            v77 = 0;
            v30 = PagedPoolZeroedArray<unsigned __int64,2>::AllocateElements(&v75, v29);
            v31 = *(unsigned int *)(a6 + 268);
            v73 = v30;
            P.LockQueue.Next = 0LL;
            v79 = 0;
            v32 = PagedPoolZeroedArray<unsigned __int64,2>::AllocateElements(&P, v31);
            v33 = v32;
            if ( v73 && v32 )
            {
              for ( i = 0; i < *(_DWORD *)(a6 + 268); ++i )
              {
                *(_QWORD *)(v73 + 8LL * i) = *(_QWORD *)(a6 + 8LL * i + 272);
                v35 = *(_DWORD *)(*(_QWORD *)(a6 + 8LL * i + 272) + 40LL);
                if ( v35 == 2 || (unsigned int)(v35 - 4) <= 1 )
                  *(_QWORD *)(v33 + 8LL * i) = *(_QWORD *)(a6 + 8LL * i + 528);
                else
                  *(_QWORD *)(v33 + 8LL * i) = 0LL;
              }
              v36 = *(_QWORD *)(a6 + 88);
              v37 = *(_QWORD *)(v36 + 56);
              if ( !v37 || (*(_DWORD *)(v36 + 112) & 0x40) != 0 )
                v37 = *(_QWORD *)(a6 + 88);
              if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
                Template_pqqqPR3XR3(
                  v36,
                  i,
                  v33,
                  v37,
                  *(_DWORD *)(a6 + 104),
                  *(_DWORD *)(a6 + 264),
                  *(_DWORD *)(a6 + 268),
                  v73,
                  v33);
            }
            if ( (volatile PKSPIN_LOCK *)P.LockQueue.Next != &P.LockQueue.Lock && P.LockQueue.Next )
              ExFreePoolWithTag(P.LockQueue.Next, 0);
            if ( v75 != &v76 && v75 )
              ExFreePoolWithTag(v75, 0);
          }
          else
          {
            v27 = *(_QWORD *)(a6 + 88);
            v28 = *(_QWORD *)(v27 + 56);
            if ( !v28 || (*(_DWORD *)(v27 + 112) & 0x40) != 0 )
              v28 = *(_QWORD *)(a6 + 88);
            if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
              Template_pqqxqqtp(
                (*(_DWORD *)(a6 + 72) >> 14) & 1,
                v26,
                a3,
                v28,
                v26,
                *(_DWORD *)(a6 + 104),
                0,
                0,
                0,
                (*(_DWORD *)(a6 + 72) & 0x4000) != 0,
                *(_QWORD *)(a6 + 272));
          }
        }
        else
        {
          v42 = *(_QWORD *)(a6 + 88);
          v43 = *(_QWORD *)(v42 + 56);
          if ( !v43 || (*(_DWORD *)(v42 + 112) & 0x40) != 0 )
            v43 = *(_QWORD *)(a6 + 88);
          v44 = Microsoft_Windows_DxgKrnlEnableBits;
          if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
          {
            Template_pqqxqqtp(
              (*(_DWORD *)(a6 + 72) >> 14) & 1,
              *(_DWORD *)(a6 + 316),
              a3,
              v43,
              0,
              *(_DWORD *)(a6 + 104),
              *(_DWORD *)(a6 + 316),
              *(_DWORD *)(a6 + 324),
              *(_DWORD *)(a6 + 332),
              (*(_DWORD *)(a6 + 72) & 0x4000) != 0,
              *(_QWORD *)(a6 + 272));
            v44 = Microsoft_Windows_DxgKrnlEnableBits;
          }
          v45 = *(_DWORD *)(a6 + 456);
          if ( v45 && (v44 & 0x400) != 0 )
          {
            v46 = *(_QWORD *)(a6 + 88);
            v47 = *(_QWORD *)(v46 + 56);
            if ( !v47 || (*(_DWORD *)(v46 + 112) & 0x40) != 0 )
              v47 = *(_QWORD *)(a6 + 88);
            Template_pqqPR2(v46, v45, a3, v47, *(_DWORD *)(a6 + 104), v45, a6 + 464);
          }
        }
        break;
      case 5:
        v48 = *(_QWORD *)(a6 + 88);
        v49 = *(_QWORD *)(v48 + 56);
        if ( !v49 || (*(_DWORD *)(v48 + 112) & 0x40) != 0 )
          v49 = *(_QWORD *)(a6 + 88);
        if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
          Template_pqq(v48, (unsigned int)&EventQueueSelect, a3, v49, *(_DWORD *)(a6 + 48), *(_DWORD *)(a6 + 104));
        break;
      case 7:
        v74 = 0LL;
        if ( *(_DWORD *)(a7 + 88) == 1 )
        {
          v50 = *(_QWORD *)(a7 + 56);
          if ( v50 )
          {
            if ( *(_DWORD *)(v50 + 48) == 8 )
              v74 = *(_QWORD *)(v50 + 88);
          }
        }
        VidSchiAddGpuWorkEntry(*(struct _VIDSCH_CONTEXT **)(a7 + 48), 1);
        if ( v74 )
        {
          v52 = *(_QWORD *)(v74 + 56);
          if ( !v52 || (*(_DWORD *)(v74 + 112) & 0x40) != 0 )
            LODWORD(v52) = v74;
        }
        else
        {
          LODWORD(v52) = 0;
        }
        v53 = *(_QWORD *)(a7 + 48);
        v54 = *(_QWORD *)(v53 + 56);
        if ( !v54 || (*(_DWORD *)(v53 + 112) & 0x40) != 0 )
          v54 = *(_QWORD *)(a7 + 48);
        if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
          Template_ppqxqpq(
            v52,
            v53,
            v51,
            v54,
            v52,
            *(_DWORD *)(a7 + 88),
            *(_QWORD *)(a7 + 104),
            *(_DWORD *)(a7 + 96),
            *(_QWORD *)(a7 + 64),
            *(_DWORD *)(v53 + 448));
        break;
      case 8:
      case 10:
      case 12:
        LODWORD(v55) = 0;
        LOBYTE(v56) = 0;
        LOBYTE(v57) = 0;
        switch ( v10 )
        {
          case 8:
            v58 = 1;
            break;
          case 10:
            v58 = 2;
            break;
          case 12:
            v56 = *(_DWORD *)(a8 + 32);
            v58 = 9;
            v55 = *(_QWORD *)(a8 + 40);
            v57 = *(_QWORD *)(a8 + 64);
            break;
          default:
            v58 = v72;
            break;
        }
        if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
        {
          v59 = *(_QWORD *)(a7 + 48);
          v60 = *(_QWORD *)(v59 + 56);
          if ( !v60 || (*(_DWORD *)(v59 + 112) & 0x40) != 0 )
            v60 = *(_QWORD *)(a7 + 48);
          Template_pqxqqqxqp(
            v55,
            v58,
            v59,
            v60,
            *(_DWORD *)(a7 + 88),
            *(_QWORD *)(a7 + 104),
            *(_DWORD *)(a7 + 96),
            v58,
            *(_DWORD *)(v59 + 448),
            v55,
            v56,
            v57);
        }
        break;
      case 9:
      case 11:
      case 13:
        if ( a7 )
        {
          v61 = *(_QWORD *)(a7 + 48);
          v62 = *(_QWORD *)(v61 + 56);
          if ( !v62 || (*(_DWORD *)(v61 + 112) & 0x40) != 0 )
            v62 = *(_QWORD *)(a7 + 48);
          if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
            Template_pqxqt(
              a1 == 11,
              v61,
              a3,
              v62,
              *(_DWORD *)(a7 + 88),
              *(_QWORD *)(a7 + 104),
              *(_DWORD *)(a7 + 96),
              (_DWORD)a1 == 11);
        }
        else if ( a6 )
        {
          if ( (_DWORD)a1 == 9 && *(_DWORD *)(a6 + 48) == 4 )
          {
            v63 = *(_QWORD *)(a6 + 88);
            v64 = *(_QWORD *)(v63 + 56);
            if ( !v64 || (*(_DWORD *)(v63 + 112) & 0x40) != 0 )
              v64 = *(_QWORD *)(a6 + 88);
            if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
              Template_pqqtt(v63, 0xC0000000, a3, v64, 4, *(_DWORD *)(a6 + 104), (*(_DWORD *)(a6 + 264) & 2) != 0);
          }
          else
          {
            v65 = *(_QWORD *)(a6 + 88);
            v66 = *(_QWORD *)(v65 + 56);
            if ( !v66 || (*(_DWORD *)(v65 + 112) & 0x40) != 0 )
              v66 = *(_QWORD *)(a6 + 88);
            if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
              Template_pqqtt(a1 == 11, v65, a3, v66, *(_DWORD *)(a6 + 48), *(_DWORD *)(a6 + 104), (_DWORD)a1 == 11);
          }
        }
        break;
      case 14:
        if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
        {
          v71 = 1;
          goto LABEL_23;
        }
        break;
      case 15:
        if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
          Template_pq(a1, &EventExitWorkerThread, a3, *(_QWORD *)(a2 + 16), 1);
        break;
      case 16:
        if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
        {
          v71 = 3;
          goto LABEL_23;
        }
        break;
      case 17:
        if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
        {
          v71 = 3;
          v20 = &EventExitWorkerThread;
          goto LABEL_24;
        }
        break;
      case 18:
        if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
        {
          v71 = 2;
LABEL_23:
          v20 = &EventEnterWorkerThread;
          goto LABEL_24;
        }
        break;
      case 19:
        if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
        {
          v71 = 2;
          v20 = &EventExitWorkerThread;
LABEL_24:
          v21 = *(_QWORD *)(a2 + 16);
LABEL_25:
          Template_pq(a1, v20, a3, v21, v71);
        }
        break;
      case 20:
        if ( (*(_DWORD *)(a2 + 2152) & 4) != 0 && !gulPriorityToYieldPriorityBand[*(unsigned int *)(a5 + 396)] )
        {
          v67 = *(_QWORD *)(a5 + 456);
          if ( v67 <= 0 && (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
          {
            v68 = *(_QWORD *)(a5 + 56);
            if ( !v68 || (*(_DWORD *)(a5 + 112) & 0x40) != 0 )
              LODWORD(v68) = a5;
            Template_qtpiixi(
              a8 != 0,
              v68,
              v67,
              *(unsigned __int16 *)(a3 + 4),
              a8 != 0,
              v68,
              *(_QWORD *)(a5 + 480),
              v67,
              *(_QWORD *)(a5 + 472),
              *(_QWORD *)(a5 + 464));
          }
        }
        break;
      default:
        break;
    }
  }
  if ( *(int *)(a2 + 5536) >= 1 )
  {
    if ( v10 == 1 )
    {
      if ( (int)a8 < 0 )
      {
        v11 = -(int)a8;
      }
      else if ( (int)a8 >= 16 )
      {
        v11 = 1;
      }
      ++*(_DWORD *)(a3 + 2548);
      ++*(_DWORD *)(a3 + 4LL * v11 + 2548);
      v14 = *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a5 + 104) + 40LL) + 24LL)
                                  + 8LL * *(unsigned int *)(*(_QWORD *)(*(_QWORD *)(a5 + 96) + 24LL) + 4LL))
                      + 8LL * *(unsigned __int16 *)(*(_QWORD *)(a5 + 96) + 4LL)
                      + 8);
      ++*(_DWORD *)(v14 + 12);
      ++*(_DWORD *)(v14 + 4LL * v11 + 12);
    }
    else if ( v10 == 3 )
    {
      ++*(_DWORD *)(a3 + 2544);
      v18 = *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a5 + 104) + 40LL) + 24LL)
                                  + 8LL * *(unsigned int *)(*(_QWORD *)(*(_QWORD *)(a5 + 96) + 24LL) + 4LL))
                      + 8LL * *(unsigned __int16 *)(*(_QWORD *)(a5 + 96) + 4LL)
                      + 8);
      ++*(_DWORD *)(v18 + 8);
    }
  }
  if ( *(int *)(a2 + 5536) >= 2 )
  {
    switch ( v10 )
    {
      case 4:
        ++*(_DWORD *)(a3 + 8LL * *(int *)(a6 + 48) + 2612);
        v13 = *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a6 + 88) + 104LL) + 40LL) + 24LL)
                                    + 8LL
                                    * *(unsigned int *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a6 + 88) + 96LL) + 24LL)
                                                      + 4LL))
                        + 8LL * *(unsigned __int16 *)(*(_QWORD *)(*(_QWORD *)(a6 + 88) + 96LL) + 4LL)
                        + 8);
        ++*(_DWORD *)(v13 + 8LL * *(int *)(a6 + 48) + 76);
        break;
      case 7:
        ++*(_DWORD *)(a3 + 16LL * *(int *)(a7 + 88) + 2684);
        v15 = *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a7 + 48) + 104LL) + 40LL) + 24LL)
                                    + 8LL
                                    * *(unsigned int *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a7 + 48) + 96LL) + 24LL)
                                                      + 4LL))
                        + 8LL * *(unsigned __int16 *)(*(_QWORD *)(*(_QWORD *)(a7 + 48) + 96LL) + 4LL)
                        + 8);
        ++*(_DWORD *)(v15 + 16LL * *(int *)(a7 + 88) + 148);
        break;
      case 8:
        v16 = 2 * (*(int *)(a7 + 88) + 168LL);
        ++*(_DWORD *)(a3 + 8 * v16);
        v17 = *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a7 + 48) + 104LL) + 40LL) + 24LL)
                                    + 8LL
                                    * *(unsigned int *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a7 + 48) + 96LL) + 24LL)
                                                      + 4LL))
                        + 8LL * *(unsigned __int16 *)(*(_QWORD *)(*(_QWORD *)(a7 + 48) + 96LL) + 4LL)
                        + 8);
        ++*(_DWORD *)(v17 + 16LL * *(int *)(a7 + 88) + 152);
        break;
      case 9:
      case 11:
      case 13:
        if ( a6 )
        {
          ++*(_DWORD *)(a3 + 8LL * *(int *)(a6 + 48) + 2616);
          v12 = *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a6 + 88) + 104LL) + 40LL) + 24LL)
                                      + 8LL
                                      * *(unsigned int *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a6 + 88) + 96LL) + 24LL)
                                                        + 4LL))
                          + 8LL * *(unsigned __int16 *)(*(_QWORD *)(*(_QWORD *)(a6 + 88) + 96LL) + 4LL)
                          + 8);
          ++*(_DWORD *)(v12 + 8LL * *(int *)(a6 + 48) + 80);
        }
        break;
      case 10:
        ++*(_DWORD *)(a3 + 16LL * *(int *)(a7 + 88) + 2692);
        v19 = *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a7 + 48) + 104LL) + 40LL) + 24LL)
                                    + 8LL
                                    * *(unsigned int *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a7 + 48) + 96LL) + 24LL)
                                                      + 4LL))
                        + 8LL * *(unsigned __int16 *)(*(_QWORD *)(*(_QWORD *)(a7 + 48) + 96LL) + 4LL)
                        + 8);
        ++*(_DWORD *)(v19 + 16LL * *(int *)(a7 + 88) + 156);
        break;
      case 12:
        ++*(_DWORD *)(a3 + 16LL * *(int *)(a7 + 88) + 2696);
        v69 = *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a7 + 48) + 104LL) + 40LL) + 24LL)
                                    + 8LL
                                    * *(unsigned int *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a7 + 48) + 96LL) + 24LL)
                                                      + 4LL))
                        + 8LL * *(unsigned __int16 *)(*(_QWORD *)(*(_QWORD *)(a7 + 48) + 96LL) + 4LL)
                        + 8);
        v70 = 2 * (*(int *)(a7 + 88) + 10LL);
        ++*(_DWORD *)(v69 + 8 * v70);
        break;
      default:
        return;
    }
  }
}
