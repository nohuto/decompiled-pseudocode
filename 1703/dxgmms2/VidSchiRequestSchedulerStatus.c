/*
 * XREFs of VidSchiRequestSchedulerStatus @ 0x1C0071334
 * Callers:
 *     VidSchInitializeAdapter @ 0x1C00708F0 (VidSchInitializeAdapter.c)
 *     VidSchFlushAdapter @ 0x1C00AB9E0 (VidSchFlushAdapter.c)
 *     VidSchTerminateAdapter @ 0x1C00ACAA0 (VidSchTerminateAdapter.c)
 * Callees:
 *     VidSchiGetSchedulerStatus @ 0x1C0012FAC (VidSchiGetSchedulerStatus.c)
 */

__int64 __fastcall VidSchiRequestSchedulerStatus(__int64 a1, int a2, int a3)
{
  unsigned int v3; // esi

  v3 = 0;
  if ( a2 != 2 )
    ExAcquireResourceExclusiveLite((PERESOURCE)(a1 + 272), 1u);
  if ( *(_DWORD *)(a1 + 268) != a2 )
  {
    *(_DWORD *)(a1 + 268) = a2;
    *(_QWORD *)(a1 + 1432) = MEMORY[0xFFFFF78000000320];
    KeSetEvent((PRKEVENT)(a1 + 1400), 0, 0);
    if ( a3 )
    {
      *(_QWORD *)(a1 + 1480) = MEMORY[0xFFFFF78000000320];
      KeWaitForSingleObject((PVOID)(a1 + 1456), Executive, 0, 0, 0LL);
      if ( (unsigned int)VidSchiGetSchedulerStatus(a1) != a2 )
        v3 = -1073741823;
    }
  }
  if ( a2 != 2 )
    ExReleaseResourceLite((PERESOURCE)(a1 + 272));
  return v3;
}
