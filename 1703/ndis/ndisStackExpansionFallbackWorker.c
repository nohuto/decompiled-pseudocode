/*
 * XREFs of ndisStackExpansionFallbackWorker @ 0x1C005AA60
 * Callers:
 *     <none>
 * Callees:
...
void __fastcall ndisStackExpansionFallbackWorker(KSPIN_LOCK *P)
{
  KSPIN_LOCK *v1; // r15
  KIRQL v3; // al
  __int128 v4; // xmm0
  ...
  *((_DWORD *)P + 40) = 0;
  ...
  if ( NetBufferList[1] )
  {
    if ( ndisNblTrackerMode )
      ndisNblTrackerTransferOwnershipInternal(NetBufferList[1], 160LL, P[100], 0x91u, 0);
    NdisFSendNetBufferListsComplete(P, NetBufferList[1], 0);
  }
  if ( NetBufferLists )
  {
    if ( ndisNblTrackerMode )
      ndisNblTrackerTransferOwnershipInternal(NetBufferLists, 160LL, P[100], 0x8Bu, 0);
