/*
 * XREFs of ?ReportTimeout@NdisWatchdogState@@QEAAXXZ @ 0x1C00F0074
 * Callers:
 *     ndisWatchdogTimeoutWorkerRoutine @ 0x1C00F0400 (ndisWatchdogTimeoutWorkerRoutine.c)
 * Callees:
...
void __fastcall NdisWatchdogState::ReportTimeout(struct _KEVENT *this)
{
  ...
  if ( v5 )
  {
    ndisFindSomeoneToBlame(
      (unsigned int)this[8].Header.Lock,
      &this[8].Header.WaitListHead,
      this[9].Header.WaitListHead.Blink);
    LODWORD(this[6].Header.WaitListHead.Blink) = 0;
    LOBYTE(this[10].Header.WaitListHead.Blink) = 1;
  }
