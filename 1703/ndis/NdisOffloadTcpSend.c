/*
 * XREFs of NdisOffloadTcpSend @ 0x1C0066CD0
 * Callers:
 *     <none>
 * Callees:
...
NDIS_STATUS __stdcall NdisOffloadTcpSend(PNDIS_OFFLOAD_HANDLE NdisOffloadHandle, PNET_BUFFER_LIST NetBufferList)
{
  ...
  if ( (v7 & 0x50000) != 0 )
  {
    ...
    if ( (v7 & 0x10000) != 0 )
    {
      ...
      ++*(_QWORD *)(ndisPcwOffsetToPerCpuData + v8 + Number * ndisPcwPerCpuDataStride + 256);
    }
    if ( (v7 & 0x40000) != 0 )
    {
      ...
      if ( Number == -1 )
        Number = KeGetPcr()->Prcb.Number;
      *(_QWORD *)(ndisPcwOffsetToPerCpuData + v8 + Number * ndisPcwPerCpuDataStride + 272) += v10;
    }
    if ( v9 != 2 )
      KeLowerIrql(v9);
  }
