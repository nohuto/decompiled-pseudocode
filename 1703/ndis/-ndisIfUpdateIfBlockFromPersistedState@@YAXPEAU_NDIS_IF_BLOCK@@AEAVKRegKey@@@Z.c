/*
 * XREFs of ?ndisIfUpdateIfBlockFromPersistedState@@YAXPEAU_NDIS_IF_BLOCK@@AEAVKRegKey@@@Z @ 0x1C00411E8
 * Callers:
 ...
 *     ??$MakePoolPtr@U_NDIS_INTERFACE_ADDITIONAL_PROPERTIES@@@@YA?AV?$unique_ptr@U_NDIS_INTERFACE_ADDITIONAL_PROPERTIES@@U?$KFreePool@U_NDIS_INTERFACE_ADDITIONAL_PROPERTIES@@@@@wistd@@K@Z @ 0x1C00CFFA8 (--$MakePoolPtr@U_NDIS_INTERFACE_ADDITIONAL_PROPERTIES@@@@YA-AV-$unique_ptr@U_NDIS_INTERFACE_ADDI.c)
 */
...
void __fastcall ndisIfUpdateIfBlockFromPersistedState(struct _NDIS_IF_BLOCK *a1, struct KRegKey *a2)
{
  ...
  if ( (int)ndisIfReadNetworkGuidFromKey(a2, &v12) >= 0
    ...
    && (unsigned __int8)byte_1C009261D >= 3u )
  {
  ...
  if ( P )
  {
    if ( (int)Ndis::BindRegistry::ReadAdditionalNetworkInterfaceParameters(a2, (struct KRegKey *)P, v6) >= 0 )
    {
      if ( (unsigned __int8)byte_1C009261D >= 4u )
        WPP_SF__guid_(195LL, &WPP_af1106a9a46b3e0e2b308003762cd579_Traceguids, &a1->InterfaceGuid);
      updated = ndisIfUpdateInterfaceFromPersistentStore(a1->NetLuid, (struct _NDIS_INTERFACE_ADDITIONAL_PROPERTIES *)P);
      if ( updated < 0 && (unsigned __int8)byte_1C009261D >= 2u )
        WPP_SF__guid_d(196LL, v8, &a1->InterfaceGuid, (unsigned int)updated);
    }
