/*
 * XREFs of ?reset@?$KArray@V?$unique_ptr@UNDIS_BIND_FILTER_LINK@@U?$default_delete@UNDIS_BIND_FILTER_LINK@@@wistd@@@wistd@@@Rtl@@AEAAXXZ @ 0x1C00B53B0
 * Callers:
...
void __fastcall Rtl::KArray<wistd::unique_ptr<NDIS_BIND_FILTER_LINK,wistd::default_delete<NDIS_BIND_FILTER_LINK>>>::reset(
        __int64 a1)
{
  __int64 v2; // rdi
  ...
  if ( *(_QWORD *)(a1 + 8) )
  {
    ...
    while ( (_DWORD)v2 )
    {
      ...
      if ( v4 )
      {
        ...
        if ( Miniport )
        {
          if ( _InterlockedExchangeAdd(Miniport + 18, 0xFFFFFFFF) == 1 )
