/*
 * XREFs of ?moveElements@?$KArray@V?$unique_ptr@UNDIS_BIND_PROTOCOL_LINK@@U?$default_delete@UNDIS_BIND_PROTOCOL_LINK@@@wistd@@@wistd@@@Rtl@@AEAAXKKK@Z @ 0x1C00B1A24
 * Callers:
...
void __fastcall Rtl::KArray<wistd::unique_ptr<NDIS_BIND_PROTOCOL_LINK,wistd::default_delete<NDIS_BIND_PROTOCOL_LINK>>>::moveElements(
        __int64 a1,
        unsigned int a2,
        unsigned int a3,
        int a4)
    ...
    else
    {
      ...
      while ( (unsigned int)v8 > a2 )
      {
        ...
        if ( v21 )
        {
          ...
          if ( v23 && _InterlockedExchangeAdd(v23 + 14, 0xFFFFFFFF) == 1 )
