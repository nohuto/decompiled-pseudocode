/*
 * XREFs of ?ndisValidatePDCapsBuffer@@YAEKPEBU_NDIS_PD_CAPABILITIES@@@Z @ 0x1C00753E4
 * Callers:
...
char __fastcall ndisValidatePDCapsBuffer(unsigned int a1, struct _NDIS_OBJECT_HEADER *a2)
{
  ...
  if ( !*(_DWORD *)&a2[1] )
  {
      ...
      && ((v5.Type & 0xC) == 0 || (v5.Type & 2) != 0) )
    {
      ...
      if ( ndisValidateTrailingElementSpace(a1, a2, v10, v9, v11) )
      {
        v13 = (unsigned int)a2[16];
        v19 = (unsigned int)a2[17];
        v18 = (unsigned int)a2[15];
        if ( ndisValidateTrailingElementSpace(a1, v12, v18, v13, v19) )
        {
          v15 = 0;
          if ( v9 )
          {
            while ( ndisValidateGFPExactMatchProfileBuffer(
                      a1 - (v10 + v15 * v11),
                      (const struct _NDIS_GFP_EXACT_MATCH_PROFILE *)(&a2->Type + v10 + v15 * v11)) )
            {
              if ( ++v15 >= v9 )
              {
                v14 = v18;
...
LABEL_30:
            ...
            if ( v13 )
            {
              while ( ndisValidateGFPWildcardMatchProfileBuffer(
                        a1 - (v14 + v16 * v19),
                        (const struct _NDIS_GFP_WILDCARD_MATCH_PROFILE *)(&a2->Type + v14 + v16 * v19)) )
              {
                v14 = v18;
                if ( ++v16 >= v13 )
                  return 1;
