/*
 * XREFs of scl_ScaleBackCurrentCharPoints @ 0x1C02BF4FC
 * Callers:
 *     fsg_CompositeInnerGridFit @ 0x1C02C694C (fsg_CompositeInnerGridFit.c)
 *     fsg_SimpleInnerGridFit @ 0x1C02C86C8 (fsg_SimpleInnerGridFit.c)
 * Callees:
 *     scl_ScaleBack @ 0x1C02BF488 (scl_ScaleBack.c)
 */

void __fastcall scl_ScaleBackCurrentCharPoints(__int64 a1, __int64 a2)
{
  _DWORD *v2; // r8
  int v3; // eax
  __int64 v6; // rdx
  int *v7; // rcx
  __int64 v8; // rdx

  v2 = *(_DWORD **)a1;
  v3 = *(__int16 *)(a1 + 80) - 1;
  if ( *(_BYTE *)(a2 + 396) )
  {
    scl_ScaleBack(
      (int *)(a2 + 264),
      a2,
      (__int64)v2,
      v2,
      (unsigned __int16)(*(_WORD *)(*(_QWORD *)(a1 + 64) + 2LL * v3) + 1));
    v6 = *(unsigned __int16 *)(*(_QWORD *)(a1 + 64) + 2LL * (*(__int16 *)(a1 + 80) - 1));
    v7 = (int *)(a2 + 280);
  }
  else
  {
    v8 = (unsigned __int16)(*(_WORD *)(*(_QWORD *)(a1 + 64) + 2LL * v3) + 1);
    scl_ScaleBack((int *)(a2 + 232), v8, (__int64)v2, *(_DWORD **)a1, (unsigned __int16)v8);
    v6 = *(unsigned __int16 *)(*(_QWORD *)(a1 + 64) + 2LL * (*(__int16 *)(a1 + 80) - 1));
    v7 = (int *)(a2 + 248);
  }
  LOWORD(v6) = v6 + 1;
  scl_ScaleBack(v7, v6, *(_QWORD *)(a1 + 8), *(_DWORD **)(a1 + 8), (unsigned __int16)v6);
}
