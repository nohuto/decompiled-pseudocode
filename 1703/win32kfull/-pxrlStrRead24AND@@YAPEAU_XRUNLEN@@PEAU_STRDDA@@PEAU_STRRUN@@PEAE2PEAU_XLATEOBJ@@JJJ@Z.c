/*
 * XREFs of ?pxrlStrRead24AND@@YAPEAU_XRUNLEN@@PEAU_STRDDA@@PEAU_STRRUN@@PEAE2PEAU_XLATEOBJ@@JJJ@Z @ 0x1C029EAF0
 * Callers:
 *     <none>
 * Callees:
 *     XLATEOBJ_iXlate @ 0x1C00A3750 (XLATEOBJ_iXlate.c)
 */

struct _XRUNLEN *__fastcall pxrlStrRead24AND(
        struct _STRDDA *a1,
        struct _STRRUN *a2,
        unsigned __int8 *a3,
        unsigned __int8 *a4,
        XLATEOBJ *pxlo,
        int a6,
        int a7)
{
  int v8; // r14d
  unsigned __int8 *v9; // rbx
  __int64 v10; // rdx
  unsigned int *v11; // r10
  __int64 v12; // r9
  __int64 v13; // rcx
  _DWORD *v14; // r8
  __int64 v15; // rsi
  unsigned int *v16; // r12
  __int64 v17; // r15
  __int64 v18; // rbp
  ULONG v19; // eax
  _DWORD *v20; // rcx
  ULONG iColora; // [rsp+78h] [rbp+40h]

  HIBYTE(iColora) = 0;
  v8 = 0;
  *((_DWORD *)a2 + 2) = *(_DWORD *)a1;
  v9 = &a3[2 * a6 + a6];
  *((_DWORD *)a2 + 3) = *((_DWORD *)a1 + 2) - *(_DWORD *)a1;
  if ( pxlo )
  {
    v15 = 0LL;
    if ( a6 != a7 )
    {
      v16 = (unsigned int *)((char *)a1 + 32);
      v17 = (unsigned int)(a7 - a6);
      do
      {
        v18 = *v16++;
        LOWORD(iColora) = *(_WORD *)v9;
        BYTE2(iColora) = v9[2];
        v19 = XLATEOBJ_iXlate(pxlo, iColora);
        HIBYTE(iColora) = HIBYTE(v19);
        if ( (_DWORD)v18 )
        {
          v8 += v18;
          v20 = (_DWORD *)((char *)a2 + 4 * v15 + 16);
          v15 += v18;
          do
          {
            *v20++ &= v19;
            LODWORD(v18) = v18 - 1;
          }
          while ( (_DWORD)v18 );
        }
        else
        {
          *((_DWORD *)a2 + v15 + 4) &= v19;
        }
        v9 += 3;
        --v17;
      }
      while ( v17 );
    }
  }
  else
  {
    v10 = 0LL;
    if ( a6 != a7 )
    {
      v11 = (unsigned int *)((char *)a1 + 32);
      v12 = (unsigned int)(a7 - a6);
      do
      {
        v13 = *v11++;
        LOWORD(iColora) = *(_WORD *)v9;
        BYTE2(iColora) = v9[2];
        if ( (_DWORD)v13 )
        {
          v8 += v13;
          v14 = (_DWORD *)((char *)a2 + 4 * v10 + 16);
          v10 += v13;
          do
          {
            *v14++ &= iColora;
            LODWORD(v13) = v13 - 1;
          }
          while ( (_DWORD)v13 );
        }
        else
        {
          *((_DWORD *)a2 + v10 + 4) &= iColora;
        }
        v9 += 3;
        --v12;
      }
      while ( v12 );
    }
  }
  return (struct _STRRUN *)((char *)a2 + 4 * v8 + 16);
}
