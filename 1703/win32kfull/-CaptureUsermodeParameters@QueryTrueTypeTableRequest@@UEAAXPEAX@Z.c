/*
 * XREFs of ?CaptureUsermodeParameters@QueryTrueTypeTableRequest@@UEAAXPEAX@Z @ 0x1C0092DD0
 * Callers:
 *     <none>
 * Callees:
 *     memmove @ 0x1C01401C0 (memmove.c)
 */

void __fastcall QueryTrueTypeTableRequest::CaptureUsermodeParameters(QueryTrueTypeTableRequest *this, int *a2)
{
  const void *v3; // r10
  _QWORD *v4; // rax
  _DWORD *v5; // r8
  size_t v6; // rdi
  _QWORD *v7; // rcx
  _DWORD *v10; // rcx

  v3 = (const void *)*((_QWORD *)a2 + 3);
  v4 = (_QWORD *)*((_QWORD *)a2 + 4);
  v5 = (_DWORD *)*((_QWORD *)a2 + 5);
  v6 = a2[12];
  if ( (v6 & 0x80000000) == 0LL
    && ((unsigned int)v6 <= *((_DWORD *)this + 11) || !*((_QWORD *)this + 6))
    && v3 == *((const void **)this + 10) )
  {
    v7 = (_QWORD *)*((_QWORD *)this + 7);
    if ( (v7 ? v4 == *((_QWORD **)this + 11) : v4 == 0LL)
      && (*((_QWORD *)this + 8) ? v5 == *((_DWORD **)this + 12) : v5 == 0LL) )
    {
      if ( *((_QWORD *)this + 6) )
      {
        memmove(*((void **)this + 6), v3, v6);
      }
      else
      {
        if ( v7 )
          *v7 = *v4;
        v10 = (_DWORD *)*((_QWORD *)this + 8);
        if ( v10 )
          *v10 = *v5;
      }
      *((_DWORD *)this + 18) = v6;
    }
  }
}
