/*
 * XREFs of ?GetOrUpdateCheckPointIfNotPresent@@YAPEAUtagCHECKPOINT@@PEAU_MOVESIZEDATA@@@Z @ 0x1C01F0224
 * Callers:
...
struct tagCHECKPOINT *__fastcall GetOrUpdateCheckPointIfNotPresent(struct _MOVESIZEDATA *a1)
{
  ...
  Prop = (struct tagCHECKPOINT *)GetProp(*(_QWORD *)a1, atomCheckpointProp, 1);
  if ( Prop
