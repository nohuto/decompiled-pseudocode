/*
 * XREFs of ?BuildDeviceCapabilities@InteractiveControlParser@@SAJPEAU_HIDP_PREPARSED_DATA@@PEAVInteractiveControlDevice@@@Z @ 0x1C02244C0
 * Callers:
        ...
        struct InteractiveControlDevice *a2)
{
  ...
  __int64 v8; // rdx
  struct _HIDP_LINK_COLLECTION_NODE *v9; // r8
  ...
  if ( LinkCollectionNodesLength > 1 )
  {
    ...
    if ( Caps == 1114112 )
    {
      Caps = InteractiveControlParser::BuildComponentInformation(PreparsedData, v8, v9, a2);
      if ( Caps < 0 )
        goto LABEL_14;
