/*
 * XREFs of LW_BrushInit @ 0x1C011A000
 * Callers:
 *     <none>
 * Callees:
 *     <none>
...
__int64 LW_BrushInit()
{
  __int64 Bitmap; // rax
  __int64 v1; // rbx
  __int64 SolidBrush; // rcx

  Bitmap = GreCreateBitmap(8LL, 8LL, 1LL, 1LL, "U");
  v1 = Bitmap;
  if ( !Bitmap )
    return 0LL;
  *(_QWORD *)(gpsi + 5336LL) = GreCreatePatternBrush(Bitmap);
  ghbrWhite = GreGetStockObject(0LL);
  ghbrBlack = GreGetStockObject(4LL);
  if ( !*(_QWORD *)(gpsi + 5336LL) )
    return 0LL;
  GreDeleteObject(v1);
  GreSetBrushOwner(*(_QWORD *)(gpsi + 5336LL), 0LL);
