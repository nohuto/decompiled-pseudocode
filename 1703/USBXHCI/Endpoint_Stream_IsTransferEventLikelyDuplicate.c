/*
 * XREFs of Endpoint_Stream_IsTransferEventLikelyDuplicate @ 0x1C00207A4
 * Callers:
 *     Endpoint_TransferEventHandler @ 0x1C000FB10 (Endpoint_TransferEventHandler.c)
 * Callees:
 *     _guard_dispatch_icall_nop @ 0x1C0010A80 (_guard_dispatch_icall_nop.c)
 */

char __fastcall Endpoint_Stream_IsTransferEventLikelyDuplicate(_QWORD *a1, __int64 a2, _BYTE *a3)
{
  char result; // al
  unsigned int v4; // ebx
  __int64 v6; // rdi
  __int64 v9; // rcx

  result = 0;
  v4 = 1;
  *a3 = 0;
  v6 = *(_QWORD *)(a2 + 136);
  if ( *(_DWORD *)(v6 + 8) )
  {
    do
    {
      v9 = *(_QWORD *)(88LL * (v4 - 1) + *(_QWORD *)(a2 + 136) + 40);
      result = (*(__int64 (__fastcall **)(__int64, _QWORD, _BYTE *))(*(_QWORD *)(v9 + 32) + 72LL))(v9, *a1, a3);
      if ( result )
        break;
      ++v4;
    }
    while ( v4 <= *(_DWORD *)(v6 + 8) );
  }
  return result;
}
