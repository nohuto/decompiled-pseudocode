/*
 * XREFs of ?CalculateMinMaxOverride@DpiInternal@@YAXKUtagSIZE@@PEAJ1QEBU_DPI_SCALE_FACTOR_COLLECTION@@@Z @ 0x1C000293C
 * Callers:
 *     ?FillOverridesAndAdjustedScaleFactor@DpiInternal@@YAXIHPEBU_DPI_SCALE_FACTOR_COLLECTION@@PEAU_DPI_INFORMATION@@@Z @ 0x1C0003CEC (-FillOverridesAndAdjustedScaleFactor@DpiInternal@@YAXIHPEBU_DPI_SCALE_FACTOR_COLLECTION@@PEAU_DP.c)
 * Callees:
 *     ??PDpiInternal@@YA_NAEBUtagSIZE@@0@Z @ 0x1C0002A08 (--PDpiInternal@@YA_NAEBUtagSIZE@@0@Z.c)
 */

void __fastcall DpiInternal::CalculateMinMaxOverride(
        DpiInternal *this,
        __int64 a2,
        struct tagSIZE a3,
        int *a4,
        int *a5)
{
  int v6; // esi
  int v7; // r11d
  int *v8; // r10
  __int64 v9; // r9
  unsigned int v10; // ecx
  __int64 v11; // r8
  __int64 v12; // rdi
  unsigned int v13; // r11d
  unsigned int v14; // esi
  int v15; // r8d
  unsigned int v16; // r9d
  int *v17; // [rsp+38h] [rbp+10h]

  *(_DWORD *)a3.cx = 0;
  *a4 = 0;
  v6 = (int)this;
  v7 = a2;
  if ( (int)a2 < SHIDWORD(a2) )
    v7 = HIDWORD(a2);
  else
    LODWORD(a2) = HIDWORD(a2);
  v8 = a5;
  v9 = 0LL;
  v10 = a5[2];
  if ( v10 )
  {
    while ( *(_DWORD *)(*((_QWORD *)a5 + 2) + 4 * v9) != v6 )
    {
      v9 = (unsigned int)(v9 + 1);
      if ( (unsigned int)v9 >= v10 )
        return;
    }
    *(_DWORD *)a3.cx = -(int)v9;
    v11 = (unsigned int)(v8[2] - 1);
    if ( (unsigned int)v11 > (unsigned int)v9 )
    {
      v12 = *((_QWORD *)v8 + 2);
      v13 = 100 * v7;
      v14 = 100 * a2;
      while ( 1 )
      {
        LODWORD(v17) = v13 / *(_DWORD *)(v12 + 4 * v11);
        HIDWORD(v17) = v14 / *(_DWORD *)(v12 + 4 * v11);
        a5 = v17;
        if ( (unsigned __int8)DpiInternal::operator>=(&a5, v8) )
          break;
        v11 = (unsigned int)(v15 - 1);
        if ( (unsigned int)v11 <= v16 )
          return;
      }
      *a4 = v15 - v16;
    }
  }
}
