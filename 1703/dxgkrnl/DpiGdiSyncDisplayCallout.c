/*
 * XREFs of DpiGdiSyncDisplayCallout @ 0x1C01C2594
 * Callers:
 *     ?RequestSyncCallout@DXGDISPLAYCALLOUTQUEUE@@QEAAJPEAUDISPLAY_CALLOUT_ENTRY@@@Z @ 0x1C0196C7C (-RequestSyncCallout@DXGDISPLAYCALLOUTQUEUE@@QEAAJPEAUDISPLAY_CALLOUT_ENTRY@@@Z.c)
 *     DpiGdiHandleAsyncDisplayCallout @ 0x1C01C2570 (DpiGdiHandleAsyncDisplayCallout.c)
 * Callees:
 *     memset @ 0x1C0015700 (memset.c)
 */

__int64 DpiGdiSyncDisplayCallout()
{
  __int64 v0; // r8
  int v1; // eax
  __int64 v2; // rdx
  __int64 v3; // rcx
  __int64 v4; // r8
  __int64 v5; // r9
  __int64 v6; // rbx
  __int64 v7; // rax
  _DWORD v9[10]; // [rsp+30h] [rbp-28h] BYREF

  memset(v9, 0, 0x20uLL);
  LOBYTE(v0) = 1;
  v9[0] = 8;
  v1 = SMgrGdiCallout(v9, 0LL, v0, 0LL, 0LL, 0LL);
  v6 = v1;
  if ( v1 < 0 || v9[6] < 0 )
  {
    v7 = WdLogNewEntry5_WdWarning(v3, v2, v4, v5);
    *(_QWORD *)(v7 + 24) = v6;
    WdLogEvent5_WdWarning(v7);
  }
  return (unsigned int)v6;
}
