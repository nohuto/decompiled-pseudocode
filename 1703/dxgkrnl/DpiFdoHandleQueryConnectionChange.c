/*
 * XREFs of DpiFdoHandleQueryConnectionChange @ 0x1C0011638
 * Callers:
 *     DxgkQueryConnectionChanges @ 0x1C0011068 (DxgkQueryConnectionChanges.c)
 * Callees:
 *     ExFreeToNPagedLookasideList @ 0x1C001178C (ExFreeToNPagedLookasideList.c)
 */

__int64 __fastcall DpiFdoHandleQueryConnectionChange(__int64 a1, __int64 a2)
{
  _QWORD *v4; // rbx
  _QWORD **v5; // rax
  unsigned int v6; // edi
  _QWORD *v8; // rcx
  int v9; // ecx
  int v10; // ecx
  int v11; // ecx
  int v12; // eax
  int v13; // ecx
  _QWORD *v14; // rax
  struct _KLOCK_QUEUE_HANDLE LockHandle; // [rsp+20h] [rbp-28h] BYREF

  *(_QWORD *)a2 = 0LL;
  *(_QWORD *)(a2 + 8) = 0LL;
  *(_QWORD *)(a2 + 16) = 0LL;
  v4 = 0LL;
  KeAcquireInStackQueuedSpinLock((PKSPIN_LOCK)(a1 + 3328), &LockHandle);
  v5 = (_QWORD **)(a1 + 3336);
  if ( *v5 == v5 )
  {
    v6 = 255;
    goto LABEL_3;
  }
  v4 = *v5;
  v8 = (_QWORD *)**v5;
  if ( (_QWORD **)(*v5)[1] != v5 || (_QWORD *)v8[1] != v4 )
    __fastfail(3u);
  *v5 = v8;
  v8[1] = v5;
  *(_OWORD *)(a2 + 24) = *(_OWORD *)(v4 + 5);
  *(_OWORD *)(a2 + 40) = *(_OWORD *)(v4 + 7);
  *(_OWORD *)(a2 + 56) = *(_OWORD *)(v4 + 9);
  *(_OWORD *)(a2 + 72) = *(_OWORD *)(v4 + 11);
  v9 = *((_DWORD *)v4 + 6);
  if ( !v9 )
  {
    if ( *(_BYTE *)(a1 + 1139) )
      *(_BYTE *)(a2 + 94) = 1;
    goto LABEL_15;
  }
  v10 = v9 - 1;
  if ( !v10 )
    goto LABEL_10;
  v13 = v10 - 1;
  if ( v13 )
  {
    if ( v13 == 1 )
    {
LABEL_10:
      *(_QWORD *)a2 = _InterlockedIncrement64(&qword_1C006FE48);
      *(_DWORD *)(a2 + 8) ^= (*(_DWORD *)(a2 + 8) ^ *((_DWORD *)v4 + 7)) & 0xFFFFFF;
      *(_BYTE *)(a2 + 88) = *((_BYTE *)v4 + 31);
      if ( *((_BYTE *)v4 + 106) )
        v11 = 9;
      else
        v11 = *((_BYTE *)v4 + 32) != 0 ? 10 : 8;
      *(_DWORD *)(a2 + 8) = (v11 << 24) | *(_DWORD *)(a2 + 8) & 0xF0FFFFFF;
      if ( *((_DWORD *)v4 + 6) == 3 )
        v12 = *((_DWORD *)v4 + 9);
      else
        v12 = -2;
      *(_DWORD *)(a2 + 12) = v12;
      *(_BYTE *)(a2 + 92) = *((_BYTE *)v4 + 104);
      *(_BYTE *)(a2 + 93) = *((_BYTE *)v4 + 105);
      *(_BYTE *)(a2 + 95) = *((_BYTE *)v4 + 107);
    }
  }
  else
  {
    v14 = (_QWORD *)WdLogNewEntry5_WdEvent();
    v14[3] = *((unsigned int *)v4 + 7);
    v14[4] = *(_QWORD *)(a1 + 24);
    v14[5] = *((unsigned __int8 *)v4 + 32);
    WdLogEvent5_WdEvent(v14);
  }
LABEL_15:
  v6 = 0;
LABEL_3:
  KeReleaseInStackQueuedSpinLock(&LockHandle);
  if ( v4 )
    ExFreeToNPagedLookasideList(&Lookaside, v4);
  return v6;
}
