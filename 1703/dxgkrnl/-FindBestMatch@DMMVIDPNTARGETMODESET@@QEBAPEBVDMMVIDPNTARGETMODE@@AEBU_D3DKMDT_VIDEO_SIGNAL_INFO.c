/*
 * XREFs of ?FindBestMatch@DMMVIDPNTARGETMODESET@@QEBAPEBVDMMVIDPNTARGETMODE@@AEBU_D3DKMDT_VIDEO_SIGNAL_INFO@@@Z @ 0x1C00088D4
 * Callers:
 *     ?PinModalityOnPathInVidPnTopology@VIDPN_MGR@@QEAAJQEAVDMMVIDPNTOPOLOGY@@PEBU_D3DKMDT_FUNCTIONAL_VIDPN_PRESENT_PATH@@@Z @ 0x1C00E3C94 (-PinModalityOnPathInVidPnTopology@VIDPN_MGR@@QEAAJQEAVDMMVIDPNTOPOLOGY@@PEBU_D3DKMDT_FUNCTIONAL_.c)
 * Callees:
 *     ?GetNextMode@DMMVIDPNSOURCEMODESET@@QEAAPEAVDMMVIDPNSOURCEMODE@@QEBV2@@Z @ 0x1C0008600 (-GetNextMode@DMMVIDPNSOURCEMODESET@@QEAAPEAVDMMVIDPNSOURCEMODE@@QEBV2@@Z.c)
 *     ?HaveEquivalentMonitorFrequencyRequirements@DMMVIDEOSIGNALMODE@@SAEAEBU_D3DKMDT_VIDEO_SIGNAL_INFO@@0E@Z @ 0x1C000D190 (-HaveEquivalentMonitorFrequencyRequirements@DMMVIDEOSIGNALMODE@@SAEAEBU_D3DKMDT_VIDEO_SIGNAL_INF.c)
 */

const struct DMMVIDPNTARGETMODE *__fastcall DMMVIDPNTARGETMODESET::FindBestMatch(
        DMMVIDPNTARGETMODESET *this,
        const struct _D3DKMDT_VIDEO_SIGNAL_INFO *a2,
        __int64 a3,
        __int64 ScanLineOrdering)
{
  struct DMMVIDPNSOURCEMODE *v4; // rdi
  DMMVIDPNTARGETMODESET *v5; // rbx
  struct DMMVIDPNSOURCEMODE *NextMode; // rbx
  int v9; // r10d

  v4 = 0LL;
  v5 = (DMMVIDPNTARGETMODESET *)*((_QWORD *)this + 6);
  if ( v5 == (DMMVIDPNTARGETMODESET *)((char *)this + 48) )
    return v4;
  NextMode = (DMMVIDPNTARGETMODESET *)((char *)v5 - 8);
  if ( !NextMode )
    return v4;
  while ( 1 )
  {
    if ( a2->ActiveSize.cx == *((_DWORD *)NextMode + 21) && a2->ActiveSize.cy == *((_DWORD *)NextMode + 22) )
    {
      ScanLineOrdering = (unsigned int)a2->ScanLineOrdering;
      v9 = *((_DWORD *)NextMode + 30);
      if ( (((unsigned __int8)v9 ^ *(_BYTE *)&a2->AdditionalSignalInfo) & 7) == 0 )
      {
        a3 = a2->VSyncFreq.Denominator * (unsigned __int64)*((unsigned int *)NextMode + 23);
        if ( a2->VSyncFreq.Numerator * (unsigned __int64)*((unsigned int *)NextMode + 24) == a3 )
        {
          ScanLineOrdering = v9 ^ (unsigned int)ScanLineOrdering;
          if ( (ScanLineOrdering & 0x1F8) == 0 )
            break;
        }
      }
    }
    if ( !v4 )
    {
      if ( DMMVIDEOSIGNALMODE::HaveEquivalentMonitorFrequencyRequirements(
             a2,
             (const struct _D3DKMDT_VIDEO_SIGNAL_INFO *)((char *)NextMode + 72),
             0) )
      {
        v4 = NextMode;
      }
    }
    NextMode = DMMVIDPNSOURCEMODESET::GetNextMode(this, NextMode, a3, ScanLineOrdering);
    if ( !NextMode )
      return v4;
  }
  return NextMode;
}
