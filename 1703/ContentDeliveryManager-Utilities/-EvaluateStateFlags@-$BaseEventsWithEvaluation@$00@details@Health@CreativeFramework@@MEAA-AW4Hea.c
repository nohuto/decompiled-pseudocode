/*
 * XREFs of ?EvaluateStateFlags@?$BaseEventsWithEvaluation@$00@details@Health@CreativeFramework@@MEAA?AW4HealthStateFlags@234@XZ @ 0x180027290
 * Callers:
 *     <none>
 * Callees:
 *     ?ReadTrackingInfo@BaseScenarioEvents@details@Health@CreativeFramework@@AEAAXXZ @ 0x18002694C (-ReadTrackingInfo@BaseScenarioEvents@details@Health@CreativeFramework@@AEAAXXZ.c)
 *     ?ReadPlacementReportedInfo@BaseScenarioEvents@details@Health@CreativeFramework@@AEAAXXZ @ 0x1800269DC (-ReadPlacementReportedInfo@BaseScenarioEvents@details@Health@CreativeFramework@@AEAAXXZ.c)
 */

__int64 __fastcall CreativeFramework::Health::details::BaseEventsWithEvaluation<1>::EvaluateStateFlags(
        CreativeFramework::Health::details::BaseScenarioEvents *this)
{
  __int64 v2; // rbx
  __int64 v3; // rax
  unsigned __int64 v4; // rsi
  char v5; // al
  char v6; // al
  unsigned __int64 v7; // rdx
  unsigned __int64 v8; // rax
  unsigned __int64 v9; // rcx
  __int64 v10; // rax
  unsigned __int64 v11; // rax
  __int64 v12; // rax
  struct _FILETIME SystemTimeAsFileTime; // [rsp+30h] [rbp+8h] BYREF

  if ( !*((_BYTE *)this + 250) )
    CreativeFramework::Health::details::BaseScenarioEvents::ReadTrackingInfo(this);
  if ( !*((_BYTE *)this + 252) )
    CreativeFramework::Health::details::BaseScenarioEvents::ReadPlacementReportedInfo(this);
  if ( *((_BYTE *)this + 98) )
  {
    v2 = 4LL;
    if ( !*((_QWORD *)this + 3) )
      v2 = 12LL;
    if ( *((_QWORD *)this + 2) )
    {
      if ( !*((_DWORD *)this + 20) )
        v2 |= 0x20uLL;
    }
    else
    {
      v2 |= 0x10uLL;
    }
    v3 = *((_QWORD *)this + 6);
    if ( !v3 )
      goto LABEL_18;
    if ( *((_BYTE *)this + 97) )
      v2 |= 0x40uLL;
    if ( !*((_BYTE *)this + 96) )
LABEL_18:
      v2 |= 0x80uLL;
    if ( v3 && !*((_DWORD *)this + 21) )
      v2 |= 0x100uLL;
  }
  else
  {
    v2 = 2LL;
  }
  if ( *((_BYTE *)this + 98) )
  {
    GetSystemTimeAsFileTime(&SystemTimeAsFileTime);
    v4 = SystemTimeAsFileTime.dwLowDateTime + (HIDWORD(*(unsigned __int64 *)&SystemTimeAsFileTime) << 32);
    v5 = 0;
    if ( *((_BYTE *)this + 97) || (v2 & 0x10) != 0 )
    {
      if ( (v2 & 0x130) != 0 )
      {
        v2 |= 2uLL;
        if ( (v2 & 0x130) != 0 )
          return v2;
      }
      v2 |= 1uLL;
      goto LABEL_45;
    }
    if ( !*((_QWORD *)this + 6) )
    {
      if ( *((_QWORD *)this + 3) + 600000000LL > v4 || *((_QWORD *)this + 4) + 600000000LL > v4 )
      {
        v6 = 1;
        v2 |= 2uLL;
      }
      else
      {
        v6 = 0;
      }
      v5 = v6 ^ 1;
LABEL_45:
      if ( !v5 )
        return v2;
      goto LABEL_46;
    }
    if ( (v2 & 0x80u) != 0LL )
    {
LABEL_46:
      v12 = *((_QWORD *)this + 19);
      if ( v12 && !*((_WORD *)this + 80) && v4 >= v12 + 36000000000LL )
        return v2 | 1;
      return v2;
    }
    GetSystemTimeAsFileTime(&SystemTimeAsFileTime);
    v7 = SystemTimeAsFileTime.dwLowDateTime + (HIDWORD(*(unsigned __int64 *)&SystemTimeAsFileTime) << 32);
    v8 = *((_QWORD *)this + 4) + 600000000LL;
    v9 = *((_QWORD *)this + 6);
    if ( v9 < v8 && v8 < v7 )
    {
      v2 |= 0x200uLL;
      goto LABEL_46;
    }
    if ( v9 >= *((_QWORD *)this + 15) )
      return v2 | 2;
    v10 = *((_QWORD *)this + 16);
    if ( !v10 )
      v10 = *((_QWORD *)this + 15) + 10000000LL * *((unsigned int *)this + 34);
    v11 = v10 + 6000000000LL;
    if ( *((_QWORD *)this + 14) <= v11 || v7 <= v11 )
      return v2 | 2;
  }
  return v2;
}
