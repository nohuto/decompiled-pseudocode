...
__int64 __fastcall RtlpGetTokenNamedObjectPath(HANDLE TokenHandle, char *Sid, char a3, UNICODE_STRING *a4)
{
  ...
  PSID v39; // [rsp+50h] [rbp-B0h] BYREF
  ...
  *(_DWORD *)&pcbLength.Length = v11;
  ...
  AppContainerSidType = NtQueryInformationToken(TokenHandle, TokenIsAppContainer, &v37, 4u, &v40);
  if ( AppContainerSidType >= 0 )
  {
    if ( !v37 )
      goto LABEL_5;
    AppContainerSidType = NtQueryInformationToken(TokenHandle, TokenAppContainerSid, &v51, 0x50u, &v40);
    if ( AppContainerSidType >= 0 )
    {
      v7 = v51;
      if ( !v51 )
...
LABEL_5:
      AppContainerSidType = NtQueryInformationToken(TokenHandle, TokenPrivateNameSpace, &TokenInformation, 4u, &v40);
      if ( AppContainerSidType < 0 )
        goto LABEL_81;
      if ( TokenInformation )
      {
      ...
      if ( v37 )
      {
        AppContainerSidType = RtlGetAppContainerSidType(v7, &v47);
        if ( AppContainerSidType >= 0 )
        {
          if ( v47 == 2 )
          {
          ...
          {
            AppContainerParent = RtlGetAppContainerParent(v7, (ULONG **)&v39);
            v8 = v39;
            AppContainerSidType = AppContainerParent;
            if ( AppContainerParent >= 0 )
            {
              AppContainerSidType = RtlConvertSidToUnicodeString(&DestinationString, v39, 1u);
              if ( AppContainerSidType >= 0 )
              {
