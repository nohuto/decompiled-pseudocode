/*
 * XREFs of _SysCtxRegOpenCurrentUserKey @ 0x14074178C
 * Callers:
...
__int64 __fastcall SysCtxRegOpenCurrentUserKey(_QWORD *a1, unsigned int a2, unsigned int a3, __int64 a4)
{
  ...
  __int64 v13; // rcx
  struct _SID_IDENTIFIER_AUTHORITY IdentifierAuthority; // [rsp+30h] [rbp-99h] BYREF
        ...
        if ( appended >= 0 )
        {
          ...
          else
          {
            ...
            if ( PoolWithTag )
            {
              ...
              appended = RtlAppendUnicodeStringToString(&Destination, &stru_14028EB10);
              if ( appended >= 0 )
              {
