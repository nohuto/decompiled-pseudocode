/*
 * XREFs of PopSqmThermalUsermodeEvent @ 0x1406D6AF4
 * Callers:
 *     PopThermalProcessUsermodeEvent @ 0x1406CBBFC (PopThermalProcessUsermodeEvent.c)
 * Callees:
 *     _TlgWrite @ 0x14007FE94 (_TlgWrite.c)
 *     _TlgKeywordOn @ 0x140082CE0 (_TlgKeywordOn.c)
 *     __security_check_cookie @ 0x140169210 (__security_check_cookie.c)
 */

void __fastcall PopSqmThermalUsermodeEvent(__int64 a1, __int64 a2, __int64 a3, __int64 a4, char a5)
{
  int v6; // r9d
  __int16 v7; // r10
  int v8; // r11d
  int v9; // [rsp+30h] [rbp-51h] BYREF
  int v10; // [rsp+34h] [rbp-4Dh] BYREF
  BOOL v11; // [rsp+38h] [rbp-49h] BYREF
  EVENT_DATA_DESCRIPTOR pData; // [rsp+40h] [rbp-41h] BYREF
  int *v13; // [rsp+60h] [rbp-21h]
  __int64 v14; // [rsp+68h] [rbp-19h]
  int *v15; // [rsp+70h] [rbp-11h]
  __int64 v16; // [rsp+78h] [rbp-9h]
  BOOL *v17; // [rsp+80h] [rbp-1h]
  __int64 v18; // [rsp+88h] [rbp+7h]
  _DWORD *v19; // [rsp+90h] [rbp+Fh]
  __int64 v20; // [rsp+98h] [rbp+17h]
  __int64 v21; // [rsp+A0h] [rbp+1Fh]
  _DWORD v22[6]; // [rsp+A8h] [rbp+27h] BYREF

  if ( hProvider.LevelPlus1 > 5 )
  {
    if ( TlgKeywordOn(&hProvider, 0x800000000000uLL) )
    {
      v9 = v8;
      v10 = v6;
      v11 = a5 != 0;
      v14 = 4LL;
      v13 = &v9;
      v15 = &v10;
      v17 = &v11;
      v19 = v22;
      v22[0] = (unsigned __int16)(2 * v7);
      v16 = 4LL;
      v18 = 4LL;
      v20 = 2LL;
      v21 = a2;
      v22[1] = 0;
      TlgWrite(&hProvider, &unk_1402ADD2F, 0LL, 0LL, 7u, &pData);
    }
  }
}
