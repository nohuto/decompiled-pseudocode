/*
 * XREFs of RtlLookupFunctionEntry @ 0x14000A970
 * Callers:
 *     RtlDispatchException @ 0x140009410 (RtlDispatchException.c)
 *     RtlUnwindEx @ 0x140009D90 (RtlUnwindEx.c)
 *     RtlpSameFunction @ 0x14015DB14 (RtlpSameFunction.c)
 *     RtlRaiseException @ 0x14023BBE0 (RtlRaiseException.c)
 *     sub_1407DDD64 @ 0x1407DDD64 (sub_1407DDD64.c)
 *     RtlInitializeHistoryTable @ 0x140820C2C (RtlInitializeHistoryTable.c)
 * Callees:
 *     RtlpxLookupFunctionTable @ 0x1400E4B40 (RtlpxLookupFunctionTable.c)
 *     ExRaiseDatatypeMisalignment @ 0x14071ED60 (ExRaiseDatatypeMisalignment.c)
 */

PRUNTIME_FUNCTION __stdcall RtlLookupFunctionEntry(
        DWORD64 ControlPc,
        PDWORD64 ImageBase,
        PUNWIND_HISTORY_TABLE HistoryTable)
{
  unsigned int GlobalHint; // ecx
  unsigned int v7; // edx
  unsigned int *v8; // r10
  unsigned __int64 v9; // r9
  PRUNTIME_FUNCTION result; // rax
  __int64 v11; // r10
  struct _RUNTIME_FUNCTION *v12; // r11
  __int64 v13; // r9
  unsigned __int128 v14; // rax
  __int64 v15; // r8
  unsigned __int64 v16; // rbx
  struct _RUNTIME_FUNCTION *v17; // rdx
  bool v18; // sf
  int v19; // r8d
  int v20; // r9d
  int v21; // eax
  __int64 Count; // rcx
  unsigned __int64 v23; // rdx
  unsigned __int64 v24; // rcx
  unsigned __int64 v25; // rcx
  __int64 v26; // rax
  DWORD LocalHint; // edx
  DWORD v28; // r9d
  DWORD64 v29; // r10
  PRUNTIME_FUNCTION FunctionEntry; // r8
  __int64 UnwindData; // rax
  unsigned __int64 v32; // r8
  __int128 v33; // [rsp+20h] [rbp-28h] BYREF
  __int64 v34; // [rsp+30h] [rbp-18h]

  if ( !HistoryTable )
    goto LABEL_10;
  if ( HistoryTable->Once )
  {
    HistoryTable->Once = 0;
    v32 = *(_QWORD *)&RtlpUnwindHistoryTable[4 * (unsigned __int8)byte_1403F8915 + 6];
    result = *(PRUNTIME_FUNCTION *)&RtlpUnwindHistoryTable[4 * (unsigned __int8)byte_1403F8915 + 8];
    if ( result )
    {
      if ( ControlPc >= v32 + result->BeginAddress && ControlPc < v32 + result->EndAddress )
      {
        *ImageBase = v32;
        return result;
      }
    }
  }
  if ( !HistoryTable->Search )
    goto LABEL_10;
  if ( ControlPc >= qword_1403F8920
    || ControlPc < qword_1403F8918
    || (GlobalHint = HistoryTable->GlobalHint, v7 = GlobalHint, GlobalHint >= RtlpUnwindHistoryTable[0]) )
  {
LABEL_38:
    if ( ControlPc >= HistoryTable->LowAddress && ControlPc < HistoryTable->HighAddress )
    {
      LocalHint = HistoryTable->LocalHint;
      v28 = LocalHint;
      if ( LocalHint < HistoryTable->Count )
      {
        while ( 1 )
        {
          v29 = HistoryTable->Entry[v28].ImageBase;
          FunctionEntry = HistoryTable->Entry[v28].FunctionEntry;
          if ( ControlPc < v29 + FunctionEntry->EndAddress && ControlPc >= v29 + FunctionEntry->BeginAddress )
            break;
          if ( ++v28 >= HistoryTable->Count )
            goto LABEL_10;
        }
        HistoryTable->LocalHint = LocalHint + 1;
        *ImageBase = v29;
        UnwindData = FunctionEntry->UnwindData;
        if ( (UnwindData & 1) != 0 )
        {
          FunctionEntry = (PRUNTIME_FUNCTION)(v29 + UnwindData - 1);
          if ( v29 <= 0x7FFFFFFEFFFFLL && ((unsigned __int8)FunctionEntry & 3) != 0 )
            ExRaiseDatatypeMisalignment();
        }
        return FunctionEntry;
      }
    }
LABEL_10:
    if ( ControlPc < *(&xmmword_1403F7020 + 1) || ControlPc >= *(&xmmword_1403F7020 + 1) + (unsigned int)qword_1403F7030 )
    {
      v11 = RtlpxLookupFunctionTable(ControlPc, &v33);
    }
    else
    {
      v11 = xmmword_1403F7020;
      v33 = *(_OWORD *)&xmmword_1403F7020;
      v34 = qword_1403F7030;
    }
    v12 = 0LL;
    if ( v11 )
    {
      v13 = *((_QWORD *)&v33 + 1);
      v14 = HIDWORD(v34) * (unsigned __int128)0xAAAAAAAAAAAAAAABuLL;
      *ImageBase = *((_QWORD *)&v33 + 1);
      v15 = *((_QWORD *)&v14 + 1) >> 3;
      if ( (unsigned int)(*((_QWORD *)&v14 + 1) >> 3) )
      {
        v16 = ControlPc - v13;
        v17 = (struct _RUNTIME_FUNCTION *)(v11 + 12LL * (unsigned int)(v15 - 1));
        if ( v16 < v17->BeginAddress )
        {
          v18 = (int)v15 - 2 < 0;
          v19 = v15 - 2;
          v20 = 0;
          if ( !v18 )
          {
            do
            {
              v21 = (v19 + v20) >> 1;
              v17 = (struct _RUNTIME_FUNCTION *)(v11 + 12LL * v21);
              if ( v16 < v17->BeginAddress )
              {
                v19 = v21 - 1;
              }
              else
              {
                if ( v16 < v17[1].BeginAddress )
                  break;
                v20 = v21 + 1;
              }
            }
            while ( v19 >= v20 );
          }
        }
        if ( v16 >= v17->BeginAddress && v16 < v17->EndAddress )
          v12 = v17;
      }
      if ( v12 )
      {
        if ( HistoryTable )
        {
          if ( !HistoryTable->Search )
          {
            Count = HistoryTable->Count;
            if ( (unsigned int)Count < 0xC )
            {
              HistoryTable->Count = Count + 1;
              HistoryTable->Entry[Count].ImageBase = *ImageBase;
              HistoryTable->Entry[(unsigned int)Count].FunctionEntry = v12;
              v23 = *ImageBase + v12->BeginAddress;
              v24 = *ImageBase + v12->EndAddress;
              if ( v23 < HistoryTable->LowAddress )
                HistoryTable->LowAddress = v23;
              if ( v24 > HistoryTable->HighAddress )
                HistoryTable->HighAddress = v24;
            }
          }
        }
      }
    }
    v25 = *ImageBase;
    if ( v12 )
    {
      v26 = v12->UnwindData;
      if ( (v26 & 1) != 0 )
      {
        v12 = (struct _RUNTIME_FUNCTION *)(v26 + v25 - 1);
        if ( v25 <= 0x7FFFFFFEFFFFLL && ((unsigned __int8)v12 & 3) != 0 )
          ExRaiseDatatypeMisalignment();
      }
    }
    return v12;
  }
  while ( 1 )
  {
    v8 = *(unsigned int **)&RtlpUnwindHistoryTable[4 * v7 + 8];
    v9 = *(_QWORD *)&RtlpUnwindHistoryTable[4 * v7 + 6];
    if ( ControlPc >= v9 + *v8 && ControlPc < v9 + v8[1] )
      break;
    if ( ++v7 >= RtlpUnwindHistoryTable[0] )
      goto LABEL_38;
  }
  result = *(PRUNTIME_FUNCTION *)&RtlpUnwindHistoryTable[4 * v7 + 8];
  HistoryTable->GlobalHint = GlobalHint + 1;
  *ImageBase = v9;
  return result;
}
