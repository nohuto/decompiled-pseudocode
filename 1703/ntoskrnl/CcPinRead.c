/*
 * XREFs of CcPinRead @ 0x140436EB0
 * Callers:
        ...
        PVOID *Buffer)
{
  ...
  v17 = &Obcb;
  if ( (Flags & 1) != 0 )
    __incgsdword(0x5E9Cu);
  else
    __incgsdword(0x5E98u);
  ...
  do
  {
    if ( Obcb )
    {
      if ( v12 == &Obcb )
      {
        Obcb = (PVOID)CcAllocateObcb(v6, v10, Obcb, *(_QWORD *)&Flags);
  ...
  *Bcb = Obcb;
  ...
  v15 = 1;
LABEL_14:
  __addgsdword(0x5ED8u, HIDWORD(KeGetCurrentThread()[1].Timer.Header.WaitListHead.Blink));
  if ( !v15 && Obcb )
    CcUnpinData(Obcb);
  return v15;
}
