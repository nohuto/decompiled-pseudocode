/*
 * XREFs of _PnpSetObjectPropertyWorker @ 0x1404DB9FC
 * Callers:
 *     _PnpSetObjectProperty @ 0x1404DDABC (_PnpSetObjectProperty.c)
 * Callees:
 *     ZwClose @ 0x14017E120 (ZwClose.c)
 *     _PnpOpenObjectRegKey @ 0x140487A28 (_PnpOpenObjectRegKey.c)
 *     _PnpSetGenericStoreProperty @ 0x1404DB4EC (_PnpSetGenericStoreProperty.c)
 *     _PnpSetMappedPropertyDispatch @ 0x1404DB530 (_PnpSetMappedPropertyDispatch.c)
 *     _PnpValidatePropertyData @ 0x1404DBC98 (_PnpValidatePropertyData.c)
 *     _PnpObjectRaisePropertyChangeEvent @ 0x1404DE868 (_PnpObjectRaisePropertyChangeEvent.c)
 */

__int64 __fastcall PnpSetObjectPropertyWorker(
        __int64 *a1,
        __int64 a2,
        signed int a3,
        __int64 a4,
        const wchar_t *a5,
        __int64 a6,
        int a7,
        __int64 a8,
        ULONG SecurityDescriptorLength,
        unsigned int a10)
{
  void *v14; // r12
  int v15; // ebx
  __int64 v16; // rdx
  int v17; // r9d
  HANDLE Handle[2]; // [rsp+50h] [rbp-38h] BYREF

  Handle[0] = 0LL;
  if ( (_WORD)a10 || !a8 && SecurityDescriptorLength )
    return (unsigned int)-1073741811;
  v14 = (void *)(a8 & -(__int64)(SecurityDescriptorLength != 0));
  v15 = PnpValidatePropertyData(v14, SecurityDescriptorLength);
  if ( v15 >= 0 )
  {
    v15 = PnpSetMappedPropertyDispatch(
            (__int64)a1,
            a2,
            a3,
            a4,
            (__int64)a5,
            a6,
            a7,
            (__int64)v14,
            SecurityDescriptorLength,
            a10);
    if ( v15 >= 0 && a3 >= 6 )
      PnpObjectRaisePropertyChangeEvent((_DWORD)a1, a2, a3, a4, (__int64)a5, a6);
    if ( v15 == -1073741802 )
    {
      if ( a4 )
      {
        v16 = a4;
        goto LABEL_8;
      }
      v15 = PnpOpenObjectRegKey((__int64)a1, a2, a3, 33554439, 0, (__int64)Handle, 0LL, 0);
      if ( v15 >= 0 )
      {
        v16 = (__int64)Handle[0];
LABEL_8:
        v15 = PnpSetGenericStoreProperty(a1, v16, a5, a6, a7, v14, SecurityDescriptorLength);
        if ( v15 >= 0 )
        {
          v17 = (int)Handle[0];
          if ( a4 )
            v17 = a4;
          PnpObjectRaisePropertyChangeEvent((_DWORD)a1, a2, a3, v17, (__int64)a5, a6);
        }
      }
    }
  }
  if ( Handle[0] )
    ZwClose(Handle[0]);
  return (unsigned int)v15;
}
