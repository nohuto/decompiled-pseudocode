...
__int64 __fastcall SepMandatorySubProcessToken(_DWORD *Token, __int64 a2, __int64 a3, _QWORD *a4)
{
  ...
  bool v38; // [rsp+30h] [rbp-99h] BYREF
...
LABEL_6:
    if ( SecurityObject >= 0 && v10 )
    {
      if ( (~(unsigned __int8)*((_WORD *)v10 + 1) & 0x10) == 0 )
      {
        if ( *((__int16 *)v10 + 1) >= 0 )
        {
          v12 = (_BYTE *)*((_QWORD *)v10 + 3);
LABEL_12:
          AceByType = RtlFindAceByType(v12, 17LL);
          if ( AceByType )
          {
            v16 = (char *)(AceByType + 8);
            TokenIntegrity = (char **)SepLocateTokenIntegrity(a2);
            v18 = TokenIntegrity;
            if ( !TokenIntegrity )
              goto LABEL_40;
            SecurityObject = RtlSidDominates(*TokenIntegrity, v16, &v38);
            if ( SecurityObject < 0 )
              goto LABEL_15;
            if ( v38 )
            {
