/*
 * XREFs of PiDcHandleInterfaceEvent @ 0x140468D50
 * Callers:
 *     PiDcHandleObjectEvent @ 0x140468B2C (PiDcHandleObjectEvent.c)
 * Callees:
 *     __security_check_cookie @ 0x140169210 (__security_check_cookie.c)
 *     _PnpGetObjectProperty @ 0x140484E40 (_PnpGetObjectProperty.c)
 *     _PnpStringFromGuid @ 0x1404E2F18 (_PnpStringFromGuid.c)
 *     PiDcContainerRequiresConfiguration @ 0x140594944 (PiDcContainerRequiresConfiguration.c)
 */

__int64 __fastcall PiDcHandleInterfaceEvent(__int64 a1)
{
  int ObjectProperty; // edx
  int v3; // [rsp+60h] [rbp-88h] BYREF
  int v4; // [rsp+64h] [rbp-84h] BYREF
  _BYTE v5[24]; // [rsp+68h] [rbp-80h] BYREF
  int v6[20]; // [rsp+80h] [rbp-68h] BYREF

  ObjectProperty = 0;
  if ( (*(_DWORD *)(a1 + 4) & 1) != 0 )
  {
    ObjectProperty = PnpGetObjectProperty(
                       PiPnpRtlCtx,
                       *(_QWORD *)(*(_QWORD *)(a1 + 8) + 16LL),
                       3,
                       0,
                       0LL,
                       (__int64)&DEVPKEY_Device_ContainerId,
                       (__int64)&v3,
                       (__int64)v5,
                       16,
                       (__int64)&v4,
                       0);
    if ( ObjectProperty >= 0 && v3 == 13 && v4 == 16 )
    {
      ObjectProperty = PnpStringFromGuid(v5, v6);
      if ( ObjectProperty >= 0 )
        return (unsigned int)PiDcContainerRequiresConfiguration((int)v6);
    }
  }
  return (unsigned int)ObjectProperty;
}
