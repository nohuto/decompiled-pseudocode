/*
 * XREFs of pIoQueryDeviceDescription @ 0x140586DD0
 * Callers:
...
__int64 __fastcall pIoQueryDeviceDescription(unsigned int **a1, __int128 *a2, __int64 a3, unsigned int a4, __int64 a5)
{
  ...
  PVOID v24; // [rsp+90h] [rbp-78h] BYREF
      ...
      if ( v11 < v12 )
      {
        while ( 1 )
        {
          *(_OWORD *)&Destination[1] = v13;
          ...
          if ( appended >= 0 )
          {
            appended = IopGetRegistryValues(Handle, &v27);
            ZwClose(Handle);
            Handle = 0LL;
            if ( appended >= 0 )
              break;
          ...
          if ( appended >= 0 )
          {
            ...
            else
            {
              ...
              if ( IopOpenRegistryKey(&KeyHandle, 0LL, (UNICODE_STRING *)&Destination[1], 0x20019u, v19) < 0
                || (appended = IopGetRegistryKeyInformation(KeyHandle, &v24),
                    ZwClose(KeyHandle),
                    KeyHandle = 0LL,
                    appended < 0) )
              {
            ...
            while ( v15 < v16 )
            {
              *(_OWORD *)&Destination[1] = v17;
              ...
              if ( appended >= 0 )
              {
                appended = IopGetRegistryValues(KeyHandle, &v30);
                ZwClose(KeyHandle);
                KeyHandle = 0LL;
                if ( appended >= 0 )
                {
