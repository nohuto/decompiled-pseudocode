/*
 * XREFs of KiHeteroChooseTargetProcessor @ 0x140203FB8
 * Callers:
 *     KiDeferredReadyThread @ 0x1400E05F0 (KiDeferredReadyThread.c)
 * Callees:
 *     KiGetHeteroCpuPolicyThread @ 0x14001AF38 (KiGetHeteroCpuPolicyThread.c)
 *     KeYieldProcessorEx @ 0x140040CC0 (KeYieldProcessorEx.c)
 *     KiGenerateHeteroSets @ 0x140203F48 (KiGenerateHeteroSets.c)
 *     KiSelectProcessorToPreempt @ 0x140204298 (KiSelectProcessorToPreempt.c)
 *     KiSelectCandidateProcessor @ 0x1402077D4 (KiSelectCandidateProcessor.c)
 */

__int64 __fastcall KiHeteroChooseTargetProcessor(__int64 a1, __int64 a2, __int64 *a3, _DWORD *a4)
{
  __int64 v4; // r15
  __int64 v6; // rbx
  int HeteroCpuPolicyThread; // eax
  __int64 v9; // r14
  __int64 v10; // rsi
  int v11; // eax
  __int64 v12; // r8
  __int64 v13; // rdi
  __int64 v14; // r9
  unsigned __int64 v15; // r8
  int v16; // ecx
  unsigned __int64 v17; // rax
  __int64 v18; // rbx
  char v19; // cl
  unsigned __int64 v20; // rax
  unsigned __int8 v21; // r9
  int v22; // edx
  unsigned __int64 v23; // rax
  __int64 v24; // rax
  __int64 v25; // rax
  __int64 v26; // rax
  __int64 v27; // r9
  char v28; // cl
  int v29; // edx
  unsigned __int64 v30; // rax
  bool v31; // zf
  int v33; // [rsp+30h] [rbp-30h] BYREF
  int v34; // [rsp+34h] [rbp-2Ch]
  int v35; // [rsp+38h] [rbp-28h]
  int v36; // [rsp+3Ch] [rbp-24h]
  __int64 v37; // [rsp+40h] [rbp-20h] BYREF
  __int64 v38; // [rsp+48h] [rbp-18h] BYREF
  __int64 v39; // [rsp+50h] [rbp-10h] BYREF
  __int64 v40; // [rsp+58h] [rbp-8h]
  int v42; // [rsp+A8h] [rbp+48h]
  int v43; // [rsp+B0h] [rbp+50h] BYREF

  v4 = *a3;
  v40 = *a3;
  v6 = a1;
  HeteroCpuPolicyThread = KiGetHeteroCpuPolicyThread(a2, a1);
  v42 = HeteroCpuPolicyThread;
  v9 = KiProcessorBlock[*(unsigned int *)(a2 + 588)];
  v10 = *(_QWORD *)(v9 + 192);
  while ( 1 )
  {
    v11 = KiGenerateHeteroSets(v10, v4, HeteroCpuPolicyThread, &v39, &v38, &v37);
    v12 = *(_QWORD *)v10;
    v13 = v37;
    if ( (*(_QWORD *)v10 & v37) != 0 || v11 || (v13 = v37, (*(_QWORD *)(v10 + 80) & v37) != 0) )
    {
      v14 = v38;
    }
    else
    {
      v13 = v4;
      v37 = v4;
      v14 = v4;
      v38 = v4;
      v39 = v4;
    }
    v15 = v13 & v12;
    if ( !v15 )
      break;
    if ( *(_QWORD *)(v6 + 200) != *(_QWORD *)(v6 + 25176) )
    {
      v14 = v38;
      if ( (v15 & *(_QWORD *)(v10 + 8)) != 0 )
        v15 &= *(_QWORD *)(v10 + 8);
    }
    v16 = 0;
    v17 = v39 & v15;
    if ( (v39 & v15) != 0 )
      goto LABEL_14;
    v17 = v14 & v15;
    if ( (v14 & v15) != 0 )
    {
      v16 = 1;
LABEL_14:
      v15 = v17;
    }
    v18 = v9;
    if ( (v15 & *(_QWORD *)(v9 + 200)) == 0 || v16 )
    {
      if ( v16 )
      {
        v18 = 0LL;
        v21 = 0;
        if ( v15 )
        {
          v22 = *(unsigned __int16 *)(v10 + 144) << 6;
          do
          {
            _BitScanForward64(&v23, v15);
            v24 = (unsigned int)KiProcessorNumberToIndexMappingTable[v22 + (v23 & 0x3F)];
            v35 = v24;
            v25 = KiProcessorBlock[v24];
            if ( !v18 || *(_BYTE *)(v25 + 23987) > v21 )
            {
              v18 = v25;
              v21 = *(_BYTE *)(v25 + 23987);
            }
            v15 &= ~*(_QWORD *)(v25 + 200);
          }
          while ( v15 );
        }
      }
      else
      {
        v19 = *(_BYTE *)(v9 + 209);
        if ( (v15 & *(_QWORD *)(v9 + 25176)) != 0 )
          v15 &= *(_QWORD *)(v9 + 25176);
        _BitScanForward64(&v20, __ROR8__(v15, v19));
        v34 = KiProcessorNumberToIndexMappingTable[64 * *(unsigned __int8 *)(v9 + 208) + (((_BYTE)v20 + v19) & 0x3F)];
        v18 = KiProcessorBlock[v34];
      }
    }
    *a4 = 1;
    v43 = 0;
    while ( _interlockedbittestandset64((volatile signed __int32 *)(v18 + 48), 0LL) )
    {
      do
        KeYieldProcessorEx(&v43);
      while ( *(_QWORD *)(v18 + 48) );
    }
    v4 = v40;
    if ( !*(_BYTE *)(v18 + 35) )
      return v18;
LABEL_42:
    _InterlockedAnd64((volatile signed __int64 *)(v18 + 48), 0LL);
    v6 = a1;
    HeteroCpuPolicyThread = v42;
  }
  if ( v14 == v13 )
    v14 = 0LL;
  v26 = KiSelectProcessorToPreempt(a2, v9, v13, v14);
  v18 = v26;
  if ( (v13 & *(_QWORD *)(v26 + 200)) == 0 )
  {
    v28 = *(_BYTE *)(v26 + 209);
    v29 = *(unsigned __int8 *)(v26 + 208);
    _BitScanForward64(&v30, __ROR8__(v13, v28));
    v36 = KiProcessorNumberToIndexMappingTable[64 * v29 + (((_BYTE)v30 + v28) & 0x3F)];
    v18 = KiProcessorBlock[v36];
  }
  v33 = 0;
  while ( _interlockedbittestandset64((volatile signed __int32 *)(v18 + 48), 0LL) )
  {
    do
      KeYieldProcessorEx(&v33);
    while ( *(_QWORD *)(v18 + 48) );
  }
  if ( (*(_QWORD *)v10 & v37) != 0 )
    goto LABEL_42;
  v31 = (*(_BYTE *)(v18 + 35) & 2) == 0;
  *a4 = 0;
  if ( !v31 )
  {
    _InterlockedAnd64((volatile signed __int64 *)(v18 + 48), 0LL);
    v18 = KiSelectCandidateProcessor(v18, a2, v37, v27);
    if ( (*(_BYTE *)(v18 + 35) & 1) == 0 )
      *a4 = 1;
  }
  return v18;
}
