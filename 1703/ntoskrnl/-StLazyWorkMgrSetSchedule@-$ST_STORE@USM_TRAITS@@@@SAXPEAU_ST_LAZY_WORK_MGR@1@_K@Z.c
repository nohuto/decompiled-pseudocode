/*
 * XREFs of ?StLazyWorkMgrSetSchedule@?$ST_STORE@USM_TRAITS@@@@SAXPEAU_ST_LAZY_WORK_MGR@1@_K@Z @ 0x140059AEC
 * Callers:
 *     ?StLazyWorkMgrRunExpiredWork@?$ST_STORE@USM_TRAITS@@@@SAXPEAU_ST_LAZY_WORK_MGR@1@_K@Z @ 0x14003A9C4 (-StLazyWorkMgrRunExpiredWork@-$ST_STORE@USM_TRAITS@@@@SAXPEAU_ST_LAZY_WORK_MGR@1@_K@Z.c)
 *     ?StLazyWorkMgrResetIdle@?$ST_STORE@USM_TRAITS@@@@SAXPEAU_ST_LAZY_WORK_MGR@1@@Z @ 0x140054BF0 (-StLazyWorkMgrResetIdle@-$ST_STORE@USM_TRAITS@@@@SAXPEAU_ST_LAZY_WORK_MGR@1@@Z.c)
 *     ?StLazyWorkMgrQueueWork@?$ST_STORE@USM_TRAITS@@@@SAXPEAU_ST_LAZY_WORK_MGR@1@W4_ST_LAZY_WORK_TYPE@1@KK@Z @ 0x140059944 (-StLazyWorkMgrQueueWork@-$ST_STORE@USM_TRAITS@@@@SAXPEAU_ST_LAZY_WORK_MGR@1@W4_ST_LAZY_WORK_TYPE.c)
 * Callees:
 *     KiQueryUnbiasedInterruptTime @ 0x14008AB1C (KiQueryUnbiasedInterruptTime.c)
 */

__int64 __fastcall ST_STORE<SM_TRAITS>::StLazyWorkMgrSetSchedule(unsigned __int64 a1, unsigned __int64 a2)
{
  unsigned __int64 UnbiasedInterruptTime; // r9
  unsigned __int64 v3; // r11
  unsigned __int64 v4; // r8
  unsigned __int64 *v5; // rbx
  unsigned __int64 v6; // rdx
  unsigned __int64 *v7; // r10
  __int64 result; // rax
  unsigned __int64 v9; // rcx
  unsigned __int64 v10; // rcx

  UnbiasedInterruptTime = a2;
  v3 = a1;
  if ( !a2 )
  {
    LOBYTE(a1) = 1;
    UnbiasedInterruptTime = KiQueryUnbiasedInterruptTime(a1);
  }
  v4 = -1LL;
  v5 = (unsigned __int64 *)(v3 + 168);
  v6 = -1LL;
  if ( v3 < v3 + 168 )
  {
    v7 = (unsigned __int64 *)(v3 + 8);
    do
    {
      if ( *v7 != -1LL )
      {
        v9 = UnbiasedInterruptTime - *(_QWORD *)(v3 + 184);
        if ( v7[1] >= v9 )
          v9 = v7[1];
        v10 = *(v7 - 1) + v9;
        if ( v10 >= v4 )
          v10 = v4;
        v4 = v10;
        if ( *v7 < v6 )
          v6 = *v7;
      }
      v7 += 3;
    }
    while ( v7 - 1 < v5 );
  }
  if ( v4 < UnbiasedInterruptTime )
    v4 = UnbiasedInterruptTime;
  if ( v6 < UnbiasedInterruptTime )
    v6 = UnbiasedInterruptTime;
  *v5 = v6;
  if ( v6 == -1LL )
  {
    *(_QWORD *)(v3 + 176) = 0LL;
  }
  else
  {
    if ( v6 < v4 )
      *(_QWORD *)(v3 + 176) = v6 - UnbiasedInterruptTime;
    else
      *(_QWORD *)(v3 + 176) = v4 - UnbiasedInterruptTime;
    if ( !*(_QWORD *)(v3 + 176) )
      *(_QWORD *)(v3 + 176) = 1LL;
  }
  result = *(_QWORD *)(v3 - 5688);
  *(_QWORD *)(result + 6552) = -*(_QWORD *)(v3 + 176);
  return result;
}
