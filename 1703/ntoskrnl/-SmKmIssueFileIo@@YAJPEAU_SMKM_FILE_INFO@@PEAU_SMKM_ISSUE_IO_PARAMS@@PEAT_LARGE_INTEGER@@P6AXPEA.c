/*
 * XREFs of ?SmKmIssueFileIo@@YAJPEAU_SMKM_FILE_INFO@@PEAU_SMKM_ISSUE_IO_PARAMS@@PEAT_LARGE_INTEGER@@P6AXPEAXPEAU_IO_STATUS_BLOCK@@K@Z3@Z @ 0x14024D714
 * Callers:
 *     SmKmIssueIo @ 0x14024DDDC (SmKmIssueIo.c)
 * Callees:
 *     _guard_dispatch_icall @ 0x140189DC0 (_guard_dispatch_icall.c)
 *     NtWriteFile @ 0x1404818C0 (NtWriteFile.c)
 *     NtReadFile @ 0x140566B00 (NtReadFile.c)
 */

__int64 __fastcall SmKmIssueFileIo(
        void **a1,
        struct _SMKM_ISSUE_IO_PARAMS *a2,
        union _LARGE_INTEGER *a3,
        void (*a4)(void *, struct _IO_STATUS_BLOCK *, unsigned int),
        PVOID ApcContext)
{
  NTSTATUS *v6; // rbx
  void *v7; // rcx
  NTSTATUS File; // eax
  struct _IO_STATUS_BLOCK *v10; // [rsp+20h] [rbp-38h]
  void *v11; // [rsp+28h] [rbp-30h]
  ULONG Length; // [rsp+30h] [rbp-28h]

  v6 = *(NTSTATUS **)a2;
  v7 = *a1;
  Length = *((_DWORD *)a2 + 4);
  v11 = (void *)*((_QWORD *)a2 + 1);
  v10 = *(struct _IO_STATUS_BLOCK **)a2;
  if ( (*((_DWORD *)a2 + 8) & 1) != 0 )
    File = NtReadFile(v7, 0LL, a4, ApcContext, v10, v11, Length, a3, 0LL);
  else
    File = NtWriteFile(v7, 0LL, a4, ApcContext, v10, v11, Length, a3, 0LL);
  if ( (File & 0xC0000000) == 0xC0000000 )
  {
    *v6 = File;
    ((void (__fastcall *)(PVOID, NTSTATUS *, _QWORD))a4)(ApcContext, v6, 0LL);
  }
  return 259LL;
}
