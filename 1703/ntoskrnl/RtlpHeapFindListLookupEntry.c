/*
 * XREFs of RtlpHeapFindListLookupEntry @ 0x140010390
 * Callers:
 *     RtlpFindEntry @ 0x14001031C (RtlpFindEntry.c)
 *     RtlpFindUCREntry @ 0x1400108C8 (RtlpFindUCREntry.c)
 * Callees:
 *     RtlpHeapListCompare @ 0x140010560 (RtlpHeapListCompare.c)
 */

_QWORD *__fastcall RtlpHeapFindListLookupEntry(__int64 a1, __int64 a2, char a3, __int64 a4, __int64 a5)
{
  _QWORD *v5; // rdi
  __int64 v6; // r14
  __int64 v8; // rbx
  int v9; // r13d
  _QWORD *v12; // rdx
  __int64 v13; // r9
  __int64 v14; // r9
  __int64 v16; // r8
  unsigned int *v17; // r9
  unsigned int v18; // edx
  int v19; // eax
  __int64 v20; // r8
  _QWORD *i; // rsi

  v5 = *(_QWORD **)(a2 + 32);
  v6 = (unsigned int)(a4 - *(_DWORD *)(a2 + 24));
  v8 = 0LL;
  v9 = a4;
  v12 = (_QWORD *)v5[1];
  if ( v5 == v12 )
    return v5;
  LOBYTE(a4) = a3;
  if ( (int)RtlpHeapListCompare(a1, v12, a5, a4) > 0 )
    return v5;
  LOBYTE(v13) = a3;
  if ( (int)RtlpHeapListCompare(a1, *v5, a5, v13) <= 0 )
    return (_QWORD *)*v5;
  if ( !*(_QWORD *)a2 && v9 == *(_DWORD *)(a2 + 8) - 1 )
  {
    if ( *(_DWORD *)(a2 + 12) )
      v6 = (unsigned int)(2 * v6);
    for ( i = *(_QWORD **)(*(_QWORD *)(a2 + 48) + 8 * v6); v5 != i; i = (_QWORD *)*i )
    {
      LOBYTE(v14) = a3;
      if ( (int)RtlpHeapListCompare(a1, i, a5, v14) <= 0 )
        return i;
    }
  }
  else
  {
    v16 = (unsigned int)v6 >> 5;
    v17 = (unsigned int *)(*(_QWORD *)(a2 + 40) + 4 * v16);
    v18 = *v17 & ~((1 << (v6 & 0x1F)) - 1);
    if ( v18 )
      goto LABEL_9;
    do
    {
      if ( (unsigned int)v16 > ((unsigned int)(*(_DWORD *)(a2 + 8) - *(_DWORD *)(a2 + 24)) >> 5) - 1 )
        break;
      ++v17;
      LODWORD(v16) = v16 + 1;
      v18 = *v17;
    }
    while ( !*v17 );
    if ( v18 )
    {
LABEL_9:
      if ( (_WORD)v18 )
      {
        if ( (_BYTE)v18 )
          v19 = RtlpBitsClearLow[(unsigned __int8)v18];
        else
          v19 = RtlpBitsClearLow[BYTE1(v18)] + 8;
      }
      else if ( (v18 & 0xFF0000) != 0 )
      {
        v19 = RtlpBitsClearLow[BYTE2(v18)] + 16;
      }
      else
      {
        v19 = RtlpBitsClearLow[(unsigned __int64)v18 >> 24] + 24;
      }
      v20 = (unsigned int)(v19 + 32 * v16);
      if ( *(_DWORD *)(a2 + 12) )
        v20 = (unsigned int)(2 * v20);
      return *(_QWORD **)(*(_QWORD *)(a2 + 48) + 8 * v20);
    }
  }
  return (_QWORD *)v8;
}
