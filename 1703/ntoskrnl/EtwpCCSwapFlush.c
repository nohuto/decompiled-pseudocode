/*
 * XREFs of EtwpCCSwapFlush @ 0x140028E80
 * Callers:
    ...
    if ( (_UNKNOWN *)((char *)&EtwpGroupMasks + result) )
    {
      ...
      if ( (result & 4) != 0 )
      {
        if ( v8 )
        {
          if ( (result & 0x100) != 0 )
          {
            v9 = (unsigned __int8)EtwpSystemLogger[2 * v7];
            result = EtwpOpenLogger((unsigned __int8)EtwpSystemLogger[2 * v7], EtwpHostSiloState, 0LL, v11);
            if ( result )
            {
              if ( *(_DWORD *)(result + 200) == a2 )
                EtwpLogKernelEvent((unsigned int)&v12, EtwpHostSiloState, v9, 1, 1317, 514);
              LOBYTE(v10) = v11[0];
