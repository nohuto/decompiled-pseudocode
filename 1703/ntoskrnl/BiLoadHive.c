/*
 * XREFs of BiLoadHive @ 0x14058CF74
 * Callers:
...
__int64 __fastcall BiLoadHive(PCWSTR SourceString, __int64 a2, HANDLE *a3)
{
  ...
  for ( i = 0; ; ++i )
  {
    ...
    if ( (unsigned __int8)BiDoesHiveExist(a2) )
    {
      ...
      if ( v7 < 0 )
      {
        BiLogMessage(4u, L"Failed open key %ws. Status: %x", L"\\Registry\\Machine", (unsigned int)v7);
        v6 = Handle;
      }
      else
      {
        ...
        else
        {
          ...
          else
          {
            ...
            if ( v8 < 0 )
            {
              ...
              BiReleasePrivilege(v15);
              LODWORD(v12) = v8;
              BiLogMessage(4u, L"Failed open newly loaded key %ws. Flags: 0x%x Status: %x", SourceString, 576LL, v12);
            }
          }
        }
      }
