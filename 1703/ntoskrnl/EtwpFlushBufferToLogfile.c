/*
 * XREFs of EtwpFlushBufferToLogfile @ 0x140550754
 * Callers:
 *     EtwpFlushBuffer @ 0x1405501C4 (EtwpFlushBuffer.c)
 *     EtwpBufferingModeFlush @ 0x14070F288 (EtwpBufferingModeFlush.c)
 * Callees:
 *     EtwEventEnabled @ 0x140047810 (EtwEventEnabled.c)
 *     EtwpQueryMaximumFileSize @ 0x1401223B4 (EtwpQueryMaximumFileSize.c)
 *     ZwWriteFile @ 0x14017E040 (ZwWriteFile.c)
 *     EtwpGenerateFileName @ 0x14045E0F4 (EtwpGenerateFileName.c)
 *     EtwpEventWriteTemplateAdmin @ 0x14070DAC8 (EtwpEventWriteTemplateAdmin.c)
 *     EtwpEventWriteTemplateMaxFileSize @ 0x14070DC28 (EtwpEventWriteTemplateMaxFileSize.c)
 */

__int64 __fastcall EtwpFlushBufferToLogfile(__int64 a1, unsigned int *a2)
{
  unsigned __int64 Length; // rbp
  __int64 v5; // rcx
  unsigned __int64 MaximumFileSize; // rsi
  NTSTATUS v7; // edi
  __int64 v8; // rcx
  int v10; // ecx
  int v11; // edx
  int v12; // r8d
  int v13; // r8d
  struct _IO_STATUS_BLOCK IoStatusBlock; // [rsp+50h] [rbp-18h] BYREF

  Length = *a2;
  if ( *(_DWORD *)(a1 + 292) )
  {
    MaximumFileSize = EtwpQueryMaximumFileSize(a1);
    if ( Length * *(unsigned int *)(v5 + 204) >= MaximumFileSize )
    {
      v10 = *(_DWORD *)(v5 + 12);
      if ( (v10 & 2) != 0 )
      {
        *(_QWORD *)(a1 + 216) = *(unsigned int *)(a1 + 4);
        *(_DWORD *)(a1 + 204) = 1;
      }
      else
      {
        if ( (v10 & 8) == 0 )
        {
          v7 = -1073741432;
          if ( (*(_DWORD *)(a1 + 816) & 0x100) == 0 )
          {
            _InterlockedOr((volatile signed __int32 *)(a1 + 816), 0x100u);
            if ( EtwEventEnabled(EtwpEventTracingProvRegHandle, &ETW_EVENT_MAX_FILE_SIZE_REACHED) )
              EtwpEventWriteTemplateMaxFileSize(a1 + 152, v11, v12, a1 + 136, a1 + 152);
          }
          goto LABEL_16;
        }
        if ( (*(_DWORD *)(a1 + 820) & 1) == 0 )
        {
          EtwpGenerateFileName(
            (unsigned __int16 *)(a1 + 168),
            (volatile signed __int32 *)(a1 + 296),
            (UNICODE_STRING *)(a1 + 184));
          _InterlockedOr((volatile signed __int32 *)(a1 + 820), 1u);
        }
      }
    }
  }
  v7 = ZwWriteFile(*(HANDLE *)(a1 + 800), 0LL, 0LL, 0LL, &IoStatusBlock, a2, Length, (PLARGE_INTEGER)(a1 + 216), 0LL);
  if ( v7 < 0 )
  {
    if ( EtwEventEnabled(EtwpEventTracingProvRegHandle, &ETW_EVENT_WRITE_FAILED) )
      EtwpEventWriteTemplateAdmin(
        a1 + 152,
        (unsigned int)&ETW_EVENT_WRITE_FAILED,
        v13,
        a1 + 136,
        a1 + 152,
        v7,
        *(_DWORD *)(a1 + 12));
  }
  else
  {
    v8 = *(_QWORD *)(a1 + 944);
    *(_QWORD *)(a1 + 216) += Length;
    _InterlockedExchangeAdd64((volatile signed __int64 *)(v8 + 5096), Length);
  }
  if ( v7 >= 0 )
  {
    ++*(_DWORD *)(a1 + 248);
    ++*(_DWORD *)(a1 + 204);
    return (unsigned int)v7;
  }
LABEL_16:
  _InterlockedIncrement((volatile signed __int32 *)(a1 + 252));
  if ( v7 == -1073741670 )
    return 0;
  return (unsigned int)v7;
}
