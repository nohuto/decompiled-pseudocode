/*
 * XREFs of DrvDbGetDriverFileMappedProperty @ 0x140743070
 * Callers:
 *     DrvDbDispatchDriverFile @ 0x140745820 (DrvDbDispatchDriverFile.c)
 * Callees:
 *     ZwClose @ 0x14017E120 (ZwClose.c)
 *     DrvDbOpenObjectRegKey @ 0x140486A00 (DrvDbOpenObjectRegKey.c)
 *     DrvDbGetRegValueMappedProperty @ 0x1404E8D50 (DrvDbGetRegValueMappedProperty.c)
 */

__int64 __fastcall DrvDbGetDriverFileMappedProperty(
        __int64 a1,
        __int64 a2,
        HANDLE a3,
        __int64 a4,
        _DWORD *a5,
        _WORD *a6,
        unsigned int a7,
        _DWORD *a8)
{
  _UNKNOWN **v11; // r11
  __int64 v12; // r8
  _QWORD *v13; // rdx
  __int64 v14; // rcx
  int RegValueMappedProperty; // ebx
  _UNKNOWN **v17; // rdi
  HANDLE Handle[2]; // [rsp+40h] [rbp-28h] BYREF

  Handle[0] = 0LL;
  v11 = &off_14028EB70;
  *a5 = 0;
  *a8 = 0;
  v12 = 0LL;
  while ( 1 )
  {
    v13 = *v11;
    if ( *((_DWORD *)*v11 + 4) == *(_DWORD *)(a4 + 16) )
    {
      v14 = *v13 - *(_QWORD *)a4;
      if ( *v13 == *(_QWORD *)a4 )
        v14 = v13[1] - *(_QWORD *)(a4 + 8);
      if ( !v14 )
        break;
    }
    v12 = (unsigned int)(v12 + 1);
    v11 += 5;
    if ( (unsigned int)v12 >= 2 )
      return (unsigned int)-1073741802;
  }
  v17 = &off_14028EB70 + 5 * v12;
  if ( !v17 )
    return (unsigned int)-1073741802;
  if ( !a3 )
  {
    RegValueMappedProperty = DrvDbOpenObjectRegKey(a1, 0LL, 4u, a2, 1, 0, Handle, 0LL);
    if ( RegValueMappedProperty < 0 )
      goto LABEL_14;
    a3 = Handle[0];
  }
  RegValueMappedProperty = DrvDbGetRegValueMappedProperty(v14, a3, (__int64)v17, a5, a6, a7, a8);
LABEL_14:
  if ( Handle[0] )
    ZwClose(Handle[0]);
  return (unsigned int)RegValueMappedProperty;
}
