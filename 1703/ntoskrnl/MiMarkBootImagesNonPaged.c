/*
 * XREFs of MiMarkBootImagesNonPaged @ 0x14081570C
 * Callers:
...
void __fastcall MiMarkBootImagesNonPaged(__int64 a1)
{
  ...
  v1 = a1 + 16;
  for ( i = *(_QWORD *)(a1 + 16); i != v1; i = *v4 )
  {
    v3 = *(_QWORD *)(i + 48);
    if ( !(unsigned int)MI_IS_PHYSICAL_ADDRESS(v3) )
    {
      v5 = ((v3 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL;
      v6 = v5 + 8LL * (unsigned int)(((unsigned __int64)*((unsigned int *)v4 + 16) + 4095) >> 12);
      while ( v5 < v6 )
      {
        v8 = MI_READ_PTE_LOCK_FREE(v5);
        v7 = 6 * (((unsigned __int64)MI_READ_PTE_LOCK_FREE((unsigned __int64)&v8) >> 12) & 0xFFFFFFFFFLL);
