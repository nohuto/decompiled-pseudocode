/*
 * XREFs of SmEtwLogStoreCorruption @ 0x14024D878
 * Callers:
 *     ?StDmPageError@?$ST_STORE@USM_TRAITS@@@@SAXPEAU_ST_DATA_MGR@1@PEAD1PEAU_ST_PAGE_LOCATION@1@J@Z @ 0x14024BF10 (-StDmPageError@-$ST_STORE@USM_TRAITS@@@@SAXPEAU_ST_DATA_MGR@1@PEAD1PEAU_ST_PAGE_LOCATION@1@J@Z.c)
 * Callees:
 *     EtwWriteEx @ 0x140094690 (EtwWriteEx.c)
 *     MmGetPhysicalAddress @ 0x1400A7A80 (MmGetPhysicalAddress.c)
 *     __security_check_cookie @ 0x140169210 (__security_check_cookie.c)
 */

NTSTATUS __fastcall SmEtwLogStoreCorruption(REGHANDLE *a1, __int64 a2, __int64 a3, void *a4, char a5, __int64 *a6)
{
  __int64 v7; // rcx
  PHYSICAL_ADDRESS PhysicalAddress; // rax
  REGHANDLE v9; // rcx
  __int64 v11; // [rsp+40h] [rbp-79h] BYREF
  void *v12; // [rsp+48h] [rbp-71h] BYREF
  PHYSICAL_ADDRESS v13; // [rsp+50h] [rbp-69h] BYREF
  struct _EVENT_DATA_DESCRIPTOR UserData; // [rsp+60h] [rbp-59h] BYREF
  void **v15; // [rsp+70h] [rbp-49h]
  int v16; // [rsp+78h] [rbp-41h]
  int v17; // [rsp+7Ch] [rbp-3Dh]
  PHYSICAL_ADDRESS *v18; // [rsp+80h] [rbp-39h]
  int v19; // [rsp+88h] [rbp-31h]
  int v20; // [rsp+8Ch] [rbp-2Dh]
  char *v21; // [rsp+90h] [rbp-29h]
  int v22; // [rsp+98h] [rbp-21h]
  int v23; // [rsp+9Ch] [rbp-1Dh]
  __int64 *v24; // [rsp+A0h] [rbp-19h]
  int v25; // [rsp+A8h] [rbp-11h]
  int v26; // [rsp+ACh] [rbp-Dh]
  char *v27; // [rsp+B0h] [rbp-9h]
  int v28; // [rsp+B8h] [rbp-1h]
  int v29; // [rsp+BCh] [rbp+3h]
  char *v30; // [rsp+C0h] [rbp+7h]
  int v31; // [rsp+C8h] [rbp+Fh]
  int v32; // [rsp+CCh] [rbp+13h]
  __int64 v33; // [rsp+120h] [rbp+67h] BYREF

  v33 = a3;
  UserData.Reserved = 0;
  v17 = 0;
  v7 = *a6;
  UserData.Ptr = (ULONGLONG)&v33;
  v11 = v7;
  v15 = &v12;
  v12 = a4;
  UserData.Size = 8;
  v16 = 8;
  PhysicalAddress = MmGetPhysicalAddress(a4);
  v20 = 0;
  v23 = 0;
  v26 = 0;
  v29 = 0;
  v32 = 0;
  v9 = *a1;
  v13 = PhysicalAddress;
  v18 = &v13;
  v21 = &a5;
  v24 = &v11;
  v27 = (char *)&v11 + 1;
  v30 = (char *)&v11 + 4;
  v19 = 8;
  v22 = 2;
  v25 = 1;
  v28 = 1;
  v31 = 4;
  return EtwWriteEx(v9, &StEventStoreCorruption, 0LL, 1u, 0LL, 0LL, 7u, &UserData);
}
