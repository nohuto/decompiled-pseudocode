        ...
        __int64 *a2)
{
  ...
  if ( (__int64 *)result == v5 )
  {
    ...
    try
    {
      ...
      std::_Uninitialized_move_al_unchecked1<std::unique_ptr<CAppSubmixClient> *,std::unique_ptr<CAppSubmixClient> *,std::allocator<std::unique_ptr<CAppSubmixClient>>>(
        *a1,
        a1[1],
        v11);
    }
    ...
    return std::vector<std::unique_ptr<CAppSubmixClient>>::_Change_array(a1, v11, v6 + 1, v9);
  }
  else
  {
    if ( v5 )
