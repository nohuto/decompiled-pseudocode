/*
 * XREFs of ?GetChannelsPeakValues@CMeterSoftware@@UEAAJIPEAM@Z @ 0x180083930
 * Callers:
 *     <none>
 * Callees:
 *     ?Lock@CCritSecLock@ATL@@QEAAXXZ @ 0x18001A360 (-Lock@CCritSecLock@ATL@@QEAAXXZ.c)
 *     __security_check_cookie @ 0x180048BE0 (__security_check_cookie.c)
 *     memset @ 0x180049B8A (memset.c)
 *     ?AudSrvTraceLoggingErrorHelper@@YAXPEBDIJ@Z @ 0x18004B5C4 (-AudSrvTraceLoggingErrorHelper@@YAXPEBDIJ@Z.c)
 *     _guard_dispatch_icall_nop @ 0x180057FC0 (_guard_dispatch_icall_nop.c)
 */

__int64 __fastcall CMeterSoftware::GetChannelsPeakValues(CMeterSoftware *this, unsigned int a2, float *a3)
{
  __int64 v3; // rdi
  float *v4; // rbp
  int v6; // ebx
  __int64 v7; // rcx
  int v8; // eax
  float *v9; // rax
  __int64 v10; // rcx
  float v11; // xmm0_4
  __int64 v12; // rcx
  LPCRITICAL_SECTION lpCriticalSection; // [rsp+20h] [rbp-1B8h] BYREF
  char v15; // [rsp+28h] [rbp-1B0h]
  _BYTE v16[384]; // [rsp+30h] [rbp-1A8h] BYREF

  v3 = a2;
  v4 = a3;
  if ( !a3 )
  {
    v6 = -2147467261;
LABEL_26:
    AudSrvTraceLoggingErrorHelper("CMeterSoftware::GetChannelsPeakValues", 688, v6);
    return (unsigned int)v6;
  }
  if ( a2 != *((_DWORD *)this + 8) )
  {
    v6 = -2147024809;
    goto LABEL_26;
  }
  v6 = 0;
  lpCriticalSection = (LPCRITICAL_SECTION)((char *)this + 48);
  v15 = 0;
  ATL::CCritSecLock::Lock(&lpCriticalSection);
  v7 = *((_QWORD *)this + 5);
  if ( v7 && *((_DWORD *)this + 8) <= 0x20u )
  {
    v8 = (*(__int64 (__fastcall **)(__int64, _BYTE *, _QWORD))(*(_QWORD *)v7 + 24LL))(v7, v16, (unsigned int)v3);
    v6 = v8;
    if ( v8 < 0 )
    {
      if ( v8 == -2147417848 || v8 == -2147023174 )
      {
        if ( (_DWORD)v3 )
          memset(v4, 0, 4 * v3);
        v12 = *((_QWORD *)this + 5);
        if ( v12 )
        {
          *((_QWORD *)this + 5) = 0LL;
          (*(void (__fastcall **)(__int64))(*(_QWORD *)v12 + 16LL))(v12);
        }
        v6 = 0;
      }
    }
    else if ( (_DWORD)v3 )
    {
      v9 = (float *)v16;
      v10 = v3;
      do
      {
        v11 = fminf(*v9, 1.0);
        if ( v11 <= 0.0 )
          v11 = 0.0;
        *v4 = v11;
        v9 += 3;
        ++v4;
        --v10;
      }
      while ( v10 );
    }
  }
  else if ( (_DWORD)v3 )
  {
    memset(v4, 0, 4 * v3);
  }
  if ( v15 )
    LeaveCriticalSection(lpCriticalSection);
  if ( v6 < 0 )
    goto LABEL_26;
  return (unsigned int)v6;
}
