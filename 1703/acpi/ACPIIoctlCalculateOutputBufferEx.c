/*
 * XREFs of ACPIIoctlCalculateOutputBufferEx @ 0x1C0025574
 * Callers:
 *     ACPIIoctlEvalPostProcessingEx @ 0x1C00254BC (ACPIIoctlEvalPostProcessingEx.c)
 *     ACPIIoctlCalculateOutputBufferEx @ 0x1C0025574 (ACPIIoctlCalculateOutputBufferEx.c)
 *     AcpiConvertObjDataToMethodArguments @ 0x1C005349C (AcpiConvertObjDataToMethodArguments.c)
 * Callees:
 *     ACPIIoctlCalculateOutputBufferEx @ 0x1C0025574 (ACPIIoctlCalculateOutputBufferEx.c)
 *     ACPIIoctlCalculateOutputBufferSizeEx @ 0x1C00256AC (ACPIIoctlCalculateOutputBufferSizeEx.c)
 *     memmove @ 0x1C002C840 (memmove.c)
 */

__int64 __fastcall ACPIIoctlCalculateOutputBufferEx(__int64 a1, __int64 a2, char a3, __int64 a4)
{
  char *v4; // rbx
  __int16 v6; // cx
  __int64 result; // rax
  unsigned int *v9; // rbp
  unsigned int v10; // ecx
  unsigned int v11; // esi
  __int64 v12; // rax
  int v13; // eax
  char *i; // [rsp+40h] [rbp+8h] BYREF
  int v15; // [rsp+58h] [rbp+20h] BYREF

  v4 = (char *)a2;
  v6 = *(_WORD *)(a1 + 2);
  if ( v6 == 1 )
  {
    *(_DWORD *)a2 = 0x80000;
    *(_QWORD *)(a2 + 4) = *(_QWORD *)(a1 + 16);
    return 0LL;
  }
  if ( (unsigned __int16)(v6 - 2) <= 1u )
  {
    *(_WORD *)a2 = (v6 != 2) + 1;
    *(_WORD *)(a2 + 2) = *(_WORD *)(a1 + 24);
    memmove((void *)(a2 + 4), *(const void **)(a1 + 32), *(unsigned int *)(a1 + 24));
    return 0LL;
  }
  if ( v6 != 4 )
    return 3222536207LL;
  v9 = *(unsigned int **)(a1 + 32);
  if ( !*v9 )
    return 3222536207LL;
  LOBYTE(a4) = 1;
  LODWORD(i) = 0;
  v15 = 0;
  result = ACPIIoctlCalculateOutputBufferSizeEx(a1, &i, &v15, a4);
  if ( (int)result >= 0 )
  {
    if ( a3 )
    {
      v10 = (unsigned __int16)i;
    }
    else
    {
      *(_WORD *)v4 = 3;
      LOWORD(v13) = (_WORD)i;
      *((_WORD *)v4 + 1) = (_WORD)i;
      v4 += 4;
      if ( (unsigned __int16)v13 >= 4u )
        v13 = (unsigned __int16)v13;
      else
        v13 = 4;
      v10 = v13 + 4;
    }
    v11 = 0;
    for ( i = &v4[v10]; v4 < i; v4 += v12 + 4 )
    {
      if ( v11 >= *v9 )
        break;
      result = ACPIIoctlCalculateOutputBufferEx(&v9[8 * v11 + 2 + 2 * v11], v4, 0LL);
      if ( (int)result < 0 )
        return result;
      LOWORD(v12) = *((_WORD *)v4 + 1);
      ++v11;
      v12 = (unsigned __int16)v12 < 4u ? 4LL : (unsigned __int16)v12;
    }
    return 0LL;
  }
  return result;
}
