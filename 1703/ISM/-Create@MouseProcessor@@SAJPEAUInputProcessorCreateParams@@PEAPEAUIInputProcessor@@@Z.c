/*
 * XREFs of ?Create@MouseProcessor@@SAJPEAUInputProcessorCreateParams@@PEAPEAUIInputProcessor@@@Z @ 0x180058B10
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall MouseProcessor::Create(struct InputProcessorCreateParams *a1, struct IInputProcessor **a2)
{
  ...
  if ( *((int *)a1 + 8) >= 0 )
    v5 = *((_DWORD *)a1 + 8);
  v6 = (MouseProcessor *)malloc(0x250uLL);
  v8 = v6;
  if ( v6 )
    memset(v6, 0, 0x250uLL);
  if ( v8 )
  ...
  if ( v9 )
  {
    ...
    {
      Template_qqq(v12, &MinInput_Warning_CheckResult, 0, 164, v11);
    }
  }
  else
  {
    ...
    if ( (Microsoft_OneCore_MinInputEnableBits & 2) != 0 )
      Template_qqq(v7, &MinInput_Warning_CheckResult, 0, 159, 14);
  }
  return v10;
}
