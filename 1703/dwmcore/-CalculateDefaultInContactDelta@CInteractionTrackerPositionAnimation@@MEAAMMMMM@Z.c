/*
 * XREFs of ?CalculateDefaultInContactDelta@CInteractionTrackerPositionAnimation@@MEAAMMMMM@Z @ 0x180182B20
 * Callers:
 *     <none>
 * Callees:
 *     <none>
 */

float __fastcall CInteractionTrackerPositionAnimation::CalculateDefaultInContactDelta(
        CInteractionTrackerPositionAnimation *this,
        float a2,
        float a3,
        float a4,
        float a5)
{
  if ( (a4 <= a2 || a3 > 0.0) && (a2 <= a5 || a3 < 0.0) )
    return a3;
  else
    return (float)(1.0 - (float)(*((float *)this + 123) * 0.0099999998)) * a3;
}
