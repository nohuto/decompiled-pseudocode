/*
 * XREFs of ?_Tidy@?$vector@USubgraphOutput@CBrushRenderingGraphBuilder@@V?$allocator@USubgraphOutput@CBrushRenderingGraphBuilder@@@std@@@std@@IEAAXXZ @ 0x180003624
 * Callers:
    ...
    while ( v1 != v3 )
    {
      ...
      if ( v4 )
      {
        ...
        if ( v5 )
        {
          ...
          while ( v5 != v6 )
          {
            std::unique_ptr<CRenderingTechniqueFragment>::~unique_ptr<CRenderingTechniqueFragment>(v5 + 8);
            v5 += 16LL;
          }
        ...
        Microsoft::WRL::ComPtr<IAccelerator>::InternalRelease(v4);
