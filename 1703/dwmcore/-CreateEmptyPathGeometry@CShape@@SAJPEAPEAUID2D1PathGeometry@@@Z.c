/*
 * XREFs of ?CreateEmptyPathGeometry@CShape@@SAJPEAPEAUID2D1PathGeometry@@@Z @ 0x1801B01B0
 * Callers:
...
__int64 __fastcall CShape::CreateEmptyPathGeometry(struct ID2D1PathGeometry **a1)
{
  ...
  {
    MilInstrumentationCheckHR(0x14u, 0LL, 0, D2DFactoryNoRef, 0x1AFu);
  }
  else
  {
    ...
    if ( v4 < 0 )
    {
      MilInstrumentationCheckHR(0x14u, 0LL, 0, v4, 0x1B0u);
    }
    else
    {
      ...
      else
      {
        ...
        MilInstrumentationCheckHR(0x14u, 0LL, 0, v6, 0x1B4u);
      }
    }
  }
  if ( v9 )
