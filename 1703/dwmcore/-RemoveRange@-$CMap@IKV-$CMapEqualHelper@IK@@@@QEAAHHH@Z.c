/*
 * XREFs of ?RemoveRange@?$CMap@IKV?$CMapEqualHelper@IK@@@@QEAAHHH@Z @ 0x18018B37C
 * Callers:
 *     ?RemoveKey@?$CMap@IKV?$CMapEqualHelper@IK@@@@QEAAHAEBI@Z @ 0x18018B32C (-RemoveKey@-$CMap@IKV-$CMapEqualHelper@IK@@@@QEAAHAEBI@Z.c)
 * Callees:
 *     ?ReallocHeap@@YAPEAXPEAX_K@Z @ 0x1800C4738 (-ReallocHeap@@YAPEAXPEAX_K@Z.c)
 */

__int64 __fastcall CMap<unsigned int,unsigned long,CMapEqualHelper<unsigned int,unsigned long>>::RemoveRange(
        __int64 a1,
        int a2)
{
  __int64 v2; // rsi
  int v4; // ecx
  LPVOID v5; // rax
  LPVOID v6; // rax

  v2 = a2;
  if ( a2 < 0 )
    return 0LL;
  v4 = *(_DWORD *)(a1 + 16);
  if ( a2 >= v4 )
    return 0LL;
  if ( a2 + 1 != v4 )
  {
    memmove((void *)(*(_QWORD *)a1 + 4LL * a2), (const void *)(*(_QWORD *)a1 + 4 * (a2 + 1LL)), 4LL * (v4 - a2 - 1));
    memmove(
      (void *)(*(_QWORD *)(a1 + 8) + 4 * v2),
      (const void *)(*(_QWORD *)(a1 + 8) + 4 * (v2 + 1)),
      4LL * (*(_DWORD *)(a1 + 16) - (int)v2 - 1));
  }
  v5 = ReallocHeap(*(void **)a1, 4LL * (*(_DWORD *)(a1 + 16) - 1));
  if ( v5 )
    *(_QWORD *)a1 = v5;
  v6 = ReallocHeap(*(void **)(a1 + 8), 4LL * (*(_DWORD *)(a1 + 16) - 1));
  if ( v6 )
    *(_QWORD *)(a1 + 8) = v6;
  --*(_DWORD *)(a1 + 16);
  return 1LL;
}
