/*
 * XREFs of ?ApiReset@Animation@Animations@Components@@QEAAJ_N@Z @ 0x18002C7E4
 * Callers:
 *     ?AnDestroyAnimation@@YAXPEAVAnimation@Animations@Components@@@Z @ 0x18002A46C (-AnDestroyAnimation@@YAXPEAVAnimation@Animations@Components@@@Z.c)
 *     ?ApiPlay@Animation@Animations@Components@@QEAAJXZ @ 0x18002C724 (-ApiPlay@Animation@Animations@Components@@QEAAJXZ.c)
 *     ?Commit@Animation@Animations@Components@@QEAAJXZ @ 0x18002CAD0 (-Commit@Animation@Animations@Components@@QEAAJXZ.c)
 *     ?CalculateValueWorker@CKeyframeAnimation@@UEAAJPEAVCExpressionValueStack@@_KPEA_N@Z @ 0x180030350 (-CalculateValueWorker@CKeyframeAnimation@@UEAAJPEAVCExpressionValueStack@@_KPEA_N@Z.c)
 *     ?ApplyPlaybackStateChanges@CKeyframeAnimation@@AEAAJPEAVCExpressionValueStack@@@Z @ 0x180030B20 (-ApplyPlaybackStateChanges@CKeyframeAnimation@@AEAAJPEAVCExpressionValueStack@@@Z.c)
 *     ?ProcessSetBindingBroken@CKeyframeAnimation@@UEAAJPEAVCResourceTable@@PEBUMILCMD_BASEEXPRESSION_SETBINDINGBROKEN@@@Z @ 0x180030E90 (-ProcessSetBindingBroken@CKeyframeAnimation@@UEAAJPEAVCResourceTable@@PEBUMILCMD_BASEEXPRESSION_.c)
 * Callees:
 *     ?GetColor@Value@Animations@Components@@QEBA?AVColor@23@XZ @ 0x180028CDC (-GetColor@Value@Animations@Components@@QEBA-AVColor@23@XZ.c)
 *     ?Round@Math@Animations@Components@@SAHM@Z @ 0x180029698 (-Round@Math@Animations@Components@@SAHM@Z.c)
 *     ?Add@?$ArrayListF@PEAVAnimation@Animations@Components@@@Animations@Components@@QEAAJAEBQEAVAnimation@23@@Z @ 0x18002AA3C (-Add@-$ArrayListF@PEAVAnimation@Animations@Components@@@Animations@Components@@QEAAJAEBQEAVAnima.c)
 *     ?PrepareSequenceForIteration@Animation@Animations@Components@@AEAAXI@Z @ 0x18002C0AC (-PrepareSequenceForIteration@Animation@Animations@Components@@AEAAXI@Z.c)
 *     ?GetTimeLength@Animation@Animations@Components@@QEBAMXZ @ 0x18002C120 (-GetTimeLength@Animation@Animations@Components@@QEBAMXZ.c)
 *     ?OnAnimationEvent@CKeyframeAnimation@@UEAAJW4AnimationEventType@@@Z @ 0x18002F6A0 (-OnAnimationEvent@CKeyframeAnimation@@UEAAJW4AnimationEventType@@@Z.c)
 *     ?SetScalarValue@CKeyframeAnimation@@UEAAJM@Z @ 0x1800302F0 (-SetScalarValue@CKeyframeAnimation@@UEAAJM@Z.c)
 *     ?Calculate@Sequence@Animations@Components@@QEAAXXZ @ 0x1800905A0 (-Calculate@Sequence@Animations@Components@@QEAAXXZ.c)
 *     ?Stop@Sequence@Animations@Components@@QEAAXXZ @ 0x180090FF0 (-Stop@Sequence@Animations@Components@@QEAAXXZ.c)
 *     ?ResetTime@Sequence@Animations@Components@@QEAAXI_N@Z @ 0x180091070 (-ResetTime@Sequence@Animations@Components@@QEAAXI_N@Z.c)
 *     _guard_dispatch_icall_nop @ 0x1800D53F0 (_guard_dispatch_icall_nop.c)
 *     ?GetQuaternion@Value@Animations@Components@@QEBA?AUQuaternion@23@XZ @ 0x1801B9744 (-GetQuaternion@Value@Animations@Components@@QEBA-AUQuaternion@23@XZ.c)
 */

__int64 __fastcall Components::Animations::Animation::ApiReset(Components::Animations::Animation *this, char a2)
{
  int v3; // esi
  __int64 (__fastcall ***v4)(CKeyframeAnimation *__hidden, float); // rbx
  __int64 (__fastcall *v5)(CKeyframeAnimation *__hidden, float); // rax
  CKeyframeAnimation *v6; // rcx
  float v7; // xmm1_4
  int v8; // eax
  bool v9; // sf
  Components::Animations::Sequence *v10; // rcx
  __int64 v11; // rcx
  void (__fastcall *v12)(__int64, __int64); // rax
  Components::Animations::AnimationManager *v14; // rbx
  __int64 (__fastcall *v15)(CKeyframeAnimation *__hidden, float); // rax
  int v16; // eax
  unsigned int v17; // edx
  __int64 v18; // rcx
  Components::Animations::Sequence *v19; // rcx
  float TimeLength; // xmm0_4
  int v21; // edx
  int v22; // edx
  unsigned int v23; // r9d
  int v24; // [rsp+30h] [rbp-10h] BYREF
  Components::Animations::Animation *v25; // [rsp+50h] [rbp+10h] BYREF

  if ( (*((_BYTE *)this + 72) & 4) == 0 )
  {
LABEL_16:
    v3 = 0;
    goto LABEL_17;
  }
  v3 = 0;
  if ( !a2 )
    goto LABEL_11;
  if ( *((_DWORD *)this + 13) == 1 )
  {
    *((_DWORD *)this + 5) = *((_DWORD *)this + 4);
    Components::Animations::Animation::PrepareSequenceForIteration(this, 0);
    v19 = *(Components::Animations::Sequence **)(v18 + 8);
LABEL_34:
    Components::Animations::Sequence::ResetTime(v19, v17, 1);
    goto LABEL_5;
  }
  if ( *((_DWORD *)this + 13) == 2 )
  {
    *((_DWORD *)this + 5) = *((_DWORD *)this + 4);
    TimeLength = Components::Animations::Animation::GetTimeLength(this);
    Components::Animations::Math::Round(TimeLength * 1000.0);
    v21 = *((_DWORD *)this + 8);
    if ( v21 >= 1 )
      v22 = v21 - 1;
    else
      v22 = 0;
    Components::Animations::Animation::PrepareSequenceForIteration(this, v22);
    v19 = (Components::Animations::Sequence *)*((_QWORD *)this + 1);
    v17 = v23;
    goto LABEL_34;
  }
LABEL_5:
  Components::Animations::Sequence::Calculate(*((Components::Animations::Sequence **)this + 1));
  v4 = (__int64 (__fastcall ***)(CKeyframeAnimation *__hidden, float))*((_QWORD *)this + 8);
  if ( !v4 )
    goto LABEL_11;
  if ( *((_DWORD *)this + 12) == 18 )
  {
    v5 = **v4;
    v6 = (CKeyframeAnimation *)*((_QWORD *)this + 8);
    if ( v5 == CKeyframeAnimation::SetScalarValue )
    {
      v7 = **(double **)(*((_QWORD *)this + 1) + 232LL);
      v8 = CKeyframeAnimation::SetScalarValue(v6, v7);
    }
    else
    {
      v8 = ((__int64 (__fastcall *)(CKeyframeAnimation *))v5)(v6);
    }
    v3 = v8;
    v9 = v8 < 0;
  }
  else
  {
    if ( *((_DWORD *)this + 12) == 35 )
    {
      v16 = ((__int64 (__fastcall *)(_QWORD))(*v4)[1])(*((_QWORD *)this + 8));
    }
    else if ( *((_DWORD *)this + 12) == 52 )
    {
      v16 = ((__int64 (__fastcall *)(_QWORD))(*v4)[2])(*((_QWORD *)this + 8));
    }
    else
    {
      switch ( *((_DWORD *)this + 12) )
      {
        case 'E':
          Components::Animations::Value::GetColor(*(_QWORD *)(*((_QWORD *)this + 1) + 232LL), (__int64)&v24);
          v15 = (*v4)[3];
          break;
        case 'F':
          Components::Animations::Value::GetColor(*(_QWORD *)(*((_QWORD *)this + 1) + 232LL), (__int64)&v24);
          v15 = (*v4)[4];
          break;
        case 'G':
          Components::Animations::Value::GetQuaternion(*(_QWORD *)(*((_QWORD *)this + 1) + 232LL), &v24);
          v15 = (*v4)[5];
          break;
        default:
          goto LABEL_11;
      }
      v16 = ((__int64 (__fastcall *)(__int64 (__fastcall ***)(CKeyframeAnimation *__hidden, float)))v15)(v4);
    }
    v3 = v16;
    v9 = v16 < 0;
  }
  if ( v9 )
    goto LABEL_17;
LABEL_11:
  if ( (*((_BYTE *)this + 72) & 1) != 0 )
  {
    v14 = Components::Animations::AnimationManager::s_PORT_pSingleton;
    v25 = this;
    Components::Animations::ArrayListF<Components::Animations::Animation *>::Add(
      (__int64)Components::Animations::AnimationManager::s_PORT_pSingleton + 80,
      &v25);
    *((_BYTE *)v14 + 176) |= 4u;
    *((_BYTE *)this + 72) &= ~1u;
  }
  v10 = (Components::Animations::Sequence *)*((_QWORD *)this + 1);
  if ( (*((_BYTE *)v10 + 264) & 1) != 0 )
    Components::Animations::Sequence::Stop(v10);
  if ( v3 >= 0 )
    goto LABEL_16;
LABEL_17:
  *((_BYTE *)this + 72) &= 0x89u;
  *((_DWORD *)this + 5) = 0;
  if ( v3 >= 0 )
  {
    v11 = *((_QWORD *)this + 8);
    v12 = *(void (__fastcall **)(__int64, __int64))(*(_QWORD *)v11 + 64LL);
    if ( (char *)v12 == (char *)CKeyframeAnimation::OnAnimationEvent )
      CKeyframeAnimation::OnAnimationEvent(v11, 2LL);
    else
      v12(v11, 2LL);
  }
  return (unsigned int)v3;
}
