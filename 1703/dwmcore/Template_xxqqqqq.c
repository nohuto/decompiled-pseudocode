/*
 * XREFs of Template_xxqqqqq @ 0x1801760E8
 * Callers:
 *     ?RecordIndependentFlipInfo@CCompositionSurfaceInfo@@QEAAJ_KI_N1IIPEA_N@Z @ 0x180175AD0 (-RecordIndependentFlipInfo@CCompositionSurfaceInfo@@QEAAJ_KI_N1IIPEA_N@Z.c)
 * Callees:
 *     __security_check_cookie @ 0x1800D2E60 (__security_check_cookie.c)
 */

ULONG Template_xxqqqqq(__int64 a1, __int64 a2, ...)
{
  struct _EVENT_DATA_DESCRIPTOR UserData; // [rsp+20h] [rbp-61h] BYREF
  va_list v4; // [rsp+30h] [rbp-51h]
  __int64 v5; // [rsp+38h] [rbp-49h]
  va_list v6; // [rsp+40h] [rbp-41h]
  __int64 v7; // [rsp+48h] [rbp-39h]
  va_list v8; // [rsp+50h] [rbp-31h]
  __int64 v9; // [rsp+58h] [rbp-29h]
  va_list v10; // [rsp+60h] [rbp-21h]
  __int64 v11; // [rsp+68h] [rbp-19h]
  va_list v12; // [rsp+70h] [rbp-11h]
  __int64 v13; // [rsp+78h] [rbp-9h]
  va_list v14; // [rsp+80h] [rbp-1h]
  __int64 v15; // [rsp+88h] [rbp+7h]
  __int64 v16; // [rsp+D0h] [rbp+4Fh] BYREF
  va_list va; // [rsp+D0h] [rbp+4Fh]
  __int64 v18; // [rsp+D8h] [rbp+57h] BYREF
  va_list va1; // [rsp+D8h] [rbp+57h]
  __int64 v20; // [rsp+E0h] [rbp+5Fh] BYREF
  va_list va2; // [rsp+E0h] [rbp+5Fh]
  __int64 v22; // [rsp+E8h] [rbp+67h] BYREF
  va_list va3; // [rsp+E8h] [rbp+67h]
  __int64 v24; // [rsp+F0h] [rbp+6Fh] BYREF
  va_list va4; // [rsp+F0h] [rbp+6Fh]
  __int64 v26; // [rsp+F8h] [rbp+77h] BYREF
  va_list va5; // [rsp+F8h] [rbp+77h]
  va_list va6; // [rsp+100h] [rbp+7Fh] BYREF

  va_start(va6, a2);
  va_start(va5, a2);
  va_start(va4, a2);
  va_start(va3, a2);
  va_start(va2, a2);
  va_start(va1, a2);
  va_start(va, a2);
  v16 = va_arg(va1, _QWORD);
  va_copy(va2, va1);
  v18 = va_arg(va2, _QWORD);
  va_copy(va3, va2);
  v20 = va_arg(va3, _QWORD);
  va_copy(va4, va3);
  v22 = va_arg(va4, _QWORD);
  va_copy(va5, va4);
  v24 = va_arg(va5, _QWORD);
  va_copy(va6, va5);
  v26 = va_arg(va6, _QWORD);
  va_copy((va_list)UserData.Ptr, va);
  *(_QWORD *)&UserData.Size = 8LL;
  va_copy(v4, va1);
  v5 = 8LL;
  va_copy(v6, va2);
  v7 = 4LL;
  va_copy(v8, va3);
  va_copy(v10, va4);
  va_copy(v12, va5);
  va_copy(v14, va6);
  v9 = 4LL;
  v11 = 4LL;
  v13 = 4LL;
  v15 = 4LL;
  return EventWrite(Microsoft_Windows_Dwm_CoreHandle, &EVTDESC_ETWGUID_SETINDEPENDENTFLIPINFO, 7u, &UserData);
}
