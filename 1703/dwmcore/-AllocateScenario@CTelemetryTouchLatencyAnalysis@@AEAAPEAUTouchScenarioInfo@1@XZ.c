/*
 * XREFs of ?AllocateScenario@CTelemetryTouchLatencyAnalysis@@AEAAPEAUTouchScenarioInfo@1@XZ @ 0x180027F3C
 * Callers:
 *     ?BeginAnalyzingInteraction@CTelemetryTouchLatencyAnalysis@@QEAAXPEBUTOUCH_TELEMETRY_BEGIN_PACKED_INFO@@_KPEBG1@Z @ 0x180028730 (-BeginAnalyzingInteraction@CTelemetryTouchLatencyAnalysis@@QEAAXPEBUTOUCH_TELEMETRY_BEGIN_PACKED.c)
 * Callees:
 *     ?RetireScenario@CTelemetryTouchLatencyAnalysis@@AEAAXPEAUTouchScenarioInfo@1@_N1@Z @ 0x180027FD4 (-RetireScenario@CTelemetryTouchLatencyAnalysis@@AEAAXPEAUTouchScenarioInfo@1@_N1@Z.c)
 */

LARGE_INTEGER *__fastcall CTelemetryTouchLatencyAnalysis::AllocateScenario(CTelemetryTouchLatencyAnalysis *this)
{
  LARGE_INTEGER *v2; // rsi
  unsigned int v3; // r8d
  unsigned __int64 *v4; // rdx
  LARGE_INTEGER *v5; // rbx
  LARGE_INTEGER v7; // rax
  unsigned __int64 v8; // rcx

  v2 = (LARGE_INTEGER *)((char *)this + 8);
  v3 = 0;
  v4 = (unsigned __int64 *)((char *)this + 64);
  while ( *((_DWORD *)v4 - 5) )
  {
    v7 = v2[7];
    if ( !v7.QuadPart )
      v7 = v2[8];
    v8 = *v4;
    if ( !*v4 )
      v8 = v4[1];
    if ( v7.QuadPart > v8 )
      v2 = (LARGE_INTEGER *)((char *)this + 200 * v3 + 8);
    ++v3;
    v4 += 25;
    if ( v3 >= 0xA )
      goto LABEL_12;
  }
  v5 = (LARGE_INTEGER *)((char *)this + 200 * v3 + 8);
  if ( v5 )
    goto LABEL_4;
LABEL_12:
  CTelemetryTouchLatencyAnalysis::RetireScenario(
    this,
    (struct CTelemetryTouchLatencyAnalysis::TouchScenarioInfo *)v2,
    1,
    0);
  v5 = v2;
LABEL_4:
  v5[4].HighPart = 1;
  ++*(_DWORD *)this;
  QueryPerformanceCounter(v5 + 22);
  return v5;
}
