/*
 * XREFs of ?SubmitSync@FxIoTarget@@QEAAJPEAVFxRequestBase@@PEAU_WDF_REQUEST_SEND_OPTIONS@@PEAK@Z @ 0x1C0029E20
 * Callers:
        ...
        unsigned int *Action)
{
  ...
  unsigned __int8 v113; // [rsp+45h] [rbp-44h]
  FxTagTracker *v114; // [rsp+48h] [rbp-41h]
  ...
  unsigned __int8 v118; // [rsp+F0h] [rbp+67h]
  unsigned __int8 v119; // [rsp+F8h] [rbp+6Fh]
  unsigned __int8 PreviousIrql; // [rsp+100h] [rbp+77h] BYREF
  unsigned int *v121; // [rsp+108h] [rbp+7Fh]
  ...
  v10 = (Request->m_RequestBaseFlags & 0x10) != 0;
  if ( Action )
    v11 = *Action;
  else
    v11 = 0;
    ...
    else
    {
      ...
      if ( (char *)SelfDestruct == (char *)FxRequestFromLookaside::SelfDestruct )
      {
        ...
        else
        {
          ...
          if ( v66 && *(_WORD *)(v66 + 8) == 4354 && *(_BYTE *)(v66 + 1667) )
          {
        ...
        if ( v45 == 1 )
        {
          if ( Request->m_Completed )
          {
            ...
            if ( m_CompletionState == 1 )
              FxIoQueue::PostRequestCompletedCallback(m_IoQueue, v110);
          }
        }
      }
    }
