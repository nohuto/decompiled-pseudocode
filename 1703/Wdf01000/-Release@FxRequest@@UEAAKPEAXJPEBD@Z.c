/*
 * XREFs of ?Release@FxRequest@@UEAAKPEAXJPEBD@Z @ 0x1C000ABA0
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall FxRequest::Release(FxRequest *this, FxObjectDebugLeakDetection *Tag, __int64 Line, const char *File)
{
  ...
  unsigned __int8 v54; // [rsp+70h] [rbp+8h]
    ...
    else
    {
      ...
      if ( (char *)SelfDestruct == (char *)FxRequestFromLookaside::SelfDestruct )
      {
        ...
        else
        {
          ...
          if ( v27 && *(_WORD *)(v27 + 8) == 4354 && *(_BYTE *)(v27 + 1667) )
          {
        ...
        if ( this->m_Completed )
        {
          ...
          if ( m_CompletionState == 1 )
          {
            ...
            if ( m_IoQueue->m_Dispatching )
              FxNonPagedObject::Unlock(m_IoQueue, PreviousIrql[0]);
            else
              FxIoQueue::DispatchEvents(m_IoQueue, PreviousIrql[0], 0LL);
          }
        }
      }
    }
