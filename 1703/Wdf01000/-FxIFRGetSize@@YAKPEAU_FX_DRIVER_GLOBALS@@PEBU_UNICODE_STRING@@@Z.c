/*
 * XREFs of ?FxIFRGetSize@@YAKPEAU_FX_DRIVER_GLOBALS@@PEBU_UNICODE_STRING@@@Z @ 0x1C001AC5C
 * Callers:
...
__int64 __fastcall FxIFRGetSize(_FX_DRIVER_GLOBALS *FxDriverGlobals, _UNICODE_STRING *RegistryPath)
{
  ...
  wchar_t valueName_buffer[12]; // [rsp+98h] [rbp-11h] BYREF
  wchar_t parametersPath_buffer[16]; // [rsp+B0h] [rbp+7h] BYREF
  _BYTE KeyValueInformation[4]; // [rsp+D0h] [rbp+27h] BYREF
  int v16; // [rsp+D4h] [rbp+2Bh]
  __int64 v17; // [rsp+DCh] [rbp+33h]
    ...
    *(_OWORD *)&oa.SecurityDescriptor = 0LL;
    if ( ZwOpenKey(&parameters.m_Key, 0x20019u, &oa) >= 0 )
    {
      ...
      v4 = ZwQueryValueKey(
             ...
             0x10u,
             &ResultLength);
      if ( ((int)(v4 + 0x80000000) < 0 || v4 == -2147483643) && v16 != 4 )
        v4 = -1073741788;
      if ( v4 >= 0 )
      {
        v3 = v17;
