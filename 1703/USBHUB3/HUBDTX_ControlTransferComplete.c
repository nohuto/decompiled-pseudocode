/*
 * XREFs of HUBDTX_ControlTransferComplete @ 0x1C0021A00
 * Callers:
 *     <none>
 * Callees:
 *     WPP_RECORDER_SF_d @ 0x1C0001950 (WPP_RECORDER_SF_d.c)
 *     Template_pqqq @ 0x1C0001CB4 (Template_pqqq.c)
 *     WPP_RECORDER_SF_dD @ 0x1C0001E10 (WPP_RECORDER_SF_dD.c)
 *     WPP_RECORDER_SF_dDDD @ 0x1C0002738 (WPP_RECORDER_SF_dDDD.c)
 *     HUBMISC_VerifierDbgBreak @ 0x1C0029B74 (HUBMISC_VerifierDbgBreak.c)
 *     _guard_dispatch_icall_nop @ 0x1C0038110 (_guard_dispatch_icall_nop.c)
 */

void __fastcall HUBDTX_ControlTransferComplete(__int64 a1, __int64 a2, __int64 a3, __int64 a4)
{
  int v4; // edi
  int v5; // esi
  __int64 v7; // r15
  _BYTE *v8; // r14
  int v9; // ebp
  int v10; // eax
  const EVENT_DESCRIPTOR *v11; // rdx
  char v12; // al
  unsigned __int16 v13; // r9
  char v14; // al
  char v15; // al
  char v16; // al
  unsigned __int8 v17; // al
  char v18; // dl
  char v19; // al
  __int64 v20; // rax
  int v21; // edi
  __int64 v22; // rax
  __int64 v23; // [rsp+20h] [rbp-68h]
  __int64 v24; // [rsp+28h] [rbp-60h]
  __int64 v25; // [rsp+30h] [rbp-58h]
  unsigned int v26; // [rsp+38h] [rbp-50h]
  _QWORD v27[4]; // [rsp+50h] [rbp-38h] BYREF

  v4 = *(_DWORD *)(a3 + 8);
  v5 = *(_DWORD *)(a4 + 260);
  v7 = a1;
  v8 = *(_BYTE **)(a4 + 2616);
  *(_DWORD *)(a4 + 240) = *(_DWORD *)(a4 + 292);
  *(_DWORD *)(a4 + 392) = 0;
  if ( v4 >= 0 )
  {
    v9 = 4012;
    goto LABEL_103;
  }
  v9 = 4004;
  if ( *(_BYTE *)(a4 + 385) == 9 && *(_WORD *)(a4 + 386) )
  {
    *(_DWORD *)(a4 + 1544) = -1073741823;
    *(_DWORD *)(a4 + 1548) = -1073733632;
  }
  else
  {
    *(_DWORD *)(a4 + 1544) = v4;
    *(_DWORD *)(a4 + 1548) = v5;
  }
  LOBYTE(a1) = *(_BYTE *)(a4 + 385);
  if ( (_BYTE)a1 == 48 && (*(_BYTE *)(a4 + 384) & 0x60) == 0 )
  {
    WPP_RECORDER_SF_dD(
      *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
      2u,
      5u,
      0xAu,
      (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
      v4,
      v5);
    if ( (*(_DWORD *)(a4 + 2420) & 4) != 0 )
      HUBMISC_VerifierDbgBreak("DeviceHwVerifierSetSelFailure", a4 + 488);
    *(_DWORD *)(a4 + 2416) = 1073807370;
    if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) != 0 )
    {
      LODWORD(v25) = v4;
      LODWORD(v24) = v5;
      LODWORD(v23) = 0;
      Template_pqqq(
        a1,
        &USBHUB3_ETW_EVENT_SET_SEL_FAILURE,
        (const GUID *)(a4 + 1500),
        *(_QWORD *)(a4 + 24),
        v23,
        v24,
        v25);
    }
    v10 = 4008;
    if ( v5 != -1073741820 )
      v10 = 4004;
    v9 = v10;
    goto LABEL_103;
  }
  if ( (_BYTE)a1 != 3 )
    goto LABEL_26;
  if ( *(_WORD *)(a4 + 386) != 48 )
  {
    if ( *(_WORD *)(a4 + 386) == 49 )
    {
      WPP_RECORDER_SF_dD(
        *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
        2u,
        5u,
        0xCu,
        (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
        v4,
        v5);
      if ( (*(_DWORD *)(a4 + 2420) & 0x4000) != 0 )
        HUBMISC_VerifierDbgBreak("DeviceHwVerifierSetU2EnableFailure", a4 + 488);
      if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) != 0 )
      {
        LODWORD(v25) = v4;
        v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_SET_U2_ENABLE_FAILURE;
        LODWORD(v24) = v5;
        LODWORD(v23) = 0;
        goto LABEL_101;
      }
      goto LABEL_103;
    }
LABEL_26:
    if ( (_BYTE)a1 == 6 && (*(_BYTE *)(a4 + 384) & 0x60) == 0 )
    {
      v12 = *(_BYTE *)(a4 + 387);
      if ( (v12 != 3 || *(_BYTE *)(a4 + 386) != 0xEE) && v12 != 6 && (*(_DWORD *)(a4 + 2420) & 1) != 0 )
        HUBMISC_VerifierDbgBreak("DeviceHwVerifierControlTransferFailure", a4 + 488);
      if ( *(_BYTE *)(a4 + 387) == 1 )
      {
        *(_DWORD *)(a4 + 2416) = 1073807360;
        WPP_RECORDER_SF_dD(
          *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
          2u,
          5u,
          0xDu,
          (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
          v4,
          v5);
        if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) == 0 )
          goto LABEL_103;
        LODWORD(v25) = v4;
        v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_DEVICE_DESCRIPTOR_FAILURE;
        LODWORD(v24) = v5;
        LODWORD(v23) = 0;
      }
      else if ( *(_BYTE *)(a4 + 387) == 2 )
      {
        *(_DWORD *)(a4 + 2416) = 1073807367;
        WPP_RECORDER_SF_dD(
          *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
          2u,
          5u,
          0xEu,
          (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
          v4,
          v5);
        if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) == 0 )
          goto LABEL_103;
        LODWORD(v25) = v4;
        v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_CONFIG_DESCRIPTOR_FAILURE;
        LODWORD(v24) = v5;
        LODWORD(v23) = 0;
      }
      else
      {
        a1 = (unsigned int)*(unsigned __int8 *)(a4 + 387) - 3;
        switch ( *(_BYTE *)(a4 + 387) )
        {
          case 3:
            v14 = *(_BYTE *)(a4 + 1988);
            if ( v14 && v14 == *(_BYTE *)(a4 + 386) )
            {
              *(_DWORD *)(a4 + 2416) = 1073807373;
              WPP_RECORDER_SF_dD(
                *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
                2u,
                5u,
                0x11u,
                (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
                v4,
                v5);
              if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) == 0 )
                goto LABEL_103;
              LODWORD(v25) = v4;
              v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_SERIAL_NUMBER_STRING_FAILURE;
              LODWORD(v24) = v5;
              LODWORD(v23) = 0;
            }
            else
            {
              LOBYTE(a1) = *(_BYTE *)(a4 + 386);
              if ( (_BYTE)a1 )
              {
                v15 = *(_BYTE *)(a4 + 1987);
                if ( v15 && v15 == (_BYTE)a1 )
                {
                  *(_DWORD *)(a4 + 2416) = 1073807375;
                  WPP_RECORDER_SF_dD(
                    *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
                    2u,
                    5u,
                    0x13u,
                    (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
                    v4,
                    v5);
                  if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) == 0 )
                    goto LABEL_103;
                  LODWORD(v25) = v4;
                  v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_PRODUCT_ID_STRING_FAILURE;
                  LODWORD(v24) = v5;
                  LODWORD(v23) = 0;
                }
                else
                {
                  if ( !v8 )
                    goto LABEL_103;
                  v16 = *(_BYTE *)(*(_QWORD *)v8 + 3LL);
                  if ( v16 && v16 == (_BYTE)a1 )
                  {
                    *(_DWORD *)(a4 + 2416) = 1073807375;
                    WPP_RECORDER_SF_dD(
                      *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
                      2u,
                      5u,
                      0x14u,
                      (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
                      v4,
                      v5);
                    if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x200) == 0 )
                      goto LABEL_103;
                    LODWORD(v25) = v4;
                    v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_BILLBOARD_STRING_FAILURE;
                    LODWORD(v24) = v5;
                    LODWORD(v23) = 0;
                  }
                  else
                  {
                    v17 = v8[8];
                    if ( v17 == 0xFF )
                      goto LABEL_103;
                    v18 = *(_BYTE *)(*(_QWORD *)v8 + 4LL * v17 + 47);
                    if ( !v18 )
                      goto LABEL_103;
                    if ( v18 != (_BYTE)a1 )
                      goto LABEL_103;
                    *(_DWORD *)(a4 + 2416) = 1073807375;
                    WPP_RECORDER_SF_dD(
                      *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
                      2u,
                      5u,
                      0x15u,
                      (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
                      v4,
                      v5);
                    if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x200) == 0 )
                      goto LABEL_103;
                    LODWORD(v25) = v4;
                    v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_ALTERNATE_MODE_STRING_FAILURE;
                    LODWORD(v24) = v5;
                    LODWORD(v23) = 0;
                  }
                }
              }
              else
              {
                *(_DWORD *)(a4 + 2416) = 1073807374;
                WPP_RECORDER_SF_dD(
                  *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
                  2u,
                  5u,
                  0x12u,
                  (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
                  v4,
                  v5);
                if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) == 0 )
                  goto LABEL_103;
                LODWORD(v25) = v4;
                v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_LANGUAGE_ID_STRING_FAILURE;
                LODWORD(v24) = v5;
                LODWORD(v23) = 0;
              }
            }
            break;
          case 6:
            *(_DWORD *)(a4 + 2416) = 1073807372;
            WPP_RECORDER_SF_dD(
              *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
              2u,
              5u,
              0x10u,
              (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
              v4,
              v5);
            if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) == 0 )
              goto LABEL_103;
            LODWORD(v25) = v4;
            v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_DEVICE_QUALIFIER_DESCRIPTOR_FAILURE;
            LODWORD(v24) = v5;
            LODWORD(v23) = 0;
            break;
          case 0xF:
            *(_DWORD *)(a4 + 2416) = 1073807371;
            WPP_RECORDER_SF_dD(
              *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
              2u,
              5u,
              0xFu,
              (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
              v4,
              v5);
            if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) == 0 )
              goto LABEL_103;
            LODWORD(v25) = v4;
            v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_BOS_DESCRIPTOR_FAILURE;
            LODWORD(v24) = v5;
            LODWORD(v23) = 0;
            break;
          default:
            v13 = 22;
LABEL_39:
            WPP_RECORDER_SF_dD(
              *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
              2u,
              5u,
              v13,
              (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
              v4,
              v5);
            goto LABEL_103;
        }
      }
      goto LABEL_101;
    }
    v19 = *(_BYTE *)(a4 + 384) & 0x60;
    if ( v19 == 64 && (_BYTE)a1 == *(_BYTE *)(a4 + 2036) )
    {
      switch ( *(_WORD *)(a4 + 388) )
      {
        case 4:
          *(_DWORD *)(a4 + 2416) = 1073807376;
          WPP_RECORDER_SF_dD(
            *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
            2u,
            5u,
            0x17u,
            (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
            v4,
            v5);
          if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) == 0 )
            goto LABEL_103;
          LODWORD(v25) = v4;
          v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_MS_EXT_CONFIG_DESCRIPTOR_FAILURE;
          LODWORD(v24) = v5;
          LODWORD(v23) = 0;
          break;
        case 6:
          *(_DWORD *)(a4 + 2416) = 1073807377;
          WPP_RECORDER_SF_dD(
            *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
            2u,
            5u,
            0x18u,
            (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
            v4,
            v5);
          if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) == 0 )
            goto LABEL_103;
          LODWORD(v25) = v4;
          v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_MS_CONTAINER_ID_DESCRIPTOR_FAILURE;
          LODWORD(v24) = v5;
          LODWORD(v23) = 0;
          break;
        case 7:
          *(_DWORD *)(a4 + 2416) = 1073807388;
          WPP_RECORDER_SF_dD(
            *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
            2u,
            5u,
            0x19u,
            (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
            v4,
            v5);
          if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) == 0 )
            goto LABEL_103;
          LODWORD(v25) = v4;
          v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_MSOS20_DESCRIPTOR_FAILURE;
          LODWORD(v24) = v5;
          LODWORD(v23) = 0;
          break;
        case 8:
          *(_DWORD *)(a4 + 2416) = 1073807390;
          WPP_RECORDER_SF_dD(
            *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
            2u,
            5u,
            0x1Au,
            (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
            v4,
            v5);
          if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x200) == 0 )
            goto LABEL_103;
          LODWORD(v25) = v4;
          v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_MSOS20_SET_ALT_ENUMERATION_FAILURE;
          LODWORD(v24) = v5;
          LODWORD(v23) = 0;
          break;
        default:
          v13 = 27;
          goto LABEL_39;
      }
LABEL_101:
      Template_pqqq(a1, v11, (const GUID *)(a4 + 1500), *(_QWORD *)(a4 + 24), v23, v24, v25);
      goto LABEL_103;
    }
    if ( (_BYTE)a1 == 49 )
    {
      if ( !v19 )
      {
        WPP_RECORDER_SF_dD(
          *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
          2u,
          5u,
          0x1Cu,
          (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
          v4,
          v5);
        if ( (*(_DWORD *)(a4 + 2420) & 8) != 0 )
          HUBMISC_VerifierDbgBreak("DeviceHwVerifierSetIsochDelayFailure", a4 + 488);
        if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) == 0 )
          goto LABEL_103;
        v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_SET_ISOCH_DELAY_FAILURE;
        goto LABEL_100;
      }
    }
    else if ( !v19
           && ((_BYTE)a1 == 1 && *(_WORD *)(a4 + 386) == 1
            || (_BYTE)a1 == 3 && !*(_WORD *)(a4 + 386) && !*(_WORD *)(a4 + 388)) )
    {
      goto LABEL_103;
    }
    v26 = (*(unsigned __int8 *)(a4 + 384) >> 5) & 3;
    WPP_RECORDER_SF_dDDD(
      *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
      v26,
      5u,
      0x1Du,
      (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
      v4,
      v5,
      v26,
      (unsigned __int8)a1);
    if ( (*(_DWORD *)(a4 + 2420) & 1) != 0 )
      HUBMISC_VerifierDbgBreak("DeviceHwVerifierControlTransferFailure", a4 + 488);
    if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x40000) == 0 )
      goto LABEL_103;
    v11 = &USBHUB3_ETW_EVENT_DEVICE_CONTROL_TRANSFER_ERROR;
LABEL_100:
    v20 = *(_QWORD *)(a4 + 8);
    LODWORD(v25) = v4;
    LODWORD(v24) = v5;
    a1 = *(unsigned __int16 *)(v20 + 200);
    LODWORD(v23) = *(unsigned __int16 *)(v20 + 200);
    goto LABEL_101;
  }
  WPP_RECORDER_SF_dD(
    *(_QWORD *)(*(_QWORD *)(a4 + 8) + 1432LL),
    2u,
    5u,
    0xBu,
    (__int64)&WPP_1456c6ef7b533877ee207863935bf626_Traceguids,
    v4,
    v5);
  if ( (*(_DWORD *)(a4 + 2420) & 0x2000) != 0 )
    HUBMISC_VerifierDbgBreak("DeviceHwVerifierSetU1EnableFailure", a4 + 488);
  if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x100) != 0 )
  {
    LODWORD(v25) = v4;
    v11 = (const EVENT_DESCRIPTOR *)&USBHUB3_ETW_EVENT_SET_U1_ENABLE_FAILURE;
    LODWORD(v24) = v5;
    LODWORD(v23) = 0;
    goto LABEL_101;
  }
LABEL_103:
  if ( v4 < 0 && ((__int64)WPP_MAIN_CB.Queue.Wcb.DmaWaitEntry.Blink & 0x40000) != 0 )
  {
    LODWORD(v25) = v4;
    LODWORD(v24) = v5;
    LODWORD(v23) = 0;
    Template_pqqq(a1, &USBHUB3_ETW_EVENT_DEVICE_CONTROL_TRANSFER_ERROR, 0LL, *(_QWORD *)(a4 + 24), v23, v24, v25);
  }
  v27[0] = 24LL;
  v27[2] = 0LL;
  v27[1] = 0LL;
  v21 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, _QWORD *))(WdfFunctions_01015 + 1992))(
          WdfDriverGlobals,
          v7,
          v27);
  if ( v21 < 0 )
  {
    v22 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, WDFDRIVER__ *, void *))(WdfFunctions_01015 + 1616))(
            WdfDriverGlobals,
            WdfDriverGlobals->Driver,
            off_1C005B110);
    LODWORD(v24) = v21;
    WPP_RECORDER_SF_d(
      *(_QWORD *)(v22 + 64),
      2u,
      2u,
      0x3Au,
      (__int64)&WPP_b6e50536a4fa3dade9fc9443e0bc8f19_Traceguids,
      v24);
  }
  HUBSM_AddEvent(a4 + 488, v9);
}
