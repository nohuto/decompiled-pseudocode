/*
 * XREFs of RtlpNameprepAsciiWorker @ 0x1408D5550
 * Callers:
 *     RtlIdnToAscii @ 0x1408D5460 (RtlIdnToAscii.c)
 *     RtlIdnToNameprepUnicode @ 0x1408D5490 (RtlIdnToNameprepUnicode.c)
 * Callees:
 *     IdnaMemAlloc @ 0x140312FC0 (IdnaMemAlloc.c)
 *     RtlpNameprepAsciiRealWorker @ 0x1403132D4 (RtlpNameprepAsciiRealWorker.c)
 *     ExFreePoolWithTag @ 0x14036E0A0 (ExFreePoolWithTag.c)
 */

__int64 __fastcall RtlpNameprepAsciiWorker(int a1, const WCHAR *a2, unsigned int a3, void *a4, LONG *a5, char a6)
{
  WCHAR *v10; // rdi
  PVOID v11; // rax
  void *v12; // rbx
  unsigned int v13; // esi
  char v15; // [rsp+38h] [rbp-30h]
  char v16; // [rsp+48h] [rbp-20h]

  v10 = (WCHAR *)IdnaMemAlloc(0x3FEuLL);
  v11 = IdnaMemAlloc(0x406uLL);
  v12 = v11;
  if ( v10 && v11 )
    v13 = RtlpNameprepAsciiRealWorker(a1, a2, a3, a4, a5, a6, v10, v15, v11, v16);
  else
    v13 = -1073741801;
  if ( v10 )
    ExFreePoolWithTag(v10, 0);
  if ( v12 )
    ExFreePoolWithTag(v12, 0);
  return v13;
}
