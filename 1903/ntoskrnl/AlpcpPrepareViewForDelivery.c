/*
 * XREFs of AlpcpPrepareViewForDelivery @ 0x14064AAAC
 * Callers:
 *     AlpcpCaptureViewAttributeInternal @ 0x14064A968 (AlpcpCaptureViewAttributeInternal.c)
 * Callees:
 *     KiUnstackDetachProcess @ 0x140090690 (KiUnstackDetachProcess.c)
 *     KiStackAttachProcess @ 0x140090AF0 (KiStackAttachProcess.c)
 *     __security_check_cookie @ 0x14019E700 (__security_check_cookie.c)
 *     memset @ 0x1401D6BC0 (memset.c)
 *     AlpcpUnlockBlob @ 0x1405E4790 (AlpcpUnlockBlob.c)
 *     MmUnsecureVirtualMemory @ 0x14060AA10 (MmUnsecureVirtualMemory.c)
 *     AlpcpLockForCachedReferenceBlob @ 0x14064A878 (AlpcpLockForCachedReferenceBlob.c)
 *     AlpcpDereferenceBlobEx @ 0x14064B150 (AlpcpDereferenceBlobEx.c)
 *     AlpcpReferenceBlob @ 0x14064B920 (AlpcpReferenceBlob.c)
 *     MmSecureVirtualMemoryAgainstWrites @ 0x14064BDA8 (MmSecureVirtualMemoryAgainstWrites.c)
 */

__int64 __fastcall AlpcpPrepareViewForDelivery(ULONG_PTR BugCheckParameter2, char a2, unsigned __int8 a3)
{
  ULONG_PTR v6; // rdi
  ULONG_PTR v7; // rsi
  int v8; // eax
  unsigned int v9; // ecx
  unsigned int v10; // ebp
  _QWORD *v12; // r14
  int v13; // eax
  __int64 v14; // rax
  _BYTE v15[48]; // [rsp+20h] [rbp-58h] BYREF

  memset(v15, 0, sizeof(v15));
  v6 = *(_QWORD *)(BugCheckParameter2 + 16);
  v7 = 0LL;
  AlpcpLockForCachedReferenceBlob(v6);
  v8 = *(_DWORD *)(v6 + 48);
  if ( (v8 & 1) == 0 )
  {
    if ( !a2 )
      goto LABEL_7;
    if ( *(_DWORD *)(BugCheckParameter2 + 76) )
      goto LABEL_7;
    v9 = *(_DWORD *)(v6 + 52);
    if ( v9 > 2 )
      goto LABEL_7;
    if ( v9 != 1 )
    {
      v12 = *(_QWORD **)(v6 + 56);
      if ( v12 == (_QWORD *)BugCheckParameter2 )
        v12 = (_QWORD *)*v12;
      v13 = *(_DWORD *)(BugCheckParameter2 + 72);
      if ( (v13 & 8) == 0 )
      {
        v14 = MmSecureVirtualMemoryAgainstWrites(*(_QWORD *)(BugCheckParameter2 + 32));
        if ( !v14 )
          goto LABEL_7;
        *(_QWORD *)(BugCheckParameter2 + 64) = v14;
        v13 = *(_DWORD *)(BugCheckParameter2 + 72);
      }
      *(_DWORD *)(BugCheckParameter2 + 72) = v13 & 0xFFFFFFFE;
      AlpcpReferenceBlob(BugCheckParameter2);
      *(_DWORD *)(v6 + 48) |= 1u;
      *(_QWORD *)(v6 + 72) = BugCheckParameter2;
      *(_QWORD *)(v6 + 80) = v12;
      goto LABEL_7;
    }
    *(_QWORD *)(v6 + 80) = BugCheckParameter2;
    *(_DWORD *)(v6 + 48) = v8 | 1;
LABEL_7:
    ++*(_DWORD *)(BugCheckParameter2 + 76);
    v10 = 0;
    goto LABEL_8;
  }
  if ( BugCheckParameter2 == *(_QWORD *)(v6 + 80) )
  {
    if ( !a2 )
    {
      v7 = *(_QWORD *)(v6 + 72);
      if ( v7 )
      {
        if ( *(_QWORD *)(v7 + 64) )
        {
          KiStackAttachProcess(*(_KPROCESS **)(v7 + 32), 0, (__int64)v15);
          MmUnsecureVirtualMemory(*(HANDLE *)(v7 + 64));
          KiUnstackDetachProcess((struct _KTHREAD *)v15, 0);
          *(_QWORD *)(v7 + 64) = 0LL;
        }
        *(_DWORD *)(v7 + 72) |= 1u;
        *(_QWORD *)(v6 + 72) = 0LL;
      }
      *(_QWORD *)(v6 + 80) = 0LL;
      *(_DWORD *)(v6 + 48) &= ~1u;
      goto LABEL_7;
    }
    if ( !*(_DWORD *)(BugCheckParameter2 + 76) )
      goto LABEL_7;
  }
  v10 = -1073741790;
LABEL_8:
  *(_DWORD *)(BugCheckParameter2 + 72) ^= (*(_DWORD *)(BugCheckParameter2 + 72) ^ (2 * a3)) & 2;
  AlpcpUnlockBlob(v6);
  if ( v7 )
    AlpcpDereferenceBlobEx(v7);
  return v10;
}
