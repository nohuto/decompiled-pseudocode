/*
 * XREFs of NtPowerInformation @ 0x14067C840
 * Callers:
 *     WheaLogInternalEvent @ 0x140199660 (WheaLogInternalEvent.c)
 *     PopRecordPepWorkorderBlackboxInformation @ 0x140305F08 (PopRecordPepWorkorderBlackboxInformation.c)
 *     PopRecordPoIrpBlackboxInformation @ 0x140306288 (PopRecordPoIrpBlackboxInformation.c)
 *     PopRecordPowerWatchdogBlackboxInformation @ 0x140306558 (PopRecordPowerWatchdogBlackboxInformation.c)
 *     PnpRecordBlackboxPnpEventWorkerInformation @ 0x14067F91C (PnpRecordBlackboxPnpEventWorkerInformation.c)
 *     PnpRecordBlackboxDeviceCompletionQueueInformation @ 0x14071A1D4 (PnpRecordBlackboxDeviceCompletionQueueInformation.c)
 *     PnpRecordBlackboxPnpEventInformation @ 0x14072E994 (PnpRecordBlackboxPnpEventInformation.c)
 *     PnpRecordBlackboxDelayedRemoveWorkerInformation @ 0x14087B860 (PnpRecordBlackboxDelayedRemoveWorkerInformation.c)
 *     TtmpCalloutArmWatchdog @ 0x1408BB6B4 (TtmpCalloutArmWatchdog.c)
 *     TtmpCalloutCreateWatchdog @ 0x1408BB78C (TtmpCalloutCreateWatchdog.c)
 *     TtmpCalloutDestroyWatchdog @ 0x1408BB7EC (TtmpCalloutDestroyWatchdog.c)
 *     TtmpCalloutWatchdogCallback @ 0x1408BB870 (TtmpCalloutWatchdogCallback.c)
 *     VslRegisterIumPowerCallbacks @ 0x140A22F50 (VslRegisterIumPowerCallbacks.c)
 * Callees:
 *     RtlStringCbLengthW @ 0x140001BA8 (RtlStringCbLengthW.c)
 *     PsIsCurrentThreadInServerSilo @ 0x140002880 (PsIsCurrentThreadInServerSilo.c)
 *     RtlUIntAdd @ 0x140003EA8 (RtlUIntAdd.c)
 *     PoClearPowerRequestInternal @ 0x1400EC820 (PoClearPowerRequestInternal.c)
 *     PoSetPowerRequestInternal @ 0x1400ECD64 (PoSetPowerRequestInternal.c)
 *     KeQueryActiveGroupCount @ 0x140106F80 (KeQueryActiveGroupCount.c)
 *     PopDisksRegisteredForIdle @ 0x14013B52C (PopDisksRegisteredForIdle.c)
 *     PoPowerOffMonitor @ 0x14015DED4 (PoPowerOffMonitor.c)
 *     PopDiagTraceSessionDisplayStateChange @ 0x14016A2B0 (PopDiagTraceSessionDisplayStateChange.c)
 *     PopEventCalloutDispatch @ 0x140181C8C (PopEventCalloutDispatch.c)
 *     ExVerifySuite @ 0x14018C1B0 (ExVerifySuite.c)
 *     PpmHeteroHgsBackupInit @ 0x14019CA08 (PpmHeteroHgsBackupInit.c)
 *     __security_check_cookie @ 0x14019E700 (__security_check_cookie.c)
 *     ZwUpdateWnfStateData @ 0x1401C39D0 (ZwUpdateWnfStateData.c)
 *     memmove @ 0x1401D6880 (memmove.c)
 *     memset @ 0x1401D6BC0 (memset.c)
 *     PpmClearExitLatencySamplingPercentage @ 0x1402EE38C (PpmClearExitLatencySamplingPercentage.c)
 *     PpmClearSimulatedIdle @ 0x1402EE3D4 (PpmClearSimulatedIdle.c)
 *     PpmSetExitLatencySamplingPercentage @ 0x1402F0080 (PpmSetExitLatencySamplingPercentage.c)
 *     PpmSetSimulatedIdle @ 0x1402F0158 (PpmSetSimulatedIdle.c)
 *     PopCapabilityCheck @ 0x1402F1894 (PopCapabilityCheck.c)
 *     PopFxIsDevicePotentialDripsConstraint @ 0x1402F3758 (PopFxIsDevicePotentialDripsConstraint.c)
 *     PpmClearSimulatedLoad @ 0x1402F6D3C (PpmClearSimulatedLoad.c)
 *     PpmSetSimulatedLoad @ 0x1402F7048 (PpmSetSimulatedLoad.c)
 *     PpmParkSetLpiCap @ 0x1402F8BB4 (PpmParkSetLpiCap.c)
 *     PopUpdatePowerButtonHoldState @ 0x140301890 (PopUpdatePowerButtonHoldState.c)
 *     ExGetWakeTimerList @ 0x14033B5D0 (ExGetWakeTimerList.c)
 *     ExAllocatePoolWithTag @ 0x14036E010 (ExAllocatePoolWithTag.c)
 *     ExFreePoolWithTag @ 0x14036E0A0 (ExFreePoolWithTag.c)
 *     PopReleasePolicyLock @ 0x140595780 (PopReleasePolicyLock.c)
 *     PopAcquirePolicyLock @ 0x1405958A8 (PopAcquirePolicyLock.c)
 *     PnpWinlogonExternalDmaNotification @ 0x1405B0A04 (PnpWinlogonExternalDmaNotification.c)
 *     PopSessionWinlogonNotification @ 0x1405B5070 (PopSessionWinlogonNotification.c)
 *     ProbeForWrite @ 0x1405F07E0 (ProbeForWrite.c)
 *     PspIsContextAdmin @ 0x14061A4B4 (PspIsContextAdmin.c)
 *     SeSinglePrivilegeCheck @ 0x140672D30 (SeSinglePrivilegeCheck.c)
 *     PopCurrentPowerState @ 0x14067E620 (PopCurrentPowerState.c)
 *     PopPowerInformationInternal @ 0x14067E674 (PopPowerInformationInternal.c)
 *     PopBlackBoxUpdate @ 0x14067F71C (PopBlackBoxUpdate.c)
 *     ExCheckFullProcessInformationAccess @ 0x140680204 (ExCheckFullProcessInformationAccess.c)
 *     PopPowerRequestCreateInfo @ 0x14069DD50 (PopPowerRequestCreateInfo.c)
 *     PopPowerRequestActionInfo @ 0x14069FA64 (PopPowerRequestActionInfo.c)
 *     SeIsAppContainerOrIdentifyLevelContext @ 0x14069FB50 (SeIsAppContainerOrIdentifyLevelContext.c)
 *     PopGetSettingNotificationName @ 0x1406A0760 (PopGetSettingNotificationName.c)
 *     PopSetPowerSettingValueAcDc @ 0x1406A09DC (PopSetPowerSettingValueAcDc.c)
 *     PopSetPowerSettingValue @ 0x1406A12E8 (PopSetPowerSettingValue.c)
 *     PopFilterCapabilities @ 0x1406A2194 (PopFilterCapabilities.c)
 *     TtmIsEnabled @ 0x1406A3134 (TtmIsEnabled.c)
 *     PopGetSettingValue @ 0x1406A3430 (PopGetSettingValue.c)
 *     PopGetSessionId @ 0x1406A39C0 (PopGetSessionId.c)
 *     PopEtEnergyTrackerQuery @ 0x1406C01EC (PopEtEnergyTrackerQuery.c)
 *     PopCapturePlatformRole @ 0x1406ECE64 (PopCapturePlatformRole.c)
 *     PopGetWakeSource @ 0x14071D570 (PopGetWakeSource.c)
 *     PopLoggingInformation @ 0x14071E104 (PopLoggingInformation.c)
 *     PopMonitorInvocation @ 0x14071EDBC (PopMonitorInvocation.c)
 *     PopSessionInputChange @ 0x140723C24 (PopSessionInputChange.c)
 *     PopUmpoSendLegacyEvent @ 0x14072531C (PopUmpoSendLegacyEvent.c)
 *     PopSuspendResumePdc @ 0x1407261E8 (PopSuspendResumePdc.c)
 *     PopDiagTraceServiceNotification @ 0x140727FA0 (PopDiagTraceServiceNotification.c)
 *     PopValidateServiceNotification @ 0x1407280D4 (PopValidateServiceNotification.c)
 *     PopDiagTraceAppPowerMessage @ 0x140728108 (PopDiagTraceAppPowerMessage.c)
 *     PopDiagTraceAppPowerMessageEnd @ 0x140728464 (PopDiagTraceAppPowerMessageEnd.c)
 *     PopSessionClosed @ 0x140728628 (PopSessionClosed.c)
 *     PopSuspendResumeInvocation @ 0x140728EE8 (PopSuspendResumeInvocation.c)
 *     PopReadHiberbootPolicy @ 0x140728F5C (PopReadHiberbootPolicy.c)
 *     PopEnforceResiliencyScenarios @ 0x1407296C4 (PopEnforceResiliencyScenarios.c)
 *     PopSetDisplayStatus @ 0x14072A294 (PopSetDisplayStatus.c)
 *     PopLogSleepDisabled @ 0x140736600 (PopLogSleepDisabled.c)
 *     PopProcessorInformation @ 0x140739DE8 (PopProcessorInformation.c)
 *     PopUpdateConsoleDisplayState @ 0x14073A74C (PopUpdateConsoleDisplayState.c)
 *     PopSessionConnectionChange @ 0x1407468A8 (PopSessionConnectionChange.c)
 *     PopAcquireTransitionLock @ 0x1407472E0 (PopAcquireTransitionLock.c)
 *     PopReleaseTransitionLock @ 0x14074733C (PopReleaseTransitionLock.c)
 *     PopCurrentPowerStatePrecise @ 0x140747960 (PopCurrentPowerStatePrecise.c)
 *     PopSessionConnectionInvocation @ 0x140747DB0 (PopSessionConnectionInvocation.c)
 *     PopSessionCreated @ 0x140747E30 (PopSessionCreated.c)
 *     PopEsGetState @ 0x140747F6C (PopEsGetState.c)
 *     PopChangeCapability @ 0x14074968C (PopChangeCapability.c)
 *     PopResetCurrentPolicies @ 0x140749ADC (PopResetCurrentPolicies.c)
 *     PopApplyPolicy @ 0x140749BB8 (PopApplyPolicy.c)
 *     PopVerifySystemPowerPolicy @ 0x140749E74 (PopVerifySystemPowerPolicy.c)
 *     PopEnableHiberFile @ 0x14074AAD0 (PopEnableHiberFile.c)
 *     PopEtEnergyTrackerCreate @ 0x1407837D0 (PopEtEnergyTrackerCreate.c)
 *     PopPdcInvocation @ 0x1407849F0 (PopPdcInvocation.c)
 *     DbgkWerCaptureLiveKernelDump @ 0x14084BF40 (DbgkWerCaptureLiveKernelDump.c)
 *     PopNotifySessionUserPowerRequestsPresent @ 0x1408A2A34 (PopNotifySessionUserPowerRequestsPresent.c)
 *     PopApplyAdminPolicy @ 0x1408A501C (PopApplyAdminPolicy.c)
 *     PopGetPowerRequestListInfo @ 0x1408A51BC (PopGetPowerRequestListInfo.c)
 *     PopSetHiberFileSize @ 0x1408A849C (PopSetHiberFileSize.c)
 *     PopSetHiberFileType @ 0x1408A85B0 (PopSetHiberFileType.c)
 *     PopThermalProcessUsermodeEvent @ 0x1408A8DCC (PopThermalProcessUsermodeEvent.c)
 *     PpmParkApplyForcedMask @ 0x1408A91F8 (PpmParkApplyForcedMask.c)
 *     PpmParkClearForcedMask @ 0x1408A930C (PpmParkClearForcedMask.c)
 *     PopBatteryDeviceState @ 0x1408A9948 (PopBatteryDeviceState.c)
 *     PopPdcCsDeviceNotification @ 0x1408ABC54 (PopPdcCsDeviceNotification.c)
 *     PopProcessSessionDisplayStateChange @ 0x1408AC07C (PopProcessSessionDisplayStateChange.c)
 *     PopScreenOff @ 0x1408AC0DC (PopScreenOff.c)
 *     PopDiagTraceDisplayBurstWin32kCallout @ 0x1408AF068 (PopDiagTraceDisplayBurstWin32kCallout.c)
 *     PopUserPresentOverride @ 0x1408B62F8 (PopUserPresentOverride.c)
 *     TtmCleanupCurrentSession @ 0x1408BD2B8 (TtmCleanupCurrentSession.c)
 *     TtmInitCurrentSession @ 0x1408BD418 (TtmInitCurrentSession.c)
 *     TtmNotifySessionDisplayBurst @ 0x1408BD79C (TtmNotifySessionDisplayBurst.c)
 *     TtmDispatchApi @ 0x1408BF490 (TtmDispatchApi.c)
 *     PopInitPlatformSettings @ 0x140A1FE8C (PopInitPlatformSettings.c)
 */

NTSTATUS __stdcall NtPowerInformation(
        POWER_INFORMATION_LEVEL InformationLevel,
        PVOID InputBuffer,
        ULONG InputBufferLength,
        PVOID OutputBuffer,
        ULONG OutputBufferLength)
{
  POWER_INFORMATION_LEVEL v8; // ebx
  __int64 v9; // rcx
  __int128 *v10; // r14
  __int64 v11; // rdx
  KPROCESSOR_MODE v12; // r14
  ULONG v13; // r13d
  void *v14; // r15
  void *v15; // r12
  int v16; // eax
  unsigned __int64 v17; // rax
  NTSTATUS WakeSource; // ebx
  unsigned __int64 v19; // rax
  char v20; // r13
  unsigned __int64 v21; // rax
  unsigned __int64 v22; // rax
  __int64 v23; // rcx
  unsigned __int64 v24; // rax
  __int64 v25; // rcx
  unsigned __int64 v26; // rax
  __int64 v27; // rcx
  LUID v28; // rcx
  unsigned __int64 v29; // rcx
  int v30; // eax
  unsigned __int64 v31; // rax
  _QWORD *v32; // rax
  __int64 v33; // rdx
  __int64 v34; // rcx
  __int128 *v35; // rax
  char *v36; // rax
  __int64 v37; // rdx
  __int64 v38; // r8
  char *v39; // rcx
  int v40; // r13d
  int v41; // edx
  int v42; // edx
  int v43; // edx
  int v44; // edx
  bool v45; // zf
  __int64 v46; // rdx
  __int64 v47; // rdx
  __int64 v48; // rcx
  bool v49; // sf
  __int64 v50; // rcx
  __int64 *v51; // rax
  int v52; // eax
  USHORT v53; // bx
  int v54; // eax
  UINT v55; // edx
  __int64 v56; // rdx
  __int64 v57; // rcx
  unsigned int *PoolWithTag; // rax
  unsigned int *v59; // rbx
  NTSTATUS v60; // eax
  __int128 *v61; // rdx
  _QWORD *v62; // rax
  int v63; // eax
  unsigned int v64; // eax
  __int64 v65; // rcx
  UINT v66; // r13d
  unsigned int v67; // eax
  unsigned int v68; // eax
  __int64 v69; // rdx
  __int64 v70; // rcx
  unsigned int v71; // eax
  unsigned int v72; // eax
  __int64 v73; // rcx
  __int64 v74; // rcx
  __int64 v75; // rcx
  __int64 v76; // rdx
  unsigned int SessionId; // eax
  char v78; // al
  int Info; // eax
  unsigned int v80; // edx
  __int64 v81; // rcx
  int PowerRequestListInfo; // eax
  int v83; // eax
  NTSTATUS SettingNotificationName; // eax
  char IsDevicePotentialDripsConstraint; // al
  __int64 v86; // rcx
  char v87; // r13
  __int64 v88; // rdx
  __int64 v89; // rcx
  __int64 v90; // r8
  unsigned int v91; // eax
  bool v92; // bl
  int v93; // ebx
  char v95; // [rsp+50h] [rbp-6F8h]
  ULONG Length; // [rsp+54h] [rbp-6F4h]
  UINT Length_4; // [rsp+58h] [rbp-6F0h] BYREF
  char v98; // [rsp+5Ch] [rbp-6ECh] BYREF
  KPROCESSOR_MODE PreviousMode; // [rsp+5Dh] [rbp-6EBh]
  char v100; // [rsp+5Eh] [rbp-6EAh]
  void *Src; // [rsp+60h] [rbp-6E8h] BYREF
  char v102; // [rsp+68h] [rbp-6E0h]
  PVOID v103; // [rsp+70h] [rbp-6D8h]
  int v104; // [rsp+78h] [rbp-6D0h]
  struct _PROCESSOR_NUMBER v105; // [rsp+7Ch] [rbp-6CCh]
  POWER_INFORMATION_LEVEL v107; // [rsp+84h] [rbp-6C4h]
  UINT v108; // [rsp+88h] [rbp-6C0h] BYREF
  char v109[8]; // [rsp+90h] [rbp-6B8h] BYREF
  PVOID P[2]; // [rsp+98h] [rbp-6B0h] BYREF
  int v111; // [rsp+A8h] [rbp-6A0h]
  unsigned __int64 v112; // [rsp+B0h] [rbp-698h]
  LUID v113; // [rsp+B8h] [rbp-690h]
  _QWORD v114[192]; // [rsp+C0h] [rbp-688h] BYREF
  wchar_t psz[32]; // [rsp+6C0h] [rbp-88h] BYREF

  v112 = (unsigned __int64)OutputBuffer;
  v8 = InformationLevel;
  v107 = InformationLevel;
  v111 = 0;
  v113 = 0LL;
  memset(v114, 0, sizeof(v114));
  Src = 0LL;
  Length_4 = 0;
  v10 = 0LL;
  v103 = 0LL;
  P[0] = 0LL;
  LOBYTE(v11) = 0;
  v95 = 0;
  v100 = 0;
  v98 = 0;
  v105 = 0;
  if ( (unsigned int)v8 > (TraceApplicationPowerMessageEnd|0x40) )
  {
    WakeSource = -1073741811;
    goto LABEL_498;
  }
  v12 = KeGetCurrentThread()->PreviousMode;
  PreviousMode = v12;
  v13 = InputBuffer != 0LL ? InputBufferLength : 0;
  v14 = (void *)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
  P[1] = v14;
  Length = OutputBuffer != 0LL ? OutputBufferLength : 0;
  v15 = (void *)(v112 & -(__int64)(Length != 0));
  if ( !v12 )
  {
    v10 = (__int128 *)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
    v103 = v10;
LABEL_59:
    if ( (unsigned int)v8 > TraceApplicationPowerMessageEnd || (v30 = -234812256, !_bittest(&v30, v8)) )
    {
      v31 = (unsigned int)(v8 - 38);
      if ( (unsigned int)v31 > 0x39 || (v9 = 0x3FFE3BE75BE26EDLL, !_bittest64(&v9, v31)) )
      {
        PopAcquirePolicyLock();
        LOBYTE(v11) = 1;
        v95 = 1;
        v100 = 1;
      }
    }
    switch ( v8 )
    {
      case 0:
      case 1:
        if ( !v10 )
        {
          if ( !v15 )
            goto LABEL_68;
LABEL_75:
          v32 = &unk_1404439E4;
          goto LABEL_76;
        }
        if ( v13 < 0xE8 )
          goto LABEL_73;
        WakeSource = PopApplyPolicy(0LL, 0LL, v10, v13);
        InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
        if ( WakeSource >= 0 )
          goto LABEL_75;
        goto LABEL_70;
      case 2:
      case 3:
        if ( !v10 || !v15 )
          goto LABEL_68;
        if ( v13 < 0xE8 )
          goto LABEL_73;
        WakeSource = PopVerifySystemPowerPolicy(v10, v114);
        InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
        if ( WakeSource < 0 )
          goto LABEL_70;
        v32 = v114;
LABEL_76:
        Src = v32;
        Length_4 = 232;
        goto LABEL_492;
      case 4:
        if ( !v10 )
        {
          if ( !v15 )
            goto LABEL_68;
LABEL_100:
          BYTE9(PopCapabilities) = PopFullWake & 1;
          byte_140443C15 = PopDisksRegisteredForIdle();
          PopFilterCapabilities(&PopCapabilities, v114);
          Src = v114;
          Length_4 = 76;
LABEL_101:
          v10 = (__int128 *)v103;
          goto LABEL_492;
        }
        if ( (PopSimulate & 1) == 0 )
          goto LABEL_68;
        if ( v13 >= 0x4C )
        {
          PopCapabilities = *v10;
          unk_140443C10 = v10[1];
          xmmword_140443C20 = v10[2];
          xmmword_140443C30 = v10[3];
          qword_140443C40 = *((_QWORD *)v10 + 8);
          dword_140443C48 = *((_DWORD *)v10 + 18);
          WakeSource = PopResetCurrentPolicies(v9, v11);
          InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
          if ( WakeSource < 0 )
            goto LABEL_70;
          goto LABEL_100;
        }
        goto LABEL_73;
      case 5:
        if ( v10 || !v15 )
          goto LABEL_68;
        PopCurrentPowerState(v114);
        goto LABEL_113;
      case 6:
        if ( !v10 || v15 )
          goto LABEL_68;
        if ( v13 < 0x18 )
          goto LABEL_73;
        v37 = *(unsigned int *)v10;
        if ( (unsigned int)v37 >= 7 )
          goto LABEL_68;
        v38 = 3 * v37;
        if ( qword_140443B48[3 * v37] )
        {
          if ( (_DWORD)v37 != 4 || (void (__noreturn *)())qword_140443BA8 != PopShutdownHandler )
            goto LABEL_68;
        }
        *(_OWORD *)((char *)&PopPowerStateHandlers + 24 * v37) = *v10;
        qword_140443B50[3 * v37] = *((_QWORD *)v10 + 2);
        word_140443B45[12 * v37] = 0;
        byte_140443B47[24 * v37] = 0;
        v39 = 0LL;
        v40 = 0;
        if ( (_DWORD)v37 )
        {
          v41 = v37 - 1;
          if ( v41 )
          {
            v42 = v41 - 1;
            if ( v42 )
            {
              v43 = v42 - 1;
              if ( v43 )
              {
                v44 = v43 - 1;
                if ( !v44 )
                {
                  v39 = (char *)&PopCapabilities + 7;
                  goto LABEL_143;
                }
                if ( v44 != 2 )
                  goto LABEL_143;
                v40 = 4;
                v45 = (PopSimulate & 0x2000) == 0;
                v39 = (char *)&unk_140443C11;
              }
              else
              {
                v40 = 5;
                v45 = (PopSimulate & 0x2000) == 0;
                v39 = (char *)&PopCapabilities + 6;
              }
            }
            else
            {
              v40 = 4;
              v45 = (PopSimulate & 0x20) == 0;
              v39 = (char *)&PopCapabilities + 5;
            }
          }
          else
          {
            v40 = 3;
            v45 = (PopSimulate & 0x40) == 0;
            v39 = (char *)&PopCapabilities + 4;
          }
        }
        else
        {
          v40 = 2;
          v45 = (PopSimulate & 8) == 0;
          v39 = (char *)&PopCapabilities + 3;
        }
        if ( !v45 )
          v39 = 0LL;
LABEL_143:
        v46 = v40 & (unsigned int)-(byte_140443B44[8 * v38] != 0);
        if ( (int)v46 > (int)qword_140443C40 )
          LODWORD(qword_140443C40) = byte_140443B44[8 * v38] != 0 ? v40 : 0;
        if ( v39 )
        {
          LOBYTE(v46) = 1;
          PopChangeCapability(v39, v46);
        }
        goto LABEL_492;
      case 7:
        if ( v10 || !v15 )
          goto LABEL_68;
        PpmHeteroHgsBackupInit();
        Src = &PpmProcessorDriverDispatchTable;
        Length_4 = 272;
        goto LABEL_492;
      case 8:
        if ( v10 || !v15 )
          goto LABEL_68;
        v32 = PopPolicy;
        goto LABEL_76;
      case 9:
        if ( !v10 )
        {
          if ( !v15 )
            goto LABEL_68;
LABEL_84:
          v35 = &PopAdminPolicy;
          goto LABEL_85;
        }
        if ( v13 < 0x18 )
          goto LABEL_73;
        WakeSource = PopApplyAdminPolicy(0LL, v10);
        InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
        if ( WakeSource >= 0 )
        {
          WakeSource = PopResetCurrentPolicies(v34, v33);
          if ( WakeSource >= 0 )
            goto LABEL_84;
        }
        goto LABEL_70;
      case 10:
        if ( !v10 || v15 )
          goto LABEL_68;
        if ( !v13 )
          goto LABEL_73;
        PopAcquireTransitionLock(3LL);
        PopAcquirePolicyLock();
        LOBYTE(v47) = 1;
        LOBYTE(v48) = *(_BYTE *)v10;
        WakeSource = PopEnableHiberFile(v48, v47);
        PopReleasePolicyLock();
        PopReleaseTransitionLock(3LL);
        goto LABEL_157;
      case 11:
        if ( v10 || !v15 )
          goto LABEL_68;
        v52 = PopProcessorInformation(v114, 1536LL, KeGetCurrentPrcb()->Group, &Length_4);
        v10 = (__int128 *)v103;
        goto LABEL_177;
      case 12:
        if ( v10 || !v15 )
          goto LABEL_68;
        LODWORD(v114[0]) = 0;
        HIDWORD(v114[0]) = dword_140442F64;
        LODWORD(v114[1]) = dword_140442F6C - dword_140442F68;
        BYTE4(v114[1]) = PopCoolingMode;
        Src = v114;
        Length_4 = 16;
        goto LABEL_492;
      case 13:
      case 18:
      case 19:
      case 20:
      case 21:
      case 22:
      case 32:
      case 33:
      case 34:
      case 49:
      case 52:
      case 53:
      case 54:
      case 79:
      case 80:
      case 81:
      case 82:
        goto LABEL_77;
      case 14:
        if ( v10 || !v15 )
          goto LABEL_68;
        v51 = &qword_140443088;
        goto LABEL_166;
      case 15:
        if ( v10 || !v15 )
          goto LABEL_68;
        v51 = &qword_140443090;
        goto LABEL_166;
      case 16:
        if ( v10 || !v15 )
          goto LABEL_68;
        if ( dword_140424780 )
          LODWORD(v114[0]) |= 1u;
        if ( PopPowerRequestAttributes[0] )
          LODWORD(v114[0]) |= 2u;
        goto LABEL_205;
      case 17:
        if ( !v10 || v15 )
          goto LABEL_68;
        if ( v13 < 0x10 )
          goto LABEL_73;
        if ( (_QWORD)PopPowerStateNotifyHandler && *(_QWORD *)v10 )
          goto LABEL_68;
        PopPowerStateNotifyHandler = *v10;
        goto LABEL_492;
      case 23:
        if ( v10 || !v15 )
          goto LABEL_68;
        WakeSource = PopLoggingInformation(P, &v108);
        InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
        if ( WakeSource < 0 )
          goto LABEL_70;
        Src = P[0];
        Length_4 = v108;
        goto LABEL_492;
      case 24:
        if ( !v10 || v15 )
          goto LABEL_68;
        if ( v13 < 8 )
          goto LABEL_73;
        v54 = PopLogSleepDisabled(*(unsigned int *)v10, *((unsigned int *)v10 + 1), 0LL, 0LL);
        goto LABEL_192;
      case 25:
        if ( !v10 )
          goto LABEL_68;
        if ( v15 )
          goto LABEL_68;
        if ( Length )
          goto LABEL_68;
        if ( v13 < 0x1C )
          goto LABEL_68;
        v55 = *((_DWORD *)v10 + 6);
        if ( !v55 || *((_DWORD *)v10 + 5) > 2u )
          goto LABEL_68;
        if ( *(_DWORD *)v10 != 1 )
        {
          WakeSource = -1073741736;
          goto LABEL_69;
        }
        WakeSource = RtlUIntAdd(0x1Cu, v55, &Length_4);
        InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
        if ( WakeSource < 0 )
          goto LABEL_70;
        if ( v13 < Length_4 )
          goto LABEL_68;
        PopGetSessionId(v57, v56);
        v54 = PopSetPowerSettingValue((LPCGUID)((char *)v10 + 4), (char *)v10 + 28);
LABEL_192:
        WakeSource = v54;
        v49 = v54 < 0;
        goto LABEL_158;
      case 28:
        PopSuspendResumePdc(1LL, v11);
        PopHiberBootForceMonitorOff = 1;
        SettingNotificationName = PoPowerOffMonitor();
        goto LABEL_374;
      case 29:
        if ( v10 || !v15 )
          goto LABEL_68;
        Length_4 = 4;
        Src = ExAllocatePoolWithTag(PagedPool, 4uLL, 0x206D654Du);
        if ( !Src )
          goto LABEL_226;
        v98 = 1;
        *(_DWORD *)Src = PopConsoleDisplayState;
        goto LABEL_492;
      case 30:
      case 31:
        if ( !v10 || v15 )
          goto LABEL_68;
        if ( v13 < 8 )
          goto LABEL_73;
        if ( v8 == TraceApplicationPowerMessage )
          PopDiagTraceAppPowerMessage(v10, v11);
        else
          PopDiagTraceAppPowerMessageEnd(v10, v11);
        goto LABEL_492;
      case 35:
        if ( v10 || !v15 )
          goto LABEL_68;
        if ( (_BYTE)v11 )
          PopReleasePolicyLock();
        v20 = 0;
        v95 = 0;
        Length_4 = 0;
        WakeSource = PopGetWakeSource(0LL, &Length_4);
        if ( WakeSource != -1073741789 )
          goto LABEL_242;
        Src = ExAllocatePoolWithTag(NonPagedPoolNx, Length_4, 0x206D654Du);
        if ( !Src )
        {
          WakeSource = -1073741670;
          InputBuffer = v14;
          goto LABEL_499;
        }
        v98 = 1;
        WakeSource = PopGetWakeSource(Src, &Length_4);
LABEL_242:
        InputBuffer = v14;
        if ( WakeSource < 0 )
          goto LABEL_499;
        v100 = 0;
        goto LABEL_492;
      case 36:
        if ( !v15 )
          goto LABEL_68;
        if ( !(_DWORD)dword_140443420 || !qword_140443418 )
        {
          WakeSource = -1073741275;
          goto LABEL_69;
        }
        Length_4 = dword_140443420 + 8;
        PoolWithTag = (unsigned int *)ExAllocatePoolWithTag(PagedPool, (unsigned int)(dword_140443420 + 8), 0x206D654Du);
        v59 = PoolWithTag;
        if ( !PoolWithTag )
        {
LABEL_226:
          WakeSource = -1073741670;
          goto LABEL_69;
        }
        Src = PoolWithTag;
        v98 = 1;
        memmove(PoolWithTag + 2, qword_140443418, (unsigned int)dword_140443420);
        *v59 = (unsigned int)dword_140443420 >> 4;
        goto LABEL_492;
      case 37:
        if ( !v10 || v15 )
          goto LABEL_68;
        if ( PreviousMode != 1 )
          goto LABEL_220;
        WakeSource = PopValidateServiceNotification(v10, v13);
        InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
        if ( WakeSource < 0 )
          goto LABEL_70;
        PopDiagTraceServiceNotification(v10);
        goto LABEL_492;
      case 38:
        if ( !v10 || v15 )
          goto LABEL_68;
        if ( v13 == 6 )
        {
          v60 = PpmSetSimulatedLoad((__int64)v10);
          goto LABEL_256;
        }
        if ( v13 != 4 )
          goto LABEL_257;
        v60 = PpmClearSimulatedLoad((struct _PROCESSOR_NUMBER *)v10);
        goto LABEL_256;
      case 39:
        if ( !v10 || v15 || v13 != 24 )
          goto LABEL_68;
        v62 = ExAllocatePoolWithTag(NonPagedPoolNx, 0x18uLL, 0x206D654Du);
        if ( !v62 )
          goto LABEL_226;
        *(_OWORD *)v62 = *v10;
        v62[2] = *((_QWORD *)v10 + 2);
        if ( !_InterlockedCompareExchange64(
                (volatile signed __int64 *)&PopShutdownNotificationCallback,
                (signed __int64)v62,
                0LL) )
          goto LABEL_101;
        ExFreePoolWithTag(v62, 0x206D654Du);
        WakeSource = -1073741823;
        v10 = (__int128 *)v103;
        goto LABEL_69;
      case 40:
        if ( !v10 || v13 != 4 || v15 )
          goto LABEL_68;
        v63 = (int)v105;
        if ( *(_DWORD *)v10 )
          v63 = 1;
        v105 = (struct _PROCESSOR_NUMBER)v63;
        PopSetPowerSettingValueAcDc(&GUID_VIDEO_BRIGHTNESS_CAPABLE);
        goto LABEL_492;
      case 41:
        if ( v10 )
          goto LABEL_68;
        if ( !v15 )
          goto LABEL_68;
        v66 = Length;
        if ( Length != 64 )
          goto LABEL_68;
        LOBYTE(v114[6]) = (unsigned int)PopEsGetState(v9, v11) == 1;
        LOBYTE(v114[7]) = TtmIsEnabled(v74);
        if ( !(unsigned __int8)TtmIsEnabled(v75) )
          goto LABEL_310;
        WakeSource = TtmInitCurrentSession();
        InputBuffer = v14;
        if ( WakeSource < 0 )
          goto LABEL_70;
        PopNotifySessionUserPowerRequestsPresent();
LABEL_310:
        PopAcquirePolicyLock();
        LODWORD(v114[2]) = 0;
        v114[0] = &PopNoMoreInput;
        v114[1] = &PopHiberBootForceMonitorOff;
        LODWORD(v114[3]) = byte_140443AD1 != 0;
        HIDWORD(v114[3]) = dword_140443ACC;
        HIDWORD(v114[2]) = dword_140443AEC;
        v114[4] = qword_140443AF0;
        HIDWORD(v114[6]) = HIDWORD(qword_140443AF8);
        v114[5] = __PAIR64__(dword_140443B00, qword_140443AF8);
        BYTE1(v114[6]) = PopLidOpened;
        Src = v114;
        Length_4 = 64;
        SessionId = PopGetSessionId(byte_140443AD1 != 0, v76);
        PopSessionCreated(SessionId);
        PopReleasePolicyLock();
        goto LABEL_493;
      case 42:
        if ( !v10 || v13 != 16 || v15 )
          goto LABEL_68;
        if ( *((_BYTE *)v10 + 13) && *((_BYTE *)v10 + 12) )
        {
          if ( PopPlatformAoAc && !PopConsoleDisplayState && *((_DWORD *)v10 + 1) == 2 )
            DbgkWerCaptureLiveKernelDump(L"InvalidDisplayStateTransition", *(unsigned int *)v10, 0LL, 0LL, 0LL, 0);
          if ( PopPlatformAoAc && *((_DWORD *)v10 + 1) != 2 )
          {
            LOBYTE(v9) = *((_BYTE *)v10 + 15);
            PopProcessSessionDisplayStateChange(v9, *((unsigned int *)v10 + 2));
          }
          PopSetDisplayStatus(*((unsigned int *)v10 + 1), v11);
          PopUpdateConsoleDisplayState(*((unsigned int *)v10 + 1));
        }
        v78 = *((_BYTE *)v10 + 14);
        if ( v78 )
        {
          if ( *((_BYTE *)v10 + 12) )
          {
            ZwUpdateWnfStateData((__int64)&WNF_PO_PRIMARY_DISPLAY_VISIBLE_STATE, (__int64)v10 + 4, 4LL);
            v78 = *((_BYTE *)v10 + 14);
          }
          if ( v78 )
            PopDiagTraceSessionDisplayStateChange(
              *((_DWORD *)v10 + 1) == 0,
              *(_DWORD *)v10,
              *((unsigned __int8 *)v10 + 12),
              *((_DWORD *)v10 + 2));
        }
        goto LABEL_492;
      case 43:
      case 72:
        if ( !v10 || !v15 )
          goto LABEL_68;
        if ( v13 != 40 )
          goto LABEL_73;
        v66 = Length;
        if ( Length != 8 )
          goto LABEL_73;
        LOBYTE(v11) = v8 == 72;
        Info = PopPowerRequestCreateInfo(v10, v11, v114);
        goto LABEL_334;
      case 44:
        if ( !v10 || v15 )
          goto LABEL_68;
        if ( v13 != 24 )
          goto LABEL_73;
        v54 = PopPowerRequestActionInfo(v10, v11);
        goto LABEL_192;
      case 45:
        if ( v10 || !v15 )
          goto LABEL_68;
        PowerRequestListInfo = PopGetPowerRequestListInfo(&Src, &Length_4);
        goto LABEL_346;
      case 46:
        if ( !v10 )
          goto LABEL_68;
        if ( v13 < 2 )
          goto LABEL_68;
        if ( !v15 )
          goto LABEL_68;
        v53 = *(_WORD *)v10;
        if ( v53 >= KeQueryActiveGroupCount() )
          goto LABEL_68;
        v52 = PopProcessorInformation(v114, 1536LL, v53, &Length_4);
LABEL_177:
        InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
        WakeSource = v52;
        if ( v52 < 0 )
          goto LABEL_70;
        Src = v114;
        goto LABEL_492;
      case 47:
        if ( !v10 || v13 != 16 )
          goto LABEL_68;
        v83 = PopUmpoSendLegacyEvent(v10);
        WakeSource = v83;
        if ( v83 >= 0 )
          goto LABEL_492;
        InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
        if ( v83 == -1073741536 )
          goto LABEL_492;
        goto LABEL_70;
      case 48:
        if ( !(_BYTE)KdDebuggerEnabled )
          goto LABEL_220;
        if ( !v10 || v15 )
          goto LABEL_68;
        switch ( v13 )
        {
          case 0x10u:
            v61 = 0LL;
LABEL_265:
            v60 = PpmParkApplyForcedMask(v10, v61);
            goto LABEL_256;
          case 0x18u:
            v61 = v10 + 1;
            goto LABEL_265;
          case 2u:
            v60 = PpmParkClearForcedMask(v10, v11);
LABEL_256:
            WakeSource = v60;
            break;
          default:
LABEL_257:
            WakeSource = -1073741811;
            break;
        }
LABEL_157:
        v49 = WakeSource < 0;
LABEL_158:
        InputBuffer = v14;
        if ( v49 )
          goto LABEL_70;
LABEL_492:
        v66 = Length;
LABEL_493:
        if ( !Src || !v15 )
          goto LABEL_412;
        if ( v66 < Length_4 )
        {
LABEL_73:
          WakeSource = -1073741789;
        }
        else
        {
          memmove(v15, Src, Length_4);
LABEL_412:
          WakeSource = 0;
        }
LABEL_69:
        InputBuffer = v14;
LABEL_70:
        v20 = v95;
        goto LABEL_499;
      case 50:
        if ( v10 || !v15 )
          goto LABEL_68;
        PowerRequestListInfo = ExGetWakeTimerList((ULONG_PTR *)&Src, &Length_4);
LABEL_346:
        InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
        WakeSource = PowerRequestListInfo;
        if ( PowerRequestListInfo < 0 )
          goto LABEL_70;
        v98 = 1;
        goto LABEL_492;
      case 51:
        if ( !v10 || !v15 )
          goto LABEL_68;
        if ( v13 < 4 )
          goto LABEL_73;
        PopAcquireTransitionLock(5LL);
        PopAcquirePolicyLock();
        WakeSource = PopSetHiberFileSize(*(unsigned int *)v10, v109);
        PopReleasePolicyLock();
        v50 = 5LL;
        goto LABEL_164;
      case 55:
        if ( !(_BYTE)KdDebuggerEnabled )
          goto LABEL_220;
        if ( !v10 || v15 )
          goto LABEL_68;
        if ( v13 == 8 )
        {
          v60 = PpmSetSimulatedIdle((__int64)v10);
          goto LABEL_256;
        }
        if ( v13 != 4 )
          goto LABEL_257;
        v60 = PpmClearSimulatedIdle((struct _PROCESSOR_NUMBER *)v10);
        goto LABEL_256;
      case 56:
        if ( !v10 || !v15 )
          goto LABEL_68;
        if ( v13 != 8 )
          goto LABEL_73;
        v66 = Length;
        if ( Length != 4 )
          goto LABEL_73;
        WakeSource = PpmParkSetLpiCap(*(_DWORD *)v10, *((_DWORD *)v10 + 1), (unsigned int *)v114);
        if ( WakeSource < 0 )
          goto LABEL_69;
        Src = v114;
        Length_4 = 4;
        goto LABEL_493;
      case 57:
        if ( !ExVerifySuite(PhoneNT) )
        {
LABEL_77:
          WakeSource = -1073741822;
          goto LABEL_69;
        }
        if ( !v10 || v13 != 1 || v15 )
          goto LABEL_68;
        LOBYTE(v73) = *(_BYTE *)v10;
        WakeSource = PopUserPresentOverride(v73);
        if ( WakeSource >= 0 )
          goto LABEL_492;
        goto LABEL_69;
      case 58:
        if ( !v15 )
          goto LABEL_68;
        if ( v14 && v13 != 20 || Length != 8 )
          goto LABEL_73;
        SettingNotificationName = PopGetSettingNotificationName(v10, v15);
        goto LABEL_374;
      case 59:
        if ( !v10 || !v15 )
          goto LABEL_68;
        if ( v13 != 16 || Length < 4 )
          goto LABEL_73;
        SettingNotificationName = PopGetSettingValue(v10, v15, Length);
        goto LABEL_374;
      case 60:
        if ( !v10 || v15 )
          goto LABEL_68;
        if ( v13 < 8 )
          goto LABEL_73;
        PopEnforceResiliencyScenarios(v10);
        goto LABEL_492;
      case 61:
        if ( !v10 )
          goto LABEL_68;
        if ( v13 != 8 )
          goto LABEL_68;
        if ( !v15 )
          goto LABEL_68;
        v66 = Length;
        if ( Length != 8 )
          goto LABEL_68;
        v67 = PopGetSessionId(v9, v11);
        PopSessionInputChange(v67, v10, v15);
        goto LABEL_493;
      case 62:
        if ( !v10 )
          goto LABEL_68;
        if ( v13 != 2 )
          goto LABEL_68;
        if ( !v15 )
          goto LABEL_68;
        v66 = Length;
        if ( Length != 8 )
          goto LABEL_68;
        v68 = PopGetSessionId(v9, v11);
        PopSessionConnectionChange(v68, v10, v15);
        v71 = PopGetSessionId(v70, v69);
        PopSessionConnectionInvocation(v71, v10);
        goto LABEL_493;
      case 63:
        if ( v10 || v15 )
          goto LABEL_68;
        v64 = PopGetSessionId(v9, v11);
        PopSessionClosed(v64);
        if ( (unsigned __int8)TtmIsEnabled(v65) )
          TtmCleanupCurrentSession();
        goto LABEL_492;
      case 64:
        if ( !v10 || v13 != 8 || v15 )
          goto LABEL_68;
        v72 = PopGetSessionId(v9, v11);
        PopSessionWinlogonNotification(v72, (__int64)v10);
        goto LABEL_492;
      case 65:
        if ( PsIsCurrentThreadInServerSilo() )
          goto LABEL_103;
        if ( v10 || !v15 )
          goto LABEL_68;
        PopFilterCapabilities(&PopCapabilities, v114);
        if ( BYTE2(v114[2]) )
          PopReadHiberbootPolicy((char *)&v114[2] + 2);
        v36 = (char *)&v114[2] + 2;
        goto LABEL_109;
      case 66:
        if ( v10 || !v15 )
          goto LABEL_68;
        IsDevicePotentialDripsConstraint = PopPlatformAoAc;
        goto LABEL_398;
      case 67:
        if ( !v10 || v13 != 168 || v15 && Length != 184 )
          goto LABEL_68;
        SettingNotificationName = PopPdcInvocation(v10, v15);
        goto LABEL_374;
      case 68:
        if ( !v10 || v13 != 8 || v15 || Length )
          goto LABEL_68;
        SettingNotificationName = PopMonitorInvocation(v10, v11);
        goto LABEL_374;
      case 69:
        if ( v10 || v15 )
          goto LABEL_68;
        WakeSource = PopInitPlatformSettings(v9, v11);
        byte_140443C14 = PopPlatformAoAc;
        goto LABEL_69;
      case 70:
        PopShutdownButtonPressTime = KeQueryPerformanceCounter(0LL).QuadPart;
        goto LABEL_492;
      case 71:
        if ( !v10 || v13 != 8 || v15 && Length )
          goto LABEL_68;
        SettingNotificationName = PopSuspendResumeInvocation(v10);
        goto LABEL_374;
      case 73:
      case 88:
        if ( v10 || v15 )
          goto LABEL_68;
        v86 = 23LL;
        if ( v8 != (SystemPowerLoggingEntry|0x40) )
          v86 = 11LL;
        PopScreenOff(v86, v11);
        goto LABEL_412;
      case 74:
        if ( PreviousMode )
          goto LABEL_220;
        if ( !v10 || v13 != 16 || v15 || Length )
          goto LABEL_68;
        SettingNotificationName = PopPdcCsDeviceNotification(v10, v11);
LABEL_374:
        WakeSource = SettingNotificationName;
        goto LABEL_69;
      case 75:
        if ( v10 || !v15 )
          goto LABEL_68;
        LODWORD(v114[0]) = PopCapturePlatformRole(v9, v11);
LABEL_205:
        Src = v114;
        Length_4 = 4;
        goto LABEL_492;
      case 76:
        if ( v10 || !v15 )
          goto LABEL_68;
        LODWORD(v114[0]) = qword_1404435A8;
        v114[1] = qword_140443740;
        v114[2] = qword_140443748;
        if ( !PopSleepStats )
        {
          WakeSource = -1073741823;
          goto LABEL_69;
        }
        v35 = (__int128 *)v114;
LABEL_85:
        Src = v35;
        Length_4 = 24;
        goto LABEL_492;
      case 77:
        if ( v10 || v15 )
          goto LABEL_68;
        v87 = PopErrataReportingIncorrectLidState;
        if ( (unsigned __int8)TtmIsEnabled(v9) )
        {
          v91 = PopGetSessionId(v89, v88);
          TtmNotifySessionDisplayBurst(v91, 6LL);
        }
        else
        {
          v92 = 0;
          if ( !PopLidOpened )
            v92 = PopConsoleExternalDisplayConnected == 0;
          if ( !v92 )
            PopEventCalloutDispatch(10, 6LL);
          if ( !v87 )
          {
            LOBYTE(v90) = v92;
            LOBYTE(v88) = PopConsoleExternalDisplayConnected;
            LOBYTE(v89) = PopLidOpened;
            PopDiagTraceDisplayBurstWin32kCallout(v89, v88, v90);
          }
        }
        goto LABEL_492;
      case 78:
        if ( PreviousMode != 1 )
        {
LABEL_220:
          WakeSource = -1073741790;
          goto LABEL_69;
        }
        if ( v15 || Length )
          goto LABEL_68;
        if ( v10 )
        {
          if ( v13 != 4 )
            goto LABEL_68;
          PpmSetExitLatencySamplingPercentage((int *)v10);
        }
        else
        {
          PpmClearExitLatencySamplingPercentage();
        }
        goto LABEL_492;
      case 83:
        if ( v10 || !v15 )
          goto LABEL_68;
        PopCurrentPowerStatePrecise(v114, 0LL);
LABEL_113:
        Src = v114;
        Length_4 = 32;
        goto LABEL_492;
      case 84:
        if ( !v10 || v15 )
          goto LABEL_68;
        if ( v13 < 0x10 || v13 < 2 * (unsigned int)*((unsigned __int16 *)v10 + 6) + 14 )
          goto LABEL_73;
        v54 = PopThermalProcessUsermodeEvent(v10, v11);
        goto LABEL_192;
      case 85:
        v80 = *((_DWORD *)v10 + 2);
        v81 = *(_QWORD *)v10;
        if ( *((_BYTE *)v10 + 12) )
          PoSetPowerRequestInternal(v81, v80);
        else
          PoClearPowerRequestInternal(v81, v80);
        goto LABEL_492;
      case 86:
        if ( !v10 || !v15 )
          goto LABEL_68;
        WakeSource = RtlStringCbLengthW((STRSAFE_PCNZWCH)v10, v13, 0LL);
        InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
        if ( WakeSource < 0 )
          goto LABEL_70;
        WakeSource = PopBatteryDeviceState(v10, v114);
        if ( WakeSource < 0 )
          goto LABEL_70;
        Src = v114;
        Length_4 = 52;
        goto LABEL_492;
      case 87:
        if ( !v10 )
          goto LABEL_68;
        if ( v13 < 8 )
          goto LABEL_68;
        v93 = *(_DWORD *)v10;
        if ( (unsigned int)(*(_DWORD *)v10 - 67) <= 0xFBC || v93 >= 4106 )
          goto LABEL_68;
        if ( (unsigned int)(v93 - 4096) > 9 )
        {
          v60 = PopPowerInformationInternal(
                  v93,
                  (_DWORD)v10,
                  v13,
                  (_DWORD)v15,
                  (__int64)&Src,
                  (__int64)&Length_4,
                  (__int64)&v98);
        }
        else
        {
          if ( !SeSinglePrivilegeCheck(SeShutdownPrivilege, PreviousMode)
            && !PopCapabilityCheck(L"terminalPowerManagement") )
          {
            WakeSource = -1073741727;
            goto LABEL_69;
          }
          v60 = TtmDispatchApi(
                  v93,
                  (_DWORD)v10,
                  v13,
                  (_DWORD)v15,
                  Length,
                  (__int64)&Src,
                  (__int64)&Length_4,
                  (__int64)&v98);
        }
        goto LABEL_256;
      case 89:
        if ( !v10 || !v15 )
          goto LABEL_68;
        if ( v13 < 4 )
          goto LABEL_73;
        PopAcquireTransitionLock(6LL);
        PopAcquirePolicyLock();
        WakeSource = PopSetHiberFileType(*(unsigned int *)v10, v109);
        PopReleasePolicyLock();
        v50 = 6LL;
LABEL_164:
        PopReleaseTransitionLock(v50);
        InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
        if ( WakeSource < 0 )
          goto LABEL_70;
        v51 = (__int64 *)v109;
LABEL_166:
        Src = v51;
        Length_4 = 8;
        goto LABEL_492;
      case 90:
        if ( !v10 || v13 < 4 || v15 )
          goto LABEL_68;
        v105 = *(struct _PROCESSOR_NUMBER *)v10;
        PopUpdatePowerButtonHoldState(v105 != 0);
        goto LABEL_492;
      case 91:
        if ( !PopPlatformAoAc )
        {
LABEL_103:
          WakeSource = -1073741637;
          goto LABEL_69;
        }
        if ( !v10 || v13 != 336 || !v15 )
        {
LABEL_68:
          WakeSource = -1073741811;
          goto LABEL_69;
        }
        IsDevicePotentialDripsConstraint = PopFxIsDevicePotentialDripsConstraint(v10);
LABEL_398:
        LOBYTE(v114[0]) = IsDevicePotentialDripsConstraint;
        v36 = (char *)v114;
LABEL_109:
        Src = v36;
        Length_4 = 1;
        goto LABEL_492;
      case 92:
        if ( !v10 || !v15 )
          goto LABEL_68;
        if ( v13 != 12 )
          goto LABEL_73;
        v66 = Length;
        if ( Length != 8 )
          goto LABEL_73;
        Info = PopEtEnergyTrackerCreate(v10, v114);
LABEL_334:
        InputBuffer = v14;
        WakeSource = Info;
        if ( Info < 0 )
          goto LABEL_70;
        Src = v114;
        Length_4 = 8;
        goto LABEL_493;
      case 93:
        if ( !v10 || !v15 )
          goto LABEL_68;
        if ( v13 != 8 )
          goto LABEL_73;
        v66 = Length;
        if ( Length < 0xC )
          goto LABEL_73;
        WakeSource = PopEtEnergyTrackerQuery(*(_QWORD *)v10, v15);
        InputBuffer = v14;
        if ( WakeSource >= 0 )
          goto LABEL_493;
        goto LABEL_70;
      case 94:
        if ( !v10 || v15 || Length )
          goto LABEL_68;
        if ( v13 != 32 )
          goto LABEL_73;
        LOBYTE(v11) = PreviousMode;
        WakeSource = PopBlackBoxUpdate(v10, v11);
        InputBuffer = v14;
        if ( WakeSource < 0 )
          goto LABEL_70;
        goto LABEL_492;
      case 95:
        if ( !v10 || v13 != 1 || v15 )
          goto LABEL_68;
        PnpWinlogonExternalDmaNotification(v10);
        goto LABEL_492;
      default:
        goto LABEL_68;
    }
  }
  if ( (unsigned int)v8 > SetPowerSettingValue || (v16 = 50462912, !_bittest(&v16, v8)) )
  {
    v17 = (unsigned int)(v8 - 30);
    if ( (unsigned int)v17 > 0x3D || (v9 = 0x209E02E385CA1E1FLL, !_bittest64(&v9, v17)) )
    {
      v19 = (unsigned int)(v8 - 37);
      if ( (unsigned int)v19 <= 0x3A && (v9 = 0x608801808940903LL, _bittest64(&v9, v19)) || (unsigned int)(v8 - 2) <= 1 )
      {
        WakeSource = SeIsAppContainerOrIdentifyLevelContext(0LL);
        v104 = WakeSource;
        if ( WakeSource < 0 )
        {
          InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
          v10 = (__int128 *)v103;
          v20 = 0;
          goto LABEL_499;
        }
        v8 = v107;
        if ( v102 )
        {
          if ( v107 != (AdministratorPowerPolicy|0x40) && v107 != (SystemPowerLoggingEntry|0x40) )
          {
            WakeSource = -1073741790;
            v104 = -1073741790;
            InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
            v10 = (__int128 *)v103;
            v20 = 0;
            goto LABEL_499;
          }
          if ( !PopCapabilityCheck(L"ID_CAP_SCREENOFF") )
          {
            WakeSource = -1073741790;
            v104 = -1073741790;
            InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
            v10 = (__int128 *)v103;
            v20 = 0;
            goto LABEL_499;
          }
        }
      }
      if ( (unsigned int)(v8 - 92) <= 1 )
      {
        LOBYTE(v9) = v12;
        WakeSource = ExCheckFullProcessInformationAccess(v9);
        v104 = WakeSource;
        if ( WakeSource < 0 )
        {
          InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
          v10 = (__int128 *)v103;
          v20 = 0;
          goto LABEL_499;
        }
        v8 = v107;
      }
      v21 = (unsigned int)(v8 - 28);
      if ( (unsigned int)v21 <= 0x32 )
      {
        v9 = 0x4000000420001LL;
        if ( _bittest64(&v9, v21) )
        {
          if ( !PspIsContextAdmin() )
          {
            WakeSource = -1073741790;
            v104 = -1073741790;
            InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
            v10 = (__int128 *)v103;
            v20 = 0;
            goto LABEL_499;
          }
        }
      }
      if ( v14 )
      {
        v22 = (unsigned int)(v8 - 11);
        if ( (unsigned int)v22 > 0x3D || (v23 = 0x2001800900000001LL, !_bittest64(&v23, v22)) )
        {
          if ( PsIsCurrentThreadInServerSilo() )
          {
            WakeSource = -1073741637;
            v104 = -1073741637;
            InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
            v10 = (__int128 *)v103;
            v20 = 0;
            goto LABEL_499;
          }
        }
        if ( v8 == AdministratorPowerPolicy
          || (v24 = (unsigned int)(v8 - 38), (unsigned int)v24 <= 0x39)
          && (v25 = 0x218000004422401LL, _bittest64(&v25, v24)) )
        {
          if ( !PspIsContextAdmin() )
          {
            WakeSource = -1073741790;
            v104 = -1073741790;
            InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
            v10 = (__int128 *)v103;
            v20 = 0;
            goto LABEL_499;
          }
        }
        if ( (unsigned int)(v8 - 2) > 1 )
        {
          v26 = (unsigned int)(v8 - 37);
          if ( (unsigned int)v26 > 0x3A || (v27 = 0x7AE005828F40BC3LL, !_bittest64(&v27, v26)) )
          {
            if ( v8 != ProcessorInformation )
            {
              v28 = SeShutdownPrivilege;
              if ( v8 == SystemReserveHiberFile )
                v28 = SeCreatePagefilePrivilege;
              v113 = v28;
              if ( !SeSinglePrivilegeCheck(v28, v12) )
              {
                WakeSource = -1073741727;
                v104 = -1073741727;
                InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
                v10 = (__int128 *)v103;
                v20 = 0;
                goto LABEL_499;
              }
            }
          }
        }
        if ( v13 )
        {
          v29 = (unsigned __int64)v14 + v13;
          if ( v29 > 0x7FFFFFFF0000LL || v29 < (unsigned __int64)v14 )
            MEMORY[0x7FFFFFFF0000] = 0;
        }
        if ( v13 > 0x40 )
        {
          v10 = (__int128 *)ExAllocatePoolWithTag(PagedPool, v13, 0x206D654Du);
          v103 = v10;
          if ( !v10 )
          {
            WakeSource = -1073741670;
            v104 = -1073741670;
            InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
            v20 = 0;
            goto LABEL_499;
          }
        }
        else
        {
          v10 = (__int128 *)psz;
          v103 = psz;
        }
        memmove(v10, v14, v13);
      }
      else
      {
        v10 = (__int128 *)v103;
      }
      if ( v15 )
        ProbeForWrite(v15, Length, 1u);
      LOBYTE(v11) = 0;
      goto LABEL_59;
    }
  }
  WakeSource = -1073741790;
  InputBuffer = (PVOID)((unsigned __int64)InputBuffer & -(__int64)(v13 != 0));
  v10 = (__int128 *)v103;
LABEL_498:
  v20 = 0;
LABEL_499:
  if ( P[0] )
    ExFreePoolWithTag(P[0], 0);
  if ( v98 )
    ExFreePoolWithTag(Src, 0x206D654Du);
  if ( v20 )
    PopReleasePolicyLock();
  if ( v10 && v10 != InputBuffer && v10 != (__int128 *)psz )
    ExFreePoolWithTag(v10, 0x206D654Du);
  return WakeSource;
}
