/*
 * XREFs of PopDiagTraceDriverVeto @ 0x1405AD970
 * Callers:
 *     PopCompleteNotifyTransitionCommon @ 0x140165A10 (PopCompleteNotifyTransitionCommon.c)
 * Callees:
 *     EtwWrite @ 0x140036280 (EtwWrite.c)
 *     RtlInitUnicodeString @ 0x1400438D0 (RtlInitUnicodeString.c)
 *     EtwEventEnabled @ 0x140047610 (EtwEventEnabled.c)
 *     PopDiagGetDriverName @ 0x140165EB0 (PopDiagGetDriverName.c)
 *     __security_check_cookie @ 0x14019E700 (__security_check_cookie.c)
 */

BOOLEAN __fastcall PopDiagTraceDriverVeto(__int64 a1, __int64 a2)
{
  BOOLEAN result; // al
  unsigned __int16 Length; // cx
  __int16 v6; // ax
  unsigned __int16 v7; // r8
  unsigned int v8; // r9d
  __int64 v9; // rax
  ULONG v10; // r9d
  __int64 v11; // rcx
  __int16 v12; // [rsp+38h] [rbp-D0h] BYREF
  __int16 v13; // [rsp+3Ch] [rbp-CCh] BYREF
  UNICODE_STRING DestinationString; // [rsp+40h] [rbp-C8h] BYREF
  struct _EVENT_DATA_DESCRIPTOR UserData; // [rsp+58h] [rbp-B0h] BYREF
  wchar_t *Buffer; // [rsp+68h] [rbp-A0h]
  int v17; // [rsp+70h] [rbp-98h]
  int v18; // [rsp+74h] [rbp-94h]
  WCHAR SourceString[64]; // [rsp+98h] [rbp-70h] BYREF

  result = 0;
  *(_QWORD *)&DestinationString.Length = 0LL;
  DestinationString.Buffer = 0LL;
  if ( PopDiagHandleRegistered )
  {
    result = EtwEventEnabled(PopDiagHandle, &POP_ETW_EVENT_DRIVERVETO);
    if ( result )
    {
      if ( PopDiagGetDriverName(a1, SourceString) < 0 )
      {
        RtlInitUnicodeString(&DestinationString, &word_1405A34F0);
        Length = DestinationString.Length;
        v6 = 0;
      }
      else
      {
        RtlInitUnicodeString(&DestinationString, SourceString);
        Length = DestinationString.Length;
        v6 = DestinationString.Length >> 1;
      }
      v7 = *(_WORD *)(a2 - 104);
      UserData.Ptr = (ULONGLONG)&v12;
      v12 = v6;
      v13 = v7 >> 1;
      *(_QWORD *)&UserData.Size = 2LL;
      v8 = 1;
      if ( v6 )
      {
        v8 = 2;
        Buffer = DestinationString.Buffer;
        v17 = Length;
        v18 = 0;
      }
      v9 = 2LL * v8;
      v10 = v8 + 1;
      *(&UserData.Ptr + v9) = (ULONGLONG)&v13;
      *((_QWORD *)&UserData.Size + v9) = 2LL;
      if ( v7 >> 1 )
      {
        v11 = 2LL * v10++;
        *(&UserData.Ptr + v11) = *(_QWORD *)(a2 - 96);
        *(&UserData.Size + 2 * v11) = v7;
        *(&UserData.Reserved + 2 * v11) = 0;
      }
      return EtwWrite(PopDiagHandle, &POP_ETW_EVENT_DRIVERVETO, 0LL, v10, &UserData);
    }
  }
  return result;
}
