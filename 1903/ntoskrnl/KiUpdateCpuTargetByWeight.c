/*
 * XREFs of KiUpdateCpuTargetByWeight @ 0x1400E8134
 * Callers:
 *     KiAssignSchedulingGroupWeights @ 0x1400E807C (KiAssignSchedulingGroupWeights.c)
 *     KiUpdateChildrenCpuTarget @ 0x1402B2D0C (KiUpdateChildrenCpuTarget.c)
 * Callees:
...
char __fastcall KiUpdateCpuTargetByWeight(unsigned __int16 *a1, char a2)
{
  ...
  if ( *((_QWORD *)a1 + 12) )
  {
    ...
    if ( v8 )
    {
      ...
      {
        LODWORD(QuadPart) = *(_DWORD *)(v8 + 20) - 1;
        *(_DWORD *)(v8 + 20) = QuadPart;
        if ( !(_DWORD)QuadPart )
  ...
  else
  {
    ...
    QuadPart = KiCyclesPerGeneration * (unsigned __int64)v3 / (unsigned int)KiGroupSchedulingTotalWeight;
    v8 = KiCyclesPerGeneration * (unsigned __int64)v3 % (unsigned int)KiGroupSchedulingTotalWeight;
    v9 = QuadPart;
    if ( v3 == v5 )
      v6 = KiCyclesPerGeneration * (unsigned __int64)v3 / (unsigned int)KiGroupSchedulingTotalWeight;
  }
