/*
 * XREFs of MiSessionCreateInternal @ 0x14074050C
 * Callers:
...
__int64 __fastcall MiSessionCreateInternal(ULONG_PTR *a1)
{
  ...
  ClearBitsAndSet = RtlFindClearBitsAndSet(qword_140466990, 1u, 0);
  if ( ClearBitsAndSet == -1 )
  {
    ...
    memmove(
      ...
      8 * (((unsigned __int64)qword_140466990->SizeOfBitMap >> 6) + ((qword_140466990->SizeOfBitMap & 0x3F) != 0)));
    ExFreePoolWithTag(qword_140466990, 0);
    qword_140466990 = v22;
    ClearBitsAndSet = RtlFindClearBitsAndSet(v22, 1u, 0);
  }
  v5 = _InterlockedExchangeAdd64((volatile signed __int64 *)&qword_1404659B0, 0xFFFFFFFFFFFFFFFFuLL);
  if ( (v5 & 2) != 0 && (v5 & 4) == 0 )
  ...
  if ( (unsigned int)MiChargeCommit((__int64)a1, 8uLL, 0) )
  {
    ...
    if ( v6 )
    {
      ...
      if ( (unsigned int)MiChargeResident(a1, ClearBitsAndSet != 0 ? 1032LL : 28LL, 0LL) )
      {
        ...
        *(_DWORD *)(v8 + 1184) = 4;
        *(_QWORD *)(v8 + 1176) = MiGetPteAddress(qword_140464998);
    ...
    if ( v7 )
      MiReleasePtes((__int64)&qword_1404669C0, v7, 1u);
  }
  --CurrentThread->SpecialApcDisable;
  ExAcquirePushLockExclusiveEx((ULONG_PTR)&qword_1404659B0, 0LL);
  _bittestandreset((signed __int32 *)qword_140466990->Buffer, ClearBitsAndSet);
LABEL_22:
