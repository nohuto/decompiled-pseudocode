/*
 * XREFs of MiWritePteShadow @ 0x1401223FC
 * Callers:
...
char __fastcall MiWritePteShadow(__int64 a1)
{
  ...
  LODWORD(CurrentThread) = MiPteHasShadow(a1);
  if ( (_DWORD)CurrentThread )
  {
    CurrentThread = KeGetCurrentThread();
    DeepFreezeStartTime = CurrentThread->ApcState.Process[2].DeepFreezeStartTime;
    if ( DeepFreezeStartTime )
    {
      LOBYTE(CurrentThread) = -(v2 & 1);
      *(_QWORD *)(DeepFreezeStartTime + 8 * ((v3 >> 3) & 0x1FF)) = v2 & 0x7FFFFFFFFFFFFFFFLL & -(__int64)((v2 & 1) != 0);
    }
  }
