/*
 * XREFs of RawInitialize @ 0x140A184F0
 * Callers:
 *     <none>
 * Callees:
...
NTSTATUS __fastcall RawInitialize(PDRIVER_OBJECT DriverObject)
{
  ...
  *(_QWORD *)&DeviceName.Length = 0LL;
  ...
  result = IoCreateDevice(DriverObject, 0, &DeviceName, 8u, 0, 0, &RawDeviceDiskObject);
  if ( result >= 0 )
  {
    DriverObject->DriverUnload = (PDRIVER_UNLOAD)RawUnload;
    RtlInitUnicodeString(&DeviceName, L"\\Device\\RawCdRom");
    v3 = IoCreateDevice(DriverObject, 0, &DeviceName, 3u, 0, 0, &RawDeviceCdRomObject);
    if ( v3 >= 0 )
    {
      RtlInitUnicodeString(&DeviceName, L"\\Device\\RawTape");
      v3 = IoCreateDevice(DriverObject, 0, &DeviceName, 0x20u, 0, 0, &RawDeviceTapeObject);
      if ( v3 >= 0 )
      {
        ...
        if ( v3 >= 0 )
        {
          ...
          qword_1404368E8 = (__int64)&RawMountedQueue;
          RawMountedQueue = (__int64)&RawMountedQueue;
          qword_140436888 = (__int64)&RawDismountedQueue;
