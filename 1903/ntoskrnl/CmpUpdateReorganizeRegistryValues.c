/*
 * XREFs of CmpUpdateReorganizeRegistryValues @ 0x140785958
 * Callers:
...
int CmpUpdateReorganizeRegistryValues()
{
  int result; // eax
  __int64 v1; // rbx
  ...
  struct _TIME_FIELDS TimeFields; // [rsp+60h] [rbp-19h] BYREF
  ...
  if ( CmpReorganizeTotalBytesSaved )
  {
    if ( CmpAccessBitForPhase == 2 )
    {
      ...
      *(_OWORD *)&ObjectAttributes.SecurityDescriptor = 0LL;
      result = ZwCreateKey(&KeyHandle, 2u, &ObjectAttributes, 0, 0LL, 0, 0LL);
      if ( result >= 0 )
      {
        ...
        if ( swprintf_s(Dst, 0x10uLL, L"%02d:%02d:%04d", (unsigned int)TimeFields.Month, Class, CreateOptions) != -1 )
        {
          ...
          do
            ++v1;
          while ( Dst[v1] );
          if ( ZwSetValueKey(KeyHandle, &DestinationString, 0, 1u, Dst, 2 * v1) >= 0 )
          {
