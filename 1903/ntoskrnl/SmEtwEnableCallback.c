/*
 * XREFs of SmEtwEnableCallback @ 0x140774990
 * Callers:
 *     <none>
 * Callees:
...
void __fastcall SmEtwEnableCallback(LPCGUID SourceId, ULONG ControlCode, __int64 Level, ULONGLONG MatchAnyKeyword)
{
  ...
  if ( ControlCode <= 1 && (dword_14058C4FC = MatchAnyKeyword | 0x40, ControlCode == 1) || ControlCode == 2 )
  {
    SmKmEtwLogStoreRundown(&qword_14058C4F0, &SmGlobals, Level);
    if ( SmEtwEnabled(3) )
    {
      for ( i = 0; i < 0x400; ++i )
      {
