/*
 * XREFs of WdipSemAddScenarioToTable @ 0x14076CB98
 * Callers:
 *     WdipSemLoadScenarioTable @ 0x14076BD10 (WdipSemLoadScenarioTable.c)
 * Callees:
 *     WdipSemFreeScenario @ 0x14076C7B4 (WdipSemFreeScenario.c)
 *     WdipSemMergeScenarios @ 0x14076CCA4 (WdipSemMergeScenarios.c)
 */

__int64 __fastcall WdipSemAddScenarioToTable(__int64 a1)
{
  int v1; // ebx
  __int64 v3; // rdx
  __int64 v4; // rsi
  __int64 v5; // rbp
  __int64 v6; // r8

  v1 = 0;
  if ( a1 )
  {
    v3 = 0LL;
    if ( dword_1404331E0 )
    {
      while ( 1 )
      {
        v4 = WdipSemScenarioTable[v3];
        v5 = (unsigned int)v3;
        v6 = *(_QWORD *)v4 - *(_QWORD *)a1;
        if ( *(_QWORD *)v4 == *(_QWORD *)a1 )
          v6 = *(_QWORD *)(v4 + 8) - *(_QWORD *)(a1 + 8);
        if ( !v6 && *(_WORD *)(v4 + 16) == *(_WORD *)(a1 + 16) )
          break;
        v3 = (unsigned int)(v3 + 1);
        if ( (unsigned int)v3 >= dword_1404331E0 )
          goto LABEL_7;
      }
      v1 = WdipSemMergeScenarios(a1, WdipSemScenarioTable[v3]);
      if ( v1 >= 0 )
      {
        WdipSemScenarioTable[v5] = a1;
        WdipSemFreeScenario(v4);
      }
    }
    else
    {
LABEL_7:
      if ( (unsigned int)dword_1404331E0 >= 0x40 )
        return (unsigned int)-1073741823;
      else
        WdipSemScenarioTable[dword_1404331E0++] = a1;
    }
  }
  else
  {
    return (unsigned int)-1073741811;
  }
  return (unsigned int)v1;
}
