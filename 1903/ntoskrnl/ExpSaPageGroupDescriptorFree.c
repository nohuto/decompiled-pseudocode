/*
 * XREFs of ExpSaPageGroupDescriptorFree @ 0x14016EB78
 * Callers:
...
__int64 __fastcall ExpSaPageGroupDescriptorFree(ULONG_PTR BugCheckParameter2)
{
  ...
  if ( v8 )
  {
    if ( (unsigned __int64)&ExSaPageGroupDescriptorArrayLock >= 0xFFFF800000000000uLL
      && byte_140467440[(((unsigned __int64)&ExSaPageGroupDescriptorArrayLock >> 39) & 0x1FF) - 256] == 1 )
    {
...
LABEL_8:
  ...
  if ( _interlockedbittestandset64((volatile signed __int32 *)&ExSaPageGroupDescriptorArrayLock, 0LL) )
    ExfAcquirePushLockExclusiveEx(&ExSaPageGroupDescriptorArrayLock, v8, (ULONG_PTR)&ExSaPageGroupDescriptorArrayLock);
  if ( v8 )
    *(_BYTE *)(v8 + 26) |= 1u;
  if ( (_DWORD)v4 )
  {
      ...
      if ( v23->LockState.0 )
      {
        if ( v23 )
        {
          ...
          if ( (__int64)v23->LockState.LockState < 0 )
            KiAbEntryRemoveFromTree((__int64)&v17->LockEntries[v22]);
...
LABEL_34:
  ...
  if ( v11 && ($6EAC78A6FCFADE0A5FA44F358736B38F *)v17->ApcState.ApcListHead[0].Flink != &v17->152 )
