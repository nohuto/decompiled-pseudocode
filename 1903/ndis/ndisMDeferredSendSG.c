/*
 * XREFs of ndisMDeferredSendSG @ 0x1C008AD90
 * Callers:
 *     <none>
 * Callees:
...
char __fastcall ndisMDeferredSendSG(struct _NDIS_MINIPORT_BLOCK *a1)
{
  _NDIS_PACKET *i; // rdi
  int v3; // r9d
  ...
  for ( i = a1->FirstPendingPacket; i && (a1->Flags & 0x400000) != 0; i = a1->FirstPendingPacket )
  {
    ...
    if ( v5 != (struct _NDIS_MINIPORT_BLOCK *)&a1->PacketList )
      a1->FirstPendingPacket = (_NDIS_PACKET *)&v5[-1].HwTimestampCurrentConfig;
    if ( ((v3 & 0x4000) != 0 || (v3 & 0x8800000) != 0 && SLOBYTE(i->Private.Flags) >= 0)
      && (unsigned __int8)ndisMLoopbackPacketX(a1, i) )
    {
      v6 = 0;
      p_Lock = &a1->Lock;
    }
    ...
    else
    {
      ...
      *((_DWORD *)v11 + 2) = 0;
      ...
      if ( *(_MDL **)((char *)&i[1].Private.Head + NdisPacketOobOffset) )
        ndisMSendCompletePacketToNetBufferLists(a1, i, v6);
      else
