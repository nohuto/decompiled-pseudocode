/*
 * XREFs of NdisMSendComplete @ 0x1C0089BE0
 * Callers:
 *     <none>
 * Callees:
...
char __fastcall NdisMSendComplete(struct _NDIS_MINIPORT_BLOCK *a1, struct _NDIS_PACKET *a2, unsigned int a3)
{
  ...
  NdisPacketOobOffset = *(_DWORD *)v14 & 0xFFFFFF00;
    ...
    if ( (NdisPacketOobOffset & 0x10) != 0 )
    {
      ...
      else
      {
        ...
        *((_DWORD *)v7 + 2) = 0;
        ...
        if ( *(_MDL **)((char *)&a2[1].Private.Head + v12) )
          LOBYTE(NdisPacketOobOffset) = ndisMSendCompletePacketToNetBufferLists(a1, a2, a3);
        else
