/*
 * XREFs of ?GetObjectsAvailableForProcessing@CSpatialCrossProcessBaseEndpoint@@IEAAJPEAI0@Z @ 0x14005C328
 * Callers:
 ...
 *     ?GetPingPongReadIndex@CSpatialCrossProcessBaseEndpoint@@IEAAJPEAI@Z @ 0x14005C4B8 (-GetPingPongReadIndex@CSpatialCrossProcessBaseEndpoint@@IEAAJPEAI@Z.c)
...
{
  int Index; // ebx
  const struct ControlData *v7; // rdx
  unsigned int v8; // r8d
  unsigned int v9; // eax
  ...
  *a3 = 0;
  *a2 = 0;
  Index = CSpatialCrossProcessBaseEndpoint::ValidateControlData((CSpatialCrossProcessBaseEndpoint *)this, this[112]);
  if ( Index < 0
  ...
  else
  {
    ...
    if ( v9 > v8 )
    {
      Index = -2147418113;
LABEL_8:
      SpatialCPTraceLoggingErrorHelper("CSpatialCrossProcessBaseEndpoint::GetObjectsAvailableForProcessing", 841, Index);
