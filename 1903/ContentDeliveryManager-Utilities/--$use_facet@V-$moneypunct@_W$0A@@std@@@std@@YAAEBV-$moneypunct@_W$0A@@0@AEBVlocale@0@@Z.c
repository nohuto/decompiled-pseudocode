/*
 * XREFs of ??$use_facet@V?$moneypunct@_W$0A@@std@@@std@@YAAEBV?$moneypunct@_W$0A@@0@AEBVlocale@0@@Z @ 0x1800B96B0
 * Callers:
...
struct std::_Facet_base *__fastcall std::use_facet<std::moneypunct<wchar_t,0>>(__int64 *a1)
{
  __int64 v2; // rsi
  ...
  v2 = std::_Facetptr<std::moneypunct<wchar_t,0>>::_Psave;
  v11 = (struct std::_Facet_base *)std::_Facetptr<std::moneypunct<wchar_t,0>>::_Psave;
  v3 = std::moneypunct<wchar_t,0>::id;
  ...
  if ( !v5 )
  {
    ...
    if ( !v5 )
    {
      ...
      {
        if ( std::moneypunct<wchar_t,0>::_Getcat(&v11, a1) == -1 )
        {
          bad_cast::bad_cast((bad_cast *)pExceptionObject, "bad cast");
          throw (bad_cast *)pExceptionObject;
        }
        v5 = v11;
        std::_Facetptr<std::moneypunct<wchar_t,0>>::_Psave = (__int64)v11;
