/*
 * XREFs of ??$AddLinkInternal@VIBitmapRealization@@@CVisualTreeDumpContext@@AEAAXAEBV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@PEAVIBitmapRealization@@_N@Z @ 0x180173C60
 * Callers:
 ...
 *     ??$_Emplace_reallocate@ULink@CTreeDumpNodeData@@@?$vector@ULink@CTreeDumpNodeData@@V?$allocator@ULink@CTreeDumpNodeData@@@std@@@std@@QEAAPEAULink@CTreeDumpNodeData@@QEAU23@$$QEAU23@@Z @ 0x18017447C (--$_Emplace_reallocate@ULink@CTreeDumpNodeData@@@-$vector@ULink@CTreeDumpNodeData@@V-$allocator@.c)
 */

void __fastcall CVisualTreeDumpContext::AddLinkInternal<IBitmapRealization>(_QWORD *a1, __int64 a2, __int64 a3)
{
  ...
  if ( a1[7] == v9 )
  {
    std::vector<CTreeDumpNodeData::Link>::_Emplace_reallocate<CTreeDumpNodeData::Link>(a1 + 5, v9, &v12);
  }
  else
  {
  ...
  if ( a3 )
    CDebugTreeDumper::RegisterNodeForVisit<IBitmapRealization>(a1[14], a3);
}
