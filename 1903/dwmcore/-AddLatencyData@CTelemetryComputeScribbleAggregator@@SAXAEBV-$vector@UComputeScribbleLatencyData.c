/*
 * XREFs of ?AddLatencyData@CTelemetryComputeScribbleAggregator@@SAXAEBV?$vector@UComputeScribbleLatencyData@@V?$allocator@UComputeScribbleLatencyData@@@std@@@std@@@Z @ 0x180193AE8
 * Callers:
...
void __fastcall CTelemetryComputeScribbleAggregator::AddLatencyData(unsigned int **a1)
{
  ...
  AcquireSRWLockExclusive(&stru_18033C900);
  v2 = (__int64)a1[1];
  v3 = *a1;
  v4 = (const char *)(v2 - (_QWORD)v3);
  if ( (unsigned __int64)(*((_QWORD *)&xmmword_18033FFD0 + 1) + (v2 - (__int64)v3) / 24) > 0xA )
  {
  ...
  else
  {
    std::list<ComputeScribbleLatencyData>::_Insert_range<std::_Vector_const_iterator<std::_Vector_val<std::_Simple_types<ComputeScribbleLatencyData>>>>(
      v4,
      (_QWORD *)xmmword_18033FFD0,
      (__int64)v3,
      v2);
  }
  ReleaseSRWLockExclusive(&stru_18033C900);
}
