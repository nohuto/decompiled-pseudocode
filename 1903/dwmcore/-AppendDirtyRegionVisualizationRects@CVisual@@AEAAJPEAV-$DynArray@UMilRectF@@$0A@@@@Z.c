/*
 * XREFs of ?AppendDirtyRegionVisualizationRects@CVisual@@AEAAJPEAV?$DynArray@UMilRectF@@$0A@@@@Z @ 0x1801C2588
 * Callers:
 *     ?DrawDirtyRegionVisualizationForCurrentNode@CDrawingContext@@AEAAJXZ @ 0x18016D1D4 (-DrawDirtyRegionVisualizationForCurrentNode@CDrawingContext@@AEAAJXZ.c)
 * Callees:
 *     ??1?$com_ptr_t@VIRenderTargetBitmap@@Uerr_returncode_policy@wil@@@wil@@QEAA@XZ @ 0x180026730 (--1-$com_ptr_t@VIRenderTargetBitmap@@Uerr_returncode_policy@wil@@@wil@@QEAA@XZ.c)
 *     ??$_Allocate@$0BA@U_Default_allocate_traits@std@@$0A@@std@@YAPEAX_K@Z @ 0x180078318 (--$_Allocate@$0BA@U_Default_allocate_traits@std@@$0A@@std@@YAPEAX_K@Z.c)
 *     ?Alloc@DefaultHeap@@SAPEAX_K@Z @ 0x180080530 (-Alloc@DefaultHeap@@SAPEAX_K@Z.c)
 *     ?AllocClear@DefaultHeap@@SAPEAX_K@Z @ 0x180089A74 (-AllocClear@DefaultHeap@@SAPEAX_K@Z.c)
 *     ??3@YAXPEAX@Z @ 0x1800932DC (--3@YAXPEAX@Z.c)
 *     ?MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJIPEAX@Z @ 0x1800969E0 (-MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJIPEAX@Z.c)
 *     ??0CKeyframeAnimation@@QEAA@PEAVCComposition@@@Z @ 0x1800C0480 (--0CKeyframeAnimation@@QEAA@PEAVCComposition@@@Z.c)
 *     ??4?$com_ptr_t@VCColorBrush@@Uerr_returncode_policy@wil@@@wil@@QEAAAEAV01@PEAVCColorBrush@@@Z @ 0x1800E0C98 (--4-$com_ptr_t@VCColorBrush@@Uerr_returncode_policy@wil@@@wil@@QEAAAEAV01@PEAVCColorBrush@@@Z.c)
 *     __security_check_cookie @ 0x1800E6FB0 (__security_check_cookie.c)
 *     memmove_0 @ 0x1800EC557 (memmove_0.c)
 *     _guard_dispatch_icall_nop @ 0x1800EC570 (_guard_dispatch_icall_nop.c)
 *     ??4?$com_ptr_t@UIDisplayDevice@Core@Display@Devices@Windows@@Uerr_returncode_policy@wil@@@wil@@QEAAAEAV01@PEAUIDisplayDevice@Core@Display@Devices@Windows@@@Z @ 0x18015AA10 (--4-$com_ptr_t@UIDisplayDevice@Core@Display@Devices@Windows@@Uerr_returncode_policy@wil@@@wil@@Q.c)
 *     ??0?$com_ptr_t@UIDXGIAdapter@@Uerr_returncode_policy@wil@@@wil@@QEAA@PEAUIDXGIAdapter@@@Z @ 0x18015BB94 (--0-$com_ptr_t@UIDXGIAdapter@@Uerr_returncode_policy@wil@@@wil@@QEAA@PEAUIDXGIAdapter@@@Z.c)
 *     ?GetDirtyRegionVisualizationData@CVisual@@QEBAPEAV?$deque@V?$tuple@V?$com_ptr_t@VCKeyframeAnimation@@Uerr_returncode_policy@wil@@@wil@@V?$com_ptr_t@VCColorBrush@@Uerr_returncode_policy@wil@@@2@V?$vector@UMilRectF@@V?$allocator@UMilRectF@@@std@@@std@@@std@@V?$allocator@V?$tuple@V?$com_ptr_t@VCKeyframeAnimation@@Uerr_returncode_policy@wil@@@wil@@V?$com_ptr_t@VCColorBrush@@Uerr_returncode_policy@wil@@@2@V?$vector@UMilRectF@@V?$allocator@UMilRectF@@@std@@@std@@@std@@@2@@std@@XZ @ 0x18016FA9C (-GetDirtyRegionVisualizationData@CVisual@@QEBAPEAV-$deque@V-$tuple@V-$com_ptr_t@VCKeyframeAnimat.c)
 *     ?_Buy@?$vector@UMilRectF@@V?$allocator@UMilRectF@@@std@@@std@@AEAA_N_K@Z @ 0x180171BB4 (-_Buy@-$vector@UMilRectF@@V-$allocator@UMilRectF@@@std@@@std@@AEAA_N_K@Z.c)
 *     ?_Tidy@?$vector@UMilRectF@@V?$allocator@UMilRectF@@@std@@@std@@AEAAXXZ @ 0x180171C18 (-_Tidy@-$vector@UMilRectF@@V-$allocator@UMilRectF@@@std@@@std@@AEAAXXZ.c)
 *     ?SetDirtyRegionVisualizationData@CVisual@@QEAAXPEAV?$deque@V?$tuple@V?$com_ptr_t@VCKeyframeAnimation@@Uerr_returncode_policy@wil@@@wil@@V?$com_ptr_t@VCColorBrush@@Uerr_returncode_policy@wil@@@2@V?$vector@UMilRectF@@V?$allocator@UMilRectF@@@std@@@std@@@std@@V?$allocator@V?$tuple@V?$com_ptr_t@VCKeyframeAnimation@@Uerr_returncode_policy@wil@@@wil@@V?$com_ptr_t@VCColorBrush@@Uerr_returncode_policy@wil@@@2@V?$vector@UMilRectF@@V?$allocator@UMilRectF@@@std@@@std@@@std@@@2@@std@@@Z @ 0x1801C4938 (-SetDirtyRegionVisualizationData@CVisual@@QEAAXPEAV-$deque@V-$tuple@V-$com_ptr_t@VCKeyframeAnima.c)
 *     ?push_back@?$deque@V?$tuple@V?$com_ptr_t@VCKeyframeAnimation@@Uerr_returncode_policy@wil@@@wil@@V?$com_ptr_t@VCColorBrush@@Uerr_returncode_policy@wil@@@2@V?$vector@UMilRectF@@V?$allocator@UMilRectF@@@std@@@std@@@std@@V?$allocator@V?$tuple@V?$com_ptr_t@VCKeyframeAnimation@@Uerr_returncode_policy@wil@@@wil@@V?$com_ptr_t@VCColorBrush@@Uerr_returncode_policy@wil@@@2@V?$vector@UMilRectF@@V?$allocator@UMilRectF@@@std@@@std@@@std@@@2@@std@@QEAAX$$QEAV?$tuple@V?$com_ptr_t@VCKeyframeAnimation@@Uerr_returncode_policy@wil@@@wil@@V?$com_ptr_t@VCColorBrush@@Uerr_returncode_policy@wil@@@2@V?$vector@UMilRectF@@V?$allocator@UMilRectF@@@std@@@std@@@2@@Z @ 0x1801C4F88 (-push_back@-$deque@V-$tuple@V-$com_ptr_t@VCKeyframeAnimation@@Uerr_returncode_policy@wil@@@wil@@.c)
 *     ?Initialize@CKeyframeAnimation@@QEAAJPEAVCResource@@IW4DCOMPOSITION_EXPRESSION_TYPE@@W4Enum@CompositionColorSpace@@W4SubchannelMaskType@@E_KW44KeyframeAnimationDelayBehavior@@MW44KeyframeAnimationDirection@@MW44KeyframeAnimationStopBehavior@@IPEAUKeyframeData@@@Z @ 0x1801E1520 (-Initialize@CKeyframeAnimation@@QEAAJPEAVCResource@@IW4DCOMPOSITION_EXPRESSION_TYPE@@W4Enum@Comp.c)
 */

__int64 __fastcall CVisual::AppendDirtyRegionVisualizationRects(__int64 a1, __int64 a2)
{
  unsigned int v4; // esi
  void *v5; // r14
  _QWORD *v6; // rax
  __int64 v7; // rdx
  __int64 v8; // rcx
  __int64 v9; // rbx
  int v10; // eax
  __int64 v11; // rcx
  _QWORD *v12; // rax
  __int128 v13; // xmm0
  CKeyframeAnimation *v14; // rax
  CKeyframeAnimation *v15; // rax
  __int64 v16; // rcx
  _QWORD *v17; // rdi
  _QWORD *v18; // r15
  _OWORD *v19; // rax
  unsigned int v20; // r9d
  __int64 DirtyRegionVisualizationData; // rax
  const void *v22; // r13
  unsigned __int64 v23; // r15
  char v24; // al
  char *v25; // r12
  size_t v26; // r15
  char *v27; // rax
  __int64 v28; // rax
  unsigned int v30; // [rsp+28h] [rbp-E0h]
  unsigned int v31; // [rsp+28h] [rbp-E0h]
  _QWORD *v32; // [rsp+88h] [rbp-80h] BYREF
  __int64 v33; // [rsp+90h] [rbp-78h] BYREF
  void *v34[2]; // [rsp+98h] [rbp-70h] BYREF
  __int64 v35; // [rsp+A8h] [rbp-60h]
  __int128 v36; // [rsp+B0h] [rbp-58h]
  _QWORD v37[3]; // [rsp+C0h] [rbp-48h] BYREF
  __int64 v38; // [rsp+D8h] [rbp-30h] BYREF
  _QWORD *v39; // [rsp+E0h] [rbp-28h] BYREF
  __int128 v40; // [rsp+E8h] [rbp-20h] BYREF

  v4 = 0;
  v5 = 0LL;
  v33 = 0LL;
  v32 = 0LL;
  if ( !*(_DWORD *)(a2 + 24) )
    goto LABEL_34;
  v6 = DefaultHeap::AllocClear(0x60uLL);
  v7 = (__int64)v6;
  if ( v6 )
  {
    v6[2] = *(_QWORD *)(a1 + 16);
    *((_DWORD *)v6 + 2) = 0;
    v6[3] = 0LL;
    v6[5] = 0LL;
    *((_DWORD *)v6 + 8) = 0;
    v6[8] = 0LL;
    *v6 = &CColorBrush::`vftable'{for `CContent'};
    v6[7] = &CLinearGradientBrush::`vftable'{for `IVisualListenerInfoProvider'};
    *((_BYTE *)v6 + 72) = 0;
  }
  else
  {
    v7 = 0LL;
  }
  wil::com_ptr_t<CColorBrush,wil::err_returncode_policy>::operator=(&v33, v7);
  v9 = v33;
  if ( !v33 )
  {
    v30 = 6244;
LABEL_7:
    v4 = -2147024882;
    MilInstrumentationCheckHR_MaybeFailFast(v8, 0LL, 0, 0x8007000E, v30, 0LL);
    goto LABEL_34;
  }
  v40 = _xmm;
  v10 = (*(__int64 (__fastcall **)(__int64, _QWORD, __int64, __int128 *))(*(_QWORD *)v33 + 88LL))(v33, 0LL, 70LL, &v40);
  v4 = v10;
  if ( v10 < 0 )
  {
    v31 = 6246;
    goto LABEL_30;
  }
  v12 = DefaultHeap::Alloc(0x60uLL);
  v5 = v12;
  if ( !v12 )
  {
    v30 = 6249;
    goto LABEL_7;
  }
  *v12 = 0LL;
  *((_DWORD *)v12 + 6) = 3;
  v13 = *(_OWORD *)(v9 + 80);
  v12[6] = 1065353216LL;
  *((_DWORD *)v12 + 18) = 1;
  v36 = v13;
  HIDWORD(v36) = 0;
  *((_OWORD *)v12 + 5) = v36;
  v14 = (CKeyframeAnimation *)DefaultHeap::AllocClear(0x220uLL);
  if ( v14 )
    v15 = CKeyframeAnimation::CKeyframeAnimation(v14, *(struct CComposition **)(a1 + 16));
  else
    v15 = 0LL;
  wil::com_ptr_t<Windows::Devices::Display::Core::IDisplayDevice,wil::err_returncode_policy>::operator=(
    (__int64 *)&v32,
    (__int64)v15);
  v17 = v32;
  if ( !v32 )
  {
    v4 = -2147024882;
    MilInstrumentationCheckHR_MaybeFailFast(v16, 0LL, 0, 0x8007000E, 0x1876u, 0LL);
    goto LABEL_32;
  }
  v10 = CKeyframeAnimation::Initialize(v32, v9, 0LL, 70LL);
  v4 = v10;
  if ( v10 < 0 )
  {
    v31 = 6278;
LABEL_30:
    v20 = v10;
    goto LABEL_31;
  }
  if ( (**(_DWORD **)(a1 + 224) & 0x40000) != 0 )
  {
LABEL_24:
    DirtyRegionVisualizationData = CVisual::GetDirtyRegionVisualizationData(a1);
    v22 = *(const void **)a2;
    v35 = 0LL;
    v23 = *(unsigned int *)(a2 + 24);
    *(_QWORD *)&v36 = DirtyRegionVisualizationData;
    *(_OWORD *)v34 = 0LL;
    v24 = std::vector<MilRectF>::_Buy((__int64)v34, v23);
    v25 = (char *)v34[0];
    if ( v24 )
    {
      v26 = 16 * v23;
      memmove_0(v34[0], v22, v26);
      v27 = &v25[v26];
    }
    else
    {
      v27 = (char *)v34[1];
    }
    v37[1] = v27;
    v37[2] = v35;
    v37[0] = v25;
    *(_OWORD *)v34 = 0LL;
    v35 = 0LL;
    wil::com_ptr_t<IDXGIAdapter,wil::err_returncode_policy>::com_ptr_t<IDXGIAdapter,wil::err_returncode_policy>(
      &v38,
      v9);
    v28 = *v17;
    v39 = v17;
    (*(void (__fastcall **)(_QWORD *))(v28 + 8))(v17);
    std::deque<std::tuple<wil::com_ptr_t<CKeyframeAnimation,wil::err_returncode_policy>,wil::com_ptr_t<CColorBrush,wil::err_returncode_policy>,std::vector<MilRectF>>>::push_back(
      v36,
      v37);
    wil::com_ptr_t<IRenderTargetBitmap,wil::err_returncode_policy>::~com_ptr_t<IRenderTargetBitmap,wil::err_returncode_policy>((__int64 *)&v39);
    wil::com_ptr_t<IRenderTargetBitmap,wil::err_returncode_policy>::~com_ptr_t<IRenderTargetBitmap,wil::err_returncode_policy>(&v38);
    std::vector<MilRectF>::_Tidy((__int64)v37);
    std::vector<MilRectF>::_Tidy((__int64)v34);
    goto LABEL_32;
  }
  v18 = DefaultHeap::Alloc(0x28uLL);
  if ( v18 )
  {
    *v18 = 0LL;
    v18[1] = 0LL;
    v18[2] = 0LL;
    v18[3] = 0LL;
    v18[4] = 0LL;
    v19 = (_OWORD *)std::_Allocate<16,std::_Default_allocate_traits,0>(0x10uLL);
    *v18 = v19;
    *v19 = 0LL;
    *(_QWORD *)*v18 = v18;
  }
  else
  {
    v18 = 0LL;
  }
  if ( v18 )
  {
    CVisual::SetDirtyRegionVisualizationData(a1, v18);
    goto LABEL_24;
  }
  v20 = -2147024882;
  v4 = -2147024882;
  v31 = 6283;
LABEL_31:
  MilInstrumentationCheckHR_MaybeFailFast(v11, 0LL, 0, v20, v31, 0LL);
LABEL_32:
  if ( v5 )
    operator delete(v5);
LABEL_34:
  wil::com_ptr_t<IRenderTargetBitmap,wil::err_returncode_policy>::~com_ptr_t<IRenderTargetBitmap,wil::err_returncode_policy>((__int64 *)&v32);
  wil::com_ptr_t<IRenderTargetBitmap,wil::err_returncode_policy>::~com_ptr_t<IRenderTargetBitmap,wil::err_returncode_policy>(&v33);
  return v4;
}
