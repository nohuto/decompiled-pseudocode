/*
 * XREFs of sub_18006ED90 @ 0x18006ED90
 * Callers:
 *     sub_18006ED90 @ 0x18006ED90 (sub_18006ED90.c)
 *     sub_18006EE48 @ 0x18006EE48 (sub_18006EE48.c)
 * Callees:
 *     sub_180063430 @ 0x180063430 (sub_180063430.c)
 *     sub_18006ED90 @ 0x18006ED90 (sub_18006ED90.c)
 *     sub_180087340 @ 0x180087340 (sub_180087340.c)
 *     ?_Xbad_function_call@std@@YAXXZ @ 0x1801273F8 (-_Xbad_function_call@std@@YAXXZ.c)
 *     _guard_dispatch_icall_nop @ 0x180127630 (_guard_dispatch_icall_nop.c)
 */

__int64 __fastcall sub_18006ED90(__int64 a1, __int64 a2, _QWORD *a3)
{
  __int64 v6; // rbx
  __int64 v7; // rdi
  __int64 v8; // rbx
  __int64 v9; // rcx
  __int64 v10; // rbx
  __int64 result; // rax
  __int64 v12; // rdi
  __int64 i; // rbx

  sub_180063430(a1 + 16);
  v6 = *a3;
  sub_180087340(*a3);
  v7 = *(_QWORD *)(v6 + 376);
  v8 = *(_QWORD *)(v6 + 384);
  while ( v7 != v8 )
  {
    v9 = *(_QWORD *)(a2 + 56);
    if ( !v9 )
    {
      std::_Xbad_function_call();
      JUMPOUT(0x18006EE44LL);
    }
    (*(void (__fastcall **)(__int64, __int64))(*(_QWORD *)v9 + 16LL))(v9, v7);
    v7 += 16LL;
  }
  v10 = *a3;
  result = sub_180087340(*a3);
  v12 = *(_QWORD *)(v10 + 40);
  for ( i = *(_QWORD *)(v10 + 32); i != v12; i += 16LL )
    result = sub_18006ED90(a1, a2, i);
  return result;
}
