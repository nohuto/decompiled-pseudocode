/*
 * XREFs of VidSchiProcessIsrSuspendContextCompleted @ 0x1C00390A8
 * Callers:
...
const signed __int64 *__fastcall VidSchiProcessIsrSuspendContextCompleted(union _SLIST_HEADER *a1, __int64 a2)
{
  ...
  result = (const signed __int64 *)a1[36].Alignment;
  Next = v4[1].Next;
  if ( !_bittest64(result, WORD2(Next->Next)) )
  {
    ...
    if ( (_DWORD)result != 1 )
    {
      ...
      v9 = ExpInterlockedPopEntrySList((PSLIST_HEADER)&Next[500]);
      if ( v9 )
      {
        v9[2].Next = v4;
        *((_QWORD *)&v9[2].Next + 1) = *(_QWORD *)(a2 + 16);
        LODWORD(v9->Next) = 17;
        return (const signed __int64 *)ExpInterlockedPushEntrySList(a1 + 106, v9 + 1);
      }
      else
      {
        v12 = WdLogNewEntry5_WdAssertion(v11, v10);
        *(_QWORD *)(v12 + 24) = WORD2(Next->Next);
        return (const signed __int64 *)WdLogEvent5_WdAssertion(v12);
      }
    }
  }
  return result;
