/*
 * XREFs of NtGdiGetPath @ 0x1C02A6930
 * Callers:
 *     <none>
 * Callees:
 *     ??0DCOBJ@@QEAA@PEAUHDC__@@@Z @ 0x1C0061358 (--0DCOBJ@@QEAA@PEAUHDC__@@@Z.c)
 *     ??1DCOBJ@@QEAA@XZ @ 0x1C0079AE0 (--1DCOBJ@@QEAA@XZ.c)
 *     ?vQuickInit@EXFORMOBJ@@QEAAXAEAVXDCOBJ@@K@Z @ 0x1C0102880 (-vQuickInit@EXFORMOBJ@@QEAAXAEAVXDCOBJ@@K@Z.c)
 *     ??1XEPATHOBJ@@QEAA@XZ @ 0x1C0149454 (--1XEPATHOBJ@@QEAA@XZ.c)
 *     ?bInactive@DC@@QEBAHXZ @ 0x1C01495F4 (-bInactive@DC@@QEBAHXZ.c)
 *     ??0XEPATHOBJ@@QEAA@PEAUHPATH__@@@Z @ 0x1C02833CC (--0XEPATHOBJ@@QEAA@PEAUHPATH__@@@Z.c)
 */

__int64 __fastcall NtGdiGetPath(HDC a1, struct _POINTL *Address, char *a3, unsigned int a4)
{
  __int64 v4; // rsi
  unsigned int v7; // edi
  int v8; // eax
  char *v9; // r12
  char *v10; // rsi
  int v11; // r14d
  char *v12; // r8
  char flags; // cl
  char v14; // dl
  ULONG v15; // ecx
  struct _PATHDATA v17; // [rsp+20h] [rbp-E8h] BYREF
  int v18; // [rsp+30h] [rbp-D8h]
  DC *v19[2]; // [rsp+38h] [rbp-D0h] BYREF
  _QWORD v20[3]; // [rsp+48h] [rbp-C0h] BYREF
  int v21; // [rsp+60h] [rbp-A8h] BYREF
  __int64 v22; // [rsp+68h] [rbp-A0h]

  v4 = a4;
  v7 = -1;
  DCOBJ::DCOBJ((DCOBJ *)v19, a1);
  if ( !v19[0] || (int)v4 < 0 )
    goto LABEL_29;
  if ( !(unsigned int)DC::bInactive(v19[0]) )
  {
    v15 = 1003;
LABEL_30:
    EngSetLastError(v15);
    goto LABEL_31;
  }
  EXFORMOBJ::vQuickInit((EXFORMOBJ *)v20, (struct XDCOBJ *)v19, 1026);
  if ( !v20[0] )
  {
LABEL_29:
    v15 = 87;
    goto LABEL_30;
  }
  XEPATHOBJ::XEPATHOBJ((XEPATHOBJ *)&v21, *((struct HPATH__ **)v19[0] + 25));
  if ( v22 )
  {
    v8 = EPATHOBJ::cTotalPts((EPATHOBJ *)&v21);
    v7 = v8;
    if ( !(_DWORD)v4 )
      goto LABEL_27;
    if ( (int)v4 >= v8 && (unsigned __int64)(int)v4 <= 0x1FFFFFFF )
    {
      *(_QWORD *)&v17.flags = 0LL;
      v17.pptfx = 0LL;
      v21 &= ~8u;
      *(_QWORD *)(v22 + 72) = *(_QWORD *)(v22 + 32);
      ProbeForWrite(Address, 8LL * (int)v4, 4u);
      ProbeForWrite(a3, (int)v4, 4u);
      v9 = a3;
      v10 = &a3[v4];
      while ( 1 )
      {
        v11 = EPATHOBJ::bEnum((EPATHOBJ *)&v21, &v17);
        if ( v17.count )
        {
          if ( !EXFORMOBJ::bXform((EXFORMOBJ *)v20, v17.pptfx, Address, v17.count) )
          {
            EngSetLastError(0x216u);
            v7 = -1;
            v18 = -1;
            goto LABEL_27;
          }
          Address += v17.count;
          v12 = &a3[v17.count];
          flags = v17.flags;
          if ( (v17.flags & 1) != 0 && a3 < v10 )
          {
            *a3++ = 6;
            flags = v17.flags;
          }
          v14 = (flags & 0x10) != 0 ? 4 : 2;
          if ( v12 <= v10 )
          {
            while ( a3 < v12 )
            {
              *a3++ = v14;
              flags = v17.flags;
            }
          }
          if ( (flags & 8) != 0 && a3 > v9 && a3 <= v10 )
            *(a3 - 1) |= 1u;
        }
        if ( !v11 )
          goto LABEL_27;
      }
    }
    EngSetLastError(0x57u);
    v7 = -1;
  }
  else
  {
    EngSetLastError(0x57u);
  }
LABEL_27:
  XEPATHOBJ::~XEPATHOBJ((XEPATHOBJ *)&v21);
LABEL_31:
  DCOBJ::~DCOBJ((DCOBJ *)v19);
  return v7;
}
