/*
 * XREFs of NtUserSetProp @ 0x1C0102500
 * Callers:
 *     <none>
 * Callees:
 *     SetSharedPropForFilteredProcesses @ 0x1C0005FF0 (SetSharedPropForFilteredProcesses.c)
 *     UserGetGlobalAtomTableOfWindow @ 0x1C01026E4 (UserGetGlobalAtomTableOfWindow.c)
 *     __security_check_cookie @ 0x1C0162AB0 (__security_check_cookie.c)
 *     memset @ 0x1C0166840 (memset.c)
 */

__int64 __fastcall NtUserSetProp(HWND a1, int a2, __int64 a3)
{
  int v5; // edi
  struct tagWND *v7; // rbx
  char *v8; // r15
  __int16 AtomFromAtomTable; // ax
  __int64 v10; // rcx
  unsigned int DLT; // eax
  __int64 GlobalAtomTableOfWindow; // rax
  int v14; // ebx
  _BYTE v15[16]; // [rsp+30h] [rbp-78h] BYREF
  struct _KAPC_STATE ApcState; // [rsp+40h] [rbp-68h] BYREF

  v5 = 0;
  EnterCrit(0LL, 1LL);
  CHMRefHwndByHandle::CHMRefHwndByHandle((CHMRefHwndByHandle *)v15, a1, 1, 1, 1, 1);
  if ( CHMRefHwndByHandle::bValid((CHMRefHwndByHandle *)v15) )
  {
    v7 = CHMRefHwndByHandle::rpwnd((CHMRefHwndByHandle *)v15);
    v8 = (char *)v7 + 144;
    AtomFromAtomTable = word_1C032C66C;
    if ( !word_1C032C66C )
    {
      DLT = DLT_JOB::getDLT();
      GetDomainLockRef(DLT);
      GlobalAtomTableOfWindow = UserGetGlobalAtomTableOfWindow(v7);
      if ( GlobalAtomTableOfWindow )
      {
        AtomFromAtomTable = UserFindAtomFromAtomTable(
                              GlobalAtomTableOfWindow,
                              L"Microsoft.Windows.WindowFactory.ViewId");
        word_1C032C66C = AtomFromAtomTable;
      }
      else
      {
        AtomFromAtomTable = word_1C032C66C;
      }
    }
    if ( (_WORD)a2 == AtomFromAtomTable )
    {
      *(_QWORD *)(*((_QWORD *)v7 + 5) + 312LL) = a3;
    }
    else if ( (*((_DWORD *)v7 + 82) & 0x10) != 0 )
    {
      memset(&ApcState, 0, sizeof(ApcState));
      KeStackAttachProcess(**(PRKPROCESS **)(*((_QWORD *)v7 + 2) + 416LL), &ApcState);
      v14 = SetSharedPropForFilteredProcesses((__int64)v7, a2, a3);
      KeUnstackDetachProcess(&ApcState);
      if ( v14 == 1 )
      {
        v5 = 1;
        goto LABEL_6;
      }
    }
    v5 = RealInternalSetProp(v8, (unsigned __int16)a2, a3, HIWORD(a2) != 0 ? 2 : 0);
  }
LABEL_6:
  CHMRefHwndByHandle::~CHMRefHwndByHandle((CHMRefHwndByHandle *)v15);
  UserSessionSwitchLeaveCrit(v10);
  return v5;
}
