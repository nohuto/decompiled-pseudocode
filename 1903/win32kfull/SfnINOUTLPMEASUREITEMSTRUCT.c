/*
 * XREFs of SfnINOUTLPMEASUREITEMSTRUCT @ 0x1C0156340
 * Callers:
 *     <none>
 * Callees:
 *     W32GetThreadWin32Thread @ 0x1C0062084 (W32GetThreadWin32Thread.c)
 *     UserSetLastError @ 0x1C008B3AC (UserSetLastError.c)
 *     ??1ReleaseAndReacquirePerObjectLocks@@QEAA@XZ @ 0x1C00ADED4 (--1ReleaseAndReacquirePerObjectLocks@@QEAA@XZ.c)
 *     ??1LeaveEnterCritProperDisposition@@QEAA@XZ @ 0x1C00ADF74 (--1LeaveEnterCritProperDisposition@@QEAA@XZ.c)
 *     ??0LeaveEnterCritProperDisposition@@QEAA@XZ @ 0x1C00ADFAC (--0LeaveEnterCritProperDisposition@@QEAA@XZ.c)
 *     ??0ReleaseAndReacquirePerObjectLocks@@QEAA@XZ @ 0x1C00AE614 (--0ReleaseAndReacquirePerObjectLocks@@QEAA@XZ.c)
 *     __security_check_cookie @ 0x1C0162AB0 (__security_check_cookie.c)
 */

__int64 __fastcall SfnINOUTLPMEASUREITEMSTRUCT(
        _QWORD *a1,
        __int64 a2,
        __int64 a3,
        __int64 a4,
        __int64 a5,
        __int64 a6,
        char a7,
        __int64 a8)
{
  int v10; // r15d
  __int64 ThreadWin32Thread; // rax
  __int64 v13; // rdx
  __int64 v14; // r8
  __int64 v15; // r9
  __int64 v16; // rsi
  __int64 v17; // rdi
  __int64 v18; // rax
  __int64 v19; // r8
  __int64 v20; // r9
  __int64 v21; // rax
  __int64 v22; // rcx
  __int64 v23; // rcx
  int v24; // ebx
  __int64 v25; // rdx
  __int64 v26; // r8
  __int64 v27; // r9
  __int64 v28; // rdx
  __int64 v29; // rcx
  __int64 v30; // r8
  __int64 v31; // rcx
  __int64 *v32; // rcx
  __int64 v33; // r8
  __int64 v34; // rdx
  _OWORD *v35; // rcx
  __int128 v36; // xmm1
  char v38; // [rsp+30h] [rbp-118h] BYREF
  _BYTE v39[3]; // [rsp+31h] [rbp-117h] BYREF
  _DWORD v40[5]; // [rsp+34h] [rbp-114h] BYREF
  unsigned __int64 v41; // [rsp+48h] [rbp-100h] BYREF
  __int64 v42; // [rsp+58h] [rbp-F0h] BYREF
  _QWORD *v43; // [rsp+60h] [rbp-E8h]
  __int64 v44; // [rsp+68h] [rbp-E0h]
  __int128 v45; // [rsp+70h] [rbp-D8h]
  __int64 v46; // [rsp+80h] [rbp-C8h]
  __int64 v47; // [rsp+88h] [rbp-C0h]
  __int128 v48; // [rsp+B0h] [rbp-98h]
  __int64 v49; // [rsp+C0h] [rbp-88h] BYREF
  int v50; // [rsp+C8h] [rbp-80h]
  int v51; // [rsp+CCh] [rbp-7Ch]
  __int64 v52; // [rsp+D0h] [rbp-78h]
  __int128 v53; // [rsp+D8h] [rbp-70h]
  __int128 v54; // [rsp+E8h] [rbp-60h]
  __int64 v55; // [rsp+F8h] [rbp-50h]
  __int64 v56; // [rsp+100h] [rbp-48h]

  v10 = a2;
  v42 = 0LL;
  v43 = 0LL;
  v44 = 0LL;
  v45 = 0uLL;
  v46 = 0LL;
  ThreadWin32Thread = W32GetThreadWin32Thread((__int64)KeGetCurrentThread(), a2, a3, a4);
  v16 = ThreadWin32Thread;
  if ( a1 )
    v17 = a1[5] - *(_QWORD *)(ThreadWin32Thread + 464);
  else
    v17 = 0LL;
  v51 = 0;
  v49 = v17;
  v50 = v10 & 0x1FFFF;
  v52 = a3;
  *(_DWORD *)(a4 + 20) = 0;
  v53 = *(_OWORD *)a4;
  v54 = *(_OWORD *)(a4 + 16);
  v55 = a5;
  v56 = a6;
  v18 = W32GetThreadWin32Thread((__int64)KeGetCurrentThread(), v13, v14, v15);
  v42 = *(_QWORD *)(v18 + 408);
  *(_QWORD *)(v18 + 408) = &v42;
  v43 = a1;
  if ( a1 )
    HMLockObject(a1);
  v21 = *(_QWORD *)(v16 + 472);
  v48 = *(_OWORD *)(v21 + 64);
  v45 = v48;
  v46 = *(_QWORD *)(v21 + 80);
  *(_QWORD *)(v21 + 72) = v17;
  if ( a1 )
    v22 = *a1;
  else
    v22 = 0LL;
  *(_QWORD *)(*(_QWORD *)(v16 + 472) + 64LL) = v22;
  if ( a1 )
    v23 = *(_QWORD *)(a1[5] + 224LL);
  else
    v23 = 0LL;
  *(_QWORD *)(*(_QWORD *)(v16 + 472) + 80LL) = v23;
  if ( gdwInAtomicOperation && (gdwExtraInstrumentations & 1) != 0 )
    KeBugCheckEx(0x160u, gdwInAtomicOperation, 0LL, 0LL, 0LL);
  ReleaseAndReacquirePerObjectLocks::ReleaseAndReacquirePerObjectLocks(
    (ReleaseAndReacquirePerObjectLocks *)v39,
    gdwInAtomicOperation,
    v19,
    v20);
  LeaveEnterCritProperDisposition::LeaveEnterCritProperDisposition((LeaveEnterCritProperDisposition *)&v38);
  EtwTraceBeginCallback(16LL);
  v24 = KeUserModeCallback(16LL, &v49, 72LL, &v41, v40);
  EtwTraceEndCallback(16LL);
  LeaveEnterCritProperDisposition::~LeaveEnterCritProperDisposition((LeaveEnterCritProperDisposition *)&v38);
  ReleaseAndReacquirePerObjectLocks::~ReleaseAndReacquirePerObjectLocks(
    (ReleaseAndReacquirePerObjectLocks *)v39,
    v25,
    v26,
    v27);
  ThreadUnlock1(v29, v28, v30);
  v31 = *(_QWORD *)(v16 + 472);
  *(_OWORD *)(v31 + 64) = v48;
  *(_QWORD *)(v31 + 80) = v46;
  if ( v24 >= 0 && v40[0] == 24 )
  {
    v32 = (__int64 *)v41;
    if ( v41 + 8 < v41 || v41 + 8 > MmUserProbeAddress )
      v32 = (__int64 *)MmUserProbeAddress;
    v33 = *v32;
    v47 = *v32;
    if ( a8 && (*(_DWORD *)(a8 + 84) & 5) != 0 || (a7 & 2) != 0 )
      return v33;
    v34 = *(_QWORD *)(gptiCurrent + 504LL);
    if ( !v34 || (*(_DWORD *)(v34 + 84) & 1) == 0 || *(_QWORD *)(v34 + 96) != a4 )
    {
      v35 = *(_OWORD **)(v41 + 16);
      if ( v35 + 2 < v35 || (unsigned __int64)(v35 + 2) > MmUserProbeAddress )
        v35 = (_OWORD *)MmUserProbeAddress;
      v36 = v35[1];
      *(_OWORD *)a4 = *v35;
      *(_OWORD *)(a4 + 16) = v36;
      return v33;
    }
  }
  return 0LL;
}
