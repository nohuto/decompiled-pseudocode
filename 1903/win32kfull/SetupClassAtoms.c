/*
 * XREFs of SetupClassAtoms @ 0x1C0135E60
 * Callers:
 *     <none>
 * Callees:
 *     <none>
...
__int64 SetupClassAtoms()
{
  ...
  *(&WPP_MAIN_CB.DeviceQueue.Size + 1) = UserAddAtomEx(L"SysCP", 1LL, 2LL);
  if ( *(&WPP_MAIN_CB.DeviceQueue.Size + 1) )
    v9 = v8 != 0 ? v7 : 0;
  else
    LOBYTE(v9) = 0;
  ...
  if ( atomDWMProp )
    ...
    v17 = 0;
  LOWORD(WPP_MAIN_CB.Queue.Wcb.CurrentIrp) = UserAddAtomEx(L"SysSwapChain", 1LL, 2LL);
  v18 = (LOWORD(WPP_MAIN_CB.Queue.Wcb.CurrentIrp) != 0) & v17;
  ...
  *(_WORD *)(gpsi + 914LL) = gatomShadow;
  ...
  WORD2(WPP_MAIN_CB.DeviceQueue.DeviceListHead.Blink) = UserAddAtomEx(L"BrokeredForeground", 1LL, 2LL);
