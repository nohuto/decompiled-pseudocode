/*
 * XREFs of ?IsCorrectVersionRegistered@_FX_DRIVER_GLOBALS@@QEAAEPEBU_UNICODE_STRING@@@Z @ 0x1C005BA2C
 * Callers:
  ...
  if ( ZwOpenKey(&hDriver.m_Key, 0x20019u, &ObjectAttributes) >= 0 )
  {
    *(&ObjectAttributes.Length + 1) = 0;
    ...
    if ( ZwOpenKey(&hWdf.m_Key, 0x20019u, &ObjectAttributes) >= 0
      && CUmdfSvcConfig::_QueryULong(hWdf.m_Key, &wdfMajorValue, &registeredMajor) >= 0
      && registeredMajor == this->WdfBindInfo->Version.Major
      && CUmdfSvcConfig::_QueryULong(hWdf.m_Key, &wdfMinorValue, &registeredMinor) >= 0
