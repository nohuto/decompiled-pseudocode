/*
 * XREFs of ?PowerPolicySetSxWakeSettings@FxPkgPnp@@QEAAJPEAU_WDF_DEVICE_POWER_POLICY_WAKE_SETTINGS@@EE@Z @ 0x1C0085AC8
 * Callers:
...
LABEL_8:
  ...
  if ( KeGetCurrentIrql() )
  {
    WPP_IFR_SF_(this->m_Globals, 3u, 0xCu, 0x39u, WPP_FxPkgPnp_cpp_Traceguids);
  }
  else
  {
    v23 = aWdfdefaultwake[28];
...
LABEL_9:
  ...
  if ( this->m_CapsQueried || KeGetCurrentIrql() || (result = FxPkgPnp::QueryForCapabilities(this), result >= 0) )
  {
    ...
    if ( DxState == PowerDeviceMaximum )
    {
      ...
      {
        WPP_IFR_SF_DD(this->m_Globals, v14, 0xCu, 0x3Au, WPP_FxPkgPnp_cpp_Traceguids, _a2, 0xC00002D3);
        return -1073741101;
      }
    }
    else if ( DxState > _a2 )
