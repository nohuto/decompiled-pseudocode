/*
 * XREFs of FxLibraryGlobalsQueryRegistrySettings @ 0x1C0059C64
 * Callers:
...
int FxLibraryGlobalsQueryRegistrySettings()
{
  ...
  *(_QWORD *)&dfxOptInName.Length = 4063292LL;
  ...
  if ( result >= 0 )
  {
    if ( CUmdfSvcConfig::_QueryULong(hWdf.m_Key, &ifrDisabledName, &ifrDisabled) >= 0 && ifrDisabled == 1 )
      unk_1C00AEEF0 = 1;
    unk_1C00AEEF1 = 0;
    if ( CUmdfSvcConfig::_QueryULong(hWdf.m_Key, &ssDisabledName, &ssDisabled) >= 0 && ssDisabled == 1 )
      unk_1C00AEEF1 = 1;
    result = CUmdfSvcConfig::_QueryULong(hWdf.m_Key, &dfxOptInName, &dfxOptIn);
    if ( result >= 0 )
      unk_1C00AEEF2 = dfxOptIn == 1;
  }
  if ( hWdf.m_Key )
