/*
 * XREFs of FxRegisterBugCheckCallback @ 0x1C0094CB8
 * Callers:
 *     FxInitialize @ 0x1C00595DC (FxInitialize.c)
 * Callees:
 *     memset @ 0x1C001B300 (memset.c)
 *     ?Register@_FX_DRIVER_TRACKER_CACHE_AWARE@@QEAAJPEAU_FX_DRIVER_GLOBALS@@@Z @ 0x1C0094760 (-Register@_FX_DRIVER_TRACKER_CACHE_AWARE@@QEAAJPEAU_FX_DRIVER_GLOBALS@@@Z.c)
...
void __fastcall FxRegisterBugCheckCallback(_FX_DRIVER_GLOBALS *FxDriverGlobals, _DRIVER_OBJECT *DriverObject)
{
  ...
  *(_QWORD *)&funcName.Length = 0LL;
  ...
  if ( FxpGetImageBase(DriverObject, &FxDriverGlobals->ImageAddress, &FxDriverGlobals->ImageSize) >= 0 && !unk_1C00AEEAA )
  {
    ...
    if ( SystemRoutineAddress )
    {
      ...
      {
        if ( (int)_FX_DRIVER_TRACKER_CACHE_AWARE::Register(&stru_1C00AEE88, v6) >= 0 )
