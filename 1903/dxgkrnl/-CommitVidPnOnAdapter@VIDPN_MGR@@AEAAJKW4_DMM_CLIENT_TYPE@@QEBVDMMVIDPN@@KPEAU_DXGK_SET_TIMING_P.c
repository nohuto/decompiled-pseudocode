/*
 * XREFs of ?CommitVidPnOnAdapter@VIDPN_MGR@@AEAAJKW4_DMM_CLIENT_TYPE@@QEBVDMMVIDPN@@KPEAU_DXGK_SET_TIMING_PATH_INFO@@PEAIPEAUD3DKMT_VIDPN_SOURCE_MASKS@@AEAK3PEAU_DXGK_SET_TIMING_RESULTS@@PEAVDXGDEVICE@@PEAVCOREDEVICEACCESS@@E@Z @ 0x1C012D788
 * Callers:
 *     ?SetTimingsFromVidPn@VIDPN_MGR@@QEAAJKW4_DMM_CLIENT_TYPE@@PEAVDMMVIDPN@@PEAUD3DKMT_VIDPN_SOURCE_MASKS@@PEAU_DMM_SET_TIMING_RESULT@@EPEAVDXGDEVICE@@PEAVCOREDEVICEACCESS@@@Z @ 0x1C01253E8 (-SetTimingsFromVidPn@VIDPN_MGR@@QEAAJKW4_DMM_CLIENT_TYPE@@PEAVDMMVIDPN@@PEAUD3DKMT_VIDPN_SOURCE_.c)
 * Callees:
 *     ?GetDisplayModeInfo@ADAPTER_DISPLAY@@QEBAQEBU_DXGK_DISPLAYMODE_INFO@@I@Z @ 0x1C0001908 (-GetDisplayModeInfo@ADAPTER_DISPLAY@@QEBAQEBU_DXGK_DISPLAYMODE_INFO@@I@Z.c)
 *     ?DxgkLogCodePointPacket@@YAXW4_DXGK_DIAG_CODE_POINT_TYPE@@IIIU_LUID@@@Z @ 0x1C0002D34 (-DxgkLogCodePointPacket@@YAXW4_DXGK_DIAG_CODE_POINT_TYPE@@IIIU_LUID@@@Z.c)
 *     ?IsVidPnSourceVisible@ADAPTER_DISPLAY@@QEBAEI@Z @ 0x1C0003AD8 (-IsVidPnSourceVisible@ADAPTER_DISPLAY@@QEBAEI@Z.c)
 *     ?IsVidPnSourceActive@ADAPTER_DISPLAY@@QEBAEI@Z @ 0x1C0003B14 (-IsVidPnSourceActive@ADAPTER_DISPLAY@@QEBAEI@Z.c)
 *     ?D3DKMDT_VPPR_GET_CONTENT_ROTATION@@YA?AW4_D3DKMDT_VIDPN_PRESENT_PATH_ROTATION@@W41@@Z @ 0x1C0003CE0 (-D3DKMDT_VPPR_GET_CONTENT_ROTATION@@YA-AW4_D3DKMDT_VIDPN_PRESENT_PATH_ROTATION@@W41@@Z.c)
 *     ?PinContentRotation@DMMVIDPNPRESENTPATH@@QEAAJW4_D3DKMDT_VIDPN_PRESENT_PATH_ROTATION@@@Z @ 0x1C0003E38 (-PinContentRotation@DMMVIDPNPRESENTPATH@@QEAAJW4_D3DKMDT_VIDPN_PRESENT_PATH_ROTATION@@@Z.c)
 *     ?reset@?$auto_rc@VDMMVIDPN@@@@QEAAXPEAVDMMVIDPN@@@Z @ 0x1C0004418 (-reset@-$auto_rc@VDMMVIDPN@@@@QEAAXPEAVDMMVIDPN@@@Z.c)
 *     ?IsSourceInTopology@DMMVIDPNTOPOLOGY@@QEBAEI@Z @ 0x1C0005570 (-IsSourceInTopology@DMMVIDPNTOPOLOGY@@QEBAEI@Z.c)
 *     ?FindPath@DMMVIDPNTOPOLOGY@@QEBAPEAVDMMVIDPNPRESENTPATH@@II@Z @ 0x1C0005854 (-FindPath@DMMVIDPNTOPOLOGY@@QEBAPEAVDMMVIDPNPRESENTPATH@@II@Z.c)
 *     ??3@YAXPEAX@Z @ 0x1C0009668 (--3@YAXPEAX@Z.c)
 *     ??_U@YAPEAX_KIW4_POOL_TYPE@@@Z @ 0x1C000C364 (--_U@YAPEAX_KIW4_POOL_TYPE@@@Z.c)
 *     ?SetGammaRamp@DMMVIDPNPRESENTPATH@@QEAAXPEBUDXGK_GAMMA_RAMP@@@Z @ 0x1C000D1C4 (-SetGammaRamp@DMMVIDPNPRESENTPATH@@QEAAXPEBUDXGK_GAMMA_RAMP@@@Z.c)
 *     ?GetPathFromTarget@DMMVIDPNTOPOLOGY@@QEBAPEAVDMMVIDPNPRESENTPATH@@I@Z @ 0x1C000D4C0 (-GetPathFromTarget@DMMVIDPNTOPOLOGY@@QEBAPEAVDMMVIDPNPRESENTPATH@@I@Z.c)
 *     DxgkIncrementGlobalConnectionChangeId @ 0x1C000E4D4 (DxgkIncrementGlobalConnectionChangeId.c)
 *     __security_check_cookie @ 0x1C00232F0 (__security_check_cookie.c)
 *     memset @ 0x1C0025300 (memset.c)
 *     ?CreateCddAllocations@ADAPTER_DISPLAY@@QEAAJPEAVDXGDEVICE@@IAEBU_D3DKMT_DISPLAYMODE@@PEAVCOREDEVICEACCESS@@@Z @ 0x1C00C8090 (-CreateCddAllocations@ADAPTER_DISPLAY@@QEAAJPEAVDXGDEVICE@@IAEBU_D3DKMT_DISPLAYMODE@@PEAVCOREDEV.c)
 *     ?DestroyCddAllocations@ADAPTER_DISPLAY@@QEAAJPEAVDXGDEVICE@@I@Z @ 0x1C00C8760 (-DestroyCddAllocations@ADAPTER_DISPLAY@@QEAAJPEAVDXGDEVICE@@I@Z.c)
 *     ?GetDisplayedPrimaryAllocation@ADAPTER_DISPLAY@@QEBAPEAVDXGADAPTERALLOCATION@@I@Z @ 0x1C00CA724 (-GetDisplayedPrimaryAllocation@ADAPTER_DISPLAY@@QEBAPEAVDXGADAPTERALLOCATION@@I@Z.c)
 *     ?GetCddPrimaryAllocation@ADAPTER_DISPLAY@@QEBAPEAVDXGADAPTERALLOCATION@@IE@Z @ 0x1C00CD000 (-GetCddPrimaryAllocation@ADAPTER_DISPLAY@@QEBAPEAVDXGADAPTERALLOCATION@@IE@Z.c)
 *     ?RemovePath@DMMVIDPNTOPOLOGY@@QEAAJIIPEAPEAVDMMVIDPNPRESENTPATH@@@Z @ 0x1C00DFA08 (-RemovePath@DMMVIDPNTOPOLOGY@@QEAAJIIPEAPEAVDMMVIDPNPRESENTPATH@@@Z.c)
 *     ?CreateClientVidPnFromLastClientCommitedVidPn@VIDPN_MGR@@QEAAJPEAV?$auto_rc@VDMMVIDPN@@@@@Z @ 0x1C00E2F48 (-CreateClientVidPnFromLastClientCommitedVidPn@VIDPN_MGR@@QEAAJPEAV-$auto_rc@VDMMVIDPN@@@@@Z.c)
 *     ??0ApplyPermissionWithinThisScope@@IEAA@QEAVProtectableFromChange@@EW4ActionPermission_T@1@E_KE@Z @ 0x1C00E3C04 (--0ApplyPermissionWithinThisScope@@IEAA@QEAVProtectableFromChange@@EW4ActionPermission_T@1@E_KE@.c)
 *     ??1ApplyPermissionWithinThisScope@@IEAA@XZ @ 0x1C00E3D70 (--1ApplyPermissionWithinThisScope@@IEAA@XZ.c)
 *     ?EnumPathTargetsFromSource@DMMVIDPNTOPOLOGY@@QEBAJI_KPEAI@Z @ 0x1C00E8854 (-EnumPathTargetsFromSource@DMMVIDPNTOPOLOGY@@QEBAJI_KPEAI@Z.c)
 *     ?UpdateActiveVidPnPresentPath@DMMVIDPNPRESENTPATH@@QEBAJPEBUDXGK_GAMMA_RAMP@@@Z @ 0x1C012405C (-UpdateActiveVidPnPresentPath@DMMVIDPNPRESENTPATH@@QEBAJPEBUDXGK_GAMMA_RAMP@@@Z.c)
 *     ?SetVidPnSourceVisibility@ADAPTER_DISPLAY@@QEAAJIEIE@Z @ 0x1C0124BF0 (-SetVidPnSourceVisibility@ADAPTER_DISPLAY@@QEAAJIEIE@Z.c)
 *     ?CommitVidPn@VIDPN_MGR@@QEAAJPEBVDMMVIDPN@@QEAXIW4_D3DKMDT_MONITOR_CONNECTIVITY_CHECKS@@EEAEBU_DMM_COMMITVIDPNREQUEST_DIAGINFO@@PEAE@Z @ 0x1C012D254 (-CommitVidPn@VIDPN_MGR@@QEAAJPEBVDMMVIDPN@@QEAXIW4_D3DKMDT_MONITOR_CONNECTIVITY_CHECKS@@EEAEBU_D.c)
 *     ?SerializeFunctionalPathDrivingTarget@DMMVIDPN@@QEBAJIQEAU_D3DKMDT_FUNCTIONAL_VIDPN_PRESENT_PATH@@@Z @ 0x1C012E01C (-SerializeFunctionalPathDrivingTarget@DMMVIDPN@@QEBAJIQEAU_D3DKMDT_FUNCTIONAL_VIDPN_PRESENT_PATH.c)
 *     ?RemoveAllPathsFromSource@DMMVIDPNTOPOLOGY@@QEAAJI@Z @ 0x1C012E2A8 (-RemoveAllPathsFromSource@DMMVIDPNTOPOLOGY@@QEAAJI@Z.c)
 *     ?PinModalityOnPathInVidPnTopology@VIDPN_MGR@@QEAAJQEAVDMMVIDPNTOPOLOGY@@PEBU_D3DKMDT_FUNCTIONAL_VIDPN_PRESENT_PATH@@@Z @ 0x1C012E384 (-PinModalityOnPathInVidPnTopology@VIDPN_MGR@@QEAAJQEAVDMMVIDPNTOPOLOGY@@PEBU_D3DKMDT_FUNCTIONAL_.c)
 *     ?AddUnpinnedPathToVidPnTopology@VIDPN_MGR@@QEAAJQEAVDMMVIDPNTOPOLOGY@@PEBU_D3DKMDT_FUNCTIONAL_VIDPN_PRESENT_PATH@@W4_D3DKMDT_MONITOR_CONNECTIVITY_CHECKS@@@Z @ 0x1C012E764 (-AddUnpinnedPathToVidPnTopology@VIDPN_MGR@@QEAAJQEAVDMMVIDPNTOPOLOGY@@PEBU_D3DKMDT_FUNCTIONAL_VI.c)
 *     ?GetDisplayModeFromPath@DMMVIDPNPRESENTPATH@@QEBAJAEAU_D3DKMT_DISPLAYMODE@@@Z @ 0x1C012EB30 (-GetDisplayModeFromPath@DMMVIDPNPRESENTPATH@@QEBAJAEAU_D3DKMT_DISPLAYMODE@@@Z.c)
 */

__int64 __fastcall VIDPN_MGR::CommitVidPnOnAdapter(
        VIDPN_MGR *a1,
        unsigned int a2,
        int a3,
        char **a4,
        int a5,
        __int64 a6,
        _DWORD *a7,
        _DWORD *a8,
        _DWORD *a9,
        __int64 a10,
        unsigned int *a11,
        struct DXGDEVICE *a12,
        struct COREDEVICEACCESS *a13,
        unsigned __int8 a14)
{
  __int64 v16; // rdx
  __int64 v17; // rcx
  __int64 v18; // r14
  int v19; // ebx
  __int64 v20; // rcx
  __int64 v21; // r14
  unsigned int v22; // ecx
  int v23; // eax
  __int64 v24; // rcx
  char **v25; // r12
  __int64 v26; // rdx
  __int64 v27; // rcx
  __int64 v28; // r8
  __int64 v29; // r9
  struct _D3DKMDT_VIDPN_PRESENT_PATH *v30; // rdi
  int ClientVidPnFromLastClientCommitedVidPn; // eax
  __int64 v32; // rdx
  __int64 v33; // rcx
  __int64 v34; // r8
  unsigned __int8 v35; // r9
  __int64 v36; // rbx
  unsigned __int64 *v37; // rbx
  unsigned int v38; // edi
  int v39; // r15d
  DMMVIDPNTOPOLOGY *v40; // rbx
  ADAPTER_DISPLAY *v41; // rcx
  __int64 v42; // r15
  int j; // r12d
  unsigned int v44; // r13d
  int k; // r12d
  __int64 v46; // rcx
  unsigned int v47; // eax
  char v48; // di
  __int64 v49; // r12
  int v50; // r15d
  int v51; // eax
  __int64 v53; // r8
  __int64 v54; // rbx
  int v55; // r9d
  int v56; // ecx
  unsigned int v57; // ecx
  signed __int64 v58; // rax
  __int64 v59; // r8
  int v60; // r9d
  char v61; // cl
  __int64 v62; // r10
  __int64 v63; // r11
  int v65; // eax
  __int64 v66; // rdx
  __int64 v67; // rcx
  unsigned int m; // r15d
  __int64 v69; // rdx
  __int64 v70; // rcx
  __int64 v71; // r12
  unsigned int v72; // r15d
  char **v73; // rdi
  __int64 v74; // rdx
  __int64 v75; // rcx
  __int64 v76; // rdx
  __int64 v77; // rcx
  int v78; // eax
  __int64 v79; // rdx
  __int64 v80; // rcx
  __int64 v81; // r8
  __int64 v82; // rdi
  struct DMMVIDPNPRESENTPATH *PathFromTarget; // rax
  __int64 v84; // r8
  DMMVIDPNPRESENTPATH *v85; // r11
  const struct _DXGK_DISPLAYMODE_INFO *v86; // rax
  __int128 v87; // xmm1
  __int64 v88; // xmm0_8
  DMMVIDPNPRESENTPATH *v89; // rax
  __int64 v90; // rdx
  __int64 v91; // rcx
  struct _D3DKMDT_VIDPN_PRESENT_PATH *v92; // rdi
  __int64 v93; // rdx
  __int64 v94; // rcx
  enum _D3DKMDT_MONITOR_CONNECTIVITY_CHECKS v95; // r9d
  int v96; // eax
  int v97; // eax
  __int64 v98; // rdx
  __int64 v99; // rcx
  __int64 v100; // r8
  struct DXGADAPTERALLOCATION *CddPrimaryAllocation; // rax
  void *v102; // rdi
  __int64 v103; // rdx
  __int64 v104; // rcx
  __int64 v105; // r8
  int v106; // ecx
  ADAPTER_DISPLAY *v107; // rcx
  bool v108; // zf
  VIDPN_MGR *v109; // r15
  int v110; // eax
  __int64 v111; // rdx
  __int64 v112; // rcx
  __int64 v113; // r8
  __int64 v114; // rax
  __int64 v115; // rax
  void *v116; // r13
  struct DXGADAPTERALLOCATION *v117; // rax
  __int64 v118; // rdx
  __int64 v119; // rcx
  int v120; // edi
  unsigned __int8 v121; // al
  unsigned int v122; // eax
  __int64 v123; // rdx
  __int64 v124; // rcx
  __int64 v125; // r8
  __int64 v126; // r13
  int v127; // eax
  __int64 v128; // rdx
  __int64 v129; // rcx
  __int64 v130; // r8
  __int64 v131; // r13
  int v132; // eax
  __int64 v133; // rdx
  __int64 v134; // rcx
  __int64 v135; // rax
  __int64 v136; // rax
  __int64 v137; // rax
  _QWORD *v138; // rax
  __int64 v139; // rdx
  __int64 v140; // rcx
  __int64 v141; // rax
  __int64 v142; // rdx
  __int64 v143; // rcx
  enum _D3DKMDT_VIDPN_PRESENT_PATH_ROTATION *Path; // r13
  __int64 v145; // rax
  const struct _DXGK_DISPLAYMODE_INFO *DisplayModeInfo; // rax
  unsigned int v147; // ecx
  __int128 v148; // xmm1
  __int64 v149; // xmm0_8
  int updated; // eax
  __int64 v151; // rdx
  __int64 v152; // rcx
  __int64 v153; // r8
  __int64 v154; // r12
  unsigned int i; // r12d
  __int64 v156; // rdx
  __int64 v157; // rcx
  __int64 v158; // rax
  DMMVIDPNPRESENTPATH *v159; // rax
  __int64 v160; // rdx
  __int64 v161; // rcx
  __int64 v162; // rax
  _QWORD *v163; // rax
  _QWORD *v164; // rax
  int v165; // eax
  __int64 v166; // rdx
  __int64 v167; // rcx
  __int64 v168; // rax
  ADAPTER_DISPLAY *v169; // rcx
  bool v170; // zf
  DMMVIDPNTOPOLOGY *v171; // rbx
  unsigned int v172; // r13d
  unsigned int *v173; // rdi
  __int64 v174; // rdx
  __int64 v175; // rcx
  __int64 v176; // rax
  bool IsSourceInTopology; // al
  void *v178; // rdi
  struct DXGADAPTERALLOCATION *DisplayedPrimaryAllocation; // rax
  __int64 v180; // rdx
  __int64 v181; // rcx
  ADAPTER_DISPLAY *v182; // rcx
  bool v183; // zf
  __int64 v184; // rdi
  int v185; // eax
  __int64 v186; // rdx
  __int64 v187; // rcx
  __int64 v188; // r8
  __int64 v189; // r13
  _QWORD *v190; // rax
  _QWORD *v191; // rax
  unsigned __int8 v192; // r9
  VIDPN_MGR *v193; // r13
  int v194; // eax
  __int64 v195; // rdx
  __int64 v196; // rcx
  __int64 v197; // r8
  __int64 v198; // rdi
  _QWORD *v199; // rax
  int v200; // eax
  __int64 v201; // rdx
  __int64 v202; // rcx
  __int64 v203; // r8
  __int64 v204; // rdi
  _QWORD *v205; // rax
  __int64 v206; // rax
  __int64 v207; // rax
  __int64 v208; // rax
  __int64 v209; // rax
  __int64 v210; // rax
  __int64 v211; // rax
  _QWORD *v212; // rax
  __int64 v213; // r15
  unsigned __int64 *v214; // rcx
  _QWORD *v215; // rax
  _QWORD *v216; // rax
  _QWORD *v217; // rax
  int v218; // eax
  __int64 v219; // rdx
  __int64 v220; // rcx
  __int64 v221; // rax
  __int64 v222; // rax
  _QWORD *v223; // rax
  __int64 v224; // rax
  _QWORD *v225; // rax
  __int64 v226; // rax
  __int64 v227; // [rsp+20h] [rbp-E0h]
  int v228; // [rsp+30h] [rbp-D0h]
  unsigned __int8 v229; // [rsp+50h] [rbp-B0h] BYREF
  char v230; // [rsp+51h] [rbp-AFh]
  char v231; // [rsp+52h] [rbp-AEh]
  unsigned int v232; // [rsp+54h] [rbp-ACh] BYREF
  __int64 v233; // [rsp+58h] [rbp-A8h] BYREF
  int v234; // [rsp+60h] [rbp-A0h]
  int v235; // [rsp+68h] [rbp-98h]
  unsigned int v236; // [rsp+6Ch] [rbp-94h]
  DMMVIDPNPRESENTPATH *v237; // [rsp+70h] [rbp-90h]
  int v238; // [rsp+78h] [rbp-88h]
  VIDPN_MGR *v239; // [rsp+80h] [rbp-80h]
  DMMVIDPNTOPOLOGY *v240; // [rsp+88h] [rbp-78h]
  DMMVIDPNTOPOLOGY *v241; // [rsp+90h] [rbp-70h]
  struct DMMVIDPN *v242; // [rsp+98h] [rbp-68h] BYREF
  struct _D3DKMDT_VIDPN_PRESENT_PATH *v243; // [rsp+A0h] [rbp-60h]
  struct DXGDEVICE *v244; // [rsp+A8h] [rbp-58h]
  _BYTE v245[24]; // [rsp+B0h] [rbp-50h] BYREF
  DMMVIDPN *v246; // [rsp+C8h] [rbp-38h]
  _DWORD *v247; // [rsp+D0h] [rbp-30h]
  struct _D3DKMT_DISPLAYMODE v248; // [rsp+D8h] [rbp-28h] BYREF
  __int64 v249; // [rsp+108h] [rbp+8h]
  __int64 v250; // [rsp+110h] [rbp+10h]
  unsigned int *v251; // [rsp+118h] [rbp+18h]
  struct COREDEVICEACCESS *v252[2]; // [rsp+120h] [rbp+20h] BYREF

  v250 = a6;
  v247 = a9;
  v249 = a10;
  v251 = a11;
  v236 = a2;
  v244 = a12;
  v235 = a3;
  v239 = a1;
  v252[0] = a13;
  v246 = (DMMVIDPN *)a4;
  memset(&v248, 0, sizeof(v248));
  v18 = *((_QWORD *)a1 + 1);
  v233 = 0LL;
  v234 = 0;
  v229 = 0;
  v230 = 0;
  if ( !v18 )
  {
    v135 = WdLogNewEntry5_WdAssertion(v17, v16);
    WdLogEvent5_WdAssertion(v135);
    v18 = *((_QWORD *)a1 + 1);
  }
  v19 = a8[6];
  v20 = (unsigned int)(a8[13] | a8[3]);
  v21 = *(_QWORD *)(v18 + 16);
  v238 = v19;
  if ( ((unsigned int)v20 & v19) != 0 )
  {
    v136 = WdLogNewEntry5_WdAssertion(v20, v16);
    WdLogEvent5_WdAssertion(v136);
  }
  v22 = a8[2];
  v23 = v22 & a8[5];
  v24 = v19 & v22;
  if ( (v19 & v23) != (_DWORD)v24 )
  {
    v137 = WdLogNewEntry5_WdAssertion(v24, v16);
    WdLogEvent5_WdAssertion(v137);
  }
  v25 = a4 + 12;
  v241 = (DMMVIDPNTOPOLOGY *)(a4 + 12);
  v243 = (struct _D3DKMDT_VIDPN_PRESENT_PATH *)operator new[](0x1E8uLL, 0x4E506456u, PagedPool);
  v30 = v243;
  if ( v243 )
  {
    operator delete(0LL);
    v242 = 0LL;
    ClientVidPnFromLastClientCommitedVidPn = VIDPN_MGR::CreateClientVidPnFromLastClientCommitedVidPn(
                                               a1,
                                               (__int64 *)&v242);
    v36 = ClientVidPnFromLastClientCommitedVidPn;
    if ( ClientVidPnFromLastClientCommitedVidPn < 0 )
    {
      v138 = (_QWORD *)WdLogNewEntry5_WdError(v33, v32, v34);
      v138[3] = *(int *)(v21 + 280);
      v138[4] = *(unsigned int *)(v21 + 276);
      v138[5] = v36;
      WdLogEvent5_WdError(v138);
    }
    else
    {
      v37 = (unsigned __int64 *)v242;
      v38 = 0;
      v240 = (struct DMMVIDPN *)((char *)v242 + 96);
      v39 = 1;
      if ( *(_DWORD *)(*(_QWORD *)(v21 + 2552) + 80LL) )
      {
        v40 = v240;
        do
        {
          if ( (v39 & a8[10]) != 0 )
          {
            if ( (int)DMMVIDPNTOPOLOGY::EnumPathTargetsFromSource(v25, v38, 0LL, &v232) < 0 || v232 == -1 )
            {
              v141 = WdLogNewEntry5_WdAssertion(v140, v139);
              WdLogEvent5_WdAssertion(v141);
            }
            Path = (enum _D3DKMDT_VIDPN_PRESENT_PATH_ROTATION *)DMMVIDPNTOPOLOGY::FindPath(
                                                                  (DMMVIDPNTOPOLOGY *)v25,
                                                                  v38,
                                                                  v232);
            if ( !Path )
            {
              v145 = WdLogNewEntry5_WdAssertion(v143, v142);
              WdLogEvent5_WdAssertion(v145);
            }
            DisplayModeInfo = ADAPTER_DISPLAY::GetDisplayModeInfo(*(DXGADAPTER ***)(v21 + 2552), v38);
            v147 = *((_DWORD *)Path + 29);
            v148 = *((_OWORD *)DisplayModeInfo + 1);
            *(_OWORD *)&v248.Width = *(_OWORD *)DisplayModeInfo;
            v149 = *((_QWORD *)DisplayModeInfo + 4);
            LODWORD(DisplayModeInfo) = *((_DWORD *)DisplayModeInfo + 10);
            *(_QWORD *)&v248.DisplayFixedOutput = v149;
            *((_DWORD *)&v248.Flags + 1) = (_DWORD)DisplayModeInfo;
            *(_OWORD *)&v248.RefreshRate.Numerator = v148;
            v248.DisplayOrientation = D3DKMDT_VPPR_GET_CONTENT_ROTATION(v147);
            updated = DMMVIDPNPRESENTPATH::UpdateActiveVidPnPresentPath((DMMVIDPNPRESENTPATH *)Path, 0LL);
            v154 = updated;
            if ( updated < 0 )
            {
              v163 = (_QWORD *)WdLogNewEntry5_WdError(v152, v151, v153);
              v163[3] = v38;
              v163[4] = v40;
              v163[5] = v154;
              WdLogEvent5_WdError(v163);
              a8[10] &= ~v39;
              a8[2] |= v39;
            }
            else
            {
              for ( i = 0; ; ++i )
              {
                if ( (int)DMMVIDPNTOPOLOGY::EnumPathTargetsFromSource((char **)v240, v38, i, &v232) < 0 )
                {
                  v158 = WdLogNewEntry5_WdAssertion(v157, v156);
                  WdLogEvent5_WdAssertion(v158);
                }
                if ( v232 == -1 )
                  break;
                v159 = DMMVIDPNTOPOLOGY::FindPath(v240, v38, v232);
                v237 = v159;
                if ( !v159 )
                {
                  v162 = WdLogNewEntry5_WdAssertion(v161, v160);
                  WdLogEvent5_WdAssertion(v162);
                  v159 = v237;
                }
                DMMVIDPNPRESENTPATH::PinContentRotation(v159, Path[29]);
              }
              v40 = v240;
            }
            v25 = (char **)v241;
          }
          ++v38;
          v39 *= 2;
        }
        while ( v38 < *(_DWORD *)(*(_QWORD *)(v21 + 2552) + 80LL) );
        v37 = (unsigned __int64 *)v242;
      }
LABEL_14:
      v41 = *(ADAPTER_DISPLAY **)(v21 + 2552);
      v42 = 0LL;
      for ( j = 1; (unsigned int)v42 < *((_DWORD *)v41 + 20); j *= 2 )
      {
        if ( (j & a8[20]) == 0 )
        {
          if ( ADAPTER_DISPLAY::IsVidPnSourceVisible(v41, v42)
            && (((a8[13] | a8[3] | a8[2]) & j) != 0 || (j & a8[5]) == 0)
            && (v127 = ADAPTER_DISPLAY::SetVidPnSourceVisibility(
                         *(ADAPTER_DISPLAY **)(v21 + 2552),
                         (unsigned int)v42,
                         0,
                         2048,
                         a14),
                v131 = v127,
                v127 < 0) )
          {
            v164 = (_QWORD *)WdLogNewEntry5_WdError(v129, v128, v130);
            v164[3] = (unsigned int)v42;
            v164[4] = *(int *)(v21 + 280);
            v164[5] = *(unsigned int *)(v21 + 276);
            v164[6] = v131;
            WdLogEvent5_WdError(v164);
            a8[20] |= j;
            a8[v42 + 21] = v131;
          }
          else
          {
            if ( (j & a8[3]) != 0 || v230 && (j & a8[2]) != 0 || *((_BYTE *)v239 + 464) )
            {
              ApplyPermissionWithinThisScope::ApplyPermissionWithinThisScope(
                (__int64)v245,
                (unsigned __int64)(v37 + 19) & -(__int64)(v37 + 12 != 0LL),
                2u,
                v35,
                v227,
                v37[17]);
              v165 = DMMVIDPNTOPOLOGY::RemoveAllPathsFromSource((DMMVIDPNTOPOLOGY *)(v37 + 12), v42);
              goto LABEL_123;
            }
            if ( (j & a8[13]) != 0 )
            {
              v171 = v240;
              v172 = 0;
              v173 = (unsigned int *)(v249 + 4LL * (unsigned int)(16 * v42));
              do
              {
                if ( *v173 == -1 )
                  break;
                ApplyPermissionWithinThisScope::ApplyPermissionWithinThisScope(
                  (__int64)v245,
                  ((unsigned __int64)v171 + 56) & ((unsigned __int128)-(__int128)(unsigned __int64)v171 >> 64),
                  2u,
                  v35,
                  v227,
                  1uLL);
                if ( (int)DMMVIDPNTOPOLOGY::RemovePath(v171, v42, *v173, 0LL) < 0 )
                {
                  v176 = WdLogNewEntry5_WdAssertion(v175, v174);
                  WdLogEvent5_WdAssertion(v176);
                }
                ApplyPermissionWithinThisScope::~ApplyPermissionWithinThisScope(
                  (ApplyPermissionWithinThisScope *)v245,
                  v174);
                ++v172;
                ++v173;
              }
              while ( v172 < 0x10 );
              IsSourceInTopology = DMMVIDPNTOPOLOGY::IsSourceInTopology(v171, v42);
              v37 = (unsigned __int64 *)v242;
              if ( !IsSourceInTopology )
                goto LABEL_127;
              v178 = 0LL;
              if ( *(_QWORD *)(v21 + 2560) )
              {
                DisplayedPrimaryAllocation = ADAPTER_DISPLAY::GetDisplayedPrimaryAllocation(
                                               *(DXGADAPTER ***)(v21 + 2552),
                                               v42);
                if ( !DisplayedPrimaryAllocation
                  || (v181 = *((unsigned int *)DisplayedPrimaryAllocation + 1), (v181 & 0x10) != 0) )
                {
                  v184 = (unsigned int)v42;
LABEL_145:
                  v191 = (_QWORD *)WdLogNewEntry5_WdTrace(v181, v180);
                  v191[3] = v184;
                  v191[4] = *(int *)(v21 + 280);
                  v191[5] = *(unsigned int *)(v21 + 276);
                  ApplyPermissionWithinThisScope::ApplyPermissionWithinThisScope(
                    (__int64)v245,
                    (unsigned __int64)(v37 + 19) & -(__int64)(v37 + 12 != 0LL),
                    2u,
                    v192,
                    v227,
                    v37[17]);
                  v165 = DMMVIDPNTOPOLOGY::RemoveAllPathsFromSource((DMMVIDPNTOPOLOGY *)(v37 + 12), v42);
LABEL_123:
                  v167 = v165 + 0x80000000;
                  if ( (v167 & 0x80000000) == 0 && v165 != -1071774919 )
                  {
                    v168 = WdLogNewEntry5_WdAssertion(v167, v166);
                    WdLogEvent5_WdAssertion(v168);
                  }
                  ApplyPermissionWithinThisScope::~ApplyPermissionWithinThisScope(
                    (ApplyPermissionWithinThisScope *)v245,
                    v166);
LABEL_127:
                  v108 = (j & a8[3]) == 0;
                  LODWORD(v233) = v233 & 0xFFFFFF50 | v235 & 0xF | 0x50;
                  v169 = *(ADAPTER_DISPLAY **)(v21 + 2552);
                  v234 = *((_DWORD *)v169 + 86);
                  BYTE4(v233) = (BYTE4(v233) & 0xFC | ((j & a8[3]) == 0)) ^ ((v236 >> 14) ^ (BYTE4(v233) | v108)) & 4;
                  if ( (j & a8[6]) != 0 )
                    v170 = ADAPTER_DISPLAY::IsVidPnSourceActive(v169, v42) == 0;
                  else
                    v170 = (j & a8[5]) == 0;
                  v193 = v239;
                  LOBYTE(v228) = v170;
                  v194 = VIDPN_MGR::CommitVidPn(
                           (ADAPTER_DISPLAY **)v239,
                           (const struct DMMVIDPN *)v37,
                           0LL,
                           v42,
                           D3DKMDT_MCC_IGNORE,
                           0,
                           v228,
                           (const struct _DMM_COMMITVIDPNREQUEST_DIAGINFO *)&v233,
                           &v229);
                  v198 = v194;
                  if ( v194 < 0 )
                  {
                    v199 = (_QWORD *)WdLogNewEntry5_WdError(v196, v195, v197);
                    v199[3] = (unsigned int)v42;
                    v199[4] = *(int *)(v21 + 280);
                    v199[5] = *(unsigned int *)(v21 + 276);
                    v199[6] = v198;
                    WdLogEvent5_WdError(v199);
                  }
                  if ( v244 && ((j & a8[3]) != 0 || *((_BYTE *)v193 + 464)) )
                  {
                    v200 = ADAPTER_DISPLAY::DestroyCddAllocations(*(PERESOURCE ***)(v21 + 2552), v244, v42);
                    v204 = v200;
                    if ( v200 < 0 )
                    {
                      v205 = (_QWORD *)WdLogNewEntry5_WdError(v202, v201, v203);
                      v205[3] = (unsigned int)v42;
                      v205[4] = *(int *)(v21 + 280);
                      v205[5] = *(unsigned int *)(v21 + 276);
                      v205[6] = v204;
                      WdLogEvent5_WdError(v205);
                    }
                  }
                  goto LABEL_21;
                }
                v178 = (void *)*((_QWORD *)DisplayedPrimaryAllocation + 2);
              }
              LODWORD(v233) = v233 & 0xFFFFFF40 | v235 & 0xF | 0x40;
              v182 = *(ADAPTER_DISPLAY **)(v21 + 2552);
              v234 = *((_DWORD *)v182 + 86);
              BYTE4(v233) = BYTE4(v233) & 0xF9 | (v236 >> 14) & 4 | 1;
              if ( (j & a8[6]) != 0 )
                v183 = ADAPTER_DISPLAY::IsVidPnSourceActive(v182, v42) == 0;
              else
                v183 = (j & a8[5]) == 0;
              LOBYTE(v228) = v183;
              v185 = VIDPN_MGR::CommitVidPn(
                       (ADAPTER_DISPLAY **)v239,
                       (const struct DMMVIDPN *)v37,
                       v178,
                       v42,
                       D3DKMDT_MCC_ENFORCE,
                       0,
                       v228,
                       (const struct _DMM_COMMITVIDPNREQUEST_DIAGINFO *)&v233,
                       &v229);
              v189 = v185;
              if ( v185 < 0 )
              {
                v190 = (_QWORD *)WdLogNewEntry5_WdError(v187, v186, v188);
                v184 = (unsigned int)v42;
                v190[3] = (unsigned int)v42;
                v190[4] = *(int *)(v21 + 280);
                v190[5] = *(unsigned int *)(v21 + 276);
                v190[6] = v189;
                WdLogEvent5_WdError(v190);
                goto LABEL_145;
              }
            }
          }
        }
LABEL_21:
        v41 = *(ADAPTER_DISPLAY **)(v21 + 2552);
        v42 = (unsigned int)(v42 + 1);
      }
      v44 = 0;
      for ( k = 1; ; k *= 2 )
      {
        v46 = *(_QWORD *)(v21 + 2552);
        LODWORD(v237) = k;
        v47 = *(_DWORD *)(v46 + 80);
        if ( v44 >= v47 )
          break;
        v48 = 1;
        v231 = 1;
        if ( (k & a8[20]) == 0 )
        {
          if ( (k & a8[2]) == 0 )
            goto LABEL_26;
          ApplyPermissionWithinThisScope::ApplyPermissionWithinThisScope(
            (__int64)v245,
            (unsigned __int64)(v37 + 19) & -(__int64)(v37 + 12 != 0LL),
            2u,
            v35,
            v227,
            v37[17]);
          v65 = DMMVIDPNTOPOLOGY::RemoveAllPathsFromSource((DMMVIDPNTOPOLOGY *)(v37 + 12), v44);
          v66 = 0x80000000LL;
          v67 = v65 + 0x80000000;
          if ( (int)v67 >= 0 && v65 != -1071774919 )
          {
            v206 = WdLogNewEntry5_WdAssertion(v67, 0x80000000LL);
            WdLogEvent5_WdAssertion(v206);
          }
          ApplyPermissionWithinThisScope::~ApplyPermissionWithinThisScope((ApplyPermissionWithinThisScope *)v245, v66);
          for ( m = 0; ; ++m )
          {
            if ( (int)DMMVIDPNTOPOLOGY::EnumPathTargetsFromSource((char **)v241, v44, m, &v232) < 0 )
            {
              v207 = WdLogNewEntry5_WdAssertion(v70, v69);
              WdLogEvent5_WdAssertion(v207);
            }
            v71 = v232;
            if ( v232 == -1 )
              break;
            if ( v48 )
            {
              v86 = ADAPTER_DISPLAY::GetDisplayModeInfo(*(DXGADAPTER ***)(v21 + 2552), v44);
              v87 = *((_OWORD *)v86 + 1);
              *(_OWORD *)&v248.Width = *(_OWORD *)v86;
              v88 = *((_QWORD *)v86 + 4);
              LODWORD(v86) = *((_DWORD *)v86 + 10);
              *(_QWORD *)&v248.DisplayFixedOutput = v88;
              *((_DWORD *)&v248.Flags + 1) = (_DWORD)v86;
              *(_OWORD *)&v248.RefreshRate.Numerator = v87;
              v89 = DMMVIDPNTOPOLOGY::FindPath(v241, v44, v71);
              if ( (int)DMMVIDPNPRESENTPATH::GetDisplayModeFromPath(v89, &v248) < 0 )
              {
                v208 = WdLogNewEntry5_WdAssertion(v91, v90);
                WdLogEvent5_WdAssertion(v208);
              }
              v231 = 0;
            }
            v92 = v243;
            if ( DMMVIDPN::SerializeFunctionalPathDrivingTarget(
                   v246,
                   v71,
                   (struct _D3DKMDT_FUNCTIONAL_VIDPN_PRESENT_PATH *const)v243) < 0 )
            {
              v209 = WdLogNewEntry5_WdAssertion(v94, v93);
              WdLogEvent5_WdAssertion(v209);
            }
            v96 = VIDPN_MGR::AddUnpinnedPathToVidPnTopology(v239, (struct DMMVIDPNTOPOLOGY *const)(v37 + 12), v92, v95);
            v82 = v96;
            if ( v96 < 0 )
            {
LABEL_154:
              v212 = (_QWORD *)WdLogNewEntry5_WdError(v80, v79, v81);
              v213 = v44;
              v214 = v37 + 12;
              v212[3] = v44;
              v212[4] = v71;
LABEL_156:
              v212[5] = v214;
              v212[6] = v82;
              WdLogEvent5_WdError(v212);
              k = (int)v237;
              goto LABEL_163;
            }
            v48 = v231;
          }
          v72 = 0;
          v73 = (char **)(v37 + 12);
          while ( 1 )
          {
            if ( (int)DMMVIDPNTOPOLOGY::EnumPathTargetsFromSource(v73, v44, v72, &v232) < 0 )
            {
              v210 = WdLogNewEntry5_WdAssertion(v75, v74);
              WdLogEvent5_WdAssertion(v210);
            }
            v71 = v232;
            if ( v232 == -1 )
              break;
            if ( DMMVIDPN::SerializeFunctionalPathDrivingTarget(
                   v246,
                   v232,
                   (struct _D3DKMDT_FUNCTIONAL_VIDPN_PRESENT_PATH *const)v243) < 0 )
            {
              v211 = WdLogNewEntry5_WdAssertion(v77, v76);
              WdLogEvent5_WdAssertion(v211);
            }
            v78 = VIDPN_MGR::PinModalityOnPathInVidPnTopology(
                    v239,
                    (struct DMMVIDPNTOPOLOGY *const)v73,
                    (const struct _D3DKMDT_FUNCTIONAL_VIDPN_PRESENT_PATH *)v243);
            v82 = v78;
            if ( v78 < 0 )
              goto LABEL_154;
            v73 = (char **)(v37 + 12);
            DMMVIDPNTOPOLOGY::GetPathFromTarget((DMMVIDPNTOPOLOGY *)(v37 + 12), v71);
            PathFromTarget = DMMVIDPNTOPOLOGY::GetPathFromTarget(v241, v71);
            DMMVIDPNPRESENTPATH::SetGammaRamp(v85, *((const struct DXGK_GAMMA_RAMP **)PathFromTarget + 23), v84);
            ++v72;
          }
          if ( v244 && v252[0] )
          {
            v97 = ADAPTER_DISPLAY::CreateCddAllocations(*(PERESOURCE ***)(v21 + 2552), v244, v44, &v248, v252[0]);
            v82 = v97;
            if ( v97 < 0 )
            {
              v212 = (_QWORD *)WdLogNewEntry5_WdError(v99, v98, v100);
              v213 = v44;
              v212[3] = v44;
              v212[4] = *(int *)(v21 + 280);
              v214 = (unsigned __int64 *)*(unsigned int *)(v21 + 276);
              goto LABEL_156;
            }
            CddPrimaryAllocation = ADAPTER_DISPLAY::GetCddPrimaryAllocation(*(ADAPTER_DISPLAY **)(v21 + 2552), v44, 0);
            if ( !*(_QWORD *)(v21 + 2560) )
            {
LABEL_66:
              v102 = 0LL;
              goto LABEL_70;
            }
LABEL_69:
            v102 = (void *)*((_QWORD *)CddPrimaryAllocation + 2);
LABEL_70:
            v106 = 32;
            k = (int)v237;
            if ( v72 <= 1 )
              v106 = 48;
            LODWORD(v233) = v233 & 0xFFFFFF00 | v235 & 0xF | v106;
            v107 = *(ADAPTER_DISPLAY **)(v21 + 2552);
            v234 = *((_DWORD *)v107 + 86);
            BYTE4(v233) = BYTE4(v233) & 0xF8 | (v236 >> 14) & 4;
            if ( ((unsigned int)v237 & a8[6]) != 0 )
              v108 = ADAPTER_DISPLAY::IsVidPnSourceActive(v107, v44) == 0;
            else
              v108 = ((unsigned int)v237 & a8[5]) == 0;
            v109 = v239;
            LOBYTE(v228) = v108;
            v110 = VIDPN_MGR::CommitVidPn(
                     (ADAPTER_DISPLAY **)v239,
                     (const struct DMMVIDPN *)v37,
                     v102,
                     v44,
                     D3DKMDT_MCC_ENFORCE,
                     0,
                     v228,
                     (const struct _DMM_COMMITVIDPNREQUEST_DIAGINFO *)&v233,
                     &v229);
            v82 = v110;
            if ( v110 >= 0 )
            {
              if ( (k & a8[5]) == 0 )
              {
                v114 = *((_QWORD *)v109 + 1);
                if ( !v114 )
                {
                  v222 = WdLogNewEntry5_WdAssertion(v112, v111);
                  WdLogEvent5_WdAssertion(v222);
                  v114 = *((_QWORD *)v109 + 1);
                }
                v115 = *(_QWORD *)(v114 + 16);
                if ( *(int *)(v115 + 2452) < 1200
                  || *(_DWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(v115 + 192) + 64LL) + 40LL) + 28LL) < 0x300Au )
                {
                  v238 |= k;
                }
              }
              goto LABEL_26;
            }
            v217 = (_QWORD *)WdLogNewEntry5_WdError(v112, v111, v113);
            v213 = v44;
            v217[3] = v44;
            v217[4] = *(int *)(v21 + 280);
            v217[5] = *(unsigned int *)(v21 + 276);
            v217[6] = v82;
            WdLogEvent5_WdError(v217);
            if ( v244 )
              ADAPTER_DISPLAY::DestroyCddAllocations(*(PERESOURCE ***)(v21 + 2552), v244, v44);
          }
          else
          {
            if ( !*(_QWORD *)(v21 + 2560) )
              goto LABEL_66;
            CddPrimaryAllocation = ADAPTER_DISPLAY::GetDisplayedPrimaryAllocation(*(DXGADAPTER ***)(v21 + 2552), v44);
            if ( CddPrimaryAllocation )
            {
              v104 = *((unsigned int *)CddPrimaryAllocation + 1);
              if ( (v104 & 0x10) == 0 )
                goto LABEL_69;
            }
            k = (int)v237;
            v213 = v44;
            if ( ((unsigned int)v237 & a8[8]) == 0 )
            {
              *v247 |= (unsigned int)v237;
              v216 = (_QWORD *)WdLogNewEntry5_WdDmmEvent(v104, v103);
              v216[3] = v44;
              v216[4] = *(int *)(v21 + 280);
              v216[5] = *(unsigned int *)(v21 + 276);
              WdLogEvent5_WdDmmEvent(v216);
              goto LABEL_26;
            }
            LODWORD(v82) = -1071775482;
            v215 = (_QWORD *)WdLogNewEntry5_WdError(v104, v103, v105);
            v215[3] = v44;
            v215[4] = *(int *)(v21 + 280);
            v215[5] = *(unsigned int *)(v21 + 276);
            WdLogEvent5_WdError(v215);
          }
LABEL_163:
          if ( !v230 )
          {
            DxgkLogCodePointPacket(0x61u, v44, v82, 0, *(_QWORD *)(v21 + 276));
            v230 = 1;
            goto LABEL_14;
          }
          a8[20] |= k;
          a8[v213 + 21] = v82;
          if ( (_DWORD)v82 == -1071774920 || (_DWORD)v82 == -1071774976 )
            v229 = 1;
        }
        ApplyPermissionWithinThisScope::ApplyPermissionWithinThisScope(
          (__int64)v245,
          (unsigned __int64)(v37 + 19) & -(__int64)(v37 + 12 != 0LL),
          2u,
          v35,
          v227,
          v37[17]);
        v218 = DMMVIDPNTOPOLOGY::RemoveAllPathsFromSource((DMMVIDPNTOPOLOGY *)(v37 + 12), v44);
        v219 = 0x80000000LL;
        v220 = v218 + 0x80000000;
        if ( (int)v220 >= 0 && v218 != -1071774919 )
        {
          v221 = WdLogNewEntry5_WdAssertion(v220, 0x80000000LL);
          WdLogEvent5_WdAssertion(v221);
        }
        ApplyPermissionWithinThisScope::~ApplyPermissionWithinThisScope((ApplyPermissionWithinThisScope *)v245, v219);
LABEL_26:
        ++v44;
      }
      v49 = 0LL;
      v50 = 1;
      if ( v47 )
      {
        do
        {
          if ( (v50 & a8[20]) == 0 )
          {
            v51 = a8[5];
            if ( (v50 & v238) != 0 )
            {
              v116 = 0LL;
              if ( (v51 & v50) == 0 )
              {
                ApplyPermissionWithinThisScope::ApplyPermissionWithinThisScope(
                  (__int64)v245,
                  (unsigned __int64)(v37 + 19) & -(__int64)(v37 + 12 != 0LL),
                  2u,
                  v35,
                  v227,
                  v37[17]);
                v132 = DMMVIDPNTOPOLOGY::RemoveAllPathsFromSource((DMMVIDPNTOPOLOGY *)(v37 + 12), v49);
                v133 = 0x80000000LL;
                v134 = v132 + 0x80000000;
                if ( (int)v134 >= 0 && v132 != -1071774919 )
                {
                  v224 = WdLogNewEntry5_WdAssertion(v134, 0x80000000LL);
                  WdLogEvent5_WdAssertion(v224);
                }
                a8[17] |= v50;
                ApplyPermissionWithinThisScope::~ApplyPermissionWithinThisScope(
                  (ApplyPermissionWithinThisScope *)v245,
                  v133);
                v120 = v233 & 0xFFFFFF0F | 0x40;
                goto LABEL_87;
              }
              if ( !*(_QWORD *)(v21 + 2560) )
                goto LABEL_86;
              v117 = ADAPTER_DISPLAY::GetDisplayedPrimaryAllocation((DXGADAPTER **)v46, v49);
              if ( v117 )
              {
                v119 = *((unsigned int *)v117 + 1);
                if ( (v119 & 0x10) == 0 )
                {
                  v116 = (void *)*((_QWORD *)v117 + 2);
LABEL_86:
                  v120 = v233 & 0xFFFFFF0F | 0x20;
LABEL_87:
                  v252[0] = 0LL;
                  v252[1] = 0LL;
                  v121 = v235;
                  if ( v235 == 4 )
                  {
                    DisplayLogSetMonitorPowerStage(0LL, 32LL, 0LL, v252);
                    v121 = v235;
                  }
                  LODWORD(v233) = v120 ^ ((unsigned __int8)v120 ^ v121) & 0xF;
                  v234 = *(_DWORD *)(*(_QWORD *)(v21 + 2552) + 344LL);
                  LOBYTE(v228) = 0;
                  BYTE4(v233) = BYTE4(v233) & 0xF8 | (v236 >> 14) & 4;
                  v122 = VIDPN_MGR::CommitVidPn(
                           (ADAPTER_DISPLAY **)v239,
                           (const struct DMMVIDPN *)v37,
                           v116,
                           v49,
                           D3DKMDT_MCC_ENFORCE,
                           1,
                           v228,
                           (const struct _DMM_COMMITVIDPNREQUEST_DIAGINFO *)&v233,
                           &v229);
                  v126 = (int)v122;
                  if ( v235 == 4 )
                    DisplayLogSetMonitorPowerStage(0LL, 2147483680LL, v122, v252);
                  if ( (int)v126 < 0 )
                  {
                    a8[20] |= v50;
                    a8[v49 + 21] = v126;
                    v225 = (_QWORD *)WdLogNewEntry5_WdError(v124, v123, v125);
                    v225[3] = (unsigned int)v49;
                    v225[4] = *(int *)(v21 + 280);
                    v225[5] = *(unsigned int *)(v21 + 276);
                    v225[6] = v126;
                    WdLogEvent5_WdError(v225);
                  }
                  goto LABEL_31;
                }
              }
              *v247 |= v50;
              a8[17] |= v50;
              v223 = (_QWORD *)WdLogNewEntry5_WdDmmEvent(v119, v118);
              v223[3] = (unsigned int)v49;
              v223[4] = *(int *)(v21 + 280);
              v223[5] = *(unsigned int *)(v21 + 276);
              WdLogEvent5_WdDmmEvent(v223);
            }
            else
            {
              a8[17] |= v50 & ~v51;
            }
          }
LABEL_31:
          v46 = *(_QWORD *)(v21 + 2552);
          v49 = (unsigned int)(v49 + 1);
          v50 *= 2;
        }
        while ( (unsigned int)v49 < *(_DWORD *)(v46 + 80) );
      }
      if ( a5 )
      {
        v53 = 0LL;
        v54 = v250;
        do
        {
          v55 = 1 << *a7;
          v56 = *(_DWORD *)(v54 + v53 + 32);
          if ( (v55 & a8[38]) != 0 )
          {
            v57 = v56 & 0xF0FFFFFF | 0xC000000;
          }
          else if ( (v55 & a8[20]) != 0 )
          {
            v57 = v56 & 0xF0FFFFFF | 0xD000000;
          }
          else
          {
            v57 = v56 & 0xF0FFFFFF | 0xE000000;
          }
          *(_DWORD *)(v54 + v53 + 32) = v57;
          v58 = DxgkIncrementGlobalConnectionChangeId();
          *(_QWORD *)(v54 + v59 + 24) = v58;
          v61 = (v60 & (a8[38] | a8[2])) != 0 ? 1 : -1;
          LOBYTE(v58) = -((v60 & (a8[38] | a8[2])) != 0);
          *(_DWORD *)(v59 + v54 + 16) &= ~1u;
          *(_BYTE *)(v59 + v54 + 48) = v61;
          *(_BYTE *)(v59 + v54 + 49) = ~(_BYTE)v58;
          a7 = (_DWORD *)(v62 + 4);
          *(_BYTE *)(v59 + v54 + 50) = v61;
          v53 = v59 + 56;
        }
        while ( v63 != 1 );
      }
      v30 = v243;
      *v251 = (v229 != 0) | *v251 & 0xFFFFFFFE;
      LODWORD(v36) = 0;
    }
    auto_rc<DMMVIDPN>::reset((__int64 *)&v242, 0LL);
  }
  else
  {
    v226 = WdLogNewEntry5_WdLowResource(v27, v26, v28, v29);
    *(_QWORD *)(v226 + 24) = *(int *)(v21 + 280);
    *(_QWORD *)(v226 + 32) = *(unsigned int *)(v21 + 276);
    WdLogEvent5_WdLowResource(v226);
    LODWORD(v36) = -1073741801;
  }
  operator delete(v30);
  return (unsigned int)v36;
}
