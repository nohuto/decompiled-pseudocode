...
LABEL_7:
    TokenInformation = 0;
    result = ZwQueryInformationToken((HANDLE)v6, 0x1Du, &TokenInformation, 4u, &ReturnLength);
    if ( result < 0 )
      return result;
    if ( !TokenInformation )
    {
  ...
  if ( !RelativePath || !v7 )
    return sub_18000BCF8((HANDLE)v6);
  result = ZwQueryInformationToken((HANDLE)0xFFFFFFFFFFFFFFFCLL, 0x2Au, &v10, 4u, &ReturnLength);
  if ( result >= 0 )
  {
    if ( !v10 )
      return sub_18000BCF8((HANDLE)v6);
    result = ZwQueryInformationToken((HANDLE)v6, 0x2Au, &v11, 4u, &ReturnLength);
    if ( result >= 0 )
    {
      if ( !(_DWORD)v11 )
        return -1073741637;
      ...
      if ( result >= 0 )
      {
        result = ZwQueryInformationToken((HANDLE)v6, 1u, Sid1, 0x58u, &ReturnLength);
        if ( result >= 0 )
        {
