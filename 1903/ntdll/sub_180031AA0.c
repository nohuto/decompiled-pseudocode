/*
 * XREFs of sub_180031AA0 @ 0x180031AA0
 * Callers:
 *     TpAllocWait @ 0x180031130 (TpAllocWait.c)
 *     TpAllocTimer @ 0x180031BA0 (TpAllocTimer.c)
 * Callees:
 *     sub_180031B40 @ 0x180031B40 (sub_180031B40.c)
 *     sub_180037608 @ 0x180037608 (sub_180037608.c)
 *     sub_180062DB0 @ 0x180062DB0 (sub_180062DB0.c)
 */

__int64 __fastcall sub_180031AA0(__int64 a1, char a2, int a3, __int64 a4, __int64 a5, __int64 a6)
{
  __int64 v6; // rdi
  __int64 result; // rax

  v6 = a4;
  if ( a4 )
    LODWORD(a4) = *(_DWORD *)(a4 + 56);
  result = sub_180037608(a1, a3, v6, a4, a5, a6);
  if ( (int)result >= 0 )
  {
    sub_180031B40(*(_QWORD *)(a1 + 144), 1LL);
    *(_QWORD *)(a1 + 240) = 0LL;
    *(_BYTE *)(a1 + 353) = a2;
    if ( v6 )
      *(_QWORD *)(a1 + 32) = *(_QWORD *)(v6 + 48);
    if ( *(_QWORD *)(a1 + 16) )
      sub_180062DB0(a1);
    return 0LL;
  }
  return result;
}
