/*
 * XREFs of sub_180068434 @ 0x180068434
 * Callers:
 *     sub_180038840 @ 0x180038840 (sub_180038840.c)
 *     sub_180038C70 @ 0x180038C70 (sub_180038C70.c)
 *     sub_180039740 @ 0x180039740 (sub_180039740.c)
 *     sub_18010BEF4 @ 0x18010BEF4 (sub_18010BEF4.c)
 * Callees:
 *     sub_1800685E0 @ 0x1800685E0 (sub_1800685E0.c)
 */

char __fastcall sub_180068434(__int64 a1, __int64 a2, _WORD *a3)
{
  __int16 v5; // r10
  bool v6; // r8
  unsigned __int64 v7; // rax
  __int64 v8; // rbx
  unsigned __int64 v9; // rdx
  unsigned __int64 v10; // rax
  __int16 v12; // [rsp+30h] [rbp+8h] BYREF
  unsigned __int64 v13; // [rsp+40h] [rbp+18h] BYREF

  v5 = sub_1800685E0(a3, a2, &v12, &v13);
  v6 = 0;
  v7 = (0x101010101010101LL
      * ((((v13 - ((v13 >> 1) & 0x5555555555555555LL)) & 0x3333333333333333LL)
        + (((v13 - ((v13 >> 1) & 0x5555555555555555LL)) >> 2) & 0x3333333333333333LL)
        + ((((v13 - ((v13 >> 1) & 0x5555555555555555LL)) & 0x3333333333333333LL)
          + (((v13 - ((v13 >> 1) & 0x5555555555555555LL)) >> 2) & 0x3333333333333333LL)) >> 4)) & 0xF0F0F0F0F0F0F0FLL)) >> 56;
  *(_QWORD *)(a1 + 56) += v7;
  v8 = a1 + 16;
  *a3 = (unsigned __int16)a3 ^ qword_180163540 ^ (v12 + v5 - v7);
  v9 = *(_QWORD *)v8;
  if ( (*(_BYTE *)(v8 + 8) & 1) != 0 )
  {
    if ( v9 )
      v9 ^= v8;
    else
      v9 = 0LL;
  }
  if ( v9 )
  {
    while ( 1 )
    {
      if ( ((unsigned int)a3 ^ (unsigned int)qword_180163540 ^ *(_DWORD *)a3) < (((_DWORD)v9 - 8) ^ (unsigned int)qword_180163540 ^ *(_DWORD *)(v9 - 8)) )
      {
        v10 = *(_QWORD *)v9;
        if ( (*(_BYTE *)(v8 + 8) & 1) != 0 )
        {
          if ( !v10 )
            return RtlRbInsertNodeEx((unsigned __int64 *)v8, v9, v6, (unsigned __int64)(a3 + 4));
          v10 ^= v9;
        }
        if ( !v10 )
          return RtlRbInsertNodeEx((unsigned __int64 *)v8, v9, v6, (unsigned __int64)(a3 + 4));
      }
      else
      {
        v10 = *(_QWORD *)(v9 + 8);
        if ( (*(_BYTE *)(v8 + 8) & 1) != 0 )
        {
          if ( !v10 )
            goto LABEL_17;
          v10 ^= v9;
        }
        if ( !v10 )
        {
LABEL_17:
          v6 = 1;
          return RtlRbInsertNodeEx((unsigned __int64 *)v8, v9, v6, (unsigned __int64)(a3 + 4));
        }
      }
      v9 = v10;
    }
  }
  return RtlRbInsertNodeEx((unsigned __int64 *)v8, v9, v6, (unsigned __int64)(a3 + 4));
}
