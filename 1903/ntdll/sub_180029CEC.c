/*
 * XREFs of sub_180029CEC @ 0x180029CEC
 * Callers:
 *     sub_1800219E8 @ 0x1800219E8 (sub_1800219E8.c)
 *     sub_18002D8FC @ 0x18002D8FC (sub_18002D8FC.c)
 *     sub_18002E4A8 @ 0x18002E4A8 (sub_18002E4A8.c)
 *     sub_18002E5E8 @ 0x18002E5E8 (sub_18002E5E8.c)
 * Callees:
 *     RtlInitUnicodeStringEx @ 0x180028DF0 (RtlInitUnicodeStringEx.c)
 *     sub_180029974 @ 0x180029974 (sub_180029974.c)
 *     sub_180029C14 @ 0x180029C14 (sub_180029C14.c)
 *     sub_180029E90 @ 0x180029E90 (sub_180029E90.c)
 *     sub_18002E1A4 @ 0x18002E1A4 (sub_18002E1A4.c)
 *     memmove @ 0x1800A32C0 (memmove.c)
 *     sub_1800CE318 @ 0x1800CE318 (sub_1800CE318.c)
 */

__int64 __fastcall sub_180029CEC(unsigned __int16 *a1, __int64 a2, __int64 a3, _OWORD *a4, __int16 a5)
{
  __m128i v9; // xmm0
  int v10; // ebx
  _WORD *v11; // rax
  __int64 v12; // rsi
  const void *v13; // rbp
  int v14; // eax
  __m128i v15; // xmm0
  unsigned __int16 v16; // ax
  unsigned __int64 v17; // xmm0_8
  _WORD *i; // rdx
  int v20; // eax
  unsigned __int64 v21; // rax
  __int64 v22; // rcx
  __m128i Src; // [rsp+30h] [rbp-28h] BYREF

  if ( (dword_18015FAB0 & 9) != 0 )
    sub_1800CE318(
      (unsigned int)"minkernel\\ntdll\\ldrfind.c",
      1738,
      (unsigned int)"LdrpResolveDllName",
      3,
      (__int64)"DLL name: %wZ\n",
      a1);
  if ( (a5 & 0x200) != 0 )
  {
    v9 = *(__m128i *)a1;
    v10 = 0;
  }
  else
  {
    v20 = sub_180029974(a1, a2);
    v9 = *(__m128i *)a2;
    v10 = v20;
  }
  Src = v9;
  if ( v10 >= 0 )
  {
    if ( (a5 & 0x200) != 0 || (v11 = (_WORD *)(a2 + 16), a2 + 16 == *(_QWORD *)(a2 + 8)) )
    {
      v12 = Src.m128i_u16[0];
      v13 = (const void *)Src.m128i_i64[1];
      v10 = sub_180029E90(&Src, Src.m128i_u16[0]);
      if ( v10 >= 0 )
      {
        memmove((void *)Src.m128i_i64[1], v13, v12 + 2);
        Src.m128i_i16[0] = v12;
      }
      *(_WORD *)a2 = 0;
    }
    else
    {
      *(_QWORD *)(a2 + 8) = v11;
      *(_DWORD *)a2 = 0x1000000;
      *v11 = 0;
    }
    if ( v10 >= 0 )
    {
      v14 = sub_180029C14(&Src, a2);
      v10 = v14;
      if ( v14 < 0 )
      {
        v21 = (unsigned int)(v14 + 1073741809);
        if ( (unsigned int)v21 <= 0x2C && (v22 = 0x1C3000000011LL, _bittest64(&v22, v21))
          || v10 == -2147483632
          || v10 == -1073741661 )
        {
          v10 = -1073741515;
        }
        sub_18002E1A4(&Src);
      }
      else
      {
        v15 = Src;
        v16 = _mm_cvtsi128_si32(Src);
        *a4 = Src;
        v17 = _mm_srli_si128(v15, 8).m128i_u64[0];
        for ( i = (_WORD *)(v16 + v17 - 2); (unsigned __int64)i >= v17; --i )
        {
          if ( *i == 92 || *i == 47 )
          {
            ++i;
            break;
          }
        }
        RtlInitUnicodeStringEx(a3, (__int64)i);
      }
    }
  }
  if ( (dword_18015FAB0 & 9) != 0 )
    sub_1800CE318(
      (unsigned int)"minkernel\\ntdll\\ldrfind.c",
      1849,
      (unsigned int)"LdrpResolveDllName",
      4,
      (__int64)"Status: 0x%08lx\n",
      v10);
  return (unsigned int)v10;
}
