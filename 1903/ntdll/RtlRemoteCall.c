/*
 * XREFs of RtlRemoteCall @ 0x1800F8C90
 * Callers:
 *     <none>
 * Callees:
 ...
 *     ZwSetContextThread @ 0x18009F770 (ZwSetContextThread.c)
        ...
        BOOLEAN AlreadySuspended)
{
  ...
  if ( AlreadySuspended || (result = ZwSuspendThread(ThreadHandle, 0LL), result >= 0) )
  {
    ...
    v16 = ZwWriteVirtualMemory(ProcessHandle, (PVOID)(ThreadContext.Rsp - 1232), &ThreadContext, 0x4D0uLL, 0LL);
    if ( v16 < 0 )
    {
      if ( !AlreadySuspended )
        ZwResumeThread(ThreadHandle, 0LL);
...
LABEL_22:
    ...
    v18 = ZwSetContextThread(ThreadHandle, &ThreadContext);
    if ( !AlreadySuspended )
      ZwResumeThread(ThreadHandle, 0LL);
    return v18;
  }
