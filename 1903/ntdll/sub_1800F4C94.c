/*
 * XREFs of sub_1800F4C94 @ 0x1800F4C94
 * Callers:
 *     sub_1800F4580 @ 0x1800F4580 (sub_1800F4580.c)
 *     sub_1800F46F8 @ 0x1800F46F8 (sub_1800F46F8.c)
 *     sub_1800F4C94 @ 0x1800F4C94 (sub_1800F4C94.c)
 * Callees:
 *     RtlAllocateHeap @ 0x18003AA20 (RtlAllocateHeap.c)
 *     DbgPrint @ 0x180053DA0 (DbgPrint.c)
 *     sub_1800F453C @ 0x1800F453C (sub_1800F453C.c)
 *     sub_1800F4C94 @ 0x1800F4C94 (sub_1800F4C94.c)
 */

int __fastcall sub_1800F4C94(_QWORD *a1, unsigned __int64 a2, __int64 a3, __int64 a4)
{
  unsigned __int64 v5; // rax
  unsigned __int64 v9; // rcx
  unsigned __int64 v10; // rbx
  unsigned __int64 v11; // rdx
  unsigned __int64 v12; // r15
  __int64 v13; // rsi
  __int64 v14; // rcx
  _QWORD *Heap; // rax

  v5 = a3 + a2 - 1;
  if ( v5 >= a1[1] && a2 <= a1[2] )
  {
    v9 = a2 - a1[1];
    if ( a2 <= a1[1] )
      v10 = 0LL;
    else
      v10 = v9 / *a1;
    v11 = (v9 + a3 - 1) % *a1;
    v12 = (v9 + a3 - 1) / *a1;
    LODWORD(v5) = 255;
    if ( v12 > 0xFF )
      v12 = 255LL;
    if ( v10 <= v12 )
    {
      v13 = (__int64)&a1[v10 + 7];
      do
      {
        if ( *a1 == 4096LL )
        {
          if ( a4 )
          {
            if ( *(_QWORD *)v13 && *(_QWORD *)v13 != a4 )
              LODWORD(v5) = DbgPrint("Error\n", v11);
            *(_QWORD *)v13 = a4;
          }
          else
          {
            v11 = v10 >> 3;
            LODWORD(v5) = v10 & 7;
            *((_BYTE *)a1 + (v10 >> 3) + 24) |= 1 << v5;
          }
        }
        else
        {
          v14 = *(_QWORD *)v13;
          if ( !*(_QWORD *)v13 )
          {
            Heap = RtlAllocateHeap(qword_180163B98, 0, 0x840uLL);
            *(_QWORD *)v13 = Heap;
            if ( !Heap )
            {
              LODWORD(v5) = DbgPrint("Not enough memory to complete\n");
              return v5;
            }
            sub_1800F453C(Heap, a1);
            *(_QWORD *)(*(_QWORD *)v13 + 8LL) = a1[1] + v10 * *a1;
            *(_QWORD *)(*(_QWORD *)v13 + 16LL) = a1[1] - 1LL + *a1 * (v10 + 1);
            v14 = *(_QWORD *)v13;
          }
          LODWORD(v5) = sub_1800F4C94(v14, a2, a3, a4);
        }
        ++v10;
        v13 += 8LL;
      }
      while ( v10 <= v12 );
    }
  }
  return v5;
}
