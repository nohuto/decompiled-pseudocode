/*
 * XREFs of TpAllocCleanupGroup @ 0x18007D2C0
 * Callers:
 *     <none>
 * Callees:
...
NTSTATUS __cdecl TpAllocCleanupGroup(PTP_CLEANUP_GROUP *CleanupGroupReturn)
{
  ...
  {
    sub_18010EFC8(CleanupGroupReturn, Ldr, v2);
    return -1073741811;
  }
  else
  {
    *CleanupGroupReturn = 0LL;
    Heap = RtlAllocateHeap(NtCurrentPeb()->ProcessHeap, dword_180166080 | 8, 0x50uLL);
    ...
    if ( result < 0 )
    {
      if ( v5 )
      {
        RtlFreeHeap(NtCurrentPeb()->ProcessHeap, dword_180166080, BaseAddress);
        return v8;
      }
    }
  }
