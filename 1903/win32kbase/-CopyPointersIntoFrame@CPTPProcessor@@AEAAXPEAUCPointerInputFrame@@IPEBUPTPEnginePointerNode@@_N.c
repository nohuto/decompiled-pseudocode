/*
 * XREFs of ?CopyPointersIntoFrame@CPTPProcessor@@AEAAXPEAUCPointerInputFrame@@IPEBUPTPEnginePointerNode@@_N@Z @ 0x1C017A334
 * Callers:
 *     ?CreateGestureCache@CPTPProcessor@@AEAAXIPEBUPTPEnginePointerNode@@_N@Z @ 0x1C017A438 (-CreateGestureCache@CPTPProcessor@@AEAAXIPEBUPTPEnginePointerNode@@_N@Z.c)
 *     ?OnEngineOutput@CPTPProcessor@@UEAAXPEBUPTPEngineOutput@@@Z @ 0x1C017B000 (-OnEngineOutput@CPTPProcessor@@UEAAXPEBUPTPEngineOutput@@@Z.c)
 * Callees:
 *     ?CommitFrame@Pointer@InputTraceLogging@@SAXPEBURIMDEV@@PEBUCPointerInputFrame@@@Z @ 0x1C0162F04 (-CommitFrame@Pointer@InputTraceLogging@@SAXPEBURIMDEV@@PEBUCPointerInputFrame@@@Z.c)
 *     ?CommitFramePointer@Pointer@InputTraceLogging@@SAXPEBURIMDEV@@PEBUCPointerInputFrame@@PEBUtagPOINTEREVENTINT@@@Z @ 0x1C0163230 (-CommitFramePointer@Pointer@InputTraceLogging@@SAXPEBURIMDEV@@PEBUCPointerInputFrame@@PEBUtagPOI.c)
 */

void __fastcall CPTPProcessor::CopyPointersIntoFrame(
        CPTPProcessor *this,
        struct CPointerInputFrame *a2,
        __int64 a3,
        const struct PTPEnginePointerNode *a4,
        bool a5)
{
  __int64 v5; // rcx
  unsigned int v7; // edi
  unsigned int v8; // edx
  __int64 v9; // rcx
  const struct RIMDEV *v10; // rbp
  const struct tagPOINTEREVENTINT *v11; // rsi

  v5 = *((_QWORD *)a2 + 16);
  v7 = 0;
  v8 = 0;
  if ( *((_DWORD *)a2 + 12) )
  {
    v9 = v5 + 176;
    do
    {
      *(_WORD *)(v9 - 8) = *((_WORD *)a4 + 2);
      *(_OWORD *)v9 = *(_OWORD *)a4;
      *(_OWORD *)(v9 + 16) = *((_OWORD *)a4 + 1);
      *(_OWORD *)(v9 + 32) = *((_OWORD *)a4 + 2);
      *(_OWORD *)(v9 + 48) = *((_OWORD *)a4 + 3);
      *(_OWORD *)(v9 + 64) = *((_OWORD *)a4 + 4);
      *(_OWORD *)(v9 + 80) = *((_OWORD *)a4 + 5);
      *(_DWORD *)(v9 + 4) = 0;
      if ( a5 )
        *((_QWORD *)a2 + 13) = ++*(_QWORD *)(v9 + 80);
      *(_DWORD *)(v9 - 172) |= 0x1000u;
      a4 = (const struct PTPEnginePointerNode *)((char *)a4 + 96);
      v9 += 496LL;
      ++v8;
    }
    while ( v8 < *((_DWORD *)a2 + 12) );
  }
  if ( a5 )
  {
    v10 = *(const struct RIMDEV **)(*((_QWORD *)a2 + 18) + 16LL);
    InputTraceLogging::Pointer::CommitFrame(v10, a2);
    if ( *((_DWORD *)a2 + 12) )
    {
      v11 = (const struct tagPOINTEREVENTINT *)(*((_QWORD *)a2 + 16) + 168LL);
      do
      {
        InputTraceLogging::Pointer::CommitFramePointer(v10, a2, v11);
        ++v7;
        v11 = (const struct tagPOINTEREVENTINT *)((char *)v11 + 496);
      }
      while ( v7 < *((_DWORD *)a2 + 12) );
    }
  }
}
