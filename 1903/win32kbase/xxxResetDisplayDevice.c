/*
 * XREFs of xxxResetDisplayDevice @ 0x1C00639B0
 * Callers:
...
__int64 __fastcall xxxResetDisplayDevice(__int64 a1, int a2, __int64 a3)
{
  ...
  else
  {
    ...
    if ( (int)IszzzResetSharedDesktopsSupported() >= 0 )
      zzzResetSharedDesktops(a1, v6 != *((_WORD *)gpsi + 3498), v4);
    ResetSystemColors();
    ...
    if ( v4 && (int)IsIAMKeyAcquiredSupported() >= 0 && (unsigned int)IAMKeyAcquired(a1) )
    {
