...
void __fastcall std::list<std::pair<std::wstring const,std::wstring>>::~list<std::pair<std::wstring const,std::wstring>>(
        __int64 a1)
{
  _QWORD *v2; // rsi
  _QWORD *v3; // rcx
  ...
  *(_QWORD *)(a1 + 8) = 0LL;
  ...
  if ( v2 != v3 )
  {
    ...
    {
      v4 = (_QWORD *)*v2;
      std::pair<std::wstring const,std::wstring>::`scalar deleting destructor'(v2 + 2);
  ...
  std::_Deallocate<16,0>(v3, (const struct std::nothrow_t *)0x50);
}
