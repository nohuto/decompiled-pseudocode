/*
 * XREFs of ??R?$_Uhash_compare@IU?$hash@I@std@@U?$equal_to@I@2@@std@@QEBA_KAEBI@Z @ 0x180059274
 * Callers:
...
__int64 __fastcall std::_Uhash_compare<unsigned int,std::hash<unsigned int>,std::equal_to<unsigned int>>::operator()(
        __int64 a1,
        unsigned __int64 a2)
{
  __int64 result; // rax
  ...
  if ( a2 < a2 + 4 )
  {
    do
    {
      v4 = *v3++ ^ (unsigned __int64)result;
      result = 0x100000001B3LL * v4;
    }
    while ( &v3[-a2] != (unsigned __int8 *)(a2 + 4 >= a2 ? 4 : 0) );
  }
  return result;
}
