/*
 * XREFs of ?FindInsertionPointOrIncrement@RawUsageIndex@details_abi@wil@@AEAAPEAEAEAUUsageIndexProperty@23@PEAEPEAX_KI@Z @ 0x18004F6CC
 * Callers:
 *     ?RecordUsageInternal@RawUsageIndex@details_abi@wil@@AEAA_NPEAX_K01I@Z @ 0x180050D78 (-RecordUsageInternal@RawUsageIndex@details_abi@wil@@AEAA_NPEAX_K01I@Z.c)
 * Callees:
 *     ?Compare@UsageIndexProperty@details_abi@wil@@QEBAHPEAX_K@Z @ 0x18004F380 (-Compare@UsageIndexProperty@details_abi@wil@@QEBAHPEAX_K@Z.c)
 *     ?LowerBound@RawUsageIndex@details_abi@wil@@AEAAPEAEPEAE_KPEAX1@Z @ 0x18004FF08 (-LowerBound@RawUsageIndex@details_abi@wil@@AEAAPEAEPEAE_KPEAX1@Z.c)
 *     ?Read@UsageIndexProperty@details_abi@wil@@QEAA_NAEAPEAEPEAE@Z @ 0x180050784 (-Read@UsageIndexProperty@details_abi@wil@@QEAA_NAEAPEAEPEAE@Z.c)
 *     ?UpdateCount@UsageIndexProperty@details_abi@wil@@QEAAXI@Z @ 0x180052430 (-UpdateCount@UsageIndexProperty@details_abi@wil@@QEAAXI@Z.c)
 */

unsigned __int8 *__fastcall wil::details_abi::RawUsageIndex::FindInsertionPointOrIncrement(
        wil::details_abi::RawUsageIndex *this,
        struct wil::details_abi::UsageIndexProperty *a2,
        unsigned __int8 *a3,
        void *a4,
        size_t a5,
        unsigned int a6)
{
  char v6; // bl
  unsigned __int64 v7; // r15
  int v8; // r12d
  unsigned __int64 v13; // rax
  unsigned __int64 v14; // rcx
  unsigned __int64 v15; // rax
  size_t v16; // r14
  unsigned __int8 *v17; // r15
  unsigned __int8 *v18; // rax
  unsigned __int8 *v19; // r8
  unsigned int v20; // r15d
  unsigned __int8 *v21; // r8
  const void *v23; // [rsp+30h] [rbp-20h] BYREF
  __int16 v24; // [rsp+38h] [rbp-18h]
  __int128 v25; // [rsp+40h] [rbp-10h]
  unsigned __int8 *v26; // [rsp+80h] [rbp+30h] BYREF

  v6 = 0;
  v7 = *((_QWORD *)this + 2);
  v8 = -1;
  LOWORD(v23) = *((_WORD *)this + 3);
  BYTE2(v23) = *((_BYTE *)this + 8);
  HIDWORD(v23) = 0;
  v24 = 0;
  v25 = 0LL;
  if ( v7 )
  {
    v13 = *((_QWORD *)this + 4) - *((_QWORD *)this + 3);
    v14 = *((unsigned int *)a2 + 1);
    v15 = v13 / v7;
    if ( v14 > v15 )
    {
      wil::details_abi::UsageIndexProperty::UpdateCount(a2, v15);
      LODWORD(v14) = *((_DWORD *)a2 + 1);
      v7 = *((_QWORD *)this + 2);
    }
    v16 = a5;
    v17 = &a3[(unsigned int)v14 * v7];
    v18 = wil::details_abi::RawUsageIndex::LowerBound(this, a3, (unsigned int)v14, a4, a5);
    a3 = v18;
    if ( v18 >= v17 )
      return a3;
    v19 = (unsigned __int8 *)*((_QWORD *)this + 4);
    v26 = v18;
    wil::details_abi::UsageIndexProperty::Read((wil::details_abi::UsageIndexProperty *)&v23, &v26, v19);
    v8 = wil::details_abi::UsageIndexProperty::Compare(&v23, a4, v16);
  }
  else
  {
    v20 = 0;
    if ( !*((_DWORD *)a2 + 1) )
      return a3;
    while ( 1 )
    {
      v21 = (unsigned __int8 *)*((_QWORD *)this + 4);
      v26 = a3;
      if ( !wil::details_abi::UsageIndexProperty::Read((wil::details_abi::UsageIndexProperty *)&v23, &v26, v21) )
        break;
      v8 = wil::details_abi::UsageIndexProperty::Compare(&v23, a4, a5);
      if ( v8 > 0 )
      {
        a3 = v26;
        if ( ++v20 < *((_DWORD *)a2 + 1) )
          continue;
      }
      goto LABEL_12;
    }
    wil::details_abi::UsageIndexProperty::UpdateCount(a2, v20);
  }
LABEL_12:
  if ( !v8 )
  {
    if ( BYTE2(v23) )
    {
      wil::details_abi::UsageIndexProperty::UpdateCount((wil::details_abi::UsageIndexProperty *)&v23, a6 + HIDWORD(v23));
    }
    else if ( !*((_BYTE *)this + 56) )
    {
LABEL_17:
      *((_BYTE *)this + 56) = v6;
      return 0LL;
    }
    v6 = 1;
    goto LABEL_17;
  }
  return a3;
}
