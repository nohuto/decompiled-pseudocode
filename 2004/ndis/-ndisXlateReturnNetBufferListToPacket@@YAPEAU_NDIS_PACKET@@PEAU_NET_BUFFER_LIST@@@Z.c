/*
 * XREFs of ?ndisXlateReturnNetBufferListToPacket@@YAPEAU_NDIS_PACKET@@PEAU_NET_BUFFER_LIST@@@Z @ 0x1C0075804
 * Callers:
...
struct _NDIS_PACKET *__fastcall ndisXlateReturnNetBufferListToPacket(struct _NET_BUFFER_LIST *a1)
{
  ...
  *(_QWORD *)(a1->Link.Region + 32) = 0LL;
  ...
  if ( *(int *)ndisNblTrackerMode >= 3 )
    ndisNblTrackerRecordEventInternal(a1, 0LL, 0x1Cu, (unsigned __int64)v2, 0);
  if ( v3 < 0 )
  {
    if ( *(int *)ndisNblTrackerMode >= 3 )
      ndisNblTrackerRecordEventInternal(a1, 0LL, 0x11u, (unsigned __int64)v2, 0);
  }
  else
  {
