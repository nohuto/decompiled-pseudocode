/*
 * XREFs of ?_SnapshotWindows@CDesktopThumbnailBase@@IEAAJXZ @ 0x1800B7698
 * Callers:
    ...
    if ( WindowListForDesktop->Flink != WindowListForDesktop )
    {
      while ( 1 )
      {
        if ( Flink )
        {
          if ( Flink[24].Flink )
          {
            v6 = (unsigned __int8)Flink[38].Flink;
            if ( (v6 & 1) != 0
              && ((unsigned __int8)~(v6 >> 2) & (unsigned __int8)~(BYTE4(Flink[38].Flink) >> 3) & 1) != 0 )
            {
              if ( Flink[2].Blink )
              {
