/*
 * XREFs of RtlpQueryEnvironmentCache @ 0x180015DC0
 * Callers:
 *     RtlQueryEnvironmentVariable @ 0x1800157E0 (RtlQueryEnvironmentVariable.c)
 * Callees:
 *     NLS_UPCASE @ 0x180016160 (NLS_UPCASE.c)
 *     memmove @ 0x1800A3C00 (memmove.c)
 */

__int64 __fastcall RtlpQueryEnvironmentCache(
        _QWORD *a1,
        __int64 a2,
        __int64 a3,
        _WORD *a4,
        unsigned __int64 a5,
        unsigned __int64 *a6)
{
  _QWORD *v8; // r11
  unsigned int v9; // edi
  unsigned int *v10; // rbp
  int v11; // r13d
  unsigned int *v12; // r10
  _WORD *v13; // r9
  unsigned __int64 v14; // r15
  __int64 v15; // r14
  unsigned __int16 v16; // r12
  int v17; // ecx
  unsigned __int16 v18; // ax
  void *Environment; // rax
  const void *v21; // rdx
  unsigned __int64 v22; // rcx
  __int64 v23; // rbx
  unsigned __int64 v24; // [rsp+20h] [rbp-48h]
  __int64 v25; // [rsp+78h] [rbp+10h]
  unsigned __int16 v26; // [rsp+80h] [rbp+18h]

  v25 = a2;
  v8 = a1;
  if ( (unsigned __int64)(a3 - 1) > 0x13 )
    return 3221225728LL;
  v9 = 0;
  v10 = (unsigned int *)((char *)&RtlpEnvironLookupTable + 56 * a3 - 56);
  v11 = *v10;
  v12 = v10 + 2;
  v24 = (unsigned __int64)&v10[4 * *v10 + 2];
  if ( (unsigned __int64)(v10 + 2) >= v24 )
  {
LABEL_12:
    if ( v11 == 3 )
    {
      Environment = (void *)*((_QWORD *)v10 + 6);
    }
    else
    {
      Environment = (void *)qword_18016B160;
      if ( !qword_18016B160 )
        Environment = NtCurrentPeb()->ProcessParameters->Environment;
    }
    *v8 = Environment;
    return 3221225728LL;
  }
  while ( 1 )
  {
    v13 = *(_WORD **)v12;
    v14 = *(_QWORD *)v12 + 2 * a3;
    if ( *(_QWORD *)v12 >= v14 )
    {
LABEL_7:
      v17 = 0;
    }
    else
    {
      v15 = a2 - (_QWORD)v13;
      while ( 1 )
      {
        v16 = *(_WORD *)((char *)v13 + v15);
        if ( *v13 != v16 )
        {
          v26 = NLS_UPCASE((unsigned __int16)*v13);
          v18 = NLS_UPCASE(v16);
          if ( v26 != v18 )
            break;
        }
        if ( (unsigned __int64)++v13 >= v14 )
          goto LABEL_7;
      }
      v17 = v26 - v18;
    }
    if ( !v17 )
      break;
    a2 = v25;
    v12 += 4;
    if ( (unsigned __int64)v12 >= v24 )
      goto LABEL_12;
  }
  v21 = (const void *)(*(_QWORD *)v12 + 2 * (a3 + 1));
  v22 = ((__int64)(*((_QWORD *)v12 + 1) - (_QWORD)v21) >> 1) - 1;
  if ( !a4 )
    goto LABEL_22;
  if ( v22 >= a5 )
  {
    if ( a5 )
      *a4 = 0;
LABEL_22:
    v9 = -1073741789;
    *a6 = v22 + 1;
    return v9;
  }
  v23 = v22;
  *a6 = v22;
  memmove(a4, v21, 2 * v22);
  a4[v23] = 0;
  return v9;
}
