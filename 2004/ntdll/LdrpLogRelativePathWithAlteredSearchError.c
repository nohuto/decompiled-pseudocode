/*
 * XREFs of LdrpLogRelativePathWithAlteredSearchError @ 0x18007D824
 * Callers:
...
__int64 __fastcall LdrpLogRelativePathWithAlteredSearchError(__int64 a1)
{
  ...
  if ( v1 && (ProcessParameters = v1->ProcessParameters) != 0LL )
    ...
    Buffer = &word_180121374;
  result = (unsigned int)_InterlockedCompareExchange(&CentennialIssueTracker_InitializedState, 1, 0);
  if ( !(_DWORD)result )
  {
    TraceLoggingRegisterEx_EtwEventRegister_EtwEventSetInformation((ULONGLONG *)&dword_180164488);
    result = (unsigned int)_InterlockedExchange(&CentennialIssueTracker_InitializedState, 2);
  }
  if ( (unsigned int)dword_180164488 > 5 && CentennialIssueTracker_InitializedState == 2 )
  {
    tlgCreate1Sz_wchar_t(v9, a1);
    tlgCreate1Sz_wchar_t(v10, Buffer);
    return tlgWriteTransfer_EtwEventWriteTransfer((int)&dword_180164488, (int)&dword_18013030E, v6, v7, 4u, &v8);
  }
  return result;
}
