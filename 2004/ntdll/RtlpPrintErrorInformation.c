/*
 * XREFs of RtlpPrintErrorInformation @ 0x18010A6E4
 * Callers:
...
ULONG RtlpPrintErrorInformation()
{
  ...
  if ( NtCurrentPeb()->Ldr )
    ...
    DbgPrint("HEAP: ");
  DbgPrint("Heap error detected at %p (heap handle %p)\n", (const void *)qword_180167808, (const void *)qword_180167800);
  if ( dword_1801677F8 > 8 )
  {
    switch ( dword_1801677F8 )
    {
      case 9:
  ...
  else if ( dword_1801677F8 == 8 )
  {
    v0 = "heap_failure_block_not_busy";
  }
  else if ( dword_1801677F8 )
  {
    switch ( dword_1801677F8 )
    {
      case 1:
  ...
  if ( qword_180167820 )
  {
    if ( NtCurrentPeb()->Ldr )
      ...
      DbgPrint("HEAP: ");
    DbgPrint("Parameter3: %p\n", (const void *)qword_180167820);
  }
  if ( qword_180167828 || qword_180167830 )
  {
    if ( NtCurrentPeb()->Ldr )
      DbgPrint("HEAP[%wZ]: ", &NtCurrentPeb()->Ldr->InLoadOrderModuleList.Flink[5].Blink);
    else
    ...
    DbgPrint(
      ...
      (const void *)qword_180167828,
      (const void *)qword_180167830);
  }
  if ( NtCurrentPeb()->Ldr )
    DbgPrint("HEAP[%wZ]: ", &NtCurrentPeb()->Ldr->InLoadOrderModuleList.Flink[5].Blink);
  else
