/*
 * XREFs of RtlCheckHeldCriticalSections @ 0x18007D590
 * Callers:
 *     RtlCheckForOrphanedCriticalSections @ 0x18007D560 (RtlCheckForOrphanedCriticalSections.c)
 * Callees:
    ...
    if ( !RtlpCsVerifyDoNotBreak )
    {
      ...
      if ( !result->Ldr->ShutdownInProgress )
      {
        ...
        else
        {
          ...
          if ( !v27 )
            return result;
          result = (struct _PEB *)ZwQueryInformationThread(a1, 0LL, v25);
          if ( (int)result < 0 )
            return result;
          UniqueThread = v26;
        }
