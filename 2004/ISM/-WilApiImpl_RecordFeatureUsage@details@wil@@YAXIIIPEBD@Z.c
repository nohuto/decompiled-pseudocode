/*
 * XREFs of ?WilApiImpl_RecordFeatureUsage@details@wil@@YAXIIIPEBD@Z @ 0x1800679F0
 * Callers:
 *     <none>
 * Callees:
 *     ?ProcessShutdownInProgress@wil@@YA_NXZ @ 0x180011DEC (-ProcessShutdownInProgress@wil@@YA_NXZ.c)
 *     ?EnsureStateData@FeatureStateManager@details@wil@@AEAA_NXZ @ 0x180032CA4 (-EnsureStateData@FeatureStateManager@details@wil@@AEAA_NXZ.c)
 ...
 *     ?wil_StagingConfig_RecordFeatureUsage@@YAJIIH@Z @ 0x18006815C (-wil_StagingConfig_RecordFeatureUsage@@YAJIIH@Z.c)
 */
...
void __fastcall wil::details::WilApiImpl_RecordFeatureUsage(wil::details *this, unsigned int a2, unsigned int a3)
{
         ...
         && wil::details::FeatureStateManager::EnsureStateData((RTL_SRWLOCK *)&wil::details::g_featureStateManager) )
  {
    wil::details_abi::SubscriptionList::OnSignaled(
      (LPCRITICAL_SECTION)*(&xmmword_180208368 + 1) + 5,
      (PSRWLOCK)*(&xmmword_180208368 + 1));
  }
}
