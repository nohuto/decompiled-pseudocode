/*
 * XREFs of ?Initialize@CursorNotificationProcessor@@QEAAJPEAUICursorBroker@@@Z @ 0x18002EB58
 * Callers:
 *     ?Initialize@Win32kInterop@@QEAAJPEAUIRawInputClient@@PEAUISystemInputRouter@@@Z @ 0x18002D44C (-Initialize@Win32kInterop@@QEAAJPEAUIRawInputClient@@PEAUISystemInputRouter@@@Z.c)
 * Callees:
 *     ??4?$ComPtr@UIMessageSession@@@WRL@Microsoft@@QEAAAEAV012@AEBV012@@Z @ 0x18002C6E4 (--4-$ComPtr@UIMessageSession@@@WRL@Microsoft@@QEAAAEAV012@AEBV012@@Z.c)
 *     ?Return_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z @ 0x180035770 (-Return_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z.c)
 *     ?reset@?$unique_com_token@UIMessageSession@@_K$$A6AXPEAU1@_K@Z$1?IMessageSessionCloseEndpointFunction@details@wil@@YAX01@Z$0A@@wil@@QEAAXPEAUIMessageSession@@_K@Z @ 0x180036A9C (-reset@-$unique_com_token@UIMessageSession@@_K$$A6AXPEAU1@_K@Z$1-IMessageSessionCloseEndpointFun.c)
 *     ?InternalRelease@?$ComPtr@UIMessageProxy@@@WRL@Microsoft@@IEAAKXZ @ 0x1800375F0 (-InternalRelease@-$ComPtr@UIMessageProxy@@@WRL@Microsoft@@IEAAKXZ.c)
 *     __security_check_cookie @ 0x18003C4E0 (__security_check_cookie.c)
 *     _guard_dispatch_icall_nop @ 0x18004A6B0 (_guard_dispatch_icall_nop.c)
 */

// Hidden C++ exception states: #wind=3
__int64 __fastcall CursorNotificationProcessor::Initialize(CursorNotificationProcessor *this, struct ICursorBroker *a2)
{
  int v4; // eax
  unsigned int v5; // ebx
  __int64 v6; // rbx
  __int64 (__fastcall *v7)(__int64, const wchar_t *, struct ICursorBroker **); // rdi
  int v8; // eax
  __int64 v9; // rbx
  __int64 (__fastcall *v10)(__int64, _QWORD, CursorNotificationProcessor *, struct ICursorBroker *); // rdi
  int v11; // eax
  int v12; // eax
  int v13; // eax
  struct ICursorBroker *v14; // rax
  struct ICursorBroker *v15; // rbx
  struct ICursorBroker *v16; // rax
  __int64 v17; // rbx
  unsigned __int64 v19; // r9
  __int64 v20; // rdx
  int *v21; // [rsp+20h] [rbp-29h]
  __int64 v22; // [rsp+30h] [rbp-19h] BYREF
  struct ICursorBroker *v23; // [rsp+38h] [rbp-11h] BYREF
  __int64 v24; // [rsp+40h] [rbp-9h] BYREF
  int v25[2]; // [rsp+48h] [rbp-1h] BYREF
  struct ICursorBroker *v26; // [rsp+50h] [rbp+7h] BYREF
  _BYTE v27[40]; // [rsp+58h] [rbp+Fh] BYREF
  wil::details::in1diag3 *retaddr; // [rsp+A8h] [rbp+5Fh]

  v23 = 0LL;
  v24 = 0LL;
  *(_QWORD *)v25 = 0LL;
  v22 = 0LL;
  Microsoft::WRL::ComPtr<IMessageProxy>::InternalRelease(&v22);
  v4 = CoreUICreate(&v22);
  v5 = v4;
  if ( v4 < 0 )
  {
    v19 = (unsigned int)v4;
    v20 = 17LL;
LABEL_21:
    wil::details::in1diag3::Return_Hr(
      retaddr,
      (void *)v20,
      (unsigned int)"onecoreuap\\windows\\moderncore\\inputv2\\systeminputhosts\\components\\win32kinterop\\lib\\cursorno"
                    "tificationprocessor.cpp",
      (const char *)v19,
      (int)v21);
    goto LABEL_15;
  }
  wil::unique_com_token<IMessageSession,unsigned __int64,void (IMessageSession *,unsigned __int64),&void wil::details::IMessageSessionCloseEndpointFunction(IMessageSession *,unsigned __int64),0>::reset(
    &v24,
    v22);
  v6 = v22;
  v7 = *(__int64 (__fastcall **)(__int64, const wchar_t *, struct ICursorBroker **))(*(_QWORD *)v22 + 72LL);
  Microsoft::WRL::ComPtr<IMessageProxy>::InternalRelease(&v23);
  v8 = v7(v6, L"Kernel\\MIT\\InputPort", &v23);
  v5 = v8;
  if ( v8 < 0 )
  {
    v19 = (unsigned int)v8;
    v20 = 21LL;
    goto LABEL_21;
  }
  v9 = v22;
  v10 = *(__int64 (__fastcall **)(__int64, _QWORD, CursorNotificationProcessor *, struct ICursorBroker *))(*(_QWORD *)v22 + 96LL);
  wil::unique_com_token<IMessageSession,unsigned __int64,void (IMessageSession *,unsigned __int64),&void wil::details::IMessageSessionCloseEndpointFunction(IMessageSession *,unsigned __int64),0>::reset(
    &v24,
    v24);
  v21 = v25;
  v11 = v10(v9, CursorNotificationProcessor::OnMitMessageReceived, this, v23);
  v5 = v11;
  if ( v11 < 0 )
  {
    v19 = (unsigned int)v11;
    v20 = 28LL;
    goto LABEL_21;
  }
  v12 = (*(__int64 (__fastcall **)(__int64, _QWORD, _BYTE *))(*(_QWORD *)v22 + 128LL))(v22, *(_QWORD *)v25, v27);
  v5 = v12;
  if ( v12 < 0 )
  {
    v19 = (unsigned int)v12;
    v20 = 32LL;
    goto LABEL_21;
  }
  v13 = NtMITCoreMsgKOpenConnectionTo(1LL, v27);
  v5 = v13 | 0x10000000;
  if ( v13 < 0 )
  {
    v19 = v5;
    v20 = 36LL;
    goto LABEL_21;
  }
  v14 = *(struct ICursorBroker **)this;
  if ( *(struct ICursorBroker **)this != a2 )
  {
    if ( a2 )
    {
      (*(void (__fastcall **)(struct ICursorBroker *))(*(_QWORD *)a2 + 8LL))(a2);
      v14 = *(struct ICursorBroker **)this;
    }
    v26 = v14;
    *(_QWORD *)this = a2;
    Microsoft::WRL::ComPtr<IMessageProxy>::InternalRelease(&v26);
  }
  Microsoft::WRL::ComPtr<IMessageSession>::operator=((__int64 *)this + 1, &v22);
  v15 = v23;
  v23 = 0LL;
  v16 = (struct ICursorBroker *)*((_QWORD *)this + 3);
  if ( v16 != v15 )
  {
    if ( v15 )
    {
      (*(void (__fastcall **)(struct ICursorBroker *))(*(_QWORD *)v15 + 8LL))(v15);
      v16 = (struct ICursorBroker *)*((_QWORD *)this + 3);
    }
    v26 = v16;
    *((_QWORD *)this + 3) = v15;
    Microsoft::WRL::ComPtr<IMessageProxy>::InternalRelease(&v26);
  }
  v17 = *(_QWORD *)v25;
  *(_QWORD *)v25 = 0LL;
  wil::unique_com_token<IMessageSession,unsigned __int64,void (IMessageSession *,unsigned __int64),&void wil::details::IMessageSessionCloseEndpointFunction(IMessageSession *,unsigned __int64),0>::reset(
    &v24,
    0LL);
  *((_QWORD *)this + 4) = v17;
  v5 = 0;
LABEL_15:
  Microsoft::WRL::ComPtr<IMessageProxy>::InternalRelease(&v22);
  wil::unique_com_token<IMessageSession,unsigned __int64,void (IMessageSession *,unsigned __int64),&void wil::details::IMessageSessionCloseEndpointFunction(IMessageSession *,unsigned __int64),0>::reset(
    &v24,
    0LL);
  Microsoft::WRL::ComPtr<IMessageProxy>::InternalRelease(&v23);
  return v5;
}
