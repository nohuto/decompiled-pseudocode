/*
 * XREFs of sub_1C005C500 @ 0x1C005C500
 * Callers:
 *     sub_1C0043DE0 @ 0x1C0043DE0 (sub_1C0043DE0.c)
 *     sub_1C005C970 @ 0x1C005C970 (sub_1C005C970.c)
 * Callees:
 *     sub_1C00125E0 @ 0x1C00125E0 (sub_1C00125E0.c)
 *     sub_1C001BAFC @ 0x1C001BAFC (sub_1C001BAFC.c)
 *     __security_check_cookie @ 0x1C001CF60 (__security_check_cookie.c)
 */

NTSTATUS __fastcall sub_1C005C500(__int64 a1, const EVENT_DESCRIPTOR *a2)
{
  NTSTATUS result; // eax
  __int64 v5; // rcx
  _DWORD v6[2]; // [rsp+60h] [rbp+7h] BYREF
  int v7; // [rsp+68h] [rbp+Fh] BYREF
  __int16 v8; // [rsp+6Ch] [rbp+13h]
  __int16 v9; // [rsp+6Eh] [rbp+15h]
  __int16 v10; // [rsp+70h] [rbp+17h]
  __int16 v11; // [rsp+72h] [rbp+19h]
  _OWORD v12[2]; // [rsp+78h] [rbp+1Fh] BYREF
  int v13; // [rsp+98h] [rbp+3Fh]

  result = 0;
  v13 = 0;
  memset(v12, 0, sizeof(v12));
  if ( dword_1C006B650 )
  {
    v7 = *(_DWORD *)(a1 + 5192);
    v8 = *(_WORD *)(a1 + 5196);
    v9 = *(_DWORD *)(a1 + 5200);
    v10 = *(_WORD *)(a1 + 5228);
    v11 = *(_WORD *)(a1 + 5230);
    sub_1C001BAFC(a1, (__int64)v12);
    LODWORD(v5) = *(_DWORD *)(v5 + 4208);
    v6[1] = *(_DWORD *)(a1 + 3124);
    v6[0] = v5;
    return sub_1C00125E0(a2, 0LL, &v7, 12LL, v12, 36LL, v6, 8LL, a1 + 2936, 71LL, 0LL);
  }
  return result;
}
