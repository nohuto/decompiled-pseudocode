/*
 * XREFs of ?EndpointRequiresAC97VistaShim@@YAHPEAUIMMDevice@@@Z @ 0x1800DBE34
 * Callers:
...
_BOOL8 __fastcall EndpointRequiresAC97VistaShim(struct IMMDevice *a1)
{
  ...
  struct IUnknown *v11; // [rsp+48h] [rbp-B8h] BYREF
  ...
  struct IUnknown *v16; // [rsp+70h] [rbp-90h] BYREF
  ...
  *(_OWORD *)pvar = 0LL;
                  ...
                  if ( v3 >= 0 )
                  {
                    ...
                    if ( v3 >= 0 && LOWORD(pvar[0]) == 31 )
                    {
                      ...
                      *v8 = 0;
                      ...
                      if ( v5 )
                      {
                        v3 = StringCbCatW(SubKey, 0x208uLL, (const unsigned __int16 *)pvar[1]);
  ...
  if ( v3 < 0 )
    AudSrvTraceLoggingErrorHelper("EndpointRequiresAC97VistaShim", 5313, v3);
  if ( v13 )
