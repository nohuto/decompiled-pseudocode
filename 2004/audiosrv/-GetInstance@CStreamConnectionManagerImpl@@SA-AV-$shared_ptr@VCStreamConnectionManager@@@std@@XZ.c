/*
 * XREFs of ?GetInstance@CStreamConnectionManagerImpl@@SA?AV?$shared_ptr@VCStreamConnectionManager@@@std@@XZ @ 0x180061BBC
 * Callers:
 ...
 *     ??$?0$$V@?$_Ref_count_obj@VCStreamConnectionManagerImpl@@@std@@QEAA@XZ @ 0x180061D00 (--$-0$$V@-$_Ref_count_obj@VCStreamConnectionManagerImpl@@@std@@QEAA@XZ.c)
...
std::_Ref_count_base **__fastcall CStreamConnectionManagerImpl::GetInstance(std::_Ref_count_base **a1)
{
  ...
  *(_OWORD *)v12 = 0LL;
  ...
  else
  {
    ...
    if ( v6 && (int)CMonitorManager::Initialize(v6, 0x12u, 9uLL, &dword_18015D930) >= 0 )
    {
