/*
 * XREFs of CmFcManagerNotifyFeatureUsage @ 0x1404E86C8
 * Callers:
 *     wil_details_FeatureReporting_ReportUsageToServiceDirect @ 0x14027C178 (wil_details_FeatureReporting_ReportUsageToServiceDirect.c)
 *     RtlNotifyFeatureUsage @ 0x14057FF30 (RtlNotifyFeatureUsage.c)
 * Callees:
 *     KeLeaveCriticalRegion @ 0x140208500 (KeLeaveCriticalRegion.c)
 *     KeInsertQueueDpc @ 0x140214C00 (KeInsertQueueDpc.c)
 *     ObGetCurrentIrql @ 0x1402C8F70 (ObGetCurrentIrql.c)
 *     RtlAcquireSwapReference @ 0x1403A3D28 (RtlAcquireSwapReference.c)
 *     RtlReleaseSwapReference @ 0x1403A409C (RtlReleaseSwapReference.c)
 *     CmFcpWorkItemQueueWork @ 0x1404E885C (CmFcpWorkItemQueueWork.c)
 *     RtlpFcAddDelayedUsageReportToBuffer @ 0x140589DFC (RtlpFcAddDelayedUsageReportToBuffer.c)
 */

__int64 __fastcall CmFcManagerNotifyFeatureUsage(__int64 a1, __int64 a2)
{
  char v3; // si
  unsigned __int8 CurrentIrql; // bp
  struct _KTHREAD *CurrentThread; // rcx
  __int64 v6; // rdi
  __int64 v7; // rcx
  int v8; // ebx

  v3 = 0;
  CurrentIrql = ObGetCurrentIrql();
  if ( CurrentIrql < 2u )
  {
    CurrentThread = KeGetCurrentThread();
    --CurrentThread->KernelApcDisable;
    v3 = 1;
  }
  v6 = (unsigned int)RtlAcquireSwapReference(qword_140C48070);
  v7 = qword_140C48098[v6];
  if ( v7 )
  {
    v8 = RtlpFcAddDelayedUsageReportToBuffer(v7, a2);
    if ( v8 >= 0 )
    {
      RtlReleaseSwapReference(qword_140C48070, v6);
      LODWORD(v6) = -1;
      if ( byte_140C48090 )
      {
        if ( CurrentIrql >= 2u )
          KeInsertQueueDpc(&stru_140C480A8, 0LL, 0LL);
        else
          CmFcpWorkItemQueueWork(&stru_140C480E8);
      }
      v8 = 0;
    }
  }
  else
  {
    v8 = -1073741670;
  }
  if ( (_DWORD)v6 != -1 )
    RtlReleaseSwapReference(qword_140C48070, v6);
  if ( v3 )
    KeLeaveCriticalRegion();
  return (unsigned int)v8;
}
