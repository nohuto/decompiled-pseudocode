/*
 * XREFs of EtwpReferenceLastBranchLookasideList @ 0x140944B20
 * Callers:
...
struct _SLIST_ENTRY *EtwpReferenceLastBranchLookasideList()
{
  ...
  v1 = 2 * KeNumberProcessors_0 * _InterlockedIncrement(&dword_140C53B80);
  do
  {
    ...
    result = (struct _SLIST_ENTRY *)(unsigned int)_InterlockedIncrement(&dword_140C53B84);
  }
  while ( (int)result < v1 );
  return result;
}
