/*
 * XREFs of MiWalkVaRange @ 0x1402BB1D0
 * Callers:
 *     MiResetVirtualMemory @ 0x140353AC4 (MiResetVirtualMemory.c)
 *     MiProcessVaRangesInfoClass @ 0x140700488 (MiProcessVaRangesInfoClass.c)
 * Callees:
 *     MiLockProtoPoolPage @ 0x140229D70 (MiLockProtoPoolPage.c)
 *     MiUnlockProtoPoolPage @ 0x14022A960 (MiUnlockProtoPoolPage.c)
 *     MiPteInShadowRange @ 0x1402386B0 (MiPteInShadowRange.c)
 *     MiCopyOnWrite @ 0x140238700 (MiCopyOnWrite.c)
 *     MiFlushTbList @ 0x14023A3C0 (MiFlushTbList.c)
 *     MiGetNextPageTable @ 0x140256100 (MiGetNextPageTable.c)
 *     MiWorkingSetIsContended @ 0x1402589C0 (MiWorkingSetIsContended.c)
 *     MiUnlockPageTableInternal @ 0x140278B10 (MiUnlockPageTableInternal.c)
 *     MiGetProtoPteAddress @ 0x14029A830 (MiGetProtoPteAddress.c)
 *     MmAccessFault @ 0x14029F070 (MmAccessFault.c)
 *     MiUnlockWorkingSetShared @ 0x1402A1770 (MiUnlockWorkingSetShared.c)
 *     MiLockWorkingSetShared @ 0x1402AC150 (MiLockWorkingSetShared.c)
 *     MiInsertTbFlushEntry @ 0x1402B0EF0 (MiInsertTbFlushEntry.c)
 *     MiActOnPte @ 0x1402BB6D0 (MiActOnPte.c)
 *     MiReleasePageFileInfo @ 0x1402D4690 (MiReleasePageFileInfo.c)
 *     MI_PROTO_FORMAT_COMBINED @ 0x140315C30 (MI_PROTO_FORMAT_COMBINED.c)
 *     MiIsPrototypePteVadLookup @ 0x140315EC0 (MiIsPrototypePteVadLookup.c)
 *     KeShouldYieldProcessor @ 0x140327BB0 (KeShouldYieldProcessor.c)
 *     MiMakeProtoLeafValid @ 0x140349AB4 (MiMakeProtoLeafValid.c)
 *     __security_check_cookie @ 0x1403CCE60 (__security_check_cookie.c)
 *     memset @ 0x14040A280 (memset.c)
 *     MiCopyOnWriteCheckConditions @ 0x140556A58 (MiCopyOnWriteCheckConditions.c)
 */

__int64 __fastcall MiWalkVaRange(unsigned __int64 a1, unsigned __int64 a2, __int64 a3, int a4, __int64 a5)
{
  __int64 v5; // r14
  __int64 v8; // r13
  unsigned __int64 v9; // rdi
  unsigned __int64 v10; // rsi
  unsigned __int64 v11; // r15
  __int64 v12; // rbx
  _KPROCESS *v13; // rdx
  unsigned __int8 v14; // r12
  __int64 NextPageTable; // rax
  unsigned __int64 v16; // rbx
  unsigned int v17; // r12d
  __int64 v18; // rbx
  __int64 ProtoPteAddress; // rsi
  int v20; // r14d
  int v21; // r15d
  __int64 v22; // rbx
  int v23; // eax
  unsigned __int64 v24; // rdx
  __int64 v25; // rax
  unsigned int v26; // edi
  int v28; // esi
  __int64 v29; // rdx
  __int64 v30; // r8
  __int64 v31; // r9
  _KPROCESS *v32; // rdx
  struct _LIST_ENTRY *Flink; // rdx
  __int64 v34; // rax
  __int64 v35; // rdx
  __int64 v36; // rsi
  __int64 v37; // rax
  __int64 v38; // rbx
  __int64 v39; // rcx
  __int64 v40; // rsi
  _KPROCESS *v41; // rdx
  int v42; // eax
  unsigned __int8 v43; // [rsp+40h] [rbp-C0h] BYREF
  unsigned __int8 v44; // [rsp+41h] [rbp-BFh]
  int ProtoLeafValid; // [rsp+44h] [rbp-BCh]
  __int64 v46; // [rsp+48h] [rbp-B8h]
  __int64 v47; // [rsp+50h] [rbp-B0h]
  int v48; // [rsp+58h] [rbp-A8h]
  unsigned __int64 v49; // [rsp+60h] [rbp-A0h]
  unsigned __int64 *v50; // [rsp+68h] [rbp-98h]
  int v51; // [rsp+70h] [rbp-90h]
  unsigned __int64 v52; // [rsp+78h] [rbp-88h]
  int v53; // [rsp+80h] [rbp-80h] BYREF
  unsigned __int64 v54; // [rsp+88h] [rbp-78h] BYREF
  __int64 v55; // [rsp+90h] [rbp-70h] BYREF
  __int64 v56; // [rsp+98h] [rbp-68h] BYREF
  _KPROCESS *Process; // [rsp+A0h] [rbp-60h]
  int v58; // [rsp+B0h] [rbp-50h] BYREF
  __int16 v59; // [rsp+B4h] [rbp-4Ch]
  __int16 v60; // [rsp+B6h] [rbp-4Ah]
  __int64 v61; // [rsp+B8h] [rbp-48h]
  __int64 v62; // [rsp+C0h] [rbp-40h]
  __int64 v63; // [rsp+C8h] [rbp-38h]
  _BYTE v64[152]; // [rsp+D0h] [rbp-30h] BYREF

  v46 = a3;
  v5 = a3;
  v54 = 0LL;
  v53 = 0;
  v56 = 0LL;
  v48 = a4;
  v60 = 0;
  memset(v64, 0, sizeof(v64));
  ProtoLeafValid = 0;
  v8 = 0LL;
  v47 = 0LL;
  v43 = 17;
  v9 = ((a1 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL;
  v10 = ((a2 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL;
  v61 = 20LL;
  v58 = 1;
  v11 = 0LL;
  v59 = 0;
  v62 = 0LL;
  v63 = 0LL;
  v52 = v10;
  v49 = 0LL;
  Process = KeGetCurrentThread()->ApcState.Process;
  v12 = (__int64)&Process[1].ActiveProcessorsPadding[6];
  v50 = &Process[1].ActiveProcessorsPadding[6];
  v44 = MiLockWorkingSetShared((__int64)&Process[1].ActiveProcessorsPadding[6]);
  v14 = v44;
  if ( v9 > v10 )
  {
    v26 = 0;
    goto LABEL_36;
  }
LABEL_2:
  if ( v11 )
  {
    MiUnlockPageTableInternal(v12, v11);
    v11 = 0LL;
    v49 = 0LL;
  }
  NextPageTable = MiGetNextPageTable(v9, v10, (__int64)&v58, v14, 0, &v53);
  v51 = 1;
  v16 = NextPageTable;
  if ( NextPageTable == v9 )
    goto LABEL_5;
  if ( (*(_DWORD *)(v5 + 48) & 0x100000) == 0
    && *(_QWORD *)(v5 + 80)
    && MiGetProtoPteAddress(v5, (unsigned __int64)((__int64)(v9 << 25) >> 16) >> 12, 0, &v55) )
  {
    v51 = 0;
    goto LABEL_5;
  }
  v26 = -1073740748;
  ProtoLeafValid = -1073740748;
  if ( v16 )
  {
    v9 = v16;
LABEL_5:
    if ( v16 )
      v49 = ((v16 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL;
    v17 = v51;
    v18 = v46;
    while ( 1 )
    {
      ProtoPteAddress = 0LL;
      v20 = 0;
      v21 = 0;
      if ( !v17 )
        goto LABEL_62;
      v22 = *(_QWORD *)v9;
      if ( MiPteInShadowRange(v9)
        && (MiFlags & 0xC00000) != 0
        && KeGetCurrentThread()->ApcState.Process->AddressPolicy != 1
        && (v22 & 1) != 0
        && ((v22 & 0x20) == 0 || (v22 & 0x42) == 0) )
      {
        Flink = KeGetCurrentThread()->ApcState.Process[1].ProcessListEntry.Flink;
        if ( Flink )
        {
          v34 = *((_QWORD *)&Flink->Flink + ((v9 >> 3) & 0x1FF));
          v35 = v22 | 0x20;
          if ( (v34 & 0x20) == 0 )
            v35 = v22;
          v22 = v35;
          if ( (v34 & 0x42) != 0 )
            v22 = v35 | 0x42;
        }
      }
      if ( !v22 )
        break;
      if ( (v22 & 1) != 0 || (v22 & 0x400) == 0 )
      {
        v18 = v46;
        goto LABEL_14;
      }
      v28 = v48;
      if ( v48 == 2 && (unsigned __int8)MI_PROTO_FORMAT_COMBINED(v22) )
      {
        v36 = v22;
        if ( qword_140C4DD40 && (v22 & 0x10) == 0 )
          v36 = v22 & ~qword_140C4DD40;
        ProtoPteAddress = v36 >> 16;
LABEL_55:
        v18 = v46;
        if ( ProtoPteAddress )
        {
LABEL_56:
          if ( ((ProtoPteAddress ^ v47) & 0xFFFFFFFFFFFFF000uLL) != 0 )
          {
            if ( v8 )
            {
              MiUnlockProtoPoolPage(v8, v43);
              v47 = 0LL;
            }
            v8 = MiLockProtoPoolPage(ProtoPteAddress, &v43);
            if ( !v8 )
            {
              MiFlushTbList((__int64)&v58, v32);
              v11 = v49;
              v12 = (__int64)v50;
              if ( v49 )
              {
                MiUnlockPageTableInternal((__int64)v50, v49);
                v11 = 0LL;
                v49 = 0LL;
              }
              v14 = v44;
              MiUnlockWorkingSetShared(v12, v44);
              MmAccessFault(2uLL, ProtoPteAddress, 0, 0LL);
              v10 = v52;
LABEL_33:
              MiLockWorkingSetShared(v12);
              v5 = v46;
              if ( v9 > v10 )
              {
                v26 = ProtoLeafValid;
                goto LABEL_35;
              }
              goto LABEL_2;
            }
            v47 = ProtoPteAddress;
          }
        }
        else
        {
LABEL_14:
          if ( v8 )
          {
            MiUnlockProtoPoolPage(v8, v43);
            v8 = 0LL;
            v47 = 0LL;
          }
        }
        v23 = MiActOnPte(v18, v17, v9, ProtoPteAddress, v48, a5, &v56, &v54);
        if ( v23 )
        {
          if ( v23 == 274 )
          {
            MiInsertTbFlushEntry((__int64)&v58, (__int64)(v9 << 25) >> 16, 1LL, 0);
            MiFlushTbList((__int64)&v58, v41);
            v42 = MiCopyOnWrite((__int64)(v9 << 25) >> 16, (ULONG_PTR *)v9, -1LL, 0);
            v18 = v46;
            v21 = v42;
            v9 -= 8LL;
            v20 = 1;
          }
          else if ( v23 == -1073741791 )
          {
            MiInsertTbFlushEntry((__int64)&v58, (__int64)(v9 << 25) >> 16, 1LL, 0);
          }
          else
          {
            ProtoLeafValid = v23;
          }
        }
        v24 = v54;
        if ( v54 )
        {
          if ( v8 )
          {
            MiUnlockProtoPoolPage(v8, v43);
            v24 = v54;
            v8 = 0LL;
            v47 = 0LL;
          }
          MiReleasePageFileInfo(v56, v24, 1LL);
        }
        goto LABEL_18;
      }
      if ( (unsigned __int8)MI_PROTO_FORMAT_COMBINED(v22) )
      {
        if ( v28 == 1 )
        {
          v18 = v46;
          ProtoLeafValid = -1073740748;
        }
        else
        {
          if ( v8 )
            MiUnlockProtoPoolPage(v8, v43);
          v37 = 0LL;
          if ( !v8 )
            v37 = v47;
          v38 = v37;
          v47 = v37;
          MiFlushTbList((__int64)&v58, (_KPROCESS *)v24);
          ProtoLeafValid = MiMakeProtoLeafValid(v9);
          if ( ProtoLeafValid >= 0 )
          {
            v9 -= 8LL;
            v47 = v38;
            v18 = v46;
            v8 = 0LL;
          }
          else
          {
            v18 = v46;
            v8 = 0LL;
            ProtoLeafValid = -1073740748;
          }
        }
        goto LABEL_18;
      }
      v24 = (__int64)(v9 << 25) >> 16;
      if ( ((v24 & 0xFFFFFFFFFFFFF000uLL) == 0x7FFE0000
         || (v24 & 0xFFFFFFFFFFFFF000uLL) == qword_140C4DC48 && qword_140C4DC48)
        && (HIDWORD(Process[2].Header.WaitListHead.Flink) & 1) == 0 )
      {
        v18 = v46;
        ProtoLeafValid = -1073740748;
      }
      else
      {
        if ( (*(_DWORD *)(v46 + 48) & 0x100000) == 0 && *(_QWORD *)(v46 + 80) )
        {
          ProtoPteAddress = MiGetProtoPteAddress(v46, v24 >> 12, 0, &v55);
          if ( (unsigned int)MiIsPrototypePteVadLookup(v22, v29, v30, v31) )
            goto LABEL_55;
          v39 = v22;
          if ( qword_140C4DD40 && (v22 & 0x10) == 0 )
            v39 = v22 & ~qword_140C4DD40;
          if ( ProtoPteAddress == v39 >> 16 )
            goto LABEL_55;
        }
        v40 = v22;
        if ( qword_140C4DD40 && (v22 & 0x10) == 0 )
          v40 = v22 & ~qword_140C4DD40;
        ProtoPteAddress = v40 >> 16;
        if ( v48 == 2 )
          goto LABEL_55;
        v18 = v46;
        ProtoLeafValid = -1073740748;
      }
LABEL_18:
      v10 = v52;
      v9 += 8LL;
      if ( v20 == 1
        || v21 < 0
        || v9 > v52
        || (v9 & 0xFFF) == 0
        || (v9 & 0x78) == 0 && MiWorkingSetIsContended((__int64)v50)
        || KeShouldYieldProcessor() )
      {
        if ( v8 )
          MiUnlockProtoPoolPage(v8, v43);
        v25 = 0LL;
        if ( !v8 )
          v25 = v47;
        v47 = v25;
        MiFlushTbList((__int64)&v58, (_KPROCESS *)v24);
        v12 = (__int64)v50;
        if ( v49 )
        {
          MiUnlockPageTableInternal((__int64)v50, v49);
          v49 = 0LL;
        }
        v14 = v44;
        MiUnlockWorkingSetShared(v12, v44);
        if ( v21 == -1073740748 )
          MiCopyOnWriteCheckConditions(v12, 3221226548LL);
        v11 = v49;
        v8 = 0LL;
        goto LABEL_33;
      }
    }
    v18 = v46;
LABEL_62:
    if ( (*(_DWORD *)(v18 + 48) & 0x100000) != 0 || !*(_QWORD *)(v18 + 80) )
      goto LABEL_14;
    ProtoPteAddress = MiGetProtoPteAddress(v18, (unsigned __int64)((__int64)(v9 << 25) >> 16) >> 12, 0, &v55);
    if ( ProtoPteAddress )
      goto LABEL_56;
    if ( v17 )
      goto LABEL_14;
    v20 = 1;
    v9 -= 8LL;
    goto LABEL_18;
  }
  v12 = (__int64)v50;
LABEL_35:
  v14 = v44;
LABEL_36:
  MiFlushTbList((__int64)&v58, v13);
  if ( v11 )
    MiUnlockPageTableInternal(v12, v11);
  MiUnlockWorkingSetShared(v12, v14);
  return v26;
}
