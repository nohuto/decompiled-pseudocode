/*
 * XREFs of KeQueryTotalCycleTimeThread @ 0x140296BE0
 * Callers:
...
ULONG64 __stdcall KeQueryTotalCycleTimeThread(PKTHREAD Thread, PULONG64 CycleTimeStamp)
{
  ...
  volatile signed __int32 *v33; // [rsp+38h] [rbp-100h] BYREF
  ...
  if ( Thread == KeGetCurrentThread() )
  {
    ...
    if ( !v12 )
      KiEndThreadAccountingPeriod((__int64)CurrentPrcb, (__int64)Thread, v9);
    if ( CycleTimeStamp )
      *CycleTimeStamp = v8;
...
LABEL_16:
    ...
    if ( (unsigned __int8)KiAcquireThreadStateLock((__int64)Thread, &v34, &v33) == 2 )
    {
