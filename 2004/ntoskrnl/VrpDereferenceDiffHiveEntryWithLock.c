/*
 * XREFs of VrpDereferenceDiffHiveEntryWithLock @ 0x1405CCEC4
 * Callers:
...
void __fastcall VrpDereferenceDiffHiveEntryWithLock(volatile signed __int64 *P)
{
  ...
  if ( v1 <= 0 )
  {
    if ( v1 )
      __fastfail(0xEu);
    v3 = P[1] & (-1LL << (dword_140C477C4 & 0x1F));
    for ( i = (volatile signed __int64 *)(qword_140C477C8
                                        + 8LL
                                        * ((37
                                          * (BYTE6(v3)
                                           + 37
                                          ...
                                          + HIBYTE(v3)) & (((unsigned int)dword_140C477C4 >> 5) - 1)));
          (*i & 1) == 0;
