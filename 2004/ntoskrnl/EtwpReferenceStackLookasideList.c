/*
 * XREFs of EtwpReferenceStackLookasideList @ 0x140932320
 * Callers:
...
struct _SLIST_ENTRY *EtwpReferenceStackLookasideList()
{
  ...
  v0 = 2 * KeNumberProcessors_0 * _InterlockedIncrement(&dword_140C53B60);
  do
  {
    ...
    result = (struct _SLIST_ENTRY *)(unsigned int)_InterlockedIncrement(&dword_140C53B64);
  }
  while ( (int)result < v0 );
  return result;
}
