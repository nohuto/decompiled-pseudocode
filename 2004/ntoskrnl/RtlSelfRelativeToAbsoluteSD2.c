/*
 * XREFs of RtlSelfRelativeToAbsoluteSD2 @ 0x14090ECE0
 * Callers:
 *     RtlpSysVolCheckOwnerAndSecurity @ 0x14071FD98 (RtlpSysVolCheckOwnerAndSecurity.c)
 * Callees:
 *     memmove @ 0x140409FC0 (memmove.c)
 *     RtlpQuerySecurityDescriptor @ 0x140756394 (RtlpQuerySecurityDescriptor.c)
 */

__int64 __fastcall RtlSelfRelativeToAbsoluteSD2(__int64 a1, unsigned int *a2)
{
  unsigned __int64 v2; // rbx
  __int16 v6; // r14
  unsigned __int64 v7; // rdx
  unsigned __int64 v8; // r12
  unsigned __int64 v9; // rax
  unsigned __int64 v10; // r15
  unsigned __int64 v11; // r13
  unsigned int v12; // ecx
  unsigned __int64 v13; // rax
  unsigned __int64 v14; // rax
  unsigned __int64 v15; // rax
  unsigned int v16; // [rsp+50h] [rbp-30h] BYREF
  unsigned int v17; // [rsp+54h] [rbp-2Ch]
  unsigned __int64 v18; // [rsp+58h] [rbp-28h] BYREF
  unsigned __int64 v19; // [rsp+60h] [rbp-20h] BYREF
  unsigned __int64 v20; // [rsp+68h] [rbp-18h] BYREF
  unsigned __int64 v21; // [rsp+70h] [rbp-10h] BYREF
  unsigned int v22; // [rsp+C0h] [rbp+40h] BYREF
  unsigned int v23; // [rsp+D0h] [rbp+50h] BYREF
  unsigned int v24; // [rsp+D8h] [rbp+58h] BYREF

  v2 = 0LL;
  v18 = 0LL;
  v19 = 0LL;
  v20 = 0LL;
  v21 = 0LL;
  v24 = 0;
  v16 = 0;
  v22 = 0;
  v23 = 0;
  if ( !a1 )
    return 3221225711LL;
  if ( !a2 )
    return 3221225712LL;
  v17 = *a2;
  if ( v17 < 0x14 )
    return 3221225485LL;
  v6 = *(_WORD *)(a1 + 2);
  if ( v6 >= 0 )
    return 3221225703LL;
  RtlpQuerySecurityDescriptor(a1, &v18, &v22, &v19, &v23, &v20, &v24, &v21, &v16);
  v7 = v18;
  v8 = v19;
  if ( v18 <= v19 )
    v9 = v19 + v23;
  else
    v9 = v18 + v22;
  v10 = v20;
  if ( v9 <= v20 )
    v9 = v20 + v24;
  v11 = v21;
  if ( v9 <= v21 )
    v9 = v21 + v16;
  v12 = 40;
  if ( v9 )
    v12 = ((v9 - a1 - 13) & 0xFFFFFFF8) + 40;
  if ( v12 <= v17 )
  {
    if ( v9 )
    {
      memmove((void *)(a1 + 40), (const void *)(a1 + 20), v12 - 40LL);
      v7 = v18;
      v6 = *(_WORD *)(a1 + 2);
    }
    v13 = v7 + 20;
    *(_WORD *)(a1 + 2) = v6 & 0x7FFF;
    if ( !v7 )
      v13 = 0LL;
    *(_QWORD *)(a1 + 8) = v13;
    v14 = v8 + 20;
    if ( !v8 )
      v14 = 0LL;
    *(_QWORD *)(a1 + 16) = v14;
    v15 = v11 + 20;
    if ( !v11 )
      v15 = 0LL;
    *(_QWORD *)(a1 + 24) = v15;
    if ( v10 )
      v2 = v10 + 20;
    *(_QWORD *)(a1 + 32) = v2;
    return 0LL;
  }
  else
  {
    *a2 = v12;
    return 3221225507LL;
  }
}
