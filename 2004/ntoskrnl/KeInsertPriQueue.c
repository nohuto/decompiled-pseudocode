/*
 * XREFs of KeInsertPriQueue @ 0x1402BCC20
 * Callers:
 *     ExpTryQueueWorkItem @ 0x1402BCA80 (ExpTryQueueWorkItem.c)
 * Callees:
 *     KiAcquireKobjectLockSafe @ 0x140280030 (KiAcquireKobjectLockSafe.c)
 *     KiExitDispatcher @ 0x140286FF0 (KiExitDispatcher.c)
 *     KiWakePriQueueWaiter @ 0x1402FCB40 (KiWakePriQueueWaiter.c)
 */

__int64 __fastcall KeInsertPriQueue(__int64 a1, __int64 a2, __int64 a3, _DWORD *SchedulerAssist, char a5)
{
  __int64 v5; // r12
  _QWORD *v6; // rdi
  unsigned __int8 v8; // r14
  unsigned __int8 CurrentIrql; // r13
  struct _KPRCB *CurrentPrcb; // rbp
  _KTHREAD *CurrentThread; // rbx
  unsigned int v12; // edx
  __int64 v13; // r8
  __int64 v14; // rax
  int v15; // ecx
  unsigned int v16; // ecx
  __int64 *v17; // rbx
  unsigned int v18; // ecx
  __int64 v20; // rcx
  __int64 **v21; // rax
  __int64 *v22; // [rsp+68h] [rbp+10h]

  v22 = (__int64 *)a2;
  v5 = (int)a3;
  v6 = (_QWORD *)(a1 + 8);
  v8 = 0;
  CurrentIrql = KeGetCurrentIrql();
  __writecr8(2uLL);
  if ( KiIrqlFlags && (KiIrqlFlags & 1) != 0 && CurrentIrql <= 0xFu )
  {
    SchedulerAssist = KeGetCurrentPrcb()->SchedulerAssist;
    a2 = (-1LL << (CurrentIrql + 1)) & 4;
    a3 = (unsigned int)a2 | SchedulerAssist[5];
    SchedulerAssist[5] = a3;
  }
  CurrentPrcb = KeGetCurrentPrcb();
  CurrentThread = CurrentPrcb->CurrentThread;
  KiAcquireKobjectLockSafe((volatile signed __int32 *)a1, a2, a3, (__int64)SchedulerAssist);
  if ( (_QWORD *)*v6 != v6
    && (CurrentThread->Queue != (_DISPATCHER_HEADER *volatile)a1 || CurrentThread->WaitReason != 15) )
  {
    v12 = 0;
    v13 = a1 + 664;
    v14 = 32LL;
    while ( 1 )
    {
      v15 = *(_DWORD *)(v13 - 4);
      v13 -= 4LL;
      v12 += v15;
      --v14;
      v16 = *(_DWORD *)(a1 + 664);
      if ( v12 >= v16 )
        break;
      if ( v14 <= v5 )
      {
        if ( v12 >= v16 )
          break;
        v17 = v22;
        v8 = KiWakePriQueueWaiter(CurrentPrcb, a1, v22, (unsigned int)v5);
        if ( !v8 )
          goto LABEL_14;
        goto LABEL_9;
      }
    }
  }
  v17 = v22;
LABEL_14:
  if ( (a5 & 2) == 0 )
  {
    ++*(_DWORD *)(a1 + 4);
    v20 = 16 * v5 + a1 + 24;
    v21 = *(__int64 ***)(v20 + 8);
    if ( *v21 != (__int64 *)v20 )
      __fastfail(3u);
    *v17 = v20;
    v17[1] = (__int64)v21;
    *v21 = v17;
    *(_QWORD *)(v20 + 8) = v17;
  }
LABEL_9:
  _InterlockedAnd((volatile signed __int32 *)a1, 0xFFFFFF7F);
  if ( (a5 & 1) != 0 )
    v18 = 3;
  else
    v18 = 0;
  KiExitDispatcher((__int64)CurrentPrcb, v18, 1LL, 0LL, CurrentIrql);
  return v8;
}
