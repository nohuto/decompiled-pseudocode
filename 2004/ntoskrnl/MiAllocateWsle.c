/*
 * XREFs of MiAllocateWsle @ 0x1402A3CA0
 * Callers:
 *     MiMakeSystemCacheRangeValid @ 0x14022D030 (MiMakeSystemCacheRangeValid.c)
 *     MiIssueHardFault @ 0x14025B740 (MiIssueHardFault.c)
 *     MiCompleteRestrictedImageFault @ 0x14025BEB0 (MiCompleteRestrictedImageFault.c)
 *     MiMakeSystemCachePteValid @ 0x14026255C (MiMakeSystemCachePteValid.c)
 *     MiCompletePrivateZeroFault @ 0x1402A2830 (MiCompletePrivateZeroFault.c)
 *     MiCompleteProtoPteFault @ 0x1402A6230 (MiCompleteProtoPteFault.c)
 *     MiResolveTransitionFault @ 0x1402A8C30 (MiResolveTransitionFault.c)
 *     MiInitializeWorkingSetList @ 0x1402FDAD4 (MiInitializeWorkingSetList.c)
 *     MiResolveProtoCombine @ 0x1403688A4 (MiResolveProtoCombine.c)
 *     MiCreateForkWsle @ 0x14055477C (MiCreateForkWsle.c)
 * Callees:
 *     MI_READ_PTE_LOCK_FREE @ 0x140238530 (MI_READ_PTE_LOCK_FREE.c)
 *     KeYieldProcessorEx @ 0x140280C50 (KeYieldProcessorEx.c)
 *     MiAddWorkingSetEntries @ 0x1402A43A0 (MiAddWorkingSetEntries.c)
 *     MiGetPagePrivilege @ 0x1402B42D0 (MiGetPagePrivilege.c)
 *     MiReleasePageFileInfo @ 0x1402D4690 (MiReleasePageFileInfo.c)
 *     MiReleaseWsSwapReservationPfn @ 0x1402E50AC (MiReleaseWsSwapReservationPfn.c)
 *     MiWritePteShadow @ 0x14032ECC0 (MiWritePteShadow.c)
 *     MiPteHasShadow @ 0x14032ED20 (MiPteHasShadow.c)
 *     MiMarkPfnVerified @ 0x14034461C (MiMarkPfnVerified.c)
 *     MI_PFN_IS_PROTO @ 0x1403EEC18 (MI_PFN_IS_PROTO.c)
 *     KeBugCheckEx @ 0x1403F70D0 (KeBugCheckEx.c)
 *     KeSetPagePrivilege @ 0x14050DA08 (KeSetPagePrivilege.c)
 *     MiIncrementVmFaultCount @ 0x14052B040 (MiIncrementVmFaultCount.c)
 *     MiLogAllocateWsleEvent @ 0x14052B068 (MiLogAllocateWsleEvent.c)
 *     MiCompleteSecureProcessFault @ 0x140542CB4 (MiCompleteSecureProcessFault.c)
 *     MiFillVirtualFaultInfo @ 0x140543014 (MiFillVirtualFaultInfo.c)
 *     MiGetVirtualFaultPageInfo @ 0x14054329C (MiGetVirtualFaultPageInfo.c)
 *     MiLockWorkingSetCoreExclusive @ 0x14054A2E0 (MiLockWorkingSetCoreExclusive.c)
 *     MiUnlockWorkingSetCoreExclusive @ 0x14054A44C (MiUnlockWorkingSetCoreExclusive.c)
 */

__int64 __fastcall MiAllocateWsle(
        __int64 a1,
        unsigned __int64 a2,
        ULONG_PTR a3,
        char a4,
        __int64 a5,
        char a6,
        __int64 a7)
{
  int v7; // esi
  unsigned __int64 v9; // rcx
  __int64 v10; // r9
  struct _KTHREAD *CurrentThread; // rax
  unsigned int Queue; // edx
  unsigned int v13; // edx
  char v14; // al
  unsigned int v15; // edx
  signed __int64 v16; // rbx
  unsigned int v17; // r12d
  unsigned int v18; // edi
  int v19; // edi
  unsigned __int8 v20; // r13
  __int64 v21; // rdx
  ULONG_PTR v22; // r14
  ULONG_PTR v23; // rbx
  __int64 v24; // rax
  __int64 v25; // rcx
  ULONG_PTR v26; // r14
  char v27; // al
  signed __int64 v28; // rcx
  char v29; // r15
  unsigned __int64 v30; // r12
  int v31; // eax
  ULONG_PTR v32; // rbx
  __int64 v33; // r8
  __int64 v34; // rdx
  __int64 v35; // r11
  __int64 VirtualFaultPageInfo; // r15
  int v37; // edx
  signed __int64 v38; // rdx
  _QWORD *v39; // r8
  int v40; // eax
  char v41; // al
  unsigned __int64 v42; // rax
  int v43; // eax
  __int64 v44; // rdx
  __int64 v45; // r9
  unsigned __int8 v47; // [rsp+30h] [rbp-50h]
  unsigned int v48; // [rsp+34h] [rbp-4Ch]
  unsigned __int64 v49; // [rsp+38h] [rbp-48h]
  int v50; // [rsp+40h] [rbp-40h] BYREF
  int v51; // [rsp+44h] [rbp-3Ch] BYREF
  int v52; // [rsp+48h] [rbp-38h] BYREF
  int v53; // [rsp+4Ch] [rbp-34h] BYREF
  __int64 v54; // [rsp+50h] [rbp-30h]
  __int128 v55; // [rsp+58h] [rbp-28h] BYREF
  __int128 v56; // [rsp+68h] [rbp-18h] BYREF
  __int64 v57; // [rsp+78h] [rbp-8h]
  ULONG_PTR BugCheckParameter2; // [rsp+D0h] [rbp+50h] BYREF

  BugCheckParameter2 = a3;
  v7 = *(_DWORD *)(a1 + 184);
  v48 = 0;
  v9 = (__int64)(a2 << 25) >> 16;
  v49 = v9;
  if ( v9 < 0xFFFFF68000000000uLL || v9 > 0xFFFFF6FFFFFFFFFFuLL )
  {
    v10 = 1LL;
    v54 = 1LL;
    v48 = 1;
  }
  else
  {
    v10 = 0LL;
    v54 = 0LL;
    if ( (v7 & 7) == 0 && (((unsigned __int8)*(_QWORD *)a3 >> 1) & 7) != 0 )
      *(_QWORD *)a3 &= 0xFFFFFFFFFFFFFFF1uLL;
  }
  CurrentThread = KeGetCurrentThread();
  LOBYTE(a3) = 0;
  v47 = 0;
  Queue = (unsigned int)CurrentThread[1].Queue;
  if ( (Queue & 0x100) != 0 )
  {
    v13 = Queue >> 9;
  }
  else
  {
    v13 = (*((_DWORD *)&CurrentThread[1].SwapListEntry + 2) >> 12) & 7;
    if ( (CurrentThread->Process[1].DirectoryTableBase & 0x10000000000000LL) != 0 )
    {
      v14 = 2;
      if ( v13 < 2 )
        v14 = v13;
      LOBYTE(v13) = v14;
    }
  }
  v15 = v13 & 7;
  v16 = a5;
  v17 = 5;
  v18 = 8 * v15;
  if ( v10 )
  {
    if ( (a6 & 2) != 0 )
    {
      LOBYTE(a3) = 9;
    }
    else if ( v15 >= 5 )
    {
      if ( (a6 & 8) != 0 )
      {
        LOBYTE(a3) = 6;
      }
      else
      {
        if ( (a5 & 1) == 0 )
        {
LABEL_26:
          v20 = a3 | (16 * (a4 & 7));
          goto LABEL_27;
        }
        a3 = (a5 & 0x20) == 0;
      }
    }
    else
    {
      LOBYTE(a3) = 7;
    }
    v47 = a3;
    goto LABEL_26;
  }
  v19 = 5;
  if ( v15 > 5 )
    v19 = v15;
  v20 = 0;
  v18 = 8 * v19;
LABEL_27:
  v21 = 0x7FFFFFFFFFFFFFFFLL;
  if ( (a6 & 1) != 0 )
  {
    v22 = BugCheckParameter2;
    v50 = 0;
    while ( _interlockedbittestandset64((volatile signed __int32 *)(v22 + 24), 0x3FuLL) )
    {
      do
        KeYieldProcessorEx(&v50, v21, a3, v10);
      while ( *(__int64 *)(v22 + 24) < 0 );
    }
    v23 = BugCheckParameter2;
    v24 = MiReleaseWsSwapReservationPfn(BugCheckParameter2, v21);
    v25 = *(_QWORD *)(qword_140C4E448 + 8 * ((*(_QWORD *)(v23 + 40) >> 39) & 0x3FFLL));
    v21 = 0x7FFFFFFFFFFFFFFFLL;
    _InterlockedAnd64((volatile signed __int64 *)(v22 + 24), 0x7FFFFFFFFFFFFFFFuLL);
    if ( v24 )
    {
      MiReleasePageFileInfo(v25, v24, 1LL);
      v21 = 0x7FFFFFFFFFFFFFFFLL;
    }
    v16 = a5;
  }
  v26 = BugCheckParameter2;
  v27 = *(_BYTE *)(BugCheckParameter2 + 35);
  if ( (v27 & 8) == 0 )
    v17 = v27 & 7;
  if ( v17 < v18 >> 3 )
  {
    v51 = 0;
    if ( _interlockedbittestandset64((volatile signed __int32 *)(BugCheckParameter2 + 24), 0x3FuLL) )
    {
      do
      {
        do
          KeYieldProcessorEx(&v51, v21, a3, v10);
        while ( *(__int64 *)(v26 + 24) < 0 );
      }
      while ( _interlockedbittestandset64((volatile signed __int32 *)(v26 + 24), 0x3FuLL) );
      v21 = 0x7FFFFFFFFFFFFFFFLL;
    }
    *(_BYTE *)(v26 + 35) = (v18 >> 3) | *(_BYTE *)(v26 + 35) & 0xF8;
    _InterlockedAnd64((volatile signed __int64 *)(v26 + 24), 0x7FFFFFFFFFFFFFFFuLL);
    v16 = a5;
    v26 = BugCheckParameter2;
  }
  if ( (*(_BYTE *)v26 & 1) == 0 )
  {
    v52 = 0;
    if ( _interlockedbittestandset64((volatile signed __int32 *)(v26 + 24), 0x3FuLL) )
    {
      do
      {
        do
          KeYieldProcessorEx(&v52, v21, a3, v10);
        while ( *(__int64 *)(v26 + 24) < 0 );
      }
      while ( _interlockedbittestandset64((volatile signed __int32 *)(v26 + 24), 0x3FuLL) );
      v21 = 0x7FFFFFFFFFFFFFFFLL;
    }
    if ( (*(_QWORD *)BugCheckParameter2 & 1) == 0 )
      *(_QWORD *)BugCheckParameter2 |= 1uLL;
    _InterlockedAnd64((volatile signed __int64 *)(v26 + 24), 0x7FFFFFFFFFFFFFFFuLL);
    v16 = a5;
    v26 = BugCheckParameter2;
  }
  if ( (v16 & 1) == 0 )
  {
    v30 = v49;
    goto LABEL_110;
  }
  v28 = 0x8000000000000000uLL;
  v29 = v7 & 7;
  if ( (v7 & 7) == 2 )
  {
    v16 |= 0x8000000000000000uLL;
    a5 = v16;
LABEL_53:
    v30 = v49;
    goto LABEL_54;
  }
  if ( (MiFlags & 0x10000) == 0 )
    goto LABEL_53;
  v30 = v49;
  if ( (v49 < 0xFFFFF68000000000uLL || v49 > 0xFFFFF6FFFFFFFFFFuLL)
    && (v16 & 0x8000000000000000uLL) == 0
    && ((MiFlags & 0x40000) != 0 || v29) )
  {
    v53 = 0;
    while ( _interlockedbittestandset64((volatile signed __int32 *)(v26 + 24), 0x3FuLL) )
    {
      do
        KeYieldProcessorEx(&v53, v21, a3, v10);
      while ( *(__int64 *)(v26 + 24) < 0 );
    }
    v32 = BugCheckParameter2;
    if ( !(unsigned int)MiGetPagePrivilege(BugCheckParameter2) )
    {
      v34 = 38LL;
      if ( !v29 )
        v34 = 7LL;
      MiMarkPfnVerified(v32, v34, v33, v10);
    }
    _InterlockedAnd64((volatile signed __int64 *)(v26 + 24), 0x7FFFFFFFFFFFFFFFuLL);
    v16 = a5;
    v26 = BugCheckParameter2;
  }
LABEL_54:
  if ( (MiFlags & 0x8000) != 0
    && (MiFlags & 0x10000) != 0
    && (unsigned int)MI_PFN_IS_PROTO(v26)
    && ((*(_QWORD *)(v26 + 40) >> 60) & 7) == 3
    && (v29 == 4 || v29 == 1) )
  {
    v55 = 0LL;
    if ( (MiGetPagePrivilege(v28) & 8) != 0 )
    {
      v31 = KeSetPagePrivilege((__int64)(v26 + 0x58000000000LL) / 48, &v55, 1024LL);
      if ( v31 < 0 )
        KeBugCheckEx(0x1Au, 0x515E4uLL, v26, v31, 0LL);
    }
  }
  v35 = a7;
  if ( a7
    && (VirtualFaultPageInfo = MiGetVirtualFaultPageInfo(a7, v30)) != 0
    && ((v37 = *(_DWORD *)(v35 + 56), (v37 & 0x20) == 0)
     || (*(_QWORD *)(v26 + 40) & 0x1000000000LL) != 0
     || (v28 = *(_QWORD *)(v26 + 8), v28 <= 0)) )
  {
    if ( (v37 & 0x10) != 0 && v47 < 6u )
      v20 = v20 & 0xF0 | 6;
  }
  else
  {
    VirtualFaultPageInfo = 0LL;
  }
  if ( v54 )
  {
    v28 = (unsigned __int64)v20 << 56;
    v16 = v16 & 0x80FFFFFFFFFFFFFFuLL | v28;
    a5 = v16;
    LOBYTE(v18) = v18 | 1;
  }
  v38 = v16;
  v39 = (_QWORD *)a2;
  if ( a2 >= 0xFFFFF6FB7DBED000uLL && a2 <= 0xFFFFF6FB7DBED7F8uLL )
  {
    if ( (unsigned int)MiPteHasShadow(v28, v16, a2, v10) )
    {
      if ( !HIBYTE(word_140C4DE08) && (v38 & 1) != 0 )
        v38 |= 0x8000000000000000uLL;
      *v39 = v38;
      MiWritePteShadow(v39, v38);
      goto LABEL_100;
    }
    if ( (HIDWORD(KeGetCurrentThread()->ApcState.Process[2].Header.WaitListHead.Flink) & 0x1000) != 0 && (v38 & 1) != 0 )
      v38 |= 0x8000000000000000uLL;
    v26 = BugCheckParameter2;
    v16 = a5;
  }
  *v39 = v38;
LABEL_100:
  if ( VirtualFaultPageInfo )
  {
    v40 = *(_DWORD *)(v35 + 56);
    if ( ((v40 & 1) == 0 || (v16 & 0x42) != 0) && ((v40 & 2) == 0 || v16 >= 0) )
    {
      LOBYTE(v18) = v18 | 4;
      if ( (v40 & 0x20) != 0 )
      {
        v41 = MiCompleteSecureProcessFault(&BugCheckParameter2, &a5, v35 + 48);
        v16 = a5;
        v26 = BugCheckParameter2;
        LOBYTE(v18) = (v18 ^ (4 * v41)) & 4 ^ v18;
      }
      if ( (v18 & 4) != 0 )
      {
        v42 = MI_READ_PTE_LOCK_FREE((unsigned __int64)&a5);
        MiFillVirtualFaultInfo(VirtualFaultPageInfo, (v42 >> 12) & 0xFFFFFFFFFLL, v16);
      }
    }
  }
LABEL_110:
  if ( v54 && (v18 & 1) == 0 )
    *(_BYTE *)v54 = v20;
  if ( (a6 & 2) != 0 )
  {
    v56 = 0LL;
    v57 = 0LL;
    MiLockWorkingSetCoreExclusive(a1, &v56);
    MiIncrementVmFaultCount(a1, 1LL);
    MiUnlockWorkingSetCoreExclusive(&v56);
  }
  else if ( (a6 & 4) == 0 )
  {
    v43 = MI_PFN_IS_PROTO(v26);
    v45 = v48 | 2;
    if ( v43 )
      v45 = v48;
    if ( v44 || (v7 & 7) == 0 )
      v45 = (unsigned int)v45 | 4;
    MiAddWorkingSetEntries(a1, v30, 1LL, v45);
  }
  if ( (DWORD1(PerfGlobalGroupMask) & 0x8000001) != 0 )
    MiLogAllocateWsleEvent(v26, v7 & 7, v30);
  return 1LL;
}
