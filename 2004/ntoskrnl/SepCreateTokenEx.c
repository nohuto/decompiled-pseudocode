/*
 * XREFs of SepCreateTokenEx @ 0x14026E3A0
 * Callers:
 *     NtCreateTokenEx @ 0x14062F9A0 (NtCreateTokenEx.c)
 *     SepCreateToken @ 0x140799EE8 (SepCreateToken.c)
 * Callees:
 *     ObfDereferenceObjectWithTag @ 0x140208290 (ObfDereferenceObjectWithTag.c)
 *     ExInitializeResourceLite @ 0x140213470 (ExInitializeResourceLite.c)
 *     SeCaptureObjectAttributeSecurityDescriptorPresent @ 0x14026DF4C (SeCaptureObjectAttributeSecurityDescriptorPresent.c)
 *     RtlSidHashInitialize @ 0x1402933C0 (RtlSidHashInitialize.c)
 *     RtlEqualSid @ 0x140298E50 (RtlEqualSid.c)
 *     RtlWalkFrameChain @ 0x1402E7840 (RtlWalkFrameChain.c)
 *     DbgPrint @ 0x140361340 (DbgPrint.c)
 *     __security_check_cookie @ 0x1403CCE60 (__security_check_cookie.c)
 *     memmove @ 0x140409FC0 (memmove.c)
 *     memset @ 0x14040A280 (memset.c)
 *     ObInsertObject @ 0x14060D6D0 (ObInsertObject.c)
 *     ObCreateObject @ 0x14060ECE0 (ObCreateObject.c)
 *     RtlCopySid @ 0x140625AA0 (RtlCopySid.c)
 *     RtlIdentifierAuthoritySid @ 0x14062F720 (RtlIdentifierAuthoritySid.c)
 *     RtlLengthRequiredSid @ 0x14062F730 (RtlLengthRequiredSid.c)
 *     SepSetTokenClaims @ 0x14062F74C (SepSetTokenClaims.c)
 *     SepReferenceLogonSession @ 0x14062F868 (SepReferenceLogonSession.c)
 *     SepSetTokenTrust @ 0x140667740 (SepSetTokenTrust.c)
 *     SeCreateAccessState @ 0x140668A20 (SeCreateAccessState.c)
 *     SeDeleteAccessState @ 0x14067A200 (SeDeleteAccessState.c)
 *     RtlCopySidAndAttributesArray @ 0x140680FC0 (RtlCopySidAndAttributesArray.c)
 *     SepAppendAdminAceToTokenAcl @ 0x140710958 (SepAppendAdminAceToTokenAcl.c)
 *     SepSetTokenUserAndGroups @ 0x14091FBCC (SepSetTokenUserAndGroups.c)
 *     SepAddTokenLogonSession @ 0x1409200E0 (SepAddTokenLogonSession.c)
 *     ExFreePoolWithTag @ 0x1409B1140 (ExFreePoolWithTag.c)
 *     ExAllocatePoolWithTag @ 0x1409B1160 (ExAllocatePoolWithTag.c)
 */

__int64 __fastcall SepCreateTokenEx(
        HANDLE *a1,
        char a2,
        unsigned int a3,
        __int64 a4,
        int a5,
        int a6,
        __int64 a7,
        _QWORD *a8,
        void **a9,
        int a10,
        __int64 a11,
        unsigned int a12,
        unsigned int a13,
        char **a14,
        void *a15,
        void *a16,
        void *a17,
        _OWORD *a18,
        __int64 a19,
        __int64 a20,
        int a21,
        __int64 a22,
        _DWORD *a23,
        char a24)
{
  PSID v26; // rdi
  unsigned int v27; // r13d
  int v28; // r12d
  __int64 result; // rax
  void *v30; // rsi
  unsigned int v31; // ebx
  unsigned int v32; // r14d
  __int64 v33; // rax
  void **v34; // r15
  BOOLEAN v35; // al
  __int64 v36; // r10
  bool v37; // bl
  int v38; // r11d
  bool v39; // r14
  int v40; // eax
  int v41; // r15d
  _DWORD *v42; // rsi
  int v43; // edx
  unsigned __int8 *v44; // rdi
  PSID_IDENTIFIER_AUTHORITY v45; // rax
  char v46; // dl
  __int64 v47; // r8
  int v48; // ecx
  __int64 v49; // r11
  __int64 v50; // rsi
  __int64 v51; // rdi
  _DWORD *PoolWithTag; // rax
  _DWORD *v53; // rsi
  _QWORD *v54; // rax
  PVOID v55; // rbx
  struct _ERESOURCE *v56; // r15
  int v57; // ebx
  __int64 v58; // rax
  unsigned int v59; // r14d
  ULONG v60; // r12d
  unsigned int v61; // ecx
  int v62; // eax
  NTSTATUS inserted; // ebx
  _QWORD *v64; // rbx
  char *v65; // rdi
  _QWORD *v66; // rax
  __int64 v67; // rax
  __int64 v68; // rax
  PVOID v69; // rax
  char *v70; // r15
  __int64 v71; // rcx
  int v72; // esi
  int v73; // edx
  unsigned int v74; // r14d
  __int64 v75; // rcx
  char *v76; // rsi
  unsigned __int8 *v77; // r14
  ULONG v78; // ebx
  unsigned __int16 *v79; // rcx
  char *v80; // rsi
  void *v81; // rcx
  int v82; // r8d
  int v83; // r9d
  unsigned int v84; // edx
  BOOLEAN v85; // al
  BOOLEAN v86; // al
  void *v87; // rdx
  int *v88; // rcx
  int v89; // r9d
  unsigned __int64 v90; // r8
  BOOLEAN v91; // al
  PVOID v92; // rdi
  void *v93; // rcx
  __int64 v94; // rdx
  _KPROCESS *Process; // rcx
  ULONG v96; // eax
  int v97; // edx
  unsigned int v98; // [rsp+30h] [rbp-D0h]
  char v99; // [rsp+50h] [rbp-B0h]
  char v100; // [rsp+51h] [rbp-AFh] BYREF
  char v101; // [rsp+52h] [rbp-AEh]
  char v102; // [rsp+53h] [rbp-ADh]
  int v103; // [rsp+54h] [rbp-ACh]
  __int64 v104; // [rsp+58h] [rbp-A8h] BYREF
  char **v105; // [rsp+60h] [rbp-A0h]
  int v106; // [rsp+68h] [rbp-98h]
  int v107; // [rsp+6Ch] [rbp-94h]
  PVOID Object; // [rsp+70h] [rbp-90h] BYREF
  unsigned int v109; // [rsp+78h] [rbp-88h]
  int v110[2]; // [rsp+80h] [rbp-80h]
  PSID v111; // [rsp+88h] [rbp-78h]
  PVOID P; // [rsp+90h] [rbp-70h]
  void *v113; // [rsp+98h] [rbp-68h] BYREF
  PSID Sid1; // [rsp+A0h] [rbp-60h]
  int v115; // [rsp+A8h] [rbp-58h]
  unsigned int v116; // [rsp+ACh] [rbp-54h]
  __int64 v117; // [rsp+B0h] [rbp-50h]
  int v118[2]; // [rsp+B8h] [rbp-48h]
  PHANDLE Handle; // [rsp+C0h] [rbp-40h]
  _QWORD *v120; // [rsp+C8h] [rbp-38h]
  __int64 v121; // [rsp+D0h] [rbp-30h]
  __int64 v122; // [rsp+D8h] [rbp-28h]
  __int64 v123; // [rsp+E0h] [rbp-20h]
  __int64 v124; // [rsp+E8h] [rbp-18h]
  void *Src; // [rsp+F0h] [rbp-10h]
  __int64 v126; // [rsp+F8h] [rbp-8h]
  __int64 v127; // [rsp+100h] [rbp+0h]
  __int64 v128; // [rsp+108h] [rbp+8h]
  struct _ACCESS_STATE PassedAccessState; // [rsp+110h] [rbp+10h] BYREF
  _BYTE v130[224]; // [rsp+1B0h] [rbp+B0h] BYREF

  v26 = a15;
  v27 = a10;
  v107 = -1;
  v124 = a7;
  v105 = a14;
  Sid1 = a16;
  Src = a17;
  v128 = a19;
  v127 = a20;
  v126 = a22;
  v116 = a3;
  v28 = 0;
  v102 = a2;
  Handle = a1;
  Object = 0LL;
  v113 = 0LL;
  v103 = 0;
  v120 = (_QWORD *)a4;
  *(_QWORD *)v110 = a9;
  *(_QWORD *)v118 = a11;
  v111 = a15;
  memset(&PassedAccessState, 0, sizeof(PassedAccessState));
  memset(v130, 0, sizeof(v130));
  v117 = 0LL;
  v99 = 0;
  v101 = 0;
  v100 = 0;
  P = 0LL;
  result = SeCaptureObjectAttributeSecurityDescriptorPresent(a4, a2, &v100);
  if ( (int)result >= 0 )
  {
    v30 = *a9;
    v31 = 0;
    if ( a10 )
    {
      do
      {
        v32 = v31;
        while ( v32 < v27 )
        {
          if ( !RtlEqualSid(v30, *(PSID *)(a11 + 16LL * v32)) || (*(_DWORD *)(a11 + 16LL * v32 + 8) & 0x20) != 0 )
            ++v32;
          else
            *(_OWORD *)(a11 + 16LL * v32) = *(_OWORD *)(a11 + 16LL * --v27);
        }
        v33 = v31++;
        v30 = *(void **)(a11 + 16 * v33);
      }
      while ( v31 < v27 );
      v34 = *(void ***)v110;
      v26 = v111;
      v30 = **(void ***)v110;
    }
    else
    {
      v34 = *(void ***)v110;
    }
    v35 = RtlEqualSid(Sid1, v30);
    v36 = 0LL;
    v37 = v35 != 0;
    if ( v26 )
    {
      v91 = RtlEqualSid(v26, v30);
      v36 = 0LL;
      LOBYTE(v38) = 1;
      v39 = v91 != 0;
    }
    else
    {
      LOBYTE(v38) = 1;
      v39 = 1;
    }
    v40 = *((_DWORD *)v34 + 2);
    if ( (v40 & 0x10) != 0 )
    {
      if ( (v40 & 6) != 0 )
        return 3221225485LL;
      v28 = 2048;
      v103 = 2048;
    }
    v106 = 0;
    if ( v27 )
    {
      v41 = 1;
      v42 = (_DWORD *)(*(_QWORD *)v118 + 8LL);
      v106 = 0;
      while ( 1 )
      {
        v43 = *v42;
        if ( ((unsigned __int8)*v42 & (unsigned __int8)v38) != 0 )
        {
          v43 |= 6u;
          *v42 = v43;
        }
        if ( (v43 & 0x10) != 0 )
        {
          if ( (v43 & 6) != 0 )
            return 3221225485LL;
          v28 |= 0x800u;
          v103 = v28;
        }
        v44 = (unsigned __int8 *)*((_QWORD *)v42 - 1);
        RtlIdentifierAuthoritySid(v44);
        v45 = RtlIdentifierAuthoritySid(SeUntrustedMandatorySid);
        v48 = *(_DWORD *)v45->Value - *(_DWORD *)v47;
        if ( *(_DWORD *)v45->Value == *(_DWORD *)v47 )
          v48 = *(unsigned __int16 *)&v45->Value[4] - *(unsigned __int16 *)(v47 + 4);
        if ( !v48 )
        {
          v82 = v44[1];
          if ( (_BYTE)v82 )
          {
            v83 = v44[1];
            if ( *(_DWORD *)&v44[4 * (v82 - 1) + 8] > 0x4000u )
              return 3221226566LL;
          }
          else
          {
            v83 = v36;
          }
          if ( (v46 & 0x40) != 0 )
            break;
        }
LABEL_20:
        if ( !v37 )
        {
          v85 = RtlEqualSid(Sid1, v44);
          v36 = 0LL;
          v38 = 1;
          v37 = v85 != 0;
        }
        if ( !v39 )
        {
          v86 = RtlEqualSid(v111, v44);
          v36 = 0LL;
          if ( v86 )
          {
            if ( (*v42 & 8) == 0 )
              return 3221225562LL;
            v38 = 1;
            v106 = v41;
            v39 = 1;
          }
          else
          {
            v38 = 1;
          }
        }
        v41 += v38;
        v42 += 4;
        if ( v41 - 1 >= v27 )
          goto LABEL_24;
      }
      if ( v107 != -1 )
        return 3221225485LL;
      v107 = v41;
      if ( (_BYTE)v82 )
      {
        v84 = *(_DWORD *)&v44[4 * (v83 - 1) + 8];
        if ( v84 >= 0x3000 )
        {
          v99 = v38;
          v28 |= 0x2000u;
LABEL_74:
          v103 = v28;
          goto LABEL_20;
        }
        if ( v84 >= 0x2000 )
        {
LABEL_72:
          v99 = v36;
          if ( v84 >= 0x2000 )
            v28 |= 0x2000u;
          goto LABEL_74;
        }
      }
      else
      {
        v84 = v36;
      }
      v101 = v38;
      goto LABEL_72;
    }
LABEL_24:
    if ( !v37 )
      return 3221225563LL;
    if ( !v39 )
      return 3221225562LL;
    v49 = v36;
    v121 = v36;
    v50 = v36;
    v122 = v36;
    v51 = v36;
    v123 = v36;
    if ( a13 )
    {
      v88 = (int *)(v105 + 1);
      do
      {
        v89 = *v88;
        if ( (*v88 & 0x7FFFFFF8) != 0 )
          return 3221225485LL;
        v90 = (unsigned int)*(v88 - 2);
        *v88 &= 3u;
        if ( (unsigned int)v90 > 0x24 )
          return 3221225568LL;
        if ( _bittest64(&v49, v90) )
          return 3221225894LL;
        v49 |= 1LL << v90;
        v121 = v49;
        if ( (v89 & 2) != 0 )
        {
          _bittestandset64(&v50, (unsigned int)v90);
          v122 = v50;
        }
        if ( (v89 & 1) != 0 )
        {
          _bittestandset64(&v51, (unsigned int)v90);
          v123 = v51;
        }
        LODWORD(v36) = v36 + 1;
        v88 += 3;
      }
      while ( (unsigned int)v36 < a13 );
    }
    if ( a23 && (*a23 & 0xFFFFFFFC) != 0 )
      return 3221225485LL;
    PoolWithTag = ExAllocatePoolWithTag(PagedPool, 0x30uLL, 0x74416553u);
    v53 = PoolWithTag;
    if ( !PoolWithTag )
      return 3221225626LL;
    *PoolWithTag = 0;
    v54 = PoolWithTag + 2;
    v53[6] = 0;
    v54[1] = v54;
    *v54 = v54;
    *((_QWORD *)v53 + 5) = v53 + 8;
    *((_QWORD *)v53 + 4) = v53 + 8;
    if ( SeTokenLeakTracking )
    {
      P = ExAllocatePoolWithTag(PagedPool, 0x120uLL, 0x74416553u);
      v55 = P;
      if ( !P )
      {
        v93 = v53;
        goto LABEL_119;
      }
    }
    else
    {
      v55 = 0LL;
    }
    v117 = ExpLuidIncrement + _InterlockedExchangeAdd64(&ExpLuid, ExpLuidIncrement);
    v56 = (struct _ERESOURCE *)ExAllocatePoolWithTag(NonPagedPoolNx, 0x68uLL, 0x6C546553u);
    if ( v56 )
    {
      v57 = 16 * v27;
      LODWORD(v111) = 16 * v27;
      if ( SepTokenSidSharingEnabled )
      {
        v59 = (v57 + 23) & 0xFFFFFFF8;
        v109 = a12;
      }
      else
      {
        v58 = **(_QWORD **)v110;
        v109 = (a12 + 7) & 0xFFFFFFF8;
        v59 = v57 + ((RtlLengthRequiredSid(*(unsigned __int8 *)(v58 + 1)) + 3) & 0xFFFFFFFC) + v109 + 16;
      }
      LODWORD(v104) = v59;
      v60 = (RtlLengthRequiredSid(*((unsigned __int8 *)Sid1 + 1)) + 3) & 0xFFFFFFFC;
      if ( Src )
        v60 += (*((unsigned __int16 *)Src + 1) + 3) & 0xFFFFFFFC;
      v61 = v59 + 1168;
      v62 = 4096;
      if ( v60 >= 0x1000 )
        v62 = v60;
      v115 = v62;
      v98 = v61 + v62;
      LOBYTE(v61) = v102;
      inserted = ObCreateObject(
                   v61,
                   (_DWORD)SeTokenObjectType,
                   (_DWORD)v120,
                   1,
                   0,
                   v59 + 1168,
                   v98,
                   0,
                   (__int64)&Object);
      if ( inserted < 0 )
      {
        ExFreePoolWithTag(v53, 0);
        if ( SeTokenLeakTracking )
          ExFreePoolWithTag(P, 0);
        ExFreePoolWithTag(v56, 0);
        return (unsigned int)inserted;
      }
      v64 = Object;
      *((_QWORD *)Object + 6) = v56;
      ExInitializeResourceLite(v56);
      v65 = (char *)Object;
      v64[2] = ExpLuidIncrement + _InterlockedExchangeAdd64(&ExpLuid, ExpLuidIncrement);
      *((_QWORD *)v65 + 4) = 0LL;
      v66 = (_QWORD *)v124;
      *((_QWORD *)v65 + 28) = 0LL;
      v105 = 0LL;
      *((_QWORD *)v65 + 3) = *v66;
      v67 = v117;
      v65[204] = 0;
      *((_QWORD *)v65 + 7) = v67;
      *((_QWORD *)v65 + 5) = *a8;
      *((_DWORD *)v65 + 48) = a5;
      *((_DWORD *)v65 + 49) = a6;
      LODWORD(v67) = v103;
      *(_OWORD *)v65 = *a18;
      *((_DWORD *)v65 + 50) = v67;
      LODWORD(v67) = v115;
      *((_DWORD *)v65 + 30) = 0;
      *((_QWORD *)v65 + 145) = 0LL;
      *((_DWORD *)v65 + 34) = v67;
      LODWORD(v67) = v106;
      *((_DWORD *)v65 + 35) = 0;
      *((_DWORD *)v65 + 36) = v67;
      *((_QWORD *)v65 + 23) = 0LL;
      *((_QWORD *)v65 + 140) = 0LL;
      *((_QWORD *)v65 + 141) = 0LL;
      *((_DWORD *)v65 + 33) = v59;
      v105 = (char **)(v65 + 184);
      *((_DWORD *)v65 + 52) = v107;
      v68 = v121;
      *((_DWORD *)v65 + 53) = 0;
      *((_QWORD *)v65 + 8) = v68;
      *((_QWORD *)v65 + 9) = v122;
      *((_QWORD *)v65 + 10) = v123;
      *((_DWORD *)v65 + 200) = 0;
      *((_QWORD *)v65 + 99) = 0LL;
      *((_QWORD *)v65 + 98) = 0LL;
      *((_QWORD *)v65 + 135) = 0LL;
      *((_QWORD *)v65 + 136) = 0LL;
      *((_QWORD *)v65 + 144) = 0LL;
      memset(v65 + 808, 0, 0x110uLL);
      v69 = P;
      v70 = v65 + 1144;
      *((_QWORD *)v65 + 138) = 0LL;
      *((_QWORD *)v65 + 139) = 0LL;
      *((_QWORD *)v65 + 142) = 0LL;
      *((_QWORD *)v65 + 143) = v69;
      if ( a23 )
        *((_DWORD *)v65 + 53) = *a23;
      if ( !v99 )
      {
        *((_QWORD *)v65 + 9) &= 0xFFFFFFEEDFE9F97BuLL;
        *((_QWORD *)v65 + 10) &= 0xFFFFFFEEDFE9F97BuLL;
      }
      if ( v101 == 1 )
      {
        *((_QWORD *)v65 + 9) &= 0x202800000uLL;
        *((_QWORD *)v65 + 10) &= 0x202800000uLL;
      }
      v71 = v124;
      *(_OWORD *)(v65 + 88) = 0LL;
      *((_QWORD *)v65 + 13) = 0LL;
      *((_DWORD *)v65 + 28) = 0;
      *((_WORD *)v65 + 58) = 0;
      v65[118] = 0;
      *((_QWORD *)v65 + 97) = v53;
      *((_QWORD *)v65 + 137) = 0LL;
      *((_QWORD *)v65 + 22) = 0LL;
      v120 = v65 + 176;
      v72 = SepReferenceLogonSession(v71, v65 + 216);
      if ( v72 < 0 )
      {
        *((_DWORD *)v65 + 50) |= 0x20u;
        *((_QWORD *)v65 + 27) = 0LL;
        ObfDereferenceObjectWithTag(v65, 0x746C6644u);
        return (unsigned int)v72;
      }
      if ( SeTokenLeakTracking )
      {
        **(_QWORD **)v70 = KeGetCurrentThread()[1].CycleTime;
        *(_QWORD *)(*(_QWORD *)v70 + 8LL) = *(_QWORD *)&KeGetCurrentThread()[1].CurrentRunTime;
        *(_DWORD *)(*(_QWORD *)v70 + 32LL) = 12;
        *(_DWORD *)(*(_QWORD *)v70 + 280LL) = 0;
        *(_DWORD *)(*(_QWORD *)v70 + 284LL) = 0;
        v94 = *(_QWORD *)v70;
        Process = KeGetCurrentThread()->ApcState.Process;
        *(_QWORD *)(v94 + 16) = *(_QWORD *)&Process[1].ActiveProcessors.Count;
        *(_DWORD *)(v94 + 24) = Process[1].ActiveProcessors.Bitmap[0];
        *(_WORD *)(v94 + 28) = WORD2(Process[1].ActiveProcessors.Bitmap[0]);
        *(_BYTE *)(v94 + 30) = BYTE6(Process[1].ActiveProcessors.Bitmap[0]);
        v96 = RtlWalkFrameChain((PVOID *)(*(_QWORD *)v70 + 40LL), 0x1Eu, 0);
        if ( KeGetCurrentIrql() < 2u )
          RtlWalkFrameChain((PVOID *)(*(_QWORD *)v70 + 40LL + 8LL * v96), 30 - v96, 1u);
        v65 = (char *)Object;
        SepAddTokenLogonSession(Object);
      }
      v73 = v110[0];
      if ( SepTokenSidSharingEnabled )
      {
        inserted = SepSetTokenUserAndGroups((_DWORD)v65, v110[0], v27, v118[0], v109);
        if ( inserted < 0 )
          goto LABEL_60;
      }
      else
      {
        v74 = v59 - (_DWORD)v111;
        *((_QWORD *)v65 + 19) = v65 + 1168;
        v75 = v27 + 1;
        *((_DWORD *)v65 + 31) = v75;
        v113 = &v65[16 * v75 + 1168];
        LODWORD(v104) = v74 - 16;
        RtlCopySidAndAttributesArray(1, v73, v74 - 16, (_DWORD)v65 + 1168, v113, (__int64)&v113, (__int64)&v104);
        RtlCopySidAndAttributesArray(v27, v118[0], v104, (_DWORD)v65 + 1184, v113, (__int64)&v113, (__int64)&v104);
      }
      RtlSidHashInitialize(*((_QWORD *)v65 + 19), *((unsigned int *)v65 + 31), v65 + 232);
      *((_QWORD *)v65 + 20) = 0LL;
      *((_DWORD *)v65 + 32) = 0;
      v76 = (char *)ExAllocatePoolWithTag(PagedPool, v60, 0x64546553u);
      *v120 = v76;
      if ( v76 )
      {
        v77 = (unsigned __int8 *)Sid1;
        *((_QWORD *)v65 + 21) = v76;
        v78 = RtlLengthRequiredSid(v77[1]);
        RtlCopySid(v78, v76, v77);
        v79 = (unsigned __int16 *)Src;
        v80 = &v76[(v78 + 3) & 0xFFFFFFFC];
        if ( Src )
        {
          v87 = Src;
          *v105 = v80;
          memmove(v80, v87, v79[1]);
        }
        inserted = SepSetTokenClaims((_DWORD)v65, v128, v127, a21, v126);
        if ( inserted >= 0 )
        {
          if ( !a24 || (inserted = SepSetTokenTrust(v65, SeProcTrustWinTcbSid), inserted >= 0) )
          {
            if ( SeTokenLeakTracking && SepTokenLeakMethodWatch == 12 )
            {
              if ( KeGetCurrentThread()->ApcState.Process[1].Header.WaitListHead.Flink == (struct _LIST_ENTRY *)SepTokenLeakProcessCid )
              {
                *(_DWORD *)(*(_QWORD *)v70 + 280LL) = _InterlockedIncrement(&SepTokenLeakMethodCount);
                v97 = *(_DWORD *)(*(_QWORD *)v70 + 280LL);
                if ( v97 >= SepTokenLeakBreakCount )
                {
                  DbgPrint("\nToken number 0x%x = 0x%p\n", v97, Object);
                  __debugbreak();
                }
              }
              v65 = (char *)Object;
            }
            if ( a24 )
            {
              v92 = Object;
              if ( KeGetCurrentThread()->ApcState.Process[1].Affinity.Bitmap[5] )
                inserted = ObInsertObject(Object, 0LL, 0, 0, 0LL, 0LL);
              if ( inserted < 0 )
              {
                *Handle = 0LL;
              }
              else
              {
                if ( !v100 )
                  SepAppendAdminAceToTokenAcl(v92);
                *Handle = v92;
              }
              return (unsigned int)inserted;
            }
            inserted = SeCreateAccessState(&PassedAccessState, v130, v116, (char *)SeTokenObjectType + 76);
            v81 = v65;
            if ( inserted < 0 )
              goto LABEL_61;
            inserted = ObInsertObject(v65, &PassedAccessState, 0, 1u, 0LL, Handle);
            SeDeleteAccessState(&PassedAccessState);
            if ( inserted < 0 )
              return (unsigned int)inserted;
            if ( !v100 )
              SepAppendAdminAceToTokenAcl(v65);
          }
        }
      }
      else
      {
        inserted = -1073741801;
      }
LABEL_60:
      v81 = v65;
LABEL_61:
      ObfDereferenceObjectWithTag(v81, 0x746C6644u);
      return (unsigned int)inserted;
    }
    ExFreePoolWithTag(v53, 0);
    if ( !SeTokenLeakTracking )
      return 3221225626LL;
    v93 = v55;
LABEL_119:
    ExFreePoolWithTag(v93, 0);
    return 3221225626LL;
  }
  return result;
}
