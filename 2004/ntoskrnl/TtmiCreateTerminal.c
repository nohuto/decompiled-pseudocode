/*
 * XREFs of TtmiCreateTerminal @ 0x1408FA16C
 * Callers:
 *     TtmInitCurrentSession @ 0x1408FB284 (TtmInitCurrentSession.c)
 *     TtmpDispatchCreateTerminal @ 0x1408FD870 (TtmpDispatchCreateTerminal.c)
 * Callees:
 *     RtlFindClearBitsAndSet @ 0x140269270 (RtlFindClearBitsAndSet.c)
 *     KeInitializeDpc @ 0x1402E3AC0 (KeInitializeDpc.c)
 *     KeInitializeTimerEx @ 0x1402F9130 (KeInitializeTimerEx.c)
 *     memset @ 0x14040A280 (memset.c)
 *     ObCreateObjectEx @ 0x140679FF0 (ObCreateObjectEx.c)
 *     ObInsertObjectEx @ 0x14067A230 (ObInsertObjectEx.c)
 *     TtmpSetTerminalPendingCleanup @ 0x1408FACF0 (TtmpSetTerminalPendingCleanup.c)
 *     TtmiUpdateActiveTerminalCount @ 0x1408FC0B8 (TtmiUpdateActiveTerminalCount.c)
 *     TtmiLogError @ 0x1408FF6C0 (TtmiLogError.c)
 *     TtmiLogTerminalCreated @ 0x1409012F8 (TtmiLogTerminalCreated.c)
 */

__int64 __fastcall TtmiCreateTerminal(
        __int64 a1,
        ACCESS_MASK a2,
        unsigned __int8 a3,
        unsigned __int64 *a4,
        PVOID DeferredContext,
        struct _DMA_ADAPTER **a6)
{
  _DWORD *v6; // r14
  ULONG ClearBitsAndSet; // esi
  unsigned int v12; // ebx
  __int64 v13; // r9
  __int64 v14; // r8
  __int64 v15; // rdx
  int v16; // eax
  char *v17; // rbx
  __int64 v18; // rdx
  struct _DMA_ADAPTER *v19; // rdi
  struct _DMA_ADAPTER **v20; // rdx
  int inserted; // eax
  struct _DMA_ADAPTER **v22; // rax
  __int64 v24; // [rsp+20h] [rbp-78h]
  __int128 v25; // [rsp+50h] [rbp-48h] BYREF
  __int128 v26; // [rsp+60h] [rbp-38h]
  __int128 v27; // [rsp+70h] [rbp-28h]

  v6 = DeferredContext;
  *a4 = 0LL;
  DeferredContext = 0LL;
  *v6 = -1;
  ClearBitsAndSet = RtlFindClearBitsAndSet((PRTL_BITMAP)(a1 + 56), 1u, 0);
  if ( ClearBitsAndSet == -1 )
  {
    v12 = -1073741756;
    TtmiLogError("TtmiAcquireTerminalId", 1604LL, 0xFFFFFFFFLL, 3221225540LL);
    v13 = 3221225540LL;
    v14 = 3221225540LL;
    v15 = 581LL;
LABEL_3:
    TtmiLogError("TtmiCreateTerminal", v15, v14, v13);
    return v12;
  }
  v26 = 0LL;
  DWORD2(v26) = a3 == 0 ? 0x200 : 0;
  v25 = 0LL;
  LODWORD(v25) = 48;
  v27 = 0LL;
  v16 = ObCreateObjectEx(0, TtmpTerminalObjectType, (int)&v25, a3, v24, 288, 0, 0, &DeferredContext, 0LL);
  v12 = v16;
  if ( v16 < 0 )
  {
    v13 = (unsigned int)v16;
    v14 = (unsigned int)v16;
    v15 = 614LL;
    _bittestandreset(*(signed __int32 **)(a1 + 64), ClearBitsAndSet);
    goto LABEL_3;
  }
  v17 = (char *)DeferredContext;
  memset(DeferredContext, 0, 0x120uLL);
  *((_DWORD *)v17 + 6) = 1416459348;
  *((_DWORD *)v17 + 7) = ClearBitsAndSet;
  if ( ClearBitsAndSet )
  {
    *((_DWORD *)v17 + 10) = 1;
    *((_DWORD *)v17 + 69) = 1;
    *((_DWORD *)v17 + 12) = 2;
    *((_QWORD *)v17 + 9) = 300000000LL;
  }
  else
  {
    *((_DWORD *)v17 + 10) = 3;
    *((_DWORD *)v17 + 69) = 3;
    LOBYTE(v18) = 1;
    *((_DWORD *)v17 + 12) = 0;
    *((_DWORD *)v17 + 9) |= 0x10u;
    v17 = (char *)DeferredContext;
    *((_QWORD *)DeferredContext + 10) = MEMORY[0xFFFFF78000000008];
    TtmiUpdateActiveTerminalCount(a1, v18, 26LL);
  }
  KeInitializeTimerEx((PKTIMER)(v17 + 88), NotificationTimer);
  KeInitializeDpc((PRKDPC)(v17 + 152), (PKDEFERRED_ROUTINE)TtmpScheduledEvaluationDpc, v17);
  *((_QWORD *)v17 + 27) = 0LL;
  *((_QWORD *)v17 + 29) = TtmpScheduledEvaluationWorker;
  *((_QWORD *)v17 + 30) = v17;
  _InterlockedAdd((volatile signed __int32 *)(a1 + 8), 1u);
  v19 = (struct _DMA_ADAPTER *)DeferredContext;
  *((_QWORD *)DeferredContext + 2) = a1;
  v20 = *(struct _DMA_ADAPTER ***)(a1 + 48);
  if ( *v20 != (struct _DMA_ADAPTER *)(a1 + 40) )
    __fastfail(3u);
  v19->DmaOperations = (_DMA_OPERATIONS *)v20;
  *(_QWORD *)&v19->Version = a1 + 40;
  *v20 = v19;
  *(_QWORD *)(a1 + 48) = v19;
  TtmiLogTerminalCreated(v19, ClearBitsAndSet);
  inserted = ObInsertObjectEx(v19, 0LL, a2, 1, 0, 0LL, a4);
  v12 = inserted;
  if ( inserted >= 0 )
  {
    v22 = a6;
    *v6 = ClearBitsAndSet;
    if ( v22 )
      *v22 = v19;
    return 0;
  }
  else
  {
    TtmiLogError("TtmiCreateTerminal", 682LL, (unsigned int)inserted, (unsigned int)inserted);
    TtmpSetTerminalPendingCleanup(a1, v19);
  }
  return v12;
}
