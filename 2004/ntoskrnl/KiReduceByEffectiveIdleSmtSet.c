/*
 * XREFs of KiReduceByEffectiveIdleSmtSet @ 0x140342AB4
 * Callers:
 *     KiChooseTargetProcessor @ 0x140288310 (KiChooseTargetProcessor.c)
 *     KiTryLocalThreadSchedule @ 0x140288A90 (KiTryLocalThreadSchedule.c)
 *     KiSelectIdleProcessor @ 0x14038DE98 (KiSelectIdleProcessor.c)
 * Callees:
 *     KiUpdateTotalCyclesCurrentThread @ 0x1402ED9C0 (KiUpdateTotalCyclesCurrentThread.c)
 */

char __fastcall KiReduceByEffectiveIdleSmtSet(__int64 a1, _QWORD *a2)
{
  __int64 v3; // rbx
  __int64 v4; // rax
  __int64 v6; // rdi
  unsigned int v7; // edx
  unsigned int v8; // ecx

  v3 = *(_QWORD *)(a1 + 33880) & ~*(_QWORD *)(a1 + 200);
  v4 = v3 & *a2;
  if ( !v4
    || (*(_QWORD *)(*(_QWORD *)(a1 + 192) + 24LL) & v4) != v3
    || (*(_BYTE *)(a1 + 12588) & 1) != 0
    || *(_DWORD *)(a1 + 31768) )
  {
    return 0;
  }
  v6 = *(_QWORD *)(a1 + 8);
  if ( !*(_BYTE *)(a1 + 32) )
  {
    _disable();
    KiUpdateTotalCyclesCurrentThread(a1, *(_QWORD *)(a1 + 8), 0LL);
    _enable();
  }
  v7 = *(_DWORD *)(v6 + 80);
  v8 = *(_DWORD *)(v6 + 84);
  if ( v7 < v8 && v8 - v7 >= KiShortExecutionCycles )
    return 0;
  *a2 &= v3;
  return 1;
}
