/*
 * XREFs of ?Transform@?$CSimdScale2DOperator@U?$Vertex_UVxN@UVertexXYW_ColorDW@@@@@VertexTransfer@@QEAQXAEBU?$Vertex_UVxN@UVertexXYW_ColorDW@@@@AEBUVertexAAFixup_UVxN@@T__m128@@2IPEAU3@@Z @ 0x1800BB2B0
 * Callers:
  ...
  *a7 = _mm_shuffle_ps(
  ...
  else if ( a6 )
  {
    ...
    do
    {
      *(__m128 *)((char *)result + (char *)a7 - (char *)a2) = _mm_add_ps(
                                                                _mm_mul_ps(
                                                                  _mm_sub_ps(*result, *(__m128 *)&v20[(_QWORD)result]),
                                                                  v21),
