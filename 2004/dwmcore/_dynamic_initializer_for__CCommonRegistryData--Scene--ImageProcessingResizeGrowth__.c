/*
 * XREFs of _dynamic_initializer_for__CCommonRegistryData::Scene::ImageProcessingResizeGrowth__ @ 0x180002F00
 * Callers:
 *     <none>
 * Callees:
 *     ?RegGetDwmDwordHelper@@YA_NPEBGPEAKW4DwmRegistrySubkey@@@Z @ 0x180031730 (-RegGetDwmDwordHelper@@YA_NPEBGPEAKW4DwmRegistrySubkey@@@Z.c)
 */

__int64 dynamic_initializer_for__CCommonRegistryData::Scene::ImageProcessingResizeGrowth__()
{
  __int64 result; // rax
  int v1; // ecx
  int v2; // [rsp+30h] [rbp+8h] BYREF

  result = RegGetDwmDwordHelper(L"ImageProcessingResizeGrowth", &v2, 1LL);
  v1 = 200;
  if ( (_BYTE)result )
    v1 = v2;
  CCommonRegistryData::Scene::ImageProcessingResizeGrowth = v1;
  return result;
}
