/*
 * XREFs of ??1CD2DResource@@MEAA@XZ @ 0x18003EC24
 * Callers:
 *     ??1CD2DBitmap@@UEAA@XZ @ 0x18003EBD8 (--1CD2DBitmap@@UEAA@XZ.c)
 *     ??_ECD2DResource@@MEAAPEAXI@Z @ 0x180239430 (--_ECD2DResource@@MEAAPEAXI@Z.c)
 *     ??1CD2DPrimitiveProperties@@MEAA@XZ @ 0x180249398 (--1CD2DPrimitiveProperties@@MEAA@XZ.c)
 *     ??1CD2DCommandList@@MEAA@XZ @ 0x18024965C (--1CD2DCommandList@@MEAA@XZ.c)
 *     ??1CD2DInk@@MEAA@XZ @ 0x18024B048 (--1CD2DInk@@MEAA@XZ.c)
 *     ??_GCD2DGenericInk@@UEAAPEAXI@Z @ 0x18024B3E0 (--_GCD2DGenericInk@@UEAAPEAXI@Z.c)
 *     ??1CD2DEffect@@MEAA@XZ @ 0x18024B7E0 (--1CD2DEffect@@MEAA@XZ.c)
 * Callees:
 *     ?UnmanageResource@CD2DResourceManager@@IEAAXPEAVCD2DResource@@@Z @ 0x18003EFC4 (-UnmanageResource@CD2DResourceManager@@IEAAXPEAVCD2DResource@@@Z.c)
 *     ??$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z @ 0x1800C4B4C (--$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z.c)
 */

void __fastcall CD2DResource::~CD2DResource(CD2DResource *this)
{
  __int64 v2; // rdx
  __int64 v3; // rcx
  CD2DResourceManager *v4; // rcx
  CD2DResource **v5; // rdx
  CD2DResource **v6; // r8
  __int64 v7; // rcx

  *(_QWORD *)this = &CD2DResource::`vftable';
  *(_QWORD *)((char *)this + *(int *)(*((_QWORD *)this + 2) + 4LL) + 16) = &CBackdropBrush::`vftable'{for `IUnknown'};
  *(_QWORD *)((char *)this + *(int *)(*((_QWORD *)this + 2) + 8LL) + 16) = &CD2DResource::`vftable'{for `IDeviceResource'};
  v2 = *(int *)(*((_QWORD *)this + 2) + 4LL);
  *(_DWORD *)((char *)this + v2 + 12) = v2 - 72;
  v3 = *(int *)(*((_QWORD *)this + 2) + 8LL);
  *(_DWORD *)((char *)this + v3 + 12) = v3 - 88;
  if ( *((_BYTE *)this + 32) )
  {
    v4 = (CD2DResourceManager *)*((_QWORD *)this + 3);
    v5 = (CD2DResource **)*((_QWORD *)this + 5);
    if ( v5[1] != (CD2DResource *)((char *)this + 40)
      || (v6 = (CD2DResource **)*((_QWORD *)this + 6), *v6 != (CD2DResource *)((char *)this + 40)) )
    {
      __fastfail(3u);
    }
    *v6 = (CD2DResource *)v5;
    v5[1] = (CD2DResource *)v6;
    CD2DResourceManager::UnmanageResource(v4, this);
  }
  v7 = *((_QWORD *)this + 7);
  if ( v7 )
  {
    std::_Deallocate<16,0>(v7, (*((_QWORD *)this + 9) - v7) & 0xFFFFFFFFFFFFFFF8uLL);
    *((_QWORD *)this + 7) = 0LL;
    *((_QWORD *)this + 8) = 0LL;
    *((_QWORD *)this + 9) = 0LL;
  }
  *(_QWORD *)this = &CMILCOMBase::`vftable';
}
