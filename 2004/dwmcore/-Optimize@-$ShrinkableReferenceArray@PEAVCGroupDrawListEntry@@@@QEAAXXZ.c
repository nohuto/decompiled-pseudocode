/*
 * XREFs of ?Optimize@?$ShrinkableReferenceArray@PEAVCGroupDrawListEntry@@@@QEAAXXZ @ 0x1800D95AC
 * Callers:
 *     ?OnEndDraw@CDrawListBatchManager@@IEAAXXZ @ 0x18007E374 (-OnEndDraw@CDrawListBatchManager@@IEAAXXZ.c)
 * Callees:
...
void __fastcall ShrinkableReferenceArray<CGroupDrawListEntry *>::Optimize(__int64 a1)
{
  unsigned int v1; // ebx
  unsigned int v3; // esi
  unsigned int v4; // ebx
    ...
    if ( (int)DynArrayImpl<0>::Grow((__int64)v11, 8u, v4, 1, 0LL) >= 0 )
    {
      ...
      if ( v3 > v4 )
      {
        ...
        if ( (_DWORD)v5 )
        {
          ...
          do
          {
            CDrawListEntry::Release(*(CDrawListEntry **)(v7 + *(_QWORD *)a1));
            v7 += 8LL;
            --v8;
          }
          while ( v8 );
