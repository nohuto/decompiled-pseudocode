/*
 * XREFs of ?GetFragmentsModuleNoRef@CShaderLinkingGraphBuilder@@SAJW4D3DShaderProfileVersion@@PEAPEAUID3D11Module@@@Z @ 0x1800B523C
 * Callers:
 *     UseFragmentsModuleNoRef @ 0x1800B4E2C (UseFragmentsModuleNoRef.c)
 *     LoadShaderBody @ 0x1800B50C8 (LoadShaderBody.c)
 *     PrepareSamplers @ 0x1800B628C (PrepareSamplers.c)
 *     ?CreateCompositingShader@@YAJW4D3D_FEATURE_LEVEL@@V?$span@$$CBUPixelFormatInfo@@$0?0@gsl@@W4DXGI_COLOR_SPACE_TYPE@@PEAPEAUID3D10Blob@@@Z @ 0x180267E48 (-CreateCompositingShader@@YAJW4D3D_FEATURE_LEVEL@@V-$span@$$CBUPixelFormatInfo@@$0-0@gsl@@W4DXGI.c)
 * Callees:
 *     ?MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJIPEAX@Z @ 0x1800393BC (-MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJIPEAX@Z.c)
 */

__int64 __fastcall CShaderLinkingGraphBuilder::GetFragmentsModuleNoRef(__int64 a1, struct ID3D11Module **a2)
{
  unsigned int v2; // ebx
  struct ID3D11Module *v4; // rax
  HRESULT v6; // eax
  __int64 v7; // rcx
  unsigned int v8; // [rsp+20h] [rbp-18h]

  v2 = 0;
  if ( !(_BYTE)a1 )
  {
    v4 = qword_180349D18;
    if ( qword_180349D18 )
      goto LABEL_4;
    v6 = D3DLoadModule(&unk_1803063F0, 0x184FAuLL, &qword_180349D18);
    v2 = v6;
    if ( v6 >= 0 )
    {
      v4 = qword_180349D18;
      goto LABEL_4;
    }
    v8 = 292;
    goto LABEL_13;
  }
  if ( (unsigned __int8)a1 != 1 )
  {
    v2 = -2147024809;
    MilInstrumentationCheckHR_MaybeFailFast(a1, 0LL, 0, -2147024809, 0x138u, 0LL);
    return v2;
  }
  v4 = qword_180349248;
  if ( !qword_180349248 )
  {
    v6 = D3DLoadModule(&unk_1802AB100, 0x207EEuLL, &qword_180349248);
    v2 = v6;
    if ( v6 >= 0 )
    {
      v4 = qword_180349248;
      goto LABEL_4;
    }
    v8 = 304;
LABEL_13:
    MilInstrumentationCheckHR_MaybeFailFast(v7, 0LL, 0, v6, v8, 0LL);
    return v2;
  }
LABEL_4:
  *a2 = v4;
  return v2;
}
