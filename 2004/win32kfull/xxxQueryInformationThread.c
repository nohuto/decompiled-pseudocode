/*
 * XREFs of xxxQueryInformationThread @ 0x1C00BC518
 * Callers:
 ...
 *     ?IsProcessUserService@@YAHQEAU_EPROCESS@@@Z @ 0x1C00BC93C (-IsProcessUserService@@YAHQEAU_EPROCESS@@@Z.c)
          ...
          if ( !(unsigned int)IsProcessDwm(v19) )
          {
            ...
            if ( !(unsigned int)IsProcessUserService(v20) )
            {
              if ( !ThreadWin32Thread || !*(_QWORD *)(ThreadWin32Thread + 448) )
              {
                a3[2] = 2;
LABEL_23:
                if ( ThreadWin32Thread && *(_DWORD *)(ThreadWin32Thread + 888) )
                  a3[3] |= 1u;
                if ( (a3[3] & 1) == 0 )
                  goto LABEL_27;
                if ( a3[2] )
