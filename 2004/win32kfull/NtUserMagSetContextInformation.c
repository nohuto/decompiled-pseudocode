/*
 * XREFs of NtUserMagSetContextInformation @ 0x1C01FF980
 * Callers:
 *     <none>
 * Callees:
 *     UserSetLastError @ 0x1C0090F00 (UserSetLastError.c)
 *     W32GetThreadWin32Thread @ 0x1C00A7BBC (W32GetThreadWin32Thread.c)
 *     __security_check_cookie @ 0x1C0157760 (__security_check_cookie.c)
 *     memset @ 0x1C015C000 (memset.c)
 *     MagSetLensContextInformation @ 0x1C01CDBF4 (MagSetLensContextInformation.c)
 */

__int64 __fastcall NtUserMagSetContextInformation(__int64 a1, int a2, ULONG64 a3, unsigned int a4)
{
  NTSTATUS v8; // ebx
  __int64 v9; // rdx
  ULONG64 v10; // rcx
  __int64 v11; // r8
  double *v12; // r14
  double *v13; // r15
  int v14; // eax
  __int64 v15; // rdi
  ULONG v16; // eax
  __int64 v17; // rdx
  __int64 v18; // r8
  unsigned int v20; // r15d
  unsigned __int64 v21; // rax
  double *v22; // rax
  unsigned int i; // ecx
  double *v24; // r8
  double *v25; // rax
  unsigned int j; // edx
  ULONG64 v27; // r8
  __int64 ThreadWin32Thread; // rax
  double *Buf1; // [rsp+48h] [rbp-200h]
  __int64 v30; // [rsp+60h] [rbp-1E8h]
  double *v31; // [rsp+68h] [rbp-1E0h]
  double *v32; // [rsp+70h] [rbp-1D8h]
  double v33; // [rsp+C0h] [rbp-188h]
  __int128 v34; // [rsp+D8h] [rbp-170h]
  int v35; // [rsp+E8h] [rbp-160h]
  __int128 v36; // [rsp+100h] [rbp-148h]
  __int128 v37; // [rsp+110h] [rbp-138h]
  __int128 v38; // [rsp+120h] [rbp-128h]
  __int128 v39; // [rsp+130h] [rbp-118h]
  __int128 v40; // [rsp+140h] [rbp-108h]
  int v41; // [rsp+150h] [rbp-F8h]
  __int128 v42; // [rsp+178h] [rbp-D0h]
  _OWORD v43[7]; // [rsp+190h] [rbp-B8h] BYREF

  v8 = -1073741811;
  Buf1 = 0LL;
  memset(v43, 0, 0x68uLL);
  v31 = 0LL;
  v32 = 0LL;
  EnterCrit(0LL, 1LL);
  if ( a1 == -3 )
  {
    v30 = -1LL;
  }
  else if ( a1 )
  {
    v30 = ValidateHwnd(a1);
    if ( !v30 )
    {
LABEL_5:
      v12 = 0LL;
      v13 = 0LL;
LABEL_6:
      v14 = 0;
      v15 = 0LL;
      goto LABEL_7;
    }
  }
  else
  {
    v30 = 0LL;
  }
  if ( a2 >= 11 )
  {
    v8 = -1073741821;
    goto LABEL_5;
  }
  if ( a2 == 4 )
  {
    if ( *(_QWORD *)(gptiCurrent + 448LL) != grpdeskRitInput )
    {
      UserSetLastError(5LL, v9, v11);
      v14 = 1;
      v12 = 0LL;
      v13 = 0LL;
      goto LABEL_131;
    }
    if ( (unsigned __int8)Enforced(grpdeskRitInput) && *(int *)(*(_QWORD *)(gptiCurrent + 416LL) + 12LL) >= 0 )
    {
      UserSetLastError(5LL, v9, v11);
      EtwTraceUIPISystemError(*(_QWORD *)(gptiCurrent + 416LL), 0LL, 7LL);
      v8 = -1073741790;
      goto LABEL_5;
    }
  }
  if ( a2 <= 6 )
  {
    if ( a2 == 6 )
    {
      if ( !a3 )
      {
        v14 = 1;
        v12 = 0LL;
        v13 = 0LL;
        goto LABEL_131;
      }
      if ( a4 < 0x10 )
      {
        v8 = -1073741306;
        v14 = 1;
        v12 = 0LL;
        v13 = 0LL;
        goto LABEL_131;
      }
      v10 = a3 + 16;
      if ( a3 + 16 < a3 || v10 > MmUserProbeAddress )
      {
        v10 = MmUserProbeAddress;
        *(_BYTE *)MmUserProbeAddress = 0;
      }
      v43[0] = *(_OWORD *)a3;
      if ( DWORD1(v43[0]) > 0x19 )
      {
        v8 = -1073741306;
        v14 = 1;
        v12 = 0LL;
        v13 = 0LL;
        goto LABEL_131;
      }
      v10 = LODWORD(v43[0]);
      if ( LODWORD(v43[0]) > 0x19 )
      {
        v8 = -1073741306;
        v14 = 1;
        v12 = 0LL;
        v13 = 0LL;
        goto LABEL_131;
      }
      v20 = DWORD1(v43[0]) + LODWORD(v43[0]);
      if ( (unsigned int)(DWORD1(v43[0]) + LODWORD(v43[0])) < LODWORD(v43[0]) )
      {
        v14 = 1;
        v12 = 0LL;
        v13 = 0LL;
        goto LABEL_131;
      }
      if ( v20 > 0x19 )
      {
        v8 = -1073741306;
        v14 = 1;
        v12 = 0LL;
        v13 = 0LL;
        goto LABEL_131;
      }
      v21 = 8LL * v20;
      v9 = 0xFFFFFFFFLL;
      if ( v21 > 0xFFFFFFFF )
      {
        v14 = 1;
        v12 = 0LL;
        v13 = 0LL;
        goto LABEL_131;
      }
      if ( (int)v21 + 8 < (unsigned int)v21 )
      {
        v14 = 1;
        v12 = 0LL;
        v13 = 0LL;
        goto LABEL_131;
      }
      v22 = (double *)Win32AllocPool((unsigned int)(v21 + 8), 1735226197LL);
      v9 = (__int64)v22;
      Buf1 = v22;
      if ( !v22 )
      {
        v8 = -1073741801;
        v14 = 1;
        v12 = 0LL;
        v13 = 0LL;
        goto LABEL_131;
      }
      v31 = v22;
      *v22 = *(double *)v43;
      for ( i = 0; i < v20; ++i )
      {
        v24 = (double *)(a3 + 8 + 8LL * i);
        if ( v24 + 1 < v24 || (unsigned __int64)(v24 + 1) > MmUserProbeAddress )
          v24 = (double *)MmUserProbeAddress;
        v22[i + 1] = *v24;
      }
    }
    else if ( a2 >= 0 )
    {
      if ( a2 <= 1 )
      {
        if ( a3 )
        {
          v14 = 1;
          v12 = 0LL;
          v13 = 0LL;
          goto LABEL_131;
        }
        Buf1 = 0LL;
      }
      else
      {
        switch ( a2 )
        {
          case 2:
            if ( !a3 )
            {
              v14 = 1;
              v12 = 0LL;
              v13 = 0LL;
              goto LABEL_131;
            }
            if ( a4 < 0x10 )
            {
              v8 = -1073741306;
              v14 = 1;
              v12 = 0LL;
              v13 = 0LL;
              goto LABEL_131;
            }
            if ( a3 + 16 < a3 || a3 + 16 > MmUserProbeAddress )
              a3 = MmUserProbeAddress;
            v43[0] = *(_OWORD *)a3;
            Buf1 = (double *)v43;
            break;
          case 3:
            if ( !a3 )
            {
              v14 = 1;
              v12 = 0LL;
              v13 = 0LL;
              goto LABEL_131;
            }
            if ( a4 < 0x20 )
            {
              v8 = -1073741306;
              v14 = 1;
              v12 = 0LL;
              v13 = 0LL;
              goto LABEL_131;
            }
            if ( a3 + 32 < a3 || a3 + 32 > MmUserProbeAddress )
              a3 = MmUserProbeAddress;
            v42 = *(_OWORD *)(a3 + 16);
            v43[0] = *(_OWORD *)a3;
            v43[1] = v42;
            Buf1 = (double *)v43;
            break;
          case 4:
            if ( !a3 )
            {
              v14 = 1;
              v12 = 0LL;
              v13 = 0LL;
              goto LABEL_131;
            }
            if ( a4 < 0x24 )
            {
              v8 = -1073741306;
              v14 = 1;
              v12 = 0LL;
              v13 = 0LL;
              goto LABEL_131;
            }
            if ( a3 + 36 < a3 || a3 + 36 > MmUserProbeAddress )
              a3 = MmUserProbeAddress;
            v34 = *(_OWORD *)(a3 + 16);
            v35 = *(_DWORD *)(a3 + 32);
            v43[0] = *(_OWORD *)a3;
            v43[1] = v34;
            LODWORD(v43[2]) = v35;
            Buf1 = (double *)v43;
            break;
          default:
            if ( !a3 )
            {
              v14 = 1;
              v12 = 0LL;
              v13 = 0LL;
              goto LABEL_131;
            }
            if ( a4 < 0x64 )
            {
              v8 = -1073741306;
              v14 = 1;
              v12 = 0LL;
              v13 = 0LL;
              goto LABEL_131;
            }
            if ( a3 + 100 < a3 || a3 + 100 > MmUserProbeAddress )
              a3 = MmUserProbeAddress;
            v36 = *(_OWORD *)(a3 + 16);
            v37 = *(_OWORD *)(a3 + 32);
            v38 = *(_OWORD *)(a3 + 48);
            v39 = *(_OWORD *)(a3 + 64);
            v40 = *(_OWORD *)(a3 + 80);
            v41 = *(_DWORD *)(a3 + 96);
            v43[0] = *(_OWORD *)a3;
            v43[1] = v36;
            v43[2] = v37;
            v43[3] = v38;
            v43[4] = v39;
            v43[5] = v40;
            LODWORD(v43[6]) = v41;
            Buf1 = (double *)v43;
            break;
        }
      }
    }
LABEL_129:
    ThreadWin32Thread = W32GetThreadWin32Thread((__int64)KeGetCurrentThread());
    v8 = MagSetLensContextInformation(*(_QWORD **)&gMagnContext, ThreadWin32Thread, v30, 0, a2, Buf1);
    v12 = v31;
    v13 = v32;
    v14 = 1;
    goto LABEL_131;
  }
  v10 = (unsigned int)(a2 - 7);
  if ( a2 != 7 )
  {
    v10 = (unsigned int)(a2 - 8);
    if ( a2 == 8 )
    {
      v14 = 1;
      v12 = 0LL;
      v13 = 0LL;
      goto LABEL_131;
    }
    v10 = (unsigned int)(a2 - 9);
    if ( a2 == 9 )
    {
      if ( !a3 )
      {
        v14 = 1;
        v12 = 0LL;
        v13 = 0LL;
        goto LABEL_131;
      }
      if ( a4 < 4 )
      {
        v8 = -1073741306;
        v14 = 1;
        v12 = 0LL;
        v13 = 0LL;
        goto LABEL_131;
      }
    }
    else
    {
      if ( !a3 )
      {
        v14 = 1;
        v12 = 0LL;
        v13 = 0LL;
        goto LABEL_131;
      }
      if ( a4 < 4 )
      {
        v8 = -1073741306;
        v14 = 1;
        v12 = 0LL;
        v13 = 0LL;
        goto LABEL_131;
      }
    }
    if ( a3 + 4 < a3 || a3 + 4 > MmUserProbeAddress )
      a3 = MmUserProbeAddress;
    LODWORD(v43[0]) = *(_DWORD *)a3;
    Buf1 = (double *)v43;
    goto LABEL_129;
  }
  if ( !a3 )
  {
    v14 = 1;
    v12 = 0LL;
    v13 = 0LL;
    goto LABEL_131;
  }
  if ( a4 < 0x10 )
  {
    v8 = -1073741306;
    v14 = 1;
    v12 = 0LL;
    v13 = 0LL;
    goto LABEL_131;
  }
  v10 = a3 + 16;
  if ( a3 + 16 < a3 || v10 > MmUserProbeAddress )
  {
    v10 = MmUserProbeAddress;
    *(_BYTE *)MmUserProbeAddress = 0;
  }
  v43[0] = *(_OWORD *)a3;
  if ( LODWORD(v43[0]) > 6 )
  {
    v8 = -1073741306;
    v14 = 1;
    v12 = 0LL;
    v13 = 0LL;
    goto LABEL_131;
  }
  v10 = 24LL * LODWORD(v43[0]);
  v9 = 0xFFFFFFFFLL;
  if ( v10 > 0xFFFFFFFF )
  {
    v14 = 1;
    v12 = 0LL;
    v13 = 0LL;
    goto LABEL_131;
  }
  if ( (int)v10 + 16 >= (unsigned int)v10 )
  {
    v25 = (double *)Win32AllocPool((unsigned int)(v10 + 16), 1735226197LL);
    v10 = (ULONG64)v25;
    Buf1 = v25;
    if ( !v25 )
    {
      v8 = -1073741801;
      v14 = 1;
      v12 = 0LL;
      v13 = 0LL;
      goto LABEL_131;
    }
    v32 = v25;
    *(_OWORD *)v25 = v43[0];
    for ( j = 0; j < *(_DWORD *)v25; ++j )
    {
      v27 = a3 + 16 + 24LL * j;
      if ( v27 + 24 < v27 || v27 + 24 > MmUserProbeAddress )
        v27 = MmUserProbeAddress;
      v33 = *(double *)(v27 + 16);
      *(_OWORD *)&v25[3 * j + 2] = *(_OWORD *)v27;
      v25[3 * j + 4] = v33;
    }
    goto LABEL_129;
  }
  v14 = 1;
  v12 = 0LL;
  v13 = 0LL;
LABEL_131:
  if ( v8 < 0 )
    goto LABEL_6;
  v15 = 1LL;
LABEL_7:
  if ( !v14 )
  {
    v16 = RtlNtStatusToDosError(v8);
    UserSetLastError(v16, v17, v18);
  }
  if ( v13 )
    Win32FreePool(v13);
  if ( v12 )
    Win32FreePool(v12);
  UserSessionSwitchLeaveCrit(v10, v9, v11);
  return v15;
}
