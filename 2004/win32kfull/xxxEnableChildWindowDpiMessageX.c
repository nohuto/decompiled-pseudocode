/*
 * XREFs of xxxEnableChildWindowDpiMessageX @ 0x1C00510B4
 * Callers:
 *     ?xxxShowGhostWindow@@YAXPEAUtagWND@@0@Z @ 0x1C0003E7C (-xxxShowGhostWindow@@YAXPEAUtagWND@@0@Z.c)
 *     NtUserEnableChildWindowDpiMessage @ 0x1C0050FF0 (NtUserEnableChildWindowDpiMessage.c)
 *     xxxMNOpenHierarchy @ 0x1C023A484 (xxxMNOpenHierarchy.c)
 *     xxxTrackPopupMenuEx @ 0x1C024BC18 (xxxTrackPopupMenuEx.c)
 * Callees:
 *     UpdateMonitorForWindowAndChildren @ 0x1C0051470 (UpdateMonitorForWindowAndChildren.c)
 *     xxxNotifyMonitorChanged @ 0x1C006B018 (xxxNotifyMonitorChanged.c)
 *     _IsTopLevelWindow @ 0x1C006EA50 (_IsTopLevelWindow.c)
 */

__int64 __fastcall xxxEnableChildWindowDpiMessageX(struct tagWND *a1)
{
  unsigned int v1; // ebx
  __int64 v3; // r9
  int v4; // r10d
  int v5; // eax
  __int64 v6; // rdx
  struct tagBWL *v8; // [rsp+30h] [rbp+8h] BYREF

  v1 = 0;
  if ( !a1 || (*(_DWORD *)(*((_QWORD *)a1 + 5) + 288LL) & 0xF) != 2 || !(unsigned int)IsTopLevelWindow(a1) )
    return 0LL;
  v5 = *(_DWORD *)(v3 + 232);
  if ( v4 )
  {
    *(_DWORD *)(v3 + 232) = v5 | 0x10000000;
    *(_DWORD *)(*((_QWORD *)a1 + 5) + 232LL) |= 0x20000000u;
    v6 = ValidateHmonitorNoRip(*(_QWORD *)(*((_QWORD *)a1 + 5) + 256LL));
  }
  else
  {
    *(_DWORD *)(v3 + 232) = v5 & 0xEFFFFFFF;
    *(_DWORD *)(*((_QWORD *)a1 + 5) + 232LL) &= ~0x20000000u;
    v6 = *(_QWORD *)(gpDispInfo + 96LL);
  }
  if ( v6 )
  {
    v8 = 0LL;
    if ( (unsigned int)UpdateMonitorForWindowAndChildren(a1, v6, &v8, 1LL) )
    {
      xxxNotifyMonitorChanged(a1, 0LL, v8);
      return 1;
    }
  }
  return v1;
}
