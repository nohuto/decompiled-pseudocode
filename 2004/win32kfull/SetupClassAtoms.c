/*
 * XREFs of SetupClassAtoms @ 0x1C0120520
 * Callers:
 *     <none>
 * Callees:
 *     <none>
...
__int64 SetupClassAtoms()
{
  ...
  *(_WORD *)(gpsi + 1378LL) = UserAddAtomEx(L"SysFrostedWindow", 1LL, 2LL);
  ...
  WORD2(WPP_MAIN_CB.DeviceQueue.DeviceListHead.Blink) = UserAddAtomEx(L"SysCP", 1LL, 2LL);
  if ( WORD2(WPP_MAIN_CB.DeviceQueue.DeviceListHead.Blink) )
    v8 = v7 != 0 ? v6 : 0;
  else
    LOBYTE(v8) = 0;
  ...
  *(_WORD *)&WPP_MAIN_CB.DeviceQueue.Busy = UserAddAtomEx(L"BrokeredForeground", 1LL, 2LL);
  if ( *(_WORD *)&WPP_MAIN_CB.DeviceQueue.Busy )
    v33 = v31 && v32;
  else
    v33 = 0;
