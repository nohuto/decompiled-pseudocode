/*
 * XREFs of ?ZeroIfNeeded@VIDMM_RECYCLE_HEAP_MGR@@UEAAXPEAX@Z @ 0x1C007AED0
 * Callers:
 *     <none>
 * Callees:
 ...
 *     ?AccessZeroTransition@VIDMM_RECYCLE_RANGE@@QEAAXXZ @ 0x1C007B0A8 (-AccessZeroTransition@VIDMM_RECYCLE_RANGE@@QEAAXXZ.c)
    ...
    if ( !(unsigned __int8)ExTryAcquirePushLockExclusiveEx(v7, 0LL) )
    {
      if ( bTracingEnabled )
      {
        ...
        if ( v17 != -1 && (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
          McTemplateK0q_EtwWriteTransfer((__int64)v9, &EventBlockThread, v10, v17);
      }
