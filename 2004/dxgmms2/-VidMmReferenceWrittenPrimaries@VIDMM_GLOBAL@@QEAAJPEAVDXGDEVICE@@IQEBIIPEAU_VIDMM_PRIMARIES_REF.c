/*
 * XREFs of ?VidMmReferenceWrittenPrimaries@VIDMM_GLOBAL@@QEAAJPEAVDXGDEVICE@@IQEBIIPEAU_VIDMM_PRIMARIES_REFERENCES@@PEAUVIDSCH_HW_QUEUE@@PEAPEAVDXGCONTEXT@@@Z @ 0x1C006BB60
 * Callers:
 *     VidMmReferenceWrittenPrimaries @ 0x1C00019C0 (VidMmReferenceWrittenPrimaries.c)
 * Callees:
  ...
  if ( !(unsigned __int8)ExTryAcquirePushLockSharedEx(v16, 0LL) )
  {
    if ( bTracingEnabled )
    {
      ...
      if ( v32 != -1 && (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
        McTemplateK0q_EtwWriteTransfer(v17, &EventBlockThread, v18, v32);
    }
