/*
 * XREFs of ?SpiSetAccessTimeOut@@YAHKPEAXHHPEAH@Z @ 0x1C013CC24
 * Callers:
...
__int64 __fastcall SpiSetAccessTimeOut(int a1, unsigned int *a2, int a3, int a4, int *a5)
{
  ...
  if ( a3 )
  {
    ...
    *a5 &= v11;
    ...
    if ( v12 )
    {
      RtlStringCchPrintfW(Dest, 40LL, L"%d", (unsigned int)qword_1C024BB94);
      v13 = -1LL;
      do
        ++v13;
      while ( Dest[v13] );
      FastWriteProfileValue(ProfileUserName, 0x12u, (const WCHAR *)L"Flags", 1u, Dest, 2 * v13 + 2);
      RtlStringCchPrintfW(Dest, 40LL, L"%d", HIDWORD(qword_1C024BB94));
      do
        ++v8;
      while ( Dest[v8] );
