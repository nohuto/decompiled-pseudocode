...
__int64 __fastcall sub_180035ED4(HKEY Handle, PSID pSid)
{
  signed int v4; // ebx
  struct _ACL *v5; // rsi
  int KeySecurity; // edi
  ...
  DWORD v18; // [rsp+A0h] [rbp+40h] BYREF
  ...
  if ( pSid && Handle != HKEY_CURRENT_USER_LOCAL_SETTINGS|0x7FFFFFF8LL )
  {
    ...
    if ( KeySecurity == 122 )
    {
      v7 = (void *)sub_180039E78(cbSecurityDescriptor, &unk_18004FFC8);
      v8 = v7;
      if ( v7 )
      {
        ...
        if ( !KeySecurity )
        {
          ...
          {
            v18 = 0;
            KeySecurity = sub_180035234(pSid, pSacl, &v15, &v18);
            if ( !KeySecurity )
              v5 = v15;
          }
          else
