/*
 * XREFs of sub_180029AAC @ 0x180029AAC
 * Callers:
 *     sub_18001E848 @ 0x18001E848 (sub_18001E848.c)
 *     sub_18001FCE0 @ 0x18001FCE0 (sub_18001FCE0.c)
 * Callees:
 *     sub_180017F24 @ 0x180017F24 (sub_180017F24.c)
 *     sub_18002880C @ 0x18002880C (sub_18002880C.c)
 *     sub_18002B388 @ 0x18002B388 (sub_18002B388.c)
 *     _o_pow @ 0x18003A7A8 (_o_pow.c)
 */

// Hidden C++ exception states: #wind=1
__int64 __fastcall sub_180029AAC(DWORD SessionId, unsigned int a2, int a3, float a4, int *a5)
{
  float v5; // xmm7_4
  unsigned int v9; // edi
  struct _RTL_CRITICAL_SECTION *v10; // rsi
  int *v11; // r13
  int v12; // r14d
  int v13; // eax
  __int64 v14; // rbx
  float v15; // xmm9_4
  __int64 **v16; // r8
  __int64 *i; // rcx
  float v18; // xmm6_4
  float v19; // xmm1_4
  __int64 *v20; // rdx
  __int64 *v21; // rdx
  bool v22; // zf
  __int64 result; // rax
  ATL::CAtlException *v24; // rbx
  __int64 v25; // [rsp+30h] [rbp-78h] BYREF
  __int64 v26; // [rsp+38h] [rbp-70h]
  ATL::CAtlException *v27; // [rsp+40h] [rbp-68h] BYREF

  v5 = a4;
  v9 = 0;
  v10 = (struct _RTL_CRITICAL_SECTION *)(qword_18004FE78 + 32);
  v26 = qword_18004FE78 + 32;
  EnterCriticalSection((LPCRITICAL_SECTION)(qword_18004FE78 + 32));
  v11 = a5;
  *a5 = 0;
  v12 = 1;
  v13 = sub_18002880C(SessionId, 1, 0LL, &v25);
  if ( v13 )
  {
    v9 = (unsigned __int16)v13 | 0x80070000;
    if ( v13 <= 0 )
      v9 = v13;
    goto LABEL_32;
  }
  v14 = v25;
  v15 = *(float *)(v25 + 224);
  v16 = (__int64 **)(v25 + 176);
  for ( i = *(__int64 **)(v25 + 176); i && *((_DWORD *)i + 4) != a2; i = (__int64 *)*i )
    ;
  if ( !a3 )
  {
    if ( i )
    {
      v20 = (__int64 *)*i;
      if ( i == *v16 )
        *v16 = v20;
      else
        *(_QWORD *)i[1] = v20;
      v21 = (__int64 *)i[1];
      if ( i == v16[1] )
        v16[1] = v21;
      else
        *(_QWORD *)(*i + 8) = v21;
      *i = (__int64)v16[4];
      v16[4] = i;
      v22 = v16[2] == (__int64 *)1;
      v16[2] = (__int64 *)((char *)v16[2] - 1);
      if ( v22 )
        sub_180017F24((__int64)v16);
    }
    if ( !*(_QWORD *)(v14 + 192) )
      *(_DWORD *)(v14 + 224) = 1065353216;
    goto LABEL_29;
  }
  if ( !i )
  {
    try
    {
      v9 = 0;
      sub_18002B388(v25 + 176, a2);
    }
    catch ( ATL::CAtlException *v27 )
    {
      v24 = v27;
      if ( *(_DWORD *)v27 == -1073741571 )
        o__resetstkoflw();
      v9 = *(_DWORD *)v24;
      v10 = (struct _RTL_CRITICAL_SECTION *)v26;
      if ( *(int *)v24 >= 0 )
      {
        v12 = 1;
        v11 = a5;
        v5 = a4;
        v14 = v25;
        goto LABEL_13;
      }
LABEL_32:
      if ( v10 )
        LeaveCriticalSection(v10);
      result = v9;
    }
  }
LABEL_13:
  v18 = o_pow();
  if ( v5 <= v18 )
    v18 = v5;
  v19 = o_pow();
  if ( v19 <= v18 )
    v19 = v18;
  *(float *)(v14 + 224) = v19;
LABEL_29:
  if ( v15 == *(float *)(v14 + 224) )
    v12 = 0;
  *v11 = v12;
  goto LABEL_32;
}
