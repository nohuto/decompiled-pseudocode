/*
 * XREFs of RaidUnitSendSrbIoControlSynchronously @ 0x14000CE9C
 * Callers:
...
LABEL_12:
  ...
  *(_WORD *)&CurrentStackLocation[-1].MajorFunction = 15;
  ...
  if ( v21 )
  {
    ...
    if ( Mdl )
    {
      if ( (int)StorProbeAndLockPages(Mdl, v24, v12) >= 0 )
        goto LABEL_21;
...
LABEL_21:
  ...
  Irp->UserIosb = (_IO_STATUS_BLOCK *)&v41;
  Irp->UserEvent = 0LL;
  if ( *(_BYTE *)v10 == 1 )
  {
    v7 = Pool2;
...
LABEL_25:
    ...
    if ( Status >= 0 )
    {
      ...
      {
        LOBYTE(v28) = Pool2[3];
        Status = PortSrbTranslateSrbToNtStatus(v28);
      }
    }
    goto LABEL_29;
  }
