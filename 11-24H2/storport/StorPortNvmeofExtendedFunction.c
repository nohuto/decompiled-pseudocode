/*
 * XREFs of StorPortNvmeofExtendedFunction @ 0x140074180
 * Callers:
 *     <none>
 * Callees:
...
LABEL_26:
                        v15 = v6;
                        return (unsigned int)RaidNtStatusToStorStatus(v15);
                      }
                      return (unsigned int)-1056964602;
                    }
                    if ( !a3 )
                      return (unsigned int)-1056964602;
                    NvmeHostId = NvmeAdapterRemoveSubsystemPortMiniport(v8);
                  }
                  else
                  {
                    if ( !a3 || !a4 )
                      return (unsigned int)-1056964602;
                    NvmeHostId = NvmeAdapterAddSubsystemPortMiniport(v8);
                  }
                }
                else
                {
                  if ( !a3 )
                    return (unsigned int)-1056964602;
                  NvmeHostId = NvmeAdapterRemoveHostGateway(v8);
                }
              }
              else
              {
                if ( !a3 || !a4 )
                  return (unsigned int)-1056964602;
                NvmeHostId = NvmeAdapterAddHostGateway(v8);
              }
LABEL_46:
              v15 = NvmeHostId;
              return (unsigned int)RaidNtStatusToStorStatus(v15);
            ...
            if ( !KeGetCurrentIrql() )
            {
              NvmeHostId = GetNvmeHostId((_OWORD *)a3);
              goto LABEL_45;
            }
          }
          else
          {
            ...
            if ( !KeGetCurrentIrql() )
            {
              NvmeHostId = GetNvmeHostNQN(0, (char *)a3);
LABEL_45:
              if ( NvmeHostId >= 0 )
                return v6;
              goto LABEL_46;
