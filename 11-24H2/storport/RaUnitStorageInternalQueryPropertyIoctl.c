/*
 * XREFs of RaUnitStorageInternalQueryPropertyIoctl @ 0x1401896B8
 * Callers:
 *     RaUnitDeviceControlIrp @ 0x14002C620 (RaUnitDeviceControlIrp.c)
 * Callees:
 *     McTemplateK0pduuuuup_EtwWriteTransfer @ 0x140052DF0 (McTemplateK0pduuuuup_EtwWriteTransfer.c)
 *     McTemplateK0pd_EtwWriteTransfer @ 0x140052EC8 (McTemplateK0pd_EtwWriteTransfer.c)
 *     McTemplateK0pqd_EtwWriteTransfer @ 0x1400569AC (McTemplateK0pqd_EtwWriteTransfer.c)
 *     StorUnitQueryBypassIOProperty @ 0x1400A1F8C (StorUnitQueryBypassIOProperty.c)
 *     __security_check_cookie @ 0x140143690 (__security_check_cookie.c)
 *     StorUnitQueryDeviceIdentifyIoCommandSetProperty @ 0x14018CFD0 (StorUnitQueryDeviceIdentifyIoCommandSetProperty.c)
 *     StorUnitQueryDeviceIdentifyNamespaceProperty @ 0x14018D394 (StorUnitQueryDeviceIdentifyNamespaceProperty.c)
 *     StorUnitQueryNamespaceIdProperty @ 0x14018D758 (StorUnitQueryNamespaceIdProperty.c)
 *     StorUnitQueryNamespaceTypeProperty @ 0x14018DB44 (StorUnitQueryNamespaceTypeProperty.c)
 */

NTSTATUS __fastcall RaUnitStorageInternalQueryPropertyIoctl(__int64 a1, __int64 a2, _BYTE *a3)
{
  int v3; // ebx
  bool v6; // zf
  unsigned __int64 v7; // rcx
  __int64 v8; // rdx
  int *v9; // rax
  const EVENT_DESCRIPTOR *v10; // rdx
  unsigned __int64 v11; // rdx
  char v12; // r14
  _BYTE *v13; // r9
  unsigned __int8 v14; // r10
  char v15; // si
  char v16; // r11
  char v17; // r15
  char *v18; // r11
  unsigned int v19; // r13d
  unsigned int *v20; // r12
  __int64 v21; // rax
  unsigned __int64 v22; // rbp
  char v23; // cl
  char v24; // r8
  _BYTE *v25; // rax
  unsigned int v26; // eax
  char v27; // al
  int v29; // ecx
  int v30; // ecx
  int v31; // ecx
  int v32; // ecx
  __int64 v33; // rcx
  int v34; // [rsp+20h] [rbp-98h]
  char v35; // [rsp+60h] [rbp-58h]
  char v36; // [rsp+61h] [rbp-57h]
  unsigned int v37; // [rsp+64h] [rbp-54h]
  GUID v38; // [rsp+68h] [rbp-50h] BYREF

  v3 = 0;
  if ( *(_DWORD *)(*(_QWORD *)(a2 + 184) + 16LL) >= 8u )
  {
    v29 = **(_DWORD **)(a2 + 24);
    if ( !v29 )
      return StorUnitQueryNamespaceIdProperty(a1);
    v30 = v29 - 2;
    if ( !v30 )
      return StorUnitQueryDeviceIdentifyNamespaceProperty(a1);
    v31 = v30 - 1;
    if ( !v31 )
      return StorUnitQueryDeviceIdentifyIoCommandSetProperty(a1);
    v32 = v31 - 3;
    if ( !v32 )
      return StorUnitQueryBypassIOProperty(a1, a2, a3);
    if ( v32 == 21 )
      return StorUnitQueryNamespaceTypeProperty(a1);
    if ( StorEtwLoggingEnabled )
    {
      v38 = 0LL;
      IoGetActivityIdIrp(a2, &v38);
      if ( (byte_140171462 & 8) != 0 )
        McTemplateK0pd_EtwWriteTransfer(v33, &EventNonReadWriteRequestComplete, &v38, a2, *(_DWORD *)(a2 + 48));
    }
    ++*(_BYTE *)(a2 + 67);
    *(_QWORD *)(a2 + 184) += 72LL;
    return IofCallDriver(*(PDEVICE_OBJECT *)(*(_QWORD *)(a1 + 24) + 8LL), (PIRP)a2);
  }
  v6 = StorEtwLoggingEnabled == 0;
  *(_QWORD *)(a2 + 56) = 0LL;
  *(_BYTE *)(a2 + 141) = -84;
  *(_DWORD *)(a2 + 48) = -1073741585;
  if ( v6 )
    goto LABEL_67;
  v38 = 0LL;
  IoGetActivityIdIrp(a2, &v38);
  v8 = *(_QWORD *)(a2 + 184);
  if ( *(_BYTE *)v8 == 14 )
  {
    if ( (byte_140171462 & 8) == 0 )
      goto LABEL_67;
    v7 = *(unsigned int *)(a2 + 48);
    v10 = &EventNonReadWriteRequestComplete;
    v34 = *(_DWORD *)(a2 + 48);
    goto LABEL_66;
  }
  if ( *(_BYTE *)v8 != 15 )
  {
    if ( *(_BYTE *)v8 != 27 )
      goto LABEL_67;
    if ( *(_BYTE *)(v8 + 1) == 7 && !*(_DWORD *)(v8 + 8) )
    {
      if ( (byte_140171462 & 0x40) != 0 )
      {
        v9 = *(int **)(a2 + 56);
        if ( v9 )
          v3 = *v9;
        McTemplateK0pqd_EtwWriteTransfer(v7, v8, &v38, a2, v3, *(_DWORD *)(a2 + 48));
      }
      goto LABEL_67;
    }
    if ( (byte_140171462 & 0x20) == 0 )
      goto LABEL_67;
    v10 = &EventPnpRequestComplete;
    v34 = *(_DWORD *)(a2 + 48);
LABEL_66:
    McTemplateK0pd_EtwWriteTransfer(v7, v10, &v38, a2, v34);
    goto LABEL_67;
  }
  if ( byte_140171461 >= 0 )
    goto LABEL_67;
  v11 = *(_QWORD *)(v8 + 8);
  v12 = 0;
  v13 = 0LL;
  v35 = 0;
  v14 = 0;
  v15 = 0;
  v16 = 0;
  v17 = 0;
  if ( *(_BYTE *)(v11 + 2) != 40 )
  {
    v23 = *(_BYTE *)(v11 + 72);
    v13 = *(_BYTE **)(v11 + 32);
    v14 = *(_BYTE *)(v11 + 11);
    v12 = *(_BYTE *)(v11 + 4);
    if ( !*(_BYTE *)(v11 + 2) )
      goto LABEL_43;
    goto LABEL_67;
  }
  v18 = 0LL;
  v36 = 0;
  if ( *(_DWORD *)(v11 + 20) )
    goto LABEL_67;
  v19 = 0;
  v37 = *(_DWORD *)(v11 + 56);
  if ( !v37 )
    goto LABEL_40;
  v20 = (unsigned int *)(v11 + 120);
  while ( 1 )
  {
    v21 = *v20;
    if ( (unsigned int)v21 >= 0x80 )
    {
      v22 = *(unsigned int *)(v11 + 16);
      if ( (unsigned int)v21 < (unsigned int)v22 )
        break;
    }
LABEL_34:
    ++v19;
    ++v20;
    if ( v19 >= v37 )
      goto LABEL_40;
  }
  if ( *(_DWORD *)(v21 + v11) != 64 )
  {
    v7 = (unsigned int)(*(_DWORD *)(v21 + v11) - 65);
    if ( *(_DWORD *)(v21 + v11) == 65 )
    {
      v7 = v21 + 56;
      if ( v21 + 56 <= v22 )
      {
        v36 = 1;
        if ( *(_BYTE *)(v21 + v11 + 10) )
          v18 = (char *)(v21 + v11 + 24);
        v12 = *(_BYTE *)(v21 + v11 + 8);
        v13 = *(_BYTE **)(v21 + v11 + 16);
        v14 = *(_BYTE *)(v21 + v11 + 9);
      }
    }
    else if ( *(_DWORD *)(v21 + v11) == 66 )
    {
      v7 = v21 + 40;
      if ( v21 + 40 <= v22 )
      {
        if ( *(_DWORD *)(v21 + v11 + 12) )
          v18 = (char *)(v21 + v11 + 32);
        v13 = *(_BYTE **)(v21 + v11 + 24);
        goto LABEL_39;
      }
    }
    goto LABEL_33;
  }
  v7 = v21 + 40;
  if ( v21 + 40 > v22 )
  {
LABEL_33:
    if ( v36 )
      goto LABEL_40;
    goto LABEL_34;
  }
  if ( *(_BYTE *)(v21 + v11 + 10) )
    v18 = (char *)(v21 + v11 + 24);
  v13 = *(_BYTE **)(v21 + v11 + 16);
LABEL_39:
  v14 = *(_BYTE *)(v21 + v11 + 9);
  v12 = *(_BYTE *)(v21 + v11 + 8);
LABEL_40:
  if ( v18 )
  {
    v23 = *v18;
    v16 = 0;
LABEL_43:
    LOBYTE(v7) = v23 - 8;
    if ( (v7 & 0x5D) == 0 )
    {
      v24 = *(_BYTE *)(v11 + 3);
      if ( v24 == 1 || !v13 || !v14 )
        goto LABEL_62;
      LOBYTE(v11) = 0;
      v7 = (unsigned __int64)&v13[v14];
      v25 = v13 + 8;
      if ( (unsigned __int8)((*v13 & 0x7F) - 114) <= 1u )
      {
        if ( (unsigned __int64)v25 <= v7 )
        {
          LOBYTE(v11) = 1;
          v16 = v13[2];
          v15 = v13[1] & 0xF;
          v17 = v13[3];
        }
      }
      else
      {
        if ( (unsigned __int64)v25 <= v7 )
        {
          v15 = v13[2] & 0xF;
          v26 = v14;
          if ( (unsigned int)(unsigned __int8)v13[7] + 8 <= v14 )
            v26 = (unsigned __int8)v13[7] + 8;
          v7 = (unsigned __int64)(v13 + 13);
          v11 = (unsigned __int64)&v13[v26];
          if ( (unsigned __int64)(v13 + 13) > v11 )
            v35 = 0;
          else
            v35 = v13[12];
          if ( (unsigned __int64)(v13 + 14) <= v11 )
            v17 = *(_BYTE *)v7;
          LOBYTE(v11) = 1;
        }
        v16 = v35;
      }
      if ( (_BYTE)v11 )
      {
        LOBYTE(v3) = v16;
        v27 = v17;
      }
      else
      {
LABEL_62:
        v15 = 0;
        v27 = 0;
      }
      McTemplateK0pduuuuup_EtwWriteTransfer(v7, v11, &v38, a2, *(_DWORD *)(a2 + 48), v24, v12, v15, v3, v27, a2);
    }
  }
LABEL_67:
  IofCompleteRequest((PIRP)a2, 0);
  return -1073741585;
}
