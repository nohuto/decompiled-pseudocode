/*
 * XREFs of ?HandleMonitorDepartureCase@@YAJQEAUMONITORSCOUNT_CALLBACK_CONTEXT@@AEBU_LUID@@IW4MONITOR_EVENT@@_KPEAU_DXGK_DISPLAY_SCENARIO_CONTEXT@@@Z @ 0x14038EA8C
 * Callers:
 *     ?DxgkHandleMonitorEvent@@YAJU_LUID@@IW4MONITOR_EVENT@@_KPEAU_D3DKMT_DISPLAY_CALLOUT_BATCH_ACTION@@@Z @ 0x14038E710 (-DxgkHandleMonitorEvent@@YAJU_LUID@@IW4MONITOR_EVENT@@_KPEAU_D3DKMT_DISPLAY_CALLOUT_BATCH_ACTION.c)
 * Callees:
 *     ?GetGlobal@DXGGLOBAL@@SAPEAV1@XZ @ 0x140009940 (-GetGlobal@DXGGLOBAL@@SAPEAV1@XZ.c)
 *     DxgkLogInternalTriageEvent @ 0x14000A8B0 (DxgkLogInternalTriageEvent.c)
 *     ??1COREADAPTERACCESS@@QEAA@XZ @ 0x14000C9A0 (--1COREADAPTERACCESS@@QEAA@XZ.c)
 *     ?ReleaseReference@DXGADAPTER@@QEAAX_K@Z @ 0x14000CC20 (-ReleaseReference@DXGADAPTER@@QEAAX_K@Z.c)
 *     ?AcquireShared@COREADAPTERACCESS@@QEAAJPEBD@Z @ 0x14000F940 (-AcquireShared@COREADAPTERACCESS@@QEAAJPEBD@Z.c)
 *     ?Release@COREADAPTERACCESS@@QEAAXXZ @ 0x14002DAC0 (-Release@COREADAPTERACCESS@@QEAAXXZ.c)
 *     ??0COREADAPTERACCESS@@QEAA@QEAVDXGADAPTER@@0@Z @ 0x14002DF18 (--0COREADAPTERACCESS@@QEAA@QEAVDXGADAPTER@@0@Z.c)
 *     ?DxgkLogCodePointPacket@@YAXW4_DXGK_DIAG_CODE_POINT_TYPE@@IIIU_LUID@@@Z @ 0x14004A724 (-DxgkLogCodePointPacket@@YAXW4_DXGK_DIAG_CODE_POINT_TYPE@@IIIU_LUID@@@Z.c)
 *     __security_check_cookie @ 0x1400A1BC0 (__security_check_cookie.c)
 *     ?UpdateEmergencyMonitorContext@MONITORSCOUNT_CALLBACK_CONTEXT@@QEAAJAEBU_LUID@@IW4_DMM_VIDPN_MONITOR_TYPE@@@Z @ 0x1401C5C00 (-UpdateEmergencyMonitorContext@MONITORSCOUNT_CALLBACK_CONTEXT@@QEAAJAEBU_LUID@@IW4_DMM_VIDPN_MON.c)
 *     ?FindPathDescriptorByTarget@CCD_TOPOLOGY@@QEBAJAEBU_LUID@@IPEAI@Z @ 0x1402680D8 (-FindPathDescriptorByTarget@CCD_TOPOLOGY@@QEBAJAEBU_LUID@@IPEAI@Z.c)
 *     ?ReferenceAdapterByLuid@DXGGLOBAL@@QEAAPEAVDXGADAPTER@@U_LUID@@PEA_K@Z @ 0x140296878 (-ReferenceAdapterByLuid@DXGGLOBAL@@QEAAPEAVDXGADAPTER@@U_LUID@@PEA_K@Z.c)
 *     ?IterateAdaptersWithCallback@DXGGLOBAL@@QEAAJP6AJPEAVDXGADAPTER@@PEAX@Z1W4_ITERATE_ADAPTER_FLAGS@@@Z @ 0x14029AE64 (-IterateAdaptersWithCallback@DXGGLOBAL@@QEAAJP6AJPEAVDXGADAPTER@@PEAX@Z1W4_ITERATE_ADAPTER_FLAGS.c)
 *     ??1CCD_TOPOLOGY@@QEAA@XZ @ 0x14031B384 (--1CCD_TOPOLOGY@@QEAA@XZ.c)
 *     ?Global@CCD_BTL@@SAAEAV1@XZ @ 0x14031CC98 (-Global@CCD_BTL@@SAAEAV1@XZ.c)
 *     ??0CCD_TOPOLOGY@@QEAA@GG@Z @ 0x14031CF20 (--0CCD_TOPOLOGY@@QEAA@GG@Z.c)
 *     ?Functionalize@CCD_TOPOLOGY@@QEAAJI_N@Z @ 0x14031FF34 (-Functionalize@CCD_TOPOLOGY@@QEAAJI_N@Z.c)
 *     ?Persist@CCD_TOPOLOGY@@QEAAJI@Z @ 0x140341C24 (-Persist@CCD_TOPOLOGY@@QEAAJI@Z.c)
 *     DxgkInvalidateMonitorConnections @ 0x14038E2E0 (DxgkInvalidateMonitorConnections.c)
 *     ?DisplayConfigHandleMonitorInvalidation@@YAJ_KPEAUMONITORSCOUNT_CALLBACK_CONTEXT@@PEAU_DXGK_DISPLAY_SCENARIO_CONTEXT@@@Z @ 0x14038EFEC (-DisplayConfigHandleMonitorInvalidation@@YAJ_KPEAUMONITORSCOUNT_CALLBACK_CONTEXT@@PEAU_DXGK_DISP.c)
 *     ?CopyRenewScope@CCD_TOPOLOGY@@QEAAJAEBV1@@Z @ 0x1403905D8 (-CopyRenewScope@CCD_TOPOLOGY@@QEAAJAEBV1@@Z.c)
 *     ?DmmIsTargetInClientVidPnTopology@@YAJQEAXIPEAE@Z @ 0x1403FDE18 (-DmmIsTargetInClientVidPnTopology@@YAJQEAXIPEAE@Z.c)
 */

__int64 __fastcall HandleMonitorDepartureCase(
        __int64 a1,
        struct _LUID *a2,
        unsigned int a3,
        int a4,
        __int64 a5,
        struct _DXGK_DISPLAY_SCENARIO_CONTEXT *a6)
{
  __int64 v7; // rsi
  __int64 v9; // rdi
  struct DXGGLOBAL *Global; // rax
  int v11; // eax
  unsigned int v12; // edi
  DXGGLOBAL *v14; // rax
  struct DXGADAPTER *v15; // rax
  DXGADAPTER *v16; // rbx
  unsigned int v17; // ebx
  int v18; // eax
  __int64 v19; // rsi
  struct DXGGLOBAL *v20; // rax
  int v21; // eax
  int v22; // eax
  __int64 v23; // r9
  struct CCD_BTL *v24; // rax
  int v25; // eax
  unsigned int v26; // esi
  int PathDescriptorByTarget; // eax
  __int64 v28; // r9
  int v29; // eax
  __int64 v30; // r8
  __int64 v31; // r9
  int v32; // eax
  __int64 v33; // rbx
  __int64 v34; // rsi
  int v35; // eax
  __int64 v36; // [rsp+20h] [rbp-E0h]
  __int64 v37; // [rsp+20h] [rbp-E0h]
  __int64 v38; // [rsp+28h] [rbp-D8h]
  __int64 v39; // [rsp+28h] [rbp-D8h]
  __int64 HighPart; // [rsp+28h] [rbp-D8h]
  __int64 LowPart; // [rsp+30h] [rbp-D0h]
  unsigned __int8 v42[8]; // [rsp+50h] [rbp-B0h] BYREF
  unsigned __int64 v43; // [rsp+58h] [rbp-A8h] BYREF
  unsigned int v44; // [rsp+60h] [rbp-A0h]
  struct _DXGK_DISPLAY_SCENARIO_CONTEXT *v45; // [rsp+68h] [rbp-98h]
  struct D3DKMT_GETPATHSMODALITY *v46[9]; // [rsp+70h] [rbp-90h] BYREF
  int v47; // [rsp+BCh] [rbp-44h]
  int v48; // [rsp+C0h] [rbp-40h]
  _BYTE v49[144]; // [rsp+E0h] [rbp-20h] BYREF

  v7 = a4;
  v9 = a3;
  v45 = a6;
  if ( a4 == 10 && a5 == 2 )
  {
    *(_BYTE *)(a1 + 25) = 0;
    Global = DXGGLOBAL::GetGlobal();
    v11 = DXGGLOBAL::IterateAdaptersWithCallback(
            (__int64)Global,
            (__int64 (__fastcall *)(_QWORD *, __int64))DisplayConfigCountMonitorsCallback,
            a1,
            (int)v7 - 6);
    v12 = v11;
    if ( v11 < 0 )
    {
      WdLogSingleEntry1((unsigned int)(v7 - 7), v11);
      WdLogGlobalForLineNumber = 3150;
      return v12;
    }
    *(_BYTE *)(a1 + 26) = 1;
    return 0LL;
  }
  if ( a5 != 5 )
  {
    v22 = DxgkInvalidateMonitorConnections(2uLL);
    if ( v22 < 0 )
    {
      WdLogSingleEntry1(4LL, v22);
      WdLogGlobalForLineNumber = 3229;
    }
    if ( (int)MONITORSCOUNT_CALLBACK_CONTEXT::UpdateEmergencyMonitorContext(a1, a2, v9) < 0 )
    {
      WdLogSingleEntry3(3LL, a2->HighPart, a2->LowPart, v9);
      WdLogGlobalForLineNumber = 3246;
    }
    if ( *(_BYTE *)(a1 + 24) )
    {
      *(_BYTE *)(a1 + 26) = 1;
      *(_BYTE *)(a1 + 31) = 1;
      CCD_TOPOLOGY::CCD_TOPOLOGY((CCD_TOPOLOGY *)v46, 8LL, 0LL, v23);
      v24 = CCD_BTL::Global();
      v25 = CCD_TOPOLOGY::CopyRenewScope((CCD_TOPOLOGY *)v46, (struct CCD_BTL *)((char *)v24 + 8));
      v44 = v25;
      if ( v25 < 0 )
      {
        v33 = v7;
        v34 = v25;
        WdLogSingleEntry5(2LL, v25, a2->HighPart, a2->LowPart, v9, v33);
        LowPart = a2->LowPart;
        HighPart = a2->HighPart;
        WdLogGlobalForLineNumber = 3341;
        DxgkLogInternalTriageEvent(
          0LL,
          0x40000LL,
          0xFFFFFFFFLL,
          L"Unable to get copy of most recent topology. (NtStatus = 0x%I64x, i_AdapterLuid = 0x%I64x%08I64x, i_TargetId = "
           "0x%I64x, i_MonitorEvent = 0x%I64x)",
          v34,
          HighPart,
          LowPart,
          v9,
          v33);
        DxgkLogCodePointPacket(0x3Du, v44, 0, 0, (__int64)*a2);
      }
      else
      {
        v26 = *((_DWORD *)DXGGLOBAL::GetGlobal() + 470);
        if ( v26 > 1 )
        {
          WdLogSingleEntry0(3LL);
          WdLogGlobalForLineNumber = 3277;
          v43 = 0LL;
          DxgkLogCodePointPacket(0x52u, v26, 0, 0, 0LL);
          CCD_TOPOLOGY::~CCD_TOPOLOGY((CCD_TOPOLOGY *)v46);
          return 0LL;
        }
        if ( *(_DWORD *)(a1 + 20) != -1 )
        {
          *(_BYTE *)(a1 + 25) = 0;
          PathDescriptorByTarget = CCD_TOPOLOGY::FindPathDescriptorByTarget((CCD_TOPOLOGY *)v46, a2, v9, 0LL);
          if ( PathDescriptorByTarget == -1073741275 )
          {
            v47 = 1;
            v48 = 4;
            v29 = CCD_TOPOLOGY::Functionalize((CCD_TOPOLOGY *)v46, 0, 0LL, v28);
            if ( v29 >= 0 )
            {
              v32 = CCD_TOPOLOGY::Persist(v46, 0LL, v30, v31);
              if ( v32 >= 0 )
              {
                *(_DWORD *)(a1 + 32) |= 0x20u;
                *(_BYTE *)(a1 + 26) = 0;
                *(_BYTE *)(a1 + 31) = 0;
              }
              else
              {
                WdLogSingleEntry1(3LL, v32);
                WdLogGlobalForLineNumber = 3311;
              }
            }
            else
            {
              WdLogSingleEntry1(3LL, v29);
              WdLogGlobalForLineNumber = 3300;
            }
          }
          else if ( PathDescriptorByTarget < 0 )
          {
            WdLogSingleEntry2(3LL, PathDescriptorByTarget, v9);
            WdLogGlobalForLineNumber = 3330;
          }
        }
      }
      CCD_TOPOLOGY::~CCD_TOPOLOGY((CCD_TOPOLOGY *)v46);
    }
    if ( *(_BYTE *)(a1 + 30) )
    {
      *(_BYTE *)(a1 + 25) = 0;
      *(_DWORD *)(a1 + 20) = v9;
    }
    v35 = DisplayConfigHandleMonitorInvalidation(5uLL, (struct MONITORSCOUNT_CALLBACK_CONTEXT *)a1, v45);
    v17 = v35;
    if ( v35 < 0 )
    {
      WdLogSingleEntry1(4LL, v35);
      WdLogGlobalForLineNumber = 3370;
      v17 = 0;
    }
    WdLogSingleEntry1(4LL, v17);
    WdLogGlobalForLineNumber = 3374;
    return v17;
  }
  v43 = 0LL;
  v14 = DXGGLOBAL::GetGlobal();
  v15 = DXGGLOBAL::ReferenceAdapterByLuid(v14, *a2, &v43);
  v16 = v15;
  if ( !v15 )
  {
    v17 = -1073741811;
    WdLogSingleEntry3(2LL, a2->HighPart, a2->LowPart, -1073741811LL);
    v38 = a2->LowPart;
    v36 = a2->HighPart;
    WdLogGlobalForLineNumber = 3173;
    DxgkLogInternalTriageEvent(
      0LL,
      0x40000LL,
      0xFFFFFFFFLL,
      L"Failed to find adapter from LUID 0x%I64x%08I64x, returning 0x%I64x.",
      v36,
      v38,
      -1073741811LL,
      0LL,
      0LL);
    return v17;
  }
  COREADAPTERACCESS::COREADAPTERACCESS((COREADAPTERACCESS *)v49, v15, 0LL);
  DXGADAPTER::ReleaseReference(v16);
  v18 = COREADAPTERACCESS::AcquireShared((COREADAPTERACCESS *)v49, 0LL);
  v19 = v18;
  if ( v18 >= 0 )
  {
    v42[0] = 0;
    if ( DmmIsTargetInClientVidPnTopology(v16, v9, v42) >= 0 && !v42[0] )
      *(_BYTE *)(a1 + 26) = 0;
    COREADAPTERACCESS::Release((COREADAPTERACCESS *)v49);
    *(_BYTE *)(a1 + 25) = 0;
    v20 = DXGGLOBAL::GetGlobal();
    v21 = DXGGLOBAL::IterateAdaptersWithCallback(
            (__int64)v20,
            (__int64 (__fastcall *)(_QWORD *, __int64))DisplayConfigCountMonitorsCallback,
            a1,
            4);
    v12 = v21;
    if ( v21 >= 0 )
    {
      v12 = 0;
    }
    else
    {
      WdLogSingleEntry1(3LL, v21);
      WdLogGlobalForLineNumber = 3217;
    }
    COREADAPTERACCESS::~COREADAPTERACCESS((COREADAPTERACCESS *)v49);
    return v12;
  }
  WdLogSingleEntry3(2LL, a2->HighPart, a2->LowPart, v18);
  v39 = a2->LowPart;
  v37 = a2->HighPart;
  WdLogGlobalForLineNumber = 3189;
  DxgkLogInternalTriageEvent(
    0LL,
    0x40000LL,
    0xFFFFFFFFLL,
    L"Failed to acquire adapter core access on adapter 0x%I64x%08I64x, NtStatus = 0x%I64x.",
    v37,
    v39,
    v19,
    0LL,
    0LL);
  COREADAPTERACCESS::~COREADAPTERACCESS((COREADAPTERACCESS *)v49);
  return (unsigned int)v19;
}
