/*
 * XREFs of ?_CollectWindows@CLivePreview@@AEAAJXZ @ 0x180040B64
 * Callers:
...
LABEL_43:
    ...
    else
    {
      if ( GetClassNameW((HWND)Blink[2].Blink, ClassName, 260) )
      {
        for ( j = 0; j < 3; ++j )
        {
          if ( !(unsigned int)_o__wcsicmp(off_1800FC1E8[j], ClassName) )
          {
            BYTE2(Blink[46].Flink) |= 0x80u;
            BYTE3(Blink[46].Flink) |= 1u;
            break;
...
LABEL_13:
          v9 = 1;
        BYTE2(Blink[46].Flink) = (v9 << 7) | BYTE2(Blink[46].Flink) & 0x7F;
      }
    }
    if ( (BYTE3(Blink[46].Flink) & 1) == 0 || SBYTE2(Blink[46].Flink) >= 0 )
    {
      ++*((_DWORD *)this + 124);
      v11 = BYTE3(Blink[46].Flink);
      if ( (v11 & 1) == 0 )
      {
        v12 = *((_DWORD *)this + 125);
        if ( v12 >= 0xA )
        {
          BYTE3(Blink[46].Flink) = v11 | 1;
        }
        else
        {
          *((_DWORD *)this + 125) = v12 + 1;
          for ( k = 0; k < *((_DWORD *)this + 72); ++k )
          {
            ...
            if ( (*(_BYTE *)(v14 + 739) & 1) == 0 && *(_DWORD *)(v14 + 128) == 1 )
            {
              rcDst = 0LL;
