/*
 * XREFs of ?OnAnimationClockChanged@CAnimationScheduler@@UEAAXU_GUID@@W4AnimationClockState@@@Z @ 0x180059440
 * Callers:
 *     <none>
 * Callees:
  ...
  if ( v6 )
  {
        ...
        if ( v9 )
        {
          ...
          else
          {
            for ( i = 0LL; (unsigned int)i < *(_DWORD *)(a1 + 32); i = (unsigned int)(i + 1) )
            {
              ...
              if ( !v13 )
                CAnimationScheduler::OnAnimationComplete((CAnimationScheduler *)(a1 - 8), v12[4].m128i_u32[2]);
            }
          }
        }
        else
    ...
    else
    {
      for ( k = 0LL; (unsigned int)k < *(_DWORD *)(a1 + 32); k = (unsigned int)(k + 1) )
      {
        ...
        v19 = *(__m128i **)(v18 + 8 * k);
        if ( v19 )
          CMILRefCountBase::AddRef(*(CMILRefCountBase **)(v18 + 8 * k));
