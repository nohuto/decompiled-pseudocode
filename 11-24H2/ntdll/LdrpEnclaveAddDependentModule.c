/*
 * XREFs of LdrpEnclaveAddDependentModule @ 0x180132D54
 * Callers:
 *     LdrpEnclaveAddDelayloadModules @ 0x1800D9400 (LdrpEnclaveAddDelayloadModules.c)
 *     LdrpEnclaveAddForwarderModules @ 0x1800D94B8 (LdrpEnclaveAddForwarderModules.c)
 * Callees:
 *     RtlpSysVolFree @ 0x180001470 (RtlpSysVolFree.c)
 *     LdrpAppendAnsiStringToFilenameBuffer @ 0x18000BF10 (LdrpAppendAnsiStringToFilenameBuffer.c)
 *     LdrpPreprocessDllName @ 0x18005A570 (LdrpPreprocessDllName.c)
 *     LdrpFindOrPrepareEnclaveModule @ 0x1800D86F0 (LdrpFindOrPrepareEnclaveModule.c)
 *     __security_check_cookie @ 0x1801659C0 (__security_check_cookie.c)
 *     memset$thunk$772440563353939046 @ 0x180172030 (memset$thunk$772440563353939046.c)
 */

__int64 __fastcall LdrpEnclaveAddDependentModule(__int64 *a1, STRING *a2)
{
  int appended; // ebx
  int v6; // [rsp+40h] [rbp-C0h] BYREF
  __int64 v7; // [rsp+48h] [rbp-B8h] BYREF
  const void *v8; // [rsp+50h] [rbp-B0h] BYREF
  _WORD *v9; // [rsp+58h] [rbp-A8h]
  _WORD v10[128]; // [rsp+60h] [rbp-A0h] BYREF
  int v11; // [rsp+160h] [rbp+60h] BYREF
  _WORD *v12; // [rsp+168h] [rbp+68h]
  _WORD v13[128]; // [rsp+170h] [rbp+70h] BYREF

  memset_thunk_772440563353939046(&v11, 0, 0x110uLL);
  memset_thunk_772440563353939046(&v8, 0, 0x110uLL);
  LODWORD(v8) = 0x1000000;
  v9 = v10;
  v11 = 0x1000000;
  v12 = v13;
  v10[0] = 0;
  v13[0] = 0;
  appended = LdrpAppendAnsiStringToFilenameBuffer(&v8, a2);
  if ( appended >= 0 )
  {
    v6 = 0x800000;
    appended = LdrpPreprocessDllName((__int64)&v8, (unsigned __int16 *)&v11, 0LL, &v6);
    if ( appended >= 0 )
      appended = LdrpFindOrPrepareEnclaveModule(a1[22], (__int64)&v11, a1[2], v6, a1[7], &v7, a1[5]);
  }
  if ( v10 != v9 )
    RtlpSysVolFree((__int64)v9);
  v9 = v10;
  LODWORD(v8) = 0x1000000;
  v10[0] = 0;
  if ( v13 != v12 )
    RtlpSysVolFree((__int64)v12);
  return (unsigned int)appended;
}
