/*
 * XREFs of PssNtWalkSnapshot @ 0x1800C4A50
 * Callers:
 *     <none>
 * Callees:
 *     PssNtValidateDescriptor @ 0x1800C4CD0 (PssNtValidateDescriptor.c)
 *     PsspWalkInfoClass_PSS_WALK_AUXILIARY_PAGES @ 0x1800C4DE0 (PsspWalkInfoClass_PSS_WALK_AUXILIARY_PAGES.c)
 *     PsspWalkInfoClass_PSS_WALK_HANDLES @ 0x1800C4F20 (PsspWalkInfoClass_PSS_WALK_HANDLES.c)
 *     PsspWalkInfoClass_PSS_WALK_THREADS @ 0x1800C50A4 (PsspWalkInfoClass_PSS_WALK_THREADS.c)
 *     ZwMapViewOfSection @ 0x180162190 (ZwMapViewOfSection.c)
 */

__int64 __fastcall PssNtWalkSnapshot(__int64 a1, int a2, _QWORD *a3, __int64 a4, int a5)
{
  __int64 result; // rax
  __int64 v10; // rcx
  unsigned int *v11; // r8
  unsigned __int64 v12; // rdx
  __int64 v13; // r9
  __int64 v14; // r9
  __int64 v15; // r10
  _QWORD *v16; // r11
  int v17; // eax
  __int64 v18; // rcx
  int v19; // ebp
  _QWORD v20[3]; // [rsp+50h] [rbp-18h] BYREF
  void *retaddr; // [rsp+68h] [rbp+0h]

  result = PssNtValidateDescriptor(a1, retaddr);
  if ( (int)result < 0 )
    return result;
  if ( a2 != 1 )
  {
    if ( a2 )
    {
      v19 = a2 - 2;
      if ( v19 )
      {
        if ( v19 != 1 )
          return 3221225475LL;
        if ( !a4 || a5 == 144 )
          return PsspWalkInfoClass_PSS_WALK_THREADS(a1, a3, a4);
      }
      else if ( !a4 || a5 == 72 )
      {
        return PsspWalkInfoClass_PSS_WALK_HANDLES(a1, a3, a4);
      }
    }
    else if ( !a4 || a5 == 80 )
    {
      return PsspWalkInfoClass_PSS_WALK_AUXILIARY_PAGES(a1, a3, a4);
    }
    return 3221225476LL;
  }
  if ( a4 && a5 != 80 )
    return 3221225476LL;
  v10 = *(_QWORD *)(a1 + 920);
  v20[0] = 0LL;
  if ( !v10 )
    return 3221226021LL;
  if ( !a3 )
    return 3221225485LL;
  if ( *a3 )
  {
    v11 = (unsigned int *)(a3 + 1);
  }
  else
  {
    result = ((__int64 (__fastcall *)(__int64, __int64, _QWORD *, _QWORD, _QWORD, _QWORD, _QWORD *, int, _DWORD, int))ZwMapViewOfSection)(
               v10,
               -1LL,
               a3,
               0LL,
               0LL,
               0LL,
               v20,
               1,
               0,
               2);
    if ( (int)result < 0 )
      return result;
    v11 = (unsigned int *)(a3 + 1);
    a3[1] = 0LL;
  }
  v12 = *v11;
  if ( v12 >= *(unsigned int *)(a1 + 916) )
    return 2147483674LL;
  if ( !a4 )
    return 261LL;
  v13 = *a3;
  *(_OWORD *)a4 = 0LL;
  v14 = v12 + v13;
  v15 = 72LL;
  *(_OWORD *)(a4 + 16) = 0LL;
  v16 = v11;
  *(_OWORD *)(a4 + 32) = 0LL;
  *(_OWORD *)(a4 + 48) = 0LL;
  *(_OWORD *)(a4 + 64) = 0LL;
  *(_QWORD *)a4 = *(_QWORD *)v14;
  *(_QWORD *)(a4 + 8) = *(_QWORD *)(v14 + 8);
  *(_DWORD *)(a4 + 16) = *(_DWORD *)(v14 + 16);
  *(_QWORD *)(a4 + 24) = *(_QWORD *)(v14 + 24);
  *(_DWORD *)(a4 + 32) = *(_DWORD *)(v14 + 32);
  *(_DWORD *)(a4 + 36) = *(_DWORD *)(v14 + 36);
  *(_DWORD *)(a4 + 40) = *(_DWORD *)(v14 + 40);
  *(_DWORD *)(a4 + 44) = *(_DWORD *)(v14 + 48);
  *(_DWORD *)(a4 + 48) = *(_DWORD *)(v14 + 52);
  *(_QWORD *)(a4 + 56) = *(_QWORD *)(v14 + 56);
  *(_DWORD *)(a4 + 64) = *(_DWORD *)(v14 + 64);
  if ( (*(_DWORD *)(a1 + 8) & 0x1000) != 0 && v12 + 76 <= *(unsigned int *)(a1 + 916) )
  {
    v17 = *(_DWORD *)(v14 + 40);
    if ( v17 == 0x1000000 || v17 == 0x40000 )
    {
      v18 = *(unsigned __int16 *)(v14 + 72);
      *(_WORD *)(a4 + 68) = v18;
      if ( (_WORD)v18 )
        *(_QWORD *)(a4 + 72) = v14 + 74;
      v15 = v18 + 74;
      v16 = a3 + 1;
    }
  }
  *v16 = v12 + ((v15 + 7) & 0xFFFFFFF8LL);
  return 0LL;
}
