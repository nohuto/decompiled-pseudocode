/*
 * XREFs of LdrpResolveProcedureAddress @ 0x180057F30
 * Callers:
 *     LdrGetProcedureAddressForCaller @ 0x180004FF0 (LdrGetProcedureAddressForCaller.c)
 *     LdrpHandleProtectedDelayload @ 0x1800096B0 (LdrpHandleProtectedDelayload.c)
 *     LdrpResolveDelayloadAddress @ 0x1800DE4C0 (LdrpResolveDelayloadAddress.c)
 * Callees:
 *     LdrpReleaseTlsEntry @ 0x1800049CC (LdrpReleaseTlsEntry.c)
 *     LdrpDestroyNode @ 0x180004B28 (LdrpDestroyNode.c)
 *     LdrpUnmapModule @ 0x1800067C8 (LdrpUnmapModule.c)
 *     LdrpLoadForwardedDll @ 0x18000B980 (LdrpLoadForwardedDll.c)
 *     LdrpLogInternal @ 0x180013D80 (LdrpLogInternal.c)
 *     RtlpxLookupFunctionTable @ 0x1800166E0 (RtlpxLookupFunctionTable.c)
 *     RtlFreeHeap @ 0x1800269F0 (RtlFreeHeap.c)
 *     RtlAcquireSRWLockExclusive @ 0x180055AE0 (RtlAcquireSRWLockExclusive.c)
 *     RtlReleaseSRWLockExclusive @ 0x1800567B0 (RtlReleaseSRWLockExclusive.c)
 *     RtlImageNtHeaderEx @ 0x1800590F0 (RtlImageNtHeaderEx.c)
 *     RtlReleaseActivationContext @ 0x18005E4D0 (RtlReleaseActivationContext.c)
 *     RtlCharToInteger @ 0x1800647F0 (RtlCharToInteger.c)
 *     RtlAddressInSectionTable @ 0x1800649C0 (RtlAddressInSectionTable.c)
 *     LdrpCheckRedirection @ 0x180072128 (LdrpCheckRedirection.c)
 *     LdrpFreeUnicodeString @ 0x18007625C (LdrpFreeUnicodeString.c)
 *     RtlGuardCheckImageBase @ 0x1800D8A50 (RtlGuardCheckImageBase.c)
 *     strrchr @ 0x180126950 (strrchr.c)
 *     __security_check_cookie @ 0x1801659C0 (__security_check_cookie.c)
 *     _guard_dispatch_icall$thunk$10345483385596137414 @ 0x180172020 (_guard_dispatch_icall$thunk$10345483385596137414.c)
 */

__int64 __fastcall LdrpResolveProcedureAddress(__int64 a1, __int64 a2, char *a3, ULONG a4, char a5, __int64 *a6)
{
  __int64 *v6; // r14
  unsigned __int64 v7; // rax
  __int64 v8; // rbx
  char v10; // r12
  int v11; // r13d
  bool v12; // si
  unsigned __int64 v13; // r14
  unsigned __int64 v14; // rdi
  __int64 v15; // rdx
  unsigned __int64 i; // r8
  __int16 v17; // ax
  __int64 v18; // rax
  int v19; // eax
  char *v20; // rdi
  int v21; // r11d
  int v22; // r9d
  int j; // eax
  int v24; // eax
  int v25; // r10d
  char *v26; // rcx
  unsigned __int8 v27; // al
  int v28; // eax
  signed int v29; // ebx
  char *v30; // rbx
  int v31; // edi
  unsigned __int64 v32; // rbx
  unsigned __int64 v35; // rbx
  __int64 v36; // r10
  __int64 v37; // r9
  __int64 v38; // rax
  ULONG v39; // ebx
  unsigned int v40; // eax
  char *v41; // rax
  char *v42; // r15
  unsigned __int64 v43; // rax
  __int64 v44; // rbx
  __int64 v45; // rax
  __int64 v46; // rdx
  _QWORD *v47; // rcx
  char v48; // al
  _QWORD *v49; // rdi
  _QWORD *v50; // rsi
  __int64 v51; // rdx
  __int64 v52; // rcx
  __int64 v53; // [rsp+40h] [rbp-C0h] BYREF
  unsigned int v54; // [rsp+48h] [rbp-B8h]
  ULONG Value; // [rsp+4Ch] [rbp-B4h] BYREF
  char **v56; // [rsp+50h] [rbp-B0h]
  __int64 v57; // [rsp+58h] [rbp-A8h] BYREF
  __int64 v58; // [rsp+60h] [rbp-A0h]
  STRING SourceString; // [rsp+68h] [rbp-98h] BYREF
  __int64 v60; // [rsp+80h] [rbp-80h]
  __int128 v61; // [rsp+88h] [rbp-78h] BYREF
  __int64 v62; // [rsp+98h] [rbp-68h]
  __int128 v63; // [rsp+A0h] [rbp-60h] BYREF
  __int128 v64; // [rsp+B0h] [rbp-50h]
  __int128 v65; // [rsp+C0h] [rbp-40h]
  __int128 v66; // [rsp+D0h] [rbp-30h]
  __int128 v67; // [rsp+E0h] [rbp-20h]
  __int128 v68; // [rsp+F0h] [rbp-10h]
  __int128 v69; // [rsp+100h] [rbp+0h]
  __int128 v70; // [rsp+110h] [rbp+10h]

  v6 = a6;
  v7 = *(_QWORD *)(a2 + 80);
  v8 = 0LL;
  v60 = a2;
  v58 = a1;
  v56 = (char **)a6;
  Value = a4;
  v53 = a2;
  v54 = 0;
  v65 = v7;
  SourceString = 0LL;
  v63 = 0LL;
  v64 = 0LL;
  v66 = 0LL;
  v67 = 0LL;
  v68 = 0LL;
  v69 = 0LL;
  v70 = 0LL;
  if ( a1 && LdrpRedirectionModule && LdrpRedirectionModule != a1 )
  {
    if ( (NtCurrentPeb()->BitField & 0x10) != 0 )
    {
      v10 = *(_BYTE *)(a1 + 104) & 1;
    }
    else if ( LdrpRedirectionCalloutFunc )
    {
      v48 = LdrpRedirectionCalloutFunc(*(_QWORD *)(a1 + 80));
      a1 = v58;
      v10 = v48;
    }
    else
    {
      v10 = 1;
    }
  }
  else
  {
    v10 = 0;
  }
  while ( 1 )
  {
    if ( v10 )
    {
      if ( a3 )
      {
        v38 = LdrpCheckRedirection(a1, v53, a3);
        if ( v38 != -4530927 )
        {
          *v6 = v38;
          return 0LL;
        }
      }
    }
    v11 = 0;
    v12 = 1;
    v57 = 0LL;
    v13 = *(_QWORD *)(v53 + 48);
    v14 = v13;
    if ( (v13 & 3) != 0 )
    {
      v14 = v13 & 0xFFFFFFFFFFFFFFFCuLL;
      v12 = (v13 & 1) == 0;
    }
    RtlImageNtHeaderEx(1LL, v14, 0LL, &v57);
    if ( !v57 )
      goto LABEL_35;
    v17 = *(_WORD *)(v57 + 24);
    v15 = 267LL;
    if ( v17 == 267 )
    {
      if ( !*(_DWORD *)(v57 + 116) )
        goto LABEL_66;
      v36 = *(unsigned int *)(v57 + 120);
      if ( (_DWORD)v36 )
      {
        v11 = *(_DWORD *)(v57 + 124);
        if ( v12 || (unsigned int)v36 < *(_DWORD *)(v57 + 84) )
        {
          v8 = v14 + v36;
          v19 = 0;
        }
        else
        {
          v15 = *(unsigned __int16 *)(v57 + 20) + v57 + 24;
          for ( i = 0LL; ; i = (unsigned int)(i + 1) )
          {
            if ( (unsigned int)i >= *(unsigned __int16 *)(v57 + 6) )
            {
              v8 = 0LL;
              goto LABEL_66;
            }
            v37 = *(unsigned int *)(v15 + 12);
            if ( (unsigned int)v36 >= (unsigned int)v37 && (unsigned int)v36 < (int)v37 + *(_DWORD *)(v15 + 16) )
              break;
            v15 += 40LL;
          }
          v8 = v14 + *(unsigned int *)(v15 + 20) - v37 + v36;
          if ( !v8 )
            goto LABEL_66;
          v19 = 0;
        }
      }
      else
      {
        v19 = -1073741822;
      }
    }
    else
    {
      v15 = 523LL;
      if ( v17 != 523 || !*(_DWORD *)(v57 + 132) )
      {
LABEL_66:
        v19 = -1073741811;
        goto LABEL_15;
      }
      v18 = *(unsigned int *)(v57 + 136);
      if ( (_DWORD)v18 )
      {
        v11 = *(_DWORD *)(v57 + 140);
        if ( v12 || (unsigned int)v18 < *(_DWORD *)(v57 + 84) )
        {
          v8 = v14 + v18;
          v19 = 0;
        }
        else
        {
          v8 = RtlAddressInSectionTable(v57, v14, (unsigned int)v18);
          v19 = 0;
          if ( !v8 )
            v19 = -1073741811;
        }
      }
      else
      {
        v19 = -1073741822;
      }
    }
LABEL_15:
    v20 = 0LL;
    if ( v19 >= 0 )
      v20 = (char *)v8;
    if ( !v20 )
      goto LABEL_35;
    if ( a3 )
    {
      LdrpLogInternal(
        (__int64)"minkernel\\ldr\\ldrsnap.c",
        838,
        (__int64)"LdrpGetProcedureAddress",
        2,
        "Locating procedure \"%s\" by name\n",
        (char)a3);
      v21 = 0;
      v22 = *((_DWORD *)v20 + 6) - 1;
      for ( j = v22; ; j = v21 + v22 )
      {
        v24 = j / 2;
        v25 = v24;
        if ( v22 < v21 )
          break;
        i = v24;
        v26 = a3;
        v15 = v13 + *(unsigned int *)(v13 + *((unsigned int *)v20 + 8) + 4LL * v24) - (_QWORD)a3;
        while ( 1 )
        {
          v27 = *v26;
          if ( *v26 != v26[v15] )
            break;
          ++v26;
          if ( !v27 )
          {
            v28 = 0;
            goto LABEL_25;
          }
        }
        v28 = v27 < (unsigned __int8)v26[v15] ? -1 : 1;
LABEL_25:
        if ( !v28 )
        {
          v29 = *(unsigned __int16 *)(v13 + *((unsigned int *)v20 + 9) + 2 * i);
          goto LABEL_27;
        }
        if ( v28 >= 0 )
          v21 = v25 + 1;
        else
          v22 = v25 - 1;
      }
      LdrpLogInternal(
        (__int64)"minkernel\\ldr\\ldrsnap.c",
        2286,
        (__int64)"LdrpNameToOrdinal",
        1,
        "Procedure \"%s\" could not be located in DLL at base 0x%p.\n",
        (char)a3);
LABEL_35:
      v31 = -1073741702;
      goto LABEL_36;
    }
    v39 = Value;
    LdrpLogInternal(
      (__int64)"minkernel\\ldr\\ldrsnap.c",
      856,
      (__int64)"LdrpGetProcedureAddress",
      2,
      "Loading procedure 0x%lx by ordinal\n",
      Value);
    if ( !v39 )
    {
      v31 = -1073741811;
      goto LABEL_36;
    }
    v29 = v39 - *((_DWORD *)v20 + 4);
LABEL_27:
    if ( (unsigned int)v29 >= *((_DWORD *)v20 + 5) )
      break;
    v15 = v29;
    v30 = (char *)(v13 + *(unsigned int *)(v13 + *((unsigned int *)v20 + 7) + 4LL * v29));
    v6 = (__int64 *)v56;
    *v56 = v30;
    if ( v30 < v20 || v30 >= &v20[v11] )
    {
      v31 = 0;
      goto LABEL_37;
    }
    v40 = v54++;
    if ( v40 >= 0x20 || (a5 & 2) != 0 || (v41 = strrchr(v30, 46), (v42 = v41) == 0LL) || (v43 = v41 - v30, v43 > 0xFFFF) )
    {
LABEL_95:
      v31 = -1073741701;
      goto LABEL_38;
    }
    a3 = v42 + 1;
    SourceString.Buffer = v30;
    SourceString.Length = v43;
    SourceString.MaximumLength = v43;
    if ( *a3 == 35 )
    {
      if ( RtlCharToInteger(a3 + 1, 0, &Value) < 0 )
        goto LABEL_95;
      a3 = 0LL;
    }
    DWORD2(v64) = *(_DWORD *)(v53 + 272);
    v31 = LdrpLoadForwardedDll(&SourceString, (int)&v63, v60, v53, 2, &v53);
    if ( v31 < 0 )
      goto LABEL_38;
    v44 = v53;
    v45 = *(_QWORD *)(v53 + 152);
    if ( *(_DWORD *)(v45 + 24) != -1
      && (*(_BYTE *)(*(_QWORD *)v45 - 56LL) & 0x20) == 0
      && _InterlockedExchangeAdd((volatile signed __int32 *)(v53 + 276), 0xFFFFFFFF) == 1 )
    {
      RtlAcquireSRWLockExclusive(
        (volatile signed __int32 *)&LdrpModuleDatatableLock,
        (volatile signed __int32 **)v15,
        i);
      v46 = *(_QWORD *)(v44 + 160);
      if ( *(_QWORD *)(v46 + 8) != v44 + 160 || (v47 = *(_QWORD **)(v44 + 168), *v47 != v44 + 160) )
        __fastfail(3u);
      *v47 = v46;
      *(_QWORD *)(v46 + 8) = v47;
      v49 = *(_QWORD **)(v44 + 152);
      v50 = (_QWORD *)*v49;
      RtlReleaseSRWLockExclusive(&LdrpModuleDatatableLock);
      if ( *(_WORD *)(v44 + 110) )
        LdrpReleaseTlsEntry(v44, 0LL);
      LdrpUnmapModule(v44, v51);
      v52 = *(_QWORD *)(v44 + 136);
      if ( (unsigned __int64)(v52 - 1) <= 0xFFFFFFFFFFFFFFFDuLL )
        RtlReleaseActivationContext(v52);
      if ( *(_QWORD *)(v44 + 80) )
        LdrpFreeUnicodeString(v44 + 72);
      RtlFreeHeap(LdrpHeap, 0, v44);
      if ( v50 == v49 )
        LdrpDestroyNode((__int64)v49);
    }
    a1 = v58;
    v8 = 0LL;
  }
  v31 = (a3 != 0LL) - 1073741512;
LABEL_36:
  v6 = (__int64 *)v56;
LABEL_37:
  if ( (a5 & 1) != 0 && v31 >= 0 )
  {
    if ( (a5 & 2) != 0 )
    {
      LOBYTE(v15) = 1;
      RtlGuardCheckImageBase(*(_QWORD *)(v53 + 184), v15);
    }
    else
    {
      v35 = *(_QWORD *)(v53 + 48);
      if ( qword_1801EA508 && (dword_1801EA4EC & 1) == 0 )
      {
        v62 = 0LL;
        v61 = 0LL;
        if ( v35 < *((_QWORD *)&xmmword_1801E7440 + 1)
          || v35 >= *((_QWORD *)&xmmword_1801E7440 + 1) + (unsigned __int64)(unsigned int)qword_1801E7450 )
        {
          RtlpxLookupFunctionTable(v35, (__int64)&v61);
        }
        else
        {
          v61 = xmmword_1801E7440;
        }
        if ( *((_QWORD *)&v61 + 1) != v35 )
          __fastfail(0x18u);
      }
    }
  }
LABEL_38:
  if ( BYTE12(v70) )
  {
    v32 = v63 - 128;
    RtlAcquireSRWLockExclusive((volatile signed __int32 *)&RtlpCachedPathLock, (volatile signed __int32 **)v15, i);
    if ( (*(_QWORD *)(v32 + 80))-- != 1LL )
      v32 = 0LL;
    RtlReleaseSRWLockExclusive(&RtlpCachedPathLock);
    if ( v32 )
      RtlFreeHeap((__int64)NtCurrentPeb()->ProcessHeap, 0, v32);
  }
  if ( v31 < 0 )
    *v6 = 0LL;
  return (unsigned int)v31;
}
