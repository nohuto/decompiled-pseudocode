/*
 * XREFs of RtlCompareUnicodeStrings @ 0x18008EE30
 * Callers:
 *     RtlpCapabilityCheckSystemCapability @ 0x18001C5C8 (RtlpCapabilityCheckSystemCapability.c)
 *     RtlpFindUnicodeStringInSection @ 0x180073BF0 (RtlpFindUnicodeStringInSection.c)
 *     LdrpSnapKernelBaseExtensions @ 0x18008D200 (LdrpSnapKernelBaseExtensions.c)
 *     RtlDeleteElementGenericTableAvl @ 0x18008E420 (RtlDeleteElementGenericTableAvl.c)
 *     RtlInsertElementGenericTableAvl @ 0x18008E510 (RtlInsertElementGenericTableAvl.c)
 *     FindNodeOrParent @ 0x18008E730 (FindNodeOrParent.c)
 *     RtlLookupElementGenericTableFullAvl @ 0x18008E820 (RtlLookupElementGenericTableFullAvl.c)
 *     RtlLookupElementGenericTableAvl @ 0x18008E920 (RtlLookupElementGenericTableAvl.c)
 *     LdrpCheckRedirection @ 0x18008EA08 (LdrpCheckRedirection.c)
 *     LdrpCheckForRetryLoading @ 0x18008EC2C (LdrpCheckForRetryLoading.c)
 *     LdrpCompareModuleName @ 0x18008F2A4 (LdrpCompareModuleName.c)
 *     LdrpSearchPath @ 0x180091DD0 (LdrpSearchPath.c)
 *     LdrpQuerySxSMUIFile @ 0x18009A09C (LdrpQuerySxSMUIFile.c)
 *     RtlpMatchUserLanguage @ 0x18009A410 (RtlpMatchUserLanguage.c)
 *     RtlpMatchUILanguage @ 0x18009A584 (RtlpMatchUILanguage.c)
 *     LdrpInitializeApplicationVerifierPackage @ 0x1800A523C (LdrpInitializeApplicationVerifierPackage.c)
 *     RtlpProcessIFEOKeyFilter @ 0x1800A5D14 (RtlpProcessIFEOKeyFilter.c)
 *     LdrpCompareRedirectedFunction @ 0x1800EAF40 (LdrpCompareRedirectedFunction.c)
 *     LdrpIsModuleUnderSystem32 @ 0x1800EFFE4 (LdrpIsModuleUnderSystem32.c)
 *     ApiSetQuerySchemaInfo @ 0x1801140BC (ApiSetQuerySchemaInfo.c)
 *     ApiSetQuerySchemaInfo_V7 @ 0x180133B80 (ApiSetQuerySchemaInfo_V7.c)
 *     ApiSetpGetExtensionNameKeyInfo @ 0x180134528 (ApiSetpGetExtensionNameKeyInfo.c)
 *     ApiSetpSearchForHostOverrideIndex_V7 @ 0x1801347F4 (ApiSetpSearchForHostOverrideIndex_V7.c)
 *     RtlpIsEmptyImageFileOptionsKey @ 0x1801362A8 (RtlpIsEmptyImageFileOptionsKey.c)
 *     _RtlpMuiRegInitLIPLanguage @ 0x180148660 (_RtlpMuiRegInitLIPLanguage.c)
 *     _RtlpMuiRegPopulateBaseLanguages @ 0x180148B20 (_RtlpMuiRegPopulateBaseLanguages.c)
 *     LdrpIsSubstringFound @ 0x18015D258 (LdrpIsSubstringFound.c)
 * Callees:
 *     RtlCompareMemory @ 0x1801642D0 (RtlCompareMemory.c)
 */

LONG __cdecl RtlCompareUnicodeStrings(
        PCWCH String1,
        SIZE_T String1Length,
        PCWCH String2,
        SIZE_T String2Length,
        BOOLEAN CaseInSensitive)
{
  SIZE_T v5; // rsi
  int v7; // r14d
  int v8; // ebp
  PCWCH v9; // rbx
  signed __int64 v10; // rdi
  unsigned __int64 v11; // r8
  unsigned __int64 v12; // r9
  SIZE_T v13; // rcx

  v5 = String1Length;
  if ( String1Length > String2Length )
    v5 = String2Length;
  v7 = String2Length;
  v8 = String1Length;
  v9 = String1;
  if ( CaseInSensitive )
  {
    v10 = (char *)String2 - (char *)String1;
    while ( v9 < &String1[v5] )
    {
      v11 = *v9;
      v12 = *(PCWCH)((char *)v9 + v10);
      if ( (_WORD)v11 != (_WORD)v12 )
      {
        if ( (unsigned int)v11 >= 0x61 )
        {
          if ( (unsigned int)v11 > 0x7A )
          {
            if ( qword_1801CC038 && (unsigned __int16)v11 >= 0xC0u )
              LOWORD(v11) = *(_WORD *)(qword_1801CC038
                                     + 2
                                     * ((v11 & 0xF)
                                      + *(unsigned __int16 *)(qword_1801CC038
                                                            + 2LL
                                                            * (((unsigned __int8)v11 >> 4)
                                                             + (unsigned int)*(unsigned __int16 *)(qword_1801CC038
                                                                                                 + 2 * (v11 >> 8))))))
                          + v11;
          }
          else
          {
            LOWORD(v11) = v11 - 32;
          }
        }
        if ( (unsigned int)v12 >= 0x61 )
        {
          if ( (unsigned int)v12 > 0x7A )
          {
            if ( qword_1801CC038 )
            {
              if ( (unsigned __int16)v12 >= 0xC0u )
                LOWORD(v12) = *(_WORD *)(qword_1801CC038
                                       + 2
                                       * ((v12 & 0xF)
                                        + *(unsigned __int16 *)(qword_1801CC038
                                                              + 2LL
                                                              * (((unsigned __int8)v12 >> 4)
                                                               + (unsigned int)*(unsigned __int16 *)(qword_1801CC038 + 2 * (v12 >> 8))))))
                            + v12;
            }
          }
          else
          {
            LOWORD(v12) = v12 - 32;
          }
        }
        if ( (_WORD)v11 != (_WORD)v12 )
          return (unsigned __int16)v11 - (unsigned __int16)v12;
      }
      ++v9;
    }
    return v8 - v7;
  }
  v13 = RtlCompareMemory(String1, String2, 2 * v5) >> 1;
  if ( v13 >= v5 )
    return v8 - v7;
  return v9[v13] - String2[v13];
}
