/*
 * XREFs of ?EndInitialization@CAudioSrv@@UEAAJJ@Z @ 0x1800CC510
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall CAudioSrv::EndInitialization(CAudioSrv *this, signed int a2)
{
  ...
  if ( !a2 )
  {
    ...
    if ( !LastError )
    {
      LastError = StartRpcServer(v5, L"Audiosrv", &unk_180182A80, SecurityDescriptor);
      *((_DWORD *)this + 14) = LastError == 0;
      if ( !LastError )
      {
        LastError = StartRpcServer(v6, L"PlaybackManagerRpc", &unk_180189570, 0LL);
        *((_DWORD *)this + 15) = LastError == 0;
        if ( !LastError )
        {
          LastError = StartRpcServer(v7, L"AudioSrvDiagnosticsRpc", &unk_180189830, 0LL);
          *((_DWORD *)this + 16) = LastError == 0;
          if ( !LastError )
          {
            LastError = StartRpcServer(v8, L"SpatialSoundDataManagerRpc", &unk_180189B20, SecurityDescriptor);
