/*
 * XREFs of s_pbmPlayToStreamStateChanged @ 0x180104890
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall s_pbmPlayToStreamStateChanged(__int64 a1, unsigned int a2)
{
  ...
  if ( wil::details::FeatureImpl<__WilFeatureTraits_Feature_Servicing_EnableLKDForAudio>::__private_IsEnabled((wil::details *)&`wil::Feature<__WilFeatureTraits_Feature_Servicing_EnableLKDForAudio>::GetImpl'::`2'::impl) )
  {
    ...
    std::unique_ptr<CWatchdogTimer<1>>::reset(&v17, v4);
  }
  else
  {
    ...
    std::unique_ptr<CWatchdogTimer_Old<1>>::reset(&v16, v9);
  }
  if ( (unsigned int)IsPbmPlayToStreamStateChangedSupported() )
    v14 = PbmPlayToStreamStateChanged(a1, a2);
  else
    v14 = 0;
