/*
 * XREFs of ?ForEachEntry@?$CLockedList_UniqueValuesOnly@VCAudioSessionNotificationProcess@@$0A@@@QEAAJ$$QEAVCListWorker@1@@Z @ 0x18000C4D8
 * Callers:
  ...
  try
  {
    ...
    if ( v6 )
    {
      std::_Destroy_range<std::allocator<CLockedList_UniqueValuesOnly<CAudioSessionNotificationProcess,0>::LockedListEntry>>(
        v6,
        *((_QWORD *)&v11 + 1));
      std::_Deallocate<16,0>(v6, (v4 - v6) & 0xFFFFFFFFFFFFFFF0uLL);
    }
    result = 0LL;
  }
  catch ( ... )
