/*
 * XREFs of TelpReadOfflineOsPolicySetting @ 0x180004C98
 * Callers:
 *     TelGetWerTelemetryModeWinRE @ 0x180004440 (TelGetWerTelemetryModeWinRE.c)
 *     TelIsTelemetryTypeAllowedWinRE @ 0x1800047D0 (TelIsTelemetryTypeAllowedWinRE.c)
 * Callees:
 *     __security_check_cookie @ 0x180001460 (__security_check_cookie.c)
 *     ?Return_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z @ 0x18000326C (-Return_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z.c)
 *     TelpReadOfflineSoftwareRegistryDword @ 0x180004E38 (TelpReadOfflineSoftwareRegistryDword.c)
 *     TelpReadOfflineSoftwareRegistryString @ 0x180004FF4 (TelpReadOfflineSoftwareRegistryString.c)
 */

__int64 __fastcall TelpReadOfflineOsPolicySetting(_DWORD *a1, __int64 a2, __int64 a3)
{
  int v3; // edi
  signed int v5; // ebx
  __int64 v6; // rdx
  int v7; // esi
  LSTATUS v8; // eax
  __int64 v9; // rdx
  __int64 v10; // rcx
  int v11; // eax
  int v12; // ecx
  int pvData; // [rsp+30h] [rbp-30h] BYREF
  HKEY hKey; // [rsp+38h] [rbp-28h] BYREF
  __int128 v16; // [rsp+40h] [rbp-20h] BYREF
  int v17; // [rsp+50h] [rbp-10h]
  wil::details::in1diag3 *retaddr; // [rsp+78h] [rbp+18h]

  v3 = 0;
  if ( !a1 )
  {
    v5 = -2147024809;
    v6 = 1864LL;
LABEL_26:
    wil::details::in1diag3::Return_Hr(retaddr, (void *)v6, a3, (const char *)(unsigned int)v5);
    return (unsigned int)v5;
  }
  pvData = 0;
  hKey = 0LL;
  v7 = 0;
  v8 = RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\RecoveryEnvironment", 0, 0x20019u, &hKey);
  v5 = v8;
  if ( v8 > 0 )
    v5 = (unsigned __int16)v8 | 0x80070000;
  if ( v5 < 0 )
  {
    *a1 = 0;
    if ( (unsigned int)(v5 + 2147024894) <= 1 )
      v5 = -2147024809;
    goto LABEL_25;
  }
  RegCloseKey(hKey);
  v5 = TelpReadOfflineSoftwareRegistryDword(
         L"OFFLINE_AUTH\\Microsoft\\Windows\\CurrentVersion\\Diagnostics\\DiagTrack",
         L"DiagTrackAuthorization",
         &pvData);
  if ( v5 >= 0 )
  {
    if ( (pvData & 8) != 0 )
      v10 = 3LL;
    else
      v10 = (pvData & 4) != 0;
    v7 = (pvData & 0x1000) != 0 ? v10 : 0;
  }
  if ( !v7 )
  {
LABEL_21:
    *a1 = v7;
    if ( v5 >= 0 )
      return (unsigned int)v5;
LABEL_25:
    v6 = 1947LL;
    goto LABEL_26;
  }
  LODWORD(hKey) = 10;
  v17 = 0;
  v16 = 0LL;
  v11 = TelpReadOfflineSoftwareRegistryString(v10, v9, &v16, &hKey);
  v5 = v11;
  if ( v11 < 0 )
  {
    if ( (unsigned int)(v11 + 2147024894) <= 1 )
      v5 = 0;
    goto LABEL_21;
  }
  v12 = (unsigned __int16)v16 - 122;
  if ( (unsigned __int16)v16 == 122 )
  {
    v12 = WORD1(v16) - 104;
    if ( WORD1(v16) == 104 )
      v12 = WORD2(v16);
  }
  if ( v12 )
    v3 = v7;
  *a1 = v3;
  return (unsigned int)v5;
}
