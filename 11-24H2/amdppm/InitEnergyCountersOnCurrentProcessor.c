  ...
  if ( (unsigned __int8)IsRaplSupported() && (unsigned __int8)IsEnergyMsrSupported() )
  {
    ...
    if ( inited >= 0 )
    {
      ...
      if ( !Number )
      {
        ...
        HIBYTE(word_140015EC0) = 1;
        LOBYTE(word_140015EC0) = BYTE1(v7) & 0x1F;
        WPP_MAIN_CB.Queue.Wcb.WaitQueueEntry.SortKey = BYTE1(v7) & 0x1F;
        if ( v5 )
        {
          qword_140015DB8[0] = (__int64)SnapEnergyCountersAndTimestamp;
          v8 = ComputeHyperThreadedProcessorEnergyUsingMsr;
        }
        else
        {
          qword_140015DB8[0] = (__int64)SnapEnergyCounters;
          v8 = ComputeProcessorEnergyUsingMsr;
        }
        qword_140015DB0[0] = (__int64)v8;
      }
    }
  }
  else
