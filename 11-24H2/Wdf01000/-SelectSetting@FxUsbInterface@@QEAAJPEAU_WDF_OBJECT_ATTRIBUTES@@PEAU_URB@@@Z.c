/*
 * XREFs of ?SelectSetting@FxUsbInterface@@QEAAJPEAU_WDF_OBJECT_ATTRIBUTES@@PEAU_URB@@@Z @ 0x14004F8DC
 * Callers:
        ...
        _URB *Urb)
{
  ...
  v6 = 0;
  if ( !this->m_ConfiguredPipes || this->m_CurAlternateSetting != Urb->UrbSelectInterface.Interface.AlternateSetting )
  {
    request.__vftable = (FxSyncRequest_vtbl *)&pendHead.Blink;
    pendHead.Blink = (_LIST_ENTRY *)&pendHead.Blink;
    _a2 = Urb->UrbHeader.Length;
    if ( (unsigned __int16)_a2 >= 0x18u )
    {
      ...
      if ( v8 >= 0 )
      {
        ...
        LODWORD(v9) = ((unsigned int)Urb->UrbSelectInterface.Interface.Length - 24) / 0x18;
        if ( (_BYTE)v9 )
