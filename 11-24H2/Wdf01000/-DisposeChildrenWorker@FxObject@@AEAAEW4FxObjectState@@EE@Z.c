/*
 * XREFs of ?DisposeChildrenWorker@FxObject@@AEAAEW4FxObjectState@@EE@Z @ 0x14004AED0
 * Callers:
...
LABEL_38:
      FxObject::SetObjectStateLocked(this, (FxObjectState)NewDeferedState);
  }
  else
  {
        ...
        for ( j = _a1->m_ChildListHead.Flink; j != p_m_ChildListHead; j = j->Flink )
        {
          if ( !FxObject::PerformEarlyDispose((FxObject *)&j[-5].Blink) )
          {
            ...
            if ( CanDefer )
              ...
              FxObject::SetObjectStateLocked(_a1, (FxObjectState)NewDeferedState);
    ...
    FxDisposeList::Add(m_DisposeList, _a1);
  }
  KeReleaseSpinLock(&_a1->m_SpinLock.m_Lock, OldIrql);
  return 0;
}
