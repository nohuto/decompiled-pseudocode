/*
 * XREFs of ?InitializeWorker@FxInterrupt@@QEAAJPEAVFxObject@@PEAU_WDF_INTERRUPT_CONFIG@@@Z @ 0x1400A9770
 * Callers:
  ...
  if ( (int)result >= 0 )
  {
    ...
    if ( this->m_PassiveHandling )
    {
      ...
      if ( !this->m_WaitLock )
      {
        ...
        v18 = FxWaitLock::_Create(m_Globals, &attributes, 0LL, 0LL, &waitLock);
        v19 = v18;
        if ( v18 < 0 )
        {
    ...
    if ( !this->m_EvtInterruptWorkItem && (!unk_1400C8090 || !this->m_EvtInterruptDpc) )
      return 0LL;
