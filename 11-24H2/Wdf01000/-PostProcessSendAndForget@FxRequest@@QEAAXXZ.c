/*
 * XREFs of ?PostProcessSendAndForget@FxRequest@@QEAAXXZ @ 0x140008EFC
 * Callers:
  ...
  if ( v4 )
  {
    if ( m_CompletionState )
    {
      if ( (m_CompletionState & 0x80u) == 0 )
      {
        ...
        {
          FxVerifierLock::Lock(v14, &PreviousIrql, a3);
        }
        else
        {
        ...
        if ( m_IoQueue->m_Dispatching )
        {
          ...
          {
            FxVerifierLock::Unlock(v15, PreviousIrql, v8);
          }
          else
          {
    ...
    else
    {
      ...
      {
        v13 = this[-1].m_OwnerListEntry2.Blink;
        if ( v13 )
