/*
 * XREFs of ?FxLibraryCreateDevice@@YAJPEAU_UNICODE_STRING@@@Z @ 0x14008535C
 * Callers:
...
int __fastcall FxLibraryCreateDevice(_UNICODE_STRING *DeviceName)
{
  ...
  while ( 1 )
  {
    ...
    Device = IoCreateDevice(
               (PDRIVER_OBJECT)WPP_GLOBAL_WDF_Control.Reserved,
               0,
               DeviceName,
               0x22u,
               0,
               0,
               (PDEVICE_OBJECT *)&WPP_GLOBAL_WDF_Control.Reserved + 1);
    if ( Device != -1073741771 )
    {
      if ( Device >= 0 )
        *(_DWORD *)(*((_QWORD *)&WPP_GLOBAL_WDF_Control.Reserved + 1) + 48LL) &= ~0x80u;
