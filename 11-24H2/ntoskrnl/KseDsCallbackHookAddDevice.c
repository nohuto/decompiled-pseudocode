/*
 * XREFs of KseDsCallbackHookAddDevice @ 0x1405CA980
 * Callers:
 *     <none>
 * Callees:
 *     RtlInitUnicodeString @ 0x1404241A0 (RtlInitUnicodeString.c)
 *     KsepDsEventAddDevice @ 0x1405CB128 (KsepDsEventAddDevice.c)
 *     __security_check_cookie @ 0x1406A5920 (__security_check_cookie.c)
 *     _guard_dispatch_icall_no_overrides @ 0x1406B3DF0 (_guard_dispatch_icall_no_overrides.c)
 *     IoGetDeviceProperty @ 0x1408BCAC0 (IoGetDeviceProperty.c)
 */

__int64 __fastcall KseDsCallbackHookAddDevice(__int64 a1, struct _DEVICE_OBJECT *a2, __int64 a3, __int64 a4)
{
  __int64 v6; // r8
  __int64 v7; // r9
  unsigned int v8; // ebx
  NTSTATUS DeviceProperty; // eax
  const WCHAR *v10; // rdx
  ULONG ResultLength; // [rsp+40h] [rbp-438h] BYREF
  UNICODE_STRING DestinationString; // [rsp+48h] [rbp-430h] BYREF
  _BYTE PropertyBuffer[1024]; // [rsp+60h] [rbp-418h] BYREF

  ResultLength = 0;
  DestinationString = 0LL;
  guard_dispatch_icall_no_overrides(a1, a2, a3, a4);
  v8 = guard_dispatch_icall_no_overrides(a1, a2, v6, v7);
  DeviceProperty = IoGetDeviceProperty(a2, DevicePropertyHardwareID, 0x400u, PropertyBuffer, &ResultLength);
  v10 = (const WCHAR *)PropertyBuffer;
  if ( DeviceProperty < 0 )
    v10 = L"<error>";
  RtlInitUnicodeString(&DestinationString, v10);
  KsepDsEventAddDevice(
    *(_QWORD *)(a1 + 24),
    *(_DWORD *)(a1 + 32),
    a1,
    (_DWORD)a2,
    v8,
    *(_QWORD *)(a1 + 48) + 24LL,
    (__int64)&DestinationString);
  return v8;
}
