/*
 * XREFs of NtAlpcCreateSectionView @ 0x1408951F0
 * Callers:
 *     <none>
 * Callees:
 *     KeLeaveCriticalRegion @ 0x140288450 (KeLeaveCriticalRegion.c)
 *     ObfDereferenceObject @ 0x1402CE210 (ObfDereferenceObject.c)
 *     ObReferenceObjectByHandle @ 0x140847200 (ObReferenceObjectByHandle.c)
 *     AlpcpCreateSectionView @ 0x1408950B8 (AlpcpCreateSectionView.c)
 *     AlpcReferenceBlobByHandle @ 0x140896580 (AlpcReferenceBlobByHandle.c)
 *     AlpcpDeleteView @ 0x14089CE2C (AlpcpDeleteView.c)
 *     AlpcpDereferenceBlobEx @ 0x14089EBC0 (AlpcpDereferenceBlobEx.c)
 *     ExRaiseDatatypeMisalignment @ 0x1408A3890 (ExRaiseDatatypeMisalignment.c)
 */

NTSTATUS __cdecl NtAlpcCreateSectionView(HANDLE PortHandle, ULONG Flags, PALPC_DATA_VIEW_ATTR ViewAttributes)
{
  struct _KTHREAD *CurrentThread; // rax
  char PreviousMode; // si
  __int64 v7; // rcx
  KPROCESSOR_MODE v8; // r9
  NTSTATUS v9; // ebx
  PVOID v10; // r13
  void *v11; // r15
  ULONG_PTR v12; // rsi
  PVOID Object[2]; // [rsp+30h] [rbp-48h] BYREF
  _ALPC_DATA_VIEW_ATTR v15; // [rsp+40h] [rbp-38h] BYREF
  ULONG_PTR BugCheckParameter2; // [rsp+98h] [rbp+20h] BYREF

  memset(&v15, 0, 28);
  BugCheckParameter2 = 0LL;
  CurrentThread = KeGetCurrentThread();
  --CurrentThread->KernelApcDisable;
  if ( Flags )
    goto LABEL_23;
  PreviousMode = KeGetCurrentThread()->PreviousMode;
  if ( PreviousMode )
  {
    if ( ((unsigned __int8)ViewAttributes & 3) != 0 )
      ExRaiseDatatypeMisalignment();
    v7 = 0x7FFFFFFF0000LL;
    if ( (unsigned __int64)ViewAttributes < 0x7FFFFFFF0000LL )
      v7 = (__int64)ViewAttributes;
    *(_BYTE *)v7 = *(_BYTE *)v7;
    *(_BYTE *)(v7 + 31) = *(_BYTE *)(v7 + 31);
    v15 = *ViewAttributes;
  }
  else
  {
    v15 = *ViewAttributes;
  }
  if ( v15.Flags || !v15.ViewSize || v15.ViewBase )
  {
LABEL_23:
    v9 = -1073741811;
  }
  else
  {
    v8 = KeGetCurrentThread()->PreviousMode;
    Object[0] = 0LL;
    v9 = ObReferenceObjectByHandle(PortHandle, 1u, AlpcPortObjectType, v8, Object, 0LL);
    if ( v9 >= 0 )
    {
      v10 = Object[0];
      v11 = (void *)AlpcReferenceBlobByHandle(*((_QWORD *)Object[0] + 2) + 40LL, v15.SectionHandle, AlpcSectionType);
      Object[1] = v11;
      if ( v11 )
      {
        v9 = AlpcpCreateSectionView((ULONG_PTR)v11, (__int64)v10, 0LL, v15.ViewSize, &BugCheckParameter2);
        if ( v9 >= 0 )
        {
          if ( PreviousMode )
          {
            v12 = BugCheckParameter2;
            ViewAttributes->ViewBase = *(PVOID *)(BugCheckParameter2 + 40);
            ViewAttributes->ViewSize = *(_QWORD *)(v12 + 48);
            if ( v9 < 0 )
              AlpcpDeleteView(v12);
          }
          else
          {
            v12 = BugCheckParameter2;
            ViewAttributes->ViewBase = *(PVOID *)(BugCheckParameter2 + 40);
            ViewAttributes->ViewSize = *(_QWORD *)(v12 + 48);
          }
          AlpcpDereferenceBlobEx(v12);
        }
        AlpcpDereferenceBlobEx((ULONG_PTR)v11);
      }
      else
      {
        v9 = -1073741816;
      }
      ObfDereferenceObject(v10);
    }
  }
  KeLeaveCriticalRegion();
  return v9;
}
