/*
 * XREFs of __asan_store2_volatile @ 0x1405A9C20
 * Callers:
 *     <none>
 * Callees:
...
char __fastcall _asan_store2_volatile(ULONG_PTR BugCheckParameter1)
{
  ULONG_PTR v1; // rdi
  ULONG_PTR v3; // rax
  int v4; // r9d
  ...
  if ( (BugCheckParameter1 & 7) == 7 )
  {
    if ( !v5 || v5 >= 8 )
    {
      ...
      if ( !v5 || v5 >= 8 || (char)(((BugCheckParameter1 + 1) & 7) + 1) <= v5 )
        goto LABEL_14;
    }
    goto LABEL_13;
  }
  if ( v5 )
  {
    if ( v5 < 8 )
    {
      ...
      if ( (char)v3 > v5 )
LABEL_13:
        LOBYTE(v3) = (unsigned __int8)KasaniReport(BugCheckParameter1, 2uLL, 1u, v1, v5);
    }
  }
LABEL_14:
  ...
  {
    LOBYTE(v4) = 1;
    LOBYTE(v3) = KcsanValidateAddress(BugCheckParameter1, 2, 1, v4, v1);
  }
  return v3;
}
