/*
 * XREFs of RtlpHpVaMgrCtxAllocatorReference @ 0x1406058F0
 * Callers:
 *     RtlpHpRegisterEnvironment @ 0x1406053F0 (RtlpHpRegisterEnvironment.c)
 *     RtlpHpVaMgrCtxStart @ 0x140605B5C (RtlpHpVaMgrCtxStart.c)
 * Callees:
 *     RtlpHpAcquireLockExclusive @ 0x14020D790 (RtlpHpAcquireLockExclusive.c)
 *     RtlpHpInitializeLock @ 0x1402B5710 (RtlpHpInitializeLock.c)
 *     RtlpHpReleaseLockExclusive @ 0x1402B9650 (RtlpHpReleaseLockExclusive.c)
 *     RtlpHpVaMgrCtxAllocatorFind @ 0x1404F8FF8 (RtlpHpVaMgrCtxAllocatorFind.c)
 */

__int64 __fastcall RtlpHpVaMgrCtxAllocatorReference(int *a1, __int64 a2)
{
  __int64 v2; // rax
  int v3; // r9d
  ULONG_PTR v4; // rbp
  int v7; // r10d
  __int64 v8; // r8
  unsigned int v9; // r14d
  char v10; // r15
  __int64 v11; // rax
  _QWORD *v12; // rbx
  __int16 v13; // ax
  char v14; // al
  char v15; // r8
  _QWORD *v17; // [rsp+50h] [rbp+8h] BYREF

  v2 = *(int *)(a2 + 4);
  v3 = a1[538];
  v4 = (ULONG_PTR)(a1 + 536);
  v17 = 0LL;
  v7 = `RtlpHpMemoryTypePageSize'::`2'::PageSize[v2];
  v8 = (v7 - 1) & (unsigned int)(v7 + v3 - 1);
  v9 = (unsigned int)(v3 - v8 + v7 - 1) >> 20;
  v10 = RtlpHpAcquireLockExclusive(a1 + 536, 1, v8);
  v11 = RtlpHpVaMgrCtxAllocatorFind((__int64)a1, a2, v9, &v17);
  v12 = (_QWORD *)v11;
  if ( v11 )
  {
    v13 = *(_WORD *)(v11 + 42);
    if ( v13 == -1 )
      v12 = 0LL;
    else
      *((_WORD *)v12 + 21) = v13 + 1;
  }
  else if ( v17 )
  {
    v12 = v17;
    RtlpHpInitializeLock(v17, *(_DWORD *)(a2 + 4) != 0);
    v12[4] = *(_QWORD *)(a2 + 16);
    *((_WORD *)v12 + 20) = v9;
    *((_WORD *)v12 + 21) = 1;
    v12[3] = a1;
    *((_BYTE *)v12 + 45) = *(_BYTE *)(a2 + 8);
    v14 = *((_BYTE *)v12 + 46) & 0xFE;
    *((_BYTE *)v12 + 44) = ((char *)v12 - (char *)a1 - 2160) / 48;
    *((_BYTE *)v12 + 46) = (16 * (*(_BYTE *)(a2 + 12) & 1)) | ((v15 | v14) & 0xF1 ^ (2 * (*(_BYTE *)(a2 + 4) & 7))) & 0xEF;
    ++a1[539];
  }
  RtlpHpReleaseLockExclusive(v4, 1LL, v10);
  if ( v12 )
    return *((unsigned __int8 *)v12 + 44);
  else
    return 0xFFFFFFFFLL;
}
