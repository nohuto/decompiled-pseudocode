/*
 * XREFs of Feature_Servicing_IntgrSched_Sleep_Enlightenment__private_IsEnabledDeviceUsageNoInline @ 0x140544888
 * Callers:
 *     HaliAcpiMachineStateInit @ 0x140701FB0 (HaliAcpiMachineStateInit.c)
 *     KiInitializeVelocity @ 0x140C2866C (KiInitializeVelocity.c)
 * Callees:
 *     Feature_Servicing_IntgrSched_Sleep_Enlightenment__private_IsEnabledFallback @ 0x1405448C0 (Feature_Servicing_IntgrSched_Sleep_Enlightenment__private_IsEnabledFallback.c)
 */

__int64 Feature_Servicing_IntgrSched_Sleep_Enlightenment__private_IsEnabledDeviceUsageNoInline()
{
  if ( (Feature_Servicing_IntgrSched_Sleep_Enlightenment__private_featureState & 0x10) != 0 )
    return Feature_Servicing_IntgrSched_Sleep_Enlightenment__private_featureState & 1;
  else
    return Feature_Servicing_IntgrSched_Sleep_Enlightenment__private_IsEnabledFallback(
             (unsigned int)Feature_Servicing_IntgrSched_Sleep_Enlightenment__private_featureState,
             3LL);
}
