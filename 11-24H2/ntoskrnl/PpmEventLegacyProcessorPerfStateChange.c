/*
 * XREFs of PpmEventLegacyProcessorPerfStateChange @ 0x14047062C
 * Callers:
 *     PpmPerfApplyProcessorState @ 0x1402ADC20 (PpmPerfApplyProcessorState.c)
 * Callees:
 *     EtwTraceKernelEvent @ 0x140285790 (EtwTraceKernelEvent.c)
 *     PpmFireWmiEvent @ 0x1405D5478 (PpmFireWmiEvent.c)
 *     __security_check_cookie @ 0x1406A6920 (__security_check_cookie.c)
 */

__int64 __fastcall PpmEventLegacyProcessorPerfStateChange(__int64 a1)
{
  _DWORD *v1; // r8
  __int64 v2; // rdx
  int v4; // esi
  int v5; // edi
  __int64 result; // rax
  unsigned int v7; // eax
  unsigned int v8; // ecx
  bool v9; // cf
  int v10; // eax
  int v11; // [rsp+30h] [rbp-50h] BYREF
  __int64 v12; // [rsp+34h] [rbp-4Ch]
  int v13; // [rsp+3Ch] [rbp-44h]
  int v14; // [rsp+40h] [rbp-40h]
  __int128 v15; // [rsp+48h] [rbp-38h] BYREF
  __int64 v16; // [rsp+58h] [rbp-28h]
  __int128 *v17; // [rsp+60h] [rbp-20h] BYREF
  int v18; // [rsp+68h] [rbp-18h]
  int v19; // [rsp+6Ch] [rbp-14h]

  v1 = *(_DWORD **)(a1 + 35256);
  v2 = *(_QWORD *)(a1 + 35248);
  v15 = 0LL;
  v16 = 0LL;
  v4 = v1[16];
  v5 = v1[21];
  if ( (xmmword_140FC6B50 & 0x8000) != 0 )
  {
    HIDWORD(v15) = 0;
    v7 = *(_DWORD *)(v2 + 528);
    v8 = v1[20];
    v18 = 24;
    if ( v8 >= v7 )
      v8 = v7;
    v9 = v8 < *(_DWORD *)(v2 + 456);
    DWORD1(v15) = v5;
    v19 = 0;
    LODWORD(v15) = v9 + 1;
    DWORD2(v15) = v1[31];
    v16 = *(_QWORD *)(a1 + 200);
    v17 = &v15;
    EtwTraceKernelEvent((int)&v17, 1, 0x80008000, 4659, 5249026);
  }
  result = (unsigned int)WmiPerfStateEventEnabled;
  if ( WmiPerfStateEventEnabled )
  {
    v10 = *(_DWORD *)(a1 + 36);
    v12 = 0LL;
    v14 = v10;
    v11 = v4;
    v13 = v5;
    return PpmFireWmiEvent(a1 + 35112, &PPM_PERFSTATE_CHANGE_GUID, 20LL, &v11);
  }
  return result;
}
