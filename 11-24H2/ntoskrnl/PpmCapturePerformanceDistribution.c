/*
 * XREFs of PpmCapturePerformanceDistribution @ 0x14047A270
 * Callers:
 *     ExpQuerySystemInformation @ 0x140ADDAE0 (ExpQuerySystemInformation.c)
 * Callees:
 *     KeGetPrcb @ 0x1402B0A10 (KeGetPrcb.c)
 *     KeWaitForSingleObject @ 0x14031DE40 (KeWaitForSingleObject.c)
 *     PopQueueTargetDpc @ 0x1403702F0 (PopQueueTargetDpc.c)
 *     PpmCapturePerformanceDistributionCallback @ 0x14047A520 (PpmCapturePerformanceDistributionCallback.c)
 *     KiLowerIrqlProcessIrqlFlags @ 0x1404F2848 (KiLowerIrqlProcessIrqlFlags.c)
 *     KiRaiseIrqlProcessIrqlFlags @ 0x1404F28AC (KiRaiseIrqlProcessIrqlFlags.c)
 *     __security_check_cookie @ 0x1406A6920 (__security_check_cookie.c)
 *     memset_0 @ 0x1406C0F40 (memset_0.c)
 */

__int64 __fastcall PpmCapturePerformanceDistribution(
        unsigned __int64 a1,
        unsigned int a2,
        int a3,
        __int64 a4,
        _DWORD *a5)
{
  unsigned __int64 v9; // rdi
  unsigned int v10; // ecx
  unsigned __int64 v11; // rdx
  __int64 v12; // rdx
  __int64 v13; // rcx
  __int64 Prcb; // r15
  unsigned __int8 CurrentIrql; // si
  __int64 v16; // rdx
  int v17; // r14d
  unsigned int v18; // r8d
  __int64 v19; // rcx
  __int64 v20; // rdx
  __int64 v21; // rcx
  __int64 result; // rax
  unsigned int v23; // eax
  __int64 Object; // [rsp+30h] [rbp-D0h] BYREF
  _QWORD v25[2]; // [rsp+38h] [rbp-C8h] BYREF
  _DWORD *v26; // [rsp+48h] [rbp-B8h] BYREF
  unsigned int v27; // [rsp+50h] [rbp-B0h]
  __int64 v28; // [rsp+54h] [rbp-ACh]
  int v29; // [rsp+5Ch] [rbp-A4h]
  _DWORD *v30; // [rsp+68h] [rbp-98h]
  _QWORD v31[2]; // [rsp+70h] [rbp-90h] BYREF
  __int16 v32; // [rsp+80h] [rbp-80h]
  int v33; // [rsp+82h] [rbp-7Eh]
  __int16 v34; // [rsp+86h] [rbp-7Ah]
  _QWORD v35[5]; // [rsp+88h] [rbp-78h] BYREF
  int v36; // [rsp+B0h] [rbp-50h]
  int v37; // [rsp+B4h] [rbp-4Ch]
  struct _KDPC Dpc; // [rsp+C0h] [rbp-40h] BYREF
  __int64 v39; // [rsp+100h] [rbp+0h] BYREF
  _QWORD v40[33]; // [rsp+108h] [rbp+8h] BYREF

  v30 = a5;
  memset_0(&v39, 0, 0x108uLL);
  v26 = (_DWORD *)a1;
  LOWORD(a1) = *(_WORD *)(a4 + 8);
  v9 = *(_QWORD *)a4;
  v29 = 0;
  v27 = a2;
  v28 = (unsigned int)(4 * a3 + 4);
  do
  {
LABEL_2:
    v10 = (unsigned __int16)a1 + 1;
    while ( !v9 )
    {
      a1 = (unsigned __int16)(a1 + 1);
      if ( (unsigned int)a1 >= v10 )
      {
        v23 = v28;
        *v30 = v28;
        if ( v23 > a2 )
          return 3221225476LL;
        *v26 = a3;
        return 0LL;
      }
      v9 = *(_QWORD *)(8 * a1 + 8);
    }
    _BitScanForward64(&v11, v9);
    v9 &= ~(1LL << v11);
    Prcb = KeGetPrcb(*((_DWORD *)qword_140F22998 + 64 * (unsigned __int16)a1 + (unsigned __int8)v11));
    CurrentIrql = KeGetCurrentIrql();
    __writecr8(2uLL);
    if ( KiIrqlFlags )
    {
      LOBYTE(v12) = 2;
      LOBYTE(v13) = CurrentIrql;
      KiRaiseIrqlProcessIrqlFlags(v13, v12);
    }
    v17 = PpmCapturePerformanceDistributionCallback(Prcb);
    if ( KiIrqlFlags )
    {
      LOBYTE(v16) = CurrentIrql;
      KiLowerIrqlProcessIrqlFlags(KeGetCurrentIrql(), v16);
    }
    __writecr8(CurrentIrql);
  }
  while ( v17 >= 0 );
  v39 = 2097153LL;
  memset_0(v40, 0, 0x100uLL);
  v18 = *((_DWORD *)KiGlobalState + *(unsigned int *)(Prcb + 36)) & 0x3F;
  v19 = *((_DWORD *)KiGlobalState + *(unsigned int *)(Prcb + 36)) >> 6;
  if ( (unsigned __int16)v39 > (unsigned int)v19 )
    goto LABEL_10;
  if ( WORD1(v39) > (unsigned int)v19 )
  {
    LOWORD(v39) = v19 + 1;
LABEL_10:
    v20 = (unsigned int)v19;
    v21 = v40[v19];
    _bittestandset64(&v21, v18);
    v40[v20] = v21;
  }
  v37 = 0;
  v33 = 0;
  v34 = 0;
  v25[1] = v25;
  v25[0] = v25;
  v31[1] = v40[0];
  v31[0] = &v39;
  v35[0] = PpmCapturePerformanceDistributionCallback;
  v35[1] = &v26;
  v35[4] = v31;
  v35[3] = &Object;
  memset(&Dpc, 0, 60);
  Object = 393217LL;
  v32 = 0;
  v35[2] = 0LL;
  v36 = 0;
  PopQueueTargetDpc(&Dpc, (__int64)v35);
  KeWaitForSingleObject(&Object, Executive, 0, 0, 0LL);
  result = (unsigned int)v36;
  if ( v36 >= 0 )
    goto LABEL_2;
  return result;
}
