/*
 * XREFs of KiSynchNumaCounterSetCallback @ 0x140A60920
 * Callers:
 *     <none>
 * Callees:
 *     KeEnumerateNextSchedulerSubNodeInNode @ 0x1402088E0 (KeEnumerateNextSchedulerSubNodeInNode.c)
 *     KiQuerySubNodeActiveAffinity @ 0x1403A8720 (KiQuerySubNodeActiveAffinity.c)
 *     ExGetExtensionTable @ 0x1403AA530 (ExGetExtensionTable.c)
 *     RtlStringCbPrintfW @ 0x14040BC90 (RtlStringCbPrintfW.c)
 *     KeInitializeSchedulerSubNodeEnumerationContext @ 0x14041DF40 (KeInitializeSchedulerSubNodeEnumerationContext.c)
 *     RtlInitUnicodeString @ 0x1404241A0 (RtlInitUnicodeString.c)
 *     ExReleaseExtensionTable @ 0x14044FE80 (ExReleaseExtensionTable.c)
 *     __security_check_cookie @ 0x1406A5920 (__security_check_cookie.c)
 *     _guard_dispatch_icall_no_overrides @ 0x1406B3DF0 (_guard_dispatch_icall_no_overrides.c)
 *     memset_0 @ 0x1406C0040 (memset_0.c)
 *     EtwDereferenceSpinLockCounters @ 0x1407ACDE8 (EtwDereferenceSpinLockCounters.c)
 *     EtwReferenceSpinLockCounters @ 0x1407ACE44 (EtwReferenceSpinLockCounters.c)
 *     PcwAddInstance @ 0x1409FE0F0 (PcwAddInstance.c)
 *     ExpPcwDisabledStatus @ 0x140A4C4F0 (ExpPcwDisabledStatus.c)
 */

NTSTATUS __fastcall KiSynchNumaCounterSetCallback(int a1, __int64 a2)
{
  int v4; // ebx
  struct _PCW_BUFFER *v6; // r12
  __int64 i; // rdi
  int v8; // r15d
  unsigned int v9; // ebx
  int v10; // ebx
  unsigned __int16 v11; // si
  __int16 v12; // r8
  __int16 v13; // ax
  unsigned __int64 v14; // r13
  int v15; // r14d
  unsigned int v16; // ecx
  unsigned __int64 v17; // rdx
  _DWORD *v18; // rbx
  PPCW_DATA Data; // [rsp+28h] [rbp-E0h]
  _QWORD DestinationString[3]; // [rsp+38h] [rbp-D0h] BYREF
  __int64 v21; // [rsp+50h] [rbp-B8h] BYREF
  __int128 v22; // [rsp+58h] [rbp-B0h] BYREF
  _DWORD *v23; // [rsp+68h] [rbp-A0h] BYREF
  __int64 v24; // [rsp+70h] [rbp-98h]
  int *v25; // [rsp+78h] [rbp-90h] BYREF
  int v26; // [rsp+80h] [rbp-88h]
  struct _PCW_DATA v27; // [rsp+88h] [rbp-80h] BYREF
  int v28; // [rsp+98h] [rbp-70h] BYREF
  int v29; // [rsp+9Ch] [rbp-6Ch]
  int v30; // [rsp+A0h] [rbp-68h]
  int v31; // [rsp+A4h] [rbp-64h]
  int v32; // [rsp+A8h] [rbp-60h]
  int v33; // [rsp+ACh] [rbp-5Ch]
  int v34; // [rsp+B0h] [rbp-58h]
  int v35; // [rsp+B4h] [rbp-54h]
  int v36; // [rsp+B8h] [rbp-50h]
  int v37; // [rsp+BCh] [rbp-4Ch]
  int v38; // [rsp+C0h] [rbp-48h]
  int v39; // [rsp+C4h] [rbp-44h]
  int v40; // [rsp+C8h] [rbp-40h]
  int v41; // [rsp+CCh] [rbp-3Ch]
  int v42; // [rsp+D0h] [rbp-38h]
  int v43; // [rsp+D4h] [rbp-34h]
  int v44; // [rsp+D8h] [rbp-30h]
  int v45; // [rsp+DCh] [rbp-2Ch]
  int v46; // [rsp+E0h] [rbp-28h]
  int v47; // [rsp+E4h] [rbp-24h]
  int v48; // [rsp+E8h] [rbp-20h]
  int v49; // [rsp+ECh] [rbp-1Ch]
  int v50; // [rsp+F0h] [rbp-18h]
  int v51; // [rsp+F4h] [rbp-14h]
  int v52; // [rsp+F8h] [rbp-10h]
  int v53; // [rsp+FCh] [rbp-Ch]
  int v54; // [rsp+100h] [rbp-8h]
  int v55; // [rsp+104h] [rbp-4h]
  int v56; // [rsp+108h] [rbp+0h]
  int v57; // [rsp+10Ch] [rbp+4h]
  int v58; // [rsp+110h] [rbp+8h]
  int v59; // [rsp+114h] [rbp+Ch]
  int v60; // [rsp+118h] [rbp+10h]
  int v61; // [rsp+11Ch] [rbp+14h]
  int v62; // [rsp+120h] [rbp+18h]
  int v63; // [rsp+124h] [rbp+1Ch]
  int v64; // [rsp+128h] [rbp+20h]
  int v65; // [rsp+12Ch] [rbp+24h]
  int v66; // [rsp+130h] [rbp+28h]
  int v67; // [rsp+134h] [rbp+2Ch]
  int v68; // [rsp+140h] [rbp+38h]
  int v69; // [rsp+144h] [rbp+3Ch]
  int v70; // [rsp+158h] [rbp+50h] BYREF
  int v71; // [rsp+15Ch] [rbp+54h]
  int v72; // [rsp+160h] [rbp+58h]
  int v73; // [rsp+164h] [rbp+5Ch]
  int v74; // [rsp+168h] [rbp+60h]
  int v75; // [rsp+16Ch] [rbp+64h]
  int v76; // [rsp+170h] [rbp+68h]
  int v77; // [rsp+174h] [rbp+6Ch]
  int v78; // [rsp+178h] [rbp+70h]
  int v79; // [rsp+17Ch] [rbp+74h]
  int v80; // [rsp+180h] [rbp+78h]
  int v81; // [rsp+184h] [rbp+7Ch]
  int v82; // [rsp+188h] [rbp+80h]
  int v83; // [rsp+18Ch] [rbp+84h]
  int v84; // [rsp+190h] [rbp+88h]
  int v85; // [rsp+194h] [rbp+8Ch]
  int v86; // [rsp+198h] [rbp+90h]
  int v87; // [rsp+19Ch] [rbp+94h]
  int v88; // [rsp+1A0h] [rbp+98h]
  int v89; // [rsp+1A4h] [rbp+9Ch]
  int v90; // [rsp+1A8h] [rbp+A0h]
  int v91; // [rsp+1ACh] [rbp+A4h]
  int v92; // [rsp+1B0h] [rbp+A8h]
  int v93; // [rsp+1B4h] [rbp+ACh]
  int v94; // [rsp+1B8h] [rbp+B0h]
  int v95; // [rsp+1BCh] [rbp+B4h]
  int v96; // [rsp+1C0h] [rbp+B8h]
  int v97; // [rsp+1C4h] [rbp+BCh]
  int v98; // [rsp+1C8h] [rbp+C0h]
  int v99; // [rsp+1CCh] [rbp+C4h]
  int v100; // [rsp+1D0h] [rbp+C8h]
  int v101; // [rsp+1D4h] [rbp+CCh]
  int v102; // [rsp+1D8h] [rbp+D0h]
  int v103; // [rsp+1DCh] [rbp+D4h]
  int v104; // [rsp+1E0h] [rbp+D8h]
  int v105; // [rsp+1E4h] [rbp+DCh]
  int v106; // [rsp+1E8h] [rbp+E0h]
  int v107; // [rsp+1ECh] [rbp+E4h]
  int v108; // [rsp+1F0h] [rbp+E8h]
  int v109; // [rsp+1F4h] [rbp+ECh]
  int v110; // [rsp+200h] [rbp+F8h]
  int v111; // [rsp+204h] [rbp+FCh]
  __int128 v112; // [rsp+218h] [rbp+110h] BYREF
  __int64 v113; // [rsp+228h] [rbp+120h]
  wchar_t pszDest[12]; // [rsp+230h] [rbp+128h] BYREF

  v113 = 0LL;
  v112 = 0LL;
  memset_0(&v28, 0, 0xB8uLL);
  memset_0(&v70, 0, 0xB8uLL);
  v21 = 0LL;
  *(_OWORD *)&DestinationString[1] = 0LL;
  if ( !a1 )
  {
    EtwReferenceSpinLockCounters();
    return 0;
  }
  v4 = a1 - 1;
  if ( !v4 )
  {
    EtwDereferenceSpinLockCounters();
    return 0;
  }
  if ( (unsigned int)(v4 - 1) >= 2 )
    return 0;
  v6 = *(struct _PCW_BUFFER **)(a2 + 24);
  v22 = 0LL;
  for ( i = 0LL; (unsigned int)i < (unsigned __int16)KeNumberNodes; i = (unsigned int)(i + 1) )
  {
    memset_0(&v28, 0, 0xB8uLL);
    v8 = 0;
    KeInitializeSchedulerSubNodeEnumerationContext(&v112, KeNodeBlock[i]);
LABEL_8:
    if ( !(unsigned int)KeEnumerateNextSchedulerSubNodeInNode(&v112, &v21) )
    {
      KiQuerySubNodeActiveAffinity(v21, (__int64)&v22, 0LL);
      v11 = WORD4(v22);
      v13 = v12 + 1;
      v14 = v22;
      v15 = WORD4(v22);
      while ( 1 )
      {
        v16 = v15 + 1;
        while ( !v14 )
        {
          v11 += v13;
          v15 = v11;
          if ( v11 >= v16 )
            goto LABEL_8;
          v14 = *(_QWORD *)(8LL * v11 + 8);
          v13 = 1;
        }
        _BitScanForward64(&v17, v14);
        v14 &= ~(1LL << v17);
        LODWORD(DestinationString[0]) = *((_DWORD *)qword_140F21E78 + 64 * v15 + (unsigned int)(unsigned __int8)v17);
        v18 = (_DWORD *)(KiProcessorBlock[LODWORD(DestinationString[0])] + 36544);
        v28 += *v18;
        v29 += v18[1];
        v30 += v18[2];
        v31 += v18[3];
        v32 += v18[4];
        v33 += v18[5];
        v34 += v18[6];
        v35 += v18[7];
        v36 += v18[8];
        v37 += v18[9];
        v38 += v18[10];
        v39 += v18[11];
        v40 += v18[12];
        v41 += v18[13];
        v42 += v18[14];
        v43 += v18[15];
        v44 += v18[16];
        v45 += v18[17];
        v46 += v18[18];
        v47 += v18[19];
        v48 += v18[20];
        v49 += v18[21];
        v50 += v18[22];
        v51 += v18[23];
        v52 += v18[24];
        v53 += v18[25];
        v54 += v18[26];
        v55 += v18[27];
        v56 += v18[28];
        v57 += v18[29];
        v58 += v18[30];
        v59 += v18[31];
        v60 += v18[32];
        v61 += v18[33];
        v62 += v18[34];
        v63 += v18[35];
        v64 += v18[36];
        v65 += v18[37];
        v66 += v18[38];
        v67 += v18[39];
        v68 += v18[42];
        v69 += v18[43];
        v70 += *v18;
        v71 += v18[1];
        v72 += v18[2];
        v73 += v18[3];
        v74 += v18[4];
        v75 += v18[5];
        v76 += v18[6];
        v77 += v18[7];
        v78 += v18[8];
        v79 += v18[9];
        v80 += v18[10];
        v81 += v18[11];
        v82 += v18[12];
        v83 += v18[13];
        v84 += v18[14];
        v85 += v18[15];
        v86 += v18[16];
        v87 += v18[17];
        v88 += v18[18];
        v89 += v18[19];
        v90 += v18[20];
        v91 += v18[21];
        v92 += v18[22];
        v93 += v18[23];
        v94 += v18[24];
        v95 += v18[25];
        v96 += v18[26];
        v97 += v18[27];
        v98 += v18[28];
        v99 += v18[29];
        v100 += v18[30];
        v101 += v18[31];
        v102 += v18[32];
        v103 += v18[33];
        v104 += v18[34];
        v105 += v18[35];
        v106 += v18[36];
        v107 += v18[37];
        v108 += v18[38];
        v109 += v18[39];
        v110 += v18[42];
        v111 += v18[43];
        LODWORD(Data) = v8;
        RtlStringCbPrintfW(pszDest, 0x16uLL, L"%u,%u", (unsigned int)i, Data);
        RtlInitUnicodeString((PUNICODE_STRING)&DestinationString[1], pszDest);
        v23 = v18;
        LODWORD(v24) = 184;
        if ( ExGetExtensionTable((struct _EX_RUNDOWN_REF *)ExpPcwExtensionHost) )
        {
          Data = (PPCW_DATA)&v23;
          v10 = guard_dispatch_icall_no_overrides(v6, &DestinationString[1], LODWORD(DestinationString[0]), 1LL);
          ExReleaseExtensionTable((struct _EX_RUNDOWN_REF *)ExpPcwExtensionHost);
        }
        else
        {
          v10 = ExpPcwDisabledStatus();
        }
        if ( v10 < 0 )
          return v10;
        v13 = 1;
        ++v8;
      }
    }
    if ( v8 )
    {
      RtlStringCbPrintfW(pszDest, 0x16uLL, L"%u,_Total", (unsigned int)i);
      RtlInitUnicodeString((PUNICODE_STRING)&DestinationString[1], pszDest);
      v9 = i + KeMaximumProcessors;
      v25 = &v28;
      v26 = 184;
      if ( ExGetExtensionTable((struct _EX_RUNDOWN_REF *)ExpPcwExtensionHost) )
      {
        Data = (PPCW_DATA)&v25;
        v10 = guard_dispatch_icall_no_overrides(v6, &DestinationString[1], v9, 1LL);
        ExReleaseExtensionTable((struct _EX_RUNDOWN_REF *)ExpPcwExtensionHost);
      }
      else
      {
        v10 = ExpPcwDisabledStatus();
      }
      if ( v10 < 0 )
        return v10;
    }
  }
  RtlStringCbPrintfW(pszDest, 0x16uLL, L"_Total");
  RtlInitUnicodeString((PUNICODE_STRING)&DestinationString[1], pszDest);
  v27.Data = &v70;
  v27.Size = 184;
  return PcwAddInstance(
           v6,
           (PCUNICODE_STRING)&DestinationString[1],
           KeMaximumProcessors + (unsigned __int16)KeNumberNodes,
           1u,
           &v27);
}
