/*
 * XREFs of IopIrqInitialize @ 0x1407262F8
 * Callers:
 *     IopInitializePlugPlayServices @ 0x140C1F07C (IopInitializePlugPlayServices.c)
 * Callees:
 *     ArbInitializeArbiterInstance @ 0x1407055B4 (ArbInitializeArbiterInstance.c)
 */

__int64 __fastcall IopIrqInitialize(__int64 a1, __int64 a2)
{
  qword_140F8AE98 = (__int64)IopIrqUnpackRequirement;
  qword_140F8AEA0 = (__int64)IopIrqPackResource;
  qword_140F8AEA8 = (__int64)IopIrqUnpackResource;
  qword_140F8AEB0 = (__int64)IopIrqScoreRequirement;
  return ArbInitializeArbiterInstance((__int64)&IopRootIrqArbiter, a2, 2, (__int64)L"RootIRQ");
}
