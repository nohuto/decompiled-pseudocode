/*
 * XREFs of __asan_load4_volatile @ 0x1405A8AA0
 * Callers:
 *     <none>
 * Callees:
...
char __fastcall _asan_load4_volatile(ULONG_PTR BugCheckParameter1)
{
  ULONG_PTR v1; // rdi
  ULONG_PTR v3; // rax
  int v4; // r9d
  ...
  else
  {
    ...
    if ( v5 && v5 < 8 && (char)(((BugCheckParameter1 + 3) & 7) + 1) > v5 )
LABEL_20:
      LOBYTE(v3) = (unsigned __int8)KasaniReport(BugCheckParameter1, 4uLL, 0, v1, v5);
  }
LABEL_21:
  if ( KcsaniValidationEnabled )
  {
    LOBYTE(v4) = 1;
    LOBYTE(v3) = KcsanValidateAddress(BugCheckParameter1, 4, 0, v4, v1);
  }
  return v3;
}
