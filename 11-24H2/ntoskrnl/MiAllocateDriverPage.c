/*
 * XREFs of MiAllocateDriverPage @ 0x140A68CD8
 * Callers:
 *     MiLockCode @ 0x14023D6F0 (MiLockCode.c)
 *     MiPrivateFixup @ 0x1402EE4F0 (MiPrivateFixup.c)
 *     MiMakeDriverPagesPrivate @ 0x140436EA8 (MiMakeDriverPagesPrivate.c)
 *     MiHandleBootImage @ 0x140C5AE90 (MiHandleBootImage.c)
 * Callees:
 *     MiWaitForFreePage @ 0x1402CF708 (MiWaitForFreePage.c)
 *     MiInitializePageColorBase @ 0x1402EF8B0 (MiInitializePageColorBase.c)
 *     MiGetSlabPage @ 0x1402F3934 (MiGetSlabPage.c)
 *     MiGetPage @ 0x1402F41B0 (MiGetPage.c)
 *     MiFindContiguousPagesEx @ 0x140304960 (MiFindContiguousPagesEx.c)
 *     MiUseProtectedSlabAllocatorForDriverPage @ 0x14043738C (MiUseProtectedSlabAllocatorForDriverPage.c)
 *     MiGetNextPageColor @ 0x14043D550 (MiGetNextPageColor.c)
 *     MiGetSlabCurrentTime @ 0x14048ED80 (MiGetSlabCurrentTime.c)
 *     MiPreInitializeSystemImagePage @ 0x1404CBDEC (MiPreInitializeSystemImagePage.c)
 */

__int64 __fastcall MiAllocateDriverPage(__int64 a1, char a2, char a3)
{
  unsigned int NextPageColor; // ebx
  int v7; // eax
  __int64 v8; // rdi
  __int64 result; // rax
  __int64 v10; // rcx
  __int64 v11; // [rsp+70h] [rbp+27h] BYREF
  __int128 v12; // [rsp+78h] [rbp+2Fh] BYREF
  int v13; // [rsp+C8h] [rbp+7Fh] BYREF

  v13 = 0;
  v12 = 0LL;
  MiInitializePageColorBase(0LL, 3, 0, (__int64)&v12);
  NextPageColor = MiGetNextPageColor((__int64)&v12);
  v7 = MiUseProtectedSlabAllocatorForDriverPage(a1, a2, &v13);
  v8 = -1LL;
  if ( !v7
    || (result = MiGetSlabPage(a1, v13, NextPageColor, 2u, (__int64 *)0xFFFFFFFFFFFFFFFFLL, 0),
        v11 = result,
        result == -1) )
  {
    result = MiGetSlabPage(a1, 5, NextPageColor, 2u, (__int64 *)0xFFFFFFFFFFFFFFFFLL, 0);
    v11 = result;
    if ( result == -1 )
    {
      if ( (a3 & 1) != 0 )
      {
        return -1LL;
      }
      else
      {
        if ( !*(_QWORD *)(a1 + 16696) )
          *(_QWORD *)(a1 + 16696) = MiGetSlabCurrentTime();
        _InterlockedIncrement64((volatile signed __int64 *)(a1 + 16688));
        if ( (int)MiFindContiguousPagesEx(
                    a1,
                    0LL,
                    *(_QWORD *)(a1 + 16680),
                    0LL,
                    0,
                    1LL,
                    1u,
                    0x80000000,
                    255,
                    0x60000001u,
                    0,
                    0LL,
                    0LL,
                    (ULONG_PTR *)&v11) < 0 )
        {
          *(_QWORD *)(a1 + 16680) = -1LL;
          while ( 1 )
          {
            result = MiGetPage(a1, NextPageColor, 0);
            v11 = result;
            if ( result != -1 )
              break;
            if ( KeGetCurrentIrql() == 2 || (KeGetPcr()->Prcb.DpcRequestSummary & 0x10001) != 0 )
              return -1LL;
            MiWaitForFreePage(a1, 0);
          }
        }
        else
        {
          ++*(_QWORD *)(a1 + 16672);
          v10 = v11;
          if ( (*(_QWORD *)(a1 + 16672) & 0x1FFLL) != 0 )
            v8 = v11 - 1;
          *(_QWORD *)(a1 + 16680) = v8;
          MiPreInitializeSystemImagePage(v10);
          return v11;
        }
      }
    }
  }
  return result;
}
