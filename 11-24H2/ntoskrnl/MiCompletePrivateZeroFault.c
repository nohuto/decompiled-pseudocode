/*
 * XREFs of MiCompletePrivateZeroFault @ 0x14021D340
 * Callers:
 *     MiResolvePrivateZeroFault @ 0x1402FE070 (MiResolvePrivateZeroFault.c)
 * Callees:
 *     MiClearPageFileReservation @ 0x140213DEC (MiClearPageFileReservation.c)
 *     MiWriteValidPteNewProtection @ 0x140219260 (MiWriteValidPteNewProtection.c)
 *     MI_READ_PTE_LOCK_FREE @ 0x14021A250 (MI_READ_PTE_LOCK_FREE.c)
 *     MiReleasePageFileInfo @ 0x14021B9A0 (MiReleasePageFileInfo.c)
 *     MiInitializePfn @ 0x14021E510 (MiInitializePfn.c)
 *     MiMapPageInHyperSpaceWorker @ 0x14021F1A0 (MiMapPageInHyperSpaceWorker.c)
 *     MiAllocateWsle @ 0x14021F980 (MiAllocateWsle.c)
 *     MiDecrementShareCountEx @ 0x140220590 (MiDecrementShareCountEx.c)
 *     MiReleaseFreshPage @ 0x140221FC0 (MiReleaseFreshPage.c)
 *     MiInsertPageInFreeOrZeroedList @ 0x140222210 (MiInsertPageInFreeOrZeroedList.c)
 *     MiSwizzleInvalidPte @ 0x14022A3B0 (MiSwizzleInvalidPte.c)
 *     MiInsertPageInList @ 0x14022CB10 (MiInsertPageInList.c)
 *     MiCheckLinearProtectedPteAccessedBit @ 0x140232A20 (MiCheckLinearProtectedPteAccessedBit.c)
 *     MiSanitizeShadowPxe @ 0x140233C54 (MiSanitizeShadowPxe.c)
 *     MiWritePteShadow @ 0x140233CD4 (MiWritePteShadow.c)
 *     MiUserPdeOrAbove @ 0x140238638 (MiUserPdeOrAbove.c)
 *     MiBadShareCount @ 0x14023EE28 (MiBadShareCount.c)
 *     MiUnlockVadTree @ 0x140261370 (MiUnlockVadTree.c)
 *     MiPteInShadowRange @ 0x1402863E0 (MiPteInShadowRange.c)
 *     HvlNotifyLongSpinWait @ 0x140293260 (HvlNotifyLongSpinWait.c)
 *     KiCheckVpBackingLongSpinWaitHypercall @ 0x140293290 (KiCheckVpBackingLongSpinWaitHypercall.c)
 *     MiLockVadTree @ 0x1402DEE70 (MiLockVadTree.c)
 *     MiAddWorkingSetEntries @ 0x1402E0980 (MiAddWorkingSetEntries.c)
 *     MiEmptyDeferredWorkingSetEntries @ 0x1402E1B00 (MiEmptyDeferredWorkingSetEntries.c)
 *     MiGetPagingFileOffset @ 0x1402E5A60 (MiGetPagingFileOffset.c)
 *     MiAdvanceFaultList @ 0x1402E5FE0 (MiAdvanceFaultList.c)
 *     MiIsAddressGlobal @ 0x1402ECF00 (MiIsAddressGlobal.c)
 *     MiLocateAddress @ 0x1402FC070 (MiLocateAddress.c)
 *     MiSetPfnIdentity @ 0x1403A00D0 (MiSetPfnIdentity.c)
 *     MiUpdatePageTableUseCount @ 0x1403E3650 (MiUpdatePageTableUseCount.c)
 *     KeBugCheckEx @ 0x1404FB990 (KeBugCheckEx.c)
 */

__int64 __fastcall MiCompletePrivateZeroFault(__int64 a1, ULONG_PTR *a2, unsigned __int64 a3)
{
  unsigned __int64 v3; // rdi
  unsigned __int64 v4; // r9
  ULONG_PTR *v5; // r13
  __int64 v6; // r15
  _QWORD *v8; // rcx
  ULONG_PTR v9; // r15
  __int64 v10; // rax
  __int64 v11; // rax
  __int64 v12; // rcx
  __int64 v13; // rax
  int v14; // esi
  int v15; // esi
  __int64 v16; // rbx
  int v17; // r11d
  unsigned __int64 v18; // rdi
  unsigned __int64 v19; // rcx
  int IsAddressGlobal; // eax
  unsigned __int64 v21; // rdx
  int v22; // r8d
  unsigned __int64 v23; // rdi
  __int64 v24; // rdx
  unsigned __int64 v25; // rax
  unsigned __int64 v26; // rdx
  __int64 v27; // rax
  __int64 v28; // r8
  unsigned __int64 v29; // rcx
  unsigned __int64 v30; // rdx
  __int64 v31; // rsi
  ULONG_PTR v32; // r14
  bool v33; // zf
  unsigned __int64 v34; // rbx
  int v35; // ecx
  unsigned int v36; // esi
  __int64 v37; // r8
  __int64 v38; // r9
  __int64 v39; // rbx
  unsigned __int64 v40; // rax
  ULONG_PTR v41; // rbx
  __int64 v42; // rax
  __int64 v43; // rdx
  char v44; // cl
  __int64 *v45; // rdi
  int v46; // eax
  __int64 v47; // rcx
  unsigned __int64 v48; // rdx
  __int64 v49; // r9
  __int16 v50; // ax
  unsigned int v51; // ecx
  int v52; // esi
  __int64 v53; // rcx
  __int64 v54; // rdi
  int v55; // ebx
  __int64 v56; // rcx
  __int64 v57; // rbx
  unsigned int v58; // edi
  __int64 v59; // rdx
  unsigned __int64 v60; // rcx
  __int64 v61; // r13
  __int64 v62; // rdi
  unsigned __int64 v63; // r8
  ULONG_PTR BugCheckParameter4; // rax
  __int64 *v65; // r9
  __int64 v66; // rdi
  __int64 v67; // rax
  __int64 v68; // rcx
  unsigned int v69; // edi
  unsigned int v70; // edi
  ULONG_PTR *v71; // rcx
  __int64 v72; // rcx
  __int64 v73; // rax
  unsigned __int64 v74; // rdi
  _KPROCESS *v75; // rcx
  unsigned __int64 KernelWaitTime; // rdx
  __int64 v77; // rax
  __int64 v78; // rcx
  __int64 v79; // rcx
  __int64 v80; // rax
  _QWORD *v81; // rdx
  int v82; // eax
  unsigned int v83; // r14d
  int v85; // ebx
  __int64 v86; // rdi
  __int64 v87; // rcx
  unsigned int v88; // edi
  __int64 v89; // rcx
  char v90; // al
  __int64 v91; // rcx
  char v92; // al
  char v93; // r8
  __int64 v94; // rax
  __int64 v95; // rax
  __int64 v96; // rdi
  unsigned int v97; // edi
  int v98; // eax
  int v99; // eax
  __int64 Address; // rax
  __int64 v101; // rdx
  BOOL v102; // [rsp+40h] [rbp-69h]
  int v103; // [rsp+44h] [rbp-65h]
  unsigned __int64 v104; // [rsp+48h] [rbp-61h] BYREF
  int v105; // [rsp+50h] [rbp-59h]
  unsigned int v106; // [rsp+54h] [rbp-55h]
  unsigned __int64 v107; // [rsp+58h] [rbp-51h]
  int v108; // [rsp+60h] [rbp-49h]
  __int64 v109; // [rsp+68h] [rbp-41h] BYREF
  _KPROCESS *Process; // [rsp+70h] [rbp-39h]
  __int64 v111; // [rsp+78h] [rbp-31h] BYREF
  unsigned __int64 v112; // [rsp+80h] [rbp-29h]
  __int64 v113; // [rsp+88h] [rbp-21h] BYREF
  __int64 v114; // [rsp+90h] [rbp-19h]
  __int64 v115; // [rsp+98h] [rbp-11h]
  __int64 v116; // [rsp+A0h] [rbp-9h]
  char v117; // [rsp+110h] [rbp+67h]
  int v120; // [rsp+120h] [rbp+77h]
  int v121; // [rsp+128h] [rbp+7Fh]

  v3 = *(_QWORD *)(a1 + 56);
  v4 = a3;
  v5 = a2;
  v6 = (*(_QWORD *)(a1 + 16) >> 9) & 0x7FFFFFFFF8LL;
  v112 = v3;
  v8 = *(_QWORD **)(a1 + 8);
  v9 = v6 - 0x98000000000LL;
  v117 = 0;
  v102 = 0;
  v109 = v8[7];
  v10 = v8[2];
  if ( (v10 & 1) != 0 )
    v102 = *(_BYTE *)(v10 & 0xFFFFFFFFFFFFFFFEuLL) == 3;
  v11 = 0LL;
  if ( (*(_BYTE *)a1 & 0x20) != 0 )
    v11 = v3;
  v116 = v11;
  v12 = (*v8 >> 9) & 0x7FFFFFFFF8LL;
  Process = KeGetCurrentThread()->ApcState.Process;
  v105 = 0;
  v13 = *(_QWORD *)(a1 + 64);
  v14 = 8208;
  v114 = v12 - 0x98000000000LL;
  v115 = v13;
  v121 = 8208;
  if ( v13 )
  {
    if ( (*(unsigned int *)(v13 + 52) | ((unsigned __int64)*(unsigned __int8 *)(v13 + 34) << 32)) == 0x7FFFFFFFELL )
      v14 = 8272;
    v121 = v14;
  }
  v15 = *(_DWORD *)(a1 + 32);
  v108 = v15;
  if ( (v15 & 2) == 0 || v9 > 0xFFFFF6BFFFFFFF78uLL || v9 < 0xFFFFF68000000000uLL )
  {
    v15 |= 0x80000000;
    v108 = v15;
  }
  if ( !*(_DWORD *)(a1 + 36) )
    __incgsdword(0x2E9Cu);
  v103 = 0;
  v106 = 0;
  v16 = v15 & 0x1F;
  v17 = 0;
  v18 = MmProtectToPteMask[v16] & 0xFFF0000000000E5EuLL | 0x21;
  if ( v9 < 0xFFFFF68000000000uLL || v9 > 0xFFFFF6FFFFFFFFFFuLL )
  {
    v23 = MmProtectToPteMask[v16] & 0xFFF0000000000E5EuLL | 0x121;
    v22 = v15 & 0x4000000;
  }
  else
  {
    if ( v9 >= 0xFFFFF6FB40000000uLL && v9 <= 0xFFFFF6FB7FFFFFFFuLL )
    {
      if ( v9 == 0xFFFFF6FB7DBEDF68uLL )
      {
        v18 = MmProtectToPteMask[v16] & 0x7FF0000000000E5ELL | 0x8000000000000021uLL;
      }
      else if ( (v15 & 0x4000000) == 0 )
      {
        v18 = MmProtectToPteMask[v16] & 0x7FF0000000000E5ELL | 0x21;
      }
      v82 = MiUserPdeOrAbove(v9);
      v4 = a3;
      if ( v82 )
        v18 |= 4uLL;
    }
    v19 = (__int64)(v9 << 25) >> 16;
    if ( (v15 & 0x4000000) != 0 && v19 >= 0xFFFFF68000000000uLL )
    {
      do
      {
        if ( v19 > 0xFFFFF6FFFFFFFFFFuLL )
          break;
        v19 = (__int64)(v19 << 25) >> 16;
      }
      while ( v19 >= 0xFFFFF68000000000uLL );
    }
    if ( v19 <= 0x7FFFFFFEFFFFLL )
      v18 |= 4uLL;
    IsAddressGlobal = MiIsAddressGlobal(v19, v18, v15 & 0x4000000, v4);
    v23 = v18 | 0x100;
    if ( !IsAddressGlobal )
      v23 = v21;
  }
  v24 = v23 | 0x42;
  if ( (v15 & 5) != 4 || v15 >= 0 )
    v24 = v23;
  v25 = v24 & 0xFFFFFFFFFFFFFFFBuLL;
  if ( (v15 & 0x40000000) == 0 )
    v25 = v24;
  if ( (v15 & 0x20000000) != 0 )
    v25 = ((unsigned __int64)(unsigned __int8)word_140E2ED84 << 8) ^ (((unsigned __int64)(unsigned __int8)word_140E2ED84 << 8) ^ v25) & 0xFFFFFFFFFFFFFEFFuLL;
  v26 = v25 & 0xFFFFFFFFFFFFFEFFuLL;
  if ( (v15 & 0x8000000) == 0 )
    v26 = v25;
  v27 = v26 | 0x80;
  if ( !v22 )
    v27 = v26;
  v104 = v27 & 0xF0FFFFFFFFFFFFFFuLL | 0xA00000000000000LL;
  if ( v4 > 1 )
  {
    v89 = *(_QWORD *)(a1 + 8);
    v90 = *(_BYTE *)(v89 + 69);
    if ( (v90 & 8) == 0 )
    {
      v117 = 1;
      *(_BYTE *)(v89 + 69) = v90 | 8;
    }
  }
  v28 = 0LL;
  v29 = 0LL;
  while ( 1 )
  {
    v30 = 0xFFFFF6FB7DBED000uLL;
    v107 = v29;
    if ( v29 >= v4 )
    {
      v70 = 273;
      goto LABEL_175;
    }
    v31 = *(_QWORD *)v9;
    if ( v9 >= 0xFFFFF6FB7DBED000uLL && v9 <= 0xFFFFF6FB7DBED7F8uLL && (v31 & 1) != 0 )
    {
      if ( ((v31 & 0x42) == 0 || (v31 & 0x20) == 0) && (MiFlags & 0x600000) != 0 )
      {
        v75 = KeGetCurrentThread()->ApcState.Process;
        if ( v75->AddressPolicy != 1 )
        {
          KernelWaitTime = v75[2].KernelWaitTime;
          if ( KernelWaitTime )
          {
            v77 = *(_QWORD *)(KernelWaitTime + 8 * ((v9 >> 3) & 0x1FF));
            if ( (v77 & 0x20) != 0 )
              v31 |= 0x20uLL;
            v78 = v31;
            v31 |= 0x42uLL;
            if ( (v77 & 0x42) == 0 )
              v31 = v78;
          }
        }
      }
      v29 = v107;
    }
    if ( *(_DWORD *)(a1 + 36) )
    {
      if ( (v31 & 0x400) == 0 && (v31 & 0x4000000) != 0 && !v29 )
      {
        v94 = MI_READ_PTE_LOCK_FREE((unsigned __int64)&v104);
        MiWriteValidPteNewProtection((ULONG_PTR)&v104, v94 | 0x40);
      }
    }
    else if ( !v31 )
    {
      v31 = 32LL * (*(_DWORD *)(a1 + 32) & 0x1F);
      v103 = v17 + 1;
      if ( qword_140E2DB80 )
      {
        if ( (qword_140E2DB80 & v31) != 0 )
          v31 |= 0x10uLL;
        else
          v31 |= qword_140E2DB80;
      }
      v111 = v31;
      v54 = v31;
      v55 = 0;
      if ( (unsigned int)MiPteInShadowRange(v9) )
      {
        v98 = MiSanitizeShadowPxe(v56, &v111);
        v54 = v111;
        v55 = v98;
      }
      *(_QWORD *)v9 = v54;
      if ( v55 )
        MiWritePteShadow(v9, v54);
    }
    v32 = *v5;
    if ( *v5 )
    {
      v33 = v5[2]-- == 1;
      *v5 = *(_QWORD *)v32;
      if ( v33 )
        v5[1] = 0LL;
    }
    v34 = 0xAAAAAAAAAAAAAAABuLL * ((__int64)(v32 + 0x220000000000LL) >> 4);
    if ( *(_DWORD *)(a1 + 36) && !*(_QWORD *)(a1 + 72) && (v31 & 0x400) != 0 )
    {
      v72 = qword_140E2DB80;
      v73 = v31;
      if ( qword_140E2DB80 && (v31 & 0x10) == 0 )
        v73 = v31 & ~qword_140E2DB80;
      if ( v73 >> 16 )
      {
        v97 = 0;
        while ( _interlockedbittestandset64((volatile signed __int32 *)(v32 + 24), 0x3FuLL) )
        {
          do
          {
            if ( (++v97 & HvlLongSpinCountMask) == 0
              && (HvlEnlightenments & 0x40) != 0
              && (unsigned __int8)KiCheckVpBackingLongSpinWaitHypercall(v72) )
            {
              HvlNotifyLongSpinWait(v97);
            }
            else
            {
              _mm_pause();
            }
          }
          while ( *(__int64 *)(v32 + 24) < 0 );
        }
        MiSetPfnIdentity(v32, 4LL);
        _InterlockedAnd64((volatile signed __int64 *)(v32 + 24), 0x7FFFFFFFFFFFFFFFuLL);
      }
      else
      {
        v31 = (v31 & 0x3E0) != 0 ? v31 & 0xFFFFFFFFFFFFFBFFuLL : 0LL;
      }
      v74 = MiMapPageInHyperSpaceWorker(v34, 0LL, 0x80000000LL, v4);
      memset64((void *)v74, v31 & 0xFFFFFFFFFFFFFFFDuLL, 0x200uLL);
      v35 = v121 | 8;
      *(_QWORD *)KeGetCurrentPrcb()->MmInternal = 0LL;
      *(_QWORD *)(((v74 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL) = CLFS_LSN_NULL_EXT;
    }
    else
    {
      v35 = v121;
    }
    v36 = v35 | 0x20;
    if ( (*(_BYTE *)a1 & 2) == 0 )
      v36 = v35;
    v121 = v36;
    MiInitializePfn(v32, v9, *(unsigned int *)(a1 + 32), v36);
    if ( v112 )
    {
      v81 = (_QWORD *)(*(_QWORD *)(v112 + 8) + 16LL * *(_QWORD *)(v112 + 24));
      if ( (__int64)(v9 << 25) >> 16 == *v81 + (*(_QWORD *)(v112 + 32) << 12) && *(_BYTE *)v112 != 5 )
        MiAdvanceFaultList(v112, v81, v37, v38);
    }
    v39 = v34 & 0xFFFFFFFFFFLL;
    if ( (*(_DWORD *)a1 & 0x10) == 0 )
      break;
    v113 = MiSwizzleInvalidPte(32 * ((v39 << 7) | *(_DWORD *)(a1 + 32) & 0x1F | 0x40));
    v85 = 0;
    v86 = v113;
    if ( (unsigned int)MiPteInShadowRange(v9) )
    {
      v99 = MiSanitizeShadowPxe(v87, &v113);
      v86 = v113;
      v85 = v99;
    }
    *(_QWORD *)v9 = v86;
    if ( v85 )
      MiWritePteShadow(v9, v86);
    if ( (*(_DWORD *)a1 & 0x100) != 0 )
      _InterlockedIncrement64((volatile signed __int64 *)Process[1].Padding);
    v88 = 0;
    while ( _interlockedbittestandset64((volatile signed __int32 *)(v32 + 24), 0x3FuLL) )
    {
      do
      {
        if ( (++v88 & HvlLongSpinCountMask) == 0
          && (HvlEnlightenments & 0x40) != 0
          && (unsigned __int8)KiCheckVpBackingLongSpinWaitHypercall(v87) )
        {
          HvlNotifyLongSpinWait(v88);
        }
        else
        {
          _mm_pause();
        }
      }
      while ( *(__int64 *)(v32 + 24) < 0 );
    }
    MiDecrementShareCountEx(v32, 0LL);
    _InterlockedAnd64((volatile signed __int64 *)(v32 + 24), 0x7FFFFFFFFFFFFFFFuLL);
LABEL_82:
    v9 += 8LL;
    v4 = a3;
    v29 = v107 + 1;
    v17 = v103;
    v28 = 0LL;
  }
  v40 = v104 & 0xFFF0000000000FFFuLL | (v39 << 12);
  v41 = v104 & 0xFFF0000000000FDFuLL | (v39 << 12);
  v42 = v40 | 0x20;
  if ( (*(_DWORD *)a1 & 2) == 0 )
    v41 = v42;
  v104 = v41;
  if ( v9 > 0xFFFFF6BFFFFFFF78uLL || v9 < 0xFFFFF68000000000uLL )
    v41 = v104;
  if ( *(_QWORD *)(a1 + 72) )
  {
    if ( _bittest64(&MiFlags, 0x24u) && (v41 & 0x20) == 0 && v9 >= 0xFFFFF6C000000000uLL )
      MiCheckLinearProtectedPteAccessedBit(v9, v41);
    *(_QWORD *)v9 = v41;
    goto LABEL_82;
  }
  if ( v9 != v114 && (*(_DWORD *)a1 & 0x200) != 0 )
  {
    v41 &= ~0x20uLL;
    v104 = v41;
    if ( v108 < 0
      && v9 <= 0xFFFFF6BFFFFFFF78uLL
      && v9 >= 0xFFFFF68000000000uLL
      && (Process[1].DirectoryTableBase & 0x800000000000LL) != 0 )
    {
      if ( v105 )
      {
        if ( v105 != 2 )
          goto LABEL_65;
      }
      else
      {
        if ( v115 )
        {
          v96 = v115;
        }
        else
        {
          MiLockVadTree(1LL);
          Address = MiLocateAddress(*(_QWORD *)(a1 + 16));
          LOBYTE(v101) = 17;
          v96 = Address;
          MiUnlockVadTree(1LL, v101);
          v41 = v104;
        }
        if ( (*(_DWORD *)(v96 + 48) & 0x600000) != 0x600000 )
        {
          v105 = 1;
          goto LABEL_65;
        }
        v105 = 2;
      }
      v41 &= 0xFFFFFFFFFFFFFFBDuLL;
      v104 = v41;
    }
  }
LABEL_65:
  v43 = *(_QWORD *)(a1 + 8);
  v44 = *(_BYTE *)(v43 + 69);
  v45 = (__int64 *)(v43 + 56);
  if ( (v44 & 9) != 8 || (v44 & 4) != 0 )
  {
    v51 = v102 & 0xFFFFFFFB;
    goto LABEL_78;
  }
  v46 = *(unsigned __int16 *)(v43 + 66);
  if ( !(_WORD)v46 )
    goto LABEL_74;
  if ( *(unsigned __int16 *)(v43 + 64) + v46 != (((unsigned int)v9 >> 3) & 0x1FF) )
  {
LABEL_138:
    MiEmptyDeferredWorkingSetEntries(v43 + 56, v43, 0xFFFFF68000000000uLL);
    goto LABEL_74;
  }
  if ( *(__int64 *)(v32 + 40) < 0 )
  {
    if ( (*(_BYTE *)(v43 + 69) & 2) == 0 )
      goto LABEL_74;
    goto LABEL_138;
  }
  if ( (*(_BYTE *)(v43 + 69) & 2) == 0 )
  {
    v47 = *v45;
    v48 = ((unsigned __int64)*(unsigned __int16 *)(v43 + 64) << 12)
        + ((__int64)(*(_QWORD *)(v43 + 72) << 25) >> 16 << 25 >> 16);
    if ( v48 < 0xFFFFF68000000000uLL || v48 > 0xFFFFF6FFFFFFFFFFuLL )
    {
      v49 = 5LL;
    }
    else
    {
      v49 = 4LL;
      if ( (*(_DWORD *)(v47 + 184) & 0xF) != 0 )
        v49 = 0LL;
    }
    MiAddWorkingSetEntries(v47, v48, *((unsigned __int16 *)v45 + 5), v49);
    *((_WORD *)v45 + 5) = 0;
  }
LABEL_74:
  v50 = *((_WORD *)v45 + 5);
  if ( v50 )
  {
    *((_WORD *)v45 + 5) = v50 + 1;
  }
  else
  {
    *((_WORD *)v45 + 5) = 1;
    *((_WORD *)v45 + 4) = ((unsigned int)v9 >> 3) & 0x1FF;
    if ( *(__int64 *)(v32 + 40) >= 0 )
      *((_BYTE *)v45 + 13) |= 2u;
    else
      *((_BYTE *)v45 + 13) &= ~2u;
  }
  v43 = *(_QWORD *)(a1 + 8);
  v51 = v102 | 4;
  if ( (*(_BYTE *)(v43 + 69) & 0x10) == 0
    || (v95 = *(_QWORD *)(v43 + 16), (v95 & 1) != 0) && *(_BYTE *)(v95 & 0xFFFFFFFFFFFFFFFEuLL) == 2 )
  {
    v41 = v104;
  }
  else
  {
    v41 = v104 & 0xFFFFFFFFFFFFFFDFuLL;
    v104 &= ~0x20uLL;
  }
LABEL_78:
  v52 = v51 | 0x10;
  if ( (*(_BYTE *)(v43 + 69) & 1) == 0 )
    v52 = v51;
  v102 = v52;
  if ( (unsigned int)MiAllocateWsle(v109, v9, v32, 0, v41, v52, v116) )
  {
    ++v106;
    goto LABEL_82;
  }
  v57 = 48 * (*(_QWORD *)(v32 + 40) & 0xFFFFFFFFFFLL) - 0x220000000000LL;
  v58 = 0;
  while ( _interlockedbittestandset64((volatile signed __int32 *)(v32 + 24), 0x3FuLL) )
  {
    do
    {
      if ( (++v58 & HvlLongSpinCountMask) == 0
        && (HvlEnlightenments & 0x40) != 0
        && (unsigned __int8)KiCheckVpBackingLongSpinWaitHypercall(v53) )
      {
        HvlNotifyLongSpinWait(v58);
      }
      else
      {
        _mm_pause();
      }
    }
    while ( *(__int64 *)(v32 + 24) < 0 );
  }
  v109 = *(_QWORD *)(v32 + 16);
  if ( (v109 & 4) != 0 )
  {
    *(_QWORD *)(v32 + 16) &= ~4uLL;
    MiClearPageFileReservation(&v109);
  }
  else
  {
    v109 = 0LL;
  }
  *(_QWORD *)(v32 + 16) &= ~2uLL;
  *(_QWORD *)(v32 + 24) |= 0x4000000000000000uLL;
  if ( (*(_BYTE *)(v32 + 34) & 7) != 6 )
    MiBadShareCount(v32);
  v59 = (*(_QWORD *)(v32 + 24) & 0x3FFFFFFFFFFFFFFFLL) - 1;
  v60 = v59 ^ (*(_QWORD *)(v32 + 24) ^ v59) & 0xC000000000000000uLL;
  *(_QWORD *)(v32 + 24) = v60;
  if ( !v59 )
  {
    v120 = *(_DWORD *)(v32 + 32);
    if ( *(__int64 *)(v32 + 40) < 0 )
    {
      v61 = *(_QWORD *)(v32 + 16) >> 5;
      v62 = ((unsigned int)*(_QWORD *)(v32 + 8) >> 3) & 0x1FF;
      v63 = MiMapPageInHyperSpaceWorker(*(_QWORD *)(v32 + 40) & 0xFFFFFFFFFFLL, 0LL, 0x80000000LL, 0xFFFFLL);
      BugCheckParameter4 = *(_QWORD *)(v63 + 8 * v62);
      v65 = (__int64 *)(v63 + 8 * v62);
      v66 = (BugCheckParameter4 >> 12) & 0xFFFFFFFFFFLL;
      if ( 48 * v66 - 0x220000000000LL != v32 )
        KeBugCheckEx(
          0x4Eu,
          6uLL,
          0xAAAAAAAAAAAAAAABuLL * ((__int64)(v32 + 0x220000000000LL) >> 4),
          (ULONG_PTR)v65,
          BugCheckParameter4);
      v67 = 32 * ((v66 << 7) | v61 & 0x1F | 0x40);
      if ( qword_140E2DB80 )
      {
        if ( (qword_140E2DB80 & v67) != 0 )
          v67 |= 0x10uLL;
        else
          v67 |= qword_140E2DB80;
      }
      *v65 = v67;
      v5 = a2;
      *(_QWORD *)KeGetCurrentPrcb()->MmInternal = 0LL;
      *(_QWORD *)(((v63 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL) = CLFS_LSN_NULL_EXT;
    }
    v60 = (*(_QWORD *)(v32 + 24) & 0x4000000000000000LL) != 0;
    LOWORD(v120) = v120 - 1;
    if ( (_WORD)v120 )
    {
      if ( (*(_QWORD *)(v32 + 24) & 0x4000000000000000LL) != 0 )
      {
        BYTE2(v120) |= 7u;
      }
      else
      {
        if ( (v120 & 0x100000) != 0 )
        {
          v92 = BYTE2(v120) & 0xF8 | 3;
        }
        else if ( (v120 & 0x80000) != 0 )
        {
          v92 = BYTE2(v120) & 0xF8 | 3;
        }
        else
        {
          v92 = BYTE2(v120) & 0xF8 | 2;
        }
        BYTE2(v120) = v92;
      }
      *(_DWORD *)(v32 + 32) = v120;
      if ( (*(_DWORD *)(v32 + 16) & 0x400LL) == 0 && (unsigned int)MiGetPagingFileOffset(*(_QWORD *)(v32 + 16)) )
      {
        v60 = (unsigned __int64)qword_140E2FF88;
        *(_BYTE *)(*((_QWORD *)qword_140E2FF88 + ((*(_QWORD *)(v32 + 40) >> 43) & 0x3FFLL)) + 723LL) = v93;
      }
    }
    else if ( (*(_QWORD *)(v32 + 24) & 0x4000000000000000LL) != 0 )
    {
      if ( (v120 & 0x10000000) != 0 )
        HIBYTE(v120) &= ~0x10u;
      *(_DWORD *)(v32 + 32) = v120;
      v68 = *((_QWORD *)qword_140E2FF88 + ((*(_QWORD *)(v32 + 40) >> 43) & 0x3FFLL));
      v112 = *(_QWORD *)(v32 + 16);
      if ( (v112 & 0x400) == 0 && ((v112 & 4) != 0 || (v112 & 2) != 0) && v112 )
        MiReleasePageFileInfo(v68, v112, 1LL);
      MiInsertPageInFreeOrZeroedList(0xAAAAAAAAAAAAAAABuLL * ((__int64)(v32 + 0x220000000000LL) >> 4));
    }
    else
    {
      v79 = MI_READ_PTE_LOCK_FREE(v32 + 16);
      v80 = -9LL;
      if ( (v79 & 0x400) != 0 )
        v80 = -2049LL;
      *(_QWORD *)(v32 + 16) = v79 & v80;
      if ( (v120 & 0x100000) == 0 )
        BYTE2(v120) = BYTE2(v120) & 0xF8 | 2;
      *(_DWORD *)(v32 + 32) = v120;
      MiInsertPageInList(v32);
    }
  }
  _InterlockedAnd64((volatile signed __int64 *)(v32 + 24), 0x7FFFFFFFFFFFFFFFuLL);
  v69 = 0;
  while ( _interlockedbittestandset64((volatile signed __int32 *)(v57 + 24), 0x3FuLL) )
  {
    do
    {
      if ( (++v69 & HvlLongSpinCountMask) == 0
        && (HvlEnlightenments & 0x40) != 0
        && (unsigned __int8)KiCheckVpBackingLongSpinWaitHypercall(v60) )
      {
        HvlNotifyLongSpinWait(v69);
      }
      else
      {
        _mm_pause();
      }
    }
    while ( *(__int64 *)(v57 + 24) < 0 );
  }
  MiDecrementShareCountEx(v57, 0LL);
  _InterlockedAnd64((volatile signed __int64 *)(v57 + 24), 0x7FFFFFFFFFFFFFFFuLL);
  v70 = -1073741801;
  while ( 1 )
  {
    v71 = (ULONG_PTR *)*v5;
    if ( !*v5 )
      break;
    v33 = v5[2]-- == 1;
    *v5 = *v71;
    if ( v33 )
      v5[1] = 0LL;
    MiReleaseFreshPage(v71);
  }
  if ( (v52 & 4) != 0 )
    --*(_WORD *)(*(_QWORD *)(a1 + 8) + 66LL);
LABEL_175:
  if ( v117 )
  {
    v91 = *(_QWORD *)(a1 + 8);
    if ( (*(_BYTE *)(v91 + 69) & 8) != 0 && *(_WORD *)(v91 + 66) )
      MiEmptyDeferredWorkingSetEntries(v91 + 56, v30, v28);
    *(_BYTE *)(*(_QWORD *)(a1 + 8) + 69LL) &= ~8u;
  }
  if ( !*(_DWORD *)(a1 + 36) && v103 )
    MiUpdatePageTableUseCount(*(_QWORD *)(a1 + 16));
  v83 = v106;
  if ( v106 )
  {
    if ( (*(_DWORD *)a1 & 0x100) != 0 )
      _InterlockedAdd64((volatile signed __int64 *)Process[1].Padding, v106);
    __addgsdword(0x2E9Cu, v83);
  }
  return v70;
}
