/*
 * XREFs of sub_180047B20 @ 0x180047B20
 * Callers:
 *     sub_1800484E0 @ 0x1800484E0 (sub_1800484E0.c)
 *     sub_180078380 @ 0x180078380 (sub_180078380.c)
 *     sub_180085380 @ 0x180085380 (sub_180085380.c)
 *     sub_180085630 @ 0x180085630 (sub_180085630.c)
 * Callees:
 *     __security_check_cookie @ 0x18000B680 (__security_check_cookie.c)
 *     sub_180011B5C @ 0x180011B5C (sub_180011B5C.c)
 *     sub_180017054 @ 0x180017054 (sub_180017054.c)
 *     sub_1800686D4 @ 0x1800686D4 (sub_1800686D4.c)
 */

_QWORD *__fastcall sub_180047B20(_QWORD *a1, __int64 a2, __int64 a3, __int64 a4)
{
  int v7; // eax
  int v8; // r8d
  _QWORD v10[5]; // [rsp+38h] [rbp-40h] BYREF

  v10[4] = a2;
  v7 = sub_180017054((__int64)v10, a2);
  sub_1800686D4((_DWORD)a1, v7, v8, -2147467263, a4, 0);
  *a1 = &Spectre::Engine::EngineNotImplException::`vftable';
  sub_180011B5C(a2);
  return a1;
}
