/*
 * XREFs of __local_stdio_printf_options @ 0x18001FDA4
 * Callers:
 *     initialize_legacy_wide_specifiers @ 0x18001FDC0 (initialize_legacy_wide_specifiers.c)
 *     initialize_msvcrt_compatibility @ 0x18001FE00 (initialize_msvcrt_compatibility.c)
 *     __scrt_initialize_default_local_stdio_options @ 0x180020D08 (__scrt_initialize_default_local_stdio_options.c)
 *     _vsnwprintf @ 0x1800212B4 (_vsnwprintf.c)
 *     vswprintf_s @ 0x18002131C (vswprintf_s.c)
 *     _vscwprintf @ 0x180021380 (_vscwprintf.c)
 * Callees:
 *     <none>
 */

unsigned __int64 *__cdecl _local_stdio_printf_options()
{
  return (unsigned __int64 *)&`__local_stdio_printf_options'::`2'::_OptionsStorage;
}
