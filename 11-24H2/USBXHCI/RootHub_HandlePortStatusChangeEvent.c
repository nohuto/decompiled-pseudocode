/*
 * XREFs of RootHub_HandlePortStatusChangeEvent @ 0x140009CC0
 * Callers:
...
__int64 __fastcall RootHub_HandlePortStatusChangeEvent(_QWORD *a1, __int64 a2)
{
  ...
  if ( RootHub_HideInvalidDebugPortStatusAndChange((__int64)a1, v2) )
  {
    if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
    {
      LOBYTE(v8) = 4;
  ...
  else
  {
    ...
    if ( !(_BYTE)result )
    {
      ...
      return ((__int64 (__fastcall *)(__int64, _QWORD))qword_14006BD90)(UcxDriverGlobals, *a1);
    }
  }
  return result;
}
