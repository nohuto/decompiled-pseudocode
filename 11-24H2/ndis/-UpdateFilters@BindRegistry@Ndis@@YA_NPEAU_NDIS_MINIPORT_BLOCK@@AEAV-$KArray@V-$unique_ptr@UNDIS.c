/*
 * XREFs of ?UpdateFilters@BindRegistry@Ndis@@YA_NPEAU_NDIS_MINIPORT_BLOCK@@AEAV?$KArray@V?$unique_ptr@UNDIS_BIND_FILTER_LINK@@U?$default_delete@UNDIS_BIND_FILTER_LINK@@@wistd@@@wistd@@$00@Rtl@@1W4Flags@ReadBindingsOptions@2@@Z @ 0x14015E690
 * Callers:
 *     ?Reload@BindRegistry@Ndis@@YAJPEAU_NDIS_MINIPORT_BLOCK@@W4Flags@ReadBindingsOptions@2@@Z @ 0x140160850 (-Reload@BindRegistry@Ndis@@YAJPEAU_NDIS_MINIPORT_BLOCK@@W4Flags@ReadBindingsOptions@2@@Z.c)
 * Callees:
 *     ?reserve@?$KArray@V?$unique_ptr@UNDIS_BIND_FILTER_LINK@@U?$default_delete@UNDIS_BIND_FILTER_LINK@@@wistd@@@wistd@@$00@Rtl@@QEAA_N_K@Z @ 0x14006CE70 (-reserve@-$KArray@V-$unique_ptr@UNDIS_BIND_FILTER_LINK@@U-$default_delete@UNDIS_BIND_FILTER_LINK.c)
 *     Ndis::BindRegistry::UpdateFiltersInner @ 0x14015D670 (Ndis--BindRegistry--UpdateFiltersInner.c)
 */

char __fastcall Ndis::BindRegistry::UpdateFilters(__int64 a1, unsigned int *a2, __int64 a3, unsigned int a4)
{
  char result; // al
  __int64 v9; // rax
  _QWORD v10[4]; // [rsp+30h] [rbp-48h] BYREF
  __int64 v11[4]; // [rsp+50h] [rbp-28h] BYREF
  __int64 v12; // [rsp+90h] [rbp+18h] BYREF

  result = Rtl::KArray<wistd::unique_ptr<NDIS_BIND_FILTER_LINK,wistd::default_delete<NDIS_BIND_FILTER_LINK>>,1>::reserve(
             a2,
             *(unsigned int *)(a3 + 4));
  if ( result )
  {
    v10[2] = *(unsigned int *)(a3 + 4);
    v9 = a2[1];
    v12 = 0LL;
    v10[1] = 0LL;
    v11[1] = 0LL;
    v11[2] = v9;
    v10[0] = a3;
    v11[0] = (__int64)a2;
    return Ndis::BindRegistry::UpdateFiltersInner(a1, v11, v10, &v12, 1u, a4);
  }
  return result;
}
