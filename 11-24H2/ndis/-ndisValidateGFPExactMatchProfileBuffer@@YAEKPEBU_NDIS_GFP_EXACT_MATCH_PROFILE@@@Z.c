/*
 * XREFs of ?ndisValidateGFPExactMatchProfileBuffer@@YAEKPEBU_NDIS_GFP_EXACT_MATCH_PROFILE@@@Z @ 0x1400A4A10
 * Callers:
...
unsigned __int8 __fastcall ndisValidateGFPExactMatchProfileBuffer(
        unsigned int a1,
        const struct _NDIS_GFP_EXACT_MATCH_PROFILE *a2)
{
  char v3; // bl
  ...
  if ( !a2->Flags && a2->TableType == NdisGfpTableTypePacketDirect )
  {
    if ( a2->NumSupportedEntries )
    {
      ...
      {
        for ( i = 0; i < HeaderGroupExactMatchProfileArrayNumElements; ++i )
        {
          v9 = HeaderGroupExactMatchProfileArrayOffset + i * HeaderGroupExactMatchProfileArrayElementSize;
          v10 = v7 + v9;
          if ( a1 - v9 < 0x18
            || *(_WORD *)(v10 + 2) < 0x18u
            || *(_BYTE *)v10 != 0x80
