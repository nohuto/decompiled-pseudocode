/*
 * XREFs of ?ndisSetFilterDefaultParameter@@YAHPEBU_UNICODE_STRING@@PEAX0E@Z @ 0x14009A558
 * Callers:
    ...
    if ( !Value )
    {
      ...
      if ( !DestinationString.Buffer )
      {
        if ( String2.Buffer )
        {
          for ( i = 0; i < 6; ++i )
          {
            if ( !RtlCompareUnicodeString((PCUNICODE_STRING)((char *)&unk_1400F4A50 + 24 * i), &String2, 1u) )
            {
              v14 = (const WCHAR *)&word_1400F5D40;
              if ( (unsigned int)(*((_DWORD *)&unk_1400F4A50 + 6 * i + 4) - 4) <= 1 )
                goto LABEL_18;
              break;
            }
          }
