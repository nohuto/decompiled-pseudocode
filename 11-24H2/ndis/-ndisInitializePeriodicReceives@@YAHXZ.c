/*
 * XREFs of ?ndisInitializePeriodicReceives@@YAHXZ @ 0x14018D7A4
 * Callers:
...
__int64 ndisInitializePeriodicReceives(void)
{
  ...
  ndisPerCpuPoisonPills = (PVOID)ExAllocatePool2(64LL, 32LL * ndisMaxNumberOfProcessors, 538985550LL);
  if ( !ndisPerCpuPoisonPills )
    goto LABEL_2;
  qword_14011B508 = (PVOID)ExAllocatePool2(64LL, 8LL * ndisMaxNumberOfProcessors, 538985550LL);
  if ( !qword_14011B508 )
    goto LABEL_2;
  qword_14011B500 = (PVOID)ExAllocatePool2(66LL, (unsigned __int64)ndisMaxNumberOfProcessors << 6, 538985550LL);
  if ( !qword_14011B500 )
    goto LABEL_2;
  v1 = ndisMaxNumberOfProcessors;
  for ( i = 0; i < ndisMaxNumberOfProcessors; ++i )
  {
    v3 = (char *)qword_14011B500 + 64 * (unsigned __int64)i;
    *((_QWORD *)v3 + 1) = v3;
    *(_QWORD *)v3 = v3;
  ...
  qword_14011B4F8 = (PVOID)ExAllocatePool2(64LL, 4LL * v1, 538985550LL);
  if ( !qword_14011B4F8 )
    goto LABEL_2;
  qword_14011B4F0 = (PVOID)ExAllocatePool2(66LL, (unsigned __int64)ndisMaxNumberOfProcessors << 7, 538985550LL);
  if ( !qword_14011B4F0 )
    goto LABEL_2;
  for ( j = 0; j < ndisNumberOfActiveProcessorsAtBoot; ++j )
  {
    ...
    NdisInitializeTimer(
      (PNDIS_TIMER)((char *)qword_14011B4F0 + v5),
      (PNDIS_TIMER_FUNCTION)ndisPeriodicReceivesTimer,
      0LL);
    KeSetTargetProcessorDpcEx((PKDPC)((char *)qword_14011B4F0 + v5 + 64), &ProcNumber);
  }
