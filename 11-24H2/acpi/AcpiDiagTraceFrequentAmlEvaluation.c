/*
 * XREFs of AcpiDiagTraceFrequentAmlEvaluation @ 0x140012B90
 * Callers:
...
void __fastcall AcpiDiagTraceFrequentAmlEvaluation(__int64 a1)
{
  ...
  if ( WPP_MAIN_CB.Queue.Wcb.DeviceContext
    && EtwEventEnabled((REGHANDLE)WPP_MAIN_CB.Queue.Wcb.DeviceContext, &ACPI_ETW_EVENT_FREQUENT_AML_METHOD) )
  {
    ...
    if ( v6 >= 0x3C )
    {
      ...
      UserData.Size = 2;
      v14 = 8;
      EtwWrite((REGHANDLE)WPP_MAIN_CB.Queue.Wcb.DeviceContext, &ACPI_ETW_EVENT_FREQUENT_AML_METHOD, 0LL, 3u, &UserData);
    }
  }
}
