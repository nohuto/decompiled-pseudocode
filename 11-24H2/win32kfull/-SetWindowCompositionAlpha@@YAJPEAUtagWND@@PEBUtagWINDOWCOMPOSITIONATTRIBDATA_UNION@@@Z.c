/*
 * XREFs of ?SetWindowCompositionAlpha@@YAJPEAUtagWND@@PEBUtagWINDOWCOMPOSITIONATTRIBDATA_UNION@@@Z @ 0x140085F90
 * Callers:
...
__int64 __fastcall SetWindowCompositionAlpha(struct tagWND *a1, const struct tagWINDOWCOMPOSITIONATTRIBDATA_UNION *a2)
{
  unsigned int v4; // ebx
  int v5; // ecx
  ...
  {
    v5 = *(_DWORD *)a2 != 0;
    if ( -__CFSHR__(v7[0], 20) == v5 )
    {
      return 0;
    }
    else
    {
      LODWORD(v7[0]) = v7[0] & 0xFFF7FFFF | (v5 << 19);
      if ( (unsigned int)SetWindowCompositionInfo(a1, (const struct WINDOWCOMPOSITIONINFO *)v7) )
        return (unsigned int)_DwmAsyncSetCompositionAttribute(a1, 36LL, a2);
      else
