/*
 * XREFs of ?FillCaptionGradient@@YAXPEAUHDC__@@PEBUtagRECT@@H@Z @ 0x14007D008
 * Callers:
 *     xxxDrawCaptionTemp @ 0x14007B5FC (xxxDrawCaptionTemp.c)
 * Callees:
 *     FillRect @ 0x140038250 (FillRect.c)
 *     ?FillGradient@@YAXPEAUHDC__@@PEBUtagRECT@@KK@Z @ 0x14007D0F8 (-FillGradient@@YAXPEAUHDC__@@PEBUtagRECT@@KK@Z.c)
 */

void __fastcall FillCaptionGradient(HDC a1, LPCRECT a2, int a3)
{
  unsigned int v6; // edi
  __int64 v7; // rdx
  __int64 v8; // rcx
  __int64 v9; // rdx
  __int64 v10; // rcx
  unsigned int v11; // r9d
  __int64 v12; // rcx
  __int64 v13; // rdx
  __int64 v14; // rcx
  HBRUSH v15; // r8

  if ( a3 )
  {
    v6 = *(_DWORD *)(*(_QWORD *)(W32GetUserSessionState(a1, a2) + 19928) + 4576LL);
    v11 = *(_DWORD *)(*(_QWORD *)(W32GetUserSessionState(v8, v7) + 19928) + 4676LL);
  }
  else
  {
    v12 = *(_QWORD *)(W32GetUserSessionState(a1, a2) + 19928);
    v6 = *(_DWORD *)(v12 + 4580);
    v10 = *(_QWORD *)(W32GetUserSessionState(v12, v13) + 19928);
    v11 = *(_DWORD *)(v10 + 4680);
  }
  if ( v6 == v11 )
  {
    v14 = *(_QWORD *)(W32GetUserSessionState(v10, v9) + 19928);
    if ( a3 )
      v15 = *(HBRUSH *)(v14 + 4712);
    else
      v15 = *(HBRUSH *)(v14 + 4720);
    FillRect(a1, a2, v15);
  }
  else
  {
    FillGradient(a1, a2, v6, v11);
  }
}
