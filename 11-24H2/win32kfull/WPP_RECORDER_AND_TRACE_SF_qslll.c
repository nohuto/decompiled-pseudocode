/*
 * XREFs of WPP_RECORDER_AND_TRACE_SF_qslll @ 0x1402E8888
 * Callers:
 *     xxxShowScrollBar @ 0x1401C8C38 (xxxShowScrollBar.c)
 * Callees:
 *     _guard_dispatch_icall @ 0x140340330 (_guard_dispatch_icall.c)
 */

_UNKNOWN **WPP_RECORDER_AND_TRACE_SF_qslll(
        __int64 a1,
        char a2,
        char a3,
        __int64 a4,
        __int64 a5,
        __int64 a6,
        __int64 a7,
        __int64 a8,
        ...)
{
  _UNKNOWN **result; // rax
  const char *v9; // rdi
  __int64 v10; // rbx
  __int64 v14; // rax
  __int64 v15; // rax
  const char *v16; // rcx
  __int64 v17; // rbx
  int v18; // [rsp+20h] [rbp-68h]
  _UNKNOWN *retaddr; // [rsp+88h] [rbp+0h] BYREF
  __int64 v20; // [rsp+D0h] [rbp+48h] BYREF
  va_list va; // [rsp+D0h] [rbp+48h]
  const char *v22; // [rsp+D8h] [rbp+50h]
  __int64 v23; // [rsp+E0h] [rbp+58h] BYREF
  va_list va1; // [rsp+E0h] [rbp+58h]
  __int64 v25; // [rsp+E8h] [rbp+60h] BYREF
  va_list va2; // [rsp+E8h] [rbp+60h]
  va_list va3; // [rsp+F0h] [rbp+68h] BYREF

  va_start(va3, a8);
  va_start(va2, a8);
  va_start(va1, a8);
  va_start(va, a8);
  v20 = va_arg(va1, _QWORD);
  v22 = va_arg(va1, const char *);
  va_copy(va2, va1);
  v23 = va_arg(va2, _QWORD);
  va_copy(va3, va2);
  v25 = va_arg(va3, _QWORD);
  result = &retaddr;
  v9 = v22;
  v10 = -1LL;
  if ( a2 )
  {
    if ( v22 )
    {
      v14 = -1LL;
      do
        ++v14;
      while ( v22[v14] );
      v15 = v14 + 1;
    }
    else
    {
      v15 = 5LL;
    }
    v16 = v22;
    if ( !v22 )
      v16 = "NULL";
    result = (_UNKNOWN **)((__int64 (__fastcall *)(__int64, __int64, void *, __int64, __int64 *, __int64, const char *, __int64, __int64 *, __int64, __int64 *, __int64, char *, __int64, _QWORD))pfnWppTraceMessage)(
                            a1,
                            43LL,
                            &WPP_4c358395f58a3c1ed8fe148dd469427a_Traceguids,
                            10LL,
                            (__int64 *)va,
                            8LL,
                            v16,
                            v15,
                            (__int64 *)va1,
                            4LL,
                            (__int64 *)va2,
                            4LL,
                            va3,
                            4LL,
                            0LL);
  }
  if ( a3 )
  {
    if ( v9 )
    {
      do
        ++v10;
      while ( v9[v10] );
      v17 = v10 + 1;
    }
    else
    {
      v17 = 5LL;
    }
    if ( !v9 )
      v9 = "NULL";
    LOWORD(v18) = 10;
    return (_UNKNOWN **)WppAutoLogTrace(
                          a4,
                          4LL,
                          9LL,
                          &WPP_4c358395f58a3c1ed8fe148dd469427a_Traceguids,
                          v18,
                          (__int64 *)va,
                          8LL,
                          v9,
                          v17,
                          (__int64 *)va1,
                          4LL,
                          (__int64 *)va2);
  }
  return result;
}
