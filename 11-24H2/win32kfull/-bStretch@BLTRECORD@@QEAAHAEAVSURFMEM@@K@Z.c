/*
 * XREFs of ?bStretch@BLTRECORD@@QEAAHAEAVSURFMEM@@K@Z @ 0x140305FC8
 * Callers:
 *     ?bStretch@BLTRECORD@@QEAAHAEAVXDCOBJ@@AEAVPROXYDCOBJ@1@KE@Z @ 0x1400D2508 (-bStretch@BLTRECORD@@QEAAHAEAVXDCOBJ@@AEAVPROXYDCOBJ@1@KE@Z.c)
 * Callees:
 *     ?pSurfMskOut@BLTRECORD@@QEAAPEAVSURFACE@@XZ @ 0x1400D4898 (-pSurfMskOut@BLTRECORD@@QEAAPEAVSURFACE@@XZ.c)
 *     EngStretchBlt @ 0x140136200 (EngStretchBlt.c)
 *     ?vMirror@BLTRECORD@@QEAAXPEAVERECTL@@@Z @ 0x1401A7798 (-vMirror@BLTRECORD@@QEAAXPEAVERECTL@@@Z.c)
 *     ?vAltUnlockFast@SURFACE@@QEAAXXZ @ 0x1401DD2B8 (-vAltUnlockFast@SURFACE@@QEAAXXZ.c)
 *     __security_check_cookie @ 0x140340250 (__security_check_cookie.c)
 */

__int64 __fastcall BLTRECORD::bStretch(BLTRECORD *this, struct SURFMEM *a2, ULONG a3)
{
  int v6; // edx
  int v7; // ecx
  LONG v8; // edx
  LONG v9; // r8d
  struct SURFACE *v10; // rax
  __int64 v11; // r8
  SURFACE *v12; // rcx
  __int64 result; // rax
  POINTL pptlHTOrg; // [rsp+60h] [rbp-9h] BYREF
  int v15; // [rsp+68h] [rbp-1h] BYREF
  __int64 v16; // [rsp+6Ch] [rbp+3h]
  int v17; // [rsp+74h] [rbp+Bh]
  __int64 v18; // [rsp+78h] [rbp+Fh]
  int v19; // [rsp+80h] [rbp+17h]
  int v20; // [rsp+84h] [rbp+1Bh]
  RECTL prclDest; // [rsp+88h] [rbp+1Fh] BYREF

  v15 = 1;
  v16 = 0LL;
  v6 = *((_DWORD *)this + 35) - *((_DWORD *)this + 33);
  LODWORD(v16) = *((_DWORD *)this + 34) - *((_DWORD *)this + 32);
  HIDWORD(v16) = v6;
  v17 = 0;
  v20 = 0;
  v18 = 0LL;
  v7 = *((_DWORD *)BLTRECORD::pSurfMskOut(this) + 28) & 0x40000;
  prclDest.bottom = v8;
  v19 = v7;
  *(_QWORD *)&prclDest.left = 0LL;
  prclDest.right = v9;
  BLTRECORD::vMirror(this, (struct ERECTL *)&prclDest);
  SURFMEM::bCreateDIB(a2, (struct _DEVBITMAPINFO *)&v15, 0LL, 0LL, 0, 0LL, 0LL, 0, 1, 0, 0);
  if ( !*(_QWORD *)a2 )
    return 0LL;
  pptlHTOrg = 0LL;
  v10 = BLTRECORD::pSurfMskOut(this);
  if ( !EngStretchBlt(
          (SURFOBJ *)(v11 + 24),
          (SURFOBJ *)(((unsigned __int64)v10 + 24) & -(__int64)(v10 != 0LL)),
          0LL,
          0LL,
          0LL,
          0LL,
          &pptlHTOrg,
          &prclDest,
          (RECTL *)((char *)this + 168),
          0LL,
          a3) )
    return 0LL;
  *((_DWORD *)this + 49) &= ~0x20000u;
  v12 = (SURFACE *)*((_QWORD *)this + 8);
  *((_DWORD *)this + 42) = 0;
  *((_DWORD *)this + 43) = 0;
  SURFACE::vAltUnlockFast(v12);
  result = 1LL;
  *((_QWORD *)this + 8) = *(_QWORD *)a2;
  return result;
}
