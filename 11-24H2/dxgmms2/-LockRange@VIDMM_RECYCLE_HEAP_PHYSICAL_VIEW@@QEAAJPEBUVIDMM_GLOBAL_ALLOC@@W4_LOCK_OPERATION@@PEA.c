/*
 * XREFs of ?LockRange@VIDMM_RECYCLE_HEAP_PHYSICAL_VIEW@@QEAAJPEBUVIDMM_GLOBAL_ALLOC@@W4_LOCK_OPERATION@@PEAVVIDMM_SEGMENT@@_K3@Z @ 0x1400C4B48
 * Callers:
 *     ?Lock@VIDMM_RECYCLE_RANGE@@QEAAJPEBUVIDMM_GLOBAL_ALLOC@@W4_LOCK_OPERATION@@PEAVVIDMM_SEGMENT@@@Z @ 0x1400C5724 (-Lock@VIDMM_RECYCLE_RANGE@@QEAAJPEBUVIDMM_GLOBAL_ALLOC@@W4_LOCK_OPERATION@@PEAVVIDMM_SEGMENT@@@Z.c)
 * Callees:
 *     DxgkLogInternalTriageEvent @ 0x140004FE8 (DxgkLogInternalTriageEvent.c)
 *     ??2@YAPEAX_KIW4DXGK_POOL_FLAGS@@@Z @ 0x14001C724 (--2@YAPEAX_KIW4DXGK_POOL_FLAGS@@@Z.c)
 *     ??3@YAXPEAX@Z @ 0x14001D2D0 (--3@YAXPEAX@Z.c)
 *     ?VidMmRecycleHeapMapSection@@YAJPEAX_K1EPEAPEAX2EPEAE@Z @ 0x1400BFDAC (-VidMmRecycleHeapMapSection@@YAJPEAX_K1EPEAPEAX2EPEAE@Z.c)
 *     ?VidMmUnmapViewAsync@@YAXPEAU_EPROCESS@@PEAX1@Z @ 0x1400C12C0 (-VidMmUnmapViewAsync@@YAXPEAU_EPROCESS@@PEAX1@Z.c)
 *     ?Lock@VIDMM_MDL_RANGE@@QEAAJPEAXW4_LOCK_OPERATION@@PEAVVIDMM_SEGMENT@@PEBUVIDMM_GLOBAL_ALLOC@@@Z @ 0x1400C4E60 (-Lock@VIDMM_MDL_RANGE@@QEAAJPEAXW4_LOCK_OPERATION@@PEAVVIDMM_SEGMENT@@PEBUVIDMM_GLOBAL_ALLOC@@@Z.c)
 *     ?VidMmUnmapView@@YAXPEAU_EPROCESS@@PEAX@Z @ 0x1400CC6A8 (-VidMmUnmapView@@YAXPEAU_EPROCESS@@PEAX@Z.c)
 */

__int64 __fastcall VIDMM_RECYCLE_HEAP_PHYSICAL_VIEW::LockRange(
        VIDMM_RECYCLE_HEAP_PHYSICAL_VIEW *this,
        const struct VIDMM_GLOBAL_ALLOC *a2,
        enum _LOCK_OPERATION a3,
        struct VIDMM_SEGMENT *a4,
        _BYTE *a5,
        _BYTE *a6)
{
  _BYTE *v6; // rsi
  void *v7; // rdi
  VIDMM_RECYCLE_HEAP_PHYSICAL_VIEW *v8; // r8
  unsigned __int64 v10; // r15
  char v11; // bp
  int v12; // r10d
  char *v13; // r13
  char *v14; // r12
  _QWORD *v15; // rax
  enum _LOCK_OPERATION v16; // r8d
  __int64 v17; // r9
  _QWORD *v18; // rbx
  void *v19; // rsi
  __int64 v20; // rdx
  int v21; // ecx
  int v22; // edi
  __int64 v23; // r8
  __int64 v24; // r9
  _DWORD *v25; // rdx
  __int64 v26; // rcx
  _QWORD *v27; // rdx
  __int64 v28; // rcx
  __int64 result; // rax
  __int64 v30; // rcx
  int v31; // eax
  __int64 v32; // rsi
  struct _EPROCESS *CurrentProcess; // r9
  int v34; // ecx
  enum _LOCK_OPERATION v36; // [rsp+A0h] [rbp+18h] BYREF
  struct VIDMM_SEGMENT *v37; // [rsp+A8h] [rbp+20h]

  v37 = a4;
  v36 = a3;
  v6 = a6;
  v7 = a5;
  v8 = (VIDMM_RECYCLE_HEAP_PHYSICAL_VIEW *)*((_QWORD *)this + 1);
  v10 = a6 - a5;
  v11 = 0;
  v12 = -1;
  v13 = 0LL;
  v14 = 0LL;
  while ( v8 != (VIDMM_RECYCLE_HEAP_PHYSICAL_VIEW *)((char *)this + 8) && v12 < 0 )
  {
    if ( *((_QWORD *)v8 - 1) > (unsigned __int64)a5 )
    {
      if ( *((_QWORD *)v8 - 2) < (unsigned __int64)a6 )
      {
        v12 = 0;
      }
      else
      {
        v12 = 1;
        v14 = (char *)v8 - 24;
      }
    }
    else
    {
      v12 = -1;
      v13 = (char *)v8 - 24;
    }
    v8 = *(VIDMM_RECYCLE_HEAP_PHYSICAL_VIEW **)v8;
  }
  v15 = (_QWORD *)operator new(48LL, 0x32316956u, 258LL);
  v18 = v15;
  if ( !v15 )
  {
    _InterlockedIncrement(&dword_14008178C);
    WdLogSingleEntry0(6LL);
    WdLogGlobalForLineNumber = 6042;
    DxgkLogInternalTriageEvent(v30, 262145LL);
    return 3221225495LL;
  }
  v15[2] = v6;
  v19 = 0LL;
  *v15 = 0LL;
  v15[1] = v7;
  v15[5] = this;
  v15[3] = 0LL;
  v15[4] = 0LL;
  v20 = *(_QWORD *)this;
  a6 = 0LL;
  a5 = 0LL;
  LOBYTE(v36) = 0;
  v21 = **(_DWORD **)(v20 + 32);
  if ( (unsigned int)(v21 - 3) <= 3 && (unsigned int)(v21 - 5) > 1 )
  {
    LOBYTE(v17) = v21 == 4;
    v31 = VidMmRecycleHeapMapSection(
            *(PVOID *)(v20 + 56),
            (__int64)v7,
            v10,
            v17,
            (void **)&a6,
            (void **)&a5,
            0,
            (unsigned __int8 *)&v36);
    v32 = v31;
    if ( v31 < 0 )
    {
      operator delete(v18);
      WdLogSingleEntry5(
        3LL,
        v10,
        v7,
        *(_QWORD *)this,
        v32,
        **(_QWORD **)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)this + 32LL) + 8LL) + 8LL));
      result = (unsigned int)v32;
      WdLogGlobalForLineNumber = 6068;
      return result;
    }
    v7 = a6;
    v19 = a5;
    v11 = v36;
  }
  v22 = VIDMM_MDL_RANGE::Lock((VIDMM_MDL_RANGE *)v18, v7, v16, v37, a2);
  v25 = *(_DWORD **)(*(_QWORD *)this + 32LL);
  if ( (unsigned int)(*v25 - 3) <= 3 )
  {
    v26 = (unsigned int)(*v25 - 5);
    if ( (unsigned int)v26 > 1 )
    {
      if ( v11 )
        CurrentProcess = 0LL;
      else
        CurrentProcess = (struct _EPROCESS *)PsGetCurrentProcess(v26, v25, v23, v24);
      v34 = dword_140081504;
      if ( (unsigned __int64)qword_1400812F8 > 0x53333333 )
        v34 = dword_1400814F4;
      if ( v10 <= (unsigned int)(v34 << 20) )
        VidMmUnmapView(CurrentProcess, v19);
      else
        VidMmUnmapViewAsync(CurrentProcess, *(PVOID *)(*(_QWORD *)this + 56LL), v19);
    }
  }
  if ( v22 < 0 )
  {
    operator delete(v18);
  }
  else
  {
    if ( v13 )
      v27 = v13 + 24;
    else
      v27 = (_QWORD *)(v18[5] + 8LL);
    if ( v14 )
      v28 = (__int64)(v14 + 24);
    else
      v28 = v18[5] + 8LL;
    *v27 = v18 + 3;
    *(_QWORD *)(v28 + 8) = v18 + 3;
    v18[4] = v27;
    v18[3] = v28;
  }
  return (unsigned int)v22;
}
