/*
 * XREFs of VidSchCreateProcess @ 0x1400FE760
 * Callers:
 *     <none>
 * Callees:
 *     DxgkLogInternalTriageEvent @ 0x140004FE8 (DxgkLogInternalTriageEvent.c)
 *     ?GetMaximumGlobalAdapterCount@DXGGLOBAL@@QEBAKXZ @ 0x1400359E0 (-GetMaximumGlobalAdapterCount@DXGGLOBAL@@QEBAKXZ.c)
 *     ?GetGlobal@DXGGLOBAL@@SAPEAV1@XZ @ 0x140035A00 (-GetGlobal@DXGGLOBAL@@SAPEAV1@XZ.c)
 *     ?GetProcessID@DXGPROCESS@@QEBAQEAXXZ @ 0x140046080 (-GetProcessID@DXGPROCESS@@QEBAQEAXXZ.c)
 */

__int64 __fastcall VidSchCreateProcess(DXGPROCESS *this)
{
  __int64 Pool2; // rax
  __int64 v3; // rbx
  DXGGLOBAL *Global; // rax
  unsigned int MaximumGlobalAdapterCount; // ebp
  __int64 v6; // rax
  __int64 v7; // rax
  __int64 v8; // rax
  __int64 v9; // rax
  __int64 v10; // rax
  void *v12; // rcx
  __int64 v13; // rcx

  if ( this )
  {
    Pool2 = ExAllocatePool2(64LL, 2896LL, 845244758LL);
    v3 = Pool2;
    if ( Pool2 )
    {
      *(_DWORD *)Pool2 = 845244758;
      *(_QWORD *)(Pool2 + 2664) = 0LL;
      *(_QWORD *)(Pool2 + 8) = this;
      *(_QWORD *)(Pool2 + 16) = *((_QWORD *)this + 8);
      *(_DWORD *)(Pool2 + 24) = (*((_DWORD *)this + 102) & 2) != 0 ? 5 : 2;
      TdrHistoryInit((struct _TDR_HISTORY *)(Pool2 + 40));
      Global = DXGGLOBAL::GetGlobal();
      MaximumGlobalAdapterCount = DXGGLOBAL::GetMaximumGlobalAdapterCount(Global);
      v6 = ExAllocatePool2(256LL, 4 * ((unsigned __int64)(MaximumGlobalAdapterCount + 31) >> 5), 845244758LL);
      *(_QWORD *)(v3 + 2624) = v6;
      if ( v6 )
      {
        v7 = ExAllocatePool2(64LL, 8LL * MaximumGlobalAdapterCount, 845244758LL);
        *(_QWORD *)(v3 + 32) = v7;
        if ( v7 )
        {
          *(_QWORD *)(v3 + 2640) = DXGPROCESS::GetProcessID(this);
          *(_QWORD *)(v3 + 2656) = *((_QWORD *)this + 7);
          v8 = *((_QWORD *)this + 8);
          if ( v8 )
            v9 = *(_QWORD *)(v8 + 96);
          else
            v9 = 0LL;
          *(_QWORD *)(v3 + 2648) = v9;
          if ( (*((_DWORD *)this + 102) & 2) != 0 )
            g_pVidSchSystemProcess = v3;
          *(_WORD *)(v3 + 2882) = 260;
          v10 = ExAllocatePool2(64LL, 260LL, 845244758LL);
          *(_QWORD *)(v3 + 2888) = v10;
          *(_WORD *)(v3 + 2880) = 0;
          if ( v10 )
            return v3;
          WdLogSingleEntry0(3LL);
          v12 = *(void **)(v3 + 32);
          WdLogGlobalForLineNumber = 3434;
          ExFreePoolWithTag(v12, 0);
        }
        else
        {
          WdLogSingleEntry0(3LL);
          WdLogGlobalForLineNumber = 3400;
        }
        ExFreePoolWithTag(*(PVOID *)(v3 + 2624), 0);
      }
      else
      {
        WdLogSingleEntry0(3LL);
        WdLogGlobalForLineNumber = 3388;
      }
      ExFreePoolWithTag((PVOID)v3, 0);
    }
    else
    {
      WdLogSingleEntry0(3LL);
      WdLogGlobalForLineNumber = 3344;
    }
  }
  else
  {
    WdLogSingleEntry0(1LL);
    WdLogGlobalForLineNumber = 3331;
    DxgkLogInternalTriageEvent(v13, 0x40000LL);
  }
  return 0LL;
}
