/*
 * XREFs of ?FindAvailableMonitoredValueStorageSlot@VIDMM_PROCESS_FENCE_STORAGE@@AEAA_NPEAUVIDMM_MONITORED_FENCE_STORAGE@@@Z @ 0x14004D0CC
 * Callers:
 *     ?AllocateFenceStorageSlot@VIDMM_PROCESS_FENCE_STORAGE@@QEAAJPEAUVIDMM_MONITORED_FENCE_STORAGE@@@Z @ 0x14001C3A4 (-AllocateFenceStorageSlot@VIDMM_PROCESS_FENCE_STORAGE@@QEAAJPEAUVIDMM_MONITORED_FENCE_STORAGE@@@.c)
 * Callees:
 *     ?AssignFenceStorageSlot@VIDMM_FENCE_STORAGE_PAGE@@QEAAXPEAUVIDMM_MONITORED_FENCE_STORAGE@@@Z @ 0x14001C6C8 (-AssignFenceStorageSlot@VIDMM_FENCE_STORAGE_PAGE@@QEAAXPEAUVIDMM_MONITORED_FENCE_STORAGE@@@Z.c)
 */

char __fastcall VIDMM_PROCESS_FENCE_STORAGE::FindAvailableMonitoredValueStorageSlot(
        VIDMM_PROCESS_FENCE_STORAGE *this,
        struct VIDMM_MONITORED_FENCE_STORAGE *a2)
{
  VIDMM_PROCESS_FENCE_STORAGE *v4; // rbx
  char v5; // bl
  VIDMM_PROCESS_FENCE_STORAGE *v6; // rax
  VIDMM_PROCESS_FENCE_STORAGE **v7; // rcx
  VIDMM_PROCESS_FENCE_STORAGE **v8; // rax
  __int64 v9; // rcx
  struct _KLOCK_QUEUE_HANDLE LockHandle; // [rsp+20h] [rbp-28h] BYREF

  memset(&LockHandle, 0, sizeof(LockHandle));
  KeAcquireInStackQueuedSpinLock((PKSPIN_LOCK)this + 15, &LockHandle);
  v4 = (VIDMM_PROCESS_FENCE_STORAGE *)*((_QWORD *)this + 11);
  if ( v4 == (VIDMM_PROCESS_FENCE_STORAGE *)((char *)this + 88) )
  {
    v5 = 0;
  }
  else
  {
    VIDMM_FENCE_STORAGE_PAGE::AssignFenceStorageSlot(*((VIDMM_FENCE_STORAGE_PAGE **)this + 11), a2);
    if ( *((_QWORD *)v4 + 12) == -1LL )
    {
      v6 = *(VIDMM_PROCESS_FENCE_STORAGE **)v4;
      if ( *(VIDMM_PROCESS_FENCE_STORAGE **)(*(_QWORD *)v4 + 8LL) != v4
        || (v7 = (VIDMM_PROCESS_FENCE_STORAGE **)*((_QWORD *)v4 + 1), *v7 != v4)
        || (*v7 = v6,
            *((_QWORD *)v6 + 1) = v7,
            v8 = (VIDMM_PROCESS_FENCE_STORAGE **)((char *)this + 56),
            v9 = *((_QWORD *)this + 7),
            *(VIDMM_PROCESS_FENCE_STORAGE **)(v9 + 8) != (VIDMM_PROCESS_FENCE_STORAGE *)((char *)this + 56)) )
      {
        __fastfail(3u);
      }
      *(_QWORD *)v4 = v9;
      *((_QWORD *)v4 + 1) = v8;
      *(_QWORD *)(v9 + 8) = v4;
      *v8 = v4;
    }
    v5 = 1;
  }
  KeReleaseInStackQueuedSpinLock(&LockHandle);
  return v5;
}
