/*
 * XREFs of ?VidSchiSubmitNextFlipRange@@YAXPEAVHwQueueStagingList@@PEAU_VIDSCH_GLOBAL@@I@Z @ 0x14000A648
 * Callers:
...
void __fastcall VidSchiSubmitNextFlipRange(struct HwQueueStagingList *a1, struct _VIDSCH_GLOBAL *a2, unsigned int a3)
{
  int v6; // ebx
  __int64 v7; // rbp
  struct VIDSCH_FLIP_QUEUE *FlipQueue; // rsi
  int v9; // eax
  unsigned int i; // edx
...
LABEL_14:
        ...
        if ( v14 )
          VidSchiCompletePendingFlipOnPlane((_DWORD)a1, (_DWORD)a2, a3, (_DWORD)FlipQueue, 9);
        goto LABEL_16;
      }
