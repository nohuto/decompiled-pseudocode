/*
 * XREFs of ?ProcessSetPersistentCache@CExpression@@QEAAJPEAVCResourceTable@@PEBUtagMILCMD_EXPRESSION_SETPERSISTENTCACHE@@PEBXI@Z @ 0x18029AFC0
 * Callers:
        ...
        unsigned int a5)
{
  ...
  else
  {
    if ( *((_QWORD *)this + 42) && a5 >> 4 )
    {
      ...
      do
      {
        Resource = (CSharedSectionBase *)CResourceTable::GetResource((__int64)a2, *(v8 - 1), 0x9Du);
        if ( Resource )
        {
