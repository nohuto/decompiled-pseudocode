/*
 * XREFs of NVMeCompletionQueuesSubmissionQueuesInit @ 0x14001C5D0
 * Callers:
 *     NVMeCleanUpSqCqMapping @ 0x14001BD1C (NVMeCleanUpSqCqMapping.c)
 *     NVMeCompletionQueueInit @ 0x14001C334 (NVMeCompletionQueueInit.c)
 * Callees:
 *     <none>
 */

__int64 __fastcall NVMeCompletionQueuesSubmissionQueuesInit(__int64 a1, __int64 a2, __int64 a3, __int64 a4)
{
  _QWORD *v5; // rcx
  int v6; // eax
  __int16 v7; // r9
  __int64 result; // rax
  int v9; // ecx

  v5 = (_QWORD *)(a2 + 192);
  if ( a4 )
    *v5 = a4;
  *(_QWORD *)(a2 + 180) = 0LL;
  if ( *(_WORD *)(a1 + 332) )
  {
    *(_WORD *)(a2 + 200) = *v5 != 0LL;
    v6 = *(unsigned __int16 *)(a1 + 330);
    if ( (unsigned __int16)v6 >= *(_WORD *)(a1 + 234) )
      v6 = *(unsigned __int16 *)(a1 + 234);
    v7 = (v6 - 1) / *(unsigned __int16 *)(a1 + 332) + 1;
    *(_WORD *)(a2 + 202) = v7;
  }
  else
  {
    v7 = *(_WORD *)(a2 + 202);
  }
  result = 0xFFFFFFFFLL;
  if ( *(_WORD *)(a2 + 200) )
    v9 = *(_DWORD *)(a1 + 4088);
  else
    v9 = -1;
  *(_DWORD *)(a2 + 204) = v9;
  if ( v7 )
    result = *(unsigned int *)(a1 + 4092);
  *(_DWORD *)(a2 + 208) = result;
  return result;
}
