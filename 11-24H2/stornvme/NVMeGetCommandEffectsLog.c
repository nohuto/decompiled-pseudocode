/*
 * XREFs of NVMeGetCommandEffectsLog @ 0x14000B4B0
 * Callers:
 *     NVMeControllerInitPart3 @ 0x14000FB50 (NVMeControllerInitPart3.c)
 *     NVMeControllerIdentifyWorkItem @ 0x1400222F0 (NVMeControllerIdentifyWorkItem.c)
 * Callees:
 *     NVMeAllocateDmaBuffer @ 0x140006160 (NVMeAllocateDmaBuffer.c)
 *     BuildGetLogPageCommand @ 0x1400065E0 (BuildGetLogPageCommand.c)
 *     WaitForCommandComplete @ 0x140008190 (WaitForCommandComplete.c)
 *     ProcessCommand @ 0x140009500 (ProcessCommand.c)
 *     LocalCommandReuse @ 0x140009790 (LocalCommandReuse.c)
 *     memset @ 0x1400314C0 (memset.c)
 */

__int64 __fastcall NVMeGetCommandEffectsLog(__int64 a1, unsigned __int8 a2)
{
  __int64 v2; // rdi
  __int64 result; // rax
  void *v6; // rcx
  unsigned int v7; // r8d
  __int64 v8; // [rsp+60h] [rbp+8h]

  v2 = *(_QWORD *)(a1 + 1104);
  v8 = 0LL;
  if ( !*(_BYTE *)(a1 + 20) )
  {
    result = *(_QWORD *)(a1 + 1560);
    if ( (*(_BYTE *)(result + 261) & 2) != 0 )
    {
      v6 = *(void **)(a1 + 3712);
      if ( v6 )
      {
        result = *(_QWORD *)(a1 + 3944);
        v8 = result;
        if ( !result )
          return result;
      }
      else
      {
        result = NVMeAllocateDmaBuffer(a1, 0x1000u);
        v6 = *(void **)(a1 + 3712);
        if ( !v6 )
          return result;
        *(_QWORD *)(a1 + 3944) = 0LL;
      }
      memset(v6, 0, 0x1000uLL);
      LocalCommandReuse(a1, a1 + 1008);
      *(_BYTE *)(v2 + 4225) |= 3u;
      LOBYTE(v7) = 5;
      *(_WORD *)(v2 + 4212) = 0;
      BuildGetLogPageCommand(a1, v2, v7, 0x1000u, v8, -1, 0LL, 0);
      *(_BYTE *)(v2 + 4225) |= 4u;
      *(_QWORD *)(v2 + 4160) = *(_QWORD *)(a1 + 3712);
      *(_QWORD *)(v2 + 4168) = v8;
      *(_DWORD *)(v2 + 4208) = 4096;
      ProcessCommand(a1, a1 + 1016);
      return WaitForCommandComplete(a1, a1 + 1016, a2);
    }
  }
  return result;
}
