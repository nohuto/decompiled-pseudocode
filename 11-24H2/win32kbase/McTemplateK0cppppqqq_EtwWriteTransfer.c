/*
 * XREFs of McTemplateK0cppppqqq_EtwWriteTransfer @ 0x1400332AC
 * Callers:
 *     EtwTraceRetrievePseudoMessage @ 0x1400A4B30 (EtwTraceRetrievePseudoMessage.c)
 *     EtwTraceRetrievePostMessage @ 0x14016A590 (EtwTraceRetrievePostMessage.c)
 * Callees:
 *     McGenEventWrite_EtwWriteTransfer @ 0x140033530 (McGenEventWrite_EtwWriteTransfer.c)
 *     __security_check_cookie @ 0x14023F3B0 (__security_check_cookie.c)
 */

__int64 __fastcall McTemplateK0cppppqqq_EtwWriteTransfer(
        __int64 a1,
        __int64 a2,
        __int64 a3,
        char a4,
        char a5,
        char a6,
        char a7,
        char a8,
        char a9,
        char a10,
        char a11)
{
  _BYTE v12[16]; // [rsp+30h] [rbp-81h] BYREF
  char *v13; // [rsp+40h] [rbp-71h]
  __int64 v14; // [rsp+48h] [rbp-69h]
  char *v15; // [rsp+50h] [rbp-61h]
  __int64 v16; // [rsp+58h] [rbp-59h]
  char *v17; // [rsp+60h] [rbp-51h]
  __int64 v18; // [rsp+68h] [rbp-49h]
  char *v19; // [rsp+70h] [rbp-41h]
  __int64 v20; // [rsp+78h] [rbp-39h]
  char *v21; // [rsp+80h] [rbp-31h]
  __int64 v22; // [rsp+88h] [rbp-29h]
  char *v23; // [rsp+90h] [rbp-21h]
  __int64 v24; // [rsp+98h] [rbp-19h]
  char *v25; // [rsp+A0h] [rbp-11h]
  __int64 v26; // [rsp+A8h] [rbp-9h]
  char *v27; // [rsp+B0h] [rbp-1h]
  __int64 v28; // [rsp+B8h] [rbp+7h]
  char v29; // [rsp+F8h] [rbp+47h] BYREF

  v29 = a4;
  v14 = 1LL;
  v13 = &v29;
  v16 = 8LL;
  v15 = &a5;
  v18 = 8LL;
  v17 = &a6;
  v19 = &a7;
  v21 = &a8;
  v23 = &a9;
  v25 = &a10;
  v27 = &a11;
  v20 = 8LL;
  v22 = 8LL;
  v24 = 4LL;
  v26 = 4LL;
  v28 = 4LL;
  return McGenEventWrite_EtwWriteTransfer(a1, a2, &W32kControlGuid, 9LL, v12);
}
