/*
 * XREFs of ??4CPointerQFrame@@QEAAAEAU0@AEBU0@@Z @ 0x140126458
 * Callers:
 *     ?CopyPTPPointerInputFrame@CTouchProcessor@@IEAAPEAUCPointerInputFrame@@PEAU2@PEAX@Z @ 0x1400E2610 (-CopyPTPPointerInputFrame@CTouchProcessor@@IEAAPEAUCPointerInputFrame@@PEAU2@PEAX@Z.c)
 * Callees:
 *     ??4CInputDest@@QEAAAEAV0@AEBV0@@Z @ 0x1401F33F8 (--4CInputDest@@QEAAAEAV0@AEBV0@@Z.c)
 */

__int64 __fastcall CPointerQFrame::operator=(__int64 a1, __int64 a2)
{
  unsigned int v4; // ecx
  unsigned int v5; // ecx
  unsigned int v6; // ecx
  unsigned int v7; // ecx

  *(_DWORD *)a1 = *(_DWORD *)a2;
  *(_QWORD *)(a1 + 8) = *(_QWORD *)(a2 + 8);
  CInputDest::operator=(a1 + 16, a2 + 16);
  *(_DWORD *)(a1 + 136) = *(_DWORD *)(a2 + 136);
  v4 = *(_DWORD *)(a2 + 140) ^ (*(_DWORD *)(a1 + 140) ^ *(_DWORD *)(a2 + 140)) & 0xFFFFFFFE;
  *(_DWORD *)(a1 + 140) = v4;
  v5 = *(_DWORD *)(a2 + 140) ^ (*(_DWORD *)(a2 + 140) ^ v4) & 0xFFFFFFFD;
  *(_DWORD *)(a1 + 140) = v5;
  v6 = *(_DWORD *)(a2 + 140) ^ (*(_DWORD *)(a2 + 140) ^ v5) & 0xFFFFFFFB;
  *(_DWORD *)(a1 + 140) = v6;
  v7 = *(_DWORD *)(a2 + 140) ^ (*(_DWORD *)(a2 + 140) ^ v6) & 0xFFFFFFF7;
  *(_DWORD *)(a1 + 140) = v7;
  *(_DWORD *)(a1 + 140) = *(_DWORD *)(a2 + 140) ^ (*(_DWORD *)(a2 + 140) ^ v7) & 0xFFFFFFEF;
  *(_DWORD *)(a1 + 144) = *(_DWORD *)(a2 + 144);
  *(_DWORD *)(a1 + 148) = *(_DWORD *)(a2 + 148);
  *(_QWORD *)(a1 + 152) = *(_QWORD *)(a2 + 152);
  return a1;
}
