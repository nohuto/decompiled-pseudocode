/*
 * XREFs of DrvGetMonitorPhysicalDimensions @ 0x1401CAF00
 * Callers:
 *     RIMGetMonitorPhysicalSize @ 0x1401E1980 (RIMGetMonitorPhysicalSize.c)
 * Callees:
 *     ?UpdateMonitorDevicesOnGraphicsDevice@EnsureMonitorDevices@@AEAAXPEAUtagGRAPHICS_DEVICE@@@Z @ 0x140015CF0 (-UpdateMonitorDevicesOnGraphicsDevice@EnsureMonitorDevices@@AEAAXPEAUtagGRAPHICS_DEVICE@@@Z.c)
 *     ?GetMonitorDevice@EnsureMonitorDevices@@QEBAXKAEAUtagVIDEO_MONITOR_DEVICE@@@Z @ 0x14008FA68 (-GetMonitorDevice@EnsureMonitorDevices@@QEBAXKAEAUtagVIDEO_MONITOR_DEVICE@@@Z.c)
 *     ??1EnsureMonitorDevices@@QEAA@XZ @ 0x14011E230 (--1EnsureMonitorDevices@@QEAA@XZ.c)
 *     __security_check_cookie @ 0x14023F3B0 (__security_check_cookie.c)
 *     _guard_dispatch_icall @ 0x14023F3F0 (_guard_dispatch_icall.c)
 *     memset @ 0x14023F500 (memset.c)
 */

__int64 __fastcall DrvGetMonitorPhysicalDimensions(
        __int64 a1,
        unsigned int *a2,
        unsigned int *a3,
        _DWORD *a4,
        _QWORD *a5)
{
  unsigned int v7; // edi
  __int64 v8; // rax
  struct tagGRAPHICS_DEVICE *v9; // rax
  struct tagGRAPHICS_DEVICE *v10; // rdx
  int v11; // r14d
  unsigned int i; // esi
  __int64 v13; // rbx
  int v14; // r13d
  __int64 v15; // rcx
  __int64 DxgkWin32kInterface; // rax
  int v17; // r8d
  __int128 v19; // [rsp+30h] [rbp-D0h] BYREF
  int v20; // [rsp+40h] [rbp-C0h]
  unsigned int *v21; // [rsp+48h] [rbp-B8h]
  unsigned int *v22; // [rsp+50h] [rbp-B0h]
  char v23[8]; // [rsp+58h] [rbp-A8h] BYREF
  unsigned int v24; // [rsp+60h] [rbp-A0h]
  __int64 v25; // [rsp+68h] [rbp-98h]
  _DWORD v26[2]; // [rsp+70h] [rbp-90h] BYREF
  __int64 v27; // [rsp+78h] [rbp-88h]
  unsigned int v28; // [rsp+80h] [rbp-80h]
  _DWORD v29[231]; // [rsp+84h] [rbp-7Ch] BYREF

  v22 = a3;
  v21 = a2;
  v7 = 0;
  v8 = *(_QWORD *)(W32GetSessionState(a1) + 88);
  if ( !a1 )
    return 3221225473LL;
  v9 = *(struct tagGRAPHICS_DEVICE **)(v8 + 1096);
  v10 = *(struct tagGRAPHICS_DEVICE **)(a1 + 2568);
  if ( !v9 || v9 != v10 )
    return 3221225486LL;
  v24 = 0;
  v25 = 0LL;
  EnsureMonitorDevices::UpdateMonitorDevicesOnGraphicsDevice((EnsureMonitorDevices *)v23, v10);
  v11 = 0;
  for ( i = 0; i < v24; ++i )
  {
    v20 = 0;
    v19 = 0LL;
    EnsureMonitorDevices::GetMonitorDevice((EnsureMonitorDevices *)v23, i, (struct tagVIDEO_MONITOR_DEVICE *)&v19);
    if ( (v19 & 2) != 0 && (v19 & 1) != 0 )
    {
      memset(v29, 0, sizeof(v29));
      v13 = *((_QWORD *)&v19 + 1);
      v14 = DWORD1(v19);
      v27 = *((_QWORD *)&v19 + 1);
      v26[0] = -7;
      v28 = DWORD1(v19);
      v26[1] = 944;
      DxgkWin32kInterface = DxDdGetDxgkWin32kInterface(v15);
      if ( (*(int (__fastcall **)(__int64, _QWORD, _DWORD *, _QWORD, _QWORD))(DxgkWin32kInterface + 264))(
             v27,
             v28,
             v26,
             0LL,
             0LL) >= 0 )
      {
        v11 = 1;
        if ( v29[226] )
        {
          v17 = v29[227];
          if ( v29[227] )
          {
            *v21 = (v29[226] + 9) / 0xAu;
            *v22 = (v17 + 9) / 0xAu;
            if ( a4 )
              *a4 = v14;
            if ( a5 )
              *a5 = v13;
            goto LABEL_18;
          }
        }
      }
    }
  }
  v7 = -1073741810;
  if ( v11 )
    v7 = -1073741823;
LABEL_18:
  EnsureMonitorDevices::~EnsureMonitorDevices((EnsureMonitorDevices *)v23);
  return v7;
}
