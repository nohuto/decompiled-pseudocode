/*
 * XREFs of ?ndisMIndicatePacketsToNetBufferLists@@YAXPEAXPEAPEAU_NDIS_PACKET@@I@Z @ 0x1C00C95C0
 * Callers:
 *     <none>
 * Callees:
 *     ?ndisNblTrackerTransferOwnershipInternal@@YAXPEAU_NET_BUFFER_LIST@@PEAUNDIS_NBL_TRACKER_HANDLE__@@1W4_NDIS_NBL_TRACKER_OWNERSHIP_EVENT@@K@Z @ 0x1C00130F4 (-ndisNblTrackerTransferOwnershipInternal@@YAXPEAU_NET_BUFFER_LIST@@PEAUNDIS_NBL_TRACKER_HANDLE__.c)
 *     NdisMPauseComplete @ 0x1C0016980 (NdisMPauseComplete.c)
 *     ?ndisInvokeNextReceiveHandler@@YAXPEAU_NET_BUFFER_LIST@@KKKPEAU_NDIS_OBJECT_HEADER@@PEAXP6AX20KKK@Z@Z @ 0x1C0036F70 (-ndisInvokeNextReceiveHandler@@YAXPEAU_NET_BUFFER_LIST@@KKKPEAU_NDIS_OBJECT_HEADER@@PEAXP6AX20KK.c)
 *     _guard_dispatch_icall_nop @ 0x1C00381E0 (_guard_dispatch_icall_nop.c)
 *     PktMonClientNblLogNdis @ 0x1C0040E3A (PktMonClientNblLogNdis.c)
 *     ?NDIS_ACQUIRE_MINIPORT_RECV_SPIN_LOCK@@YAXPEAU_NDIS_MINIPORT_BLOCK@@PEAE@Z @ 0x1C0069D14 (-NDIS_ACQUIRE_MINIPORT_RECV_SPIN_LOCK@@YAXPEAU_NDIS_MINIPORT_BLOCK@@PEAE@Z.c)
 *     ?ndisXlateRecvPacketArrayToNetBufferLists@@YAEPEAU_NDIS_PKT_ARRAY_TO_NBLS@@@Z @ 0x1C00735B8 (-ndisXlateRecvPacketArrayToNetBufferLists@@YAEPEAU_NDIS_PKT_ARRAY_TO_NBLS@@@Z.c)
 *     ?ndisXlateReturnNetBufferListToPacket@@YAPEAU_NDIS_PACKET@@PEAU_NET_BUFFER_LIST@@@Z @ 0x1C0073864 (-ndisXlateReturnNetBufferListToPacket@@YAPEAU_NDIS_PACKET@@PEAU_NET_BUFFER_LIST@@@Z.c)
 *     ?NDIS_FREE_XFER_DATA_PACKET@@YAXPEAU_NDIS_PACKET@@@Z @ 0x1C0090330 (-NDIS_FREE_XFER_DATA_PACKET@@YAXPEAU_NDIS_PACKET@@@Z.c)
 */

void __fastcall ndisMIndicatePacketsToNetBufferLists(
        struct _NDIS_MINIPORT_BLOCK *a1,
        struct _NDIS_PACKET **a2,
        unsigned int a3)
{
  int v3; // ebx
  bool v6; // al
  struct _NET_BUFFER_LIST *v7; // rsi
  bool v8; // r13
  unsigned int v9; // r14d
  unsigned int v10; // r12d
  _NDIS_MINIPORT_STATE RecvState; // eax
  unsigned __int64 *p_RecvLock; // rcx
  KIRQL v13; // dl
  __int64 v14; // r8
  int v15; // ebx
  _PKTMON_COMPONENT_CONTEXT *CompContext; // rax
  unsigned int v17; // r13d
  struct _NET_BUFFER_LIST *Alignment; // rbx
  struct _NDIS_PACKET *v19; // rdx
  __int64 NdisPacketOobOffset; // rax
  unsigned int v21; // eax
  bool v22; // zf
  KIRQL v23; // dl
  KIRQL v24; // dl
  struct _NET_BUFFER_LIST *v25; // rbx
  unsigned int v26; // esi
  __int64 v27; // rax
  __int64 v28; // r14
  struct _NDIS_PACKET **v29; // rbx
  struct _NDIS_PACKET *v30; // rcx
  __int64 v31; // rax
  _QWORD v32[2]; // [rsp+40h] [rbp-38h] BYREF
  unsigned int v33; // [rsp+50h] [rbp-28h]
  __int128 v34; // [rsp+54h] [rbp-24h]
  unsigned int v35[2]; // [rsp+64h] [rbp-14h]
  int v36; // [rsp+6Ch] [rbp-Ch]
  bool v37; // [rsp+C0h] [rbp+48h]
  KIRQL NewIrql; // [rsp+D0h] [rbp+58h] BYREF
  int v40; // [rsp+D8h] [rbp+60h]

  v33 = a3;
  v34 = 0uLL;
  v3 = 0;
  *(_QWORD *)v35 = 0LL;
  v36 = 0;
  NewIrql = 0;
  v32[1] = a2;
  v32[0] = a1;
  while ( 1 )
  {
    v6 = ndisXlateRecvPacketArrayToNetBufferLists((struct _NDIS_PKT_ARRAY_TO_NBLS *)v32);
    v7 = *(struct _NET_BUFFER_LIST **)((char *)&v34 + 4);
    v8 = v6;
    v9 = HIDWORD(v34);
    v37 = v6;
    if ( !*(_QWORD *)((char *)&v34 + 4) )
      goto LABEL_28;
    v40 = HIDWORD(v34);
    v10 = HIDWORD(v34) - v3;
    NDIS_ACQUIRE_MINIPORT_RECV_SPIN_LOCK(a1, &NewIrql);
    RecvState = a1->RecvState;
    p_RecvLock = &a1->RecvLock;
    a1->MiniportRecvLockThread = 0LL;
    v13 = NewIrql;
    if ( (unsigned int)(RecvState - 5) <= 1 )
      break;
    a1->OutstandingReceives += v10;
    KeReleaseSpinLock(p_RecvLock, v13);
    v15 = v35[1];
    if ( (a1->Flags & 0x8000) != 0 && (v35[1] & 2) == 0 )
    {
      _InterlockedExchangeAdd((volatile signed __int32 *)&a1->IndicatedPacketsCount, v10);
      v15 = v35[1];
      v9 = HIDWORD(v34);
      v7 = *(struct _NET_BUFFER_LIST **)((char *)&v34 + 4);
    }
    if ( *(_DWORD *)ndisNblTrackerMode )
      ndisNblTrackerTransferOwnershipInternal(
        v7,
        a1->NblTracker,
        (unsigned __int64)a1->Next.IndicateNetBufferListsTracker,
        (v15 & 2 | 0x100u) >> 1,
        0);
    if ( byte_1C00F5390 )
    {
      CompContext = a1->PktMonEdge.CompContext;
      if ( CompContext )
      {
        if ( (*((_DWORD *)CompContext + 14) & 1) != 0 )
          PktMonClientNblLogNdis((__int64)&a1->PktMonEdge, (__int64)v7, v14, 1LL);
      }
    }
    ndisInvokeNextReceiveHandler(
      v7,
      0,
      v35[0],
      v15,
      (struct _NDIS_FILTER_BLOCK *)a1->Next.IndicateNetBufferListsObject,
      a1->Next.IndicateNetBufferListsContext,
      (void (*)(void *, struct _NET_BUFFER_LIST *, unsigned int, unsigned int, unsigned int))a1->Next.IndicateNetBufferListsHandler);
    if ( (v15 & 2) != 0 )
    {
      v17 = a1->Flags & 0x40000;
      if ( *(_DWORD *)ndisNblTrackerMode )
        ndisNblTrackerTransferOwnershipInternal(
          v7,
          a1->Next.IndicateNetBufferListsTracker,
          (unsigned __int64)a1->NblTracker,
          142LL,
          0);
      do
      {
        Alignment = (struct _NET_BUFFER_LIST *)v7->Link.Alignment;
        v19 = ndisXlateReturnNetBufferListToPacket(v7);
        NdisPacketOobOffset = v19->Private.NdisPacketOobOffset;
        if ( *(unsigned int *)((char *)&v19->Private.Count + NdisPacketOobOffset) != -1073741670 )
        {
          if ( v17 )
          {
            if ( (v19[-1].Reserved[1] & 1) != 0 && *(_DWORD *)&v19[-1].ProtocolReserved[4] == -1 )
              NDIS_FREE_XFER_DATA_PACKET(v19);
            else
              ((void (__fastcall *)(void *))a1->MiniportReturnPacketHandler)(a1->MiniportAdapterContext);
          }
          else
          {
            *(unsigned int *)((char *)&v19->Private.Count + NdisPacketOobOffset) = 0;
          }
        }
        v7 = Alignment;
      }
      while ( Alignment );
      NDIS_ACQUIRE_MINIPORT_RECV_SPIN_LOCK(a1, &NewIrql);
      v21 = a1->OutstandingReceives - v10;
      v22 = a1->RecvState == NdisMiniportPausing;
      a1->OutstandingReceives = v21;
      if ( v22 && !v21 )
      {
        v24 = NewIrql;
        a1->RecvState = NdisMiniportPaused;
        a1->MiniportRecvLockThread = 0LL;
        KeReleaseSpinLock(&a1->RecvLock, v24);
        NdisMPauseComplete(a1);
        goto LABEL_34;
      }
      v23 = NewIrql;
      a1->MiniportRecvLockThread = 0LL;
      KeReleaseSpinLock(&a1->RecvLock, v23);
      v8 = v37;
    }
    v3 = v40;
    *(_QWORD *)((char *)&v34 + 4) = 0LL;
LABEL_28:
    if ( !v8 )
      goto LABEL_34;
  }
  KeReleaseSpinLock(p_RecvLock, NewIrql);
  do
  {
    v25 = (struct _NET_BUFFER_LIST *)v7->Link.Alignment;
    ndisXlateReturnNetBufferListToPacket(v7);
    v7 = v25;
  }
  while ( v25 );
  v9 -= v10;
LABEL_34:
  if ( v9 < a3 )
  {
    v26 = a1->Flags & 0x40000;
    v27 = v9;
    v28 = a3 - v9;
    v29 = &a2[v27];
    do
    {
      v30 = *v29++;
      v31 = v30->Private.NdisPacketOobOffset;
      if ( *(unsigned int *)((char *)&v30->Private.Count + v31) != -1073741670 )
      {
        if ( v26 )
        {
          if ( (v30[-1].Reserved[1] & 1) != 0 && *(_DWORD *)&v30[-1].ProtocolReserved[4] == -1 )
            NDIS_FREE_XFER_DATA_PACKET(v30);
          else
            a1->MiniportReturnPacketHandler(a1->MiniportAdapterContext, v30);
        }
        else
        {
          *(unsigned int *)((char *)&v30->Private.Count + v31) = 0;
        }
      }
      --v28;
    }
    while ( v28 );
  }
}
