/*
 * XREFs of NdisSetSessionCompartmentId @ 0x1C00B6E90
 * Callers:
 *     ?ndisNsiSetAllSessionInformation@@YAJPEAU_NM_REQUEST_SET_ALL_PARAMETERS@@@Z @ 0x1C00B6E40 (-ndisNsiSetAllSessionInformation@@YAJPEAU_NM_REQUEST_SET_ALL_PARAMETERS@@@Z.c)
 * Callees:
 ...
 *     ?ndisCmLazyInitializeCleanup@@YAXXZ @ 0x1C00B6CEC (-ndisCmLazyInitializeCleanup@@YAXXZ.c)
...
__int64 __fastcall NdisSetSessionCompartmentId(unsigned int a1, unsigned int a2)
{
  ...
  v7 = KeAcquireSpinLockRaiseToDpc(&qword_1C00F5D78);
  ...
  else
  {
    ...
    if ( !Pool2 )
    {
      KeReleaseSpinLock(&qword_1C00F5D78, v7);
      if ( v5 )
