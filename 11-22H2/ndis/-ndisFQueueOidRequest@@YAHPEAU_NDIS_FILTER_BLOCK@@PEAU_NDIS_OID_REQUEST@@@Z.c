/*
 * XREFs of ?ndisFQueueOidRequest@@YAHPEAU_NDIS_FILTER_BLOCK@@PEAU_NDIS_OID_REQUEST@@@Z @ 0x1C0024960
 * Callers:
...
__int64 __fastcall ndisFQueueOidRequest(struct _NDIS_FILTER_BLOCK *a1, struct _NDIS_OID_REQUEST *a2)
{
  ...
  *(_QWORD *)&a2->NdisReserved[8] = 0LL;
  ...
  if ( (a1->Miniport->PnPFlags & 0x100) == 0 )
  {
    ...
    {
      if ( *(_QWORD *)(v7 + 96) )
        TraceLoggingWriteOidRequestQueuedActivity<0,_NDIS_FILTER_BLOCK>(a1, v7);
      else
        TraceLoggingWriteOidRequestQueuedActivity<1,_NDIS_FILTER_BLOCK>(a1, v7);
    }
