/*
 * XREFs of ?ndisQueryDriverImageName@@YAXPEAU_UNICODE_STRING@@0@Z @ 0x1C00303B8
 * Callers:
...
void __fastcall ndisQueryDriverImageName(struct _UNICODE_STRING *a1, struct _UNICODE_STRING *a2)
{
  ...
  wchar_t *Pool2; // rax
  const WCHAR *v13; // rax
  PVOID P[2]; // [rsp+38h] [rbp-29h] BYREF
  _RTL_QUERY_REGISTRY_TABLE QueryTable[2]; // [rsp+48h] [rbp-19h] BYREF
...
LABEL_11:
  ...
  if ( v4 < v7 )
  {
    ...
    Pool2 = (wchar_t *)ExAllocatePool2(64LL, v11, 1851868238);
    a2->Buffer = Pool2;
    if ( Pool2 )
    {
      v13 = (const WCHAR *)P[1];
