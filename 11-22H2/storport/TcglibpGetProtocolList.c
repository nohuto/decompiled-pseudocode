/*
 * XREFs of TcglibpGetProtocolList @ 0x1C007D704
 * Callers:
 *     TcglibDiscoverDevice @ 0x1C00797CC (TcglibDiscoverDevice.c)
 * Callees:
 *     TcglibEalAllocateIfRecvCommand @ 0x1C0054C10 (TcglibEalAllocateIfRecvCommand.c)
 *     TcglibEalExecuteCommandSync @ 0x1C005515C (TcglibEalExecuteCommandSync.c)
 *     TcglibEalFreeCommand @ 0x1C0055408 (TcglibEalFreeCommand.c)
 *     TcglibEalGetCommandPayload @ 0x1C00554D4 (TcglibEalGetCommandPayload.c)
 *     TcglibEalLogCommand @ 0x1C005551C (TcglibEalLogCommand.c)
 */

__int64 __fastcall TcglibpGetProtocolList(int *a1, _DWORD *a2)
{
  int IfRecvCommand; // ebp
  __int64 v5; // r8
  unsigned int v6; // edx
  unsigned int v7; // r8d
  unsigned __int64 v8; // r10
  _BYTE *v9; // r9
  unsigned int v11; // [rsp+58h] [rbp+10h] BYREF
  unsigned __int64 v12; // [rsp+60h] [rbp+18h] BYREF
  unsigned __int8 *v13; // [rsp+68h] [rbp+20h] BYREF

  v13 = 0LL;
  v12 = 0LL;
  v11 = 0;
  *a2 = 0;
  IfRecvCommand = TcglibEalAllocateIfRecvCommand((__int64)a1, 0, 0, 2048, (__int64 *)&v12);
  if ( IfRecvCommand >= 0 )
  {
    TcglibEalLogCommand(a1, "GetProtocolList", 0, 0, 0LL, 0LL);
    IfRecvCommand = TcglibEalExecuteCommandSync((__int64)a1, v12, v5);
    if ( IfRecvCommand >= 0 )
    {
      TcglibEalGetCommandPayload(a1, v12, &v13, &v11);
      if ( v11 >= 8 )
      {
        v6 = 0;
        v7 = v13[7] + (v13[6] << 8);
        if ( v7 )
        {
          v8 = v11;
          v9 = v13 + 8;
          do
          {
            if ( (int)(v6 + 8) + 1LL > v8 )
              break;
            if ( *v9 )
            {
              if ( *v9 == 1 )
              {
                *a2 |= 2u;
              }
              else if ( *v9 == 2 )
              {
                *a2 |= 4u;
              }
            }
            else
            {
              *a2 |= 1u;
            }
            ++v6;
            ++v9;
          }
          while ( v6 < v7 );
        }
      }
    }
  }
  if ( v12 )
    TcglibEalFreeCommand(a1, v12);
  return (unsigned int)IfRecvCommand;
}
