/*
 * XREFs of RaDriverAddDevice @ 0x1C003F5C0
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall RaDriverAddDevice(PDRIVER_OBJECT DriverObject, PDEVICE_OBJECT DeviceObject)
{
  ...
  Adapter = IoCreateDevice(DriverObject, 48 * RaidLogListSize + 6080, &DeviceName, 4u, 0x100u, 0, &SourceDevice);
  if ( Adapter >= 0 )
  {
    DeviceExtension = SourceDevice->DeviceExtension;
    Adapter = RaidCreateAdapter(DeviceExtension);
    if ( Adapter < 0 )
      goto LABEL_9;
