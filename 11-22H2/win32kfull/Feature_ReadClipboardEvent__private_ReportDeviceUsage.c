/*
 * XREFs of Feature_ReadClipboardEvent__private_ReportDeviceUsage @ 0x1C013C564
 * Callers:
...
void Feature_ReadClipboardEvent__private_ReportDeviceUsage()
{
  __int64 v0; // [rsp+30h] [rbp+8h]

  v0 = (unsigned int)Feature_ReadClipboardEvent__private_featureState;
  if ( (Feature_ReadClipboardEvent__private_featureState & 0x10) == 0 )
  {
    LODWORD(v0) = Feature_ReadClipboardEvent__private_featureState | 1;
    wil_details_FeatureReporting_ReportUsageToService((__int64)&Feature_ReadClipboardEvent__private_descriptor, v0, 3LL);
