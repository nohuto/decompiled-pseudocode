/*
 * XREFs of xxxPerformTargetingWithinPwnd @ 0x1C01E6F4C
 * Callers:
 *     PerformTargetingWithinPwnd @ 0x1C01E6890 (PerformTargetingWithinPwnd.c)
 * Callees:
 *     _GetProp @ 0x1C00F21FC (_GetProp.c)
 *     xxxSendTransformableMessageTimeout @ 0x1C01271B0 (xxxSendTransformableMessageTimeout.c)
 *     __security_check_cookie @ 0x1C0138430 (__security_check_cookie.c)
 *     memset_0 @ 0x1C0141600 (memset_0.c)
 *     TouchTargetingCreateContact @ 0x1C024DF88 (TouchTargetingCreateContact.c)
 */

__int64 __fastcall xxxPerformTargetingWithinPwnd(
        unsigned __int64 *BugCheckParameter2,
        __int64 a2,
        _DWORD *a3,
        __int64 a4)
{
  __int64 result; // rax
  __int128 *v8; // r14
  __int128 v9; // xmm0
  int v10; // eax
  __int64 v11; // rdx
  __int64 v12; // rcx
  unsigned int v13; // edx
  __int128 v14; // [rsp+50h] [rbp-B0h] BYREF
  __int128 v15; // [rsp+60h] [rbp-A0h] BYREF
  int v16; // [rsp+70h] [rbp-90h] BYREF
  __int64 v17; // [rsp+74h] [rbp-8Ch]
  __int128 v18; // [rsp+7Ch] [rbp-84h]
  __int128 v19; // [rsp+8Ch] [rbp-74h]
  int v20; // [rsp+9Ch] [rbp-64h]
  __int128 v21; // [rsp+A0h] [rbp-60h] BYREF
  __int128 v22; // [rsp+B0h] [rbp-50h] BYREF
  _OWORD v23[2]; // [rsp+C0h] [rbp-40h] BYREF
  __int64 v24; // [rsp+E0h] [rbp-20h]
  int v25; // [rsp+E8h] [rbp-18h]
  int v26; // [rsp+170h] [rbp+70h]
  int v27; // [rsp+174h] [rbp+74h]
  int v28; // [rsp+178h] [rbp+78h]
  int v29; // [rsp+17Ch] [rbp+7Ch]

  *a3 = 0;
  a3[1] = 0;
  result = GetProp((__int64)BugCheckParameter2, (unsigned __int16)gatomPtrTargetFlags, 1LL, a4);
  if ( (result & 0xFFFD) != 0 )
  {
    result = TouchTargetingEnabledForInput(*(_QWORD *)(a2 + 40), a2 + 8);
    if ( (_DWORD)result )
    {
      v21 = 0LL;
      v22 = 0LL;
      v8 = 0LL;
      v18 = 0LL;
      v19 = 0LL;
      memset_0(v23, 0, 0xB8uLL);
      v29 = 0;
      v28 = 0;
      if ( (unsigned int)_GetDeviceRects(a2 + 8, &v21, &v22) )
      {
        v14 = v22;
        v15 = v21;
        _SetHimetricToPixelRatio(&v15, &v14, v23);
        v8 = &v21;
      }
      v9 = *(_OWORD *)(a2 + 112);
      v16 = *(_DWORD *)(a2 + 12);
      v17 = *(_QWORD *)(a2 + 40);
      v10 = *(_DWORD *)(a2 + 108);
      v18 = v9;
      v19 = v9;
      if ( (v10 & 2) != 0 )
        v20 = *(_DWORD *)(a2 + 144);
      else
        v20 = 0;
      result = TouchTargetingCreateContact(&v16, v8, 1LL, v23);
      if ( v26 )
      {
        if ( v27 )
        {
          v17 = v24;
          v20 = v25;
          v18 = v23[1];
          v19 = v23[0];
          _InterlockedIncrement(&glSendMessage);
          v11 = xxxSendTransformableMessageTimeout(BugCheckParameter2, 0x24Du, 0LL, (__int128 *)&v16, 0, 0, 0LL, 1, 0);
          result = 4093LL;
          if ( ((unsigned __int16)(v11 >> 20) & 0xFFFu) <= 0xFFD )
          {
            v12 = (v11 >> 10) & 0x3FF;
            *a3 = v12;
            if ( (unsigned int)v12 > 0x1FF )
            {
              result = (unsigned int)(v12 - 1024);
              *a3 = result;
            }
            v13 = v11 & 0x3FF;
            a3[1] = v13;
            if ( v13 > 0x1FF )
            {
              result = v13 - 1024;
              a3[1] = result;
            }
          }
        }
      }
    }
  }
  return result;
}
