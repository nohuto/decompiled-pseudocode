/*
 * XREFs of ?OnPeerConnected@DiagnosticCallbacksManager@@MEAAJPEAVBamoPeer@DiagnosticCallbacks_AutoBamos@@PEA_NPEAPEAVBamoPrincipal@Bamo@Microsoft@@@Z @ 0x1801058B0
 * Callers:
 *     <none>
 * Callees:
 *     <none>
 */

__int64 __fastcall DiagnosticCallbacksManager::OnPeerConnected(
        DiagnosticCallbacksManager *this,
        struct DiagnosticCallbacks_AutoBamos::BamoPeer *a2,
        bool *a3,
        struct Microsoft::Bamo::BamoPrincipal **a4)
{
  __int64 result; // rax

  *a3 = 1;
  result = 0LL;
  *a4 = 0LL;
  return result;
}
