/*
 * XREFs of Spectre::Framework::ValidateTopologyIndexCount @ 0x1800E341C
 * Callers:
...
__int64 __fastcall Spectre::Framework::ValidateTopologyIndexCount(unsigned int a1, int a2)
{
  __int64 result; // rax
  ...
  if ( a2 )
  {
    if ( a2 == 1 && a1 < 3 )
    {
      std::string::string(v3, (__int64)"Number of indices is less than three");
  ...
  else
  {
    ...
    if ( a1 != (_DWORD)result )
    {
      std::string::string(v3, (__int64)"Number of indices is not a multiple of three");
      Spectre::Framework::GeometryProcessingException::GeometryProcessingException(pExceptionObject, (const char *)v3);
      throw (Spectre::Framework::GeometryProcessingException *)pExceptionObject;
    }
  }
