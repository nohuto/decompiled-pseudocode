/*
 * XREFs of ?SetNormals@Mesh@Engine@Spectre@@QEAAXQEBUVector3@Math@Utils@3@_N@Z @ 0x180049628
 * Callers:
 ...
 *     std::_Func_impl_no_alloc__lambda_7334b5a9a3bbefa9324c75fdace13832__void_std::shared_ptr_Spectre::Utils::Math::Vector3__const_&_void___::_Do_call @ 0x180049E20 (std--_Func_impl_no_alloc__lambda_7334b5a9a3bbefa9324c75fdace13832__void_std--shared_ea_180049E20.c)
...
{
  const struct Spectre::Utils::Math::Vector3 *v2; // rdx
  unsigned int *v3; // rcx
  bool v4; // r8
  unsigned int v5; // eax
  int v6; // r8d
  ...
  if ( Spectre::Engine::Mesh::IsMapped(this) )
  {
    std::string::string(
      v10,
      (__int64)"Mesh::SetNormals() -- Modifying mesh buffers apart from MappedResource while it is mapped is not supported.");
  ...
  if ( !v2 )
  {
    std::string::string(
      v9,
      (__int64)"Mesh::SetNormals() -- normalArray must be non-null and contain N elements, where N=Mesh::GetVertexCount()");
  ...
  Spectre::Engine::Mesh::SetNormalsInternal((Spectre::Engine::Mesh *)v3, v2, v3[30], v4);
}
