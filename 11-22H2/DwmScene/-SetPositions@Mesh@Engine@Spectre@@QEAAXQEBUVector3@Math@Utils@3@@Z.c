/*
 * XREFs of ?SetPositions@Mesh@Engine@Spectre@@QEAAXQEBUVector3@Math@Utils@3@@Z @ 0x180049778
 * Callers:
        ...
        const struct Spectre::Utils::Math::Vector3 *const a2)
{
  ...
  if ( Spectre::Engine::Mesh::IsMapped(this) )
  {
    std::string::string(
      v9,
      (__int64)"Mesh::SetPositions() -- Modifying mesh buffers apart from MappedResource while it is mapped is not supported.");
  ...
  if ( !v2 )
  {
    std::string::string(
      v8,
      (__int64)"Mesh::SetPositions() -- positionArray must be non-null and contain N elements, where N=Mesh::GetVertexCount()");
  ...
  Spectre::Engine::Mesh::SetPositionsInternal((Spectre::Engine::Mesh *)v3, v2, v3[30]);
}
