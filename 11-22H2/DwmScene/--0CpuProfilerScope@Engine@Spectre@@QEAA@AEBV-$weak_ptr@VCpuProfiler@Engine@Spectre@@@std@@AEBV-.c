/*
 * XREFs of ??0CpuProfilerScope@Engine@Spectre@@QEAA@AEBV?$weak_ptr@VCpuProfiler@Engine@Spectre@@@std@@AEBV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@4@@Z @ 0x180030F54
 * Callers:
...
__int64 *__fastcall Spectre::Engine::CpuProfilerScope::CpuProfilerScope(__int64 *a1, __int64 a2)
{
  Spectre::Engine::Mutex *v4; // [rsp+20h] [rbp-18h] BYREF
  std::_Ref_count_base *v5; // [rsp+28h] [rbp-10h]

  *a1 = 0LL;
  a1[1] = 0LL;
  a1[2] = 0LL;
  std::weak_ptr<Spectre::Engine::GpuProfiler>::lock(a2, &v4);
  if ( v4 )
  {
    a1[2] = Spectre::Engine::CpuProfiler::BeginTimeSpanMarker(v4);
    std::weak_ptr<Spectre::Engine::Scene>::operator=<Spectre::Engine::Scene>(a1, &v4);
  }
  if ( v5 )
