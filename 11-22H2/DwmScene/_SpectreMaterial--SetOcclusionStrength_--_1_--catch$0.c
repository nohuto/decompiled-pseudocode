/*
 * XREFs of _SpectreMaterial::SetOcclusionStrength_::_1_::catch$0 @ 0x1800E3F36
 * Callers:
 *     <none>
 * Callees:
 *     ?Return_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z @ 0x18000F024 (-Return_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z.c)
 */

__int64 __fastcall SpectreMaterial::SetOcclusionStrength_::_1_::catch_0(__int64 a1, __int64 a2)
{
  wil::details::in1diag3::Return_Hr(
    *(wil::details::in1diag3 **)(a2 + 40),
    (void *)0xB5,
    (__int64)"onecoreuap\\windows\\dwm\\spectre\\dwmscene\\source\\spectrematerial.cpp",
    (const char *)0x80004005LL);
  return 0LL;
}
