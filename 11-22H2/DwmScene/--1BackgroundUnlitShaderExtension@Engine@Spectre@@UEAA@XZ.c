/*
 * XREFs of ??1BackgroundUnlitShaderExtension@Engine@Spectre@@UEAA@XZ @ 0x18007FDD0
 * Callers:
 *     ??_GBackgroundUnlitShaderExtension@Engine@Spectre@@UEAAPEAXI@Z @ 0x18007FE10 (--_GBackgroundUnlitShaderExtension@Engine@Spectre@@UEAAPEAXI@Z.c)
 * Callees:
 *     ??1?$unique_ptr@ULightConstants@Engine@Spectre@@V?$function@$$A6AXPEAULightConstants@Engine@Spectre@@@Z@std@@@std@@QEAA@XZ @ 0x18007CB94 (--1-$unique_ptr@ULightConstants@Engine@Spectre@@V-$function@$$A6AXPEAULightConstants@Engine@Spec.c)
 */

void __fastcall Spectre::Engine::BackgroundUnlitShaderExtension::~BackgroundUnlitShaderExtension(
        Spectre::Engine::BackgroundUnlitShaderExtension *this,
        __int64 a2)
{
  *(_QWORD *)this = &Spectre::Engine::BackgroundUnlitShaderExtension::`vftable';
  std::unique_ptr<Spectre::Engine::LightConstants,std::function<void (Spectre::Engine::LightConstants *)>>::~unique_ptr<Spectre::Engine::LightConstants,std::function<void (Spectre::Engine::LightConstants *)>>(
    (__int64)this + 112,
    a2);
  Spectre::Engine::IShaderExtension::~IShaderExtension(this);
}
