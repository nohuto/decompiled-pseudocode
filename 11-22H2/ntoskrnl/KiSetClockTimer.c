/*
 * XREFs of KiSetClockTimer @ 0x1402C2568
 * Callers:
 *     KiSetClockTimerKTimerDeadlines @ 0x1402C1C34 (KiSetClockTimerKTimerDeadlines.c)
 *     KePrepareNonClockOwnerForIdle @ 0x1402C1CAC (KePrepareNonClockOwnerForIdle.c)
 *     PpmIdleExecuteTransition @ 0x1402C52F0 (PpmIdleExecuteTransition.c)
 *     KeResumeClockTimerFromIdle @ 0x1402C7030 (KeResumeClockTimerFromIdle.c)
 *     KiUpdateRunTime @ 0x1402C74B0 (KiUpdateRunTime.c)
 *     KiTransitionSchedulingGroupGeneration @ 0x1403084DC (KiTransitionSchedulingGroupGeneration.c)
 *     KiSetClockIntervalOneShot @ 0x140340658 (KiSetClockIntervalOneShot.c)
 *     KeUpdatePendingQosRequest @ 0x140460E1C (KeUpdatePendingQosRequest.c)
 *     KiResumeClockTimer @ 0x14056FEE8 (KiResumeClockTimer.c)
 * Callees:
 *     RtlGetInterruptTimePrecise @ 0x1402C42B0 (RtlGetInterruptTimePrecise.c)
 *     KiSetNextClockTickDueTime @ 0x1402C84E0 (KiSetNextClockTickDueTime.c)
 *     _tlgWriteTransfer_EtwWriteTransfer @ 0x1402F6B24 (_tlgWriteTransfer_EtwWriteTransfer.c)
 *     KiShouldRearmClockTimer @ 0x14034094C (KiShouldRearmClockTimer.c)
 *     __security_check_cookie @ 0x1403D7680 (__security_check_cookie.c)
 */

__int64 __fastcall KiSetClockTimer(__int64 a1, __int64 a2, int a3, int a4, char a5, char a6)
{
  __int64 v7; // rsi
  __int64 result; // rax
  __int64 v11; // rdx
  __int64 v12; // r14
  char v13; // bl
  __int64 v14; // rcx
  char v15; // [rsp+30h] [rbp-D0h] BYREF
  char v16; // [rsp+31h] [rbp-CFh] BYREF
  int v17; // [rsp+34h] [rbp-CCh] BYREF
  int v18; // [rsp+38h] [rbp-C8h] BYREF
  __int64 v19; // [rsp+40h] [rbp-C0h] BYREF
  __int64 v20; // [rsp+48h] [rbp-B8h] BYREF
  __int64 v21; // [rsp+50h] [rbp-B0h] BYREF
  __int64 v22; // [rsp+58h] [rbp-A8h] BYREF
  char v23[32]; // [rsp+60h] [rbp-A0h] BYREF
  __int64 *v24; // [rsp+80h] [rbp-80h]
  __int64 v25; // [rsp+88h] [rbp-78h]
  __int64 *v26; // [rsp+90h] [rbp-70h]
  __int64 v27; // [rsp+98h] [rbp-68h]
  __int64 *v28; // [rsp+A0h] [rbp-60h]
  __int64 v29; // [rsp+A8h] [rbp-58h]
  __int64 *v30; // [rsp+B0h] [rbp-50h]
  __int64 v31; // [rsp+B8h] [rbp-48h]
  int *v32; // [rsp+C0h] [rbp-40h]
  __int64 v33; // [rsp+C8h] [rbp-38h]
  int *v34; // [rsp+D0h] [rbp-30h]
  __int64 v35; // [rsp+D8h] [rbp-28h]
  char *v36; // [rsp+E0h] [rbp-20h]
  __int64 v37; // [rsp+E8h] [rbp-18h]
  char *v38; // [rsp+F0h] [rbp-10h]
  __int64 v39; // [rsp+F8h] [rbp-8h]

  v17 = a4;
  v7 = 2LL * a4;
  result = RtlGetInterruptTimePrecise(&v19);
  v12 = result - a2;
  if ( a2 > 0 )
    v12 = a2;
  if ( (unsigned int)dword_140C02F60 > 5 )
  {
    v19 = result;
    v24 = &v20;
    v20 = a2;
    v25 = 8LL;
    v21 = v12 - result;
    v27 = 8LL;
    v26 = &v21;
    v22 = v12;
    v28 = &v22;
    v30 = &v19;
    v32 = &v18;
    v34 = &v17;
    v36 = &v15;
    v38 = &v16;
    v29 = 8LL;
    v31 = 8LL;
    v18 = a3;
    v33 = 4LL;
    v35 = 4LL;
    v15 = a5;
    v37 = 1LL;
    v16 = a6;
    v39 = 1LL;
    result = tlgWriteTransfer_EtwWriteTransfer(&dword_140C02F60, &byte_14002D597, 0LL, 0LL, 10, v23);
  }
  *(_QWORD *)(a1 + 8 * v7 + 36376) = v12;
  v13 = (*(_BYTE *)(a1 + 8 * v7 + 36388) ^ (2 * a5)) & 2;
  *(_DWORD *)(a1 + 8 * v7 + 36384) = a3;
  *(_BYTE *)(a1 + 8 * v7 + 36388) = *(_BYTE *)(a1 + 8 * v7 + 36388) ^ v13 | 1;
  if ( a6 )
  {
    LOBYTE(v11) = 1;
    result = KiShouldRearmClockTimer(a1, v11);
    if ( (_BYTE)result )
    {
      LOBYTE(v14) = 1;
      *(_DWORD *)(a1 + 36368) = 2;
      return KiSetNextClockTickDueTime(v14);
    }
  }
  return result;
}
