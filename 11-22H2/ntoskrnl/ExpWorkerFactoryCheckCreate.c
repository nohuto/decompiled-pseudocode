/*
 * XREFs of ExpWorkerFactoryCheckCreate @ 0x1402B8D10
 * Callers:
...
__int64 __fastcall ExpWorkerFactoryCheckCreate(PSLIST_ENTRY ListEntry, PKLOCK_QUEUE_HANDLE LockHandle, char a3)
{
  ...
  _KSPIN_LOCK_QUEUE *volatile v11; // rax
  __int64 result; // rax
    ...
    *((_DWORD *)&ListEntry[25].Next + 2) &= 0xFFFFCE0F;
    ...
    if ( !v4->LockQueue.Next )
    {
      if ( v4 == (PKLOCK_QUEUE_HANDLE)_InterlockedCompareExchange64(
                                        ...
                                        (signed __int64)v4) )
        goto LABEL_13;
      v11 = (_KSPIN_LOCK_QUEUE *volatile)KxWaitForLockChainValid(v4);
    }
    v4->LockQueue.Next = 0LL;
    Lock = (__int64)v4->LockQueue.Lock;
    if ( (((unsigned __int8)Lock ^ (unsigned __int8)_InterlockedExchange64((volatile __int64 *)&v11->Lock, Lock)) & 4) != 0 )
    {
      _InterlockedOr(v46, 0);
      KiHaltOnAddressWakeEntireList(_InterlockedExchange64(&KiHaltOnAddressHashTable[((unsigned __int64)&v11->Lock >> 5) & 0x7F], 0LL));
    }
LABEL_13:
...
LABEL_14:
  ...
  if ( !a3 || (*v17 & 0x30) != 0x20 )
  {
    if ( (unsigned __int8)ExpCheckThreadHistory(ListEntry, v17) )
    {
      v29 = *v18;
      if ( (*v18 & 0x30) == 0 )
      {
