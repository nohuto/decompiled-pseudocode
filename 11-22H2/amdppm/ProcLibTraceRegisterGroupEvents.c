/*
 * XREFs of ProcLibTraceRegisterGroupEvents @ 0x1C00041C0
 * Callers:
...
BOOLEAN __fastcall ProcLibTraceRegisterGroupEvents(__int64 a1, unsigned int a2)
{
  ...
  if ( a2 )
  {
    ...
    if ( a2 != 1 )
      v4 = (const EVENT_DESCRIPTOR *)&PPM_ETW_HARDWARE_DEBUG_REGISTER_RUNDOWN;
  }
  else
  {
    v4 = &PPM_ETW_HARDWARE_DEBUG_REGISTER_ALLOC;
  }
  result = EtwEventEnabled((REGHANDLE)WPP_MAIN_CB.Queue.ListEntry.Flink, v4);
  if ( result )
  {
    ...
    for ( j = 0; j < *(unsigned __int8 *)(a1 + 308); ++j )
    {
      ...
      *(_QWORD *)&UserData.Size = 8LL;
      ...
      v33 = 0;
      EtwWrite((REGHANDLE)WPP_MAIN_CB.Queue.ListEntry.Flink, v4, 0LL, (ULONG)8, &UserData);
