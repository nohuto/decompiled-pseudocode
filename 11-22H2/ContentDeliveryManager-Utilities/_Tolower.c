/*
 * XREFs of _Tolower @ 0x180005FCC
 * Callers:
 *     ?do_tolower@?$ctype@D@std@@MEBADD@Z @ 0x18000B580 (-do_tolower@-$ctype@D@std@@MEBADD@Z.c)
 *     ?do_tolower@?$ctype@D@std@@MEBAPEBDPEADPEBD@Z @ 0x18000B5A0 (-do_tolower@-$ctype@D@std@@MEBAPEBDPEADPEBD@Z.c)
 * Callees:
 *     _GetLocaleHandleById @ 0x18000D928 (_GetLocaleHandleById.c)
 *     _GetLocaleHandleByType @ 0x18000D948 (_GetLocaleHandleByType.c)
 *     __crtLCMapStringA_Stub @ 0x18000D9FC (__crtLCMapStringA_Stub.c)
 */

int __cdecl Tolower(int a1, const _Ctypevec *a2)
{
  int v4; // esi
  __int64 LocaleHandleById; // rbp
  unsigned int v6; // r14d
  __int64 LocaleHandleByType; // rax
  int result; // eax
  const __int16 **p_Table; // rdx
  unsigned int v10; // edx
  int v11; // edi
  int v12; // eax
  bool v13; // zf
  char v14; // [rsp+78h] [rbp+10h] BYREF
  char v15; // [rsp+79h] [rbp+11h]
  char v16; // [rsp+7Ah] [rbp+12h]
  unsigned __int8 v17; // [rsp+80h] [rbp+18h] BYREF
  unsigned __int8 v18; // [rsp+81h] [rbp+19h]

  v4 = 2;
  if ( a2 )
  {
    LocaleHandleByType = GetLocaleHandleByType(a2);
    v6 = *(&a2->_Page + 1);
    LocaleHandleById = LocaleHandleByType;
  }
  else
  {
    LocaleHandleById = GetLocaleHandleById(2LL);
    v6 = ___lc_codepage_func();
  }
  if ( !LocaleHandleById )
  {
    if ( (unsigned int)(a1 - 65) <= 0x19 )
      a1 += 32;
    return a1;
  }
  if ( (unsigned int)a1 < 0x100 )
  {
    if ( !a2 )
    {
      if ( isupper(a1) )
        goto LABEL_17;
      return a1;
    }
    p_Table = &a2->_Table;
    if ( (a2->_Table[(unsigned __int8)a1] & 1) == 0 )
      return a1;
LABEL_13:
    v10 = ((unsigned int)(*p_Table)[BYTE1(a1)] >> 15) & 1;
    v11 = a1 >> 8;
    goto LABEL_14;
  }
  p_Table = &a2->_Table;
  if ( a2 )
    goto LABEL_13;
LABEL_17:
  v11 = a1 >> 8;
  v10 = __pctype_func()[BYTE1(a1)] & 0x8000;
LABEL_14:
  if ( v10 )
  {
    v14 = v11;
    v15 = a1;
    v16 = 0;
  }
  else
  {
    v14 = a1;
    v4 = 1;
    v15 = 0;
  }
  v12 = _crtLCMapStringA_Stub(0LL, LocaleHandleById, 256LL, &v14, v4, &v17, 3, v6, 1);
  if ( !v12 )
    return a1;
  v13 = v12 == 1;
  result = v17;
  if ( !v13 )
    return v18 | (v17 << 8);
  return result;
}
