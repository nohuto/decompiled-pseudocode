/*
 * XREFs of ?_Getcat@?$num_get@_WV?$istreambuf_iterator@_WU?$char_traits@_W@std@@@std@@@std@@SA_KPEAPEBVfacet@locale@2@PEBV42@@Z @ 0x1800B7870
 * Callers:
...
__int64 __fastcall std::num_get<wchar_t,std::istreambuf_iterator<wchar_t>>::_Getcat(_QWORD *a1, std::locale *a2)
{
  ...
  _DWORD *v5; // rbx
  char *v6; // rax
  _BYTE v8[112]; // [rsp+20h] [rbp-78h] BYREF
  ...
  if ( a1 && !*a1 )
  {
    ...
    *a1 = v5;
    if ( (v4 & 1) != 0 )
      std::_Locinfo::~_Locinfo((std::_Locinfo *)v8);
  }
  return 4LL;
}
