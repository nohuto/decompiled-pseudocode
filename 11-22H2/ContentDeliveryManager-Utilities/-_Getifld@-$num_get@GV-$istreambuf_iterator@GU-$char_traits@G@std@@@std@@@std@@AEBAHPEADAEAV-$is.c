/*
 * XREFs of ?_Getifld@?$num_get@GV?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@@std@@AEBAHPEADAEAV?$istreambuf_iterator@GU?$char_traits@G@std@@@2@1HAEBVlocale@2@@Z @ 0x18006162C
 * Callers:
 *     ?do_get@?$num_get@GV?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@@std@@MEBA?AV?$istreambuf_iterator@GU?$char_traits@G@std@@@2@V32@0AEAVios_base@2@AEAHAEAG@Z @ 0x180062F80 (-do_get@-$num_get@GV-$istreambuf_iterator@GU-$char_traits@G@std@@@std@@@std@@MEBA-AV-$istreambuf.c)
 *     ?do_get@?$num_get@GV?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@@std@@MEBA?AV?$istreambuf_iterator@GU?$char_traits@G@std@@@2@V32@0AEAVios_base@2@AEAHAEAI@Z @ 0x180063090 (-do_get@-$num_get@GV-$istreambuf_iterator@GU-$char_traits@G@std@@@std@@@std@@MEBA-A_ea_180063090.c)
 *     ?do_get@?$num_get@GV?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@@std@@MEBA?AV?$istreambuf_iterator@GU?$char_traits@G@std@@@2@V32@0AEAVios_base@2@AEAHAEAJ@Z @ 0x180063190 (-do_get@-$num_get@GV-$istreambuf_iterator@GU-$char_traits@G@std@@@std@@@std@@MEBA-A_ea_180063190.c)
 *     ?do_get@?$num_get@GV?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@@std@@MEBA?AV?$istreambuf_iterator@GU?$char_traits@G@std@@@2@V32@0AEAVios_base@2@AEAHAEAK@Z @ 0x180063290 (-do_get@-$num_get@GV-$istreambuf_iterator@GU-$char_traits@G@std@@@std@@@std@@MEBA-A_ea_180063290.c)
 *     ?do_get@?$num_get@GV?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@@std@@MEBA?AV?$istreambuf_iterator@GU?$char_traits@G@std@@@2@V32@0AEAVios_base@2@AEAHAEAPEAX@Z @ 0x180063670 (-do_get@-$num_get@GV-$istreambuf_iterator@GU-$char_traits@G@std@@@std@@@std@@MEBA-A_ea_180063670.c)
 *     ?do_get@?$num_get@GV?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@@std@@MEBA?AV?$istreambuf_iterator@GU?$char_traits@G@std@@@2@V32@0AEAVios_base@2@AEAHAEA_J@Z @ 0x180063770 (-do_get@-$num_get@GV-$istreambuf_iterator@GU-$char_traits@G@std@@@std@@@std@@MEBA-A_ea_180063770.c)
 *     ?do_get@?$num_get@GV?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@@std@@MEBA?AV?$istreambuf_iterator@GU?$char_traits@G@std@@@2@V32@0AEAVios_base@2@AEAHAEA_K@Z @ 0x180063870 (-do_get@-$num_get@GV-$istreambuf_iterator@GU-$char_traits@G@std@@@std@@@std@@MEBA-A_ea_180063870.c)
 *     ?do_get@?$num_get@GV?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@@std@@MEBA?AV?$istreambuf_iterator@GU?$char_traits@G@std@@@2@V32@0AEAVios_base@2@AEAHAEA_N@Z @ 0x180063970 (-do_get@-$num_get@GV-$istreambuf_iterator@GU-$char_traits@G@std@@@std@@@std@@MEBA-A_ea_180063970.c)
 * Callees:
 *     __security_check_cookie @ 0x1800224C0 (__security_check_cookie.c)
 *     ?_Tidy@?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@QEAAX_N_K@Z @ 0x180043370 (-_Tidy@-$basic_string@DU-$char_traits@D@std@@V-$allocator@D@2@@std@@QEAAX_N_K@Z.c)
 *     ??$_Find_elem@G@std@@YA_KPEAGG@Z @ 0x18004F6DC (--$_Find_elem@G@std@@YA_KPEAGG@Z.c)
 *     ??$use_facet@V?$ctype@G@std@@@std@@YAAEBV?$ctype@G@0@AEBVlocale@0@@Z @ 0x18004FF40 (--$use_facet@V-$ctype@G@std@@@std@@YAAEBV-$ctype@G@0@AEBVlocale@0@@Z.c)
 *     ??$use_facet@V?$numpunct@G@std@@@std@@YAAEBV?$numpunct@G@0@AEBVlocale@0@@Z @ 0x180050108 (--$use_facet@V-$numpunct@G@std@@@std@@YAAEBV-$numpunct@G@0@AEBVlocale@0@@Z.c)
 *     ??0?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@QEAA@_KD@Z @ 0x180051890 (--0-$basic_string@DU-$char_traits@D@std@@V-$allocator@D@2@@std@@QEAA@_KD@Z.c)
 *     ??D?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@QEBAAEBGXZ @ 0x180053B44 (--D-$istreambuf_iterator@GU-$char_traits@G@std@@@std@@QEBAAEBGXZ.c)
 *     ?_Inc@?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@AEAAXXZ @ 0x180061AA0 (-_Inc@-$istreambuf_iterator@GU-$char_traits@G@std@@@std@@AEAAXXZ.c)
 *     ?append@?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@QEAAAEAV12@_KD@Z @ 0x1800628D8 (-append@-$basic_string@DU-$char_traits@D@std@@V-$allocator@D@2@@std@@QEAAAEAV12@_KD@Z.c)
 *     ?equal@?$istreambuf_iterator@_WU?$char_traits@_W@std@@@std@@QEBA_NAEBV12@@Z @ 0x180063F44 (-equal@-$istreambuf_iterator@_WU-$char_traits@_W@std@@@std@@QEBA_NAEBV12@@Z.c)
 *     ?grouping@?$numpunct@G@std@@QEBA?AV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@2@XZ @ 0x18006493C (-grouping@-$numpunct@G@std@@QEBA-AV-$basic_string@DU-$char_traits@D@std@@V-$allocator@D@2@@2@XZ.c)
 *     ?thousands_sep@?$numpunct@G@std@@QEBAGXZ @ 0x18006538C (-thousands_sep@-$numpunct@G@std@@QEBAGXZ.c)
 *     _guard_xfg_dispatch_icall_nop @ 0x1800D4360 (_guard_xfg_dispatch_icall_nop.c)
 */

// Hidden C++ exception states: #wind=2
__int64 __fastcall std::num_get<unsigned short,std::istreambuf_iterator<unsigned short>>::_Getifld(
        __int64 a1,
        char *a2,
        __int64 a3,
        __int64 a4,
        __int16 a5,
        std::locale *a6)
{
  const struct std::locale::facet *v8; // rbx
  char v9; // r13
  const struct std::locale::facet *v10; // rax
  char *v11; // r14
  _WORD *v12; // rax
  __int64 v13; // rcx
  _WORD *v14; // rax
  int v15; // eax
  unsigned __int64 v16; // r15
  unsigned int v17; // ebx
  __int64 v18; // r8
  _WORD *v19; // rax
  __int64 v20; // rsi
  _WORD *v21; // rax
  _WORD *v22; // rax
  unsigned __int64 v23; // r12
  __int16 *v24; // rax
  unsigned __int64 v25; // rax
  char v26; // al
  void **v27; // rax
  void **v28; // rax
  void **v29; // rax
  void **v30; // r9
  void **v31; // rax
  void **v32; // rcx
  char v33; // dl
  void **v34; // rax
  void **v36; // rax
  char v37; // [rsp+30h] [rbp-89h]
  __int16 v38; // [rsp+32h] [rbp-87h]
  char *v40; // [rsp+40h] [rbp-79h]
  char *v41; // [rsp+48h] [rbp-71h]
  void *v42[3]; // [rsp+50h] [rbp-69h] BYREF
  unsigned __int64 v43; // [rsp+68h] [rbp-51h]
  void *v44[4]; // [rsp+70h] [rbp-49h] BYREF
  _WORD v45[28]; // [rsp+90h] [rbp-29h] BYREF

  v41 = a2;
  v8 = std::use_facet<std::numpunct<unsigned short>>(a6);
  std::numpunct<unsigned short>::grouping(v8, v44);
  v9 = 0;
  if ( v44[2] )
    v38 = std::numpunct<unsigned short>::thousands_sep(v8);
  else
    v38 = 0;
  v10 = std::use_facet<std::ctype<unsigned short>>(a6);
  (*(void (__fastcall **)(const struct std::locale::facet *, const char *, char *, _WORD *))(*(_QWORD *)v10 + 88LL))(
    v10,
    "0123456789ABCDEFabcdef-+Xx",
    "",
    v45);
  v11 = a2;
  if ( (unsigned __int8)std::istreambuf_iterator<wchar_t>::equal(a3, a4) )
    goto LABEL_10;
  v12 = (_WORD *)std::istreambuf_iterator<unsigned short>::operator*(a3);
  v13 = a3;
  if ( *v12 == v45[23] )
  {
    *a2 = 43;
  }
  else
  {
    v14 = (_WORD *)std::istreambuf_iterator<unsigned short>::operator*(a3);
    if ( *v14 != v45[22] )
      goto LABEL_10;
    *a2 = 45;
    v13 = a3;
  }
  v11 = a2 + 1;
  std::istreambuf_iterator<unsigned short>::_Inc(v13);
LABEL_10:
  v15 = a5 & 0xE00;
  v16 = 8LL;
  if ( v15 == 1024 )
  {
    v17 = 8;
  }
  else if ( v15 == 2048 )
  {
    v17 = 16;
  }
  else
  {
    v17 = v15 != 0 ? 0xA : 0;
  }
  v37 = 0;
  if ( (unsigned __int8)std::istreambuf_iterator<wchar_t>::equal(a3, a4)
    || (v19 = (_WORD *)std::istreambuf_iterator<unsigned short>::operator*(a3), *v19 != v45[0]) )
  {
    v20 = 0LL;
    if ( !v17 )
      goto LABEL_29;
  }
  else
  {
    v9 = 1;
    std::istreambuf_iterator<unsigned short>::_Inc(a3);
    v20 = 0LL;
    if ( !(unsigned __int8)std::istreambuf_iterator<wchar_t>::equal(a3, a4)
      && ((v21 = (_WORD *)std::istreambuf_iterator<unsigned short>::operator*(a3), *v21 == v45[25])
       || (v22 = (_WORD *)std::istreambuf_iterator<unsigned short>::operator*(a3), *v22 == v45[24])) )
    {
      if ( (v17 & 0xFFFFFFEF) == 0 )
      {
        v17 = 16;
        v9 = 0;
        std::istreambuf_iterator<unsigned short>::_Inc(a3);
        goto LABEL_26;
      }
    }
    else if ( !v17 )
    {
      v17 = 8;
      goto LABEL_26;
    }
  }
  if ( v17 != 10 )
  {
LABEL_26:
    if ( v17 != 8 )
      v16 = 22LL;
    goto LABEL_30;
  }
LABEL_29:
  v16 = 10LL;
LABEL_30:
  LOBYTE(v18) = v9;
  std::string::string((__int64)v42, 1LL, v18);
  v40 = a2 + 31;
  v23 = v43;
  while ( !(unsigned __int8)std::istreambuf_iterator<wchar_t>::equal(a3, a4) )
  {
    v24 = (__int16 *)std::istreambuf_iterator<unsigned short>::operator*(a3);
    v25 = std::_Find_elem<unsigned short>(v45, *v24);
    if ( v25 >= v16 )
    {
      v29 = v42;
      v30 = (void **)v42[0];
      if ( v23 >= 0x10 )
        v29 = (void **)v42[0];
      if ( !*((_BYTE *)v29 + v20) || !v38 )
        goto LABEL_52;
      if ( *(_WORD *)std::istreambuf_iterator<unsigned short>::operator*(a3) != v38 )
        break;
      std::string::append(v42, 1LL);
      ++v20;
      goto LABEL_49;
    }
    v26 = `std::num_get<wchar_t,std::istreambuf_iterator<wchar_t>>::_Getifld'::`2'::_Src[v25];
    *v11 = v26;
    if ( (v37 || v26 != 48) && v11 < v40 )
    {
      ++v11;
      v37 = 1;
    }
    v9 = 1;
    v27 = v42;
    if ( v23 >= 0x10 )
      v27 = (void **)v42[0];
    if ( *((_BYTE *)v27 + v20) != 127 )
    {
      v28 = v42;
      if ( v23 >= 0x10 )
        v28 = (void **)v42[0];
      ++*((_BYTE *)v28 + v20);
LABEL_49:
      v23 = v43;
    }
    std::istreambuf_iterator<unsigned short>::_Inc(a3);
  }
  v30 = (void **)v42[0];
LABEL_52:
  if ( v20 )
  {
    v31 = v42;
    if ( v43 >= 0x10 )
      v31 = v30;
    if ( *((char *)v31 + v20) <= 0 )
      v9 = 0;
    else
      ++v20;
  }
  v32 = v44;
  if ( v44[3] >= (void *)0x10 )
    v32 = (void **)v44[0];
  if ( v9 )
  {
    while ( v20 )
    {
      v33 = *(_BYTE *)v32;
      if ( *(_BYTE *)v32 == 127 )
        break;
      if ( --v20 )
      {
        v34 = v42;
        if ( v43 >= 0x10 )
          v34 = v30;
        if ( v33 != *((_BYTE *)v34 + v20) )
          goto LABEL_67;
      }
      else
      {
        v36 = v42;
        if ( v43 >= 0x10 )
          v36 = v30;
        if ( v33 < *(char *)v36 )
          goto LABEL_67;
      }
      if ( *((char *)v32 + 1) > 0 )
        v32 = (void **)((char *)v32 + 1);
    }
    if ( !v37 )
      *v11++ = 48;
  }
  else
  {
LABEL_67:
    v11 = v41;
  }
  *v11 = 0;
  std::string::_Tidy(v42, 1, 0LL);
  std::string::_Tidy(v44, 1, 0LL);
  return v17;
}
