/*
 * XREFs of ?writeName@JSONOutputArchive@cereal@@QEAAXXZ @ 0x1800F41F4
 * Callers:
 *     ??$process@AEAI@?$OutputArchive@VJSONOutputArchive@cereal@@$0A@@cereal@@AEAAXAEAI@Z @ 0x1800EFEEC (--$process@AEAI@-$OutputArchive@VJSONOutputArchive@cereal@@$0A@@cereal@@AEAAXAEAI@Z.c)
 *     ??$process@AEAV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@@?$OutputArchive@VJSONOutputArchive@cereal@@$0A@@cereal@@AEAAXAEAV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@@Z @ 0x1800EFF98 (--$process@AEAV-$basic_string@GU-$char_traits@G@std@@V-$allocator@G@2@@std@@@-$OutputArchive@VJS.c)
 *     ??$process@H@?$OutputArchive@VJSONOutputArchive@cereal@@$0A@@cereal@@AEAAX$$QEAH@Z @ 0x1800F005C (--$process@H@-$OutputArchive@VJSONOutputArchive@cereal@@$0A@@cereal@@AEAAX$$QEAH@Z.c)
 *     ??$process@V?$NameValuePair@AEAG@cereal@@V?$NameValuePair@AEAV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@2@V?$NameValuePair@AEAI@2@V?$NameValuePair@AEAV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@@2@V32@@?$OutputArchive@VJSONOutputArchive@cereal@@$0A@@cereal@@AEAAX$$QEAV?$NameValuePair@AEAG@1@$$QEAV?$NameValuePair@AEAV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@1@$$QEAV?$NameValuePair@AEAI@1@$$QEAV?$NameValuePair@AEAV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@@1@1@Z @ 0x1800F00B8 (--$process@V-$NameValuePair@AEAG@cereal@@V-$NameValuePair@AEAV-$basic_string@DU-$char_traits@D@s.c)
 *     ??$process@V?$NameValuePair@AEAI@cereal@@V?$NameValuePair@AEAV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@@2@V?$NameValuePair@AEAV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@2@@?$OutputArchive@VJSONOutputArchive@cereal@@$0A@@cereal@@AEAAX$$QEAV?$NameValuePair@AEAI@1@$$QEAV?$NameValuePair@AEAV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@@1@$$QEAV?$NameValuePair@AEAV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@1@@Z @ 0x1800F0168 (--$process@V-$NameValuePair@AEAI@cereal@@V-$NameValuePair@AEAV-$basic_string@GU-$char_traits@G@s.c)
 *     ??$process@V?$NameValuePair@AEAJ@cereal@@V?$NameValuePair@AEAV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@2@V?$NameValuePair@AEAG@2@V32@V?$NameValuePair@AEAI@2@V?$NameValuePair@AEAV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@@2@V32@@?$OutputArchive@VJSONOutputArchive@cereal@@$0A@@cereal@@AEAAX$$QEAV?$NameValuePair@AEAJ@1@$$QEAV?$NameValuePair@AEAV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@1@$$QEAV?$NameValuePair@AEAG@1@1$$QEAV?$NameValuePair@AEAI@1@$$QEAV?$NameValuePair@AEAV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@@1@1@Z @ 0x1800F022C (--$process@V-$NameValuePair@AEAJ@cereal@@V-$NameValuePair@AEAV-$basic_string@DU-$char_traits@D@s.c)
 *     ??$process@V?$NameValuePair@AEAV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@cereal@@V?$NameValuePair@AEAG@2@V12@V?$NameValuePair@AEAI@2@V?$NameValuePair@AEAV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@@2@V12@@?$OutputArchive@VJSONOutputArchive@cereal@@$0A@@cereal@@AEAAX$$QEAV?$NameValuePair@AEAV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@1@$$QEAV?$NameValuePair@AEAG@1@0$$QEAV?$NameValuePair@AEAI@1@$$QEAV?$NameValuePair@AEAV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@@1@0@Z @ 0x1800F02FC (--$process@V-$NameValuePair@AEAV-$basic_string@DU-$char_traits@D@std@@V-$allocator@D@2@@std@@@ce.c)
 *     ??$process@V?$NameValuePair@AEAV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@cereal@@V?$NameValuePair@AEAI@2@V?$NameValuePair@AEAV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@@2@V12@@?$OutputArchive@VJSONOutputArchive@cereal@@$0A@@cereal@@AEAAX$$QEAV?$NameValuePair@AEAV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@1@$$QEAV?$NameValuePair@AEAI@1@$$QEAV?$NameValuePair@AEAV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@@1@0@Z @ 0x1800F03B4 (--$process@V-$NameValuePair@AEAV-$basic_string@DU-$char_traits@D@std@@V-$allocator@_ea_1800F03B4.c)
 *     ??$save@VJSONOutputArchive@cereal@@V?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@V?$allocator@V?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@4@@cereal@@YAXAEAVJSONOutputArchive@0@AEBV?$vector@V?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@V?$allocator@V?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@2@@std@@@Z @ 0x1800F0690 (--$save@VJSONOutputArchive@cereal@@V-$basic_string@DU-$char_traits@D@std@@V-$allocator@D@2@@std@.c)
 *     ?startNode@JSONOutputArchive@cereal@@QEAAXXZ @ 0x1800F3DF0 (-startNode@JSONOutputArchive@cereal@@QEAAXXZ.c)
 *     ??$process@V?$NameValuePair@AEA_N@cereal@@V12@V?$NameValuePair@AEAW4RotationState@Rotation@Private@Transitions@Udwm@winrt@@@2@@?$OutputArchive@VJSONOutputArchive@cereal@@$0A@@cereal@@AEAAX$$QEAV?$NameValuePair@AEA_N@1@0$$QEAV?$NameValuePair@AEAW4RotationState@Rotation@Private@Transitions@Udwm@winrt@@@1@@Z @ 0x1800F47A0 (--$process@V-$NameValuePair@AEA_N@cereal@@V12@V-$NameValuePair@AEAW4RotationState@Rotation@Priva.c)
 * Callees:
 *     ??$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z @ 0x1800558D4 (--$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z.c)
 *     __security_check_cookie @ 0x18005C460 (__security_check_cookie.c)
 *     ??$?HDU?$char_traits@D@std@@V?$allocator@D@1@@std@@YA?AV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@0@QEBD$$QEAV10@@Z @ 0x1800EF108 (--$-HDU-$char_traits@D@std@@V-$allocator@D@1@@std@@YA-AV-$basic_string@DU-$char_traits@D@std@@V-.c)
 *     ??$_UIntegral_to_string@DI@std@@YA?AV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@0@I@Z @ 0x1800EFBE8 (--$_UIntegral_to_string@DI@std@@YA-AV-$basic_string@DU-$char_traits@D@std@@V-$allocator@D@2@@0@I.c)
 *     ?StartArray@?$PrettyWriter@V?$BasicOStreamWrapper@V?$basic_ostream@DU?$char_traits@D@std@@@std@@@tip_rapidjson@@U?$UTF8@D@2@U32@VCrtAllocator@2@$01@tip_rapidjson@@QEAA_NXZ @ 0x1800F1F68 (-StartArray@-$PrettyWriter@V-$BasicOStreamWrapper@V-$basic_ostream@DU-$char_traits@D@std@@@std@@.c)
 *     ?StartObject@?$PrettyWriter@V?$BasicOStreamWrapper@V?$basic_ostream@DU?$char_traits@D@std@@@std@@@tip_rapidjson@@U?$UTF8@D@2@U32@VCrtAllocator@2@$01@tip_rapidjson@@QEAA_NXZ @ 0x1800F2004 (-StartObject@-$PrettyWriter@V-$BasicOStreamWrapper@V-$basic_ostream@DU-$char_traits@D@std@@@std@.c)
 *     ?String@?$PrettyWriter@V?$BasicOStreamWrapper@V?$basic_ostream@DU?$char_traits@D@std@@@std@@@tip_rapidjson@@U?$UTF8@D@2@U32@VCrtAllocator@2@$01@tip_rapidjson@@QEAA_NPEBDI_N@Z @ 0x1800F20A0 (-String@-$PrettyWriter@V-$BasicOStreamWrapper@V-$basic_ostream@DU-$char_traits@D@std@@@std@@@tip.c)
 */

// Hidden C++ exception states: #wind=5
void __fastcall cereal::JSONOutputArchive::writeName(cereal::JSONOutputArchive *this)
{
  unsigned __int64 v2; // rdx
  __int64 v3; // rdi
  __int64 v4; // rax
  __int64 v5; // rsi
  _BYTE *v6; // rdx
  __int64 v7; // r8
  __int64 v8; // rcx
  unsigned int v9; // edx
  _QWORD *v10; // rax
  __int64 v11; // rdx
  _QWORD *v12; // rcx
  __int128 *v13; // rdx
  _BYTE *v14; // rax
  __int128 v15; // [rsp+28h] [rbp-21h] BYREF
  __int128 v16; // [rsp+38h] [rbp-11h]
  void *v17[3]; // [rsp+48h] [rbp-1h] BYREF
  unsigned __int64 v18; // [rsp+60h] [rbp+17h]
  void *v19[3]; // [rsp+68h] [rbp+1Fh] BYREF
  unsigned __int64 v20; // [rsp+80h] [rbp+37h]

  v2 = (unsigned __int64)(*((_QWORD *)this + 61) - 1LL + *((_QWORD *)this + 60)) >> 2;
  v3 = (*((_DWORD *)this + 122) - 1 + *((_DWORD *)this + 120)) & 3;
  v4 = *((_QWORD *)this + 58);
  v5 = *(_QWORD *)(v4 + 8 * (v2 & (*((_QWORD *)this + 59) - 1LL)));
  if ( *(_DWORD *)(v5 + 4 * v3) == 2 )
  {
    tip_rapidjson::PrettyWriter<tip_rapidjson::BasicOStreamWrapper<std::ostream>,tip_rapidjson::UTF8<char>,tip_rapidjson::UTF8<char>,tip_rapidjson::CrtAllocator,2>::StartArray((_QWORD **)this + 41);
    *(_DWORD *)(*(_QWORD *)(*((_QWORD *)this + 58)
                          + 8
                          * (((unsigned __int64)(*((_QWORD *)this + 60) + *((_QWORD *)this + 61) - 1LL) >> 2) & (*((_QWORD *)this + 59) - 1LL)))
              + 4LL * ((*((_DWORD *)this + 120) + *((_DWORD *)this + 122) - 1) & 3)) = 3;
  }
  else if ( !*(_DWORD *)(v5 + 4 * v3) )
  {
    *(_DWORD *)(*(_QWORD *)(v4 + 8 * (v2 & (*((_QWORD *)this + 59) - 1LL))) + 4 * v3) = 1;
    tip_rapidjson::PrettyWriter<tip_rapidjson::BasicOStreamWrapper<std::ostream>,tip_rapidjson::UTF8<char>,tip_rapidjson::UTF8<char>,tip_rapidjson::CrtAllocator,2>::StartObject((_QWORD **)this + 41);
  }
  if ( *(_DWORD *)(v5 + 4 * v3) != 3 )
  {
    v6 = (_BYTE *)*((_QWORD *)this + 51);
    if ( v6 )
    {
      v14 = (_BYTE *)*((_QWORD *)this + 51);
      if ( *v6 )
      {
        do
          ++v14;
        while ( *v14 );
      }
      tip_rapidjson::PrettyWriter<tip_rapidjson::BasicOStreamWrapper<std::ostream>,tip_rapidjson::UTF8<char>,tip_rapidjson::UTF8<char>,tip_rapidjson::CrtAllocator,2>::String(
        (__int64 **)this + 41,
        (__int64)v6,
        (_DWORD)v14 - (_DWORD)v6);
      *((_QWORD *)this + 51) = 0LL;
    }
    else
    {
      v7 = (*((_DWORD *)this + 110) + *((_DWORD *)this + 112) - 1) & 3;
      v8 = *(_QWORD *)(*((_QWORD *)this + 53)
                     + 8
                     * (((unsigned __int64)(*((_QWORD *)this + 55) + *((_QWORD *)this + 56) - 1LL) >> 2) & (*((_QWORD *)this + 54) - 1LL)));
      v9 = *(_DWORD *)(v8 + 4 * v7);
      *(_DWORD *)(v8 + 4 * v7) = v9 + 1;
      std::_UIntegral_to_string<char,unsigned int>(v17, v9);
      v10 = std::operator+<char>(v19, (__int64)"value", v17);
      v11 = v10[2];
      v10[2] = v11;
      v12 = v10;
      if ( v10[3] >= 0x10uLL )
        v12 = (_QWORD *)*v10;
      *((_BYTE *)v12 + v11) = 0;
      *(_QWORD *)&v15 = 0LL;
      v16 = 0uLL;
      v15 = *(_OWORD *)v10;
      v16 = *((_OWORD *)v10 + 1);
      v10[2] = 0LL;
      v10[3] = 15LL;
      *(_BYTE *)v10 = 0;
      if ( v20 >= 0x10 )
        std::_Deallocate<16,0>(v19[0], v20 + 1);
      v19[2] = 0LL;
      v20 = 15LL;
      LOBYTE(v19[0]) = 0;
      if ( v18 >= 0x10 )
        std::_Deallocate<16,0>(v17[0], v18 + 1);
      v17[2] = 0LL;
      v18 = 15LL;
      LOBYTE(v17[0]) = 0;
      v13 = &v15;
      if ( *((_QWORD *)&v16 + 1) >= 0x10uLL )
        v13 = (__int128 *)v15;
      tip_rapidjson::PrettyWriter<tip_rapidjson::BasicOStreamWrapper<std::ostream>,tip_rapidjson::UTF8<char>,tip_rapidjson::UTF8<char>,tip_rapidjson::CrtAllocator,2>::String(
        (__int64 **)this + 41,
        (__int64)v13,
        v16);
      if ( *((_QWORD *)&v16 + 1) >= 0x10uLL )
        std::_Deallocate<16,0>((void *)v15, *((_QWORD *)&v16 + 1) + 1LL);
    }
  }
}
