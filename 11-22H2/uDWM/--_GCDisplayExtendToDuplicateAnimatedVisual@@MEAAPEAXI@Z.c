/*
 * XREFs of ??_GCDisplayExtendToDuplicateAnimatedVisual@@MEAAPEAXI@Z @ 0x1800BB530
 * Callers:
 *     <none>
 * Callees:
 *     ?__global_delete@@YAXPEAX_K@Z @ 0x1800585BC (-__global_delete@@YAXPEAX_K@Z.c)
 *     _guard_xfg_dispatch_icall_nop @ 0x1800663A0 (_guard_xfg_dispatch_icall_nop.c)
 *     ??1CDisplayExtendToDuplicateAnimatedVisual@@MEAA@XZ @ 0x1800BB450 (--1CDisplayExtendToDuplicateAnimatedVisual@@MEAA@XZ.c)
 */

CDisplayExtendToDuplicateAnimatedVisual *__fastcall CDisplayExtendToDuplicateAnimatedVisual::`scalar deleting destructor'(
        CDisplayExtendToDuplicateAnimatedVisual *this,
        char a2)
{
  CDisplayExtendToDuplicateAnimatedVisual::~CDisplayExtendToDuplicateAnimatedVisual(this);
  if ( (a2 & 1) != 0 )
  {
    if ( (a2 & 4) != 0 )
      __global_delete(this);
    else
      (*(void (__fastcall **)(WPF::HeapBase *, CDisplayExtendToDuplicateAnimatedVisual *))(*(_QWORD *)WPF::g_pProcessHeap
                                                                                         + 32LL))(
        WPF::g_pProcessHeap,
        this);
  }
  return this;
}
