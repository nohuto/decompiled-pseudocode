/*
 * XREFs of ?CheckAndSendFindMyPenWnf@CTouchProcessor@@AEAAXPEBUCPointerInputFrame@@@Z @ 0x1C01BC554
 * Callers:
        ...
        if ( v5 )
        {
          ...
          if ( v10.QuadPart < (unsigned __int64)PerformanceCounter.QuadPart )
          {
            ...
            if ( (int)ZwUpdateWnfStateData(&WNF_ISM_INPUT_UPDATE_AFTER_TRACK_INTERVAL, 0LL, 0LL, 0LL, v11) < 0 )
              MicrosoftTelemetryAssertTriggeredArgsKM("IXPTelAssert", 0x20000, 17106);
          }
        }
      }
    }
