...
char __fastcall std::_Tree<std::_Tmap_traits<std::wstring,std::variant<unsigned long,bool,float>,std::less<void>,std::allocator<std::pair<std::wstring const,std::variant<unsigned long,bool,float>>>,0>>::_Lower_bound_duplicate<std::wstring>(
        __int64 a1,
        __int64 a2,
        _QWORD *a3)
{
  ...
  if ( !*(_BYTE *)(a2 + 25) )
  {
    ...
    if ( a3[3] >= 8uLL )
      a3 = (_QWORD *)*a3;
    if ( (int)std::_Traits_compare<std::char_traits<unsigned short>>(a3, v7, v5, v6) >= 0 )
      return 1;
  }
  return v3;
}
