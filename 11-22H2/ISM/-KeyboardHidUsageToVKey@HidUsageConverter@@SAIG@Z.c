/*
 * XREFs of ?KeyboardHidUsageToVKey@HidUsageConverter@@SAIG@Z @ 0x180103A78
 * Callers:
 ...
 *     ??9?$_Tree_const_iterator@V?$_Tree_val@U?$_Tree_simple_types@U?$pair@$$CBGI@std@@@std@@@std@@@std@@QEBA_NAEBV01@@Z @ 0x1800650D8 (--9-$_Tree_const_iterator@V-$_Tree_val@U-$_Tree_simple_types@U-$pair@$$CBGI@std@@@std@@@std@@@st.c)
...
UINT __fastcall HidUsageConverter::KeyboardHidUsageToVKey(USHORT a1)
{
  ...
  if ( wil::details::FeatureImpl<__WilFeatureTraits_Feature_AmbientDeviceLighting>::__private_IsEnabled(`wil::Feature<__WilFeatureTraits_Feature_AmbientDeviceLighting>::GetImpl'::`2'::impl) )
  {
    ...
    for ( i = (unsigned __int8 *)&unk_18022D240; *i != ChangedUsageList; ++i )
    {
  ...
  else
  {
    ...
    for ( j = (unsigned __int8 *)&unk_18022D240; *j != ChangedUsageList; ++j )
    {
      if ( (unsigned int)++v1 >= 0x100 )
        return 0;
    }
