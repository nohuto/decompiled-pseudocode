/*
 * XREFs of ?OnInput@DragNDropProcessor@@UEAAJPEAUInputInfo@@PEAVInputContext@@PEAUContextualProcessorResponse@@@Z @ 0x18016D2B0
 * Callers:
 *     <none>
 * Callees:
 *     ??$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z @ 0x18001A954 (--$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z.c)
 *     __security_check_cookie @ 0x1800574C0 (__security_check_cookie.c)
 *     ??_V@YAXPEAX@Z @ 0x180057AD8 (--_V@YAXPEAX@Z.c)
 *     _guard_xfg_dispatch_icall_nop @ 0x1800793F0 (_guard_xfg_dispatch_icall_nop.c)
 *     ??$find@X@?$_Hash@V?$_Umap_traits@I_KV?$_Uhash_compare@IU?$hash@I@std@@U?$equal_to@I@2@@std@@V?$allocator@U?$pair@$$CBI_K@std@@@2@$0A@@std@@@std@@QEAA?AV?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBI_K@std@@@std@@@std@@@1@AEBI@Z @ 0x1800AABA8 (--$find@X@-$_Hash@V-$_Umap_traits@I_KV-$_Uhash_compare@IU-$hash@I@std@@U-$equal_to@I@2@@std@@V-$.c)
 *     ??0Lock@Bamo@Microsoft@@QEAA@PEAVBaseBamoConnection@12@@Z @ 0x1800B6B74 (--0Lock@Bamo@Microsoft@@QEAA@PEAVBaseBamoConnection@12@@Z.c)
 *     ??1Lock@Bamo@Microsoft@@QEAA@XZ @ 0x1800B6C88 (--1Lock@Bamo@Microsoft@@QEAA@XZ.c)
 *     ??$find@X@?$_Hash@V?$_Umap_traits@KUPointerCache@EdgyProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUPointerCache@EdgyProcessor@@@std@@@4@$0A@@std@@@std@@QEAA?AV?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKUPointerCache@EdgyProcessor@@@std@@@std@@@std@@@1@AEBK@Z @ 0x1800E6314 (--$find@X@-$_Hash@V-$_Umap_traits@KUPointerCache@EdgyProcessor@@V-$_Uhash_compare@KU-$hash@K@std.c)
 *     ?RelativePointToScreen@CInputTransform@@SA?AUTransformedPoint@@AEBU2@AEBUtagINPUT_TRANSFORM@@@Z @ 0x1800F7A40 (-RelativePointToScreen@CInputTransform@@SA-AUTransformedPoint@@AEBU2@AEBUtagINPUT_TRANSFORM@@@Z.c)
 *     ??0?$VariableSizedPayloadStorage@UInputInfo@@@@QEAA@PEBUInputInfo@@@Z @ 0x1800FC1D8 (--0-$VariableSizedPayloadStorage@UInputInfo@@@@QEAA@PEBUInputInfo@@@Z.c)
 *     ??$count@X@?$_Hash@V?$_Uset_traits@KV?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@K@2@$0A@@std@@@std@@QEBA_KAEBK@Z @ 0x180106168 (--$count@X@-$_Hash@V-$_Uset_traits@KV-$_Uhash_compare@KU-$hash@K@std@@U-$equal_to@K@2@@std@@V-$a.c)
 *     ?OnManualDragEnded@ManualDragAreaClientProxy@@QEAAXI@Z @ 0x180167FB8 (-OnManualDragEnded@ManualDragAreaClientProxy@@QEAAXI@Z.c)
 *     ??$_Emplace_reallocate@AEBUDragInfoStruct@@@?$vector@UDragInfoStruct@@V?$allocator@UDragInfoStruct@@@std@@@std@@QEAAPEAUDragInfoStruct@@QEAU2@AEBU2@@Z @ 0x180168DDC (--$_Emplace_reallocate@AEBUDragInfoStruct@@@-$vector@UDragInfoStruct@@V-$allocator@UDragInfoStru.c)
 *     ??$_Emplace_reallocate@AEBUtagPOINT@@@?$vector@UtagPOINT@@V?$allocator@UtagPOINT@@@std@@@std@@QEAAPEAUtagPOINT@@QEAU2@AEBU2@@Z @ 0x180168F20 (--$_Emplace_reallocate@AEBUtagPOINT@@@-$vector@UtagPOINT@@V-$allocator@UtagPOINT@@@std@@@std@@QE.c)
 *     ??$_Insert_or_assign@AEBIAEA_K@?$unordered_map@I_KU?$hash@I@std@@U?$equal_to@I@2@V?$allocator@U?$pair@$$CBI_K@std@@@2@@std@@AEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBI_K@std@@@std@@@std@@@std@@_N@1@AEBIAEA_K@Z @ 0x180169150 (--$_Insert_or_assign@AEBIAEA_K@-$unordered_map@I_KU-$hash@I@std@@U-$equal_to@I@2@V-$allocator@U-.c)
 *     ??$_Try_emplace@AEBI$$V@?$_Hash@V?$_Umap_traits@I_KV?$_Uhash_compare@IU?$hash@I@std@@U?$equal_to@I@2@@std@@V?$allocator@U?$pair@$$CBI_K@std@@@2@$0A@@std@@@std@@IEAA?AU?$pair@PEAU?$_List_node@U?$pair@$$CBI_K@std@@PEAX@std@@_N@1@AEBI@Z @ 0x180169280 (--$_Try_emplace@AEBI$$V@-$_Hash@V-$_Umap_traits@I_KV-$_Uhash_compare@IU-$hash@I@std@@U-$equal_to.c)
 *     ??$_Try_emplace@AEBK$$V@?$_Hash@V?$_Umap_traits@KUMessageInfo@DragNDropProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUMessageInfo@DragNDropProcessor@@@std@@@4@$0A@@std@@@std@@IEAA?AU?$pair@PEAU?$_List_node@U?$pair@$$CBKUMessageInfo@DragNDropProcessor@@@std@@PEAX@std@@_N@1@AEBK@Z @ 0x180169394 (--$_Try_emplace@AEBK$$V@-$_Hash@V-$_Umap_traits@KUMessageInfo@DragNDropProcessor@@V-$_Uhash_comp.c)
 *     ??$_Try_emplace@AEBK$$V@?$_Hash@V?$_Umap_traits@KUtagPOINTF@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUtagPOINTF@@@std@@@3@$0A@@std@@@std@@IEAA?AU?$pair@PEAU?$_List_node@U?$pair@$$CBKUtagPOINTF@@@std@@PEAX@std@@_N@1@AEBK@Z @ 0x18016950C (--$_Try_emplace@AEBK$$V@-$_Hash@V-$_Umap_traits@KUtagPOINTF@@V-$_Uhash_compare@KU-$hash@K@std@@U.c)
 *     ??$_Try_emplace@AEBK$$V@?$_Hash@V?$_Umap_traits@KW4ContextualProcessorDecision@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKW4ContextualProcessorDecision@@@std@@@3@$0A@@std@@@std@@IEAA?AU?$pair@PEAU?$_List_node@U?$pair@$$CBKW4ContextualProcessorDecision@@@std@@PEAX@std@@_N@1@AEBK@Z @ 0x180169964 (--$_Try_emplace@AEBK$$V@-$_Hash@V-$_Umap_traits@KW4ContextualProcessorDecision@@V-$_Uhash_compar.c)
 *     ??$_Try_emplace@AEBK$$V@?$_Hash@V?$_Umap_traits@K_KV?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBK_K@std@@@2@$0A@@std@@@std@@IEAA?AU?$pair@PEAU?$_List_node@U?$pair@$$CBK_K@std@@PEAX@std@@_N@1@AEBK@Z @ 0x180169A80 (--$_Try_emplace@AEBK$$V@-$_Hash@V-$_Umap_traits@K_KV-$_Uhash_compare@KU-$hash@K@std@@U-$equal_to.c)
 *     ??$emplace@AEAIAEA_K@?$_Hash@V?$_Umap_traits@I_KV?$_Uhash_compare@IU?$hash@I@std@@U?$equal_to@I@2@@std@@V?$allocator@U?$pair@$$CBI_K@std@@@2@$0A@@std@@@std@@QEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBI_K@std@@@std@@@std@@@std@@_N@1@AEAIAEA_K@Z @ 0x180169B94 (--$emplace@AEAIAEA_K@-$_Hash@V-$_Umap_traits@I_KV-$_Uhash_compare@IU-$hash@I@std@@U-$equal_to@I@.c)
 *     ??$emplace@AEAKAEA_K@?$_Hash@V?$_Umap_traits@K_KV?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBK_K@std@@@2@$0A@@std@@@std@@QEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBK_K@std@@@std@@@std@@@std@@_N@1@AEAKAEA_K@Z @ 0x180169CBC (--$emplace@AEAKAEA_K@-$_Hash@V-$_Umap_traits@K_KV-$_Uhash_compare@KU-$hash@K@std@@U-$equal_to@K@.c)
 *     ??$emplace@AEAKUPointerCache@DragNDropProcessor@@@?$_Hash@V?$_Umap_traits@KUPointerCache@DragNDropProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUPointerCache@DragNDropProcessor@@@std@@@4@$0A@@std@@@std@@QEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKUPointerCache@DragNDropProcessor@@@std@@@std@@@std@@@std@@_N@1@AEAK$$QEAUPointerCache@DragNDropProcessor@@@Z @ 0x180169DE4 (--$emplace@AEAKUPointerCache@DragNDropProcessor@@@-$_Hash@V-$_Umap_traits@KUPointerCache@DragNDr.c)
 *     ??$emplace@AEAKUResultInfo@DragNDropProcessor@@@?$_Hash@V?$_Umap_traits@KUResultInfo@DragNDropProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUResultInfo@DragNDropProcessor@@@std@@@4@$0A@@std@@@std@@QEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKUResultInfo@DragNDropProcessor@@@std@@@std@@@std@@@std@@_N@1@AEAK$$QEAUResultInfo@DragNDropProcessor@@@Z @ 0x180169F78 (--$emplace@AEAKUResultInfo@DragNDropProcessor@@@-$_Hash@V-$_Umap_traits@KUResultInfo@DragNDropPr.c)
 *     ?CancelDrag@DragNDropProcessor@@QEAAJKPEAVBamoDragManagerClientProxy@@@Z @ 0x18016B020 (-CancelDrag@DragNDropProcessor@@QEAAJKPEAVBamoDragManagerClientProxy@@@Z.c)
 *     ?DragEnded@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Windows@@2@Z @ 0x18016B5FC (-DragEnded@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Windows.c)
 *     ?DragInProgress@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Windows@@2@Z @ 0x18016B750 (-DragInProgress@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Wi.c)
 *     ?DragStarted@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Windows@@2I22@Z @ 0x18016B8A4 (-DragStarted@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Windo.c)
 *     ?DropTargetChanged@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Windows@@_K2@Z @ 0x18016BA2C (-DropTargetChanged@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System.c)
 *     ?FeedAnimationDataIfNeeded@DragNDropProcessor@@AEAAXKUtagPOINT@@UtagPOINTF@@_N@Z @ 0x18016BCAC (-FeedAnimationDataIfNeeded@DragNDropProcessor@@AEAAXKUtagPOINT@@UtagPOINTF@@_N@Z.c)
 *     ?GetVelocityForPointerId@DragNDropProcessor@@AEAA?AUtagPOINTF@@K@Z @ 0x18016C55C (-GetVelocityForPointerId@DragNDropProcessor@@AEAA-AUtagPOINTF@@K@Z.c)
 *     ?HitTest@DragNDropProcessor@@AEAAJUtagPOINT@@W4InputType@@U_GUID@@_NPEAUHitTestResult@@PEAU2@PEA_K6PEAPEAXPEAPEAVBamoDragSourceClientProxy@@PEAPEAVBamoDragManagerClientProxy@@PEAPEAVInputSite@@PEAPEAV8@@Z @ 0x18016C694 (-HitTest@DragNDropProcessor@@AEAAJUtagPOINT@@W4InputType@@U_GUID@@_NPEAUHitTestResult@@PEAU2@PEA.c)
 *     ?MakeContextualProcessingDecision@DragNDropProcessor@@AEAAXAEBV?$ComPtr@VManualDragAreaClientProxy@@@WRL@Microsoft@@KPEAVInputContext@@PEAUContextualProcessorResponse@@@Z @ 0x18016CE94 (-MakeContextualProcessingDecision@DragNDropProcessor@@AEAAXAEBV-$ComPtr@VManualDragAreaClientPro.c)
 *     ?erase@?$_Hash@V?$_Umap_traits@KUMessageInfo@DragNDropProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUMessageInfo@DragNDropProcessor@@@std@@@4@$0A@@std@@@std@@QEAA_KAEBK@Z @ 0x18016F3D4 (-erase@-$_Hash@V-$_Umap_traits@KUMessageInfo@DragNDropProcessor@@V-$_Uhash_compare@KU-$hash@K@st.c)
 *     ?erase@?$_Hash@V?$_Umap_traits@KUPointerCache@ResizeProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUPointerCache@ResizeProcessor@@@std@@@4@$0A@@std@@@std@@QEAA_KAEBK@Z @ 0x18016F49C (-erase@-$_Hash@V-$_Umap_traits@KUPointerCache@ResizeProcessor@@V-$_Uhash_compare@KU-$hash@K@std@.c)
 *     ?erase@?$_Hash@V?$_Umap_traits@KUtagPOINT@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUtagPOINT@@@std@@@3@$0A@@std@@@std@@QEAA_KAEBK@Z @ 0x18016F618 (-erase@-$_Hash@V-$_Umap_traits@KUtagPOINT@@V-$_Uhash_compare@KU-$hash@K@std@@U-$equal_to@K@2@@st.c)
 *     ?erase@?$_Hash@V?$_Umap_traits@KV?$ComPtr@VDataSourcePrincipal@@@WRL@Microsoft@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKV?$ComPtr@VDataSourcePrincipal@@@WRL@Microsoft@@@std@@@5@$0A@@std@@@std@@QEAA_KAEBK@Z @ 0x18016F6C8 (-erase@-$_Hash@V-$_Umap_traits@KV-$ComPtr@VDataSourcePrincipal@@@WRL@Microsoft@@V-$_Uhash_compar.c)
 *     ?erase@?$_Hash@V?$_Umap_traits@KW4ContextualProcessorDecision@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKW4ContextualProcessorDecision@@@std@@@3@$0A@@std@@@std@@QEAA_KAEBK@Z @ 0x18016F790 (-erase@-$_Hash@V-$_Umap_traits@KW4ContextualProcessorDecision@@V-$_Uhash_compare@KU-$hash@K@std@.c)
 *     ?GetInputType@PointerManipulationHelper@@SA?AW4InputType@Input@Internal@UI@Windows@@W42@@Z @ 0x1801C8F8C (-GetInputType@PointerManipulationHelper@@SA-AW4InputType@Input@Internal@UI@Windows@@W42@@Z.c)
 *     ?GetKeyModifiers@PointerManipulationHelper@@SA?AW4VirtualKeyModifiers@System@Windows@@PEAUInputInfo@@@Z @ 0x1801C8FC8 (-GetKeyModifiers@PointerManipulationHelper@@SA-AW4VirtualKeyModifiers@System@Windows@@PEAUInputI.c)
 *     ?GetPositionRelativeToSite@PointerManipulationHelper@@SA?AUtagPOINT@@PEAVInputSite@@U2@@Z @ 0x1801C8FFC (-GetPositionRelativeToSite@PointerManipulationHelper@@SA-AUtagPOINT@@PEAVInputSite@@U2@@Z.c)
 *     ?GetTransformFromRootToSite@PointerManipulationHelper@@SA?AUtagINPUT_TRANSFORM@@PEAVInputSite@@@Z @ 0x1801C90A4 (-GetTransformFromRootToSite@PointerManipulationHelper@@SA-AUtagINPUT_TRANSFORM@@PEAVInputSite@@@.c)
 *     ?OnPointerInfoFrame@DragGestureTracker@@QEAAJAEAUtagPOINTER_INFO@@@Z @ 0x1801C915C (-OnPointerInfoFrame@DragGestureTracker@@QEAAJAEAUtagPOINTER_INFO@@@Z.c)
 */

// Hidden C++ exception states: #wind=4
__int64 __fastcall DragNDropProcessor::OnInput(
        DragNDropProcessor *this,
        struct InputInfo *a2,
        struct InputContext *a3,
        struct ContextualProcessorResponse *a4)
{
  struct ContextualProcessorResponse *v4; // r14
  int *v5; // r12
  unsigned int KeyModifiers; // ebx
  __int64 v8; // rsi
  _QWORD *v9; // rcx
  __int64 v10; // r13
  struct tagPOINT v11; // rdi
  __int64 v12; // rax
  __int64 v13; // rbx
  __int64 v14; // rcx
  __int128 v15; // xmm8
  __int64 v16; // rax
  __int64 v17; // rax
  DragGestureTracker *v18; // rcx
  __int64 v19; // rax
  int v20; // xmm6_4
  int v21; // xmm7_4
  __int64 v22; // rcx
  _DWORD *v23; // rax
  struct InputSite *v24; // rcx
  struct InputSite *v25; // rcx
  __int64 v26; // r14
  unsigned int v27; // esi
  __int64 v28; // rax
  unsigned __int64 v29; // r13
  __int64 v30; // rax
  __int64 v31; // rax
  struct Microsoft::Bamo::BaseBamoConnection *v32; // rdx
  _BYTE *v33; // rdx
  __int64 v34; // r8
  const char *v35; // r9
  _QWORD *v36; // rsi
  _QWORD *v37; // r14
  __int64 v38; // r14
  __int128 *TransformFromRootToSite; // rax
  float *v40; // rax
  struct tagPOINT v41; // rsi
  __int64 v42; // rax
  struct InputSite *v43; // rcx
  float v44; // xmm0_4
  __int64 v45; // r14
  unsigned int v46; // esi
  __int64 v47; // rax
  __int64 v48; // rsi
  __int64 v49; // r14
  unsigned int v50; // edi
  __int64 v51; // rax
  void *v52; // rcx
  unsigned int v53; // esi
  __int64 v54; // rax
  __int64 v55; // r8
  const char *v56; // r9
  ManualDragAreaClientProxy *v57; // rcx
  char *v58; // rbx
  unsigned int v59; // ebx
  _QWORD *v60; // rax
  __int64 v61; // rcx
  int v63; // [rsp+50h] [rbp-B8h]
  __int64 v64; // [rsp+58h] [rbp-B0h]
  unsigned int v65; // [rsp+78h] [rbp-90h] BYREF
  int v66; // [rsp+7Ch] [rbp-8Ch]
  unsigned int v67; // [rsp+80h] [rbp-88h] BYREF
  unsigned int v68; // [rsp+84h] [rbp-84h]
  struct tagPOINT PositionRelativeToSite; // [rsp+88h] [rbp-80h] BYREF
  struct tagPOINT v70; // [rsp+90h] [rbp-78h] BYREF
  unsigned __int64 v71; // [rsp+98h] [rbp-70h] BYREF
  int v72; // [rsp+A0h] [rbp-68h]
  struct tagPOINT v73; // [rsp+A8h] [rbp-60h] BYREF
  __int64 v74; // [rsp+B0h] [rbp-58h] BYREF
  unsigned int v75; // [rsp+B8h] [rbp-50h]
  __int64 v76; // [rsp+C0h] [rbp-48h] BYREF
  unsigned int InputType; // [rsp+C8h] [rbp-40h]
  struct ContextualProcessorResponse *v78; // [rsp+D0h] [rbp-38h]
  __int128 v79; // [rsp+D8h] [rbp-30h] BYREF
  _BYTE *v80; // [rsp+E8h] [rbp-20h]
  __int64 v81; // [rsp+F0h] [rbp-18h]
  __int64 *v82; // [rsp+F8h] [rbp-10h]
  struct tagPOINT v83; // [rsp+100h] [rbp-8h]
  struct tagPOINT v84; // [rsp+108h] [rbp+0h]
  struct tagPOINT v85; // [rsp+110h] [rbp+8h] BYREF
  __int64 v86; // [rsp+118h] [rbp+10h] BYREF
  __int64 v87; // [rsp+120h] [rbp+18h] BYREF
  struct InputInfo *v88; // [rsp+128h] [rbp+20h]
  __int128 v89; // [rsp+138h] [rbp+30h] BYREF
  __int64 v90; // [rsp+148h] [rbp+40h] BYREF
  float v91; // [rsp+150h] [rbp+48h] BYREF
  __int64 v92; // [rsp+158h] [rbp+50h] BYREF
  Microsoft::BamoImpl::BamoImplObject *v93[3]; // [rsp+160h] [rbp+58h] BYREF
  void *v94[2]; // [rsp+178h] [rbp+70h] BYREF
  __int64 v95; // [rsp+188h] [rbp+80h]
  struct tagPOINT v96; // [rsp+190h] [rbp+88h]
  unsigned int v97[4]; // [rsp+1B8h] [rbp+B0h] BYREF
  __int128 v98[4]; // [rsp+1C8h] [rbp+C0h] BYREF
  _BYTE v99[16]; // [rsp+208h] [rbp+100h] BYREF
  _BYTE v100[16]; // [rsp+218h] [rbp+110h] BYREF
  _BYTE v101[16]; // [rsp+228h] [rbp+120h] BYREF
  _BYTE v102[16]; // [rsp+238h] [rbp+130h] BYREF
  _BYTE v103[16]; // [rsp+248h] [rbp+140h] BYREF
  _BYTE v104[16]; // [rsp+258h] [rbp+150h] BYREF
  _BYTE v105[16]; // [rsp+268h] [rbp+160h] BYREF
  _BYTE v106[16]; // [rsp+278h] [rbp+170h] BYREF
  _BYTE v107[16]; // [rsp+288h] [rbp+180h] BYREF
  _BYTE v108[16]; // [rsp+298h] [rbp+190h] BYREF
  _BYTE v109[16]; // [rsp+2A8h] [rbp+1A0h] BYREF
  _BYTE v110[16]; // [rsp+2B8h] [rbp+1B0h] BYREF
  __int128 v111; // [rsp+2C8h] [rbp+1C0h] BYREF
  unsigned __int64 v112; // [rsp+2D8h] [rbp+1D0h]
  float v113[2]; // [rsp+2E0h] [rbp+1D8h] BYREF
  __int64 v114; // [rsp+2E8h] [rbp+1E0h]
  struct tagPOINTF v115; // [rsp+2F0h] [rbp+1E8h]
  __int128 v116; // [rsp+2F8h] [rbp+1F0h] BYREF
  __int128 v117; // [rsp+308h] [rbp+200h]

  v4 = a4;
  v78 = a4;
  v82 = (__int64 *)a3;
  v5 = (int *)a2;
  v88 = a2;
  *((_DWORD *)a4 + 2) = 4;
  KeyModifiers = PointerManipulationHelper::GetKeyModifiers(a2);
  v68 = KeyModifiers;
  v75 = *v5;
  InputType = PointerManipulationHelper::GetInputType(v75);
  v8 = 0LL;
  v72 = 0;
  if ( v5[79] )
  {
    v9 = (_QWORD *)((char *)this + 48);
    while ( 1 )
    {
      v10 = 144 * v8;
      v81 = 144 * v8;
      v65 = v5[36 * v8 + 81];
      v11 = *(struct tagPOINT *)&v5[36 * v8 + 92];
      v83 = v11;
      if ( (v5[36 * v8 + 83] & 4) != 0 )
      {
        std::_Hash<std::_Umap_traits<unsigned long,EdgyProcessor::PointerCache,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,EdgyProcessor::PointerCache>>,0>>::find<void>(
          v9,
          &v90,
          (const unsigned __int8 *)&v65);
        v12 = v90;
        if ( v90 == *((_QWORD *)this + 7) )
        {
          *(struct tagPOINT *)&v111 = v11;
          *((struct tagPOINT *)&v111 + 1) = v11;
          v112 = __PAIR64__(KeyModifiers, v75);
          std::_Hash<std::_Umap_traits<unsigned long,DragNDropProcessor::PointerCache,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,DragNDropProcessor::PointerCache>>,0>>::emplace<unsigned long &,DragNDropProcessor::PointerCache>(
            (float *)this + 12,
            (__int64)v99,
            (unsigned __int8 *)&v65,
            (__int64)&v111);
        }
        else
        {
          *(struct tagPOINT *)(v90 + 28) = v11;
          *(_DWORD *)(v12 + 40) = KeyModifiers;
        }
      }
      else
      {
        std::_Hash<std::_Umap_traits<unsigned long,ResizeProcessor::PointerCache,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,ResizeProcessor::PointerCache>>,0>>::erase(
          v9,
          &v65);
      }
      std::_Hash<std::_Umap_traits<unsigned long,EdgyProcessor::PointerCache,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,EdgyProcessor::PointerCache>>,0>>::find<void>(
        (_QWORD *)this + 14,
        &v87,
        (const unsigned __int8 *)&v65);
      v13 = v87;
      if ( v87 == *((_QWORD *)this + 15) )
        goto LABEL_75;
      v14 = *(_QWORD *)(v87 + 32);
      if ( !v14 )
        goto LABEL_75;
      v73 = v11;
      v76 = 0LL;
      v74 = 0LL;
      v86 = 0LL;
      v15 = *(_OWORD *)(*(__int64 (__fastcall **)(__int64))(*(_QWORD *)(v14 + 8) + 80LL))(v14 + 8);
      v89 = v15;
      if ( (int)DragNDropProcessor::HitTest(
                  (__int64)this,
                  v11,
                  v75,
                  &v89,
                  0,
                  0LL,
                  &v73,
                  &v76,
                  &v74,
                  &v86,
                  0LL,
                  0LL,
                  0LL,
                  0LL) < 0
        || (v16 = *(_QWORD *)(v13 + 56)) != 0 && !*(_BYTE *)(v16 + 480)
        || (v17 = *(_QWORD *)(v13 + 48)) != 0 && !*(_BYTE *)(v17 + 480) )
      {
        if ( **(_DWORD **)(v13 + 64) == 4 )
          DragNDropProcessor::CancelDrag(this, v65, *(struct BamoDragManagerClientProxy **)(v13 + 32));
        goto LABEL_75;
      }
      v18 = *(DragGestureTracker **)(v13 + 64);
      if ( v18 )
      {
        DragGestureTracker::OnPointerInfoFrame(v18, (struct tagPOINTER_INFO *)((char *)v5 + v10 + 320));
        v19 = *(_QWORD *)(v13 + 64);
        v20 = *(_DWORD *)(v19 + 4);
        v21 = *(_DWORD *)(v19 + 8);
        v22 = *(_QWORD *)std::_Hash<std::_Umap_traits<unsigned long,tagPOINTF,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,tagPOINTF>>,0>>::_Try_emplace<unsigned long const &,>(
                           (float *)this + 76,
                           (__int64)v100,
                           (unsigned __int8 *)&v65);
        *(_DWORD *)(v22 + 20) = v20;
        *(_DWORD *)(v22 + 24) = v21;
      }
      v23 = *(_DWORD **)(v13 + 64);
      if ( *v23 == 2 )
      {
        if ( (*((_BYTE *)v5 + v10 + 332) & 4) != 0 )
        {
          DragGestureTracker::OnPointerInfoFrame(
            *(DragGestureTracker **)(v13 + 64),
            (struct tagPOINTER_INFO *)((char *)v5 + v10 + 320));
          *(_DWORD *)(*(_QWORD *)std::_Hash<std::_Umap_traits<unsigned long,enum ContextualProcessorDecision,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,enum ContextualProcessorDecision>>,0>>::_Try_emplace<unsigned long const &,>(
                                   (_QWORD *)v4 + 2,
                                   (__int64)v101,
                                   (unsigned __int8 *)&v65)
                    + 20LL) = 1;
        }
        else
        {
          *v23 = 1;
        }
      }
      if ( **(_DWORD **)(v13 + 64) == 1 )
        *(_DWORD *)(*(_QWORD *)std::_Hash<std::_Umap_traits<unsigned long,enum ContextualProcessorDecision,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,enum ContextualProcessorDecision>>,0>>::_Try_emplace<unsigned long const &,>(
                                 (_QWORD *)v4 + 2,
                                 (__int64)v102,
                                 (unsigned __int8 *)&v65)
                  + 20LL) = 4;
      v114 = 0LL;
      v115 = 0LL;
      v113[0] = (float)v11.x;
      v113[1] = (float)v83.y;
      v115 = **(struct tagPOINTF **)&DragNDropProcessor::GetVelocityForPointerId(this, v97, v65);
      PositionRelativeToSite = v11;
      v24 = *(struct InputSite **)(v13 + 56);
      if ( v24 )
        PositionRelativeToSite = PointerManipulationHelper::GetPositionRelativeToSite(v24, v11);
      v70 = 0LL;
      v25 = *(struct InputSite **)(v13 + 48);
      if ( v25 )
        v70 = PointerManipulationHelper::GetPositionRelativeToSite(v25, v11);
      if ( **(_DWORD **)(v13 + 64) == 3
        && !std::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>::count<void>(
              (__int64)v4 + 152,
              (const unsigned __int8 *)&v65) )
      {
        v26 = (*(__int64 (__fastcall **)(__int64))(*(_QWORD *)(*(_QWORD *)(v13 + 24) + 8LL) + 32LL))(*(_QWORD *)(v13 + 24) + 8LL);
        v76 = 0LL;
        v27 = v65;
        v28 = (*(__int64 (__fastcall **)(DragNDropProcessor *))(*(_QWORD *)this + 48LL))(this);
        InputETW::InputGesture::DragStarted(
          v28,
          v27,
          (int *)&PositionRelativeToSite,
          (int *)&v73,
          (int *)&v70,
          0LL,
          v68,
          v74,
          InputType,
          v26,
          v64);
        (*(void (__fastcall **)(__int64, _QWORD, struct tagPOINT *, struct tagPOINT *, struct tagPOINT *, float *, _QWORD, unsigned int, __int64, unsigned int, __int64, _QWORD, _DWORD))(*(_QWORD *)(*(_QWORD *)(v13 + 32) + 8LL) + 24LL))(
          *(_QWORD *)(v13 + 32) + 8LL,
          v65,
          &PositionRelativeToSite,
          &v73,
          &v70,
          v113,
          0LL,
          v68,
          v74,
          InputType,
          v26,
          0LL,
          0);
        std::_Hash<std::_Umap_traits<unsigned long,unsigned __int64,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,unsigned __int64>>,0>>::emplace<unsigned long &,unsigned __int64 &>(
          (_QWORD *)this + 30,
          (__int64)v103,
          (unsigned __int8 *)&v65,
          &v76);
        **(_DWORD **)(v13 + 64) = 4;
      }
      if ( **(_DWORD **)(v13 + 64) == 4 )
        break;
LABEL_74:
      LODWORD(v8) = v72;
LABEL_75:
      v58 = (char *)this + 432;
      if ( std::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>::count<void>(
             (__int64)this + 432,
             (const unsigned __int8 *)&v65) )
      {
        v59 = v65;
        v60 = (_QWORD *)std::_Hash<std::_Umap_traits<unsigned long,DragNDropProcessor::MessageInfo,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,DragNDropProcessor::MessageInfo>>,0>>::_Try_emplace<unsigned long const &,>(
                          (float *)this + 28,
                          (__int64)v110,
                          (unsigned __int8 *)&v65);
        v4 = v78;
        DragNDropProcessor::MakeContextualProcessingDecision(v61, *v60 + 40LL, v59, v82, (__int64)v78);
        v58 = (char *)this + 432;
      }
      else
      {
        v4 = v78;
      }
      if ( (*((_BYTE *)v5 + v10 + 332) & 4) == 0 )
      {
        std::_Hash<std::_Umap_traits<unsigned long,enum ContextualProcessorDecision,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,enum ContextualProcessorDecision>>,0>>::erase(
          v58,
          &v65);
        std::_Hash<std::_Umap_traits<unsigned long,Microsoft::WRL::ComPtr<DataSourcePrincipal>,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,Microsoft::WRL::ComPtr<DataSourcePrincipal>>>,0>>::erase(
          (char *)this + 368,
          &v65);
        std::_Hash<std::_Umap_traits<unsigned long,DragNDropProcessor::MessageInfo,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,DragNDropProcessor::MessageInfo>>,0>>::erase(
          (char *)this + 112,
          &v65);
        std::_Hash<std::_Umap_traits<unsigned long,tagPOINT,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,tagPOINT>>,0>>::erase(
          (char *)this + 304,
          &v65);
        std::_Hash<std::_Umap_traits<unsigned long,tagPOINT,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,tagPOINT>>,0>>::erase(
          (char *)this + 240,
          &v65);
      }
      v8 = (unsigned int)(v8 + 1);
      v72 = v8;
      KeyModifiers = v68;
      v9 = (_QWORD *)((char *)this + 48);
      if ( (unsigned int)v8 >= v5[79] )
        return 0LL;
    }
    *(_QWORD *)&v89 = *(_QWORD *)(*(_QWORD *)std::_Hash<std::_Umap_traits<unsigned long,unsigned __int64,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,unsigned __int64>>,0>>::_Try_emplace<unsigned long const &,>(
                                               (_QWORD *)this + 30,
                                               (__int64)v104,
                                               (unsigned __int8 *)&v65)
                                + 24LL);
    DragNDropProcessor::FeedAnimationDataIfNeeded(this, v65, PositionRelativeToSite, v115, 0);
    v79 = 0LL;
    v80 = 0LL;
    v111 = 0LL;
    v29 = 0LL;
    v112 = 0LL;
    LOBYTE(v66) = 0;
    v30 = *(_QWORD *)(v13 + 24);
    if ( !v30 )
      goto LABEL_55;
    v71 = 0LL;
    v67 = 0;
    v31 = *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(v30 + 32) + 24LL) + 32LL);
    if ( *(int *)(v31 + 8) <= 0 )
      v32 = 0LL;
    else
      v32 = *(struct Microsoft::Bamo::BaseBamoConnection **)(v31 + 16);
    Microsoft::Bamo::Lock::Lock(v93, v32);
    (*(void (__fastcall **)(__int64, unsigned __int64 *, unsigned int *))(*(_QWORD *)(*(_QWORD *)(v13 + 24) + 8LL) + 48LL))(
      *(_QWORD *)(v13 + 24) + 8LL,
      &v71,
      &v67);
    v36 = (_QWORD *)v71;
    v37 = (_QWORD *)(v71 + 8LL * v67);
    if ( (_QWORD *)v71 != v37 )
    {
      v33 = (_BYTE *)*((_QWORD *)&v79 + 1);
      do
      {
        if ( v33 == v80 )
        {
          std::vector<tagPOINT>::_Emplace_reallocate<tagPOINT const &>((const void **)&v79, v33, v36);
          v33 = (_BYTE *)*((_QWORD *)&v79 + 1);
        }
        else
        {
          *(_QWORD *)v33 = *v36;
          v33 = (_BYTE *)(*((_QWORD *)&v79 + 1) + 8LL);
          *((_QWORD *)&v79 + 1) += 8LL;
        }
        ++v36;
      }
      while ( v36 != v37 );
    }
    Microsoft::Bamo::Lock::~Lock((Microsoft::Bamo::Lock *)v93, (__int64)v33, v34, v35);
    v67 = 0;
    if ( (__int64)(*((_QWORD *)&v79 + 1) - v79) >> 3 )
    {
      v38 = *((_QWORD *)&v111 + 1);
      do
      {
        TransformFromRootToSite = (__int128 *)PointerManipulationHelper::GetTransformFromRootToSite(
                                                v94,
                                                *(_QWORD *)(v13 + 48));
        v98[0] = *TransformFromRootToSite;
        v98[1] = TransformFromRootToSite[1];
        v98[2] = TransformFromRootToSite[2];
        v98[3] = TransformFromRootToSite[3];
        v71 = _mm_unpacklo_ps(
                (__m128)COERCE_UNSIGNED_INT((float)*(int *)(v79 + 8LL * v67)),
                (__m128)COERCE_UNSIGNED_INT((float)*(int *)(v79 + 8LL * v67 + 4))).m128_u64[0];
        v40 = (float *)CInputTransform::RelativePointToScreen(&v91, (float *)&v71, v98);
        v84.x = v11.x + (int)*v40 - *(_DWORD *)(v13 + 72);
        v84.y = v83.y + (int)v40[1] - *(_DWORD *)(v13 + 76);
        v71 = 0LL;
        if ( *std::_Hash<std::_Umap_traits<unsigned int,unsigned __int64,std::_Uhash_compare<unsigned int,std::hash<unsigned int>,std::equal_to<unsigned int>>,std::allocator<std::pair<unsigned int const,unsigned __int64>>,0>>::find<void>(
                (_QWORD *)(v13 + 80),
                &v92,
                (const unsigned __int8 *)&v67) == *(_QWORD *)(v13 + 88) )
          std::_Hash<std::_Umap_traits<unsigned int,unsigned __int64,std::_Uhash_compare<unsigned int,std::hash<unsigned int>,std::equal_to<unsigned int>>,std::allocator<std::pair<unsigned int const,unsigned __int64>>,0>>::emplace<unsigned int &,unsigned __int64 &>(
            (_QWORD *)(v13 + 80),
            (__int64)v109,
            (unsigned __int8 *)&v67,
            &v71);
        *(_OWORD *)v94 = v15;
        v41 = v84;
        if ( (int)DragNDropProcessor::HitTest(
                    (__int64)this,
                    v84,
                    v75,
                    v94,
                    0,
                    0LL,
                    &v85,
                    &v71,
                    &v74,
                    0LL,
                    0LL,
                    0LL,
                    0LL,
                    0LL) >= 0 )
        {
          v42 = std::_Hash<std::_Umap_traits<unsigned int,unsigned __int64,std::_Uhash_compare<unsigned int,std::hash<unsigned int>,std::equal_to<unsigned int>>,std::allocator<std::pair<unsigned int const,unsigned __int64>>,0>>::_Try_emplace<unsigned int const &,>(
                  (_QWORD *)(v13 + 80),
                  (__int64)v105,
                  (unsigned __int8 *)&v67);
          if ( v71 != *(_QWORD *)(*(_QWORD *)v42 + 24LL) )
          {
            LOBYTE(v66) = 1;
            std::unordered_map<unsigned int,unsigned __int64>::_Insert_or_assign<unsigned int const &,unsigned __int64 &>(
              (_QWORD *)(v13 + 80),
              (__int64)v106,
              (unsigned __int8 *)&v67,
              &v71);
          }
        }
        v43 = *(struct InputSite **)(v13 + 56);
        if ( v43 )
          v41 = PointerManipulationHelper::GetPositionRelativeToSite(v43, v41);
        *(_QWORD *)&v116 = v71;
        *((float *)&v116 + 2) = (float)v41.x;
        *((float *)&v116 + 3) = (float)v41.y;
        *((float *)&v117 + 2) = (float)v85.x;
        *((float *)&v117 + 3) = (float)v85.y;
        v44 = (float)*(int *)(v79 + 8LL * v67 + 4);
        *(float *)&v117 = (float)*(int *)(v79 + 8LL * v67);
        *((float *)&v117 + 1) = v44;
        if ( v38 == v29 )
        {
          std::vector<DragInfoStruct>::_Emplace_reallocate<DragInfoStruct const &>(
            (const void **)&v111,
            (_BYTE *)v38,
            &v116);
          v29 = v112;
          v38 = *((_QWORD *)&v111 + 1);
        }
        else
        {
          *(_OWORD *)v38 = v116;
          *(_OWORD *)(v38 + 16) = v117;
          v38 += 32LL;
          *((_QWORD *)&v111 + 1) = v38;
        }
        ++v67;
      }
      while ( v67 < (unsigned __int64)((__int64)(*((_QWORD *)&v79 + 1) - v79) >> 3) );
      v5 = (int *)v88;
    }
    else
    {
LABEL_55:
      v38 = *((_QWORD *)&v111 + 1);
    }
    if ( v76 == (_QWORD)v89 && !(_BYTE)v66 )
    {
      v49 = v76;
      v48 = v74;
    }
    else
    {
      v45 = (v38 - (__int64)v111) >> 5;
      v46 = v65;
      v47 = (*(__int64 (__fastcall **)(DragNDropProcessor *))(*(_QWORD *)this + 48LL))(this);
      InputETW::InputGesture::DropTargetChanged(
        v47,
        v46,
        (int *)&PositionRelativeToSite,
        (int *)&v73,
        (int *)&v70,
        v76,
        v68,
        v45,
        v74);
      v48 = v74;
      v63 = v45;
      v49 = v76;
      (*(void (__fastcall **)(__int64, _QWORD, struct tagPOINT *, struct tagPOINT *, struct tagPOINT *, float *, __int64, unsigned int, _QWORD, int, __int64))(*(_QWORD *)(*(_QWORD *)(v13 + 32) + 8LL) + 56LL))(
        *(_QWORD *)(v13 + 32) + 8LL,
        v65,
        &PositionRelativeToSite,
        &v73,
        &v70,
        v113,
        v76,
        v68,
        v111,
        v63,
        v74);
    }
    *(_QWORD *)(*(_QWORD *)std::_Hash<std::_Umap_traits<unsigned long,unsigned __int64,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,unsigned __int64>>,0>>::_Try_emplace<unsigned long const &,>(
                             (_QWORD *)this + 30,
                             (__int64)v107,
                             (unsigned __int8 *)&v65)
              + 24LL) = v49;
    if ( (v5[(unsigned __int64)v81 / 4 + 83] & 4) != 0 )
    {
      DragNDropProcessor::FeedAnimationDataIfNeeded(this, v65, PositionRelativeToSite, v115, 0);
      if ( std::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>::count<void>(
             (__int64)v78 + 152,
             (const unsigned __int8 *)&v65) )
      {
        DragNDropProcessor::CancelDrag(this, v65, *(struct BamoDragManagerClientProxy **)(v13 + 32));
LABEL_69:
        if ( (_QWORD)v111 )
          std::_Deallocate<16,0>((void *)v111, (v29 - v111) & 0xFFFFFFFFFFFFFFE0uLL);
        if ( (_QWORD)v79 )
          std::_Deallocate<16,0>((void *)v79, (unsigned __int64)&v80[-v79] & 0xFFFFFFFFFFFFFFF8uLL);
        v10 = v81;
        goto LABEL_74;
      }
      v50 = v65;
      v51 = (*(__int64 (__fastcall **)(DragNDropProcessor *))(*(_QWORD *)this + 48LL))(this);
      InputETW::InputGesture::DragInProgress(
        v51,
        v50,
        (int *)&PositionRelativeToSite,
        (int *)&v73,
        (int *)&v70,
        v49,
        v68,
        v48);
      (*(void (__fastcall **)(__int64, _QWORD, struct tagPOINT *, struct tagPOINT *, struct tagPOINT *, float *, __int64, unsigned int, __int64))(*(_QWORD *)(*(_QWORD *)(v13 + 32) + 8LL) + 32LL))(
        *(_QWORD *)(v13 + 32) + 8LL,
        v65,
        &PositionRelativeToSite,
        &v73,
        &v70,
        v113,
        v49,
        v68,
        v48);
    }
    else
    {
      DragNDropProcessor::FeedAnimationDataIfNeeded(this, v65, PositionRelativeToSite, v115, 1u);
      v53 = v65;
      v54 = (*(__int64 (__fastcall **)(DragNDropProcessor *))(*(_QWORD *)this + 48LL))(this);
      InputETW::InputGesture::DragEnded(
        v54,
        v53,
        (int *)&PositionRelativeToSite,
        (int *)&v73,
        (int *)&v70,
        v49,
        v68,
        v74);
      (*(void (__fastcall **)(__int64, _QWORD, struct tagPOINT *, struct tagPOINT *, struct tagPOINT *, float *, __int64, unsigned int, __int64))(*(_QWORD *)(*(_QWORD *)(v13 + 32) + 8LL) + 40LL))(
        *(_QWORD *)(v13 + 32) + 8LL,
        v65,
        &PositionRelativeToSite,
        &v73,
        &v70,
        v113,
        v49,
        v68,
        v74);
      v57 = *(ManualDragAreaClientProxy **)(v13 + 40);
      if ( v57 )
        ManualDragAreaClientProxy::OnManualDragEnded(v57);
      VariableSizedPayloadStorage<InputInfo>::VariableSizedPayloadStorage<InputInfo>((__int64)v94, v5, v55, v56);
      v95 = v86;
      v96 = v11;
      std::_Hash<std::_Umap_traits<unsigned long,DragNDropProcessor::ResultInfo,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,DragNDropProcessor::ResultInfo>>,0>>::emplace<unsigned long &,DragNDropProcessor::ResultInfo>(
        (float *)this + 44,
        (__int64)v108,
        (unsigned __int8 *)&v65,
        (__int64 *)v94);
      v52 = v94[0];
      if ( v94[0] )
        operator delete[](v94[0]);
    }
    DragNDropProcessor::MakeContextualProcessingDecision((__int64)v52, v13 + 40, v65, v82, (__int64)v78);
    goto LABEL_69;
  }
  return 0LL;
}
