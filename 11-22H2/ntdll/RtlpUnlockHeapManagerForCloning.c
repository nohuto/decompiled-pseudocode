/*
 * XREFs of RtlpUnlockHeapManagerForCloning @ 0x1800FFD58
 * Callers:
 *     RtlLockHeapManagerForCloning @ 0x1800FD7E0 (RtlLockHeapManagerForCloning.c)
 *     RtlUnlockHeapManagerForCloning @ 0x1800FE420 (RtlUnlockHeapManagerForCloning.c)
 * Callees:
 *     RtlpWakeByAddress @ 0x18002A1DC (RtlpWakeByAddress.c)
 *     RtlReleaseSRWLockExclusive @ 0x180033DA0 (RtlReleaseSRWLockExclusive.c)
 *     RtlBackoff @ 0x180033F80 (RtlBackoff.c)
 *     RtlpCSparseBitmapUnlock @ 0x18004AE04 (RtlpCSparseBitmapUnlock.c)
 *     RtlpCreateDeferredCriticalSectionEvent @ 0x180082DF0 (RtlpCreateDeferredCriticalSectionEvent.c)
 *     ZwSetEvent @ 0x18009EFB0 (ZwSetEvent.c)
 *     RtlpNotOwnerCriticalSection @ 0x1800F3940 (RtlpNotOwnerCriticalSection.c)
 *     RtlRaiseStatus @ 0x18010F220 (RtlRaiseStatus.c)
 *     RtlpHpUnlockHeapForCloning @ 0x180121F54 (RtlpHpUnlockHeapForCloning.c)
 */

signed __int64 __fastcall RtlpUnlockHeapManagerForCloning(unsigned int a1, unsigned int a2)
{
  struct _PEB *v2; // r13
  unsigned int i; // ebp
  void **ProcessHeaps; // rax
  void *v7; // rbx
  volatile signed __int64 *v8; // rcx
  __int64 v9; // rdx
  __int64 v10; // rbx
  _BYTE *v12; // rdi
  signed __int32 v13; // r14d
  HANDLE DeferredCriticalSectionEvent; // r10
  int v15; // eax
  signed __int64 result; // rax
  signed __int32 v17; // ebx
  HANDLE v18; // r10
  signed __int32 v19[8]; // [rsp+0h] [rbp-68h] BYREF
  int v20; // [rsp+20h] [rbp-48h] BYREF
  char v21; // [rsp+24h] [rbp-44h]
  void *v22; // [rsp+28h] [rbp-40h]
  unsigned int v23; // [rsp+78h] [rbp+10h] BYREF
  unsigned int v24; // [rsp+80h] [rbp+18h] BYREF

  v2 = NtCurrentPeb();
  for ( i = 0; i < a2; ++i )
  {
    ProcessHeaps = v2->ProcessHeaps;
    v7 = ProcessHeaps[i];
    if ( *((_DWORD *)v7 + 4) == -571548178 )
    {
      if ( (*((_BYTE *)v7 + 20) & 1) == 0 )
        RtlpHpUnlockHeapForCloning(ProcessHeaps[i], a1);
    }
    else if ( (*((_BYTE *)v7 + 112) & 1) == 0 )
    {
      if ( *((_BYTE *)v7 + 418) == 2 )
      {
        v8 = (volatile signed __int64 *)*((_QWORD *)v7 + 51);
        if ( v8 )
        {
          if ( a1 )
            *v8 = 1LL;
          RtlReleaseSRWLockExclusive(v8);
        }
      }
      if ( a1 )
      {
        v9 = *((_QWORD *)v7 + 44);
        *((_WORD *)v7 + 208) = 0;
        *(_QWORD *)(v9 + 16) = NtCurrentTeb()->ClientId.UniqueThread;
        *(_DWORD *)(v9 + 8) = -2;
        *(_DWORD *)(v9 + 12) = 1;
        *(_QWORD *)(v9 + 24) = 0LL;
      }
      v10 = *((_QWORD *)v7 + 44);
      if ( (*(_DWORD *)(v10 + 12))-- == 1 )
      {
        *(_QWORD *)(v10 + 16) = 0LL;
        v12 = (_BYTE *)(v10 + 8);
        v13 = _InterlockedCompareExchange((volatile signed __int32 *)(v10 + 8), -1, -2);
        if ( v13 != -2 )
        {
          if ( (*v12 & 1) != 0 )
            RtlpNotOwnerCriticalSection((const void **)v10);
          DeferredCriticalSectionEvent = *(HANDLE *)(v10 + 24);
          if ( !DeferredCriticalSectionEvent )
            DeferredCriticalSectionEvent = RtlpCreateDeferredCriticalSectionEvent(v10);
          v23 = 0;
          while ( v13 != _InterlockedCompareExchange((volatile signed __int32 *)v12, (v13 & 2 | 1) + v13, v13) )
          {
            RtlBackoff(&v23);
            _m_prefetchw(v12);
            v13 = *(_DWORD *)v12;
          }
          if ( (v13 & 2) != 0 )
          {
            if ( DeferredCriticalSectionEvent == (HANDLE)-1LL )
            {
              _InterlockedOr(v19, 0);
              RtlpWakeByAddress(v10 + 8, 0);
            }
            else
            {
              v15 = ZwSetEvent();
              if ( v15 < 0 )
                RtlRaiseStatus((unsigned int)v15);
            }
          }
        }
      }
    }
  }
  if ( a1 )
  {
    qword_180182FD0 = (__int64)NtCurrentTeb()->ClientId.UniqueThread;
    dword_180182FC8 = -2;
    dword_180182FCC = 1;
    qword_180182FD8 = 0LL;
    qword_1801859E0 = -1LL;
    qword_1801859D8 = 1LL;
  }
  v20 = 1;
  v22 = &unk_1801859C0;
  v21 = -1;
  result = RtlpCSparseBitmapUnlock((__int64)&v20);
  if ( !--dword_180182FCC )
  {
    qword_180182FD0 = 0LL;
    result = (unsigned int)_InterlockedCompareExchange(&dword_180182FC8, -1, -2);
    v17 = result;
    if ( (_DWORD)result != -2 )
    {
      if ( (dword_180182FC8 & 1) != 0 )
        RtlpNotOwnerCriticalSection(&RtlpProcessHeapsListLock);
      v18 = (HANDLE)qword_180182FD8;
      if ( !qword_180182FD8 )
        v18 = RtlpCreateDeferredCriticalSectionEvent((__int64)&RtlpProcessHeapsListLock);
      v24 = 0;
      while ( 1 )
      {
        result = (unsigned int)_InterlockedCompareExchange(&dword_180182FC8, (v17 & 2 | 1) + v17, v17);
        if ( v17 == (_DWORD)result )
          break;
        RtlBackoff(&v24);
        _m_prefetchw(&dword_180182FC8);
        v17 = dword_180182FC8;
      }
      if ( (v17 & 2) != 0 )
      {
        if ( v18 == (HANDLE)-1LL )
        {
          _InterlockedOr(v19, 0);
          return RtlpWakeByAddress((unsigned __int64)&dword_180182FC8, 0);
        }
        else
        {
          result = ZwSetEvent();
          if ( (int)result < 0 )
            RtlRaiseStatus((unsigned int)result);
        }
      }
    }
  }
  return result;
}
