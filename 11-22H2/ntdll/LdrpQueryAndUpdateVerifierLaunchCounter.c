/*
 * XREFs of LdrpQueryAndUpdateVerifierLaunchCounter @ 0x1800DF020
 * Callers:
...
__int64 __fastcall LdrpQueryAndUpdateVerifierLaunchCounter(unsigned __int16 *a1, _DWORD *a2)
{
  ...
  if ( (int)RtlpOpenImageFileOptionsKeyEx(&DestinationString.Length, 0xBu, 0, &KeyHandle) < 0 )
  {
    if ( (int)RtlpOpenImageFileOptionsKeyEx(&DestinationString.Length, 9u, 0, &KeyHandle) < 0 )
    {
      *a2 = 1;
      return 0LL;
    }
