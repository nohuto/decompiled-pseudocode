...
__int64 LdrpCodeAuthzInitialize()
{
  ...
  if ( NtOpenKey(&KeyHandle, 3u, (POBJECT_ATTRIBUTES)&::ObjectAttributes) < 0
    || (v4 = NtQueryValueKey(
               KeyHandle,
               (PUNICODE_STRING)&stru_1801326D8,
  ...
  {
    v9 = 0LL;
    if ( NtOpenKey(&v9, 0x20019u, &stru_18017E2C0) >= 0 )
    {
    ...
    if ( NtOpenKey(&Handle, 1u, (POBJECT_ATTRIBUTES)&stru_180131160) >= 0 )
    {
      if ( NtQueryValueKey(
             Handle,
    ...
    if ( v1 )
    {
      ...
      else if ( LdrLoadDll(0LL, 0LL, (PUNICODE_STRING)&stru_1801326C8, &DllHandle) >= 0 )
      {
        v6 = DllHandle;
        if ( LdrGetProcedureAddressForCaller(DllHandle, (PANSI_STRING)&stru_1801326E8, 0, &ProcedureAddress, 0, retaddr) >= 0
          && ProcedureAddress )
        {
          LdrpSaferIsDllAllowedRoutine = __ROR8__(
