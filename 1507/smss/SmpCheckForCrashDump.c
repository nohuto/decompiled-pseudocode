 ...
 *     SmpSetPagefileTooSmallForDump @ 0x14001157C (SmpSetPagefileTooSmallForDump.c)
 */

bool __fastcall SmpCheckForCrashDump(struct _UNICODE_STRING *a1)
{
  int v1; // edi
  bool v2; // bl
  ...
  else
  {
      ...
      && Buffer[1] == 875976004 )
    {
      ...
      if ( (int)SmpGetCrashParameters(&UnicodeString) >= 0
        && (int)SmpGetDumpDestination((int)Buffer, (int)&UnicodeString, (int)FileHandle, v1, &DestinationString) >= 0 )
      {
        v2 = (int)SmpSavePageFile(FileHandle) >= 0;
      }
    }
    NtClose(FileHandle);
    FileHandle = (void *)-1LL;
