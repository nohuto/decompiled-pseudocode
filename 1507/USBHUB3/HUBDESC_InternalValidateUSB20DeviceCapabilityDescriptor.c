/*
 * XREFs of HUBDESC_InternalValidateUSB20DeviceCapabilityDescriptor @ 0x1C002C370
 * Callers:
 *     HUBDESC_InternalValidateDeviceCapabilityDescriptor @ 0x1C002C71C (HUBDESC_InternalValidateDeviceCapabilityDescriptor.c)
 * Callees:
 *     WPP_RECORDER_SF_d @ 0x1C00015F4 (WPP_RECORDER_SF_d.c)
 *     WPP_RECORDER_SF_ @ 0x1C0001974 (WPP_RECORDER_SF_.c)
 *     WPP_RECORDER_SF_dD @ 0x1C0001A3C (WPP_RECORDER_SF_dD.c)
 *     WPP_RECORDER_SF_DDD @ 0x1C001EAFC (WPP_RECORDER_SF_DDD.c)
 *     WPP_RECORDER_SF_DDDD @ 0x1C0027848 (WPP_RECORDER_SF_DDDD.c)
 *     HUBDESC_ShouldEnforceWin8ValidationImmutable @ 0x1C0027CD0 (HUBDESC_ShouldEnforceWin8ValidationImmutable.c)
 *     _guard_dispatch_icall_nop @ 0x1C00342F0 (_guard_dispatch_icall_nop.c)
 */

char __fastcall HUBDESC_InternalValidateUSB20DeviceCapabilityDescriptor(
        unsigned __int8 *a1,
        __int64 a2,
        unsigned int *a3,
        int *a4,
        int *a5,
        __int64 a6)
{
  int *v6; // rsi
  int v10; // r14d
  __int64 v11; // r15
  unsigned int v12; // r13d
  __int64 v13; // rdx
  __int64 v14; // r8
  __int64 v15; // rdx
  char v16; // bl
  int v17; // eax
  int *v18; // r14
  int v19; // ecx
  int v20; // eax
  int v21; // ecx
  int v22; // edx
  unsigned int v23; // eax
  int v25; // [rsp+20h] [rbp-68h]
  __int64 v26; // [rsp+28h] [rbp-60h]
  __int64 v27; // [rsp+30h] [rbp-58h]
  int v28; // [rsp+90h] [rbp+8h] BYREF
  unsigned int *v29; // [rsp+A0h] [rbp+18h]

  v29 = a3;
  v6 = a4;
  if ( a4 )
  {
    if ( *(_BYTE *)(a2 + 48) )
      *a4 = 0;
  }
  else
  {
    v28 = 0;
    v6 = &v28;
  }
  v10 = (_DWORD)a1 - *(_DWORD *)(a2 + 56);
  v11 = a6;
  v12 = *(_DWORD *)(a2 + 72) - v10;
  if ( *a1 < 7u )
  {
    (*(void (__fastcall **)(_QWORD, __int64))(a2 + 24))(*(_QWORD *)(a2 + 40), 146LL);
    if ( v12 < 7 )
    {
      v16 = 1;
      *v6 = 1;
      WPP_RECORDER_SF_DDD(v11, 2u, v14, 0xC8u, (__int64)&WPP_2371d6fec51640206d5aea3557771e4a_Traceguids, v10, *a1, 7);
      goto LABEL_46;
    }
    *v6 = 2;
    *a3 = 7;
    WPP_RECORDER_SF_DDDD(v11, v13, v14, 0xC7u, v25);
  }
  if ( *a1 > 7u )
  {
    if ( *(_BYTE *)(a2 + 12) || (unsigned __int16)(*(_WORD *)a2 - 513) <= 0xEu )
      *v6 = 2;
    WPP_RECORDER_SF_DDD(
      v11,
      2u,
      (__int64)a3,
      0xC9u,
      (__int64)&WPP_2371d6fec51640206d5aea3557771e4a_Traceguids,
      v10,
      *a1,
      7);
    (*(void (__fastcall **)(_QWORD, __int64))(a2 + 24))(*(_QWORD *)(a2 + 40), 145LL);
  }
  if ( *v29 > v12 )
  {
    if ( HUBDESC_ShouldEnforceWin8ValidationImmutable(a2) )
      *v6 = 2;
    LODWORD(v26) = v10;
    WPP_RECORDER_SF_d(v11, 2u, 5u, 0xCAu, (__int64)&WPP_2371d6fec51640206d5aea3557771e4a_Traceguids, v26);
    v15 = 148LL;
LABEL_44:
    (*(void (__fastcall **)(_QWORD, __int64))(a2 + 24))(*(_QWORD *)(a2 + 40), v15);
    goto LABEL_45;
  }
  if ( (*(_DWORD *)(a1 + 3) & 0xFFFF00E1) != 0 )
  {
    if ( *(_BYTE *)(a2 + 15) )
      *v6 = 2;
    LODWORD(v26) = *(_DWORD *)(a1 + 3);
    WPP_RECORDER_SF_d(v11, 2u, 5u, 0xCBu, (__int64)&WPP_2371d6fec51640206d5aea3557771e4a_Traceguids, v26);
    (*(void (__fastcall **)(_QWORD, __int64))(a2 + 24))(*(_QWORD *)(a2 + 40), 147LL);
  }
  v17 = *(_DWORD *)(a1 + 3);
  if ( (v17 & 2) != 0 )
  {
    v18 = a5;
    *a5 |= 1u;
    v19 = *v18 ^ (*v18 ^ (*(_DWORD *)(a1 + 3) >> 1)) & 2;
    *v18 = v19;
    v20 = *(_DWORD *)(a1 + 3);
    if ( (v20 & 8) != 0 )
    {
      if ( (v20 & 0xF00) != 0 )
      {
        v21 = v19 | 4;
        *v18 = v21;
        *v18 = v21 ^ ((unsigned __int8)v21 ^ (unsigned __int8)(*(_DWORD *)(a1 + 3) >> 4)) & 0xF0;
      }
      else
      {
        if ( HUBDESC_ShouldEnforceWin8ValidationImmutable(a2) )
          *v6 = 2;
        WPP_RECORDER_SF_(v11, 2u, 5u, 0xCCu, (__int64)&WPP_2371d6fec51640206d5aea3557771e4a_Traceguids);
        (*(void (__fastcall **)(_QWORD, __int64))(a2 + 24))(*(_QWORD *)(a2 + 40), 149LL);
      }
    }
    v22 = *(_DWORD *)(a1 + 3);
    if ( (v22 & 0x10) == 0 )
      goto LABEL_45;
    if ( (v22 & 8) != 0 && ((*(_DWORD *)(a1 + 3) >> 4) & 0xF00u) <= (*(_DWORD *)(a1 + 3) & 0xF00u) )
    {
      if ( HUBDESC_ShouldEnforceWin8ValidationImmutable(a2) )
        *v6 = 2;
      v23 = *(_DWORD *)(a1 + 3);
      LODWORD(v27) = (v23 >> 8) & 0xF;
      LODWORD(v26) = (unsigned __int16)v23 >> 12;
      WPP_RECORDER_SF_dD(v11, 2u, 5u, 0xCDu, (__int64)&WPP_2371d6fec51640206d5aea3557771e4a_Traceguids, v26, v27);
      v15 = 151LL;
      goto LABEL_44;
    }
    if ( (v22 & 0xF000) == 0 )
    {
      if ( HUBDESC_ShouldEnforceWin8ValidationImmutable(a2) )
        *v6 = 2;
      WPP_RECORDER_SF_(v11, 2u, 5u, 0xCEu, (__int64)&WPP_2371d6fec51640206d5aea3557771e4a_Traceguids);
      v15 = 152LL;
      goto LABEL_44;
    }
    *v18 |= 8u;
    *v18 ^= (*v18 ^ (*(_DWORD *)(a1 + 3) >> 4)) & 0xF00;
  }
  else if ( (v17 & 4) != 0 )
  {
    if ( HUBDESC_ShouldEnforceWin8ValidationImmutable(a2) )
      *v6 = 2;
    LODWORD(v26) = *(_DWORD *)(a1 + 3);
    WPP_RECORDER_SF_d(v11, 2u, 5u, 0xCFu, (__int64)&WPP_2371d6fec51640206d5aea3557771e4a_Traceguids, v26);
    v15 = 150LL;
    goto LABEL_44;
  }
LABEL_45:
  v16 = 1;
LABEL_46:
  if ( *v6 )
  {
    v16 = 0;
    WPP_RECORDER_SF_(v11, 2u, 5u, 0xD0u, (__int64)&WPP_2371d6fec51640206d5aea3557771e4a_Traceguids);
  }
  return v16;
}
