/*
 * XREFs of ?_UpdateResourcesForMonitorHelper@CLivePreview@@AEAAJPEBVCTopLevelWindow@@PEAULivePreviewResource@@@Z @ 0x18006B388
 * Callers:
 ...
 *     ?CreateRoundRectRgnCoerceInvalid@@YAPEAUHRGN__@@HHHHHH@Z @ 0x180068158 (-CreateRoundRectRgnCoerceInvalid@@YAPEAUHRGN__@@HHHHHH@Z.c)
        ...
        struct LivePreviewResource *a3)
{
  ...
  if ( v6 && *(char *)(*((_QWORD *)a2 + 93) + 554LL) >= 0 )
    UnionRect((LPRECT)a3, (const RECT *)a3, &rcDst);
  v7 = *((_QWORD *)a2 + 93);
  if ( (*(_BYTE *)(v7 + 554) & 0x40) == 0 && *(_DWORD *)(v7 + 112) == 1 )
  {
    if ( v6 )
    ...
    if ( IntersectRect(&rcSrc2, &rcDst, (const RECT *)a3 + 7) )
    {
      if ( UnionRect(&rc1, (const RECT *)a3 + 5, &rcSrc2) && !EqualRect(&rc1, (const RECT *)a3 + 5) )
      {
        ...
        if ( !RoundRectRgnCoerceInvalid )
        {
          ...
          MilInstrumentationCheckHR(0x14u, 0LL, 0LL, v5, 0x4D6u);
        ...
        if ( UnionRect(&rc1, &rcSrc2, &rcDst) && !EqualRect(&rc1, &rcDst) )
        {
          ...
          if ( !v23 )
          {
            ...
            MilInstrumentationCheckHR(0x14u, 0LL, 0LL, v5, 0x4DCu);
            return (unsigned int)v5;
          }
          SetLastError(0);
