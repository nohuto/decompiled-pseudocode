/*
 * XREFs of ?PostActivateLivePreview@CDesktopManager@@QEAAJPEBUMILCMD_DWM_REDIRECTION_ACTIVATELIVEPREVIEW@@@Z @ 0x18001BC80
 * Callers:
 *     ?PostActivateLivePreview@CWindowList@@AEAAJPEBUMILCMD_DWM_REDIRECTION_ACTIVATELIVEPREVIEW@@@Z @ 0x180001EC0 (-PostActivateLivePreview@CWindowList@@AEAAJPEBUMILCMD_DWM_REDIRECTION_ACTIVATELIVEPREVIEW@@@Z.c)
 *     ?ShellWindowChange@CWindowList@@AEAAJPEBUMILCMD_DWM_REDIRECTION_NOTIFYSHELLWINDOWCHANGE@@@Z @ 0x180046478 (-ShellWindowChange@CWindowList@@AEAAJPEBUMILCMD_DWM_REDIRECTION_NOTIFYSHELLWINDOWCHANGE@@@Z.c)
 * Callees:
 *     ?MilInstrumentationCheckHR@@YAXKQEBJIJI@Z @ 0x180004308 (-MilInstrumentationCheckHR@@YAXKQEBJIJI@Z.c)
 *     ?WinSqmIsOptedIn_CachedInDwm@@YA_NXZ @ 0x180008598 (-WinSqmIsOptedIn_CachedInDwm@@YA_NXZ.c)
 *     ?OnSizeChanged@CIconicAnimatedVisual@@UEAAXPEBVCSecondaryWindowRepresentation@@@Z @ 0x1800488B0 (-OnSizeChanged@CIconicAnimatedVisual@@UEAAXPEBVCSecondaryWindowRepresentation@@@Z.c)
 */

__int64 __fastcall CDesktopManager::PostActivateLivePreview(
        CDesktopManager *this,
        const struct MILCMD_DWM_REDIRECTION_ACTIVATELIVEPREVIEW *a2)
{
  unsigned int v2; // esi
  WPARAM v5; // r14
  __int64 v6; // rax
  int v8; // ebx
  signed int LastError; // eax
  _DWORD v10[10]; // [rsp+30h] [rbp-28h] BYREF

  v2 = 0;
  v5 = 0LL;
  if ( *((_DWORD *)a2 + 1) )
  {
    v8 = *((_DWORD *)a2 + 6);
    if ( WinSqmIsOptedIn_CachedInDwm() )
    {
      v10[2] = v8;
      v10[0] = 1;
      WinSqmAddToStream(0LL, 4017LL, 1LL, v10);
    }
  }
  if ( *((_DWORD *)this + 378) )
  {
    v6 = (*(__int64 (__fastcall **)(WPF::ProcessHeapImpl *, __int64))(*(_QWORD *)WPF::g_pProcessHeap + 8LL))(
           WPF::g_pProcessHeap,
           56LL);
    v5 = v6;
    if ( v6 )
    {
      *(_DWORD *)v6 = *((_DWORD *)a2 + 1);
      *(_QWORD *)(v6 + 8) = *((_QWORD *)a2 + 1);
      *(_QWORD *)(v6 + 16) = *((_QWORD *)a2 + 2);
      *(_DWORD *)(v6 + 24) = *((_DWORD *)a2 + 6);
      *(_DWORD *)(v6 + 28) = *((_DWORD *)a2 + 7);
      *(_DWORD *)(v6 + 32) = 0;
      *(_OWORD *)(v6 + 36) = *((_OWORD *)a2 + 2);
      SetLastError(0);
      if ( PostThreadMessageW(*((_DWORD *)this + 378), 0x405u, v5, 0LL) )
      {
        v5 = 0LL;
      }
      else
      {
        LastError = GetLastError();
        if ( LastError > 0 )
          LastError = (unsigned __int16)LastError | 0x80070000;
        if ( LastError >= 0 )
          LastError = -2003304445;
        v2 = LastError;
        MilInstrumentationCheckHR(0x14u, 0LL, 0LL, LastError, 0x83Cu);
      }
    }
    else
    {
      v2 = -2147024882;
      MilInstrumentationCheckHR(0x14u, 0LL, 0LL, -2147024882, 0x82Du);
    }
  }
  else
  {
    v2 = -2147467259;
    MilInstrumentationCheckHR(0x14u, 0LL, 0LL, -2147467259, 0x824u);
  }
  (*(void (__fastcall **)(WPF::ProcessHeapImpl *, WPARAM))(*(_QWORD *)WPF::g_pProcessHeap + 32LL))(
    WPF::g_pProcessHeap,
    v5);
  return v2;
}
