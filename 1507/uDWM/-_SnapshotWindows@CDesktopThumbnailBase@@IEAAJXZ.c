/*
 * XREFs of ?_SnapshotWindows@CDesktopThumbnailBase@@IEAAJXZ @ 0x180099B60
 * Callers:
      ...
      while ( 1 )
      {
        if ( Flink )
        {
                 ...
                 LODWORD(Flink[7].Flink)) )
          {
            if ( Flink[23].Flink )
            {
              Blink = (unsigned __int8)Flink[34].Blink;
              if ( (Blink & 1) != 0
                && ((unsigned __int8)~(Blink >> 2) & (unsigned __int8)~(BYTE4(Flink[34].Blink) >> 4) & 1) != 0 )
              {
                if ( Flink[2].Blink )
                {
                         ...
                         Flink) )
                  {
                    ...
                    {
                      v9 = CDesktopThumbnailBase::_AddWindow(this, (struct CTopLevelWindow *)Flink[23].Flink, v7, v8);
                      v2 = v9;
                      if ( v9 < 0 )
                        break;
                    }
