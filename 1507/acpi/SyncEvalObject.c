/*
 * XREFs of SyncEvalObject @ 0x1C0001840
 * Callers:
 *     AMLIEvalNameSpaceObject @ 0x1C0001750 (AMLIEvalNameSpaceObject.c)
 *     AMLIEvalPackageElement @ 0x1C001AD78 (AMLIEvalPackageElement.c)
 * Callees:
 *     AsyncEvalObject @ 0x1C0002000 (AsyncEvalObject.c)
 *     GetObjectPath @ 0x1C0005854 (GetObjectPath.c)
 *     RestartContext @ 0x1C0006090 (RestartContext.c)
 *     LogSchedEvent @ 0x1C001F290 (LogSchedEvent.c)
 *     LogError @ 0x1C00442B8 (LogError.c)
 *     PrintDebugMessage @ 0x1C00459AC (PrintDebugMessage.c)
 *     NestAsyncEvalObject @ 0x1C0046F48 (NestAsyncEvalObject.c)
 */

__int64 __fastcall SyncEvalObject(__int64 a1, int a2, int a3, int a4)
{
  char v8; // bp
  int v9; // eax
  struct _KTHREAD *CurrentThread; // rcx
  __int64 *v11; // rax
  __int64 *v12; // rdi
  struct _KTHREAD *v13; // r9
  __int64 v14; // r10
  __int64 CurrentIrql; // r8
  unsigned __int32 v16; // ebx
  unsigned int v17; // ebx
  __int64 v18; // rdx
  unsigned int restarted; // eax
  unsigned int v20; // ebx
  __int64 result; // rax
  NTSTATUS v22; // ebx
  __int64 ObjectPath; // rax
  __int64 *v24; // rdx
  void *v25; // rdi
  PLARGE_INTEGER Timeout; // [rsp+20h] [rbp-88h]
  __int64 *v27; // [rsp+40h] [rbp-68h] BYREF
  __int64 v28; // [rsp+48h] [rbp-60h] BYREF
  int v29; // [rsp+50h] [rbp-58h]
  unsigned int v30; // [rsp+58h] [rbp-50h] BYREF
  PSLIST_ENTRY ListEntry; // [rsp+60h] [rbp-48h]
  struct _KEVENT Event; // [rsp+68h] [rbp-40h] BYREF
  int v33; // [rsp+B0h] [rbp+8h] BYREF

  v28 = a1 + 112;
  v29 = 8;
  v27 = &v28;
  v8 = 0;
  v9 = EmClientRuleEvaluate(&GUID_EM_RULE_AMLI_ALLOW_INVALID_RECLAIM_MEMORY_MAP_ON_EVAL, &v27, 1LL, &v33);
  if ( v9 >= 0 && v33 == 2 || v9 == -1073741818 && v33 )
  {
    v8 = 1;
    _InterlockedIncrement(&gAllowInvalidReclaimMemoryMap);
  }
  KeInitializeEvent(&Event, SynchronizationEvent, 0);
  byte_1C005A0C8 = KeAcquireSpinLockRaiseToDpc(&SpinLock);
  CurrentThread = KeGetCurrentThread();
  v11 = (__int64 *)RunningContextListHead;
  v12 = 0LL;
  if ( (__int64 *)RunningContextListHead != &RunningContextListHead )
  {
    while ( (struct _KTHREAD *)v11[2] != CurrentThread )
    {
      v11 = (__int64 *)*v11;
      if ( v11 == &RunningContextListHead )
        goto LABEL_5;
    }
    v12 = v11 - 49;
  }
LABEL_5:
  KeReleaseSpinLock(&SpinLock, byte_1C005A0C8);
  if ( !v12 )
  {
    v13 = KeGetCurrentThread();
    v14 = (unsigned int)gReadyQueue;
    CurrentIrql = KeGetCurrentIrql();
    if ( (gDebugger & 0x8000) != 0 && P )
    {
      v16 = _InterlockedExchangeAdd((_DWORD *)&qword_1C005A1E8 + 1, 1u);
      if ( (_DWORD)qword_1C005A1E8 == 204 )
        v17 = v16 % 0xCC;
      else
        v17 = v16 % (unsigned int)qword_1C005A1E8;
      v18 = 72LL * v17;
      *(_QWORD *)((char *)P + v18 + 8) = MEMORY[0xFFFFF78000000008];
      *(_DWORD *)((char *)P + v18) = 1398361667;
      *(_QWORD *)((char *)P + v18 + 16) = v13;
      *(_QWORD *)((char *)P + v18 + 24) = 0LL;
      *(_QWORD *)((char *)P + v18 + 32) = 0LL;
      *(_QWORD *)((char *)P + v18 + 40) = v14;
      *(_QWORD *)((char *)P + v18 + 48) = CurrentIrql;
      *(_QWORD *)((char *)P + v18 + 56) = a1;
      *(_QWORD *)((char *)P + v18 + 64) = 0LL;
    }
    restarted = AsyncEvalObject(a1, a2, a3, a4, (__int64)EvalMethodComplete, (__int64)&v30, 0);
    goto LABEL_12;
  }
  if ( (v12[8] & 0x100) != 0 )
  {
    LogError(3222536198LL);
    PrintDebugMessage(179, 0, 0, 0, 0LL);
    v20 = -1072431098;
  }
  else
  {
    LogSchedEvent(1314085198, (_DWORD)v12, KeGetCurrentIrql(), a1, 0LL);
    v20 = NestAsyncEvalObject(a1, a2, a3, a4, (__int64)Timeout, (__int64)&v30);
    if ( v20 == 32772 )
    {
      restarted = RestartContext((PSLIST_ENTRY)v12);
LABEL_12:
      v20 = restarted;
    }
  }
  if ( KeGetCurrentIrql() >= 2u )
  {
    if ( v20 == 32772 )
    {
      LogError(3222536198LL);
      ObjectPath = GetObjectPath(a1);
      v24 = qword_1C0023E70;
      if ( ObjectPath )
        LODWORD(v24) = ObjectPath;
      v25 = (void *)ObjectPath;
      PrintDebugMessage(181, (_DWORD)v24, 0, 0, 0LL);
      v20 = -1072431098;
      if ( v25 )
        ExFreePoolWithTag(v25, 0);
    }
  }
  else if ( v20 == 32772 )
  {
    while ( 1 )
    {
      v22 = KeWaitForSingleObject(&Event, Executive, 0, 0, 0LL);
      if ( v22 )
        break;
      v20 = v30;
      if ( v30 == 32771 )
        v20 = RestartContext(ListEntry);
      if ( v20 != 32772 )
        goto LABEL_15;
    }
    LogError(3222536198LL);
    PrintDebugMessage(180, v22, 0, 0, 0LL);
    v20 = -1072431098;
  }
LABEL_15:
  result = v20;
  if ( v8 )
    _InterlockedDecrement(&gAllowInvalidReclaimMemoryMap);
  return result;
}
