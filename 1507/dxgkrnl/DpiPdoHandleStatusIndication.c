/*
 * XREFs of DpiPdoHandleStatusIndication @ 0x1C00288B0
 * Callers:
 *     <none>
 * Callees:
...
void __fastcall DpiPdoHandleStatusIndication(_QWORD *IoObject, PVOID Context, PIO_WORKITEM IoWorkItem)
{
  ...
  while ( 1 )
  {
    KeAcquireInStackQueuedSpinLock((PKSPIN_LOCK)(v3 + 2136), &LockHandle);
    if ( *v6 == (__int64 *)v6 )
    {
      v7 = 0LL;
      *(_BYTE *)(v3 + 2160) = 0;
    ...
    if ( *(_DWORD *)(v3 + 236) == 2 && *(_DWORD *)(v3 + 2584) != 1 && *(_DWORD *)(v3 + 248) == 1 )
    {
      ...
      if ( v14 )
      {
        ...
        LOBYTE(v25) = 0;
        LOBYTE(v24) = 0;
        if ( !(unsigned int)DpiPdoHandleChildConnectionChange((_DWORD)IoObject, v24, v25, 1) )
        {
          _m_prefetchw((const void *)(v3 + 2252));
          if ( (_InterlockedOr((volatile signed __int32 *)(v3 + 2252), 1u) & 1) == 0 )
            IoInvalidateDeviceRelations(*(PDEVICE_OBJECT *)(v3 + 152), BusRelations);
        }
      }
      else if ( *(_BYTE *)(v3 + 1058) )
      {
        LOBYTE(v13) = 1;
        if ( (int)DpiAcquireCoreSyncAccessSafe(IoObject, v13) >= 0 )
        {
          DxgkInvalidateVidPnDisplayModeListCache(*(_QWORD *)(v3 + 2504));
          MonitorRebuildMonitorModeListCache(*(struct DXGADAPTER **)(v3 + 2504));
          LOBYTE(v22) = 1;
...
LABEL_31:
  ...
  if ( *(_BYTE *)(v3 + 451) )
    _InterlockedDecrement((volatile signed __int32 *)(*(_QWORD *)(*(_QWORD *)(v3 + 24) + 64LL) + 2680LL));
