/*
 * XREFs of BmlCompareSourceModes @ 0x1C0099310
 * Callers:
 *     BmlGetNextBestSourceMode @ 0x1C0099080 (BmlGetNextBestSourceMode.c)
 * Callees:
 *     BmlGetPixelFormatPreference @ 0x1C0099434 (BmlGetPixelFormatPreference.c)
 *     BmlCompareSourceModesWithConstraint @ 0x1C009946C (BmlCompareSourceModesWithConstraint.c)
 *     BmlCompareSourceModesWithMonitors @ 0x1C00995C4 (BmlCompareSourceModesWithMonitors.c)
 *     BmlCompareModeRegions @ 0x1C0099774 (BmlCompareModeRegions.c)
 *     BmlCompareSourceModesWithContentRes @ 0x1C017E380 (BmlCompareSourceModesWithContentRes.c)
 */

__int64 __fastcall BmlCompareSourceModes(__int64 a1, __int64 a2, __int64 a3, __int64 a4, __int64 a5)
{
  unsigned __int16 v6; // r15
  __int64 v9; // rbx
  __int64 result; // rax
  unsigned int v11; // edi
  unsigned int v12; // ebx
  int PixelFormatPreference; // r12d
  int v14; // eax
  int v15; // r8d
  unsigned int v16; // eax
  __int64 v17; // rax
  __int64 v18; // rax
  __int64 v19; // rax

  v6 = a3;
  if ( !a1 )
  {
    v17 = WdLogNewEntry5_WdAssertion(0LL, a2, a3, a4);
    WdLogEvent5_WdAssertion(v17);
  }
  if ( !a2 )
  {
    v18 = WdLogNewEntry5_WdAssertion(a1, a2, a3, a4);
    WdLogEvent5_WdAssertion(v18);
  }
  if ( a4 == a5 )
  {
    v19 = WdLogNewEntry5_WdAssertion(a1, a2, a3, a4);
    WdLogEvent5_WdAssertion(v19);
  }
  if ( !a4 )
    return 0xFFFFFFFFLL;
  if ( !a5 )
    return 1LL;
  v9 = 104LL * v6;
  if ( *(_BYTE *)(*(_QWORD *)(v9 + a1 + 16) + 117LL) && (*(_DWORD *)(a1 + 8) & 2) != 0
    || (result = BmlCompareSourceModesWithConstraint(a1, v6, a4, a5), !(_DWORD)result) )
  {
    v11 = 1;
    if ( !*(_BYTE *)(*(_QWORD *)(v9 + a1 + 16) + 117LL)
      || (*(_DWORD *)(v9 + a1 + 116) & 1) == 0
      || (result = BmlCompareSourceModesWithContentRes(a1, v6, a4, a5), !(_DWORD)result) )
    {
      v12 = *(_DWORD *)(a4 + 96);
      PixelFormatPreference = BmlGetPixelFormatPreference(*(unsigned int *)(a5 + 96));
      v14 = BmlGetPixelFormatPreference(v12);
      if ( v14 <= PixelFormatPreference )
      {
        if ( v14 >= PixelFormatPreference )
        {
          v11 = BmlCompareModeRegions(a4 + 76, a5 + 76);
          if ( v11 )
          {
            v16 = BmlCompareSourceModesWithMonitors(a1, v6, v15, a4, a5);
            if ( v16 )
              return v16;
          }
        }
        else
        {
          return (unsigned int)-1;
        }
      }
      return v11;
    }
  }
  return result;
}
