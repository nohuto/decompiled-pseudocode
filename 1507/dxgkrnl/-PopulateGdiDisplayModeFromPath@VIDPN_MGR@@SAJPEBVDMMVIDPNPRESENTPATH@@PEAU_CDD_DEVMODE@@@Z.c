/*
 * XREFs of ?PopulateGdiDisplayModeFromPath@VIDPN_MGR@@SAJPEBVDMMVIDPNPRESENTPATH@@PEAU_CDD_DEVMODE@@@Z @ 0x1C009BD9C
 * Callers:
 *     FillDevmodeFromVidPn @ 0x1C009BF3C (FillDevmodeFromVidPn.c)
 * Callees:
 *     ?GetGraphicsInfo@DMMVIDPNSOURCEMODE@@QEBAPEBU_D3DKMDT_GRAPHICS_RENDERING_FORMAT@@XZ @ 0x1C0002F84 (-GetGraphicsInfo@DMMVIDPNSOURCEMODE@@QEBAPEBU_D3DKMDT_GRAPHICS_RENDERING_FORMAT@@XZ.c)
 *     ?GetBitsPerPixel@DMMVIDPNSOURCEMODE@@QEBAIXZ @ 0x1C0002FB4 (-GetBitsPerPixel@DMMVIDPNSOURCEMODE@@QEBAIXZ.c)
 *     ?reset@?$auto_rc@VDMMVIDPNSOURCEMODESET@@@@QEAAXPEAVDMMVIDPNSOURCEMODESET@@@Z @ 0x1C0007780 (-reset@-$auto_rc@VDMMVIDPNSOURCEMODESET@@@@QEAAXPEAVDMMVIDPNSOURCEMODESET@@@Z.c)
 *     ?DivideAndRound@DMMVIDEOSIGNALMODE@@SA_K_K0@Z @ 0x1C0008450 (-DivideAndRound@DMMVIDEOSIGNALMODE@@SA_K_K0@Z.c)
 *     ?reset@?$auto_rc@VDMMVIDPNTARGETMODESET@@@@QEAAXPEAVDMMVIDPNTARGETMODESET@@@Z @ 0x1C0009774 (-reset@-$auto_rc@VDMMVIDPNTARGETMODESET@@@@QEAAXPEAVDMMVIDPNTARGETMODESET@@@Z.c)
 *     ?DmmMapVSyncFromRationalToInteger@@YAIAEBU_D3DDDI_RATIONAL@@W4_D3DDDI_VIDEO_SIGNAL_SCANLINE_ORDERING@@PEAE@Z @ 0x1C009A7B4 (-DmmMapVSyncFromRationalToInteger@@YAIAEBU_D3DDDI_RATIONAL@@W4_D3DDDI_VIDEO_SIGNAL_SCANLINE_ORDE.c)
 *     ?PopulatePublicGdiDisplayModeInfo@VIDPN_MGR@@SAJ_K000W4_D3DDDI_VIDEO_SIGNAL_SCANLINE_ORDERING@@W4_D3DKMDT_VIDPN_PRESENT_PATH_ROTATION@@W4_D3DKMDT_VIDPN_PRESENT_PATH_SCALING@@QEAU_devicemodeW@@@Z @ 0x1C009BC54 (-PopulatePublicGdiDisplayModeInfo@VIDPN_MGR@@SAJ_K000W4_D3DDDI_VIDEO_SIGNAL_SCANLINE_ORDERING@@W.c)
 *     ?DmmCalculatePresentationVSync@@YAXPEBU_D3DKMDT_VIDEO_SIGNAL_INFO@@PEAU_D3DDDI_RATIONAL@@@Z @ 0x1C009C3A8 (-DmmCalculatePresentationVSync@@YAXPEBU_D3DKMDT_VIDEO_SIGNAL_INFO@@PEAU_D3DDDI_RATIONAL@@@Z.c)
 */

__int64 __fastcall VIDPN_MGR::PopulateGdiDisplayModeFromPath(
        const struct DMMVIDPNPRESENTPATH *a1,
        struct _CDD_DEVMODE *a2,
        __int64 a3,
        __int64 a4)
{
  __int64 v6; // rbx
  __int64 v7; // rax
  __int64 v8; // rbx
  __int64 v9; // rdx
  __int64 v10; // rcx
  __int64 v11; // r8
  __int64 v12; // r9
  DMMVIDPNSOURCEMODE *v13; // r13
  __int64 v14; // rdx
  __int64 v15; // rcx
  __int64 v16; // r8
  __int64 v17; // r9
  __int64 v18; // rbx
  __int64 v19; // rax
  __int64 v20; // rbx
  __int64 v21; // rbx
  const struct _D3DKMDT_GRAPHICS_RENDERING_FORMAT *GraphicsInfo; // rsi
  enum _D3DKMDT_VIDPN_PRESENT_PATH_SCALING v23; // r14d
  D3DKMDT_VIDPN_PRESENT_PATH_ROTATION Rotation; // r15d
  enum _D3DDDI_VIDEO_SIGNAL_SCANLINE_ORDERING v25; // ebp
  const struct _D3DDDI_RATIONAL *v26; // r9
  unsigned int v27; // eax
  UINT cy; // edi
  unsigned int v29; // ebx
  unsigned int BitsPerPixel; // eax
  __int64 v31; // rdx
  __int64 v32; // rcx
  int v33; // edi
  __int64 v34; // r8
  __int64 v35; // r9
  __int64 v37; // rax
  __int64 v38; // rax
  __int64 v39; // rax
  __int64 v40; // rax
  __int64 v41; // rax
  __int64 v42; // rax
  __int64 v43; // rax
  _QWORD *v44; // rbx
  const struct _D3DKMDT_GRAPHICS_RENDERING_FORMAT *v45; // rax
  __int64 v46; // rcx
  __int64 v47; // rax
  __int64 v48; // rcx
  __int64 v49; // rax
  const struct _D3DKMDT_GRAPHICS_RENDERING_FORMAT *v50; // [rsp+80h] [rbp+8h] BYREF
  __int64 v51; // [rsp+88h] [rbp+10h] BYREF
  __int64 v52; // [rsp+90h] [rbp+18h]

  if ( !a1 )
  {
    v37 = WdLogNewEntry5_WdAssertion(0LL, a2, a3, a4);
    WdLogEvent5_WdAssertion(v37);
  }
  if ( !a2 )
  {
    v38 = WdLogNewEntry5_WdAssertion(a1, a2, a3, a4);
    WdLogEvent5_WdAssertion(v38);
  }
  v6 = *((_QWORD *)a1 + 11);
  if ( !v6 )
  {
    v39 = WdLogNewEntry5_WdAssertion(a1, a2, a3, a4);
    WdLogEvent5_WdAssertion(v39);
  }
  v50 = 0LL;
  v7 = *(_QWORD *)(v6 + 104);
  if ( v7 )
  {
    _InterlockedIncrement((volatile signed __int32 *)(v7 + 96));
    v8 = *(_QWORD *)(v6 + 104);
    if ( v8 )
      goto LABEL_9;
  }
  else
  {
    v8 = 0LL;
  }
  v40 = WdLogNewEntry5_WdAssertion(a1, a2, a3, a4);
  WdLogEvent5_WdAssertion(v40);
LABEL_9:
  auto_rc<DMMVIDPNSOURCEMODESET>::reset((__int64 *)&v50, v8);
  v13 = *(DMMVIDPNSOURCEMODE **)&v50[4].Stride;
  if ( !v13 )
  {
    v41 = WdLogNewEntry5_WdAssertion(v10, v9, v11, v12);
    WdLogEvent5_WdAssertion(v41);
  }
  auto_rc<DMMVIDPNSOURCEMODESET>::reset((__int64 *)&v50, 0LL);
  v18 = *((_QWORD *)a1 + 12);
  v19 = *(_QWORD *)(v18 + 104);
  if ( v19 )
  {
    _InterlockedIncrement((volatile signed __int32 *)(v19 + 96));
    v20 = *(_QWORD *)(v18 + 104);
    if ( v20 )
      goto LABEL_13;
  }
  else
  {
    v20 = 0LL;
  }
  v42 = WdLogNewEntry5_WdAssertion(v15, v14, v16, v17);
  WdLogEvent5_WdAssertion(v42);
LABEL_13:
  v51 = v20;
  v21 = *(_QWORD *)(v20 + 144);
  v52 = v21;
  if ( !v21 )
  {
    v43 = WdLogNewEntry5_WdAssertion(v15, v14, v16, v17);
    WdLogEvent5_WdAssertion(v43);
  }
  auto_rc<DMMVIDPNTARGETMODESET>::reset(&v51, 0LL);
  GraphicsInfo = DMMVIDPNSOURCEMODE::GetGraphicsInfo(v13);
  v50 = GraphicsInfo;
  *(_QWORD *)((char *)a2 + 220) = *(_QWORD *)(v21 + 92);
  v51 = v21 + 72;
  DmmCalculatePresentationVSync(
    (const struct _D3DKMDT_VIDEO_SIGNAL_INFO *)(v21 + 72),
    (struct _D3DDDI_RATIONAL *)((char *)a2 + 220));
  v23 = *((_DWORD *)a1 + 28);
  Rotation = *((_DWORD *)a1 + 29);
  v25 = (int)(*(_DWORD *)(v21 + 120) << 29) >> 29;
  v27 = DmmMapVSyncFromRationalToInteger(v26, (unsigned int)v25, 0LL, (__int64)v26);
  cy = GraphicsInfo->PrimSurfSize.cy;
  LODWORD(GraphicsInfo) = GraphicsInfo->PrimSurfSize.cx;
  v29 = v27;
  BitsPerPixel = DMMVIDPNSOURCEMODE::GetBitsPerPixel(v13);
  v33 = VIDPN_MGR::PopulatePublicGdiDisplayModeInfo(
          BitsPerPixel,
          (unsigned int)GraphicsInfo,
          cy,
          v29,
          v25,
          Rotation,
          v23,
          (struct _devicemodeW *const)a2);
  if ( v33 < 0 )
  {
    v44 = (_QWORD *)WdLogNewEntry5_WdWarning(v32, v31, v34, v35);
    v45 = v50;
    v44[3] = v50->PrimSurfSize.cx;
    v44[4] = v45->PrimSurfSize.cy;
    v46 = (unsigned int)DMMVIDPNSOURCEMODE::GetBitsPerPixel(v13);
    v47 = v51;
    v44[5] = v46;
    v48 = (unsigned int)DMMVIDEOSIGNALMODE::DivideAndRound(*(unsigned int *)(v47 + 20), *(unsigned int *)(v47 + 24));
    v49 = v52;
    v44[6] = v48;
    v44[7] = (int)(*(_DWORD *)(v49 + 120) << 29) >> 29;
    WdLogEvent5_WdWarning(v44);
    return (unsigned int)v33;
  }
  else
  {
    *((_DWORD *)a2 + 57) = *((_DWORD *)v13 + 24);
    return 0LL;
  }
}
