/*
 * XREFs of ?SetFloatProperty@CAffineTransform2DEffectMarshaler@DirectComposition@@UEAAJIMPEA_N@Z @ 0x1C00DD660
 * Callers:
 *     <none>
 * Callees:
 *     <none>
 */

__int64 __fastcall DirectComposition::CAffineTransform2DEffectMarshaler::SetFloatProperty(
        DirectComposition::CAffineTransform2DEffectMarshaler *this,
        int a2,
        float a3,
        bool *a4)
{
  unsigned int v4; // r10d
  bool v6; // cl
  __int64 v7; // rdx

  v4 = 0;
  if ( a2 == 3 )
  {
    if ( *((float *)this + 32) != a3 )
    {
      *((float *)this + 32) = a3;
      v6 = 1;
      goto LABEL_9;
    }
    goto LABEL_7;
  }
  if ( (unsigned int)(a2 - 4) <= 5 )
  {
    v6 = 1;
    v7 = (a2 & 1) + 2LL * ((unsigned int)(a2 - 4) >> 1);
    if ( *((float *)this + v7 + 26) != a3 )
    {
      *((float *)this + v7 + 26) = a3;
LABEL_9:
      *a4 = v6;
      return v4;
    }
LABEL_7:
    v6 = 0;
    goto LABEL_9;
  }
  return (unsigned int)-1073741811;
}
