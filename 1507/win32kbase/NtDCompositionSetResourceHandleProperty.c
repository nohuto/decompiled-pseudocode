/*
 * XREFs of NtDCompositionSetResourceHandleProperty @ 0x1C0019180
 * Callers:
 *     <none>
 * Callees:
 *     ?ReferenceHandleAndLock@CApplicationChannel@DirectComposition@@SAJIPEAPEAV12@@Z @ 0x1C002CF70 (-ReferenceHandleAndLock@CApplicationChannel@DirectComposition@@SAJIPEAPEAV12@@Z.c)
 *     ?SetResourceHandleProperty@CApplicationChannel@DirectComposition@@QEAAJIIPEAX@Z @ 0x1C00303F4 (-SetResourceHandleProperty@CApplicationChannel@DirectComposition@@QEAAJIIPEAX@Z.c)
 *     _guard_dispatch_icall_nop @ 0x1C0085AE0 (_guard_dispatch_icall_nop.c)
 */

__int64 __fastcall NtDCompositionSetResourceHandleProperty(unsigned int a1, unsigned int a2, unsigned int a3, void *a4)
{
  int v7; // edi
  void *v8; // r9
  DirectComposition::CApplicationChannel *v9; // rbx
  DirectComposition::CApplicationChannel *v11; // [rsp+20h] [rbp-18h] BYREF

  v11 = 0LL;
  v7 = DirectComposition::CApplicationChannel::ReferenceHandleAndLock(a1, &v11);
  if ( v7 >= 0 )
  {
    v8 = a4;
    v9 = v11;
    v7 = DirectComposition::CApplicationChannel::SetResourceHandleProperty(v11, a2, a3, v8);
    (**(void (__fastcall ***)(DirectComposition::CApplicationChannel *))v9)(v9);
  }
  return (unsigned int)v7;
}
