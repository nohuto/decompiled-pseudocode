/*
 * XREFs of ?bModifyWorldTransform@XDCOBJ@@QEAAHPEBU_XFORML@@K@Z @ 0x1C0019AD0
 * Callers:
 *     NtGdiModifyWorldTransform @ 0x1C00E4CC0 (NtGdiModifyWorldTransform.c)
 * Callees:
 *     ?bMultToWorld@EXFORMOBJ@@QEAAHPEAVMATRIX@@K@Z @ 0x1C0006E40 (-bMultToWorld@EXFORMOBJ@@QEAAHPEAVMATRIX@@K@Z.c)
 *     ?bWorldMatrixInRange@@YAHPEAVMATRIX@@@Z @ 0x1C0032744 (-bWorldMatrixInRange@@YAHPEAVMATRIX@@@Z.c)
 *     ?vQuickInit@EXFORMOBJ@@QEAAXAEAVXDCOBJ@@K@Z @ 0x1C0032CA0 (-vQuickInit@EXFORMOBJ@@QEAAXAEAVXDCOBJ@@K@Z.c)
 *     ?bWorldXformIdentity@@YAHPEBU_XFORML@@@Z @ 0x1C0101278 (-bWorldXformIdentity@@YAHPEBU_XFORML@@@Z.c)
 *     __security_check_cookie @ 0x1C015BB90 (__security_check_cookie.c)
 *     memcmp @ 0x1C015BBC0 (memcmp.c)
 */

__int64 __fastcall XDCOBJ::bModifyWorldTransform(XDCOBJ *this, const struct _XFORML *a2, unsigned int a3)
{
  unsigned int v4; // ebx
  __int64 v6; // rcx
  __int64 v7; // rdx
  __int64 v9; // rax
  __int64 v10; // rcx
  __int64 v11; // rcx
  __int64 v12; // rcx
  __int64 v13; // rax
  __int64 v14; // rcx
  struct MATRIX *v15[2]; // [rsp+20h] [rbp-40h] BYREF
  __int128 Buf1; // [rsp+30h] [rbp-30h] BYREF
  __int128 v17; // [rsp+40h] [rbp-20h]

  v4 = 0;
  if ( a3 == 1 )
    goto LABEL_6;
  if ( a3 > 1 )
  {
    if ( a3 <= 3 )
    {
      vConvertXformToMatrix(a2, &Buf1);
      if ( (*(_DWORD *)(*(_QWORD *)(*(_QWORD *)this + 80LL) + 352LL) & 2) != 0
        || (EXFORMOBJ::vQuickInit((EXFORMOBJ *)v15, this, 0x203u), v15[0])
        && EXFORMOBJ::bMultToWorld(v15, (struct MATRIX *)&Buf1, a3) )
      {
        if ( (unsigned int)bWorldMatrixInRange((struct MATRIX *)&Buf1) )
        {
          v13 = *(_QWORD *)this;
          *(_OWORD *)(v13 + 408) = Buf1;
          *(_OWORD *)(v13 + 424) = v17;
          v14 = *(_QWORD *)(*(_QWORD *)this + 80LL);
          *(_OWORD *)(v14 + 256) = Buf1;
          *(_OWORD *)(v14 + 272) = v17;
          if ( !memcmp(&Buf1, gmxIdentity_LToL, 0x20uLL) )
            goto LABEL_13;
          goto LABEL_11;
        }
      }
    }
    else if ( a3 == 4 )
    {
      if ( (unsigned int)bWorldXformIdentity(a2) )
      {
LABEL_6:
        v7 = *(_QWORD *)this;
        if ( (*(_DWORD *)(*(_QWORD *)(*(_QWORD *)this + 80LL) + 352LL) & 2) != 0 )
          return 1;
        *(_OWORD *)(v7 + 408) = *(_OWORD *)&gmxIdentity_LToL;
        *(_OWORD *)(v7 + 424) = *((_OWORD *)&gmxIdentity_LToL + 1);
        v12 = *(_QWORD *)(*(_QWORD *)this + 80LL);
        *(_OWORD *)(v12 + 256) = *(_OWORD *)&gmxIdentity_LToL;
        *(_OWORD *)(v12 + 272) = *((_OWORD *)&gmxIdentity_LToL + 1);
LABEL_13:
        *(_DWORD *)(*(_QWORD *)(*(_QWORD *)this + 80LL) + 352LL) |= 0x80B2u;
        return 1;
      }
      vConvertXformToMatrix(v6, &Buf1);
      if ( !(unsigned int)bWorldMatrixInRange((struct MATRIX *)&Buf1) )
        return v4;
      v9 = *(_QWORD *)this;
      *(_OWORD *)(v9 + 408) = Buf1;
      *(_OWORD *)(v9 + 424) = v17;
      v10 = *(_QWORD *)(*(_QWORD *)this + 80LL);
      *(_OWORD *)(v10 + 256) = Buf1;
      *(_OWORD *)(v10 + 272) = v17;
LABEL_11:
      v11 = *(_QWORD *)this;
      *(_DWORD *)(*(_QWORD *)(*(_QWORD *)this + 80LL) + 352LL) |= 0x80B0u;
      *(_DWORD *)(*(_QWORD *)(v11 + 80) + 352LL) &= ~2u;
      return 1;
    }
  }
  return v4;
}
