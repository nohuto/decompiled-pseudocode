/*
 * XREFs of ?SetCustomFlick@@YAHPEAUtagCUSTOM_FLICK@@@Z @ 0x1C0225624
 * Callers:
...
__int64 __fastcall SetCustomFlick(struct tagCUSTOM_FLICK *a1)
{
  ...
  *(_DWORD *)((char *)&Destination.Buffer + 2) = 0;
  ...
  v3 = OpenDeviceCfgKey(Destination.MaximumLength, Destination.Buffer, 0x20006u, &KeyHandle, 1);
  if ( v3 >= 0 )
  {
    RtlInitUnicodeString(&DestinationString, L"ModifierKeys");
    v3 = ZwSetValueKey(KeyHandle, &DestinationString, 0, 4u, (char *)a1 + 16, 4u);
    if ( v3 >= 0 )
    {
      RtlInitUnicodeString(&DestinationString, L"KeyCode");
      v3 = ZwSetValueKey(KeyHandle, &DestinationString, 0, 4u, (char *)a1 + 20, 4u);
      if ( v3 >= 0 )
      {
        RtlInitUnicodeString(&DestinationString, L"FriendlyName");
        v3 = ZwSetValueKey(KeyHandle, &DestinationString, 0, 1u, (char *)a1 + 28, *((_DWORD *)a1 + 6));
      }
    }
  }
  if ( KeyHandle )
