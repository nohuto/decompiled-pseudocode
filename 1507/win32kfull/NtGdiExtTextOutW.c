/*
 * XREFs of NtGdiExtTextOutW @ 0x1C0098B40
 * Callers:
 *     <none>
 * Callees:
 *     GreExtTextOutRect @ 0x1C0098EB8 (GreExtTextOutRect.c)
 *     GreExtTextOutWInternal @ 0x1C00EA458 (GreExtTextOutWInternal.c)
 *     __security_check_cookie @ 0x1C015BB90 (__security_check_cookie.c)
 *     memmove @ 0x1C015D180 (memmove.c)
 */

__int64 __fastcall NtGdiExtTextOutW(
        HDC a1,
        int a2,
        int a3,
        unsigned int a4,
        struct tagRECT *a5,
        __int64 a6,
        unsigned int a7,
        int *Src,
        unsigned int a9)
{
  struct tagRECT *v10; // rbx
  __int64 v11; // rax
  char *v12; // rdx
  unsigned int v13; // esi
  unsigned int v14; // r12d
  __int64 v15; // rcx
  __int64 v16; // r15
  char *v17; // rdi
  char *v18; // rcx
  unsigned __int64 v19; // rdx
  char *v20; // rsi
  char *v21; // rdi
  size_t v22; // r8
  __int64 v23; // rdx
  __int64 v24; // r8
  __int64 v25; // r9
  unsigned int v26; // r14d
  int *v28; // [rsp+68h] [rbp-170h]
  struct tagRECT v32; // [rsp+B8h] [rbp-120h] BYREF
  char v33; // [rsp+D0h] [rbp-108h] BYREF

  v10 = a5;
  v11 = a6;
  v12 = (char *)Src;
  v28 = Src;
  if ( a7 > 0xFFFF )
    return 0LL;
  if ( a5 )
  {
    if ( (a4 & 6) != 0 )
    {
      if ( (unsigned __int64)a5 >= W32UserProbeAddress )
        v10 = (struct tagRECT *)W32UserProbeAddress;
      v32 = *v10;
      v10 = &v32;
      v11 = a6;
    }
    else
    {
      v10 = 0LL;
    }
  }
  if ( !a7 )
  {
    if ( v10 && (a4 & 2) != 0 )
      return (unsigned int)GreExtTextOutRect(a1);
    else
      return 1;
  }
  if ( (a4 & 6) != 0 && !v10 || !v11 )
    return 0;
  v13 = 0;
  v14 = (30 * a7 + 7) & 0xFFFFFFF8;
  if ( Src )
  {
    v13 = 4 * a7;
    if ( (a4 & 0x2000) != 0 )
      v13 = 8 * a7;
  }
  v15 = v14 + 2 * a7 + ((v13 + 7) & 0xFFFFFFF8);
  if ( (unsigned int)v15 <= 0xC0 )
  {
    v16 = 0LL;
    v17 = &v33;
LABEL_16:
    if ( v12 )
    {
      if ( v13 )
      {
        v18 = &v12[v13];
        if ( (unsigned __int64)v18 > W32UserProbeAddress || v18 < v12 )
          *W32UserProbeAddress = 0;
      }
      memmove(v17, v12, v13);
      v19 = a6;
      v28 = (int *)v17;
      v17 += (v13 + 7) & 0xFFFFFFF8;
    }
    else
    {
      v19 = a6;
    }
    v20 = v17;
    v21 = &v17[v14];
    v22 = 2LL * (int)a7;
    if ( v22 )
    {
      if ( (v19 & 1) != 0 )
        ExRaiseDatatypeMisalignment();
      if ( v22 + v19 > W32UserProbeAddress || v22 + v19 < v19 )
        *W32UserProbeAddress = 0;
    }
    memmove(v21, (const void *)v19, v22);
    v26 = GreExtTextOutWInternal(a1, a2, a3, a4, v10, (unsigned __int16 *)v21, a7, v28, v20, a9);
    if ( v16 )
      FreeTmpBuffer(v16, v23, v24, v25);
    return v26;
  }
  v16 = AllocFreeTmpBuffer(v15);
  v17 = (char *)v16;
  if ( v16 )
  {
    v12 = (char *)Src;
    goto LABEL_16;
  }
  return 0LL;
}
