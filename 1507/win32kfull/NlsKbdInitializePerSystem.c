/*
 * XREFs of NlsKbdInitializePerSystem @ 0x1C0138854
 * Callers:
 *     xxxInitWindowStation @ 0x1C0138550 (xxxInitWindowStation.c)
 * Callees:
 *     ?NlsSetKeyStateToggle@@YAXE@Z @ 0x1C0138A00 (-NlsSetKeyStateToggle@@YAXE@Z.c)
 *     __security_check_cookie @ 0x1C015BB90 (__security_check_cookie.c)
 */

void NlsKbdInitializePerSystem()
{
  UNICODE_STRING v0; // [rsp+30h] [rbp-D0h] BYREF
  UNICODE_STRING String1; // [rsp+40h] [rbp-C0h] BYREF
  UNICODE_STRING v2; // [rsp+50h] [rbp-B0h] BYREF
  struct _UNICODE_STRING v3; // [rsp+60h] [rbp-A0h] BYREF
  UNICODE_STRING String2; // [rsp+70h] [rbp-90h] BYREF
  UNICODE_STRING v5; // [rsp+80h] [rbp-80h] BYREF
  ...
  _RTL_QUERY_REGISTRY_TABLE QueryTable; // [rsp+C0h] [rbp-40h] BYREF
  ...
  RtlInitUnicodeString(&v3, L"yes");
  RtlInitUnicodeString(&v5, L"no");
  RtlInitUnicodeString(&v6, L"home");
  RtlInitUnicodeString(&v7, L"clear");
  RtlInitUnicodeString(&String1, 0LL);
  RtlInitUnicodeString(&v0, 0LL);
  ...
  if ( RtlQueryRegistryValues(3u, L"WOW\\keyboard", &QueryTable, 0LL, 0LL) >= 0 )
  {
    ...
    if ( RtlEqualUnicodeString(&v0, &v5, 1u) )
      fNlsKbdConfiguration &= ~1u;
    if ( RtlEqualUnicodeString(&v2, &v7, 1u) )
      fNlsKbdConfiguration &= ~4u;
    ExFreePoolWithTag(String1.Buffer, 0);
    ExFreePoolWithTag(v0.Buffer, 0);
