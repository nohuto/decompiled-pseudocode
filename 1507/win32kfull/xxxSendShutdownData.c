/*
 * XREFs of xxxSendShutdownData @ 0x1C0215440
 * Callers:
 *     xxxProcessEventMessage @ 0x1C0071E4C (xxxProcessEventMessage.c)
 * Callees:
 *     ?RtlStringCchCopyW@@YAJPEAG_KPEBG@Z @ 0x1C005E4A8 (-RtlStringCchCopyW@@YAJPEAG_KPEBG@Z.c)
 *     _GetProp @ 0x1C006524C (_GetProp.c)
 *     SfnCOPYDATA @ 0x1C00F1580 (SfnCOPYDATA.c)
 *     _GetWindowIcon @ 0x1C01235D8 (_GetWindowIcon.c)
 *     __security_check_cookie @ 0x1C015BB90 (__security_check_cookie.c)
 */

__int64 __fastcall xxxSendShutdownData(__int64 a1, ULONG_PTR a2)
{
  ULONG_PTR v2; // rsi
  __int64 v4; // rbx
  unsigned int v5; // eax
  unsigned __int64 v6; // rdx
  char *Prop; // rax
  __int64 v8; // rax
  __int64 v10; // [rsp+48h] [rbp-C0h] BYREF
  __int64 v11; // [rsp+50h] [rbp-B8h]
  __int64 *v12; // [rsp+58h] [rbp-B0h]
  __int64 v13; // [rsp+68h] [rbp-A0h] BYREF
  __int64 WindowIcon; // [rsp+70h] [rbp-98h]
  unsigned __int16 v15[261]; // [rsp+78h] [rbp-90h] BYREF
  unsigned __int16 v16[267]; // [rsp+282h] [rbp+17Ah] BYREF

  v2 = a2;
  if ( !gspwndBSDR )
    return 0LL;
  if ( a1 == 0xFFFF )
  {
LABEL_19:
    v12 = &v13;
    v13 = a1;
    v10 = 0LL;
    LODWORD(v11) = 1064;
    SfnCOPYDATA(
      (__int64 *)gspwndBSDR,
      0x329u,
      v2,
      (__int64)&v10,
      *(_QWORD *)(gspwndBSDR + 144),
      *(_QWORD *)(gpsi + 744LL));
    return 0LL;
  }
  if ( !a2 )
  {
    WindowIcon = 0LL;
    v16[0] = 0;
    v15[0] = 0;
    v8 = ValidateHwndEx(a1, 0LL, 0LL);
    if ( v8 )
      *(_DWORD *)(v8 + 288) &= ~0x4000u;
    goto LABEL_19;
  }
  v4 = ValidateHwndEx(a1, 0LL, 0LL);
  if ( v4 )
  {
    if ( *(_QWORD *)v4 == ghwndBlocking )
    {
      v2 = 2LL;
    }
    else if ( *(_DWORD *)(v4 + 304) != 1 )
    {
      return 0LL;
    }
    *(_DWORD *)(v4 + 288) |= 0x4000u;
    WindowIcon = GetWindowIcon(v4, 1);
    v5 = *(_DWORD *)(v4 + 216);
    if ( v5 )
    {
      v6 = ((unsigned __int64)v5 >> 1) + 1;
      if ( v6 > 0x105 )
        v6 = 261LL;
      RtlStringCchCopyW((char *)v16, v6, *(char **)(v4 + 224));
    }
    else
    {
      v16[0] = 0;
    }
    Prop = (char *)GetProp(v4, (unsigned __int16)gatomShutdownBlockingReason, 1LL);
    if ( Prop )
      RtlStringCchCopyW((char *)v15, 261LL, Prop);
    else
      v15[0] = 0;
    goto LABEL_19;
  }
  return 0LL;
}
