/*
 * XREFs of UsbhDeleteOrphanPdo @ 0x1C005381C
 * Callers:
 *     UsbhDeletePdo @ 0x1C0053984 (UsbhDeletePdo.c)
 * Callees:
 *     UsbhDecPdoIoCount @ 0x1C00139E0 (UsbhDecPdoIoCount.c)
 *     PdoExt @ 0x1C0013AD0 (PdoExt.c)
 *     SET_PDO_HWPNPSTATE @ 0x1C001E760 (SET_PDO_HWPNPSTATE.c)
 *     UsbhRemoveAndDeletePdo @ 0x1C0054CBC (UsbhRemoveAndDeletePdo.c)
 */

_DWORD *__fastcall UsbhDeleteOrphanPdo(ULONG_PTR BugCheckParameter4, __int64 a2, __int64 a3, __int64 a4)
{
  _DWORD *v5; // rbx
  __int64 v6; // rdx
  __int64 v7; // r8
  __int64 v8; // r9
  _DWORD *result; // rax
  __int64 v10; // r9
  __int64 v11; // r8
  __int64 v12; // r9
  void *v13; // rcx
  void *v14; // rcx
  void *v15; // rcx
  void *v16; // rcx
  void *v17; // rcx
  void *v18; // rcx
  void *v19; // rcx

  v5 = PdoExt(BugCheckParameter4, a2, a3, a4);
  result = PdoExt(BugCheckParameter4, v6, v7, v8);
  if ( result[281] != 6 )
  {
    SET_PDO_HWPNPSTATE(BugCheckParameter4, 6LL, 6LL, v10);
    KeWaitForSingleObject(v5 + 322, Executive, 0, 0, 0LL);
    v13 = (void *)*((_QWORD *)v5 + 142);
    if ( v13 )
    {
      ExFreePoolWithTag(v13, 0);
      *((_QWORD *)v5 + 142) = 0LL;
    }
    v14 = (void *)*((_QWORD *)v5 + 279);
    if ( v14 )
    {
      ExFreePoolWithTag(v14, 0);
      *((_QWORD *)v5 + 279) = 0LL;
    }
    v15 = (void *)*((_QWORD *)v5 + 351);
    if ( v15 )
    {
      ExFreePoolWithTag(v15, 0);
      *((_QWORD *)v5 + 351) = 0LL;
    }
    v16 = (void *)*((_QWORD *)v5 + 298);
    if ( v16 )
    {
      ExFreePoolWithTag(v16, 0);
      *((_QWORD *)v5 + 298) = 0LL;
    }
    v17 = (void *)*((_QWORD *)v5 + 299);
    if ( v17 )
    {
      ExFreePoolWithTag(v17, 0);
      *((_QWORD *)v5 + 299) = 0LL;
    }
    v18 = (void *)*((_QWORD *)v5 + 312);
    if ( v18 )
    {
      ExFreePoolWithTag(v18, 0);
      *((_QWORD *)v5 + 312) = 0LL;
    }
    v19 = (void *)*((_QWORD *)v5 + 106);
    if ( v19 )
    {
      ExFreePoolWithTag(v19, 0);
      *((_QWORD *)v5 + 106) = 0LL;
    }
    if ( (v5[354] & 1) != 0 )
    {
      UsbhDecPdoIoCount(BugCheckParameter4, BugCheckParameter4, v11, v12);
      KeWaitForSingleObject(v5 + 308, Executive, 0, 0, 0LL);
    }
    v5[354] = 16;
    return (_DWORD *)UsbhRemoveAndDeletePdo(BugCheckParameter4);
  }
  return result;
}
