/*
 * XREFs of ?NdisPDRequestFetchNotification@@YAXPEAUPD_QUEUE_HANDLE__@@@Z @ 0x1C006C820
 * Callers:
 *     <none>
 * Callees:
 *     WPP_SF_q @ 0x1C0037744 (WPP_SF_q.c)
 */

void __fastcall NdisPDRequestFetchNotification(struct PD_QUEUE_HANDLE__ *a1)
{
  if ( (unsigned __int8)byte_1C0085324 >= 5u )
    WPP_SF_q(0x65u, &WPP_907f2c296662aafae9742c49c7fcbed7_Traceguids, (__int64)a1);
  _InterlockedExchange((volatile __int32 *)a1 + 14, 1);
}
