/*
 * XREFs of ?ndisFreePDRSSObjects@@YAXPEAU_LIST_ENTRY@@0@Z @ 0x1C00E07A8
 * Callers:
...
void __fastcall ndisFreePDRSSObjects(struct _LIST_ENTRY *a1, struct _LIST_ENTRY *a2)
{
  ...
  _LIST_ENTRY *v5; // rcx
  NDIS_PD_QUEUE_TRACKER *p_Blink; // rsi
  _LIST_ENTRY *v7; // rax
  _LIST_ENTRY *v8; // rcx
  ...
  while ( 1 )
  {
    ...
    if ( Flink->Blink != a1 || v5->Blink != Flink )
      __fastfail(3u);
    a1->Flink = v5;
    p_Blink = (NDIS_PD_QUEUE_TRACKER *)&Flink[-2].Blink;
    v5->Blink = a1;
    Flink->Flink = 0LL;
  ...
  while ( 1 )
  {
    ...
    if ( v7 != (_LIST_ENTRY *)24 )
      NDIS_PD_COUNTER::`scalar deleting destructor'((_UNICODE_STRING *)&v7[-2].Blink);
  }
}
