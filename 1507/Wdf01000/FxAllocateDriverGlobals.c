/*
 * XREFs of FxAllocateDriverGlobals @ 0x1C0027888
 * Callers:
 *     FxLibraryCommonRegisterClient @ 0x1C002753C (FxLibraryCommonRegisterClient.c)
 * Callees:
 *     AllocAndInitializeTelemetryContext @ 0x1C00070F4 (AllocAndInitializeTelemetryContext.c)
...
_WDF_DRIVER_GLOBALS *__fastcall FxAllocateDriverGlobals()
{
  ...
  if ( result )
  {
    ...
    *(_QWORD *)&v1->DriverFlags = &FxLibraryGlobals.FxDriverGlobalsList;
    if ( Flink->Blink != &FxLibraryGlobals.FxDriverGlobalsList )
      __fastfail(3u);
