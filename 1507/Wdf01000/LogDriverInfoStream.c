/*
 * XREFs of LogDriverInfoStream @ 0x1C0080164
 * Callers:
 ...
 *     GetDriverInfo @ 0x1C007FE64 (GetDriverInfo.c)
 *     GetFirstHardwareId @ 0x1C0080108 (GetFirstHardwareId.c)
...
void __fastcall LogDriverInfoStream(_FX_DRIVER_GLOBALS *DriverGlobals, FxDevice *Fdo)
{
  ...
  *(_QWORD *)&manufacturer.m_UnicodeString.Length = 0LL;
  ...
  if ( Fdo )
  {
    FxGetDevicePropertyString((unsigned __int64)Fdo, 5, &setupClass.m_UnicodeString);
    FxGetDevicePropertyString((unsigned __int64)Fdo, 15, &busEnum.m_UnicodeString);
    FxGetDevicePropertyString((unsigned __int64)Fdo, 1, &hardwareIDs.m_UnicodeString);
    GetFirstHardwareId(&hardwareIDs.m_UnicodeString);
    FxGetDevicePropertyString((unsigned __int64)Fdo, 8, &manufacturer.m_UnicodeString);
  ...
  if ( Tlgg_TelemetryProviderProv.LevelPlus1 > 5 && TlgKeywordOn(v7, 0x200000000000uLL) )
  {
    ...
    *(_QWORD *)&_TlgData[26].Size = 1LL;
    ...
    TlgWrite(v8, (char *)&TlgEvent_1._TlgChannel, v9, v10, 0x1Fu, _TlgData);
  }
