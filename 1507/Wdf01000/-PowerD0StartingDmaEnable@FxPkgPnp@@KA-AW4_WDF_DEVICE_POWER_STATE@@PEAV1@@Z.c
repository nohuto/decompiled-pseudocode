/*
 * XREFs of ?PowerD0StartingDmaEnable@FxPkgPnp@@KA?AW4_WDF_DEVICE_POWER_STATE@@PEAV1@@Z @ 0x1C001E750
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall FxPkgPnp::PowerD0StartingDmaEnable(FxPkgPnp *This)
{
  ...
  if ( m_DmaEnablerList )
  {
    ...
    while ( 1 )
    {
      ...
      if ( !NextEntryLocked )
        break;
      if ( (int)FxDmaEnabler::PowerUp((FxDmaEnabler *)NextEntryLocked->m_TransactionedObject, v17, v18) < 0 )
      {
        v3 = 0;
        break;
      }
  ...
  if ( m_EnumInfo )
  {
    ...
    m_EnumInfo->m_ChildListList.AcquireLock(&m_EnumInfo->m_ChildListList, m_Globals, (unsigned __int8 *)&v19);
