/*
 * XREFs of ?AddAsyncFlushResponse@CLegacySurfaceManager@@QEAAJPEAVCChannelContext@@II@Z @ 0x180062894
 * Callers:
        ...
        int a4)
{
  ...
  if ( v12 )
  {
    ...
    if ( v13 )
    {
      while ( 1 )
      {
        matched = CWindowNode::AddResponseToOutstandingDxSurfaces<ProcessTokenMatchPolicy>(v13[1], &v17, v12);
        v8 = matched;
        if ( matched < 0 )
          break;
        v13 = RtlEnumerateGenericTableWithoutSplaying((PRTL_GENERIC_TABLE)((char *)this + 136), &RestartKey);
