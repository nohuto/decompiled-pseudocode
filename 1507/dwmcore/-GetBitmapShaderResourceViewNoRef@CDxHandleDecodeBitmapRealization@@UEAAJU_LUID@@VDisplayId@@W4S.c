/*
 * XREFs of ?GetBitmapShaderResourceViewNoRef@CDxHandleDecodeBitmapRealization@@UEAAJU_LUID@@VDisplayId@@W4StereoContext@@PEAPEAUID3D11ShaderResourceView@@@Z @ 0x1801523D0
 * Callers:
 *     <none>
 * Callees:
    ...
    if ( v9 >= 0 )
    {
      ...
      if ( v10 >= 0 )
      {
        ...
        if ( BitmapShaderResourceViewFromBitmapSourceNoRef < 0 )
          MilInstrumentationCheckHR(0x14u, 0LL, 0, BitmapShaderResourceViewFromBitmapSourceNoRef, 0xAAu);
      }
      else
      {
        MilInstrumentationCheckHR(0x14u, 0LL, 0, v10, 0xA2u);
      }
    }
    else
    {
      MilInstrumentationCheckHR(0x14u, 0LL, 0, v9, 0x96u);
    }
    if ( v20 )
