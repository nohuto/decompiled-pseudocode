/*
 * XREFs of TppCancelTimer @ 0x18003CF70
 * Callers:
 *     TppWaitCompletion @ 0x18000B250 (TppWaitCompletion.c)
 *     TppCancelWait @ 0x18003C930 (TppCancelWait.c)
 *     TpSetTimerEx @ 0x18003CE70 (TpSetTimerEx.c)
 *     TpReleaseTimer @ 0x18003ED60 (TpReleaseTimer.c)
 *     TpWaitForTimer @ 0x18007D560 (TpWaitForTimer.c)
 *     TppTimerpStopCallbackGeneration @ 0x18007DF70 (TppTimerpStopCallbackGeneration.c)
 * Callees:
 *     TppETWTimerCancelled @ 0x180002350 (TppETWTimerCancelled.c)
 *     RtlAcquireSRWLockExclusive @ 0x18002A460 (RtlAcquireSRWLockExclusive.c)
 *     RtlReleaseSRWLockExclusive @ 0x180033470 (RtlReleaseSRWLockExclusive.c)
 *     TppPHDelete @ 0x18003D0E0 (TppPHDelete.c)
 *     TppUpdateSubQueueTimer @ 0x18003D3C0 (TppUpdateSubQueueTimer.c)
 *     TppItePush @ 0x18007DED4 (TppItePush.c)
 *     NtWaitForAlertByThreadId @ 0x180095430 (NtWaitForAlertByThreadId.c)
 */

char __fastcall TppCancelTimer(__int64 a1, volatile signed __int64 *a2, __int64 a3, __int64 a4)
{
  char v4; // bl
  char v5; // r12
  bool v7; // r14
  __int64 v10; // r15
  __int64 v11; // rdx
  char *v12; // rdx
  __int64 v13; // r8
  __int64 v14; // r9
  _BYTE v15[40]; // [rsp+20h] [rbp-28h] BYREF

  v4 = 1;
  v5 = a3;
  v7 = (*(_BYTE *)(a1 + 346) & 2) != 0;
  if ( (*(_BYTE *)(a1 + 346) & 1) != 0 )
  {
    v10 = (__int64)(a2 + 1);
    if ( (*(_BYTE *)(a1 + 346) & 2) == 0 )
      v10 = (__int64)(a2 + 16);
    if ( MEMORY[0x7FFE0386] )
      TppETWTimerCancelled(v10, a1);
    RtlAcquireSRWLockExclusive(a2, (char *)a2, a3, a4);
    if ( *(_BYTE *)(a1 + 344) )
    {
      TppPHDelete(v10 + 16, a1 + 240);
      TppPHDelete(v10 + 8, a1 + 280);
      LOBYTE(v11) = v7;
      TppUpdateSubQueueTimer(v10, v11);
      *(_BYTE *)(a1 + 344) = 0;
      RtlReleaseSRWLockExclusive(a2);
      *(_DWORD *)(a1 + 340) = 0;
      *(_QWORD *)(a1 + 320) = 0LL;
      *(_BYTE *)(a1 + 346) = 0;
      if ( !v5 )
        RtlReleaseSRWLockExclusive((volatile signed __int64 *)(a1 + 232));
    }
    else
    {
      RtlReleaseSRWLockExclusive(a2);
      *(_BYTE *)(a1 + 346) |= 4u;
      TppItePush(a1 + 328, v15);
      RtlReleaseSRWLockExclusive((volatile signed __int64 *)(a1 + 232));
      NtWaitForAlertByThreadId(a1 + 328, 0LL);
      if ( v5 )
        RtlAcquireSRWLockExclusive((volatile signed __int64 *)(a1 + 232), v12, v13, v14);
      return 0;
    }
    return v4;
  }
  else
  {
    *(_QWORD *)(a1 + 320) = 0LL;
    if ( !(_BYTE)a3 )
      RtlReleaseSRWLockExclusive((volatile signed __int64 *)(a1 + 232));
    return 0;
  }
}
