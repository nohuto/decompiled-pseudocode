/*
 * XREFs of RtlpCompareKnownAces @ 0x1800CEEF0
 * Callers:
 *     RtlpCompareAces @ 0x1800CEEA8 (RtlpCompareAces.c)
 * Callees:
 *     RtlEqualSid @ 0x18004A6C0 (RtlEqualSid.c)
 *     RtlEqualPrefixSid @ 0x18004D770 (RtlEqualPrefixSid.c)
 *     RtlInitializeSid @ 0x18004EAE0 (RtlInitializeSid.c)
 *     __security_check_cookie @ 0x180084090 (__security_check_cookie.c)
 *     memset @ 0x180098540 (memset.c)
 */

bool __fastcall RtlpCompareKnownAces(unsigned __int8 *a1, unsigned __int8 *a2, _WORD *a3, _WORD *a4)
{
  __int64 v6; // rcx
  int v10; // ecx
  _WORD *v11; // rdx
  int v12; // [rsp+20h] [rbp-40h] BYREF
  __int16 v13; // [rsp+24h] [rbp-3Ch]
  _DWORD v14[12]; // [rsp+28h] [rbp-38h] BYREF

  v6 = *a2;
  if ( RtlBaseAceType[v6] != RtlBaseAceType[*a1] || RtlIsSystemAceType[v6] && ((a1[1] ^ a2[1]) & 0xC0) != 0 )
    return 0;
  if ( !RtlEqualSid(a2 + 8, (_WORD *)a1 + 4) )
  {
    if ( (a2[1] & 3 | ~a2[1] & 8) != 8 || !a3 && !a4 )
      return 0;
    memset(v14, 0, sizeof(v14));
    v12 = 0;
    v13 = 768;
    if ( (int)RtlInitializeSid((__int64)v14, (__int64)&v12, 1u) < 0 )
      return 0;
    v14[2] = 0;
    if ( !RtlEqualPrefixSid(a1 + 8, v14) )
      return 0;
    v10 = *((_DWORD *)a1 + 4);
    if ( v10 )
    {
      if ( v10 != 1 || !a4 )
        return 0;
      v11 = a4;
    }
    else
    {
      if ( !a3 )
        return 0;
      v11 = a3;
    }
    return RtlEqualSid(a2 + 8, v11);
  }
  return 1;
}
