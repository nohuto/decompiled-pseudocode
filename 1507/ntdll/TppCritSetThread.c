/*
 * XREFs of TppCritSetThread @ 0x180070F24
 * Callers:
...
int __fastcall TppCritSetThread(HANDLE *a1)
{
  ...
  HANDLE TokenHandle; // [rsp+48h] [rbp-80h] BYREF
  _QWORD v8[2]; // [rsp+50h] [rbp-78h] BYREF
  ...
  *a1 = 0LL;
  ...
  if ( (v2->ProcessEnvironmentBlock->NtGlobalFlag & 0x100000) != 0 )
  {
    ...
    if ( (int)v2 >= 0 )
    {
      ...
      if ( NtDuplicateToken(TokenHandle, 0x24u, &ObjectAttributes, 0, TokenImpersonation, &Handle) >= 0 )
      {
      ...
      LODWORD(v2) = NtClose(TokenHandle);
    }
  }
  return (int)v2;
}
