/*
 * XREFs of SHA256Final @ 0x180050E6C
 * Callers:
 *     RtlDeriveCapabilitySidsFromName @ 0x180050F90 (RtlDeriveCapabilitySidsFromName.c)
 * Callees:
 *     DWORDToBigEndian @ 0x18004F470 (DWORDToBigEndian.c)
 *     SHA256Update @ 0x180051240 (SHA256Update.c)
 *     __security_check_cookie @ 0x180084090 (__security_check_cookie.c)
 *     memset @ 0x180098540 (memset.c)
 */

__int64 __fastcall SHA256Final(_DWORD *a1, _DWORD *a2)
{
  unsigned int v2; // esi
  __int64 v5; // rdi
  int v6; // edx
  __int64 result; // rax
  _DWORD v8[4]; // [rsp+20h] [rbp-88h] BYREF
  _BYTE v9[80]; // [rsp+30h] [rbp-78h] BYREF

  v2 = a1[9];
  v5 = 64 - (v2 & 0x3F);
  if ( (unsigned int)v5 <= 8 )
    v5 = (unsigned int)(v5 + 64);
  memset(v9, 0, (unsigned int)(v5 - 8));
  v6 = (v2 >> 29) | (8 * a1[8]);
  v9[0] = 0x80;
  v8[0] = v6;
  v8[1] = 8 * v2;
  DWORDToBigEndian((_DWORD *)((char *)&v8[2] + v5), (__int64)v8, 2LL);
  SHA256Update(a1, v9, (unsigned int)v5);
  DWORDToBigEndian(a2, (__int64)a1, 8LL);
  a1[8] = 0;
  a1[9] = 0;
  result = 0LL;
  *a1 = 1779033703;
  a1[1] = -1150833019;
  a1[2] = 1013904242;
  a1[3] = -1521486534;
  a1[4] = 1359893119;
  a1[5] = -1694144372;
  a1[6] = 528734635;
  a1[7] = 1541459225;
  memset(a1 + 10, 0, 0x40uLL);
  return result;
}
