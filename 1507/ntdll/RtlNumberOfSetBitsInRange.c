/*
 * XREFs of RtlNumberOfSetBitsInRange @ 0x1800D3340
 * Callers:
 *     RtlNumberOfClearBitsInRange @ 0x1800D3160 (RtlNumberOfClearBitsInRange.c)
...
__int64 __fastcall RtlNumberOfSetBitsInRange(__int64 a1, unsigned int a2, unsigned int a3)
{
  ...
  v9 = (a3 + a2 - 1) & 7;
  if ( a2 >> 3 == v7 )
    return RtlpBitsClearTotal[(unsigned __int8)~(*v6 & RtlBaseAceType[v8 + 32] & RtlBaseAceType[v9 + 1 + 16])];
  if ( (((unsigned __int8)a3 | (unsigned __int8)a2) & 0x3F) != 0 )
  {
    if ( (a2 & 7) != 0 )
    {
      v13 = *v6++;
      ++v5;
      v4 = RtlpBitsClearTotal[(unsigned __int8)~(v13 & RtlBaseAceType[v8 + 32])];
    }
    v14 = 8 - (v5 & 7);
    if ( (v14 & 0xFFFFFFF7) != 0 )
    {
    ...
    return v4 + RtlpBitsClearTotal[(unsigned __int8)~(*v6 & RtlBaseAceType[v9 + 1 + 16])];
  }
  else
  {
    v11 = ((a3 - 1) >> 6) + 1;
