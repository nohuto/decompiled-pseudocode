/*
 * XREFs of RtlFindLastBackwardRunClear @ 0x18005A360
 * Callers:
 *     <none>
 * Callees:
 *     <none>
...
ULONG __cdecl RtlFindLastBackwardRunClear(PRTL_BITMAP BitMapHeader, ULONG FromIndex, PULONG StartingRunIndex)
{
  ...
  else
  {
    ...
    {
      v6 = FromIndex & 0x1F;
      if ( (*v5 | ~dword_180106450[v6 + 1]) == 0xFFFFFFFF )
      {
    ...
    while ( FromIndex != -1 && _bittest64((const signed __int64 *)Buffer, FromIndex) == 1 )
      --FromIndex;
    v7 = FromIndex;
    if ( (dword_180106450[FromIndex & 0x1F] & *v5) == 0 )
    {
      v7 = FromIndex - (FromIndex & 0x1F) - 1;
