/*
 * XREFs of AVrfpChainDuplicateVerificationLayers @ 0x1800C5350
 * Callers:
      ...
      do
      {
        ...
        if ( *v5 )
        {
          ...
          do
          {
            ...
            ClosestThunkDuplicate = AVrfpFindClosestThunkDuplicate(v1, *v4, *v7);
            v10 = ClosestThunkDuplicate;
            if ( ClosestThunkDuplicate )
            {
              if ( (AVrfpDebug & 0x10) != 0 )
              {
                DbgPrint("AVRF: Chaining (%ws: %s) to %ws\n", *v4, *v7, v1[3]);
                ClosestThunkDuplicate = v10;
              }
