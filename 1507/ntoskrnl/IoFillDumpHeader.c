/*
 * XREFs of IoFillDumpHeader @ 0x1401F2BD0
 * Callers:
 *     IopConstructInMemoryDumpHeader @ 0x1401F4568 (IopConstructInMemoryDumpHeader.c)
 *     IopWriteCapsuleTriageDumpToFirmware @ 0x1401F5200 (IopWriteCapsuleTriageDumpToFirmware.c)
 *     KeInitializeCrashDumpHeader @ 0x1401F5C94 (KeInitializeCrashDumpHeader.c)
 *     IopLiveDumpEndMirroringCallback @ 0x1403FEF9C (IopLiveDumpEndMirroringCallback.c)
 *     DbgkpTriageDumpFillHeaders @ 0x14066B298 (DbgkpTriageDumpFillHeaders.c)
 * Callees:
 *     memmove @ 0x140195740 (memmove.c)
 *     memset @ 0x140195A80 (memset.c)
 *     ExFreePoolWithTag @ 0x140288010 (ExFreePoolWithTag.c)
 *     MmGetPhysicalMemoryRanges @ 0x14051CF60 (MmGetPhysicalMemoryRanges.c)
 */

__int64 __fastcall IoFillDumpHeader(
        _DWORD *a1,
        int a2,
        int a3,
        __int64 a4,
        __int64 a5,
        __int64 a6,
        __int64 a7,
        __int64 a8)
{
  void *v9; // r11
  unsigned __int64 v10; // r10
  unsigned __int64 v11; // rax
  void *v12; // rcx
  size_t v13; // r8
  PPHYSICAL_MEMORY_RANGE PhysicalMemoryRanges; // rax
  PPHYSICAL_MEMORY_RANGE v15; // r8
  __int64 v16; // rdi
  LARGE_INTEGER *p_NumberOfBytes; // rdx
  __int64 QuadPart; // rcx
  __int64 v19; // r9
  signed __int64 v20; // rcx
  __int64 v21; // rax
  __int64 result; // rax

  v9 = a1;
  v10 = 2048LL;
  if ( ((unsigned __int8)a1 & 4) != 0 )
  {
    *a1 = 1162297680;
    v9 = a1 + 1;
    v10 = 2047LL;
  }
  memset64(v9, 0x4547415045474150uLL, v10 >> 1);
  if ( (v10 & 1) != 0 )
    *((_DWORD *)v9 + v10 - 1) = 1162297680;
  *((_QWORD *)a1 + 9) = a5;
  *((_QWORD *)a1 + 10) = a6;
  *((_QWORD *)a1 + 11) = a7;
  a1[1] = 875976004;
  a1[14] = a3;
  *((_QWORD *)a1 + 8) = a4;
  if ( a8 )
  {
    *((_QWORD *)a1 + 2) = *(_QWORD *)(*(_QWORD *)(a8 + 184) + 40LL) & 0xFFFFFFFFFFFFF000uLL;
  }
  else
  {
    v11 = __readcr3();
    *((_QWORD *)a1 + 2) = v11 & 0xFFFFFFFFFFFFF000uLL;
  }
  a1[12] = 34404;
  a1[998] = a2;
  *((_QWORD *)a1 + 3) = MmPfnDatabase;
  *((_QWORD *)a1 + 4) = &PsLoadedModuleList;
  *((_QWORD *)a1 + 5) = &PsActiveProcessHead;
  *((_QWORD *)a1 + 16) = &KdDebuggerDataBlock;
  a1[13] = KeNumberProcessors_0;
  a1[2] = 15;
  a1[3] = 10240;
  if ( (unsigned int)(a2 - 5) <= 1 || !MmPhysicalMemoryBlock )
    goto LABEL_24;
  if ( KeGetCurrentIrql() > 1u )
  {
    v12 = a1 + 34;
    v13 = 16 * ((unsigned int)(*(_DWORD *)MmPhysicalMemoryBlock - 1) + 2LL);
    if ( v13 > 0x2BC )
      v13 = 700LL;
    memmove(v12, MmPhysicalMemoryBlock, v13);
    goto LABEL_25;
  }
  PhysicalMemoryRanges = MmGetPhysicalMemoryRanges();
  v15 = PhysicalMemoryRanges;
  if ( !PhysicalMemoryRanges )
  {
LABEL_24:
    memset(a1 + 34, 0, 0x2BCuLL);
    goto LABEL_25;
  }
  v16 = 0LL;
  p_NumberOfBytes = &PhysicalMemoryRanges->NumberOfBytes;
  QuadPart = PhysicalMemoryRanges->NumberOfBytes.QuadPart;
  v19 = 0LL;
  while ( QuadPart )
  {
    v16 = (unsigned int)(v16 + 1);
    v19 += QuadPart >> 12;
    QuadPart = PhysicalMemoryRanges[(unsigned int)v16].NumberOfBytes.QuadPart;
  }
  *((_QWORD *)a1 + 18) = v19;
  if ( 16 * ((unsigned __int64)(unsigned int)(v16 - 1) + 2) > 0x2BC )
    v16 = 42LL;
  a1[34] = v16;
  if ( (_DWORD)v16 )
  {
    v20 = (char *)a1 - (char *)PhysicalMemoryRanges;
    do
    {
      *(LONGLONG *)((char *)&p_NumberOfBytes[18].QuadPart + v20) = p_NumberOfBytes[-1].QuadPart >> 12;
      v21 = p_NumberOfBytes->QuadPart;
      p_NumberOfBytes += 2;
      *(LONGLONG *)((char *)&p_NumberOfBytes[17].QuadPart + v20) = v21 >> 12;
      --v16;
    }
    while ( v16 );
  }
  ExFreePoolWithTag(v15, 0);
LABEL_25:
  memset(a1 + 210, 0, 0xBB8uLL);
  *((_QWORD *)a1 + 481) = 0LL;
  a1[966] = 0;
  *((_QWORD *)a1 + 482) = 0LL;
  a1[960] = -2147483645;
  a1[961] = 1;
  a1[1002] = MEMORY[0xFFFFF78000000014];
  a1[1003] = MEMORY[0xFFFFF78000000018];
  a1[1036] = MEMORY[0xFFFFF78000000008];
  a1[1037] = MEMORY[0xFFFFF7800000000C];
  a1[1040] = MEMORY[0xFFFFF78000000264];
  a1[1041] = MEMORY[0xFFFFF780000002D0];
  result = MEMORY[0xFFFFF780000002C4];
  a1[1044] = 0;
  a1[1045] = result;
  return result;
}
