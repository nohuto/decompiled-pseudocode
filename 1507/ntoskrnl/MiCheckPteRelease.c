/*
 * XREFs of MiCheckPteRelease @ 0x1402265D8
 * Callers:
...
__int64 __fastcall MiCheckPteRelease(__int64 a1, ULONG_PTR a2)
{
  ...
  if ( v3 < qword_14034EC70 || v3 >= qword_14034EC70 + (qword_14034EC60 << 21) )
    KeBugCheckEx(0xDAu, 0x301uLL, a1 << 25 >> 16, 0LL, 0LL);
  v4 = 2LL * (unsigned int)((a1 - qword_14034FC80) >> 3);
  if ( _bittest64((const signed __int64 *)qword_14034F588, v4) )
    KeBugCheckEx(0xDAu, 0x303uLL, v3, a2, 0LL);
  LOBYTE(v5) = 2;
  if ( v4 && *(_DWORD *)(qword_14034F588 + 4 * ((v4 - 2) >> 5)) >> ((v4 - 2) & 0x1F) == 2 )
    KeBugCheckEx(0xDAu, 0x304uLL, v3, a2, 0LL);
  v6 = v4 + 1;
  for ( i = _bittest64((const signed __int64 *)qword_14034F588, v4 + 1);
        i == 1;
        i = _bittest64((const signed __int64 *)qword_14034F588, v6) )
  {
    v6 += 2LL;
  }
  ...
  for ( j = v4; j < v9; j += 2LL )
  {
    if ( _bittest64((const signed __int64 *)qword_14034F588, j) == 1 )
  ...
  {
    v16 = v4 & 0x1F;
    v14 = (volatile signed __int32 *)(qword_14034F588 + 4 * (v4 >> 5));
    if ( v4 >= v11 )
      break;
    LOBYTE(v12) = 1;
    if ( v16 + 1 <= 0x20 )
