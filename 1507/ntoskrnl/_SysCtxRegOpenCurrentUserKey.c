/*
 * XREFs of _SysCtxRegOpenCurrentUserKey @ 0x140719880
 * Callers:
...
__int64 __fastcall SysCtxRegOpenCurrentUserKey(_QWORD *a1, char a2, ACCESS_MASK a3, HANDLE *a4)
{
  wchar_t *v5; // rdi
  int appended; // ebx
  PSID v10; // rsi
    ...
    if ( appended >= 0 )
    {
      ...
      if ( appended >= 0 )
      {
        ...
        if ( PoolWithTag )
        {
          ...
          appended = RtlAppendUnicodeStringToString(&Destination, &stru_140294D20);
          if ( appended >= 0 )
          {
            ...
            if ( appended >= 0 )
            {
              ...
              if ( appended >= 0 )
                appended = SysCtxRegOpenKey(v13, v23, (const WCHAR *)Destination.Buffer + 15, a2, a3, a4);
            }
          }
        }
        else
