/*
 * XREFs of PopDiagTracePreSleepNotification @ 0x140565A98
 * Callers:
...
char __fastcall PopDiagTracePreSleepNotification(int a1, int a2, int a3, int a4)
{
  _UNKNOWN **v4; // rax
  REGHANDLE v6; // rbx
  ...
  {
    v6 = PopDiagHandle;
    LOBYTE(v4) = EtwEventEnabled(PopDiagHandle, &POP_ETW_EVENT_PRESLEEP_NOTIFICATION2);
    if ( (_BYTE)v4 )
    {
      UserData.Reserved = 0;
      v12 = 0;
      ...
      v14 = 4;
      v17 = 4;
      LOBYTE(v4) = EtwWrite(v6, &POP_ETW_EVENT_PRESLEEP_NOTIFICATION2, 0LL, 4u, &UserData);
    }
  }
  return (char)v4;
}
