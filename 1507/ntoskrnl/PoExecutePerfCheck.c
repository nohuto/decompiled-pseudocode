/*
 * XREFs of PoExecutePerfCheck @ 0x1400A3DE0
 * Callers:
 *     KeClockInterruptNotify @ 0x1400A6130 (KeClockInterruptNotify.c)
 * Callees:
 *     KiInsertQueueDpc @ 0x1400A48A0 (KiInsertQueueDpc.c)
 */

void PoExecutePerfCheck()
{
  __int64 v0; // rcx
  unsigned __int64 v1; // r8
  signed __int64 v2; // r9
  int v3; // r10d
  unsigned __int64 v4; // rax
  unsigned __int64 v5; // rax
  char v6; // cl
  __int64 *v7; // rdx
  __int64 v8; // r8
  __int64 v9; // rax

  if ( PpmIdleCheckIdleDurationExpiration )
  {
    v6 = 0;
    if ( KeNumberNodes )
    {
      v7 = KeNodeBlock;
      v8 = (unsigned __int16)KeNumberNodes;
      do
      {
        v9 = *v7++;
        if ( *(_QWORD *)(v9 + 72) )
          v6 = 1;
        --v8;
      }
      while ( v8 );
      if ( v6 )
        KiInsertQueueDpc((unsigned int)&PpmCheckIdleExpirationDpc, 0, 0, 0, 0);
    }
  }
  v0 = PpmCheckLastExecutionTime;
  if ( PpmCheckLastExecutionTime )
  {
    v1 = (unsigned int)KeTimeIncrement;
    if ( PpmCheckPeriod > (unsigned __int64)(unsigned int)KeTimeIncrement )
      v1 = PpmCheckPeriod;
    v2 = MEMORY[0xFFFFF78000000008];
    if ( v1 + PpmCheckLastExecutionTime <= MEMORY[0xFFFFF78000000008] )
    {
      v3 = 0;
      if ( PpmCheckLastExecutionTime != 1 )
      {
        if ( PpmCheckLastExecutionTime + v1 + KeMaximumIncrement > MEMORY[0xFFFFF78000000008] )
          v2 = v1 + PpmCheckLastExecutionTime;
        v4 = v2 - v1 - PpmCheckLastExecutionTime;
        if ( v4 >= v1 )
        {
          v3 = 64;
          v5 = v4 / v1;
          if ( (unsigned int)v5 < 0x40 )
            v3 = v5;
        }
      }
      if ( v0 == _InterlockedCompareExchange64(&PpmCheckLastExecutionTime, v2, PpmCheckLastExecutionTime) )
        KiInsertQueueDpc((unsigned int)&PpmCheckStartDpc, v3, 0, 0, 0);
    }
  }
}
