/*
 * XREFs of RtlNumberOfSetBitsInRange @ 0x140247F08
 * Callers:
 *     RtlNumberOfClearBitsInRange @ 0x140247D4C (RtlNumberOfClearBitsInRange.c)
...
ULONG __cdecl RtlNumberOfSetBitsInRange(PRTL_BITMAP BitMapHeader, ULONG StartingIndex, ULONG Length)
{
  ...
  if ( StartingIndex >> 3 == v7 )
    return RtlpBitsClearTotal[(unsigned __int8)~(*v6 & byte_140295400[v8] & byte_140297800[v9 + 1])];
  if ( (((unsigned __int8)Length | (unsigned __int8)StartingIndex) & 0x3F) != 0 )
  {
    if ( (StartingIndex & 7) != 0 )
    {
      v13 = *v6++;
      ++v5;
      v4 = RtlpBitsClearTotal[(unsigned __int8)~(v13 & byte_140295400[v8])];
    }
    v14 = 8 - (v5 & 7);
    if ( (v14 & 0xFFFFFFF7) != 0 )
    {
    ...
    return v4 + RtlpBitsClearTotal[(unsigned __int8)~(*v6 & byte_140297800[v9 + 1])];
  }
  else
  {
    v11 = ((Length - 1) >> 6) + 1;
