/*
 * XREFs of CmpParseInfBuffer @ 0x1407D9018
 * Callers:
 *     EmpParseInfDatabase @ 0x1407CD974 (EmpParseInfDatabase.c)
 * Callees:
 *     ExFreePoolWithTag @ 0x140288010 (ExFreePoolWithTag.c)
 *     ExAllocatePoolWithTag @ 0x140288E60 (ExAllocatePoolWithTag.c)
 *     CmpGetToken @ 0x1407D92F8 (CmpGetToken.c)
 *     CmpAppendValue @ 0x1407D9584 (CmpAppendValue.c)
 *     CmpAppendLine @ 0x1407D9600 (CmpAppendLine.c)
 *     CmpAppendSection @ 0x1407D9680 (CmpAppendSection.c)
 */

_QWORD *__fastcall CmpParseInfBuffer(__int64 a1, unsigned int a2, int *a3)
{
  __int64 *v3; // r13
  __int64 v4; // rdi
  char v6; // r12
  _QWORD *result; // rax
  _QWORD *v8; // r14
  unsigned int v9; // r15d
  __int64 v10; // rax
  char v11; // di
  char v12; // si
  __int64 v13; // r8
  int v14; // ebx
  __int64 v15; // r8
  __int64 v16; // r8
  __int64 v17; // r8
  __int64 *P; // [rsp+20h] [rbp-30h]
  __int64 v19; // [rsp+28h] [rbp-28h] BYREF
  __int64 v20; // [rsp+30h] [rbp-20h]
  int v21; // [rsp+38h] [rbp-18h] BYREF
  __int64 *v22; // [rsp+40h] [rbp-10h]
  char v23; // [rsp+48h] [rbp-8h]
  int v25; // [rsp+A8h] [rbp+58h]

  v3 = 0LL;
  v4 = a2;
  v6 = 0;
  result = ExAllocatePoolWithTag(PagedPool, 0x30uLL, 0x69704D43u);
  v8 = result;
  if ( !result )
    return result;
  v9 = 1;
  *result = 0LL;
  result[1] = 0LL;
  result[2] = 0LL;
  result[3] = 0LL;
  *((_DWORD *)result + 8) = 0;
  result[5] = 0LL;
  v10 = a1 + v4;
  v19 = a1;
  v11 = 0;
  v20 = v10;
  v12 = 0;
  v25 = 1;
  P = 0LL;
  do
  {
    CmpGetToken(&v19, v10, &v21);
    v13 = 6LL;
    if ( v9 <= 6 )
    {
      if ( v9 != 6 )
      {
        if ( v9 == 1 )
        {
          v14 = v21;
          if ( v21 )
          {
            if ( v21 != 1 )
            {
              if ( v21 != 2 )
                goto LABEL_56;
              v9 = 2;
            }
            goto LABEL_24;
          }
          goto LABEL_55;
        }
        if ( v9 == 2 )
        {
          v14 = v21;
          if ( v21 != 3 )
          {
            if ( v21 != 4 )
              goto LABEL_56;
            v9 = 3;
            v6 = v23;
            P = v22;
            goto LABEL_24;
          }
          v23 = 0;
          v22 = &EmptyValue;
          v6 = 0;
        }
        else
        {
          if ( v9 != 3 )
          {
            if ( v9 == 4 )
            {
              v14 = v21;
              if ( v21 )
              {
                if ( v21 != 1 )
                {
                  v11 = 1;
                  v12 = 1;
                  goto LABEL_59;
                }
                LOBYTE(v13) = v6;
                if ( !(unsigned __int8)CmpAppendSection(v8, P, v13) )
                  goto LABEL_56;
                P = 0LL;
                goto LABEL_35;
              }
              LOBYTE(v13) = v6;
              if ( !(unsigned __int8)CmpAppendSection(v8, P, v13) )
                goto LABEL_56;
              P = 0LL;
            }
            else
            {
              v14 = v21;
              if ( v21 )
              {
                if ( v21 != 1 )
                {
                  if ( v21 == 2 )
                  {
                    v9 = 2;
                  }
                  else
                  {
                    if ( v21 != 4 )
                    {
                      v11 = 1;
                      v12 = 1;
                      goto LABEL_59;
                    }
                    v3 = v22;
                    v9 = 6;
                    v6 = v23;
                  }
                }
                goto LABEL_24;
              }
            }
            goto LABEL_55;
          }
          v14 = v21;
          if ( v21 != 3 )
            goto LABEL_56;
        }
        v9 = 4;
        goto LABEL_24;
      }
      v14 = v21;
      if ( v21 )
      {
        switch ( v21 )
        {
          case 1:
            if ( (unsigned __int8)CmpAppendLine(v8, 0LL, 0LL) )
            {
              LOBYTE(v16) = v6;
              if ( (unsigned __int8)CmpAppendValue(v8, v3, v16) )
              {
                v3 = 0LL;
                goto LABEL_35;
              }
            }
            break;
          case 5:
            LOBYTE(v13) = v6;
            if ( !(unsigned __int8)CmpAppendLine(v8, v3, v13) )
              goto LABEL_56;
            v3 = 0LL;
            v9 = 8;
            goto LABEL_24;
          case 6:
            if ( (unsigned __int8)CmpAppendLine(v8, 0LL, 0LL) )
            {
              LOBYTE(v15) = v6;
              if ( (unsigned __int8)CmpAppendValue(v8, v3, v15) )
              {
                v3 = 0LL;
                v9 = 7;
                goto LABEL_24;
              }
            }
            break;
          default:
            goto LABEL_56;
        }
      }
      else if ( (unsigned __int8)CmpAppendLine(v8, 0LL, 0LL) )
      {
        LOBYTE(v17) = v6;
        if ( (unsigned __int8)CmpAppendValue(v8, v3, v17) )
        {
          v3 = 0LL;
          goto LABEL_55;
        }
      }
    }
    else
    {
      if ( v9 != 7 )
      {
        if ( v9 != 8 )
        {
          v14 = v21;
          if ( v21 )
          {
            if ( v21 != 1 )
            {
              if ( v21 != 6 )
                goto LABEL_56;
              v9 = 7;
              goto LABEL_24;
            }
            goto LABEL_35;
          }
          goto LABEL_55;
        }
        v14 = v21;
        if ( v21 )
        {
          if ( v21 != 1 )
          {
            if ( v21 != 4 )
              goto LABEL_56;
            v13 = 0LL;
            goto LABEL_22;
          }
          v23 = 0;
          v22 = &EmptyValue;
          v6 = 0;
          if ( !(unsigned __int8)CmpAppendValue(v8, &EmptyValue, 0LL) )
          {
            v12 = 1;
            v11 = 1;
            goto LABEL_59;
          }
LABEL_35:
          v9 = 5;
          goto LABEL_24;
        }
        v23 = 0;
        v22 = &EmptyValue;
        v6 = 0;
        if ( !(unsigned __int8)CmpAppendValue(v8, &EmptyValue, 0LL) )
          v12 = 1;
LABEL_55:
        v11 = 1;
        goto LABEL_24;
      }
      v14 = v21;
      if ( v21 == 4 )
      {
        LOBYTE(v13) = v23;
LABEL_22:
        if ( !(unsigned __int8)CmpAppendValue(v8, v22, v13) )
          goto LABEL_56;
        v9 = 9;
        goto LABEL_24;
      }
      if ( v21 != 6 )
      {
LABEL_56:
        v11 = 1;
        v12 = 1;
        goto LABEL_59;
      }
      v23 = 0;
      v22 = &EmptyValue;
      v6 = 0;
      if ( (unsigned __int8)CmpAppendValue(v8, &EmptyValue, 0LL) )
        goto LABEL_24;
    }
    v11 = 1;
    v12 = 1;
LABEL_24:
    if ( !v12 )
    {
      if ( v14 == 1 )
        ++v25;
      goto LABEL_27;
    }
LABEL_59:
    *a3 = v25;
    if ( P && v6 )
      ExFreePoolWithTag(P, 0);
    if ( v3 && v6 )
      ExFreePoolWithTag(v3, 0);
    ExFreePoolWithTag(v8, 0);
    v8 = 0LL;
LABEL_27:
    v10 = v20;
  }
  while ( !v11 );
  if ( v8 )
    v8[1] = 0LL;
  return v8;
}
