/*
 * XREFs of SeValidSecurityDescriptor @ 0x1404AB594
 * Callers:
 ...
 *     VerifierSeValidSecurityDescriptor @ 0x14074254C (VerifierSeValidSecurityDescriptor.c)
 * Callees:
...
BOOLEAN __stdcall SeValidSecurityDescriptor(ULONG Length, PSECURITY_DESCRIPTOR SecurityDescriptor)
{
  ...
  if ( *((_BYTE *)SecurityDescriptor + v4) != 1 )
    return 0;
  v5 = *((_BYTE *)SecurityDescriptor + v4 + 1);
  if ( v5 > 0xFu )
    return 0;
  if ( Length - (unsigned int)v4 < 4 * (unsigned int)v5 + 8 )
    return 0;
  ...
  if ( (_DWORD)v6 )
  {
    ...
    if ( *((_BYTE *)SecurityDescriptor + v6) != 1 )
      return 0;
    v7 = *((_BYTE *)SecurityDescriptor + v6 + 1);
    if ( v7 > 0xFu || Length - (unsigned int)v6 < 4 * (unsigned int)v7 + 8 )
      return 0;
  }
