/*
 * XREFs of AdtpBuildAccessReasonAuditStringInternal @ 0x14026CFDC
 * Callers:
...
LABEL_123:
                          ...
                          v49 = v56 + 1 + (Destination.Length >> 1);
                          PoolWithTag = (char *)ExAllocatePoolWithTag(PagedPool, 2LL * v49, 0x6B416553u);
                          if ( PoolWithTag )
                          {
                            *v71 = 1;
                            if ( v63.Length )
                              memmove(PoolWithTag, v63.Buffer, v63.Length);
                            Buffer = Destination.Buffer;
                            if ( Length )
                              memmove(&PoolWithTag[v63.Length], Destination.Buffer, Length);
