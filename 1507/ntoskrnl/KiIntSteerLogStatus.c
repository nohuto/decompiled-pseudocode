/*
 * XREFs of KiIntSteerLogStatus @ 0x140209704
 * Callers:
 *     KiIntSteerEventTraceControlCallback @ 0x1405C2C8C (KiIntSteerEventTraceControlCallback.c)
 * Callees:
 *     EtwWrite @ 0x140014D30 (EtwWrite.c)
 *     KiIntSteerLogProc @ 0x1400E9FE4 (KiIntSteerLogProc.c)
 *     EtwEventEnabled @ 0x1400EA070 (EtwEventEnabled.c)
 *     KiIntSteerLogState @ 0x140155CB8 (KiIntSteerLogState.c)
 *     __security_check_cookie @ 0x1401716B0 (__security_check_cookie.c)
 */

char __fastcall KiIntSteerLogStatus(char a1)
{
  REGHANDLE v1; // rdi
  const EVENT_DESCRIPTOR *v2; // rbx
  const EVENT_DESCRIPTOR *v4; // rsi
  const EVENT_DESCRIPTOR *v5; // rax
  ULONG_PTR *i; // rbx
  ULONG_PTR *j; // rdi
  struct _EVENT_DATA_DESCRIPTOR UserData; // [rsp+30h] [rbp-29h] BYREF
  int *v10; // [rsp+40h] [rbp-19h]
  int v11; // [rsp+48h] [rbp-11h]
  int v12; // [rsp+4Ch] [rbp-Dh]
  int *v13; // [rsp+50h] [rbp-9h]
  int v14; // [rsp+58h] [rbp-1h]
  int v15; // [rsp+5Ch] [rbp+3h]
  int *v16; // [rsp+60h] [rbp+7h]
  int v17; // [rsp+68h] [rbp+Fh]
  int v18; // [rsp+6Ch] [rbp+13h]
  __int64 *v19; // [rsp+70h] [rbp+17h]
  int v20; // [rsp+78h] [rbp+1Fh]
  int v21; // [rsp+7Ch] [rbp+23h]

  v1 = KiIntSteerEtwHandle;
  v2 = &PPM_ETW_INTERRUPT_STEERING_MASK_CHANGE;
  if ( a1 )
    v2 = &KMPnPRundownEvt_SleepStudy_ConnectionResource;
  if ( KiIntSteerEtwHandle && EtwEventEnabled(KiIntSteerEtwHandle, v2) )
  {
    UserData.Reserved = 0;
    v12 = 0;
    v15 = 0;
    v18 = 0;
    v21 = 0;
    UserData.Ptr = (ULONGLONG)&KiIntSteerLoadPercent;
    UserData.Size = 4;
    v10 = &KiIntTrackRootCount;
    v13 = &KiIntSteerMaskCount;
    v16 = &KiIntSteerMask;
    v19 = qword_1403328C8;
    v11 = 4;
    v14 = 4;
    v17 = 2;
    v20 = 160 * (unsigned __int16)KiIntSteerMask;
    EtwWrite(v1, v2, 0LL, 5u, &UserData);
  }
  KiIntSteerLogProc(a1);
  v4 = &PPM_ETW_INTERRUPT_STEERING_STATE_RETARGET;
  v5 = &PPM_ETW_INTERRUPT_STEERING_STATE_RUNDOWN;
  if ( a1 )
    v4 = &PPM_ETW_INTERRUPT_STEERING_STATE_RUNDOWN;
  if ( KiIntSteerEtwHandle )
  {
    LOBYTE(v5) = EtwEventEnabled(KiIntSteerEtwHandle, v4);
    if ( (_BYTE)v5 )
    {
      for ( i = (ULONG_PTR *)KiIntTrackRootList; i != &KiIntTrackRootList; i = (ULONG_PTR *)*i )
      {
        if ( !a1 )
        {
          v5 = (const EVENT_DESCRIPTOR *)i[19];
          if ( (const EVENT_DESCRIPTOR *)i[21] == v5 )
            continue;
        }
        for ( j = (ULONG_PTR *)i[2]; j != i + 2; j = (ULONG_PTR *)*j )
          LOBYTE(v5) = KiIntSteerLogState((__int64)j, v4);
      }
    }
  }
  return (char)v5;
}
