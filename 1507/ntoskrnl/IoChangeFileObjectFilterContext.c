/*
 * XREFs of IoChangeFileObjectFilterContext @ 0x140071654
 * Callers:
 *     FsRtlInsertPerFileObjectContext @ 0x1400714D0 (FsRtlInsertPerFileObjectContext.c)
 *     FsRtlPTeardownPerFileObjectContexts @ 0x1404A586C (FsRtlPTeardownPerFileObjectContexts.c)
 * Callees:
 *     IopGetFileObjectExtension @ 0x1400716C0 (IopGetFileObjectExtension.c)
 */

__int64 __fastcall IoChangeFileObjectFilterContext(__int64 a1)
{
  signed __int64 v1; // r9
  signed __int64 v2; // r11
  char v3; // r10
  signed __int64 v4; // rax
  __int64 v6; // [rsp+30h] [rbp+8h] BYREF

  v6 = 0LL;
  if ( !*(_QWORD *)(a1 + 208) )
    return 3221225485LL;
  IopGetFileObjectExtension(a1, 3LL, &v6);
  if ( v3 )
    v4 = _InterlockedCompareExchange64((volatile signed __int64 *)(v6 + 32), v1, 0LL);
  else
    v4 = _InterlockedCompareExchange64((volatile signed __int64 *)(v6 + 32), v2, v1) - v1;
  return v4 != 0 ? 0xC0000021 : 0;
}
