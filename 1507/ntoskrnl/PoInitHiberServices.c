...
void __fastcall PoInitHiberServices(char a1)
{
  ...
  EmClientQueryRuleState(&GUID_EM_REMOVE_BAD_S3_PAGE_RULE, &KeyHandle);
  if ( (int)BcdOpenSystemStore((__int64)&KeyHandle) >= 0 )
  {
    PopBcdEstablishResumeObject((__int64)KeyHandle, 0LL);
    ...
    if ( PoolWithTag )
    {
             ...
             &ReturnLength) >= 0 )
      {
        ...
        if ( *(_BYTE *)v1 )
        {
          ...
          {
            v2 = 1;
            PoDisableSleepStates(2, 8, &KeyHandle);
          }
        }
      }
    }
