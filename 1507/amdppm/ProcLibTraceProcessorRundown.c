/*
 * XREFs of ProcLibTraceProcessorRundown @ 0x1C00134EC
 * Callers:
...
char __fastcall ProcLibTraceProcessorRundown(__int64 a1)
{
  ...
  LOBYTE(ProcessorNumber) = EtwEventEnabled(ProcLibEtwHandle, &PPM_ETW_PROCESSOR_FIRMWARE_RUNDOWN);
  if ( (_BYTE)ProcessorNumber )
  {
    ...
    if ( ProcessorNumber >= 0 )
    {
      ...
      v26 = 2;
      v29 = 1;
      LOBYTE(ProcessorNumber) = EtwWrite(ProcLibEtwHandle, &PPM_ETW_PROCESSOR_FIRMWARE_RUNDOWN, 0LL, 5u, &UserData);
    }
  }
