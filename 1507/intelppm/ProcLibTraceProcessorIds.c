/*
 * XREFs of ProcLibTraceProcessorIds @ 0x1C001A738
 * Callers:
...
char __fastcall ProcLibTraceProcessorIds(__int64 a1)
{
  ...
  LOBYTE(ProcessorNumber) = EtwEventEnabled(ProcLibEtwHandle, &PPM_ETW_PROCESSOR_ID_RUNDOWN);
  if ( (_BYTE)ProcessorNumber )
  {
    ...
    if ( ProcessorNumber >= 0 )
    {
      ...
      UserData.Size = 2;
      v7 = 1;
      LOBYTE(ProcessorNumber) = EtwWrite(ProcLibEtwHandle, &PPM_ETW_PROCESSOR_ID_RUNDOWN, 0LL, 5u, &UserData);
    }
  }
