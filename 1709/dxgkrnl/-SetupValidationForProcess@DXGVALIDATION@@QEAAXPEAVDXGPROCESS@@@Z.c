/*
 * XREFs of ?SetupValidationForProcess@DXGVALIDATION@@QEAAXPEAVDXGPROCESS@@@Z @ 0x1C0008CAC
 * Callers:
        ...
        && ProcessInformationLength >= 0x10 )
      {
        ...
        if ( v3 )
        {
          if ( ZwQueryInformationProcess(ProcessHandle, ProcessImageFileNameWin32, v3, ProcessInformationLength, 0LL) >= 0 )
          {
            ...
            RtlQueryRegistryValuesEx(2LL, &DeviceObject.Dpc.DpcListEntry, &v8, v4, 0LL);
          }
          operator delete(v4);
        }
      }
