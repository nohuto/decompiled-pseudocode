/*
 * XREFs of ?ReadVirtualFunctionConfig@ADAPTER_RENDER@@QEAAJW4DXG_VIRTUAL_GPU_TYPE@@PEAU_DXGKARG_READVIRTUALFUNCTIONCONFIG@@@Z @ 0x1C0189834
 * Callers:
 *     DxgkDdiReadVirtualFunctionConfig @ 0x1C018A4A8 (DxgkDdiReadVirtualFunctionConfig.c)
 *     ?VirtualGpuDriverEscape@DXGK_VIRTUAL_GPU_PARAV@@UEAAJPEAU_DXGK_VIRTUALGPUDRIVERESCAPE@@@Z @ 0x1C018E640 (-VirtualGpuDriverEscape@DXGK_VIRTUAL_GPU_PARAV@@UEAAJPEAU_DXGK_VIRTUALGPUDRIVERESCAPE@@@Z.c)
 * Callees:
 *     _guard_dispatch_icall_nop @ 0x1C0016710 (_guard_dispatch_icall_nop.c)
 */

__int64 __fastcall ADAPTER_RENDER::ReadVirtualFunctionConfig(__int64 a1, int a2, __int64 a3)
{
  __int64 v3; // rbx
  __int64 v5; // rcx
  __int64 v6; // rax
  __int64 v8; // rax

  v3 = *(unsigned int *)(a3 + 8);
  if ( a2 == 1 )
    v5 = *(_QWORD *)(a1 + 1232);
  else
    v5 = *(_QWORD *)(a1 + 1224);
  if ( v5 )
  {
    if ( (unsigned int)v3 < *(_DWORD *)(v5 + 12) && *(_QWORD *)(*(_QWORD *)(v5 + 24) + 8 * v3) )
    {
      _mm_lfence();
      v5 = *(_QWORD *)(*(_QWORD *)(v5 + 24) + 8 * v3);
    }
    else
    {
      v6 = WdLogNewEntry5_WdError(v5);
      *(_QWORD *)(v6 + 24) = v3;
      WdLogEvent5_WdError(v6);
      v5 = 0LL;
    }
    if ( v5 )
      return (*(__int64 (__fastcall **)(__int64, __int64))(*(_QWORD *)v5 + 48LL))(v5, a3);
  }
  v8 = WdLogNewEntry5_WdError(v5);
  *(_QWORD *)(v8 + 24) = *(unsigned int *)(a3 + 8);
  WdLogEvent5_WdError(v8);
  return 3221225485LL;
}
