/*
 * XREFs of ?ConfigureSharedResourceHelper@DXGDEVICE@@QEAAJEPEAXPEAVDXGSHAREDRESOURCE@@EH@Z @ 0x1C0093178
 * Callers:
 *     ?ConfigureSharedResource@DXGDEVICE@@QEAAJEPEAXIE@Z @ 0x1C00932C8 (-ConfigureSharedResource@DXGDEVICE@@QEAAJEPEAXIE@Z.c)
 *     ?OpenForDevice@DXGDXGIKEYEDMUTEX@@QEAAJPEAVDXGDEVICE@@@Z @ 0x1C01C75B0 (-OpenForDevice@DXGDXGIKEYEDMUTEX@@QEAAJPEAVDXGDEVICE@@@Z.c)
 * Callees:
 *     ??0DXGAUTOMUTEX@@QEAA@QEAVDXGFASTMUTEX@@E@Z @ 0x1C0002EF0 (--0DXGAUTOMUTEX@@QEAA@QEAVDXGFASTMUTEX@@E@Z.c)
 *     ?Release@DXGAUTOMUTEX@@QEAAXXZ @ 0x1C0002F30 (-Release@DXGAUTOMUTEX@@QEAAXXZ.c)
 *     ?Acquire@DXGAUTOMUTEX@@QEAAXXZ @ 0x1C0002FA0 (-Acquire@DXGAUTOMUTEX@@QEAAXXZ.c)
 *     ??_GDXGPROCESSSHAREDACCESS@@QEAAPEAXI@Z @ 0x1C001D8A0 (--_GDXGPROCESSSHAREDACCESS@@QEAAPEAXI@Z.c)
 *     ??2@YAPEAX_KIW4_POOL_TYPE@@@Z @ 0x1C00A3090 (--2@YAPEAX_KIW4_POOL_TYPE@@@Z.c)
 */

__int64 __fastcall DXGDEVICE::ConfigureSharedResourceHelper(
        PERESOURCE *this,
        __int64 a2,
        void *a3,
        struct DXGSHAREDRESOURCE *a4,
        unsigned __int8 a5,
        int a6)
{
  char v8; // r14
  __int64 v10; // rbp
  PVOID v11; // rdi
  unsigned int v12; // esi
  __int64 *v13; // rbx
  DXGPROCESSSHAREDACCESS *v14; // rcx
  __int64 *i; // rdx
  char *v16; // rax
  __int64 v17; // rcx
  __int64 v18; // rcx
  _QWORD *v19; // rax
  __int64 v21; // rax
  _QWORD *v22; // rax
  NTSTATUS v23; // eax
  __int64 v24; // rdx
  __int64 v25; // rcx
  __int64 v26; // r8
  __int64 v27; // rax
  __int64 v28; // rax
  DXGPROCESSSHAREDACCESS **v29; // rdx
  DXGPROCESSSHAREDACCESS **v30; // r8
  _BYTE v31[40]; // [rsp+30h] [rbp-28h] BYREF
  PVOID Object; // [rsp+60h] [rbp+8h] BYREF

  v8 = a2;
  if ( !*((_DWORD *)this + 18) && !ExIsResourceAcquiredExclusiveLite(this[13]) )
  {
    v21 = WdLogNewEntry5_WdAssertion(this);
    *(_QWORD *)(v21 + 24) = 5144LL;
    WdLogEvent5_WdAssertion(v21);
  }
  if ( !a4 )
  {
    v22 = (_QWORD *)WdLogNewEntry5_WdWarning(this, a2, a3);
    v22[4] = 0LL;
    v22[3] = this;
LABEL_26:
    v22[5] = -1073741811LL;
    WdLogEvent5_WdWarning(v22);
    return 3221225485LL;
  }
  v10 = *((_QWORD *)a4 + 18);
  if ( !v10 || a6 && *(PERESOURCE **)(v10 + 72) != this )
  {
    v22 = (_QWORD *)WdLogNewEntry5_WdWarning(this, a2, a3);
    v22[3] = this;
    v22[4] = a4;
    goto LABEL_26;
  }
  v11 = 0LL;
  v12 = 0;
  if ( v8
    || (v23 = ObReferenceObjectByHandle(a3, 0x1000u, (POBJECT_TYPE)PsProcessType, 1, &Object, 0LL),
        v11 = Object,
        v12 = v23,
        v23 >= 0) )
  {
    DXGAUTOMUTEX::DXGAUTOMUTEX((DXGAUTOMUTEX *)v31, (struct DXGFASTMUTEX *const)(v10 + 8));
    DXGAUTOMUTEX::Acquire((DXGAUTOMUTEX *)v31);
    v13 = (__int64 *)(v10 + 48);
    v14 = 0LL;
    for ( i = *(__int64 **)(v10 + 48); i != v13; i = (__int64 *)*i )
    {
      if ( v8 )
      {
        if ( *((_BYTE *)i + 16) )
          goto LABEL_31;
      }
      else if ( !*((_BYTE *)i + 16) && (PVOID)i[3] == v11 )
      {
LABEL_31:
        v14 = (DXGPROCESSSHAREDACCESS *)(i - 1);
        break;
      }
    }
    if ( a5 )
    {
      if ( !v14 )
      {
        v16 = (char *)operator new(0x28uLL, 0x4B677844u, PagedPool);
        if ( v16 )
        {
          v16[24] = v8;
          *((_QWORD *)v16 + 4) = v11;
          v18 = *v13;
          v19 = v16 + 8;
          if ( *(__int64 **)(*v13 + 8) != v13 )
            __fastfail(3u);
          *v19 = v18;
          v11 = 0LL;
          v19[1] = v13;
          *(_QWORD *)(v18 + 8) = v19;
          *v13 = (__int64)v19;
        }
        else
        {
          v28 = WdLogNewEntry5_WdLowResource(v17);
          v12 = -1073741801;
          *(_QWORD *)(v28 + 24) = -1073741801LL;
          WdLogEvent5_WdLowResource(v28);
        }
      }
    }
    else if ( v14 )
    {
      v29 = (DXGPROCESSSHAREDACCESS **)*((_QWORD *)v14 + 1);
      if ( v29[1] != (DXGPROCESSSHAREDACCESS *)((char *)v14 + 8)
        || (v30 = (DXGPROCESSSHAREDACCESS **)*((_QWORD *)v14 + 2), *v30 != (DXGPROCESSSHAREDACCESS *)((char *)v14 + 8)) )
      {
        __fastfail(3u);
      }
      *v30 = (DXGPROCESSSHAREDACCESS *)v29;
      v29[1] = (DXGPROCESSSHAREDACCESS *)v30;
      DXGPROCESSSHAREDACCESS::`scalar deleting destructor'(v14);
    }
    if ( v11 )
      ObfDereferenceObject(v11);
    if ( v31[8] )
      DXGAUTOMUTEX::Release((DXGAUTOMUTEX *)v31);
  }
  else
  {
    v27 = WdLogNewEntry5_WdWarning(v25, v24, v26);
    *(_QWORD *)(v27 + 24) = a3;
    WdLogEvent5_WdWarning(v27);
  }
  return v12;
}
