/*
 * XREFs of ?EDIDV1_ObtainMonitorSerialNumber@@YAJPEBEPEAG@Z @ 0x1C000F930
 * Callers:
...
__int64 __fastcall EDIDV1_ObtainMonitorSerialNumber(unsigned __int8 *a1, unsigned __int16 *a2)
{
  ...
  if ( !a1 || !a2 )
    return 3221225485LL;
  v15[10] = 0LL;
  result = MonDescParser::EDID_PARSER::Initialize((MonDescParser::EDID_PARSER *)v15, a1, 0x80u);
  if ( (int)result >= 0 )
  {
    for ( i = 0; i < 4; i = v4 + 1 )
    {
      if ( MonDescParser::EDID_PARSER::Is18ByteDispDesc((MonDescParser::EDID_PARSER *)v15, i) )
      {
        if ( *(_BYTE *)(v5 + 18LL * v4 + 57) == 0xFF )
        {
          if ( MonDescParser::EDID_PARSER::GetDispDescAsASCIIStringLength((MonDescParser::EDID_PARSER *)v15, v4) )
          {
            MonDescParser::EDID_PARSER::GetDispDescAsASCIIString((MonDescParser::EDID_PARSER *)v15, v4, v6);
            return 0LL;
          }
        }
      }
