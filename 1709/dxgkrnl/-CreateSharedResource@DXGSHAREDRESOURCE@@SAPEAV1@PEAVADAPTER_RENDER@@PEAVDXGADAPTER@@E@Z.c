/*
 * XREFs of ?CreateSharedResource@DXGSHAREDRESOURCE@@SAPEAV1@PEAVADAPTER_RENDER@@PEAVDXGADAPTER@@E@Z @ 0x1C00912C8
 * Callers:
 *     ?OpenResourceObject@DXGDEVICE@@QEAAJPEAU_D3DKMT_CREATEALLOCATION@@PEAU_D3DDDI_ALLOCATIONINFO2@@EPEAU_EPROCESS@@PEAU_DXGSHAREDALLOCOBJECT@@PEAVDXGRESOURCEREFERENCE@@PEAEPEBU_D3DKM_CREATESTANDARDALLOCATION@@PEAVDXGAUTOMUTEX@@PEAVDXGAUTOPUSHLOCK@@@Z @ 0x1C0079A50 (-OpenResourceObject@DXGDEVICE@@QEAAJPEAU_D3DKMT_CREATEALLOCATION@@PEAU_D3DDDI_ALLOCATIONINFO2@@E.c)
 * Callees:
 *     ??_GDXGSHAREDRESOURCE_NONPAGED@@QEAAPEAXI@Z @ 0x1C0023EE8 (--_GDXGSHAREDRESOURCE_NONPAGED@@QEAAPEAXI@Z.c)
 *     ??0DXGSHAREDRESOURCE@@IEAA@PEAVADAPTER_RENDER@@PEAVDXGADAPTER@@@Z @ 0x1C0091370 (--0DXGSHAREDRESOURCE@@IEAA@PEAVADAPTER_RENDER@@PEAVDXGADAPTER@@@Z.c)
 *     ??2@YAPEAX_KIW4_POOL_TYPE@@@Z @ 0x1C00A3090 (--2@YAPEAX_KIW4_POOL_TYPE@@@Z.c)
 *     ??2@YAPEAX_KIHW4_POOL_TYPE@@@Z @ 0x1C00F925C (--2@YAPEAX_KIHW4_POOL_TYPE@@@Z.c)
 */

struct DXGSHAREDRESOURCE *__fastcall DXGSHAREDRESOURCE::CreateSharedResource(
        struct ADAPTER_RENDER *a1,
        struct DXGADAPTER *a2,
        char a3)
{
  __int64 v3; // rax
  DXGSHAREDRESOURCE_NONPAGED *v4; // rdi
  DXGSHAREDRESOURCE *v8; // rax
  DXGSHAREDRESOURCE *v9; // rbx
  unsigned int v11; // eax
  KSPIN_LOCK *v12; // rax
  DXGSHAREDRESOURCE *v13; // rax

  v3 = *((_QWORD *)a1 + 2);
  v4 = 0LL;
  if ( *(int *)(v3 + 1968) >= 0x2000 )
    v11 = *(_DWORD *)(v3 + 248);
  else
    v11 = 1;
  if ( v11 > 1 )
  {
    v12 = (KSPIN_LOCK *)operator new(0x18uLL, 0x4B677844u, 1, (enum _POOL_TYPE)512);
    v4 = (DXGSHAREDRESOURCE_NONPAGED *)v12;
    if ( v12 )
    {
      *v12 = 0LL;
      v12[1] = 0LL;
      KeInitializeSpinLock(v12 + 2);
    }
    else
    {
      v4 = 0LL;
    }
    if ( !v4 )
      return 0LL;
  }
  if ( a3 )
  {
    v13 = (DXGSHAREDRESOURCE *)operator new(0xE8uLL, 0x4B677844u, PagedPool);
    v9 = v13;
    if ( v13 )
    {
      DXGSHAREDRESOURCE::DXGSHAREDRESOURCE(v13, a1, a2);
      *((_DWORD *)v9 + 48) = 0;
      *((_DWORD *)v9 + 49) = 0;
      *((_DWORD *)v9 + 50) = 0;
      *((_QWORD *)v9 + 26) = 0LL;
      *((_QWORD *)v9 + 27) = 0LL;
      *((_DWORD *)v9 + 3) |= 0x20u;
      *(_QWORD *)v9 = &DXGSHAREDRESOURCECA::`vftable';
      goto LABEL_7;
    }
    goto LABEL_12;
  }
  v8 = (DXGSHAREDRESOURCE *)operator new(0xC0uLL, 0x4B677844u, PagedPool);
  if ( !v8 )
  {
LABEL_12:
    v9 = 0LL;
    goto LABEL_7;
  }
  v9 = DXGSHAREDRESOURCE::DXGSHAREDRESOURCE(v8, a1, a2);
LABEL_7:
  if ( v9 )
  {
    if ( v4 )
    {
      *((_QWORD *)v9 + 21) = v4;
      *(_QWORD *)v4 = v9;
    }
  }
  else if ( v4 )
  {
    DXGSHAREDRESOURCE_NONPAGED::`scalar deleting destructor'(v4);
  }
  return v9;
}
