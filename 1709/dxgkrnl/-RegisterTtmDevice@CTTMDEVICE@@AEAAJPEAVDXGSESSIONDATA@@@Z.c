/*
 * XREFs of ?RegisterTtmDevice@CTTMDEVICE@@AEAAJPEAVDXGSESSIONDATA@@@Z @ 0x1C01CAA98
 * Callers:
 *     ?SetConsoleSession@DXGSESSIONMGR@@QEAAXE@Z @ 0x1C0090314 (-SetConsoleSession@DXGSESSIONMGR@@QEAAXE@Z.c)
 *     ?CreateTtmDeviceWorker@DXGSESSIONMGR@@CAXPEAX@Z @ 0x1C01218F0 (-CreateTtmDeviceWorker@DXGSESSIONMGR@@CAXPEAX@Z.c)
 * Callees:
 *     ?GetGlobal@DXGGLOBAL@@SAPEAV1@XZ @ 0x1C0002A50 (-GetGlobal@DXGGLOBAL@@SAPEAV1@XZ.c)
 *     memset @ 0x1C0016A80 (memset.c)
 */

__int64 __fastcall CTTMDEVICE::RegisterTtmDevice(CTTMDEVICE *this, struct DXGSESSIONDATA *a2)
{
  __int64 v4; // rcx
  __int64 v5; // rcx
  __int64 v6; // rax
  char v7; // bl
  __int64 v8; // rdx
  __int64 v9; // rcx
  __int64 v10; // r8
  __int64 v11; // r9
  __int64 v12; // rax
  __int64 v13; // rbx
  int v14; // eax
  __int64 v15; // rdx
  __int64 v16; // r9
  int v17; // eax
  __int64 v18; // rcx
  _QWORD *v19; // rax
  struct DXGSESSIONDATA **v20; // rdx
  struct DXGSESSIONDATA *v21; // rax
  _QWORD v23[5]; // [rsp+30h] [rbp-28h] BYREF

  memset(v23, 0, 0x20uLL);
  if ( *(struct _KTHREAD **)(*((_QWORD *)DXGGLOBAL::GetGlobal(v4) + 73) + 160LL) != KeGetCurrentThread() )
  {
    v6 = WdLogNewEntry5_WdAssertion(v5);
    *(_QWORD *)(v6 + 24) = 4085LL;
    WdLogEvent5_WdAssertion(v6);
  }
  KeEnterCriticalRegion();
  ExAcquirePushLockExclusiveEx((char *)this + 888, 0LL);
  *((_QWORD *)this + 112) = KeGetCurrentThread();
  v7 = *((_BYTE *)this + 904);
  *((_QWORD *)this + 112) = 0LL;
  ExReleasePushLockExclusiveEx((char *)this + 888, 0LL);
  KeLeaveCriticalRegion();
  if ( v7 )
  {
    v12 = WdLogNewEntry5_WdEvent(v9, v8, v10, v11);
    LODWORD(v13) = -1073741130;
    *(_QWORD *)(v12 + 24) = this;
    *(_QWORD *)(v12 + 32) = -1073741130LL;
    WdLogEvent5_WdEvent(v12);
  }
  else
  {
    v14 = _InterlockedExchangeAdd((volatile signed __int32 *)this + 156, 1u);
    v15 = (unsigned int)(v14 >> 31);
    LODWORD(v15) = v14 % 32;
    *((_DWORD *)this + 2 * v15 + 157) = 0x40000000;
    v23[0] = CTTMDEVICE::CloseCallback;
    *((_DWORD *)this + 2 * (unsigned int)(v14 % 32) + 158) = MEMORY[0xFFFFF78000000008] / 0x2710uLL;
    v16 = *((unsigned int *)this + 21);
    v23[2] = &CTTMDEVICE::SetDisplayStateCallback;
    v17 = TtmNotifyDeviceArrival(1LL, this, v23, v16, (char *)this + 608);
    v13 = v17;
    if ( v17 >= 0 )
    {
      v20 = (struct DXGSESSIONDATA **)*((_QWORD *)a2 + 2323);
      v21 = (CTTMDEVICE *)((char *)this + 16);
      if ( *v20 != (struct DXGSESSIONDATA *)((char *)a2 + 18576) )
        __fastfail(3u);
      *(_QWORD *)v21 = (char *)a2 + 18576;
      *((_QWORD *)this + 3) = v20;
      *v20 = v21;
      *((_QWORD *)a2 + 2323) = v21;
      *((_QWORD *)this + 4) = a2;
    }
    else
    {
      v19 = (_QWORD *)WdLogNewEntry5_WdError(v18);
      v19[3] = *((unsigned int *)this + 20);
      v19[4] = *((_QWORD *)this + 8);
      v19[5] = v13;
      WdLogEvent5_WdError(v19);
    }
  }
  return (unsigned int)v13;
}
