/*
 * XREFs of DNG_StretchCol @ 0x1C000E220
 * Callers:
 *     RenderNineGridInternal @ 0x1C0010030 (RenderNineGridInternal.c)
 * Callees:
 *     ?DNG_CopyMemory@@YAHPEAU_DNGINTERNALDATA@@PEAX1J@Z @ 0x1C000E1B0 (-DNG_CopyMemory@@YAHPEAU_DNGINTERNALDATA@@PEAX1J@Z.c)
 *     DNG_DrawRow @ 0x1C0010DB0 (DNG_DrawRow.c)
 */

__int64 __fastcall DNG_StretchCol(struct _DNGINTERNALDATA *a1, _DWORD *a2)
{
  __int64 v2; // r15
  int v4; // r10d
  __int64 v5; // r12
  unsigned int v6; // r14d
  unsigned int v7; // ebp
  __int64 v8; // rdi
  __int64 v9; // r13
  __int64 result; // rax
  int v11; // edx
  __int64 v12; // rsi
  int v13; // [rsp+70h] [rbp+8h]
  int v14; // [rsp+78h] [rbp+10h]
  __int64 v15; // [rsp+80h] [rbp+18h]

  v4 = *((_DWORD *)a1 + 4);
  v5 = *((_QWORD *)a1 + 4);
  v6 = a2[1];
  v7 = 0;
  v15 = *((_QWORD *)a1 + 1);
  v2 = v15;
  v8 = v15 - 4LL * v4 * a2[6];
  v13 = a2[3];
  v9 = v15 - 4LL * v4 * a2[7];
  v14 = a2[2];
  result = (unsigned int)(*a2 * *((_DWORD *)a1 + 10));
  v11 = 1;
  v12 = v5 - 4LL * (int)result;
  if ( v8 != v9 )
  {
    do
    {
      if ( v11 )
      {
        *((_QWORD *)a1 + 1) = v8;
        *((_QWORD *)a1 + 4) = v12;
        DNG_DrawRow(a1);
      }
      else
      {
        result = DNG_CopyMemory(
                   a1,
                   (char *)(v8 + 4LL * *(int *)a1),
                   (char *)(v8 + 4 * (*(int *)a1 + (__int64)v4)),
                   4 * *((_DWORD *)a1 + 6));
        if ( !(_DWORD)result )
          break;
      }
      v11 = (v6 + v14 < v6) + v13;
      v7 += v11;
      if ( v7 < *((_DWORD *)a1 + 12) )
        v12 -= 4LL * v11 * *((_DWORD *)a1 + 10);
      v4 = *((_DWORD *)a1 + 4);
      v6 += v14;
      result = -(__int64)v4;
      v8 -= 4LL * v4;
    }
    while ( v8 != v9 );
    v2 = v15;
  }
  *((_QWORD *)a1 + 1) = v2;
  *((_QWORD *)a1 + 4) = v5;
  return result;
}
