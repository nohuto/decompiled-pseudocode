/*
 * XREFs of SfnOUTLPRECT @ 0x1C0059940
 * Callers:
 *     <none>
 * Callees:
 *     ??0ReleaseAndReacquirePerObjectLocks@@QEAA@XZ @ 0x1C005CEB0 (--0ReleaseAndReacquirePerObjectLocks@@QEAA@XZ.c)
 *     ??1ReleaseAndReacquirePerObjectLocks@@QEAA@XZ @ 0x1C005CF3C (--1ReleaseAndReacquirePerObjectLocks@@QEAA@XZ.c)
 *     ??0LeaveEnterCritProperDisposition@@QEAA@XZ @ 0x1C005CFBC (--0LeaveEnterCritProperDisposition@@QEAA@XZ.c)
 *     ??1LeaveEnterCritProperDisposition@@QEAA@XZ @ 0x1C005D018 (--1LeaveEnterCritProperDisposition@@QEAA@XZ.c)
 *     UserSetLastError @ 0x1C0064344 (UserSetLastError.c)
 *     W32GetThreadWin32Thread @ 0x1C00AA650 (W32GetThreadWin32Thread.c)
 *     memset @ 0x1C0145A00 (memset.c)
 */

__int64 __fastcall SfnOUTLPRECT(
        volatile signed __int32 *a1,
        int a2,
        __int64 a3,
        _OWORD *a4,
        __int64 a5,
        __int64 a6,
        char a7,
        __int64 a8)
{
  __int64 ThreadWin32Thread; // rax
  __int64 v13; // rsi
  __int64 v14; // rdi
  __int64 v15; // rax
  __int64 v16; // rax
  __int128 v17; // xmm6
  __int64 v18; // xmm7_8
  __int64 v19; // rcx
  __int64 v20; // rcx
  int v21; // ebx
  __int64 v22; // rdx
  __int64 v23; // rcx
  __int64 v24; // rcx
  __int64 *v25; // rcx
  __int64 v26; // rdx
  __int128 *v28; // rcx
  _QWORD v29[10]; // [rsp+48h] [rbp-F0h] BYREF
  _QWORD v30[3]; // [rsp+98h] [rbp-A0h] BYREF
  __int128 v31; // [rsp+B0h] [rbp-88h]
  __int64 v32; // [rsp+C0h] [rbp-78h]
  __int128 v33; // [rsp+C8h] [rbp-70h]
  char v34; // [rsp+140h] [rbp+8h] BYREF
  char v35; // [rsp+148h] [rbp+10h] BYREF
  int v36; // [rsp+150h] [rbp+18h] BYREF
  unsigned __int64 v37; // [rsp+158h] [rbp+20h] BYREF

  ThreadWin32Thread = W32GetThreadWin32Thread(KeGetCurrentThread());
  v13 = ThreadWin32Thread;
  if ( a1 )
    v14 = (__int64)a1 - *(_QWORD *)(ThreadWin32Thread + 448);
  else
    v14 = 0LL;
  memset(v29, 0, 0x28uLL);
  v29[0] = v14;
  LODWORD(v29[1]) = a2;
  v29[2] = a3;
  v29[3] = a5;
  v29[4] = a6;
  v15 = W32GetThreadWin32Thread(KeGetCurrentThread());
  v30[0] = *(_QWORD *)(v15 + 392);
  *(_QWORD *)(v15 + 392) = v30;
  v30[1] = a1;
  if ( a1 )
    _InterlockedIncrement(a1 + 2);
  v16 = *(_QWORD *)(v13 + 456);
  v17 = *(_OWORD *)(v16 + 64);
  v31 = v17;
  v18 = *(_QWORD *)(v16 + 80);
  v32 = v18;
  *(_QWORD *)(v16 + 72) = v14;
  if ( a1 )
    v19 = *(_QWORD *)a1;
  else
    v19 = 0LL;
  *(_QWORD *)(*(_QWORD *)(v13 + 456) + 64LL) = v19;
  if ( a1 )
    v20 = *((_QWORD *)a1 + 35);
  else
    v20 = 0LL;
  *(_QWORD *)(*(_QWORD *)(v13 + 456) + 80LL) = v20;
  if ( gdwInAtomicOperation && (gdwExtraInstrumentations & 1) != 0 )
    KeBugCheckEx(0x160u, gdwInAtomicOperation, 0LL, 0LL, 0LL);
  ReleaseAndReacquirePerObjectLocks::ReleaseAndReacquirePerObjectLocks((ReleaseAndReacquirePerObjectLocks *)&v34);
  LeaveEnterCritProperDisposition::LeaveEnterCritProperDisposition((LeaveEnterCritProperDisposition *)&v35);
  EtwTraceBeginCallback(34LL);
  v21 = KeUserModeCallback(34LL, v29, 40LL, &v37, &v36);
  EtwTraceEndCallback(34LL);
  LeaveEnterCritProperDisposition::~LeaveEnterCritProperDisposition((LeaveEnterCritProperDisposition *)&v35);
  ReleaseAndReacquirePerObjectLocks::~ReleaseAndReacquirePerObjectLocks((ReleaseAndReacquirePerObjectLocks *)&v34);
  ThreadUnlock1(v23, v22);
  v24 = *(_QWORD *)(v13 + 456);
  *(_OWORD *)(v24 + 64) = v17;
  *(_QWORD *)(v24 + 80) = v18;
  if ( v21 < 0 || v36 != 24 )
    return 0LL;
  v25 = (__int64 *)v37;
  if ( v37 + 8 < v37 || v37 + 8 > MmUserProbeAddress )
    v25 = (__int64 *)MmUserProbeAddress;
  v26 = *v25;
  v29[5] = *v25;
  if ( (!a8 || (*(_DWORD *)(a8 + 84) & 5) == 0) && (a7 & 2) == 0 )
  {
    v28 = *(__int128 **)(v37 + 16);
    if ( v28 + 1 < v28 || (unsigned __int64)(v28 + 1) > MmUserProbeAddress )
      v28 = (__int128 *)MmUserProbeAddress;
    v33 = *v28;
    *a4 = v33;
  }
  return v26;
}
