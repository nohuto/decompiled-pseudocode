/*
 * XREFs of ?xxxTabletSetDisplayOrientation@@YAXJPEAU_devicemodeW@@@Z @ 0x1C01BA3D4
 * Callers:
...
void __fastcall xxxTabletSetDisplayOrientation(DWORD a1, struct _devicemodeW *a2)
{
  ...
  _QWORD v9[10]; // [rsp+40h] [rbp-148h] BYREF
  struct _devicemodeW v10; // [rsp+90h] [rbp-F8h] BYREF
  ...
  if ( (dmFields & 0x80u) != 0 )
  {
    ...
    if ( v4 >= 0 )
    {
      ...
      {
        while ( (unsigned int)xxxUserChangeDisplaySettings(0LL, a2, 0LL, 9LL, 0LL, 0, v9) == -2 )
        {
