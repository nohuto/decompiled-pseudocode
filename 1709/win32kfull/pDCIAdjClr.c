/*
 * XREFs of pDCIAdjClr @ 0x1C007828C
 * Callers:
 *     HT_CreateStandardMonoPattern @ 0x1C0042140 (HT_CreateStandardMonoPattern.c)
 *     HT_HalftoneBitmap @ 0x1C007A0A0 (HT_HalftoneBitmap.c)
 *     HT_CreateHalftoneBrush @ 0x1C023E370 (HT_CreateHalftoneBrush.c)
 * Callees:
 *     AdjustSrcDevGamma @ 0x1C0079298 (AdjustSrcDevGamma.c)
 *     DivFD6 @ 0x1C00794D8 (DivFD6.c)
 *     RaisePower @ 0x1C007AEC8 (RaisePower.c)
 *     Log @ 0x1C007B120 (Log.c)
 *     ComputeColorSpaceXForm @ 0x1C010DE1C (ComputeColorSpaceXForm.c)
 *     CompareMemory @ 0x1C0117C80 (CompareMemory.c)
 *     __security_check_cookie @ 0x1C0143530 (__security_check_cookie.c)
 *     GenCMYMaskXlate @ 0x1C023F5B8 (GenCMYMaskXlate.c)
 *     TintAngle @ 0x1C023F7D8 (TintAngle.c)
 */

__int64 __fastcall pDCIAdjClr(__int64 a1, __m128i *a2, _QWORD *a3, unsigned int a4, __int16 a5, int a6, _DWORD *a7)
{
  __int64 v11; // rdi
  int v12; // r12d
  __int16 v13; // dx
  __int16 v14; // r8
  __int16 v15; // r14
  int v16; // ebx
  __int64 v17; // xmm1_8
  __int16 v18; // cx
  __int8 v19; // r8d^2
  __int64 v20; // xmm1_8
  __int128 v21; // xmm0
  __int128 v22; // xmm1
  __int128 v23; // xmm0
  __int128 v24; // xmm1
  __int128 v25; // xmm0
  __int128 v26; // xmm1
  __int128 v27; // xmm0
  __int128 v28; // xmm1
  __int128 v29; // xmm0
  __int128 v30; // xmm1
  __int128 v31; // xmm0
  __int128 v32; // xmm1
  __int128 v33; // xmm0
  __int128 v34; // xmm1
  int v35; // ecx
  __int16 v36; // cx
  int v37; // r9d
  __int16 v38; // cx
  char v39; // si
  int v40; // eax
  __int64 v41; // rdx
  __int64 v42; // r9
  __int64 v43; // r8
  __int8 v44; // cl
  char v45; // al
  unsigned int v46; // ebx
  __m128i v47; // xmm2
  int v48; // ebx
  int v49; // eax
  __int64 v50; // xmm1_8
  int v51; // edx
  __int128 v52; // xmm0
  __int128 v53; // xmm1
  __int128 v54; // xmm0
  __int128 v55; // xmm1
  __int128 v56; // xmm0
  __int128 v57; // xmm1
  __int128 v58; // xmm0
  __int128 v59; // xmm1
  __int128 v60; // xmm1
  __int128 v61; // xmm0
  __int128 v62; // xmm1
  __int128 v63; // xmm0
  __int128 v64; // xmm1
  int v65; // eax
  int v67; // ebx
  char v68; // r15
  unsigned __int64 v69; // r15
  unsigned __int16 epi16; // ax
  unsigned int v71; // ebx
  unsigned int v72; // r12d
  int v73; // r15d
  unsigned int v74; // ecx
  __int64 v75; // xmm1_8
  __int128 v76; // xmm0
  __int128 v77; // xmm0
  __int128 v78; // xmm1
  __int128 v79; // xmm0
  __int128 v80; // xmm1
  __int128 v81; // xmm0
  __int128 v82; // xmm1
  __int128 v83; // xmm0
  __int128 v84; // xmm1
  __int128 v85; // xmm0
  __int128 v86; // xmm1
  __int128 v87; // xmm0
  __int128 v88; // xmm1
  int v89; // eax
  __int64 v90; // r8
  __int64 v91; // r9
  __int64 v92; // r8
  __int64 v93; // r9
  unsigned int *v94; // r8
  unsigned int i; // ecx
  unsigned int v96; // eax
  int v97; // ebx
  char v98; // dl
  char v99; // cl
  char v100; // r8
  unsigned __int8 v101; // r9
  __int64 v102; // r10
  char *v103; // rcx
  char v104; // al
  int v105; // eax
  char v106; // al
  __int16 v107; // [rsp+30h] [rbp-D0h]
  int v108; // [rsp+30h] [rbp-D0h]
  __int128 v109; // [rsp+38h] [rbp-C8h]
  __int128 v110; // [rsp+48h] [rbp-B8h]
  char *v111; // [rsp+58h] [rbp-A8h]
  __int64 v112; // [rsp+60h] [rbp-A0h]
  __int128 v113; // [rsp+70h] [rbp-90h] BYREF
  __int128 v114; // [rsp+80h] [rbp-80h]
  __int128 v115; // [rsp+90h] [rbp-70h] BYREF
  __m256i v116; // [rsp+A0h] [rbp-60h] BYREF
  __int128 v117; // [rsp+C0h] [rbp-40h]
  __int128 v118; // [rsp+D0h] [rbp-30h]
  __int128 v119; // [rsp+E0h] [rbp-20h]
  __int128 v120; // [rsp+F0h] [rbp-10h]
  __int128 v121; // [rsp+100h] [rbp+0h] BYREF
  __int128 v122; // [rsp+110h] [rbp+10h]
  __int128 v123; // [rsp+120h] [rbp+20h]
  __int128 v124; // [rsp+130h] [rbp+30h]
  __int128 v125; // [rsp+140h] [rbp+40h]
  int v126; // [rsp+150h] [rbp+50h]
  __int128 v127; // [rsp+160h] [rbp+60h] BYREF
  __int64 v128; // [rsp+170h] [rbp+70h]
  __int128 v129; // [rsp+178h] [rbp+78h]
  __int64 v130; // [rsp+188h] [rbp+88h]

  if ( a1 && (v11 = a1 + 40, *(_DWORD *)(a1 + 40) == 1414021956) )
  {
    EngAcquireSemaphore(*(HSEMAPHORE *)(a1 + 48));
    if ( !a3 )
      return v11;
    if ( (a5 & 0x200) != 0 && dwABPreMul )
    {
      dwABPreMul = 0;
      v94 = (unsigned int *)&unk_1C03243B4;
      for ( i = 1; i < 0x100; ++i )
      {
        v96 = (i - 16777217) / i;
        *v94++ = v96;
      }
    }
    if ( a4 + 312 >= a4 )
    {
      v111 = (char *)EngAllocMem(1u, a4 + 312, 0x30355448u);
      *a3 = v111;
      if ( v111 )
      {
        v12 = *(_DWORD *)(v11 + 184);
        v13 = a5 | 8;
        DWORD1(v109) = a6;
        v107 = v12;
        if ( (v12 & 0x80u) == 0 )
          v13 = a5;
        if ( HIBYTE(a6) == 1 || (v14 = v13, HIBYTE(a6) == 0xFE) && (v14 = v13, (v12 & 0x300) == 0x300) )
          v14 = v13 | 0x41;
        v15 = v14 & 0xFFEF;
        v112 = (v14 & 0x10) != 0 ? 3956LL : 856LL;
        v16 = 24;
        if ( (v14 & 8) == 0 )
          v15 = v14;
        if ( !a2 )
          a2 = (__m128i *)(a1 + 12);
        v17 = a2[1].m128i_i64[0];
        v18 = _mm_cvtsi128_si32(*a2);
        v127 = (__int128)*a2;
        v128 = v17;
        if ( v18 != 24 || (v19 = BYTE2(v127), (WORD1(v127) & 0xFFFC) != 0) )
        {
          v127 = DefaultCA;
          v19 = BYTE2(DefaultCA);
          v128 = 0LL;
        }
        v20 = *(_QWORD *)(v11 + 212);
        v129 = *(_OWORD *)(v11 + 196);
        v21 = *(_OWORD *)(v11 + 220);
        v130 = v20;
        v22 = *(_OWORD *)(v11 + 236);
        WORD1(v127) = v19 & 3;
        v113 = v21;
        v23 = *(_OWORD *)(v11 + 252);
        v114 = v22;
        v24 = *(_OWORD *)(v11 + 268);
        v115 = v23;
        v25 = *(_OWORD *)(v11 + 284);
        *(_OWORD *)v116.m256i_i8 = v24;
        v26 = *(_OWORD *)(v11 + 300);
        *(_OWORD *)&v116.m256i_u64[2] = v25;
        v27 = *(_OWORD *)(v11 + 316);
        v117 = v26;
        v28 = *(_OWORD *)(v11 + 332);
        v118 = v27;
        v29 = *(_OWORD *)(v11 + 348);
        v119 = v28;
        v30 = *(_OWORD *)(v11 + 364);
        v120 = v29;
        v31 = *(_OWORD *)(v11 + 380);
        v121 = v30;
        v32 = *(_OWORD *)(v11 + 396);
        v122 = v31;
        v33 = *(_OWORD *)(v11 + 412);
        v123 = v32;
        v34 = *(_OWORD *)(v11 + 428);
        v35 = *(_DWORD *)(v11 + 444);
        v124 = v33;
        v125 = v34;
        v126 = v35;
        if ( WORD2(v127) > 8u )
          WORD2(v127) = 0;
        if ( WORD3(v127) < 0x540u )
        {
          WORD3(v127) = 1344;
        }
        else if ( WORD3(v127) > 0xFDE8u )
        {
          WORD3(v127) = -536;
        }
        if ( WORD4(v127) < 0x540u )
        {
          WORD4(v127) = 1344;
        }
        else if ( WORD4(v127) > 0xFDE8u )
        {
          WORD4(v127) = -536;
        }
        if ( WORD5(v127) < 0x540u )
        {
          WORD5(v127) = 1344;
        }
        else if ( WORD5(v127) > 0xFDE8u )
        {
          WORD5(v127) = -536;
        }
        if ( WORD6(v127) > 0xFA0u )
          WORD6(v127) = 4000;
        v36 = 6000;
        if ( HIWORD(v127) < 0x1770u || (v36 = 10000, HIWORD(v127) > 0x2710u) )
          HIWORD(v127) = v36;
        v37 = 100;
        if ( (__int16)v128 < -100 )
        {
          LOWORD(v128) = -100;
        }
        else if ( (__int16)v128 > 100 )
        {
          LOWORD(v128) = 100;
        }
        if ( SWORD1(v128) < -100 )
        {
          WORD1(v128) = -100;
        }
        else if ( SWORD1(v128) > 100 )
        {
          WORD1(v128) = 100;
        }
        v38 = WORD2(v128);
        if ( SWORD2(v128) < -100 )
        {
          v38 = -100;
          WORD2(v128) = -100;
        }
        else if ( SWORD2(v128) > 100 )
        {
          v38 = 100;
          WORD2(v128) = 100;
        }
        if ( SHIWORD(v128) < -100 )
        {
          HIWORD(v128) = -100;
        }
        else if ( SHIWORD(v128) > 100 )
        {
          HIWORD(v128) = 100;
        }
        if ( (v15 & 1) != 0 || v38 == -100 )
          HIDWORD(v128) = 65436;
        v39 = 2;
        if ( (v15 & 2) != 0 )
          WORD1(v127) = v19 & 2 | 1;
        LOBYTE(v37) = HIBYTE(a6);
        LOWORD(v127) = v15 & 0x8059;
        v40 = AdjustSrcDevGamma(v11, (unsigned int)&v113, (unsigned int)&v127, v37, v15);
        v43 = 1024LL;
        if ( !v40 )
        {
          v41 = 0LL;
          while ( 1 )
          {
            v44 = *((_BYTE *)&v127 + v41);
            --v16;
            v45 = *((_BYTE *)&v129 + v41++);
            if ( v44 != v45 )
              break;
            if ( !v16 )
            {
              v46 = v113;
              v47 = (__m128i)v127;
              goto LABEL_53;
            }
          }
        }
        v67 = v113 & 0x1007;
        LODWORD(v113) = v67;
        if ( (v15 & 0x40) != 0 )
        {
          v67 |= 0x2000u;
          LODWORD(v113) = v67;
        }
        v47 = (__m128i)v127;
        v68 = _mm_cvtsi128_si32(_mm_srli_si128((__m128i)v127, 2));
        if ( (v68 & 2) != 0 )
        {
          if ( !LogFilterMax )
            LogFilterMax = Log(8000000LL, v41, 1024LL);
          v67 |= 0x100u;
          LODWORD(v113) = v67;
        }
        if ( (v68 & 1) != 0 )
        {
          v67 |= 0x200u;
          LODWORD(v113) = v67;
        }
        v69 = (-(__int64)((v67 & 1) != 0) & 0xFFFFFFFFFFFFFFDCuLL) + v11 + 72;
        if ( (((unsigned __int16)_mm_cvtsi128_si32(v47) ^ (unsigned __int16)v129) & 0x8008) != 0 )
        {
          ComputeColorSpaceXForm(
            v11,
            (-(__int64)((v67 & 1) != 0) & 0xFFFFFFFFFFFFFFDCuLL) + v11 + 72,
            (char *)&v121 + 4,
            0xFFFFFFFFLL);
          v47 = (__m128i)v127;
          v67 = v113;
        }
        epi16 = _mm_extract_epi16(v47, 2);
        if ( epi16 != WORD2(v129) )
        {
          ComputeColorSpaceXForm(v11, v11 + 36, (char *)&v116.m256i_u64[2] + 4, epi16);
          v47 = (__m128i)v127;
          v67 = v113;
        }
        if ( (v67 & 0x2000) != 0 || (unsigned int)CompareMemory(v69, v11 + 36, 36LL) )
          v71 = v67 & 0xFFFF7FFF;
        else
          v71 = v67 | 0x8000;
        v72 = 100 * (unsigned __int16)_mm_extract_epi16(v47, 6);
        v73 = 100 * (unsigned __int16)_mm_extract_epi16(v47, 7);
        v116.m256i_i64[0] = __PAIR64__(v73, v72);
        if ( v72 || v73 != 1000000 )
        {
          v46 = v71 | 8;
          LODWORD(v113) = v46;
          v116.m256i_i32[2] = DivFD6(0LL, v72, v43, v42);
          v116.m256i_i32[3] = DivFD6(0LL, (unsigned int)(1000000 - v73), v90, v91);
          v116.m256i_i32[4] = DivFD6(1000000LL, v73 - v72, v92, v93);
        }
        else
        {
          v46 = v71 & 0xFFFFFFF7;
          *(__int64 *)((char *)v116.m256i_i64 + 4) = 1000000LL;
          LODWORD(v113) = v46;
          v116.m256i_i32[0] = 0;
          *(__int64 *)((char *)&v116.m256i_i64[1] + 4) = 0LL;
        }
        if ( (_WORD)v128 )
        {
          v46 |= 0x10u;
          HIDWORD(v114) = RaisePower(1015000LL, (unsigned int)(__int16)v128, 2LL);
          LODWORD(v113) = v46;
        }
        if ( WORD1(v128) )
        {
          v46 |= 0x20u;
          LODWORD(v113) = v46;
          LODWORD(v115) = 3750 * SWORD1(v128);
        }
        if ( (v46 & 0x2000) != 0 )
        {
          LOWORD(v12) = v107;
        }
        else
        {
          DWORD1(v115) = 10000 * (SWORD2(v128) + 100);
          if ( DWORD1(v115) != 1000000 )
          {
            v46 |= 0x40u;
            LODWORD(v113) = v46;
          }
          if ( HIWORD(v128) )
          {
            TintAngle((unsigned int)SHIWORD(v128), v41, (char *)&v115 + 8, (char *)&v115 + 12);
            v46 = v113 | 0x80;
            LODWORD(v113) = v113 | 0x80;
          }
          LOWORD(v12) = v107;
          v74 = v46;
          if ( (v107 & 4) != 0 && (v46 & 0x1001) == 0x1000 )
          {
            v46 |= 0x400u;
            LODWORD(v113) = v46;
            if ( (v107 & 1) != 0 )
            {
              v46 = v74 | 0xC00;
              LODWORD(v113) = v74 | 0xC00;
            }
          }
        }
        v75 = v128;
        v76 = v113;
        *(__m128i *)(v11 + 196) = v47;
        *(_OWORD *)(v11 + 220) = v76;
        v77 = v115;
        *(_QWORD *)(v11 + 212) = v75;
        *(_OWORD *)(v11 + 236) = v114;
        v78 = *(_OWORD *)v116.m256i_i8;
        *(_OWORD *)(v11 + 252) = v77;
        v79 = *(_OWORD *)&v116.m256i_u64[2];
        *(_OWORD *)(v11 + 268) = v78;
        v80 = v117;
        *(_OWORD *)(v11 + 284) = v79;
        v81 = v118;
        *(_OWORD *)(v11 + 300) = v80;
        v82 = v119;
        *(_OWORD *)(v11 + 316) = v81;
        v83 = v120;
        *(_OWORD *)(v11 + 332) = v82;
        v84 = v121;
        *(_OWORD *)(v11 + 348) = v83;
        v85 = v122;
        *(_OWORD *)(v11 + 364) = v84;
        v86 = v123;
        *(_OWORD *)(v11 + 380) = v85;
        v87 = v124;
        *(_OWORD *)(v11 + 396) = v86;
        v88 = v125;
        v89 = v126;
        *(_OWORD *)(v11 + 412) = v87;
        *(_OWORD *)(v11 + 428) = v88;
        *(_DWORD *)(v11 + 444) = v89;
LABEL_53:
        v48 = v46 & 0x3DB4BFFF;
        LODWORD(v113) = v48;
        if ( (v48 & 0x97FE) == 0 )
        {
          v48 |= 0x80000000;
          LODWORD(v113) = v48;
        }
        if ( (v48 & 0x84C0) == 0 || (v48 & 0x2000) != 0 )
        {
          v48 |= 0x40000000u;
          LODWORD(v113) = v48;
        }
        if ( (v15 & 0x20) != 0 || (v12 & 0x1000) != 0 )
        {
          v48 |= 0x80000u;
          LODWORD(v113) = v48;
        }
        if ( (v15 & 0x80u) == 0 )
        {
LABEL_60:
          if ( (v15 & 4) != 0 )
          {
            v48 |= 0x4000u;
            LODWORD(v113) = v48;
          }
          HIDWORD(v110) = 1000000;
          v129 = 0uLL;
          v130 = 0LL;
          LODWORD(v109) = 67372032;
          BYTE4(v109) = 3;
          if ( (v48 & 0x2000) != 0 )
          {
            v48 |= 0x40000u;
            LOBYTE(v109) = 1;
            LODWORD(v113) = v48;
            v49 = 0xFFFF;
            DWORD1(v110) = 0;
          }
          else
          {
            DWORD1(v110) = 4096;
            v49 = 4095;
          }
          DWORD2(v109) = v49;
          HIDWORD(v109) = v49;
          LODWORD(v110) = v49;
          switch ( HIBYTE(a6) )
          {
            case 1u:
              BYTE6(v109) = 0;
              goto LABEL_68;
            case 2u:
LABEL_138:
              *(_QWORD *)&v129 = 0x20000000100000LL;
              BYTE3(v109) = 6;
              *((_QWORD *)&v129 + 1) = 0x1000000400000LL;
              v130 = 0x4000000020000LL;
              *(_WORD *)((char *)&v109 + 1) = 1284;
              DWORD1(v110) = 0;
              goto LABEL_68;
            case 5u:
            case 6u:
              WORD1(v109) = 0;
              *(_QWORD *)&v110 = 255LL;
              BYTE1(v109) = 0;
              *((_QWORD *)&v109 + 1) = 0xFF000000FFLL;
              *(_QWORD *)&v129 = 0x100000000LL;
              DWORD2(v129) = 2;
              goto LABEL_68;
            case 0xFCu:
              if ( BYTE2(a6) )
              {
                v106 = BYTE2(a6);
                if ( BYTE2(a6) != 4 )
                  v106 = 0;
                BYTE6(v109) = v106;
              }
              LODWORD(v110) = 126975;
              *((_QWORD *)&v109 + 1) = 0x3EFFF0001EFFFLL;
              v130 = -1LL;
              *(_QWORD *)&v129 = 0x7E00000001F0000LL;
              *((_QWORD *)&v129 + 1) = -134217728LL;
              *(_WORD *)((char *)&v109 + 1) = 2308;
              BYTE3(v109) = 15;
              goto LABEL_68;
            case 0xFDu:
              *(_QWORD *)&v129 = 0x3E00000001F0000LL;
              LODWORD(v110) = 126975;
              *((_QWORD *)&v109 + 1) = 0x1EFFF0001EFFFLL;
              v130 = 0x7FFF7FFF7FFF7FFFLL;
              *((_QWORD *)&v129 + 1) = 0x7FFF7FFF7C000000LL;
              *(_WORD *)((char *)&v109 + 1) = 2308;
              BYTE3(v109) = 14;
              goto LABEL_68;
          }
          if ( HIBYTE(a6) != 254 )
          {
            if ( HIBYTE(a6) == 255 )
            {
              BYTE6(v109) = 4;
              LODWORD(v113) = v48 | 0x4000;
              goto LABEL_138;
            }
LABEL_68:
            DWORD2(v110) = DstOrderTable[BYTE6(v109)];
            *(_OWORD *)(v111 + 24) = v109;
            *(_OWORD *)(v111 + 40) = v110;
            v50 = v128;
            *(__m128i *)v111 = v47;
            *((_QWORD *)v111 + 2) = v50;
            *(_DWORD *)(v112 + v11 + 12) = *((_DWORD *)&v129 + BYTE9(v110));
            *(_DWORD *)(v112 + v11 + 8) = *((_DWORD *)&v129 + BYTE10(v110));
            *(_DWORD *)(v112 + v11 + 4) = *((_DWORD *)&v129 + BYTE11(v110));
            *(_DWORD *)(v112 + v11 + 24) = *((_DWORD *)&v129 + (unsigned int)BYTE9(v110) + 3);
            v51 = *((_DWORD *)&v129 + (unsigned int)BYTE10(v110) + 3);
            v52 = v113;
            *(_DWORD *)(v112 + v11 + 16) = *((_DWORD *)&v129 + (unsigned int)(HIBYTE(DWORD2(v110)) + 3));
            v53 = v114;
            *(_DWORD *)(v112 + v11 + 20) = v51;
            *(_OWORD *)(v111 + 56) = v52;
            v54 = v115;
            *(_OWORD *)(v111 + 72) = v53;
            v55 = *(_OWORD *)v116.m256i_i8;
            *(_OWORD *)(v111 + 88) = v54;
            v56 = *(_OWORD *)&v116.m256i_u64[2];
            *(_OWORD *)(v111 + 104) = v55;
            v57 = v117;
            *(_OWORD *)(v111 + 120) = v56;
            v58 = v118;
            *(_OWORD *)(v111 + 136) = v57;
            v59 = v119;
            *(_OWORD *)(v111 + 152) = v58;
            *(_OWORD *)(v111 + 168) = v59;
            v60 = v121;
            *(_OWORD *)(v111 + 184) = v120;
            v61 = v122;
            *(_OWORD *)(v111 + 200) = v60;
            v62 = v123;
            *(_OWORD *)(v111 + 216) = v61;
            v63 = v124;
            *(_OWORD *)(v111 + 232) = v62;
            v64 = v125;
            v65 = v126;
            *(_OWORD *)(v111 + 248) = v63;
            *(_OWORD *)(v111 + 264) = v64;
            *((_DWORD *)v111 + 70) = v65;
            *((_QWORD *)v111 + 36) = v11 + 32;
            *((_QWORD *)v111 + 37) = v11 + 808;
            *((_QWORD *)v111 + 38) = v11 + 832;
            return v11;
          }
          if ( (v12 & 0x100) == 0 )
          {
            *(_QWORD *)&v129 = 0x38000000070000LL;
            LODWORD(v110) = 20479;
            *((_QWORD *)&v109 + 1) = 0x4FFF00004FFFLL;
            BYTE3(v109) = 10;
            LODWORD(v113) = v48 & 0xFFFEFFFF;
            DWORD2(v129) = 29360128;
            *(_WORD *)((char *)&v109 + 1) = 1796;
            BYTE6(v109) = 4;
            goto LABEL_68;
          }
          v97 = v48 | 0x10000;
          LOBYTE(v108) = 0;
          LODWORD(v113) = v97;
          if ( *(_BYTE *)(v11 + 804) )
          {
            GenCMYMaskXlate(
              v11 + 548,
              v12 & 0x2000,
              *(unsigned __int8 *)(v11 + 448),
              *(unsigned __int8 *)(v11 + 449),
              *(unsigned __int8 *)(v11 + 450));
            *(_BYTE *)(v11 + 804) = 0;
          }
          if ( (v12 & 0x2000) != 0 )
          {
            LOWORD(v108) = -254;
          }
          else
          {
            v39 = 0;
            BYTE1(v108) = 0;
          }
          HIBYTE(v108) = -((v12 & 0x2000) != 0);
          BYTE2(v108) = HIBYTE(v108) ^ *(_BYTE *)(v11 + 452);
          if ( (v12 & 0x200) != 0 )
          {
            WORD3(v109) = -1280;
            *(_QWORD *)((char *)&v129 + 4) = 0xFF000000FF0000LL;
            LODWORD(v129) = 16711680;
LABEL_163:
            HIDWORD(v130) = v108;
            LODWORD(v130) = v108;
            HIDWORD(v129) = v108;
            goto LABEL_68;
          }
          v98 = 10;
          v99 = *(_BYTE *)(v11 + 453);
          v100 = 7;
          *(_WORD *)((char *)&v109 + 1) = 1796;
          DWORD2(v109) = (*(unsigned __int8 *)(v11 + 450) << 12) - 1;
          BYTE3(v109) = 10;
          HIDWORD(v109) = (*(unsigned __int8 *)(v11 + 449) << 12) - 1;
          LODWORD(v110) = (*(unsigned __int8 *)(v11 + 448) << 12) - 1;
          LODWORD(v129) = *(unsigned __int16 *)(v11 + 458);
          DWORD1(v129) = *(unsigned __int16 *)(v11 + 456);
          DWORD2(v129) = *(unsigned __int16 *)(v11 + 454);
          if ( (unsigned __int8)(v99 - 4) > 1u )
          {
            WORD1(v109) = 2310;
            v101 = -6;
            BYTE7(v109) = -6;
            v100 = 6;
            v98 = 9;
            if ( (v12 & 0x2000) == 0 )
            {
LABEL_155:
              v105 = *(_DWORD *)(v11 + 472);
              if ( v105 )
              {
                LODWORD(v113) = v97 | 0x20000;
                HIDWORD(v110) = v105;
                BYTE1(v109) = 0;
                BYTE2(v109) = v100 - 4;
                BYTE3(v109) = v98 - 4;
                switch ( v101 )
                {
                  case 0xF8u:
                    BYTE7(v109) = -11;
                    break;
                  case 0xF9u:
                    BYTE7(v109) = -10;
                    break;
                  case 0xFAu:
                    BYTE7(v109) = -9;
                    break;
                }
              }
              BYTE6(v109) = 0;
              goto LABEL_163;
            }
            v97 |= 0x2000000u;
            BYTE2(v108) = *(_BYTE *)(v11 + 803);
            v104 = *(_BYTE *)(v11 + 548);
          }
          else
          {
            if ( v99 == 4 )
            {
              v101 = -7;
              v102 = 292LL;
            }
            else
            {
              v101 = -8;
              v39 |= 1u;
              v102 = 365LL;
            }
            v97 |= 0x400000u;
            BYTE7(v109) = v101;
            LOBYTE(v108) = v39 & 3;
            v103 = (char *)*(&p8BPPXlate + (v39 & 3));
            BYTE2(v108) = v103[v102];
            v104 = *v103;
          }
          HIBYTE(v108) = v104;
          LODWORD(v113) = v97;
          goto LABEL_155;
        }
        v48 |= 0x100000u;
        LODWORD(v113) = v48;
        if ( (v15 & 0x100) != 0 )
        {
          v48 |= 0x200000u;
        }
        else
        {
          if ( (v15 & 0x200) != 0 )
          {
            v48 |= 0x800000u;
            LODWORD(v113) = v48;
          }
          if ( (v15 & 0x400) == 0 )
            goto LABEL_60;
          v48 |= 0x1000000u;
        }
        LODWORD(v113) = v48;
        goto LABEL_60;
      }
    }
    *a7 = -2;
    EngReleaseSemaphore(*(HSEMAPHORE *)(v11 + 8));
  }
  else
  {
    *a7 = -12;
  }
  return 0LL;
}
