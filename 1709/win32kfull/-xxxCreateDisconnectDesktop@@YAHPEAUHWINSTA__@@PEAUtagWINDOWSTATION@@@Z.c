/*
 * XREFs of ?xxxCreateDisconnectDesktop@@YAHPEAUHWINSTA__@@PEAUtagWINDOWSTATION@@@Z @ 0x1C00D7E24
 * Callers:
 *     xxxCreateDesktopEx @ 0x1C00D9EA0 (xxxCreateDesktopEx.c)
 * Callees:
 *     _CloseDesktop @ 0x1C0048EAC (_CloseDesktop.c)
 *     ?SetDisconnectDesktopSecurity@@YAJPEAUHDESK__@@@Z @ 0x1C00D7FB0 (-SetDisconnectDesktopSecurity@@YAJPEAUHDESK__@@@Z.c)
 *     xxxCreateDesktopEx @ 0x1C00D9EA0 (xxxCreateDesktopEx.c)
 */

__int64 __fastcall xxxCreateDisconnectDesktop(HWINSTA a1, struct tagWINDOWSTATION *a2, __int64 a3)
{
  __int64 EmptyRgnPublic; // rbx
  NTSTATUS v6; // eax
  NTSTATUS v7; // edi
  struct _UNICODE_STRING DestinationString; // [rsp+40h] [rbp-40h] BYREF
  int v10; // [rsp+50h] [rbp-30h] BYREF
  HWINSTA v11; // [rsp+58h] [rbp-28h]
  struct _UNICODE_STRING *p_DestinationString; // [rsp+60h] [rbp-20h]
  int v13; // [rsp+68h] [rbp-18h]
  __int128 v14; // [rsp+70h] [rbp-10h]
  HANDLE Handle; // [rsp+B0h] [rbp+30h] BYREF
  PVOID Object; // [rsp+B8h] [rbp+38h] BYREF

  EmptyRgnPublic = CreateEmptyRgnPublic(a1, a2, a3);
  if ( EmptyRgnPublic )
  {
    RtlInitUnicodeString(&DestinationString, L"Disconnect");
    p_DestinationString = &DestinationString;
    v10 = 48;
    v11 = a1;
    v13 = 192;
    v14 = 0LL;
    if ( (int)xxxCreateDesktopEx((unsigned int)&v10, 0, 0x2000000, 0, (__int64)&Handle, 1) < 0 )
    {
      GreDeleteObject(EmptyRgnPublic);
    }
    else if ( (int)SetDisconnectDesktopSecurity((HDESK)Handle) < 0
           || (v6 = ObReferenceObjectByHandle(Handle, 0, (POBJECT_TYPE)ExDesktopObjectType, 1, &Object, 0LL),
               gspdeskDisconnect = Object,
               v6 < 0) )
    {
      GreDeleteObject(EmptyRgnPublic);
      CloseDesktop((unsigned __int64)Handle, 1);
      gspdeskDisconnect = 0LL;
    }
    else
    {
      *(_QWORD *)(*(_QWORD *)(*((_QWORD *)Object + 1) + 16LL) + 216LL) = EmptyRgnPublic;
      KeAttachProcess(gpepCSRSS);
      v7 = ObOpenObjectByPointer(gspdeskDisconnect, 0x200u, 0LL, 0x1F0003u, 0LL, 0, &ghDisconnectDesk);
      if ( v7 >= 0 )
        v7 = ObOpenObjectByPointer(a2, 0, 0LL, 0x1F0003u, 0LL, 0, &ghDisconnectWinSta);
      KeDetachProcess();
      if ( v7 >= 0 )
        return 1LL;
      GreDeleteObject(EmptyRgnPublic);
      *(_QWORD *)(*(_QWORD *)(*((_QWORD *)gspdeskDisconnect + 1) + 16LL) + 216LL) = 0LL;
      if ( ghDisconnectDesk )
      {
        ObCloseHandle(ghDisconnectDesk, 0);
        ghDisconnectDesk = 0LL;
      }
      CloseDesktop((unsigned __int64)Handle, 1);
    }
  }
  return 0LL;
}
