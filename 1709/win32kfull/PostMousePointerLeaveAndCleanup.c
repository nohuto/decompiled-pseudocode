/*
 * XREFs of PostMousePointerLeaveAndCleanup @ 0x1C01C0BE0
 * Callers:
 *     xxxProcessEventMessage @ 0x1C00CFAB0 (xxxProcessEventMessage.c)
 * Callees:
 *     ?StopMiPIdleNotificationTimer@@YAXPEAUtagWND@@@Z @ 0x1C000B6CC (-StopMiPIdleNotificationTimer@@YAXPEAUtagWND@@@Z.c)
 *     ?GetMiPWindowFlags@@YA_KPEAUtagWND@@@Z @ 0x1C000D130 (-GetMiPWindowFlags@@YA_KPEAUtagWND@@@Z.c)
 *     InternalSetProp @ 0x1C009B790 (InternalSetProp.c)
 *     _PostTransformableMessage @ 0x1C00CCAC8 (_PostTransformableMessage.c)
 */

__int64 __fastcall PostMousePointerLeaveAndCleanup(__int64 a1, struct tagWND *a2)
{
  __int64 *v2; // rdi
  __int64 v5; // rdi
  __int64 MiPWindowFlags; // rax
  __int64 v7; // rcx

  v2 = (__int64 *)((char *)a2 + 16);
  if ( !a2 )
    v2 = (__int64 *)gptiCurrent;
  v5 = *v2;
  if ( *(_QWORD *)(v5 + 1224) )
  {
    MiPWindowFlags = GetMiPWindowFlags(a2);
    if ( (MiPWindowFlags & 1) == 0 )
    {
      if ( a2 )
        InternalSetProp((__int64)a2, (unsigned __int16)gatomMiPFlags, MiPWindowFlags | 1, 5);
      if ( !a1 || v5 != *(_QWORD *)(a1 + 16) )
      {
        **(_DWORD **)(v5 + 1224) &= ~4u;
        **(_DWORD **)(v5 + 1224) &= ~0x10u;
        **(_DWORD **)(v5 + 1224) &= ~2u;
      }
      v7 = *(_QWORD *)(v5 + 1224);
      if ( (*(_DWORD *)v7 & 1) != 0 )
      {
        if ( !a2 )
          goto LABEL_15;
        PostTransformableMessage(
          (__int64)a2,
          586LL,
          ((unsigned __int16)(*(_WORD *)(v7 + 36) & 0xE1F7) << 16) | 1LL,
          0LL,
          0);
      }
      if ( a2 && *(_QWORD *)a2 != *(_QWORD *)(*(_QWORD *)(v5 + 408) + 432LL) )
      {
LABEL_16:
        if ( a2 )
          StopMiPIdleNotificationTimer(a2);
        return 1LL;
      }
LABEL_15:
      *(_QWORD *)(*(_QWORD *)(v5 + 408) + 432LL) = 0LL;
      goto LABEL_16;
    }
  }
  return 1LL;
}
