/*
 * XREFs of ?IsCandidateTransformOwner@@YAHPEAUtagWND@@0@Z @ 0x1C00F8C7C
 * Callers:
...
_BOOL8 __fastcall IsCandidateTransformOwner(struct tagWND *a1, struct tagWND *a2)
{
  ...
  if ( !(unsigned int)IsTitleWindow(a1, 1LL)
    || *(_QWORD *)(v6 + 408) != *(_QWORD *)(v7 + 408) && *(_QWORD *)(v6 + 400) != *(_QWORD *)(v7 + 400) )
  {
    return 0LL;
  }
  if ( (((unsigned __int16)(*((_DWORD *)a1 + 92) >> 8) ^ (unsigned __int16)(*((_DWORD *)a2 + 92) >> 8)) & 0x1FF) != 0 )
    return 0LL;
  if ( !v4 && !v5 )
    return 1LL;
  ...
  if ( v4 )
  {
    if ( !v11 && !(unsigned int)IntersectRect(v13, &v10->left, &v9->left) )
      return 0LL;
  }
  if ( !v5 || v11 )
    return 1LL;
