/*
 * XREFs of ?CancelInputState@@YAXPEAUtagTHREADINFO@@K@Z @ 0x1C01405C0
 * Callers:
 *     zzzAttachToQueue @ 0x1C00CAED8 (zzzAttachToQueue.c)
 * Callees:
 *     xxxInternalEnumWindow @ 0x1C0054338 (xxxInternalEnumWindow.c)
 *     ??0UserAtomicCheck@@QEAA@XZ @ 0x1C006E870 (--0UserAtomicCheck@@QEAA@XZ.c)
 *     ??1UserAtomicCheck@@QEAA@XZ @ 0x1C006E8C8 (--1UserAtomicCheck@@QEAA@XZ.c)
 *     xxxFocusSetInputContext @ 0x1C008585C (xxxFocusSetInputContext.c)
 *     zzzInputFocusLostWindowEvent @ 0x1C0085FF4 (zzzInputFocusLostWindowEvent.c)
 *     ?QueueNotifyTransformableMessage@@YAXPEAUtagWND@@I_K_JHH@Z @ 0x1C00E34AC (-QueueNotifyTransformableMessage@@YAXPEAUtagWND@@I_K_JHH@Z.c)
 *     UnlockCaptureWindow @ 0x1C01048B0 (UnlockCaptureWindow.c)
 */

void __fastcall CancelInputState(struct tagTHREADINFO *a1, int a2)
{
  int v4; // ebx
  __int64 v5; // rbx
  __int64 v6; // rdx
  _QWORD *v7; // rcx
  __int64 v8; // rbx
  __int64 v9; // rbx
  _QWORD *v10; // rcx
  __int64 v11; // rdx
  __int64 v12; // rdx
  __int64 v13; // rcx
  __int64 v14; // [rsp+30h] [rbp-40h] BYREF
  unsigned int ThreadId; // [rsp+38h] [rbp-38h]
  unsigned int v16; // [rsp+3Ch] [rbp-34h]
  __int64 v17; // [rsp+40h] [rbp-30h] BYREF
  __int64 v18; // [rsp+48h] [rbp-28h]
  _QWORD v19[3]; // [rsp+58h] [rbp-18h] BYREF
  char v20; // [rsp+90h] [rbp+20h] BYREF

  UserAtomicCheck::UserAtomicCheck((UserAtomicCheck *)&v20);
  if ( a2 )
  {
    v4 = a2 - 1;
    if ( v4 )
    {
      if ( v4 != 1 )
        goto LABEL_12;
      *(_DWORD *)(*((_QWORD *)a1 + 51) + 380LL) &= ~0x100000u;
      v8 = *(_QWORD *)(*((_QWORD *)a1 + 51) + 104LL);
      v17 = *(_QWORD *)(gptiCurrent + 392LL);
      *(_QWORD *)(gptiCurrent + 392LL) = &v17;
      v18 = v8;
      if ( v8 )
        _InterlockedIncrement((volatile signed __int32 *)(v8 + 8));
      QueueNotifyTransformableMessage((struct tagWND *)v8, 0x1Fu, 0LL, 0LL, 0, 0);
      v7 = (_QWORD *)*((_QWORD *)a1 + 51);
      if ( v8 == v7[13] )
        UnlockCaptureWindow((__int64)v7);
    }
    else
    {
      v5 = *(_QWORD *)(*((_QWORD *)a1 + 51) + 112LL);
      v17 = *(_QWORD *)(gptiCurrent + 392LL);
      *(_QWORD *)(gptiCurrent + 392LL) = &v17;
      v18 = v5;
      if ( v5 )
        _InterlockedIncrement((volatile signed __int32 *)(v5 + 8));
      QueueNotifyTransformableMessage((struct tagWND *)v5, 8u, 0LL, 0LL, 0, 0);
      if ( (*gpsi & 4) != 0 )
        xxxFocusSetInputContext((__int64 *)v5, 0, 1);
      v7 = (_QWORD *)(*((_QWORD *)a1 + 51) + 112LL);
      if ( v5 == *v7 )
      {
        HMAssignmentUnlock(v7);
        if ( v5 )
        {
          v7 = (_QWORD *)gpqForeground;
          if ( *((_QWORD *)a1 + 51) == gpqForeground )
            zzzInputFocusLostWindowEvent(0LL, 5u);
        }
      }
    }
  }
  else
  {
    v9 = *(_QWORD *)(*((_QWORD *)a1 + 51) + 120LL);
    v17 = *(_QWORD *)(gptiCurrent + 392LL);
    *(_QWORD *)(gptiCurrent + 392LL) = &v17;
    v18 = v9;
    if ( v9 )
      _InterlockedIncrement((volatile signed __int32 *)(v9 + 8));
    QueueNotifyTransformableMessage((struct tagWND *)v9, 0x86u, 0LL, 0LL, 0, 0);
    QueueNotifyTransformableMessage(
      (struct tagWND *)v9,
      6u,
      (unsigned __int8)(*(_BYTE *)(v9 + 71) & 0x20) << 16,
      0LL,
      0,
      0);
    v10 = (_QWORD *)(*((_QWORD *)a1 + 51) + 120LL);
    if ( v9 == *v10 )
      HMAssignmentUnlock(v10);
    v14 = *(_QWORD *)(v9 + 16);
    ThreadId = (unsigned int)PsGetThreadId(**(PETHREAD **)(v9 + 16));
    v16 = v16 & 0xFFFFFFFC | 2;
    v11 = *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(v9 + 16) + 432LL) + 8LL) + 16LL) + 112LL);
    v19[0] = *(_QWORD *)(gptiCurrent + 392LL);
    *(_QWORD *)(gptiCurrent + 392LL) = v19;
    v19[1] = v11;
    if ( v11 )
      _InterlockedIncrement((volatile signed __int32 *)(v11 + 8));
    xxxInternalEnumWindow(
      *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(*(_QWORD *)(v9 + 16) + 432LL) + 8LL) + 16LL) + 112LL),
      (__int64 (__fastcall *)(__int64, __int64))xxxActivateApp,
      (__int64)&v14,
      2u);
    ThreadUnlock1(v13, v12);
  }
  ThreadUnlock1(v7, v6);
LABEL_12:
  UserAtomicCheck::~UserAtomicCheck((UserAtomicCheck *)&v20);
}
