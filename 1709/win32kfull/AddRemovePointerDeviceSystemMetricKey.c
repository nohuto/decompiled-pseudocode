/*
 * XREFs of AddRemovePointerDeviceSystemMetricKey @ 0x1C00D6D90
 * Callers:
...
int __fastcall AddRemovePointerDeviceSystemMetricKey(int a1, int a2)
{
  int result; // eax
  const WCHAR *v3; // rdx
    ...
    *(_OWORD *)&ObjectAttributes.SecurityDescriptor = 0LL;
    ...
    if ( result >= 0 )
    {
      ...
      if ( ZwSetValueKey(KeyHandle, &ValueName, 0, 4u, &Data, 4u) >= 0 )
        *(_DWORD *)(&WPP_MAIN_CB.DeviceQueue.Size + 1) = Data;
