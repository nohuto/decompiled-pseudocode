/*
 * XREFs of ?xxxTM_MoveDragRect@@YAXPEAU_MOVESIZEDATA@@_J@Z @ 0x1C01DC5F0
 * Callers:
 *     ?xxxMS_TrackMove@@YAXPEAUtagWND@@W4_WM_VALUE@@_K_JPEAU_MOVESIZEDATA@@@Z @ 0x1C01DAD48 (-xxxMS_TrackMove@@YAXPEAUtagWND@@W4_WM_VALUE@@_K_JPEAU_MOVESIZEDATA@@@Z.c)
 * Callees:
 *     xxxMinMaximizeEx @ 0x1C004AB9C (xxxMinMaximizeEx.c)
 *     GreIsWindowResizeInProgress @ 0x1C0090F40 (GreIsWindowResizeInProgress.c)
 *     xxxSendTransformableMessageTimeout @ 0x1C00A8A60 (xxxSendTransformableMessageTimeout.c)
 *     bSetDevDragRect @ 0x1C00E35F0 (bSetDevDragRect.c)
 *     __security_check_cookie @ 0x1C0143530 (__security_check_cookie.c)
 *     ?MoveRect@@YAHPEAU_MOVESIZEDATA@@KW4_MOVERECT_STYLE@@@Z @ 0x1C01D8938 (-MoveRect@@YAHPEAU_MOVESIZEDATA@@KW4_MOVERECT_STYLE@@@Z.c)
 *     ?SizeRect@@YAHPEAU_MOVESIZEDATA@@KPEAUtagMONITOR@@PEAK@Z @ 0x1C01D8D68 (-SizeRect@@YAHPEAU_MOVESIZEDATA@@KPEAUtagMONITOR@@PEAK@Z.c)
 *     ?SwapRect@@YAXPEAUtagRECT@@0@Z @ 0x1C01D8FFC (-SwapRect@@YAXPEAUtagRECT@@0@Z.c)
 *     ?xxxSizeOrMoveRect@@YAHPEAU_MOVESIZEDATA@@KPEAKPEAUtagRECT@@PEAPEAUtagCHECKPOINT@@@Z @ 0x1C01DBAC0 (-xxxSizeOrMoveRect@@YAHPEAU_MOVESIZEDATA@@KPEAKPEAUtagRECT@@PEAPEAUtagCHECKPOINT@@@Z.c)
 *     xxxDrawDragRectEx @ 0x1C01DD710 (xxxDrawDragRectEx.c)
 */

void __fastcall xxxTM_MoveDragRect(struct _MOVESIZEDATA *a1, int a2)
{
  __int64 v2; // rax
  HWND *v4; // rcx
  int v5; // edi
  char v7; // r14
  bool v8; // zf
  unsigned int v9; // r15d
  __int64 v10; // rcx
  unsigned __int64 v11; // r8
  __int64 v12; // rcx
  __int128 v13; // xmm2
  int v14; // eax
  int v15; // eax
  unsigned __int64 v16; // r8
  __int64 *v17; // rcx
  __int128 *v18; // r14
  unsigned __int64 v19; // r8
  __int64 *v20; // rcx
  unsigned int v21; // [rsp+50h] [rbp-29h] BYREF
  struct tagRECT *v22; // [rsp+58h] [rbp-21h] BYREF
  char *v23; // [rsp+60h] [rbp-19h]
  __int64 v24; // [rsp+68h] [rbp-11h]
  struct tagRECT v25; // [rsp+70h] [rbp-9h] BYREF
  __int128 v26; // [rsp+80h] [rbp+7h] BYREF

  v2 = *((_QWORD *)a1 + 26);
  v4 = (HWND *)*((_QWORD *)a1 + 2);
  v5 = 0;
  v21 = 0;
  v22 = 0LL;
  v7 = 0;
  v24 = v2;
  if ( (unsigned int)GreIsWindowResizeInProgress(*v4, 0LL) )
  {
    *((_DWORD *)a1 + 50) |= 1u;
    return;
  }
  v8 = *((_DWORD *)a1 + 44) == 9;
  v23 = (char *)a1 + 72;
  *(_OWORD *)((char *)a1 + 40) = *(_OWORD *)((char *)a1 + 24);
  *(_OWORD *)((char *)a1 + 72) = *(_OWORD *)((char *)a1 + 56);
  if ( v8 )
  {
    if ( (*((_DWORD *)a1 + 49) & 0x800) != 0 )
    {
      if ( !(unsigned int)xxxSizeOrMoveRect(a1, a2, &v21, &v25, (struct tagCHECKPOINT **)&v22) )
        return;
      v7 = v21;
      if ( (v21 & 1) != 0 )
      {
        SwapRect(v22, &v25);
        xxxMinMaximizeEx(*((struct tagWND **)a1 + 2), 9u, 4, 0LL, 0LL);
        SwapRect(&v25, v22);
        v9 = 532;
        goto LABEL_12;
      }
    }
    else if ( !(unsigned int)MoveRect((__int64)a1, a2, 0) )
    {
      return;
    }
    v9 = 534;
    goto LABEL_12;
  }
  if ( !(unsigned int)SizeRect(a1, a2, 0LL, &v21) )
    return;
  v7 = v21;
  v9 = 532;
LABEL_12:
  if ( (v7 & 2) != 0 )
  {
    v10 = *((_QWORD *)a1 + 2);
    if ( (*(_BYTE *)(v10 + 70) & 1) != 0 )
    {
      v11 = *((int *)a1 + 44);
      v26 = *(_OWORD *)((char *)a1 + 40);
      _InterlockedAdd(&glSendMessage, 1u);
      xxxSendTransformableMessageTimeout((__int64 *)v10, v9, v11, (struct _LARGE_STRING *)&v26, 0, 0, 0LL, 1u, 0);
      xxxMinMaximizeEx(*((struct tagWND **)a1 + 2), 3u, 22, *((_QWORD *)a1 + 29), 0LL);
      v12 = *((_QWORD *)a1 + 2);
      if ( (*(_BYTE *)(v12 + 305) & 3) != 0 || (*(_BYTE *)(v12 + 71) & 1) != 0 )
        v5 = 128;
      *((_DWORD *)a1 + 49) = v5 | *((_DWORD *)a1 + 49) & 0xFFFFFF7F;
      v13 = *(_OWORD *)(v12 + 128);
      v26 = v13;
      *(_OWORD *)((char *)a1 + 40) = v13;
      *(_OWORD *)((char *)a1 + 72) = v13;
      *(_OWORD *)((char *)a1 + 24) = v13;
      *(_OWORD *)((char *)a1 + 56) = v13;
      goto LABEL_29;
    }
  }
  v14 = *((_DWORD *)a1 + 49);
  if ( (v14 & 0x4000000) != 0 )
  {
    if ( (v14 & 0x20) != 0 )
    {
      if ( (v7 & 8) != 0 )
      {
        xxxDrawDragRectEx(a1);
      }
      else if ( (v7 & 4) != 0 )
      {
        bSetDevDragRect(*(HDEV *)(gpDispInfo + 32LL), 0LL, 0LL, 0);
      }
    }
    v15 = *((_DWORD *)a1 + 49);
    if ( (v15 & 0x2000000) != 0 )
    {
      if ( (v15 & 0x20) == 0 )
      {
        v18 = (__int128 *)v23;
        goto LABEL_28;
      }
      v16 = *((int *)a1 + 44);
      v17 = (__int64 *)*((_QWORD *)a1 + 2);
      v26 = *(_OWORD *)((char *)a1 + 72);
      *((_DWORD *)a1 + 49) = v15 & 0xFFFFFFDF;
      _InterlockedAdd(&glSendMessage, 1u);
      xxxSendTransformableMessageTimeout(v17, v9, v16, (struct _LARGE_STRING *)&v26, 0, 0, 0LL, 1u, 0);
      xxxDrawDragRectEx(a1);
      *((_DWORD *)a1 + 49) |= 0x20u;
    }
  }
  v18 = (__int128 *)((char *)a1 + 40);
LABEL_28:
  v19 = *((int *)a1 + 44);
  v20 = (__int64 *)*((_QWORD *)a1 + 2);
  v26 = *v18;
  _InterlockedAdd(&glSendMessage, 1u);
  xxxSendTransformableMessageTimeout(v20, v9, v19, (struct _LARGE_STRING *)&v26, 0, 0, 0LL, 1u, 0);
  xxxDrawDragRectEx(a1);
LABEL_29:
  if ( *((_DWORD *)a1 + 44) == 9
    && ((*(_DWORD *)(*((_QWORD *)a1 + 2) + 368LL) & 0xF) == 2 || *((_QWORD *)a1 + 26) == v24) )
  {
    *((_DWORD *)a1 + 42) += v26 - (__int16)a2;
    *((_DWORD *)a1 + 43) += DWORD1(v26) - SHIWORD(a2);
  }
}
