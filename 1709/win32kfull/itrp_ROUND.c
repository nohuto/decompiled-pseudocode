/*
 * XREFs of itrp_ROUND @ 0x1C02D1890
 * Callers:
 *     <none>
 * Callees:
 *     InvokeRoundValue @ 0x1C02C94DC (InvokeRoundValue.c)
 */

__int64 __fastcall itrp_ROUND(__int64 a1, int a2)
{
  __int64 result; // rax

  if ( (qword_1C0327C90 ^ *(_QWORD *)qword_1C0327C90 ^ MEMORY[0xFFFFF78000000330] ^ *(_QWORD *)(qword_1C0327C90 + 424)) == *(_QWORD *)(qword_1C0327C90 + 432)
    && (qword_1C0327C78 - *(_QWORD *)qword_1C0327C90) >> 2 )
  {
    qword_1C0327C78 -= 4LL;
    *(_DWORD *)qword_1C0327C78 = InvokeRoundValue(
                                   *(_DWORD *)(qword_1C0327C90 + 128),
                                   *(_DWORD *)qword_1C0327C78,
                                   *(_DWORD *)(qword_1C0327C90 + 4LL * (a2 - 104) + 32));
    result = a1;
    qword_1C0327C78 += 4LL;
  }
  else
  {
    result = qword_1C0327CE8;
    dword_1C0327CE0 = 4368;
  }
  return result;
}
