/*
 * XREFs of cjComputeGLYPHSET_OLDBIDI @ 0x1C022ADC4
 * Callers:
 *     bLoadGlyphSet @ 0x1C0228FB8 (bLoadGlyphSet.c)
 * Callees:
 *     memset @ 0x1C0145A00 (memset.c)
 *     fs_GetGlyphIDs @ 0x1C02B5BB8 (fs_GetGlyphIDs.c)
 */

__int64 __fastcall cjComputeGLYPHSET_OLDBIDI(int a1, char **a2, int a3)
{
  unsigned int v3; // ebx
  int v4; // eax
  char **v6; // r12
  int v7; // ebp
  int v8; // r15d
  const char *v10; // rcx
  const char *v11; // r14
  __int64 v12; // rax
  int v13; // ebp
  char *v14; // rax
  char *v15; // rdi
  char *v16; // rsi
  int v17; // edx
  int v18; // r9d
  __int64 v19; // rbp
  __int64 v20; // rcx
  char *v21; // r15
  int v22; // r12d
  int v23; // ecx
  __int64 v24; // rax
  __int64 v25; // rax
  __int64 v26; // rax
  int i; // ebp
  char *v28; // [rsp+30h] [rbp-58h]
  unsigned __int16 v31; // [rsp+A0h] [rbp+18h] BYREF
  __int64 v32; // [rsp+A8h] [rbp+20h]

  v3 = 0;
  v4 = a3 & 0xFF00;
  *a2 = 0LL;
  v6 = a2;
  v7 = 0;
  v8 = 0;
  switch ( v4 )
  {
    case 61440:
      v10 = (const char *)a0123456789;
      break;
    case 61696:
      v10 = " ";
      break;
    case 61952:
      v10 = (const char *)&unk_1C02ECD30;
      break;
    default:
      return 0LL;
  }
  v11 = v10;
  while ( *((_WORD *)v10 + 1) )
  {
    v12 = *((unsigned __int16 *)v10 + 1);
    ++v8;
    v7 += v12;
    v10 += 2 * v12 + 4;
  }
  v13 = v7 + 256;
  v14 = (char *)EngAllocMem(0, 4 * (v13 + 4 * v8 + 8), 0x64667454u);
  v15 = v14;
  if ( v14 )
  {
    *(_DWORD *)v14 = 4 * (v13 + 4 * v8 + 8);
    *((_DWORD *)v14 + 1) = 4;
    *((_DWORD *)v14 + 2) = v13;
    *((_DWORD *)v14 + 3) = v8 + 1;
    v16 = &v14[16 * v8 + 32];
    v28 = &v16[4 * v13 - 1024];
    memset(v28, 0, 0x400uLL);
    v19 = v8;
    v20 = 0LL;
    v32 = 0LL;
    if ( v8 > 0 )
    {
      v21 = v15 + 18;
      while ( 1 )
      {
        v22 = 0;
        *((_WORD *)v21 - 1) = *(_WORD *)v11;
        *(_WORD *)v21 = *((_WORD *)v11 + 1);
        *(_QWORD *)(v21 + 6) = v16;
        if ( *((_WORD *)v11 + 1) )
          break;
LABEL_18:
        v25 = *((unsigned __int16 *)v11 + 1);
        ++v20;
        v21 += 16;
        v32 = v20;
        v11 += 2 * v25 + 4;
        if ( v20 >= v19 )
        {
          v6 = a2;
          goto LABEL_20;
        }
      }
      while ( !(unsigned int)fs_GetGlyphIDs(
                               a1,
                               v17,
                               (unsigned __int16)(*(_WORD *)&v11[2 * v22 + 4] + a3),
                               v18,
                               (__int64)&v31) )
      {
        v23 = v31;
        v17 = (int)v28;
        *(_DWORD *)v16 = v31;
        v16 += 4;
        v24 = v22++;
        *(_DWORD *)&v28[4 * *(unsigned __int16 *)&v11[2 * v24 + 4]] = v23;
        if ( v22 >= *((unsigned __int16 *)v11 + 1) )
        {
          v20 = v32;
          goto LABEL_18;
        }
      }
LABEL_27:
      EngFreeMem(v15);
      return 0LL;
    }
LABEL_20:
    *(_WORD *)&v15[16 * v19 + 16] = a3;
    v26 = 2 * v19;
    *(_WORD *)&v15[8 * v26 + 18] = 256;
    *(_QWORD *)&v15[8 * v26 + 24] = v16;
    for ( i = 0; i < 256; ++i )
    {
      if ( !*(_DWORD *)v16 )
      {
        if ( (unsigned int)fs_GetGlyphIDs(a1, v17, a3 + i, v18, (__int64)&v31) )
          goto LABEL_27;
        *(_DWORD *)v16 = v31;
      }
      v16 += 4;
    }
    v3 = *(_DWORD *)v15;
    *v6 = v15;
  }
  return v3;
}
