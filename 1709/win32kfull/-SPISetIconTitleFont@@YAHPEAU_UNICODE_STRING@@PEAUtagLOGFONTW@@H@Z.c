/*
 * XREFs of ?SPISetIconTitleFont@@YAHPEAU_UNICODE_STRING@@PEAUtagLOGFONTW@@H@Z @ 0x1C01B7A74
 * Callers:
...
__int64 __fastcall SPISetIconTitleFont(struct _UNICODE_STRING *a1, struct tagLOGFONTW *a2, int a3)
{
  ...
  if ( v8 != v9 )
  {
    ...
    a2->lfHeight = EngMulDiv(a2->lfHeight, *(unsigned __int16 *)(gpsi + 9974LL), v10);
  }
  FontFromUserProfile = CreateFontFromUserProfile(a1, a2, 0x9Au);
  v12 = FontFromUserProfile;
  if ( FontFromUserProfile )
  {
    if ( !(unsigned int)UserSetAltScaleFont((HBRUSH)FontFromUserProfile, (__int64 *)&v23) )
    {
    ...
    if ( a3 )
    {
      if ( a2 )
      {
        if ( (unsigned int)GreExtGetObjectW((HBRUSH)v12, 92LL, v24) )
