/*
 * XREFs of ?RemoveLocked@FxTransactionedList@@IEAAEPEAUFxTransactionedEntry@@@Z @ 0x1C002E9C8
 * Callers:
...
char __fastcall FxTransactionedList::RemoveLocked(FxTransactionedList *this, FxTransactionedEntry *Entry)
{
  ...
  if ( Entry->m_Transaction == FxTransactionActionAdd )
  {
    ...
    if ( Flink->Blink != p_m_TransactionLink
      || (Blink = p_m_TransactionLink->Blink, Blink->Flink != p_m_TransactionLink) )
    {
      __fastfail(3u);
    }
    Blink->Flink = Flink;
  ...
  else
  {
    ...
    if ( (FxTransactionedEntry *)Entry->m_ListLink.Flink->Blink != Entry
      || (v6 = Entry->m_ListLink.Blink, (FxTransactionedEntry *)v6->Flink != Entry) )
    {
      __fastfail(3u);
    }
    v6->Flink = v5;
