/*
 * XREFs of ?ReleaseOrReplaceThreadTableEntry@FxVerifierLock@@SAXPEAU_KTHREAD@@PEAV1@@Z @ 0x1C0082610
 * Callers:
...
void __fastcall FxVerifierLock::ReleaseOrReplaceThreadTableEntry(unsigned __int64 curThread, FxVerifierLock *pLock)
{
  ...
  if ( Thread )
  {
    ...
    if ( Flink->Blink != &pLock->m_ThreadTableEntry.HashChain
      || (Blink = pLock->m_ThreadTableEntry.HashChain.Blink, Blink->Flink != &pLock->m_ThreadTableEntry.HashChain) )
    {
      __fastfail(3u);
    }
    Blink->Flink = Flink;
    ...
    if ( PerThreadPassiveLockList
      || (PerThreadPassiveLockList = pLock->m_ThreadTableEntry.PerThreadDispatchLockList) != 0LL )
    {
