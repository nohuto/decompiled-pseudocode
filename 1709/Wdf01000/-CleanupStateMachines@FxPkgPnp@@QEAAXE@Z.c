/*
 * XREFs of ?CleanupStateMachines@FxPkgPnp@@QEAAXE@Z @ 0x1C009CC3C
 * Callers:
 ...
 *     ?CleanupPowerCallback@FxPowerPolicyOwnerSettings@@QEAAXXZ @ 0x1C00A031C (-CleanupPowerCallback@FxPowerPolicyOwnerSettings@@QEAAXXZ.c)
 *     ?SetFinished@FxEventQueue@@QEAAEPEAUFxCREvent@@@Z @ 0x1C00A2B34 (-SetFinished@FxEventQueue@@QEAAEPEAUFxCREvent@@@Z.c)
 */
...
void __fastcall FxPkgPnp::CleanupStateMachines(FxPkgPnp *this, unsigned __int8 CleanupPnp)
{
  ...
  if ( !FxEventQueue::SetFinished(&this->m_PowerPolicyMachine, &eventOnStack) )
  {
    ...
    if ( !v6->m_ObjectSize )
      v7 = 0LL;
    WPP_IFR_SF_qq(this->m_Globals, 4u, 0xCu, 0x24u, WPP_FxPkgPnp_cpp_Traceguids, v7, v6->m_DeviceObject.m_DeviceObject);
    KeEnterCriticalRegion();
    KeWaitForSingleObject(&eventOnStack, Executive, 0, 0, 0LL);
    KeLeaveCriticalRegion();
  }
  ...
  if ( !FxEventQueue::SetFinished(&this->m_PowerMachine, &eventOnStack) )
  {
    ...
    if ( !v8->m_ObjectSize )
      v9 = 0LL;
    WPP_IFR_SF_qq(this->m_Globals, 4u, 0xCu, 0x25u, WPP_FxPkgPnp_cpp_Traceguids, v9, v8->m_DeviceObject.m_DeviceObject);
    KeEnterCriticalRegion();
    KeWaitForSingleObject(&eventOnStack, Executive, 0, 0, 0LL);
    KeLeaveCriticalRegion();
  }
