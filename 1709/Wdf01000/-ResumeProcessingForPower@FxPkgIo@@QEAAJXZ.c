/*
 * XREFs of ?ResumeProcessingForPower@FxPkgIo@@QEAAJXZ @ 0x1C0019C10
 * Callers:
...
__int64 __fastcall FxPkgIo::ResumeProcessingForPower(FxPkgIo *this)
{
  ...
  {
    FxVerifierLock::Lock(v23, &irql, v4);
    v7 = irql;
  }
  else
  {
  ...
  if ( Blink != &this->m_IoQueueListHead )
  {
    do
    {
      if ( LODWORD(Blink[1].Flink) == 1 )
      {
        ...
        {
          v24 = (FxTagTracker *)Blink[-60].Blink;
          if ( v24 )
  ...
  *(_WORD *)&this->m_PowerStateOn = 1;
  if ( v11 < 0 && (v25 = (FxVerifierLock *)this[-1].m_DispatchTable[27]) != 0LL )
    FxVerifierLock::Unlock(v25, v7, v8);
  else
  ...
  for ( i = Next; i; i = i->Next )
  {
    ...
    if ( BYTE2(i[-99].Next) )
    {
      if ( SLOBYTE(v14[3].Next) < 0 && (v26 = (FxVerifierLock *)i[-121].Next) != 0LL )
      {
  ...
  while ( v15 )
  {
    ...
    {
      if ( SLOBYTE(v16[3].Next) < 0 && (v27 = (FxVerifierLock *)v15[-121].Next) != 0LL )
