/*
 * XREFs of ?DispatchRequestToDriver@FxIoQueue@@QEAAXPEAVFxRequest@@@Z @ 0x1C000C290
 * Callers:
...
void __fastcall FxIoQueue::DispatchRequestToDriver(FxIoQueue *this, FxRequest *pRequest, char a3)
{
  ...
  _LIST_ENTRY *v67; // r10
  FxRequestContext *v68; // rcx
  _LIST_ENTRY *Flink; // rcx
  FX_POOL_HEADER *v70; // rax
  ...
  unsigned __int8 v95; // [rsp+40h] [rbp-68h] BYREF
  unsigned __int8 v96; // [rsp+41h] [rbp-67h] BYREF
  unsigned __int8 PreviousIrql; // [rsp+42h] [rbp-66h] BYREF
  unsigned __int8 v98; // [rsp+43h] [rbp-65h] BYREF
  unsigned __int8 v99[4]; // [rsp+44h] [rbp-64h] BYREF
  _LIST_ENTRY *v100; // [rsp+48h] [rbp-60h]
  ULONG_PTR retaddr; // [rsp+A8h] [rbp+0h]
  unsigned __int8 v103; // [rsp+B8h] [rbp+10h]
  unsigned __int8 v104; // [rsp+C0h] [rbp+18h]
  unsigned __int8 v105; // [rsp+C8h] [rbp+20h] BYREF
  ...
  if ( pRequest->m_Globals->FxVerifierIO )
  {
    ...
    v64 = pRequest->m_Globals;
    if ( v64->FxVerifierOn )
      FxRequest::Vf_VerifyRequestIsNotCompleted(pRequest, v64);
    FxNonPagedObject::Unlock(pRequest, PreviousIrql, v63);
  }
  v6 = 0LL;
  if ( pRequest->m_Completed )
  {
    ...
    if ( !pRequest->m_ObjectSize )
      v65 = 0LL;
    WPP_IFR_SF_q(pRequest->m_Globals, 2u, 0x10u, 0x27u, WPP_FxRequest_cpp_Traceguids, v65);
  ...
  v103 = m_IrpAllocation;
  v104 = pRequest->m_CanComplete;
  if ( SLOBYTE(pRequest->m_ObjectFlags) < 0 )
  {
    v67 = pRequest[-1].m_OwnerListEntry2.Blink;
    if ( v67 )
    {
      FxTagTracker::UpdateTagHistory(
        (FxTagTracker *)v67,
        ...
        TagRelease,
        pRequest->m_Refcnt - 1);
      m_IrpAllocation = v103;
      m_CanComplete = (FxObjectDebugLeakDetection *)v104;
    }
  }
    ...
    else
    {
      ...
      if ( (char *)SelfDestruct == (char *)FxRequestFromLookaside::SelfDestruct )
      {
        ...
        if ( m_RequestContext )
        {
          ...
          v68 = pRequest->m_RequestContext;
          if ( v68 )
            ((void (__fastcall *)(FxRequestContext *, __int64))v68->~FxRequestContext)(v68, 1LL);
        }
        ...
        {
          v70 = FxObject::_CleanupPointer(pRequest->m_Globals, pRequest);
          ExFreePoolWithTag(v70->Base, 0);
        }
        else
        {
          if ( SLOBYTE(pRequest->m_ObjectFlags) < 0 )
          ...
          if ( v47 && *(_WORD *)(v47 + 8) == 4354 && *(_BYTE *)(v47 + 1683) )
          {
      ...
      else
      {
        ...
        if ( (char *)SelfDestruct == (char *)FxObject::SelfDestruct )
        {
          ...
          if ( (char *)v52 == (char *)FxRegKey::`scalar deleting destructor' )
          {
            ...
            v54 = *(_QWORD **)&pRequest->m_NPLock.m_DbgFlagIsInitialized;
          ...
          else
          {
            if ( (char *)v52 == (char *)FxRequest::`scalar deleting destructor' )
            {
              ...
              if ( v58 )
              {
                ...
                v71 = pRequest->m_RequestContext;
                if ( v71 )
                  ((void (__fastcall *)(FxRequestContext *, __int64))v71->~FxRequestContext)(v71, 1LL);
              }
              ...
              if ( SLOBYTE(pRequest->m_ObjectFlags) < 0 )
              {
                v72 = pRequest[-1].m_ForwardProgressList.Flink;
                if ( v72 )
                {
                  FxVerifierLock::`scalar deleting destructor'((FxVerifierLock *)v72, (unsigned int)v51);
                  pRequest[-1].m_ForwardProgressList.Flink = 0LL;
                }
              }
              pRequest->m_NPLock.m_DbgFlagIsInitialized = 0;
    ...
    if ( MajorFunction != 4 )
    {
      if ( MajorFunction == 14 )
      {
        if ( v5->m_IoDeviceControl.Method )
        {
          ...
          v105 = 0;
          if ( m_CallbackLock )
            m_CallbackLock->Lock(m_CallbackLock, &v105);
...
LABEL_25:
        if ( !m_Globals->FxVerifierOn )
          return;
        FxNonPagedObject::Lock(pRequest, v99, a3);
        m_VerifierFlags = pRequest->m_VerifierFlags;
        FxNonPagedObject::Unlock(pRequest, v99[0], v94);
