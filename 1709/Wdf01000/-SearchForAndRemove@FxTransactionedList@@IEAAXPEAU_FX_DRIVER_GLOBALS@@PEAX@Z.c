/*
 * XREFs of ?SearchForAndRemove@FxTransactionedList@@IEAAXPEAU_FX_DRIVER_GLOBALS@@PEAX@Z @ 0x1C008643C
 * Callers:
        ...
        void *EntryData)
{
  ...
  for ( i = this->m_TransactionHead.Flink; i != &this->m_TransactionHead; i = i->Flink )
  {
    ...
    if ( this->Compare(this, (FxTransactionedEntry *)&i[-1], EntryData) )
    {
      ...
      {
        Flink = i->Flink;
        if ( i->Flink->Blink != i || (p_Flink = &v8->m_TransactionLink.Blink->Flink, *p_Flink != i) )
          __fastfail(3u);
        *p_Flink = Flink;
        v4 = 1;
