/*
 * XREFs of ?VidSchiWorkerThreadTimerCallback@@YAXPEAU_EX_TIMER@@PEAX@Z @ 0x1C00130A0
 * Callers:
 *     <none>
 * Callees:
...
void __fastcall VidSchiWorkerThreadTimerCallback(struct _EX_TIMER *a1, char *a2)
{
  ...
  KeAcquireInStackQueuedSpinLockAtDpcLevel((PKSPIN_LOCK)a2 + 236, &LockHandle);
  ...
  else
  {
    ...
    if ( *((_DWORD *)a2 + 15) )
    {
      do
      {
        VidSchiCheckYieldExitCondition(*(struct _VIDSCH_NODE **)&a2[8 * v5 + 424]);
        if ( v9 )
          v4 = 1;
      ...
      if ( v4 )
      {
        *((_QWORD *)a2 + 173) = MEMORY[0xFFFFF78000000320];
        KeSetEvent((PRKEVENT)(a2 + 1352), 0, 0);
      }
    }
  }
