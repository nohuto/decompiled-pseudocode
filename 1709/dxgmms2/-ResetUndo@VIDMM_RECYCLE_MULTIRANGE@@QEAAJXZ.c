/*
 * XREFs of ?ResetUndo@VIDMM_RECYCLE_MULTIRANGE@@QEAAJXZ @ 0x1C004C0C0
 * Callers:
 *     ?ResetUndo@VIDMM_RECYCLE_HEAP_MGR@@UEAAJPEAX@Z @ 0x1C004C070 (-ResetUndo@VIDMM_RECYCLE_HEAP_MGR@@UEAAJPEAX@Z.c)
 * Callees:
 *     McTemplateK0q @ 0x1C0016F44 (McTemplateK0q.c)
 *     ?Map@VIDMM_RECYCLE_MULTIRANGE@@QEAAPEAX_K0H@Z @ 0x1C007470C (-Map@VIDMM_RECYCLE_MULTIRANGE@@QEAAPEAX_K0H@Z.c)
 *     ?Unmap@VIDMM_RECYCLE_MULTIRANGE@@QEAAXXZ @ 0x1C00747C4 (-Unmap@VIDMM_RECYCLE_MULTIRANGE@@QEAAXXZ.c)
 */

__int64 __fastcall VIDMM_RECYCLE_MULTIRANGE::ResetUndo(VIDMM_RECYCLE_MULTIRANGE *this)
{
  int v1; // ebp
  ULONG_PTR v2; // r8
  __int64 v3; // rdi
  VIDMM_RECYCLE_MULTIRANGE *v4; // rbx
  ULONG Protect; // esi
  int v6; // edx
  int v7; // eax
  NTSTATUS v8; // eax
  __int64 v9; // rdx
  __int64 v11; // rax
  PVOID BaseAddress; // [rsp+50h] [rbp+8h] BYREF
  ULONG_PTR RegionSize; // [rsp+58h] [rbp+10h] BYREF

  v1 = 0;
  v2 = *((_QWORD *)this + 5) - *((_QWORD *)this + 6);
  LODWORD(v3) = 0;
  v4 = this;
  BaseAddress = 0LL;
  RegionSize = v2;
  if ( g_IsInternalReleaseOrDbg )
  {
    *(_QWORD *)(WdLogNewEntry5_WdTrace(this) + 24) = this;
    v2 = RegionSize;
  }
  if ( *((_BYTE *)v4 + 153) )
  {
    LODWORD(v3) = -1071775472;
LABEL_16:
    if ( bTracingEnabled && ((unsigned __int8)Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
      McTemplateK0q((__int64)this, (__int64)&EventPerformanceWarning, v2, 18);
    goto LABEL_12;
  }
  Protect = 1028;
  this = *(VIDMM_RECYCLE_MULTIRANGE **)(*((_QWORD *)v4 + 10) + 32LL);
  v6 = *(_DWORD *)this;
  if ( (unsigned int)(*(_DWORD *)this - 3) <= 3 )
  {
    if ( v6 != 3 )
      Protect = 4;
    VIDMM_RECYCLE_MULTIRANGE::Map(v4, 0LL, v2, 1);
    if ( *((_BYTE *)v4 + 152) && (this = (VIDMM_RECYCLE_MULTIRANGE *)*((_QWORD *)v4 + 20)) != 0LL )
    {
      this = (VIDMM_RECYCLE_MULTIRANGE *)((char *)this + *((_QWORD *)v4 + 21));
      v1 = 1;
      BaseAddress = this;
    }
    else
    {
      LODWORD(v3) = -1071775472;
    }
  }
  else
  {
    BaseAddress = (PVOID)*((_QWORD *)v4 + 6);
    v7 = 4;
    if ( v6 != 2 )
      v7 = 1028;
    Protect = v7;
  }
  if ( (int)v3 >= 0 )
  {
    v8 = ZwAllocateVirtualMemory((HANDLE)0xFFFFFFFFFFFFFFFFLL, &BaseAddress, 0LL, &RegionSize, 0x1000000u, Protect);
    v3 = v8;
    if ( v8 < 0 )
    {
      v11 = WdLogNewEntry5_WdEvent(this, v9);
      *(_QWORD *)(v11 + 24) = v3;
      WdLogEvent5_WdEvent(v11);
    }
  }
  if ( (_DWORD)v3 == -1071775472 )
    goto LABEL_16;
LABEL_12:
  if ( v1 )
    VIDMM_RECYCLE_MULTIRANGE::Unmap(v4);
  return (unsigned int)v3;
}
