/*
 * XREFs of ?Create@CWindowsPolicyManager@@SAJPEAPEAV1@@Z @ 0x180004DEC
 * Callers:
 *     ActivatePolicyManager @ 0x1800048F0 (ActivatePolicyManager.c)
 * Callees:
...
__int64 __fastcall CWindowsPolicyManager::Create(struct CWindowsPolicyManager **a1)
{
  ...
  CWindowsPolicyManager *v3; // rax
  CWindowsPolicyManager *v4; // rbx
  signed int v5; // edi

  ProcessHeap = GetProcessHeap();
  v3 = (CWindowsPolicyManager *)HeapAlloc(ProcessHeap, 0, 0x40uLL);
  v4 = v3;
  if ( v3 )
  {
    *((_DWORD *)v3 + 6) = 1;
  ...
  if ( v4 )
  {
    v5 = CWindowsPolicyManager::Initialize(v4);
    if ( v5 < 0 )
