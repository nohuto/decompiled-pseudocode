/*
 * XREFs of ndisFLoopbackNetBufferLists @ 0x1C002478C
 * Callers:
 *     NdisSendNetBufferLists @ 0x1C0012F00 (NdisSendNetBufferLists.c)
 *     ndisFilterSendNetBufferLists @ 0x1C00162F0 (ndisFilterSendNetBufferLists.c)
 *     NdisFSendNetBufferLists @ 0x1C0017530 (NdisFSendNetBufferLists.c)
 *     ndisSendNBLToFilter @ 0x1C005A120 (ndisSendNBLToFilter.c)
 * Callees:
 *     NdisFSendNetBufferListsComplete @ 0x1C00172C0 (NdisFSendNetBufferListsComplete.c)
 *     WPP_SF_qq @ 0x1C0039520 (WPP_SF_qq.c)
 *     ndisFDoLoopbackNetBufferList @ 0x1C0059138 (ndisFDoLoopbackNetBufferList.c)
 *     ndisFIsLoopbackNetBuffer @ 0x1C0059794 (ndisFIsLoopbackNetBuffer.c)
 */

_QWORD *__fastcall ndisFLoopbackNetBufferLists(
        struct _LOCK_STATE_EX *NdisFilterHandle,
        PNET_BUFFER_LIST NetBufferList,
        __int64 a3,
        __int64 a4,
        _QWORD *a5)
{
  PNET_BUFFER_LIST v7; // rbx
  struct _NET_BUFFER_LIST *Alignment; // rsi
  $CD907EB97BD987F691570AC389926C95 *FirstNetBuffer; // rdi
  _QWORD *result; // rax
  unsigned __int64 v11; // [rsp+38h] [rbp-40h]

  if ( (unsigned __int8)byte_1C009874A >= 4u )
    WPP_SF_qq(98LL, &WPP_adcd8872ba6737e2e82ea4a41ce92341_Traceguids, NdisFilterHandle, NetBufferList);
  v7 = NetBufferList;
  do
  {
    Alignment = (struct _NET_BUFFER_LIST *)v7->Link.Alignment;
    v11 = v7->Link.Alignment;
    if ( (v7->Flags & 0x800) == 0 )
    {
      FirstNetBuffer = ($CD907EB97BD987F691570AC389926C95 *)v7->FirstNetBuffer;
      do
      {
        ndisFIsLoopbackNetBuffer(NdisFilterHandle);
        FirstNetBuffer = ($CD907EB97BD987F691570AC389926C95 *)FirstNetBuffer->Link.Alignment;
      }
      while ( FirstNetBuffer );
      Alignment = (struct _NET_BUFFER_LIST *)v11;
    }
    v7 = Alignment;
  }
  while ( Alignment );
  result = a5;
  *a5 = NetBufferList;
  if ( (unsigned __int8)byte_1C009874A >= 4u )
    return (_QWORD *)WPP_SF_qq(99LL, &WPP_adcd8872ba6737e2e82ea4a41ce92341_Traceguids, NdisFilterHandle, NetBufferList);
  return result;
}
