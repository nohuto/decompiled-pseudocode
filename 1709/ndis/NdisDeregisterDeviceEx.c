/*
 * XREFs of NdisDeregisterDeviceEx @ 0x1C0002690
 * Callers:
 *     <none>
 * Callees:
 *     WPP_SF_q @ 0x1C0039450 (WPP_SF_q.c)
 *     ndisDereferencePackage @ 0x1C00B837C (ndisDereferencePackage.c)
 *     ndisReferencePackage @ 0x1C00B83A0 (ndisReferencePackage.c)
 */

void __stdcall NdisDeregisterDeviceEx(NDIS_HANDLE NdisDeviceHandle)
{
  char *v2; // rcx
  char v3; // al
  KSPIN_LOCK *v4; // rdi
  KIRQL v5; // al
  NDIS_HANDLE *v6; // r9
  NDIS_HANDLE *v7; // r8
  KSPIN_LOCK *v8; // rcx
  NDIS_HANDLE *v9; // r9
  NDIS_HANDLE *v10; // rdx
  __int64 v11; // rdi
  NDIS_HANDLE *v12; // r9
  NDIS_HANDLE *v13; // r8

  if ( (unsigned __int8)ndisWppEnabledLevelPerFlag >= 4u )
    WPP_SF_q(106LL, &WPP_94bf37800e1134dcb4e413b6850d45c3_Traceguids, NdisDeviceHandle);
  ndisReferencePackage(&ndisPkgs);
  v2 = (char *)*((_QWORD *)NdisDeviceHandle + 3);
  v3 = *v2;
  if ( *v2 == 2 )
  {
    v4 = (KSPIN_LOCK *)(v2 + 392);
    v5 = KeAcquireSpinLockRaiseToDpc((PKSPIN_LOCK)v2 + 49);
    v6 = (NDIS_HANDLE *)*((_QWORD *)NdisDeviceHandle + 1);
    if ( v6[1] != (char *)NdisDeviceHandle + 8
      || (v7 = (NDIS_HANDLE *)*((_QWORD *)NdisDeviceHandle + 2), *v7 != (char *)NdisDeviceHandle + 8) )
    {
      __fastfail(3u);
    }
LABEL_6:
    *v7 = v6;
    v6[1] = v7;
LABEL_7:
    v8 = v4;
LABEL_8:
    KeReleaseSpinLock(v8, v5);
    goto LABEL_9;
  }
  switch ( v3 )
  {
    case 4:
      v4 = (KSPIN_LOCK *)(v2 + 40);
      v5 = KeAcquireSpinLockRaiseToDpc((PKSPIN_LOCK)v2 + 5);
      v6 = (NDIS_HANDLE *)*((_QWORD *)NdisDeviceHandle + 1);
      if ( v6[1] != (char *)NdisDeviceHandle + 8
        || (v7 = (NDIS_HANDLE *)*((_QWORD *)NdisDeviceHandle + 2), *v7 != (char *)NdisDeviceHandle + 8) )
      {
        __fastfail(3u);
      }
      goto LABEL_6;
    case 17:
      v11 = *((_QWORD *)v2 + 473);
      v5 = KeAcquireSpinLockRaiseToDpc((PKSPIN_LOCK)(v11 + 392));
      v12 = (NDIS_HANDLE *)*((_QWORD *)NdisDeviceHandle + 1);
      if ( v12[1] != (char *)NdisDeviceHandle + 8
        || (v13 = (NDIS_HANDLE *)*((_QWORD *)NdisDeviceHandle + 2), *v13 != (char *)NdisDeviceHandle + 8) )
      {
        __fastfail(3u);
      }
      *v13 = v12;
      v8 = (KSPIN_LOCK *)(v11 + 392);
      v12[1] = v13;
      goto LABEL_8;
    case -86:
      v4 = (KSPIN_LOCK *)((char *)IoGetDriverObjectExtension(**((PDRIVER_OBJECT **)v2 + 1), (PVOID)0x4E4D4944) + 392);
      v5 = KeAcquireSpinLockRaiseToDpc(v4);
      v9 = (NDIS_HANDLE *)*((_QWORD *)NdisDeviceHandle + 1);
      if ( v9[1] != (char *)NdisDeviceHandle + 8
        || (v10 = (NDIS_HANDLE *)*((_QWORD *)NdisDeviceHandle + 2), *v10 != (char *)NdisDeviceHandle + 8) )
      {
        __fastfail(3u);
      }
      *v10 = v9;
      v9[1] = v10;
      goto LABEL_7;
  }
LABEL_9:
  ndisDereferencePackage(&ndisPkgs);
  IoDeleteSymbolicLink((PUNICODE_STRING)NdisDeviceHandle + 18);
  IoDeleteDevice(*((PDEVICE_OBJECT *)NdisDeviceHandle + 4));
  if ( (unsigned __int8)ndisWppEnabledLevelPerFlag >= 4u )
    WPP_SF_q(107LL, &WPP_94bf37800e1134dcb4e413b6850d45c3_Traceguids, NdisDeviceHandle);
}
