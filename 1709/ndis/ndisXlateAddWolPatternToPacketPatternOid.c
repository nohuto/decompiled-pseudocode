/*
 * XREFs of ndisXlateAddWolPatternToPacketPatternOid @ 0x1C00E2574
 * Callers:
 *     ndisOidPrePMAddWOLPattern @ 0x1C00ADAD0 (ndisOidPrePMAddWOLPattern.c)
 * Callees:
 *     memmove @ 0x1C0024F40 (memmove.c)
 *     memset @ 0x1C0025280 (memset.c)
 *     WPP_SF_q @ 0x1C0039450 (WPP_SF_q.c)
 *     WPP_SF_qD @ 0x1C0039484 (WPP_SF_qD.c)
 *     ??$?8U_NDIS_PM_WOL_PATTERN@@U?$KFreePool@U_NDIS_PM_WOL_PATTERN@@@@@wistd@@YA_NAEBV?$unique_ptr@U_NDIS_PM_WOL_PATTERN@@U?$KFreePool@U_NDIS_PM_WOL_PATTERN@@@@@0@$$T@Z @ 0x1C0048070 (--$-8U_NDIS_PM_WOL_PATTERN@@U-$KFreePool@U_NDIS_PM_WOL_PATTERN@@@@@wistd@@YA_NAEBV-$unique_ptr@U.c)
 *     ??$MakeExtendedPoolPtr@U_NDIS_PM_PACKET_PATTERN@@@@YA?AV?$unique_ptr@U_NDIS_PM_PACKET_PATTERN@@U?$KFreePool@U_NDIS_PM_PACKET_PATTERN@@@@@wistd@@KKPEAK@Z @ 0x1C00DED18 (--$MakeExtendedPoolPtr@U_NDIS_PM_PACKET_PATTERN@@@@YA-AV-$unique_ptr@U_NDIS_PM_PACKET_PATTERN@@U.c)
 */

__int64 __fastcall ndisXlateAddWolPatternToPacketPatternOid(__int64 a1, _BYTE *a2)
{
  _DWORD *v2; // r14
  unsigned int v3; // esi
  int v6; // r9d
  _DWORD *PoolWithTag; // rax
  _DWORD *v8; // rdi
  __int64 v9; // rdx
  unsigned int v10; // ecx
  PVOID v11; // rcx
  _DWORD *v12; // rbx
  int v13; // eax
  unsigned int v15; // [rsp+40h] [rbp+8h] BYREF
  PVOID P; // [rsp+48h] [rbp+10h] BYREF

  v2 = *(_DWORD **)(a1 + 40);
  v3 = 0;
  v15 = 0;
  *a2 = 0;
  v6 = v2[3];
  if ( v6 != 1 )
  {
    if ( (unsigned __int8)byte_1C0098752 >= 4u )
      WPP_SF_qD(0x6Fu, &WPP_02d7300eb5383415542bdbf7dfb5e076_Traceguids, a1, v6);
    *a2 = 1;
    return (unsigned int)-1073741637;
  }
  PoolWithTag = ExAllocatePoolWithTag(NonPagedPoolNx, 0x20uLL, 0x7877444Eu);
  v8 = PoolWithTag;
  if ( PoolWithTag )
    memset(PoolWithTag, 0, 0x20uLL);
  P = v8;
  if ( !wistd::operator==<_NDIS_PM_WOL_PATTERN,KFreePool<_NDIS_PM_WOL_PATTERN>>(&P) )
  {
    *v8 = *(_DWORD *)(a1 + 32);
    *((_QWORD *)v8 + 1) = *(_QWORD *)(a1 + 40);
    v8[4] = *(_DWORD *)(a1 + 48);
    v9 = (unsigned int)v2[41];
    v10 = v9 + v2[43];
    if ( v10 < (unsigned int)v9 )
    {
      if ( (unsigned __int8)byte_1C0098752 >= 4u )
        WPP_SF_q(0x71u, &WPP_02d7300eb5383415542bdbf7dfb5e076_Traceguids, a1);
      v3 = -1073741811;
      *a2 = 1;
    }
    else
    {
      MakeExtendedPoolPtr<_NDIS_PM_PACKET_PATTERN>(&P, v9, v10, &v15);
      if ( !wistd::operator==<_NDIS_PM_WOL_PATTERN,KFreePool<_NDIS_PM_WOL_PATTERN>>(&P) )
      {
        v12 = P;
        *((_DWORD *)P + 1) = v2[37];
        *v12 = v2[2];
        v13 = v2[41];
        v12[2] = v13;
        v12[3] = v13 + 24;
        v12[4] = v2[43];
        memmove(v12 + 6, (char *)v2 + (unsigned int)v2[40], (unsigned int)v12[2]);
        memmove((char *)v12 + (unsigned int)v12[3], (char *)v2 + (unsigned int)v2[42], (unsigned int)v12[4]);
        *(_DWORD *)(a1 + 48) = v15;
        *(_DWORD *)(a1 + 32) = -50265853;
        *(_QWORD *)(a1 + 40) = v12;
        *((_QWORD *)v8 + 3) = v12;
        *(_QWORD *)(a1 + 144) = v8;
        return v3;
      }
      if ( (unsigned __int8)byte_1C0098752 >= 4u )
        WPP_SF_q(0x72u, &WPP_02d7300eb5383415542bdbf7dfb5e076_Traceguids, a1);
      v11 = P;
      v3 = -1073741670;
      *a2 = 1;
      if ( v11 )
        ExFreePoolWithTag(v11, 0);
    }
    goto LABEL_11;
  }
  if ( (unsigned __int8)byte_1C0098752 >= 4u )
    WPP_SF_q(0x70u, &WPP_02d7300eb5383415542bdbf7dfb5e076_Traceguids, a1);
  *a2 = 1;
  v3 = -1073741670;
  if ( v8 )
LABEL_11:
    ExFreePoolWithTag(v8, 0);
  return v3;
}
