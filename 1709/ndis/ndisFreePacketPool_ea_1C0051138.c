/*
 * XREFs of ndisFreePacketPool @ 0x1C0051138
 * Callers:
 *     NdisFreePacketPool @ 0x1C0050D40 (NdisFreePacketPool.c)
 *     ndisVerifierFreePacketPool @ 0x1C0066300 (ndisVerifierFreePacketPool.c)
 * Callees:
 *     ?ndisFreePacketPoolHdr@@YAXPEAU_NDIS_PKT_POOL_HDR@@PEAU_NDIS_PKT_POOL@@@Z @ 0x1C0050644 (-ndisFreePacketPoolHdr@@YAXPEAU_NDIS_PKT_POOL_HDR@@PEAU_NDIS_PKT_POOL@@@Z.c)
 */
...
void __fastcall ndisFreePacketPool(struct _NDIS_PKT_POOL *a1)
{
  ...
  while ( p_AgingBlocks->Flink != p_AgingBlocks )
  {
    ...
    if ( p_AgingBlocks->Flink->Blink != p_AgingBlocks
  ...
  while ( p_FreeBlocks->Flink != p_FreeBlocks )
  {
    ...
    if ( p_FreeBlocks->Flink->Blink != p_FreeBlocks || (v8 = *(_LIST_ENTRY **)v7, *(char **)(*(_QWORD *)v7 + 8LL) != v7) )
      __fastfail(3u);
  ...
  while ( p_UsedBlocks->Flink != p_UsedBlocks )
  {
    ...
    if ( p_UsedBlocks->Flink->Blink != p_UsedBlocks
      || (v11 = *(_LIST_ENTRY **)v10, *(char **)(*(_QWORD *)v10 + 8LL) != v10) )
    {
      __fastfail(3u);
    }
