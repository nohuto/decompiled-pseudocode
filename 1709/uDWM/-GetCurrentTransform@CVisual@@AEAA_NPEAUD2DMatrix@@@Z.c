/*
 * XREFs of ?GetCurrentTransform@CVisual@@AEAA_NPEAUD2DMatrix@@@Z @ 0x180009EC4
 * Callers:
 *     ?GetLocalToWorldTransform@CVisual@@QEAAXPEAUD2DMatrix@@@Z @ 0x18000A034 (-GetLocalToWorldTransform@CVisual@@QEAAXPEAUD2DMatrix@@@Z.c)
 *     ?DoHitTest@CVisual@@UEAA_NAEBUtagPOINT@@PEAPEAV1@PEAI@Z @ 0x180018E90 (-DoHitTest@CVisual@@UEAA_NAEBUtagPOINT@@PEAPEAV1@PEAI@Z.c)
 *     ?DoHitTest@CTopLevelWindow@@UEAA_NAEBUtagPOINT@@PEAPEAVCVisual@@PEAI@Z @ 0x18001D510 (-DoHitTest@CTopLevelWindow@@UEAA_NAEBUtagPOINT@@PEAPEAVCVisual@@PEAI@Z.c)
 *     ?ProcessSyncDwmMessage@CWindowList@@UEAAJW4DWMCMD@@PEAXI_NKPEBU_REMOTE_PORT_VIEW@@PEAJPEAI@Z @ 0x18002B020 (-ProcessSyncDwmMessage@CWindowList@@UEAAJW4DWMCMD@@PEAXI_NKPEBU_REMOTE_PORT_VIEW@@PEAJPEAI@Z.c)
 *     ?UpdateTransform@CVisual@@AEAAJXZ @ 0x180079978 (-UpdateTransform@CVisual@@AEAAJXZ.c)
 * Callees:
 *     ?D2DMatrixMultiply@@YAPEAUD2DMatrix@@PEAU1@PEBU1@1@Z @ 0x180024C90 (-D2DMatrixMultiply@@YAPEAUD2DMatrix@@PEAU1@PEBU1@1@Z.c)
 *     _guard_dispatch_icall_nop @ 0x180046790 (_guard_dispatch_icall_nop.c)
 *     ?D2DMatrixRotationZ@@YAPEAUD2DMatrix@@PEAU1@M@Z @ 0x1800A0700 (-D2DMatrixRotationZ@@YAPEAUD2DMatrix@@PEAU1@M@Z.c)
 */

char __fastcall CVisual::GetCurrentTransform(CVisual *this, struct D2DMatrix *a2)
{
  char v4; // si
  char v5; // r14
  char v6; // r15
  float v7; // xmm0_4
  float v8; // xmm2_4
  float v9; // xmm2_4
  char v10; // r9
  char v11; // r9
  double v13; // xmm5_8
  float v14; // xmm2_4
  float v15; // xmm2_4
  double v16; // xmm1_8
  float v17; // xmm0_4
  float v18; // xmm0_4
  double v19; // xmm0_8
  double v20; // xmm1_8
  double v21; // xmm0_8
  double v22; // xmm1_8
  float v23; // [rsp+20h] [rbp-40h] BYREF
  int v24; // [rsp+24h] [rbp-3Ch]
  int v25; // [rsp+28h] [rbp-38h]
  int v26; // [rsp+2Ch] [rbp-34h]
  int v27; // [rsp+30h] [rbp-30h]
  float v28; // [rsp+34h] [rbp-2Ch]
  int v29; // [rsp+38h] [rbp-28h]
  int v30; // [rsp+3Ch] [rbp-24h]
  int v31; // [rsp+40h] [rbp-20h]
  int v32; // [rsp+44h] [rbp-1Ch]
  int v33; // [rsp+48h] [rbp-18h]
  int v34; // [rsp+4Ch] [rbp-14h]
  float v35; // [rsp+50h] [rbp-10h]
  float v36; // [rsp+54h] [rbp-Ch]
  int v37; // [rsp+58h] [rbp-8h]
  int v38; // [rsp+5Ch] [rbp-4h]

  v4 = 0;
  v5 = 0;
  v6 = 0;
  if ( (*((_BYTE *)this + 84) & 1) != 0 )
  {
    (*(void (__fastcall **)(CVisual *))(*(_QWORD *)this + 152LL))(this);
    v4 = 1;
  }
  else
  {
    *(_QWORD *)((char *)a2 + 52) = 0LL;
    *((_DWORD *)a2 + 15) = 1065353216;
    *((_DWORD *)a2 + 10) = 1065353216;
    *((_DWORD *)a2 + 5) = 1065353216;
    *(_DWORD *)a2 = 1065353216;
    *(_QWORD *)((char *)a2 + 44) = 0LL;
    *((_QWORD *)a2 + 4) = 0LL;
    *((_QWORD *)a2 + 3) = 0LL;
    *(_QWORD *)((char *)a2 + 12) = 0LL;
    *(_QWORD *)((char *)a2 + 4) = 0LL;
  }
  v7 = COERCE_DOUBLE(COERCE_UNSIGNED_INT64((float)((float)*((double *)this + 21) - 1.0)) & _xmm);
  if ( v7 > 0.0000011920929
    || (v8 = COERCE_DOUBLE(COERCE_UNSIGNED_INT64((float)((float)*((double *)this + 22) - 1.0)) & _xmm),
        v8 > 0.0000011920929) )
  {
    v5 = 1;
    v4 = 1;
  }
  v9 = COERCE_DOUBLE(COERCE_UNSIGNED_INT64((float)*((double *)this + 20)) & _xmm);
  if ( v9 > 0.0000011920929 )
  {
    v10 = 0;
    v4 = 1;
  }
  else
  {
    v10 = 1;
  }
  v11 = v10 ^ 1;
  if ( v5 || v11 )
  {
    v13 = *((double *)this + 18);
    v14 = COERCE_DOUBLE(COERCE_UNSIGNED_INT64((float)v13) & _xmm);
    if ( v14 > 0.0000011920929
      || (v15 = COERCE_DOUBLE(COERCE_UNSIGNED_INT64((float)*((double *)this + 19)) & _xmm), v15 > 0.0000011920929) )
    {
      *(_QWORD *)&v16 = *((_QWORD *)this + 19) ^ _xmm;
      v34 = 0;
      v32 = 0;
      v31 = 0;
      v30 = 0;
      v29 = 0;
      v27 = 0;
      v26 = 0;
      v25 = 0;
      v24 = 0;
      v17 = COERCE_DOUBLE(*(_QWORD *)&v13 ^ _xmm);
      v38 = 1065353216;
      v33 = 1065353216;
      v28 = 1.0;
      v23 = 1.0;
      v37 = 1065353216;
      v35 = v17;
      v18 = v16;
      v36 = v18;
      D2DMatrixMultiply(a2, a2, (const struct D2DMatrix *)&v23);
      v6 = 1;
    }
    if ( v5 )
    {
      v19 = *((double *)this + 21);
      v20 = *((double *)this + 22);
      v37 = 0;
      v36 = 0.0;
      v35 = 0.0;
      v34 = 0;
      v32 = 0;
      v31 = 0;
      v30 = 0;
      v29 = 0;
      v27 = 0;
      v26 = 0;
      v25 = 0;
      v24 = 0;
      v33 = 1065353216;
      v38 = 1065353216;
      v23 = v19;
      v28 = v20;
      D2DMatrixMultiply(a2, a2, (const struct D2DMatrix *)&v23);
    }
    if ( v11 )
    {
      D2DMatrixRotationZ((struct D2DMatrix *)&v23, *((double *)this + 20) * 0.01745329238474369);
      D2DMatrixMultiply(a2, a2, (const struct D2DMatrix *)&v23);
    }
    if ( v6 )
    {
      v21 = *((double *)this + 18);
      v22 = *((double *)this + 19);
      v34 = 0;
      v32 = 0;
      v31 = 0;
      v30 = 0;
      v29 = 0;
      v27 = 0;
      v26 = 0;
      v25 = 0;
      v24 = 0;
      v38 = 1065353216;
      v33 = 1065353216;
      v28 = 1.0;
      v23 = 1.0;
      v37 = 1065353216;
      v35 = v21;
      v36 = v22;
      D2DMatrixMultiply(a2, a2, (const struct D2DMatrix *)&v23);
    }
  }
  return v4;
}
