/*
 * XREFs of ?Initialize@CFlickVisual@@MEAAJPEAUIDwmChannel@@@Z @ 0x180088CC0
 * Callers:
...
__int64 __fastcall CFlickVisual::Initialize(struct CImage **this, struct IDwmChannel *a2)
{
  ...
  if ( v4 >= 0 )
  {
    ...
    if ( v6 >= 0 )
    {
      ...
      {
        MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0, inserted, 0x27u);
      }
    }
    else
    {
      MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0, v6, 0x26u);
    }
  }
  else
  {
