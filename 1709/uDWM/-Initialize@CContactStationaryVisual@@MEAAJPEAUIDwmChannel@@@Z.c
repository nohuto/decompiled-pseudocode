/*
 * XREFs of ?Initialize@CContactStationaryVisual@@MEAAJPEAUIDwmChannel@@@Z @ 0x1800847E0
 * Callers:
...
__int64 __fastcall CContactStationaryVisual::Initialize(struct CImage **this, struct IDwmChannel *a2)
{
  ...
  if ( v4 >= 0 )
  {
    ...
    if ( v6 >= 0 )
    {
      ...
      {
        MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0, inserted, 0x3Fu);
      }
    }
    else
    {
      MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0, v6, 0x3Eu);
    }
  }
  else
  {
