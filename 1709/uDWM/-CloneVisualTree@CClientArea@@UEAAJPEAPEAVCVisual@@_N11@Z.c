/*
 * XREFs of ?CloneVisualTree@CClientArea@@UEAAJPEAPEAVCVisual@@_N11@Z @ 0x18002C4C0
 * Callers:
 *     ?CloneVisualTree@CVisual@@UEAAJPEAPEAV1@_N11@Z @ 0x180018840 (-CloneVisualTree@CVisual@@UEAAJPEAPEAV1@_N11@Z.c)
 * Callees:
 *     ?CloneVisualTree@CVisual@@UEAAJPEAPEAV1@_N11@Z @ 0x180018840 (-CloneVisualTree@CVisual@@UEAAJPEAPEAV1@_N11@Z.c)
 *     ?MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJI@Z @ 0x18002BDF0 (-MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJI@Z.c)
 *     ?Create@CClientArea@@SAJPEAUIDwmChannel@@IPEAPEAV1@@Z @ 0x18002C3AC (-Create@CClientArea@@SAJPEAUIDwmChannel@@IPEAPEAV1@@Z.c)
 *     _guard_dispatch_icall_nop @ 0x180046790 (_guard_dispatch_icall_nop.c)
 */

__int64 __fastcall CClientArea::CloneVisualTree(CClientArea *this, struct CVisual **a2)
{
  __int64 v2; // rax
  unsigned int v3; // ebx
  __int64 v6; // rcx
  int v7; // eax
  __int64 v8; // rax
  int v9; // eax
  int v10; // eax
  struct CClientArea *v12; // [rsp+30h] [rbp-28h] BYREF
  unsigned int v13; // [rsp+60h] [rbp+8h] BYREF

  v2 = *((_QWORD *)this + 30);
  v3 = 0;
  v13 = 0;
  if ( v2 )
  {
    v6 = *(_QWORD *)(v2 + 24);
    if ( v6 )
    {
      v7 = (*(__int64 (__fastcall **)(__int64, _QWORD, unsigned int *))(*(_QWORD *)v6 + 320LL))(
             v6,
             *(_QWORD *)(*((_QWORD *)this + 2) + 16LL),
             &v13);
      v3 = v7;
      if ( v7 < 0 )
      {
        MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0, v7, 0x84u);
      }
      else
      {
        v8 = *((_QWORD *)this + 2);
        v12 = 0LL;
        v9 = CClientArea::Create(*(struct IDwmChannel **)(v8 + 16), v13, &v12);
        v3 = v9;
        if ( v9 < 0 )
        {
          MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0, v9, 0x88u);
        }
        else
        {
          *a2 = v12;
          v10 = CVisual::CloneVisualTree(this, a2);
          v3 = v10;
          if ( v10 < 0 )
            MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0, v10, 0x93u);
        }
      }
    }
  }
  (*(void (__fastcall **)(_QWORD, _QWORD))(**(_QWORD **)(*((_QWORD *)this + 2) + 16LL) + 128LL))(
    *(_QWORD *)(*((_QWORD *)this + 2) + 16LL),
    v13);
  return v3;
}
