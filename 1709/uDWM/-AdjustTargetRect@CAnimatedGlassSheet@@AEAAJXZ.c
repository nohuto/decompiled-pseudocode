/*
 * XREFs of ?AdjustTargetRect@CAnimatedGlassSheet@@AEAAJXZ @ 0x18007DD00
 * Callers:
  ...
  if ( v4 )
  {
    ...
    if ( GetMonitorInfoW(v4, &mi) )
    {
      ...
      if ( mi.rcWork.left == left )
        this[26].left = left - (int)(*((double *)CDesktopManager::s_pDesktopManagerInstance + 55) * -8.0);
      right = this[26].right;
      if ( mi.rcWork.right == right )
        this[26].right = right - (int)(*((double *)v7 + 55) * 8.0);
      top = this[26].top;
      if ( mi.rcWork.top == top )
        this[26].top = top - (int)(*((double *)v7 + 56) * -8.0);
      bottom = this[26].bottom;
      if ( mi.rcWork.bottom == bottom )
        this[26].bottom = bottom - (int)(*((double *)v7 + 56) * 8.0);
    ...
    else
    {
      ...
      v3 = LastError;
      MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0, LastError, 0x232u);
    }
  }
  return v3;
}
