/*
 * XREFs of ?Create@InputRedirectionComponent@@SAJPEAUIInputDeviceInfoStore@@PEAPEAV1@@Z @ 0x1800A6F3C
 * Callers:
...
__int64 __fastcall InputRedirectionComponent::Create(
        ...
        struct InputRedirectionComponent **a2)
{
  int v4; // ebx
  int v5; // r9d
  ...
  if ( a2 )
  {
    v6 = (InputRedirectionComponent *)malloc(0x90uLL);
    v7 = v6;
    if ( v6 )
      memset(v6, 0, 0x90uLL);
    if ( v7 )
    ...
    {
      McTemplateU0qqq(v10, &MinInput_Warning_CheckResult, 0, 101, v9);
    }
    if ( v4 < 0 )
