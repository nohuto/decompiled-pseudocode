/*
 * XREFs of ?LogMPCClickerInputReport@ISMTracing@@SAXPEAUInputInfo@@AEB_N@Z @ 0x18005F6B8
 * Callers:
 *     ?OnInputReport@MPCInputProviderBase@@UEAAJPEAUInputInfo@@@Z @ 0x1800600C0 (-OnInputReport@MPCInputProviderBase@@UEAAJPEAUInputInfo@@@Z.c)
 * Callees:
 *     _TlgWrite @ 0x180001118 (_TlgWrite.c)
 *     ?Instance@ISMTracing@@KAPEAV1@XZ @ 0x180049F08 (-Instance@ISMTracing@@KAPEAV1@XZ.c)
 *     __security_check_cookie @ 0x1800CAB30 (__security_check_cookie.c)
 */

void __fastcall ISMTracing::LogMPCClickerInputReport(struct InputInfo *a1, const bool *a2)
{
  __int64 v4; // rcx
  __int64 v5; // rcx
  int v6; // [rsp+30h] [rbp-49h] BYREF
  int v7; // [rsp+34h] [rbp-45h] BYREF
  int v8; // [rsp+38h] [rbp-41h] BYREF
  EVENT_DATA_DESCRIPTOR pData; // [rsp+40h] [rbp-39h] BYREF
  int *v10; // [rsp+60h] [rbp-19h]
  __int64 v11; // [rsp+68h] [rbp-11h]
  int *v12; // [rsp+70h] [rbp-9h]
  __int64 v13; // [rsp+78h] [rbp-1h]
  char *v14; // [rsp+80h] [rbp+7h]
  __int64 v15; // [rsp+88h] [rbp+Fh]
  const bool *v16; // [rsp+90h] [rbp+17h]
  __int64 v17; // [rsp+98h] [rbp+1Fh]
  char *v18; // [rsp+A0h] [rbp+27h]
  __int64 v19; // [rsp+A8h] [rbp+2Fh]
  int *v20; // [rsp+B0h] [rbp+37h]
  __int64 v21; // [rsp+B8h] [rbp+3Fh]

  if ( (*((_BYTE *)a1 + 700) & 7) != 0 )
  {
    v4 = *((_QWORD *)ISMTracing::Instance() + 1);
    if ( *(_DWORD *)v4 > 5u && (*(_BYTE *)(v4 + 16) & 1) != 0 && (*(_QWORD *)(v4 + 24) & 1LL) == *(_QWORD *)(v4 + 24) )
    {
      v6 = *(_DWORD *)a1;
      v10 = &v6;
      v7 = *((_DWORD *)a1 + 1);
      v12 = &v7;
      v14 = (char *)a1 + 8;
      v18 = (char *)a1 + 704;
      v8 = *((_DWORD *)a1 + 175);
      v20 = &v8;
      v11 = 4LL;
      v13 = 4LL;
      v15 = 4LL;
      v16 = a2;
      v17 = 1LL;
      v19 = 4LL;
      v21 = 4LL;
      TlgWrite((TraceLoggingHProvider)v4, &unk_1800EEF50, 0LL, 0LL, 8u, &pData);
    }
  }
  else
  {
    v5 = *((_QWORD *)ISMTracing::Instance() + 1);
    if ( *(_DWORD *)v5 > 4u && (*(_BYTE *)(v5 + 16) & 1) != 0 && (*(_QWORD *)(v5 + 24) & 1LL) == *(_QWORD *)(v5 + 24) )
    {
      v8 = *(_DWORD *)a1;
      v10 = &v8;
      v7 = *((_DWORD *)a1 + 1);
      v12 = &v7;
      v14 = (char *)a1 + 8;
      v18 = (char *)a1 + 704;
      v6 = *((_DWORD *)a1 + 175);
      v20 = &v6;
      v11 = 4LL;
      v13 = 4LL;
      v15 = 4LL;
      v16 = a2;
      v17 = 1LL;
      v19 = 4LL;
      v21 = 4LL;
      TlgWrite((TraceLoggingHProvider)v5, &unk_1800EEE4C, 0LL, 0LL, 8u, &pData);
    }
  }
}
