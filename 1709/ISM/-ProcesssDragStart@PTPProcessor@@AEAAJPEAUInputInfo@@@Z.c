/*
 * XREFs of ?ProcesssDragStart@PTPProcessor@@AEAAJPEAUInputInfo@@@Z @ 0x18006EC88
 * Callers:
 *     ?OnInputReport@PTPProcessor@@UEAAJPEAUInputInfo@@@Z @ 0x18006E8A0 (-OnInputReport@PTPProcessor@@UEAAJPEAUInputInfo@@@Z.c)
 * Callees:
 *     McTemplateU0qqq @ 0x180001C0C (McTemplateU0qqq.c)
 *     ?UpdateLastRecognizedPTPGesture@PTPProcessor@@AEAAXK@Z @ 0x18006EBD8 (-UpdateLastRecognizedPTPGesture@PTPProcessor@@AEAAXK@Z.c)
 *     ?DeliverMouseMessage@PTPProcessor@@AEAAJPEAUInputInfo@@G_N@Z @ 0x18006F1C8 (-DeliverMouseMessage@PTPProcessor@@AEAAJPEAUInputInfo@@G_N@Z.c)
 *     ?DeliverTouchpadGestureMessage@PTPProcessor@@AEAAJPEAUInputInfo@@_N1@Z @ 0x18006F2A8 (-DeliverTouchpadGestureMessage@PTPProcessor@@AEAAJPEAUInputInfo@@_N1@Z.c)
 */

__int64 __fastcall PTPProcessor::ProcesssDragStart(PTPProcessor *this, struct InputInfo *a2)
{
  int v2; // eax
  unsigned int v3; // ebx
  int v5; // eax
  __int64 v6; // rcx
  int v7; // r9d

  v2 = *((_DWORD *)this + 150);
  v3 = 0;
  if ( (v2 & 0x40) == 0 )
  {
    if ( (v2 & 0x100000) != 0 )
    {
      *((_DWORD *)this + 149) = 2;
      v5 = PTPProcessor::DeliverMouseMessage(this, a2, 1u, 1);
      v3 = v5;
      if ( v5 < 0 )
      {
        if ( (Microsoft_OneCore_MinInputEnableBits & 2) == 0 )
          return v3;
        v7 = 467;
        goto LABEL_5;
      }
    }
    else if ( (v2 & 0x2080) != 0 )
    {
      *((_DWORD *)this + 149) = 3;
      v5 = PTPProcessor::DeliverTouchpadGestureMessage(this, a2, 0, 1);
      v3 = v5;
      if ( v5 < 0 )
      {
        if ( (Microsoft_OneCore_MinInputEnableBits & 2) == 0 )
          return v3;
        v7 = 472;
        goto LABEL_5;
      }
    }
    else if ( (v2 & 0x400000) != 0 )
    {
      *((_DWORD *)this + 149) = 4;
      v5 = PTPProcessor::DeliverTouchpadGestureMessage(this, a2, 0, 1);
      v3 = v5;
      if ( v5 < 0 )
      {
        if ( (Microsoft_OneCore_MinInputEnableBits & 2) == 0 )
          return v3;
        v7 = 477;
        goto LABEL_5;
      }
    }
LABEL_18:
    PTPProcessor::UpdateLastRecognizedPTPGesture(this, *((_DWORD *)this + 150));
    return v3;
  }
  *((_DWORD *)this + 149) = 1;
  v5 = PTPProcessor::DeliverMouseMessage(this, a2, 0, 1);
  v3 = v5;
  if ( v5 >= 0 )
    goto LABEL_18;
  if ( (Microsoft_OneCore_MinInputEnableBits & 2) != 0 )
  {
    v7 = 462;
LABEL_5:
    McTemplateU0qqq(v6, &MinInput_Warning_CheckResult, 11, v7, v5);
  }
  return v3;
}
