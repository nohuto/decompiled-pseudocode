/*
 * XREFs of ?OpenStoreKey@CAudioSessionStore@@AEAAJXZ @ 0x18008644C
 * Callers:
 ...
 *     WPP_SF_qSSq @ 0x180089F7C (WPP_SF_qSSq.c)
 */
...
LABEL_23:
  if ( hKey )
    RegCloseKey(hKey);
  if ( ThreadUserStringSid < 0 )
    AudSrvTraceLoggingErrorHelper("CAudioSessionStore::OpenStoreKey", 5950, ThreadUserStringSid);
  return (unsigned int)ThreadUserStringSid;
}
