/*
 * XREFs of Counter_CreateCommonBufferInstance @ 0x1C005F770
 * Callers:
...
NTSTATUS __fastcall Counter_CreateCommonBufferInstance(PPCW_INSTANCE *a1)
{
  ...
  v3 = *a1;
  if ( *(_BYTE *)(WPP_MAIN_CB.DeviceQueue.Lock + 28) && !a1[2] )
  {
    Data = *((_DWORD *)v3 + 44);
    if ( *((_DWORD *)v3 + 45) == 1 )
    ...
    return PcwCreateInstance(
             a1 + 2,
             (PPCW_REGISTRATION)WPP_MAIN_CB.DeviceQueue.DeviceListHead.Blink,
             &DestinationString,
             1u,
             &v5);
  }
