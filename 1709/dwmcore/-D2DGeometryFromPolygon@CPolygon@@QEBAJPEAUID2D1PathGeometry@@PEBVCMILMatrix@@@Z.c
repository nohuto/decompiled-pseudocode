/*
 * XREFs of ?D2DGeometryFromPolygon@CPolygon@@QEBAJPEAUID2D1PathGeometry@@PEBVCMILMatrix@@@Z @ 0x18017E878
 * Callers:
 *     ?DrawBspPolygonList@CDrawingContext@@QEAAJPEAVCVisual@@0@Z @ 0x18013C558 (-DrawBspPolygonList@CDrawingContext@@QEAAJPEAVCVisual@@0@Z.c)
 * Callees:
 *     ?MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJI@Z @ 0x180076954 (-MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJI@Z.c)
 *     ?Multiply@CMILMatrix@@SAXAEBV1@0PEAV1@@Z @ 0x18007B590 (-Multiply@CMILMatrix@@SAXAEBV1@0PEAV1@@Z.c)
 *     ??$ReleaseInterface@UID2D1PrivateCompositorBuffer@@@@YAXAEAPEAUID2D1PrivateCompositorBuffer@@@Z @ 0x1800BC2AC (--$ReleaseInterface@UID2D1PrivateCompositorBuffer@@@@YAXAEAPEAUID2D1PrivateCompositorBuffer@@@Z.c)
 *     __security_check_cookie @ 0x1800C2B30 (__security_check_cookie.c)
 *     _guard_dispatch_icall_nop @ 0x1800C5DD0 (_guard_dispatch_icall_nop.c)
 *     ?Transform@CMILMatrix@@QEBAXPEBUMilPoint4F@@PEAU2@I@Z @ 0x1801908A8 (-Transform@CMILMatrix@@QEBAXPEBUMilPoint4F@@PEAU2@I@Z.c)
 */

__int64 __fastcall CPolygon::D2DGeometryFromPolygon(
        const struct MilPoint4F **this,
        struct ID2D1PathGeometry *a2,
        const struct CMILMatrix *a3)
{
  __int128 v3; // xmm0
  int v4; // eax
  __int128 v5; // xmm1
  __int128 v7; // xmm0
  __int128 v8; // xmm1
  __int64 v9; // r11
  __m128 v10; // xmm6
  __int64 (__fastcall *v11)(__int64, __int64 *); // rax
  __m128 v12; // xmm7
  signed int v13; // eax
  unsigned int v14; // ebx
  unsigned int v15; // eax
  __int64 v16; // rbx
  __int64 v17; // rdi
  __m128 v18; // xmm0
  __m128 v19; // xmm1
  void (__fastcall *v20)(__int64, unsigned __int64); // rax
  signed int v21; // eax
  __int64 v23[2]; // [rsp+38h] [rbp-59h] BYREF
  _OWORD v24[4]; // [rsp+48h] [rbp-49h] BYREF
  int v25; // [rsp+88h] [rbp-9h]
  float v26; // [rsp+98h] [rbp+7h] BYREF
  float v27; // [rsp+9Ch] [rbp+Bh]
  float v28; // [rsp+A4h] [rbp+13h]

  v3 = *(_OWORD *)a3;
  v4 = *((_DWORD *)a3 + 16);
  v5 = *((_OWORD *)a3 + 1);
  v23[0] = 0LL;
  v24[0] = v3;
  v7 = *((_OWORD *)a3 + 2);
  v25 = v4;
  v24[1] = v5;
  v8 = *((_OWORD *)a3 + 3);
  v24[2] = v7;
  v24[3] = v8;
  CMILMatrix::Multiply((const struct CMILMatrix *)(this + 18), (const struct CMILMatrix *)v24, (struct CMILMatrix *)v24);
  CMILMatrix::Transform((CMILMatrix *)v24, this[2], (struct MilPoint4F *)&v26, 1u);
  v10 = (__m128)LODWORD(v26);
  v12 = (__m128)LODWORD(v27);
  v10.m128_f32[0] = v26 / v28;
  v11 = *(__int64 (__fastcall **)(__int64, __int64 *))(*(_QWORD *)v9 + 136LL);
  v12.m128_f32[0] = v27 / v28;
  v26 = v26 / v28;
  v27 = v27 / v28;
  v13 = v11(v9, v23);
  v14 = v13;
  if ( v13 < 0 )
  {
    MilInstrumentationCheckHR_MaybeFailFast(20LL, 0LL, 0, v13, 0x22Bu);
  }
  else
  {
    (*(void (__fastcall **)(__int64, unsigned __int64, _QWORD))(*(_QWORD *)v23[0] + 40LL))(
      v23[0],
      _mm_unpacklo_ps(v10, v12).m128_u64[0],
      0LL);
    v15 = *((_DWORD *)this + 10);
    if ( v15 > 1 )
    {
      v16 = 20LL;
      v17 = v15 - 1;
      do
      {
        CMILMatrix::Transform(
          (CMILMatrix *)v24,
          (const struct MilPoint4F *)((char *)this[2] + v16),
          (struct MilPoint4F *)&v26,
          1u);
        v18 = (__m128)LODWORD(v26);
        v19 = (__m128)LODWORD(v27);
        v20 = *(void (__fastcall **)(__int64, unsigned __int64))(*(_QWORD *)v23[0] + 80LL);
        v26 = v26 / v28;
        v18.m128_f32[0] = v26;
        v27 = v27 / v28;
        v19.m128_f32[0] = v27;
        v20(v23[0], _mm_unpacklo_ps(v18, v19).m128_u64[0]);
        v16 += 20LL;
        --v17;
      }
      while ( v17 );
    }
    (*(void (__fastcall **)(__int64, __int64))(*(_QWORD *)v23[0] + 64LL))(v23[0], 1LL);
    v21 = (*(__int64 (__fastcall **)(__int64))(*(_QWORD *)v23[0] + 72LL))(v23[0]);
    v14 = v21;
    if ( v21 < 0 )
      MilInstrumentationCheckHR_MaybeFailFast(20LL, 0LL, 0, v21, 0x23Fu);
  }
  ReleaseInterface<ID2D1PrivateCompositorBuffer>(v23);
  return v14;
}
