/*
 * XREFs of ?AddCompSurfInfoUpdate@CComposition@@QEAAJPEAVCCompositionSurfaceInfo@@@Z @ 0x1800AB9FC
 * Callers:
...
__int64 __fastcall CComposition::AddCompSurfInfoUpdate(CComposition *this, struct CCompositionSurfaceInfo *a2)
{
  ...
  unsigned int v6; // edx
  unsigned int v7; // eax
  struct CCompositionSurfaceInfo **v9; // rax
  signed int v10; // eax
  ...
  if ( (_DWORD)v4 )
  {
    v9 = *(struct CCompositionSurfaceInfo ***)v2;
    while ( a2 != *v9 )
    {
      ++v5;
      ++v9;
...
LABEL_2:
    ...
    if ( v3 < 0 )
      MilInstrumentationCheckHR_MaybeFailFast(20LL, 0LL, 0, v3, 0x322u);
    else
