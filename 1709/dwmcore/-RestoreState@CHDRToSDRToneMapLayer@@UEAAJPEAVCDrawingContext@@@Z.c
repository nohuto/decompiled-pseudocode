/*
 * XREFs of ?RestoreState@CHDRToSDRToneMapLayer@@UEAAJPEAVCDrawingContext@@@Z @ 0x18014B820
 * Callers:
 *     <none>
 * Callees:
 *     ?MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJI@Z @ 0x180076954 (-MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJI@Z.c)
 *     ?RestoreState@CExternalLayer@@UEAAJPEAVCDrawingContext@@@Z @ 0x180081490 (-RestoreState@CExternalLayer@@UEAAJPEAVCDrawingContext@@@Z.c)
 *     ?RenderLayer@CHDRToSDRToneMapLayer@@IEAAJPEAVCDrawingContext@@@Z @ 0x18014B5E8 (-RenderLayer@CHDRToSDRToneMapLayer@@IEAAJPEAVCDrawingContext@@@Z.c)
 */

__int64 __fastcall CHDRToSDRToneMapLayer::RestoreState(
        CHDRToSDRToneMapLayer *this,
        struct CDrawingContext *a2,
        __int64 a3,
        double a4)
{
  signed int v6; // eax
  __int64 v7; // r8
  unsigned int v8; // ebx
  signed int v9; // eax

  v6 = CExternalLayer::RestoreState(this, a2);
  v8 = v6;
  if ( v6 < 0 )
  {
    MilInstrumentationCheckHR_MaybeFailFast(20LL, 0LL, 0, v6, 0x3Du);
  }
  else
  {
    v9 = CHDRToSDRToneMapLayer::RenderLayer(this, a2, v7, a4);
    v8 = v9;
    if ( v9 < 0 )
      MilInstrumentationCheckHR_MaybeFailFast(20LL, 0LL, 0, v9, 0x3Fu);
  }
  return v8;
}
