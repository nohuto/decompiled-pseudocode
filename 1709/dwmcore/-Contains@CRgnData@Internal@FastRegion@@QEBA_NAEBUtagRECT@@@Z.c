/*
 * XREFs of ?Contains@CRgnData@Internal@FastRegion@@QEBA_NAEBUtagRECT@@@Z @ 0x180030140
 * Callers:
        ...
        const struct tagRECT *a2)
{
  ...
  if ( a2->left >= *((_DWORD *)this + 1) )
  {
    ...
    if ( right <= *((_DWORD *)this + 2) )
    {
      ...
      {
        bottom = a2->bottom;
        if ( bottom <= *((_DWORD *)this + 2 * *(_DWORD *)this + 1) )
        {
          ...
          for ( i = (LONG *)((char *)this + 20); ; i += 2 )
          {
            ...
            {
              v8 = left;
              for ( j = (LONG *)((char *)i + *(i - 1) - 8); j < (LONG *)((char *)i + i[1]) && *j <= v8; j += 2 )
              {
                if ( j[1] > v8 )
                  v8 = j[1];
              }
