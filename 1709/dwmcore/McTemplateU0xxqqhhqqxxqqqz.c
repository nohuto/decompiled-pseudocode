/*
 * XREFs of McTemplateU0xxqqhhqqxxqqqz @ 0x18012D6F0
 * Callers:
 *     ?RetireScenario@CTelemetryTouchLatencyAnalysis@@AEAAXPEAUTouchScenarioInfo@1@_N1@Z @ 0x18012B968 (-RetireScenario@CTelemetryTouchLatencyAnalysis@@AEAAXPEAUTouchScenarioInfo@1@_N1@Z.c)
 * Callees:
 *     __security_check_cookie @ 0x1800C2B30 (__security_check_cookie.c)
 *     McGenEventWriteUM @ 0x180120D58 (McGenEventWriteUM.c)
 */

ULONG McTemplateU0xxqqhhqqxxqqqz(__int64 a1, __int64 a2, ...)
{
  const wchar_t *v2; // rcx
  __int64 v3; // rax
  int v4; // edx
  struct _EVENT_DATA_DESCRIPTOR v6; // [rsp+20h] [rbp-E0h] BYREF
  va_list v7; // [rsp+30h] [rbp-D0h]
  __int64 v8; // [rsp+38h] [rbp-C8h]
  va_list v9; // [rsp+40h] [rbp-C0h]
  __int64 v10; // [rsp+48h] [rbp-B8h]
  va_list v11; // [rsp+50h] [rbp-B0h]
  __int64 v12; // [rsp+58h] [rbp-A8h]
  va_list v13; // [rsp+60h] [rbp-A0h]
  __int64 v14; // [rsp+68h] [rbp-98h]
  va_list v15; // [rsp+70h] [rbp-90h]
  __int64 v16; // [rsp+78h] [rbp-88h]
  va_list v17; // [rsp+80h] [rbp-80h]
  __int64 v18; // [rsp+88h] [rbp-78h]
  va_list v19; // [rsp+90h] [rbp-70h]
  __int64 v20; // [rsp+98h] [rbp-68h]
  va_list v21; // [rsp+A0h] [rbp-60h]
  __int64 v22; // [rsp+A8h] [rbp-58h]
  va_list v23; // [rsp+B0h] [rbp-50h]
  __int64 v24; // [rsp+B8h] [rbp-48h]
  va_list v25; // [rsp+C0h] [rbp-40h]
  __int64 v26; // [rsp+C8h] [rbp-38h]
  va_list v27; // [rsp+D0h] [rbp-30h]
  __int64 v28; // [rsp+D8h] [rbp-28h]
  va_list v29; // [rsp+E0h] [rbp-20h]
  __int64 v30; // [rsp+E8h] [rbp-18h]
  va_list v31; // [rsp+F0h] [rbp-10h]
  __int64 v32; // [rsp+F8h] [rbp-8h]
  const wchar_t *v33; // [rsp+100h] [rbp+0h]
  int v34; // [rsp+108h] [rbp+8h]
  int v35; // [rsp+10Ch] [rbp+Ch]
  __int64 v36; // [rsp+140h] [rbp+40h] BYREF
  va_list va; // [rsp+140h] [rbp+40h]
  __int64 v38; // [rsp+148h] [rbp+48h] BYREF
  va_list va1; // [rsp+148h] [rbp+48h]
  __int64 v40; // [rsp+150h] [rbp+50h] BYREF
  va_list va2; // [rsp+150h] [rbp+50h]
  __int64 v42; // [rsp+158h] [rbp+58h] BYREF
  va_list va3; // [rsp+158h] [rbp+58h]
  __int64 v44; // [rsp+160h] [rbp+60h] BYREF
  va_list va4; // [rsp+160h] [rbp+60h]
  __int64 v46; // [rsp+168h] [rbp+68h] BYREF
  va_list va5; // [rsp+168h] [rbp+68h]
  __int64 v48; // [rsp+170h] [rbp+70h] BYREF
  va_list va6; // [rsp+170h] [rbp+70h]
  __int64 v50; // [rsp+178h] [rbp+78h] BYREF
  va_list va7; // [rsp+178h] [rbp+78h]
  __int64 v52; // [rsp+180h] [rbp+80h] BYREF
  va_list va8; // [rsp+180h] [rbp+80h]
  __int64 v54; // [rsp+188h] [rbp+88h] BYREF
  va_list va9; // [rsp+188h] [rbp+88h]
  __int64 v56; // [rsp+190h] [rbp+90h] BYREF
  va_list va10; // [rsp+190h] [rbp+90h]
  __int64 v58; // [rsp+198h] [rbp+98h] BYREF
  va_list va11; // [rsp+198h] [rbp+98h]
  __int64 v60; // [rsp+1A0h] [rbp+A0h] BYREF
  va_list va12; // [rsp+1A0h] [rbp+A0h]
  const wchar_t *v62; // [rsp+1A8h] [rbp+A8h]
  va_list va13; // [rsp+1B0h] [rbp+B0h] BYREF

  va_start(va13, a2);
  va_start(va12, a2);
  va_start(va11, a2);
  va_start(va10, a2);
  va_start(va9, a2);
  va_start(va8, a2);
  va_start(va7, a2);
  va_start(va6, a2);
  va_start(va5, a2);
  va_start(va4, a2);
  va_start(va3, a2);
  va_start(va2, a2);
  va_start(va1, a2);
  va_start(va, a2);
  v36 = va_arg(va1, _QWORD);
  va_copy(va2, va1);
  v38 = va_arg(va2, _QWORD);
  va_copy(va3, va2);
  v40 = va_arg(va3, _QWORD);
  va_copy(va4, va3);
  v42 = va_arg(va4, _QWORD);
  va_copy(va5, va4);
  v44 = va_arg(va5, _QWORD);
  va_copy(va6, va5);
  v46 = va_arg(va6, _QWORD);
  va_copy(va7, va6);
  v48 = va_arg(va7, _QWORD);
  va_copy(va8, va7);
  v50 = va_arg(va8, _QWORD);
  va_copy(va9, va8);
  v52 = va_arg(va9, _QWORD);
  va_copy(va10, va9);
  v54 = va_arg(va10, _QWORD);
  va_copy(va11, va10);
  v56 = va_arg(va11, _QWORD);
  va_copy(va12, va11);
  v58 = va_arg(va12, _QWORD);
  va_copy(va13, va12);
  v60 = va_arg(va13, _QWORD);
  v62 = va_arg(va13, const wchar_t *);
  v2 = v62;
  va_copy(v7, va);
  v8 = 8LL;
  va_copy(v9, va1);
  va_copy(v11, va2);
  va_copy(v13, va3);
  va_copy(v15, va4);
  va_copy(v17, va5);
  va_copy(v19, va6);
  va_copy(v21, va7);
  va_copy(v23, va8);
  va_copy(v25, va9);
  va_copy(v27, va10);
  va_copy(v29, va11);
  va_copy(v31, va12);
  v10 = 8LL;
  v12 = 4LL;
  v14 = 4LL;
  v16 = 2LL;
  v18 = 2LL;
  v20 = 4LL;
  v22 = 4LL;
  v24 = 8LL;
  v26 = 8LL;
  v28 = 4LL;
  v30 = 4LL;
  v32 = 4LL;
  if ( v62 )
  {
    v3 = -1LL;
    do
      ++v3;
    while ( v62[v3] );
    v4 = 2 * v3 + 2;
  }
  else
  {
    v4 = 10;
  }
  v34 = v4;
  v35 = 0;
  if ( !v62 )
    v2 = L"NULL";
  v33 = v2;
  return McGenEventWriteUM(
           &Microsoft_Windows_Dwm_Core_Provider_Context,
           &EVTDESC_TELEMETRY_TOUCH_ANALYSIS_SCENARIORETIRE_EVENT,
           0xFu,
           &v6);
}
