/*
 * XREFs of ?IsPixelAligned@@YA_NAEBUMilRectF@@@Z @ 0x1800BC2C8
 * Callers:
 *     ?CreateHitTestRegions@CPrimitiveGroup@@AEBAJXZ @ 0x180026030 (-CreateHitTestRegions@CPrimitiveGroup@@AEBAJXZ.c)
 *     ?BuildFromRectFs@CShape@@SAJPEBV?$TMilRect_@MUMilRectF@@UMilPointAndSizeF@@UNotNeeded@RectUniqueness@@@@IPEAPEAV1@@Z @ 0x1801CAFC4 (-BuildFromRectFs@CShape@@SAJPEBV-$TMilRect_@MUMilRectF@@UMilPointAndSizeF@@UNotNeeded@RectUnique.c)
 * Callees:
 *     <none>
 */

// local variable allocation has failed, the output may be wrong!
char __fastcall IsPixelAligned(const struct MilRectF *a1, __int64 a2, double a3)
{
  float v3; // xmm3_4
  char v4; // dl
  int v5; // eax
  float v7; // xmm3_4
  int v8; // eax
  float v9; // xmm3_4
  int v10; // eax
  float v11; // xmm3_4
  int v12; // eax
  __m128 v13; // rt1
  __m128 v14; // rt1
  __m128 v15; // rt1
  __m128 v16; // rt1
  float v17; // [rsp+20h] [rbp+8h]
  float v18; // [rsp+20h] [rbp+8h]
  float v19; // [rsp+20h] [rbp+8h]
  float v20; // [rsp+20h] [rbp+8h]

  v3 = *(float *)a1;
  v4 = 0;
  if ( (*(_DWORD *)a1 & 0x7FFFFFFFu) > 0x497FFFF0 )
  {
    *(float *)&a3 = (float)(int)v3 - v3;
    v13.m128_f32[0] = FLOAT_N0_5;
    *(__m128 *)&a3 = _mm_cmple_ss(*(__m128 *)&a3, v13);
    v5 = (int)v3 - LODWORD(a3);
  }
  else
  {
    v17 = v3 + 6291456.25;
    v5 = (int)(LODWORD(v17) << 10) >> 11;
  }
  if ( (float)COERCE_DOUBLE(COERCE_UNSIGNED_INT64((float)(v3 - (float)v5)) & _xmm) <= 0.00390625 )
  {
    v7 = *((float *)a1 + 1);
    if ( (LODWORD(v7) & 0x7FFFFFFFu) > 0x497FFFF0 )
    {
      *(float *)&a3 = (float)(int)v7 - v7;
      v14.m128_f32[0] = FLOAT_N0_5;
      *(__m128 *)&a3 = _mm_cmple_ss(*(__m128 *)&a3, v14);
      v8 = (int)v7 - LODWORD(a3);
    }
    else
    {
      v18 = v7 + 6291456.25;
      v8 = (int)(LODWORD(v18) << 10) >> 11;
    }
    if ( (float)COERCE_DOUBLE(COERCE_UNSIGNED_INT64((float)(v7 - (float)v8)) & _xmm) <= 0.00390625 )
    {
      v9 = *((float *)a1 + 2);
      if ( (LODWORD(v9) & 0x7FFFFFFFu) > 0x497FFFF0 )
      {
        *(float *)&a3 = (float)(int)v9 - v9;
        v15.m128_f32[0] = FLOAT_N0_5;
        *(__m128 *)&a3 = _mm_cmple_ss(*(__m128 *)&a3, v15);
        v10 = (int)v9 - LODWORD(a3);
      }
      else
      {
        v19 = v9 + 6291456.25;
        v10 = (int)(LODWORD(v19) << 10) >> 11;
      }
      if ( (float)COERCE_DOUBLE(COERCE_UNSIGNED_INT64((float)(v9 - (float)v10)) & _xmm) <= 0.00390625 )
      {
        v11 = *((float *)a1 + 3);
        if ( (LODWORD(v11) & 0x7FFFFFFFu) > 0x497FFFF0 )
        {
          *(float *)&a3 = (float)(int)v11 - v11;
          v16.m128_f32[0] = FLOAT_N0_5;
          v12 = (int)v11 - _mm_cmple_ss(*(__m128 *)&a3, v16).m128_u32[0];
        }
        else
        {
          v20 = v11 + 6291456.25;
          v12 = (int)(LODWORD(v20) << 10) >> 11;
        }
        return (float)COERCE_DOUBLE(COERCE_UNSIGNED_INT64((float)(v11 - (float)v12)) & _xmm) <= 0.00390625;
      }
    }
  }
  return v4;
}
