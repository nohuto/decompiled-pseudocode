/*
 * XREFs of RaidAllocateDeviceProperty @ 0x1C001CA98
 * Callers:
...
NTSTATUS __fastcall RaidAllocateDeviceProperty(PDEVICE_OBJECT DeviceObject, __int64 a2, _QWORD *a3)
{
  ...
  {
    PoolWithTag = ExAllocatePoolWithTag(NonPagedPoolNx, (unsigned int)NumberOfBytes, 0x51536152u);
    if ( PoolWithTag )
    {
      ...
      if ( DeviceProperty < 0 )
        ExFreePoolWithTag(PoolWithTag, 0x51536152u);
      else
        *a3 = PoolWithTag;
