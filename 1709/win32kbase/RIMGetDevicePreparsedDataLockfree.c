/*
 * XREFs of RIMGetDevicePreparsedDataLockfree @ 0x1C0101CB0
 * Callers:
 *     NtRIMGetDevicePreparsedDataLockfree @ 0x1C00FFF80 (NtRIMGetDevicePreparsedDataLockfree.c)
 *     RIMGetDevicePreparsedData @ 0x1C0101B80 (RIMGetDevicePreparsedData.c)
 * Callees:
 *     RawInputManagerDeviceObjectResolveHandle @ 0x1C000D6B0 (RawInputManagerDeviceObjectResolveHandle.c)
 *     memmove @ 0x1C00AB840 (memmove.c)
 */

__int64 __fastcall RIMGetDevicePreparsedDataLockfree(char *a1, char *a2, unsigned int *a3, char a4)
{
  int v7; // esi
  PVOID v8; // rdi
  char *v9; // rax
  __int64 v10; // rax
  unsigned int v11; // edx
  const void *v12; // r9
  __int64 v13; // rax
  PVOID Object; // [rsp+28h] [rbp-10h] BYREF

  v7 = RawInputManagerDeviceObjectResolveHandle(a1, 3u, 1, &Object);
  if ( v7 >= 0 )
  {
    v8 = Object;
    v9 = (char *)Object + 64;
    if ( !Object )
      v9 = 0LL;
    if ( v9[48] == 2 )
    {
      v10 = *((_QWORD *)v9 + 57);
      v11 = *(_DWORD *)(v10 + 104);
      v12 = *(const void **)(v10 + 16);
      if ( a2 )
      {
        if ( a4 )
        {
          if ( a3 + 1 < a3 || (unsigned __int64)(a3 + 1) > MmUserProbeAddress )
            *(_BYTE *)MmUserProbeAddress = 0;
          v13 = *a3;
          if ( v11 < (unsigned int)v13 )
            v13 = v11;
          if ( (unsigned __int64)&a2[v13] > MmUserProbeAddress || &a2[v13] <= a2 )
            *(_BYTE *)MmUserProbeAddress = 0;
        }
        else
        {
          LODWORD(v13) = *a3;
          if ( v11 < *a3 )
            LODWORD(v13) = v11;
        }
        memmove(a2, v12, (unsigned int)v13);
      }
      else if ( a4 )
      {
        if ( (unsigned __int64)a3 >= MmUserProbeAddress )
          a3 = (unsigned int *)MmUserProbeAddress;
        *a3 = v11;
      }
      else
      {
        *a3 = v11;
      }
    }
    else
    {
      v7 = -1073741811;
    }
    ObfDereferenceObject(v8);
  }
  return (unsigned int)v7;
}
