/*
 * XREFs of KeyboardClassGetWaitWakeEnableState @ 0x1C000D150
 * Callers:
...
char __fastcall KeyboardClassGetWaitWakeEnableState(__int64 a1)
{
  ...
  ULONG Length; // [rsp+30h] [rbp-1B8h] BYREF
  int v14; // [rsp+34h] [rbp-1B4h] BYREF
  void *DeviceRegKey; // [rsp+38h] [rbp-1B0h] BYREF
  struct _UNICODE_STRING DestinationString; // [rsp+40h] [rbp-1A8h] BYREF
  _BYTE OutputBuffer[80]; // [rsp+50h] [rbp-198h] BYREF
  _OSVERSIONINFOEXW VersionInfo; // [rsp+A0h] [rbp-148h] BYREF
      ...
      if ( v5 >= 0 )
      {
        ...
        if ( v5 >= 0 )
        {
          if ( !OutputBuffer[5] )
          {
            ...
            {
              LOBYTE(v5) = -OutputBuffer[3];
              v10 = OutputBuffer[3] != 0 ? 2 : 0;
            }
          }
