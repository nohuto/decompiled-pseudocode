/*
 * XREFs of ?MakeAndInitialize@?$ProcessLocalStorageData@UProcessLocalData@details_abi@wil@@@details_abi@wil@@CAJPEBG$$QEAV?$unique_any_t@V?$mutex_t@V?$unique_storage@U?$resource_policy@PEAXP6AXPEAX@Z$1?CloseHandle@details@wil@@YAX0@ZU?$integral_constant@_K$0A@@wistd@@PEAX$0A@$$T@details@wil@@@details@wil@@Uerr_returncode_policy@3@@wil@@@3@PEAPEAV123@@Z @ 0x140032E44
 * Callers:
 *     ?Acquire@?$ProcessLocalStorageData@UProcessLocalData@details_abi@wil@@@details_abi@wil@@SAJPEBDPEAPEAV123@@Z @ 0x140031BF4 (-Acquire@-$ProcessLocalStorageData@UProcessLocalData@details_abi@wil@@@details_abi@wil@@SAJPEBDP.c)
 * Callees:
 *     memset @ 0x14001D9AC (memset.c)
 *     ?CloseHandle@details@wil@@YAXPEAX@Z @ 0x140031E00 (-CloseHandle@details@wil@@YAXPEAX@Z.c)
 *     ?CreateFromValueInternal@SemaphoreValue@details_abi@wil@@AEAAJPEBG_N_K@Z @ 0x140031E24 (-CreateFromValueInternal@SemaphoreValue@details_abi@wil@@AEAAJPEBG_N_K@Z.c)
 *     ?FreeProcessHeap@details@wil@@YAXPEAX@Z @ 0x140032258 (-FreeProcessHeap@details@wil@@YAXPEAX@Z.c)
 *     ?Return_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z @ 0x140033668 (-Return_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z.c)
 *     ?Return_Hr_NoOriginate@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z @ 0x1400336E4 (-Return_Hr_NoOriginate@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z.c)
 *     ?_FailFastImmediate_Unexpected@in1diag3@details@wil@@YAXXZ @ 0x1400340BC (-_FailFastImmediate_Unexpected@in1diag3@details@wil@@YAXXZ.c)
 */

__int64 __fastcall wil::details_abi::ProcessLocalStorageData<wil::details_abi::ProcessLocalData>::MakeAndInitialize(
        unsigned __int16 *a1,
        _QWORD *a2,
        _QWORD *a3)
{
  HANDLE ProcessHeap; // rax
  unsigned __int64 v7; // rax
  wil::details::in1diag3 *v8; // rcx
  __int64 v9; // r8
  _QWORD *v10; // rbx
  unsigned int v11; // edi
  void *v12; // rdx
  int v13; // eax
  wil::details *v14; // rbp
  wil::details *v15; // rsi
  _QWORD *v16; // rax
  wil::details *v18[2]; // [rsp+20h] [rbp-28h] BYREF
  wil::details::in1diag3 *retaddr; // [rsp+48h] [rbp+0h]

  *a3 = 0LL;
  ProcessHeap = GetProcessHeap();
  v7 = (unsigned __int64)HeapAlloc(ProcessHeap, 8u, 0x78uLL);
  v10 = (_QWORD *)v7;
  if ( v7 )
  {
    *(_OWORD *)v18 = 0LL;
    if ( (v7 & 3) != 0 )
      wil::details::in1diag3::_FailFastImmediate_Unexpected(v8);
    v13 = wil::details_abi::SemaphoreValue::CreateFromValueInternal(
            (wil::details_abi::SemaphoreValue *)v18,
            a1,
            v9,
            v7 >> 2);
    v11 = v13;
    if ( v13 >= 0 )
    {
      if ( v10 )
      {
        *(_DWORD *)v10 = 1;
        v14 = 0LL;
        v10[1] = *a2;
        *a2 = 0LL;
        v15 = 0LL;
        v10[2] = v18[0];
        v18[0] = 0LL;
        v10[3] = v18[1];
        v18[1] = 0LL;
        memset(v10 + 4, 0, 0x58uLL);
        *((_WORD *)v10 + 16) = 88;
        *((_DWORD *)v10 + 9) = 1;
        memset(v10 + 5, 0, 0x50uLL);
      }
      else
      {
        v14 = v18[1];
        v15 = v18[0];
      }
      v16 = v10;
      v10 = 0LL;
      v11 = 0;
      *a3 = v16;
    }
    else
    {
      wil::details::in1diag3::Return_Hr_NoOriginate(
        retaddr,
        (void *)0x140,
        (unsigned int)"wil",
        (const char *)(unsigned int)v13,
        (int)v18[0]);
      v14 = v18[1];
      v15 = v18[0];
    }
    if ( v14 )
      wil::details::CloseHandle(v14, v12);
    if ( v15 )
      wil::details::CloseHandle(v15, v12);
  }
  else
  {
    v11 = -2147024882;
    wil::details::in1diag3::Return_Hr(
      retaddr,
      (void *)0x13D,
      (unsigned int)"wil",
      (const char *)0x8007000ELL,
      (int)v18[0]);
  }
  if ( v10 )
    wil::details::FreeProcessHeap((wil::details *)v10, v12);
  return v11;
}
