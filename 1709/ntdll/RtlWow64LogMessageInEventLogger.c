/*
 * XREFs of RtlWow64LogMessageInEventLogger @ 0x1800D7C70
 * Callers:
 *     <none>
 * Callees:
...
int __fastcall RtlWow64LogMessageInEventLogger(__int16 a1, __int64 a2, unsigned int a3)
{
  ...
  result = LdrLoadDll(0LL, 0LL, (PUNICODE_STRING)&stru_180114248, &DllHandle);
  if ( result >= 0 )
  {
    if ( LdrGetProcedureAddressForCaller(DllHandle, (PANSI_STRING)&stru_180114268, 0, &ProcedureAddress, 0, retaddr) >= 0
      && LdrGetProcedureAddressForCaller(DllHandle, (PANSI_STRING)&stru_180114258, 0, &v12, 0, retaddr) >= 0
      && LdrGetProcedureAddressForCaller(DllHandle, (PANSI_STRING)&stru_180114278, 0, &v11, 0, retaddr) >= 0 )
    {
