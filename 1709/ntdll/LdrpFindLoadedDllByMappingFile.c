/*
 * XREFs of LdrpFindLoadedDllByMappingFile @ 0x18007A228
 * Callers:
  ...
  *(_OWORD *)&ObjectAttributes.SecurityDescriptor = 0LL;
  ...
  if ( LoadedDllByMapping >= 0 )
  {
    ...
    if ( LoadedDllByMapping >= 0 )
    {
      v14 = 0LL;
      v15 = 0LL;
      LoadedDllByMapping = ZwMapViewOfSection(Handle, -1LL, &v14, 0LL, 0LL, 0LL, &v15, 1, 0, 2);
      if ( LoadedDllByMapping >= 0 )
      {
        LoadedDllByMapping = RtlImageNtHeaderEx(0, v14, v15, &v11);
        if ( LoadedDllByMapping >= 0 )
          LoadedDllByMapping = LdrpFindLoadedDllByMapping(v14, v11, a2, a3);
        NtUnmapViewOfSection(-1LL, v14, v7);
      }
      NtClose(Handle);
    }
    NtClose(FileHandle);
