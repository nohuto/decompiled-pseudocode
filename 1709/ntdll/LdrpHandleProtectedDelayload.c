/*
 * XREFs of LdrpHandleProtectedDelayload @ 0x180026850
 * Callers:
 *     LdrResolveDelayLoadedAPI @ 0x180029F10 (LdrResolveDelayLoadedAPI.c)
 * Callees:
 *     RtlAddressInSectionTable @ 0x180020840 (RtlAddressInSectionTable.c)
 *     LdrpWriteBackProtectedDelayLoad @ 0x180022944 (LdrpWriteBackProtectedDelayLoad.c)
 *     LdrpGetDelayloadExportDll @ 0x180022A6C (LdrpGetDelayloadExportDll.c)
 *     RtlImageNtHeaderEx @ 0x180028970 (RtlImageNtHeaderEx.c)
 *     LdrpDereferenceModule @ 0x180029804 (LdrpDereferenceModule.c)
 *     RtlSectionTableFromVirtualAddress @ 0x1800299D0 (RtlSectionTableFromVirtualAddress.c)
 *     LdrpLoadForwardedDll @ 0x18002BF34 (LdrpLoadForwardedDll.c)
 *     RtlReleasePath @ 0x180038070 (RtlReleasePath.c)
 *     RtlFreeHeap @ 0x18003ECC0 (RtlFreeHeap.c)
 *     RtlAllocateHeap @ 0x180040DF0 (RtlAllocateHeap.c)
 *     RtlGuardCheckImageBase @ 0x18007981C (RtlGuardCheckImageBase.c)
 *     LdrpParseForwarderDescription @ 0x18007A4B8 (LdrpParseForwarderDescription.c)
 *     LdrpRedirectDelayloadFailure @ 0x180083A48 (LdrpRedirectDelayloadFailure.c)
 *     __security_check_cookie @ 0x180090C90 (__security_check_cookie.c)
 *     _guard_dispatch_icall_nop @ 0x1800A3A60 (_guard_dispatch_icall_nop.c)
 *     memset @ 0x1800A6C80 (memset.c)
 *     LdrpLogDbgPrint @ 0x1800D0E14 (LdrpLogDbgPrint.c)
 *     AVrfCallAPILookupCallback @ 0x1800D9A3C (AVrfCallAPILookupCallback.c)
 */

__int64 __fastcall LdrpHandleProtectedDelayload(
        __int64 a1,
        __int64 a2,
        __int64 a3,
        __int64 a4,
        _QWORD *a5,
        unsigned int a6)
{
  int v7; // r12d
  __int64 v8; // r15
  __int64 v9; // rdi
  int DelayloadExportDll; // eax
  int v11; // ebx
  __int64 v12; // r13
  _QWORD *v13; // rbx
  __int64 v14; // rsi
  __int64 v15; // r12
  _BYTE *Heap; // r14
  __int64 v17; // rcx
  __int64 v18; // rax
  __int64 v19; // rdx
  __int64 v20; // rax
  const char *v21; // r15
  int v22; // esi
  __int64 v23; // rdi
  __int64 v24; // rbx
  unsigned __int64 v25; // r14
  bool v26; // bl
  unsigned __int64 v27; // rdi
  char *v28; // r13
  int v29; // eax
  __int16 v30; // ax
  __int64 v31; // rax
  int v32; // r11d
  int v33; // r11d
  int v34; // r10d
  int v35; // r9d
  const char *v36; // rax
  signed __int64 v37; // rdx
  unsigned __int8 v38; // cl
  int v39; // eax
  unsigned int v40; // esi
  char *v41; // rdi
  int ForwardedDll; // ebx
  __int64 v43; // rsi
  unsigned int v44; // ecx
  __int64 v45; // r15
  unsigned int v47; // eax
  __int64 v48; // r8
  __int64 v49; // rax
  __int64 v50; // r8
  int v51; // [rsp+40h] [rbp-578h]
  unsigned int v52; // [rsp+44h] [rbp-574h]
  int v53; // [rsp+48h] [rbp-570h] BYREF
  int v54; // [rsp+4Ch] [rbp-56Ch]
  __int64 v55; // [rsp+50h] [rbp-568h] BYREF
  unsigned int v56; // [rsp+58h] [rbp-560h]
  char *v57; // [rsp+60h] [rbp-558h]
  _BYTE *v58; // [rsp+68h] [rbp-550h]
  __int64 v59; // [rsp+70h] [rbp-548h] BYREF
  char *v60; // [rsp+78h] [rbp-540h] BYREF
  __int64 v61; // [rsp+80h] [rbp-538h]
  __int64 v62; // [rsp+88h] [rbp-530h] BYREF
  __int64 v63; // [rsp+90h] [rbp-528h]
  __int64 v64; // [rsp+98h] [rbp-520h] BYREF
  const char *v65; // [rsp+A0h] [rbp-518h] BYREF
  void (__fastcall *v66)(char **, __int64, char *, _QWORD, _QWORD); // [rsp+A8h] [rbp-510h]
  char *v67; // [rsp+B0h] [rbp-508h] BYREF
  __int64 v68; // [rsp+B8h] [rbp-500h]
  __int64 v69; // [rsp+C0h] [rbp-4F8h]
  __int64 v70; // [rsp+C8h] [rbp-4F0h]
  _QWORD *v71; // [rsp+D0h] [rbp-4E8h]
  char **v72; // [rsp+D8h] [rbp-4E0h]
  __int64 v73; // [rsp+E0h] [rbp-4D8h]
  __int64 v74; // [rsp+E8h] [rbp-4D0h]
  _BYTE v75[16]; // [rsp+F0h] [rbp-4C8h] BYREF
  _QWORD v76[16]; // [rsp+100h] [rbp-4B8h] BYREF
  _BYTE v77[1024]; // [rsp+180h] [rbp-438h] BYREF

  v73 = a4;
  v7 = a3;
  v74 = a3;
  v8 = a2;
  v69 = a2;
  v9 = a1;
  v61 = a1;
  v71 = a5;
  v59 = 0LL;
  DelayloadExportDll = LdrpGetDelayloadExportDll(a1, a2, &v55, a6, (__int64)a5);
  v11 = DelayloadExportDll;
  v54 = DelayloadExportDll;
  if ( DelayloadExportDll >= 0 )
  {
    v12 = v55;
    RtlGuardCheckImageBase(*(_QWORD *)(v55 + 48), 0LL);
    v13 = (_QWORD *)(*(_QWORD *)(v9 + 48) + *(unsigned int *)(v8 + 12));
    v63 = (__int64)v13;
    v14 = a5 - v13;
    v68 = v14;
    LODWORD(v15) = 0;
    if ( *v13 )
    {
      do
        v15 = (unsigned int)(v15 + 1);
      while ( v13[v15] );
    }
    if ( (unsigned int)v15 > 0x80 )
    {
      Heap = (_BYTE *)RtlAllocateHeap(LdrpHeap, (unsigned int)(NtdllBaseTag + 0x40000), 8LL * (unsigned int)v15);
      v58 = Heap;
      if ( !Heap )
      {
        Heap = v77;
        v58 = v77;
        v13 = v71;
        v63 = (__int64)v71;
        LODWORD(v15) = v15 - v14;
        if ( (unsigned int)v15 > 0x80 )
          LODWORD(v15) = 128;
        LODWORD(v14) = 0;
        v68 = 0LL;
      }
    }
    else
    {
      Heap = v77;
      v58 = v77;
    }
    if ( g_ShimsEnabled )
      v66 = (void (__fastcall *)(char **, __int64, char *, _QWORD, _QWORD))(__ROR8__(
                                                                              g_pfnSE_GetProcAddressForCaller,
                                                                              64 - (MEMORY[0x7FFE0330] & 0x3Fu)) ^ MEMORY[0x7FFE0330]);
    else
      v66 = 0LL;
    v17 = 0LL;
    v52 = 0;
    if ( !(_DWORD)v15 )
      goto LABEL_56;
    v18 = 0LL;
    v70 = 0LL;
    v51 = v53;
    while ( 1 )
    {
      v72 = (char **)&Heap[8 * v18];
      *v72 = 0LL;
      if ( (_DWORD)v17 != (_DWORD)v14 )
      {
        if ( v13[v18] - *(_QWORD *)(v9 + 48) >= (unsigned __int64)*(unsigned int *)(v9 + 64) )
        {
          v44 = v52;
          goto LABEL_55;
        }
        v17 = v52;
      }
      v57 = 0LL;
      v60 = 0LL;
      v19 = *(_QWORD *)(v9 + 48);
      v20 = *(_QWORD *)(v19
                      + *(unsigned int *)(v8 + 16)
                      + 8 * (((__int64)v13 + 8 * v17 - (v19 + *(unsigned int *)(v8 + 12))) >> 3));
      if ( v20 < 0 )
      {
        v21 = 0LL;
        v22 = (unsigned __int16)v20;
      }
      else
      {
        v21 = (const char *)(v20 + v19 + 2);
        v22 = 0;
      }
      v65 = v21;
      v53 = v22;
      v23 = v12;
      v62 = v12;
      v56 = 0;
      v24 = *(_QWORD *)(v12 + 80);
      memset(v76, 0, sizeof(v76));
      LODWORD(v76[3]) = 0;
      v76[4] = v24;
      while ( 1 )
      {
        v25 = *(_QWORD *)(v23 + 48);
        v26 = 1;
        v27 = v25;
        v64 = 0LL;
        v28 = 0LL;
        if ( (v25 & 3) != 0 )
        {
          v27 = v25 & 0xFFFFFFFFFFFFFFFCuLL;
          v26 = (v25 & 1) == 0;
        }
        v29 = RtlImageNtHeaderEx(1LL, v27, 0LL, &v64);
        if ( !v64 )
          goto LABEL_103;
        v30 = *(_WORD *)(v64 + 24);
        if ( v30 == 267 )
        {
          if ( *(_DWORD *)(v64 + 116) )
          {
            v48 = *(unsigned int *)(v64 + 120);
            if ( (_DWORD)v48 )
            {
              v32 = *(_DWORD *)(v64 + 124);
              v51 = v32;
              if ( v26 || (unsigned int)v48 < *(_DWORD *)(v64 + 84) )
              {
                v28 = (char *)(v27 + v48);
                goto LABEL_25;
              }
              v49 = RtlSectionTableFromVirtualAddress(v64, 267LL, v48);
              v28 = v49
                  ? (char *)(v50 + v27 + *(unsigned int *)(v49 + 20) - (unsigned __int64)*(unsigned int *)(v49 + 12))
                  : 0LL;
              if ( v28 )
              {
                v29 = 0;
                goto LABEL_104;
              }
            }
          }
        }
        else if ( v30 == 523 )
        {
          if ( *(_DWORD *)(v64 + 132) )
          {
            v31 = *(unsigned int *)(v64 + 136);
            if ( (_DWORD)v31 )
            {
              v32 = *(_DWORD *)(v64 + 140);
              v51 = v32;
              if ( v26 || (unsigned int)v31 < *(_DWORD *)(v64 + 84) )
              {
                v28 = (char *)(v27 + v31);
                goto LABEL_25;
              }
              v28 = (char *)RtlAddressInSectionTable();
              if ( v28 )
              {
                v29 = 0;
LABEL_103:
                v32 = v51;
LABEL_104:
                if ( v29 >= 0 )
                  goto LABEL_25;
              }
            }
          }
        }
        v28 = 0LL;
        v32 = v51;
LABEL_25:
        if ( !v28 )
          goto LABEL_113;
        if ( v21 )
          break;
        if ( (LdrpDebugFlags & 5) != 0 )
        {
          LdrpLogDbgPrint(
            (unsigned int)"minkernel\\ntdll\\ldrsnap.c",
            703,
            (unsigned int)"LdrpGetProcedureAddress",
            2,
            "Loading procedure 0x%lx by ordinal\n",
            v22);
          v32 = v51;
        }
        if ( !v22 )
        {
          ForwardedDll = -1073741811;
          goto LABEL_114;
        }
        v40 = v22 - *((_DWORD *)v28 + 4);
LABEL_41:
        if ( v40 >= *((_DWORD *)v28 + 5) )
        {
          ForwardedDll = (v21 != 0LL) - 1073741512;
          goto LABEL_114;
        }
        v41 = (char *)(v25 + *(unsigned int *)(v25 + *((unsigned int *)v28 + 7) + 4LL * (int)v40));
        v57 = v41;
        v60 = v41;
        if ( v41 < v28 || v41 >= &v28[v32] )
        {
          ForwardedDll = 0;
          goto LABEL_44;
        }
        v47 = v56++;
        if ( v47 >= 0x20 )
        {
          ForwardedDll = -1073741701;
          goto LABEL_44;
        }
        ForwardedDll = LdrpParseForwarderDescription(v41, v75, &v65, &v53);
        if ( ForwardedDll < 0 )
          goto LABEL_44;
        LODWORD(v76[3]) = *(_DWORD *)(v62 + 272);
        ForwardedDll = LdrpLoadForwardedDll((unsigned int)v75, (unsigned int)v76, v55, v62, 2, (__int64)&v62);
        if ( ForwardedDll < 0 )
          goto LABEL_44;
        v23 = v62;
        LdrpDereferenceModule(v62);
        v21 = v65;
        v22 = v53;
      }
      if ( (LdrpDebugFlags & 5) != 0 )
        LdrpLogDbgPrint(
          (unsigned int)"minkernel\\ntdll\\ldrsnap.c",
          685,
          (unsigned int)"LdrpGetProcedureAddress",
          2,
          "Locating procedure \"%s\" by name\n",
          v21);
      v33 = 0;
      v34 = *((_DWORD *)v28 + 6) - 1;
      v35 = v34 / 2;
      if ( v34 >= 0 )
      {
        while ( 1 )
        {
          v36 = v21;
          v37 = v25 + *(unsigned int *)(v25 + *((unsigned int *)v28 + 8) + 4LL * v35) - (_QWORD)v21;
          while ( 1 )
          {
            v38 = *v36;
            if ( *v36 != v36[v37] )
              break;
            ++v36;
            if ( !v38 )
            {
              v39 = 0;
              goto LABEL_34;
            }
          }
          v39 = v38 < (unsigned int)v36[v37] ? -1 : 1;
LABEL_34:
          if ( !v39 )
            break;
          if ( v39 >= 0 )
            v33 = v35 + 1;
          else
            v34 = v35 - 1;
          v35 = (v34 + v33) / 2;
          if ( v34 < v33 )
            goto LABEL_106;
        }
        v40 = *(unsigned __int16 *)(v25 + *((unsigned int *)v28 + 9) + 2LL * v35);
        v32 = v51;
        goto LABEL_41;
      }
LABEL_106:
      if ( (LdrpDebugFlags & 3) != 0 )
        LdrpLogDbgPrint(
          (unsigned int)"minkernel\\ntdll\\ldrsnap.c",
          1414,
          (unsigned int)"LdrpNameToOrdinal",
          1,
          "Procedure \"%s\" could not be located in DLL at base 0x%p.\n",
          v21,
          (const void *)v25);
      if ( (LdrpDebugFlags & 0x40) != 0 )
        __debugbreak();
LABEL_113:
      ForwardedDll = -1073741702;
LABEL_114:
      v41 = v57;
LABEL_44:
      if ( BYTE4(v76[15]) )
        RtlReleasePath(v76[0]);
      if ( ForwardedDll < 0 )
      {
        v41 = 0LL;
        v60 = 0LL;
      }
      v53 = ForwardedDll;
      if ( ForwardedDll >= 0 )
      {
        v43 = v61;
        if ( AvrfpAPILookupCallbacksEnabled )
        {
          AVrfCallAPILookupCallback(*(_QWORD *)(v61 + 48), *(_QWORD *)(v55 + 48), (_DWORD)v41, 1, (__int64)&v60);
          v41 = v60;
        }
        if ( v66 )
        {
          v67 = 0LL;
          v66(&v67, v55, v41, *(_QWORD *)(v43 + 48), 0LL);
          if ( v67 )
            v41 = v67;
        }
      }
      *v72 = v41;
      v44 = v52;
      LODWORD(v14) = v68;
      Heap = v58;
      v9 = v61;
      v12 = v55;
      v8 = v69;
      if ( v52 == (_DWORD)v68 )
      {
        v54 = ForwardedDll;
        v59 = *(_QWORD *)&v58[8 * (unsigned int)v68];
      }
      v13 = (_QWORD *)v63;
LABEL_55:
      v17 = v44 + 1;
      v52 = v17;
      v18 = ++v70;
      if ( (unsigned int)v17 >= (unsigned int)v15 )
      {
LABEL_56:
        if ( v54 < 0 )
        {
          v45 = LdrpRedirectDelayloadFailure(v9, v12, v8, v74, v73, (__int64)v71, v54);
          v59 = v45;
          if ( v45 && ((unsigned int)(v54 + 1073741512) <= 1 || v54 == -1073741702 || v54 == -1073740671) )
            *(_QWORD *)&Heap[8 * (unsigned int)v14] = v45;
        }
        else
        {
          v45 = v59;
        }
        LdrpWriteBackProtectedDelayLoad(v9, (__int64)v13, (__int64)Heap, v15, v14);
        if ( v77 != Heap )
          RtlFreeHeap(LdrpHeap, 0LL, Heap);
        LdrpDereferenceModule(v12);
        return v45;
      }
    }
  }
  v45 = LdrpRedirectDelayloadFailure(v9, 0, v8, v7, a4, (__int64)a5, DelayloadExportDll);
  v59 = v45;
  if ( v45 && v11 == -1073741515 )
    LdrpWriteBackProtectedDelayLoad(v9, (__int64)a5, (__int64)&v59, 1u, 0);
  return v45;
}
