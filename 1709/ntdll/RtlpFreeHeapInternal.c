/*
 * XREFs of RtlpFreeHeapInternal @ 0x18003E860
 * Callers:
 *     RtlFreeHeap @ 0x18003ECC0 (RtlFreeHeap.c)
 *     RtlpHpFreeWithExceptionProtection @ 0x18003F2E0 (RtlpHpFreeWithExceptionProtection.c)
 * Callees:
 *     RtlpIsSubSegmentReuseable @ 0x180014FA0 (RtlpIsSubSegmentReuseable.c)
 *     RtlpFreeUserBlock @ 0x180015054 (RtlpFreeUserBlock.c)
 *     RtlNtStatusToDosError @ 0x18001AD60 (RtlNtStatusToDosError.c)
 *     RtlpHpSegPageRangeShrink @ 0x1800383A4 (RtlpHpSegPageRangeShrink.c)
 *     RtlpFreeHeap @ 0x18003AD60 (RtlpFreeHeap.c)
 *     RtlpHpVsContextFree @ 0x18003E180 (RtlpHpVsContextFree.c)
 *     RtlGetCurrentServiceSessionId @ 0x18003F2C0 (RtlGetCurrentServiceSessionId.c)
 *     RtlpHpLfhSubsegmentFreeBlock @ 0x180040530 (RtlpHpLfhSubsegmentFreeBlock.c)
 *     RtlpHpSizeHeap @ 0x180045DE0 (RtlpHpSizeHeap.c)
 *     RtlpHpExtrasGet @ 0x180049278 (RtlpHpExtrasGet.c)
 *     RtlpGetHeapProtection @ 0x18004B238 (RtlpGetHeapProtection.c)
 *     RtlpLfhBucketUsageUpdate @ 0x18004C518 (RtlpLfhBucketUsageUpdate.c)
 *     RtlCSparseBitmapBitmaskRead @ 0x18006206C (RtlCSparseBitmapBitmaskRead.c)
 *     RtlpHpLargeFree @ 0x18006F490 (RtlpHpLargeFree.c)
 *     RtlpProbeUserBufferSafe @ 0x180077930 (RtlpProbeUserBufferSafe.c)
 *     RtlpLogHeapFailure @ 0x18009FBBC (RtlpLogHeapFailure.c)
 *     ZwProtectVirtualMemory @ 0x1800A0AC0 (ZwProtectVirtualMemory.c)
 *     RtlpInterlockedPushEntrySList @ 0x1800A3B50 (RtlpInterlockedPushEntrySList.c)
 *     RtlpInterlockedFlushSList @ 0x1800A3B90 (RtlpInterlockedFlushSList.c)
 *     RtlpCallInterceptRoutine @ 0x1800EFE04 (RtlpCallInterceptRoutine.c)
 *     RtlpHpStackTraceRemoveStack @ 0x180100A6C (RtlpHpStackTraceRemoveStack.c)
 *     RtlpLogHeapFreeEvent @ 0x1801016FC (RtlpLogHeapFreeEvent.c)
 *     RtlpHpSizeHeapInternal @ 0x180103004 (RtlpHpSizeHeapInternal.c)
 *     RtlpGetReservedBlockSize @ 0x180106330 (RtlpGetReservedBlockSize.c)
 *     RtlpValidateLFHBlock @ 0x18010674C (RtlpValidateLFHBlock.c)
 *     RtlpHpSegGetDescriptorValidateSafe @ 0x1801088E8 (RtlpHpSegGetDescriptorValidateSafe.c)
 */

__int64 __fastcall RtlpFreeHeapInternal(__int64 a1, unsigned __int64 a2, int a3, __int64 *a4, _WORD *a5)
{
  __int64 v5; // r12
  unsigned __int64 v8; // r14
  unsigned __int64 v10; // r13
  int v11; // edx
  int v12; // r8d
  int v13; // ecx
  int v14; // edx
  __int64 v15; // rcx
  unsigned int v16; // ebp
  __int64 v17; // rdi
  int v18; // eax
  __int64 v19; // rcx
  __int64 v20; // rdi
  unsigned __int64 v21; // rdx
  __int64 DescriptorValidateSafe; // rbx
  unsigned __int64 v23; // rdx
  unsigned int v24; // ebp
  __int64 v25; // r9
  _DWORD *SharedData; // rcx
  __int64 v27; // rcx
  __int64 v29; // rcx
  __int64 v30; // rdx
  __int64 v31; // rdx
  __int64 v32; // rcx
  __int64 v33; // r9
  __int64 v34; // rcx
  __int64 v35; // rax
  __int64 v36; // rdx
  __int64 v37; // rcx
  __int64 v38; // r9
  __int64 v39; // rcx
  unsigned __int8 v40; // al
  __int64 v41; // rax
  int v42; // r8d
  __int64 v43; // rax
  struct _TEB *v44; // rbx
  __int64 v45; // rax
  struct _TEB *v46; // rbx
  unsigned __int8 v47; // cl
  __int64 v48; // rax
  unsigned __int16 v49; // ax
  int v50; // eax
  __int64 v51; // rdx
  int v52; // eax
  unsigned __int16 v53; // ax
  __int64 v54; // rcx
  __int64 v55; // rcx
  unsigned __int64 v56; // rdx
  __int64 v57; // rax
  __int64 v58; // rbx
  unsigned int v59; // ecx
  __int64 v60; // r8
  __int64 v61; // rcx
  __int64 v62; // r9
  __int64 v63; // rcx
  unsigned int v64; // r8d
  int v65; // edx
  signed __int32 v66; // ebx
  unsigned __int64 v67; // rcx
  _QWORD *v68; // rax
  int v69; // r8d
  unsigned int v70; // edx
  signed __int64 *v71; // rcx
  __int64 v72; // rdx
  __int64 v73; // rcx
  int v74; // eax
  __int64 v75; // r8
  int v76; // eax
  _QWORD *v77; // r9
  signed __int64 v78; // rdx
  __int64 *v79; // r13
  _DWORD *v80; // rbx
  __int64 v81; // rcx
  _DWORD *v82; // rdx
  __int64 v83; // rax
  __int64 v84; // r13
  __int64 v85; // rbx
  unsigned __int16 ReservedBlockSize; // ax
  __int64 v87; // rcx
  unsigned int HeapProtection; // eax
  int v89; // edx
  signed __int64 v90; // rax
  signed __int32 v91; // eax
  int v92; // ecx
  __int64 v93; // rax
  signed __int32 v94; // eax
  _QWORD *v95; // r9
  unsigned int i; // r8d
  __int64 v97; // rax
  volatile signed __int32 *v98; // rdx
  _QWORD *v99; // rcx
  __int64 v100; // rcx
  unsigned int v101; // [rsp+30h] [rbp-68h] BYREF
  char v102[4]; // [rsp+34h] [rbp-64h] BYREF
  unsigned __int64 v103; // [rsp+38h] [rbp-60h]
  __int64 v104; // [rsp+40h] [rbp-58h]
  int v105; // [rsp+48h] [rbp-50h]
  unsigned __int64 v106; // [rsp+50h] [rbp-48h] BYREF
  unsigned __int64 v107; // [rsp+58h] [rbp-40h] BYREF
  int v108; // [rsp+68h] [rbp-30h]
  signed __int64 v109; // [rsp+A0h] [rbp+8h]

  v5 = 0LL;
  v8 = a2;
  v10 = 0LL;
  if ( *(_DWORD *)(a1 + 16) != -571548178 )
  {
    if ( (*(_DWORD *)(a1 + 116) & 0x1000000) != 0 )
      goto LABEL_75;
    if ( (*(_BYTE *)(a1 + 120) & 1) != 0 )
    {
      v10 = RtlpProbeUserBufferSafe(a1, a2);
    }
    else if ( (a2 & 0xF) != 0 )
    {
      RtlpLogHeapFailure(9, a1, a2, 0, 0LL, 0LL);
    }
    else
    {
      v10 = a2 - 16;
      _m_prefetchw((const void *)(a2 - 16));
      if ( *(_BYTE *)(a2 - 16 + 15) == 5 )
        v10 -= 16LL * *(unsigned __int8 *)(v10 + 14);
      if ( (*(_BYTE *)(v10 + 15) & 0x3F) == 0 )
      {
        RtlpLogHeapFailure(8, a1, v10, 0, 0LL, 0LL);
        v10 = 0LL;
      }
    }
    if ( v10 )
    {
      if ( *(_BYTE *)(v8 - 1) != 5 )
        goto LABEL_74;
      if ( *(char *)(v10 + 15) >= 0 )
      {
        if ( *(_DWORD *)(a1 + 124) )
        {
          v47 = ((unsigned __int16)(*(_WORD *)(a1 + 136) ^ *(_WORD *)(v10 + 8)) >> 8) ^ ((unsigned int)(*(_DWORD *)(a1 + 136) ^ *(_DWORD *)(v10 + 8)) >> 16);
          v108 = *(_DWORD *)(a1 + 136) ^ *(_DWORD *)(v10 + 8);
          if ( HIBYTE(v108) != ((unsigned __int8)v108 ^ v47) )
            goto LABEL_156;
        }
      }
      else if ( !(unsigned __int8)RtlpValidateLFHBlock(a1, v10) )
      {
LABEL_156:
        RtlpLogHeapFailure(3, a1, v10, v8, 0LL, 0LL);
        goto LABEL_131;
      }
      if ( *(char *)(v10 + 15) >= 0 )
      {
        if ( *(_DWORD *)(a1 + 124) )
        {
          v50 = *(_DWORD *)(v10 + 8);
          v105 = v50;
          if ( (v50 & *(_DWORD *)(a1 + 124)) != 0 )
            v105 = *(_DWORD *)(a1 + 136) ^ v50;
          v49 = v105;
        }
        else
        {
          v49 = *(_WORD *)(v10 + 8);
        }
      }
      else
      {
        if ( (unsigned __int16)RtlpLFHKey ^ (unsigned __int16)(*(_WORD *)(v10 + 8) ^ a1 ^ (v10 >> 4)) )
          v48 = 0LL;
        else
          v48 = *(_QWORD *)(v10
                          - ((unsigned __int64)((unsigned int)RtlpLFHKey ^ *(_DWORD *)(v10 + 8) ^ (unsigned int)a1 ^ (unsigned int)(v10 >> 4)) >> 12));
        v49 = *(_WORD *)(v48 + 36);
      }
      v51 = v49;
      if ( *(_BYTE *)(v10 + 15) == 4 )
      {
        if ( *(_DWORD *)(a1 + 124) )
        {
          v52 = *(_DWORD *)(v10 + 8);
          v105 = v52;
          if ( (v52 & *(_DWORD *)(a1 + 124)) != 0 )
            v105 = *(_DWORD *)(a1 + 136) ^ v52;
          v53 = v105;
        }
        else
        {
          v53 = *(_WORD *)(v10 + 8);
        }
        v54 = v51 + *(_QWORD *)(v10 - 16) - v53;
      }
      else
      {
        v54 = 16LL * v49;
      }
      if ( v54 + v10 < v8 )
        goto LABEL_156;
      if ( (a3 & 0x3C000102) != 0
        || (*(_BYTE *)(v8 - 1) != 5 ? (v55 = 0LL) : (v55 = v8 - 16LL * *(unsigned __int8 *)(v8 - 16 + 14)),
            (int)RtlpCallInterceptRoutine(*(_DWORD *)(v8 - 8), a1, v8, 3, v55) >= 0) )
      {
LABEL_74:
        if ( *(char *)(v10 + 15) < 0 )
        {
          v24 = 1;
          if ( (unsigned __int16)RtlpLFHKey ^ (unsigned __int16)(a1 ^ *(_WORD *)(v10 + 8) ^ (v10 >> 4))
            || (v5 = *(_QWORD *)(v10
                               - ((unsigned __int64)((unsigned int)RtlpLFHKey ^ (unsigned int)a1 ^ *(_DWORD *)(v10 + 8) ^ (unsigned int)(v10 >> 4)) >> 12))) == 0 )
          {
            RtlpLogHeapFailure(3, a1, v10, 0, v5, v5);
          }
          else
          {
            _m_prefetchw((const void *)v5);
            v56 = (unsigned __int16)(*(_DWORD *)(v10 + 12) >> 8);
            v57 = *(_QWORD *)v5;
            v104 = *(_QWORD *)(v5 + 8);
            v103 = v56;
            v58 = *(_QWORD *)(*(_QWORD *)v57 + 24LL);
            v59 = RtlpLFHKey ^ v58 ^ v104 ^ *(_DWORD *)(v104 + 24);
            v60 = (unsigned __int16)v59;
            v61 = (unsigned int)v56 * HIWORD(v59);
            if ( v104 + v61 + v60 == v10 )
            {
              if ( (unsigned int)RtlGetCurrentServiceSessionId(v61, v56) )
                v63 = (__int64)NtCurrentPeb()->SharedData + 550;
              else
                v63 = 2147353472LL;
              if ( *(_BYTE *)v63 && (NtCurrentPeb()->TracingFlags & 1) != 0 )
                RtlpLogHeapFreeEvent(*(_QWORD *)(v58 + 24), v10 + 16, 2LL, v62);
              v64 = 0;
              if ( MEMORY[0x7FFE036A] > 1u )
                v64 = 100;
              v65 = 0;
              while ( 1 )
              {
                v66 = *(_DWORD *)(v5 + 32);
                if ( (v66 & 0x80000000) == 0
                  && v66 == _InterlockedCompareExchange((volatile signed __int32 *)(v5 + 32), v66 | 0x80000000, v66) )
                {
                  break;
                }
                if ( ++v65 > v64 )
                  goto LABEL_184;
              }
              if ( v66 == -1 )
              {
LABEL_184:
                *(_BYTE *)(v10 + 15) = 0x80;
                v72 = v10 + 16;
                v73 = v5 + 16;
                goto LABEL_185;
              }
              v67 = (unsigned int)v103;
              *(_BYTE *)(v10 + 15) = 0x80;
              _bittestandreset64(*(signed __int64 **)(v104 + 40), v67);
              if ( *(_WORD *)(v5 + 16) )
              {
                v68 = (_QWORD *)RtlpInterlockedFlushSList(v5 + 16);
                v69 = 0;
                if ( v68 )
                {
                  do
                  {
                    v70 = *((_DWORD *)v68 - 1);
                    ++v69;
                    v68 = (_QWORD *)*v68;
                    v71 = *(signed __int64 **)(v104 + 40);
                    v103 = (unsigned __int16)(v70 >> 8);
                    _bittestandreset64(v71, v103);
                  }
                  while ( v68 );
                  v24 = 1;
                }
              }
              else
              {
                v69 = 0;
              }
              v74 = v69 + 1 + (unsigned __int16)v66;
              v75 = *(_QWORD *)v5;
              v76 = ((_DWORD)v103 << 16) | v74;
              if ( (_WORD)v76 == *(_WORD *)(v5 + 40) )
              {
                v77 = (_QWORD *)*(unsigned int *)(v75 + 168);
                v78 = *(unsigned int *)(*(_QWORD *)v75 + 32LL);
                if ( *(_DWORD *)(v75 + 164) != 1
                  || (unsigned int)v78 < (unsigned int)v77
                  || (v78 = (unsigned int)(v78 - (_DWORD)v77), (unsigned int)v78 >= *(_DWORD *)(*(_QWORD *)v75 + 36LL)) )
                {
                  v79 = *(__int64 **)v5;
                  v80 = (_DWORD *)_InterlockedExchange64((volatile __int64 *)(*(_QWORD *)v5 + 8LL), 0LL);
                  if ( !v80 )
                    goto LABEL_197;
                  _m_prefetchw(v80 + 11);
                  if ( _InterlockedAnd(v80 + 11, 0xFFFFFFF9) == 6 )
                  {
                    v81 = **(_QWORD **)v80;
                    *(_QWORD *)v80 = 0LL;
LABEL_195:
                    v82 = v80 + 12;
LABEL_196:
                    RtlpInterlockedPushEntrySList(v81, v82);
                    goto LABEL_197;
                  }
                  if ( !RtlpIsSubSegmentReuseable((__int64)v79, (__int64)v80) )
                    goto LABEL_197;
                  do
                  {
                    v91 = v80[11];
                    if ( !v91 || (v91 & 2) != 0 )
                      goto LABEL_197;
                  }
                  while ( v91 != _InterlockedCompareExchange(v80 + 11, v91 | 2, v91) );
                  v77 = *(_QWORD **)v80;
                  v92 = 0;
                  while ( 1 )
                  {
                    v93 = ((_BYTE)v92 + (unsigned __int8)*((_WORD *)v77 + 87)) & 0xF;
                    v78 = v77[v93 + 2];
                    v75 = (__int64)&v77[v93];
                    if ( v78 )
                    {
                      if ( (*(_DWORD *)(v78 + 44) & 1) == 0
                        && v78 == _InterlockedCompareExchange64(
                                    (volatile signed __int64 *)(v75 + 16),
                                    (signed __int64)v80,
                                    v78) )
                      {
                        _m_prefetchw((const void *)(v78 + 44));
                        if ( _InterlockedAnd((volatile signed __int32 *)(v78 + 44), 0xFFFFFFFD) == 2 )
                        {
                          v81 = **(_QWORD **)v78;
                          *(_QWORD *)v78 = 0LL;
                          v82 = (_DWORD *)(v78 + 48);
                          goto LABEL_196;
                        }
LABEL_197:
                        v83 = *v79;
                        v84 = *(_QWORD *)v5;
                        v85 = *(_QWORD *)(v83 + 24);
                        if ( (*(_BYTE *)(v5 + 38) & 3) != 0 )
                        {
                          v107 = (*(_QWORD *)(v5 + 8) + 4151LL) & 0xFFFFFFFFFFFFF000uLL;
                          ReservedBlockSize = RtlpGetReservedBlockSize(v5, v78, v75, v77);
                          v87 = *(_QWORD *)(v85 + 24);
                          v106 = 16 * ReservedBlockSize * (unsigned __int64)*(unsigned __int16 *)(v5 + 40);
                          HeapProtection = RtlpGetHeapProtection(v87, 1LL);
                          ZwProtectVirtualMemory(-1LL, &v107, &v106, HeapProtection, v102);
                        }
                        *(_DWORD *)(*(_QWORD *)(v5 + 8) + 20LL) = 0;
                        RtlpFreeUserBlock(v85, *(__int64 **)(v5 + 8));
                        v89 = -*(unsigned __int16 *)(v5 + 40);
                        do
                        {
                          v90 = *(_QWORD *)(v84 + 160);
                          LODWORD(v109) = v90 + v89;
                          HIDWORD(v109) = HIDWORD(v90) - 1;
                        }
                        while ( v90 != _InterlockedCompareExchange64((volatile signed __int64 *)(v84 + 160), v109, v90) );
                        *(_QWORD *)(v5 + 8) = 0LL;
                        _InterlockedIncrement((volatile signed __int32 *)(v85 + 60));
                        *(_DWORD *)(v5 + 32) = 0;
                        _m_prefetchw((const void *)(v5 + 44));
                        if ( _InterlockedAnd((volatile signed __int32 *)(v5 + 44), 0xFFFFFFFE) == 1 )
                        {
                          v72 = v5 + 48;
                          v73 = **(_QWORD **)v5;
                          *(_QWORD *)v5 = 0LL;
                          goto LABEL_185;
                        }
                        goto LABEL_186;
                      }
                    }
                    else if ( !_InterlockedCompareExchange64(
                                 (volatile signed __int64 *)(v75 + 16),
                                 (signed __int64)v80,
                                 0LL) )
                    {
                      goto LABEL_197;
                    }
                    if ( (unsigned int)++v92 >= 0x10 )
                    {
                      v81 = *(_QWORD *)(*(_QWORD *)(**(_QWORD **)v80 + 24LL)
                                      + 8LL * *(unsigned __int16 *)(*(_QWORD *)v80 + 172LL)
                                      + 1192)
                          + 144LL;
                      goto LABEL_195;
                    }
                  }
                }
              }
              *(_DWORD *)(v5 + 32) = v76;
              if ( (*(_DWORD *)(v5 + 44) & 2) != 0 || !RtlpIsSubSegmentReuseable(v75, v5) )
                goto LABEL_186;
              do
              {
                v94 = *(_DWORD *)(v5 + 44);
                if ( !v94 || (v94 & 2) != 0 )
                  goto LABEL_186;
              }
              while ( v94 != _InterlockedCompareExchange((volatile signed __int32 *)(v5 + 44), v94 | 2, v94) );
              v95 = *(_QWORD **)v5;
              for ( i = 0; i < 0x10; ++i )
              {
                v97 = ((_BYTE)i + (unsigned __int8)*((_WORD *)v95 + 87)) & 0xF;
                v98 = (volatile signed __int32 *)v95[v97 + 2];
                v99 = &v95[v97];
                if ( v98 )
                {
                  if ( (v98[11] & 1) == 0
                    && v98 == (volatile signed __int32 *)_InterlockedCompareExchange64(v99 + 2, v5, (signed __int64)v98) )
                  {
                    _m_prefetchw((const void *)(v98 + 11));
                    v5 = 0LL;
                    if ( _InterlockedAnd(v98 + 11, 0xFFFFFFFD) == 2 )
                    {
                      v100 = **(_QWORD **)v98;
                      *(_QWORD *)v98 = 0LL;
                      RtlpInterlockedPushEntrySList(v100, v98 + 12);
                    }
                    goto LABEL_76;
                  }
                }
                else if ( !_InterlockedCompareExchange64(v99 + 2, v5, 0LL) )
                {
                  goto LABEL_186;
                }
              }
              v72 = v5 + 48;
              v73 = *(_QWORD *)(*(_QWORD *)(**(_QWORD **)v5 + 24LL)
                              + 8LL * *(unsigned __int16 *)(*(_QWORD *)v5 + 172LL)
                              + 1192)
                  + 144LL;
LABEL_185:
              RtlpInterlockedPushEntrySList(v73, v72);
LABEL_186:
              v5 = 0LL;
            }
            else
            {
              v5 = 0LL;
              RtlpLogHeapFailure(3, *(_QWORD *)(v58 + 24), v10, 0, 0LL, 0LL);
            }
          }
LABEL_76:
          if ( a5 )
            *a5 = v5;
          if ( a4 )
            *a4 = v5;
          if ( (dword_180160378 & 1) != 0
            && (dword_180160378 & 2) != 0
            && a1 != RtlpHpMetadataHeap
            && NtCurrentPeb()->ProcessHeap
            && (a3 & 0x10000000) == 0 )
          {
            goto LABEL_235;
          }
          return v24;
        }
LABEL_75:
        v40 = RtlpFreeHeap(a1, a3 | 2u, v10, v8);
        v24 = v40;
        if ( !v40 )
          return v24;
        goto LABEL_76;
      }
    }
LABEL_131:
    NtCurrentTeb()->LastStatusValue = -1073741811;
    v46 = NtCurrentTeb();
    v46->LastErrorValue = RtlNtStatusToDosError(-1073741811);
    return 0;
  }
  if ( (RtlpHpAppCompatFlags & 2) != 0 )
  {
    if ( a2 )
    {
      if ( (_WORD)a2 || (v41 = RtlCSparseBitmapBitmaskRead(a1, 2 * (a2 >> 20))) != 0 && (_DWORD)v41 != 3 )
        v8 -= 16LL;
    }
  }
  v11 = a3 & 1 | 2;
  if ( (a3 & 8) == 0 )
    v11 = a3 & 1;
  if ( (a3 & 4) != 0 )
    v11 |= 0x80000000;
  if ( (a3 & 0x100) != 0 )
    v11 |= 0x100u;
  if ( (a3 & 0xE00) != 0 )
    v11 |= a3 & 0xE00;
  if ( (a3 & 0x10) != 0 )
    v11 |= 0x2000000u;
  if ( (a3 & 2) != 0 )
    v11 |= 0x1000000u;
  v12 = 0;
  v13 = v11 | *(_DWORD *)(a1 + 20);
  v14 = *(_DWORD *)(a1 + 64);
  v15 = v13 & 0x11000001;
  if ( v14 )
    LOBYTE(v12) = v14 == LODWORD(NtCurrentTeb()->ClientId.UniqueThread);
  v16 = v15 | 1;
  v17 = 0LL;
  if ( !v12 )
    v16 = v15;
  if ( a5 )
  {
    v17 = RtlpHpExtrasGet(a1, v8, v16, a4);
    if ( (unsigned __int64)(v17 - 1) > 0xFFFFFFFFFFFFFFFDuLL )
      *a5 = 0;
    else
      *a5 = *(_WORD *)v17;
  }
  else if ( a4 )
  {
    *a4 = RtlpHpSizeHeapInternal(a1, v8, v16, 0LL);
  }
  if ( (v16 & 0x1000000) == 0 )
  {
    if ( *(_DWORD *)(a1 + 56) )
    {
      if ( (v17 || (v17 = RtlpHpExtrasGet(a1, v8, v16, a4)) != 0) && v17 != -1 && (*(_BYTE *)(v17 + 2) & 0xF) != 0 )
      {
        if ( (RtlpHpAppCompatFlags & 2) != 0 )
        {
          if ( (_WORD)v8 || (v43 = RtlCSparseBitmapBitmaskRead(v15, 2 * (v8 >> 20))) != 0 && (_DWORD)v43 != 3 )
            v42 = v8 + 16;
          else
            v42 = v8;
        }
        else
        {
          v42 = v8;
        }
        if ( (int)RtlpCallInterceptRoutine(*(_BYTE *)(v17 + 2) & 0xF, a1, v42, 3, v17 + 16) < 0 )
          goto LABEL_107;
      }
    }
  }
  if ( (*(_DWORD *)(a1 + 20) & 0x10000000) != 0 && RtlpHpSizeHeap(a1, v8, v16) == -1 )
  {
    RtlpLogHeapFailure(9, a1, v8, 0, 0LL, 0LL);
LABEL_107:
    v24 = 0;
    goto LABEL_108;
  }
  if ( (_WORD)v8 )
  {
    v18 = 0;
LABEL_27:
    v19 = 104LL * v18;
    v20 = v19 + a1 + 112;
    if ( (RtlpHpAppCompatFlags & 1) != 0 )
    {
      DescriptorValidateSafe = RtlpHpSegGetDescriptorValidateSafe(v19 + a1 + 112, v8);
    }
    else
    {
      v21 = v8 & *(_QWORD *)v20;
      if ( (RtlpHeapKey ^ v20 ^ v21 ^ *(_QWORD *)(v21 + 0x10)) == 0xA2E64EADA2E64EADuLL )
        DescriptorValidateSafe = v21 + 32 * ((unsigned __int64)(unsigned int)(v8 - v21) >> *(_BYTE *)(v20 + 8));
      else
        DescriptorValidateSafe = 0LL;
    }
    if ( !DescriptorValidateSafe || (*(_BYTE *)(DescriptorValidateSafe + 24) & 1) == 0 )
      goto LABEL_121;
    if ( (*(_BYTE *)(DescriptorValidateSafe + 24) & 2) != 0 )
    {
      if ( (*(_BYTE *)(DescriptorValidateSafe + 24) & 0xC) == 0
        && v8 != (DescriptorValidateSafe & *(_QWORD *)v20)
               + ((DescriptorValidateSafe - (DescriptorValidateSafe & *(_QWORD *)v20)) >> 5 << *(_BYTE *)(v20 + 8)) )
      {
        goto LABEL_121;
      }
    }
    else
    {
      DescriptorValidateSafe += -32LL * *(unsigned __int8 *)(DescriptorValidateSafe + 31);
      if ( (*(_BYTE *)(DescriptorValidateSafe + 24) & 1) == 0
        || (*(_BYTE *)(DescriptorValidateSafe + 24) & 2) == 0
        || (*(_BYTE *)(DescriptorValidateSafe + 24) & 0xC) == 0 )
      {
        goto LABEL_121;
      }
    }
    if ( DescriptorValidateSafe )
    {
      v23 = (DescriptorValidateSafe & *(_QWORD *)v20)
          + ((DescriptorValidateSafe - (DescriptorValidateSafe & *(_QWORD *)v20)) >> 5 << *(_BYTE *)(v20 + 8));
      if ( v8 <= v23 )
      {
        RtlpHpSegPageRangeShrink(v20, DescriptorValidateSafe, 0, v16);
        v24 = 1;
        if ( (unsigned int)RtlGetCurrentServiceSessionId(v32, v31) )
          v34 = (__int64)NtCurrentPeb()->SharedData + 550;
        else
          v34 = 2147353472LL;
        if ( *(_BYTE *)v34 && (NtCurrentPeb()->TracingFlags & 1) != 0 )
          RtlpLogHeapFreeEvent(*(_QWORD *)(v20 + 96), v8, 3LL, v33);
      }
      else
      {
        if ( (*(_BYTE *)(DescriptorValidateSafe + 24) & 4) != 0 )
        {
          v24 = RtlpHpLfhSubsegmentFreeBlock(*(_QWORD *)(v20 + 80), v23, v8, v16);
        }
        else
        {
          v24 = RtlpHpVsContextFree(*(_QWORD *)(v20 + 88), v23, v8, v16, &v101);
          if ( v24 )
          {
            if ( v101 <= 0x3FF0 )
            {
              v29 = *(_QWORD *)(v20 + 80);
              v30 = RtlpLfhBucketIndexMap[(unsigned __int64)(v101 + 15) >> 4];
              if ( (*(_QWORD *)(v29 + 8 * v30 + 208) & 1) != 0 )
                RtlpLfhBucketUsageUpdate(v29, v30, 0LL);
            }
          }
        }
        SharedData = NtCurrentPeb()->SharedData;
        if ( SharedData && *SharedData )
          v27 = (__int64)NtCurrentPeb()->SharedData + 550;
        else
          v27 = 2147353472LL;
        if ( *(_BYTE *)v27 && (NtCurrentPeb()->TracingFlags & 1) != 0 )
        {
          if ( !v24 )
            goto LABEL_108;
          RtlpLogHeapFreeEvent(*(_QWORD *)(v20 + 96), v8, (~*(_BYTE *)(DescriptorValidateSafe + 24) & 4 | 8u) >> 2, v25);
        }
      }
      goto LABEL_43;
    }
LABEL_121:
    RtlpLogHeapFailure(9, *(_QWORD *)(v20 + 96), v8, 0, 0LL, 0LL);
    v24 = 0;
    goto LABEL_43;
  }
  v35 = RtlCSparseBitmapBitmaskRead(v15, 2 * (v8 >> 20));
  if ( v35 )
  {
    v18 = v35 - 1;
    if ( v18 != 2 )
      goto LABEL_27;
  }
  v24 = RtlpHpLargeFree(a1, v8, v16) != 0;
  if ( (unsigned int)RtlGetCurrentServiceSessionId(v37, v36) )
    v39 = (__int64)NtCurrentPeb()->SharedData + 550;
  else
    v39 = 2147353472LL;
  if ( *(_BYTE *)v39 && (NtCurrentPeb()->TracingFlags & 1) != 0 )
  {
    if ( !v24 )
      goto LABEL_108;
    RtlpLogHeapFreeEvent(a1, v8, 3LL, v38);
  }
LABEL_43:
  if ( !v24 )
  {
LABEL_108:
    NtCurrentTeb()->LastStatusValue = -1073741811;
    v44 = NtCurrentTeb();
    v44->LastErrorValue = RtlNtStatusToDosError(-1073741811);
    return v24;
  }
  if ( a4 )
  {
    v45 = *a4;
    if ( (RtlpHpAppCompatFlags & 2) != 0 )
      v45 -= 16LL;
    *a4 = v45;
  }
  if ( (dword_180160378 & 1) != 0
    && (dword_180160378 & 2) != 0
    && a1 != RtlpHpMetadataHeap
    && NtCurrentPeb()->ProcessHeap )
  {
LABEL_235:
    RtlpHpStackTraceRemoveStack(a1, v8);
  }
  return v24;
}
