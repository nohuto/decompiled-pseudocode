...
__int64 __fastcall RtlpHpLargeAllocationDestroy(unsigned __int64 a1, __int64 a2)
{
  const GUID *v4; // r9
  __int64 v5; // rcx
  const GUID *v7; // [rsp+38h] [rbp+10h]

  v7 = (const GUID *)(*(_QWORD *)(a1 + 24) & 0xFFFFFFFFFFFF0000uLL);
  ZwFreeVirtualMemory();
  if ( (RtlpHpHeapFeatures & 8) != 0 )
    RtlpHpTlLogVAChange(0x8000, 0LL, v7, v4);
  if ( (unsigned int)RtlGetCurrentServiceSessionId() )
    v5 = (__int64)NtCurrentPeb()->SharedData + 558;
  else
    v5 = 2147353480LL;
  if ( *(_BYTE *)v5 )
    RtlpHeapLogRangeRelease(a2, (__int64)v7, 0LL);
