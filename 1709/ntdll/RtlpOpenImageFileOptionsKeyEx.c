/*
 * XREFs of RtlpOpenImageFileOptionsKeyEx @ 0x180079A84
 * Callers:
 ...
 *     RtlOpenImageFileOptionsKey @ 0x1800E0F20 (RtlOpenImageFileOptionsKey.c)
...
LABEL_7:
  ...
  if ( (int)result >= 0 )
  {
    ...
    if ( inited >= 0 )
    {
      ...
      {
        Handle[0] = 0LL;
        IFEOKeyFilterKey = RtlpCreateIFEOKeyFilterKey(Handle, v28, v5);
        v18 = Handle[0];
        ...
        if ( IFEOKeyFilterKey >= 0 )
        {
          ...
          if ( inited >= 0 )
          {
            inited = ZwSetValueKey(v18, Handle, 0LL, 1LL, *((_QWORD *)a1 + 1), a1[1]);
            if ( inited >= 0 )
            {
              v29 = 1;
              inited = RtlInitUnicodeStringEx((__int64)Handle, (__int64)L"UseFilter");
              if ( inited >= 0 )
                inited = ZwSetValueKey(v28, Handle, 0LL, 4LL, &v29, 4);
            }
          }
        }
        NtClose(v28);
