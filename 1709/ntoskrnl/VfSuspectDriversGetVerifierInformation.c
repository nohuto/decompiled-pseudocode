/*
 * XREFs of VfSuspectDriversGetVerifierInformation @ 0x1407BD404
 * Callers:
 *     VfGetVerifierInformation @ 0x1407CA3C4 (VfGetVerifierInformation.c)
 * Callees:
 *     KeReleaseMutex @ 0x1400760B0 (KeReleaseMutex.c)
 *     VfAvlCleanupLockContext @ 0x1400F9A6C (VfAvlCleanupLockContext.c)
 *     memmove @ 0x140192C00 (memmove.c)
 *     VfAvlInitializeLockContext @ 0x14027812C (VfAvlInitializeLockContext.c)
 *     VfDriverLock @ 0x1407A2530 (VfDriverLock.c)
 *     VfTargetDriversGetZeroCounters @ 0x1407B6B8C (VfTargetDriversGetZeroCounters.c)
 *     VfAvlEnumerateNodes @ 0x1407BDE78 (VfAvlEnumerateNodes.c)
 */

__int64 __fastcall VfSuspectDriversGetVerifierInformation(
        __int64 a1,
        unsigned int a2,
        unsigned int *a3,
        __int64 a4,
        int a5)
{
  __int64 v6; // rdi
  unsigned int v7; // r14d
  int v8; // ebp
  __int64 *v9; // r15
  __int64 *v10; // r8
  int v11; // ebx
  __int64 v12; // r8
  __int64 v13; // rcx
  void *v14; // rcx
  _BYTE v16[16]; // [rsp+20h] [rbp-78h] BYREF
  _QWORD v17[2]; // [rsp+30h] [rbp-68h] BYREF
  int v18; // [rsp+40h] [rbp-58h]
  __int64 *v21; // [rsp+B0h] [rbp+18h]

  v6 = a1;
  *a3 = 0;
  v7 = 0;
  v8 = 0;
  VfDriverLock();
  v9 = (__int64 *)VfSuspectDriversList;
  if ( (__int64 *)VfSuspectDriversList != &VfSuspectDriversList )
  {
    while ( 1 )
    {
      v10 = v9;
      v6 += v7;
      v21 = v9;
      v9 = (__int64 *)*v9;
      v11 = *((unsigned __int16 *)v10 + 12);
      v7 = (v11 + (a5 != 0 ? 273 : 137)) & 0xFFFFFFF8;
      *a3 += v7;
      if ( *a3 > a2 )
        break;
      *(_DWORD *)v6 = v7;
      *(_DWORD *)(v6 + 4) = MmVerifierData;
      *(_DWORD *)(v6 + 24) = dword_14036A0C4;
      *(_DWORD *)(v6 + 28) = dword_14036A0C8;
      *(_DWORD *)(v6 + 76) = dword_14036A0F8;
      *(_DWORD *)(v6 + 32) = dword_14036A0CC;
      *(_DWORD *)(v6 + 36) = dword_14036A0D0;
      *(_DWORD *)(v6 + 40) = dword_14036A0D4;
      *(_DWORD *)(v6 + 44) = dword_14036A0D8;
      *(_DWORD *)(v6 + 48) = dword_14036A0DC;
      *(_DWORD *)(v6 + 52) = dword_14036A0E0;
      *(_DWORD *)(v6 + 56) = dword_14036A0E4;
      *(_DWORD *)(v6 + 60) = dword_14036A0E8;
      *(_DWORD *)(v6 + 64) = dword_14036A0EC;
      *(_DWORD *)(v6 + 68) = *((_DWORD *)v10 + 4);
      *(_DWORD *)(v6 + 72) = *((_DWORD *)v10 + 5);
      if ( *((_DWORD *)v10 + 4) <= *((_DWORD *)v10 + 5) )
      {
        VfTargetDriversGetZeroCounters(v6, a5);
      }
      else
      {
        VfTargetDriversGetZeroCounters(v6, a5);
        v17[0] = v12;
        v17[1] = v6;
        v18 = a5;
        VfAvlInitializeLockContext((__int64)v16, 1);
        VfAvlEnumerateNodes(v13, v16, ViTargetAddCountersCallback, v17);
        VfAvlCleanupLockContext((__int64)v16);
      }
      *(_WORD *)(v6 + 8) = v11;
      *(_WORD *)(v6 + 10) = v11 + 2;
      v14 = (void *)(v6 + (a5 != 0 ? 264 : 128));
      *(_QWORD *)(v6 + 16) = v14;
      memmove(v14, (const void *)v21[4], (unsigned __int16)v11);
      *(_WORD *)(*(_QWORD *)(v6 + 16) + 2 * ((unsigned __int64)(unsigned __int16)v11 >> 1)) = 0;
      *(_QWORD *)(v6 + 16) += a4 - a1;
      if ( v9 == &VfSuspectDriversList )
        goto LABEL_9;
    }
    v8 = -1073741820;
  }
LABEL_9:
  ViDriversLoadLockOwner = 0LL;
  KeReleaseMutex(&ViDriversLoadLock, 0);
  if ( v8 >= 0 )
    *(_DWORD *)v6 = 0;
  return (unsigned int)v8;
}
