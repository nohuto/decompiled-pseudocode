/*
 * XREFs of SepAdtAuditThisEventByCategoryWithContext @ 0x14046BE30
 * Callers:
        ...
        struct _SECURITY_SUBJECT_CONTEXT *a3)
{
  ...
  unsigned int i; // r10d
  int v19; // esi
  struct _SECURITY_SUBJECT_CONTEXT SubjectContext; // [rsp+20h] [rbp-38h] BYREF
  ...
  if ( SepTokenPolicyCounterByCategory[a1] )
  {
    ...
    if ( p_SubjectContext->ClientToken || (ClientToken = p_SubjectContext->PrimaryToken) != 0LL )
    {
      if ( ClientToken[118] == 2 )
      {
        ...
        {
          v19 = (unsigned __int8)ClientToken[((unsigned __int64)v11 >> 1) + 88] >> (4 * (v11 & 1));
          if ( (v19 & 1) != 0 )
          {
            if ( (v5 & 2) != 0 )
              goto LABEL_29;
