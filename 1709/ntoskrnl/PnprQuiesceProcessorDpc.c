/*
 * XREFs of PnprQuiesceProcessorDpc @ 0x14042CDC0
 * Callers:
 *     <none>
 * Callees:
 *     KeSaveExtendedAndSupervisorState @ 0x14000A790 (KeSaveExtendedAndSupervisorState.c)
 *     KeRestoreExtendedAndSupervisorState @ 0x14000B274 (KeRestoreExtendedAndSupervisorState.c)
 *     KeGetProcessorNumberFromIndex @ 0x1400E6240 (KeGetProcessorNumberFromIndex.c)
 *     _guard_dispatch_icall @ 0x140189900 (_guard_dispatch_icall.c)
 *     PnprGetStackLimits @ 0x140200000 (PnprGetStackLimits.c)
 *     KeResumeClockTimerSafe @ 0x140203278 (KeResumeClockTimerSafe.c)
 *     KeSuspendClockTimerSafe @ 0x140203310 (KeSuspendClockTimerSafe.c)
 *     KeRestoreProcessorSpecificFeatures @ 0x140203D30 (KeRestoreProcessorSpecificFeatures.c)
 *     PnprMirrorMarkedPages @ 0x14042CA74 (PnprMirrorMarkedPages.c)
 */

void __fastcall PnprQuiesceProcessorDpc(
        struct _KDPC *Dpc,
        _BYTE *DeferredContext,
        PVOID SystemArgument1,
        PVOID SystemArgument2)
{
  unsigned int v4; // r14d
  int v5; // edi
  __int64 v6; // rax
  bool v7; // bl
  __int64 Group; // r15
  __int64 v9; // rbp
  bool v10; // si
  __int64 v11; // r8
  __int64 v12; // rcx
  int v13; // eax
  int v14; // eax
  PVOID v15[14]; // [rsp+20h] [rbp-78h] BYREF
  int v16; // [rsp+90h] [rbp-8h]
  unsigned int v17; // [rsp+A8h] [rbp+10h] BYREF
  _PROCESSOR_NUMBER ProcNumber; // [rsp+B0h] [rbp+18h] BYREF
  __int64 v19; // [rsp+B8h] [rbp+20h] BYREF

  v4 = (unsigned int)SystemArgument2;
  v19 = 0LL;
  v5 = (int)SystemArgument1;
  ProcNumber = 0;
  v6 = PnprContext;
  *DeferredContext = 1;
  v7 = 1;
  _InterlockedAdd((volatile signed __int32 *)(v6 + 184), 1u);
  while ( *(_DWORD *)(PnprContext + 184) < (int)SystemArgument1 )
    _mm_pause();
  if ( (*(_DWORD *)(PnprContext + 64) & 0x20) != 0 )
  {
    _disable();
    v7 = (v16 & 0x200) != 0;
    _InterlockedAdd((volatile signed __int32 *)(PnprContext + 192), 1u);
    while ( *(_DWORD *)(PnprContext + 192) < (int)SystemArgument1 )
      _mm_pause();
  }
  KeGetProcessorNumberFromIndex((ULONG)SystemArgument2, &ProcNumber);
  Group = ProcNumber.Group;
  v9 = 1LL << ProcNumber.Number;
  if ( ((1LL << ProcNumber.Number) & *(_QWORD *)(**(_QWORD **)(PnprContext + 16) + 8LL * ProcNumber.Group)) != 0 )
  {
    while ( *(int *)(PnprContext + 200) < 1 )
      _mm_pause();
    KeSuspendClockTimerSafe();
    v10 = 0;
    if ( ((MEMORY[0xFFFFF780000003D8] | MEMORY[0xFFFFF780000005F0]) & 0xFFFFFFFFFFFFFFFCuLL) != 0 )
      v10 = (int)KeSaveExtendedAndSupervisorState(
                   (MEMORY[0xFFFFF780000003D8] | MEMORY[0xFFFFF780000005F0]) & 0xFFFFFFFFFFFFFFFCuLL,
                   v15) >= 0;
    HalGetProcessorIdByNtNumber(v4, &v17);
    if ( (*(_DWORD *)(PnprContext + 20896) & 2) != 0 )
    {
      LOBYTE(v11) = 1;
      (*(void (__fastcall **)(_QWORD, _QWORD, __int64))(PnprContext + 20936))(
        *(_QWORD *)(PnprContext + 20880),
        v17,
        v11);
    }
    if ( (*(_DWORD *)(PnprContext + 64) & 0x20) != 0 )
      ((void (__fastcall *)(_QWORD, __int64 *))off_140353428[0])(*(_QWORD *)(PnprContext + 20872), &v19);
    ((void (__fastcall *)(_QWORD))off_1403533C0[0])(*(_QWORD *)(PnprContext + 20872));
    if ( (*(_DWORD *)(PnprContext + 20896) & 2) != 0 )
      (*(void (__fastcall **)(_QWORD, _QWORD, _QWORD))(PnprContext + 20936))(*(_QWORD *)(PnprContext + 20880), v17, 0LL);
    KeResumeClockTimerSafe();
    if ( (*(_DWORD *)(PnprContext + 64) & 0x20) != 0 && v7 )
      _enable();
    KeRestoreProcessorSpecificFeatures();
    if ( v10 )
      KeRestoreExtendedAndSupervisorState((__int64)v15);
    _InterlockedAdd((volatile signed __int32 *)(PnprContext + 204), 1u);
  }
  if ( *(_DWORD *)(*(_QWORD *)(PnprContext + 24) + 4LL) && (*(_DWORD *)(PnprContext + 64) & 8) == 0 )
  {
    while ( *(int *)(PnprContext + 200) < 2 )
      _mm_pause();
    if ( (int)PnprMirrorMarkedPages() < 0 )
    {
      v12 = PnprContext;
      v13 = *(_DWORD *)(PnprContext + 20984);
      if ( !v13 )
        v13 = 2241;
      *(_DWORD *)(PnprContext + 20984) = v13;
      v14 = *(_DWORD *)(v12 + 20988);
      if ( !v14 )
        v14 = 1;
      *(_DWORD *)(v12 + 20988) = v14;
    }
    _InterlockedAdd((volatile signed __int32 *)(PnprContext + 204), 1u);
  }
  while ( *(int *)(PnprContext + 200) < 4 )
    _mm_pause();
  PnprGetStackLimits((char **)(PnprContext + 216 + 8LL * v4), (unsigned __int64 *)(PnprContext + 10456 + 8LL * v4));
  _InterlockedAdd((volatile signed __int32 *)(PnprContext + 204), 1u);
  while ( *(int *)(PnprContext + 200) < 5 )
    _mm_pause();
  if ( (*(_DWORD *)(PnprContext + 64) & 0x20) != 0 )
  {
    while ( *(int *)(PnprContext + 200) < 6 )
      _mm_pause();
    if ( (v9 & *(_QWORD *)(**(_QWORD **)(PnprContext + 16) + 8 * Group)) != 0 )
    {
      ((void (__fastcall *)(__int64))off_140353430[0])(v19);
    }
    else if ( v7 )
    {
      _enable();
    }
  }
  _InterlockedAdd((volatile signed __int32 *)(PnprContext + 188), 1u);
  while ( *(_DWORD *)(PnprContext + 188) < v5 )
    _mm_pause();
}
