/*
 * XREFs of RtlIpv6AddressToStringExW @ 0x14012CC90
 * Callers:
 *     <none>
 * Callees:
 *     RtlIpv6AddressToStringW @ 0x14012CD70 (RtlIpv6AddressToStringW.c)
 *     __security_check_cookie @ 0x14015D720 (__security_check_cookie.c)
 *     swprintf_s @ 0x140163370 (swprintf_s.c)
 *     memmove @ 0x140192C00 (memmove.c)
 */

NTSTATUS __stdcall RtlIpv6AddressToStringExW(
        const struct in6_addr *Address,
        ULONG ScopeId,
        USHORT Port,
        PWSTR AddressString,
        PULONG AddressStringLength)
{
  WCHAR *p_Src; // rdx
  wchar_t *v9; // rax
  wchar_t *v10; // rbx
  ULONG v11; // ebx
  bool v12; // cf
  __int16 Src; // [rsp+20h] [rbp-D8h] BYREF
  WCHAR S; // [rsp+22h] [rbp-D6h] BYREF
  _BYTE v16[14]; // [rsp+A2h] [rbp-56h] BYREF

  if ( !Address || !AddressStringLength || !AddressString && *AddressStringLength )
    return -1073741811;
  if ( Port )
    Src = 91;
  p_Src = (WCHAR *)&Src;
  if ( Port )
    p_Src = &S;
  v9 = RtlIpv6AddressToStringW(Address, p_Src);
  v10 = v9;
  if ( ScopeId )
    v10 = &v9[swprintf_s(v9, (v16 - (_BYTE *)v9) >> 1, L"%%%u", ScopeId)];
  if ( Port )
    v10 += swprintf_s(v10, (v16 - (_BYTE *)v10) >> 1, L"]:%u", (unsigned __int16)__ROR2__(Port, 8));
  v11 = (((char *)v10 - (char *)&Src) >> 1) + 1;
  v12 = *AddressStringLength < v11;
  *AddressStringLength = v11;
  if ( v12 )
    return -1073741811;
  memmove(AddressString, &Src, 2LL * v11);
  return 0;
}
