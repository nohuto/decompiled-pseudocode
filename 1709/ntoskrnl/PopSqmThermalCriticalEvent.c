/*
 * XREFs of PopSqmThermalCriticalEvent @ 0x14070CB64
 * Callers:
...
char __fastcall PopSqmThermalCriticalEvent(__int64 a1, unsigned int a2, int a3)
{
  ...
  if ( DeviceNode )
  {
    DeviceAttachmentBaseRef = *(PDEVICE_OBJECT *)(a1 + 864);
    if ( pCallbackContext.LevelPlus1 > 5 )
    {
      ...
      if ( (_BYTE)DeviceAttachmentBaseRef )
      {
        ...
        LOBYTE(DeviceAttachmentBaseRef) = TlgWrite(&pCallbackContext, &unk_1402D2E77, 0LL, 0LL, 0xBu, &pData);
      }
    }
  }
  if ( v7 )
