/*
 * XREFs of RtlCreateRvaList @ 0x1404F6AE4
 * Callers:
 *     MiParseImageLoadConfig @ 0x1404F6FD8 (MiParseImageLoadConfig.c)
 * Callees:
 *     memmove @ 0x140192C00 (memmove.c)
 *     memset @ 0x140192F40 (memset.c)
 *     ExFreePoolWithTag @ 0x1402B2440 (ExFreePoolWithTag.c)
 *     ExAllocatePoolWithTag @ 0x1402B3110 (ExAllocatePoolWithTag.c)
 *     RtlpCompressRvaList @ 0x1404D3E70 (RtlpCompressRvaList.c)
 */

__int64 __fastcall RtlCreateRvaList(__int64 a1, __int64 a2, __int64 a3, __int64 a4, void *Src, SIZE_T a6, __int64 *a7)
{
  __int64 v7; // r14
  __int64 v9; // rbx
  int v10; // edi
  __int64 v11; // rbp
  unsigned __int64 v12; // rdi
  __int64 v13; // rsi
  unsigned __int64 v14; // r12
  _QWORD *PoolWithTag; // rax
  SIZE_T v16; // r8
  char *v17; // rsi
  __int64 v18; // r8
  __int64 v19; // r9
  const void *v20; // rdx
  char *v21; // rsi
  __int64 v23; // [rsp+88h] [rbp+10h] BYREF
  unsigned int v24; // [rsp+90h] [rbp+18h] BYREF
  int v25; // [rsp+94h] [rbp+1Ch]

  v25 = HIDWORD(a3);
  v7 = (unsigned int)a4;
  v24 = 0;
  v23 = 0LL;
  v9 = 0LL;
  v10 = RtlpCompressRvaList(0LL, a1, a3, a4, 0LL, &v23, (int *)&v24);
  if ( v10 >= 0 )
  {
    v11 = v24;
    if ( (unsigned int)v7 <= 1 )
      v12 = 0LL;
    else
      v12 = (((unsigned __int64)((unsigned int)v7 * v24) + 63) >> 3) & 0x1FFFFFFFFFFFFFF8LL;
    v13 = v23;
    v14 = (v23 + 7) & 0xFFFFFFFFFFFFFFF8uLL;
    a6 = v12 + 4 * v7 + v14 + 64;
    PoolWithTag = ExAllocatePoolWithTag(PagedPool, a6, 0x4C617652u);
    v9 = (__int64)PoolWithTag;
    if ( PoolWithTag )
    {
      v16 = a6;
      PoolWithTag[3] = v13;
      v17 = (char *)(PoolWithTag + 8);
      *PoolWithTag = v11;
      *((_DWORD *)PoolWithTag + 2) = v7;
      PoolWithTag[2] = PoolWithTag + 8;
      memset(PoolWithTag + 8, 0, v16 - 64);
      v20 = Src;
      v21 = &v17[v14];
      if ( Src )
      {
        *(_QWORD *)(v9 + 48) = v21;
        memmove(v21, v20, 4 * v7);
        v21 += 4 * v7;
      }
      if ( v12 )
      {
        *(_QWORD *)(v9 + 40) = v21;
        *(_QWORD *)(v9 + 32) = (unsigned int)(v7 * v11);
      }
      v10 = RtlpCompressRvaList(v9, a1, v18, v19, *(_DWORD **)(v9 + 16), &v23, (int *)&v24);
      if ( v10 < 0 )
      {
        ExFreePoolWithTag((PVOID)v9, 0x4C617652u);
        v9 = 0LL;
      }
    }
    else
    {
      v10 = -1073741670;
    }
  }
  *a7 = v9;
  return (unsigned int)v10;
}
