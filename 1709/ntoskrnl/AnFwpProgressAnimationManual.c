/*
 * XREFs of AnFwpProgressAnimationManual @ 0x1407D3DA8
 * Callers:
 *     AnFwDisplayProgressIndicator @ 0x1407CFA7C (AnFwDisplayProgressIndicator.c)
 * Callees:
 *     BgpFwQueryPerformanceCounter @ 0x14013B95C (BgpFwQueryPerformanceCounter.c)
 *     BgpTxtDisplayCharacter @ 0x14013BA14 (BgpTxtDisplayCharacter.c)
 *     LogFwStat @ 0x1407CFD10 (LogFwStat.c)
 */

LONGLONG AnFwpProgressAnimationManual()
{
  LARGE_INTEGER v0; // rdi
  LONGLONG v1; // rbx
  LONGLONG v2; // rdx
  LONGLONG result; // rax
  __int16 v4; // cx
  __int64 v5; // rbx
  LARGE_INTEGER v6; // [rsp+60h] [rbp+8h] BYREF

  v0 = BgpFwQueryPerformanceCounter(&v6);
  v1 = v6.QuadPart / 33;
  v2 = 10 * (v6.QuadPart / 33) / 100;
  result = qword_14038D878;
  if ( v2 + v0.QuadPart >= qword_14038D878 + v6.QuadPart / 33 )
  {
    v4 = word_1403581C4;
    if ( word_1403581C4 != -7989 && (qword_14038D878 - v2 - v0.QuadPart) / v1 >= 2 )
      v4 = ++word_1403581C4;
    if ( (unsigned __int16)(v4 + 8110) <= 0x79u )
    {
      LogFwStat(1, 1, 0LL);
      BgpTxtDisplayCharacter(qword_14035A260, (unsigned __int16)word_1403581C4, 0, 0LL, 0LL);
      LogFwStat(0, 1, 0LL);
      v4 = word_1403581C4;
    }
    if ( v4 == -7989 )
      word_1403581C4 = -8118;
    else
      word_1403581C4 = v4 + 1;
    qword_14038D878 = v0.QuadPart;
    v5 = v1 / 2;
    result = *(_QWORD *)&BgpFwQueryPerformanceCounter(0LL) - v0.QuadPart;
    if ( result > v5 )
    {
      result = *(_QWORD *)&BgpFwQueryPerformanceCounter(0LL) - v5;
      qword_14038D878 = result;
    }
  }
  return result;
}
