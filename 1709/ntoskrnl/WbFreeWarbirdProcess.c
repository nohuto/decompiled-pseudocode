/*
 * XREFs of WbFreeWarbirdProcess @ 0x140575EBC
 * Callers:
 *     WbDispatchOperation @ 0x140501A60 (WbDispatchOperation.c)
 *     WbGetWarbirdProcess @ 0x140501C80 (WbGetWarbirdProcess.c)
 *     sub_140575E88 @ 0x140575E88 (sub_140575E88.c)
 * Callees:
 *     ExFreePoolWithTag @ 0x1402B2440 (ExFreePoolWithTag.c)
 *     WbFreeWarbirdEncryptionSegment @ 0x140451B30 (WbFreeWarbirdEncryptionSegment.c)
 *     sub_140502100 @ 0x140502100 (sub_140502100.c)
 *     sub_140502354 @ 0x140502354 (sub_140502354.c)
 *     sub_14057601C @ 0x14057601C (sub_14057601C.c)
 *     WbFreeMemoryBlock @ 0x14057631C (WbFreeMemoryBlock.c)
 */

__int64 __fastcall WbFreeWarbirdProcess(_DWORD *P)
{
  unsigned int v1; // edi
  unsigned int i; // esi
  unsigned int j; // esi
  char *v5; // rsi
  __int64 v6; // rdx
  __int64 v7; // rax
  unsigned int k; // esi

  v1 = 0;
  if ( P )
  {
    for ( i = 0; i < P[23]; ++i )
      sub_140502100((__int64)P, *(_QWORD *)(P[22] * i + *((_QWORD *)P + 13)));
    sub_14057601C(P + 22);
    for ( j = 0; j < P[3]; ++j )
      sub_140502354((__int64)P, *(_QWORD *)(j * P[2] + *((_QWORD *)P + 3)));
    sub_14057601C(P + 2);
    v5 = (char *)(P + 16);
    while ( 1 )
    {
      v6 = *(_QWORD *)v5;
      if ( *(char **)(*(_QWORD *)v5 + 8LL) != v5 || (v7 = *(_QWORD *)v6, *(_QWORD *)(*(_QWORD *)v6 + 8LL) != v6) )
        __fastfail(3u);
      *(_QWORD *)v5 = v7;
      *(_QWORD *)(v7 + 8) = v5;
      if ( (char *)v6 == v5 )
        break;
      sub_140502354((__int64)P, v6);
    }
    for ( k = 0; k < P[35]; ++k )
      WbFreeWarbirdEncryptionSegment(*(_QWORD **)(k * P[34] + *((_QWORD *)P + 19)));
    sub_14057601C(P + 34);
    if ( P[47] )
    {
      do
        WbFreeMemoryBlock(*(PVOID **)(P[46] * v1++ + *((_QWORD *)P + 25)));
      while ( v1 < P[47] );
    }
    sub_14057601C(P + 46);
    ExFreePoolWithTag(P, 0x42524157u);
  }
  return 0LL;
}
