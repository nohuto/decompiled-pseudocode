/*
 * XREFs of ?StUnlockAndUnmapRegion@?$ST_STORE@USM_TRAITS@@@@SAXPEAU_ST_DATA_MGR@1@KPEAD@Z @ 0x1402712C4
 * Callers:
 *     ?StDmCurrentRegionSet@?$ST_STORE@USM_TRAITS@@@@SAJPEAU_ST_DATA_MGR@1@KK@Z @ 0x140006E2C (-StDmCurrentRegionSet@-$ST_STORE@USM_TRAITS@@@@SAJPEAU_ST_DATA_MGR@1@KK@Z.c)
 *     ?StCompactionPerformEmergency@?$ST_STORE@USM_TRAITS@@@@SAJPEAU_ST_DATA_MGR@1@@Z @ 0x14026AAE0 (-StCompactionPerformEmergency@-$ST_STORE@USM_TRAITS@@@@SAJPEAU_ST_DATA_MGR@1@@Z.c)
 *     ?StCompactionPerformInMem@?$ST_STORE@USM_TRAITS@@@@SAJPEAU_ST_DATA_MGR@1@K@Z @ 0x14026AD74 (-StCompactionPerformInMem@-$ST_STORE@USM_TRAITS@@@@SAJPEAU_ST_DATA_MGR@1@K@Z.c)
 * Callees:
 *     ?SmStUnmapVirtualRegion@?$SMKM_STORE@USM_TRAITS@@@@SAXPEAU1@KKKPEAXK@Z @ 0x140269804 (-SmStUnmapVirtualRegion@-$SMKM_STORE@USM_TRAITS@@@@SAXPEAU1@KKKPEAXK@Z.c)
 *     SmFpFree @ 0x140272EDC (SmFpFree.c)
 */

__int16 __fastcall ST_STORE<SM_TRAITS>::StUnlockAndUnmapRegion(__int64 a1, unsigned int a2, __int64 a3)
{
  __int64 v3; // r9
  __int64 v5; // rbx
  __int64 v6; // rax
  __int64 v7; // r10
  unsigned __int64 v8; // rcx
  unsigned __int64 v9; // r10
  int v10; // ecx
  __int64 v11; // r9
  __int64 v12; // rax
  __int64 v13; // r10
  unsigned __int64 v14; // rcx
  unsigned __int64 v15; // r10
  int v17; // [rsp+20h] [rbp-18h]

  v3 = *(_QWORD *)(a1 + 800);
  v5 = a2;
  if ( (*(_BYTE *)(v3 + 6021) & 4) != 0 )
  {
    SMKM_STORE<SM_TRAITS>::SmStUnmapVirtualRegion(v3, a2, a3, v3, v17, 1u);
  }
  else
  {
    v6 = *(_QWORD *)(v3 + 6216);
    v7 = *(_QWORD *)(v6 + 8LL * a2);
    v8 = v7 & 0xFFFFFFFFFFFFFFFEuLL;
    v9 = v7 & 0xFFFFFFFFFFFFFFF8uLL;
    *(_QWORD *)(v6 + 8LL * a2) = v8;
    if ( (v8 & 3) == 0 )
      SmFpFree(v3 + 6592, 5LL, KeGetCurrentThread(), v9);
  }
  v10 = 0;
  if ( !(unsigned __int8)*(_DWORD *)(a1 + 776) )
    v10 = *(unsigned __int16 *)(*(_QWORD *)(a1 + 1032) + 2 * v5) >> 13;
  v11 = *(_QWORD *)(a1 + 800);
  if ( (*(_BYTE *)(v11 + 6021) & 4) != 0 )
  {
    LOWORD(v12) = SMKM_STORE<SM_TRAITS>::SmStUnmapVirtualRegion(*(_QWORD *)(a1 + 800), v5, a3, v11, v17, (8 * v10) | 4);
  }
  else
  {
    v12 = *(_QWORD *)(v11 + 6216);
    v13 = *(_QWORD *)(v12 + 8 * v5);
    v14 = v13 & 0xFFFFFFFFFFFFFFFDuLL;
    v15 = v13 & 0xFFFFFFFFFFFFFFF8uLL;
    *(_QWORD *)(v12 + 8 * v5) = v14;
    if ( (v14 & 3) == 0 )
      LOWORD(v12) = SmFpFree(v11 + 6592, 5LL, KeGetCurrentThread(), v15);
  }
  return v12;
}
