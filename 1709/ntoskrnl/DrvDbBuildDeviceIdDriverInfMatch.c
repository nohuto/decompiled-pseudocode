/*
 * XREFs of DrvDbBuildDeviceIdDriverInfMatch @ 0x1405846EC
 * Callers:
...
int __fastcall DrvDbBuildDeviceIdDriverInfMatch(size_t a1, unsigned __int8 *a2, wchar_t *a3, unsigned int a4)
{
  ...
  pcchRemaining = 0LL;
  if ( !v4 )
    return RtlStringCchCopyExW(a3, a4, &word_1405F52C0, 0LL, 0LL, 0x800u);
  if ( v4 > 2u )
  {
    if ( v4 == 3 )
      return RtlStringCchPrintfExW(a3, a4, &ppszDestEnd, &pcchRemaining, 0x800u, L"%ws\\*", &word_1405F52C0);
    else
      return -1073741811;
  }
  else
  {
    ...
    result = RtlStringCchPrintfExW(a3, a4, &ppszDestEnd, &pcchRemaining, 0x800u, L"%ws\\%X", &word_1405F52C0, v6);
    if ( result >= 0 )
    {
      v8 = a2[1];
      if ( v8 != 0xFF )
