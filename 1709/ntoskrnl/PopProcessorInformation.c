/*
 * XREFs of PopProcessorInformation @ 0x14058ECEC
 * Callers:
 *     NtPowerInformation @ 0x1404E90A0 (NtPowerInformation.c)
 * Callees:
 *     ExAcquirePushLockSharedEx @ 0x140084BB0 (ExAcquirePushLockSharedEx.c)
 *     KeGetPrcb @ 0x14008D0A4 (KeGetPrcb.c)
 *     KeEnumerateNextProcessor @ 0x14008F8D0 (KeEnumerateNextProcessor.c)
 *     KeQueryGroupAffinity @ 0x1400AE8F0 (KeQueryGroupAffinity.c)
 *     PopReleaseRwLock @ 0x1400B1870 (PopReleaseRwLock.c)
 *     PpmPerfGetCurrentState @ 0x140129A10 (PpmPerfGetCurrentState.c)
 */

__int64 __fastcall PopProcessorInformation(__int64 a1, __int64 a2, USHORT a3, _DWORD *a4)
{
  KAFFINITY GroupAffinity; // rdi
  unsigned int v8; // ebx
  unsigned __int64 v9; // r15
  struct _KTHREAD *CurrentThread; // rax
  unsigned int v11; // r14d
  __int64 Prcb; // rbp
  __int64 v13; // rax
  int v14; // eax
  __int64 v15; // rcx
  unsigned __int16 *v17[2]; // [rsp+30h] [rbp-48h] BYREF
  USHORT v18; // [rsp+40h] [rbp-38h]
  ULONG v19; // [rsp+90h] [rbp+18h] BYREF

  GroupAffinity = KeQueryGroupAffinity(a3);
  v8 = 0;
  v9 = (0x101010101010101LL
      * ((((GroupAffinity - ((GroupAffinity >> 1) & 0x5555555555555555LL)) & 0x3333333333333333LL)
        + (((GroupAffinity - ((GroupAffinity >> 1) & 0x5555555555555555LL)) >> 2) & 0x3333333333333333LL)
        + ((((GroupAffinity - ((GroupAffinity >> 1) & 0x5555555555555555LL)) & 0x3333333333333333LL)
          + (((GroupAffinity - ((GroupAffinity >> 1) & 0x5555555555555555LL)) >> 2) & 0x3333333333333333LL)) >> 4)) & 0xF0F0F0F0F0F0F0FLL)) >> 56;
  if ( (unsigned int)(24 * v9) > 0x600 )
  {
    if ( a4 )
      *a4 = 0;
    return (unsigned int)-1073741789;
  }
  else
  {
    if ( (_DWORD)v9 )
    {
      CurrentThread = KeGetCurrentThread();
      v11 = 0;
      --CurrentThread->KernelApcDisable;
      ExAcquirePushLockSharedEx((ULONG_PTR)&PpmIdlePolicyLock, 0LL);
      v18 = a3;
      v17[1] = (unsigned __int16 *)GroupAffinity;
      v17[0] = 0LL;
      while ( !(unsigned int)KeEnumerateNextProcessor(&v19, v17) && v11 < (unsigned int)v9 )
      {
        Prcb = KeGetPrcb(v19);
        *(_DWORD *)(a1 + 24LL * v11) = *(unsigned __int8 *)(Prcb + 209);
        PpmPerfGetCurrentState(
          Prcb,
          (_DWORD *)(a1 + 8 + 24LL * v11),
          (unsigned int *)(a1 + 12 + 24LL * v11),
          0LL,
          0LL,
          0LL);
        v13 = *(_QWORD *)(Prcb + 24176);
        if ( v13 )
          v14 = *(_DWORD *)(v13 + 316);
        else
          v14 = *(_DWORD *)(Prcb + 68);
        *(_DWORD *)(a1 + 24LL * v11 + 4) = v14;
        v15 = *(_QWORD *)(Prcb + 23808);
        if ( v15 )
        {
          *(_DWORD *)(a1 + 24LL * v11 + 16) = *(_DWORD *)(v15 + 28);
          *(_DWORD *)(a1 + 24LL * v11 + 20) = *(_DWORD *)(v15 + 12) + 1;
        }
        else
        {
          *(_QWORD *)(a1 + 24LL * v11 + 16) = 0LL;
        }
        ++v11;
      }
      PopReleaseRwLock((ULONG_PTR)&PpmIdlePolicyLock);
    }
    *a4 = 24 * v9;
  }
  return v8;
}
