/*
 * XREFs of PopDisplayBurstSuppressWorker @ 0x140705B90
 * Callers:
 *     <none>
 * Callees:
...
char PopDisplayBurstSuppressWorker()
{
  ...
  if ( (GUID *)PopWdiCurrentScenario == &NullGuid )
  {
    if ( !PopDisableDisplayBurstOnPowerSourceChange && pCallbackContext.LevelPlus1 > 5 )
    {
      ...
      if ( (_BYTE)v0 )
      {
        ...
        v9 = 1LL;
        v11 = 1LL;
        LOBYTE(v0) = TlgWrite(&pCallbackContext, &unk_1402D1DAD, 0LL, 0LL, 4u, &pData);
      }
    }
  }
  else
  {
    ...
    if ( pCallbackContext.LevelPlus1 > 5 && TlgKeywordOn(&pCallbackContext, 0x400000000000uLL) )
    {
      ...
      v14 = 1LL;
      v16 = 1LL;
      TlgWrite(&pCallbackContext, &unk_1402D1D4C, 0LL, 0LL, 4u, &v12);
    }
