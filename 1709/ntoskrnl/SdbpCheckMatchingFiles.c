/*
 * XREFs of SdbpCheckMatchingFiles @ 0x140541F70
 * Callers:
 *     <none>
 * Callees:
 *     ExFreePoolWithTag @ 0x1402B2440 (ExFreePoolWithTag.c)
 *     SdbGetStringTagPtr @ 0x1404F5740 (SdbGetStringTagPtr.c)
 *     SdbFindFirstTag @ 0x140529448 (SdbFindFirstTag.c)
 *     SdbpInitializeSearchDBContext @ 0x140541D04 (SdbpInitializeSearchDBContext.c)
 *     SdbpResolveMatchingFile @ 0x140542220 (SdbpResolveMatchingFile.c)
 *     AslFileNotFound @ 0x140542AA4 (AslFileNotFound.c)
 *     AslFileMappingDelete @ 0x140544E28 (AslFileMappingDelete.c)
 *     AslFileMappingCreate @ 0x140545E54 (AslFileMappingCreate.c)
 *     SdbpCheckAllAttributes @ 0x140771748 (SdbpCheckAllAttributes.c)
 *     AslLogCallPrintf @ 0x140772254 (AslLogCallPrintf.c)
 */

__int64 __fastcall SdbpCheckMatchingFiles(int *a1, __int64 a2, __int64 a3, __int64 a4, unsigned int a5, _QWORD *a6)
{
  __int64 v6; // rsi
  int v8; // r15d
  int *v9; // r12
  unsigned int v10; // r14d
  PVOID v11; // rdi
  PVOID v12; // r13
  unsigned int FirstTag; // eax
  _WORD *StringTagPtr; // r12
  __int64 v15; // rdx
  int v16; // eax
  int *v17; // r8
  int v18; // r15d
  unsigned __int64 v19; // rax
  __int64 v20; // rcx
  __int64 result; // rax
  const char *v22; // r9
  int v23; // r8d
  unsigned int v24; // eax
  int v25; // ecx
  int v26; // [rsp+30h] [rbp-20h] BYREF
  PVOID P; // [rsp+38h] [rbp-18h] BYREF
  __int64 v28; // [rsp+40h] [rbp-10h]
  int *v29; // [rsp+48h] [rbp-8h]
  int v32; // [rsp+A0h] [rbp+50h]

  v32 = a3;
  v6 = (__int64)a6;
  v26 = 0;
  v8 = a2;
  P = 0LL;
  v9 = a1;
  v10 = 0;
  v11 = 0LL;
  v12 = 0LL;
  if ( a6[11] )
  {
    v26 = 1;
    goto LABEL_21;
  }
  FirstTag = SdbFindFirstTag(a3, a5, 24577);
  if ( FirstTag )
  {
    StringTagPtr = (_WORD *)SdbGetStringTagPtr(a3, FirstTag);
    if ( StringTagPtr )
    {
      if ( *(_QWORD *)(v6 + 8) && *StringTagPtr == 42 )
      {
        v24 = SdbpCheckAllAttributes((unsigned int)&v26, v8, a3, a5, *(_QWORD *)(v6 + 8));
        if ( (unsigned int)AslFileNotFound(v24) || v25 >= 0 )
          v10 = 1;
        else
          AslLogCallPrintf(
            1,
            (unsigned int)"SdbpCheckMatchingFiles",
            2132,
            (unsigned int)"Failed to check file attributes [%x]");
        goto LABEL_30;
      }
      v15 = -1LL;
      do
        ++v15;
      while ( StringTagPtr[v15] );
      v16 = *(_DWORD *)v6;
      v28 = v15;
      if ( (v16 & 1) != 0 )
      {
LABEL_10:
        v17 = *(int **)(v6 + 72);
        v18 = 0;
        v29 = v17;
        if ( *v17 <= 0 )
          goto LABEL_17;
        while ( 1 )
        {
          if ( !(unsigned int)SdbpResolveMatchingFile(a2, v6, StringTagPtr, (unsigned int)v15, &v17[12 * v18 + 2], &P) )
          {
            AslLogCallPrintf(
              1,
              (unsigned int)"SdbpCheckMatchingFiles",
              2182,
              (unsigned int)"Failed to resolve matching file");
            v11 = P;
            goto LABEL_30;
          }
          v11 = P;
          a6 = 0LL;
          if ( (int)AslFileMappingCreate((unsigned int)&a6, (_DWORD)P, 0, 0, 0LL) >= 0 )
          {
            v12 = a6;
            if ( (int)SdbpCheckAllAttributes((unsigned int)&v26, a2, v32, a5, (__int64)a6) >= 0 )
            {
              if ( v18 > 0 )
                *(_DWORD *)(v6 + 80) = 1;
LABEL_17:
              v19 = (unsigned __int16)*StringTagPtr;
              LOWORD(v19) = v19 - 37;
              if ( (unsigned __int16)v19 <= 0x37u )
              {
                v20 = 0x80000000000201LL;
                if ( _bittest64(&v20, v19) )
                  *(_DWORD *)(v6 + 80) = 1;
              }
              v9 = a1;
LABEL_21:
              v10 = 1;
              goto LABEL_22;
            }
            AslFileMappingDelete(v12);
            v12 = 0LL;
            if ( v11 )
              ExFreePoolWithTag(v11, 0x74705041u);
          }
          else
          {
            if ( v11 )
              ExFreePoolWithTag(v11, 0x74705041u);
            v12 = a6;
          }
          v17 = v29;
          ++v18;
          LODWORD(v15) = v28;
          v11 = 0LL;
          P = 0LL;
          if ( v18 >= *v29 )
            goto LABEL_17;
        }
      }
      if ( (unsigned int)SdbpInitializeSearchDBContext(v6) )
      {
        LODWORD(v15) = v28;
        goto LABEL_10;
      }
      v22 = "Failed to initialize SEARCHDBCONTEXT";
      v23 = 2158;
    }
    else
    {
      v22 = "Failed to get the string from the database";
      v23 = 2107;
    }
    AslLogCallPrintf(1, (unsigned int)"SdbpCheckMatchingFiles", v23, (_DWORD)v22);
LABEL_30:
    v9 = a1;
  }
LABEL_22:
  AslFileMappingDelete(v12);
  if ( v11 )
    ExFreePoolWithTag(v11, 0x74705041u);
  result = v10;
  *v9 = v26;
  return result;
}
