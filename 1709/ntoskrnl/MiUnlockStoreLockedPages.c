/*
 * XREFs of MiUnlockStoreLockedPages @ 0x14022D744
 * Callers:
 *     MmStoreProbeAndLockPages @ 0x14022DB24 (MmStoreProbeAndLockPages.c)
 *     SmKmUnlockMdl @ 0x140273F34 (SmKmUnlockMdl.c)
 * Callees:
 *     MiWriteCompletePfn @ 0x1400A5320 (MiWriteCompletePfn.c)
 *     MiReleasePageFileInfo @ 0x1400C0554 (MiReleasePageFileInfo.c)
 *     MiLockPageInline @ 0x1400E1510 (MiLockPageInline.c)
 */

int __fastcall MiUnlockStoreLockedPages(__int64 a1)
{
  unsigned __int64 *v1; // rsi
  unsigned int v3; // ebx
  unsigned __int64 v4; // r14
  unsigned __int64 v5; // rax
  __int64 v6; // rbp
  unsigned __int8 v7; // r15
  struct _KEVENT *v8; // rcx

  v1 = (unsigned __int64 *)(a1 + 48);
  v3 = 0;
  v4 = a1
     + 48
     + 8
     * ((*(unsigned int *)(a1 + 40) + 4095LL + (unsigned __int64)((*(_DWORD *)(a1 + 32) + *(_DWORD *)(a1 + 44)) & 0xFFF)) >> 12);
  do
  {
    v5 = *v1;
    if ( *v1 == -1LL )
      break;
    v6 = 48 * v5 - 0x58000000000LL;
    v7 = MiLockPageInline(v6);
    if ( (*(_BYTE *)(v6 + 34) & 0x10) != 0 )
      v3 |= 2u;
    else
      v3 &= ~2u;
    v5 = MiWriteCompletePfn(v6, v3);
    v8 = v5 ? *(struct _KEVENT **)(qword_140388AF0 + 8 * ((*(_QWORD *)(v6 + 40) >> 40) & 0x3FFLL)) : 0LL;
    _InterlockedAnd64((volatile signed __int64 *)(v6 + 24), 0x7FFFFFFFFFFFFFFFuLL);
    __writecr8(v7);
    if ( v5 )
      LODWORD(v5) = MiReleasePageFileInfo(v8, v5, 0);
    ++v1;
  }
  while ( (unsigned __int64)v1 < v4 );
  *(_WORD *)(a1 + 10) &= ~2u;
  return v5;
}
