/*
 * XREFs of PnpTraceDmaGuardDevicePolicy @ 0x14058C8FC
 * Callers:
 *     PipDmgSaveDeviceDmarPolicy @ 0x14058C660 (PipDmgSaveDeviceDmarPolicy.c)
 * Callees:
 *     _TlgWrite @ 0x1400AE914 (_TlgWrite.c)
 *     _TlgKeywordOn @ 0x1400BA32C (_TlgKeywordOn.c)
 *     __security_check_cookie @ 0x14015D720 (__security_check_cookie.c)
 */

void PnpTraceDmaGuardDevicePolicy()
{
  __int64 v0; // r9
  int v1; // r10d
  int v2; // [rsp+30h] [rbp-19h] BYREF
  EVENT_DATA_DESCRIPTOR pData; // [rsp+40h] [rbp-9h] BYREF
  _DWORD *v4; // [rsp+60h] [rbp+17h]
  __int64 v5; // [rsp+68h] [rbp+1Fh]
  __int64 v6; // [rsp+70h] [rbp+27h]
  _DWORD v7[2]; // [rsp+78h] [rbp+2Fh] BYREF
  int *v8; // [rsp+80h] [rbp+37h]
  __int64 v9; // [rsp+88h] [rbp+3Fh]

  if ( stru_140354A70.LevelPlus1 > 5 )
  {
    if ( TlgKeywordOn(&stru_140354A70, 0x400000000000uLL) )
    {
      v2 = v1;
      v4 = v7;
      v6 = *(_QWORD *)(v0 + 48);
      v7[0] = *(unsigned __int16 *)(v0 + 40);
      v8 = &v2;
      v5 = 2LL;
      v7[1] = 0;
      v9 = 4LL;
      TlgWrite(&stru_140354A70, &unk_1402CFD34, 0LL, 0LL, 5u, &pData);
    }
  }
}
