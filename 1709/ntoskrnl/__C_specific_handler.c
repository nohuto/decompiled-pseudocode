/*
 * XREFs of __C_specific_handler @ 0x14015D750
 * Callers:
 *     __GSHandlerCheck_SEH @ 0x14017D508 (__GSHandlerCheck_SEH.c)
 * Callees:
 *     RtlUnwindEx @ 0x1400D5CB0 (RtlUnwindEx.c)
 *     KeCheckStackAndTargetAddress @ 0x1400D9B10 (KeCheckStackAndTargetAddress.c)
 *     _NLG_Notify @ 0x14015E200 (_NLG_Notify.c)
 *     __NLG_Return2 @ 0x14015E230 (__NLG_Return2.c)
 */

EXCEPTION_DISPOSITION __cdecl _C_specific_handler(
        struct _EXCEPTION_RECORD *ExceptionRecord,
        void *EstablisherFrame,
        struct _CONTEXT *ContextRecord,
        struct _DISPATCHER_CONTEXT *DispatcherContext)
{
  char *ImageBase; // r15
  unsigned __int64 v9; // rbp
  ULONG *HandlerData; // rbx
  ULONG v11; // esi
  __int64 v12; // rax
  int v13; // eax
  ULONG ScopeIndex; // edi
  unsigned __int64 v16; // rsi
  __int64 v17; // rcx
  ULONG i; // r9d
  __int64 v19; // rax
  ULONG v20; // edx
  _QWORD v21[2]; // [rsp+30h] [rbp-38h] BYREF

  KeCheckStackAndTargetAddress(ContextRecord->Rip, ContextRecord->Rsp);
  ImageBase = (char *)DispatcherContext->ImageBase;
  v9 = DispatcherContext->ControlPc - (_QWORD)ImageBase;
  HandlerData = (ULONG *)DispatcherContext->HandlerData;
  if ( (ExceptionRecord->ExceptionFlags & 0x66) != 0 )
  {
    ScopeIndex = DispatcherContext->ScopeIndex;
    v16 = DispatcherContext->TargetIp - (_QWORD)ImageBase;
    while ( 1 )
    {
      v20 = *HandlerData;
      if ( ScopeIndex >= *HandlerData )
        break;
      v17 = 2LL * ScopeIndex;
      if ( v9 >= HandlerData[4 * ScopeIndex + 1] && v9 < HandlerData[4 * ScopeIndex + 2] )
      {
        if ( (ExceptionRecord->ExceptionFlags & 0x20) != 0 )
        {
          for ( i = 0; i < v20; ++i )
          {
            if ( v16 >= HandlerData[4 * i + 1]
              && v16 < HandlerData[4 * i + 2]
              && HandlerData[4 * i + 4] == HandlerData[4 * ScopeIndex + 4]
              && HandlerData[4 * i + 3] == HandlerData[4 * ScopeIndex + 3] )
            {
              break;
            }
          }
          if ( i != v20 )
            return 1;
        }
        v19 = HandlerData[4 * ScopeIndex + 4];
        if ( (_DWORD)v19 )
        {
          if ( v16 == v19 )
            return 1;
        }
        else
        {
          DispatcherContext->ScopeIndex = ScopeIndex + 1;
          LOBYTE(v17) = 1;
          ((void (__fastcall *)(__int64, void *))&ImageBase[HandlerData[4 * ScopeIndex + 3]])(v17, EstablisherFrame);
        }
      }
      ++ScopeIndex;
    }
  }
  else
  {
    v11 = DispatcherContext->ScopeIndex;
    v21[0] = ExceptionRecord;
    v21[1] = ContextRecord;
    while ( v11 < *HandlerData )
    {
      if ( v9 >= HandlerData[4 * v11 + 1] && v9 < HandlerData[4 * v11 + 2] && HandlerData[4 * v11 + 4] )
      {
        v12 = HandlerData[4 * v11 + 3];
        if ( (_DWORD)v12 == 1 )
          goto LABEL_10;
        v13 = ((__int64 (__fastcall *)(_QWORD *, void *))&ImageBase[v12])(v21, EstablisherFrame);
        if ( v13 < 0 )
          return 0;
        if ( v13 > 0 )
        {
LABEL_10:
          NLG_Notify(&ImageBase[HandlerData[4 * v11 + 4]], EstablisherFrame, 1LL);
          RtlUnwindEx(
            EstablisherFrame,
            &ImageBase[HandlerData[4 * v11 + 4]],
            ExceptionRecord,
            (PVOID)ExceptionRecord->ExceptionCode,
            (struct _CONTEXT *)DispatcherContext->ContextRecord,
            DispatcherContext->HistoryTable);
          _NLG_Return2();
        }
      }
      ++v11;
    }
  }
  return 1;
}
