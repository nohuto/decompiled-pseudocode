/*
 * XREFs of ACPIInterruptServiceRoutine @ 0x1C00219F0
 * Callers:
 *     <none>
 * Callees:
 *     CLEAR_PM1_STATUS_BITS @ 0x1C0021B48 (CLEAR_PM1_STATUS_BITS.c)
 *     ACPIGpeEnableDisableEvents @ 0x1C0022134 (ACPIGpeEnableDisableEvents.c)
 *     ACPIReadGpeStatusRegister @ 0x1C0022470 (ACPIReadGpeStatusRegister.c)
 *     READ_PM1_STATUS @ 0x1C0024460 (READ_PM1_STATUS.c)
 *     _guard_dispatch_icall_nop @ 0x1C00319E0 (_guard_dispatch_icall_nop.c)
 */

bool __fastcall ACPIInterruptServiceRoutine(__int64 a1, __int64 a2)
{
  unsigned int v3; // edi
  __int64 v4; // rdx
  int v5; // ebx
  _BOOL8 v6; // r8
  unsigned __int8 v7; // al
  int v8; // edi
  int v9; // esi
  signed __int32 v10; // eax
  unsigned int v11; // esi
  signed __int32 v12; // edx
  unsigned int i; // r9d

  v3 = 0;
  v5 = (unsigned __int16)(READ_PM1_STATUS() & (*((_WORD *)AcpiInformation + 56) | 0x8401));
  if ( *((_WORD *)AcpiInformation + 51) )
  {
    while ( 1 )
    {
      v7 = ACPIReadGpeStatusRegister(v3);
      v4 = v3;
      LOBYTE(v4) = *((_BYTE *)GpeCurEnable + v3);
      if ( ((unsigned __int8)v4 & v7) != 0 )
        break;
      if ( ++v3 >= *((unsigned __int16 *)AcpiInformation + 51) )
        goto LABEL_4;
    }
    v5 |= 0x10000u;
  }
LABEL_4:
  if ( (AcpiOverrideAttributes & 0x200) != 0 && !v5 )
    v5 = 0x10000;
  LOBYTE(v6) = 0;
  if ( AcpiPccSciReferenceCount )
  {
    if ( AcpiPccLegacySubspace && *(_DWORD *)(AcpiPccLegacySubspace + 312) )
      v6 = (**(_WORD **)(AcpiPccLegacySubspace + 48) & 2) != 0;
    if ( AcpiPccSubspaces )
    {
      for ( i = 0; i < AcpiPccSubspaceCount; ++i )
      {
        v4 = AcpiPccSubspaces + 528LL * i;
        if ( *(_QWORD *)(v4 + 48)
          && (*(_DWORD *)(v4 + 312) && (**(_WORD **)(v4 + 48) & 2) != 0 || (**(_WORD **)(v4 + 48) & 8) != 0) )
        {
          LOBYTE(v6) = 1;
        }
      }
    }
    if ( v6 )
      v5 |= 0x20000u;
  }
  v8 = v5 & 0x11;
  if ( (v5 & 0x11) != 0 )
  {
    CLEAR_PM1_STATUS_BITS(v5 & 0x11, v4, v6);
    if ( (v5 & 1) != 0 )
      (*(void (**)(void))(PmHalDispatchTable + 8))();
    v5 &= ~v8;
  }
  if ( v5 )
  {
    v9 = v5 | 0x10000;
    if ( (v5 & ~*(_DWORD *)(a2 + 88)) != 0 )
      v9 = v5;
    if ( (v9 & 0x10000) != 0 )
      ACPIGpeEnableDisableEvents(0LL);
    CLEAR_PM1_STATUS_BITS((unsigned __int16)v9, v4, v6);
    v10 = *(_DWORD *)(a2 + 88);
    v11 = v9 | 0x80000000;
    do
    {
      v12 = v10;
      v10 = _InterlockedCompareExchange((volatile signed __int32 *)(a2 + 88), v11 | v10, v10);
    }
    while ( v12 != v10 );
    v8 |= v11 & ~v10;
    if ( v8 < 0 )
      KeInsertQueueDpc((PRKDPC)(a2 + 96), 0LL, 0LL);
  }
  return v8 != 0;
}
