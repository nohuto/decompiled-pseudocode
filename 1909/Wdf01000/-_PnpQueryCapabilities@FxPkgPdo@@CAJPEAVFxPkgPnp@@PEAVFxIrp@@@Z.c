/*
 * XREFs of ?_PnpQueryCapabilities@FxPkgPdo@@CAJPEAVFxPkgPnp@@PEAVFxIrp@@@Z @ 0x1C000E1E0
 * Callers:
 *     <none>
 * Callees:
 *     ?Allocate@MxWorkItem@@QEAAJPEAU_DEVICE_OBJECT@@PEAX@Z @ 0x1C000CA78 (-Allocate@MxWorkItem@@QEAAJPEAU_DEVICE_OBJECT@@PEAX@Z.c)
 *     ?HandleQueryCapabilities@FxPkgPdo@@AEAAXPEAU_DEVICE_CAPABILITIES@@0@Z @ 0x1C000E320 (-HandleQueryCapabilities@FxPkgPdo@@AEAAXPEAU_DEVICE_CAPABILITIES@@0@Z.c)
...
{
  int StackCapabilities; // ebx
  _DEVICE_CAPABILITIES *v5; // rdi
  void *v6; // r8
  ...
  MxDeviceObject DeviceInStack; // [rsp+20h] [rbp-A8h] BYREF
  unsigned __int64 HighLimit; // [rsp+28h] [rbp-A0h] BYREF
  unsigned __int64 LowLimit; // [rsp+30h] [rbp-98h] BYREF
  _STACK_DEVICE_CAPABILITIES Capabilities; // [rsp+40h] [rbp-88h] BYREF
  ...
  if ( (unsigned __int64)&HighLimit - LowLimit >= 0x3000 )
  {
    ...
    StackCapabilities = GetStackCapabilities(m_Globals, &DeviceInStack, 0LL, &Capabilities);
...
LABEL_6:
  ...
  v12 = DeviceInStack.m_DeviceObject;
  ...
  IoQueueWorkItem((PIO_WORKITEM)v12, FxPkgPdo::_QueryCapsWorkItem, DelayedWorkQueue, Irp->m_Irp);
  return 259LL;
}
