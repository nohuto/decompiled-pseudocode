/*
 * XREFs of ?_GetFileObjectFromWdm@FxFileObject@@SAJPEAVFxDevice@@W4_WDF_FILEOBJECT_CLASS@@PEAU_FILE_OBJECT@@PEAPEAV1@@Z @ 0x1C000AA30
 * Callers:
        ...
        FxFileObject **ppFxFileObject)
{
  ...
  if ( pWdmFileObject || pDevice->m_Exclusive && v5 == 4 )
  {
    ...
    _a2 = 0LL;
    FsContext = 0LL;
    if ( SLOBYTE(pDevice->m_ObjectFlags) < 0 && (m_PkgWmi = (FxVerifierLock *)pDevice[-1].m_PkgWmi) != 0LL )
    {
    ...
    {
LABEL_11:
      if ( SLOBYTE(pDevice->m_ObjectFlags) < 0 && (v18 = (FxVerifierLock *)pDevice[-1].m_PkgWmi) != 0LL )
