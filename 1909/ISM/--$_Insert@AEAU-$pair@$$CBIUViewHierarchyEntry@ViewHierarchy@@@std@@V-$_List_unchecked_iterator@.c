/*
 * XREFs of ??$_Insert@AEAU?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@std@@@std@@@2@@?$_Hash@V?$_Umap_traits@IUViewHierarchyEntry@ViewHierarchy@@V?$_Uhash_compare@IU?$hash@I@std@@U?$equal_to@I@2@@std@@V?$allocator@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@4@$0A@@std@@@std@@IEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@std@@@std@@@std@@_N@1@AEAU?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@1@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@std@@@std@@@1@@Z @ 0x18000A7C0
 * Callers:
 *     ?OnAdded@ViewHierarchy@@UEAAJPEAUIMessageList@@HPEAUIUnknown@@@Z @ 0x18000A2D0 (-OnAdded@ViewHierarchy@@UEAAJPEAUIMessageList@@HPEAUIUnknown@@@Z.c)
 *     ??$_Insert@AEAU?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@std@@@std@@@2@@?$_Hash@V?$_Umap_traits@IUViewHierarchyEntry@ViewHierarchy@@V?$_Uhash_compare@IU?$hash@I@std@@U?$equal_to@I@2@@std@@V?$allocator@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@4@$0A@@std@@@std@@IEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@std@@@std@@@std@@_N@1@AEAU?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@1@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@std@@@std@@@1@@Z @ 0x18000A7C0 (--$_Insert@AEAU-$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@V-$_List_unchecked_iterator@.c)
 *     ??$emplace@AEBUpiecewise_construct_t@std@@V?$tuple@AEBI@2@V?$tuple@$$V@2@@?$_Hash@V?$_Umap_traits@IUViewHierarchyEntry@ViewHierarchy@@V?$_Uhash_compare@IU?$hash@I@std@@U?$equal_to@I@2@@std@@V?$allocator@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@4@$0A@@std@@@std@@QEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@std@@@std@@@std@@_N@1@AEBUpiecewise_construct_t@1@$$QEAV?$tuple@AEBI@1@$$QEAV?$tuple@$$V@1@@Z @ 0x180058B1C (--$emplace@AEBUpiecewise_construct_t@std@@V-$tuple@AEBI@2@V-$tuple@$$V@2@@-$_Hash@V-$_Umap_trait.c)
 * Callees:
 *     ??$_Insert@AEAU?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@std@@@std@@@2@@?$_Hash@V?$_Umap_traits@IUViewHierarchyEntry@ViewHierarchy@@V?$_Uhash_compare@IU?$hash@I@std@@U?$equal_to@I@2@@std@@V?$allocator@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@4@$0A@@std@@@std@@IEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@std@@@std@@@std@@_N@1@AEAU?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@1@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@std@@@std@@@1@@Z @ 0x18000A7C0 (--$_Insert@AEAU-$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@V-$_List_unchecked_iterator@.c)
 *     ?_Init@?$_Hash@V?$_Umap_traits@IUViewHierarchyEntry@ViewHierarchy@@V?$_Uhash_compare@IU?$hash@I@std@@U?$equal_to@I@2@@std@@V?$allocator@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@4@$0A@@std@@@std@@IEAAX_K@Z @ 0x18000AB60 (-_Init@-$_Hash@V-$_Umap_traits@IUViewHierarchyEntry@ViewHierarchy@@V-$_Uhash_compare@IU-$hash@I@.c)
 *     ?erase@?$list@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@V?$allocator@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@2@@std@@QEAA?AV?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@std@@@std@@@2@V?$_List_const_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@@std@@@std@@@2@@Z @ 0x180059DA0 (-erase@-$list@U-$pair@$$CBIUViewHierarchyEntry@ViewHierarchy@@@std@@V-$allocator@U-$pair@$$CBIUV.c)
 */

__int64 __fastcall std::_Hash<std::_Umap_traits<unsigned int,ViewHierarchy::ViewHierarchyEntry,std::_Uhash_compare<unsigned int,std::hash<unsigned int>,std::equal_to<unsigned int>>,std::allocator<std::pair<unsigned int const,ViewHierarchy::ViewHierarchyEntry>>,0>>::_Insert<std::pair<unsigned int const,ViewHierarchy::ViewHierarchyEntry> &,std::_List_unchecked_iterator<std::_List_val<std::_List_simple_types<std::pair<unsigned int const,ViewHierarchy::ViewHierarchyEntry>>>>>(
        __int64 a1,
        __int64 a2,
        unsigned __int8 *a3,
        __int64 *a4)
{
  __int64 v7; // rax
  unsigned __int8 *v8; // r10
  __int64 v9; // r9
  __int64 v10; // rdi
  __int64 v11; // rdx
  __int64 v12; // rcx
  __int64 v13; // rax
  __int64 v14; // rdx
  __int64 v15; // r9
  __int64 v16; // rdx
  __int64 v17; // rax
  __int64 v18; // rax
  __int64 v19; // rdx
  float v20; // xmm0_4
  float v21; // xmm1_4
  __int64 result; // rax
  int v23; // eax
  _QWORD *v24; // rax
  __int64 v25; // r15
  __int64 v26; // rdi
  __int64 **v27; // rax
  __int64 *v28; // rcx
  char v29[64]; // [rsp+28h] [rbp-40h] BYREF
  __int64 v30; // [rsp+70h] [rbp+8h] BYREF
  __int64 *v31; // [rsp+80h] [rbp+18h] BYREF
  __int64 *v32; // [rsp+88h] [rbp+20h]

  v32 = a4;
  v30 = a1;
  v7 = 0xCBF29CE484222325uLL;
  v8 = a3;
  if ( a3 < a3 + 4 )
  {
    do
      v7 = 0x100000001B3LL * (*v8++ ^ (unsigned __int64)v7);
    while ( v8 - a3 != 4 );
  }
  v9 = v7 & *(_QWORD *)(a1 + 48);
  v10 = *(_QWORD *)(a1 + 8);
  v11 = *(_QWORD *)(a1 + 24);
  v12 = *(_QWORD *)(v11 + 16 * v9);
  if ( v10 != v12 )
  {
    v10 = **(_QWORD **)(v11 + 16 * v9 + 8);
    while ( v10 != v12 )
    {
      v10 = *(_QWORD *)(v10 + 8);
      v23 = *(_DWORD *)(v10 + 16);
      if ( *(_DWORD *)a3 == v23 )
      {
        try
        {
          std::list<std::pair<unsigned int const,ViewHierarchy::ViewHierarchyEntry>>::erase(a1 + 8, &v31, a4);
          *(_QWORD *)a2 = v10;
          *(_BYTE *)(a2 + 8) = 0;
          result = a2;
        }
        catch ( ... )
        {
          std::list<std::pair<unsigned int const,ViewHierarchy::ViewHierarchyEntry>>::erase(v30 + 8, &v30, v32);
          throw;
        }
        return result;
      }
    }
  }
  v31 = a4;
  v13 = *a4;
  if ( v10 != *a4 )
  {
    *(_QWORD *)a4[1] = v13;
    **(_QWORD **)(v13 + 8) = v10;
    **(_QWORD **)(v10 + 8) = a4;
    v14 = *(_QWORD *)(v10 + 8);
    *(_QWORD *)(v10 + 8) = *(_QWORD *)(v13 + 8);
    *(_QWORD *)(v13 + 8) = a4[1];
    a4[1] = v14;
  }
  v15 = 2 * v9;
  v16 = *(_QWORD *)(a1 + 24);
  v17 = *(_QWORD *)(v16 + 8 * v15);
  if ( v17 == *(_QWORD *)(a1 + 8) )
  {
    *(_QWORD *)(v16 + 8 * v15) = a4;
    *(_QWORD *)(*(_QWORD *)(a1 + 24) + 8 * v15 + 8) = a4;
  }
  else if ( v17 == v10 )
  {
    *(_QWORD *)(v16 + 8 * v15) = a4;
  }
  else
  {
    v27 = *(__int64 ***)(v16 + 8 * v15 + 8);
    v28 = *v27;
    *(_QWORD *)(v16 + 8 * v15 + 8) = *v27;
    if ( v28 != a4 )
      *(_QWORD *)(*(_QWORD *)(a1 + 24) + 8 * v15 + 8) = *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a1 + 24) + 8 * v15 + 8) + 8LL);
  }
  v18 = *(_QWORD *)(a1 + 16);
  v19 = *(_QWORD *)(a1 + 56);
  v20 = (float)(int)v18;
  if ( v18 < 0 )
    v20 = v20 + 1.8446744e19;
  v21 = (float)(int)v19;
  if ( v19 < 0 )
    v21 = v21 + 1.8446744e19;
  if ( (float)(v20 / v21) > *(float *)a1 )
  {
    try
    {
      if ( (unsigned __int64)v19 >= 0x200 )
      {
        if ( (unsigned __int64)v19 < 0xFFFFFFFFFFFFFFFLL )
          v19 *= 2LL;
      }
      else
      {
        v19 *= 8LL;
      }
      std::_Hash<std::_Umap_traits<unsigned int,ViewHierarchy::ViewHierarchyEntry,std::_Uhash_compare<unsigned int,std::hash<unsigned int>,std::equal_to<unsigned int>>,std::allocator<std::pair<unsigned int const,ViewHierarchy::ViewHierarchyEntry>>,0>>::_Init(
        a1,
        v19);
      v24 = *(_QWORD **)(a1 + 8);
      if ( (_QWORD *)*v24 != v24 )
      {
        v25 = v24[1];
        do
        {
          v26 = **(_QWORD **)(a1 + 8);
          std::_Hash<std::_Umap_traits<unsigned int,ViewHierarchy::ViewHierarchyEntry,std::_Uhash_compare<unsigned int,std::hash<unsigned int>,std::equal_to<unsigned int>>,std::allocator<std::pair<unsigned int const,ViewHierarchy::ViewHierarchyEntry>>,0>>::_Insert<std::pair<unsigned int const,ViewHierarchy::ViewHierarchyEntry> &,std::_List_unchecked_iterator<std::_List_val<std::_List_simple_types<std::pair<unsigned int const,ViewHierarchy::ViewHierarchyEntry>>>>>(
            a1,
            v29,
            v26 + 16);
        }
        while ( v26 != v25 );
      }
    }
    catch ( ... )
    {
      std::_Hash<std::_Umap_traits<unsigned int,ViewHierarchy::ViewHierarchyEntry,std::_Uhash_compare<unsigned int,std::hash<unsigned int>,std::equal_to<unsigned int>>,std::allocator<std::pair<unsigned int const,ViewHierarchy::ViewHierarchyEntry>>,0>>::erase(
        v30,
        &v31,
        v31);
      throw;
    }
  }
  *(_QWORD *)a2 = a4;
  *(_BYTE *)(a2 + 8) = 1;
  return a2;
}
