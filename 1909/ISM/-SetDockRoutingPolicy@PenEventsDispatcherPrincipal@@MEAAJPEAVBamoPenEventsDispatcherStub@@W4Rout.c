/*
 * XREFs of ?SetDockRoutingPolicy@PenEventsDispatcherPrincipal@@MEAAJPEAVBamoPenEventsDispatcherStub@@W4RoutingPolicy@Input@Devices@Internal@Windows@@@Z @ 0x180032D90
 * Callers:
 *     <none>
 * Callees:
 *     __security_check_cookie @ 0x18002C200 (__security_check_cookie.c)
 *     ?PenPolicyToString@InputTraceLogging@@CAPEBDW4RoutingPolicy@Input@Devices@Internal@Windows@@@Z @ 0x180032808 (-PenPolicyToString@InputTraceLogging@@CAPEBDW4RoutingPolicy@Input@Devices@Internal@Windows@@@Z.c)
 *     _guard_dispatch_icall_nop @ 0x1800378B0 (_guard_dispatch_icall_nop.c)
 *     _TlgCreateSz @ 0x180052EFC (_TlgCreateSz.c)
 *     _TlgKeywordOn @ 0x18005300C (_TlgKeywordOn.c)
 *     _TlgWrite @ 0x180053034 (_TlgWrite.c)
 *     ?_Log_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z @ 0x1800563B0 (-_Log_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z.c)
 *     ?Provider@InputTraceLogging@@SAQEBU_TlgProvider_t@@XZ @ 0x18007B848 (-Provider@InputTraceLogging@@SAQEBU_TlgProvider_t@@XZ.c)
 */

__int64 __fastcall PenEventsDispatcherPrincipal::SetDockRoutingPolicy(__int64 a1, __int64 a2, int a3)
{
  const struct _TlgProvider_t *v5; // rax
  __int64 v6; // rdx
  const char *v7; // rax
  TraceLoggingHProvider v8; // r10
  int v9; // eax
  bool v10; // si
  _QWORD *v11; // rdi
  _QWORD *i; // rbx
  int v13; // eax
  UINT32 cData; // [rsp+20h] [rbp-58h]
  EVENT_DATA_DESCRIPTOR pData; // [rsp+30h] [rbp-48h] BYREF
  struct _EVENT_DATA_DESCRIPTOR pDesc; // [rsp+50h] [rbp-28h] BYREF
  wil::details::in1diag3 *retaddr; // [rsp+78h] [rbp+0h]

  v5 = InputTraceLogging::Provider();
  if ( *(_DWORD *)v5 > 4u && TlgKeywordOn(v5, 0x200uLL) )
  {
    v7 = InputTraceLogging::PenPolicyToString(a3);
    TlgCreateSz(&pDesc, v7);
    TlgWrite(v8, &unk_18019B376, 0LL, 0LL, 3u, &pData);
  }
  v9 = *(_DWORD *)(a1 + 196);
  *(_DWORD *)(a1 + 196) = a3;
  v10 = a3 != 0;
  if ( (v9 != 0) != (a3 != 0) )
  {
    v11 = *(_QWORD **)(a1 + 64);
    for ( i = (_QWORD *)*v11; i != v11; i = (_QWORD *)*i )
    {
      LOBYTE(v6) = v10;
      v13 = (*(__int64 (__fastcall **)(__int64, __int64))(*(_QWORD *)(i[3] + 8LL) + 72LL))(i[3] + 8LL, v6);
      if ( v13 < 0 )
        wil::details::in1diag3::_Log_Hr(
          retaddr,
          (void *)0x89,
          (unsigned int)"onecoreuap\\windows\\moderncore\\inputv2\\inputprocessors\\devices\\stylus\\events\\server\\pene"
                        "ventsdispatcherprincipal.cpp",
          (const char *)(unsigned int)v13,
          cData);
    }
  }
  return 0LL;
}
