/*
 * XREFs of SfnINOUTDRAG @ 0x1C0218D60
 * Callers:
 *     <none>
 * Callees:
 *     UserSetLastError @ 0x1C002862C (UserSetLastError.c)
 *     ??1ReleaseAndReacquirePerObjectLocks@@QEAA@XZ @ 0x1C004ECF4 (--1ReleaseAndReacquirePerObjectLocks@@QEAA@XZ.c)
 *     ??1LeaveEnterCritProperDisposition@@QEAA@XZ @ 0x1C004ED94 (--1LeaveEnterCritProperDisposition@@QEAA@XZ.c)
 *     ??0LeaveEnterCritProperDisposition@@QEAA@XZ @ 0x1C004EDCC (--0LeaveEnterCritProperDisposition@@QEAA@XZ.c)
 *     ??0ReleaseAndReacquirePerObjectLocks@@QEAA@XZ @ 0x1C004F434 (--0ReleaseAndReacquirePerObjectLocks@@QEAA@XZ.c)
 *     W32GetThreadWin32Thread @ 0x1C00AC2AC (W32GetThreadWin32Thread.c)
 */

__int64 __fastcall SfnINOUTDRAG(
        _QWORD *a1,
        __int64 a2,
        __int64 a3,
        __int128 *a4,
        __int64 a5,
        __int64 a6,
        char a7,
        __int64 a8)
{
  int v10; // r12d
  __int64 ThreadWin32Thread; // rax
  __int64 v13; // rdx
  __int64 v14; // r8
  __int64 v15; // rsi
  __int64 v16; // rdi
  __int64 v17; // rax
  __int64 v18; // r8
  __int64 v19; // rax
  __int64 v20; // rcx
  __int64 v21; // rcx
  int v22; // ebx
  __int64 v23; // rdx
  __int64 v24; // r8
  __int64 v25; // rdx
  __int64 v26; // rcx
  __int64 v27; // r8
  __int64 v28; // rcx
  __int64 *v29; // rcx
  __int64 v30; // r8
  __int64 v31; // rdx
  __int128 *v32; // rcx
  __int128 v33; // xmm1
  __int128 v34; // xmm2
  __int64 v36; // [rsp+48h] [rbp-100h] BYREF
  _QWORD *v37; // [rsp+50h] [rbp-F8h]
  __int64 v38; // [rsp+58h] [rbp-F0h]
  __int128 v39; // [rsp+60h] [rbp-E8h]
  __int64 v40; // [rsp+70h] [rbp-D8h]
  __int64 v41; // [rsp+78h] [rbp-D0h]
  __int128 v42; // [rsp+A0h] [rbp-A8h]
  __int64 v43; // [rsp+B0h] [rbp-98h] BYREF
  int v44; // [rsp+B8h] [rbp-90h]
  int v45; // [rsp+BCh] [rbp-8Ch]
  __int64 v46; // [rsp+C0h] [rbp-88h]
  __int64 v47; // [rsp+C8h] [rbp-80h]
  __int64 v48; // [rsp+D0h] [rbp-78h]
  __int128 v49; // [rsp+D8h] [rbp-70h]
  __int128 v50; // [rsp+E8h] [rbp-60h]
  __int128 v51; // [rsp+F8h] [rbp-50h]
  char v52; // [rsp+150h] [rbp+8h] BYREF
  char v53; // [rsp+158h] [rbp+10h] BYREF
  int v54; // [rsp+160h] [rbp+18h] BYREF
  unsigned __int64 v55; // [rsp+168h] [rbp+20h] BYREF

  v10 = a2;
  v36 = 0LL;
  v37 = 0LL;
  v38 = 0LL;
  v39 = 0uLL;
  v40 = 0LL;
  ThreadWin32Thread = W32GetThreadWin32Thread((__int64)KeGetCurrentThread(), a2, a3);
  v15 = ThreadWin32Thread;
  if ( a1 )
    v16 = a1[5] - *(_QWORD *)(ThreadWin32Thread + 464);
  else
    v16 = 0LL;
  v45 = 0;
  v43 = v16;
  v44 = v10;
  v46 = a3;
  *((_DWORD *)a4 + 5) = 0;
  *((_DWORD *)a4 + 11) = 0;
  v49 = *a4;
  v50 = a4[1];
  v51 = a4[2];
  v47 = a5;
  v48 = a6;
  v17 = W32GetThreadWin32Thread((__int64)KeGetCurrentThread(), v13, v14);
  v36 = *(_QWORD *)(v17 + 408);
  *(_QWORD *)(v17 + 408) = &v36;
  v37 = a1;
  if ( a1 )
    HMLockObject(a1);
  v19 = *(_QWORD *)(v15 + 472);
  v42 = *(_OWORD *)(v19 + 64);
  v39 = v42;
  v40 = *(_QWORD *)(v19 + 80);
  *(_QWORD *)(v19 + 72) = v16;
  if ( a1 )
    v20 = *a1;
  else
    v20 = 0LL;
  *(_QWORD *)(*(_QWORD *)(v15 + 472) + 64LL) = v20;
  if ( a1 )
    v21 = *(_QWORD *)(a1[5] + 224LL);
  else
    v21 = 0LL;
  *(_QWORD *)(*(_QWORD *)(v15 + 472) + 80LL) = v21;
  if ( gdwInAtomicOperation && (gdwExtraInstrumentations & 1) != 0 )
    KeBugCheckEx(0x160u, gdwInAtomicOperation, 0LL, 0LL, 0LL);
  ReleaseAndReacquirePerObjectLocks::ReleaseAndReacquirePerObjectLocks(
    (ReleaseAndReacquirePerObjectLocks *)&v52,
    gdwInAtomicOperation,
    v18);
  LeaveEnterCritProperDisposition::LeaveEnterCritProperDisposition((LeaveEnterCritProperDisposition *)&v53);
  EtwTraceBeginCallback(5LL);
  v22 = KeUserModeCallback(5LL, &v43, 88LL, &v55, &v54);
  EtwTraceEndCallback(5LL);
  LeaveEnterCritProperDisposition::~LeaveEnterCritProperDisposition((LeaveEnterCritProperDisposition *)&v53);
  ReleaseAndReacquirePerObjectLocks::~ReleaseAndReacquirePerObjectLocks(
    (ReleaseAndReacquirePerObjectLocks *)&v52,
    v23,
    v24);
  ThreadUnlock1(v26, v25, v27);
  v28 = *(_QWORD *)(v15 + 472);
  *(_OWORD *)(v28 + 64) = v42;
  *(_QWORD *)(v28 + 80) = v40;
  if ( v22 >= 0 && v54 == 24 )
  {
    v29 = (__int64 *)v55;
    if ( v55 + 8 < v55 || v55 + 8 > MmUserProbeAddress )
      v29 = (__int64 *)MmUserProbeAddress;
    v30 = *v29;
    v41 = *v29;
    if ( a8 && (*(_DWORD *)(a8 + 84) & 5) != 0 || (a7 & 2) != 0 )
      return v30;
    v31 = *(_QWORD *)(gptiCurrent + 504LL);
    if ( !v31 || (*(_DWORD *)(v31 + 84) & 1) == 0 || *(__int128 **)(v31 + 96) != a4 )
    {
      v32 = *(__int128 **)(v55 + 16);
      if ( v32 + 3 < v32 || (unsigned __int64)(v32 + 3) > MmUserProbeAddress )
        v32 = (__int128 *)MmUserProbeAddress;
      v33 = v32[1];
      v34 = v32[2];
      *a4 = *v32;
      a4[1] = v33;
      a4[2] = v34;
      return v30;
    }
  }
  return 0LL;
}
