/*
 * XREFs of xInsertMetricsPlusRFONTOBJ @ 0x1C0098CE4
 * Callers:
 *     ?bInsertMetricsPlus@RFONTOBJ@@QEAAHPEAPEAU_GLYPHDATA@@G@Z @ 0x1C0098C94 (-bInsertMetricsPlus@RFONTOBJ@@QEAAHPEAPEAU_GLYPHDATA@@G@Z.c)
 * Callees:
 *     ?bInsertMetrics@RFONTOBJ@@QEAAHPEAPEAU_GLYPHDATA@@G@Z @ 0x1C0098F78 (-bInsertMetrics@RFONTOBJ@@QEAAHPEAPEAU_GLYPHDATA@@G@Z.c)
 *     ?bCheckMetricsCache@RFONTOBJ@@QEAAHXZ @ 0x1C0098FC8 (-bCheckMetricsCache@RFONTOBJ@@QEAAHXZ.c)
 *     ?QueryFontData@PFFOBJ@@QEAAJPEAUDHPDEV__@@PEAU_FONTOBJ@@KKPEAU_GLYPHDATA@@PEAXK@Z @ 0x1C009CF80 (-QueryFontData@PFFOBJ@@QEAAJPEAUDHPDEV__@@PEAU_FONTOBJ@@KKPEAU_GLYPHDATA@@PEAXK@Z.c)
 *     ?vXlatGlyphArray@RFONTOBJ@@QEAAXPEBGIPEAKKH@Z @ 0x1C009D0E0 (-vXlatGlyphArray@RFONTOBJ@@QEAAXPEBGIPEAKKH@Z.c)
 *     ?bInsertMetricsPlusPath@RFONTOBJ@@QEAAHPEAPEAU_GLYPHDATA@@G@Z @ 0x1C014B66C (-bInsertMetricsPlusPath@RFONTOBJ@@QEAAHPEAPEAU_GLYPHDATA@@G@Z.c)
 *     ?pgbCheckGlyphCache@RFONTOBJ@@QEAAPEAX_K@Z @ 0x1C014B8EC (-pgbCheckGlyphCache@RFONTOBJ@@QEAAPEAX_K@Z.c)
 *     __security_check_cookie @ 0x1C0163A80 (__security_check_cookie.c)
 *     ?QueryGlyphMetricsPlusBits@PFFOBJ@@QEAAJPEAUDHPDEV__@@PEAU_FONTOBJ@@KKPEAPEAU_GLYPHDATA@@PEAPEAU_GLYPHBITS@@@Z @ 0x1C01679B4 (-QueryGlyphMetricsPlusBits@PFFOBJ@@QEAAJPEAUDHPDEV__@@PEAU_FONTOBJ@@KKPEAPEAU_GLYPHDATA@@PEAPEAU.c)
 *     memset @ 0x1C0168440 (memset.c)
 */

__int64 __fastcall xInsertMetricsPlusRFONTOBJ(RFONTOBJ *this, struct _GLYPHDATA **a2, unsigned __int16 a3)
{
  __int64 v3; // rbx
  unsigned int v4; // edi
  __int64 v8; // r13
  unsigned int v9; // r12d
  struct _GLYPHDATA *v10; // rax
  __int64 result; // rax
  __int64 v12; // r13
  unsigned __int64 v13; // r15
  void *v14; // r13
  struct _GLYPHDATA *v15; // rax
  unsigned int FontData; // eax
  __int64 v17; // rdx
  __int64 v18; // rax
  __int64 v19; // r8
  unsigned int v20; // eax
  unsigned __int16 v21; // [rsp+40h] [rbp-59h] BYREF
  struct _GLYPHDATA *v22; // [rsp+48h] [rbp-51h] BYREF
  struct _GLYPHBITS *v23; // [rsp+50h] [rbp-49h] BYREF
  _QWORD v24[3]; // [rsp+58h] [rbp-41h] BYREF
  struct _GLYPHDATA v25; // [rsp+70h] [rbp-29h] BYREF

  v3 = *(_QWORD *)this;
  v4 = 0;
  v8 = *(_QWORD *)(*(_QWORD *)this + 480LL);
  if ( (*(_DWORD *)(*(_QWORD *)this + 84LL) & 2) != 0 )
  {
    v21 = a3;
    RFONTOBJ::vXlatGlyphArray(this, &v21, 1u, (unsigned int *)&v22, 0, 0);
    v9 = (unsigned int)v22;
    v3 = *(_QWORD *)this;
  }
  else
  {
    v9 = a3;
  }
  if ( v9 == *(_DWORD *)(v3 + 460) )
  {
    v10 = *(struct _GLYPHDATA **)(v8 + 8);
    if ( v10 )
    {
      *a2 = v10;
      return 1LL;
    }
  }
  if ( !*(_DWORD *)(v3 + 88) )
    return RFONTOBJ::bInsertMetrics(this, a2, a3);
  result = RFONTOBJ::bCheckMetricsCache(this);
  if ( (_DWORD)result )
  {
    v12 = *(_QWORD *)this;
    if ( *(_DWORD *)(*(_QWORD *)this + 88LL) == 2 )
      return RFONTOBJ::bInsertMetricsPlusPath(this, a2, a3);
    v24[0] = *(_QWORD *)(v12 + 128);
    if ( *(struct PDEV **)(v24[0] + 88LL) == qword_1C0330E80 )
    {
      *(_DWORD *)(v3 + 644) = 6;
      v19 = *(_QWORD *)this;
      v22 = 0LL;
      v20 = *(_DWORD *)(v3 + 636);
      v23 = 0LL;
      if ( (unsigned int)PFFOBJ::QueryGlyphMetricsPlusBits(
                           (PFFOBJ *)v24,
                           *(struct DHPDEV__ **)(v19 + 112),
                           (struct _FONTOBJ *)v19,
                           v9,
                           v20,
                           &v22,
                           &v23) == -1 )
        return v4;
      v22->gdf.pgb = v23;
      *a2 = v22;
    }
    else
    {
      memset(&v25, 0, sizeof(v25));
      v13 = *(unsigned int *)(v3 + 632);
      if ( v13 > *(_QWORD *)(v3 + 592) - *(_QWORD *)(v3 + 584) )
      {
        LODWORD(v13) = PFFOBJ::QueryFontData(
                         (PFFOBJ *)v24,
                         *(struct DHPDEV__ **)(v12 + 112),
                         (struct _FONTOBJ *)v12,
                         1u,
                         v9,
                         &v25,
                         0LL,
                         0);
        if ( (_DWORD)v13 == -1 )
          return v4;
      }
      v14 = RFONTOBJ::pgbCheckGlyphCache(this, (unsigned int)v13);
      v15 = &v25;
      if ( !*(_DWORD *)(*(_QWORD *)this + 640LL) )
        v15 = *(struct _GLYPHDATA **)(v3 + 512);
      FontData = PFFOBJ::QueryFontData(
                   (PFFOBJ *)v24,
                   *(struct DHPDEV__ **)(*(_QWORD *)this + 112LL),
                   *(struct _FONTOBJ **)this,
                   1u,
                   v9,
                   v15,
                   v14,
                   v13);
      v17 = FontData;
      if ( FontData == -1 )
        return v4;
      if ( *(_DWORD *)(*(_QWORD *)this + 640LL) )
      {
        v18 = *(_QWORD *)(v3 + 512);
        *(_OWORD *)v18 = *(_OWORD *)&v25.gdf.pgb;
        *(_QWORD *)(v18 + 16) = *(_QWORD *)&v25.fxA;
      }
      *a2 = *(struct _GLYPHDATA **)(v3 + 512);
      *(_QWORD *)(v3 + 512) += (-(__int64)(*(_DWORD *)(*(_QWORD *)this + 640LL) != 0) & 0xFFFFFFFFFFFFFFD8uLL) + 64;
      if ( v14 )
        *(_QWORD *)(v3 + 584) += v17;
    }
    return 1;
  }
  return result;
}
