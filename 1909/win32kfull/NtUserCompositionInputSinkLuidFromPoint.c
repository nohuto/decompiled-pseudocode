/*
 * XREFs of NtUserCompositionInputSinkLuidFromPoint @ 0x1C0229D60
 * Callers:
 *     <none>
 * Callees:
 *     RequestInputSinkInfoFromPoint @ 0x1C000669C (RequestInputSinkInfoFromPoint.c)
 *     UserSetLastError @ 0x1C002862C (UserSetLastError.c)
 *     memset @ 0x1C0168440 (memset.c)
 */

__int64 __fastcall NtUserCompositionInputSinkLuidFromPoint(
        int a1,
        __int64 *a2,
        _QWORD *a3,
        _QWORD *a4,
        _OWORD *Address)
{
  __int64 v9; // rdx
  __int64 v10; // rcx
  __int64 v11; // r8
  __int64 v12; // r9
  __int64 v13; // rcx
  int v14; // ebx
  ULONG64 v15; // rcx
  __int64 CurrentProcess; // rax
  ULONG64 v17; // rcx
  __int64 CurrentProcessWow64Process; // rax
  __int64 v19; // rcx
  int v20; // ebx
  _OWORD *v21; // rdi
  __int64 v22; // rax
  char v23; // dl
  __int128 v25[7]; // [rsp+50h] [rbp-D8h] BYREF
  __int64 v26; // [rsp+C0h] [rbp-68h]
  _OWORD v27[4]; // [rsp+E0h] [rbp-48h] BYREF

  ExEnterCriticalRegionAndAcquireResourceExclusive(gpresDitCompositionInputSinkQuery);
  EnterCrit(0LL, 1LL);
  memset(v25, 0, sizeof(v25));
  if ( a1 != 6 && a1 )
  {
    v13 = 87LL;
LABEL_4:
    v14 = 0;
    UserSetLastError(v13, v9, v11, v12);
    goto LABEL_25;
  }
  CurrentProcess = PsGetCurrentProcess(v10, v9, v11, v12);
  if ( (unsigned int)IsProcessDwm(CurrentProcess) )
  {
    v13 = 5LL;
    goto LABEL_4;
  }
  v17 = (ULONG64)(a2 + 1);
  if ( a2 + 1 < a2 || v17 > MmUserProbeAddress )
    a2 = (__int64 *)MmUserProbeAddress;
  v26 = *a2;
  *(_QWORD *)((char *)v25 + 4) = v26;
  CurrentProcessWow64Process = PsGetCurrentProcessWow64Process(v17);
  ProbeForWrite(a3, 8uLL, CurrentProcessWow64Process != 0 ? 1 : 4);
  v20 = LODWORD(v25[0]) | 1;
  LODWORD(v25[0]) |= 1u;
  if ( a4 )
  {
    ProbeForWrite(a4, 8uLL, 8u);
    v20 |= 2u;
    LODWORD(v25[0]) = v20;
    *a4 = 0LL;
  }
  v21 = Address;
  if ( Address )
  {
    v22 = PsGetCurrentProcessWow64Process(v19);
    ProbeForWrite(Address, 0x40uLL, v22 != 0 ? 1 : 4);
    LODWORD(v25[0]) = v20 | 4;
    memset(v27, 0, sizeof(v27));
    *Address = v27[0];
    Address[1] = v27[1];
    Address[2] = v27[2];
    Address[3] = v27[3];
  }
  HIDWORD(v25[0]) = a1;
  v14 = RequestInputSinkInfoFromPoint(v25);
  if ( v14 )
  {
    v15 = MmUserProbeAddress;
    if ( (unsigned __int64)a3 >= MmUserProbeAddress )
      a3 = (_QWORD *)MmUserProbeAddress;
    *a3 = *(_QWORD *)((char *)&v25[1] + 4);
    v23 = v25[0];
    if ( (v25[0] & 2) != 0 )
    {
      v15 = MmUserProbeAddress;
      if ( (unsigned __int64)a4 >= MmUserProbeAddress )
        a4 = (_QWORD *)MmUserProbeAddress;
      *a4 = *(_QWORD *)&v25[2];
    }
    if ( (v23 & 4) != 0 )
    {
      v15 = MmUserProbeAddress;
      if ( (unsigned __int64)Address >= MmUserProbeAddress )
        v21 = (_OWORD *)MmUserProbeAddress;
      *v21 = *(__int128 *)((char *)&v25[2] + 8);
      v21[1] = *(__int128 *)((char *)&v25[3] + 8);
      v21[2] = *(__int128 *)((char *)&v25[4] + 8);
      v21[3] = *(__int128 *)((char *)&v25[5] + 8);
    }
  }
LABEL_25:
  UserSessionSwitchLeaveCrit(v15);
  ExReleaseResourceAndLeaveCriticalRegion(gpresDitCompositionInputSinkQuery);
  return v14;
}
