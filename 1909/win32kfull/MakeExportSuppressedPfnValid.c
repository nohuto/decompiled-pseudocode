/*
 * XREFs of MakeExportSuppressedPfnValid @ 0x1C005026C
 * Callers:
...
{
  NTSTATUS result; // eax
  __int64 ProcessInformation; // [rsp+30h] [rbp-9h] BYREF
  char v4; // [rsp+38h] [rbp-1h] BYREF
  _QWORD v5[2]; // [rsp+40h] [rbp+7h] BYREF
  _DWORD v6[2]; // [rsp+50h] [rbp+17h] BYREF
  char *v7; // [rsp+58h] [rbp+1Fh]
  ...
  if ( result >= 0 && (ProcessInformation & 0x200000000LL) != 0 )
  {
    ...
    v11[0] = a1 & 0xFFF;
    v6[0] = 1;
    return ZwSetInformationVirtualMemory(-1LL, 2LL, 1LL, v5, v6, 40);
  }
  return result;
}
