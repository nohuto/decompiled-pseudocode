/*
 * XREFs of ?IsProcessPriorityByClassBackground@PriorityBoost@@YA_NPEBUtagPROCESSINFO@@@Z @ 0x1C0211884
 * Callers:
 *     ?AdjustProcessPriorityForDrag@@YAXPEAU_MOVESIZEDATA@@@Z @ 0x1C01F9854 (-AdjustProcessPriorityForDrag@@YAXPEAU_MOVESIZEDATA@@@Z.c)
 * Callees:
 *     ??0CAutoPushLockSh@@QEAA@PEAU_EX_PUSH_LOCK@@@Z @ 0x1C00FAE6C (--0CAutoPushLockSh@@QEAA@PEAU_EX_PUSH_LOCK@@@Z.c)
 */

bool __fastcall PriorityBoost::IsProcessPriorityByClassBackground(PriorityBoost *this, const struct tagPROCESSINFO *a2)
{
  __int64 v3; // r8
  __int64 v4; // r9
  __int64 v6; // [rsp+30h] [rbp+8h] BYREF

  CAutoPushLockSh::CAutoPushLockSh((CAutoPushLockSh *)&v6, (struct _EX_PUSH_LOCK *)&`anonymous namespace'::boostLock);
  LOBYTE(this) = (*((_DWORD *)this + 203) & 0x60000) == 0;
  ExReleasePushLockSharedEx(v6, 0LL, v3, v4);
  KeLeaveCriticalRegion();
  return (char)this;
}
