/*
 * XREFs of vDrawGlyph @ 0x1C0166818
 * Callers:
 *     vStringBitmapTextOut @ 0x1C01669E0 (vStringBitmapTextOut.c)
 * Callees:
 *     <none>
 */

char __fastcall vDrawGlyph(_DWORD *a1, unsigned int a2, __int64 a3)
{
  __int64 v3; // rax
  __int64 v5; // r14
  int v6; // r9d
  int v7; // r10d
  int v8; // edx
  int v9; // ecx
  __int64 v10; // rbx
  unsigned int v11; // esi
  unsigned int v12; // r11d
  unsigned int v13; // r8d
  unsigned int v14; // r11d
  __int64 v15; // r10
  __int64 v16; // rdi
  _BYTE *v17; // r10
  char v18; // bp
  __int64 v19; // rbp
  unsigned int v20; // r8d
  _BYTE *v21; // r9
  _BYTE *v22; // rcx
  unsigned int i; // edx
  char v24; // cl
  unsigned int v25; // r14d
  __int64 v26; // r12
  unsigned __int8 *v27; // r8
  _BYTE *v28; // r9
  unsigned int v29; // edi
  unsigned __int16 v30; // dx
  unsigned __int8 *v31; // r8
  __int64 v33; // [rsp+40h] [rbp+18h]

  v3 = *(_QWORD *)(a3 + 8);
  v5 = a2;
  v6 = *(_DWORD *)(*(_QWORD *)v3 + 8LL);
  v7 = *(_DWORD *)(*(_QWORD *)v3 + 12LL);
  if ( v6 && v7 )
  {
    v8 = *(_DWORD *)(a3 + 16);
    v9 = *(_DWORD *)(a3 + 20);
    v10 = *(_QWORD *)v3 + 16LL;
    v11 = (unsigned int)(v6 + 7) >> 3;
    if ( v8 < 0 )
    {
      LODWORD(v3) = -v8;
      if ( v6 <= -v8 )
        return v3;
      v6 -= v8;
      v8 = 0;
    }
    if ( v9 < 0 )
    {
      LODWORD(v3) = -v9;
      if ( v7 <= -v9 )
        return v3;
      v7 -= v9;
      v9 = 0;
    }
    if ( (unsigned int)v8 < *a1 )
    {
      v12 = a1[1];
      if ( v9 < v12 )
      {
        v13 = *a1 - v8;
        v33 = v5;
        if ( v13 >= v6 )
          v13 = v6;
        v14 = v12 - v9;
        if ( v14 >= v7 )
          v14 = v7;
        v15 = (__int64)a1 + (unsigned int)(v5 * v9) + 8;
        v16 = v5;
        v17 = (_BYTE *)(((__int64)v8 >> 3) + v15);
        v18 = v8 & 7;
        if ( (v8 & 7) != 0 )
        {
          v24 = v8 & 7;
          v25 = ((v13 + v8) >> 3) - (v8 >> 3);
          v26 = ((_BYTE)v13 + (_BYTE)v8) & 7;
          do
          {
            v27 = (unsigned __int8 *)v10;
            v28 = v17;
            v17 += v16;
            v10 += v11;
            v29 = v25;
            v30 = *v27;
            v31 = v27 + 1;
            if ( v25 )
            {
              do
              {
                LOWORD(v3) = v30 >> v18;
                --v29;
                *v28++ |= v30 >> v18;
                v30 <<= 8;
                if ( v31 != (unsigned __int8 *)v10 )
                {
                  LOWORD(v3) = *v31;
                  v30 |= v3;
                  ++v31;
                }
              }
              while ( v29 );
              v24 = v18;
            }
            if ( (_DWORD)v26 )
              *v28 |= byte_1C02E3760[v26] & (unsigned __int8)(v30 >> v24);
            else
              v24 = v18;
            v16 = v33;
            --v14;
          }
          while ( v14 );
        }
        else
        {
          v19 = v13 & 7;
          v20 = v13 >> 3;
          do
          {
            v21 = (_BYTE *)v10;
            v22 = v17;
            v10 += v11;
            v17 += v5;
            for ( i = v20; i; --i )
            {
              LOBYTE(v3) = *v21++;
              *v22++ |= v3;
            }
            if ( (_DWORD)v19 )
            {
              LOBYTE(v3) = byte_1C02E3760[v19] & *v21;
              *v22 |= v3;
            }
            --v14;
          }
          while ( v14 );
        }
      }
    }
  }
  return v3;
}
