/*
 * XREFs of ?GetMonitorCapabilityFromInf@@YAKPEAU_DEVICE_OBJECT@@PEAUtagModeCap@@E@Z @ 0x1C00861A4
 * Callers:
 *     ?GetMonitorCapability@@YAKPEAU_DEVICE_OBJECT@@PEAUtagModeCap@@PEAU_FREQUENCY_RAGE@@E@Z @ 0x1C008557C (-GetMonitorCapability@@YAKPEAU_DEVICE_OBJECT@@PEAUtagModeCap@@PEAU_FREQUENCY_RAGE@@E@Z.c)
 * Callees:
 *     ?InsertModecapList@@YAKPEAUtagModeCap@@0K@Z @ 0x1C0085AD0 (-InsertModecapList@@YAKPEAUtagModeCap@@0K@Z.c)
 *     ?ParseModeCap@@YAHPEAGPEAUtagModeCap@@H@Z @ 0x1C00864C4 (-ParseModeCap@@YAHPEAGPEAUtagModeCap@@H@Z.c)
 *     __security_check_cookie @ 0x1C00B7F30 (__security_check_cookie.c)
 *     __report_rangecheckfailure @ 0x1C00B7F70 (__report_rangecheckfailure.c)
 *     memset @ 0x1C00BF640 (memset.c)
 */

__int64 __fastcall GetMonitorCapabilityFromInf(struct _DEVICE_OBJECT *a1, struct tagModeCap *a2, char a3)
{
  unsigned int inserted; // esi
  ULONG v6; // r14d
  ULONG v7; // edx
  unsigned __int64 v8; // r9
  unsigned __int64 v9; // rax
  int v10; // edi
  unsigned int v11; // ebx
  ULONG ResultLength; // [rsp+30h] [rbp-D0h] BYREF
  struct _UNICODE_STRING DestinationString; // [rsp+38h] [rbp-C8h] BYREF
  void *KeyHandle; // [rsp+48h] [rbp-B8h] BYREF
  ULONG v16; // [rsp+50h] [rbp-B0h]
  __int64 v17; // [rsp+58h] [rbp-A8h] BYREF
  __int64 v18; // [rsp+60h] [rbp-A0h]
  unsigned __int64 v19; // [rsp+68h] [rbp-98h]
  HANDLE Handle; // [rsp+70h] [rbp-90h] BYREF
  void *DeviceRegKey; // [rsp+78h] [rbp-88h] BYREF
  struct _OBJECT_ATTRIBUTES ObjectAttributes; // [rsp+80h] [rbp-80h] BYREF
  _DWORD KeyInformation[64]; // [rsp+B0h] [rbp-50h] BYREF
  WCHAR SourceString[8]; // [rsp+1B0h] [rbp+B0h] BYREF
  wchar_t Dst[128]; // [rsp+1C0h] [rbp+C0h] BYREF

  if ( IoOpenDeviceRegistryKey(a1, 2u, 0x20019u, &DeviceRegKey) < 0 )
    return 0LL;
  inserted = 0;
  memset(&ObjectAttributes, 0, sizeof(ObjectAttributes));
  *(_QWORD *)&DestinationString.Length = 0LL;
  DestinationString.Buffer = 0LL;
  RtlInitUnicodeString(&DestinationString, L"MODES");
  ObjectAttributes.RootDirectory = DeviceRegKey;
  ObjectAttributes.Length = 48;
  ObjectAttributes.ObjectName = &DestinationString;
  ObjectAttributes.Attributes = 576;
  *(_OWORD *)&ObjectAttributes.SecurityDescriptor = 0LL;
  if ( ZwOpenKey(&KeyHandle, 0x20019u, &ObjectAttributes) >= 0 )
  {
    memset(KeyInformation, 0, sizeof(KeyInformation));
    v6 = 0;
    v17 = 0LL;
    v7 = 0;
    v18 = 0LL;
    v19 = 0LL;
    while ( ZwEnumerateKey(KeyHandle, v7, KeyBasicInformation, KeyInformation, 0x100u, &ResultLength) >= 0 )
    {
      ++v6;
      DestinationString.Buffer = (PWSTR)&KeyInformation[4];
      v16 = v6;
      DestinationString.Length = KeyInformation[3];
      DestinationString.MaximumLength = KeyInformation[3];
      v8 = LOWORD(KeyInformation[3]);
      if ( LOWORD(KeyInformation[3]) >= 0x100u )
        v8 = 256LL;
      wcsncpy_s(Dst, 0x80uLL, (const wchar_t *)&KeyInformation[4], v8 >> 1);
      if ( DestinationString.Length < 0x100u )
      {
        v9 = DestinationString.Length & 0xFFFE;
        if ( v9 >= 0x100 )
          _report_rangecheckfailure();
        *(wchar_t *)((char *)Dst + v9) = 0;
      }
      Dst[127] = 0;
      if ( (unsigned int)ParseModeCap(Dst, (struct tagModeCap *)&v17, 0) )
      {
        ObjectAttributes.RootDirectory = KeyHandle;
        ObjectAttributes.Length = 48;
        ObjectAttributes.ObjectName = &DestinationString;
        ObjectAttributes.Attributes = 576;
        *(_OWORD *)&ObjectAttributes.SecurityDescriptor = 0LL;
        if ( ZwOpenKey(&Handle, 0x2000000u, &ObjectAttributes) >= 0 )
        {
          v10 = 0;
          v11 = 0;
          wcscpy(SourceString, L"Mode1");
          do
          {
            RtlInitUnicodeString(&DestinationString, SourceString);
            if ( ZwQueryValueKey(
                   Handle,
                   &DestinationString,
                   KeyValueFullInformation,
                   KeyInformation,
                   0x100u,
                   &ResultLength) >= 0 )
            {
              wcsncpy_s(Dst, 0x80uLL, (const wchar_t *)((char *)KeyInformation + KeyInformation[2]), 0x7FuLL);
              v19 = 0xFFFFFFFF00000000uLL;
              LODWORD(v18) = -1;
              HIDWORD(v18) = a3 == 0 ? 0x38 : 0;
              if ( (unsigned int)ParseModeCap(Dst, (struct tagModeCap *)&v17, 1) )
                inserted = InsertModecapList((struct tagModeCap *)&v17, a2, inserted);
              v10 = 1;
            }
            else if ( v10 )
            {
              break;
            }
            ++SourceString[4];
            ++v11;
          }
          while ( v11 < 9 );
          ZwClose(Handle);
          v6 = v16;
        }
      }
      v7 = v6;
    }
    ZwClose(KeyHandle);
  }
  ZwClose(DeviceRegKey);
  return inserted;
}
