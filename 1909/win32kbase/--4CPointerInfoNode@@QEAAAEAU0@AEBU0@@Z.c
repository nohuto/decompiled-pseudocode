/*
 * XREFs of ??4CPointerInfoNode@@QEAAAEAU0@AEBU0@@Z @ 0x1C015E14C
 * Callers:
 *     ?CopyPTPPointerInputFrame@CTouchProcessor@@IEAAPEAUCPointerInputFrame@@PEAU2@PEAX@Z @ 0x1C0162218 (-CopyPTPPointerInputFrame@CTouchProcessor@@IEAAPEAUCPointerInputFrame@@PEAU2@PEAX@Z.c)
 * Callees:
 *     ??4CInputDest@@QEAAAEAV0@AEBV0@@Z @ 0x1C004EF64 (--4CInputDest@@QEAAAEAV0@AEBV0@@Z.c)
 */

__int64 __fastcall CPointerInfoNode::operator=(__int64 a1, __int64 a2)
{
  int v4; // r8d
  int v5; // eax
  int v6; // ecx
  int v7; // eax
  int v8; // ecx
  int v9; // eax
  int v10; // ecx
  int v11; // eax
  int v12; // ecx
  int v13; // eax
  int v14; // ecx
  int v15; // eax
  int v16; // edx
  int v17; // ecx
  int v18; // eax
  int v19; // ecx
  int v20; // eax
  int v21; // ecx
  int v22; // eax
  int v23; // ecx
  int v24; // eax
  int v25; // ecx
  int v26; // eax
  int v27; // ecx
  int v28; // eax
  int v29; // ecx
  int v30; // eax
  int v31; // ecx
  int v32; // eax
  int v33; // ecx
  int v34; // eax
  int v35; // ecx
  int v36; // ecx
  int v37; // eax
  int v38; // ecx
  int v39; // eax
  int v40; // ecx
  int v41; // eax
  int v42; // ecx
  int v43; // eax
  int v44; // ecx
  int v45; // eax
  int v46; // ecx
  int v47; // eax
  int v48; // ecx
  __int64 v49; // r8

  *(_DWORD *)a1 ^= (*(_DWORD *)a1 ^ *(_DWORD *)a2) & 1;
  v4 = *(_DWORD *)a1 ^ (*(_DWORD *)a2 ^ *(_DWORD *)a1) & 2;
  *(_DWORD *)a1 = v4;
  v5 = v4 ^ (*(_DWORD *)a2 ^ v4) & 4;
  *(_DWORD *)a1 = v5;
  v6 = v5 ^ (*(_DWORD *)a2 ^ v5) & 8;
  *(_DWORD *)a1 = v6;
  v7 = v6 ^ (*(_DWORD *)a2 ^ v6) & 0x10;
  *(_DWORD *)a1 = v7;
  v8 = v7 ^ (*(_DWORD *)a2 ^ v7) & 0x20;
  *(_DWORD *)a1 = v8;
  v9 = v8 ^ (*(_DWORD *)a2 ^ v8) & 0x40;
  *(_DWORD *)a1 = v9;
  v10 = v9 ^ (*(_DWORD *)a2 ^ v9) & 0x80;
  *(_DWORD *)a1 = v10;
  v11 = v10 ^ (*(_DWORD *)a2 ^ v10) & 0x100;
  *(_DWORD *)a1 = v11;
  v12 = v11 ^ (*(_DWORD *)a2 ^ v11) & 0x200;
  *(_DWORD *)a1 = v12;
  v13 = v12 ^ (*(_DWORD *)a2 ^ v12) & 0x400;
  *(_DWORD *)a1 = v13;
  v14 = v13 ^ (*(_DWORD *)a2 ^ v13) & 0x800;
  *(_DWORD *)a1 = v14;
  v15 = v14 ^ (*(_DWORD *)a2 ^ v14) & 0x1000;
  *(_DWORD *)a1 = v15;
  v16 = v15 ^ (*(_DWORD *)a2 ^ v15) & 0x2000;
  *(_DWORD *)a1 = v16;
  v17 = v16 ^ (*(_DWORD *)a2 ^ v16) & 0x4000;
  *(_DWORD *)a1 = v17;
  v18 = v17 ^ (*(_DWORD *)a2 ^ v17) & 0x8000;
  *(_DWORD *)a1 = v18;
  v19 = v18 ^ (*(_DWORD *)a2 ^ v18) & 0x10000;
  *(_DWORD *)a1 = v19;
  v20 = v19 ^ (*(_DWORD *)a2 ^ v19) & 0x20000;
  *(_DWORD *)a1 = v20;
  v21 = v20 ^ (*(_DWORD *)a2 ^ v20) & 0x40000;
  *(_DWORD *)a1 = v21;
  v22 = v21 ^ (*(_DWORD *)a2 ^ v21) & 0x80000;
  *(_DWORD *)a1 = v22;
  v23 = v22 ^ (*(_DWORD *)a2 ^ v22) & 0x100000;
  *(_DWORD *)a1 = v23;
  v24 = v23 ^ (*(_DWORD *)a2 ^ v23) & 0x200000;
  *(_DWORD *)a1 = v24;
  v25 = v24 ^ (*(_DWORD *)a2 ^ v24) & 0x400000;
  *(_DWORD *)a1 = v25;
  v26 = v25 ^ (*(_DWORD *)a2 ^ v25) & 0x800000;
  *(_DWORD *)a1 = v26;
  v27 = v26 ^ (*(_DWORD *)a2 ^ v26) & 0x1000000;
  *(_DWORD *)a1 = v27;
  v28 = v27 ^ (*(_DWORD *)a2 ^ v27) & 0x2000000;
  *(_DWORD *)a1 = v28;
  v29 = v28 ^ (*(_DWORD *)a2 ^ v28) & 0x4000000;
  *(_DWORD *)a1 = v29;
  v30 = v29 ^ (*(_DWORD *)a2 ^ v29) & 0x8000000;
  *(_DWORD *)a1 = v30;
  v31 = v30 ^ (*(_DWORD *)a2 ^ v30) & 0x10000000;
  *(_DWORD *)a1 = v31;
  v32 = v31 ^ (*(_DWORD *)a2 ^ v31) & 0x20000000;
  *(_DWORD *)a1 = v32;
  v33 = v32 ^ (v32 ^ *(_DWORD *)a2) & 0x40000000;
  *(_DWORD *)a1 = v33;
  v34 = v33 ^ *(_DWORD *)a2;
  v35 = *(_DWORD *)(a1 + 4);
  *(_DWORD *)a1 = *(_DWORD *)a2 ^ v34 & 0x7FFFFFFF;
  *(_DWORD *)(a1 + 4) ^= (*(_DWORD *)(a2 + 4) ^ v35) & 1;
  v36 = *(_DWORD *)(a1 + 4) ^ (*(_DWORD *)(a2 + 4) ^ *(_DWORD *)(a1 + 4)) & 2;
  *(_DWORD *)(a1 + 4) = v36;
  v37 = v36 ^ ((unsigned __int8)v36 ^ (unsigned __int8)*(_DWORD *)(a2 + 4)) & 4;
  *(_DWORD *)(a1 + 4) = v37;
  v38 = v37 ^ ((unsigned __int8)v37 ^ (unsigned __int8)*(_DWORD *)(a2 + 4)) & 8;
  *(_DWORD *)(a1 + 4) = v38;
  v39 = v38 ^ ((unsigned __int8)v38 ^ (unsigned __int8)*(_DWORD *)(a2 + 4)) & 0x10;
  *(_DWORD *)(a1 + 4) = v39;
  v40 = v39 ^ ((unsigned __int8)v39 ^ (unsigned __int8)*(_DWORD *)(a2 + 4)) & 0x20;
  *(_DWORD *)(a1 + 4) = v40;
  v41 = v40 ^ ((unsigned __int8)v40 ^ (unsigned __int8)*(_DWORD *)(a2 + 4)) & 0x40;
  *(_DWORD *)(a1 + 4) = v41;
  v42 = v41 ^ (v41 ^ *(_DWORD *)(a2 + 4)) & 0x80;
  *(_DWORD *)(a1 + 4) = v42;
  v43 = v42 ^ ((unsigned __int16)v42 ^ (unsigned __int16)*(_DWORD *)(a2 + 4)) & 0x100;
  *(_DWORD *)(a1 + 4) = v43;
  v44 = v43 ^ ((unsigned __int16)v43 ^ (unsigned __int16)*(_DWORD *)(a2 + 4)) & 0x200;
  *(_DWORD *)(a1 + 4) = v44;
  v45 = v44 ^ ((unsigned __int16)v44 ^ (unsigned __int16)*(_DWORD *)(a2 + 4)) & 0x400;
  *(_DWORD *)(a1 + 4) = v45;
  v46 = v45 ^ (v45 ^ *(_DWORD *)(a2 + 4)) & 0x800;
  *(_DWORD *)(a1 + 4) = v46;
  v47 = v46 ^ (v46 ^ *(_DWORD *)(a2 + 4)) & 0x1000;
  *(_DWORD *)(a1 + 4) = v47;
  v48 = v47 ^ (v47 ^ *(_DWORD *)(a2 + 4)) & 0x2000;
  *(_DWORD *)(a1 + 4) = v48;
  *(_DWORD *)(a1 + 4) = v48 ^ (v48 ^ *(_DWORD *)(a2 + 4)) & 0x4000;
  *(_DWORD *)(a1 + 8) = *(_DWORD *)(a2 + 8);
  *(_QWORD *)(a1 + 16) = *(_QWORD *)(a2 + 16);
  CInputDest::operator=(a1 + 24, a2 + 24, 0x4000LL);
  *(_DWORD *)(a1 + 152) = *(_DWORD *)(a2 + 152);
  *(_QWORD *)(a1 + 156) = *(_QWORD *)(a2 + 156);
  *(_DWORD *)(a1 + 164) = *(_DWORD *)(a2 + 164);
  *(_OWORD *)(a1 + 168) = *(_OWORD *)(a2 + 168);
  *(_OWORD *)(a1 + 184) = *(_OWORD *)(a2 + 184);
  *(_OWORD *)(a1 + 200) = *(_OWORD *)(a2 + 200);
  *(_OWORD *)(a1 + 216) = *(_OWORD *)(a2 + 216);
  *(_OWORD *)(a1 + 232) = *(_OWORD *)(a2 + 232);
  *(_OWORD *)(a1 + 248) = *(_OWORD *)(a2 + 248);
  *(_OWORD *)(a1 + 264) = *(_OWORD *)(a2 + 264);
  *(_OWORD *)(a1 + 280) = *(_OWORD *)(a2 + 280);
  *(_OWORD *)(a1 + 296) = *(_OWORD *)(a2 + 296);
  *(_OWORD *)(a1 + 312) = *(_OWORD *)(a2 + 312);
  *(_OWORD *)(a1 + 328) = *(_OWORD *)(a2 + 328);
  *(_QWORD *)(a1 + 344) = *(_QWORD *)(a2 + 344);
  *(_DWORD *)(a1 + 352) = *(_DWORD *)(a2 + 352);
  *(_DWORD *)(a1 + 356) = *(_DWORD *)(a2 + 356);
  CInputDest::operator=(a1 + 360, a2 + 360, v49);
  *(_DWORD *)(a1 + 488) = *(_DWORD *)(a2 + 488);
  return a1;
}
