/*
 * XREFs of ?IVDeSerializeIVTouchInputPacket@@YAJPEAU_IVTouchInputPacket@@AEAUCIVSerializer@@@Z @ 0x1C01873E4
 * Callers:
...
__int64 __fastcall IVDeSerializeIVTouchInputPacket(struct _IVTouchInputPacket *a1, struct CIVSerializer *a2)
{
  ...
  *((_QWORD *)a1 + 1) = *((_QWORD *)v2 + 2) + (*((_QWORD *)a1 + 1) & 0xFFFFFFFFFFFFFFFEuLL);
  v5 = CIVSerializer::Deserialize(v2, (struct _IVRIMPOINTERRAWDATA **)a1 + 12);
  if ( v5 >= 0 )
    v5 = CIVSerializer::Deserialize(v2, (unsigned __int16 **)a1 + 13);
  if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED && LOWORD(WPP_GLOBAL_Control->DeviceType) )
  {
    LOBYTE(v4) = 5;
    WPP_RECORDER_SF_(
