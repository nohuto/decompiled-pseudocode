/*
 * XREFs of USBParseGetMicArrayDescriptor @ 0x1C002C14C
 * Callers:
...
__int64 __fastcall USBParseGetMicArrayDescriptor(__int64 a1, __int64 a2, _QWORD *a3)
{
  ...
  _QWORD v17[5]; // [rsp+48h] [rbp-60h] BYREF
  ...
  SetMemory = USBHwGetSetMemory(a1, 0x85u, 0, v4, *(unsigned __int8 *)(*(_QWORD *)(v8 + 48) + 2LL), 0, 0x12u, v17);
  if ( SetMemory < 0 )
  {
  ...
  {
    PoolWithTag = ExAllocatePoolWithTag((POOL_TYPE)512, LOWORD(v17[2]), 0x41627845u);
    if ( PoolWithTag )
    {
    ...
    if ( v11 >= 0 )
    {
      *v7 = PoolWithTag[9];
      ...
      if ( PoolWithTag[17] )
      {
        ...
        while ( v3 < (unsigned __int16)PoolWithTag[17] );
      }
    }
    ExFreePool(PoolWithTag);
  }
