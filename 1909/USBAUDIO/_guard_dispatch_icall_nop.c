/*
 * XREFs of _guard_dispatch_icall_nop @ 0x1C00118E0
 * Callers:
 *     WPP_RECORDER_SF_ @ 0x1C0001008 (WPP_RECORDER_SF_.c)
 *     WPP_RECORDER_SF_d @ 0x1C00010CC (WPP_RECORDER_SF_d.c)
 *     WPP_RECORDER_SF_q @ 0x1C00011B4 (WPP_RECORDER_SF_q.c)
 *     WPP_RECORDER_SF_qZ @ 0x1C000129C (WPP_RECORDER_SF_qZ.c)
 *     WPP_RECORDER_SF_qd @ 0x1C0001404 (WPP_RECORDER_SF_qd.c)
 *     WPP_RECORDER_SF_qdd @ 0x1C0001524 (WPP_RECORDER_SF_qdd.c)
 *     WPP_RECORDER_SF_qq @ 0x1C0001670 (WPP_RECORDER_SF_qq.c)
 *     WPP_RECORDER_SF_qqddd @ 0x1C0001738 (WPP_RECORDER_SF_qqddd.c)
 *     DeviceCreate @ 0x1C0001BE0 (DeviceCreate.c)
 *     DeviceRemove @ 0x1C0001F40 (DeviceRemove.c)
 *     WPP_RECORDER_SF_qddd @ 0x1C000207C (WPP_RECORDER_SF_qddd.c)
 *     WPP_RECORDER_SF_ii @ 0x1C00023D4 (WPP_RECORDER_SF_ii.c)
 *     WPP_RECORDER_SF_did @ 0x1C0002860 (WPP_RECORDER_SF_did.c)
 *     WPP_RECORDER_SF_sd @ 0x1C0002950 (WPP_RECORDER_SF_sd.c)
 *     USBHwGetCurrentFrame @ 0x1C0002CA4 (USBHwGetCurrentFrame.c)
 *     WPP_RECORDER_SF_DDDddd @ 0x1C0003118 (WPP_RECORDER_SF_DDDddd.c)
 *     WPP_RECORDER_SF_di @ 0x1C000325C (WPP_RECORDER_SF_di.c)
 *     WPP_RECORDER_SF_id @ 0x1C0003328 (WPP_RECORDER_SF_id.c)
 *     WPP_RECORDER_SF_qqd @ 0x1C00033F4 (WPP_RECORDER_SF_qqd.c)
 *     WPP_RECORDER_SF_qqqd @ 0x1C00034E0 (WPP_RECORDER_SF_qqqd.c)
 *     USBType1CompleteCallback @ 0x1C00040B0 (USBType1CompleteCallback.c)
 *     USBType1LockDelayCompleteCallback @ 0x1C0004350 (USBType1LockDelayCompleteCallback.c)
 *     USBType1BuildIsochUrbRequest @ 0x1C0004484 (USBType1BuildIsochUrbRequest.c)
 *     WPP_RECORDER_SF_dD @ 0x1C0005544 (WPP_RECORDER_SF_dD.c)
 *     WPP_RECORDER_SF_DDDD @ 0x1C0006658 (WPP_RECORDER_SF_DDDD.c)
 *     USBMidiOutCompleteCallback @ 0x1C0007110 (USBMidiOutCompleteCallback.c)
 *     WPP_RECORDER_SF_dd @ 0x1C000738C (WPP_RECORDER_SF_dd.c)
 *     InitializeSideband @ 0x1C0007528 (InitializeSideband.c)
 *     DeInitSideband @ 0x1C0007788 (DeInitSideband.c)
 *     SidebandDispatchIrp @ 0x1C0007B68 (SidebandDispatchIrp.c)
 *     ?QueryInterface@CSidebandDevice@@UEAAJAEBU_GUID@@PEAPEAX@Z @ 0x1C0007E50 (-QueryInterface@CSidebandDevice@@UEAAJAEBU_GUID@@PEAPEAX@Z.c)
 *     ?AddRef@CSidebandDevice@@UEAAKXZ @ 0x1C0007E70 (-AddRef@CSidebandDevice@@UEAAKXZ.c)
 *     ?Release@CSidebandDevice@@UEAAKXZ @ 0x1C0007E90 (-Release@CSidebandDevice@@UEAAKXZ.c)
 *     WPP_RECORDER_SF_ddd @ 0x1C0007EB0 (WPP_RECORDER_SF_ddd.c)
 *     WPP_RECORDER_SF_s @ 0x1C0007F9C (WPP_RECORDER_SF_s.c)
 *     ?IsFeedbackSupported@CSidebandDevice@@AEAAJPEAU_KSDEVICE@@KPEAH@Z @ 0x1C00093C0 (-IsFeedbackSupported@CSidebandDevice@@AEAAJPEAU_KSDEVICE@@KPEAH@Z.c)
 *     ?IrpMJCreateHandler@CSidebandDevice@@UEAAJPEAU_DEVICE_OBJECT@@PEAU_IRP@@@Z @ 0x1C000B5C0 (-IrpMJCreateHandler@CSidebandDevice@@UEAAJPEAU_DEVICE_OBJECT@@PEAU_IRP@@@Z.c)
 *     ?IrpMJCloseHandler@CSidebandDevice@@UEAAJPEAU_DEVICE_OBJECT@@PEAU_IRP@@@Z @ 0x1C000B740 (-IrpMJCloseHandler@CSidebandDevice@@UEAAJPEAU_DEVICE_OBJECT@@PEAU_IRP@@@Z.c)
 *     WPP_RECORDER_SF_dq @ 0x1C000C790 (WPP_RECORDER_SF_dq.c)
 *     ?GetSiop@CSidebandDevice@@AEAAJPEAU_IRP@@@Z @ 0x1C000EBDC (-GetSiop@CSidebandDevice@@AEAAJPEAU_IRP@@@Z.c)
 *     ?SetSiop@CSidebandDevice@@AEAAJPEAU_IRP@@@Z @ 0x1C000EEFC (-SetSiop@CSidebandDevice@@AEAAJPEAU_IRP@@@Z.c)
 *     USBD_UrbFree @ 0x1C000FA30 (USBD_UrbFree.c)
 *     USBDInternal_QueryUsbVerifierSettings @ 0x1C000FB80 (USBDInternal_QueryUsbVerifierSettings.c)
 *     USBD_CreateHandle @ 0x1C0010034 (USBD_CreateHandle.c)
 *     USBD_CloseHandle @ 0x1C0010478 (USBD_CloseHandle.c)
 *     USBD_SelectInterfaceWithEndpointOffloadUrbAllocateAndBuild @ 0x1C001052C (USBD_SelectInterfaceWithEndpointOffloadUrbAllocateAndBuild.c)
 *     ?NonDelegatingRelease@CUnknown@@UEAAKXZ @ 0x1C0010B70 (-NonDelegatingRelease@CUnknown@@UEAAKXZ.c)
 *     ?NonDelegatingQueryInterface@CUnknown@@UEAAJAEBU_GUID@@PEAPEAX@Z @ 0x1C0010BB0 (-NonDelegatingQueryInterface@CUnknown@@UEAAJAEBU_GUID@@PEAPEAX@Z.c)
 *     WppTraceCallback @ 0x1C0020010 (WppTraceCallback.c)
 *     DeviceStart @ 0x1C0020270 (DeviceStart.c)
 *     DeviceStop @ 0x1C0020920 (DeviceStop.c)
 *     DeviceSurpriseRemoval @ 0x1C0020AF0 (DeviceSurpriseRemoval.c)
 *     DeviceSetPowerState @ 0x1C0020D30 (DeviceSetPowerState.c)
 *     DriverEntry @ 0x1C0020ECC (DriverEntry.c)
 *     DriverUnload @ 0x1C0021380 (DriverUnload.c)
 *     FilterCreateFilterPropertySets @ 0x1C0021694 (FilterCreateFilterPropertySets.c)
 *     FilterCreateFilterFactory @ 0x1C0021804 (FilterCreateFilterFactory.c)
 *     PinGoToLowPowerState @ 0x1C0021C20 (PinGoToLowPowerState.c)
 *     PinGoToFullPowerState @ 0x1C0021D20 (PinGoToFullPowerState.c)
 *     PinCreate @ 0x1C0021E20 (PinCreate.c)
 *     PinClose @ 0x1C0022210 (PinClose.c)
 *     PinProcess @ 0x1C00222B0 (PinProcess.c)
 *     PinSetDataFormat @ 0x1C0022380 (PinSetDataFormat.c)
 *     PinSetDeviceState @ 0x1C00225C0 (PinSetDeviceState.c)
 *     PinBuildDescriptors @ 0x1C0022840 (PinBuildDescriptors.c)
 *     TopologyProcessFeatureUnit @ 0x1C0023A70 (TopologyProcessFeatureUnit.c)
 *     TopologyPUUpDownMix @ 0x1C0024370 (TopologyPUUpDownMix.c)
 *     TopologyPUPrologic @ 0x1C00244E0 (TopologyPUPrologic.c)
 *     TopologyProcessProcessUnit @ 0x1C0024C40 (TopologyProcessProcessUnit.c)
 *     TopologyProcessMIDIElement @ 0x1C0024E30 (TopologyProcessMIDIElement.c)
 *     TopologyBuildFilterTopology @ 0x1C002505C (TopologyBuildFilterTopology.c)
 *     PropertyGetSetSampleRate @ 0x1C0025740 (PropertyGetSetSampleRate.c)
 *     PropertyGetAudioPosition @ 0x1C00257F0 (PropertyGetAudioPosition.c)
 *     PropertySetClientBufferDuration @ 0x1C00258C0 (PropertySetClientBufferDuration.c)
 *     PropertyGetAudioPositionEx @ 0x1C0025940 (PropertyGetAudioPositionEx.c)
 *     SetCopyProtectionOnTerminal @ 0x1C0025E94 (SetCopyProtectionOnTerminal.c)
 *     PropertyDrmAudioStream_SetContentId @ 0x1C00260A0 (PropertyDrmAudioStream_SetContentId.c)
 *     PropertySetFeatureValue @ 0x1C0026220 (PropertySetFeatureValue.c)
 *     PropertySetBooleanSideband @ 0x1C00262F4 (PropertySetBooleanSideband.c)
 *     PropertySetBoolean @ 0x1C00263A0 (PropertySetBoolean.c)
 *     PropertyInitializeBooleanCache @ 0x1C0026570 (PropertyInitializeBooleanCache.c)
 *     PropertySetDbLevelSideband @ 0x1C0026784 (PropertySetDbLevelSideband.c)
 *     PropertySetDbLevel @ 0x1C0026850 (PropertySetDbLevel.c)
 *     PropertyInitializeDbLevelCache @ 0x1C0026A40 (PropertyInitializeDbLevelCache.c)
 *     PropertyInitializeGeqLevelCache @ 0x1C00271A0 (PropertyInitializeGeqLevelCache.c)
 *     PropertyGetSetMuxSource @ 0x1C00273D0 (PropertyGetSetMuxSource.c)
 *     PropertyInitializeMuxCache @ 0x1C0027520 (PropertyInitializeMuxCache.c)
 *     PropertySetMixerLevels @ 0x1C00277F0 (PropertySetMixerLevels.c)
 *     PropertyInitializeMixerControlRange @ 0x1C00279C0 (PropertyInitializeMixerControlRange.c)
 *     PropertyGetSetChannelConfiguration @ 0x1C0027AC0 (PropertyGetSetChannelConfiguration.c)
 *     PropertyGetProcessControlRange @ 0x1C0027C0C (PropertyGetProcessControlRange.c)
 *     PropertySetTopologyNodeEnable @ 0x1C0027CF0 (PropertySetTopologyNodeEnable.c)
 *     WmiDispatchSystemControl @ 0x1C0028CE0 (WmiDispatchSystemControl.c)
 *     ProcessIrpsThread @ 0x1C0028EC0 (ProcessIrpsThread.c)
 *     USBDeviceStart @ 0x1C00291B0 (USBDeviceStart.c)
 *     USBDeviceStop @ 0x1C0029940 (USBDeviceStop.c)
 *     USBHwGetSetProperty @ 0x1C0029E6C (USBHwGetSetProperty.c)
 *     USBParseConvertControlUnits @ 0x1C002DB7C (USBParseConvertControlUnits.c)
 *     USBParseConvertMIDIJacksAndElements @ 0x1C002DC54 (USBParseConvertMIDIJacksAndElements.c)
 *     USBCntrlGetSetDbLevel @ 0x1C002E910 (USBCntrlGetSetDbLevel.c)
 *     USBType1PinFormatChange @ 0x1C002F810 (USBType1PinFormatChange.c)
 *     USBMidiOutAllocateRequest @ 0x1C0030EAC (USBMidiOutAllocateRequest.c)
 *     USBMIDICreateEventPackets @ 0x1C0031234 (USBMIDICreateEventPackets.c)
 * Callees:
 *     <none>
 */

__int64 __fastcall guard_dispatch_icall_nop()
{
  __int64 (*v0)(void); // rax

  return v0();
}
