/*
 * XREFs of ??0CRenderingTechnique@@QEAA@PEAVCBrushRenderingGraph@@$$QEAV?$unique_ptr@VCRenderingTechniqueFragment@@U?$default_delete@VCRenderingTechniqueFragment@@@std@@@std@@@Z @ 0x180042FE0
 * Callers:
 *     ?CreateTechniqueForFragment@CBrushRenderingGraphBuilder@@IEAAJ$$QEAV?$unique_ptr@VCRenderingTechniqueFragment@@U?$default_delete@VCRenderingTechniqueFragment@@@std@@@std@@PEAI@Z @ 0x180042F0C (-CreateTechniqueForFragment@CBrushRenderingGraphBuilder@@IEAAJ$$QEAV-$unique_ptr@VCRenderingTech.c)
 * Callees:
 *     ?CollectStateFromAllFragments@CRenderingTechnique@@AEAAXXZ @ 0x180043580 (-CollectStateFromAllFragments@CRenderingTechnique@@AEAAXXZ.c)
 */

CRenderingTechnique *__fastcall CRenderingTechnique::CRenderingTechnique(
        CRenderingTechnique *this,
        __int64 a2,
        __int64 *a3)
{
  __int64 v4; // rax

  *(_QWORD *)this = a2;
  v4 = *a3;
  *a3 = 0LL;
  *((_QWORD *)this + 1) = v4;
  *((_QWORD *)this + 2) = &CDeviceResourceTable<CD3DConstantBuffer,CRenderingTechnique,CD3DDeviceLevel1>::`vftable';
  *((_QWORD *)this + 3) = this;
  *((_QWORD *)this + 4) = (char *)this + 64;
  *((_QWORD *)this + 5) = (char *)this + 64;
  *((_DWORD *)this + 12) = 2;
  *(_QWORD *)((char *)this + 52) = 2LL;
  *((_QWORD *)this + 10) = 0LL;
  *((_DWORD *)this + 22) = 0;
  *(_QWORD *)((char *)this + 268) = 0LL;
  *((_BYTE *)this + 276) = 0;
  CRenderingTechnique::CollectStateFromAllFragments(this);
  return this;
}
