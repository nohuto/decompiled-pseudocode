/*
 * XREFs of ?DeriveCheckCandidatesList@COverlayContext@@CAJAEBV?$span@PEAVCOverlayContext@@$0?0@gsl@@PEAV?$vector_facade@PEAVOverlayPlaneInfo@COverlayContext@@V?$buffer_impl@PEAVOverlayPlaneInfo@COverlayContext@@$0BA@$00Vliberal_expansion_policy@detail@@@detail@@@detail@@PEA_N@Z @ 0x1800E5DA4
 * Callers:
 *     ?ComputeOverlayConfigurationForAdapter@COverlayContext@@CAJAEBV?$span@PEAVCOverlayContext@@$0?0@gsl@@@Z @ 0x18006FC34 (-ComputeOverlayConfigurationForAdapter@COverlayContext@@CAJAEBV-$span@PEAVCOverlayContext@@$0-0@.c)
 * Callees:
 *     ??3@YAXPEAX@Z @ 0x18007004C (--3@YAXPEAX@Z.c)
 *     ?MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJIPEAX@Z @ 0x1800738CC (-MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJIPEAX@Z.c)
 *     ?clear@?$vector_facade@UDesktopPlaneInfo@COverlayContext@@V?$buffer_impl@UDesktopPlaneInfo@COverlayContext@@$01$00Vliberal_expansion_policy@detail@@@detail@@@detail@@QEAAXXZ @ 0x1800D85DC (-clear@-$vector_facade@UDesktopPlaneInfo@COverlayContext@@V-$buffer_impl@UDesktopPlaneInfo@COver.c)
 *     ?RemoveUnbackedSurfaces@COverlayContext@@CAXPEAV?$vector_facade@PEAVOverlayPlaneInfo@COverlayContext@@V?$buffer_impl@PEAVOverlayPlaneInfo@COverlayContext@@$0BA@$00Vliberal_expansion_policy@detail@@@detail@@@detail@@@Z @ 0x1800E6020 (-RemoveUnbackedSurfaces@COverlayContext@@CAXPEAV-$vector_facade@PEAVOverlayPlaneInfo@COverlayCon.c)
 *     __security_check_cookie @ 0x1800E7E60 (__security_check_cookie.c)
 *     memset_0 @ 0x1800E8D0E (memset_0.c)
 *     ??$?0V?$vector_facade@PEAVOverlayPlaneInfo@COverlayContext@@V?$buffer_impl@PEAVOverlayPlaneInfo@COverlayContext@@$0BA@$00Vliberal_expansion_policy@detail@@@detail@@@detail@@X@?$span@PEAVOverlayPlaneInfo@COverlayContext@@$0?0@gsl@@QEAA@AEAV?$vector_facade@PEAVOverlayPlaneInfo@COverlayContext@@V?$buffer_impl@PEAVOverlayPlaneInfo@COverlayContext@@$0BA@$00Vliberal_expansion_policy@detail@@@detail@@@detail@@@Z @ 0x180180FC0 (--$-0V-$vector_facade@PEAVOverlayPlaneInfo@COverlayContext@@V-$buffer_impl@PEAVOverlayPlaneInfo@.c)
 *     ??$insert_unchecked@AEBUDesktopPlaneInfo@COverlayContext@@@?$vector_facade@UDesktopPlaneInfo@COverlayContext@@V?$buffer_impl@UDesktopPlaneInfo@COverlayContext@@$01$00Vliberal_expansion_policy@detail@@@detail@@@detail@@IEAA?AV?$basic_iterator@UDesktopPlaneInfo@COverlayContext@@@1@V?$basic_iterator@$$CBUDesktopPlaneInfo@COverlayContext@@@1@AEBUDesktopPlaneInfo@COverlayContext@@@Z @ 0x1801812F4 (--$insert_unchecked@AEBUDesktopPlaneInfo@COverlayContext@@@-$vector_facade@UDesktopPlaneInfo@COv.c)
 *     ??A?$span@PEAVOverlayPlaneInfo@COverlayContext@@$0?0@gsl@@QEBAAEAPEAVOverlayPlaneInfo@COverlayContext@@_J@Z @ 0x1801822A0 (--A-$span@PEAVOverlayPlaneInfo@COverlayContext@@$0-0@gsl@@QEBAAEAPEAVOverlayPlaneInfo@COverlayCo.c)
 *     ?CheckPaddingCorrectness@COverlayContext@@CA_NAEBV?$span@PEAVOverlayPlaneInfo@COverlayContext@@$0?0@gsl@@AEBVOverlayPlaneInfo@1@@Z @ 0x180183C68 (-CheckPaddingCorrectness@COverlayContext@@CA_NAEBV-$span@PEAVOverlayPlaneInfo@COverlayContext@@$.c)
 *     ?DeriveDesktopPlaneAttributes@COverlayContext@@AEAAJAEBV?$span@PEAVOverlayPlaneInfo@COverlayContext@@$0?0@gsl@@_N11PEAIPEAUDXGI_MULTIPLANE_OVERLAY_ATTRIBUTES@@@Z @ 0x1801840C8 (-DeriveDesktopPlaneAttributes@COverlayContext@@AEAAJAEBV-$span@PEAVOverlayPlaneInfo@COverlayCont.c)
 *     ?FindLowestPriorityOverlayCandidateIndex@COverlayContext@@CAHAEBV?$span@PEAVOverlayPlaneInfo@COverlayContext@@$0?0@gsl@@@Z @ 0x180184A54 (-FindLowestPriorityOverlayCandidateIndex@COverlayContext@@CAHAEBV-$span@PEAVOverlayPlaneInfo@COv.c)
 *     ?GetContextCandidates@COverlayContext@@CAXPEAV1@AEBV?$span@PEAVOverlayPlaneInfo@COverlayContext@@$0?0@gsl@@PEAV?$vector_facade@PEAVOverlayPlaneInfo@COverlayContext@@V?$buffer_impl@PEAVOverlayPlaneInfo@COverlayContext@@$0BA@$00Vliberal_expansion_policy@detail@@@detail@@@detail@@@Z @ 0x180184BA4 (-GetContextCandidates@COverlayContext@@CAXPEAV1@AEBV-$span@PEAVOverlayPlaneInfo@COverlayContext@.c)
 *     ?IsHigherProprity@COverlayContext@@CA_NAEBVOverlayPlaneInfo@1@0@Z @ 0x1801853F4 (-IsHigherProprity@COverlayContext@@CA_NAEBVOverlayPlaneInfo@1@0@Z.c)
 *     ?RectContainsRect@COverlayContext@@CA_NAEBUtagRECT@@0@Z @ 0x180185C90 (-RectContainsRect@COverlayContext@@CA_NAEBUtagRECT@@0@Z.c)
 *     ?RequiresFrontPlane@OverlayPlaneInfo@COverlayContext@@QEBA_NXZ @ 0x180185D78 (-RequiresFrontPlane@OverlayPlaneInfo@COverlayContext@@QEBA_NXZ.c)
 *     ?clear_region@?$vector_facade@PEAVOverlayPlaneInfo@COverlayContext@@V?$buffer_impl@PEAVOverlayPlaneInfo@COverlayContext@@$0BA@$00Vliberal_expansion_policy@detail@@@detail@@@detail@@IEAAX_K0@Z @ 0x180186178 (-clear_region@-$vector_facade@PEAVOverlayPlaneInfo@COverlayContext@@V-$buffer_impl@PEAVOverlayPl.c)
 *     ?erase@?$vector_facade@PEAVCOverlayContext@@V?$buffer_impl@PEAVCOverlayContext@@$0BA@$00Vliberal_expansion_policy@detail@@@detail@@@detail@@QEAA?AV?$basic_iterator@PEAVCOverlayContext@@@2@V?$basic_iterator@QEAVCOverlayContext@@@2@@Z @ 0x180186678 (-erase@-$vector_facade@PEAVCOverlayContext@@V-$buffer_impl@PEAVCOverlayContext@@$0BA@$00Vliberal.c)
 *     ?reserve_region@?$vector_facade@PEAVOverlayPlaneInfo@COverlayContext@@V?$buffer_impl@PEAVOverlayPlaneInfo@COverlayContext@@$0BA@$00Vliberal_expansion_policy@detail@@@detail@@@detail@@IEAAPEAPEAVOverlayPlaneInfo@COverlayContext@@_K0@Z @ 0x180186844 (-reserve_region@-$vector_facade@PEAVOverlayPlaneInfo@COverlayContext@@V-$buffer_impl@PEAVOverlay.c)
 */

__int64 __fastcall COverlayContext::DeriveCheckCandidatesList(__int64 *a1, _QWORD *a2, char *a3)
{
  __int64 v3; // r12
  _QWORD *v4; // rdi
  __int64 *v5; // r14
  __int64 *v6; // r15
  unsigned __int64 v7; // r12
  char v8; // r11
  unsigned __int64 v9; // r9
  unsigned __int64 v10; // r12
  __int64 v11; // r13
  __int64 v12; // r15
  __int64 v13; // r11
  __m128i **v14; // r9
  __int64 v15; // r8
  __m128i *v16; // rdx
  __int32 *v17; // r10
  _OWORD *v18; // rcx
  __int64 v19; // r8
  char **v20; // rbx
  char **v21; // rcx
  __int64 *v22; // r15
  char v23; // r14
  char *v24; // r13
  _QWORD *v25; // r12
  unsigned int v27; // r12d
  unsigned __int64 v28; // rdi
  __int64 v29; // rbx
  unsigned int v30; // r14d
  int v31; // edi
  unsigned int v32; // r15d
  const struct COverlayContext::OverlayPlaneInfo *v33; // rbx
  const struct COverlayContext::OverlayPlaneInfo **v34; // rax
  __int64 v35; // r14
  __m128i **v36; // rcx
  __m128i *v37; // rdi
  __int32 *v38; // rdi
  char v40; // r13
  __int64 v41; // rcx
  bool v42; // al
  bool v43; // r12
  bool v44; // bl
  __int64 v45; // rbx
  __int64 v46; // r15
  __int64 v47; // rcx
  __int64 v48; // rdx
  __int64 v49; // rax
  unsigned int v50; // edx
  int v51; // eax
  __int64 v52; // r9
  __int64 v53; // rcx
  __int64 v54; // rcx
  __int64 v55; // r10
  __int64 v56; // r8
  __int64 v57; // rbx
  __int64 *v58; // r8
  __int64 v59; // r10
  signed int v60; // eax
  __int64 v61; // rcx
  __int64 v62; // r9
  __int64 *v63; // rdi
  __int64 v64; // r15
  __int128 v65; // xmm0
  __int128 v66; // xmm1
  __int128 v67; // xmm0
  __int64 v68; // rax
  __int128 v69; // xmm1
  __int128 v70; // xmm0
  __int128 v71; // xmm1
  __int128 v72; // xmm0
  _BYTE *v73; // rcx
  __int64 v74; // r8
  __int64 v75; // r9
  _BYTE *v76; // rcx
  __int64 v77; // r8
  _DWORD v78[3]; // [rsp+44h] [rbp-BCh] BYREF
  unsigned int v79; // [rsp+50h] [rbp-B0h]
  _QWORD *v80; // [rsp+58h] [rbp-A8h]
  __int64 *v81; // [rsp+60h] [rbp-A0h] BYREF
  _QWORD v82[2]; // [rsp+68h] [rbp-98h] BYREF
  char v83[8]; // [rsp+78h] [rbp-88h] BYREF
  _BYTE v84[16]; // [rsp+80h] [rbp-80h] BYREF
  __int64 v85; // [rsp+90h] [rbp-70h] BYREF
  __int64 v86; // [rsp+98h] [rbp-68h]
  __m128i v87; // [rsp+A0h] [rbp-60h]
  char *v88; // [rsp+D0h] [rbp-30h]
  _QWORD v89[3]; // [rsp+D8h] [rbp-28h] BYREF
  __int128 v90; // [rsp+F0h] [rbp-10h] BYREF
  _OWORD v91[6]; // [rsp+100h] [rbp+0h] BYREF
  char v92; // [rsp+160h] [rbp+60h]
  char v93; // [rsp+188h] [rbp+88h] BYREF
  _BYTE *v94; // [rsp+190h] [rbp+90h] BYREF
  _BYTE *v95; // [rsp+198h] [rbp+98h]
  char *v96; // [rsp+1A0h] [rbp+A0h]
  _BYTE v97[128]; // [rsp+1A8h] [rbp+A8h] BYREF
  char v98; // [rsp+228h] [rbp+128h] BYREF
  __int64 v99[28]; // [rsp+230h] [rbp+130h] BYREF

  v3 = *a1;
  v4 = a2;
  v5 = (__int64 *)a1[1];
  v6 = a1;
  v88 = a3;
  v7 = v3;
  v8 = 0;
  v80 = a2;
  v81 = a1;
  v79 = 0;
  v9 = (unsigned __int64)&v5[v7];
  if ( *a3 )
  {
    v58 = v5;
    if ( v5 == (__int64 *)v9 )
    {
LABEL_85:
      if ( ((a2[1] - *a2) & 0xFFFFFFFFFFFFFFF8uLL) != 0 )
      {
        gsl::span<COverlayContext::OverlayPlaneInfo *,-1>::span<COverlayContext::OverlayPlaneInfo *,-1>(v84, a2);
        *(_QWORD *)&v78[1] = *v4 + 8LL * (int)COverlayContext::FindLowestPriorityOverlayCandidateIndex(v84);
        detail::vector_facade<COverlayContext *,detail::buffer_impl<COverlayContext *,16,1,detail::liberal_expansion_policy>>::erase(
          v4,
          &v81,
          &v78[1]);
        COverlayContext::RemoveUnbackedSurfaces(v4);
        v8 = 0;
      }
      else
      {
        while ( v5 != (__int64 *)v9 )
        {
          v59 = *(_QWORD *)(*v5 + 12552);
          if ( (*(_QWORD *)(*v5 + 12560) - v59) / 120 && *(_BYTE *)(v59 + 112) )
          {
            *(_BYTE *)(*v5 + 13479) = 0;
            break;
          }
          ++v5;
        }
      }
    }
    else
    {
      while ( (unsigned __int64)((*(_QWORD *)(*v58 + 12560) - *(_QWORD *)(*v58 + 12552)) / 120LL) <= 1 )
      {
        if ( ++v58 == (__int64 *)v9 )
          goto LABEL_85;
      }
      *(_BYTE *)(*v58 + 13478) = 0;
    }
  }
  else
  {
    v10 = (v7 * 8) >> 3;
    *(_QWORD *)&v90 = (char *)v91 + 8;
    LOBYTE(v78[0]) = 0;
    if ( (unsigned __int64)v5 > v9 )
      v10 = 0LL;
    *((_QWORD *)&v90 + 1) = (char *)v91 + 8;
    v82[0] = 0LL;
    *(_QWORD *)&v91[0] = &v93;
    v11 = 0LL;
    v82[1] = 0LL;
    *(_QWORD *)&v78[1] = v10;
    if ( v10 )
    {
      do
      {
        v12 = *v5;
        *(_QWORD *)(v12 + 13464) = 0LL;
        *(_QWORD *)(v12 + 13472) = 0LL;
        *(_BYTE *)(v12 + 13479) = *(_BYTE *)(v12 + 13441);
        detail::vector_facade<COverlayContext::DesktopPlaneInfo,detail::buffer_impl<COverlayContext::DesktopPlaneInfo,2,1,detail::liberal_expansion_policy>>::clear((_QWORD *)(v12 + 12552));
        if ( (unsigned int)((__int64)(*(_QWORD *)(v12 + 200) - *(_QWORD *)(v12 + 192)) >> 8) )
        {
          v27 = 0;
          do
          {
            v28 = (unsigned __int64)v27 << 8;
            *(_DWORD *)(v28 + *(_QWORD *)(v12 + 192) + 232) = v27;
            v29 = *(_QWORD *)(v12 + 192);
            ++v27;
            *(_QWORD *)detail::vector_facade<COverlayContext::OverlayPlaneInfo *,detail::buffer_impl<COverlayContext::OverlayPlaneInfo *,16,1,detail::liberal_expansion_policy>>::reserve_region(
                         &v90,
                         (__int64)(*((_QWORD *)&v90 + 1) - v90) >> 3) = v28 + v29;
          }
          while ( v27 < (unsigned int)((__int64)(*(_QWORD *)(v12 + 200) - *(_QWORD *)(v12 + 192)) >> 8) );
          v10 = *(_QWORD *)&v78[1];
        }
        ++v5;
        ++v11;
      }
      while ( v11 != v10 );
    }
    while ( ((*((_QWORD *)&v90 + 1) - (_QWORD)v90) & 0xFFFFFFFFFFFFFFF8uLL) != 0 )
    {
      gsl::span<COverlayContext::OverlayPlaneInfo *,-1>::span<COverlayContext::OverlayPlaneInfo *,-1>(v89, &v90);
      v30 = v89[0];
      v31 = -1;
      if ( v89[0] )
      {
        v31 = 0;
        v32 = 1;
        if ( LODWORD(v89[0]) > 1 )
        {
          do
          {
            v33 = *(const struct COverlayContext::OverlayPlaneInfo **)gsl::span<COverlayContext::OverlayPlaneInfo *,-1>::operator[](
                                                                        v89,
                                                                        v31);
            v34 = (const struct COverlayContext::OverlayPlaneInfo **)gsl::span<COverlayContext::OverlayPlaneInfo *,-1>::operator[](
                                                                       v89,
                                                                       v32);
            if ( COverlayContext::IsHigherProprity(*v34, v33) )
              v31 = v32;
            ++v32;
          }
          while ( v32 < v30 );
        }
      }
      v35 = v31;
      v36 = *(__m128i ***)(v90 + 8LL * v31);
      v37 = *v36;
      v87 = (*v36)[9];
      if ( (_mm_srli_si128(v87, 8).m128i_i32[1] & 0x2000) != 0 )
        v38 = (__int32 *)v82;
      else
        v38 = &v37[841].m128i_i32[2];
      if ( *v38 < (unsigned int)(v87.m128i_i32[0] - 1)
        && (*((_BYTE *)v36 + 184)
          ? (unsigned int)v38[2] < v87.m128i_i32[2]
          : v38[1] < (unsigned int)(v87.m128i_i32[1] - 1)) )
      {
        v40 = COverlayContext::OverlayPlaneInfo::RequiresFrontPlane((COverlayContext::OverlayPlaneInfo *)v36);
        v42 = COverlayContext::RectContainsRect((const struct tagRECT *)(v41 + 60), (const struct tagRECT *)(v41 + 76));
        v43 = v42;
        if ( v40 )
        {
          v44 = v42;
        }
        else if ( !v42 || (v44 = 1, LOBYTE(v78[0])) )
        {
          v45 = v90;
          gsl::span<COverlayContext::OverlayPlaneInfo *,-1>::span<COverlayContext::OverlayPlaneInfo *,-1>(v84, v80);
          v44 = (unsigned __int8)COverlayContext::CheckPaddingCorrectness(v84, *(_QWORD *)(v45 + 8 * v35)) != 0;
        }
        if ( *v81 > 1 )
        {
          v46 = v90;
          gsl::span<COverlayContext::OverlayPlaneInfo *,-1>::span<COverlayContext::OverlayPlaneInfo *,-1>(&v85, v80);
          v47 = v86;
          v48 = v86 + 8 * v85;
          if ( v86 != v48 )
          {
            while ( *(_QWORD *)(*(_QWORD *)(v46 + 8 * v35) + 16LL) != *(_QWORD *)(*(_QWORD *)v47 + 16LL) )
            {
              v47 += 8LL;
              if ( v47 == v48 )
                goto LABEL_58;
            }
            v44 = 0;
          }
        }
LABEL_58:
        if ( v44 )
        {
          v49 = **(_QWORD **)(v90 + 8 * v35);
          if ( *(_BYTE *)(v49 + 13477) )
          {
            if ( v40 != *(_BYTE *)(v49 + 13476) )
              v44 = 0;
          }
          else
          {
            *(_BYTE *)(v49 + 13476) = v40;
          }
          if ( v44 )
          {
            v50 = 0;
            v51 = LOBYTE(v78[0]);
            if ( !v43 )
              v51 = 1;
            v52 = *v80;
            v53 = v80[1] - *v80;
            v78[0] = v51;
            v54 = v53 >> 3;
            if ( (_DWORD)v54 )
            {
              v55 = *(_QWORD *)(v90 + 8 * v35);
              do
              {
                v56 = *(_QWORD *)(v52 + 8LL * v50);
                if ( *(_QWORD *)v55 == *(_QWORD *)v56 && *(_DWORD *)(v55 + 232) < *(_DWORD *)(v56 + 232) )
                  break;
                ++v50;
              }
              while ( v50 != (_DWORD)v54 );
            }
            v57 = v90;
            *(_QWORD *)detail::vector_facade<COverlayContext::OverlayPlaneInfo *,detail::buffer_impl<COverlayContext::OverlayPlaneInfo *,16,1,detail::liberal_expansion_policy>>::reserve_region(
                         v80,
                         (8LL * v50) >> 3) = *(_QWORD *)(v57 + 8 * v35);
            *(_BYTE *)(**(_QWORD **)(v90 + 8 * v35) + 13477LL) = 1;
            ++*v38;
            if ( *(_BYTE *)(*(_QWORD *)(v90 + 8 * v35) + 184LL) )
              ++v38[2];
            else
              ++v38[1];
          }
        }
      }
      *(_QWORD *)&v78[1] = v90 + 8 * v35;
      detail::vector_facade<COverlayContext *,detail::buffer_impl<COverlayContext *,16,1,detail::liberal_expansion_policy>>::erase(
        &v90,
        v83,
        &v78[1]);
    }
    v4 = v80;
    COverlayContext::RemoveUnbackedSurfaces(v80);
    v6 = v81;
    v13 = 0LL;
    v14 = (__m128i **)v81[1];
    v15 = *v81 & 0x1FFFFFFFFFFFFFFFLL;
    if ( v14 > &v14[*v81] )
      v15 = 0LL;
    if ( v15 )
    {
      do
      {
        v16 = *v14;
        v17 = (__int32 *)v82;
        v87 = (*v14)[9];
        if ( (_mm_srli_si128(v87, 8).m128i_i32[1] & 0x2000) == 0 )
          v17 = &v16[841].m128i_i32[2];
        if ( v16[839].m128i_i8[4]
          && v16[9].m128i_i32[0] > 2u
          && v16[9].m128i_i32[1] > 2u
          && *(_DWORD *)(*(_QWORD *)(v16[841].m128i_i64[0] + 64) + 96LL) == 1
          && v16[842].m128i_i8[5]
          && *v17 < (unsigned int)(v87.m128i_i32[0] - 1)
          && v17[1] < (unsigned int)(v87.m128i_i32[1] - 1) )
        {
          v16[842].m128i_i8[6] = 1;
        }
        ++v14;
        ++v13;
      }
      while ( v13 != v15 );
    }
    v18 = (_OWORD *)v90;
    v19 = (__int64)(*((_QWORD *)&v90 + 1) - v90) >> 3;
    if ( v19 )
    {
      detail::vector_facade<COverlayContext::OverlayPlaneInfo *,detail::buffer_impl<COverlayContext::OverlayPlaneInfo *,16,1,detail::liberal_expansion_policy>>::clear_region(
        &v90,
        0LL,
        v19,
        v14);
      v18 = (_OWORD *)v90;
    }
    *(_QWORD *)&v90 = 0LL;
    if ( v18 == (_OWORD *)((char *)v91 + 8) )
      v18 = 0LL;
    operator delete(v18);
    v8 = 0;
  }
  v20 = (char **)v6[1];
  v21 = v20;
  v22 = (__int64 *)&v20[*v6];
  v81 = v22;
  if ( v20 != (char **)v22 )
  {
    while ( !(*v21)[13479] )
    {
      if ( ++v21 == (char **)v22 )
        goto LABEL_23;
    }
    v8 = 1;
  }
LABEL_23:
  if ( ((v4[1] - *v4) & 0xFFFFFFFFFFFFFFF8uLL) != 0 || (v23 = 0, v8) )
    v23 = 1;
  while ( 1 )
  {
    if ( v20 == (char **)v22 )
    {
      *v88 = v23;
      return v79;
    }
    v24 = *v20;
    v25 = *v20 + 12552;
    detail::vector_facade<COverlayContext::DesktopPlaneInfo,detail::buffer_impl<COverlayContext::DesktopPlaneInfo,2,1,detail::liberal_expansion_policy>>::clear(v25);
    if ( v23 )
      break;
LABEL_27:
    ++v20;
  }
  v78[0] = 0;
  memset_0(v99, 0, sizeof(v99));
  v94 = v97;
  v95 = v97;
  v96 = &v98;
  gsl::span<COverlayContext::OverlayPlaneInfo *,-1>::span<COverlayContext::OverlayPlaneInfo *,-1>(v84, v4);
  COverlayContext::GetContextCandidates(v24, v84, &v94);
  gsl::span<COverlayContext::OverlayPlaneInfo *,-1>::span<COverlayContext::OverlayPlaneInfo *,-1>(&v85, &v94);
  v60 = COverlayContext::DeriveDesktopPlaneAttributes((COverlayContext *)v24, v24[13478], (__int64)v78, (__int64)v99);
  v79 = v60;
  if ( v60 >= 0 )
  {
    if ( v78[0] )
    {
      v63 = v99;
      v64 = v78[0];
      do
      {
        v65 = *(_OWORD *)v63;
        v66 = *((_OWORD *)v63 + 1);
        v92 = v24[13479];
        v90 = v65;
        v67 = *((_OWORD *)v63 + 2);
        v68 = v25[1];
        v91[0] = v66;
        v69 = *((_OWORD *)v63 + 3);
        *(_QWORD *)&v78[1] = v68;
        v91[1] = v67;
        v70 = *((_OWORD *)v63 + 4);
        v91[2] = v69;
        v71 = *((_OWORD *)v63 + 5);
        v91[3] = v70;
        v72 = *((_OWORD *)v63 + 6);
        v91[4] = v71;
        v91[5] = v72;
        detail::vector_facade<COverlayContext::DesktopPlaneInfo,detail::buffer_impl<COverlayContext::DesktopPlaneInfo,2,1,detail::liberal_expansion_policy>>::insert_unchecked<COverlayContext::DesktopPlaneInfo const &>(
          v25,
          v83,
          &v78[1],
          &v90);
        v63 += 14;
        --v64;
      }
      while ( v64 );
      v22 = v81;
      v4 = v80;
    }
    v73 = v94;
    v74 = (v95 - v94) >> 3;
    if ( v74 )
    {
      detail::vector_facade<COverlayContext::OverlayPlaneInfo *,detail::buffer_impl<COverlayContext::OverlayPlaneInfo *,16,1,detail::liberal_expansion_policy>>::clear_region(
        &v94,
        0LL,
        v74,
        v62);
      v73 = v94;
    }
    v94 = 0LL;
    if ( v73 == v97 )
      v73 = 0LL;
    operator delete(v73);
    goto LABEL_27;
  }
  MilInstrumentationCheckHR_MaybeFailFast(v61, 0LL, 0, v60, 0x954u, 0LL);
  v76 = v94;
  v77 = (v95 - v94) >> 3;
  if ( v77 )
  {
    detail::vector_facade<COverlayContext::OverlayPlaneInfo *,detail::buffer_impl<COverlayContext::OverlayPlaneInfo *,16,1,detail::liberal_expansion_policy>>::clear_region(
      &v94,
      0LL,
      v77,
      v75);
    v76 = v94;
  }
  v94 = 0LL;
  if ( v76 == v97 )
    v76 = 0LL;
  operator delete(v76);
  return v79;
}
