/*
 * XREFs of ?Initialize@AudioJournal@@AEAAJXZ @ 0x18004FCC0
 * Callers:
...
__int64 __fastcall AudioJournal::Initialize(AudioJournal *this)
{
  ...
  if ( !g_AudioJournal )
  {
    ...
    {
      memset_0(&unk_1801B7D98, 0, 0x4B0uLL);
      dword_1801B8210 = 100;
      GetSystemTimeAsFileTime(&SystemTimeAsFileTime);
      *(struct _FILETIME *)&qword_1801B7D90 = SystemTimeAsFileTime;
      if ( (unsigned int)AudioJournal::ReadVersion(v3) == 1 )
      {
        v5 = **(union _ULARGE_INTEGER **)&AudioJournal::ReadLastLogTime(v4);
        *(union _ULARGE_INTEGER *)&qword_1801B7D90 = v5;
        if ( !v5.QuadPart )
        {
          GetSystemTimeAsFileTime(&SystemTimeAsFileTime);
          *(struct _FILETIME *)&qword_1801B7D90 = SystemTimeAsFileTime;
          AudioJournal::WriteValue((AudioJournal *)&g_AudioJournal, L"LastLogTime", 0xBu, &qword_1801B7D90, 8u);
        }
        if ( AudioJournal::ReadStreamInfo(
               (AudioJournal *)v5.QuadPart,
               (struct AudioJournal::StreamInfo *)&unk_1801B7D98) < 0 )
          AudioJournal::WriteStreamInfo(
            (AudioJournal *)&g_AudioJournal,
            (struct AudioJournal::StreamInfo *)&unk_1801B7D98);
        if ( AudioJournal::ReadStreamInfo(v6, (struct AudioJournal::StreamInfo *)&unk_1801B7FF0) < 0 )
          AudioJournal::WriteStreamInfo(
            (AudioJournal *)&g_AudioJournal,
            (struct AudioJournal::StreamInfo *)&unk_1801B7FF0);
        if ( AudioJournal::ReadStreamInfo(v7, (struct AudioJournal::StreamInfo *)&unk_1801B8180) < 0 )
          AudioJournal::WriteStreamInfo(
            (AudioJournal *)&g_AudioJournal,
            (struct AudioJournal::StreamInfo *)&unk_1801B8180);
      }
      else
      {
