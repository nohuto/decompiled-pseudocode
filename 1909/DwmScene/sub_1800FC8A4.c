/*
 * XREFs of sub_1800FC8A4 @ 0x1800FC8A4
 * Callers:
 *     sub_1800FE640 @ 0x1800FE640 (sub_1800FE640.c)
 *     sub_1800FE9F0 @ 0x1800FE9F0 (sub_1800FE9F0.c)
 * Callees:
 *     sub_18000E4E8 @ 0x18000E4E8 (sub_18000E4E8.c)
 *     sub_18000FC30 @ 0x18000FC30 (sub_18000FC30.c)
 *     sub_1800FC7BC @ 0x1800FC7BC (sub_1800FC7BC.c)
 *     sub_1800FCB8C @ 0x1800FCB8C (sub_1800FCB8C.c)
 *     sub_1800FCC40 @ 0x1800FCC40 (sub_1800FCC40.c)
 *     ??$_Resetp@V__ExceptionPtr@@@?$shared_ptr@V__ExceptionPtr@@@std@@AEAAXPEAV__ExceptionPtr@@@Z_3 @ 0x1800FCCF4 (--$_Resetp@V__ExceptionPtr@@@-$shared_ptr@V__ExceptionPtr@@@std@@AEAAXPEAV__ExceptionPtr@@@Z_3.c)
 *     sub_1800FCF68 @ 0x1800FCF68 (sub_1800FCF68.c)
 *     sub_18011DAEC @ 0x18011DAEC (sub_18011DAEC.c)
 *     _CxxThrowException @ 0x180125B88 (_CxxThrowException.c)
 *     ??2@YAPEAX_K@Z @ 0x180126218 (--2@YAPEAX_K@Z.c)
 *     memset @ 0x1801272D8 (memset.c)
 *     _Mtx_lock @ 0x18012751A (_Mtx_lock.c)
 *     _Mtx_unlock @ 0x180127520 (_Mtx_unlock.c)
 *     ?_Throw_C_error@std@@YAXH@Z @ 0x180127526 (-_Throw_C_error@std@@YAXH@Z.c)
 *     _guard_dispatch_icall_nop @ 0x180127740 (_guard_dispatch_icall_nop.c)
 */

// Hidden C++ exception states: #wind=7
_QWORD *__fastcall sub_1800FC8A4(__int64 a1, _QWORD *a2, __int64 *a3, _QWORD *a4)
{
  void *v8; // rax
  __int64 v9; // r8
  __int64 v10; // rdi
  __int64 v11; // rax
  __int64 v12; // rax
  __int64 v13; // rdi
  __int64 v14; // rcx
  __int64 v15; // rax
  __int64 *v16; // rcx
  __int64 v17; // rdx
  __int64 v18; // rax
  void (__fastcall ***v19)(_QWORD, __int64); // rcx
  __int64 *v20; // rcx
  __int64 v21; // rcx
  __int64 v22; // rcx
  __int64 *v23; // rcx
  __int64 v24; // rdx
  __int64 v25; // rax
  void (__fastcall ***v26)(_QWORD, __int64); // rcx
  struct _Mtx_internal_imp_t *v27; // rbx
  int v28; // eax
  __int64 v29; // rdx
  __int64 v30; // rax
  int v31; // eax
  char v33; // [rsp+30h] [rbp-D0h]
  __int64 v34; // [rsp+48h] [rbp-B8h] BYREF
  __int64 *v35; // [rsp+50h] [rbp-B0h] BYREF
  __int64 *v36; // [rsp+58h] [rbp-A8h] BYREF
  __int128 v37; // [rsp+60h] [rbp-A0h] BYREF
  __int128 v38; // [rsp+70h] [rbp-90h] BYREF
  __int64 v39; // [rsp+80h] [rbp-80h]
  _QWORD *v40; // [rsp+88h] [rbp-78h]
  _QWORD *v41; // [rsp+90h] [rbp-70h]
  char v42[8]; // [rsp+98h] [rbp-68h] BYREF
  _QWORD v43[4]; // [rsp+A0h] [rbp-60h] BYREF
  _BYTE pExceptionObject[64]; // [rsp+C0h] [rbp-40h] BYREF
  char v45[240]; // [rsp+100h] [rbp+0h] BYREF

  v39 = -2LL;
  v40 = a2;
  v41 = a4;
  *a2 = 0LL;
  a2[1] = 0LL;
  v8 = operator new(0xB0uLL);
  v10 = (__int64)v8;
  if ( v8 )
  {
    memset(v8, 0, 0xB0uLL);
    v36 = &v34;
    v11 = *a4;
    *a4 = 0LL;
    v34 = v11;
    v12 = sub_1800FCF68(v45, a3);
    v8 = (void *)sub_1800FC7BC(v10, v12, &v34);
  }
  *a2 = 0LL;
  a2[1] = 0LL;
  LOBYTE(v9) = v33;
  std::shared_ptr<__ExceptionPtr>::_Resetp<__ExceptionPtr>(a2, v8, v9);
  v13 = *a2;
  *(_QWORD *)(v13 + 8) = *(_QWORD *)(***(__int64 (__fastcall ****)(_QWORD, char *))(a1 + 136))(
                                      *(_QWORD *)(a1 + 136),
                                      v42);
  v14 = *a3;
  if ( *a3 )
  {
    v37 = 0LL;
    v15 = a3[1];
    if ( v15 )
    {
      _InterlockedIncrement((volatile signed __int32 *)(v15 + 8));
      v14 = *a3;
      v15 = a3[1];
    }
    *(_QWORD *)&v37 = v14;
    *((_QWORD *)&v37 + 1) = v15;
    v16 = (__int64 *)sub_1800FCB8C(&v35, &v37);
    v17 = *a2;
    v18 = *v16;
    *v16 = 0LL;
    v19 = *(void (__fastcall ****)(_QWORD, __int64))(v17 + 32);
    *(_QWORD *)(v17 + 32) = v18;
    if ( v19 )
      (**v19)(v19, 1LL);
    v20 = v35;
  }
  else
  {
    v21 = a3[2];
    if ( !v21 )
    {
      sub_18000E4E8(v43);
      sub_18011DAEC(pExceptionObject, v43, 0LL);
      throw (Spectre::Utils::SpectreException *)pExceptionObject;
    }
    (*(void (__fastcall **)(__int64))(*(_QWORD *)v21 + 8LL))(v21);
    v38 = 0LL;
    v22 = a3[3];
    if ( v22 )
    {
      _InterlockedIncrement((volatile signed __int32 *)(v22 + 8));
      v22 = a3[3];
    }
    *(_QWORD *)&v38 = a3[2];
    *((_QWORD *)&v38 + 1) = v22;
    v23 = (__int64 *)sub_1800FCC40(&v36, &v38);
    v24 = *a2;
    v25 = *v23;
    *v23 = 0LL;
    v26 = *(void (__fastcall ****)(_QWORD, __int64))(v24 + 32);
    *(_QWORD *)(v24 + 32) = v25;
    if ( v26 )
      (**v26)(v26, 1LL);
    v20 = v36;
  }
  if ( v20 )
    (*(void (__fastcall **)(__int64 *, __int64))*v20)(v20, 1LL);
  v27 = (struct _Mtx_internal_imp_t *)(a1 + 56);
  v28 = Mtx_lock((_Mtx_t)(a1 + 56));
  if ( v28 )
    std::_Throw_C_error(v28);
  v29 = *(_QWORD *)(a1 + 32);
  if ( *(_QWORD *)(a1 + 40) == v29 )
  {
    sub_18000FC30((char **)(a1 + 24), (char *)v29, a2);
  }
  else
  {
    *(_QWORD *)v29 = 0LL;
    *(_QWORD *)(v29 + 8) = 0LL;
    v30 = a2[1];
    if ( v30 )
    {
      _InterlockedIncrement((volatile signed __int32 *)(v30 + 8));
      v27 = (struct _Mtx_internal_imp_t *)(a1 + 56);
    }
    *(_QWORD *)v29 = *a2;
    *(_QWORD *)(v29 + 8) = a2[1];
    *(_QWORD *)(a1 + 32) += 16LL;
  }
  v31 = Mtx_unlock(v27);
  if ( v31 )
    std::_Throw_C_error(v31);
  if ( *a4 )
    (*(void (__fastcall **)(_QWORD, __int64))(*(_QWORD *)*a4 + 8LL))(*a4, 1LL);
  return a2;
}
