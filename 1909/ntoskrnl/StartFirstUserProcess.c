/*
 * XREFs of StartFirstUserProcess @ 0x1409FD234
 * Callers:
 *     Phase1InitializationIoReady @ 0x1409FED48 (Phase1InitializationIoReady.c)
 * Callees:
 *     KeDelayExecutionThread @ 0x14003AEC0 (KeDelayExecutionThread.c)
 *     RtlCopyUnicodeString @ 0x140043EC0 (RtlCopyUnicodeString.c)
 *     InbvIsBootDriverInstalled @ 0x14018B000 (InbvIsBootDriverInstalled.c)
 *     ZwClose @ 0x1401C0E30 (ZwClose.c)
 *     ZwSetInformationProcess @ 0x1401C0FD0 (ZwSetInformationProcess.c)
 *     ZwResumeThread @ 0x1401C1690 (ZwResumeThread.c)
 *     KeBugCheckEx @ 0x1401C46A0 (KeBugCheckEx.c)
 *     memset @ 0x1401D77C0 (memset.c)
 *     FinalizeBootLogo @ 0x140291D6C (FinalizeBootLogo.c)
 *     ExAllocatePoolWithTag @ 0x14036E010 (ExAllocatePoolWithTag.c)
 *     ExFreePoolWithTag @ 0x14036E0A0 (ExFreePoolWithTag.c)
 *     RtlCreateUserProcessEx @ 0x1409FD410 (RtlCreateUserProcessEx.c)
 *     QueryRegistryHideMachine @ 0x1409FD6B4 (QueryRegistryHideMachine.c)
 *     RegistryOverwriteCentralProcessor @ 0x140A386A4 (RegistryOverwriteCentralProcessor.c)
 */

void StartFirstUserProcess()
{
  __int64 MaximumLength; // rsi
  __int64 v1; // rbx
  SIZE_T v2; // r14
  char *PoolWithTag; // rax
  char *v4; // rdi
  __int128 v5; // xmm0
  int v6; // r8d
  int v7; // r9d
  ULONG_PTR UserProcess; // rbx
  NTSTATUS v9; // eax
  NTSTATUS v10; // eax
  UNICODE_STRING DestinationString; // [rsp+30h] [rbp-49h] BYREF
  HANDLE v12[18]; // [rsp+40h] [rbp-39h] BYREF
  int ProcessInformation; // [rsp+E0h] [rbp+67h] BYREF
  LARGE_INTEGER Interval; // [rsp+E8h] [rbp+6Fh] BYREF

  *(_DWORD *)(&DestinationString.MaximumLength + 1) = 0;
  memset(v12, 0, 0x68uLL);
  if ( (unsigned __int8)QueryRegistryHideMachine() )
    RegistryOverwriteCentralProcessor();
  MaximumLength = stru_14058C930.MaximumLength;
  v1 = stru_14058C920.MaximumLength + 1148LL;
  v2 = stru_14058C930.MaximumLength + v1;
  PoolWithTag = (char *)ExAllocatePoolWithTag(NonPagedPoolNx, v2, 0x62537350u);
  v4 = PoolWithTag;
  if ( !PoolWithTag )
    KeBugCheckEx(0x6Du, 0xFFFFFFFFC000009AuLL, 0LL, 0LL, 0LL);
  memset(PoolWithTag, 0, v2);
  *((_DWORD *)v4 + 1) = v1;
  *((_QWORD *)v4 + 16) = &v4[v1];
  *(_DWORD *)v4 = v1;
  *((_DWORD *)v4 + 2) = 4194305;
  *((_QWORD *)v4 + 126) = MaximumLength;
  *((_QWORD *)v4 + 10) = 0LL;
  *((_QWORD *)v4 + 11) = 0LL;
  *((_QWORD *)v4 + 8) = v4 + 1088;
  *((_WORD *)v4 + 29) = stru_14058C920.MaximumLength;
  RtlCopyUnicodeString((PUNICODE_STRING)(v4 + 56), &stru_14058C920);
  *((_QWORD *)v4 + 13) = &v4[*((unsigned __int16 *)v4 + 29) + 1088];
  *((_WORD *)v4 + 49) = 60;
  RtlCopyUnicodeString((PUNICODE_STRING)v4 + 6, &NtInitialUserProcess);
  v5 = *((_OWORD *)v4 + 6);
  DestinationString.Buffer = (wchar_t *)*((_QWORD *)v4 + 16);
  *((_OWORD *)v4 + 7) = v5;
  DestinationString.Length = 0;
  DestinationString.MaximumLength = MaximumLength;
  RtlCopyUnicodeString(&DestinationString, &stru_14058C930);
  UserProcess = (int)RtlCreateUserProcessEx((int)v4 + 96, (_DWORD)v4, v6, v7, (__int64)v12);
  if ( InbvIsBootDriverInstalled() )
    FinalizeBootLogo();
  if ( (UserProcess & 0x80000000) != 0LL )
    KeBugCheckEx(0x6Du, UserProcess, 0LL, 1uLL, 0LL);
  ProcessInformation = 1;
  v9 = ZwSetInformationProcess(v12[1], ProcessBreakOnTermination, &ProcessInformation, 4u);
  if ( v9 < 0 )
    KeBugCheckEx(0x6Du, v9, 0LL, 2uLL, 0LL);
  v10 = ZwResumeThread(v12[2], 0LL);
  if ( v10 < 0 )
    KeBugCheckEx(0x6Du, v10, 0LL, 3uLL, 0LL);
  byte_140467F64 = 1;
  Interval.QuadPart = -50000000LL;
  KeDelayExecutionThread(0, 0, &Interval);
  ZwClose(v12[2]);
  ZwClose(v12[1]);
  ExFreePoolWithTag(v4, 0);
}
