/*
 * XREFs of EtwpReferenceLastBranchLookasideList @ 0x1409068AC
 * Callers:
...
struct _SLIST_ENTRY *EtwpReferenceLastBranchLookasideList()
{
  ...
  v1 = 2 * KeNumberProcessors_0 * _InterlockedIncrement(&dword_14046B580);
  do
  {
    ...
    result = (struct _SLIST_ENTRY *)(unsigned int)_InterlockedIncrement(&dword_14046B584);
  }
  while ( (int)result < v1 );
  return result;
}
