/*
 * XREFs of MiRelocateImage @ 0x140644A3C
 * Callers:
 *     MiCreateNewSection @ 0x14064FBD4 (MiCreateNewSection.c)
 *     MiGetSystemAddressForImage @ 0x140712478 (MiGetSystemAddressForImage.c)
 * Callees:
 *     ExAcquirePushLockExclusiveEx @ 0x14003EAB0 (ExAcquirePushLockExclusiveEx.c)
 *     KiLeaveGuardedRegionUnsafe @ 0x14003EEB0 (KiLeaveGuardedRegionUnsafe.c)
 *     MmGetSessionIdEx @ 0x14003F230 (MmGetSessionIdEx.c)
 *     KeAbPostRelease @ 0x14003F5C0 (KeAbPostRelease.c)
 *     MiChargeCommit @ 0x140052270 (MiChargeCommit.c)
 *     MiWalkEntireImage @ 0x140053AF0 (MiWalkEntireImage.c)
 *     PsSetSystemPagePriorityThread @ 0x14008E57C (PsSetSystemPagePriorityThread.c)
 *     PsRevertToUserPagePriorityThread @ 0x14008E600 (PsRevertToUserPagePriorityThread.c)
 *     MiLegacyImageArchitecture @ 0x14008FEFC (MiLegacyImageArchitecture.c)
 *     MiGetControlAreaPartition @ 0x1400979D0 (MiGetControlAreaPartition.c)
 *     MI_UNLOCK_RELOCATIONS_EXCLUSIVE @ 0x1400979EC (MI_UNLOCK_RELOCATIONS_EXCLUSIVE.c)
 *     MI_LOCK_RELOCATIONS_EXCLUSIVE @ 0x140097BE8 (MI_LOCK_RELOCATIONS_EXCLUSIVE.c)
 *     MI_READ_PTE_LOCK_FREE @ 0x1400A5D60 (MI_READ_PTE_LOCK_FREE.c)
 *     MiOffsetToProtos @ 0x1400B5C30 (MiOffsetToProtos.c)
 *     MiUpdateControlAreaCommitCount @ 0x1400C01D8 (MiUpdateControlAreaCommitCount.c)
 *     ExfTryToWakePushLock @ 0x1400C6390 (ExfTryToWakePushLock.c)
 *     __security_check_cookie @ 0x14019EE20 (__security_check_cookie.c)
 *     _guard_dispatch_icall @ 0x1401CD170 (_guard_dispatch_icall.c)
 *     memmove @ 0x1401D7480 (memmove.c)
 *     memset @ 0x1401D77C0 (memset.c)
 *     MiSetDeleteOnClose @ 0x1402BA41C (MiSetDeleteOnClose.c)
 *     MiGetSubsectionFromPte @ 0x1402D8B08 (MiGetSubsectionFromPte.c)
 *     MiSetLeafPfnBuddy @ 0x1402D9458 (MiSetLeafPfnBuddy.c)
 *     MiMakeUnusedImageExtentsCold @ 0x1402DDDA4 (MiMakeUnusedImageExtentsCold.c)
 *     ExAllocatePoolWithTag @ 0x14036E010 (ExAllocatePoolWithTag.c)
 *     ExFreePoolWithTag @ 0x14036E0A0 (ExFreePoolWithTag.c)
 *     MiDoesPageRequireRetpolineFixups @ 0x140642248 (MiDoesPageRequireRetpolineFixups.c)
 *     MiCompressRelocations @ 0x140643860 (MiCompressRelocations.c)
 *     MiMapImageInSystemSpace @ 0x140643C44 (MiMapImageInSystemSpace.c)
 *     MiParseImageLoadConfig @ 0x140643DC8 (MiParseImageLoadConfig.c)
 *     MiUnmapImageInSystemSpace @ 0x14064437C (MiUnmapImageInSystemSpace.c)
 *     MiScanRelocationPage @ 0x140645250 (MiScanRelocationPage.c)
 *     MiUpdateCfgSystemWideBitmap @ 0x140648F88 (MiUpdateCfgSystemWideBitmap.c)
 *     MiSelectImageBase @ 0x14064904C (MiSelectImageBase.c)
 *     MiLogRelocationFaults @ 0x14066BB84 (MiLogRelocationFaults.c)
 *     MiFreeRelocations @ 0x14072BFC8 (MiFreeRelocations.c)
 *     MiFreeImageLoadConfig @ 0x14072C058 (MiFreeImageLoadConfig.c)
 */

__int64 __fastcall MiRelocateImage(__int64 a1, __int64 a2, unsigned int a3, int a4, __int64 a5, int a6)
{
  unsigned int v10; // edx
  int v11; // r10d
  __int64 v12; // r11
  unsigned int v13; // r12d
  unsigned int v14; // r8d
  __int64 v15; // r9
  __int64 v16; // rdi
  unsigned int v17; // eax
  ULONG_PTR v18; // r13
  char v19; // r10
  unsigned int v20; // eax
  unsigned __int64 v21; // r10
  char v22; // cl
  __int16 v23; // r11
  unsigned int v24; // r8d
  unsigned int v25; // r11d
  __int64 v26; // r15
  __int64 v27; // r8
  bool v28; // zf
  __int64 v29; // rcx
  __int64 v30; // rdi
  SIZE_T v31; // rbx
  char *PoolWithTag; // rax
  char *v33; // rsi
  char *v34; // rcx
  int v35; // r14d
  char *v36; // rdi
  int updated; // edi
  __int64 v38; // rbx
  unsigned __int64 v39; // rcx
  size_t v40; // rdi
  char *v41; // rdi
  __int64 v42; // rcx
  unsigned int v43; // ecx
  unsigned int v44; // r8d
  char v45; // dl
  unsigned int v46; // edx
  unsigned __int64 v47; // rdi
  int v48; // r9d
  int v49; // eax
  int v50; // edi
  __int64 *v51; // r8
  _QWORD *v52; // r12
  struct _KTHREAD *v53; // r12
  __int64 v54; // rdi
  __int64 v55; // rbx
  unsigned int SessionId; // edi
  unsigned int v58; // ecx
  unsigned int v59; // r9d
  unsigned __int64 v60; // rdx
  unsigned __int64 v61; // rdx
  unsigned int v62; // eax
  unsigned __int16 v63; // r9
  _WORD *v64; // rdx
  unsigned int v65; // eax
  _QWORD *v66; // rdi
  __int64 v67; // r15
  unsigned int v68; // r10d
  unsigned int v69; // r9d
  unsigned int v70; // r10d
  unsigned __int64 v71; // rax
  int v72; // edx
  _QWORD *v73; // r8
  unsigned __int64 v74; // r8
  __int64 ControlAreaPartition; // rax
  unsigned int v76; // r11d
  unsigned __int64 v77; // r15
  volatile signed __int64 *v78; // rdi
  char v79; // bl
  __int64 v80; // [rsp+20h] [rbp-148h]
  unsigned int v81; // [rsp+30h] [rbp-138h]
  char v82; // [rsp+30h] [rbp-138h]
  unsigned int v83; // [rsp+30h] [rbp-138h]
  unsigned int v84; // [rsp+34h] [rbp-134h]
  unsigned int v85; // [rsp+34h] [rbp-134h]
  unsigned int v86; // [rsp+38h] [rbp-130h]
  int v88; // [rsp+44h] [rbp-124h]
  unsigned int v89; // [rsp+48h] [rbp-120h]
  __int64 v90; // [rsp+50h] [rbp-118h] BYREF
  struct _KTHREAD *CurrentThread; // [rsp+58h] [rbp-110h]
  __int64 v92; // [rsp+60h] [rbp-108h]
  __int64 v93; // [rsp+68h] [rbp-100h]
  __int64 v94; // [rsp+70h] [rbp-F8h] BYREF
  unsigned int v95; // [rsp+78h] [rbp-F0h]
  size_t Size; // [rsp+80h] [rbp-E8h]
  unsigned __int64 v97; // [rsp+88h] [rbp-E0h]
  __int64 v98; // [rsp+90h] [rbp-D8h]
  ULONG_PTR v99; // [rsp+98h] [rbp-D0h]
  char *v100; // [rsp+A0h] [rbp-C8h]
  __int64 v101; // [rsp+A8h] [rbp-C0h]
  __int64 v102; // [rsp+B0h] [rbp-B8h]
  unsigned __int64 v103[3]; // [rsp+B8h] [rbp-B0h] BYREF
  _QWORD v104[10]; // [rsp+D0h] [rbp-98h] BYREF

  v92 = a2;
  v93 = a1;
  memset(v104, 0, sizeof(v104));
  v90 = 0LL;
  v102 = *(_QWORD *)(a1 + 56);
  if ( MiLegacyImageArchitecture(*(_WORD *)(v102 + 48)) )
  {
    v10 = v11 + 34;
    if ( (*(_BYTE *)(v12 + 44) & 1) != 0 )
      return 0LL;
    v13 = (*(_WORD *)(v12 + 46) & 0x1000) != 0 ? 0xC000007B : 0;
  }
  else
  {
    v13 = -1073741701;
  }
  v14 = v10;
  v15 = *(unsigned int *)(a2 + 88);
  LODWORD(v90) = v15;
  v16 = *(unsigned int *)(a2 + 92);
  v86 = v16;
  HIDWORD(v90) = v16;
  v17 = v10;
  if ( !(_DWORD)v15 || !(_DWORD)v16 )
  {
    if ( (_DWORD)v15 || (_DWORD)v16 )
      return v13;
    v10 &= ~2u;
    v14 = v17 & 0xFFFFFFFD;
  }
  if ( (*(_BYTE *)(a1 + 15) & 1) != 0 )
    return 0LL;
  v18 = *(_QWORD *)a1;
  v99 = v18;
  v101 = *(_QWORD *)(v18 + 96);
  v19 = *(_BYTE *)(a1 + 14) & 0x40;
  if ( (a3 & 0xFFF) > (v19 != 0 ? 4092 : 4088) || ((v19 != 0 ? 3 : 7) & a3) != 0 )
    return v13;
  v20 = a3 | 1;
  if ( !v19 )
    v20 = a3;
  v84 = v20;
  v21 = *(_QWORD *)(a1 + 32);
  v97 = v21;
  v22 = v14;
  if ( v14 >= 0x20 && !a4 )
  {
    v23 = *(_WORD *)(v12 + 46);
    if ( (v23 & 0x40) == 0 && MmRegistryState != -1 && (v23 & 0x1000) == 0 )
      return 0LL;
  }
  v24 = *(_DWORD *)(a1 + 8);
  v25 = v24;
  v89 = v24;
  if ( (v22 & 2) == 0 )
  {
    v25 = (v20 >> 12) + 1;
    v89 = v25;
  }
  v95 = v24 << 12;
  if ( (int)v16 + (int)v15 > v24 << 12 )
    return v13;
  if ( (v22 & 2) == 0 )
    goto LABEL_96;
  if ( (int)v16 + (int)v15 <= (unsigned int)v15 )
    return v13;
  if ( (v22 & 2) == 0 )
  {
LABEL_96:
    v26 = v15;
    v27 = v15;
    goto LABEL_21;
  }
  v26 = v15;
  v27 = v15;
  if ( v15 + v21 <= v21 )
    return v13;
LABEL_21:
  v28 = (v22 & 2) == 0;
  v29 = v16;
  if ( !v28 && v21 + v16 + v27 <= v21 )
    return v13;
  v30 = v25;
  Size = 8LL * v25;
  v31 = v29 + Size + 112;
  v94 = 0LL;
  v81 = v10 & 0xFFFFFFF6;
  v98 = 0LL;
  v88 = 0;
  CurrentThread = KeGetCurrentThread();
  PoolWithTag = (char *)ExAllocatePoolWithTag(PagedPool, v31, 0x65526D4Du);
  v33 = PoolWithTag;
  v100 = PoolWithTag;
  if ( !PoolWithTag )
    return 3221225626LL;
  v34 = PoolWithTag;
  do
    v34 += 4096;
  while ( v34 < &PoolWithTag[v31] );
  *((_QWORD *)PoolWithTag + 5) = 0LL;
  *((_QWORD *)PoolWithTag + 6) = 0LL;
  PoolWithTag[68] = 0;
  *((_QWORD *)PoolWithTag + 1) = 0LL;
  *((_QWORD *)PoolWithTag + 7) = v30;
  *((_QWORD *)PoolWithTag + 2) = 0LL;
  *((_QWORD *)PoolWithTag + 13) = 0LL;
  memset(PoolWithTag + 72, 0, 0x20uLL);
  *((_QWORD *)v33 + 3) = 0LL;
  v35 = v81;
  if ( (v81 & 2) == 0 )
  {
LABEL_29:
    updated = MiSelectImageBase(v93, (_DWORD)v33, a4, a6, (__int64)&v94);
    if ( updated < 0 )
    {
      MiFreeImageLoadConfig(v33 + 72);
      ExFreePoolWithTag(v33, 0);
      if ( !a4 )
        updated = v13;
      goto LABEL_72;
    }
    v38 = v94;
    updated = MiUpdateCfgSystemWideBitmap(v18, v94, v33 + 72);
    if ( updated < 0 )
      goto LABEL_155;
    *((_DWORD *)v33 + 16) = v84;
    v39 = v97;
    *((_QWORD *)v33 + 5) = v38 - v97;
    if ( a5 == -1 )
      *((_QWORD *)v33 + 6) = 0LL;
    else
      *((_QWORD *)v33 + 6) = a5 - v39;
    *(_QWORD *)v33 = v33 + 112;
    v40 = Size;
    memset(v33 + 112, 0, Size);
    v41 = &v33[v40 + 112];
    v92 = (__int64)v41;
    *((_QWORD *)v33 + 4) = v41;
    v35 |= 4u;
    v82 = v35;
    if ( (v35 & 2) == 0 )
      goto LABEL_95;
    memmove(v41, (const void *)(v26 + v98), v86);
    LODWORD(v26) = 0;
    v42 = *((_QWORD *)v33 + 12);
    if ( v42 )
    {
      LODWORD(v26) = v104[2] & 0xFFFFF;
      while ( (_DWORD)v26 )
      {
        v26 = (unsigned int)(v26 - 1);
        if ( *(_QWORD *)(v42 + 8 * v26 + 56) )
          goto LABEL_35;
      }
      LODWORD(v26) = 0;
    }
LABEL_35:
    v43 = HIDWORD(v90);
    v83 = HIDWORD(v90);
    v35 &= ~0x10u;
    v44 = 0;
    if ( HIDWORD(v90) )
    {
      v45 = v35;
      while ( v43 >= 0xA )
      {
        v46 = *((_DWORD *)v41 + 1);
        v85 = v46;
        if ( v46 > v43 || (v46 & 1) != 0 || v46 < 8 )
          goto LABEL_100;
        v47 = *(unsigned int *)v41;
        if ( (v47 & 0xFFF) != 0 )
          v35 &= ~4u;
        if ( (int)v47 + 4096 < (unsigned int)v47 || (int)v47 + 4096 > v95 || (unsigned int)v47 < v44 )
          goto LABEL_100;
        if ( v46 == 8 )
        {
          v43 -= 8;
          v83 = v43;
          v35 &= ~4u;
        }
        else
        {
          if ( (MiFlags & 0x40000) != 0 && (*(_DWORD *)(v18 + 92) & 0xC0000) != 0 )
          {
            if ( (MiOffsetToProtos((_DWORD *)v18, v47, v103)[8] & 4) != 0 )
            {
              updated = -1073741701;
              goto LABEL_71;
            }
            v43 = v83;
            v46 = v85;
          }
          LODWORD(Size) = (unsigned int)v47 >> 12;
          v48 = v92;
          *(_QWORD *)&v33[8 * (v47 >> 12) + 112] |= v92;
          LODWORD(v97) = (v47 & 0xFFFFF000) + 4096;
          v83 = v43 - v46;
          updated = MiScanRelocationPage(v98, v47, (v46 - 8) >> 1, v48 + 8, v18, (__int64)v33);
          if ( updated < 0 )
          {
            if ( updated == -1073741207 )
            {
              updated = 0;
            }
            else if ( updated == -1073741701 )
            {
              updated = v13;
            }
            goto LABEL_71;
          }
          v35 |= 0x10u;
          v49 = Size;
          if ( (unsigned int)Size <= (unsigned int)v26 )
            v49 = v26;
          LODWORD(v26) = v49;
          v43 = v83;
          v44 = v97;
        }
        v45 = v35;
        v41 = (char *)(v85 + v92);
        v92 = (__int64)v41;
        if ( !v43 )
          goto LABEL_53;
      }
      if ( v43 != 8 )
        goto LABEL_100;
      v50 = HIDWORD(v90) - 8;
      LOBYTE(v35) = v45;
    }
    else
    {
LABEL_53:
      v50 = v86;
    }
    MiUnmapImageInSystemSpace((__int64)v104);
    PsRevertToUserPagePriorityThread((__int64)CurrentThread, v88);
    LOBYTE(v35) = v35 & 0xFE;
    v82 = v35;
    v51 = (__int64 *)*((_QWORD *)v33 + 2);
    if ( !v51 )
    {
LABEL_55:
      MiLogRelocationFaults(v18, &v90, v33);
      *((_QWORD *)v33 + 7) = (unsigned int)(v26 + 1);
      if ( (v35 & 0x10) != 0 && (v35 & 4) != 0 )
      {
        v33 = MiCompressRelocations((__int64)v33, v50);
        v52 = v33 + 112;
LABEL_58:
        v52[(unsigned __int64)*((unsigned int *)v33 + 16) >> 12] |= 1uLL;
        if ( (*(_DWORD *)(v18 + 56) & 0x40000000) != 0 )
        {
          v66 = 0LL;
          v67 = *(_QWORD *)(v93 + 64);
          v68 = *((_DWORD *)v33 + 14);
          if ( v89 <= v68 )
            v68 = v89;
          v69 = 0;
          if ( v68 )
          {
            do
            {
              if ( *v52 || MiDoesPageRequireRetpolineFixups((__int64)(v33 + 72), v69) )
              {
                v71 = MI_READ_PTE_LOCK_FREE(v67 + 8LL * v69);
                v103[1] = v71;
                if ( (v71 & 0x400) != 0 )
                {
                  *((_QWORD *)v33 + 13) = 0LL;
                  updated = -1073740023;
                  goto LABEL_71;
                }
                if ( (v71 & 0x800) != 0 )
                {
                  v72 = 1;
                  if ( qword_140465800 )
                  {
                    if ( (v71 & 0x10) != 0 )
                      v71 &= ~0x10uLL;
                    else
                      v71 &= ~qword_140465800;
                  }
                  v73 = (_QWORD *)(48 * ((v71 >> 12) & 0xFFFFFFFFFLL) - 0x58000000000LL);
                  if ( (v73[2] & 0x400LL) != 0 && (*(_BYTE *)(MiGetSubsectionFromPte(v73[2]) + 34) & 2) != 0 )
                    v72 = 0;
                  if ( v72 )
                  {
                    MiSetLeafPfnBuddy(v73, 0LL);
                    if ( *((_QWORD *)v33 + 13) )
                      MiSetLeafPfnBuddy(v66, v74);
                    else
                      *((_QWORD *)v33 + 13) = v74;
                    v66 = (_QWORD *)v74;
                  }
                }
              }
              ++v69;
              ++v52;
            }
            while ( v69 < v70 );
          }
          ControlAreaPartition = MiGetControlAreaPartition(v18);
          v77 = v76;
          if ( !(unsigned int)MiChargeCommit(ControlAreaPartition, v76, 0) )
          {
            *((_QWORD *)v33 + 13) = 0LL;
            updated = -1073741670;
            goto LABEL_71;
          }
          v53 = CurrentThread;
          --CurrentThread->SpecialApcDisable;
          v78 = (volatile signed __int64 *)(v93 + 40);
          ExAcquirePushLockExclusiveEx(v93 + 40, 0LL);
          MiUpdateControlAreaCommitCount(v18, v77);
          v79 = _InterlockedExchangeAdd64(v78, 0xFFFFFFFFFFFFFFFFuLL);
          if ( (v79 & 2) != 0 && (v79 & 4) == 0 )
            ExfTryToWakePushLock(v78);
          KeAbPostRelease((ULONG_PTR)v78);
          KiLeaveGuardedRegionUnsafe((__int64)v53);
          LOBYTE(v35) = v82;
        }
        else
        {
          v53 = CurrentThread;
        }
        LOBYTE(v35) = v35 | 8;
        MI_LOCK_RELOCATIONS_EXCLUSIVE((__int64)v53, (__int64)v33);
        v54 = v101;
        *(_QWORD *)(v101 + 32) = v33;
        *(_QWORD *)(v93 + 32) = v94;
        v55 = v102;
        *(_QWORD *)v102 += *((_QWORD *)v33 + 5);
        *(_DWORD *)(v18 + 92) |= 0x400000u;
        if ( (*(_DWORD *)(v18 + 56) & 0x800) != 0 )
          MiSetDeleteOnClose(v18, 0);
        if ( (MiFlags & 0x4000) == 0
          || (*(_DWORD *)(v18 + 92) & 0xC0000) == 0
          || (qword_140436480
            ? (updated = qword_140436480(*(_QWORD *)(v54 + 40) & 0xFFFFFFFFFFFFFFF8uLL))
            : (updated = -1073741637),
              updated >= 0) )
        {
          if ( a4 )
            SessionId = -1;
          else
            SessionId = MmGetSessionIdEx((__int64)KeGetCurrentThread()->ApcState.Process);
          if ( a6 != 1 || *((_QWORD *)v33 + 12) )
          {
            if ( (*(_DWORD *)(v18 + 56) & 0x40000000) != 0 )
            {
              MiWalkEntireImage(v18, 0LL, 8, SessionId);
              if ( (HvlEnlightenments & 0x400000) != 0 )
                MiMakeUnusedImageExtentsCold(v18);
            }
            updated = MiWalkEntireImage(v18, 0LL, 2, SessionId);
          }
          else
          {
            updated = 0;
          }
        }
        MI_UNLOCK_RELOCATIONS_EXCLUSIVE((__int64)v53, (__int64)v33);
        if ( updated >= 0 && !a6 )
          *(_BYTE *)(v55 + 51) |= 4u;
LABEL_71:
        if ( (v35 & 8) != 0 )
          goto LABEL_72;
LABEL_155:
        MiFreeRelocations(v18, v33);
        goto LABEL_72;
      }
LABEL_95:
      v52 = v33 + 112;
      goto LABEL_58;
    }
    while ( 1 )
    {
      v58 = v26;
      v59 = *((_DWORD *)v51 + 3);
      v60 = *(_QWORD *)&v33[8 * ((unsigned __int64)v59 >> 12) + 112];
      LODWORD(v26) = v59 >> 12;
      if ( v59 >> 12 <= v58 )
        LODWORD(v26) = v58;
      if ( v60 > 1 )
      {
        v61 = v60 & 0xFFFFFFFFFFFFFFFEuLL;
        v62 = *(_DWORD *)(v61 + 4) - 8;
        v63 = v59 & 0xFFF;
        v64 = (_WORD *)(v61 + 8);
        v65 = v62 >> 1;
        if ( v65 )
          break;
      }
LABEL_93:
      v51 = (__int64 *)*v51;
      if ( !v51 )
        goto LABEL_55;
    }
    while ( *v64 < 0x1000u || (unsigned __int16)(*v64 & 0xFFF) >= v63 )
    {
      ++v64;
      if ( !--v65 )
        goto LABEL_93;
    }
LABEL_100:
    updated = v13;
    goto LABEL_71;
  }
  if ( (int)MiMapImageInSystemSpace((__int64 *)v18, 3, (__int64)v104) < 0 )
  {
    ExFreePoolWithTag(v33, 0);
    MiSetDeleteOnClose(v18, 0);
    return 0LL;
  }
  v36 = (char *)v104[0];
  v98 = v104[0];
  v88 = PsSetSystemPagePriorityThread((__int64)CurrentThread, 1);
  v35 = v81 | 1;
  updated = MiParseImageLoadConfig(v18, v36, LODWORD(v104[2]) << 12, v92, v80, (__int64)(v33 + 72));
  if ( updated >= 0 )
    goto LABEL_29;
  ExFreePoolWithTag(v33, 0);
LABEL_72:
  if ( (v35 & 1) != 0 )
  {
    MiUnmapImageInSystemSpace((__int64)v104);
    PsRevertToUserPagePriorityThread((__int64)CurrentThread, v88);
  }
  return (unsigned int)updated;
}
