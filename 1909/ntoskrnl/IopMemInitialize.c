/*
 * XREFs of IopMemInitialize @ 0x140765964
 * Callers:
 *     IopInitializePlugPlayServices @ 0x1409F8468 (IopInitializePlugPlayServices.c)
 * Callees:
 *     ArbAddMmConfigRangeAsBootReserved @ 0x140765B8C (ArbAddMmConfigRangeAsBootReserved.c)
 *     RtlAddRange @ 0x1407666C0 (RtlAddRange.c)
 *     ArbAddInaccessibleAllocationRange @ 0x140766F10 (ArbAddInaccessibleAllocationRange.c)
 *     ArbInitializeArbiterInstance @ 0x1407670FC (ArbInitializeArbiterInstance.c)
 */

int __fastcall IopMemInitialize(__int64 a1, int a2)
{
  int result; // eax

  qword_140460298 = (__int64)&IopGenericUnpackRequirement;
  qword_1404602A0 = (__int64)IopGenericPackResource;
  qword_1404602A8 = (__int64)IopGenericUnpackResource;
  qword_1404602B0 = (__int64)IopGenericScoreRequirement;
  qword_140460318 = (__int64)IopMemFindSuitableRange;
  qword_1404602E8 = (__int64)IopMemQueryConflict;
  result = ArbInitializeArbiterInstance((unsigned int)&IopRootMemArbiter, a2, 3, (unsigned int)L"RootMemory");
  if ( result >= 0 )
  {
    result = RtlAddRange(RangeList, 0LL, 0xFFFuLL, 0, 0, 0LL, 0LL);
    if ( result >= 0 )
    {
      result = ArbAddInaccessibleAllocationRange(&IopRootMemArbiter, RangeList);
      if ( result >= 0 )
        return ArbAddMmConfigRangeAsBootReserved(&IopRootMemArbiter, RangeList);
    }
  }
  return result;
}
