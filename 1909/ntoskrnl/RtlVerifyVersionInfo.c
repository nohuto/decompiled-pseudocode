/*
 * XREFs of RtlVerifyVersionInfo @ 0x14017DF60
 * Callers:
 ...
 * Callees:
 *     RtlpVerGetConditionMask @ 0x14017E200 (RtlpVerGetConditionMask.c)
...
NTSTATUS __stdcall RtlVerifyVersionInfo(PRTL_OSVERSIONINFOEXW VersionInfo, ULONG TypeMask, ULONGLONG ConditionMask)
{
  ...
  if ( !result )
  {
        ...
        do
        {
          ...
          if ( (v12 & (1 << v11)) != 0 )
          {
            ...
            if ( v15 )
            {
              if ( v15 != 1 )
                return -1073741811;
              if ( (VersionInformation[70] & (unsigned __int16)v13) != 0 )
                v6 = 1;
            }
            else if ( (VersionInformation[70] & (unsigned __int16)v13) == 0 )
            {
              return -1073741735;
            }
          }
    ...
    v16 = (unsigned __int8)RtlpVerCompare(v9, VersionInfo->dwMinorVersion, VersionInformation[2], (unsigned int)v20, 1) == 0;
    v8 = v20[0];
    if ( v16 )
    {
      if ( !v20[0] )
...
LABEL_10:
    ...
    if ( (v4 & 8) != 0 )
    {
      ...
      if ( !(unsigned __int8)RtlpVerCompare(v19, VersionInfo->dwPlatformId, VersionInformation[4], (unsigned int)v20, 0) )
        return -1073741735;
    }
    if ( v4 < 0 )
    {
      v17 = (ConditionMask & 0x8000000000000000uLL) == 0LL ? 0 : RtlpVerGetConditionMask(ConditionMask, 128LL);
      if ( !(unsigned __int8)RtlpVerCompare(
                               v17,
