/*
 * XREFs of MmAllocateVirtualMemory @ 0x14067CCE0
 * Callers:
 *     PsDispatchIumService @ 0x140308164 (PsDispatchIumService.c)
 *     NtAllocateVirtualMemoryEx @ 0x14067CC80 (NtAllocateVirtualMemoryEx.c)
 *     WbAllocateMemoryBlock @ 0x1406AFBDC (WbAllocateMemoryBlock.c)
 * Callees:
 *     memset @ 0x1401D77C0 (memset.c)
 *     MiAllocateVirtualMemoryCommon @ 0x14067CE50 (MiAllocateVirtualMemoryCommon.c)
 *     MiCaptureAllocateMapExtendedParameters @ 0x14067D884 (MiCaptureAllocateMapExtendedParameters.c)
 */

__int64 __fastcall MmAllocateVirtualMemory(
        int a1,
        __int64 *a2,
        __int64 *a3,
        int a4,
        int a5,
        volatile void *Address,
        int a7,
        char a8,
        int a9,
        int a10,
        __int64 a11)
{
  __int64 v15; // rcx
  __int64 v16; // rcx
  int MapExtendedParameters; // ecx
  __int64 v19; // [rsp+68h] [rbp-60h]
  __int64 v20; // [rsp+70h] [rbp-58h]
  _BYTE v21[64]; // [rsp+80h] [rbp-48h] BYREF

  memset(v21, 0, sizeof(v21));
  if ( a8 )
  {
    v15 = (__int64)a2;
    if ( (unsigned __int64)a2 >= 0x7FFFFFFF0000LL )
      v15 = 0x7FFFFFFF0000LL;
    *(_QWORD *)v15 = *(_QWORD *)v15;
    v16 = (__int64)a3;
    if ( (unsigned __int64)a3 >= 0x7FFFFFFF0000LL )
      v16 = 0x7FFFFFFF0000LL;
    *(_QWORD *)v16 = *(_QWORD *)v16;
  }
  v19 = *a2;
  v20 = *a3;
  MapExtendedParameters = MiCaptureAllocateMapExtendedParameters(Address, v21);
  if ( MapExtendedParameters >= 0 )
  {
    MapExtendedParameters = MiAllocateVirtualMemoryCommon(a1, a4, a5, (__int64)v21, a8, a9, a10, a11);
    if ( MapExtendedParameters >= 0 )
    {
      *a2 = v19;
      *a3 = v20;
    }
  }
  return (unsigned int)MapExtendedParameters;
}
