/*
 * XREFs of MiTrimAllSystemPagableMemory @ 0x1402C8D50
 * Callers:
 *     MmTrimAllSystemPagableMemory @ 0x1402C9520 (MmTrimAllSystemPagableMemory.c)
 *     MmVerifierTrimMemory @ 0x140963D98 (MmVerifierTrimMemory.c)
 * Callees:
 *     KiLeaveGuardedRegionUnsafe @ 0x14003EEB0 (KiLeaveGuardedRegionUnsafe.c)
 *     MiGetSessionVm @ 0x140075FE4 (MiGetSessionVm.c)
 *     KeAreInterruptsEnabled @ 0x1400A3040 (KeAreInterruptsEnabled.c)
 *     MiPurgePartitionStandby @ 0x14016105C (MiPurgePartitionStandby.c)
 *     MiEmptyTargetedWorkingSet @ 0x1402C8188 (MiEmptyTargetedWorkingSet.c)
 */

__int64 __fastcall MiTrimAllSystemPagableMemory(int a1, int a2)
{
  _DWORD *v2; // r14
  unsigned __int64 SessionVm; // rbx
  _DWORD *v4; // rsi
  __int64 v6; // r12
  unsigned int v7; // r8d
  _DWORD *v8; // rdx
  _DWORD *v9; // rcx
  unsigned int v11; // edi
  struct _KTHREAD *CurrentThread; // rbp
  int v13; // r13d
  __int16 v14; // dx
  __int64 v15; // r8
  __int64 v16; // r9
  __int64 v17; // rcx
  ULONG_PTR *v18; // rcx

  v2 = &unk_140466628;
  SessionVm = 1LL;
  v4 = &unk_140466780;
  v6 = 3LL;
  if ( !a1 )
  {
    v7 = 0;
    v8 = &unk_140466628;
    v9 = &unk_140466780;
    do
    {
      SessionVm = (unsigned __int64)v9;
      if ( v9 && *v8 != v9[1] )
        break;
      ++v7;
      v9 += 80;
      ++v8;
    }
    while ( v7 < 3 );
    if ( v7 == 6 )
      return 0LL;
  }
  if ( KeGetCurrentIrql() > 1u )
    return 0LL;
  v11 = 0;
  CurrentThread = 0LL;
  v13 = 0;
  if ( _InterlockedIncrement(&dword_140466608) <= 1 && KeAreInterruptsEnabled() )
  {
    CurrentThread = KeGetCurrentThread();
    v13 = 1;
    CurrentThread->SpecialApcDisable += v14;
    if ( !a1 )
    {
      do
      {
        SessionVm = (unsigned __int64)v4;
        if ( v4 && *v2 != v4[1] )
        {
          v11 = 1;
          MiEmptyTargetedWorkingSet((__int64)v4);
          *v2 = v4[1];
        }
        v4 += 80;
        ++v2;
        --v6;
      }
      while ( v6 );
      goto LABEL_22;
    }
    if ( a1 == 1 )
    {
      SessionVm = (unsigned __int64)&CurrentThread->ApcState.Process[1].IdealNode[6];
      v17 = SessionVm;
    }
    else
    {
      if ( (CurrentThread->ApcState.Process[1].DirectoryTableBase & 0x1000000000000LL) == 0 )
      {
LABEL_22:
        if ( a2 == 1 && v11 == 1 )
        {
          if ( a1 == 1 )
            v18 = *(ULONG_PTR **)(qword_140465E88 + 8LL * *(unsigned __int16 *)(SessionVm + 174));
          else
            v18 = &MiSystemPartition;
          MiPurgePartitionStandby((__int64)v18, 8u, v15, v16);
        }
        goto LABEL_28;
      }
      SessionVm = MiGetSessionVm();
      v17 = SessionVm;
    }
    MiEmptyTargetedWorkingSet(v17);
    v11 = 1;
    goto LABEL_22;
  }
LABEL_28:
  _InterlockedDecrement(&dword_140466608);
  if ( v13 == 1 )
    KiLeaveGuardedRegionUnsafe((__int64)CurrentThread);
  return v11;
}
