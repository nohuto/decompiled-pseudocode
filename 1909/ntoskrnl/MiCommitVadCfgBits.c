/*
 * XREFs of MiCommitVadCfgBits @ 0x1405EB568
 * Callers:
 *     MiMapViewOfImageSection @ 0x1405EAA40 (MiMapViewOfImageSection.c)
 *     MiAllocateVirtualMemory @ 0x140604C50 (MiAllocateVirtualMemory.c)
 *     MmProtectVirtualMemory @ 0x140605BE0 (MmProtectVirtualMemory.c)
 *     MiReserveUserMemory @ 0x140609670 (MiReserveUserMemory.c)
 *     MiMapViewOfDataSection @ 0x14060AE10 (MiMapViewOfDataSection.c)
 *     MiCfgInitializeProcess @ 0x14067F8C0 (MiCfgInitializeProcess.c)
 *     MiMapViewOfPhysicalSection @ 0x14077D4A4 (MiMapViewOfPhysicalSection.c)
 * Callees:
 *     MiIsProcessCfgEnabled @ 0x14001CC70 (MiIsProcessCfgEnabled.c)
 *     MiMarkProcessCfgBits @ 0x1405EB7BC (MiMarkProcessCfgBits.c)
 *     MiSelectCfgBitMap @ 0x1405EB824 (MiSelectCfgBitMap.c)
 *     MiMarkPrivateOpenCfgBits @ 0x1406D42E8 (MiMarkPrivateOpenCfgBits.c)
 */

__int64 __fastcall MiCommitVadCfgBits(__int64 a1, __int64 a2, char *a3)
{
  __int64 v4; // r14
  _KPROCESS *Process; // rbp
  unsigned __int64 *v6; // r9
  __int64 v7; // r11
  __int64 v8; // r12
  unsigned __int64 v9; // rdi
  unsigned __int64 v10; // rbx
  __int64 **v11; // rax
  __int64 *v12; // r8
  int v13; // r10d
  __int64 v14; // r15
  unsigned __int64 v15; // rax
  __int16 v16; // r9
  bool v17; // al
  __int64 v18; // rsi
  int v19; // edx
  int v20; // r10d
  int v21; // r11d
  __int64 result; // rax
  unsigned __int64 v23; // rcx
  __int16 v24; // cx
  __int64 v25; // rdx
  __int64 v26; // rcx
  char v27; // [rsp+98h] [rbp+20h] BYREF

  v4 = 0LL;
  Process = KeGetCurrentThread()->ApcState.Process;
  if ( !MiIsProcessCfgEnabled() )
    return 0LL;
  v8 = *(unsigned int *)(v7 + 24);
  v9 = (v8 | ((unsigned __int64)*(unsigned __int8 *)(v7 + 32) << 32)) << 12;
  v10 = ((*(unsigned int *)(v7 + 28) | ((unsigned __int64)*(unsigned __int8 *)(v7 + 33) << 32)) << 12) + 4096;
  if ( v6 )
  {
    v9 = *v6;
    v10 = (v6[1] | 0xFFF) + 1;
  }
  else if ( (*(_DWORD *)(v7 + 48) & 0x100000) == 0 )
  {
    v11 = *(__int64 ***)(v7 + 72);
    v12 = *v11;
    v13 = *((_DWORD *)*v11 + 14);
    if ( (v13 & 0x20) != 0 )
    {
      v4 = *v12;
      v14 = *(_QWORD *)(*v12 + 56);
      v15 = Process[1].ActiveProcessors.Bitmap[6];
      if ( !v15 || (v16 = *(_WORD *)(v15 + 8)) == 0 )
        v16 = -31132;
      v17 = *(_WORD *)(v14 + 48) == 0x8664;
      if ( *(_WORD *)(v14 + 48) == v16 )
        v17 = 1;
      v6 = 0LL;
      if ( !v17 )
      {
        v27 = 0;
        a3 = &v27;
      }
    }
  }
  v18 = MiSelectCfgBitMap(Process, v9, v4, v6);
  result = MiMarkProcessCfgBits(v20, v19, v21, v9, v10, v18, (__int64)a3);
  v23 = Process[1].ActiveProcessors.Bitmap[6];
  if ( v23 )
  {
    v24 = *(_WORD *)(v23 + 8);
    if ( (v24 == 332 || v24 == 452) && (int)result >= 0 )
    {
      v25 = *(_QWORD *)&Process[1].IdealNode[14];
      v26 = v25 + 440;
      if ( v18 != v25 + 440 )
        return MiMarkPrivateOpenCfgBits(v26, v9, v10, 0LL);
      v26 = v25 + 472;
      if ( v9 <= 0x100000000LL )
      {
        if ( v10 > 0x100000000LL )
          v10 = 0x100000000LL;
        return MiMarkPrivateOpenCfgBits(v26, v9, v10, 0LL);
      }
    }
  }
  return result;
}
