/*
 * XREFs of DownLevelLanguageNameToLangID @ 0x14018F064
 * Callers:
 *     LdrpGetParentLangId @ 0x14016AA7C (LdrpGetParentLangId.c)
 *     LdrResSearchResource @ 0x1406764C0 (LdrResSearchResource.c)
 *     RtlCultureNameToLCID @ 0x14076B670 (RtlCultureNameToLCID.c)
 *     RtlLoadString @ 0x1408CF720 (RtlLoadString.c)
 *     CmSelectQualifiedInstallLanguage @ 0x140A13ADC (CmSelectQualifiedInstallLanguage.c)
 * Callees:
 *     ?IsNeutralLanguageItem@@YAHPEBUCultureDataType@@@Z @ 0x1401824D8 (-IsNeutralLanguageItem@@YAHPEBUCultureDataType@@@Z.c)
 *     bsearch @ 0x1401A0BC0 (bsearch.c)
 */

__int64 __fastcall DownLevelLanguageNameToLangID(const void *a1, int a2)
{
  char v2; // bl
  _BYTE *v3; // rax
  wchar_t **v4; // rcx

  v2 = a2;
  if ( a1
    && (a2 & 0xFFFFFFFD) == 0
    && (v3 = bsearch(a1, &off_140370EA0, 0x1B3uLL, 8uLL, CompareLangName)) != 0LL
    && ((v4 = &(&off_140371C40)[2 * word_1403892D0[(v3 - (_BYTE *)&off_140370EA0) >> 3]], (v2 & 2) != 0)
     || !IsNeutralLanguageItem((const struct CultureDataType *)v4)) )
  {
    return *((unsigned __int16 *)v4 + 4);
  }
  else
  {
    return 0LL;
  }
}
