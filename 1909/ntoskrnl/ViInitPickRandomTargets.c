/*
 * XREFs of ViInitPickRandomTargets @ 0x140A1E47C
 * Callers:
...
__int64 ViInitPickRandomTargets()
{
  ...
  if ( VfRandomVerifiedDrivers )
  {
    ...
    if ( (unsigned int)(ViExpectedDriversCount + 2) > 0x200 )
      v2 = 512;
    qword_1409C7F68 = (__int64)&VfRandomTargetsBitMap;
    ...
    for ( i = 0; i < VfRandomVerifiedDrivers; ++i )
    {
      ...
      if ( _bittest((const signed __int32 *)qword_1409C7F68, Number) )
      {
        if ( v3 )
        {
          ++v1;
        ...
        else
        {
          ...
          while ( 1 )
          {
            ...
            if ( !v6 )
              v6 = 1;
            if ( !_bittest((const signed __int32 *)qword_1409C7F68, v6) )
              break;
            if ( v6 == Number )
              goto LABEL_20;
          }
          _bittestandset((signed __int32 *)qword_1409C7F68, v6);
          if ( v6 == Number )
            break;
        }
      }
      ...
      {
        _bittestandset((signed __int32 *)qword_1409C7F68, Number);
      }
    }
LABEL_20:
