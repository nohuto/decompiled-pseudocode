/*
 * XREFs of MiCheckPteRelease @ 0x1402DD4B4
 * Callers:
...
__int64 __fastcall MiCheckPteRelease(__int64 a1, ULONG_PTR a2)
{
  ...
  v5 = 2LL * (unsigned int)((v3 - qword_1404666D0) >> 3);
  if ( _bittest64((const signed __int64 *)qword_140466320, v5) )
    KeBugCheckEx(0xDAu, 0x303uLL, v4, a2, 0LL);
  LOBYTE(v6) = 2;
  if ( v5 && *(_DWORD *)(qword_140466320 + 4 * ((v5 - 2) >> 5)) >> ((v5 - 2) & 0x1F) == 2 )
    KeBugCheckEx(0xDAu, 0x304uLL, v4, a2, 0LL);
  for ( i = v5 + 1; _bittest64((const signed __int64 *)qword_140466320, i) == 1; i += 2LL )
    ;
  ...
  for ( j = v5; j < v9; j += 2LL )
  {
    if ( _bittest64((const signed __int64 *)qword_140466320, j) == 1 )
  ...
  {
    v16 = v5 & 0x1F;
    v14 = (volatile signed __int32 *)(qword_140466320 + 4 * (v5 >> 5));
    if ( v5 >= v11 )
      break;
    LOBYTE(v12) = 1;
    if ( v16 + 1 <= 0x20 )
