/*
 * XREFs of AuthzBasepEvaluateSetRelationship @ 0x1400D419C
 * Callers:
 *     AuthzBasepEvaluateExpression @ 0x1400D2D80 (AuthzBasepEvaluateExpression.c)
 * Callees:
 *     AuthzBasepValueInSet @ 0x1400D3B94 (AuthzBasepValueInSet.c)
 *     AuthzBasepGetNextValue @ 0x1400D3CC0 (AuthzBasepGetNextValue.c)
 *     AuthzBasepRestartOperandValueEnumeration @ 0x1400D47E4 (AuthzBasepRestartOperandValueEnumeration.c)
 *     memset @ 0x1401D77C0 (memset.c)
 */

__int64 __fastcall AuthzBasepEvaluateSetRelationship(int a1, __int64 a2, _DWORD *a3)
{
  int v6; // edi
  int v7; // r13d
  int v8; // esi
  __int64 v9; // r12
  int v10; // eax
  unsigned int v12; // r12d
  int v13; // eax
  char v14; // al
  char v15; // al
  unsigned int v16; // r12d
  int NextValue; // eax
  char v18; // al
  int v19; // eax
  unsigned int v20; // r12d
  int v21; // eax
  char v22; // al
  _BYTE v23[40]; // [rsp+20h] [rbp-50h] BYREF
  _BYTE v24[40]; // [rsp+48h] [rbp-28h] BYREF
  int v25; // [rsp+A0h] [rbp+30h]

  v6 = 0;
  memset(v23, 0, sizeof(v23));
  memset(v24, 0, sizeof(v24));
  v7 = 0;
  *a3 = 0;
  v25 = 0;
  if ( !a1 )
  {
    if ( *(_DWORD *)(a2 + 12) != 1 && *(_DWORD *)(a2 + 52) != 1 && *(_DWORD *)(a2 + 4) != *(_DWORD *)(a2 + 44) )
      return (unsigned int)v6;
    while ( 1 )
    {
      v16 = *(_DWORD *)(a2 + 8);
      NextValue = AuthzBasepGetNextValue(a2, (__int64)v23);
      v6 = NextValue;
      if ( NextValue == -2147483622 )
        break;
      if ( NextValue < 0 )
        goto LABEL_11;
      if ( *(_WORD *)(a2 + 40) == 4 )
      {
        v6 = AuthzBasepGetNextValue(a2, (__int64)v24);
        if ( v6 < 0 )
          goto LABEL_11;
        v18 = *(_BYTE *)(a2 + 4);
        *(_DWORD *)&v23[8] = 0;
        v23[4] = v18;
        *(_WORD *)v23 = *(_WORD *)a2;
        *(_QWORD *)&v23[16] = *(_QWORD *)(a2 + 16);
        *(_DWORD *)&v23[24] = *(_DWORD *)(a2 + 8);
        *(_QWORD *)&v23[32] = *(_QWORD *)(a2 + 32) + v16;
      }
      ++v25;
      v6 = AuthzBasepValueInSet((__int128 *)v23, (__int16 *)(a2 + 40), a3, 0);
      if ( v6 < 0 )
      {
LABEL_11:
        *a3 = -1;
        return (unsigned int)v6;
      }
      if ( *a3 != 1 )
        return (unsigned int)v6;
    }
    v19 = *(_DWORD *)(a2 + 12);
    v6 = 0;
    if ( v19 == 1 && !*(_BYTE *)(a2 + 4) )
    {
      if ( *(_DWORD *)(a2 + 52) == 1 )
        goto LABEL_43;
      if ( *(_DWORD *)(a2 + 44) == 1 )
        return (unsigned int)v6;
    }
    if ( *(_DWORD *)(a2 + 52) != 1 )
    {
LABEL_46:
      AuthzBasepRestartOperandValueEnumeration(a2);
      while ( 1 )
      {
        v20 = *(_DWORD *)(a2 + 48);
        v21 = AuthzBasepGetNextValue(a2 + 40, (__int64)v23);
        v6 = v21;
        if ( v21 == -2147483622 )
          break;
        if ( v21 < 0 )
          goto LABEL_11;
        if ( *(_WORD *)a2 == 4 )
        {
          v6 = AuthzBasepGetNextValue(a2 + 40, (__int64)v24);
          if ( v6 < 0 )
            goto LABEL_11;
          v22 = *(_BYTE *)(a2 + 44);
          *(_DWORD *)&v23[8] = 0;
          v23[4] = v22;
          *(_WORD *)v23 = *(_WORD *)(a2 + 40);
          *(_QWORD *)&v23[16] = *(_QWORD *)(a2 + 56);
          *(_DWORD *)&v23[24] = *(_DWORD *)(a2 + 48);
          *(_QWORD *)&v23[32] = *(_QWORD *)(a2 + 72) + v20;
        }
        ++v7;
        v6 = AuthzBasepValueInSet((__int128 *)v23, (__int16 *)a2, a3, 0);
        if ( v6 < 0 )
          goto LABEL_11;
        if ( *a3 != 1 )
          return (unsigned int)v6;
      }
      v6 = 0;
      if ( v25 != v7 )
        *a3 = 0;
      return (unsigned int)v6;
    }
LABEL_43:
    if ( !*(_BYTE *)(a2 + 44) && v19 != 1 && *(_DWORD *)(a2 + 4) == 1 )
      return (unsigned int)v6;
    goto LABEL_46;
  }
  v8 = a1 - 1;
  if ( v8 )
  {
    if ( v8 != 1 )
      return (unsigned int)v6;
    while ( 1 )
    {
      v12 = *(_DWORD *)(a2 + 8);
      v13 = AuthzBasepGetNextValue(a2, (__int64)v23);
      v6 = v13;
      if ( v13 == -2147483622 )
        return 0;
      if ( v13 < 0 )
        goto LABEL_11;
      if ( *(_WORD *)(a2 + 40) == 4 )
      {
        v6 = AuthzBasepGetNextValue(a2, (__int64)v24);
        if ( v6 < 0 )
          goto LABEL_11;
        v14 = *(_BYTE *)(a2 + 4);
        *(_DWORD *)&v23[8] = 0;
        v23[4] = v14;
        *(_WORD *)v23 = *(_WORD *)a2;
        *(_QWORD *)&v23[16] = *(_QWORD *)(a2 + 16);
        *(_DWORD *)&v23[24] = *(_DWORD *)(a2 + 8);
        *(_QWORD *)&v23[32] = *(_QWORD *)(a2 + 32) + v12;
      }
      v6 = AuthzBasepValueInSet((__int128 *)v23, (__int16 *)(a2 + 40), a3, 1);
      if ( v6 < 0 )
        goto LABEL_11;
      if ( *a3 )
        return (unsigned int)v6;
    }
  }
  if ( *(_DWORD *)(a2 + 12) == 1 || *(_DWORD *)(a2 + 52) == 1 || *(_DWORD *)(a2 + 4) >= *(_DWORD *)(a2 + 44) )
  {
    while ( 1 )
    {
      v9 = *(unsigned int *)(a2 + 48);
      v10 = AuthzBasepGetNextValue(a2 + 40, (__int64)v23);
      v6 = v10;
      if ( v10 == -2147483622 )
        return 0;
      if ( v10 < 0 )
        goto LABEL_11;
      if ( *(_WORD *)a2 == 4 )
      {
        v6 = AuthzBasepGetNextValue(a2 + 40, (__int64)v24);
        if ( v6 < 0 )
          goto LABEL_11;
        v15 = *(_BYTE *)(a2 + 44);
        *(_DWORD *)&v23[8] = 0;
        v23[4] = v15;
        *(_WORD *)v23 = *(_WORD *)(a2 + 40);
        *(_QWORD *)&v23[16] = *(_QWORD *)(a2 + 56);
        *(_DWORD *)&v23[24] = *(_DWORD *)(a2 + 48);
        *(_QWORD *)&v23[32] = *(_QWORD *)(a2 + 72) + v9;
      }
      v6 = AuthzBasepValueInSet((__int128 *)v23, (__int16 *)a2, a3, 0);
      if ( v6 < 0 )
        goto LABEL_11;
      if ( *a3 != 1 )
        return (unsigned int)v6;
    }
  }
  return (unsigned int)v6;
}
