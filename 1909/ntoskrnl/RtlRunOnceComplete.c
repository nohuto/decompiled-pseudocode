/*
 * XREFs of RtlRunOnceComplete @ 0x1406EB0A0
 * Callers:
...
NTSTATUS __stdcall RtlRunOnceComplete(PRTL_RUN_ONCE RunOnce, ULONG Flags, PVOID Context)
{
  ...
  Value = RunOnce->Value;
  v7 = (unsigned __int64)Context & 0xFFFFFFFFFFFFFFFCuLL | (v4 >= 2 ? 2 : 0);
  v8 = RunOnce->Value & 3;
  if ( v8 == 1 )
  {
    if ( (v4 & 1) != 0 )
  ...
  if ( (v4 & 1) != 0 )
    return -1073741584;
  v12 = RunOnce->Value;
  if ( v12 == _InterlockedCompareExchange64((volatile signed __int64 *)RunOnce, v7, Value) )
    return 0;
  return -1073741771;
}
