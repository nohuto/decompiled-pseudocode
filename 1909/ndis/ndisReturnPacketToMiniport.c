/*
 * XREFs of ndisReturnPacketToMiniport @ 0x1C00AF434
 * Callers:
 *     NdisReturnPackets @ 0x1C0098C40 (NdisReturnPackets.c)
 *     ndisReturnNetBufferListsToPackets @ 0x1C00AF300 (ndisReturnNetBufferListsToPackets.c)
 * Callees:
 *     ?NDIS_STACK_RESERVED_FROM_PACKET@@YAXPEAU_NDIS_PACKET@@PEAPEAU_NDIS_STACK_RESERVED@@@Z @ 0x1C0032744 (-NDIS_STACK_RESERVED_FROM_PACKET@@YAXPEAU_NDIS_PACKET@@PEAPEAU_NDIS_STACK_RESERVED@@@Z.c)
 *     _guard_dispatch_icall_nop @ 0x1C0041350 (_guard_dispatch_icall_nop.c)
 *     ndisMQueueWorkItem @ 0x1C009B7F8 (ndisMQueueWorkItem.c)
 *     ?NDIS_FREE_XFER_DATA_PACKET@@YAXPEAU_NDIS_PACKET@@@Z @ 0x1C00A2FC4 (-NDIS_FREE_XFER_DATA_PACKET@@YAXPEAU_NDIS_PACKET@@@Z.c)
 */

void __fastcall ndisReturnPacketToMiniport(__int64 a1, PNDIS_PACKET Packet)
{
  char v4; // si
  void (__fastcall *v5)(_QWORD, PNDIS_PACKET); // r8
  int v6; // eax
  int v7; // ecx
  struct _NDIS_STACK_RESERVED *v8; // [rsp+30h] [rbp+8h] BYREF

  v4 = 0;
  NDIS_STACK_RESERVED_FROM_PACKET(Packet, &v8);
  if ( (*(_DWORD *)(a1 + 120) & 0x40000) != 0 )
    goto LABEL_7;
  KeAcquireSpinLockAtDpcLevel((PKSPIN_LOCK)(a1 + 96));
  *(_QWORD *)(a1 + 520) = KeGetCurrentThread();
  if ( !*(_BYTE *)(a1 + 89) )
  {
    *(_BYTE *)(a1 + 89) = 1;
    v4 = 1;
    *(_QWORD *)(a1 + 1864) = KeGetCurrentThread();
  }
  if ( (*(_DWORD *)(a1 + 120) & 0x40000) != 0 || v4 )
  {
LABEL_7:
    v5 = *(void (__fastcall **)(_QWORD, PNDIS_PACKET))(a1 + 2160);
    *(_QWORD *)v8 = 0LL;
    v6 = Packet[-1].Reserved[1];
    v7 = *(_DWORD *)&Packet[-1].ProtocolReserved[4] - 1;
    *(_DWORD *)&Packet[-1].ProtocolReserved[4] = v7;
    if ( (v6 & 1) != 0 && v7 == -1 )
      NDIS_FREE_XFER_DATA_PACKET(Packet);
    else
      v5(*(_QWORD *)(a1 + 2168), Packet);
    if ( (*(_DWORD *)(a1 + 120) & 0x8000) != 0 )
      _InterlockedDecrement((volatile signed __int32 *)(a1 + 1828));
  }
  else
  {
    *(_QWORD *)v8 = *(_QWORD *)(a1 + 152);
    *(_QWORD *)(a1 + 152) = Packet;
    ndisMQueueWorkItem(a1, 2, 0LL);
  }
  if ( (*(_DWORD *)(a1 + 120) & 0x40000) == 0 )
  {
    if ( v4 )
    {
      *(_QWORD *)(a1 + 1864) = 0LL;
      *(_BYTE *)(a1 + 89) = 0;
    }
    *(_QWORD *)(a1 + 520) = 0LL;
    KeReleaseSpinLockFromDpcLevel((PKSPIN_LOCK)(a1 + 96));
  }
}
