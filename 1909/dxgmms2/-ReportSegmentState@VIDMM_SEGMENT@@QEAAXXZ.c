/*
 * XREFs of ?ReportSegmentState@VIDMM_SEGMENT@@QEAAXXZ @ 0x1C00C1470
 * Callers:
 *     ?ReportVidMmStateWorker@VIDMM_GLOBAL@@QEAAXXZ @ 0x1C00AEEB4 (-ReportVidMmStateWorker@VIDMM_GLOBAL@@QEAAXXZ.c)
 * Callees:
 *     McTemplateK0pqx @ 0x1C002647C (McTemplateK0pqx.c)
 *     McTemplateK0pppttxqx @ 0x1C0029560 (McTemplateK0pppttxqx.c)
 *     ?LogLockAllocationBackingStoreToEtw@VIDMM_SEGMENT@@SAXPEAU_VIDMM_GLOBAL_ALLOC@@@Z @ 0x1C006A8A0 (-LogLockAllocationBackingStoreToEtw@VIDMM_SEGMENT@@SAXPEAU_VIDMM_GLOBAL_ALLOC@@@Z.c)
 */

void __fastcall VIDMM_SEGMENT::ReportSegmentState(VIDMM_SEGMENT *this, __int64 a2, _BOOL8 a3)
{
  char *v3; // rbp
  char *v5; // rdi
  char *v6; // r12
  _QWORD **v7; // r12
  _QWORD *v8; // r14
  _QWORD *v9; // rax
  _QWORD **v10; // rax
  _QWORD *v11; // r15
  _QWORD *v12; // rbp
  __int64 **v13; // r13
  __int64 v14; // rsi
  char *v15; // rdi
  char *i; // rbx
  __int64 v17; // [rsp+30h] [rbp-58h]
  __int64 v18; // [rsp+38h] [rbp-50h]
  __int64 v19; // [rsp+48h] [rbp-40h]

  v3 = (char *)this + 136;
  v5 = (char *)*((_QWORD *)this + 17);
  while ( v5 != v3 )
  {
    v6 = v5;
    v5 = *(char **)v5;
    v7 = (_QWORD **)(v6 + 16);
    v8 = *v7;
    if ( *v7 != v7 )
    {
      do
      {
        v9 = v8;
        v8 = (_QWORD *)*v8;
        v10 = (_QWORD **)(v9 + 2);
        v11 = *v10;
        if ( *v10 != v10 )
        {
          v12 = v10;
          do
          {
            v13 = (__int64 **)(v11 - 7);
            v11 = (_QWORD *)*v11;
            v14 = **v13;
            if ( *(VIDMM_SEGMENT **)(v14 + 136) == this )
            {
              a3 = *(_DWORD *)(*(_QWORD *)(v14 + 504) + 12LL) != 0;
              if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
              {
                LODWORD(v19) = *((_DWORD *)this + 4) + 1;
                LODWORD(v18) = 0;
                LODWORD(v17) = *(_DWORD *)(*(_QWORD *)(v14 + 504) + 12LL) != 0;
                McTemplateK0pppttxqx(
                  (unsigned int)v19,
                  (__int64)v13[1],
                  (const GUID *)a3,
                  *(_QWORD *)((*v13)[1] + 8),
                  v13[1][3],
                  v13,
                  v17,
                  v18,
                  *(_QWORD *)(v14 + 16),
                  v19,
                  *(_QWORD *)(v14 + 144));
              }
            }
            if ( *(VIDMM_SEGMENT **)(v14 + 264) == this )
            {
              a3 = *(_DWORD *)(*(_QWORD *)(v14 + 504) + 12LL) != 0;
              if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
              {
                LODWORD(v19) = *((_DWORD *)this + 4) + 1;
                LODWORD(v18) = 1;
                LODWORD(v17) = *(_DWORD *)(*(_QWORD *)(v14 + 504) + 12LL) != 0;
                McTemplateK0pppttxqx(
                  (unsigned int)v19,
                  (__int64)v13[1],
                  (const GUID *)a3,
                  *(_QWORD *)((*v13)[1] + 8),
                  v13[1][3],
                  v13,
                  v17,
                  v18,
                  *(_QWORD *)(v14 + 16),
                  v19,
                  *(_QWORD *)(v14 + 144));
              }
            }
            if ( (*((_DWORD *)this + 20) & 0x1001) != 0 )
              VIDMM_SEGMENT::LogLockAllocationBackingStoreToEtw((struct _VIDMM_GLOBAL_ALLOC *)v14, a2, (const GUID *)a3);
          }
          while ( v11 != v12 );
        }
      }
      while ( v8 != v7 );
      v3 = (char *)this + 136;
    }
  }
  v15 = (char *)this + 176;
  for ( i = (char *)*((_QWORD *)this + 22); i != v15; i = *(char **)i )
  {
    if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
      McTemplateK0pqx(
        (unsigned int)(*(_DWORD *)(*((_QWORD *)i - 31) + 16LL) + 1),
        &EventReportMarkedGlobalAllocation,
        (const GUID *)a3);
  }
}
