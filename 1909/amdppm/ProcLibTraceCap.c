/*
 * XREFs of ProcLibTraceCap @ 0x1C001D020
 * Callers:
...
char __fastcall ProcLibTraceCap(__int64 a1, int a2, int a3, int a4, char a5, PCEVENT_DESCRIPTOR EventDescriptor)
{
  ...
  struct _PROCESSOR_NUMBER v10; // [rsp+38h] [rbp-49h] BYREF
  ...
  v6 = EventDescriptor;
  v10 = 0;
  LOBYTE(ProcessorNumber) = EtwEventEnabled((REGHANDLE)WPP_MAIN_CB.Queue.ListEntry.Flink, EventDescriptor);
  if ( (_BYTE)ProcessorNumber )
  {
    ...
    if ( ProcessorNumber >= 0 )
    {
      ...
      UserData.Size = 2;
      v13 = 1;
      LOBYTE(ProcessorNumber) = EtwWrite((REGHANDLE)WPP_MAIN_CB.Queue.ListEntry.Flink, v6, 0LL, 6u, &UserData);
    }
  }
