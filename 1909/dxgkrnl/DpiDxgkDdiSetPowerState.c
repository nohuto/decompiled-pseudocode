/*
 * XREFs of DpiDxgkDdiSetPowerState @ 0x1C01584DC
 * Callers:
 *     DpiFdoSetAdapterPowerState @ 0x1C0157B84 (DpiFdoSetAdapterPowerState.c)
 *     DpiPdoSetDevicePower @ 0x1C0157F20 (DpiPdoSetDevicePower.c)
 *     DpiFdoHandleSystemPower @ 0x1C0158078 (DpiFdoHandleSystemPower.c)
 * Callees:
 *     _guard_dispatch_icall_nop @ 0x1C00250D0 (_guard_dispatch_icall_nop.c)
 *     McTemplateK0pqtqq @ 0x1C0035958 (McTemplateK0pqtqq.c)
 */

__int64 __fastcall DpiDxgkDdiSetPowerState(__int64 a1, __int64 a2, const GUID *a3, int a4, unsigned int a5)
{
  __int64 v7; // rbx
  __int64 v8; // rdi
  int v9; // eax
  __int64 v10; // rcx
  const GUID *v11; // r8
  __int64 v12; // r14
  _QWORD *v13; // rax
  __int64 v15; // [rsp+20h] [rbp-38h]
  __int64 v16; // [rsp+28h] [rbp-30h]
  __int64 v17; // [rsp+30h] [rbp-28h]
  __int64 v18; // [rsp+38h] [rbp-20h]

  v7 = a4;
  v8 = (unsigned int)a3;
  if ( bTracingEnabled && (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
    McTemplateK0pqtqq(a1, &EventEnterDdiSetPowerState, a3, a2, (_DWORD)a3, a4, a5, 0);
  v9 = (*(__int64 (__fastcall **)(__int64, _QWORD, _QWORD, _QWORD))(a1 + 224))(
         a2,
         (unsigned int)v8,
         (unsigned int)v7,
         a5);
  v12 = v9;
  if ( bTracingEnabled && (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
  {
    LODWORD(v18) = v9;
    LODWORD(v17) = a5;
    LODWORD(v16) = v7;
    LODWORD(v15) = v8;
    McTemplateK0pqtqq(v10, &EventExitDdiSetPowerState, v11, a2, v15, v16, v17, v18);
  }
  v13 = (_QWORD *)WdLogNewEntry5_WdEvent(v10);
  v13[3] = a1;
  v13[4] = v8;
  v13[5] = v7;
  v13[6] = (int)a5;
  v13[7] = v12;
  WdLogEvent5_WdEvent(v13);
  return (unsigned int)v12;
}
