/*
 * XREFs of ?HandleAdapterTeardownEvent@@YAXPEAU_DEVICE_OBJECT@@PEAXPEAU_IO_WORKITEM@@@Z @ 0x1C01ED860
 * Callers:
 *     ?NotifyHardwareContentProtectionTeardown@DXGADAPTER@@QEAAXI@Z @ 0x1C00332CC (-NotifyHardwareContentProtectionTeardown@DXGADAPTER@@QEAAXI@Z.c)
 * Callees:
 *     ?GetGlobal@DXGGLOBAL@@SAPEAV1@XZ @ 0x1C000C510 (-GetGlobal@DXGGLOBAL@@SAPEAV1@XZ.c)
 *     McTemplateK0pqq @ 0x1C0035844 (McTemplateK0pqq.c)
 */

void __fastcall HandleAdapterTeardownEvent(__int64 IoObject, unsigned int *Context, PIO_WORKITEM IoWorkItem)
{
  __int64 v5; // rcx
  _QWORD *v6; // rax
  __int64 v7; // rcx
  const GUID *v8; // r8
  __int64 v9; // rdx
  __int64 v10; // rcx
  __int64 v11; // r8
  __int64 v12; // rax
  unsigned int v13; // [rsp+20h] [rbp-48h]
  unsigned int v14; // [rsp+28h] [rbp-40h]
  int v15; // [rsp+40h] [rbp-28h] BYREF
  __int64 v16; // [rsp+44h] [rbp-24h]
  int v17; // [rsp+4Ch] [rbp-1Ch]
  __int64 v18; // [rsp+50h] [rbp-18h]
  __int64 v19; // [rsp+58h] [rbp-10h]

  _InterlockedIncrement((volatile signed __int32 *)DXGGLOBAL::GetGlobal(IoObject, (__int64)Context) + 276);
  v6 = (_QWORD *)WdLogNewEntry5_WdEvent(v5);
  v6[3] = *(_QWORD *)Context;
  v6[4] = Context[2];
  v6[5] = Context[3];
  WdLogEvent5_WdEvent(v6);
  if ( bTracingEnabled && (Microsoft_Windows_DxgKrnlEnableBits & 0x40) != 0 )
  {
    v14 = Context[3];
    v13 = Context[2];
    McTemplateK0pqq(v7, &HardwareContentProtectionTeardown, v8, *(_QWORD *)Context, v13, v14);
  }
  v16 = 0LL;
  v17 = 0;
  v19 = 0LL;
  v15 = 11;
  v18 = 1LL;
  SMgrGdiCallout(&v15, 0x200000000LL, 0LL, 0LL, 0LL, 0LL);
  if ( (int)ZwUpdateWnfStateData(
              &WNF_DX_HARDWARE_CONTENT_PROTECTION_TILT_NOTIFICATION,
              Context + 4,
              8LL,
              0LL,
              0LL,
              0,
              0) < 0 )
  {
    v12 = WdLogNewEntry5_WdError(v10, v9, v11);
    *(_QWORD *)(v12 + 24) = 2220LL;
    WdLogEvent5_WdError(v12);
  }
  ExFreePoolWithTag(Context, 0);
  if ( IoWorkItem )
    IoFreeWorkItem(IoWorkItem);
}
