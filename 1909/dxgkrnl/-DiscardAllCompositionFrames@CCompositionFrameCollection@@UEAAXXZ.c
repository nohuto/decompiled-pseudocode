/*
 * XREFs of ?DiscardAllCompositionFrames@CCompositionFrameCollection@@UEAAXXZ @ 0x1C001B160
 * Callers:
 *     ??_GCCompositionFrameCollection@@IEAAPEAXI@Z @ 0x1C001B09C (--_GCCompositionFrameCollection@@IEAAPEAXI@Z.c)
 * Callees:
 *     ?Release@CPushLockCriticalSection@@QEAAXXZ @ 0x1C0013400 (-Release@CPushLockCriticalSection@@QEAAXXZ.c)
 *     _guard_dispatch_icall_nop @ 0x1C00250D0 (_guard_dispatch_icall_nop.c)
 *     ?AcquireExclusive@CPushLockCriticalSection@@QEAAXXZ @ 0x1C005A4B8 (-AcquireExclusive@CPushLockCriticalSection@@QEAAXXZ.c)
 */

void __fastcall CCompositionFrameCollection::DiscardAllCompositionFrames(CCompositionFrameCollection *this)
{
  CPushLockCriticalSection *v1; // rsi
  char *v2; // rdi
  char **v3; // rax
  char **v4; // rcx
  char **v5; // rbx
  char **v6; // rdx

  v1 = (CCompositionFrameCollection *)((char *)this + 32);
  v2 = (char *)this + 16;
  CPushLockCriticalSection::AcquireExclusive((CCompositionFrameCollection *)((char *)this + 32));
  v3 = (char **)*((_QWORD *)v2 + 1);
  if ( *v3 != v2 || (v4 = (char **)v3[1], *v4 != (char *)v3) )
LABEL_9:
    __fastfail(3u);
  *((_QWORD *)v2 + 1) = v4;
  *v4 = v2;
  while ( v3 != (char **)v2 )
  {
    v5 = v3 - 2;
    (*((void (__fastcall **)(char **))*(v3 - 2) + 4))(v3 - 2);
    (*((void (__fastcall **)(char **))*v5 + 1))(v5);
    v3 = (char **)*((_QWORD *)v2 + 1);
    if ( *v3 != v2 )
      goto LABEL_9;
    v6 = (char **)v3[1];
    if ( *v6 != (char *)v3 )
      goto LABEL_9;
    *((_QWORD *)v2 + 1) = v6;
    *v6 = v2;
  }
  CPushLockCriticalSection::Release(v1);
}
