/*
 * XREFs of BmlCompareSourceModes @ 0x1C00DBE38
 * Callers:
 *     BmlGetNextBestSourceMode @ 0x1C00DBA64 (BmlGetNextBestSourceMode.c)
 * Callees:
 *     BmlCompareSourceModesWithConstraint @ 0x1C00DBF60 (BmlCompareSourceModesWithConstraint.c)
 *     BmlCompareSourceModesWithMonitors @ 0x1C00DD33C (BmlCompareSourceModesWithMonitors.c)
 *     BmlGetPixelFormatPreference @ 0x1C00DDC20 (BmlGetPixelFormatPreference.c)
 *     BmlCompareModeRegions @ 0x1C014E628 (BmlCompareModeRegions.c)
 *     BmlCompareSourceModesWithContentRes @ 0x1C02C1730 (BmlCompareSourceModesWithContentRes.c)
 */

__int64 __fastcall BmlCompareSourceModes(__int64 a1, __int64 a2, unsigned __int16 a3, __int64 a4, __int64 a5)
{
  unsigned int v9; // edi
  __int64 v10; // rbx
  __int64 result; // rax
  unsigned int v12; // ebx
  int PixelFormatPreference; // r15d
  int v14; // eax
  int v15; // r8d
  unsigned int v16; // eax
  __int64 v17; // rax
  __int64 v18; // rax
  __int64 v19; // rax

  if ( !a1 )
  {
    v17 = WdLogNewEntry5_WdAssertion(0LL, a2);
    WdLogEvent5_WdAssertion(v17);
  }
  if ( !a2 )
  {
    v18 = WdLogNewEntry5_WdAssertion(a1, a2);
    WdLogEvent5_WdAssertion(v18);
  }
  if ( a4 == a5 )
  {
    v19 = WdLogNewEntry5_WdAssertion(a1, a2);
    WdLogEvent5_WdAssertion(v19);
  }
  if ( !a4 )
    return 0xFFFFFFFFLL;
  if ( !a5 )
    return 1LL;
  v9 = 1;
  v10 = 104LL * a3;
  if ( *(_BYTE *)(*(_QWORD *)(v10 + a1 + 16) + 129LL) && (*(_DWORD *)(a1 + 8) & 1) != 0
    || (result = BmlCompareSourceModesWithConstraint(a1, a3, a4, a5), !(_DWORD)result) )
  {
    if ( !*(_BYTE *)(*(_QWORD *)(v10 + a1 + 16) + 129LL)
      || (*(_DWORD *)(v10 + a1 + 116) & 1) == 0
      || (result = BmlCompareSourceModesWithContentRes(a1, a3, a4, a5), !(_DWORD)result) )
    {
      v12 = *(_DWORD *)(a4 + 96);
      PixelFormatPreference = BmlGetPixelFormatPreference(*(unsigned int *)(a5 + 96));
      v14 = BmlGetPixelFormatPreference(v12);
      if ( v14 <= PixelFormatPreference )
      {
        if ( v14 >= PixelFormatPreference )
        {
          v9 = BmlCompareModeRegions(a4 + 76, a5 + 76);
          if ( v9 )
          {
            v16 = BmlCompareSourceModesWithMonitors(a1, a3, v15, a4, a5);
            if ( v16 )
              return v16;
          }
        }
        else
        {
          return (unsigned int)-1;
        }
      }
      return v9;
    }
  }
  return result;
}
