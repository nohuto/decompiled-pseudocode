...
_QWORD *__fastcall std::_Allocate<16,std::_Default_allocate_traits,0>(SIZE_T dwBytes)
{
  ...
  else
  {
    ...
    if ( dwBytes + 39 < dwBytes )
      v2 = -1LL;
    v3 = GetProcessHeap();
    v4 = HeapAlloc(v3, 0, v2);
    v6 = v4;
    if ( !v4 )
    {
      _o__invalid_parameter_noinfo_noreturn(0LL, v5);
      __debugbreak();
    }
