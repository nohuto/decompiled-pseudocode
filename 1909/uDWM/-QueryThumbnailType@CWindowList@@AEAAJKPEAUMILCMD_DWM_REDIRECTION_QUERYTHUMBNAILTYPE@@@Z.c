/*
 * XREFs of ?QueryThumbnailType@CWindowList@@AEAAJKPEAUMILCMD_DWM_REDIRECTION_QUERYTHUMBNAILTYPE@@@Z @ 0x1800917C4
 * Callers:
 ...
 * Callees:
 *     ?GetThumbnailType@CSecondaryWindowRepresentation@@QEBA?AW4THUMBNAIL_TYPE@@XZ @ 0x1800046BC (-GetThumbnailType@CSecondaryWindowRepresentation@@QEBA-AW4THUMBNAIL_TYPE@@XZ.c)
    ...
    if ( v9 >= 0 )
    {
      ...
      if ( v11 >= 0 )
        *((_DWORD *)a3 + 3) = CSecondaryWindowRepresentation::GetThumbnailType((__int64)v13);
        ...
        MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v11, 0x10DCu);
    }
    else
    {
      MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v9, 0x10DBu);
    }
  }
  else
  {
