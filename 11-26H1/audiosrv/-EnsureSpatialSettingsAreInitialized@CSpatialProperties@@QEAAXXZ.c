/*
 * XREFs of ?EnsureSpatialSettingsAreInitialized@CSpatialProperties@@QEAAXXZ @ 0x180140C6C
 * Callers:
...
void __fastcall CSpatialProperties::EnsureSpatialSettingsAreInitialized(LPCRITICAL_SECTION lpCriticalSection)
{
  ...
  const wchar_t *v17; // [rsp+78h] [rbp+27h] BYREF
  ...
  if ( !LOBYTE(lpCriticalSection[2].LockCount) )
  {
    ...
    if ( DebugInfo )
    {
      ...
      {
        v3 = *(const wchar_t **)&DebugInfo[1].Type;
