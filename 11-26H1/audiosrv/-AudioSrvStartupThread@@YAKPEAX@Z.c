/*
 * XREFs of ?AudioSrvStartupThread@@YAKPEAX@Z @ 0x180152700
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall AudioSrvStartupThread(PVOID Parameter)
{
  ...
  if ( *((_DWORD *)Parameter + 2) )
  {
    do
    {
      HeapFree(g_hHeap, 0, *(LPVOID *)(*((_QWORD *)Parameter + 2) + 8 * v4));
      v4 = (unsigned int)(v4 + 1);
    }
    while ( (unsigned int)v4 < *((_DWORD *)Parameter + 2) );
  }
  HeapFree(g_hHeap, 0, *((LPVOID *)Parameter + 2));
  HeapFree(g_hHeap, 0, Parameter);
  return 0LL;
}
