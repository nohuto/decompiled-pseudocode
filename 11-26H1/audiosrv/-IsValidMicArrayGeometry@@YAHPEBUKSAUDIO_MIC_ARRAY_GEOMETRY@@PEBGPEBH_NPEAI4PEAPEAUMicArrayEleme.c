/*
 * XREFs of ?IsValidMicArrayGeometry@@YAHPEBUKSAUDIO_MIC_ARRAY_GEOMETRY@@PEBGPEBH_NPEAI4PEAPEAUMicArrayElementErrorDetails@@@Z @ 0x1800351B4
 * Callers:
...
LABEL_4:
  ...
  if ( a1->usMicArrayType && (unsigned int)a1->usMicArrayType - 1 >= 2 )
    v11 |= 0x10u;
  if ( a1->wHorizontalAngleBegin < -31416 || a1->wHorizontalAngleBegin > 0 )
    v11 |= 0x100u;
  if ( a1->wHorizontalAngleEnd < 0 || a1->wHorizontalAngleEnd > 31416 )
    v11 |= 0x200u;
  wVerticalAngleBegin = a1->wVerticalAngleBegin;
  if ( wVerticalAngleBegin < -15708 || wVerticalAngleBegin > 15708 )
    v11 |= 0x20u;
  if ( a1->wVerticalAngleEnd < -15708 || a1->wVerticalAngleEnd > 15708 )
    v11 |= 0x40u;
  ...
  if ( !(_WORD)usNumberOfMicrophones )
    goto LABEL_58;
  p_wYCoord = &a1->KsMicCoord[0].wYCoord;
  do
  {
    v21 = *(p_wYCoord - 2);
    if ( v21
      ...
      && *(p_wYCoord - 2) != 2
      && *(p_wYCoord - 2) != 3
      && (unsigned int)(unsigned __int16)*(p_wYCoord - 2) - 4 > 1 )
    {
      if ( (v21 & 0xF) == 0xF )
        v15 |= 4u;
      else
    ...
    v22 = p_wYCoord[3];
    if ( v22 < -31416 || v22 > 31416 )
      v15 |= 0x8000u;
    v23 = p_wYCoord[2];
    if ( v23 < -15708 || v23 > 15708 )
      v15 |= 0x4000u;
    if ( (v23 == -15708 || v23 == 15708) && v22 )
      v15 |= 0x100000u;
    if ( v22 > -15708 && v22 < 15708 )
      v16 = 1;
    if ( v22 || v23 )
      v17 = 0;
    if ( *p_wYCoord || p_wYCoord[1] )
      v18 = 0;
    for ( i = v19 + 1; i < (unsigned __int16)usNumberOfMicrophones; ++i )
    {
      if ( *(p_wYCoord - 1) == a1->KsMicCoord[i].wXCoord
        && *p_wYCoord == a1->KsMicCoord[i].wYCoord
        && p_wYCoord[1] == a1->KsMicCoord[i].wZCoord
        && p_wYCoord[3] == a1->KsMicCoord[i].wHorizontalAngle
        && p_wYCoord[2] == a1->KsMicCoord[i].wVerticalAngle )
      {
        v15 |= 0x10000u;
      }
    }
