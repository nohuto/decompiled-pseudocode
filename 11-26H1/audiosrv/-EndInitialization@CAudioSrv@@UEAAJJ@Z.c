/*
 * XREFs of ?EndInitialization@CAudioSrv@@UEAAJJ@Z @ 0x1800D8020
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall CAudioSrv::EndInitialization(CAudioSrv *this, signed int a2)
{
  ...
  if ( !a2 )
  {
    ...
    {
      LastError = ((__int64 (__fastcall *)(struct IMMDeviceEnumerator *, _QWORD))g_DeviceEnumerator->lpVtbl->RegisterEndpointNotificationCallback)(
                    g_DeviceEnumerator,
    ...
    if ( !LastError )
    {
      LastError = StartRpcServer(v5, L"Audiosrv", &unk_18017C840, SecurityDescriptor);
      *((_DWORD *)this + 14) = LastError == 0;
      if ( !LastError )
      {
        LastError = StartRpcServer(v6, L"PlaybackManagerRpc", &unk_180183380, 0LL);
        *((_DWORD *)this + 15) = LastError == 0;
        if ( !LastError )
        {
          LastError = StartRpcServer(v7, L"AudioSrvDiagnosticsRpc", &unk_180183700, 0LL);
          *((_DWORD *)this + 16) = LastError == 0;
          if ( !LastError )
          {
            LastError = StartRpcServer(v8, L"SpatialSoundDataManagerRpc", &unk_180183940, SecurityDescriptor);
