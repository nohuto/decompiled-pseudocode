/*
 * XREFs of ?AcquireReference@CCaptureNotifier@@QEAA?AV?$shared_ptr@VCCaptureReference@@@std@@K_N0@Z @ 0x180076BC4
 * Callers:
 *     ?AcquirePowerReferenceForStream@CPowerReferenceManager@@QEAA?AV?$shared_ptr@VCPowerReference@@@std@@PEAVCVADServer@@@Z @ 0x180011774 (-AcquirePowerReferenceForStream@CPowerReferenceManager@@QEAA-AV-$shared_ptr@VCPowerReference@@@s.c)
 * Callees:
 *     ?_Decref@_Ref_count_base@std@@QEAAXXZ @ 0x180011720 (-_Decref@_Ref_count_base@std@@QEAAXXZ.c)
 *     ?lock@_Mutex_base@std@@QEAAXXZ @ 0x180013110 (-lock@_Mutex_base@std@@QEAAXXZ.c)
 *     ??2@YAPEAX_K@Z @ 0x18001E210 (--2@YAPEAX_K@Z.c)
 *     ?UpdateAudioCaptureStatus@CCaptureNotifier@@AEAAXW4REFERENCE_OPERATION@@K_N1@Z @ 0x180076CEC (-UpdateAudioCaptureStatus@CCaptureNotifier@@AEAAXW4REFERENCE_OPERATION@@K_N1@Z.c)
 */

// Hidden C++ exception states: #wind=3
_QWORD *__fastcall CCaptureNotifier::AcquireReference(
        std::_Mutex_base *this,
        _QWORD *a2,
        unsigned int a3,
        char a4,
        char a5)
{
  _QWORD *v7; // rdi
  volatile signed __int32 *v9; // rax
  volatile signed __int32 *v10; // rbx
  char v11; // r14
  __int64 v12; // r9
  std::_Ref_count_base *v13; // rcx
  const char *v14; // r9
  wil::details::in1diag3 *retaddr; // [rsp+78h] [rbp+0h]

  v7 = a2;
  *a2 = 0LL;
  a2[1] = 0LL;
  v9 = (volatile signed __int32 *)operator new(0x20uLL);
  v10 = v9;
  if ( v9 )
  {
    *((_DWORD *)v9 + 2) = 1;
    *((_DWORD *)v9 + 3) = 1;
    *(_QWORD *)v9 = &std::_Ref_count_obj2<CCaptureReference>::`vftable';
    *((_QWORD *)v9 + 2) = this;
    *((_DWORD *)v9 + 6) = a3;
    *((_BYTE *)v9 + 28) = a4;
    v11 = a5;
    *((_BYTE *)v9 + 29) = a5;
  }
  else
  {
    v10 = 0LL;
    v11 = a5;
  }
  try
  {
    std::_Mutex_base::lock(this);
    LOBYTE(v12) = a4;
    CCaptureNotifier::UpdateAudioCaptureStatus(this, 0LL, a3, v12, v11);
    if ( v10 )
      _InterlockedIncrement(v10 + 2);
    *v7 = v10 + 4;
    v13 = (std::_Ref_count_base *)v7[1];
    v7[1] = v10;
    if ( v13 )
      std::_Ref_count_base::_Decref(v13);
    _Mtx_unlock(this);
    if ( v10 )
      std::_Ref_count_base::_Decref((std::_Ref_count_base *)v10);
  }
  catch ( ... )
  {
    wil::details::in1diag3::Log_CaughtException(
      retaddr,
      (void *)0x2FD,
      (unsigned int)"avcore\\audiocore\\server\\audiosrv\\dll\\powerreference.cpp",
      v14);
    return a2;
  }
  return v7;
}
