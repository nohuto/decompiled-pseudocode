/*
 * XREFs of ProcLibTraceCStateDomainRundown @ 0x14004216C
 * Callers:
...
BOOLEAN __fastcall ProcLibTraceCStateDomainRundown(__int64 a1)
{
  ...
  result = EtwEventEnabled((REGHANDLE)WPP_MAIN_CB.Queue.ListEntry.Flink, &PPM_ETW_CSTATE_DOMAIN_FIRMWARE_RUNDOWN);
  if ( result )
  {
    UserData.Ptr = a1 + 36;
