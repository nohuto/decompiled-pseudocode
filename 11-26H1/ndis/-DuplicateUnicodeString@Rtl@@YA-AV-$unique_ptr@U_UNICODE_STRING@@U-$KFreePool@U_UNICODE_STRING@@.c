/*
 * XREFs of ?DuplicateUnicodeString@Rtl@@YA?AV?$unique_ptr@U_UNICODE_STRING@@U?$KFreePool@U_UNICODE_STRING@@@@@wistd@@AEBU_UNICODE_STRING@@K@Z @ 0x14016ABE0
 * Callers:
...
_QWORD *__fastcall Rtl::DuplicateUnicodeString(_QWORD *a1, const void **a2, unsigned int a3)
{
  ...
  else
  {
    ...
    wistd::unique_ptr<_UNICODE_STRING,KFreePool<_UNICODE_STRING>>::reset((void **)&v8, 0LL);
  }
  return a1;
}
