/*
 * XREFs of ?AddStaticProtocolBinding@BindStack@Ndis@@QEAAJPEB_W@Z @ 0x140160D00
 * Callers:
...
__int64 __fastcall Ndis::BindStack::AddStaticProtocolBinding(Ndis::BindStack *this, const wchar_t *a2)
{
  ...
  if ( P )
  {
    for ( i = 0LL; ; ++i )
    {
      ...
      if ( !(unsigned __int8)Ndis::sortProtocolsByPointer(&this->Protocols._p[i], &P) )
        break;
    }
    if ( !(unsigned __int8)Ndis::sortProtocolsByPointer(&P, &this->Protocols._p[i]) )
      goto LABEL_10;
