/*
 * XREFs of ?ndisSetDevicePowerOnComplete@@YAJPEAU_DEVICE_OBJECT@@PEAU_IRP@@PEAX@Z @ 0x140082880
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall ndisSetDevicePowerOnComplete(struct _DEVICE_OBJECT *a1, struct _IRP *a2, char *a3)
{
  ...
  ndisReferenceMiniportNoCheck((struct _NDIS_MINIPORT_BLOCK *)a3, 0xCu);
  *((_QWORD *)a3 + 677) = a2;
  ExQueueWorkItem((PWORK_QUEUE_ITEM)(a3 + 5384), (WORK_QUEUE_TYPE)40);
  if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
  {
    LOBYTE(v6) = 4;
    WPP_RECORDER_SF_q(
