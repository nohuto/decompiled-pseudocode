/*
 * XREFs of ndisGetProcessorInfo @ 0x14018F81C
 * Callers:
...
__int64 ndisGetProcessorInfo()
{
  ...
  Pool2 = (_DWORD *)ExAllocatePool2(64LL, 20 * (ndisMaxNumberOfProcessors + 2LL + 2LL * ndisMaxNumRssCpus), 538985550LL);
...
LABEL_7:
  ...
  ndisRssProcessors = (void *)ExAllocatePool2(64LL, 4LL * ndisMaxNumRssCpus, 538985550LL);
    ...
    if ( LogicalProcessorInformation >= 0 )
    {
      ...
      while ( v11 < ndisNumberOfActiveProcessorsAtBoot && (unsigned int)v12 < ndisMaxNumRssCpus )
      {
        ...
        if ( (_DWORD)v12 )
        {
          while ( 1 )
          {
            ProcessorIndexFromNumber = KeGetProcessorIndexFromNumber((PPROCESSOR_NUMBER)ndisRssProcessors + v13);
...
LABEL_25:
    ...
    else
    {
      v9 = ndisRssBaseCpu;
      for ( i = 0LL;
            v9 < ndisNumberOfActiveProcessorsAtBoot && (unsigned int)i < ndisMaxNumRssCpus;
            i = (unsigned int)(i + 1) )
      {
        KeGetProcessorNumberFromIndex(v9++, (PPROCESSOR_NUMBER)ndisRssProcessors + i);
      }
