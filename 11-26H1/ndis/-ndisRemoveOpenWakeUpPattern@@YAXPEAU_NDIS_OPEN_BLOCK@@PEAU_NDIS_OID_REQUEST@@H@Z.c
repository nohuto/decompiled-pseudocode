/*
 * XREFs of ?ndisRemoveOpenWakeUpPattern@@YAXPEAU_NDIS_OPEN_BLOCK@@PEAU_NDIS_OID_REQUEST@@H@Z @ 0x1400B8414
 * Callers:
 *     ?ndisPostSetRemoveWakeUpPattern@@YAXPEAU_NDIS_REQ_TRACKER@@@Z @ 0x1400B7700 (-ndisPostSetRemoveWakeUpPattern@@YAXPEAU_NDIS_REQ_TRACKER@@@Z.c)
 * Callees:
  ...
  if ( !a3 )
  {
    ...
    while ( Next )
    {
      ...
      if ( InformationBuffer[4] == LODWORD(Next[8].Next) && InformationBuffer[2] == LODWORD(Next[7].Next) )
      {
        ...
        v13 = LODWORD(Next[7].Next) + 24LL;
        v17 = v12;
        HIDWORD(Next[7].Next) = 0;
        if ( !memcmp(v10, &Next[6], v13)
