/*
 * XREFs of ?ndisPostRemoveMiniportWakeUpPattern@@YAXPEAU_NDIS_MINIPORT_BLOCK@@PEAU_NDIS_OID_REQUEST@@H@Z @ 0x1400B72DC
 * Callers:
  ...
  if ( !a3 )
  {
    ...
    while ( Next )
    {
      ...
      if ( InformationBuffer[4] == LODWORD(Next[8].Next) && InformationBuffer[2] == LODWORD(Next[7].Next) )
      {
        ...
        v14 = LODWORD(Next[7].Next) + 24LL;
        v21 = v13;
        HIDWORD(Next[7].Next) = 0;
        if ( !memcmp(v12, &Next[6], v14)
