/*
 * XREFs of ?CreateDeviceDriverSecurityDescriptor@@YAJPEAXEEPEAU_ACL@@@Z @ 0x14018FEC4
 * Callers:
...
NTSTATUS __fastcall CreateDeviceDriverSecurityDescriptor(void *a1, __int64 a2, char a3, struct _ACL *a4)
{
  NTSTATUS result; // eax
  __int64 v7; // rdx
    ...
    if ( DaclSecurityDescriptor >= 0 && DaclPresent )
    {
      ...
      if ( DaclSecurityDescriptor >= 0 )
      {
        ...
        if ( SaclPresent )
        {
          ...
          Pool2 = (void *)ExAllocatePool2(256LL, AbsoluteSecurityDescriptorSize, 1685275726LL);
          if ( Pool2 )
          {
            AclSize = v9->AclSize;
            DaclSize = v9->AclSize;
            v13 = (ACL *)ExAllocatePool2(256LL, AclSize, 1633961038LL);
            if ( v13 )
            {
