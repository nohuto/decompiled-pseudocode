/*
 * XREFs of ?ndisDevicePnPEventNotifyMiniport@@YAXPEAU_NDIS_MINIPORT_BLOCK@@W4_NDIS_DEVICE_PNP_EVENT@@PEAXK@Z @ 0x14007C2A0
 * Callers:
        ...
        unsigned int a4)
{
  ...
  _QWORD Parameter[4]; // [rsp+68h] [rbp-41h] BYREF
  struct _NET_DEVICE_PNP_EVENT v29; // [rsp+88h] [rbp-21h] BYREF
  ...
  *((_DWORD *)&v29.DevicePnPEvent + 1) = 0;
  ...
  v29.DevicePnPEvent = a2;
  v29.InformationBuffer = a3;
  v29.InformationBufferLength = v4;
...
LABEL_11:
  ...
  else
  {
    ...
    if ( PnPEventNotifyHandler )
      PnPEventNotifyHandler(a1->MiniportAdapterContext, a2, a3, v4);
  }
  if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
  {
    LODWORD(v24) = a2;
