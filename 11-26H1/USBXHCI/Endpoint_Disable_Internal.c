/*
 * XREFs of Endpoint_Disable_Internal @ 0x14001EB24
 * Callers:
 *     Endpoint_Disable @ 0x14001DD5C (Endpoint_Disable.c)
 *     Endpoint_Enable @ 0x14001F2E0 (Endpoint_Enable.c)
 * Callees:
 *     Endpoint_IsCandidateForSplitTransactionHSIIWorkaround @ 0x14001F068 (Endpoint_IsCandidateForSplitTransactionHSIIWorkaround.c)
 *     WPP_RECORDER_SF_ddql @ 0x1400340CC (WPP_RECORDER_SF_ddql.c)
 *     TR_Disable_Internal @ 0x14003D720 (TR_Disable_Internal.c)
 *     Controller_ClearHSIIWorkaround @ 0x140040AA0 (Controller_ClearHSIIWorkaround.c)
 *     XilEndpoint_FreeStreamContextArray @ 0x140047A64 (XilEndpoint_FreeStreamContextArray.c)
 */

void __fastcall Endpoint_Disable_Internal(__int64 *a1, char a2, int a3)
{
  char v3; // si
  int v5; // eax
  __int64 v6; // rdx
  _DWORD *v7; // rdi
  __int64 v8; // rdx
  __int64 v9; // rdx
  __int64 v10; // rax
  __int64 v11; // rcx
  unsigned int i; // ebp

  v3 = a2;
  if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
    WPP_RECORDER_SF_ddql(*(_QWORD *)(*a1 + 72), a2, a3, *(unsigned __int8 *)(a1[2] + 143));
  v5 = *((_DWORD *)a1 + 32);
  if ( v5 == 3 || v5 == 7 )
  {
    v9 = 120LL * (unsigned int)(*(_DWORD *)(a1[2] + 44) - 1);
    v10 = *(_QWORD *)(*(_QWORD *)(*a1 + 152) + 48LL);
    if ( *(_DWORD *)(v9 + v10 + 108) )
      _InterlockedDecrement((volatile signed __int32 *)(v9 + v10 + 108));
  }
  if ( (unsigned __int8)Endpoint_IsCandidateForSplitTransactionHSIIWorkaround(a1) )
  {
    if ( *((_BYTE *)a1 + 38) )
    {
      v11 = *a1;
      *((_BYTE *)a1 + 38) = 0;
      if ( _InterlockedExchangeAdd((volatile signed __int32 *)(v11 + 1056), 0xFFFFFFFF) == 1 )
        Controller_ClearHSIIWorkaround();
    }
  }
  if ( !*((_BYTE *)a1 + 36) && !*((_BYTE *)a1 + 1352) || v3 )
    v3 = 1;
  if ( *((_BYTE *)a1 + 37) )
  {
    v7 = (_DWORD *)a1[18];
    if ( v7 )
    {
      for ( i = 1; i <= v7[2]; ++i )
      {
        LOBYTE(v6) = v3;
        TR_Disable_Internal(*(_QWORD *)(104LL * (i - 1) + a1[18] + 48), v6);
      }
    }
  }
  else
  {
    v7 = 0LL;
    LOBYTE(v6) = v3;
    TR_Disable_Internal(a1[11], v6);
  }
  if ( v3 )
  {
    if ( v7 )
    {
      XilEndpoint_FreeStreamContextArray(v7);
      if ( v7 == (_DWORD *)a1[17] )
      {
        ExFreePoolWithTag(v7, 0x49434858u);
        a1[17] = 0LL;
      }
      a1[18] = 0LL;
    }
    v8 = a1[10];
    if ( v8 )
    {
      if ( v8 != *(_QWORD *)(*a1 + 72) )
        imp_WppRecorderLogDelete(WPP_GLOBAL_Control);
    }
    a1[10] = 0LL;
  }
}
