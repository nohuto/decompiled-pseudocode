/*
 * XREFs of sub_14001FFBC @ 0x14001FFBC
 * Callers:
 *     sub_14001FFA0 @ 0x14001FFA0 (sub_14001FFA0.c)
 *     sub_140071BC0 @ 0x140071BC0 (sub_140071BC0.c)
 * Callees:
 *     sub_140020480 @ 0x140020480 (sub_140020480.c)
 *     sub_1400B6010 @ 0x1400B6010 (sub_1400B6010.c)
 */

__int64 __fastcall sub_14001FFBC(__int64 a1, _DWORD *a2, _QWORD *a3)
{
  unsigned int v6; // edi
  _QWORD *i; // rbx
  _DWORD *v8; // rcx
  int v9; // ebp
  __int64 v11; // rbx
  int v12; // eax

  v6 = 0;
  if ( !a1 )
    return (unsigned int)-2147024809;
  if ( a3 )
  {
    *a3 = 0LL;
    if ( !(unsigned int)sub_140020480(a2) )
    {
      for ( i = &unk_1400B94E8; ; i += 3 )
      {
        if ( !i[1] )
          return (unsigned int)-2147467262;
        v8 = (_DWORD *)*(i - 1);
        if ( v8 )
        {
          v9 = 0;
          if ( *v8 != *a2 || v8[1] != a2[1] || v8[2] != a2[2] || v8[3] != a2[3] )
            continue;
        }
        else
        {
          v9 = 1;
        }
        if ( i[1] == 1LL )
        {
          v11 = a1 + *i;
          sub_1400B6010(v11);
          *a3 = v11;
          return v6;
        }
        v12 = sub_1400B6010(a1);
        if ( !v12 || !v9 && v12 < 0 )
          return (unsigned int)v12;
      }
    }
    sub_1400B6010(a1);
    *a3 = a1;
  }
  else
  {
    return (unsigned int)-2147467261;
  }
  return v6;
}
