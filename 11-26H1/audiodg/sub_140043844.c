/*
 * XREFs of sub_140043844 @ 0x140043844
 * Callers:
 *     sub_140003E20 @ 0x140003E20 (sub_140003E20.c)
 * Callees:
 *     sub_140003238 @ 0x140003238 (sub_140003238.c)
 *     sub_1400115FC @ 0x1400115FC (sub_1400115FC.c)
 *     sub_14005A040 @ 0x14005A040 (sub_14005A040.c)
 *     sub_14005A470 @ 0x14005A470 (sub_14005A470.c)
 *     sub_1400B6010 @ 0x1400B6010 (sub_1400B6010.c)
 */

__int64 __fastcall sub_140043844(__int64 a1, __int64 a2, __int64 a3)
{
  int v5; // ebx
  __int64 v7; // [rsp+38h] [rbp+10h] BYREF

  v7 = 0LL;
  if ( *(_DWORD *)(a1 + 120) || (int)sub_1400B6010(a3) < 0 )
  {
    v5 = 0;
  }
  else
  {
    if ( *(_QWORD *)(a1 + 184) != v7 )
      sub_1400115FC((_QWORD *)(a1 + 184), v7);
    v5 = sub_1400B6010(a3);
    if ( v5 < 0 )
    {
      if ( off_1400E73D8 != (_UNKNOWN *)&off_1400E73D8
        && (*((_DWORD *)off_1400E73D8 + 7) & 0x20000) != 0
        && *((_BYTE *)off_1400E73D8 + 25) >= 2u )
      {
        sub_14005A040(*((_QWORD *)off_1400E73D8 + 2), 47LL, &unk_1400C9A58);
      }
      sub_14005A470("CPipeInstance::InitializePlaybackConstrictorInterface", 3798LL, (unsigned int)v5);
    }
  }
  sub_140003238(&v7);
  return (unsigned int)v5;
}
