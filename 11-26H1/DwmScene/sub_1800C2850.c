/*
 * XREFs of sub_1800C2850 @ 0x1800C2850
 * Callers:
 *     sub_1800C31E8 @ 0x1800C31E8 (sub_1800C31E8.c)
 * Callees:
 *     sub_180011790 @ 0x180011790 (sub_180011790.c)
 *     sub_180011B48 @ 0x180011B48 (sub_180011B48.c)
 *     sub_180013518 @ 0x180013518 (sub_180013518.c)
 *     sub_180029634 @ 0x180029634 (sub_180029634.c)
 *     sub_180029F88 @ 0x180029F88 (sub_180029F88.c)
 */

__int64 __fastcall sub_1800C2850(void **a1, unsigned __int64 *a2)
{
  __int64 v4; // rdi
  unsigned __int64 v5; // rax
  char *v6; // rax
  __int64 v7; // rdx
  __int64 v8; // rbx
  void *v9; // rcx
  __int64 v10; // r9
  _QWORD v12[5]; // [rsp+20h] [rbp-28h] BYREF

  v4 = ((_BYTE *)a1[1] - (_BYTE *)*a1) >> 4;
  v5 = sub_180011B48(*a2);
  v6 = (char *)sub_180011790(v5);
  v7 = (__int64)a1[1];
  v8 = (__int64)v6;
  v12[2] = *a2;
  v9 = *a1;
  v12[0] = a1;
  sub_180029634(v9, v7, v6);
  v10 = *a2;
  v12[1] = 0LL;
  sub_180029F88((__int64)a1, v8, v4, v10);
  return sub_180013518((__int64)v12);
}
