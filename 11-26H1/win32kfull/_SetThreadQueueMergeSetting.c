/*
 * XREFs of _SetThreadQueueMergeSetting @ 0x1402C2630
 * Callers:
 *     NtUserSetThreadQueueMergeSetting @ 0x1402BE2B0 (NtUserSetThreadQueueMergeSetting.c)
 * Callees:
  ...
  else
  {
    ...
    if ( v8 == CurrentProcessWin32Process )
    {
      if ( (unsigned int)IsImmersiveAppRestricted(v8) )
      {
        if ( (v2 & 1) != 0 )
        {
          if ( tagQ::AreMultipleThreadsAttached(*(tagQ **)(v5 + 464))
            || FindShellFrameThreadFromAssociation((struct tagTHREADINFO *)v5) )
          {
            v3 = 170;
            goto LABEL_19;
          }
          _InterlockedOr((volatile signed __int32 *)(v5 + 520), 0x40u);
        }
        else
        {
          _InterlockedAnd((volatile signed __int32 *)(v5 + 520), 0xFFFFFFBF);
        }
        return 1LL;
      }
      v3 = 50;
