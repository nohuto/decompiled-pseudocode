/*
 * XREFs of ?SetWindowCompositionNonClientRTLLayout@@YAJPEAUtagWND@@PEBUtagWINDOWCOMPOSITIONATTRIBDATA_UNION@@@Z @ 0x1401E5570
 * Callers:
        ...
        const struct tagWINDOWCOMPOSITIONATTRIBDATA_UNION *a2)
{
  ...
  {
    if ( -__CFSHR__(v7[0], 3) == v4 )
    {
      return 0;
    }
    else
    {
      LODWORD(v7[0]) = v7[0] & 0xFFFFFFFB | (4 * v4);
      if ( (unsigned int)SetWindowCompositionInfo(a1, (const struct WINDOWCOMPOSITIONINFO *)v7) )
        return (unsigned int)_DwmAsyncSetCompositionAttribute(a1, 6LL, a2);
      else
