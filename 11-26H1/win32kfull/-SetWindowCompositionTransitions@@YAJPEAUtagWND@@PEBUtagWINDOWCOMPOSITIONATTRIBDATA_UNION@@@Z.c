/*
 * XREFs of ?SetWindowCompositionTransitions@@YAJPEAUtagWND@@PEBUtagWINDOWCOMPOSITIONATTRIBDATA_UNION@@@Z @ 0x1401E59FC
 * Callers:
        ...
        const struct tagWINDOWCOMPOSITIONATTRIBDATA_UNION *a2)
{
  ...
  {
    if ( -__CFSHR__(v7[0], 7) == v4 )
    {
      return 0;
    }
    else
    {
      LODWORD(v7[0]) = v7[0] & 0xFFFFFFBF | (v4 << 6);
      if ( (unsigned int)SetWindowCompositionInfo(a1, (const struct WINDOWCOMPOSITIONINFO *)v7) )
        return (unsigned int)_DwmAsyncSetCompositionAttribute(a1, 3LL, a2);
      else
