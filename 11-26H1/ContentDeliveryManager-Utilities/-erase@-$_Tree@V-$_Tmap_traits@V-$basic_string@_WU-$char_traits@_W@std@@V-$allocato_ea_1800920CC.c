/*
 * XREFs of ?erase@?$_Tree@V?$_Tmap_traits@V?$basic_string@_WU?$char_traits@_W@std@@V?$allocator@_W@2@@std@@V12@U?$less@V?$basic_string@_WU?$char_traits@_W@std@@V?$allocator@_W@2@@std@@@2@V?$allocator@U?$pair@$$CBV?$basic_string@_WU?$char_traits@_W@std@@V?$allocator@_W@2@@std@@V12@@std@@@2@$0A@@std@@@std@@QEAA?AV?$_Tree_iterator@V?$_Tree_val@U?$_Tree_simple_types@U?$pair@$$CBV?$basic_string@_WU?$char_traits@_W@std@@V?$allocator@_W@2@@std@@V12@@std@@@std@@@std@@@2@V?$_Tree_const_iterator@V?$_Tree_val@U?$_Tree_simple_types@U?$pair@$$CBV?$basic_string@_WU?$char_traits@_W@std@@V?$allocator@_W@2@@std@@V12@@std@@@std@@@std@@@2@@Z @ 0x1800920CC
 * Callers:
 *     ?erase@?$_Tree@V?$_Tmap_traits@V?$basic_string@_WU?$char_traits@_W@std@@V?$allocator@_W@2@@std@@V12@U?$less@V?$basic_string@_WU?$char_traits@_W@std@@V?$allocator@_W@2@@std@@@2@V?$allocator@U?$pair@$$CBV?$basic_string@_WU?$char_traits@_W@std@@V?$allocator@_W@2@@std@@V12@@std@@@2@$0A@@std@@@std@@QEAA?AV?$_Tree_iterator@V?$_Tree_val@U?$_Tree_simple_types@U?$pair@$$CBV?$basic_string@_WU?$char_traits@_W@std@@V?$allocator@_W@2@@std@@V12@@std@@@std@@@std@@@2@V?$_Tree_const_iterator@V?$_Tree_val@U?$_Tree_simple_types@U?$pair@$$CBV?$basic_string@_WU?$char_traits@_W@std@@V?$allocator@_W@2@@std@@V12@@std@@@std@@@std@@@2@0@Z @ 0x180092020 (-erase@-$_Tree@V-$_Tmap_traits@V-$basic_string@_WU-$char_traits@_W@std@@V-$allocato_ea_180092020.c)
 * Callees:
 *     ?_Xout_of_range@std@@YAXPEBD@Z @ 0x180004800 (-_Xout_of_range@std@@YAXPEBD@Z.c)
 *     ?_Lrotate@?$_Tree@V?$_Tmap_traits@PEBGPEBU_WNF_STATE_NAME@@Uless@Triggers@CreativeFramework@@V?$allocator@U?$pair@QEBGPEBU_WNF_STATE_NAME@@@std@@@std@@$0A@@std@@@std@@IEAAXPEAU?$_Tree_node@U?$pair@QEBGPEBU_WNF_STATE_NAME@@@std@@PEAX@2@@Z @ 0x18005C2E0 (-_Lrotate@-$_Tree@V-$_Tmap_traits@PEBGPEBU_WNF_STATE_NAME@@Uless@Triggers@CreativeFramework@@V-$.c)
 *     ?_Min@?$_Tree_val@U?$_Tree_simple_types@U?$pair@$$CBV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@V12@@std@@@std@@@std@@SAPEAU?$_Tree_node@U?$pair@$$CBV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@V12@@std@@PEAX@2@PEAU32@@Z @ 0x18005C330 (-_Min@-$_Tree_val@U-$_Tree_simple_types@U-$pair@$$CBV-$basic_string@GU-$char_traits@G@std@@V-$al.c)
 *     ?_Rrotate@?$_Tree@V?$_Tmap_traits@PEBGPEBU_WNF_STATE_NAME@@Uless@Triggers@CreativeFramework@@V?$allocator@U?$pair@QEBGPEBU_WNF_STATE_NAME@@@std@@@std@@$0A@@std@@@std@@IEAAXPEAU?$_Tree_node@U?$pair@QEBGPEBU_WNF_STATE_NAME@@@std@@PEAX@2@@Z @ 0x18005C59C (-_Rrotate@-$_Tree@V-$_Tmap_traits@PEBGPEBU_WNF_STATE_NAME@@Uless@Triggers@CreativeFramework@@V-$.c)
 *     ??1?$pair@$$CBV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@V12@@std@@QEAA@XZ @ 0x18006104C (--1-$pair@$$CBV-$basic_string@GU-$char_traits@G@std@@V-$allocator@G@2@@std@@V12@@std@@QEAA@XZ.c)
 *     ?_Max@?$_Tree_val@U?$_Tree_simple_types@U?$pair@$$CBV?$basic_string@_WU?$char_traits@_W@std@@V?$allocator@_W@2@@std@@V12@@std@@@std@@@std@@SAPEAU?$_Tree_node@U?$pair@$$CBV?$basic_string@_WU?$char_traits@_W@std@@V?$allocator@_W@2@@std@@V12@@std@@PEAX@2@PEAU32@@Z @ 0x18006516C (-_Max@-$_Tree_val@U-$_Tree_simple_types@U-$pair@$$CBV-$basic_string@_WU-$char_traits@_W@std@@V-$.c)
 */

char **__fastcall std::_Tree<std::_Tmap_traits<std::wstring,std::wstring,std::less<std::wstring>,std::allocator<std::pair<std::wstring const,std::wstring>>,0>>::erase(
        _QWORD *a1,
        char **a2,
        char *a3)
{
  _QWORD *v5; // r14
  char *v6; // rsi
  __int64 v7; // r10
  __int64 *v8; // r11
  char *v9; // rbp
  char v10; // r9
  char *i; // rbx
  __int64 v12; // r9
  _QWORD *v13; // r8
  _QWORD *v14; // rax
  __int64 v15; // rdx
  __int64 v16; // rax
  _BYTE *v17; // rdx
  _QWORD *v18; // rax
  char v19; // cl
  __int64 v20; // r11
  _BYTE *v21; // rdx
  __int64 v22; // rcx
  __int64 v23; // rax
  char **result; // rax

  if ( a3[25] )
    std::_Xout_of_range("invalid map/set<T> iterator");
  v5 = a3 + 16;
  v6 = a3;
  v7 = *((_QWORD *)a3 + 2);
  v8 = (__int64 *)(a3 + 8);
  v9 = a3;
  v10 = *(_BYTE *)(v7 + 25);
  if ( v10 )
  {
    for ( i = (char *)*v8; !i[25] && a3 == *((char **)i + 2); i = (char *)*((_QWORD *)i + 1) )
      a3 = i;
  }
  else
  {
    i = (char *)std::_Tree_val<std::_Tree_simple_types<std::pair<std::wstring const,std::wstring>>>::_Min(*((_QWORD **)a3 + 2));
  }
  if ( *(_BYTE *)(*(_QWORD *)v6 + 25LL) )
    goto LABEL_13;
  if ( v10 )
  {
    v7 = *(_QWORD *)v6;
LABEL_13:
    v12 = *v8;
    if ( !*(_BYTE *)(v7 + 25) )
      *(_QWORD *)(v7 + 8) = v12;
    if ( *(char **)(*a1 + 8LL) == v6 )
    {
      *(_QWORD *)(*a1 + 8LL) = v7;
    }
    else if ( *(char **)v12 == v6 )
    {
      *(_QWORD *)v12 = v7;
    }
    else
    {
      *(_QWORD *)(v12 + 16) = v7;
    }
    v13 = (_QWORD *)*a1;
    if ( *(char **)*a1 == v6 )
    {
      if ( *(_BYTE *)(v7 + 25) )
        v14 = (_QWORD *)v12;
      else
        v14 = std::_Tree_val<std::_Tree_simple_types<std::pair<std::wstring const,std::wstring>>>::_Min((_QWORD *)v7);
      *v13 = v14;
    }
    v15 = *a1;
    if ( *(char **)(*a1 + 16LL) == v6 )
    {
      if ( *(_BYTE *)(v7 + 25) )
        v16 = v12;
      else
        v16 = std::_Tree_val<std::_Tree_simple_types<std::pair<std::wstring const,std::wstring>>>::_Max(v7);
      *(_QWORD *)(v15 + 16) = v16;
    }
    v17 = v9 + 24;
    goto LABEL_42;
  }
  v7 = *((_QWORD *)i + 2);
  if ( i == v6 )
    goto LABEL_13;
  *(_QWORD *)(*(_QWORD *)v6 + 8LL) = i;
  *(_QWORD *)i = *(_QWORD *)v6;
  if ( i == (char *)*v5 )
  {
    v12 = (__int64)i;
  }
  else
  {
    v12 = *((_QWORD *)i + 1);
    if ( !*(_BYTE *)(v7 + 25) )
      *(_QWORD *)(v7 + 8) = v12;
    *(_QWORD *)v12 = v7;
    *((_QWORD *)i + 2) = *v5;
    *(_QWORD *)(*v5 + 8LL) = i;
  }
  if ( *(char **)(*a1 + 8LL) == v6 )
  {
    *(_QWORD *)(*a1 + 8LL) = i;
  }
  else
  {
    v18 = (_QWORD *)*v8;
    if ( *(char **)*v8 == v6 )
      *v18 = i;
    else
      v18[2] = i;
  }
  v17 = v6 + 24;
  *((_QWORD *)i + 1) = *v8;
  v19 = i[24];
  i[24] = v6[24];
  v6[24] = v19;
LABEL_42:
  if ( *v17 != 1 )
    goto LABEL_65;
  while ( 1 )
  {
    v20 = v12;
    if ( v7 == *(_QWORD *)(*a1 + 8LL) || *(_BYTE *)(v7 + 24) != 1 )
      break;
    v21 = *(_BYTE **)v12;
    if ( v7 == *(_QWORD *)v12 )
    {
      v21 = *(_BYTE **)(v12 + 16);
      if ( !v21[24] )
      {
        v21[24] = 1;
        *(_BYTE *)(v12 + 24) = 0;
        std::_Tree<std::_Tmap_traits<unsigned short const *,_WNF_STATE_NAME const *,CreativeFramework::Triggers::less,std::allocator<std::pair<unsigned short const * const,_WNF_STATE_NAME const *>>,0>>::_Lrotate(
          (__int64)a1,
          v12);
        v21 = *(_BYTE **)(v12 + 16);
      }
      if ( !v21[25] )
      {
        if ( *(_BYTE *)(*(_QWORD *)v21 + 24LL) != 1 || *(_BYTE *)(*((_QWORD *)v21 + 2) + 24LL) != 1 )
        {
          if ( *(_BYTE *)(*((_QWORD *)v21 + 2) + 24LL) == 1 )
          {
            *(_BYTE *)(*(_QWORD *)v21 + 24LL) = 1;
            v21[24] = 0;
            std::_Tree<std::_Tmap_traits<unsigned short const *,_WNF_STATE_NAME const *,CreativeFramework::Triggers::less,std::allocator<std::pair<unsigned short const * const,_WNF_STATE_NAME const *>>,0>>::_Rrotate(
              (__int64)a1,
              v21);
            v21 = *(_BYTE **)(v12 + 16);
          }
          v21[24] = *(_BYTE *)(v12 + 24);
          *(_BYTE *)(v12 + 24) = 1;
          *(_BYTE *)(*((_QWORD *)v21 + 2) + 24LL) = 1;
          std::_Tree<std::_Tmap_traits<unsigned short const *,_WNF_STATE_NAME const *,CreativeFramework::Triggers::less,std::allocator<std::pair<unsigned short const * const,_WNF_STATE_NAME const *>>,0>>::_Lrotate(
            (__int64)a1,
            v12);
          break;
        }
LABEL_59:
        v21[24] = 0;
      }
    }
    else
    {
      if ( !v21[24] )
      {
        v21[24] = 1;
        *(_BYTE *)(v12 + 24) = 0;
        std::_Tree<std::_Tmap_traits<unsigned short const *,_WNF_STATE_NAME const *,CreativeFramework::Triggers::less,std::allocator<std::pair<unsigned short const * const,_WNF_STATE_NAME const *>>,0>>::_Rrotate(
          (__int64)a1,
          (_QWORD *)v12);
        v21 = *(_BYTE **)v12;
      }
      if ( !v21[25] )
      {
        v22 = *((_QWORD *)v21 + 2);
        if ( *(_BYTE *)(v22 + 24) != 1 || *(_BYTE *)(*(_QWORD *)v21 + 24LL) != 1 )
        {
          if ( *(_BYTE *)(*(_QWORD *)v21 + 24LL) == 1 )
          {
            *(_BYTE *)(v22 + 24) = 1;
            v21[24] = 0;
            std::_Tree<std::_Tmap_traits<unsigned short const *,_WNF_STATE_NAME const *,CreativeFramework::Triggers::less,std::allocator<std::pair<unsigned short const * const,_WNF_STATE_NAME const *>>,0>>::_Lrotate(
              (__int64)a1,
              (__int64)v21);
            v21 = *(_BYTE **)v12;
          }
          v21[24] = *(_BYTE *)(v12 + 24);
          *(_BYTE *)(v12 + 24) = 1;
          *(_BYTE *)(*(_QWORD *)v21 + 24LL) = 1;
          std::_Tree<std::_Tmap_traits<unsigned short const *,_WNF_STATE_NAME const *,CreativeFramework::Triggers::less,std::allocator<std::pair<unsigned short const * const,_WNF_STATE_NAME const *>>,0>>::_Rrotate(
            (__int64)a1,
            (_QWORD *)v12);
          break;
        }
        goto LABEL_59;
      }
    }
    v12 = *(_QWORD *)(v12 + 8);
    v7 = v20;
  }
  *(_BYTE *)(v7 + 24) = 1;
LABEL_65:
  std::pair<std::wstring const,std::wstring>::~pair<std::wstring const,std::wstring>((_QWORD *)v6 + 4);
  operator delete(v6);
  v23 = a1[1];
  if ( v23 )
    a1[1] = v23 - 1;
  result = a2;
  *a2 = i;
  return result;
}
