/*
 * XREFs of _CreativeFramework::TargetedContentLayoutHelpers::LayoutResolver::ProcessLayoutBindings_::_1_::catch$11 @ 0x1800BEC3F
 * Callers:
 *     <none>
 * Callees:
 *     _CxxThrowException_0 @ 0x180021CDC (_CxxThrowException_0.c)
 *     ??$?0PEBG@HStringReference@Wrappers@WRL@Microsoft@@QEAA@AEBQEBGUDummy@Details@23@@Z @ 0x180023724 (--$-0PEBG@HStringReference@Wrappers@WRL@Microsoft@@QEAA@AEBQEBGUDummy@Details@23@@Z.c)
 *     ??$verify_hresult@J@wil@@YAJJ@Z @ 0x180028118 (--$verify_hresult@J@wil@@YAJJ@Z.c)
 *     ?Get@HString@Wrappers@WRL@Microsoft@@QEBAPEAUHSTRING__@@XZ @ 0x180030138 (-Get@HString@Wrappers@WRL@Microsoft@@QEBAPEAUHSTRING__@@XZ.c)
 *     ?Get@HStringReference@Wrappers@WRL@Microsoft@@QEBAPEAUHSTRING__@@XZ @ 0x180030144 (-Get@HStringReference@Wrappers@WRL@Microsoft@@QEBAPEAUHSTRING__@@XZ.c)
 *     ?Throw_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z @ 0x18003E0D8 (-Throw_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z.c)
 *     ?SetBoundedLayoutType@Settings@TargetedContentLayoutHelpers@CreativeFramework@@YAJPEAUHSTRING__@@0@Z @ 0x1800B1224 (-SetBoundedLayoutType@Settings@TargetedContentLayoutHelpers@CreativeFramework@@YAJPEAUHSTRING__@.c)
 *     _guard_dispatch_icall$thunk$10345483385596137414 @ 0x1800C0010 (_guard_dispatch_icall$thunk$10345483385596137414.c)
 */

void __fastcall __noreturn CreativeFramework::TargetedContentLayoutHelpers::LayoutResolver::ProcessLayoutBindings_::_1_::catch_11(
        __int64 a1,
        __int64 a2)
{
  __int64 v3; // rbx
  HSTRING v4; // rax
  __int64 v5; // r9
  unsigned int v6; // eax
  int v7; // eax
  unsigned int v8; // r8d
  HSTRING_HEADER *v9; // rax
  CreativeFramework::TargetedContentLayoutHelpers::Settings *v10; // rax
  HSTRING v11; // rdx
  HSTRING v12; // r8
  HSTRING v13; // r9
  unsigned int v14; // eax
  int v15; // eax
  int v16; // [rsp+20h] [rbp-28h]

  Microsoft::WRL::Wrappers::HString::Get((Microsoft::WRL::Wrappers::HString *)(a2 + 72));
  v3 = *(_QWORD *)(a2 + 96);
  v4 = Microsoft::WRL::Wrappers::HString::Get((Microsoft::WRL::Wrappers::HString *)(v3 + 8));
  v6 = (*(__int64 (__fastcall **)(__int64, HSTRING))(*(_QWORD *)v5 + 72LL))(v5, v4);
  v7 = wil::verify_hresult<long>(v6);
  if ( v7 < 0 )
    wil::details::in1diag3::Throw_Hr(
      *(wil::details::in1diag3 **)(a2 + 584),
      280LL,
      (__int64)"shellcommon\\shell\\contentdeliverymanager\\utils\\lib\\targetedcontentlayoutresolver\\layoutresolver.cpp",
      (const char *)(unsigned int)v7,
      v16);
  v9 = Microsoft::WRL::Wrappers::HStringReference::HStringReference(
         (HSTRING_HEADER *)(a2 + 480),
         (const WCHAR **)&off_1800DF4A0,
         v8);
  Microsoft::WRL::Wrappers::HStringReference::Get((Microsoft::WRL::Wrappers::HStringReference *)v9);
  v10 = (CreativeFramework::TargetedContentLayoutHelpers::Settings *)Microsoft::WRL::Wrappers::HString::Get((Microsoft::WRL::Wrappers::HString *)(v3 + 8));
  v14 = CreativeFramework::TargetedContentLayoutHelpers::Settings::SetBoundedLayoutType(v10, v11, v12, v13);
  v15 = wil::verify_hresult<long>(v14);
  if ( v15 < 0 )
    wil::details::in1diag3::Throw_Hr(
      *(wil::details::in1diag3 **)(a2 + 584),
      281LL,
      (__int64)"shellcommon\\shell\\contentdeliverymanager\\utils\\lib\\targetedcontentlayoutresolver\\layoutresolver.cpp",
      (const char *)(unsigned int)v15,
      v16);
  throw;
}
