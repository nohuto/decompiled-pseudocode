/*
 * XREFs of GetWmiBiosIdleStateInfo @ 0x14000B020
 * Callers:
 *     <none>
 * Callees:
 *     _guard_dispatch_icall @ 0x140010390 (_guard_dispatch_icall.c)
 */

__int64 __fastcall GetWmiBiosIdleStateInfo(__int64 a1, unsigned int a2, __int64 a3, unsigned int *a4)
{
  unsigned int v5; // ebp
  unsigned int v8; // ebx
  __int64 v9; // rax
  __int64 v10; // rsi
  __int64 v11; // rax
  int v12; // eax
  unsigned int *v13; // rax
  __int64 v14; // rax
  __int64 v15; // rcx
  __int64 v16; // r9
  __int64 v17; // rdi
  __int64 v18; // r8
  __int64 i; // rdx
  unsigned __int8 v20; // al
  int v21; // eax
  _DWORD *v22; // rax

  v5 = 0;
  v8 = 72;
  v9 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64))(WdfFunctions_01015 + 3008))(WdfDriverGlobals, a1);
  v10 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, void *))(WdfFunctions_01015 + 1616))(
          WdfDriverGlobals,
          v9,
          off_140018040);
  (*(void (__fastcall **)(PWDF_DRIVER_GLOBALS, _QWORD, _QWORD))(WdfFunctions_01015 + 2504))(
    WdfDriverGlobals,
    *(_QWORD *)(v10 + 208),
    0LL);
  if ( a2 < 0x48 )
  {
    v22 = *(_DWORD **)(v10 + 528);
    if ( v22 && *v22 )
      v8 = 32 * *v22 + 40;
LABEL_20:
    v5 = -1073741789;
    goto LABEL_21;
  }
  *(_WORD *)a3 = AcpiInfo;
  *(_WORD *)(a3 + 2) = word_140019142;
  *(_DWORD *)(a3 + 4) = *(_DWORD *)(v10 + 56);
  v11 = *(_QWORD *)(v10 + 280);
  if ( (v11 & 0x7F070) != 0 )
    v12 = 2;
  else
    v12 = (v11 & 7) != 0;
  *(_DWORD *)(a3 + 8) = v12;
  *(_QWORD *)(a3 + 12) = 0LL;
  *(_QWORD *)(a3 + 24) = 0LL;
  v13 = *(unsigned int **)(v10 + 528);
  if ( !v13 )
  {
    *(_OWORD *)(a3 + 32) = 0LL;
    *(_OWORD *)(a3 + 48) = 0LL;
    *(_QWORD *)(a3 + 64) = 0LL;
    goto LABEL_21;
  }
  v14 = *v13;
  if ( (_DWORD)v14 )
  {
    v8 = 32 * v14 + 40;
    if ( a2 < v8 )
      goto LABEL_20;
  }
  *(_DWORD *)(a3 + 32) = v14;
  if ( (_DWORD)v14 )
  {
    v15 = 0LL;
    v16 = v14;
    v17 = a3 + 40;
    do
    {
      v18 = *(_QWORD *)(v10 + 528);
      for ( i = 0LL; i < 4; ++i )
        *(_DWORD *)(v17 + 4 * i) = *(unsigned __int8 *)(i + v15 + v18 + 4);
      *(_QWORD *)(v17 + 16) = *(_QWORD *)(v15 + v18 + 8);
      v20 = *(_BYTE *)(v15 + v18 + 16);
      *(_BYTE *)(v17 + 24) = v20;
      if ( v20 > 3u )
        *(_BYTE *)(v17 + 24) = 3;
      *(_WORD *)(v17 + 26) = *(_WORD *)(v15 + v18 + 18);
      v21 = *(_DWORD *)(v15 + v18 + 20);
      v15 += 20LL;
      *(_DWORD *)(v17 + 28) = v21;
      v17 += 32LL;
      --v16;
    }
    while ( v16 );
  }
LABEL_21:
  (*(void (__fastcall **)(PWDF_DRIVER_GLOBALS, _QWORD))(WdfFunctions_01015 + 2512))(
    WdfDriverGlobals,
    *(_QWORD *)(v10 + 208));
  if ( a4 )
    *a4 = v8;
  return v5;
}
