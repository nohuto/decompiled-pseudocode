/*
 * XREFs of UsbhBusIfAddDevice @ 0x1400572E0
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall UsbhBusIfAddDevice(PDEVICE_OBJECT DeviceObject)
{
  ...
  while ( 1 )
  {
    ...
    BusInterface = (int)UsbhGetBusInterface(DeviceObject, v5);
  ...
  while ( 1 )
  {
    ...
    if ( (int)v11 >= 0 )
    {
      ...
      *((_WORD *)v3 + 2400) = v9;
      ...
      if ( (int)BusInterface >= 0 )
      {
        ...
        Log((__int64)DeviceObject, 8, 1198091570, (int)BusInterface, 0LL);
        if ( v16 >= 0 )
          v3[640] |= 0x20000000u;
        Log((__int64)DeviceObject, 8, 1198093105, 48LL, 0LL);
        UsbhGetBusInterface(DeviceObject, 0);
        Log((__int64)DeviceObject, 8, 1198093106, (int)BusInterface, 0LL);
        if ( v17 >= 0 )
          v3[640] |= 0x800000u;
        Log((__int64)DeviceObject, 8, 1197895729, 64LL, 0LL);
        LODWORD(BusInterface) = UsbhGetBusInterface(DeviceObject, 0);
        Log((__int64)DeviceObject, 8, 1197895730, (int)BusInterface, 0LL);
        if ( (int)BusInterface >= 0 )
          v3[640] |= 0x80000000;
      }
      break;
    ...
    LODWORD(BusInterface) = 0;
    if ( v10 != 3 )
      LODWORD(BusInterface) = v11;
  }
  Log((__int64)DeviceObject, 8, 1651730790, 0LL, (int)BusInterface);
