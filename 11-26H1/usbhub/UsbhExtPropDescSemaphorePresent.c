/*
 * XREFs of UsbhExtPropDescSemaphorePresent @ 0x1400422F0
 * Callers:
 *     UsbhInstallMsOs20RegistryProperties @ 0x1400430C0 (UsbhInstallMsOs20RegistryProperties.c)
 *     UsbhInstallMsOsExtendedProperties @ 0x1400431A4 (UsbhInstallMsOsExtendedProperties.c)
 * Callees:
 *     PdoExt @ 0x140016350 (PdoExt.c)
 *     UsbhGetPdoRegistryParameter @ 0x140035B00 (UsbhGetPdoRegistryParameter.c)
 *     UsbhSetPdoRegistryParameter @ 0x14003647C (UsbhSetPdoRegistryParameter.c)
 *     WPP_RECORDER_SF_d @ 0x14003E3EC (WPP_RECORDER_SF_d.c)
 *     __security_check_cookie @ 0x140061BD0 (__security_check_cookie.c)
 */

bool __fastcall UsbhExtPropDescSemaphorePresent(PDEVICE_OBJECT DeviceObject)
{
  _DWORD *v2; // rbx
  int PdoRegistryParameter; // eax
  int v4; // eax
  int v6; // ecx
  __int64 v8; // [rsp+30h] [rbp-51h]
  __int64 v9; // [rsp+30h] [rbp-51h]
  ULONG v10; // [rsp+38h] [rbp-49h]
  ULONG v11; // [rsp+38h] [rbp-49h]
  ULONG v12; // [rsp+38h] [rbp-49h]
  int v13; // [rsp+48h] [rbp-39h] BYREF
  int v14; // [rsp+4Ch] [rbp-35h] BYREF
  int Data; // [rsp+50h] [rbp-31h] BYREF
  WCHAR v16[12]; // [rsp+58h] [rbp-29h] BYREF
  WCHAR v17[16]; // [rsp+70h] [rbp-11h] BYREF
  WCHAR SourceString[24]; // [rsp+90h] [rbp+Fh] BYREF

  wcscpy(SourceString, L"ExtPropDescSemaphore");
  wcscpy(v16, L"RevisionId");
  wcscpy(v17, L"VendorRevision");
  Data = 0;
  v14 = 0;
  v13 = 0;
  LOBYTE(v10) = 1;
  v2 = PdoExt((__int64)DeviceObject);
  PdoRegistryParameter = UsbhGetPdoRegistryParameter(DeviceObject, v16, &v14, 4u, 0LL, 0LL, v10);
  if ( (int)(PdoRegistryParameter + 0x80000000) >= 0
    && PdoRegistryParameter != -1073741772
    && WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED
    && LOWORD(WPP_GLOBAL_Control->DeviceType) )
  {
    LODWORD(v8) = PdoRegistryParameter;
    WPP_RECORDER_SF_d(
      (__int64)WPP_GLOBAL_Control->DeviceExtension,
      0,
      1u,
      0xAu,
      (__int64)&WPP_f1b4c9c59f9f315fbcaf54f6e2fa0cb0_Traceguids,
      v8);
  }
  LOBYTE(v11) = 1;
  v4 = UsbhGetPdoRegistryParameter(DeviceObject, v17, &v13, 4u, 0LL, 0LL, v11);
  if ( (int)(v4 + 0x80000000) >= 0
    && v4 != -1073741772
    && WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED
    && LOWORD(WPP_GLOBAL_Control->DeviceType) )
  {
    LODWORD(v9) = v4;
    WPP_RECORDER_SF_d(
      (__int64)WPP_GLOBAL_Control->DeviceExtension,
      0,
      1u,
      0xBu,
      (__int64)&WPP_f1b4c9c59f9f315fbcaf54f6e2fa0cb0_Traceguids,
      v9);
  }
  LOBYTE(v12) = 1;
  if ( (int)UsbhGetPdoRegistryParameter(DeviceObject, SourceString, &Data, 4u, 0LL, 0LL, v12) >= 0
    && v14 == *((unsigned __int16 *)v2 + 706)
    && ((v2[706] & 0x400) != 0 ? v13 == *(unsigned __int16 *)(*((_QWORD *)v2 + 360) + 4LL) : v13 == 0) )
  {
    return 1;
  }
  Data = 1;
  if ( (int)UsbhSetPdoRegistryParameter(DeviceObject, SourceString, 4u, &Data, 4u) < 0 )
    return 1;
  v14 = *((unsigned __int16 *)v2 + 706);
  if ( (int)UsbhSetPdoRegistryParameter(DeviceObject, v16, 4u, &v14, 4u) < 0 )
    return 1;
  if ( (v2[706] & 0x400) != 0 )
    v6 = *(unsigned __int16 *)(*((_QWORD *)v2 + 360) + 4LL);
  else
    v6 = 0;
  v13 = v6;
  return (int)UsbhSetPdoRegistryParameter(DeviceObject, v17, 4u, &v13, 4u) < 0;
}
