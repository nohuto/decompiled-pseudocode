/*
 * XREFs of ?CreateGlyphsFromAtlas@CTopLevelWindow@@CAJQEAX@Z @ 0x180004980
 * Callers:
    ...
    if ( *(_QWORD *)(v1 + 8 * i) )
    {
      for ( j = 0LL; (unsigned int)j < 0xA; j = (unsigned int)(j + 1) )
      {
        ...
        if ( (unsigned int)(i - 4) <= 1 )
          v7 = (char *)&unk_1800F4A30 + v6 + 4;
        else
          v7 = (char *)&unk_1800F4A30 + v6;
        if ( *v7 )
        {
          for ( k = 0LL; (unsigned int)k < 4; k = (unsigned int)(k + 1) )
          {
            BitmapsFromAtlasImageStrip = CTopLevelWindow::CreateBitmapsFromAtlasImageStrip(
                                         ...
                                         + 192
                                         + *(_QWORD *)(v1 + 8 * i)
                                         + *(unsigned int *)((char *)&unk_1800F4A30 + v6 + 8));
            v2 = BitmapsFromAtlasImageStrip;
            if ( BitmapsFromAtlasImageStrip < 0 )
            {
              MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0, BitmapsFromAtlasImageStrip, 0x23Eu, 0LL);
              return v2;
            }
            v1 = CTopLevelWindow::s_rgpwfWindowFrames;
          }
