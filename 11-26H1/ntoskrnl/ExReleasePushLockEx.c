/*
 * XREFs of ExReleasePushLockEx @ 0x14027AF50
 * Callers:
 *     <none>
 * Callees:
 *     KiLowerIrqlProcessIrqlFlags @ 0x140246770 (KiLowerIrqlProcessIrqlFlags.c)
 *     ?KiAbEntryFreeAndEnableInterrupts@LegacyAutoBoost@@YAXPEAU_KLOCK_ENTRY@@PEAU_KTHREAD@@PEAXKPEAT_KLOCK_ENTRY_BOOST_BITMAP@@@Z @ 0x140272190 (-KiAbEntryFreeAndEnableInterrupts@LegacyAutoBoost@@YAXPEAU_KLOCK_ENTRY@@PEAU_KTHREAD@@PEAXKPEAT_.c)
 *     ?KiAbpEntryUnlink@AutoBoost@@YAXPEAU_AB_THREAD_ENTRY@1@@Z @ 0x14027AAF0 (-KiAbpEntryUnlink@AutoBoost@@YAXPEAU_AB_THREAD_ENTRY@1@@Z.c)
 *     ?KiAbpUnboostThread@AutoBoost@@YAXPEAU_KTHREAD@@PEAXT_AB_BOOST_BITMAP@1@PEAU_SINGLE_LIST_ENTRY@@@Z @ 0x14027C800 (-KiAbpUnboostThread@AutoBoost@@YAXPEAU_KTHREAD@@PEAXT_AB_BOOST_BITMAP@1@PEAU_SINGLE_LIST_ENTRY@@.c)
 *     ExfReleasePushLock @ 0x1402E3120 (ExfReleasePushLock.c)
 *     KiRaiseIrqlProcessIrqlFlags @ 0x1405209F0 (KiRaiseIrqlProcessIrqlFlags.c)
 *     KiRemoveSystemWorkPriorityKick @ 0x14052FA20 (KiRemoveSystemWorkPriorityKick.c)
 *     KeBugCheckEx @ 0x1405339B0 (KeBugCheckEx.c)
 */

void __fastcall ExReleasePushLockEx(unsigned __int64 a1, ULONG_PTR BugCheckParameter1)
{
  __int64 v2; // rdi
  char v3; // bl
  signed __int64 v5; // rax
  signed __int64 v6; // rdx
  __int64 v7; // rbp
  unsigned __int8 CurrentIrql; // r15
  struct _KTHREAD *CurrentThread; // r10
  int v10; // r12d
  unsigned __int64 v11; // rdx
  __int64 *AutoBoostThreadState; // rax
  __int64 v13; // r9
  unsigned int v14; // r11d
  __int64 v15; // rsi
  char v16; // al
  struct _KTHREAD *v17; // rcx
  char v18; // r8
  struct _KPRCB *CurrentPrcb; // rcx
  unsigned __int32 *SchedulerAssist; // r8
  signed __int8 v21; // al
  signed __int8 v22; // tt
  signed __int8 v23; // al
  signed __int8 v24; // tt
  char v25; // dl
  __int64 v26; // rbx
  struct _KPRCB *v27; // rcx
  unsigned __int32 *v28; // r8
  struct _KTHREAD *v29; // rdx
  char *v30; // r10
  unsigned int v31; // r8d
  unsigned __int64 v32; // r9
  LegacyAutoBoost *v33; // rcx
  unsigned __int32 v34; // eax
  unsigned __int32 v35; // ett
  unsigned __int32 v36; // eax
  __int64 v37; // rdx
  unsigned __int32 v38; // ett
  struct _KPRCB *v39; // rcx
  unsigned __int32 *v40; // r8
  unsigned __int32 v41; // eax
  __int64 v42; // rdx
  unsigned __int32 v43; // ett

  v2 = 0LL;
  v3 = BugCheckParameter1;
  if ( (BugCheckParameter1 & 0xFFFFFFF8) != 0 )
    KeBugCheckEx(0x152u, (unsigned int)BugCheckParameter1, a1, 0LL, 0LL);
  _m_prefetchw((const void *)a1);
  v5 = *(_QWORD *)a1;
  v6 = *(_QWORD *)a1 - 16LL;
  if ( (*(_QWORD *)a1 & 0xFFFFFFFFFFFFFFF0uLL) <= 0x10 )
    v6 = 0LL;
  if ( (v5 & 2) != 0 || v5 != _InterlockedCompareExchange64((volatile signed __int64 *)a1, v6, v5) )
    ExfReleasePushLock(a1, v6);
  if ( (v3 & 2) == 0 )
  {
    if ( (KiAbpGlobalState & 1) != 0 )
    {
      v7 = 0LL;
      CurrentIrql = -1;
      _disable();
      CurrentThread = KeGetCurrentThread();
      v10 = 1;
      v11 = 0LL;
      AutoBoostThreadState = (__int64 *)CurrentThread->AutoBoostThreadState;
      v13 = *AutoBoostThreadState;
      v14 = *(unsigned __int8 *)(*AutoBoostThreadState + 14);
      while ( 1 )
      {
        if ( (unsigned int)v11 >= v14 )
        {
          if ( (*((_DWORD *)&CurrentThread->0 + 1) & 0x10000) == 0 )
            KeBugCheckEx(0x162u, (ULONG_PTR)CurrentThread, a1, 0LL, 0LL);
          goto LABEL_32;
        }
        v15 = v13 + 48LL * (unsigned int)v11 + 16;
        v16 = *(_BYTE *)(v15 + 33);
        if ( (v16 & 0x10) == 0 )
        {
          v17 = *(struct _KTHREAD **)v15;
          v18 = *(_BYTE *)(v15 + 32) & 2;
          if ( v18 )
            v17 = (struct _KTHREAD *)v17->SListFaultAddress;
          if ( v17 == (struct _KTHREAD *)a1 && (v16 & 2) != 0 )
            break;
        }
        v11 = (unsigned int)(v11 + 1);
      }
      v7 = v13 + 48LL * (unsigned int)v11 + 16;
      *(_BYTE *)(v15 + 33) = v16 & 0xFC | 1;
      if ( v18 )
      {
        CurrentIrql = KeGetCurrentIrql();
        if ( CurrentIrql != 2 )
          __writecr8(2uLL);
        if ( KiIrqlFlags )
        {
          LOBYTE(v11) = 2;
          KiRaiseIrqlProcessIrqlFlags(CurrentIrql, v11);
        }
        CurrentPrcb = KeGetCurrentPrcb();
        SchedulerAssist = (unsigned __int32 *)CurrentPrcb->SchedulerAssist;
        if ( SchedulerAssist )
        {
          _m_prefetchw(SchedulerAssist);
          v34 = *SchedulerAssist;
          do
          {
            v11 = v34;
            LODWORD(v11) = v34 & 0xFFDFFFFF;
            v35 = v34;
            v34 = _InterlockedCompareExchange((volatile signed __int32 *)SchedulerAssist, v34 & 0xFFDFFFFF, v34);
          }
          while ( v35 != v34 );
          if ( (v34 & 0x200000) != 0 )
            KiRemoveSystemWorkPriorityKick(CurrentPrcb, v11, SchedulerAssist, v13);
        }
        _enable();
        v10 = 0;
        _m_prefetchw((const void *)(v15 + 35));
        v21 = *(_BYTE *)(v15 + 35);
        do
        {
          v22 = v21;
          v21 = _InterlockedCompareExchange8((volatile signed __int8 *)(v15 + 35), v21 | 1, v21);
        }
        while ( v22 != v21 );
        if ( (v21 & 1) != 0 )
        {
          do
          {
            while ( (*(_BYTE *)(v15 + 35) & 1) != 0 )
              _mm_pause();
            _m_prefetchw((const void *)(v15 + 35));
            v23 = *(_BYTE *)(v15 + 35);
            do
            {
              v24 = v23;
              v23 = _InterlockedCompareExchange8((volatile signed __int8 *)(v15 + 35), v23 | 1, v23);
            }
            while ( v24 != v23 );
          }
          while ( (v23 & 1) != 0 );
        }
        AutoBoost::KiAbpEntryUnlink((AutoBoost *)v15, (struct AutoBoost::_AB_THREAD_ENTRY *)v11);
        _InterlockedAnd8((volatile signed __int8 *)(v15 + 35), 0xFEu);
      }
      v25 = *(_BYTE *)(v15 + 36);
      v26 = *(_QWORD *)(v15 + 40);
      *(_DWORD *)(v15 + 40) = 0;
      *(_QWORD *)v15 = 0LL;
      *(_WORD *)(v15 + 32) = 0;
      *(_WORD *)(v15 - 48LL * (v25 & 0x7F) - 8) |= 1 << (v25 & 0xF);
      v2 = v26;
      if ( v10 )
      {
LABEL_32:
        v27 = KeGetCurrentPrcb();
        v28 = (unsigned __int32 *)v27->SchedulerAssist;
        if ( v28 )
        {
          _m_prefetchw(v28);
          v36 = *v28;
          do
          {
            v37 = v36;
            LODWORD(v37) = v36 & 0xFFDFFFFF;
            v38 = v36;
            v36 = _InterlockedCompareExchange((volatile signed __int32 *)v28, v36 & 0xFFDFFFFF, v36);
          }
          while ( v38 != v36 );
          if ( (v36 & 0x200000) != 0 )
            KiRemoveSystemWorkPriorityKick(v27, v37, v28, v13);
        }
        _enable();
        goto LABEL_39;
      }
      if ( KiIrqlFlags )
        KiLowerIrqlProcessIrqlFlags(KeGetCurrentIrql(), CurrentIrql);
      __writecr8(CurrentIrql);
LABEL_39:
      if ( (_DWORD)v2 )
        AutoBoost::KiAbpUnboostThread(*(_QWORD *)(v7 - 48LL * (*(_BYTE *)(v7 + 36) & 0x7F) - 16), a1, v2, 0LL);
    }
    else
    {
      v29 = KeGetCurrentThread();
      _disable();
      v30 = (char *)v29->AutoBoostThreadState;
      v31 = 0;
      v32 = a1 & 0x7FFFFFFFFFFFFFFCLL;
      while ( v31 < (unsigned __int8)v30[12] )
      {
        v33 = (LegacyAutoBoost *)&v30[88 * v31 + 16];
        if ( (*(_QWORD *)v33 & 0x7FFFFFFFFFFFFFFCLL) == v32 && *((_BYTE *)v33 + 10) && (*(_QWORD *)v33 & 1) == 0 )
        {
          *((_BYTE *)v33 + 10) = 0;
          if ( v33 )
          {
            LegacyAutoBoost::KiAbEntryFreeAndEnableInterrupts(
              v33,
              (struct _KLOCK_ENTRY *)v29,
              (struct _KTHREAD *)a1,
              (void *)1,
              0LL);
            return;
          }
          break;
        }
        ++v31;
      }
      if ( (*((_DWORD *)&v29->0 + 1) & 0x10000) == 0 )
        KeBugCheckEx(0x162u, (ULONG_PTR)v29, a1, 0LL, 0LL);
      v39 = KeGetCurrentPrcb();
      v40 = (unsigned __int32 *)v39->SchedulerAssist;
      if ( v40 )
      {
        _m_prefetchw(v40);
        v41 = *v40;
        do
        {
          v42 = v41;
          LODWORD(v42) = v41 & 0xFFDFFFFF;
          v43 = v41;
          v41 = _InterlockedCompareExchange((volatile signed __int32 *)v40, v41 & 0xFFDFFFFF, v41);
        }
        while ( v43 != v41 );
        if ( (v41 & 0x200000) != 0 )
          KiRemoveSystemWorkPriorityKick(v39, v42, v40, v32);
      }
      _enable();
    }
  }
}
