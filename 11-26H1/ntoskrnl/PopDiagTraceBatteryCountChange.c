/*
 * XREFs of PopDiagTraceBatteryCountChange @ 0x1407D2E84
 * Callers:
 *     PopBatteryApplyCompositeState @ 0x140946EB0 (PopBatteryApplyCompositeState.c)
 * Callees:
 *     EtwEventEnabled @ 0x140212D90 (EtwEventEnabled.c)
 *     EtwWrite @ 0x140212EF0 (EtwWrite.c)
 *     __security_check_cookie @ 0x140722910 (__security_check_cookie.c)
 *     SshpSessionManagerSendControlEvent @ 0x140B2FF48 (SshpSessionManagerSendControlEvent.c)
 */

__int64 __fastcall PopDiagTraceBatteryCountChange(int a1, int a2, int a3)
{
  bool v4; // [rsp+38h] [rbp-69h] BYREF
  bool v5; // [rsp+40h] [rbp-61h] BYREF
  int v6; // [rsp+48h] [rbp-59h] BYREF
  int v7; // [rsp+50h] [rbp-51h] BYREF
  int v8; // [rsp+58h] [rbp-49h] BYREF
  _QWORD v9[10]; // [rsp+68h] [rbp-39h] BYREF
  struct _EVENT_DATA_DESCRIPTOR UserData; // [rsp+B8h] [rbp+17h] BYREF
  int *v11; // [rsp+C8h] [rbp+27h]
  __int64 v12; // [rsp+D0h] [rbp+2Fh]
  int *v13; // [rsp+D8h] [rbp+37h]
  __int64 v14; // [rsp+E0h] [rbp+3Fh]
  int v15; // [rsp+108h] [rbp+67h] BYREF
  int v16; // [rsp+110h] [rbp+6Fh] BYREF
  int v17; // [rsp+118h] [rbp+77h] BYREF

  v17 = a3;
  v16 = a2;
  v15 = a1;
  if ( byte_140E67628
    && EtwEventEnabled(
         *(REGHANDLE *)&PopSleepstudySessionLock.PriorityFloorCounts[16],
         &POP_ETW_EVENT_BATTERY_COUNT_CHANGE) )
  {
    UserData.Ptr = (ULONGLONG)&v15;
    *(_QWORD *)&UserData.Size = 4LL;
    v11 = &v16;
    v12 = 4LL;
    v13 = &v17;
    v14 = 4LL;
    EtwWrite(
      *(REGHANDLE *)&PopSleepstudySessionLock.PriorityFloorCounts[16],
      &POP_ETW_EVENT_BATTERY_COUNT_CHANGE,
      0LL,
      3u,
      &UserData);
  }
  v9[1] = 4LL;
  v9[3] = 4LL;
  v4 = PopLidOpened != 0;
  v5 = PopConsoleExternalDisplayConnected != 0;
  v8 = v17;
  v7 = v16;
  v6 = v15;
  v9[0] = &v6;
  v9[2] = &v7;
  v9[4] = &v8;
  v9[6] = &v4;
  v9[8] = &v5;
  v9[5] = 4LL;
  v9[7] = 1LL;
  v9[9] = 1LL;
  return SshpSessionManagerSendControlEvent(SLEEPSTUDY_EVT_BATTERY_COUNT_CHANGE_CONTROL_EVENT, 5LL, v9);
}
