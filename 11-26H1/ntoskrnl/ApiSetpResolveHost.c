/*
 * XREFs of ApiSetpResolveHost @ 0x14051CD10
 * Callers:
 *     ApiSetResolveToHost @ 0x140529334 (ApiSetResolveToHost.c)
 * Callees:
 *     ApiSetpGetContractKeyInfo @ 0x14052953C (ApiSetpGetContractKeyInfo.c)
 *     ApiSetpGetSearchKeyHash @ 0x1406DB28C (ApiSetpGetSearchKeyHash.c)
 *     ApiSetpSearchForHostOverrideIndex_V7 @ 0x1406DB45C (ApiSetpSearchForHostOverrideIndex_V7.c)
 *     ApiSetpSearchForSectionIndex_V7 @ 0x1406DB504 (ApiSetpSearchForSectionIndex_V7.c)
 */

__int64 __fastcall ApiSetpResolveHost(__int64 a1, __int64 a2, __int64 a3, __int64 a4, __int64 a5, char *a6, __int64 a7)
{
  unsigned __int16 v8; // ax
  char v9; // r15
  unsigned __int16 v10; // ax
  int v11; // r8d
  int v12; // r9d
  __int64 v13; // r13
  __int64 v14; // rsi
  __int64 v15; // r14
  __int64 v16; // rcx
  unsigned __int16 v17; // r8
  unsigned __int16 v18; // r9
  unsigned __int16 v19; // dx
  char v20; // al
  unsigned __int16 v21; // ax
  int v22; // r8d
  int v23; // r9d
  __int64 v24; // rsi
  __int64 v25; // r14
  char v26; // al
  __int64 v27; // rcx
  bool v28; // al
  unsigned __int16 v29; // ax
  int v30; // edx
  int v31; // r8d
  int v32; // r9d
  __int64 v33; // rsi
  __int64 v34; // r14
  unsigned int v35; // r10d
  unsigned __int64 v36; // rdx
  __int16 v37; // ax
  unsigned int v38; // eax
  __int64 v39; // rcx
  __int64 v40; // rdx
  int v42; // [rsp+20h] [rbp-40h]
  __int128 v43; // [rsp+30h] [rbp-30h] BYREF
  __int128 v44; // [rsp+40h] [rbp-20h]
  __int64 v45; // [rsp+50h] [rbp-10h]

  v8 = a3;
  v45 = 0LL;
  LOBYTE(a3) = 1;
  *(_OWORD *)a7 = 0LL;
  v9 = 0;
  v43 = 0LL;
  v44 = 0LL;
  if ( !(unsigned __int8)ApiSetpGetContractKeyInfo(a2, v8, a3, &v43) )
    goto LABEL_27;
  if ( HIDWORD(v43) == 1 )
  {
    v29 = ApiSetpSearchForSectionIndex_V7(a1, a1 + 44, &v43);
    if ( v29 == 0xFFFF )
      goto LABEL_27;
    v13 = *(unsigned __int16 *)(a1 + 18);
    LOBYTE(v30) = 1;
    v33 = *(unsigned int *)(a1 + 44);
    LOBYTE(v32) = (*(_BYTE *)(a1 + 2) & 0x10) != 0;
    LOBYTE(v31) = (*(_BYTE *)(a1 + 2) & 8) != 0;
    v34 = a1 + v29 * (unsigned __int64)*(unsigned __int8 *)(a1 + 52) - v13;
    if ( *(_DWORD *)(v33 + v34) != (unsigned int)ApiSetpGetSearchKeyHash(
                                                   (unsigned int)&v43,
                                                   v30,
                                                   v31,
                                                   v32,
                                                   *(_BYTE *)(a1 + 55)) )
      goto LABEL_27;
    v17 = *(_WORD *)(v33 + v34 + 8);
    v18 = *(_WORD *)(v33 + v34 + 4);
    v19 = *(_WORD *)(v33 + v34 + 6);
    v20 = *(_BYTE *)(v33 + v34 + 10);
    goto LABEL_20;
  }
  if ( HIDWORD(v43) != 2 )
  {
    if ( HIDWORD(v43) != 3 )
      goto LABEL_27;
    v10 = ApiSetpSearchForSectionIndex_V7(a1, a1 + 68, &v43);
    if ( v10 == 0xFFFF )
      goto LABEL_27;
    v13 = *(unsigned __int16 *)(a1 + 18);
    v14 = *(unsigned int *)(a1 + 68);
    LOBYTE(v12) = (*(_BYTE *)(a1 + 2) & 0x10) != 0;
    LOBYTE(v11) = (*(_BYTE *)(a1 + 2) & 8) != 0;
    v15 = a1 + v10 * (unsigned __int64)*(unsigned __int8 *)(a1 + 76) - v13;
    if ( *(_DWORD *)(v14 + v15) != (unsigned int)ApiSetpGetSearchKeyHash(
                                                   (unsigned int)&v43,
                                                   0,
                                                   v11,
                                                   v12,
                                                   *(_BYTE *)(a1 + 79))
      || (*(_BYTE *)(v14 + v15 + 10) & 4) != 0 && (unsigned int)WORD4(v44) > *(unsigned __int8 *)(v14 + v15 + 9) )
    {
      goto LABEL_27;
    }
    v16 = *(unsigned int *)(a1 + 44)
        + *(unsigned __int8 *)(a1 + 52) * (unsigned __int64)*(unsigned __int16 *)(v14 + v15 + 4)
        - v13;
    v17 = *(_WORD *)(v16 + a1 + 8);
    v18 = *(_WORD *)(v16 + a1 + 4);
    v19 = *(_WORD *)(v16 + a1 + 6);
    v20 = *(_BYTE *)(v16 + a1 + 10);
LABEL_20:
    v28 = (v20 & 2) != 0;
    goto LABEL_21;
  }
  v9 = 1;
  v21 = ApiSetpSearchForSectionIndex_V7(a1, a1 + 56, &v43);
  if ( v21 != 0xFFFF )
  {
    v13 = *(unsigned __int16 *)(a1 + 18);
    v24 = *(unsigned int *)(a1 + 56);
    LOBYTE(v23) = (*(_BYTE *)(a1 + 2) & 0x10) != 0;
    LOBYTE(v22) = (*(_BYTE *)(a1 + 2) & 8) != 0;
    v25 = a1 + v21 * (unsigned __int64)*(unsigned __int8 *)(a1 + 64) - v13;
    if ( *(_DWORD *)(v24 + v25) != (unsigned int)ApiSetpGetSearchKeyHash(
                                                   (unsigned int)&v43,
                                                   0,
                                                   v22,
                                                   v23,
                                                   *(_BYTE *)(a1 + 67)) )
    {
      v9 = 0;
      goto LABEL_27;
    }
    v26 = *(_BYTE *)(v24 + v25 + 6);
    if ( (v26 & 4) == 0 || (v26 & 2) == 0 && !*(_DWORD *)(v24 + v25 + 8) )
    {
      v27 = *(unsigned int *)(a1 + 44)
          + *(unsigned __int8 *)(a1 + 52) * (unsigned __int64)*(unsigned __int16 *)(v24 + v25 + 4)
          - v13;
      v17 = *(_WORD *)(v27 + a1 + 8);
      v18 = *(_WORD *)(v27 + a1 + 4);
      v19 = *(_WORD *)(v27 + a1 + 6);
      v28 = (*(_BYTE *)(v27 + a1 + 10) & 2) != 0;
LABEL_21:
      v35 = v19;
      v9 = 1;
      if ( v28 )
      {
        v36 = *(unsigned int *)(a1 + 92)
            + v18 * (unsigned __int64)*(unsigned __int8 *)(a1 + 100)
            - (unsigned __int16)v13;
        if ( *(_BYTE *)(v36 + a1 + 10) <= *(_BYTE *)(a1 + 3) )
        {
          *(_QWORD *)(a7 + 8) = a1
                              + *(unsigned int *)(v36 + a1 + 4)
                              - (unsigned __int64)(unsigned __int16)v13
                              + *(unsigned int *)(a1 + 36);
          v37 = 2 * *(_WORD *)(v36 + a1 + 8);
          *(_WORD *)a7 = v37;
          *(_WORD *)(a7 + 2) = v37;
          if ( a5 )
          {
            if ( v17 )
            {
              LOWORD(v42) = *(_WORD *)a5 >> 1;
              v38 = ApiSetpSearchForHostOverrideIndex_V7(a1, v35, v17, *(_QWORD *)(a5 + 8), v42);
              if ( v38 != 0xFFFF )
              {
                v39 = *(unsigned __int16 *)(a1 + 18);
                v40 = *(unsigned int *)(a1 + 104) + v38 * (unsigned __int64)*(unsigned __int16 *)(a1 + 114) - v39;
                *(_QWORD *)(a7 + 8) = a1 + *(unsigned int *)(v40 + a1 + 8) - v39 + *(unsigned int *)(a1 + 36);
                *(_WORD *)(a7 + 2) = 2 * *(_WORD *)(v40 + a1 + 14);
                *(_WORD *)a7 = 2 * *(_WORD *)(v40 + a1 + 14);
              }
            }
          }
        }
      }
    }
  }
LABEL_27:
  *a6 = v9;
  return 0LL;
}
