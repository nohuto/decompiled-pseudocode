/*
 * XREFs of __asan_store8_volatile @ 0x1405E3B80
 * Callers:
 *     <none>
 * Callees:
...
void __fastcall _asan_store8_volatile(ULONG_PTR BugCheckParameter1)
{
  signed __int8 v2; // cl
  char v3; // dl
  char v4; // dl
  ...
  if ( (BugCheckParameter1 & 7) == 0 )
  {
    if ( !v2 || v2 >= 8 )
      goto LABEL_18;
LABEL_35:
    KasaniReport(BugCheckParameter1, 8uLL, 1u, retaddr, v2);
  }
  if ( v2 && v2 < 8 )
    goto LABEL_27;
...
LABEL_18:
  if ( v2 == 9 && KeGetCurrentIrql() >= 2u )
    KasaniReport(BugCheckParameter1, 8uLL, 1u, retaddr, 9u);
  if ( KcsaniValidationEnabled )
    KcsanValidateAddress(BugCheckParameter1, 8, 1, 1, retaddr);
}
