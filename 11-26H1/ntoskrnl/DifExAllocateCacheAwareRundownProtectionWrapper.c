/*
 * XREFs of DifExAllocateCacheAwareRundownProtectionWrapper @ 0x140653100
 * Callers:
 *     <none>
 * Callees:
...
PEX_RUNDOWN_REF_CACHE_AWARE __fastcall DifExAllocateCacheAwareRundownProtectionWrapper(
        ...
        ULONG PoolTag)
{
  ...
  PEX_RUNDOWN_REF_CACHE_AWARE CacheAwareRundownProtection; // [rsp+30h] [rbp-28h]
  _UNKNOWN *retaddr; // [rsp+58h] [rbp+0h]

  v15 = 0LL;
  CacheAwareRundownProtection = 0LL;
  ...
  if ( APIThunkContextById )
  {
    ...
    v8 = 0;
    *((_QWORD *)&v15 + 1) = __PAIR64__(PoolType, PoolTag);
  ...
  CacheAwareRundownProtection = ExAllocateCacheAwareRundownProtection((POOL_TYPE)PoolType, PoolTag);
  if ( v6 )
  {
  ...
  return CacheAwareRundownProtection;
}
