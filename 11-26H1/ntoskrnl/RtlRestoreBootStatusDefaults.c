/*
 * XREFs of RtlRestoreBootStatusDefaults @ 0x140805A88
 * Callers:
...
__int64 __fastcall RtlRestoreBootStatusDefaults(HANDLE FileHandle)
{
  ...
  *(_WORD *)((char *)Buffer + 9) = 286;
  ...
  if ( v5 >= 0 )
  {
    ...
    {
      if ( (HANDLE)NormalizationListLock.CycleTime == FileHandle )
      {
        KernelStack = NormalizationListLock.KernelStack;
        if ( NormalizationListLock.KernelStack )
        {
          *(_OWORD *)NormalizationListLock.KernelStack = Buffer[0];
          KernelStack[1] = Buffer[1];
          KernelStack[2] = Buffer[2];
          KernelStack[3] = v12;
          KernelStack[4] = v13;
          KernelStack[5] = v14;
          KernelStack[6] = v15;
