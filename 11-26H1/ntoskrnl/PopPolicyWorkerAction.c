/*
 * XREFs of PopPolicyWorkerAction @ 0x140A37680
 * Callers:
 *     <none>
 * Callees:
 *     EtwTraceKernelEvent @ 0x1402DAC90 (EtwTraceKernelEvent.c)
 *     PopThermalSxExit @ 0x140435AC4 (PopThermalSxExit.c)
 *     PopGetPolicyWorker @ 0x14043864C (PopGetPolicyWorker.c)
 *     PopSetPowerActionState @ 0x1404F4374 (PopSetPowerActionState.c)
 *     _guard_dispatch_icall_no_overrides @ 0x1407311E0 (_guard_dispatch_icall_no_overrides.c)
 *     PopIssueActionRequest @ 0x140A37878 (PopIssueActionRequest.c)
 *     PopCompleteAction @ 0x140A386C8 (PopCompleteAction.c)
 *     PopAcquirePolicyLock @ 0x140C04BF0 (PopAcquirePolicyLock.c)
 *     PopReleasePolicyLock @ 0x140C04C40 (PopReleasePolicyLock.c)
 */

__int64 __fastcall PopPolicyWorkerAction(__int64 a1, __int64 a2)
{
  char v2; // si
  char v3; // bp
  __int64 v4; // rdx
  __int64 v5; // rcx
  __int64 v6; // r8
  __int64 v7; // r9
  __int64 v8; // r15
  unsigned int v9; // edi
  unsigned int v10; // ebx
  int v11; // r14d
  unsigned int v12; // edi
  __int64 *i; // rbx
  __int64 v14; // rdx
  __int64 v15; // rcx
  __int64 v17; // [rsp+20h] [rbp-48h]
  __int64 v18; // [rsp+30h] [rbp-38h] BYREF
  unsigned int v19; // [rsp+38h] [rbp-30h]
  int v20; // [rsp+3Ch] [rbp-2Ch]
  _QWORD v21[2]; // [rsp+40h] [rbp-28h] BYREF

  v2 = 0;
  v3 = 0;
  PopAcquirePolicyLock(a1, a2);
  qword_140F0FD28 = (__int64)KeGetCurrentThread();
  if ( byte_140F0FB61 == 1 )
  {
    v8 = (int)qword_140F0FB64;
    v9 = HIDWORD(qword_140F0FB64);
    v10 = dword_140F0FB6C;
    PopSetPowerActionState(2);
    v11 = dword_140F0FB74;
    v12 = PopIssueActionRequest(0LL, (unsigned int)v8, v9, v10);
    v3 = 1;
    if ( (byte_140F0FB60 & 2) != 0 )
    {
      if ( HIDWORD(qword_140F0FB64) == 5 )
        v11 = 6;
      dword_140F0FB74 = v11;
      dword_140F0FB6C = dword_140F0FB6C & 0x7FFFFFDC | 0x80000020;
      LOBYTE(byte_140F0FB60) = byte_140F0FB60 & 0xFD;
      PopSetPowerActionState(1);
      PopGetPolicyWorker(2);
    }
    else
    {
      if ( (xmmword_140FBFC10 & 0x8000) != 0 )
      {
        v20 = 0;
        v21[0] = &v18;
        v18 = v8;
        v19 = v12;
        v21[1] = 16LL;
        EtwTraceKernelEvent((int)v21, 1, 0x80008000, 4643, 5249026);
      }
      dword_140F0FB70 = v12;
      PopSetPowerActionState(0);
      for ( i = (__int64 *)qword_140F105B0; i != &qword_140F105B0; i = (__int64 *)*i )
        PopCompleteAction(i[2], v12);
      PopGetPolicyWorker(1);
      v2 = 1;
    }
  }
  qword_140F0FD28 = 0LL;
  PopReleasePolicyLock(v5, v4, v6, v7, v17);
  if ( v2 )
    PopThermalSxExit();
  LOBYTE(stru_140F10828.OtherOperationCount) = 0;
  if ( v3 && qword_140E67530 )
    guard_dispatch_icall_no_overrides(v15, v14);
  if ( KeGetCurrentThread()->WaitBlock[3].SpareLong )
    __fastfail(0x20u);
  return 0LL;
}
