/*
 * XREFs of PopRequestPowerIrp @ 0x14026FDC4
 * Callers:
 *     PopFxReleasePowerIrp @ 0x1403B60D8 (PopFxReleasePowerIrp.c)
 *     PopScanIdleList @ 0x1404C65A0 (PopScanIdleList.c)
 *     PoRequestPowerIrp @ 0x1404CD690 (PoRequestPowerIrp.c)
 *     DifPoRequestPowerIrpWrapper @ 0x140694330 (DifPoRequestPowerIrpWrapper.c)
 * Callees:
 *     PopDiagTraceIrpStart @ 0x140218C6C (PopDiagTraceIrpStart.c)
 *     PopAllocateIrp @ 0x14026F82C (PopAllocateIrp.c)
 *     PopQueueQuerySetIrp @ 0x1403B2B18 (PopQueueQuerySetIrp.c)
 *     PopPepUpdateConstraints @ 0x1403B37EC (PopPepUpdateConstraints.c)
 *     PopFxDereferenceDevice @ 0x1403B61F4 (PopFxDereferenceDevice.c)
 *     PopLogNotifyDevice @ 0x14046AC48 (PopLogNotifyDevice.c)
 *     PopFxGetDeviceDStateReason @ 0x1404C430C (PopFxGetDeviceDStateReason.c)
 *     IofCallDriverSpecifyReturn @ 0x1404DC7D0 (IofCallDriverSpecifyReturn.c)
 *     PopFxNotifyPreDIrpIssue @ 0x140518984 (PopFxNotifyPreDIrpIssue.c)
 */

__int64 __fastcall PopRequestPowerIrp(void *a1, __int64 a2, int a3, __int64 a4, __int64 a5, char a6, PIRP Irp)
{
  volatile __int64 *p_Type; // rbp
  char v9; // r14
  char v11; // cl
  __int64 result; // rax
  __int64 v13; // r8
  __int64 v14; // r9
  PIRP v15; // rdi
  struct _IO_STACK_LOCATION *CurrentStackLocation; // rax
  volatile signed __int32 *v17; // rcx
  __int64 v18; // rsi
  struct _IO_STACK_LOCATION *v19; // rbp
  __int64 v20; // rcx
  int v21; // ecx
  int v22; // eax
  __int64 v23; // rdx
  signed __int32 v24; // eax
  signed __int32 v25; // ett
  char v26; // al
  __int64 v27; // rcx
  __int64 v28; // [rsp+60h] [rbp-28h] BYREF
  void *retaddr; // [rsp+88h] [rbp+0h]

  p_Type = (volatile __int64 *)&Irp->Type;
  v28 = 0LL;
  v9 = a2;
  if ( Irp )
    _InterlockedExchange64((volatile __int64 *)&Irp->Type, 0LL);
  Irp = 0LL;
  if ( !(_BYTE)a2 )
    goto LABEL_4;
  if ( (unsigned __int8)(a2 - 2) > 1u )
    return 3221225712LL;
  if ( (_BYTE)xmmword_140F0FAE0 )
    v11 = 1;
  else
LABEL_4:
    v11 = 0;
  result = PopAllocateIrp(a1, a2, a2, (_BYTE)a2 != 0, a3, v11, a6, a4, a5, &Irp, &v28);
  if ( (int)result >= 0 && (_DWORD)result != 259 )
  {
    v15 = Irp;
    if ( Irp )
    {
      CurrentStackLocation = Irp->Tail.Overlay.CurrentStackLocation;
      v17 = (volatile signed __int32 *)&PopRequestCompletion;
      v18 = v28;
      CurrentStackLocation[-1].CompletionRoutine = (PIO_COMPLETION_ROUTINE)&PopRequestCompletion;
      CurrentStackLocation[-1].Context = (PVOID)v18;
      CurrentStackLocation[-1].Control = -32;
      if ( p_Type )
        _InterlockedExchange64(p_Type, (__int64)v15);
      v19 = v15->Tail.Overlay.CurrentStackLocation;
      if ( v9 )
      {
        v19[-1].Parameters.Create.Options = 1;
        v19[-1].Parameters.Read.ByteOffset.LowPart = a3;
        if ( (_BYTE)xmmword_140F0FAE0 && (!*(_QWORD *)(v18 + 200) || (unsigned int)PopFxGetDeviceDStateReason() == 1) )
        {
          v21 = 3;
          v22 = WORD2(xmmword_140F0FAE0) >> 12;
          if ( DWORD2(xmmword_140F0FAE0) == 7 )
          {
            v21 = 3 - (v22 != 5);
          }
          else if ( v22 != 5 )
          {
            v21 = DWORD2(xmmword_140F0FAE0);
          }
          v19[-1].Parameters.Create.EaLength = v21;
          v19[-1].Parameters.Read.Length = DWORD1(xmmword_140F0FAE0);
        }
        if ( (xmmword_140FBFC10 & 0x8000) != 0 )
          PopLogNotifyDevice(a1, 0LL, v15);
        v20 = *(_QWORD *)(v18 + 200);
        if ( !v20 || v9 != 2 || (unsigned __int8)PopFxNotifyPreDIrpIssue(v20, v15) )
          PopQueueQuerySetIrp(v15);
      }
      else
      {
        v23 = *(_QWORD *)(v18 + 200);
        if ( v23 )
        {
          _m_prefetchw((const void *)(v23 + 32));
          v24 = *(_DWORD *)(v23 + 32);
          do
          {
            v25 = v24;
            v24 = _InterlockedCompareExchange((volatile signed __int32 *)(v23 + 32), v24, v24);
          }
          while ( v25 != v24 );
          v26 = v24 & 1;
          *(_BYTE *)(v18 + 208) = v26;
          v26 ^= 1u;
          *(_BYTE *)(v18 + 209) = v26;
          v27 = *(_QWORD *)(v18 + 200);
          if ( v26 )
          {
            v17 = *(volatile signed __int32 **)(v27 + 56);
            if ( _InterlockedIncrement(v17 + 32) == 1 )
              PopPepUpdateConstraints(v17, 5LL, 0LL);
          }
          else
          {
            PopFxDereferenceDevice(v27, 0LL, v13, v14);
            *(_QWORD *)(v18 + 200) = 0LL;
          }
        }
        v19[-1].Parameters.Read.Length = a3;
        PopDiagTraceIrpStart((__int64)v17, (__int64)v15);
        IofCallDriverSpecifyReturn(v19[-1].DeviceObject, v15, retaddr);
      }
      return 259LL;
    }
    else
    {
      return 3221225626LL;
    }
  }
  return result;
}
