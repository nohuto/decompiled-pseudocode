/*
 * XREFs of ObpCreateDefaultObjectTypeSD @ 0x1407C3E28
 * Callers:
...
__int64 __fastcall ObpCreateDefaultObjectTypeSD(__int64 *a1)
{
  ...
  v2 = RtlLengthSid(RtlpBootStatHandleLock.StateSaveArea);
  v3 = RtlLengthSid(SeAliasAdminsSid) + v2;
  v4 = v3 + RtlLengthSid(*(PSID *)&RtlpBootStatHandleLock.WaitRegister.Flags) + 44;
  ...
  if ( Pool2 )
  {
    ...
    *(_BYTE *)Pool2 = 1;
    ...
    if ( Acl < 0
      || (Acl = RtlpAddKnownAce((int)v6 + 20, 2, 0, 983041, RtlpBootStatHandleLock.StateSaveArea, 0), Acl < 0)
