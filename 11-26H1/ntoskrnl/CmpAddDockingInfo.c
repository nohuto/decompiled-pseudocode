/*
 * XREFs of CmpAddDockingInfo @ 0x140CE9638
 * Callers:
...
__int64 __fastcall CmpAddDockingInfo(__int64 a1, __int64 a2)
{
  ...
  LODWORD(Size) = 4;
  result = NtSetValueKey(a1, (__int64)&CmpStrDockingStateString, 0LL, 4, &v9, Size);
  if ( (int)result >= 0 )
  {
    LODWORD(v9) = *(unsigned __int16 *)(a2 + 6);
    LODWORD(Sizea) = 4;
    result = NtSetValueKey(a1, (__int64)&CmpStrCapabilitiesString, 0LL, 4, &v9, Sizea);
    if ( (int)result >= 0 )
    {
      LODWORD(v9) = *(_DWORD *)(a2 + 8);
      LODWORD(Sizeb) = 4;
      result = NtSetValueKey(a1, (__int64)&CmpStrDockIdString, 0LL, 4, &v9, Sizeb);
      if ( (int)result >= 0 )
      {
        LODWORD(v9) = *(_DWORD *)(a2 + 12);
        LODWORD(Sizec) = 4;
        return NtSetValueKey(a1, (__int64)&CmpStrSerialNumberString, 0LL, 4, &v9, Sizec);
      }
    }
  }
  return result;
