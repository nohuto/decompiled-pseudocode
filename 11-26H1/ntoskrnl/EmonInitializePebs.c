/*
 * XREFs of EmonInitializePebs @ 0x140BED300
 * Callers:
      ...
      if ( (v5 & 0x1000) == 0 )
      {
        ...
        if ( (_DWORD)v7 )
        {
          ...
          if ( v8 )
          {
            ...
            {
              if ( v9 == 1 )
                LODWORD(IommuInterfaceStateChangeCallbackPushLock.ReadOperationCount) = 200;
              else
                LODWORD(IommuInterfaceStateChangeCallbackPushLock.ReadOperationCount) = 0;
            }
            else
            {
              LODWORD(IommuInterfaceStateChangeCallbackPushLock.ReadOperationCount) = 192;
            }
          }
          else
          {
            LODWORD(IommuInterfaceStateChangeCallbackPushLock.ReadOperationCount) = 176;
          }
        }
        else
        {
          LODWORD(IommuInterfaceStateChangeCallbackPushLock.ReadOperationCount) = 144;
        }
