/*
 * XREFs of ExReleaseDisownedFastResourceShared @ 0x1406D2350
 * Callers:
...
LONG_PTR __fastcall ExReleaseDisownedFastResourceShared(ULONG_PTR BugCheckParameter2, ULONG_PTR a2)
{
  ...
  if ( (*(_QWORD *)BugCheckParameter2 & 1) != 0 )
    return ExpReleaseDisownedFastResourceExclusive(BugCheckParameter2, a2);
  else
    return ExpReleaseDisownedFastResourceShared(BugCheckParameter2, a2);
}
