/*
 * XREFs of MiScrubNode @ 0x140AC8020
 * Callers:
 *     MiScrubMemoryWorker @ 0x140884240 (MiScrubMemoryWorker.c)
 * Callees:
 *     MiPfnsWorthTrying @ 0x140290280 (MiPfnsWorthTrying.c)
 *     MiClaimPhysicalRun @ 0x140290C60 (MiClaimPhysicalRun.c)
 *     MiAdjustCachedStacks @ 0x1402A7914 (MiAdjustCachedStacks.c)
 *     MiScrubPage @ 0x140408114 (MiScrubPage.c)
 *     MiMakePageBad @ 0x1404B42D8 (MiMakePageBad.c)
 *     MiScrubInterrupted @ 0x1404C3F28 (MiScrubInterrupted.c)
 *     MiScrubLargePageRegions @ 0x140AC8350 (MiScrubLargePageRegions.c)
 */

void __fastcall MiScrubNode(__int64 a1)
{
  __int64 v1; // rdi
  unsigned int *v3; // r8
  __int64 v4; // rbx
  __int64 v5; // r14
  unsigned int *v6; // r10
  unsigned int v7; // r9d
  unsigned __int64 v8; // rbp
  __int64 v9; // r15
  unsigned int i; // r13d
  __int64 v11; // rax
  __int64 v12; // rcx
  unsigned __int64 v13; // r12
  unsigned __int64 v14; // rbx
  unsigned __int64 j; // rdi
  __int64 v16; // r11
  unsigned __int64 v17; // rax
  int v18; // eax
  __int64 v19; // rax
  __int64 v20; // r8
  unsigned __int64 v21; // rcx
  __int64 v22; // rax
  __int64 v23; // [rsp+50h] [rbp-58h]
  int v24; // [rsp+B0h] [rbp+8h] BYREF
  __int64 v25; // [rsp+B8h] [rbp+10h]
  __int64 v26; // [rsp+C0h] [rbp+18h]
  unsigned int *v27; // [rsp+C8h] [rbp+20h]

  v1 = *(_QWORD *)a1;
  v24 = 0;
  v23 = v1;
  v3 = *(unsigned int **)(v1 + 80);
  v25 = *(_QWORD *)(v1 + 48);
  v4 = *v3;
  v26 = v4;
  LODWORD(v5) = v4;
  v6 = &v3[4 * v4 + 4];
  v27 = v6;
  v7 = -1;
  while ( (_DWORD)v5 )
  {
    v5 = (unsigned int)(v5 - 1);
    if ( v6[2 * v5] == *(_DWORD *)(a1 + 8) )
    {
      v8 = *(_QWORD *)(a1 + 192);
      v7 = v5;
      v21 = *(_QWORD *)&v3[4 * (unsigned int)v5 + 4];
      v22 = *(_QWORD *)&v3[4 * (unsigned int)v5 + 6];
      if ( v8 >= v21 || v8 == -1LL )
      {
        if ( v8 >= v22 + v21 - 1 )
        {
          v8 = v22 + v21 - 1;
        }
        else
        {
          LODWORD(v4) = v4 + 1;
          v26 = (unsigned int)v4;
        }
        if ( v8 != -1LL )
          goto LABEL_6;
        break;
      }
    }
  }
  if ( v7 == -1 )
    return;
  LODWORD(v5) = v7;
  v8 = *(_QWORD *)&v3[4 * v7 + 6] + *(_QWORD *)&v3[4 * v7 + 4] - 1LL;
LABEL_6:
  LODWORD(v9) = v5 + 1;
  for ( i = 0; i < (unsigned int)v4; ++i )
  {
    if ( !(_DWORD)v9 )
      LODWORD(v9) = **(_DWORD **)(v1 + 80);
    v9 = (unsigned int)(v9 - 1);
    if ( v6[2 * v9] == *(_DWORD *)(a1 + 8) )
    {
      v11 = *(_QWORD *)(v1 + 80);
      v12 = 16LL * (unsigned int)v9;
      v13 = *(_QWORD *)(v12 + v11 + 16);
      v14 = *(_QWORD *)(v12 + v11 + 24) + v13 - 1;
      if ( (_DWORD)v9 == (_DWORD)v5 )
      {
        if ( i )
          v13 = v8 + 1;
        else
          v14 = v8;
      }
      for ( j = 48 * v14 - 0x220000000000LL; ; j -= 48LL )
      {
        if ( v14 < v13 || !v13 )
        {
LABEL_34:
          v6 = v27;
          LODWORD(v4) = v26;
          v1 = v23;
          break;
        }
        if ( MiScrubInterrupted(a1) )
        {
          *(_QWORD *)(56320LL * *(unsigned int *)(a1 + 8) + *(_QWORD *)(v16 + 16) + 14160) = v14;
          return;
        }
        v17 = MiPfnsWorthTrying((__int16 *)v16, j, 1LL, 0x7000000u, &v24);
        if ( v17 )
        {
          if ( v17 >= v14 - v13 + 1 )
            goto LABEL_34;
          j += 48 - 48 * v17;
          v14 += 1 - v17;
          v19 = MiScrubLargePageRegions(a1, j);
          if ( v19 )
          {
            v14 += v19;
            if ( MiScrubInterrupted(a1) )
            {
              *(_QWORD *)(56320LL * *(unsigned int *)(a1 + 8) + *(_QWORD *)(v25 + 16) + 14160) = v14 - 1;
              return;
            }
            j += 48 * v20;
          }
        }
        else
        {
          if ( (v24 & 1) != 0 )
            MiAdjustCachedStacks(v25, 1);
          if ( !MiClaimPhysicalRun(v25, v14, 1LL, qword_140E2D920, a1 + 48, 117440512, 0, 0LL, 0LL, &v24) )
          {
            v18 = MiScrubPage(a1, v14, 0, 0LL, 0);
            MiMakePageBad(v14, ((v18 >> 31) & 1) + 2);
          }
        }
        --v14;
      }
    }
  }
}
