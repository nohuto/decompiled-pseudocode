/*
 * XREFs of IpmiLibpKcsSendRequestToBMC @ 0x1407229B0
 * Callers:
...
__int64 __fastcall IpmiLibpKcsSendRequestToBMC(__int64 a1, __int64 a2, unsigned __int16 a3, __int64 a4)
{
  ...
  HIDWORD(WheapConfigTableLock.Timer.Dpc) = 7;
  v7 = 0;
  if ( (unsigned int)IpmiLibpKcsSpinRegister(a1, a4, 2LL) )
    goto LABEL_22;
  IpmiLibReadOneByte(v8, 0);
  HIDWORD(WheapConfigTableLock.Timer.Dpc) = 0;
  ...
  if ( OneByte >= 0 || (OneByte & 0x40) != 0 )
    goto LABEL_22;
  IpmiLibReadOneByte(v8, 0);
  HIDWORD(WheapConfigTableLock.Timer.Dpc) = 1;
  while ( a3 > 1u )
  {
    IpmiLibWriteOneByte(v11, 0, *(_BYTE *)(v7 + a2));
    if ( (WheapConfigTableLock.Timer.Header.Type & 1) != 0 && (unsigned int)IpmiLibpHandleSendInterrupt(v8, a4) )
      goto LABEL_22;
    if ( (unsigned int)IpmiLibpKcsSpinRegister(v8, a4, 2LL) )
      goto LABEL_22;
  ...
  if ( (IpmiLibWriteOneByte(v11, 1u, 0x62u),
        HIDWORD(WheapConfigTableLock.Timer.Dpc) = 2,
        (WheapConfigTableLock.Timer.Header.Type & 1) != 0)
...
LABEL_22:
  ...
  else
  {
    ...
    HIDWORD(WheapConfigTableLock.Timer.Dpc) = 3;
    return 0;
  }
  return v15;
}
