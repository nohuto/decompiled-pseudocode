/*
 * XREFs of Feature_TestUex12__private_IsEnabledFallback @ 0x1406E79D0
 * Callers:
 *     Feature_TestUex12__private_IsEnabledNoReportingNoInline @ 0x1406E79F0 (Feature_TestUex12__private_IsEnabledNoReportingNoInline.c)
 * Callees:
 *     wil_details_IsEnabledFallback @ 0x1404FAECC (wil_details_IsEnabledFallback.c)
 */

__int64 __fastcall Feature_TestUex12__private_IsEnabledFallback(__int64 a1, unsigned int a2)
{
  return wil_details_IsEnabledFallback(a1, a2, (volatile signed __int32 **)&Feature_TestUex12__private_descriptor, 0);
}
