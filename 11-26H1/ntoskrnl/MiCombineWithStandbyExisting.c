/*
 * XREFs of MiCombineWithStandbyExisting @ 0x140307CC4
 * Callers:
 *     MiConvertStandbyToProto @ 0x140306E68 (MiConvertStandbyToProto.c)
 * Callees:
 *     MiUpdatePfnPriority @ 0x1402992A0 (MiUpdatePfnPriority.c)
 *     MiMakePrototypePteDirect @ 0x1402D27B0 (MiMakePrototypePteDirect.c)
 *     MiUnlockProtoPoolPage @ 0x1402D3E40 (MiUnlockProtoPoolPage.c)
 *     MiLockProtoPoolPage @ 0x1402F50D0 (MiLockProtoPoolPage.c)
 *     MiUnlinkPageFromListEx @ 0x1402F7250 (MiUnlinkPageFromListEx.c)
 *     MiIsDecayPfn @ 0x1402F9850 (MiIsDecayPfn.c)
 *     MiGetPfnSlabType @ 0x1402FDC40 (MiGetPfnSlabType.c)
 *     MiConfirmCombinePageContents @ 0x140308860 (MiConfirmCombinePageContents.c)
 *     MiLockAndDecrementShareCount @ 0x1403091DC (MiLockAndDecrementShareCount.c)
 *     MiPfnReferenceCountIsZero @ 0x14031C374 (MiPfnReferenceCountIsZero.c)
 *     MiWriteRepurposedTransitionPte @ 0x140342030 (MiWriteRepurposedTransitionPte.c)
 *     MiDiscardTransitionPfnEx @ 0x140369FE0 (MiDiscardTransitionPfnEx.c)
 *     MiMapSinglePage @ 0x14036A87C (MiMapSinglePage.c)
 *     MiRecheckCombineVm @ 0x14040424C (MiRecheckCombineVm.c)
 *     MiTryLockLeafPage @ 0x14047F28C (MiTryLockLeafPage.c)
 *     MiStandbyPageStillCombineCandidate @ 0x1404C5A04 (MiStandbyPageStillCombineCandidate.c)
 */

__int64 __fastcall MiCombineWithStandbyExisting(__int64 *a1, __int64 a2, __int64 a3)
{
  unsigned __int64 v3; // rdi
  __int64 v5; // r14
  unsigned __int64 v6; // rbp
  unsigned __int64 *v7; // rdi
  __int64 v8; // rbx
  unsigned int v9; // ebx
  ULONG_PTR v10; // r13
  __int64 v11; // r15
  __int64 v12; // rax
  __int64 v13; // r12
  __int64 result; // rax
  unsigned __int64 v15; // rcx
  unsigned __int64 v16; // r15
  __int64 v17; // rbx
  unsigned __int64 v18; // rdi
  int v19; // eax
  __int64 v20; // r8
  __int64 v21; // r9
  unsigned int v22; // ebx
  unsigned int v23; // r9d
  unsigned int v24; // r9d
  unsigned __int64 PrototypePteDirect; // rax
  __int64 v26; // rbx
  __int64 v27; // [rsp+20h] [rbp-58h]
  unsigned int v29; // [rsp+88h] [rbp+10h]
  unsigned __int64 v30; // [rsp+90h] [rbp+18h] BYREF
  unsigned __int64 v31; // [rsp+98h] [rbp+20h]

  v3 = *(_QWORD *)(a2 + 16);
  v5 = a1[14];
  v30 = 0LL;
  v27 = *a1;
  v29 = *((_DWORD *)a1 + 35);
  v6 = a1[8];
  v7 = (unsigned __int64 *)(((v3 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL);
  v8 = *(_QWORD *)(a3 + 48);
  v31 = a3 + 32;
  v9 = v8 & 0x1F;
  v10 = (__int64)(v6 + 0x220000000000LL) / 48;
  v11 = *(_QWORD *)(stru_140E2EB88.ThreadLock + 8LL * *(unsigned __int16 *)(v5 + 174));
  v12 = MiLockProtoPoolPage(a3 + 32, 0LL);
  v13 = v12;
  if ( !v12 )
    return 3221225557LL;
  if ( _interlockedbittestandset64((volatile signed __int32 *)(v6 + 24), 0x3FuLL) )
  {
    MiUnlockProtoPoolPage(v12, 0x11u);
    return 3221225557LL;
  }
  v15 = *v7;
  if ( (*(_QWORD *)a1[9] & 1) == 0 || (*v7 & 0xC00) != 0x800 )
    goto LABEL_5;
  if ( qword_140E2D740 )
  {
    if ( (v15 & 0x10) != 0 )
      v15 &= ~0x10uLL;
    else
      v15 &= qword_140E2D748;
  }
  if ( ((v15 >> 12) & 0xFFFFFFFFFFLL) != v10
    || !(unsigned int)MiStandbyPageStillCombineCandidate(v6, v11, v7, v9)
    || !(unsigned int)MiRecheckCombineVm(v27, v29, v6, v5) )
  {
LABEL_5:
    MiUnlockProtoPoolPage(v13, 0x12u);
LABEL_6:
    result = 3221225496LL;
LABEL_7:
    _InterlockedAnd64((volatile signed __int64 *)(v6 + 24), 0x7FFFFFFFFFFFFFFFuLL);
    return result;
  }
  MiTryLockLeafPage(v31, 1LL, &v30);
  v16 = v30;
  if ( !v30 )
  {
    v22 = -1073741739;
    goto LABEL_21;
  }
  v17 = *(_QWORD *)(stru_140E2EB88.ThreadLock + 8 * ((*(_QWORD *)(v30 + 40) >> 43) & 0x3FFLL));
  v18 = a1[12] + 4096;
  MiMapSinglePage(v18, (__int64)(v30 + 0x220000000000LL) / 48, 3221225488LL, 0LL);
  v19 = MiConfirmCombinePageContents(a1, (__int64)(v16 + 0x220000000000LL) / 48, v17);
  *(_QWORD *)(((v18 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL) = *(_QWORD *)&CLFS_LSN_NULL_EXT;
  if ( !v19 )
  {
    _InterlockedAnd64((volatile signed __int64 *)(v16 + 24), 0x7FFFFFFFFFFFFFFFuLL);
    v22 = -1073741800;
LABEL_21:
    MiUnlockProtoPoolPage(v13, 0x12u);
    result = v22;
    goto LABEL_7;
  }
  if ( (MiUnlinkPageFromListEx(v6, 0) & 3) != 0 )
  {
    _InterlockedAnd64((volatile signed __int64 *)(v16 + 24), 0x7FFFFFFFFFFFFFFFuLL);
    MiUnlockProtoPoolPage(v13, 0x12u);
    MiDiscardTransitionPfnEx(v6, 0LL, v20, v21);
    goto LABEL_6;
  }
  *(_QWORD *)(v6 + 24) &= 0xC000000000000000uLL;
  v23 = *(_DWORD *)(v6 + 32);
  if ( (*(_QWORD *)(v6 + 40) & 0x20000000000000LL) != 0
    || (*(_DWORD *)(v6 + 32) & 0x8000000) != 0
    && (v6 < 0xFFFFDE0000000000uLL
     || v6 >= 48 * qword_140E2D7A0 - 0x21FFFFFFFFD0LL
     || MiIsDecayPfn(v10)
     || (v23 & 0x70000) != 0x60000 && (unsigned int)MiGetPfnSlabType(v6) == 9) )
  {
    v24 = 5;
  }
  else
  {
    v24 = HIBYTE(v23) & 7;
  }
  MiUpdatePfnPriority(v16, v24, 0);
  _InterlockedAnd64((volatile signed __int64 *)(v16 + 24), 0x7FFFFFFFFFFFFFFFuLL);
  MiUnlockProtoPoolPage(v13, 0x12u);
  PrototypePteDirect = MiMakePrototypePteDirect(v31);
  MiWriteRepurposedTransitionPte(v6, PrototypePteDirect | 0x800);
  *(_QWORD *)(v6 + 24) |= 0x4000000000000000uLL;
  v26 = *(_QWORD *)(v6 + 40);
  MiPfnReferenceCountIsZero(v6, v10);
  _InterlockedAnd64((volatile signed __int64 *)(v6 + 24), 0x7FFFFFFFFFFFFFFFuLL);
  MiLockAndDecrementShareCount(48 * (v26 & 0xFFFFFFFFFFLL) - 0x220000000000LL);
  ++*(_QWORD *)a1[13];
  return 0LL;
}
