/*
 * XREFs of PiAuGetStateDirectorySecurityObject @ 0x1407A8710
 * Callers:
...
__int64 __fastcall PiAuGetStateDirectorySecurityObject(char a1, _QWORD *a2)
{
  ...
  v7 = 2 * (RtlLengthSid(*(PSID *)&RtlpBootStatHandleLock.WaitRegister.Flags) + v6) + 56;
  ...
  if ( Pool2 )
  {
    ...
    if ( Acl >= 0 )
    {
      Acl = RtlpAddKnownAce((int)v9, 2, 3, 1245599, *(void **)&RtlpBootStatHandleLock.WaitRegister.Flags, 0);
      if ( Acl >= 0 )
      {
        Acl = RtlpAddKnownAce((int)v9, 2, 2, 32, *(void **)&RtlpBootStatHandleLock.WaitRegister.Flags, 0);
        if ( Acl >= 0 )
        {
