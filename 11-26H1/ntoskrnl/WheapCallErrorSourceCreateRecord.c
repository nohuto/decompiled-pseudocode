/*
 * XREFs of WheapCallErrorSourceCreateRecord @ 0x1403DE2D8
 * Callers:
    ...
    if ( (unsigned int)a1[10] <= 0x12 )
    {
      ...
      if ( a1[27] != 3 )
      {
        while ( 1 )
        {
          ...
          {
            p_CurrentRunTime = &WheapInUsePageOfflineNotifyLock.CurrentRunTime;
            if ( v3 )
              p_CurrentRunTime = (unsigned int *)&unk_140EEF340;
            if ( *(_QWORD *)&p_CurrentRunTime[16 * (__int64)a1[10]] )
            {
