/*
 * XREFs of IopCreateArcNamesCd @ 0x140CC26E4
 * Callers:
...
__int64 __fastcall IopCreateArcNamesCd(__int64 a1)
{
  ...
  __int64 OutputBuffer; // [rsp+100h] [rbp-8h] BYREF
  ...
  *(GUID *)&StartingOffset[0].LowPart = GUID_DEVINTERFACE_CDROM;
  ...
  while ( 1 )
  {
    ...
    if ( v9 && *v9 )
    {
      ...
      if ( DeviceObjectPointer < 0
        || (RtlStringCchPrintfA(pszDest, 0x80uLL, "\\Device\\CdRom%d", HIDWORD(OutputBuffer)),
