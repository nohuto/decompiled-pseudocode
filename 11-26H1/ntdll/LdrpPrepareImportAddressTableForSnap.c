/*
 * XREFs of LdrpPrepareImportAddressTableForSnap @ 0x1800773A0
 * Callers:
...
NTSTATUS __fastcall LdrpPrepareImportAddressTableForSnap(__int64 a1)
{
  ...
  PIMAGE_NT_HEADERS OutHeaders; // [rsp+60h] [rbp+18h] BYREF
  ...
  *(_QWORD *)(a1 + 112) = v6;
  if ( v7 )
  {
    ...
    Config = LdrImageDirectoryEntryToLoadConfigEx(*(PVOID *)(v1 + 48));
    v9 = OutHeaders;
    if ( Config )
    {
      if ( *(_DWORD *)Config >= 0x94u
        && (OutHeaders->OptionalHeader.DllCharacteristics & 0x4000) != 0
    ...
    if ( !*(_QWORD *)(a1 + 112) )
    {
      ...
      if ( VirtualAddress )
      {
        ...
        {
          v17 = v16[3];
          if ( VirtualAddress >= (unsigned int)v17 && VirtualAddress < (unsigned int)v17 + v16[4] )
          {
