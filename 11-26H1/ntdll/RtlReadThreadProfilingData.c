/*
 * XREFs of RtlReadThreadProfilingData @ 0x180100130
 * Callers:
 *     <none>
 * Callees:
 *     <none>
        ...
        PPERFORMANCE_DATA PerformanceData)
{
  ...
  do
  {
    ...
    if ( (Flags & 1) != 0 )
    {
      ...
      {
        WaitReasonBitMap = PerformanceData->WaitReasonBitMap;
        PerformanceData->WaitReasonBitMap = WaitReasonBitMap | _InterlockedExchange64(
                                                                 (volatile __int64 *)PerformanceDataHandle + 3,
                                                                 0LL);
      }
    }
    if ( (Flags & 2) != 0 )
