/*
 * XREFs of ?ComputePartialIntermediate@CShapeTree@@CA_NAEBVCMILMatrix@@AEBUD2D_VECTOR_2F@@AEBV?$TMilRect_@MUD2D_RECT_F@@UD3D_RECT_F@@UD2D_POINTANDSIZE_F@@UNotNeeded@RectUniqueness@@@@2PEAV4@@Z @ 0x18028B2AC
 * Callers:
...
char __fastcall CShapeTree::ComputePartialIntermediate(struct CMILMatrix *a1, float *a2, FLOAT *a3, int *a4, float *a5)
{
  ...
  *a5 = (float)(COERCE_FLOAT(LODWORD(v14) ^ _xmm) + left) * *a2;
  a5[1] = (float)(COERCE_FLOAT(LODWORD(v16) ^ _xmm) + v19) * a2[1];
  a5[2] = (float)(COERCE_FLOAT(LODWORD(v14) ^ _xmm) + right) * *a2;
  a5[3] = (float)(COERCE_FLOAT(LODWORD(v16) ^ _xmm) + v20) * a2[1];
  return result;
}
