/*
 * XREFs of VidSchiStartNodeYield @ 0x140030A00
 * Callers:
 *     VidSchiUpdateLastCompletedPresentTimestamp @ 0x14000B434 (VidSchiUpdateLastCompletedPresentTimestamp.c)
 *     VidSchiScheduleCommandToRun @ 0x140021700 (VidSchiScheduleCommandToRun.c)
 *     VidSchiSelectContext @ 0x1400230C0 (VidSchiSelectContext.c)
 *     VidSchiProcessDpcCompletedPacket @ 0x140027C1C (VidSchiProcessDpcCompletedPacket.c)
 *     ?VidSchiUpdateNodeYieldStatus@@YAXPEAU_VIDSCH_NODE@@@Z @ 0x140030808 (-VidSchiUpdateNodeYieldStatus@@YAXPEAU_VIDSCH_NODE@@@Z.c)
 * Callees:
 *     McTemplateK0x_EtwWriteTransfer @ 0x140031A48 (McTemplateK0x_EtwWriteTransfer.c)
 *     McTemplateK0pqxx_EtwWriteTransfer @ 0x140031B5C (McTemplateK0pqxx_EtwWriteTransfer.c)
 */

__int64 __fastcall VidSchiStartNodeYield(__int64 a1, __int64 a2, unsigned __int64 a3, int a4)
{
  __int64 v4; // rdi
  unsigned __int64 v8; // rbp
  __int64 v9; // r8
  __int64 v10; // rcx
  __int64 v11; // r8
  unsigned __int64 v12; // rax
  __int64 result; // rax
  unsigned __int64 v14; // rcx
  unsigned __int64 v15; // r9
  unsigned __int64 v16; // rcx
  unsigned __int64 v17; // r8

  v4 = *(_QWORD *)(a1 + 24);
  v8 = *(unsigned int *)(v4 + 232);
  if ( !*(_QWORD *)(v4 + 208) )
  {
    v9 = (_DWORD)v8 == 10 && a4 == 16 ? 9500LL : (unsigned int)(100 * a4 * v8);
    ExSetTimer(*(_QWORD *)(v4 + 200), -v9, v9, 0LL);
    if ( (byte_14008A201 & 1) != 0 )
      McTemplateK0x_EtwWriteTransfer(v10, &EventYieldSetExpirationTimer, v11, *(_QWORD *)(v4 + 16));
  }
  v12 = 100LL * *(_QWORD *)(a1 + 2128);
  *(_QWORD *)(a1 + 2120) = a2;
  result = a2 + 2 * (v12 / v8) + v12 / v8;
  *(_QWORD *)(v4 + 208) = result;
  if ( (byte_14008A201 & 1) != 0 )
  {
    v14 = *(_QWORD *)(v4 + 208) - a2;
    if ( is_mul_ok(v14, 0x989680uLL) )
      v15 = v14 * (unsigned __int128)0x989680uLL / a3;
    else
      v15 = 10000000 * (v14 % a3) / a3 + 10000000 * (v14 / a3);
    v16 = *(_QWORD *)(a1 + 2128);
    if ( is_mul_ok(v16, 0x989680uLL) )
    {
      v17 = v16 * (unsigned __int128)0x989680uLL / a3;
    }
    else
    {
      v16 /= a3;
      v17 = 10000000 * (*(_QWORD *)(a1 + 2128) % a3) / a3 + 10000000 * v16;
    }
    return McTemplateK0pqxx_EtwWriteTransfer(
             v16,
             (unsigned int)&EventYieldStartNode,
             v17,
             *(_QWORD *)(v4 + 16),
             *(_WORD *)(a1 + 4),
             v17,
             v15);
  }
  return result;
}
