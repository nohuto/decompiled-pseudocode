/*
 * XREFs of ?Dispatch@FxPkgPnp@@MEAAJPEAU_IRP@@@Z @ 0x14005B170
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall FxPkgPnp::Dispatch(FxPkgPnp *this, _IRP *Irp)
{
  ...
  const void *v13; // r8
  FxDeviceBase *v14; // r8
  unsigned __int16 *p_m_ObjectSize; // r10
  const void **p_m_DeviceObject; // r11
  const void *v17; // r8
  bool v18; // zf
  ...
  if ( CurrentStackLocation->MajorFunction == 27 )
  {
    if ( level <= 5 )
    {
      if ( level == 5 || !CurrentStackLocation->MinorFunction || level == 1 || level == 2 || level == 3 )
        goto LABEL_21;
      v18 = level == 4;
    }
    else
    {
      switch ( level )
      {
        ...
        case 7u:
          ...
          _a5 = (const void *)((unsigned __int64)this->m_DeviceBase ^ 0xFFFFFFFFFFFFFFF8uLL);
...
LABEL_10:
  ...
  if ( CurrentStackLocation->MinorFunction )
  {
    if ( level - 2 < 2 )
    {
      ...
      p_m_ObjectSize = &v14->m_ObjectSize;
      p_m_DeviceObject = (const void **)&v14->m_DeviceObject.m_DeviceObject;
