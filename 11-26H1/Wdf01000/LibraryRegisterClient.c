/*
 * XREFs of LibraryRegisterClient @ 0x1400770E0
 * Callers:
 *     <none>
 * Callees:
 *     WdfBindClientHelper @ 0x140077290 (WdfBindClientHelper.c)
...
int __fastcall LibraryRegisterClient(
        _WDF_BIND_INFO *Info,
        _WDF_DRIVER_GLOBALS **WdfDriverGlobalsParam,
        _CLIENT_INFO **Context)
{
  ...
  *Context = 0LL;
  ...
  if ( v7 >= 0 )
  {
    result = FxLibraryCommonRegisterClient(Info, WdfDriverGlobalsParam, v6);
    if ( result >= 0 )
    {
      *Context = (_CLIENT_INFO *)&(*WdfDriverGlobalsParam)[-8];
      *(_QWORD *)&(*WdfDriverGlobalsParam)[-3].DriverName[8] = Info;
    }
  }
  else
  {
    ...
    if ( v8 >= 0 )
    {
      ...
      LibraryLogEvent(
        *(_DRIVER_OBJECT **)&WPP_GLOBAL_WDF_Control.SectorSize,
        -2147024895,
        v7,
        insertString,
        rawData,
