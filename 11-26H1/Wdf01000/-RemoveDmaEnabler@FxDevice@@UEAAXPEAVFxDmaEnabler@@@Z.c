/*
 * XREFs of ?RemoveDmaEnabler@FxDevice@@UEAAXPEAVFxDmaEnabler@@@Z @ 0x140095700
 * Callers:
 *     <none>
 * Callees:
 *     WPP_IFR_SF_qq @ 0x14000642C (WPP_IFR_SF_qq.c)
 *     ?Remove@FxTransactionedList@@QEAAXPEAU_FX_DRIVER_GLOBALS@@PEAUFxTransactionedEntry@@@Z @ 0x1400638C8 (-Remove@FxTransactionedList@@QEAAXPEAU_FX_DRIVER_GLOBALS@@PEAUFxTransactionedEntry@@@Z.c)
 */

void __fastcall FxDevice::RemoveDmaEnabler(FxDevice *this, unsigned __int64 Enabler)
{
  FxPkgPnp *m_PkgPnp; // rbx
  _FX_DRIVER_GLOBALS *m_Globals; // rcx
  const void *v5; // rdx

  m_PkgPnp = this->m_PkgPnp;
  if ( m_PkgPnp )
  {
    m_Globals = m_PkgPnp->m_Globals;
    if ( m_Globals->FxVerboseOn )
    {
      v5 = (const void *)(Enabler ^ 0xFFFFFFFFFFFFFFF8uLL);
      if ( !*(_WORD *)(Enabler + 10) )
        v5 = 0LL;
      WPP_IFR_SF_qq(m_Globals, 5u, 0xCu, 0xDu, WPP_FxPkgPnpKM_cpp_Traceguids, (const void *)Enabler, v5);
    }
    FxTransactionedList::Remove(
      m_PkgPnp->m_DmaEnablerList,
      m_PkgPnp->m_Globals,
      (FxTransactionedEntry *)(Enabler + 120));
  }
}
