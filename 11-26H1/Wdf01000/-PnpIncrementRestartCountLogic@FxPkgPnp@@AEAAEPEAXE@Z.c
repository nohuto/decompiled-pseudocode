/*
 * XREFs of ?PnpIncrementRestartCountLogic@FxPkgPnp@@AEAAEPEAXE@Z @ 0x1400A4364
 * Callers:
...
LABEL_2:
  ...
  else
  {
    ...
    if ( !m_AchievedStart )
    {
      v11 = this->m_Globals;
      value.LowPart = 0;
      if ( (int)FxRegKey::_QueryValue(v11, RestartKey, &valueNameStartAchieved, 4u, &value, &length, &type) >= 0
        && length == 4
        && type == 4 )
      {
