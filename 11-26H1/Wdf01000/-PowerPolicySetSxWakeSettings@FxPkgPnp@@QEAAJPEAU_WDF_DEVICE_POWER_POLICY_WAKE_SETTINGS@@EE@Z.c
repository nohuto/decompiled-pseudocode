/*
 * XREFs of ?PowerPolicySetSxWakeSettings@FxPkgPnp@@QEAAJPEAU_WDF_DEVICE_POWER_POLICY_WAKE_SETTINGS@@EE@Z @ 0x14004CC54
 * Callers:
...
LABEL_9:
  ...
  if ( this->m_CapsQueried || KeGetCurrentIrql() || (result = FxPkgPnp::QueryForCapabilities(this), result >= 0) )
  {
    ...
    if ( DxState == PowerDeviceMaximum )
    {
      ...
      {
        WPP_IFR_SF_DD(this->m_Globals, _a2 - 2, 0xCu, 0x3Bu, WPP_FxPkgPnp_cpp_Traceguids, _a2, -1073741101);
        return -1073741101;
      }
    }
    else if ( DxState > _a2 )
