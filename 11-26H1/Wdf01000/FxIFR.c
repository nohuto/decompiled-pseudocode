/*
 * XREFs of FxIFR @ 0x140032A50
 * Callers:
 *     WPP_IFR_SF_qcq @ 0x140005A8C (WPP_IFR_SF_qcq.c)
 *     WPP_IFR_SF_qq @ 0x14000642C (WPP_IFR_SF_qq.c)
 *     WPP_IFR_SF_qdD @ 0x140007548 (WPP_IFR_SF_qdD.c)
 *     WPP_IFR_SF_qqd @ 0x14000EA80 (WPP_IFR_SF_qqd.c)
 *     WPP_IFR_SF_qDqD @ 0x140010ADC (WPP_IFR_SF_qDqD.c)
 *     WPP_IFR_SF_qd @ 0x140012548 (WPP_IFR_SF_qd.c)
 *     WPP_IFR_SF_qDd @ 0x140017BE4 (WPP_IFR_SF_qDd.c)
 *     WPP_IFR_SF_i @ 0x140026104 (WPP_IFR_SF_i.c)
 *     WPP_IFR_SF_DDid @ 0x1400262E0 (WPP_IFR_SF_DDid.c)
 *     WPP_IFR_SF_sq @ 0x140026708 (WPP_IFR_SF_sq.c)
 *     WPP_IFR_SF_D @ 0x140026890 (WPP_IFR_SF_D.c)
 *     WPP_IFR_SF_qLsqd @ 0x1400306F4 (WPP_IFR_SF_qLsqd.c)
 *     WPP_IFR_SF_qqLL @ 0x1400328C0 (WPP_IFR_SF_qqLL.c)
 *     WPP_IFR_SF_qqDqq @ 0x140037DC4 (WPP_IFR_SF_qqDqq.c)
 *     WPP_IFR_SF_qqq @ 0x140039C38 (WPP_IFR_SF_qqq.c)
 *     WPP_IFR_SF_dqd @ 0x14003E928 (WPP_IFR_SF_dqd.c)
 *     WPP_IFR_SF_qdd @ 0x14003EFD8 (WPP_IFR_SF_qdd.c)
 *     WPP_IFR_SF_Lq @ 0x140045FC0 (WPP_IFR_SF_Lq.c)
 *     WPP_IFR_SF_q @ 0x1400488BC (WPP_IFR_SF_q.c)
 *     WPP_IFR_SF_ @ 0x14004B128 (WPP_IFR_SF_.c)
 *     WPP_IFR_SF_qLL @ 0x14004D454 (WPP_IFR_SF_qLL.c)
 *     WPP_IFR_SF_Dqqd @ 0x140051AB4 (WPP_IFR_SF_Dqqd.c)
 *     WPP_IFR_SF_dd @ 0x140055174 (WPP_IFR_SF_dd.c)
 *     WPP_IFR_SF_DDd @ 0x140055DDC (WPP_IFR_SF_DDd.c)
 *     WPP_IFR_SF_qiid @ 0x14005646C (WPP_IFR_SF_qiid.c)
 *     WPP_IFR_SF_qid @ 0x1400565E4 (WPP_IFR_SF_qid.c)
 *     WPP_IFR_SF_DD @ 0x14005676C (WPP_IFR_SF_DD.c)
 *     WPP_IFR_SF_qdqqqqd @ 0x140058508 (WPP_IFR_SF_qdqqqqd.c)
 *     WPP_IFR_SF_qL @ 0x140059D40 (WPP_IFR_SF_qL.c)
 *     WPP_IFR_SF_qqcLq @ 0x14005B500 (WPP_IFR_SF_qqcLq.c)
 *     WPP_IFR_SF_qqcq @ 0x14005B670 (WPP_IFR_SF_qqcq.c)
 *     WPP_IFR_SF_qqcqL @ 0x14005B7F8 (WPP_IFR_SF_qqcqL.c)
 *     WPP_IFR_SF_qqcqLd @ 0x14005B968 (WPP_IFR_SF_qqcqLd.c)
 *     WPP_IFR_SF_qqdid @ 0x14005BF20 (WPP_IFR_SF_qqdid.c)
 *     WPP_IFR_SF_qqLdqqq @ 0x1400613B4 (WPP_IFR_SF_qqLdqqq.c)
 *     WPP_IFR_SF_qqdq @ 0x140069AAC (WPP_IFR_SF_qqdq.c)
 *     WPP_IFR_SF_qqqd @ 0x14006A614 (WPP_IFR_SF_qqqd.c)
 *     WPP_IFR_SF_qdq @ 0x14006FF90 (WPP_IFR_SF_qdq.c)
 *     WPP_IFR_SF_qqqq @ 0x140072174 (WPP_IFR_SF_qqqq.c)
 *     WPP_IFR_SF_ddLLdiDD @ 0x14007C004 (WPP_IFR_SF_ddLLdiDD.c)
 *     WPP_IFR_SF_LLd @ 0x14007EC2C (WPP_IFR_SF_LLd.c)
 *     WPP_IFR_SF_Zd @ 0x14007F05C (WPP_IFR_SF_Zd.c)
 *     WPP_IFR_SF_qLqd @ 0x14007F22C (WPP_IFR_SF_qLqd.c)
 *     WPP_IFR_SF_qcDqd @ 0x14007F3A0 (WPP_IFR_SF_qcDqd.c)
 *     WPP_IFR_SF_sd @ 0x14007F544 (WPP_IFR_SF_sd.c)
 *     WPP_IFR_SF_qqddd @ 0x14007F870 (WPP_IFR_SF_qqddd.c)
 *     WPP_IFR_SF_qqqqq @ 0x14007FA08 (WPP_IFR_SF_qqqqq.c)
 *     WPP_IFR_SF_Dd @ 0x1400803F8 (WPP_IFR_SF_Dd.c)
 *     WPP_IFR_SF_DDDDDDD @ 0x140080610 (WPP_IFR_SF_DDDDDDD.c)
 *     WPP_IFR_SF_dq @ 0x140080778 (WPP_IFR_SF_dq.c)
 *     WPP_IFR_SF_qqLLL @ 0x1400809D8 (WPP_IFR_SF_qqLLL.c)
 *     WPP_IFR_SF_ddiid @ 0x140080BEC (WPP_IFR_SF_ddiid.c)
 *     WPP_IFR_SF_qDqd @ 0x140080D24 (WPP_IFR_SF_qDqd.c)
 *     WPP_IFR_SF_qqqi @ 0x140080F28 (WPP_IFR_SF_qqqi.c)
 *     WPP_IFR_SF_Dqq @ 0x140082D1C (WPP_IFR_SF_Dqq.c)
 *     WPP_IFR_SF_Lqd @ 0x140082E0C (WPP_IFR_SF_Lqd.c)
 *     WPP_IFR_SF_LqqLd @ 0x140086408 (WPP_IFR_SF_LqqLd.c)
 *     WPP_IFR_SF_Lqqd @ 0x140086548 (WPP_IFR_SF_Lqqd.c)
 *     WPP_IFR_SF_qddd @ 0x140086668 (WPP_IFR_SF_qddd.c)
 *     WPP_IFR_SF_qiqd @ 0x1400867D8 (WPP_IFR_SF_qiqd.c)
 *     WPP_IFR_SF_ql @ 0x1400868F4 (WPP_IFR_SF_ql.c)
 *     WPP_IFR_SF_qqdd @ 0x140086A1C (WPP_IFR_SF_qqdd.c)
 *     WPP_IFR_SF_dqq @ 0x140087E34 (WPP_IFR_SF_dqq.c)
 *     WPP_IFR_SF_qld @ 0x140087F24 (WPP_IFR_SF_qld.c)
 *     WPP_IFR_SF_qqiDq @ 0x140088018 (WPP_IFR_SF_qqiDq.c)
 *     WPP_IFR_SF_qqii @ 0x14008814C (WPP_IFR_SF_qqii.c)
 *     WPP_IFR_SF_qqiid @ 0x140088258 (WPP_IFR_SF_qqiid.c)
 *     WPP_IFR_SF_qqqD @ 0x14008838C (WPP_IFR_SF_qqqD.c)
 *     WPP_IFR_SF_qqqDd @ 0x1400884A0 (WPP_IFR_SF_qqqDd.c)
 *     WPP_IFR_SF_sqqii @ 0x1400885D8 (WPP_IFR_SF_sqqii.c)
 *     WPP_IFR_SF_qD @ 0x140088B34 (WPP_IFR_SF_qD.c)
 *     WPP_IFR_SF_qDPq @ 0x1400891B4 (WPP_IFR_SF_qDPq.c)
 *     WPP_IFR_SF_qDPqd @ 0x1400892C4 (WPP_IFR_SF_qDPqd.c)
 *     WPP_IFR_SF_qLLd @ 0x140089404 (WPP_IFR_SF_qLLd.c)
 *     WPP_IFR_SF_qPPd @ 0x140089520 (WPP_IFR_SF_qPPd.c)
 *     WPP_IFR_SF_qc @ 0x14008963C (WPP_IFR_SF_qc.c)
 *     WPP_IFR_SF_dLd @ 0x14008A6F8 (WPP_IFR_SF_dLd.c)
 *     WPP_IFR_SF_qLqdi @ 0x14008A7E8 (WPP_IFR_SF_qLqdi.c)
 *     WPP_IFR_SF_qi @ 0x14008A920 (WPP_IFR_SF_qi.c)
 *     WPP_IFR_SF_qii @ 0x14008A9F0 (WPP_IFR_SF_qii.c)
 *     WPP_IFR_SF_Ddd @ 0x14008B3FC (WPP_IFR_SF_Ddd.c)
 *     WPP_IFR_SF_qll @ 0x14008CD68 (WPP_IFR_SF_qll.c)
 *     WPP_IFR_SF_ds @ 0x14008D7D4 (WPP_IFR_SF_ds.c)
 *     WPP_IFR_SF_s @ 0x14008D908 (WPP_IFR_SF_s.c)
 *     WPP_IFR_SF_Dddd @ 0x14008DC2C (WPP_IFR_SF_Dddd.c)
 *     WPP_IFR_SF_iid @ 0x14008DF50 (WPP_IFR_SF_iid.c)
 *     WPP_IFR_SF_iqdd @ 0x14008E040 (WPP_IFR_SF_iqdd.c)
 *     WPP_IFR_SF_qddq @ 0x14008E160 (WPP_IFR_SF_qddq.c)
 *     WPP_IFR_SF_cd @ 0x140091694 (WPP_IFR_SF_cd.c)
 *     WPP_IFR_SF_qqcDq @ 0x1400917B4 (WPP_IFR_SF_qqcDq.c)
 *     WPP_IFR_SF_qqcDqd @ 0x1400918F4 (WPP_IFR_SF_qqcDqd.c)
 *     WPP_IFR_SF_qsd @ 0x140091A58 (WPP_IFR_SF_qsd.c)
 *     WPP_IFR_SF_sl @ 0x140092B54 (WPP_IFR_SF_sl.c)
 *     WPP_IFR_SF_qDDd @ 0x140092DAC (WPP_IFR_SF_qDDd.c)
 *     WPP_IFR_SF_qdid @ 0x140092F20 (WPP_IFR_SF_qdid.c)
 *     WPP_IFR_SF_dddd @ 0x1400938A8 (WPP_IFR_SF_dddd.c)
 *     WPP_IFR_SF_sSLSs @ 0x140093A18 (WPP_IFR_SF_sSLSs.c)
 *     WPP_IFR_SF_qqqqd @ 0x140094168 (WPP_IFR_SF_qqqqd.c)
 *     WPP_IFR_SF_dqqd @ 0x140094964 (WPP_IFR_SF_dqqd.c)
 *     WPP_IFR_SF_qDiD @ 0x140094B88 (WPP_IFR_SF_qDiD.c)
 *     WPP_IFR_SF_Sd @ 0x140095780 (WPP_IFR_SF_Sd.c)
 *     WPP_IFR_SF_qZ @ 0x140095A70 (WPP_IFR_SF_qZ.c)
 *     WPP_IFR_SF_Z @ 0x140097014 (WPP_IFR_SF_Z.c)
 *     WPP_IFR_SF_dZq @ 0x1400971C0 (WPP_IFR_SF_dZq.c)
 *     WPP_IFR_SF_sqq @ 0x140097860 (WPP_IFR_SF_sqq.c)
 *     WPP_IFR_SF_ddi @ 0x140097B6C (WPP_IFR_SF_ddi.c)
 *     WPP_IFR_SF_iiddiidd @ 0x140097C68 (WPP_IFR_SF_iiddiidd.c)
 *     WPP_IFR_SF_qqsd @ 0x140097EE4 (WPP_IFR_SF_qqsd.c)
 *     WPP_IFR_SF_sqqq @ 0x140098080 (WPP_IFR_SF_sqqq.c)
 *     WPP_IFR_SF_DqD @ 0x140098500 (WPP_IFR_SF_DqD.c)
 *     WPP_IFR_SF_qqDD @ 0x1400985F0 (WPP_IFR_SF_qqDD.c)
 *     WPP_IFR_SF_qZd @ 0x140099AE8 (WPP_IFR_SF_qZd.c)
 *     WPP_IFR_SF_dqdd @ 0x14009A440 (WPP_IFR_SF_dqdd.c)
 *     WPP_IFR_SF_iii @ 0x14009A7B8 (WPP_IFR_SF_iii.c)
 *     WPP_IFR_SF_qqD @ 0x14009B3D8 (WPP_IFR_SF_qqD.c)
 *     WPP_IFR_SF_qqDdqq @ 0x14009B4CC (WPP_IFR_SF_qqDdqq.c)
 *     WPP_IFR_SF_qqDq @ 0x14009B620 (WPP_IFR_SF_qqDq.c)
 *     WPP_IFR_SF_qqDqqq @ 0x14009B730 (WPP_IFR_SF_qqDqqq.c)
 *     WPP_IFR_SF_qqDqqqd @ 0x14009B880 (WPP_IFR_SF_qqDqqqd.c)
 *     WPP_IFR_SF_qqdqdd @ 0x14009C1D4 (WPP_IFR_SF_qqdqdd.c)
 *     WPP_IFR_SF_qqqqi @ 0x14009C36C (WPP_IFR_SF_qqqqi.c)
 *     WPP_IFR_SF_qqDLd @ 0x14009D17C (WPP_IFR_SF_qqDLd.c)
 *     WPP_IFR_SF_qqLd @ 0x14009D2BC (WPP_IFR_SF_qqLd.c)
 *     WPP_IFR_SF_qqqdD @ 0x14009D424 (WPP_IFR_SF_qqqdD.c)
 *     WPP_IFR_SF_qdL @ 0x14009E834 (WPP_IFR_SF_qdL.c)
 *     WPP_IFR_SF_qs @ 0x14009E924 (WPP_IFR_SF_qs.c)
 *     WPP_IFR_SF_dii @ 0x14009ECD8 (WPP_IFR_SF_dii.c)
 *     WPP_IFR_SF_idii @ 0x14009EDD4 (WPP_IFR_SF_idii.c)
 *     WPP_IFR_SF_dddddd @ 0x1400A03B0 (WPP_IFR_SF_dddddd.c)
 *     WPP_IFR_SF__guid_d @ 0x1400A0D48 (WPP_IFR_SF__guid_d.c)
 *     WPP_IFR_SF_DDDDd @ 0x1400A0E20 (WPP_IFR_SF_DDDDd.c)
 *     WPP_IFR_SF_qcd @ 0x1400A1770 (WPP_IFR_SF_qcd.c)
 *     WPP_IFR_SF_qqs @ 0x1400A1B90 (WPP_IFR_SF_qqs.c)
 *     WPP_IFR_SF_PqD @ 0x1400A2100 (WPP_IFR_SF_PqD.c)
 *     WPP_IFR_SF_ddddd @ 0x1400A4B30 (WPP_IFR_SF_ddddd.c)
 *     WPP_IFR_SF_sqqLd @ 0x1400A5074 (WPP_IFR_SF_sqqLd.c)
 *     WPP_IFR_SF_sqqd @ 0x1400A5234 (WPP_IFR_SF_sqqd.c)
 *     WPP_IFR_SF_LL @ 0x1400A72A8 (WPP_IFR_SF_LL.c)
 *     WPP_IFR_SF_cqcqq @ 0x1400A7374 (WPP_IFR_SF_cqcqq.c)
 *     WPP_IFR_SF_dDqq @ 0x1400A74AC (WPP_IFR_SF_dDqq.c)
 *     WPP_IFR_SF_dddDqqd @ 0x1400A75C0 (WPP_IFR_SF_dddDqqd.c)
 *     WPP_IFR_SF_L @ 0x1400A7B48 (WPP_IFR_SF_L.c)
 *     WPP_IFR_SF_qDDDDsdisd @ 0x1400AA820 (WPP_IFR_SF_qDDDDsdisd.c)
 * Callees:
 *     ?FxVerifierDbgBreakPoint@@YAXPEAU_FX_DRIVER_GLOBALS@@@Z @ 0x140083568 (-FxVerifierDbgBreakPoint@@YAXPEAU_FX_DRIVER_GLOBALS@@@Z.c)
 *     memmove @ 0x1400AD500 (memmove.c)
 */

__int64 FxIFR(
        _FX_DRIVER_GLOBALS *FxDriverGlobals,
        unsigned __int8 MessageLevel,
        unsigned int MessageFlags,
        _GUID *MessageGuid,
        unsigned __int16 MessageNumber,
        ...)
{
  char *WdfLogHeader; // rbp
  va_list v8; // rcx
  __int64 v9; // rbx
  unsigned __int64 v10; // rax
  __int64 v11; // rax
  unsigned __int64 v12; // rbx
  __int64 v13; // rsi
  signed __int32 v14; // eax
  unsigned __int64 v15; // rbx
  signed __int32 v16; // r9d
  unsigned __int16 v17; // dx
  __int64 v18; // rdi
  char *v19; // rax
  char *v20; // rsi
  size_t *v21; // rax
  const void *v22; // rcx
  size_t v23; // rbx
  const void **v24; // rdi
  unsigned int v26; // ecx
  unsigned int v27; // ecx
  char *ap; // [rsp+20h] [rbp-48h] BYREF
  const void *v29; // [rsp+98h] [rbp+30h] BYREF
  va_list va; // [rsp+98h] [rbp+30h]
  va_list va1; // [rsp+A0h] [rbp+38h] BYREF

  va_start(va1, MessageNumber);
  va_start(va, MessageNumber);
  v29 = va_arg(va1, const void *);
  if ( unk_1400C90D8 )
    return 0LL;
  WdfLogHeader = (char *)FxDriverGlobals->WdfLogHeader;
  if ( WdfLogHeader )
  {
    va_copy(v8, va);
    v9 = 0LL;
    ap = 0LL;
    while ( *(_QWORD *)v8 )
    {
      v10 = *((_QWORD *)v8 + 1);
      v8 += 16;
      if ( v10 )
      {
        if ( v10 > 0x100 )
          goto $drop_message;
        v9 += (unsigned __int16)v10;
      }
    }
    v11 = 4 - (v9 & 3);
    if ( (v9 & 3) == 0 )
      v11 = 0LL;
    v12 = v11 + v9;
    if ( v12 > 0x100 )
    {
$drop_message:
      _InterlockedIncrement((volatile signed __int32 *)WdfLogHeader + 8);
      return 3221225473LL;
    }
    if ( FxDriverGlobals->FxVerifierOn )
    {
      if ( strncmp(WdfLogHeader + 36, FxDriverGlobals->Public.DriverName, 0x20uLL)
        || RtlCompareMemory(WdfLogHeader, &WdfTraceGuid, 0x10uLL) != 16
        || *((char **)WdfLogHeader + 2) != WdfLogHeader + 72
        || (v26 = *((_DWORD *)WdfLogHeader + 6), *((unsigned __int16 *)WdfLogHeader + 14) > v26)
        || *((unsigned __int16 *)WdfLogHeader + 15) > v26
        || v26 >= 0x10000 )
      {
        FxVerifierDbgBreakPoint(FxDriverGlobals);
      }
    }
    if ( WdfLogHeader[68] )
      v13 = 36LL;
    else
      v13 = 28LL;
    v14 = *((_DWORD *)WdfLogHeader + 7);
    v15 = v13 + v12;
    LODWORD(ap) = 0;
    do
    {
      v16 = v14;
      if ( *((_QWORD *)WdfLogHeader + 2) + (unsigned __int64)*((unsigned int *)WdfLogHeader + 6) < v15
                                                                                                 + *((_QWORD *)WdfLogHeader + 2)
                                                                                                 + (unsigned __int16)v14 )
      {
        LOWORD(ap) = v15;
        v17 = 0;
      }
      else
      {
        v17 = v14;
        LOWORD(ap) = v15 + v14;
      }
      WORD1(ap) = v17;
      v14 = _InterlockedCompareExchange((volatile signed __int32 *)WdfLogHeader + 7, (signed __int32)ap, v14);
    }
    while ( v16 != v14 );
    v18 = *((_QWORD *)WdfLogHeader + 2) + v17;
    *(_WORD *)(v18 + 8) = HIWORD(v14);
    *(_WORD *)(v18 + 2) = v15;
    *(_WORD *)(v18 + 10) = MessageNumber;
    *(_DWORD *)(v18 + 4) = _InterlockedIncrement((volatile signed __int32 *)WdfLogHeader + 8);
    *(_GUID *)(v18 + 12) = *MessageGuid;
    if ( WdfLogHeader[68] )
    {
      *(_WORD *)v18 = 12876;
      ap = 0LL;
      if ( WdfLogHeader[69] )
      {
        KeQuerySystemTimePrecise(&ap);
        v19 = ap;
      }
      else
      {
        v19 = (char *)MEMORY[0xFFFFF78000000014];
      }
      *(_QWORD *)(v18 + 28) = v19;
    }
    else
    {
      *(_WORD *)v18 = 21068;
    }
    v20 = (char *)(v18 + v13);
    va_copy((va_list)v21, va1);
    v22 = v29;
    if ( v29 )
    {
      do
      {
        v23 = *v21;
        v24 = (const void **)(v21 + 1);
        if ( *v21 )
        {
          memmove(v20, v22, *v21);
          v20 += v23;
        }
        v22 = *v24;
        v21 = (size_t *)(v24 + 1);
      }
      while ( *v24 );
    }
    if ( FxDriverGlobals->FxVerifierOn )
    {
      if ( strncmp(WdfLogHeader + 36, FxDriverGlobals->Public.DriverName, 0x20uLL)
        || RtlCompareMemory(WdfLogHeader, &WdfTraceGuid, 0x10uLL) != 16
        || *((char **)WdfLogHeader + 2) != WdfLogHeader + 72
        || (v27 = *((_DWORD *)WdfLogHeader + 6), *((unsigned __int16 *)WdfLogHeader + 14) > v27)
        || *((unsigned __int16 *)WdfLogHeader + 15) > v27
        || v27 >= 0x10000 )
      {
        FxVerifierDbgBreakPoint(FxDriverGlobals);
      }
    }
    return 0LL;
  }
  return 3221225473LL;
}
