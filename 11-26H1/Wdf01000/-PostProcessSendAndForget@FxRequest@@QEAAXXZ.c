/*
 * XREFs of ?PostProcessSendAndForget@FxRequest@@QEAAXXZ @ 0x140037FB0
 * Callers:
  ...
  if ( v4 )
  {
    if ( m_CompletionState )
    {
      if ( (m_CompletionState & 0x80u) == 0 )
      {
        ...
        {
          v8 = KeAcquireSpinLockRaiseToDpc(&m_IoQueue->m_NPLock.m_Lock);
        }
        ...
        if ( m_IoQueue->m_Dispatching )
        {
          ...
          {
            FxVerifierLock::Unlock(v16, v8, (unsigned __int8)Blink);
          }
          else
          {
            KeReleaseSpinLock(&m_IoQueue->m_NPLock.m_Lock, v8);
          }
        }
        else
        {
          FxIoQueue::DispatchEvents(m_IoQueue, v8, 0LL, v9);
        }
      }
