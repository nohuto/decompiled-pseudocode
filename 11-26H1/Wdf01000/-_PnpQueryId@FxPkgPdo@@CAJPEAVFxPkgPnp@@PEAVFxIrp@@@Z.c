/*
 * XREFs of ?_PnpQueryId@FxPkgPdo@@CAJPEAVFxPkgPnp@@PEAVFxIrp@@@Z @ 0x14006A850
 * Callers:
 *     <none>
 * Callees:
 *     ?CompletePnpRequest@FxPkgPnp@@IEAAJPEAVFxIrp@@J@Z @ 0x140008014 (-CompletePnpRequest@FxPkgPnp@@IEAAJPEAVFxIrp@@J@Z.c)
 *     WPP_IFR_SF_qDd @ 0x140017BE4 (WPP_IFR_SF_qDd.c)
 *     ?FxCalculateTotalMultiSzStringSize@@YA_KPEBG@Z @ 0x14006AA58 (-FxCalculateTotalMultiSzStringSize@@YA_KPEBG@Z.c)
 *     memmove @ 0x1400AD500 (memmove.c)
 */

__int64 __fastcall FxPkgPdo::_PnpQueryId(FxPkgPnp *This, FxIrp *Irp)
{
  _FX_DRIVER_GLOBALS *m_Globals; // r13
  signed int Status; // ebx
  unsigned int _a3; // ebp
  FxDeviceBase *m_DeviceBase; // rdi
  __int64 v8; // rax
  size_t v9; // r14
  void *v10; // rax
  unsigned __int64 v11; // rbx
  FxDeviceBase *v12; // rax
  unsigned __int16 *p_m_ObjectSize; // rdx
  const void *_a2; // rax
  unsigned __int16 v15; // r9
  __int64 v16; // rax
  const void *v17; // rbx
  size_t v18; // rdi
  void *Pool2; // rax
  unsigned __int64 v20; // r14
  unsigned int globals; // [rsp+38h] [rbp-30h]

  m_Globals = This->m_Globals;
  Status = Irp->m_Irp->IoStatus.Status;
  _a3 = Irp->m_Irp->Tail.Overlay.CurrentStackLocation->Parameters.Read.Length;
  if ( _a3 )
  {
    if ( _a3 == 1 || _a3 == 2 )
    {
      v16 = 1808LL;
      if ( _a3 != 1 )
        v16 = 1816LL;
      v17 = *(FxPkgPnp_vtbl **)((char *)&This->__vftable + v16);
      if ( v17 )
        v18 = FxCalculateTotalMultiSzStringSize(*(const wchar_t **)((char *)&This->__vftable + v16));
      else
        v18 = 4LL;
      Pool2 = (void *)ExAllocatePool2(256LL, v18, m_Globals->Tag);
      v20 = (unsigned __int64)Pool2;
      if ( !Pool2 )
        goto LABEL_12;
      if ( v17 )
        memmove(Pool2, v17, v18);
      Irp->m_Irp->IoStatus.Information = v20;
LABEL_24:
      Status = 0;
      return FxPkgPnp::CompletePnpRequest(This, Irp, Status);
    }
    if ( _a3 != 3 && _a3 != 5 )
    {
LABEL_25:
      if ( Status >= 0 )
        return FxPkgPnp::CompletePnpRequest(This, Irp, Status);
      goto LABEL_13;
    }
    if ( _a3 == 3 )
      m_DeviceBase = *(FxDeviceBase **)&This[1].m_NPLock.m_DbgFlagIsInitialized;
    else
      m_DeviceBase = *(FxDeviceBase **)&This[1].m_PnpCapsAddress;
  }
  else
  {
    m_DeviceBase = This[1].m_DeviceBase;
  }
  if ( !m_DeviceBase )
    goto LABEL_25;
  v8 = -1LL;
  do
    ++v8;
  while ( *((_WORD *)&m_DeviceBase->FxNonPagedObject::FxObject::__vftable + v8) );
  v9 = 2 * v8 + 2;
  v10 = (void *)ExAllocatePool2(256LL, v9, m_Globals->Tag);
  v11 = (unsigned __int64)v10;
  if ( v10 )
  {
    memmove(v10, m_DeviceBase, v9);
    Irp->m_Irp->IoStatus.Information = v11;
    goto LABEL_24;
  }
LABEL_12:
  Status = -1073741670;
LABEL_13:
  Irp->m_Irp->IoStatus.Information = 0LL;
  v12 = This->m_DeviceBase;
  p_m_ObjectSize = &v12->m_ObjectSize;
  _a2 = (const void *)((unsigned __int64)v12 ^ 0xFFFFFFFFFFFFFFF8uLL);
  if ( Status == -1073741637 )
  {
    v15 = 20;
    globals = -1073741637;
  }
  else
  {
    v15 = 21;
    globals = Status;
  }
  if ( !*p_m_ObjectSize )
    _a2 = 0LL;
  WPP_IFR_SF_qDd(m_Globals, (unsigned __int8)p_m_ObjectSize, 0xCu, v15, WPP_FxPkgPdo_cpp_Traceguids, _a2, _a3, globals);
  return FxPkgPnp::CompletePnpRequest(This, Irp, Status);
}
