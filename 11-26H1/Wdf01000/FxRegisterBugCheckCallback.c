/*
 * XREFs of FxRegisterBugCheckCallback @ 0x14006CE18
 * Callers:
...
void __fastcall FxRegisterBugCheckCallback(_FX_DRIVER_GLOBALS *FxDriverGlobals, _DRIVER_OBJECT *DriverObject)
{
  ...
  *(_OWORD *)&FxDriverGlobals->BugCheckCallbackRecord.Checksum = 0LL;
  ...
  if ( FxpGetImageBase(DriverObject, &FxDriverGlobals->ImageAddress, &FxDriverGlobals->ImageSize) >= 0 && !unk_1400C9091 )
  {
    if ( FxTrackDriverForMiniDumpLog )
    {
      if ( _FX_DRIVER_TRACKER_CACHE_AWARE::Register((_FX_DRIVER_TRACKER_CACHE_AWARE *)&unk_1400C9070, v5) >= 0 )
