/*
 * XREFs of PiRegStateReadStackCreationSettingsFromKey @ 0x1400DB434
 * Callers:
 ...
 *     SeUtilSecurityInfoFromSecurityDescriptor @ 0x1400DB8D4 (SeUtilSecurityInfoFromSecurityDescriptor.c)
        ...
        STACK_CREATION_SETTINGS *StackCreationSettings)
{
  ...
  *(_QWORD *)&StackCreationSettings->Characteristics = 0LL;
  ...
  if ( inited >= 0 )
  {
    inited = CmRegUtilUcValueGetFullBuffer(ClassOrDeviceKey, &DestinationString, v6, v7, &keyInfo);
    v4 = keyInfo;
    v8 = inited;
    if ( inited >= 0 )
    {
      LOBYTE(v9) = 1;
      v8 = SeCaptureSecurityDescriptor((char *)keyInfo + keyInfo->DataOffset, 0LL, 1LL, v9, &newSecurityDescriptor);
      goto LABEL_5;
    }
  }
  else
...
LABEL_5:
  ...
  {
    v8 = SeUtilSecurityInfoFromSecurityDescriptor(
