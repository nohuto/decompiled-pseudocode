/*
 * XREFs of ?WaitForSentIoToComplete@FxUsbPipe@@UEAAXXZ @ 0x14006E9C0
 * Callers:
 *     <none>
 * Callees:
 *     ?CancelRepeaters@FxUsbPipeContinuousReader@@QEAAXXZ @ 0x140004DC8 (-CancelRepeaters@FxUsbPipeContinuousReader@@QEAAXXZ.c)
 *     ?EnterCRAndWaitAndLeave@FxCREvent@@QEAAJXZ @ 0x140007B28 (-EnterCRAndWaitAndLeave@FxCREvent@@QEAAJXZ.c)
 *     WPP_IFR_SF_qd @ 0x140012548 (WPP_IFR_SF_qd.c)
 *     WPP_IFR_SF_q @ 0x1400488BC (WPP_IFR_SF_q.c)
 */

void __fastcall FxUsbPipe::WaitForSentIoToComplete(FxUsbPipe *this)
{
  _FX_DRIVER_GLOBALS **p_m_Globals; // r14
  const void *v2; // rbx
  unsigned __int16 *p_m_ObjectSize; // rsi
  const void *_a1; // rax
  const void *v6; // rax

  p_m_Globals = &this->m_Globals;
  v2 = (const void *)((unsigned __int64)this ^ 0xFFFFFFFFFFFFFFF8uLL);
  p_m_ObjectSize = &this->m_ObjectSize;
  if ( this->m_Reader )
  {
    if ( (*p_m_Globals)->FxVerboseOn )
    {
      _a1 = (const void *)((unsigned __int64)this ^ 0xFFFFFFFFFFFFFFF8uLL);
      if ( !*p_m_ObjectSize )
        _a1 = 0LL;
      WPP_IFR_SF_q(*p_m_Globals, 5u, 0xEu, 0x19u, WPP_FxUsbPipe_cpp_Traceguids, _a1);
    }
    FxCREvent::EnterCRAndWaitAndLeave(&this->m_Reader->m_WorkItem->m_WorkItemCompleted);
    if ( (*p_m_Globals)->FxVerboseOn )
    {
      v6 = v2;
      if ( !*p_m_ObjectSize )
        v6 = 0LL;
      WPP_IFR_SF_qd(*p_m_Globals, 5u, 0xEu, 0x1Au, WPP_FxUsbPipe_cpp_Traceguids, v6, this->m_Reader->m_NumReaders);
    }
    FxUsbPipeContinuousReader::CancelRepeaters(this->m_Reader);
  }
  if ( (*p_m_Globals)->FxVerboseOn )
  {
    if ( !*p_m_ObjectSize )
      v2 = 0LL;
    WPP_IFR_SF_q(*p_m_Globals, 5u, 0xEu, 0x1Bu, WPP_FxUsbPipe_cpp_Traceguids, v2);
  }
  FxCREvent::EnterCRAndWaitAndLeave(&this->m_SentIoEvent);
}
