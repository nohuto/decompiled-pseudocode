/*
 * XREFs of ?_Reset_map@?$deque@U_MIT_KEYBOARD_OVERRIDER_INPUT_MESSAGE@@V?$allocator@U_MIT_KEYBOARD_OVERRIDER_INPUT_MESSAGE@@@std@@@std@@AEAAXXZ @ 0x1800F4018
 * Callers:
...
void __fastcall std::deque<_MIT_KEYBOARD_OVERRIDER_INPUT_MESSAGE>::_Reset_map(__int64 a1)
{
  _QWORD *v1; // rsi
  __int64 v3; // rdi
  void **v4; // rbx
  ...
  else
  {
    do
    {
      ...
      if ( v5 )
        std::_Deallocate<16>(v5, (struct std::nothrow_t *)0x170);
    }
    while ( v3 > 0 );
    v6 = (_QWORD *)(a1 + 16);
  }
