/*
 * XREFs of ?OnDeviceAttach@GameInputProcessor@@AEAAXPEAUDeviceInfo@@@Z @ 0x180048670
 * Callers:
 *     <none>
 * Callees:
  ...
  if ( GameInputProcessor::IsLegacyPnpDevice(a2) && (*((_DWORD *)a2 + 1) == 4 || *((_DWORD *)a2 + 1) == 2) )
  {
    ...
    else
    {
      ...
      if ( v7 == *((_DWORD **)this + 23) )
      {
        try
        {
          std::vector<Windows::UI::Color>::_Emplace_reallocate<Windows::UI::Color const &>((char *)this + 168, v7, a2);
        }
        catch ( ... )
        {
