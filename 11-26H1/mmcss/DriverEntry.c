...
NTSTATUS __stdcall DriverEntry(_DRIVER_OBJECT *DriverObject, PUNICODE_STRING RegistryPath)
{
  ...
  size_t v4; // rdi
  void *Pool2; // rax
  PCLIENT_ID ClientId; // [rsp+20h] [rbp-48h]
  struct _KEVENT Event; // [rsp+40h] [rbp-28h] BYREF
  _DRIVER_OBJECT *v9; // [rsp+58h] [rbp-10h]
  void *ThreadHandle; // [rsp+80h] [rbp+18h] BYREF
    ...
    if ( Pool2 )
    {
      ...
      if ( SystemInformation >= 0 )
      {
        ...
        if ( SystemInformation >= 0 )
        {
          ...
          if ( SystemInformation >= 0 )
          {
            Event.Header.LockNV = 65545;
            Event.Header.WaitListHead.Flink = (struct _LIST_ENTRY *)&CiKernelCalloutTable;
            Event.Header.SignalState = 0x10000;
            Event.Header.WaitListHead.Blink = (struct _LIST_ENTRY *)&CiKernelInterface;
            v9 = DriverObject;
            SystemInformation = ExRegisterExtension(&CiKernelExtensionRegistration, 65537LL, &Event);
            if ( SystemInformation >= 0 )
            {
