/*
 * XREFs of rimPenButtonsPolicyUpdateState @ 0x140137648
 * Callers:
 *     rimApplyPointerDevicePolicies @ 0x1401AF62C (rimApplyPointerDevicePolicies.c)
 * Callees:
 *     RIMIsPenPointerDevice @ 0x14007611C (RIMIsPenPointerDevice.c)
 *     RIMIsPointerDownTransition @ 0x14013770C (RIMIsPointerDownTransition.c)
 *     MicrosoftTelemetryAssertTriggeredArgsKM @ 0x1401B44F4 (MicrosoftTelemetryAssertTriggeredArgsKM.c)
 */

__int64 __fastcall rimPenButtonsPolicyUpdateState(__int64 a1, _DWORD *a2, int a3)
{
  __int64 v5; // r8
  __int64 result; // rax

  if ( !(unsigned int)RIMIsPenPointerDevice(a1) )
    MicrosoftTelemetryAssertTriggeredArgsKM("IXPTelAssert", 0x20000LL, 1246LL);
  if ( a3 )
  {
    if ( (a2[632] & 2) != 0 )
      a2[8] = a2[8] & 0xFFFFFFDF | ~(8 * (unsigned __int8)a2[611]) & 0x20;
    else
      a2[8] &= ~0x20u;
  }
  result = RIMIsPointerDownTransition((unsigned int)a2[671], (unsigned int)a2[611], v5);
  if ( (_DWORD)result )
  {
    result = (16 * a2[632]) ^ (a2[8] ^ (16 * a2[632])) & 0xFFFFFFEF;
    a2[8] = result;
  }
  return result;
}
