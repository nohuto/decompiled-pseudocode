/*
 * XREFs of ??$GreAcquireSemaphoreShared@$02$$V@@YAXAEAUSESSION_GLOBALS@Base@Gre@@@Z @ 0x14001CE94
 * Callers:
...
struct _GRETHREAD *__fastcall GreAcquireSemaphoreShared<3,>(_DWORD *a1, __int64 a2, int a3)
{
  ...
  if ( HIDWORD(WPP_MAIN_CB.Queue.Wcb.DeviceContext) && (Microsoft_Windows_Win32kEnableBits & 0x10) != 0 )
    McTemplateK0pz_EtwWriteTransfer((_DWORD)a1, (unsigned int)&LockAcquireShared, a3, *a1 + 728, (__int64)L"DCVisRgn");
  EngAcquireSemaphoreShared((HSEMAPHORE)(*(_QWORD *)a1 + 728LL));
  result = GreGetCurrentThreadCrossSessionCheck();
  v6 = result;
  if ( result )
  {
    v7 = *(_QWORD *)result;
    if ( (*(_QWORD *)result & 0xFFFFFFDFFFFFFFF8uLL) != 0 && (v7 & 8) == 0 )
    {
      v8 = 38;
      for ( i = 0LL; i < 0x40; ++i )
      {
