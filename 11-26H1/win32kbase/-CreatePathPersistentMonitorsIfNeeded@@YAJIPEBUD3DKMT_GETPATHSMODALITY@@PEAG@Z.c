/*
 * XREFs of ?CreatePathPersistentMonitorsIfNeeded@@YAJIPEBUD3DKMT_GETPATHSMODALITY@@PEAG@Z @ 0x140198C8C
 * Callers:
 *     DrvSetDisplayConfig @ 0x140175E80 (DrvSetDisplayConfig.c)
 * Callees:
 *     _guard_dispatch_icall @ 0x14024BC70 (_guard_dispatch_icall.c)
 */

__int64 __fastcall CreatePathPersistentMonitorsIfNeeded(
        __int64 a1,
        const struct D3DKMT_GETPATHSMODALITY *a2,
        unsigned __int16 *a3)
{
  unsigned int v5; // ebp
  unsigned int i; // r14d
  __int64 v7; // rdi
  __int64 v8; // rcx
  __int64 v9; // rdx
  __int64 DxgkWin32kInterface; // rax
  int v11; // eax

  v5 = 0;
  if ( a3 )
    *a3 = 0;
  for ( i = 0; i < *((unsigned __int16 *)a2 + 10); ++i )
  {
    v7 = 296LL * i;
    v8 = *(_QWORD *)((char *)a2 + v7 + 56);
    if ( (v8 & 0x1100000000000000LL) != 0x100000000000000LL )
    {
      if ( (v8 & 0x200000000000000LL) != 0 )
      {
        DxgkWin32kInterface = DxDdGetDxgkWin32kInterface(v8, 0x100000000000000LL);
        v11 = (*(__int64 (__fastcall **)(_QWORD, _QWORD, _QWORD, __int64))(DxgkWin32kInterface + 224))(
                *(_QWORD *)((char *)a2 + v7 + 72),
                *(unsigned int *)((char *)a2 + v7 + 84),
                0LL,
                2LL);
        v5 = v11;
        if ( v11 < 0 )
        {
          WdLogSingleEntry4(
            4LL,
            *(int *)((char *)a2 + v7 + 76),
            *(unsigned int *)((char *)a2 + v7 + 72),
            *(unsigned int *)((char *)a2 + v7 + 84),
            v11);
          WdLogGlobalForLineNumber = 14181;
          return v5;
        }
        if ( (*(_QWORD *)((_BYTE *)a2 + v7 + 56) & 0x1000000000000000LL) != 0 )
        {
          WdLogSingleEntry3(5LL, *(int *)((char *)a2 + v7 + 76));
          WdLogGlobalForLineNumber = 14191;
        }
        else if ( a3 )
        {
          ++*a3;
        }
      }
      else
      {
        v9 = *(unsigned int *)((char *)a2 + v7 + 84);
        if ( (v8 & 0x1000000000000000LL) == 0 )
        {
          WdLogSingleEntry3(4LL, v9);
          WdLogGlobalForLineNumber = 14162;
          return (unsigned int)-1073741811;
        }
        WdLogSingleEntry4(4LL, v9, *(int *)((char *)a2 + v7 + 76), *(unsigned int *)((char *)a2 + v7 + 72), 1LL);
        WdLogGlobalForLineNumber = 14154;
      }
    }
  }
  return v5;
}
