/*
 * XREFs of ??0CCrossContainerHostReadOnlySharedSectionMarshaler@DirectComposition@@QEAA@XZ @ 0x1400558DC
 * Callers:
 *     ?CreatePrivateMarshaler@CApplicationChannel@DirectComposition@@IEAAJIPEAPEAVCResourceMarshaler@2@@Z @ 0x1401C2478 (-CreatePrivateMarshaler@CApplicationChannel@DirectComposition@@IEAAJIPEAPEAVCResourceMarshaler@2.c)
 * Callees:
 *     ??0CSharedSectionBaseMarshaler@DirectComposition@@IEAA@W4MIL_RESOURCE_TYPE@@@Z @ 0x140055E3C (--0CSharedSectionBaseMarshaler@DirectComposition@@IEAA@W4MIL_RESOURCE_TYPE@@@Z.c)
 */

_QWORD *__fastcall DirectComposition::CCrossContainerHostReadOnlySharedSectionMarshaler::CCrossContainerHostReadOnlySharedSectionMarshaler(
        DirectComposition::CCrossContainerHostReadOnlySharedSectionMarshaler *this)
{
  _QWORD *v1; // rcx
  _QWORD *result; // rax

  DirectComposition::CSharedSectionBaseMarshaler::CSharedSectionBaseMarshaler(this, 132LL);
  result = v1;
  *v1 = &DirectComposition::CCrossContainerHostReadOnlySharedSectionMarshaler::`vftable';
  return result;
}
