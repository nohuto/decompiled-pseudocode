/*
 * XREFs of NvmeDeviceHierarchicalReset @ 0x14012C1C0
 * Callers:
 *     NvmeControllerCommandTimeoutDetectThread @ 0x14012B190 (NvmeControllerCommandTimeoutDetectThread.c)
 * Callees:
 *     NvmeControllerResetRecovery @ 0x14012BD30 (NvmeControllerResetRecovery.c)
 */

char __fastcall NvmeDeviceHierarchicalReset(PVOID SystemArgument2, PSLIST_HEADER ListHead)
{
  USHORT DepthSList; // ax
  char v5; // bp
  int v6; // r14d
  PSLIST_ENTRY v7; // rdi
  PSLIST_ENTRY v8; // rsi
  __int64 v9; // rax
  NTSTATUS v10; // edi
  bool v11; // zf
  __int64 v12; // rdx
  unsigned int v13; // ecx
  __int64 v14; // rdi
  unsigned int v15; // edx
  unsigned __int64 v16; // rsi
  signed __int32 v17; // ecx
  _QWORD *v18; // rcx
  signed __int32 v20[8]; // [rsp+0h] [rbp-78h] BYREF
  __int128 v21; // [rsp+30h] [rbp-48h]
  union _LARGE_INTEGER Timeout; // [rsp+90h] [rbp+18h] BYREF

  DepthSList = ExQueryDepthSList(ListHead);
  v5 = 0;
  v6 = 0;
  if ( DepthSList )
  {
    *(_DWORD *)(*(_QWORD *)(*((_QWORD *)SystemArgument2 + 161) + 32LL) + 48LL) = DepthSList;
    *(_DWORD *)(*(_QWORD *)(*((_QWORD *)SystemArgument2 + 161) + 32LL) + 52LL) = 0;
    v7 = ExpInterlockedPopEntrySList(ListHead);
    if ( v7 )
    {
      do
      {
        v8 = v7 - 32;
        v7[-25].Next = (_SLIST_ENTRY *)((unsigned __int64)v7[-25].Next & ~0x80uLL);
        if ( _InterlockedCompareExchange((volatile signed __int32 *)&v7[-1].Next->Next + 2, 1, 0) )
        {
          _InterlockedDecrement((volatile signed __int32 *)(*(_QWORD *)(*((_QWORD *)SystemArgument2 + 161) + 32LL) + 48LL));
          ExReleaseRundownProtectionCacheAware(*((PEX_RUNDOWN_REF_CACHE_AWARE *)&v8[7].Next + 1));
        }
        else
        {
          IoQueueWorkItem(
            (PIO_WORKITEM)v8[31].Next->Next,
            NvmeNamespaceResetRecoveryRoutine,
            DelayedWorkQueue,
            &v7[-32]);
          ++*((_DWORD *)&v8[31].Next->Next + 3);
          ++v6;
        }
        v7 = v7->Next;
      }
      while ( v7 );
      if ( v6 )
      {
        v9 = *((_QWORD *)SystemArgument2 + 161);
        Timeout.QuadPart = -300000000LL;
        v10 = KeWaitForSingleObject((PVOID)(*(_QWORD *)(v9 + 32) + 24LL), Executive, 0, 0, &Timeout);
        if ( v10 != 258 )
        {
          if ( *(_DWORD *)(*(_QWORD *)(*((_QWORD *)SystemArgument2 + 161) + 32LL) + 52LL) )
            v5 = NvmeControllerResetRecovery((unsigned __int16 *)SystemArgument2, 3);
          else
            v5 = 1;
        }
        if ( (*((_BYTE *)SystemArgument2 + 1360) & 0x10) != 0 )
        {
          v11 = *(_DWORD *)SystemArgument2 == 1314276178;
          v21 = 0LL;
          LODWORD(v21) = v10;
          BYTE4(v21) = v5;
          if ( v11 )
          {
            v12 = *(_QWORD *)(*((_QWORD *)SystemArgument2 + 171) + 8LL * KeGetCurrentProcessorNumberEx(0LL));
            v13 = *(_DWORD *)(v12 + 12);
            v14 = v12 + 64;
            _InterlockedOr(v20, 0);
            if ( v13 )
            {
              if ( v12 != -64 )
              {
                v15 = _InterlockedIncrement((volatile signed __int32 *)v12) % v13;
                v16 = (unsigned __int64)v15 << 6;
                v17 = v15;
                if ( (*((_DWORD *)SystemArgument2 + 334) & 1) != 0 )
                  v17 = _InterlockedIncrement((volatile signed __int32 *)SystemArgument2 + 352);
                *(_DWORD *)(v16 + v14 + 4) = v17;
                *(_DWORD *)(v16 + v14) = 65538;
                v18 = (_QWORD *)(v16 + v14 + 8);
                if ( (*((_DWORD *)SystemArgument2 + 334) & 2) != 0 )
                  KeQuerySystemTimePrecise(v18);
                else
                  *v18 = MEMORY[0xFFFFF78000000014];
                *(_OWORD *)(v16 + v14 + 16) = v21;
                *(_OWORD *)(v16 + v14 + 32) = 0LL;
              }
            }
          }
        }
      }
    }
    LOBYTE(DepthSList) = v5;
  }
  return DepthSList;
}
