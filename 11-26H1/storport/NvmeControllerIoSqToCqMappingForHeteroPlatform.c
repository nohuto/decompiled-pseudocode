/*
 * XREFs of NvmeControllerIoSqToCqMappingForHeteroPlatform @ 0x1400F7948
 * Callers:
 *     NvmeControllerIoSqToCqMapping @ 0x1400F74F8 (NvmeControllerIoSqToCqMapping.c)
 * Callees:
 *     <none>
 */

__int64 __fastcall NvmeControllerIoSqToCqMappingForHeteroPlatform(__int64 a1)
{
  unsigned int v1; // edx
  unsigned int v2; // r12d
  unsigned int v4; // r14d
  unsigned int v5; // r15d
  unsigned int v6; // r11d
  _QWORD *v7; // rbp
  unsigned int v9; // ebx
  __int64 v10; // rsi
  unsigned int v11; // r9d
  __int64 v12; // rdi
  __int64 v13; // r10
  __int64 v14; // rax
  char v15; // r9
  unsigned int v16; // r9d
  unsigned int i; // edi
  unsigned int v18; // esi
  __int64 v19; // r10
  __int64 v20; // rbp
  unsigned int v21; // ebx
  unsigned __int64 v22; // rcx
  __int64 v23; // rcx
  unsigned int j; // ebx
  unsigned int v25; // edi
  __int64 v26; // r9
  __int64 v27; // rsi
  unsigned int v28; // r10d

  v1 = 0;
  v2 = -1;
  v4 = 0;
  v5 = 0;
  v6 = 0;
  if ( !g_HeterogenousCPU || *(_WORD *)(a1 + 22) < 2u )
    return 3221225473LL;
  v7 = g_CpuInfo;
  if ( !g_CpuInfo )
    return 3221225701LL;
  v9 = 1;
  if ( *(_DWORD *)(a1 + 908) <= 1u )
    goto LABEL_27;
  do
  {
    v10 = *(_QWORD *)(a1 + 912);
    v11 = 0;
    v12 = 32LL * v9;
    v13 = *(_QWORD *)(v12 + v10 + 24);
    if ( v13 )
    {
      while ( v11 < 0x40 )
      {
        v14 = *(_QWORD *)(v12 + v10 + 8);
        if ( _bittest64(&v14, v11) )
        {
          v11 += *(unsigned __int16 *)(v12 + v10 + 16) << 6;
          break;
        }
        ++v11;
      }
      v15 = *(_BYTE *)(56LL * v11 + v7[4] + 4);
      if ( v15 )
      {
        if ( v15 == 1 )
        {
          *(_DWORD *)(v13 + 28) = *(_DWORD *)(v13 + 28) & 0xFFFFFFF0 | 1;
          ++v5;
        }
      }
      else
      {
        *(_DWORD *)(v13 + 28) &= 0xFFFFFFF0;
        ++v4;
      }
    }
    ++v9;
  }
  while ( v9 < *(_DWORD *)(a1 + 908) );
  if ( v4 && v5 && v5 + v4 == *(unsigned __int16 *)(a1 + 26) )
  {
    while ( v6 < *(unsigned __int16 *)(a1 + 22) )
    {
      v16 = 0;
      for ( i = 0; i < v5 && v6 < *(unsigned __int16 *)(a1 + 22); ++i )
      {
        v18 = *(unsigned __int16 *)(a1 + 26);
        v19 = *(_QWORD *)(a1 + 728) + 192LL * v6;
        if ( v16 >= v18 )
          goto LABEL_27;
        v20 = *(_QWORD *)(a1 + 864);
        while ( 1 )
        {
          v21 = v16 + 1;
          if ( (*(_DWORD *)(((unsigned __int64)v16 << 7) + v20 + 28) & 0xF) == 1 )
            break;
          ++v16;
          if ( v21 >= v18 )
            goto LABEL_27;
        }
        ++v6;
        v22 = (unsigned __int64)v16++ << 7;
        *(_WORD *)(v19 + 138) = *(_WORD *)(v22 + v20 + 32);
      }
      v23 = 0LL;
      for ( j = 0; j < v4 && v6 < *(unsigned __int16 *)(a1 + 22); ++j )
      {
        v25 = *(unsigned __int16 *)(a1 + 26);
        v26 = *(_QWORD *)(a1 + 728) + 192LL * v6;
        if ( (unsigned int)v23 >= v25 )
          goto LABEL_27;
        v27 = *(_QWORD *)(a1 + 864);
        while ( 1 )
        {
          v28 = v23 + 1;
          if ( (*(_DWORD *)(((unsigned __int64)(unsigned int)v23 << 7) + v27 + 28) & 0xF) == 0 )
            break;
          v23 = v28;
          if ( v28 >= v25 )
            goto LABEL_27;
        }
        ++v6;
        *(_WORD *)(v26 + 138) = *(_WORD *)((v23 << 7) + v27 + 32);
        v23 = v28;
      }
      if ( v6 < *(unsigned __int16 *)(a1 + 22) )
      {
        if ( v2 != -1 && v2 >= v6 )
          goto LABEL_27;
        v2 = v6;
      }
    }
  }
  else
  {
LABEL_27:
    *(_QWORD *)(a1 + 136) |= 0x8000000uLL;
    return (unsigned int)-1073741595;
  }
  return v1;
}
