/*
 * XREFs of ??$Write@U?$_tlgWrapperByVal@$03@@U?$_tlgWrapperByVal@$07@@U2@U?$_tlgWrapBuffer@U_UNICODE_STRING@@@@U2@U?$_tlgWrapperArray@$07@@@?$_tlgWriteTemplate@$$A6AJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2IPEAU_EVENT_DATA_DESCRIPTOR@@@Z$1?_tlgWriteTransfer_EtwWriteTransfer@@YAJ0122I3@ZPEBU2@PEBU2@@@SAJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2AEBU?$_tlgWrapperByVal@$03@@AEBU?$_tlgWrapperByVal@$07@@4AEBU?$_tlgWrapBuffer@U_UNICODE_STRING@@@@4AEBU?$_tlgWrapperArray@$07@@@Z @ 0x140002250
 * Callers:
 *     ?NodeUsageTelemetry@DXGADAPTER@@QEAAX_N@Z @ 0x14019CF80 (-NodeUsageTelemetry@DXGADAPTER@@QEAAX_N@Z.c)
 * Callees:
 *     _tlgWriteTransfer_EtwWriteTransfer @ 0x140049164 (_tlgWriteTransfer_EtwWriteTransfer.c)
 *     __security_check_cookie @ 0x1400A59A0 (__security_check_cookie.c)
 */

__int64 __fastcall _tlgWriteTemplate<long (_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),&long _tlgWriteTransfer_EtwWriteTransfer(_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),_GUID const *,_GUID const *>::Write<_tlgWrapperByVal<4>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapBuffer<_UNICODE_STRING>,_tlgWrapperByVal<8>,_tlgWrapperArray<8>>(
        __int64 a1,
        __int64 a2,
        __int64 a3,
        __int64 a4,
        __int64 a5,
        __int64 a6,
        __int64 a7,
        unsigned __int16 **a8,
        __int64 a9,
        __int64 *a10)
{
  int v10; // ecx
  _BYTE v12[32]; // [rsp+30h] [rbp-89h] BYREF
  __int64 v13; // [rsp+50h] [rbp-69h]
  __int64 v14; // [rsp+58h] [rbp-61h]
  __int64 v15; // [rsp+60h] [rbp-59h]
  __int64 v16; // [rsp+68h] [rbp-51h]
  __int64 v17; // [rsp+70h] [rbp-49h]
  __int64 v18; // [rsp+78h] [rbp-41h]
  _DWORD *v19; // [rsp+80h] [rbp-39h]
  __int64 v20; // [rsp+88h] [rbp-31h]
  __int64 v21; // [rsp+90h] [rbp-29h]
  _DWORD v22[2]; // [rsp+98h] [rbp-21h] BYREF
  __int64 v23; // [rsp+A0h] [rbp-19h]
  __int64 v24; // [rsp+A8h] [rbp-11h]
  __int64 *v25; // [rsp+B0h] [rbp-9h]
  __int64 v26; // [rsp+B8h] [rbp-1h]
  __int64 v27; // [rsp+C0h] [rbp+7h]
  int v28; // [rsp+C8h] [rbp+Fh]
  int v29; // [rsp+CCh] [rbp+13h]

  v26 = 2LL;
  v29 = 0;
  v24 = 8LL;
  v20 = 2LL;
  v27 = *a10;
  v28 = 8 * *((unsigned __int16 *)a10 + 4);
  v23 = a9;
  v19 = v22;
  v25 = a10 + 1;
  v10 = **a8;
  v21 = *((_QWORD *)*a8 + 1);
  v17 = a7;
  v15 = a6;
  v13 = a5;
  v22[0] = v10;
  v22[1] = 0;
  v18 = 8LL;
  v16 = 8LL;
  v14 = 4LL;
  return tlgWriteTransfer_EtwWriteTransfer(&dword_140166660, a2, 0LL, 0LL, 10, v12);
}
