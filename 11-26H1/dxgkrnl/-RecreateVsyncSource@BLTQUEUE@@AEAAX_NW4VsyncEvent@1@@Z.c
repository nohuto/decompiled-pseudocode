/*
 * XREFs of ?RecreateVsyncSource@BLTQUEUE@@AEAAX_NW4VsyncEvent@1@@Z @ 0x14018F994
 * Callers:
 *     ?BltQueueWorker@BLTQUEUE@@QEAAXXZ @ 0x14009A6DC (-BltQueueWorker@BLTQUEUE@@QEAAXXZ.c)
 *     ?DisableHwVSyncWorker@BLTQUEUE@@AEAAXXZ @ 0x140283020 (-DisableHwVSyncWorker@BLTQUEUE@@AEAAXXZ.c)
 *     ?GlobalVmBusStatusChangeWorker@BLTQUEUE@@AEAAXXZ @ 0x1402833D4 (-GlobalVmBusStatusChangeWorker@BLTQUEUE@@AEAAXXZ.c)
 *     ?HostMonitorPowerStateWorker@BLTQUEUE@@AEAAXXZ @ 0x14028346C (-HostMonitorPowerStateWorker@BLTQUEUE@@AEAAXXZ.c)
 *     ?StartVSync@BLTQUEUE@@AEAAX_NW4VsyncEvent@1@@Z @ 0x140283D74 (-StartVSync@BLTQUEUE@@AEAAX_NW4VsyncEvent@1@@Z.c)
 *     ?Startup@BLTQUEUE@@QEAAJXZ @ 0x1404149D4 (-Startup@BLTQUEUE@@QEAAJXZ.c)
 * Callees:
 *     ?GetGlobal@DXGGLOBAL@@SAPEAV1@XZ @ 0x14000E010 (-GetGlobal@DXGGLOBAL@@SAPEAV1@XZ.c)
 *     McTemplateK0qqq_EtwWriteTransfer @ 0x14005A918 (McTemplateK0qqq_EtwWriteTransfer.c)
 *     _guard_dispatch_icall @ 0x1400A5A80 (_guard_dispatch_icall.c)
 *     ??0BLTQUEUE_HW_VSYNC_SOURCE@@QEAA@PEAVDXGDODPRESENT@@PEAU_KEVENT@@@Z @ 0x14018FAA4 (--0BLTQUEUE_HW_VSYNC_SOURCE@@QEAA@PEAVDXGDODPRESENT@@PEAU_KEVENT@@@Z.c)
 *     ??0BLTQUEUE_REMOTE_VSYNC_SOURCE@@QEAA@PEAVDXGDODPRESENT@@@Z @ 0x1402845DC (--0BLTQUEUE_REMOTE_VSYNC_SOURCE@@QEAA@PEAVDXGDODPRESENT@@@Z.c)
 *     ??0BLTQUEUE_EMULATED_VSYNC_SOURCE@@QEAA@PEAVDXGDODPRESENT@@PEAU_EX_TIMER@@1@Z @ 0x140284B34 (--0BLTQUEUE_EMULATED_VSYNC_SOURCE@@QEAA@PEAVDXGDODPRESENT@@PEAU_EX_TIMER@@1@Z.c)
 *     ?IsVmConnectedToHost@DXGGLOBAL@@QEBAEXZ @ 0x1403BCBA0 (-IsVmConnectedToHost@DXGGLOBAL@@QEBAEXZ.c)
 */

int __fastcall BLTQUEUE::RecreateVsyncSource(__int64 a1, char a2, char a3)
{
  int v3; // r14d
  BLTQUEUE_HW_VSYNC_SOURCE **v4; // rdi
  void (__fastcall ***v7)(_QWORD, _QWORD); // rcx
  DXGGLOBAL *Global; // rax
  struct DXGDODPRESENT *v10; // rdx
  BLTQUEUE_HW_VSYNC_SOURCE *v11; // rax
  __int64 v12; // rcx
  __int64 v13; // r8
  struct _EX_TIMER *v14; // r9
  struct _EX_TIMER *v15; // r8
  struct DXGDODPRESENT *v16; // rdx

  v3 = *(_DWORD *)(a1 + 136);
  v4 = (BLTQUEUE_HW_VSYNC_SOURCE **)(a1 + 128);
  v7 = *(void (__fastcall ****)(_QWORD, _QWORD))(a1 + 128);
  if ( v7 )
  {
    (**v7)(v7, 0LL);
    *v4 = 0LL;
  }
  if ( *(_DWORD *)(a1 + 136) == 1 )
  {
    v11 = BLTQUEUE_HW_VSYNC_SOURCE::BLTQUEUE_HW_VSYNC_SOURCE(
            (BLTQUEUE_HW_VSYNC_SOURCE *)(a1 + 8),
            *(struct DXGDODPRESENT **)a1,
            *(struct _KEVENT **)(a1 + 392));
  }
  else
  {
    Global = DXGGLOBAL::GetGlobal();
    if ( !DXGGLOBAL::IsVmConnectedToHost(Global) || a2 )
    {
      v14 = *(struct _EX_TIMER **)(a1 + 152);
      v15 = *(struct _EX_TIMER **)(a1 + 144);
      v16 = *(struct DXGDODPRESENT **)a1;
      *(_DWORD *)(a1 + 136) = 2;
      v11 = BLTQUEUE_EMULATED_VSYNC_SOURCE::BLTQUEUE_EMULATED_VSYNC_SOURCE(
              (BLTQUEUE_EMULATED_VSYNC_SOURCE *)(a1 + 8),
              v16,
              v15,
              v14);
    }
    else
    {
      v10 = *(struct DXGDODPRESENT **)a1;
      *(_DWORD *)(a1 + 136) = 3;
      v11 = BLTQUEUE_REMOTE_VSYNC_SOURCE::BLTQUEUE_REMOTE_VSYNC_SOURCE((BLTQUEUE_REMOTE_VSYNC_SOURCE *)(a1 + 8), v10);
    }
  }
  *v4 = v11;
  if ( bTracingEnabled && (Microsoft_Windows_DxgKrnlEnableBits & 0x100) != 0 )
    LODWORD(v11) = McTemplateK0qqq_EtwWriteTransfer(
                     v12,
                     &EventBltQueueVsyncRecreateSource,
                     v13,
                     v3,
                     *(_DWORD *)(a1 + 136),
                     a3);
  return (int)v11;
}
