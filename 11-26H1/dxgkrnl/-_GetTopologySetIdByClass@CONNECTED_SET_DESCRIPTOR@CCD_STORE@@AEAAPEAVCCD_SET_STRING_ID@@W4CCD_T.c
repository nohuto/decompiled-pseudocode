/*
 * XREFs of ?_GetTopologySetIdByClass@CONNECTED_SET_DESCRIPTOR@CCD_STORE@@AEAAPEAVCCD_SET_STRING_ID@@W4CCD_TOPOLOGY_CLASS@@_N@Z @ 0x140339538
 * Callers:
 *     ?GetRecentTopologySetId@CONNECTED_SET_DESCRIPTOR@CCD_STORE@@QEAAPEBVCCD_SET_STRING_ID@@W4CCD_TOPOLOGY_CLASS@@@Z @ 0x140339480 (-GetRecentTopologySetId@CONNECTED_SET_DESCRIPTOR@CCD_STORE@@QEAAPEBVCCD_SET_STRING_ID@@W4CCD_TOP.c)
 *     ?SetRecentTopologySetId@CONNECTED_SET_DESCRIPTOR@CCD_STORE@@QEAAJAEBVCCD_SET_STRING_ID@@W4CCD_TOPOLOGY_CLASS@@@Z @ 0x1404016D0 (-SetRecentTopologySetId@CONNECTED_SET_DESCRIPTOR@CCD_STORE@@QEAAJAEBVCCD_SET_STRING_ID@@W4CCD_TO.c)
 * Callees:
 *     ??2@YAPEAX_KIW4DXGK_POOL_FLAGS@@@Z @ 0x1400358E0 (--2@YAPEAX_KIW4DXGK_POOL_FLAGS@@@Z.c)
 */

__int64 __fastcall CCD_STORE::CONNECTED_SET_DESCRIPTOR::_GetTopologySetIdByClass(__int64 a1, int a2, char a3)
{
  __int64 v3; // rsi
  __int64 v6; // rbx
  __int64 v8; // rax

  v3 = a2;
  switch ( a2 )
  {
    case 1:
      v6 = 56LL;
      break;
    case 2:
      v6 = 64LL;
      break;
    case 4:
      v6 = 72LL;
      break;
    case 8:
      v6 = 80LL;
      break;
    default:
      if ( a2 != 15 )
      {
        WdLogSingleEntry1(1LL);
        WdLogGlobalForLineNumber = 1027;
      }
      v6 = 88LL;
      break;
  }
  if ( !*(_QWORD *)(v6 + a1) && a3 )
  {
    v8 = operator new(0x38uLL, 0x63644356u, 256LL);
    if ( v8 )
    {
      *(_DWORD *)v8 = 0;
      *(_OWORD *)(v8 + 8) = 0LL;
      *(_QWORD *)(v8 + 24) = 0LL;
      *(_WORD *)(v8 + 32) = 0;
      *(_OWORD *)(v8 + 40) = 0LL;
      *(_QWORD *)(v6 + a1) = v8;
    }
    else
    {
      *(_QWORD *)(v6 + a1) = 0LL;
      WdLogSingleEntry4(6LL, 56LL, v3, a1, *(_QWORD *)(a1 + 96));
      WdLogGlobalForLineNumber = 1041;
    }
  }
  return *(_QWORD *)(v6 + a1);
}
