/*
 * XREFs of ?IsMonitoredFenceOpenedAsNativeFence@DXGSYNCOBJECT@@QEAA_NPEAVADAPTER_RENDER@@@Z @ 0x1403DA678
 * Callers:
 *     ?Initialize@DXGDEVICESYNCOBJECT@@QEAAJPEAVDXGSYNCOBJECT@@U_D3DDDI_SYNCHRONIZATIONOBJECT_FLAGS@@IPEAE@Z @ 0x140200460 (-Initialize@DXGDEVICESYNCOBJECT@@QEAAJPEAVDXGSYNCOBJECT@@U_D3DDDI_SYNCHRONIZATIONOBJECT_FLAGS@@I.c)
 * Callees:
 *     ?FindAdapterObject@DXGSYNCOBJECTCA@@QEAAPEAVDXGADAPTERSYNCOBJECTCA@@PEAVADAPTER_RENDER@@@Z @ 0x1400469A4 (-FindAdapterObject@DXGSYNCOBJECTCA@@QEAAPEAVDXGADAPTERSYNCOBJECTCA@@PEAVADAPTER_RENDER@@@Z.c)
 */

bool __fastcall DXGSYNCOBJECT::IsMonitoredFenceOpenedAsNativeFence(DXGSYNCOBJECT *this, struct ADAPTER_RENDER *a2)
{
  if ( (*((_DWORD *)this + 106) & 4) != 0 )
    return *((_BYTE *)DXGSYNCOBJECTCA::FindAdapterObject(this, a2) + 149);
  else
    return *((_BYTE *)this + 589);
}
