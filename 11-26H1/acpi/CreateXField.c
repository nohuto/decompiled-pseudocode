/*
 * XREFs of CreateXField @ 0x140011840
 * Callers:
 *     CreateField @ 0x14003CBF0 (CreateField.c)
 *     CreateDWordField @ 0x14003D1B0 (CreateDWordField.c)
 *     CreateByteField @ 0x14003D580 (CreateByteField.c)
 *     CreateWordField @ 0x14003E8C0 (CreateWordField.c)
 *     CreateBitField @ 0x140042BF0 (CreateBitField.c)
 *     CreateQWordField @ 0x14004D720 (CreateQWordField.c)
 * Callees:
 *     LogError @ 0x140007244 (LogError.c)
 *     AcpiDiagTraceAmlError @ 0x1400072EC (AcpiDiagTraceAmlError.c)
 *     PrintDebugMessage @ 0x140007434 (PrintDebugMessage.c)
 *     HeapAlloc @ 0x1400090C0 (HeapAlloc.c)
 *     CreateNameSpaceObject @ 0x140010FE0 (CreateNameSpaceObject.c)
 *     ValidateArgTypes @ 0x140013230 (ValidateArgTypes.c)
 *     memset @ 0x140072740 (memset.c)
 */

__int64 __fastcall CreateXField(__int64 a1, __int64 a2, __int64 a3, _QWORD *a4)
{
  unsigned int NameSpaceObject; // ebx
  __int64 v9; // rbp
  __int64 *v10; // rax
  __int64 v11; // rax
  __int64 v12; // rdx

  NameSpaceObject = ValidateArgTypes(a1, *(_QWORD *)(a2 + 80), 0LL, "BI");
  if ( !NameSpaceObject )
  {
    NameSpaceObject = CreateNameSpaceObject(
                        *(_QWORD **)(a1 + 320),
                        *(_QWORD *)(a3 + 32),
                        *(_QWORD *)(a1 + 80),
                        *(_QWORD *)(a1 + 88),
                        (__int64 *)(a2 + 64),
                        0);
    if ( !NameSpaceObject )
    {
      v9 = *(_QWORD *)(a2 + 64);
      *(_WORD *)(v9 + 66) = 14;
      *(_DWORD *)(v9 + 88) = 48;
      v10 = HeapAlloc(*(_QWORD **)(a1 + 320), 1145455176, 0x30u);
      *(_QWORD *)(v9 + 96) = v10;
      if ( v10 )
      {
        memset(v10, 0, *(unsigned int *)(v9 + 88));
        v11 = *(_QWORD *)(a2 + 80);
        v12 = *(_QWORD *)(v9 + 96);
        *a4 = v12;
        *(_QWORD *)v12 = *(_QWORD *)(v11 + 32);
        *(_DWORD *)(v12 + 8) = *(_DWORD *)(*(_QWORD *)(a2 + 80) + 24LL);
      }
      else
      {
        NameSpaceObject = -1073741670;
        LogError(-1073741670);
        AcpiDiagTraceAmlError(a1, -1073741670);
        PrintDebugMessage(0x26u, 0LL, 0LL, 0LL, 0LL);
      }
    }
  }
  return NameSpaceObject;
}
