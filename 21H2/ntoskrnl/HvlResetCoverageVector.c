/*
 * XREFs of HvlResetCoverageVector @ 0x1405436CC
 * Callers:
 *     ExpCovQueryHypervisorInformation @ 0x140A032A0 (ExpCovQueryHypervisorInformation.c)
 * Callees:
 *     HvlpReleaseHypercallPage @ 0x14039D8F0 (HvlpReleaseHypercallPage.c)
 *     HvcallInitiateHypercall @ 0x14039DF00 (HvcallInitiateHypercall.c)
 *     HvlpAcquireHypercallPage @ 0x14039DF90 (HvlpAcquireHypercallPage.c)
 *     __security_check_cookie @ 0x1403DF760 (__security_check_cookie.c)
 */

char HvlResetCoverageVector()
{
  _QWORD *v0; // rbx
  __int128 v2; // [rsp+20h] [rbp-E0h] BYREF
  __int64 v3; // [rsp+30h] [rbp-D0h]
  __int64 v4; // [rsp+38h] [rbp-C8h]
  __int128 v5; // [rsp+40h] [rbp-C0h] BYREF
  __int64 v6; // [rsp+50h] [rbp-B0h]
  __int64 v7; // [rsp+58h] [rbp-A8h]
  _BYTE v8[144]; // [rsp+60h] [rbp-A0h] BYREF
  _BYTE v9[2064]; // [rsp+F0h] [rbp-10h] BYREF

  v6 = 0LL;
  LODWORD(v7) = 0;
  v3 = 0LL;
  LODWORD(v4) = 0;
  v5 = 0LL;
  v2 = 0LL;
  v0 = HvlpAcquireHypercallPage((PHYSICAL_ADDRESS *)&v5, 1, (__int64)v9, 1032LL);
  HvlpAcquireHypercallPage((PHYSICAL_ADDRESS *)&v2, 2, (__int64)v8, 72LL);
  *(_DWORD *)v0 = 3;
  LOBYTE(v0) = (unsigned __int16)HvcallInitiateHypercall(147) == 0;
  HvlpReleaseHypercallPage((__int64)&v2);
  HvlpReleaseHypercallPage((__int64)&v5);
  return (char)v0;
}
