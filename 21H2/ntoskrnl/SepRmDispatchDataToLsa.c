/*
 * XREFs of SepRmDispatchDataToLsa @ 0x1402400A8
 * Callers:
    ...
    *(_DWORD *)&RequestMessage[40] = *(_DWORD *)(a1 + 32);
    ...
    else if ( (unsigned int)(v6 - 4) <= 2 )
    {
      ...
      if ( (unsigned int)v11 > 0x1D0 )
      {
        ...
        {
          v14 = SepAdtCopyToLsaSharedMemory(*(HANDLE *)v4, v12, v11);
          v2 = v14;
          if ( v14 < 0 )
          {
...
LABEL_5:
            if ( v2 >= 0 )
            {
              if ( *(_DWORD *)(a1 + 32) != 3 || *(_QWORD *)(a1 + 40) )
                v8 = ZwRequestWaitReplyPort(
                       *(HANDLE *)(v4 + 8),
                       (PPORT_MESSAGE)RequestMessage,
                       (PPORT_MESSAGE)ReplyMessage);
              else
                v8 = ZwRequestPort(*(HANDLE *)(v4 + 8), (PPORT_MESSAGE)RequestMessage);
              v2 = v8;
              if ( v8 >= 0 )
              {
                v9 = *(void **)(a1 + 40);
              ...
              {
                RegionSize = 0LL;
                return (unsigned int)ZwFreeVirtualMemory(
                                       *(HANDLE *)v4,
                                       (PVOID *)&RequestMessage[48],
                                       &RegionSize,
                                       0x8000u);
              }
            }
