/*
 * XREFs of VfCheckUserHandle @ 0x140A9466C
 * Callers:
...
__int16 __fastcall VfCheckUserHandle(ULONG_PTR BugCheckParameter2)
{
  ...
  {
    v7 = *i;
    LODWORD(v3) = VfDriverIsKernelImageAddress(*i);
    if ( !(_DWORD)v3 )
      break;
    if ( KernelVerifier )
      goto LABEL_9;
LABEL_24:
  ...
  if ( !(_DWORD)v3 )
  {
    if ( !ViDriverXDVBase )
      return v3;
    LOWORD(v3) = ViDriverXDVImageSize;
...
LABEL_9:
  ...
  if ( !(_DWORD)v3 )
  {
    ...
    {
LABEL_16:
      LOWORD(v3) = ViHandleBreaksEnabled;
      if ( ViHandleBreaksEnabled )
        LOWORD(v3) = VerifierBugCheckIfAppropriate(
                       0xC4u,
                       0xF6uLL,
                       BugCheckParameter2,
                       (ULONG_PTR)Process,
