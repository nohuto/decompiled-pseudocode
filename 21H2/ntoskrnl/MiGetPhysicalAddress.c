/*
 * XREFs of MiGetPhysicalAddress @ 0x14027B6A4
 * Callers:
 *     MmGetPhysicalAddress @ 0x14027B670 (MmGetPhysicalAddress.c)
 *     RtlMarkHiberPhase @ 0x14038D4F0 (RtlMarkHiberPhase.c)
 * Callees:
 *     MiGetSystemRegionType @ 0x14027B080 (MiGetSystemRegionType.c)
 *     MiSetNonPagedPoolNoSteal @ 0x14027C3BC (MiSetNonPagedPoolNoSteal.c)
 *     MiQueuePinDriverAddressLog @ 0x14027DD28 (MiQueuePinDriverAddressLog.c)
 *     MI_IS_PHYSICAL_ADDRESS @ 0x1402FDD20 (MI_IS_PHYSICAL_ADDRESS.c)
 *     MI_READ_PTE_LOCK_FREE @ 0x140317A10 (MI_READ_PTE_LOCK_FREE.c)
 *     MiPteInShadowRange @ 0x140317A80 (MiPteInShadowRange.c)
 *     MiFillPteHierarchy @ 0x140352E50 (MiFillPteHierarchy.c)
 *     MiVaToPfnEx @ 0x1403B8520 (MiVaToPfnEx.c)
 */

__int64 __fastcall MiGetPhysicalAddress(unsigned __int64 a1, __int64 *a2, _DWORD *a3)
{
  int v6; // eax
  __int64 v7; // rdi
  __int64 v8; // rdi
  unsigned __int64 v9; // r14
  __int64 v10; // rbx
  __int64 v11; // rbx
  __int64 v12; // rdi
  int SystemRegionType; // ebp
  __int64 v14; // rax
  unsigned __int64 v15; // rbx
  bool v16; // zf
  unsigned __int64 v17; // rbx
  __int64 result; // rax
  struct _LIST_ENTRY *Flink; // rax
  char v20; // dl
  __int64 v21; // rax
  _QWORD v23[9]; // [rsp+20h] [rbp-48h] BYREF
  __int64 v24; // [rsp+80h] [rbp+18h] BYREF

  *a3 = 0;
  memset(v23, 0, 32);
  MiFillPteHierarchy(a1, v23);
  v6 = MI_IS_PHYSICAL_ADDRESS(a1);
  v7 = v6;
  if ( v6 )
  {
    v15 = MiVaToPfnEx(a1);
    v16 = (MI_READ_PTE_LOCK_FREE(v23[v7]) & 0x800) == 0;
  }
  else
  {
    v8 = 4LL;
    do
    {
      v9 = v23[--v8];
      v10 = *(_QWORD *)v9;
      if ( (unsigned int)MiPteInShadowRange(v9)
        && (MiFlags & 0xC00000) != 0
        && KeGetCurrentThread()->ApcState.Process->AddressPolicy != 1 )
      {
        if ( (v10 & 1) == 0 )
          return 0LL;
        if ( (v10 & 0x20) == 0 || (v10 & 0x42) == 0 )
        {
          Flink = KeGetCurrentThread()->ApcState.Process[1].ProcessListEntry.Flink;
          if ( Flink )
          {
            v20 = v10 | 0x20;
            v21 = *((_QWORD *)&Flink->Flink + ((v9 >> 3) & 0x1FF));
            if ( (v21 & 0x20) == 0 )
              v20 = v10;
            LOBYTE(v10) = v20;
            if ( (v21 & 0x42) != 0 )
              LOBYTE(v10) = v20 | 0x42;
          }
        }
      }
      if ( (v10 & 1) == 0 )
        return 0LL;
    }
    while ( v8 != 1 );
    v11 = v23[0];
    v24 = MI_READ_PTE_LOCK_FREE(v23[0]);
    v12 = v24;
    SystemRegionType = MiGetSystemRegionType(a1);
    if ( SystemRegionType == 12 )
      MiQueuePinDriverAddressLog(a1, v12, 0LL);
    if ( (v12 & 1) == 0 )
      return 0LL;
    if ( SystemRegionType == 5 )
    {
      MiSetNonPagedPoolNoSteal(v11);
      v14 = MI_READ_PTE_LOCK_FREE(v11);
      LOWORD(v12) = v14;
      v24 = v14;
    }
    v15 = ((unsigned __int64)MI_READ_PTE_LOCK_FREE(&v24) >> 12) & 0xFFFFFFFFFFLL;
    v16 = (v12 & 0x800) == 0;
  }
  if ( !v16 )
    *a3 = 1;
  v17 = v15 << 12;
  HIDWORD(v24) = HIDWORD(v17);
  result = 1LL;
  LODWORD(v24) = (a1 & 0xFFF) + v17;
  *a2 = v24;
  return result;
}
