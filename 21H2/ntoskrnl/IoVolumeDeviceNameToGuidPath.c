/*
 * XREFs of IoVolumeDeviceNameToGuidPath @ 0x14068BB80
 * Callers:
  ...
  *(_WORD *)(Pool2 + 20) = *(_WORD *)a1;
  ...
  if ( DeviceObjectPointer >= 0 )
  {
    ...
    if ( v12 )
    {
      ...
      OutputBuffer = (_DWORD *)ExAllocatePool2(256LL, OutputBufferLength, 541536836LL);
      if ( OutputBuffer )
      {
        v20 = 1;
        KeInitializeEvent(&Event, NotificationEvent, 0);
        ...
        if ( v14 )
        {
          ...
          if ( DeviceObjectPointer >= 0 )
          {
            ...
            if ( OutputBuffer[1] )
            {
              while ( 1 )
              {
                v16 = (const wchar_t *)((char *)OutputBuffer + (unsigned int)OutputBuffer[6 * v15 + 2]);
                DeviceObject = (PDEVICE_OBJECT)(3LL * v15);
                if ( !wcsnicmp(L"\\??\\Volume", v16, 0xAuLL) )
                  break;
                if ( ++v15 >= OutputBuffer[1] )
                  goto LABEL_18;
              }
              v17 = OutputBuffer[2 * (_QWORD)DeviceObject + 3];
              *a2 = v17;
              v17 += 2;
              a2[1] = v17;
