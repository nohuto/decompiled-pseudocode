/*
 * XREFs of MiFreeZeroPagesNeeded @ 0x14045C75A
 * Callers:
 *     MiPruneStandbyPages @ 0x14045C812 (MiPruneStandbyPages.c)
 *     MiRebalanceZeroFreeLists @ 0x1405B3D90 (MiRebalanceZeroFreeLists.c)
 * Callees:
 *     MiNodeFreeZeroPages @ 0x140264BD8 (MiNodeFreeZeroPages.c)
 *     MiGetNodeStandbyPageCount @ 0x14045C7CC (MiGetNodeStandbyPageCount.c)
 */

__int64 __fastcall MiFreeZeroPagesNeeded(__int64 a1, unsigned int a2, unsigned int a3)
{
  __int64 v6; // r9
  __int64 v7; // r11

  if ( (unsigned __int64)MiNodeFreeZeroPages(*(_QWORD *)(a1 + 16) + 24512LL * a2, a3, 4096) >= 0x200
    || (unsigned __int64)MiGetNodeStandbyPageCount(a1, a2, a3, v6) < 0x1000 )
  {
    return 0LL;
  }
  else
  {
    return 1024 - v7;
  }
}
