/*
 * XREFs of CmpGetStateSepKeysRedirectionPathValue @ 0x140B132D0
 * Callers:
 *     CmpFindRedirectedDriverServiceStateNode @ 0x140B13268 (CmpFindRedirectedDriverServiceStateNode.c)
 * Callees:
 *     CmpFindValueByName @ 0x1406B4108 (CmpFindValueByName.c)
 *     HvpGetCellFlat @ 0x1406BF400 (HvpGetCellFlat.c)
 *     HvpReleaseCellFlat @ 0x1406BF450 (HvpReleaseCellFlat.c)
 *     HvpReleaseCellPaged @ 0x1407C97C0 (HvpReleaseCellPaged.c)
 *     HvpGetCellPaged @ 0x1407C9820 (HvpGetCellPaged.c)
 *     CmpFindSubKeyByName @ 0x14082F108 (CmpFindSubKeyByName.c)
 *     CmpValueToData @ 0x14082F8AC (CmpValueToData.c)
 *     CmpFindHiveSubKey @ 0x140B14608 (CmpFindHiveSubKey.c)
 */

char __fastcall CmpGetStateSepKeysRedirectionPathValue(
        ULONG_PTR BugCheckParameter3,
        ULONG_PTR a2,
        __int64 a3,
        __int64 a4,
        _DWORD *a5)
{
  unsigned int SubKeyByName; // edi
  unsigned int v10; // edi
  __int64 v11; // rax
  unsigned int ValueByName; // edi
  ULONG_PTR v13; // rax
  PVOID v14; // rax
  bool v15; // zf
  __int64 v16; // r9
  unsigned __int16 v17; // r8
  unsigned __int16 i; // dx
  __int64 v19; // [rsp+30h] [rbp-30h] BYREF
  __int64 v20; // [rsp+38h] [rbp-28h] BYREF
  __int64 v21; // [rsp+40h] [rbp-20h] BYREF
  _QWORD v22[3]; // [rsp+48h] [rbp-18h] BYREF
  unsigned int v23; // [rsp+90h] [rbp+30h] BYREF
  int v24; // [rsp+94h] [rbp+34h]

  v24 = HIDWORD(a3);
  v19 = 0xFFFFFFFFLL;
  v20 = 0xFFFFFFFFLL;
  v21 = 0xFFFFFFFFLL;
  v23 = 0;
  v22[1] = 0LL;
  v22[0] = 0xFFFFFFFFLL;
  if ( !(unsigned __int8)CmpFindHiveSubKey(BugCheckParameter3, a2, (__int64)v22) )
    return 0;
  SubKeyByName = CmpFindSubKeyByName(BugCheckParameter3);
  if ( (*(_BYTE *)(BugCheckParameter3 + 140) & 1) != 0 )
    HvpReleaseCellFlat(BugCheckParameter3, v22);
  else
    HvpReleaseCellPaged(BugCheckParameter3, (unsigned int *)v22);
  if ( SubKeyByName == -1 )
    return 0;
  if ( !((*(_BYTE *)(BugCheckParameter3 + 140) & 1) != 0
       ? HvpGetCellFlat(BugCheckParameter3, SubKeyByName, &v19)
       : HvpGetCellPaged(BugCheckParameter3, SubKeyByName, (unsigned int *)&v19)) )
    return 0;
  v10 = CmpFindSubKeyByName(BugCheckParameter3);
  if ( (*(_BYTE *)(BugCheckParameter3 + 140) & 1) != 0 )
    HvpReleaseCellFlat(BugCheckParameter3, &v19);
  else
    HvpReleaseCellPaged(BugCheckParameter3, (unsigned int *)&v19);
  if ( v10 == -1 )
    return 0;
  v11 = (*(_BYTE *)(BugCheckParameter3 + 140) & 1) != 0
      ? HvpGetCellFlat(BugCheckParameter3, v10, &v20)
      : HvpGetCellPaged(BugCheckParameter3, v10, (unsigned int *)&v20);
  if ( !v11 )
    return 0;
  ValueByName = CmpFindValueByName(BugCheckParameter3, v11, (int)&CmpTargetNtPathString);
  if ( (*(_BYTE *)(BugCheckParameter3 + 140) & 1) != 0 )
    HvpReleaseCellFlat(BugCheckParameter3, &v20);
  else
    HvpReleaseCellPaged(BugCheckParameter3, (unsigned int *)&v20);
  if ( ValueByName == -1 )
    return 0;
  v13 = (*(_BYTE *)(BugCheckParameter3 + 140) & 1) != 0
      ? HvpGetCellFlat(BugCheckParameter3, ValueByName, &v21)
      : HvpGetCellPaged(BugCheckParameter3, ValueByName, (unsigned int *)&v21);
  if ( !v13 )
    return 0;
  v14 = CmpValueToData(BugCheckParameter3, ValueByName, v13, &v23, a5);
  v15 = (*(_BYTE *)(BugCheckParameter3 + 140) & 1) == 0;
  *(_QWORD *)(a4 + 8) = v14;
  if ( v15 )
    HvpReleaseCellPaged(BugCheckParameter3, (unsigned int *)&v21);
  else
    HvpReleaseCellFlat(BugCheckParameter3, &v21);
  v16 = *(_QWORD *)(a4 + 8);
  if ( !v16 )
    return 0;
  v17 = v23;
  *(_WORD *)(a4 + 2) = v23;
  *(_WORD *)a4 = 0;
  if ( v17 )
  {
    for ( i = 0; i < v17; *(_WORD *)a4 = i )
    {
      if ( !*(_WORD *)(v16 + 2 * ((unsigned __int64)i >> 1)) )
        break;
      i += 2;
    }
  }
  return 1;
}
