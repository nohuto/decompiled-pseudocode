/*
 * XREFs of MiLoadSectionIntoVsmEnclave @ 0x14097A714
 * Callers:
 *     NtLoadEnclaveData @ 0x14097B500 (NtLoadEnclaveData.c)
 * Callees:
 *     MiUnlockVad @ 0x140281C44 (MiUnlockVad.c)
 *     MiAllocatePool @ 0x1402828F0 (MiAllocatePool.c)
 *     MiLockVad @ 0x14030B7F0 (MiLockVad.c)
 *     MiVadDeleted @ 0x14030EB80 (MiVadDeleted.c)
 *     MiCommitExistingVad @ 0x14032C1B0 (MiCommitExistingVad.c)
 *     MiUnlockAndDereferenceVad @ 0x14032E700 (MiUnlockAndDereferenceVad.c)
 *     MiCountCommittedPages @ 0x1405A8968 (MiCountCommittedPages.c)
 *     MiPrefetchControlArea @ 0x1406B85C8 (MiPrefetchControlArea.c)
 *     MiDecommitRegion @ 0x1407BC9B0 (MiDecommitRegion.c)
 *     VslLoadEnclaveModule @ 0x140932778 (VslLoadEnclaveModule.c)
 *     MiMapImageForEnclaveUse @ 0x14097A9C4 (MiMapImageForEnclaveUse.c)
 *     MiUnmapImageForEnclaveUse @ 0x14097AC04 (MiUnmapImageForEnclaveUse.c)
 *     ExFreePoolWithTag @ 0x140A6E010 (ExFreePoolWithTag.c)
 */

__int64 __fastcall MiLoadSectionIntoVsmEnclave(
        __int64 a1,
        ULONG_PTR a2,
        char a3,
        __int64 a4,
        unsigned __int64 a5,
        ULONG a6,
        struct _MDL *a7,
        _QWORD *a8)
{
  __int64 v8; // rax
  struct _KTHREAD *CurrentThread; // r14
  _DWORD *Pool; // rbx
  int v15; // edi
  __int64 v17; // r8
  unsigned __int64 v18; // r13
  unsigned __int64 v19; // r15
  unsigned __int64 v20; // rdi
  unsigned __int64 v21; // r14
  __int64 v22; // r12
  _QWORD *v23; // rcx
  _WORD v24[2]; // [rsp+60h] [rbp-20h] BYREF
  int v25; // [rsp+64h] [rbp-1Ch] BYREF
  __int64 v26; // [rsp+68h] [rbp-18h] BYREF
  unsigned __int64 v27; // [rsp+70h] [rbp-10h] BYREF
  __int64 v28; // [rsp+78h] [rbp-8h]
  char v29; // [rsp+B8h] [rbp+38h] BYREF

  v8 = *(_QWORD *)(a2 + 72);
  v24[0] = 0;
  v25 = 0;
  CurrentThread = KeGetCurrentThread();
  v27 = 0LL;
  v29 = 0;
  v28 = v8;
  MiUnlockVad((__int64)CurrentThread, a2);
  Pool = 0LL;
  v26 = 0LL;
  if ( a6 < 0x10 )
  {
    v15 = -1073741820;
LABEL_3:
    MiLockVad((__int64)CurrentThread, a2);
    goto LABEL_4;
  }
  Pool = MiAllocatePool(256, 0x30uLL, 0x4D456D4Du);
  if ( !Pool )
  {
    v15 = -1073741670;
    goto LABEL_3;
  }
  LOBYTE(v17) = a3;
  v15 = MiMapImageForEnclaveUse(a7->Next, a4, v17, &v26, &v27);
  if ( v15 < 0 )
    goto LABEL_3;
  v18 = v27;
  v19 = a5;
  v20 = ((unsigned __int64)*(unsigned int *)(*(_QWORD *)v27 + 8LL) << 12) + a5 - 1;
  v27 = v20;
  MiLockVad((__int64)CurrentThread, a2);
  if ( (unsigned int)MiVadDeleted(a2) || MiCountCommittedPages(v19, v20, a2, a1 + 1664) )
  {
    v15 = -1073741800;
  }
  else
  {
    v21 = v18 + 128;
    v22 = *(_QWORD *)(v18 + 136) << 9;
    do
    {
      v15 = MiCommitExistingVad(
              a2,
              v19 + (((*(_QWORD *)(v21 + 8) << 9) - v22) & 0xFFFFFFFFFFFFF000uLL),
              (unsigned __int64)*(unsigned int *)(v21 + 44) << 12,
              4u,
              0,
              0LL,
              0,
              0,
              0LL,
              &v25,
              v24);
      if ( v15 < 0 )
        goto LABEL_23;
      v21 = *(_QWORD *)(v21 + 16);
    }
    while ( v21 );
    MiPrefetchControlArea(v18, CLFS_LSN_NULL_EXT, 0LL, 1, 0, 0xFFFFFFFF);
    v15 = VslLoadEnclaveModule(
            *(_QWORD *)(v28 + 24),
            v19,
            *(_QWORD *)(*(_QWORD *)(v18 + 96) + 56LL),
            v26,
            a7,
            a6,
            &v29,
            Pool + 9,
            Pool + 10);
    if ( v15 < 0 )
    {
LABEL_23:
      MiDecommitRegion(a2, v19, v27);
      goto LABEL_4;
    }
    if ( v29 )
    {
      Pool[8] = 2;
      *((_QWORD *)Pool + 2) = v19;
      v23 = *(_QWORD **)(a2 + 88);
      if ( *v23 != a2 + 80 )
        __fastfail(3u);
      *(_QWORD *)Pool = a2 + 80;
      *((_QWORD *)Pool + 1) = v23;
      *v23 = Pool;
      *(_QWORD *)(a2 + 88) = Pool;
      *a8 = Pool;
      Pool = 0LL;
    }
  }
LABEL_4:
  MiUnlockAndDereferenceVad((char *)a2);
  if ( v26 )
    MiUnmapImageForEnclaveUse();
  if ( Pool )
    ExFreePoolWithTag(Pool, 0);
  return (unsigned int)v15;
}
