/*
 * XREFs of RtlpPrintErrorInformation @ 0x18010AA54
 * Callers:
...
ULONG RtlpPrintErrorInformation()
{
  ...
  if ( NtCurrentPeb()->Ldr )
    ...
    DbgPrint("HEAP: ");
  DbgPrint("Heap error detected at %p (heap handle %p)\n", (const void *)qword_180169808, (const void *)qword_180169800);
  if ( dword_1801697F8 > 8 )
  {
    switch ( dword_1801697F8 )
    {
      case 9:
  ...
  else if ( dword_1801697F8 == 8 )
  {
    v0 = "heap_failure_block_not_busy";
  }
  else if ( dword_1801697F8 )
  {
    switch ( dword_1801697F8 )
    {
      case 1:
  ...
  if ( qword_180169820 )
  {
    if ( NtCurrentPeb()->Ldr )
      ...
      DbgPrint("HEAP: ");
    DbgPrint("Parameter3: %p\n", (const void *)qword_180169820);
  }
  if ( qword_180169828 || qword_180169830 )
  {
    if ( NtCurrentPeb()->Ldr )
      DbgPrint("HEAP[%wZ]: ", &NtCurrentPeb()->Ldr->InLoadOrderModuleList.Flink[5].Blink);
    else
    ...
    DbgPrint(
      ...
      (const void *)qword_180169828,
      (const void *)qword_180169830);
  }
  if ( NtCurrentPeb()->Ldr )
    DbgPrint("HEAP[%wZ]: ", &NtCurrentPeb()->Ldr->InLoadOrderModuleList.Flink[5].Blink);
  else
