/*
 * XREFs of ?Add@CPreWalkVisual@CPreComputeContext@@SAXPEAVCVisual@@PEBVCVisualTree@@_N2PEAV?$vector@VCPreWalkVisual@CPreComputeContext@@V?$allocator@VCPreWalkVisual@CPreComputeContext@@@std@@@std@@@Z @ 0x1800D104C
 * Callers:
...
void __fastcall CPreComputeContext::CPreWalkVisual::Add(__int64 a1, __int64 a2, char a3, char a4, __int64 *a5)
{
  ...
  if ( a1 )
  {
    ...
    for ( i = *a5; ; i += 16LL )
    {
      if ( i == a5[1] )
      {
        std::vector<CPreComputeContext::CPreWalkVisual>::emplace_back<CVisual * &,bool &,bool &>(
          (__int64)a5,
          &v8,
          &v9,
          &v10);
