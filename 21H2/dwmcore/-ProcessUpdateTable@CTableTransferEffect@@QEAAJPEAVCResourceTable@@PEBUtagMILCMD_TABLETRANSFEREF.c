/*
 * XREFs of ?ProcessUpdateTable@CTableTransferEffect@@QEAAJPEAVCResourceTable@@PEBUtagMILCMD_TABLETRANSFEREFFECT_UPDATETABLE@@PEBXI@Z @ 0x1802421D8
 * Callers:
        ...
        unsigned int a5)
{
  ...
  if ( a4 )
  {
    if ( !*((_DWORD *)a3 + 2) )
    {
      v9 = (char *)this + 32 * *((unsigned int *)a3 + 3) + 8 * *((unsigned int *)a3 + 3) + 168;
      *((_DWORD *)v9 + 6) = 0;
      DynArrayImpl<0>::ShrinkToSize((__int64)v9, 4u);
    }
    v10 = DynArray<float,0>::AddMultipleAndSet(
            (unsigned __int64)this + 32 * *((unsigned int *)a3 + 3) + 8 * *((unsigned int *)a3 + 3) + 168,
            a4,
            a5 >> 2);
    v5 = v10;
    if ( v10 < 0 )
      MilInstrumentationCheckHR_MaybeFailFast(v11, 0LL, 0LL, v10, 0x2Au);
  }
  return v5;
}
