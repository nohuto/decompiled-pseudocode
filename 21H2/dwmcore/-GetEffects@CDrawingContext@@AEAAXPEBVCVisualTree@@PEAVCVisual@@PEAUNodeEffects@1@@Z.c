/*
 * XREFs of ?GetEffects@CDrawingContext@@AEAAXPEBVCVisualTree@@PEAVCVisual@@PEAUNodeEffects@1@@Z @ 0x18009DBB0
 * Callers:
 *     ?PreSubgraph@CDrawingContext@@QEAAJPEBVCVisualTree@@PEA_N@Z @ 0x18009FA80 (-PreSubgraph@CDrawingContext@@QEAAJPEBVCVisualTree@@PEA_N@Z.c)
 * Callees:
 *     ?GetEffectInternal@CVisual@@QEBAPEAVCEffect@@XZ @ 0x18004D434 (-GetEffectInternal@CVisual@@QEBAPEAVCEffect@@XZ.c)
 *     ?GetCount@CPtrArrayBase@@IEBA_KXZ @ 0x180062474 (-GetCount@CPtrArrayBase@@IEBA_KXZ.c)
 *     ?LayerVisualHasTreeEffect@CDrawingContext@@AEBA_NPEBVCLayerVisual@@@Z @ 0x1800D90C8 (-LayerVisualHasTreeEffect@CDrawingContext@@AEBA_NPEBVCLayerVisual@@@Z.c)
 *     __security_check_cookie @ 0x180100650 (__security_check_cookie.c)
 *     _guard_xfg_dispatch_icall_nop @ 0x1801051D0 (_guard_xfg_dispatch_icall_nop.c)
 *     ?GetColorSpace@CVisual@@QEBA?AW4DXGI_COLOR_SPACE_TYPE@@XZ @ 0x1801B29B4 (-GetColorSpace@CVisual@@QEBA-AW4DXGI_COLOR_SPACE_TYPE@@XZ.c)
 *     ?GetResampleMode@CVisual@@QEBA?AW4Enum@CompositionResampleMode@@XZ @ 0x1801B2BB4 (-GetResampleMode@CVisual@@QEBA-AW4Enum@CompositionResampleMode@@XZ.c)
 */

void __fastcall CDrawingContext::GetEffects(
        CDrawingContext *this,
        struct CVisual **a2,
        struct CVisual *a3,
        struct CDrawingContext::NodeEffects *a4)
{
  __int64 v7; // rax
  float v8; // xmm0_4
  float v9; // xmm1_4
  float v10; // xmm7_4
  int v11; // eax
  float v12; // xmm0_4
  float v13; // xmm1_4
  _DWORD *v14; // r8
  float v15; // xmm1_4
  struct CEffect *EffectInternal; // r14
  struct CEffect *v17; // rax
  float v18; // edx
  float *v19; // rcx
  __int64 v20; // rcx
  _BYTE *v21; // rdx
  _QWORD *v22; // r9
  unsigned int v23; // eax
  void (__fastcall ***v24)(_QWORD, _BYTE *); // rcx
  enum DXGI_COLOR_SPACE_TYPE ColorSpace; // eax
  int v26; // r10d
  bool v27; // zf
  int v28; // eax
  _BYTE v29[16]; // [rsp+28h] [rbp-70h] BYREF

  *(_QWORD *)a4 = a3;
  if ( a3 == a2[8] )
  {
    v9 = *(float *)&FLOAT_1_0;
  }
  else
  {
    v7 = *((_QWORD *)a3 + 28);
    if ( (*(_DWORD *)(v7 + 4) & 0x8000000) != 0 )
    {
      v18 = *(float *)(v7 + 12);
      v19 = (float *)(v7 + 12);
      if ( (LODWORD(v18) & 0x7F000000) != 0x5000000 )
      {
        do
        {
          v19 = (float *)((char *)v19 + (LODWORD(v18) & 0xFFFFFF) + 4);
          v18 = *v19;
        }
        while ( (*(_DWORD *)v19 & 0x7F000000) != 0x5000000 );
      }
      v8 = v19[1];
    }
    else
    {
      v8 = *(float *)&FLOAT_1_0;
    }
    v9 = fminf(1.0, fmaxf(v8, 0.0));
  }
  *((float *)a4 + 13) = v9;
  v10 = *(float *)&FLOAT_1_0;
  if ( (**((_DWORD **)a3 + 29) & 0x400000) != 0 )
  {
    EffectInternal = CVisual::GetEffectInternal(a3);
    if ( (*(unsigned __int8 (__fastcall **)(struct CEffect *, __int64))(*(_QWORD *)EffectInternal + 56LL))(
           EffectInternal,
           56LL) )
    {
      v10 = *((float *)EffectInternal + 20);
    }
  }
  v11 = *((_DWORD *)this + 776);
  v12 = fminf(1.0, fmaxf(v10, 0.0));
  if ( v11 )
    v13 = *(float *)(*((_QWORD *)this + 390) + 4LL * (unsigned int)(v11 - 1));
  else
    v13 = *(float *)&FLOAT_1_0;
  *((float *)a4 + 10) = v12 * v13;
  v14 = (_DWORD *)*((_QWORD *)a3 + 29);
  if ( (*v14 & 0x2000000) != 0 )
  {
    v20 = (unsigned int)v14[1];
    v21 = v14 + 2;
    v22 = 0LL;
    v23 = 0;
    if ( (_DWORD)v20 )
    {
      while ( *v21 != 7 )
      {
        ++v23;
        ++v21;
        if ( v23 >= (unsigned int)v20 )
          goto LABEL_45;
      }
    }
    else
    {
LABEL_45:
      if ( v23 >= (unsigned int)v20 )
        goto LABEL_47;
    }
    v22 = (_QWORD *)((char *)v14 + v20 + 8LL * v23 - (((_BYTE)v20 + 15) & 7) + 15);
LABEL_47:
    if ( *v22 )
      *((_BYTE *)a4 + 209) = 1;
  }
  if ( (**((_DWORD **)a3 + 29) & 0x400000) != 0 )
  {
    v17 = CVisual::GetEffectInternal(a3);
    if ( (*(unsigned __int8 (__fastcall **)(struct CEffect *, __int64))(*(_QWORD *)v17 + 56LL))(v17, 59LL) )
      *((_BYTE *)a4 + 210) = 1;
  }
  if ( (*(_DWORD *)(*((_QWORD *)a3 + 28) + 4LL) & 0x4000000) != 0 )
  {
    v24 = (void (__fastcall ***)(_QWORD, _BYTE *))(*((_QWORD *)this + 4)
                                                 + 8LL
                                                 + *(int *)(*(_QWORD *)(*((_QWORD *)this + 4) + 8LL) + 12LL));
    (**v24)(v24, v29);
    ColorSpace = CVisual::GetColorSpace(a3);
    if ( ColorSpace != v26 )
      *((_BYTE *)a4 + 212) = 1;
  }
  if ( (*(unsigned __int8 (__fastcall **)(struct CVisual *, __int64))(*(_QWORD *)a3 + 56LL))(a3, 91LL) )
    *((_BYTE *)a4 + 211) = CDrawingContext::LayerVisualHasTreeEffect(this, a3);
  v15 = *((float *)a4 + 10);
  if ( v15 < 1.0 && COERCE_FLOAT(COERCE_UNSIGNED_INT(v15 - 1.0) & _xmm) >= 0.0000011920929 )
  {
    if ( !(unsigned int)CPtrArrayBase::GetCount((struct CVisual *)((char *)a3 + 80))
      && (*((_BYTE *)a3 + 100) & 1) != 0
      && *((_DWORD *)this + 77) != 4
      || *((_DWORD *)this + 78) == 1
      || *((_DWORD *)a3 + 27) == 1 )
    {
      *((_DWORD *)a4 + 11) = 1;
    }
    else
    {
      *((_DWORD *)a4 + 11) = 2;
      if ( EventEnabled(Microsoft_Windows_Dwm_Core_Provider_Context[0], &EVTDESC_ETWGUID_LAYEREVENT_BeginLayer_Start) )
      {
        if ( (unsigned int)CPtrArrayBase::GetCount((struct CVisual *)((char *)a3 + 80)) )
          *((_DWORD *)a4 + 51) |= 0x20u;
        if ( (*((_BYTE *)a3 + 100) & 1) == 0 )
          *((_DWORD *)a4 + 51) |= 0x40u;
      }
    }
  }
  if ( (*(_DWORD *)(*((_QWORD *)a3 + 28) + 4LL) & 0x2000000) != 0 )
    *((_DWORD *)a4 + 12) = CVisual::GetResampleMode(a3);
  if ( *((_BYTE *)a4 + 210) || *((_BYTE *)a4 + 211) || *((_BYTE *)a4 + 212) || *((_DWORD *)a4 + 12) )
  {
    v27 = *((_BYTE *)a4 + 211) == 0;
    *((_DWORD *)a4 + 50) = 3;
    v28 = *((_DWORD *)a4 + 50);
    if ( !v27 )
      v28 = 4;
    *((_DWORD *)a4 + 50) = v28;
  }
}
