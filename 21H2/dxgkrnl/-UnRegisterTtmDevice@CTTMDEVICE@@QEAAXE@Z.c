/*
 * XREFs of ?UnRegisterTtmDevice@CTTMDEVICE@@QEAAXE@Z @ 0x1C0349064
 * Callers:
 *     ?SetConsoleSession@DXGSESSIONMGR@@QEAAXE@Z @ 0x1C01DAB10 (-SetConsoleSession@DXGSESSIONMGR@@QEAAXE@Z.c)
 *     ?DestroyTtmDeviceWorker@DXGSESSIONMGR@@CAXPEAX@Z @ 0x1C0347AF0 (-DestroyTtmDeviceWorker@DXGSESSIONMGR@@CAXPEAX@Z.c)
 * Callees:
 *     DxgkLogInternalTriageEvent @ 0x1C0008E10 (DxgkLogInternalTriageEvent.c)
 *     ?DXGGLOBAL_GetGlobal@@YAPEAVDXGGLOBAL@@XZ @ 0x1C000BBD0 (-DXGGLOBAL_GetGlobal@@YAPEAVDXGGLOBAL@@XZ.c)
 *     ?Attach@CPROCESSATTACHHELPER@@QEAAXPEAU_EPROCESS@@@Z @ 0x1C00138D4 (-Attach@CPROCESSATTACHHELPER@@QEAAXPEAU_EPROCESS@@@Z.c)
 *     __security_check_cookie @ 0x1C002B170 (__security_check_cookie.c)
 */

void __fastcall CTTMDEVICE::UnRegisterTtmDevice(CTTMDEVICE *this, char a2)
{
  __int64 v4; // rax
  struct _KEVENT *p_Event; // rax
  __int64 v6; // rdx
  CTTMDEVICE **v7; // r8
  CTTMDEVICE **v8; // rdx
  char v9; // al
  struct _KEVENT Event; // [rsp+58h] [rbp-29h] BYREF
  struct _KAPC_STATE ApcState; // [rsp+70h] [rbp-11h] BYREF
  char v12; // [rsp+A0h] [rbp+1Fh]

  memset(&Event, 0, sizeof(Event));
  if ( *(struct _KTHREAD **)(*((_QWORD *)DXGGLOBAL_GetGlobal() + 122) + 344LL) != KeGetCurrentThread() )
  {
    WdLogSingleEntry1(1LL, 6005LL);
    DxgkLogInternalTriageEvent(
      0LL,
      262146,
      -1,
      (__int64)L"DXGGLOBAL::GetGlobal()->GetSessionMgr()->IsTtmDeviceListLockOwner()",
      6005LL,
      0LL,
      0LL,
      0LL,
      0LL);
  }
  if ( *((_BYTE *)this + 913) )
  {
    if ( !a2
      || (WdLogSingleEntry1(1LL, 6012LL),
          DxgkLogInternalTriageEvent(0LL, 262146, -1, (__int64)L"!m_DeleteOnStop || !Wait", 6012LL, 0LL, 0LL, 0LL, 0LL),
          *((_BYTE *)this + 913)) )
    {
      if ( *(CTTMDEVICE **)this != this )
      {
        WdLogSingleEntry1(1LL, 6018LL);
        DxgkLogInternalTriageEvent(
          0LL,
          262146,
          -1,
          (__int64)L"!m_DeleteOnStop || IsListEmpty(&m_ListSessionMan)",
          6018LL,
          0LL,
          0LL,
          0LL,
          0LL);
      }
    }
  }
  if ( !*((_QWORD *)this + 8) )
  {
    WdLogSingleEntry1(1LL, 6027LL);
    DxgkLogInternalTriageEvent(0LL, 262146, -1, (__int64)L"m_pOwnerAdapter != NULL", 6027LL, 0LL, 0LL, 0LL, 0LL);
  }
  v4 = _InterlockedExchangeAdd((volatile signed __int32 *)this + 156, 1u) & 0x1F;
  *((_DWORD *)this + 2 * v4 + 157) = 1073741825;
  *((_DWORD *)this + 2 * (unsigned int)v4 + 158) = MEMORY[0xFFFFF78000000008] / 0x2710uLL;
  ExWaitForRundownProtectionRelease((PEX_RUNDOWN_REF)this + 6);
  if ( *((_QWORD *)this + 7) )
  {
    WdLogSingleEntry1(1LL, 6047LL);
    DxgkLogInternalTriageEvent(0LL, 262146, -1, (__int64)L"m_pWaitEvent == NULL", 6047LL, 0LL, 0LL, 0LL, 0LL);
  }
  if ( a2 )
  {
    KeInitializeEvent(&Event, SynchronizationEvent, 0);
    p_Event = &Event;
  }
  else
  {
    p_Event = 0LL;
  }
  *((_QWORD *)this + 7) = p_Event;
  v6 = *((_QWORD *)this + 4);
  v12 = 0;
  CPROCESSATTACHHELPER::Attach(&ApcState, *(PRKPROCESS *)(v6 + 18648));
  *((_QWORD *)this + 4) = 0LL;
  v7 = (CTTMDEVICE **)*((_QWORD *)this + 2);
  if ( v7[1] != (CTTMDEVICE *)((char *)this + 16)
    || (v8 = (CTTMDEVICE **)*((_QWORD *)this + 3), *v8 != (CTTMDEVICE *)((char *)this + 16)) )
  {
    __fastfail(3u);
  }
  *v8 = (CTTMDEVICE *)v7;
  v7[1] = (CTTMDEVICE *)v8;
  TtmNotifyDeviceDeparture(1LL, this);
  v9 = v12;
  if ( v12 )
  {
    KeUnstackDetachProcess(&ApcState);
    v9 = 0;
    v12 = 0;
  }
  if ( a2 )
  {
    KeWaitForSingleObject(&Event, Executive, 0, 0, 0LL);
    v9 = v12;
  }
  if ( v9 )
    KeUnstackDetachProcess(&ApcState);
}
