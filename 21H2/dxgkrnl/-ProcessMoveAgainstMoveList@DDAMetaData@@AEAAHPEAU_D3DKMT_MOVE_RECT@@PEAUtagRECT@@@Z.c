/*
 * XREFs of ?ProcessMoveAgainstMoveList@DDAMetaData@@AEAAHPEAU_D3DKMT_MOVE_RECT@@PEAUtagRECT@@@Z @ 0x1C0327554
 * Callers:
 *     ?ProcessNewMoveRegions@DDAMetaData@@AEAAHPEAU_D3DKMT_PRESENT_RGNS@@@Z @ 0x1C0327838 (-ProcessNewMoveRegions@DDAMetaData@@AEAAHPEAU_D3DKMT_PRESENT_RGNS@@@Z.c)
 * Callees:
 *     DxgkLogInternalTriageEvent @ 0x1C0008E10 (DxgkLogInternalTriageEvent.c)
 *     ?AddToDirtyList@DDAMetaData@@AEAAHPEAUtagRECT@@@Z @ 0x1C0325C84 (-AddToDirtyList@DDAMetaData@@AEAAHPEAUtagRECT@@@Z.c)
 *     ?AddToMoveList@DDAMetaData@@AEAAHPEAU_D3DKMT_MOVE_RECT@@@Z @ 0x1C0325E54 (-AddToMoveList@DDAMetaData@@AEAAHPEAU_D3DKMT_MOVE_RECT@@@Z.c)
 *     ?ProcessNewDirtyRects@DDAMetaData@@AEAAHPEAU_D3DKMT_PRESENT_RGNS@@@Z @ 0x1C032772C (-ProcessNewDirtyRects@DDAMetaData@@AEAAHPEAU_D3DKMT_PRESENT_RGNS@@@Z.c)
 *     ?RectsOverlap@@YA?AW4RECT_OVERLAP@@PEAUtagRECT@@00@Z @ 0x1C0328868 (-RectsOverlap@@YA-AW4RECT_OVERLAP@@PEAUtagRECT@@00@Z.c)
 */

__int64 __fastcall DDAMetaData::ProcessMoveAgainstMoveList(
        DDAMetaData *this,
        struct _D3DKMT_MOVE_RECT *a2,
        struct tagRECT *a3)
{
  unsigned int v5; // ebp
  __int64 v6; // r14
  RECT *p_DestRect; // r12
  UINT v8; // r8d
  int v9; // ecx
  UINT v10; // ecx
  int v11; // ebx
  __int64 v12; // rcx
  __int64 v13; // r8
  int v14; // r13d
  __int64 v15; // r8
  int v16; // eax
  __int64 v17; // rbx
  const wchar_t *v18; // r9
  __int64 *v20; // rax
  __int64 v21; // rdx
  __int64 v22; // rax
  struct _D3DKMT_PRESENT_RGNS v23; // [rsp+50h] [rbp-48h] BYREF

  v5 = 0;
  v6 = **((_QWORD **)this + 6);
  if ( *((_DWORD *)this + 11) )
  {
    p_DestRect = &a2->DestRect;
    while ( 1 )
    {
      v8 = *(_DWORD *)(v6 + 4);
      v9 = *(_DWORD *)v6 + *(_DWORD *)(v6 + 16) - *(_DWORD *)(v6 + 8);
      v23.DirtyRectCount = *(_DWORD *)v6;
      LODWORD(v23.pDirtyRects) = v9;
      v10 = v8 + *(_DWORD *)(v6 + 20) - *(_DWORD *)(v6 + 12);
      *(&v23.DirtyRectCount + 1) = v8;
      HIDWORD(v23.pDirtyRects) = v10;
      v11 = RectsOverlap(p_DestRect, v6 + 8, 0LL);
      v14 = RectsOverlap(v12, &v23, v13);
      v16 = RectsOverlap(a3, v6 + 8, v15);
      if ( v11 || v14 || v16 )
        break;
      ++v5;
      v6 += 24LL;
      if ( v5 >= *((_DWORD *)this + 11) )
        goto LABEL_7;
    }
    if ( !(unsigned int)DDAMetaData::AddToDirtyList(this, (struct tagRECT *)(v6 + 8)) )
    {
      v17 = 900LL;
      WdLogSingleEntry1(2LL, 900LL);
      v18 = L"Failed to add existing move to dirty list";
      goto LABEL_11;
    }
    v20 = (__int64 *)*((_QWORD *)this + 6);
    *(_QWORD *)&v23.DirtyRectCount = 1LL;
    v23.pDirtyRects = p_DestRect;
    v21 = *v20;
    v22 = (unsigned int)--*((_DWORD *)this + 11);
    *(_OWORD *)v6 = *(_OWORD *)(v21 + 24 * v22);
    *(_QWORD *)(v6 + 16) = *(_QWORD *)(v21 + 24 * v22 + 16);
    *(_OWORD *)&v23.MoveRectCount = 0LL;
    if ( !(unsigned int)DDAMetaData::ProcessNewDirtyRects(this, &v23) )
    {
      v17 = 908LL;
      WdLogSingleEntry1(2LL, 908LL);
      v18 = L"Failed to add new move to dirty list";
      goto LABEL_11;
    }
  }
  else
  {
LABEL_7:
    if ( !(unsigned int)DDAMetaData::AddToMoveList(this, a2) )
    {
      v17 = 925LL;
      WdLogSingleEntry1(2LL, 925LL);
      v18 = L"Failed to add new to move list";
LABEL_11:
      DxgkLogInternalTriageEvent(0LL, 0x40000, -1, (__int64)v18, v17, 0LL, 0LL, 0LL, 0LL);
      return 0LL;
    }
  }
  return 1LL;
}
