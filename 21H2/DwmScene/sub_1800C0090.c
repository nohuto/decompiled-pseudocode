/*
 * XREFs of sub_1800C0090 @ 0x1800C0090
 * Callers:
 *     <none>
 * Callees:
 *     ??0bad_function_call@std@@QEAA@XZ @ 0x180010000 (--0bad_function_call@std@@QEAA@XZ.c)
 *     ??4?$shared_ptr@V__ExceptionPtr@@@std@@QEAAAEAV01@$$QEAV01@@Z @ 0x18001145C (--4-$shared_ptr@V__ExceptionPtr@@@std@@QEAAAEAV01@$$QEAV01@@Z.c)
 *     ??4?$shared_ptr@V__ExceptionPtr@@@std@@QEAAAEAV01@AEBV01@@Z @ 0x1800114D8 (--4-$shared_ptr@V__ExceptionPtr@@@std@@QEAAAEAV01@AEBV01@@Z.c)
 *     sub_1800BA3D0 @ 0x1800BA3D0 (sub_1800BA3D0.c)
 *     sub_1800BA714 @ 0x1800BA714 (sub_1800BA714.c)
 *     sub_1800BAA58 @ 0x1800BAA58 (sub_1800BAA58.c)
 *     sub_1800BAD9C @ 0x1800BAD9C (sub_1800BAD9C.c)
 *     sub_1800BB0E0 @ 0x1800BB0E0 (sub_1800BB0E0.c)
 *     sub_1800BB424 @ 0x1800BB424 (sub_1800BB424.c)
 *     sub_1800BCCC4 @ 0x1800BCCC4 (sub_1800BCCC4.c)
 *     sub_1800BD110 @ 0x1800BD110 (sub_1800BD110.c)
 *     sub_1800C24FC @ 0x1800C24FC (sub_1800C24FC.c)
 *     _CxxThrowException @ 0x180123958 (_CxxThrowException.c)
 *     __security_check_cookie @ 0x180123FC0 (__security_check_cookie.c)
 *     _guard_dispatch_icall_nop @ 0x180125480 (_guard_dispatch_icall_nop.c)
 */

// Hidden C++ exception states: #wind=5
_QWORD *__fastcall sub_1800C0090(__int64 *a1, _QWORD *a2, __int64 a3, __int128 *a4)
{
  __int64 v7; // rdx
  signed __int32 v8; // eax
  _QWORD *v9; // r14
  __int64 *v10; // rdi
  __int64 v11; // r12
  __int64 v12; // rcx
  __int64 *v13; // rax
  volatile signed __int32 *v14; // rbx
  __int64 v15; // rcx
  __int64 *v16; // rax
  volatile signed __int32 *v17; // rbx
  __int64 v18; // rcx
  __int64 v19; // rax
  __int64 *v20; // rbx
  __int64 *v21; // r14
  volatile signed __int32 *v22; // rbx
  __int64 v23; // rcx
  __int64 v24; // rax
  __int64 *v25; // rbx
  volatile signed __int32 *v26; // rbx
  __int64 v27; // rcx
  __int64 v28; // rax
  __int64 *v29; // rbx
  volatile signed __int32 *v30; // rbx
  __int64 v31; // rcx
  __int64 *v32; // rax
  volatile signed __int32 *v33; // rbx
  _QWORD *v34; // r14
  __int64 *v35; // rdi
  __int64 v36; // r15
  __int64 v37; // rcx
  __int64 *v38; // rax
  volatile signed __int32 *v39; // rbx
  __int64 v40; // rcx
  __int64 *v41; // rax
  _QWORD *v42; // rbx
  volatile signed __int32 *v43; // rdi
  __int64 v44; // rcx
  __int64 *v45; // rax
  volatile signed __int32 *v46; // rdi
  _QWORD *v47; // rdi
  __int128 v49; // [rsp+30h] [rbp-D0h]
  int v50; // [rsp+40h] [rbp-C0h] BYREF
  int v51; // [rsp+44h] [rbp-BCh] BYREF
  int v52; // [rsp+48h] [rbp-B8h] BYREF
  int v53; // [rsp+4Ch] [rbp-B4h] BYREF
  int v54; // [rsp+50h] [rbp-B0h] BYREF
  _QWORD *v55; // [rsp+58h] [rbp-A8h] BYREF
  __int128 v56; // [rsp+60h] [rbp-A0h] BYREF
  __int128 v57; // [rsp+70h] [rbp-90h] BYREF
  __int128 v58; // [rsp+80h] [rbp-80h] BYREF
  __int128 v59; // [rsp+90h] [rbp-70h] BYREF
  __int128 v60; // [rsp+A0h] [rbp-60h] BYREF
  __int128 v61; // [rsp+B0h] [rbp-50h] BYREF
  __int128 v62; // [rsp+C0h] [rbp-40h] BYREF
  __int128 v63; // [rsp+D0h] [rbp-30h] BYREF
  __int128 v64; // [rsp+E0h] [rbp-20h] BYREF
  _QWORD *v65; // [rsp+F0h] [rbp-10h]
  __int128 v66; // [rsp+100h] [rbp+0h] BYREF
  __int128 v67; // [rsp+110h] [rbp+10h] BYREF
  __int128 v68; // [rsp+120h] [rbp+20h] BYREF
  __int128 v69; // [rsp+130h] [rbp+30h] BYREF
  __int128 v70; // [rsp+140h] [rbp+40h] BYREF
  __int128 v71; // [rsp+150h] [rbp+50h] BYREF
  __int128 v72; // [rsp+160h] [rbp+60h] BYREF
  __int128 v73; // [rsp+170h] [rbp+70h] BYREF
  __int128 v74; // [rsp+180h] [rbp+80h] BYREF
  __int64 v75; // [rsp+190h] [rbp+90h] BYREF
  volatile signed __int32 *v76; // [rsp+198h] [rbp+98h]
  __int64 v77; // [rsp+1A0h] [rbp+A0h] BYREF
  volatile signed __int32 *v78; // [rsp+1A8h] [rbp+A8h]
  __int64 v79; // [rsp+1B0h] [rbp+B0h] BYREF
  __int64 v80; // [rsp+1C0h] [rbp+C0h] BYREF
  __int64 v81; // [rsp+1D0h] [rbp+D0h] BYREF
  volatile signed __int32 *v82; // [rsp+1D8h] [rbp+D8h]
  __int64 v83; // [rsp+1E0h] [rbp+E0h] BYREF
  __int64 v84; // [rsp+1F0h] [rbp+F0h] BYREF
  __int64 v85; // [rsp+200h] [rbp+100h] BYREF
  volatile signed __int32 *v86; // [rsp+208h] [rbp+108h]
  __int64 v87; // [rsp+210h] [rbp+110h] BYREF
  __int64 v88; // [rsp+220h] [rbp+120h] BYREF
  __int64 v89; // [rsp+230h] [rbp+130h] BYREF
  volatile signed __int32 *v90; // [rsp+238h] [rbp+138h]
  __int64 v91; // [rsp+240h] [rbp+140h] BYREF
  volatile signed __int32 *v92; // [rsp+248h] [rbp+148h]
  __int64 v93; // [rsp+250h] [rbp+150h] BYREF
  volatile signed __int32 *v94; // [rsp+258h] [rbp+158h]
  __int64 v95; // [rsp+260h] [rbp+160h] BYREF
  volatile signed __int32 *v96; // [rsp+268h] [rbp+168h]
  char v97[8]; // [rsp+270h] [rbp+170h] BYREF
  volatile signed __int32 *v98; // [rsp+278h] [rbp+178h]
  _BYTE pExceptionObject[24]; // [rsp+280h] [rbp+180h] BYREF
  __int128 v100; // [rsp+298h] [rbp+198h] BYREF

  v65 = a2;
  v55 = a2;
  v100 = 0LL;
  sub_1800BD110((__int64)&v100);
  v7 = *((_QWORD *)a4 + 1);
  if ( !v7 )
  {
LABEL_89:
    std::bad_function_call::bad_function_call((std::bad_function_call *)pExceptionObject);
    throw (std::bad_weak_ptr *)pExceptionObject;
  }
  do
  {
    v8 = *(_DWORD *)(v7 + 8);
    if ( !v8 )
      goto LABEL_89;
  }
  while ( v8 != _InterlockedCompareExchange((volatile signed __int32 *)(v7 + 8), v8 + 1, v8) );
  v49 = *a4;
  v9 = (_QWORD *)(v100 + 56);
  v10 = a1 + 8;
  v11 = 3LL;
  do
  {
    v58 = 0LL;
    v12 = *(v10 - 6);
    if ( v12 )
    {
      _InterlockedIncrement((volatile signed __int32 *)(v12 + 8));
      v12 = *(v10 - 6);
    }
    *(_QWORD *)&v58 = *(v10 - 7);
    *((_QWORD *)&v58 + 1) = v12;
    v67 = 0LL;
    if ( *((_QWORD *)&v49 + 1) )
      _InterlockedIncrement((volatile signed __int32 *)(*((_QWORD *)&v49 + 1) + 8LL));
    v67 = v49;
    v13 = sub_1800BAA58(a3, &v75, &v67, &v58);
    std::shared_ptr<__ExceptionPtr>::operator=(v9 - 6, v13);
    v14 = v76;
    if ( v76 )
    {
      if ( _InterlockedExchangeAdd(v76 + 2, 0xFFFFFFFF) == 1 )
      {
        (**(void (__fastcall ***)(volatile signed __int32 *))v14)(v14);
        if ( _InterlockedExchangeAdd(v14 + 3, 0xFFFFFFFF) == 1 )
          (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v14 + 8LL))(v14);
      }
    }
    v59 = 0LL;
    v15 = *v10;
    if ( *v10 )
    {
      _InterlockedIncrement((volatile signed __int32 *)(v15 + 8));
      v15 = *v10;
    }
    *(_QWORD *)&v59 = *(v10 - 1);
    *((_QWORD *)&v59 + 1) = v15;
    v68 = 0LL;
    if ( *((_QWORD *)&v49 + 1) )
      _InterlockedIncrement((volatile signed __int32 *)(*((_QWORD *)&v49 + 1) + 8LL));
    v68 = v49;
    v16 = sub_1800BA3D0(a3, &v77, &v68, &v59);
    std::shared_ptr<__ExceptionPtr>::operator=(v9, v16);
    v17 = v78;
    if ( v78 )
    {
      if ( _InterlockedExchangeAdd(v78 + 2, 0xFFFFFFFF) == 1 )
      {
        (**(void (__fastcall ***)(volatile signed __int32 *))v17)(v17);
        if ( _InterlockedExchangeAdd(v17 + 3, 0xFFFFFFFF) == 1 )
          (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v17 + 8LL))(v17);
      }
    }
    v9 += 2;
    v10 += 2;
    --v11;
  }
  while ( v11 );
  v50 = 0;
  v51 = 0;
  sub_1800BCCC4(a1 + 13, (__int64)&v79, &v50);
  v18 = v79;
  v60 = 0LL;
  v19 = *(_QWORD *)(v79 + 48);
  if ( v19 )
    _InterlockedIncrement((volatile signed __int32 *)(v19 + 8));
  v60 = *(_OWORD *)(v18 + 40);
  v69 = 0LL;
  if ( *((_QWORD *)&v49 + 1) )
    _InterlockedIncrement((volatile signed __int32 *)(*((_QWORD *)&v49 + 1) + 8LL));
  v69 = v49;
  v20 = sub_1800BAD9C(a3, &v81, &v69, &v60);
  v21 = (__int64 *)(v100 + 104);
  sub_1800BCCC4((__int64 *)(v100 + 104), (__int64)&v80, &v51);
  std::shared_ptr<__ExceptionPtr>::operator=((_QWORD *)(v80 + 40), v20);
  v22 = v82;
  if ( v82 )
  {
    if ( _InterlockedExchangeAdd(v82 + 2, 0xFFFFFFFF) == 1 )
    {
      (**(void (__fastcall ***)(volatile signed __int32 *))v22)(v22);
      if ( _InterlockedExchangeAdd(v22 + 3, 0xFFFFFFFF) == 1 )
        (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v22 + 8LL))(v22);
    }
  }
  v52 = 1;
  v53 = 1;
  sub_1800BCCC4(a1 + 13, (__int64)&v83, &v52);
  v23 = v83;
  v61 = 0LL;
  v24 = *(_QWORD *)(v83 + 48);
  if ( v24 )
    _InterlockedIncrement((volatile signed __int32 *)(v24 + 8));
  v61 = *(_OWORD *)(v23 + 40);
  v70 = 0LL;
  if ( *((_QWORD *)&v49 + 1) )
    _InterlockedIncrement((volatile signed __int32 *)(*((_QWORD *)&v49 + 1) + 8LL));
  v70 = v49;
  v25 = sub_1800BAD9C(a3, &v85, &v70, &v61);
  sub_1800BCCC4(v21, (__int64)&v84, &v53);
  std::shared_ptr<__ExceptionPtr>::operator=((_QWORD *)(v84 + 40), v25);
  v26 = v86;
  if ( v86 )
  {
    if ( _InterlockedExchangeAdd(v86 + 2, 0xFFFFFFFF) == 1 )
    {
      (**(void (__fastcall ***)(volatile signed __int32 *))v26)(v26);
      if ( _InterlockedExchangeAdd(v26 + 3, 0xFFFFFFFF) == 1 )
        (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v26 + 8LL))(v26);
    }
  }
  v54 = 2;
  LODWORD(v55) = 2;
  sub_1800BCCC4(a1 + 13, (__int64)&v87, &v54);
  v27 = v87;
  v62 = 0LL;
  v28 = *(_QWORD *)(v87 + 48);
  if ( v28 )
    _InterlockedIncrement((volatile signed __int32 *)(v28 + 8));
  v62 = *(_OWORD *)(v27 + 40);
  v71 = 0LL;
  if ( *((_QWORD *)&v49 + 1) )
    _InterlockedIncrement((volatile signed __int32 *)(*((_QWORD *)&v49 + 1) + 8LL));
  v71 = v49;
  v29 = sub_1800BAD9C(a3, &v89, &v71, &v62);
  sub_1800BCCC4(v21, (__int64)&v88, &v55);
  std::shared_ptr<__ExceptionPtr>::operator=((_QWORD *)(v88 + 40), v29);
  v30 = v90;
  if ( v90 )
  {
    if ( _InterlockedExchangeAdd(v90 + 2, 0xFFFFFFFF) == 1 )
    {
      (**(void (__fastcall ***)(volatile signed __int32 *))v30)(v30);
      if ( _InterlockedExchangeAdd(v30 + 3, 0xFFFFFFFF) == 1 )
        (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v30 + 8LL))(v30);
    }
  }
  v63 = 0LL;
  v31 = a1[16];
  if ( v31 )
  {
    _InterlockedIncrement((volatile signed __int32 *)(v31 + 8));
    v31 = a1[16];
  }
  *(_QWORD *)&v63 = a1[15];
  *((_QWORD *)&v63 + 1) = v31;
  v72 = 0LL;
  if ( *((_QWORD *)&v49 + 1) )
    _InterlockedIncrement((volatile signed __int32 *)(*((_QWORD *)&v49 + 1) + 8LL));
  v72 = v49;
  v32 = sub_1800BA714(a3, &v91, &v72, &v63);
  std::shared_ptr<__ExceptionPtr>::operator=((_QWORD *)(v100 + 120), v32);
  v33 = v92;
  if ( v92 )
  {
    if ( _InterlockedExchangeAdd(v92 + 2, 0xFFFFFFFF) == 1 )
    {
      (**(void (__fastcall ***)(volatile signed __int32 *))v33)(v33);
      if ( _InterlockedExchangeAdd(v33 + 3, 0xFFFFFFFF) == 1 )
        (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v33 + 8LL))(v33);
    }
  }
  v34 = (_QWORD *)(v100 + 136);
  v35 = a1 + 18;
  v36 = 3LL;
  do
  {
    v64 = 0LL;
    v37 = *v35;
    if ( *v35 )
    {
      _InterlockedIncrement((volatile signed __int32 *)(v37 + 8));
      v37 = *v35;
    }
    *(_QWORD *)&v64 = *(v35 - 1);
    *((_QWORD *)&v64 + 1) = v37;
    v73 = 0LL;
    if ( *((_QWORD *)&v49 + 1) )
      _InterlockedIncrement((volatile signed __int32 *)(*((_QWORD *)&v49 + 1) + 8LL));
    v73 = v49;
    v38 = sub_1800BB0E0(a3, &v93, &v73, &v64);
    std::shared_ptr<__ExceptionPtr>::operator=(v34, v38);
    v39 = v94;
    if ( v94 )
    {
      if ( _InterlockedExchangeAdd(v94 + 2, 0xFFFFFFFF) == 1 )
      {
        (**(void (__fastcall ***)(volatile signed __int32 *))v39)(v39);
        if ( _InterlockedExchangeAdd(v39 + 3, 0xFFFFFFFF) == 1 )
          (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v39 + 8LL))(v39);
      }
    }
    v34 += 2;
    v35 += 2;
    --v36;
  }
  while ( v36 );
  v56 = 0LL;
  v40 = a1[24];
  if ( v40 )
  {
    _InterlockedIncrement((volatile signed __int32 *)(v40 + 8));
    v40 = a1[24];
  }
  *(_QWORD *)&v56 = a1[23];
  *((_QWORD *)&v56 + 1) = v40;
  v74 = 0LL;
  if ( *((_QWORD *)&v49 + 1) )
    _InterlockedIncrement((volatile signed __int32 *)(*((_QWORD *)&v49 + 1) + 8LL));
  v74 = v49;
  v41 = sub_1800BB424(a3, &v95, &v74, &v56);
  v42 = (_QWORD *)v100;
  std::shared_ptr<__ExceptionPtr>::operator=((_QWORD *)(v100 + 184), v41);
  v43 = v96;
  if ( v96 )
  {
    if ( _InterlockedExchangeAdd(v96 + 2, 0xFFFFFFFF) == 1 )
    {
      (**(void (__fastcall ***)(volatile signed __int32 *))v43)(v43);
      if ( _InterlockedExchangeAdd(v43 + 3, 0xFFFFFFFF) == 1 )
        (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v43 + 8LL))(v43);
    }
    v42 = (_QWORD *)v100;
  }
  v57 = 0LL;
  v44 = a1[26];
  if ( v44 )
  {
    _InterlockedIncrement((volatile signed __int32 *)(v44 + 8));
    v44 = a1[26];
    v42 = (_QWORD *)v100;
  }
  *(_QWORD *)&v57 = a1[25];
  *((_QWORD *)&v57 + 1) = v44;
  v66 = 0LL;
  if ( *((_QWORD *)&v49 + 1) )
  {
    _InterlockedIncrement((volatile signed __int32 *)(*((_QWORD *)&v49 + 1) + 8LL));
    v42 = (_QWORD *)v100;
  }
  v66 = v49;
  v45 = (__int64 *)sub_1800C24FC(a3, v97, &v66, &v57);
  std::shared_ptr<__ExceptionPtr>::operator=(v42 + 25, v45);
  v46 = v98;
  if ( v98 )
  {
    if ( _InterlockedExchangeAdd(v98 + 2, 0xFFFFFFFF) == 1 )
    {
      (**(void (__fastcall ***)(volatile signed __int32 *))v46)(v46);
      if ( _InterlockedExchangeAdd(v46 + 3, 0xFFFFFFFF) == 1 )
        (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v46 + 8LL))(v46);
    }
    v42 = (_QWORD *)v100;
  }
  std::shared_ptr<__ExceptionPtr>::operator=(v42 + 27, a1 + 27);
  std::shared_ptr<__ExceptionPtr>::operator=(v42 + 29, a1 + 29);
  std::shared_ptr<__ExceptionPtr>::operator=(v42 + 31, a1 + 31);
  std::shared_ptr<__ExceptionPtr>::operator=(v42 + 33, a1 + 33);
  std::shared_ptr<__ExceptionPtr>::operator=(v42 + 35, a1 + 35);
  std::shared_ptr<__ExceptionPtr>::operator=(v42 + 37, a1 + 37);
  std::shared_ptr<__ExceptionPtr>::operator=(v42 + 39, a1 + 39);
  v47 = v65;
  *v65 = v42;
  v47[1] = *((_QWORD *)&v100 + 1);
  if ( *((_QWORD *)&v49 + 1) )
  {
    if ( _InterlockedExchangeAdd((volatile signed __int32 *)(*((_QWORD *)&v49 + 1) + 8LL), 0xFFFFFFFF) == 1 )
    {
      (***((void (__fastcall ****)(_QWORD))&v49 + 1))(*((_QWORD *)&v49 + 1));
      if ( _InterlockedExchangeAdd((volatile signed __int32 *)(*((_QWORD *)&v49 + 1) + 12LL), 0xFFFFFFFF) == 1 )
        (*(void (__fastcall **)(_QWORD))(**((_QWORD **)&v49 + 1) + 8LL))(*((_QWORD *)&v49 + 1));
    }
  }
  return v47;
}
