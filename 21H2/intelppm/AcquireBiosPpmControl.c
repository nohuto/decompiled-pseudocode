/*
 * XREFs of AcquireBiosPpmControl @ 0x1C0001990
 * Callers:
 *     ProcessSystemSleepStateNotify @ 0x1C00016D0 (ProcessSystemSleepStateNotify.c)
 *     ProcLibDeviceStart @ 0x1C0022D64 (ProcLibDeviceStart.c)
 * Callees:
 *     <none>
 */

char __fastcall AcquireBiosPpmControl(__int64 a1)
{
  __int64 v1; // rax

  v1 = *(_QWORD *)(a1 + 272);
  if ( (v1 & 0x7F070) != 0 && dword_1C001CEB0 && byte_1C001CEB5 )
  {
    __outbyte(dword_1C001CEB0, byte_1C001CEB5);
    v1 = *(_QWORD *)(a1 + 272);
  }
  if ( (v1 & 0x70000000) != 0 )
  {
    if ( dword_1C001CEB0 )
    {
      LOBYTE(v1) = byte_1C001CEB4;
      if ( byte_1C001CEB4 )
        __outbyte(dword_1C001CEB0, byte_1C001CEB4);
    }
  }
  return v1;
}
