/*
 * XREFs of ?zzzMNFadeSelection@@YAHAEBV?$SmartObjStackRef@UtagMENU@@@@PEAUtagITEM@@@Z @ 0x1C022FE00
 * Callers:
 *     ?xxxMNDismissWithNotify@@YAXPEAUtagMENUSTATE@@AEBV?$SmartObjStackRef@UtagPOPUPMENU@@@@PEAUtagITEM@@I_J@Z @ 0x1C022F774 (-xxxMNDismissWithNotify@@YAXPEAUtagMENUSTATE@@AEBV-$SmartObjStackRef@UtagPOPUPMENU@@@@PEAUtagITE.c)
 * Callees:
 *     NtGdiBitBltInternal @ 0x1C003DD70 (NtGdiBitBltInternal.c)
 *     MNGetPopupFromMenu @ 0x1C0043DBC (MNGetPopupFromMenu.c)
 *     ??1?$SmartObjStackRefBase@UtagPOPUPMENU@@@@IEAA@XZ @ 0x1C0068D40 (--1-$SmartObjStackRefBase@UtagPOPUPMENU@@@@IEAA@XZ.c)
 *     ?Init@?$SmartObjStackRefBase@UtagPOPUPMENU@@@@AEAAXPEAUtagPOPUPMENU@@@Z @ 0x1C00F227C (-Init@-$SmartObjStackRefBase@UtagPOPUPMENU@@@@AEAAXPEAUtagPOPUPMENU@@@Z.c)
 *     ??4?$SmartObjStackRefBase@UtagPOPUPMENU@@@@IEAAAEAV0@QEAUtagPOPUPMENU@@@Z @ 0x1C00FB014 (--4-$SmartObjStackRefBase@UtagPOPUPMENU@@@@IEAAAEAV0@QEAUtagPOPUPMENU@@@Z.c)
 *     __security_check_cookie @ 0x1C01593A0 (__security_check_cookie.c)
 *     ?CreateFadeInternal@@YAPEAUHDC__@@PEAUtagWND@@PEAUtagRECT@@KKK@Z @ 0x1C01E1818 (-CreateFadeInternal@@YAPEAUHDC__@@PEAUtagWND@@PEAUtagRECT@@KKK@Z.c)
 *     zzzShowFade @ 0x1C01E2E4C (zzzShowFade.c)
 */

__int64 __fastcall zzzMNFadeSelection(__int64 a1, __int64 a2)
{
  unsigned int v4; // edi
  __int64 v5; // rcx
  _DWORD *v6; // rax
  __int64 v7; // rbx
  _DWORD *v8; // r9
  HDC v9; // rbp
  HDC DCEx; // rbx
  _QWORD v12[2]; // [rsp+60h] [rbp-48h] BYREF
  struct tagRECT v13; // [rsp+70h] [rbp-38h] BYREF

  v13 = 0LL;
  SmartObjStackRefBase<tagPOPUPMENU>::Init(v12, 0LL);
  if ( gbDisableAlpha )
    goto LABEL_12;
  if ( ((unsigned int)gpdwCPUserPreferencesMask & 0x80000400) != 0x80000400 )
    goto LABEL_12;
  v4 = 1;
  if ( (unsigned int)CInputGlobals::GetLastInputType(gpInputGlobals) == 1
    || (unsigned int)CInputGlobals::GetLastInputType(gpInputGlobals) == 4
    || CInputGlobals::WasLastInputJournalling(gpInputGlobals) )
  {
    goto LABEL_12;
  }
  v5 = *(_QWORD *)(a1 + 16);
  if ( !v5 )
    v5 = **(_QWORD **)a1;
  v6 = MNGetPopupFromMenu(v5, 0LL);
  SmartObjStackRefBase<tagPOPUPMENU>::operator=(v12, (__int64)v6);
  if ( !*(_QWORD *)v12[0]
    || (v7 = *(_QWORD *)(*(_QWORD *)v12[0] + 16LL)) == 0
    || (v8 = *(_DWORD **)a2,
        v13.left = *(_DWORD *)(*(_QWORD *)(v7 + 40) + 104LL) + *(_DWORD *)(*(_QWORD *)a2 + 64LL),
        v13.top = *(_DWORD *)(*(_QWORD *)(v7 + 40) + 108LL) + v8[17],
        v13.right = v13.left + v8[18],
        v13.bottom = v13.top + v8[19],
        (v9 = CreateFadeInternal(
                0LL,
                &v13,
                350LL,
                (*(_DWORD *)(*(_QWORD *)(v7 + 40) + 288LL) & 0xF) == 0 ? 0x100 : 0,
                *(_DWORD *)(*(_QWORD *)(v7 + 40) + 288LL))) == 0LL) )
  {
LABEL_12:
    v4 = 0;
  }
  else
  {
    DCEx = (HDC)_GetDCEx(v7, 0LL, 1073807360LL);
    NtGdiBitBltInternal(
      v9,
      0,
      0,
      *(_DWORD *)(*(_QWORD *)a2 + 72LL),
      *(_DWORD *)(*(_QWORD *)a2 + 76LL),
      DCEx,
      *(_DWORD *)(*(_QWORD *)a2 + 64LL),
      *(_DWORD *)(*(_QWORD *)a2 + 68LL),
      13369376,
      0,
      0);
    _ReleaseDC(DCEx);
    zzzShowFade();
  }
  SmartObjStackRefBase<tagPOPUPMENU>::~SmartObjStackRefBase<tagPOPUPMENU>(v12);
  return v4;
}
