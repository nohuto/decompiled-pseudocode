/*
 * XREFs of InitClipboardILDef @ 0x1C0134A30
 * Callers:
 *     <none>
 * Callees:
 *     WPP_RECORDER_SF_ @ 0x1C004DA78 (WPP_RECORDER_SF_.c)
 *     ?GetClipILIndexFromName@@YAIPEAGI@Z @ 0x1C021ED3C (-GetClipILIndexFromName@@YAIPEAGI@Z.c)
 */
              ...
              while ( ZwEnumerateValueKey(KeyHandle, v6, KeyValueBasicInformation, (PVOID)v1, v5, &Length) >= 0 )
              {
                if ( *(_DWORD *)(v1 + 4) == 1 )
                {
                  ...
                  if ( ClipILIndexFromName != -1 )
                    dword_1C032B054[6 * ClipILIndexFromName] = 1;
                }
                if ( ++v6 >= v0[8] )
                  goto LABEL_5;
              }
