/*
 * XREFs of xxxCsEvent @ 0x1C0003004
 * Callers:
 *     xxxChangeMonitorFlags @ 0x1C010D918 (xxxChangeMonitorFlags.c)
 *     NtUserEvent @ 0x1C01F3210 (NtUserEvent.c)
 *     xxxMessageEvent @ 0x1C0242C68 (xxxMessageEvent.c)
 * Callees:
 *     xxxSendMessage @ 0x1C0050D34 (xxxSendMessage.c)
 *     PopAndFreeAlwaysW32ThreadLock @ 0x1C0061D10 (PopAndFreeAlwaysW32ThreadLock.c)
 *     PushW32ThreadLock @ 0x1C007F6F0 (PushW32ThreadLock.c)
 *     memmove @ 0x1C0160280 (memmove.c)
 */

__int64 __fastcall xxxCsEvent(void *Src, unsigned __int16 a2)
{
  __int64 v4; // rdi
  _WORD *v5; // rax
  _WORD *v6; // rbx
  int v7; // esi
  __int64 v8; // rax
  int v9; // edx
  _DWORD *v10; // rax
  _DWORD *v11; // rdi
  int v12; // r14d
  __int64 i; // rsi
  int j; // esi
  _QWORD *v15; // rax
  __int64 v16; // r14
  __int128 v18; // [rsp+30h] [rbp-68h] BYREF
  __int64 v19; // [rsp+40h] [rbp-58h]
  __int128 v20; // [rsp+50h] [rbp-48h] BYREF
  __int64 v21; // [rsp+60h] [rbp-38h]
  __int128 v22; // [rsp+68h] [rbp-30h] BYREF
  __int64 v23; // [rsp+78h] [rbp-20h]

  v20 = 0LL;
  v21 = 0LL;
  v22 = 0LL;
  v23 = 0LL;
  v4 = (unsigned int)a2 + 8;
  v5 = (_WORD *)Win32AllocPoolWithQuotaZInit(v4, 895775573LL);
  v6 = v5;
  if ( !v5 )
    return 16392LL;
  memmove(v5, Src, (unsigned int)v4);
  v6[3] = a2;
  v7 = 0;
  v8 = *(_QWORD *)&WPP_MAIN_CB.Queue.Wcb.NumberOfMapRegisters;
  if ( *(_QWORD *)&WPP_MAIN_CB.Queue.Wcb.NumberOfMapRegisters )
  {
    v9 = *(_DWORD *)v6;
    do
    {
      if ( ((v9 & *(_DWORD *)(v8 + 40)) == 0 || v6[2]) && ((v9 & *(_DWORD *)(v8 + 40)) != 0 || !v6[2]) )
        ++v7;
      v8 = *(_QWORD *)(v8 + 24);
    }
    while ( v8 );
  }
  v10 = (_DWORD *)Win32AllocPoolWithQuotaZInit(8LL * v7 + 8, 912552789LL);
  v11 = v10;
  if ( !v10 )
  {
    Win32FreePool(v6);
    return 16392LL;
  }
  *v10 = v7;
  v12 = 0;
  for ( i = *(_QWORD *)&WPP_MAIN_CB.Queue.Wcb.NumberOfMapRegisters; i && v12 < *v11; i = *(_QWORD *)(i + 24) )
  {
    if ( ((*(_DWORD *)v6 & *(_DWORD *)(i + 40)) == 0 || v6[2]) && ((*(_DWORD *)v6 & *(_DWORD *)(i + 40)) != 0 || !v6[2]) )
    {
      *(_QWORD *)&v18 = &v11[2 * v12 + 2];
      *((_QWORD *)&v18 + 1) = *(_QWORD *)(i + 48);
      HMAssignmentLock(&v18, 0LL);
      ++v12;
    }
  }
  PushW32ThreadLock(v6, &v22, Win32FreePool);
  PushW32ThreadLock(v11, &v20, DestroyEventPacketTargets);
  for ( j = 0; j < *v11; ++j )
  {
    v18 = 0LL;
    v19 = 0LL;
    v15 = (_QWORD *)HMAssignmentUnlock(&v11[2 * j + 2]);
    if ( v15 )
    {
      v16 = ValidateHwnd(*v15);
      if ( v16 )
      {
        *(_QWORD *)&v18 = *(_QWORD *)(gptiCurrent + 416LL);
        *(_QWORD *)(gptiCurrent + 416LL) = &v18;
        *((_QWORD *)&v18 + 1) = v16;
        HMLockObject(v16);
        xxxSendMessage(v16, 60LL, 0LL, v6);
        ThreadUnlock1();
      }
    }
  }
  PopAndFreeAlwaysW32ThreadLock(&v20);
  PopAndFreeAlwaysW32ThreadLock(&v22);
  return 0LL;
}
