/*
 * XREFs of xxxRedrawTitle @ 0x1C00A6034
 * Callers:
 *     xxxRealDefWindowProc @ 0x1C0067528 (xxxRealDefWindowProc.c)
 *     ?xxxDWP_SetIcon@@YAPEAUHICON__@@PEAUtagWND@@_KPEAU1@@Z @ 0x1C00A36F0 (-xxxDWP_SetIcon@@YAPEAUHICON__@@PEAUtagWND@@_KPEAU1@@Z.c)
 *     xxxEnableMenuItem @ 0x1C00BC4A0 (xxxEnableMenuItem.c)
 *     NtUserRedrawTitle @ 0x1C014B000 (NtUserRedrawTitle.c)
 *     ?xxxSetClassIconEnum@@YAHPEAUtagWND@@_J@Z @ 0x1C016DFC0 (-xxxSetClassIconEnum@@YAHPEAUtagWND@@_J@Z.c)
 *     xxxHelpLoop @ 0x1C024A384 (xxxHelpLoop.c)
 * Callees:
 *     ?xxxCallHook@@YAHH_K_JH@Z @ 0x1C004DAF8 (-xxxCallHook@@YAHH_K_JH@Z.c)
 *     xxxSendMessage @ 0x1C0050D34 (xxxSendMessage.c)
 *     ?IsTrayWindow@@YA_NPEAUtagWND@@W4TrayCheckOption@@@Z @ 0x1C006B9F0 (-IsTrayWindow@@YA_NPEAUtagWND@@W4TrayCheckOption@@@Z.c)
 *     PostShellHookMessages @ 0x1C00A6110 (PostShellHookMessages.c)
 *     xxxDrawCaptionBar @ 0x1C00BAE5C (xxxDrawCaptionBar.c)
 */

__int64 __fastcall xxxRedrawTitle(unsigned __int64 *BugCheckParameter2, char a2)
{
  char v3; // si
  _BYTE *v4; // rcx
  unsigned int v5; // ebp
  unsigned __int64 v7; // rbx
  HDC DCEx; // rbx

  v3 = a2;
  v4 = (_BYTE *)BugCheckParameter2[5];
  v5 = 1;
  if ( (v4[31] & 0x10) != 0 )
  {
    if ( (v4[30] & 0xC0) == 0xC0 )
    {
      if ( (v4[16] & 0x40) != 0 && *(_QWORD *)(BugCheckParameter2[2] + 432) == gpqForeground )
        v3 = a2 | 1;
      if ( gihmodUserApiHook < 0 )
      {
        DCEx = (HDC)_GetDCEx(BugCheckParameter2, 0LL, 65537LL);
        xxxDrawCaptionBar((ULONG_PTR)BugCheckParameter2, DCEx);
        _ReleaseDC(DCEx);
      }
      else
      {
        xxxSendMessage((ULONG_PTR)BugCheckParameter2);
      }
    }
    else
    {
      v5 = 0;
    }
  }
  if ( IsTrayWindow((__int64)BugCheckParameter2, 1) && (v3 & 0xC) != 0 )
  {
    v7 = *BugCheckParameter2;
    xxxCallHook(6, *BugCheckParameter2, 0LL, 10);
    PostShellHookMessages(6LL, v7);
  }
  return v5;
}
