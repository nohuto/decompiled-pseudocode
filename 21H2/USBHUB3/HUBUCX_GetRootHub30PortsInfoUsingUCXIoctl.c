/*
 * XREFs of HUBUCX_GetRootHub30PortsInfoUsingUCXIoctl @ 0x1C00258F4
 * Callers:
 *     HUBHSM_GettingRootHub30PortsInfo @ 0x1C00097C0 (HUBHSM_GettingRootHub30PortsInfo.c)
 * Callees:
 *     WPP_RECORDER_SF_d @ 0x1C0001C04 (WPP_RECORDER_SF_d.c)
 *     HUBSM_AddEvent @ 0x1C000B3FC (HUBSM_AddEvent.c)
 *     _guard_dispatch_icall_nop @ 0x1C00437E0 (_guard_dispatch_icall_nop.c)
 */

__int64 __fastcall HUBUCX_GetRootHub30PortsInfoUsingUCXIoctl(__int64 a1)
{
  int v2; // edx
  __int64 v3; // rcx
  __int64 v4; // rax
  unsigned __int64 v5; // rcx
  __int64 v6; // rdi
  int v7; // eax
  unsigned __int16 v8; // r9
  __int64 v9; // rcx
  _QWORD *v10; // r8
  unsigned int v11; // r10d
  __int64 v12; // rcx
  _QWORD *v13; // rdx
  _QWORD *v14; // r9
  __int64 result; // rax
  int v16; // edi
  __int64 v17; // [rsp+28h] [rbp-51h]
  __int128 v18; // [rsp+60h] [rbp-19h] BYREF
  __int128 v19; // [rsp+70h] [rbp-9h]
  __int128 v20; // [rsp+80h] [rbp+7h]
  __int64 v21; // [rsp+90h] [rbp+17h]
  unsigned __int64 v22; // [rsp+E0h] [rbp+67h] BYREF
  __int64 v23; // [rsp+E8h] [rbp+6Fh] BYREF
  __int64 v24; // [rsp+F0h] [rbp+77h] BYREF
  __int64 v25; // [rsp+F8h] [rbp+7Fh] BYREF

  DWORD1(v18) = 0;
  v24 = 0LL;
  v23 = 0LL;
  v25 = 0LL;
  v22 = 0LL;
  if ( !*(_WORD *)(a1 + 114) )
  {
    *(_QWORD *)(a1 + 136) = 0LL;
    v2 = 2042;
    v3 = a1 + 1264;
    return HUBSM_AddEvent(v3, v2);
  }
  v4 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, _QWORD))(WdfFunctions_01015 + 336))(
         WdfDriverGlobals,
         *(_QWORD *)(a1 + 16));
  v5 = *(_QWORD *)(a1 + 16);
  v6 = v4;
  *(_QWORD *)&v19 = 0LL;
  v21 = 0LL;
  v20 = v5;
  v18 = 0LL;
  *((_QWORD *)&v19 + 1) = 0x100000001LL;
  LODWORD(v18) = 56;
  v7 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int128 *, __int64, unsigned __int64 *))(WdfFunctions_01015
                                                                                               + 1976))(
         WdfDriverGlobals,
         &v18,
         v4,
         &v22);
  if ( v7 < 0 )
  {
    v22 = 0LL;
    if ( WPP_RECORDER_INITIALIZED == (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
    {
LABEL_22:
      if ( v22 )
        (*(void (__fastcall **)(PWDF_DRIVER_GLOBALS))(WdfFunctions_01015 + 1664))(WdfDriverGlobals);
      if ( v25 )
      {
        (*(void (__fastcall **)(PWDF_DRIVER_GLOBALS))(WdfFunctions_01015 + 1664))(WdfDriverGlobals);
        *(_QWORD *)(a1 + 136) = 0LL;
      }
      v3 = a1 + 1264;
      v2 = 2038;
      return HUBSM_AddEvent(v3, v2);
    }
    v8 = 50;
LABEL_6:
    LODWORD(v17) = v7;
    WPP_RECORDER_SF_d(
      *(_QWORD *)(a1 + 2520),
      2u,
      3u,
      v8,
      (__int64)&WPP_a5663512bb0f384af60f2cebe8e2cf44_Traceguids,
      v17);
    goto LABEL_22;
  }
  v9 = 224LL * *(unsigned __int16 *)(a1 + 114);
  v19 = 0LL;
  v20 = 0LL;
  v21 = 0LL;
  *(_QWORD *)&v20 = *(_QWORD *)(a1 + 16);
  v18 = 0LL;
  LODWORD(v18) = 56;
  *((_QWORD *)&v19 + 1) = 0x100000001LL;
  v7 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int128 *, _QWORD, __int64, __int64, __int64 *, __int64))(WdfFunctions_01015 + 1536))(
         WdfDriverGlobals,
         &v18,
         (unsigned int)ExDefaultNonPagedPoolType,
         1882409045LL,
         v9,
         &v25,
         a1 + 136);
  if ( v7 < 0 )
  {
    if ( WPP_RECORDER_INITIALIZED == (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
      goto LABEL_22;
    v8 = 51;
    goto LABEL_6;
  }
  v21 = 0LL;
  *(_QWORD *)&v19 = 0LL;
  *((_QWORD *)&v19 + 1) = 0x100000001LL;
  v20 = v22;
  v18 = 0LL;
  LODWORD(v18) = 56;
  v7 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int128 *, _QWORD, __int64, __int64, __int64 *, __int64 *))(WdfFunctions_01015 + 1536))(
         WdfDriverGlobals,
         &v18,
         (unsigned int)ExDefaultNonPagedPoolType,
         1882409045LL,
         16LL,
         &v24,
         &v23);
  if ( v7 < 0 )
  {
    v24 = 0LL;
    if ( WPP_RECORDER_INITIALIZED == (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
      goto LABEL_22;
    v8 = 52;
    goto LABEL_6;
  }
  v10 = *(_QWORD **)(a1 + 136);
  v11 = 0;
  *(_DWORD *)v23 = 16;
  *(_WORD *)(v23 + 4) = *(_WORD *)(a1 + 114);
  *(_WORD *)(v23 + 6) = 24;
  *(_QWORD *)(v23 + 8) = v10;
  v12 = *(unsigned __int16 *)(v23 + 4);
  v13 = &v10[v12];
  v14 = &v13[3 * v12];
  if ( (_WORD)v12 )
  {
    do
    {
      *v10 = v13;
      ++v11;
      v13[2] = v14;
      ++v10;
      *((_WORD *)v13 + 1) = 0;
      v14 += 24;
      *((_DWORD *)v13 + 3) = 48;
      v13 += 3;
    }
    while ( v11 < *(unsigned __int16 *)(v23 + 4) );
  }
  v7 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, unsigned __int64, __int64, __int64, _QWORD, _QWORD, _QWORD, _QWORD, _QWORD))(WdfFunctions_01015 + 1528))(
         WdfDriverGlobals,
         v6,
         v22,
         4788243LL,
         v24,
         0LL,
         0LL,
         0LL,
         0LL,
         0LL);
  if ( v7 < 0 )
  {
    if ( WPP_RECORDER_INITIALIZED == (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
      goto LABEL_22;
    v8 = 53;
    goto LABEL_6;
  }
  (*(void (__fastcall **)(PWDF_DRIVER_GLOBALS, unsigned __int64, __int64 (__fastcall *)(__int64, __int64, __int64, __int64), __int64))(WdfFunctions_01015 + 2080))(
    WdfDriverGlobals,
    v22,
    HUBUCX_GetRootHub30PortsInfoUsingUCXIoctlComplete,
    a1);
  result = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, unsigned __int64, __int64, _QWORD))(WdfFunctions_01015 + 2024))(
             WdfDriverGlobals,
             v22,
             v6,
             0LL);
  if ( !(_BYTE)result )
  {
    result = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, unsigned __int64))(WdfFunctions_01015 + 2032))(
               WdfDriverGlobals,
               v22);
    v16 = result;
    if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
    {
      LODWORD(v17) = result;
      result = WPP_RECORDER_SF_d(
                 *(_QWORD *)(a1 + 2520),
                 2u,
                 3u,
                 0x36u,
                 (__int64)&WPP_a5663512bb0f384af60f2cebe8e2cf44_Traceguids,
                 v17);
    }
    if ( v16 < 0 )
      goto LABEL_22;
  }
  return result;
}
