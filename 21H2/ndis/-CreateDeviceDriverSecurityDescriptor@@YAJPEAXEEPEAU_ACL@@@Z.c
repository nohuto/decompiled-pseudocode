/*
 * XREFs of ?CreateDeviceDriverSecurityDescriptor@@YAJPEAXEEPEAU_ACL@@@Z @ 0x1C014646C
 * Callers:
    ...
    if ( DaclSecurityDescriptor >= 0 && DaclPresent )
    {
      ...
      if ( DaclSecurityDescriptor >= 0 )
      {
        ...
        if ( SaclPresent )
        {
          ...
          {
            DaclSize = v9->AclSize;
            v12 = (ACL *)ExAllocatePoolWithTag(PagedPool, DaclSize, 0x6164444Eu);
            if ( v12 )
            {
                ...
                if ( Owner )
                {
                  ...
                  if ( DaclSecurityDescriptor >= 0 )
                  {
                    ...
                    if ( DaclSecurityDescriptor >= 0 )
                    {
                      ...
                      if ( DaclSecurityDescriptor >= 0 )
                      {
                        DaclSecurityDescriptor = RtlSetDaclSecurityDescriptor(PoolWithTag, 1u, v9, 0);
                        if ( DaclSecurityDescriptor >= 0 )
                          DaclSecurityDescriptor = ObSetSecurityObjectByPointer(a1, v10, PoolWithTag);
                      }
                    }
                  }
                }
