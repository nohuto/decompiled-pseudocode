/*
 * XREFs of ?ndisReferenceFilterByHandle@@YAEPEAU_NDIS_FILTER_BLOCK@@W4_NDIS_LWF_REFTAG@@@Z @ 0x1C0069730
 * Callers:
...
bool __fastcall ndisReferenceFilterByHandle(struct _NDIS_FILTER_BLOCK *a1, enum _NDIS_LWF_REFTAG a2)
{
  bool v3; // bl
  KIRQL v4; // si
  ...
  for ( i = (struct _NDIS_FILTER_BLOCK *)ndisGlobalFilterList; i; i = i->NextGlobalFilter )
  {
    if ( i == a1 )
    {
      v7 = 0;
