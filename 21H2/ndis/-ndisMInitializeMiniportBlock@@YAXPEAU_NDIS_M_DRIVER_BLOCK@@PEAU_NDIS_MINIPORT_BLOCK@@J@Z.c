/*
 * XREFs of ?ndisMInitializeMiniportBlock@@YAXPEAU_NDIS_M_DRIVER_BLOCK@@PEAU_NDIS_MINIPORT_BLOCK@@J@Z @ 0x1C0031F08
 * Callers:
 ...
 *     ?ndisAllocateWatchdog@@YAPEAUNDISWATCHDOG__@@XZ @ 0x1C00FE2A8 (-ndisAllocateWatchdog@@YAPEAUNDISWATCHDOG__@@XZ.c)
...
void __fastcall ndisMInitializeMiniportBlock(struct _NDIS_M_DRIVER_BLOCK *a1, struct _NDIS_MINIPORT_BLOCK *a2, int a3)
{
  ...
  *(_QWORD *)&a2->DriverPowerState = 1LL;
  ...
  a2->RefCountTracker = (NDIS_REFCOUNT_HANDLE__ *)NdisAllocateRefCount(0x6Eu, 0);
  ...
  if ( DriverHandle->HookType != NdisMiniportHookDriverTypeWdi
    ...
    || (v7 = 5456LL, value->OidRequestHandler == DriverHandle->MiniportDriverCharacteristics.OidRequestHandler) )
  {
    v7 = 5448LL;
  }
