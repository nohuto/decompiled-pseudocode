/*
 * XREFs of _lambda_f0f9811b609bc44f6b5f596bb37acf9a_::operator() @ 0x18002B398
 * Callers:
 *     ??$_Callback_once@V?$tuple@$$QEAV_lambda_f0f9811b609bc44f6b5f596bb37acf9a_@@AEAVexception_ptr@std@@@std@@U?$integer_sequence@_K$0A@@2@$00@std@@YAHPEAX0PEAPEAX@Z @ 0x18003A940 (--$_Callback_once@V-$tuple@$$QEAV_lambda_f0f9811b609bc44f6b5f596bb37acf9a_@@AEAVexception_ptr@st.c)
 * Callees:
 *     ??$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z @ 0x180011524 (--$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z.c)
 *     ?IsEdition@@YA_N_K@Z @ 0x180027E10 (-IsEdition@@YA_N_K@Z.c)
 *     ??$?0AEAY0BD@$$CBGM$0A@@?$pair@$$CBV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@V?$variant@K_NM@2@@std@@QEAA@AEAY0BD@$$CBG$$QEAM@Z @ 0x18002B5D4 (--$-0AEAY0BD@$$CBGM$0A@@-$pair@$$CBV-$basic_string@GU-$char_traits@G@std@@V-$allocator@G@2@@std@.c)
 *     ??0?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@QEAA@QEBG@Z @ 0x18002B7B4 (--0-$basic_string@GU-$char_traits@G@std@@V-$allocator@G@2@@std@@QEAA@QEBG@Z.c)
 *     ??0?$map@V?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@V?$variant@K_NM@2@U?$less@X@2@V?$allocator@U?$pair@$$CBV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@V?$variant@K_NM@2@@std@@@2@@std@@QEAA@V?$initializer_list@U?$pair@$$CBV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@V?$variant@K_NM@2@@std@@@1@@Z @ 0x18002BFAC (--0-$map@V-$basic_string@GU-$char_traits@G@std@@V-$allocator@G@2@@std@@V-$variant@K_NM@2@U-$less.c)
 *     ?AddAndPopulateInputType@MPCConstantManager@@QEAAXPEAUIMPCInputProviderBase@@AEBV?$map@V?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@V?$variant@K_NM@2@U?$less@X@2@V?$allocator@U?$pair@$$CBV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@V?$variant@K_NM@2@@std@@@2@@std@@@Z @ 0x180036260 (-AddAndPopulateInputType@MPCConstantManager@@QEAAXPEAUIMPCInputProviderBase@@AEBV-$map@V-$basic_.c)
 *     ??$_Erase_tree@V?$allocator@U?$_Tree_node@U?$pair@$$CBV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@V?$variant@K_NM@2@@std@@PEAX@std@@@std@@@?$_Tree_val@U?$_Tree_simple_types@U?$pair@$$CBV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@V?$variant@K_NM@2@@std@@@std@@@std@@QEAAXAEAV?$allocator@U?$_Tree_node@U?$pair@$$CBV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@V?$variant@K_NM@2@@std@@PEAX@std@@@1@PEAU?$_Tree_node@U?$pair@$$CBV?$basic_string@GU?$char_traits@G@std@@V?$allocator@G@2@@std@@V?$variant@K_NM@2@@std@@PEAX@1@@Z @ 0x180036390 (--$_Erase_tree@V-$allocator@U-$_Tree_node@U-$pair@$$CBV-$basic_string@GU-$char_traits@G@std@@V-$.c)
 *     __security_check_cookie @ 0x18003C220 (__security_check_cookie.c)
 *     ??_M@YAXPEAX_K1P6AX0@Z@Z @ 0x18003CF54 (--_M@YAXPEAX_K1P6AX0@Z@Z.c)
 *     ?_FailFast_Unexpected@in1diag3@details@wil@@YAXPEAXIPEBD@Z @ 0x18003DE74 (-_FailFast_Unexpected@in1diag3@details@wil@@YAXPEAXIPEBD@Z.c)
 */

// Hidden C++ exception states: #wind=9
void __fastcall lambda_f0f9811b609bc44f6b5f596bb37acf9a_::operator()(_QWORD *a1)
{
  int v2; // edi
  const char *v3; // r9
  struct BamoMPCConstantManagerClientPrincipal *v4; // rbx
  float v5; // [rsp+28h] [rbp-E0h] BYREF
  int v6; // [rsp+2Ch] [rbp-DCh] BYREF
  int v7; // [rsp+30h] [rbp-D8h] BYREF
  float v8; // [rsp+34h] [rbp-D4h] BYREF
  __int64 v9; // [rsp+38h] [rbp-D0h] BYREF
  void *v10[3]; // [rsp+40h] [rbp-C8h] BYREF
  _QWORD v11[2]; // [rsp+58h] [rbp-B0h] BYREF
  _BYTE v12[32]; // [rsp+68h] [rbp-A0h] BYREF
  int v13; // [rsp+88h] [rbp-80h]
  char v14; // [rsp+8Ch] [rbp-7Ch]
  _BYTE v15[40]; // [rsp+90h] [rbp-78h] BYREF
  _BYTE v16[40]; // [rsp+B8h] [rbp-50h] BYREF
  _BYTE v17[40]; // [rsp+E0h] [rbp-28h] BYREF
  _BYTE v18[40]; // [rsp+108h] [rbp+0h] BYREF
  _BYTE v19[40]; // [rsp+130h] [rbp+28h] BYREF
  _BYTE v20[40]; // [rsp+158h] [rbp+50h] BYREF
  _BYTE v21[32]; // [rsp+180h] [rbp+78h] BYREF
  float v22; // [rsp+1A0h] [rbp+98h]
  char v23; // [rsp+1A4h] [rbp+9Ch]
  __int64 v24; // [rsp+1A8h] [rbp+A0h] BYREF
  wil::details::in1diag3 *retaddr; // [rsp+1C0h] [rbp+B8h]

  v2 = 0;
  v6 = 1056964608;
  v7 = 1056964608;
  v8 = FLOAT_0_40000001;
  *(float *)&v9 = FLOAT_0_18000001;
  if ( IsEdition(10LL) )
  {
    v2 = 2500;
    v6 = 1070134723;
    v7 = 1070134723;
    v8 = 0.5;
    LODWORD(v9) = 1056964608;
  }
  v4 = MPCConstantManager::s_instance;
  if ( !MPCConstantManager::s_instance )
  {
    wil::details::in1diag3::_FailFast_Unexpected(
      retaddr,
      (void *)0x29,
      (unsigned int)"onecoreuap\\windows\\moderncore\\inputv2\\inputprocessors\\devices\\mpc\\lib\\mpcconstantmanager.cpp",
      v3);
    __debugbreak();
  }
  std::wstring::wstring(v12, L"ActivationDelta");
  v13 = v2;
  v14 = 0;
  v5 = FLOAT_600_0;
  std::pair<std::wstring const,std::variant<unsigned long,bool,float>>::pair<std::wstring const,std::variant<unsigned long,bool,float>>(
    v15,
    L"MouseParams_DeltaToRadiansRatio",
    &v5);
  std::pair<std::wstring const,std::variant<unsigned long,bool,float>>::pair<std::wstring const,std::variant<unsigned long,bool,float>>(
    v16,
    L"MouseParams_ResetHorizontalAngleRadians",
    &v6);
  std::pair<std::wstring const,std::variant<unsigned long,bool,float>>::pair<std::wstring const,std::variant<unsigned long,bool,float>>(
    v17,
    L"MouseParams_ResetVerticalAngleRadians",
    &v7);
  std::pair<std::wstring const,std::variant<unsigned long,bool,float>>::pair<std::wstring const,std::variant<unsigned long,bool,float>>(
    v18,
    L"MouseParams_ClampHorizontalAngleRadians",
    &v8);
  std::pair<std::wstring const,std::variant<unsigned long,bool,float>>::pair<std::wstring const,std::variant<unsigned long,bool,float>>(
    v19,
    L"MouseParams_ClampVerticalAngleRadians",
    &v9);
  v5 = FLOAT_0_00108;
  std::pair<std::wstring const,std::variant<unsigned long,bool,float>>::pair<std::wstring const,std::variant<unsigned long,bool,float>>(
    v20,
    L"MouseParams_PointScaleFactorXY",
    &v5);
  std::wstring::wstring(v21, L"MouseParams_PointScaleFactorWheel");
  v22 = FLOAT_0_00075000001;
  v23 = 2;
  v11[0] = v12;
  v11[1] = &v24;
  std::map<std::wstring,std::variant<unsigned long,bool,float>>::map<std::wstring,std::variant<unsigned long,bool,float>>(
    v10,
    v11);
  MPCConstantManager::AddAndPopulateInputType(
    v4,
    (*a1 + 40LL) & ((unsigned __int128)-(__int128)(unsigned __int64)*a1 >> 64),
    v10);
  std::_Tree_val<std::_Tree_simple_types<std::pair<std::wstring const,std::variant<unsigned long,bool,float>>>>::_Erase_tree<std::allocator<std::_Tree_node<std::pair<std::wstring const,std::variant<unsigned long,bool,float>>,void *>>>(
    v10,
    v10,
    *((_QWORD *)v10[0] + 1));
  std::_Deallocate<16,0>(v10[0], (const struct std::nothrow_t *)0x48);
  `eh vector destructor iterator'(
    v12,
    0x28uLL,
    8uLL,
    std::pair<std::wstring const,std::variant<unsigned long,bool,float>>::~pair<std::wstring const,std::variant<unsigned long,bool,float>>);
}
