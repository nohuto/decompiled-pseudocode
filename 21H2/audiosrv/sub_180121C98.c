/*
 * XREFs of sub_180121C98 @ 0x180121C98
 * Callers:
 *     sub_180121A98 @ 0x180121A98 (sub_180121A98.c)
 * Callees:
 *     sub_18000F708 @ 0x18000F708 (sub_18000F708.c)
 *     sub_18004BD84 @ 0x18004BD84 (sub_18004BD84.c)
 *     _guard_dispatch_icall_nop @ 0x180074470 (_guard_dispatch_icall_nop.c)
 *     sub_1801219B0 @ 0x1801219B0 (sub_1801219B0.c)
 */

// Hidden C++ exception states: #wind=1
__int64 __fastcall sub_180121C98(__int64 a1, char *a2)
{
  int v4; // eax
  unsigned int v5; // ebx
  unsigned int v7; // ebx
  int v8; // eax
  unsigned int v9; // edi
  _UNKNOWN *retaddr; // [rsp+38h] [rbp+0h]
  unsigned int v11; // [rsp+40h] [rbp+8h] BYREF
  __int64 v12; // [rsp+48h] [rbp+10h] BYREF

  *a2 = 0;
  v11 = 0;
  v4 = (*(__int64 (__fastcall **)(__int64, unsigned int *))(*(_QWORD *)a1 + 24LL))(a1, &v11);
  v5 = v4;
  if ( v4 >= 0 )
  {
    v7 = 0;
    if ( v11 )
    {
      while ( !*a2 )
      {
        v12 = 0LL;
        v8 = (*(__int64 (__fastcall **)(__int64, _QWORD, __int64 *))(*(_QWORD *)a1 + 32LL))(a1, v7, &v12);
        v9 = v8;
        if ( v8 < 0 )
        {
          sub_18004BD84(
            (int)retaddr,
            368,
            (int)"avcore\\audiocore\\server\\lib\\audioserviceutil\\spatialpolicy.cpp",
            v8);
          sub_18000F708(&v12);
          return v9;
        }
        *a2 = sub_1801219B0(v12, 3);
        sub_18000F708(&v12);
        if ( ++v7 >= v11 )
          return 0LL;
      }
    }
    return 0LL;
  }
  else
  {
    sub_18004BD84((int)retaddr, 362, (int)"avcore\\audiocore\\server\\lib\\audioserviceutil\\spatialpolicy.cpp", v4);
    return v5;
  }
}
