/*
 * XREFs of Controller_WdfEvtDeviceD0EntryPostInterruptsEnabled @ 0x1C00149A0
 * Callers:
 *     <none>
 * Callees:
 *     WPP_RECORDER_SF_ @ 0x1C000A588 (WPP_RECORDER_SF_.c)
 *     WPP_RECORDER_SF_d @ 0x1C0010010 (WPP_RECORDER_SF_d.c)
 *     Wmi_CreateControllerCapabilities @ 0x1C00117FC (Wmi_CreateControllerCapabilities.c)
 *     Controller_Start @ 0x1C0011B68 (Controller_Start.c)
 *     RootHub_DetectAndAcknowledgePortResume @ 0x1C0011EE8 (RootHub_DetectAndAcknowledgePortResume.c)
 *     WPP_RECORDER_SF_qd @ 0x1C0014644 (WPP_RECORDER_SF_qd.c)
 *     Etw_ControllerRundown @ 0x1C0014B40 (Etw_ControllerRundown.c)
 *     Command_D0EntryPostInterruptsEnabled @ 0x1C0014DE4 (Command_D0EntryPostInterruptsEnabled.c)
 *     WPP_RECORDER_SF_qDD @ 0x1C0016390 (WPP_RECORDER_SF_qDD.c)
 *     _guard_dispatch_icall_nop @ 0x1C00199B0 (_guard_dispatch_icall_nop.c)
 *     McTemplateK0pqqh_EtwWriteTransfer @ 0x1C0035568 (McTemplateK0pqqh_EtwWriteTransfer.c)
 *     McTemplateK0pqqq_EtwWriteTransfer @ 0x1C00355FC (McTemplateK0pqqq_EtwWriteTransfer.c)
 */

__int64 __fastcall Controller_WdfEvtDeviceD0EntryPostInterruptsEnabled(__int64 a1, unsigned int a2)
{
  __int64 v4; // rbx
  char v5; // al
  int v6; // edx
  int v7; // r8d
  char v8; // r14
  int v9; // eax
  int v10; // edx
  int v11; // ecx
  int v12; // r8d
  unsigned int v13; // edi
  __int64 v14; // rbp
  unsigned int i; // edi
  int v17; // r9d
  __int64 v18; // [rsp+30h] [rbp-38h]

  v4 = *(_QWORD *)((*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, void *))(WdfFunctions_01023 + 1616))(
                     WdfDriverGlobals,
                     a1,
                     off_1C00613D8)
                 + 8);
  v5 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64))(WdfFunctions_01023 + 3104))(WdfDriverGlobals, a1);
  v8 = v5;
  if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.BufferChainingDpc & 8) != 0 )
    McTemplateK0pqqh_EtwWriteTransfer(
      *(unsigned __int8 *)(v4 + 468),
      (unsigned int)&USBXHCI_ETW_EVENT_CONTROLLER_D0_ENTRY_POST_INTERRUPTS_START,
      v7,
      *(_QWORD *)(v4 + 8),
      a2,
      v5,
      *(_BYTE *)(v4 + 468));
  if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
  {
    LOBYTE(v6) = 4;
    WPP_RECORDER_SF_qDD(
      *(_QWORD *)(v4 + 72),
      v6,
      4,
      74,
      (__int64)&WPP_ff2e52b0a40430e0f7756a6ff2f45ac0_Traceguids,
      a1,
      a2,
      *(_DWORD *)(v4 + 420));
  }
  if ( *(_DWORD *)(v4 + 464) && *(_QWORD *)(v4 + 792) && qword_1C0062AE8 )
    qword_1C0062AE8();
  if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
  {
    LODWORD(v18) = a2;
    WPP_RECORDER_SF_qd(
      *(_QWORD *)(v4 + 72),
      4u,
      4u,
      0x79u,
      (__int64)&WPP_ff2e52b0a40430e0f7756a6ff2f45ac0_Traceguids,
      v4,
      v18);
  }
  v9 = Controller_Start(v4);
  v13 = v9;
  if ( v9 < 0 )
  {
    if ( WPP_RECORDER_INITIALIZED == (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
      goto LABEL_14;
    v17 = 75;
    goto LABEL_22;
  }
  v14 = *(_QWORD *)(v4 + 152);
  for ( i = 1; i <= *(_DWORD *)(v14 + 16); ++i )
    RootHub_DetectAndAcknowledgePortResume((_QWORD *)v14, i, 1);
  v9 = Command_D0EntryPostInterruptsEnabled(*(_QWORD *)(v4 + 144), a2);
  v13 = v9;
  if ( v9 < 0 )
  {
    if ( WPP_RECORDER_INITIALIZED == (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
      goto LABEL_14;
    v17 = 77;
LABEL_22:
    LOBYTE(v10) = 2;
    WPP_RECORDER_SF_d(*(_QWORD *)(v4 + 72), v10, 4, v17, (__int64)&WPP_ff2e52b0a40430e0f7756a6ff2f45ac0_Traceguids, v9);
    goto LABEL_14;
  }
  if ( (*(_BYTE *)(v4 + 336) & 0x10) != 0 )
  {
    v13 = -1073741637;
    if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
    {
      LOBYTE(v10) = 2;
      WPP_RECORDER_SF_(*(_QWORD *)(v4 + 72), v10, 4, 78, (__int64)&WPP_ff2e52b0a40430e0f7756a6ff2f45ac0_Traceguids);
    }
  }
  else
  {
    Wmi_CreateControllerCapabilities(*(__int64 **)(v4 + 160));
  }
LABEL_14:
  if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.BufferChainingDpc & 8) != 0 )
    McTemplateK0pqqq_EtwWriteTransfer(
      v11,
      (unsigned int)&USBXHCI_ETW_EVENT_CONTROLLER_D0_ENTRY_POST_INTERRUPTS_COMPLETE,
      v12,
      *(_QWORD *)(v4 + 8),
      a2,
      v8,
      v13);
  Etw_ControllerRundown(0LL, v4);
  return v13;
}
