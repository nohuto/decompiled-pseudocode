/*
 * XREFs of ?IsCompanionRequiredForDevice@FxCompanionLibrary@@QEAAEPEAVFxDevice@@PEAPEBG@Z @ 0x1C0042734
 * Callers:
 ...
 *     ?OpenServiceKey@CUmdfSvcConfig@@QEAAJPEBGKPEAPEAX@Z @ 0x1C008E408 (-OpenServiceKey@CUmdfSvcConfig@@QEAAJPEBGKPEAPEAX@Z.c)
 */
        ...
        const wchar_t **CompanionName)
{
  ...
  char v6; // r12
  char v7; // r13
  int SecureSoftwareDeviceTag; // eax
  ...
  void *companionSvcKey; // [rsp+C0h] [rbp-40h] BYREF
  CUmdfSvcConfig umdfSvcConfig; // [rsp+C8h] [rbp-38h] BYREF
  _UNICODE_STRING companionKeyName; // [rsp+D8h] [rbp-28h] BYREF
  _UNICODE_STRING serviceNameUnicodeString; // [rsp+E8h] [rbp-18h] BYREF
  FxDevice *v40; // [rsp+F8h] [rbp-8h]
...
LABEL_9:
  ...
  *(_QWORD *)&deviceData.Lcid = &serviceNameUnicodeString;
  ...
  if ( globals >= 0 )
  {
    ...
    v16 = FxRegKey::_QueryValue(m_Globals, kmdfSvcKey.m_Key, &companionServiceListKeyName, 0, 0LL, &dataLength, 0LL);
    ...
    v17 = dataLength;
    v18 = FxPoolAllocator(m_Globals, &m_Globals->FxPoolFrameworks, 1u, dataLength, m_Globals->Tag, retaddr);
    v4 = (FX_POOL_TRACKER *)v18;
    if ( !v18 )
    {
    ...
    SecureSoftwareDeviceTag = FxRegKey::_QueryValue(
                                ...
                                &dataLength,
                                &dataType);
    if ( SecureSoftwareDeviceTag < 0 || dataType != 7 )
    {
      v9 = 30;
      goto LABEL_39;
    }
    if ( FxRegKey::_VerifyMultiSzString(m_Globals, &companionServiceListKeyName, (wchar_t *)v4, dataLength) < 0 )
      goto $exit_2;
    SecureSoftwareDeviceTag = CUmdfSvcConfig::OpenServiceKey(&umdfSvcConfig, (const wchar_t *)v4, v19, &companionSvcKey);
    if ( SecureSoftwareDeviceTag >= 0 )
    {
      SecureSoftwareDeviceTag = CUmdfSvcConfig::IsCompanionSecure(companionSvcKey, secureCompanionFromInf);
      if ( SecureSoftwareDeviceTag >= 0 )
      {
        if ( !v7 && secureCompanionFromInf[0] )
        {
          ...
          *(_QWORD *)&deviceData.Lcid = 0LL;
          ...
          SecureSoftwareDeviceTag = propertyType;
          if ( propertyType != 7 )
          {
            v9 = 34;
            goto LABEL_39;
