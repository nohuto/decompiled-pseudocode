/*
 * XREFs of ?wil_details_FeatureStateCache_ReevaluateCachedFeatureEnabledState@@YA?ATwil_details_FeatureStateCache@@PEAT1@T1@PEBUwil_details_FeatureDescriptor@@@Z @ 0x1C001CC64
 * Callers:
        ...
        const wil_details_FeatureDescriptor *descriptor)
{
  ...
  if ( WPP_GLOBAL_WDF_Control.DeviceObjectExtension )
    v3 = ((unsigned int (__fastcall *)())WPP_GLOBAL_WDF_Control.DeviceObjectExtension)();
  ...
  if ( (exchange & 2) == 0 && WPP_GLOBAL_WDF_Control.DeviceLock.Header.WaitListHead.Blink )
    ((void (__fastcall *)(wil_details_FeatureStateCache *, wil_FeatureChangeTime, unsigned int))WPP_GLOBAL_WDF_Control.DeviceLock.Header.WaitListHead.Blink)(
      featureState,
