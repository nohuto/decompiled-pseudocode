/*
 * XREFs of imp_WdfPdoInitAssignDeviceID @ 0x1C0030A30
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall imp_WdfPdoInitAssignDeviceID(
        ...
        const _UNICODE_STRING *DeviceID)
{
  ...
  if ( (int)result >= 0 )
  {
    ...
    if ( (int)result >= 0 )
    {
      ...
      {
        v8 = DeviceInit->Pdo.DeviceID;
        if ( v8 )
          return FxDuplicateUnicodeString(v8->m_Globals, DeviceID, &v8->m_UnicodeString);
