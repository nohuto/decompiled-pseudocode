/*
 * XREFs of ?ProcessTapsFor3orMoreContacts@CPTPEngine@@AEAAHPEAUPTPInput@@K@Z @ 0x1C0202358
 * Callers:
 *     ?RunStateMachine@CPTPEngine@@AEAAXPEAUPTPInput@@@Z @ 0x1C020273C (-RunStateMachine@CPTPEngine@@AEAAXPEAUPTPInput@@@Z.c)
 * Callees:
 *     ?ResetDragThresholds@CPTPEngine@@AEAAXPEAUPTPInput@@@Z @ 0x1C020269C (-ResetDragThresholds@CPTPEngine@@AEAAXPEAUPTPInput@@@Z.c)
 *     ?Send2FingerTap@CPTPEngine@@AEAAXPEAUPTPInput@@@Z @ 0x1C0203798 (-Send2FingerTap@CPTPEngine@@AEAAXPEAUPTPInput@@@Z.c)
 *     ?SendMouseUpIfPending@CPTPEngine@@AEAAXPEAI@Z @ 0x1C0203D04 (-SendMouseUpIfPending@CPTPEngine@@AEAAXPEAI@Z.c)
 *     ?SendShellOutput@CBasePTPEngine@@IEAAXW4Action@Shell@Payload@PTPEngineOutput@@@Z @ 0x1C0203E50 (-SendShellOutput@CBasePTPEngine@@IEAAXW4Action@Shell@Payload@PTPEngineOutput@@@Z.c)
 *     ?SendTelemetryOutput@CBasePTPEngine@@IEAAXW4Action@Telemetry@Payload@PTPTelemetryOutput@@TParameters@345@@Z @ 0x1C0203EE0 (-SendTelemetryOutput@CBasePTPEngine@@IEAAXW4Action@Telemetry@Payload@PTPTelemetryOutput@@TParame.c)
 *     ?SuppressContactForMultiFingerTap@CPTPEngine@@AEAAHPEBUCContactState@@@Z @ 0x1C020430C (-SuppressContactForMultiFingerTap@CPTPEngine@@AEAAHPEBUCContactState@@@Z.c)
 */

__int64 __fastcall CPTPEngine::ProcessTapsFor3orMoreContacts(CPTPEngine *this, struct PTPInput *a2, unsigned int a3)
{
  unsigned int v3; // r9d
  int v4; // r14d
  unsigned int v5; // r10d
  char *v8; // rdx
  int v9; // r15d
  char *v10; // r8
  unsigned int v11; // eax
  char *v12; // r11
  int v13; // esi
  unsigned int v14; // r12d
  int v15; // r13d
  int *v16; // rdx
  int v17; // r8d
  int v18; // r9d
  int v19; // r10d
  int v20; // eax
  int v21; // eax
  int v22; // eax
  int v23; // eax
  __int128 v25; // [rsp+20h] [rbp-40h]
  __int128 v26; // [rsp+40h] [rbp-20h] BYREF
  __int64 v27; // [rsp+50h] [rbp-10h]

  v3 = *((_DWORD *)this + 984);
  v4 = 1;
  v5 = a3;
  if ( (v3 & 1) != 0 || (v3 & 2) != 0 || (v3 & 4) != 0 )
    return 0LL;
  v8 = (char *)this + 1208;
  v9 = 0;
  v10 = (char *)this + 3608;
  while ( v8 != v10 )
  {
    if ( (*(_DWORD *)v8 & 1) != 0 )
    {
      if ( (unsigned int)CPTPEngine::SuppressContactForMultiFingerTap(this, (const struct CContactState *)v8) )
      {
        ++v9;
      }
      else if ( *((_DWORD *)v8 + 35) < *((_DWORD *)this + 933) )
      {
        v4 = 0;
      }
    }
    v8 += 400;
  }
  v11 = *((_DWORD *)this + 904);
  if ( (v11 <= 2 || v5 >= v11) && (v3 & 0x1000000) == 0 )
    return 0LL;
  v12 = (char *)a2 + 56;
  v13 = 0;
  v14 = 0;
  if ( !*((_DWORD *)a2 + 12) )
    goto LABEL_34;
  while ( 1 )
  {
    v15 = v13;
    if ( !*((_DWORD *)v12 + 3)
      || (unsigned int)CPTPEngine::SuppressContactForMultiFingerTap(
                         this,
                         (CPTPEngine *)((char *)this
                                      + 400 * (unsigned int)(*((_DWORD *)v12 + 1) % *((_DWORD *)this + 4))
                                      + 1208)) )
    {
      v3 = *((_DWORD *)this + 984);
      goto LABEL_30;
    }
    v20 = *v16;
    if ( (*v16 & 4) != 0 || (v20 & 0x10) != 0 || (v20 & 8) != 0 )
      break;
    if ( v17 )
    {
      if ( (v20 & 0x40000) == 0 )
      {
        *((_DWORD *)this + 984) &= ~0x1000000u;
        v3 = *((_DWORD *)this + 984);
        goto LABEL_34;
      }
    }
    else
    {
      *v16 = v20 | 0x40000;
      v19 = *((_DWORD *)v12 + 3);
    }
    ++v13;
    v3 = *((_DWORD *)this + 984);
    if ( (v19 & 0x48000) != 0x40000 )
      v13 = v15;
    if ( (*((_DWORD *)this + 984) & 0x1000000) == 0 && v13 )
    {
      v3 |= 0x1000000u;
      v21 = *((_DWORD *)this + 904) - v9;
      *((_DWORD *)this + 984) = v3;
      *((_DWORD *)this + 905) = v21;
    }
LABEL_30:
    ++v14;
    v12 += 96;
    if ( v14 >= *((_DWORD *)a2 + 12) )
      goto LABEL_34;
  }
  v3 = v18 & 0xFEFFFFFF;
  *((_DWORD *)this + 984) = v3;
LABEL_34:
  if ( (v3 & 0x1000000) == 0 )
    return 0LL;
  CPTPEngine::SendMouseUpIfPending(this, (unsigned int *)this + 903);
  v22 = *((_DWORD *)this + 984);
  if ( (v22 & 8) != 0 || (v22 & 0x10) != 0 )
  {
    *((_DWORD *)this + 984) = v22 & 0xFEFFFFFF;
    return 0LL;
  }
  if ( v13 == *((_DWORD *)a2 + 12) - v9 )
  {
    v23 = *((_DWORD *)this + 905);
    if ( v23 == 3 )
    {
      CBasePTPEngine::SendShellOutput(this, 0LL);
      v25 = 0LL;
      LODWORD(v25) = 3;
      goto LABEL_42;
    }
    if ( v23 == 4 )
    {
      CBasePTPEngine::SendShellOutput(this, 1LL);
      v25 = 0LL;
      LODWORD(v25) = 4;
LABEL_42:
      v26 = v25;
      v27 = 0LL;
      CBasePTPEngine::SendTelemetryOutput(this, 4LL, &v26);
    }
    else if ( v23 == 2 && (*((_DWORD *)this + 116) & 0x400) != 0 )
    {
      if ( v4 )
        CPTPEngine::Send2FingerTap(this, a2);
    }
    *((_DWORD *)this + 984) &= ~0x1000000u;
    CPTPEngine::ResetDragThresholds(this, a2);
  }
  return 1LL;
}
