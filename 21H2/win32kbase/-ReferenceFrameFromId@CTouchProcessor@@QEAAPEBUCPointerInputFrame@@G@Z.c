/*
 * XREFs of ?ReferenceFrameFromId@CTouchProcessor@@QEAAPEBUCPointerInputFrame@@G@Z @ 0x1C019AD00
 * Callers:
        ...
        unsigned __int16 a2)
{
  ...
  __int16 v6; // dx
  CInpLockGuard *v8[8]; // [rsp+30h] [rbp-58h] BYREF
  ...
  ThreadPointerData = CTouchProcessor::GetThreadPointerData(
                        v2,
                        (struct _LIST_ENTRY *)gptiCurrent + 68,
                        v3,
                        0LL,
                        0LL,
                        0LL);
  if ( ThreadPointerData )
    v4 = CTouchProcessor::ReferenceFrame((struct _KTHREAD **)v2, HIDWORD(ThreadPointerData[1].Blink));
  if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED && LOWORD(WPP_GLOBAL_Control->DeviceType) )
  {
    LOBYTE(v6) = 5;
    WPP_RECORDER_SF_(
