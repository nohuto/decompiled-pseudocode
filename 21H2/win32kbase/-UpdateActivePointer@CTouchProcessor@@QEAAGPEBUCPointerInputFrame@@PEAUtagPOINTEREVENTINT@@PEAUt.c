/*
 * XREFs of ?UpdateActivePointer@CTouchProcessor@@QEAAGPEBUCPointerInputFrame@@PEAUtagPOINTEREVENTINT@@PEAUtagCHitTestState@@PEAVCInputDest@@PEAKPEAGPEAIKG@Z @ 0x1C019FE78
 * Callers:
  ...
  *a8 = 0;
  ...
  if ( NodeById )
  {
    if ( (a3[2].y & 1) == 0 )
    {
      ...
      if ( !CTouchProcessor::SetNewValidState(this, y, NodeById) )
      {
        ...
        {
          LOBYTE(v27) = 2;
          WPP_RECORDER_SF_((_DWORD)gBaseLog, v27, 7, 56, (__int64)&WPP_4ea2b35ef3aa38c2c6a59c3c8ae69e8c_Traceguids);
          if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
          {
            if ( LOWORD(WPP_GLOBAL_Control->DeviceType) )
            {
  ...
  else
  {
    ...
    if ( !NodeById )
    {
      ...
      {
        LOBYTE(v21) = 2;
        WPP_RECORDER_SF_((_DWORD)gBaseLog, v21, 7, 52, (__int64)&WPP_4ea2b35ef3aa38c2c6a59c3c8ae69e8c_Traceguids);
        if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
        {
          if ( LOWORD(WPP_GLOBAL_Control->DeviceType) )
          {
