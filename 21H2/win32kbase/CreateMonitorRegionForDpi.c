/*
 * XREFs of CreateMonitorRegionForDpi @ 0x1C006DB60
 * Callers:
 *     GetMonitorDC @ 0x1C00183FC (GetMonitorDC.c)
 *     ?SetMonitorRegion@@YAXPEAUtagMONITOR@@PEAUHRGN__@@1@Z @ 0x1C006D8F4 (-SetMonitorRegion@@YAXPEAUtagMONITOR@@PEAUHRGN__@@1@Z.c)
 * Callees:
 *     GreCreateRectRgnIndirect @ 0x1C0028800 (GreCreateRectRgnIndirect.c)
 *     WPP_RECORDER_AND_TRACE_SF_ @ 0x1C0037614 (WPP_RECORDER_AND_TRACE_SF_.c)
 *     GetMonitorRectForDpi @ 0x1C006DCC8 (GetMonitorRectForDpi.c)
 */

struct HOBJ__ *__fastcall CreateMonitorRegionForDpi(__int64 a1, unsigned __int16 a2)
{
  int v2; // edx
  struct HOBJ__ *v3; // rbx
  int v4; // r8d
  struct _SLIST_ENTRY v6; // [rsp+40h] [rbp-18h] BYREF

  v6 = *(struct _SLIST_ENTRY *)GetMonitorRectForDpi(&v6, a1, a2);
  v3 = GreCreateRectRgnIndirect(&v6);
  if ( !v3 )
  {
    LOBYTE(v2) = WPP_GLOBAL_Control != (PDEVICE_OBJECT)&WPP_GLOBAL_Control
              && (HIDWORD(WPP_GLOBAL_Control->Timer) & 0x1000000) != 0
              && BYTE1(WPP_GLOBAL_Control->Timer) >= 2u;
    if ( (_BYTE)v2 || WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
    {
      LOBYTE(v4) = WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED;
      WPP_RECORDER_AND_TRACE_SF_(
        WPP_GLOBAL_Control->AttachedDevice,
        v2,
        v4,
        WPP_MAIN_CB.Queue.ListEntry.Flink,
        2,
        25,
        10,
        (__int64)&WPP_5d862fe0dad535d6d4e6e742a9eec372_Traceguids);
    }
  }
  return v3;
}
