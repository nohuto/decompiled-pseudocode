/*
 * XREFs of ndisInvokeCloseAdapterComplete @ 0x1C00AC930
 * Callers:
 *     ?ndisMQueuedFinishClose@@YAXPEAX@Z @ 0x1C00AC7E0 (-ndisMQueuedFinishClose@@YAXPEAX@Z.c)
 * Callees:
 *     _guard_dispatch_icall_nop @ 0x1C0027180 (_guard_dispatch_icall_nop.c)
 *     WPP_SF_qq @ 0x1C003C398 (WPP_SF_qq.c)
 */

void __fastcall ndisInvokeCloseAdapterComplete(__int64 a1)
{
  __int64 v1; // rdi

  v1 = *(_QWORD *)(a1 + 24);
  if ( (unsigned __int8)byte_1C00895CD >= 4u )
    WPP_SF_qq(0x5Fu, &WPP_5a7dd1586dbc3ec36657a43c35bc8834_Traceguids, v1, a1);
  (*(void (__fastcall **)(_QWORD))(v1 + 120))(*(_QWORD *)(a1 + 32));
  if ( (unsigned __int8)byte_1C00895CD >= 4u )
    WPP_SF_qq(0x60u, &WPP_5a7dd1586dbc3ec36657a43c35bc8834_Traceguids, v1, a1);
}
