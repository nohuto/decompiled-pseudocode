/*
 * XREFs of ndisSetPower @ 0x1C009C35C
 * Callers:
 *     ndisPowerDispatch @ 0x1C001FC10 (ndisPowerDispatch.c)
 *     ndisPowerIrpWorker @ 0x1C00D49B0 (ndisPowerIrpWorker.c)
 * Callees:
 *     ndisSetDevicePower @ 0x1C0011174 (ndisSetDevicePower.c)
 *     WPP_SF_qq @ 0x1C003C398 (WPP_SF_qq.c)
 *     WPP_SF_qD @ 0x1C004451C (WPP_SF_qD.c)
 *     ndisDereferencePackage @ 0x1C009B138 (ndisDereferencePackage.c)
 *     ndisReferencePackage @ 0x1C009B308 (ndisReferencePackage.c)
 *     ndisSetSystemPower @ 0x1C009C3F4 (ndisSetSystemPower.c)
 */

__int64 __fastcall ndisSetPower(PIRP Irp, __int64 a2, __int64 a3)
{
  unsigned int v4; // ebx
  int v7; // ecx
  unsigned int v8; // eax

  v4 = 0;
  ndisReferencePackage((__int64)&ndisPkgs);
  if ( (unsigned __int8)byte_1C00895D5 >= 4u )
    WPP_SF_qq(0x66u, &WPP_7b68669345513bc3aa7a551c4dacedad_Traceguids, a3, Irp);
  v7 = *(_DWORD *)(a2 + 16);
  if ( v7 )
  {
    if ( v7 != 1 )
      goto LABEL_7;
    v8 = ndisSetDevicePower(Irp, a2, (enum _NDIS_DEVICE_POWER_STATE)*(_DWORD *)(a2 + 24), a3);
  }
  else
  {
    v8 = ndisSetSystemPower(Irp);
  }
  v4 = v8;
LABEL_7:
  if ( (unsigned __int8)byte_1C00895D5 >= 4u )
    WPP_SF_qD(0x67u, &WPP_7b68669345513bc3aa7a551c4dacedad_Traceguids, a3, v4);
  ndisDereferencePackage((__int64)&ndisPkgs);
  return v4;
}
