/*
 * XREFs of ?ReEnableTemporaryUnbind@BindRules@Ndis@@YAXPEAUBindStack@2@@Z @ 0x1C00A6A38
 * Callers:
 *     ?Apply@BindRules@Ndis@@YAXPEAU_NDIS_MINIPORT_BLOCK@@@Z @ 0x1C00A527C (-Apply@BindRules@Ndis@@YAXPEAU_NDIS_MINIPORT_BLOCK@@@Z.c)
 * Callees:
 *     __security_check_cookie @ 0x1C0026780 (__security_check_cookie.c)
 *     WPP_SF_dd @ 0x1C0042EFC (WPP_SF_dd_ea_1C0042EFC.c)
 *     WPP_SF_Zq @ 0x1C004EAC4 (WPP_SF_Zq.c)
 *     ?ndisGetBindLinkNameForTracing@@YAXPEAUNDIS_BIND_FILTER_LINK@@PEAUNDIS_PNPTRACE_LOCALS@@@Z @ 0x1C00E2FA4 (-ndisGetBindLinkNameForTracing@@YAXPEAUNDIS_BIND_FILTER_LINK@@PEAUNDIS_PNPTRACE_LOCALS@@@Z.c)
 *     ?ndisGetBindLinkNameForTracing@@YAXPEAUNDIS_BIND_PROTOCOL_LINK@@PEAUNDIS_PNPTRACE_LOCALS@@@Z @ 0x1C00E30C0 (-ndisGetBindLinkNameForTracing@@YAXPEAUNDIS_BIND_PROTOCOL_LINK@@PEAUNDIS_PNPTRACE_LOCALS@@@Z.c)
 */

void __fastcall Ndis::BindRules::ReEnableTemporaryUnbind(Ndis::BindRules *this, struct Ndis::BindStack *a2)
{
  int v2; // r15d
  __int64 i; // rdi
  __int64 v5; // r14
  __int64 v6; // rbx
  int v7; // r15d
  __int64 j; // rdi
  __int64 v9; // r14
  __int64 v10; // rbx
  int v11; // r8d
  char v12; // al
  int v13; // r8d
  char v14; // al
  __int64 v15; // [rsp+20h] [rbp-C8h] BYREF
  unsigned __int16 *v16; // [rsp+28h] [rbp-C0h]

  v2 = *((_DWORD *)this + 10);
  for ( i = 0LL; (_DWORD)i != v2; i = (unsigned int)(i + 1) )
  {
    if ( (unsigned int)i >= *((_DWORD *)this + 10) )
      __fastfail(0xBAD0FFu);
    v5 = *((_QWORD *)this + 6);
    v6 = *(_QWORD *)(v5 + 8 * i);
    if ( !*(_BYTE *)(v6 + 25) && !*(_DWORD *)(v6 + 72) )
    {
      v13 = *(_DWORD *)(v6 + 48);
      *(_DWORD *)(v6 + 48) = v13 & 0xFFFFDFFF;
      if ( (v13 != 0) == ((v13 & 0xFFFFDFFF) != 0) && *(_QWORD *)(v6 + 16) )
      {
        v14 = 0;
      }
      else
      {
        if ( (unsigned __int8)byte_1C00895E3 >= 5u )
          WPP_SF_dd(0xEu, &WPP_dd4cdbe2961831c412ed9f171c8a4c99_Traceguids, v13, v13 & 0xFFFFDFFF);
        *(_BYTE *)(*(_QWORD *)(v6 + 16) + 5312LL) = 1;
        v14 = 1;
      }
      if ( v14 && (unsigned __int8)byte_1C00895E3 >= 4u )
      {
        ndisGetBindLinkNameForTracing(
          *(struct NDIS_BIND_FILTER_LINK **)(v5 + 8 * i),
          (struct NDIS_PNPTRACE_LOCALS *)&v15);
        if ( (unsigned __int8)byte_1C00895E3 >= 4u )
          WPP_SF_Zq(0xAu, &WPP_339be45e8b703252d0c5449fb14c5e43_Traceguids, v16, v15);
      }
    }
  }
  v7 = *((_DWORD *)this + 4);
  for ( j = 0LL; (_DWORD)j != v7; j = (unsigned int)(j + 1) )
  {
    if ( (unsigned int)j >= *((_DWORD *)this + 4) )
      __fastfail(0xBAD0FFu);
    v9 = *((_QWORD *)this + 3);
    v10 = *(_QWORD *)(v9 + 8 * j);
    if ( !*(_BYTE *)(v10 + 25) && !*(_DWORD *)(v10 + 72) )
    {
      v11 = *(_DWORD *)(v10 + 48);
      *(_DWORD *)(v10 + 48) = v11 & 0xFFFFDFFF;
      if ( (v11 != 0) == ((v11 & 0xFFFFDFFF) != 0) && *(_QWORD *)(v10 + 16) )
      {
        v12 = 0;
      }
      else
      {
        if ( (unsigned __int8)byte_1C00895E3 >= 5u )
          WPP_SF_dd(0xEu, &WPP_dd4cdbe2961831c412ed9f171c8a4c99_Traceguids, v11, v11 & 0xFFFFDFFF);
        *(_BYTE *)(*(_QWORD *)(v10 + 16) + 5312LL) = 1;
        v12 = 1;
      }
      if ( v12 && (unsigned __int8)byte_1C00895E3 >= 4u )
      {
        ndisGetBindLinkNameForTracing(
          *(struct NDIS_BIND_PROTOCOL_LINK **)(v9 + 8 * j),
          (struct NDIS_PNPTRACE_LOCALS *)&v15);
        WPP_SF_Zq(0xBu, &WPP_339be45e8b703252d0c5449fb14c5e43_Traceguids, v16, v15);
      }
    }
  }
}
