/*
 * XREFs of ACPIBuildProcessDevicePhasePrx @ 0x1C0013050
 * Callers:
 *     <none>
 * Callees:
 *     AMLIReferenceHandleEx @ 0x1C000E210 (AMLIReferenceHandleEx.c)
 *     ACPIBuildCompleteCommon @ 0x1C000F100 (ACPIBuildCompleteCommon.c)
 *     ACPIBuildScheduleDpc @ 0x1C0013240 (ACPIBuildScheduleDpc.c)
 *     WPP_RECORDER_SF_dLqss @ 0x1C0013270 (WPP_RECORDER_SF_dLqss.c)
 *     FreeDataBuffs @ 0x1C001B580 (FreeDataBuffs.c)
 *     ACPIBuildDevicePowerNodes @ 0x1C00285EC (ACPIBuildDevicePowerNodes.c)
 *     ACPIWriteEventLogEntry @ 0x1C004D554 (ACPIWriteEventLogEntry.c)
 */

__int64 __fastcall ACPIBuildProcessDevicePhasePrx(__int64 a1, __int64 a2, __int64 a3, int a4)
{
  _QWORD *v5; // rbp
  int v6; // r15d
  __int64 v7; // rcx
  __int64 v8; // r14
  __int64 v9; // r13
  __int64 *v10; // rbx
  int v11; // edi
  KIRQL v12; // dl
  __int64 v13; // rax
  __int64 *v14; // rbx
  __int64 *v15; // rax
  __int64 v16; // rcx
  ULONG_PTR v17; // rdx
  __int64 *v18; // rdx
  signed __int32 v19; // ecx
  KIRQL v20; // bl

  v5 = *(_QWORD **)(a1 + 40);
  v6 = 0;
  v7 = a1 + 80;
  v8 = (unsigned int)(*(_DWORD *)(a1 + 28) - 19) >> 1;
  v9 = (unsigned int)(v8 + 1);
  *(_DWORD *)(a1 + 32) = 2 * v8 + 20;
  if ( v5[v9 + 51] || v5[v9 + 45] )
  {
    if ( !*(_QWORD *)(a1 + 56) )
      goto LABEL_10;
    dword_1C0074698 = 0;
    byte_1C007469C = 0;
LABEL_24:
    FreeDataBuffs(v7, 1LL);
    goto LABEL_10;
  }
  v10 = (__int64 *)v5[89];
  v11 = *(_DWORD *)&aPs0Ps1Ps2[4 * v8];
  v12 = ExAcquireSpinLockShared(&ACPINamespaceLock);
  v13 = *v10;
  v14 = *(__int64 **)(*v10 + 24);
  v15 = (__int64 *)(v13 + 24);
  if ( v15 == v14 )
  {
LABEL_6:
    v14 = 0LL;
  }
  else
  {
    while ( v11 != *((_DWORD *)v14 + 10) )
    {
      v14 = (__int64 *)*v14;
      if ( v15 == v14 )
        goto LABEL_6;
    }
  }
  ExReleaseSpinLockShared(&ACPINamespaceLock, v12);
  if ( v14 )
    AMLIReferenceHandleEx((__int64)(v14 + 15));
  else
    v16 = 0LL;
  v5[v9 + 51] = v16;
  v17 = *(_QWORD *)(a1 + 56);
  if ( v17 )
  {
    if ( *(_WORD *)(a1 + 82) != 4 )
    {
      ACPIWriteEventLogEntry(3221553168LL, 0LL, 0LL);
      goto LABEL_10;
    }
    v6 = ACPIBuildDevicePowerNodes((ULONG_PTR)v5, v17);
    dword_1C0074698 = 0;
    v7 = a1 + 80;
    byte_1C007469C = 0;
    goto LABEL_24;
  }
LABEL_10:
  v18 = qword_1C002C340;
  if ( v5 )
  {
    a3 = v5[1];
    a4 = 0;
    if ( (a3 & 0x200000000000LL) != 0 )
    {
      a4 = 0;
      if ( (a3 & 0x400000000000LL) != 0 )
        v18 = (__int64 *)v5[71];
    }
  }
  WPP_RECORDER_SF_dLqss(WPP_GLOBAL_Control->DeviceExtension, (_DWORD)v18, a3, a4);
  v19 = *(_DWORD *)(a1 + 32);
  if ( v6 == -1073741738 )
  {
    *(_DWORD *)(a1 + 48) = -1073741738;
    ACPIBuildCompleteCommon((volatile signed __int32 *)(a1 + 24), 2);
  }
  else
  {
    if ( v6 < 0 )
    {
      *(_DWORD *)(a1 + 48) = v6;
      KeBugCheckEx(0xA5u, 3uLL, 0LL, v6, 0LL);
    }
    *(_DWORD *)(a1 + 32) = 2;
    _InterlockedCompareExchange((volatile signed __int32 *)(a1 + 24), v19, 1);
    v20 = KeAcquireSpinLockRaiseToDpc(&AcpiBuildQueueLock);
    ACPIBuildScheduleDpc();
    KeReleaseSpinLock(&AcpiBuildQueueLock, v20);
  }
  return (unsigned int)v6;
}
