/*
 * XREFs of DpiMiracastSendSyncUserModeRequest @ 0x1C0197E9C
 * Callers:
 *     DpiMiracastInterfaceChange @ 0x1C002B510 (DpiMiracastInterfaceChange.c)
 * Callees:
 *     ?IsMiniportListMutexOwnedByCurrentThread@@YAEXZ @ 0x1C000EB34 (-IsMiniportListMutexOwnedByCurrentThread@@YAEXZ.c)
 *     Template_pdqqq @ 0x1C002ECC4 (Template_pdqqq.c)
 *     DxgkIsAdapterCoreSyncAcquired @ 0x1C0149C04 (DxgkIsAdapterCoreSyncAcquired.c)
 */

__int64 __fastcall DpiMiracastSendSyncUserModeRequest(
        __int64 a1,
        __int64 a2,
        void *a3,
        __int64 a4,
        PVOID a5,
        __int64 a6,
        PIO_STATUS_BLOCK IoStatusBlock)
{
  __int64 v9; // rcx
  __int64 v10; // rax
  __int64 v11; // rdi
  __int64 v12; // rcx
  __int64 v13; // rax
  struct _DEVICE_OBJECT *v14; // rdi
  struct _FILE_OBJECT *v15; // rsi
  __int64 v16; // rdx
  __int64 v17; // rcx
  __int64 v18; // r8
  __int64 v19; // r9
  __int64 v20; // rax
  PIRP v22; // rax
  __int64 v23; // rcx
  __int64 v24; // rax
  NTSTATUS v25; // eax
  __int64 v26; // rcx
  __int64 v27; // rax
  __int64 v28; // rcx
  __int64 v29; // r8
  __int64 v30; // rax
  PVOID OutputBuffer; // [rsp+20h] [rbp-68h]
  __int64 OutputBufferLength; // [rsp+28h] [rbp-60h]
  __int64 InternalDeviceIoControl; // [rsp+30h] [rbp-58h]
  PKEVENT v34; // [rsp+38h] [rbp-50h]
  struct _KEVENT Event; // [rsp+50h] [rbp-38h] BYREF

  if ( IsMiniportListMutexOwnedByCurrentThread() || *(struct _KTHREAD **)(a1 + 88) == KeGetCurrentThread() )
  {
    v30 = WdLogNewEntry5_WdError(v9);
    LODWORD(v14) = -1073741790;
    *(_QWORD *)(v30 + 24) = -1073741790LL;
    WdLogEvent5_WdError(v30);
  }
  else
  {
    ExEnterCriticalRegionAndAcquireFastMutexUnsafe(a1 + 32);
    *(_QWORD *)(a1 + 88) = KeGetCurrentThread();
    v10 = *(_QWORD *)(a1 + 440);
    if ( v10
      && ((v11 = *(_QWORD *)(v10 + 64), ExIsResourceAcquiredSharedLite(*(PERESOURCE *)(v11 + 168)))
       || DxgkIsAdapterCoreSyncAcquired(*(DXGADAPTER **)(v11 + 3688), 1)) )
    {
      v13 = WdLogNewEntry5_WdError(v12);
      LODWORD(v14) = -1073741790;
      *(_QWORD *)(v13 + 24) = -1073741790LL;
      WdLogEvent5_WdError(v13);
      *(_QWORD *)(a1 + 88) = 0LL;
      ExReleaseFastMutexUnsafeAndLeaveCriticalRegion(a1 + 32);
    }
    else
    {
      v15 = *(struct _FILE_OBJECT **)(a1 + 176);
      v14 = *(struct _DEVICE_OBJECT **)(a1 + 184);
      if ( v15 )
        ObfReferenceObject(*(PVOID *)(a1 + 176));
      *(_QWORD *)(a1 + 88) = 0LL;
      ExReleaseFastMutexUnsafeAndLeaveCriticalRegion(a1 + 32);
      if ( !v15 )
      {
        v20 = WdLogNewEntry5_WdWarning(v17, v16, v18, v19);
        *(_QWORD *)(v20 + 24) = a1;
        WdLogEvent5_WdWarning(v20);
        return 3221226166LL;
      }
      if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x200000) != 0 )
        Template_pdqqq(v17, &EventEnterMiracastSendUserModeRequest, v18, *(_QWORD *)(a1 + 96), 1, 148, 148, 0);
      KeInitializeEvent(&Event, NotificationEvent, 0);
      v22 = IoBuildDeviceIoControlRequest(0x230800u, v14, a3, 0x94u, a5, 0x94u, 0, &Event, IoStatusBlock);
      if ( v22 )
      {
        v22->Tail.Overlay.CurrentStackLocation[-1].FileObject = v15;
        v22->IoStatus.Status = -1073741637;
        LODWORD(v14) = IofCallDriver(v14, v22);
        if ( (_DWORD)v14 == 259 )
        {
          v25 = KeWaitForSingleObject(&Event, Executive, 0, 0, 0LL);
          v14 = (struct _DEVICE_OBJECT *)v25;
          if ( v25 )
          {
            v27 = WdLogNewEntry5_WdError(v26);
            *(_QWORD *)(v27 + 24) = v14;
            WdLogEvent5_WdError(v27);
          }
          else
          {
            LODWORD(v14) = IoStatusBlock->Status;
          }
        }
      }
      else
      {
        LODWORD(v14) = -1073741801;
        v24 = WdLogNewEntry5_WdLowResource(v23);
        *(_QWORD *)(v24 + 24) = -1073741801LL;
        WdLogEvent5_WdLowResource(v24);
      }
      ObfDereferenceObject(v15);
      if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x200000) != 0 )
      {
        LODWORD(v34) = (_DWORD)v14;
        LODWORD(InternalDeviceIoControl) = IoStatusBlock->Information;
        LODWORD(OutputBufferLength) = 148;
        LODWORD(OutputBuffer) = 1;
        Template_pdqqq(
          v28,
          &EventLeaveMiracastSendUserModeRequest,
          v29,
          *(_QWORD *)(a1 + 96),
          OutputBuffer,
          OutputBufferLength,
          InternalDeviceIoControl,
          v34);
      }
    }
  }
  return (unsigned int)v14;
}
