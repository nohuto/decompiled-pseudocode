/*
 * XREFs of SmpOpenKnownDllsHandles @ 0x140005740
 * Callers:
...
__int64 __fastcall SmpOpenKnownDllsHandles(struct _UNICODE_STRING *a1, int a2, void **a3, void **a4)
{
  ...
  NTSTATUS v9; // eax
  WCHAR *Heap; // rax
  void *SymbolicLinkHandle; // [rsp+38h] [rbp-49h] BYREF
  void *FileHandle; // [rsp+40h] [rbp-41h] BYREF
  struct _UNICODE_STRING LinkTarget; // [rsp+48h] [rbp-39h] BYREF
  ...
  void *v18; // [rsp+98h] [rbp+17h] BYREF
  int v19; // [rsp+A0h] [rbp+1Fh] BYREF
  const wchar_t *v20; // [rsp+A8h] [rbp+27h]
  _IO_STATUS_BLOCK IoStatusBlock; // [rsp+B0h] [rbp+2Fh] BYREF
      ...
      *(_QWORD *)&LinkTarget.Length = 0LL;
      ...
      if ( Heap )
      {
        ...
        if ( v8 >= 0 )
        {
          ...
          ObjectAttributes.Attributes = 64;
          *(_OWORD *)&ObjectAttributes.SecurityDescriptor = 0LL;
          v8 = NtOpenFile(&v18, 0x100001u, &ObjectAttributes, &IoStatusBlock, 3u, 0x21u);
          if ( v8 >= 0 )
          {
            *a3 = FileHandle;
            *a4 = v18;
