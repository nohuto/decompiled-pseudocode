/*
 * XREFs of SmpStartCsr @ 0x140003620
 * Callers:
...
__int64 __fastcall SmpStartCsr(__int64 a1)
{
  ...
  *(_OWORD *)BaseAddress = 0LL;
  ...
  if ( CoreProcessIds >= 0 )
  {
    CoreProcessIds = SmpExecuteCommand((__int64)&UnicodeString, 0, v6, 5152, &v19);
    ...
    if ( CoreProcessIds >= 0 )
    {
      ...
      if ( v7 >= 0 )
      {
        ...
        *((_DWORD *)v4 + 2) = ProcessInformation;
        v9 = _InterlockedIncrement(&dword_140026208);
        if ( v9 > SmpNumberInitialSessions )
          _InterlockedDecrement(&dword_140026208);
        else
