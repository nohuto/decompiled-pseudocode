        ...
        unsigned int *a7)
{
  ...
  unsigned int v35; // [rsp+E8h] [rbp-18h]
  unsigned int v36; // [rsp+ECh] [rbp-14h]
  wchar_t Str[2]; // [rsp+F0h] [rbp-10h] BYREF
  ULONG v38; // [rsp+F4h] [rbp-Ch]
              ...
              if ( v38 )
              {
                do
                {
                  if ( ZwEnumerateKey(v28, v15, KeyBasicInformation, KeyInformation, 0xFEu, &ResultLength) < 0 )
                    break;
                  Str[(unsigned __int64)v36 >> 1] = 0;
                  if ( wtoi(Str) && wtoi(Str) != *a7 )
                  {
                    ValueName.Length = v36;
                    ObjectAttributes.Length = 48;
                    ValueName.MaximumLength = v36 + 2;
                    ObjectAttributes.Attributes = 576;
                    ...
                    *(_OWORD *)&ObjectAttributes.SecurityDescriptor = 0LL;
                    if ( ZwOpenKey(&Handle, 0x20019u, &ObjectAttributes) >= 0 )
                    {
                      ...
                      if ( ZwQueryValueKey(
                             ...
                             &ResultLength) >= 0
                        && KeyInformation[1] == 4
                        && (*(_DWORD *)((char *)KeyInformation + v35) > Data || Data == -1) )
                      {
                        Data = *(_DWORD *)((char *)KeyInformation + v35);
                      }
                      ZwClose(Handle);
                    }
                    Handle = 0LL;
