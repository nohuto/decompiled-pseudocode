/*
 * XREFs of PopCheckConsoleTimeouts @ 0x1406749A0
 * Callers:
...
char PopCheckConsoleTimeouts()
{
  ...
  v2 = PopInputTimeout;
  v3 = DisplayTimeout;
  if ( !BYTE5(qword_1403023C4) || !PopInputTimeout )
  {
    if ( PopInputTimeout )
    {
      if ( PopInputTimeout == (_DWORD)qword_1403023C4 )
        goto LABEL_8;
    }
    else
    {
      BYTE5(qword_1403023C4) = 0;
    }
    PopPrintEx(3u, "PopAdaptive: Input timeout: %u->%u\n", qword_1403023C4, PopInputTimeout);
    LODWORD(qword_1403023C4) = v2;
    LOBYTE(DisplayTimeout) = PopDiagTraceInputTimeout(v2, BYTE5(qword_1403023C4), BYTE4(qword_1403023C4));
    DWORD2(PopLazyContext) = v2;
    LOBYTE(qword_140302410) = 1;
    byte_1403023E4 = v2 != 0;
    if ( !v2 )
      LOBYTE(DisplayTimeout) = PopSetSessionUserStatus(v0, 0);
  }
LABEL_8:
  if ( BYTE4(qword_1403023CC) && (_DWORD)v3 )
  {
    LOBYTE(DisplayTimeout) = -1;
    v4 = v3 + (dword_1403023DC - dword_1403023D8) / 0x3E8u;
    if ( v4 > 0xFFFFFFFF )
      LODWORD(v4) = -1;
    LODWORD(v3) = v4;
  }
  if ( (_DWORD)v3 != (_DWORD)qword_1403023CC )
  {
    PopPrintEx(3u, "PopAdaptive: Display timeout: %u->%u\n", qword_1403023CC, v3);
    LODWORD(qword_1403023CC) = v3;
    LOBYTE(DisplayTimeout) = PopDiagTraceDisplayTimeout(v3, BYTE5(qword_1403023CC), BYTE4(qword_1403023CC));
