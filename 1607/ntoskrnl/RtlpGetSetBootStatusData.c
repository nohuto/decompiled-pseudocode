...
NTSTATUS __fastcall RtlpGetSetBootStatusData(HANDLE FileHandle, char a2, int a3, void *a4, unsigned int a5, _DWORD *a6)
{
  ...
  struct _IO_STATUS_BLOCK IoStatusBlock; // [rsp+60h] [rbp-79h] BYREF
  ...
  if ( result >= 0 )
  {
    ...
    else
    {
      ...
      {
        if ( a2 )
          result = ZwReadFile(FileHandle, 0LL, 0LL, 0LL, &IoStatusBlock, a4, Length, &ByteOffset, 0LL);
        else
          result = ZwWriteFile(FileHandle, 0LL, 0LL, 0LL, &IoStatusBlock, a4, Length, &ByteOffset, 0LL);
        if ( result >= 0 )
        {
          if ( a6 )
