/*
 * XREFs of RtlpSetupExtendedBlock @ 0x140213748
 * Callers:
 *     RtlpAllocateHeapInternal @ 0x1400937B8 (RtlpAllocateHeapInternal.c)
 * Callees:
 *     ExAcquireResourceExclusiveLite @ 0x140067CE0 (ExAcquireResourceExclusiveLite.c)
 *     ExReleaseResourceLite @ 0x1400684C0 (ExReleaseResourceLite.c)
 *     RtlpAnalyzeHeapFailure @ 0x1402183D8 (RtlpAnalyzeHeapFailure.c)
 */

__int64 __fastcall RtlpSetupExtendedBlock(
        __int64 a1,
        unsigned __int8 a2,
        __int64 a3,
        __int64 a4,
        unsigned __int16 a5,
        int a6)
{
  char v7; // cl
  char v8; // r15
  __int64 v10; // rdx
  int v11; // ecx
  unsigned __int16 v12; // ax
  int v14; // [rsp+28h] [rbp-20h]

  v7 = *(_BYTE *)(a3 - 1);
  v8 = 0;
  if ( v7 == 4 )
  {
    if ( ((a2 | *(_BYTE *)(a1 + 116)) & 1) == 0 )
    {
      ExAcquireResourceExclusiveLite(*(PERESOURCE *)(a1 + 352), 1u);
      v8 = 1;
    }
    if ( *(_DWORD *)(a1 + 124) )
    {
      *(_DWORD *)(a3 - 8) ^= *(_DWORD *)(a1 + 136);
      if ( *(_BYTE *)(a3 - 5) != (*(_BYTE *)(a3 - 8) ^ (unsigned __int8)(*(_BYTE *)(a3 - 7) ^ *(_BYTE *)(a3 - 6))) )
        RtlpAnalyzeHeapFailure(a1);
    }
    *(_WORD *)(a3 - 8) += a5;
    LOWORD(v10) = *(_WORD *)(a3 - 8);
    *(_BYTE *)(a3 - 2) = a5 >> 4;
    if ( *(_DWORD *)(a1 + 124) )
    {
      *(_BYTE *)(a3 - 5) = *(_BYTE *)(a3 - 8) ^ *(_BYTE *)(a3 - 7) ^ *(_BYTE *)(a3 - 6);
      *(_DWORD *)(a3 - 8) ^= *(_DWORD *)(a1 + 136);
    }
    *(_BYTE *)(a5 + a3 - 2) = a5 >> 4;
  }
  else
  {
    if ( v7 == 5 )
    {
      LOWORD(v10) = *(_WORD *)(a3 - 4) ^ *(_WORD *)(a1 + 140);
    }
    else if ( (v7 & 0x40) != 0 )
    {
      LOWORD(v10) = *(_WORD *)(a3 + 16LL * (v7 & 0x3F) - 4);
    }
    else if ( (v7 & 0x3F) == 0x3F )
    {
      if ( *(_DWORD *)(a1 + 124) )
      {
        v11 = *(_DWORD *)(a3 - 8);
        LOWORD(v14) = v11;
        if ( (*(_DWORD *)(a1 + 124) & v11) != 0 )
          v14 = *(_DWORD *)(a1 + 136) ^ v11;
        v12 = v14;
      }
      else
      {
        v12 = *(_WORD *)(a3 - 8);
      }
      v10 = *(_QWORD *)(a3 + 16LL * v12 - 16);
    }
    else
    {
      LOWORD(v10) = v7 & 0x3F;
    }
    *(_BYTE *)(a5 + a3 - 2) = a5 >> 4;
    *(_BYTE *)(a3 - 1) = (a5 >> 4) | *(_BYTE *)(a3 - 1) & 0xC0 | 0x40;
  }
  *(_BYTE *)(a5 + a3 - 1) = 5;
  *(_WORD *)(a5 + a3 - 4) = a5 + v10;
  *(_DWORD *)(a5 + a3 - 8) = a6;
  if ( v8 )
    ExReleaseResourceLite(*(PERESOURCE *)(a1 + 352));
  return a5 + a3;
}
