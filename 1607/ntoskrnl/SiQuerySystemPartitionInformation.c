/*
 * XREFs of SiQuerySystemPartitionInformation @ 0x14053B914
 * Callers:
 *     SyspartGetSystemPartition @ 0x14053B8CC (SyspartGetSystemPartition.c)
 * Callees:
...
__int64 __fastcall SiQuerySystemPartitionInformation(void *a1, unsigned int a2, unsigned int *a3)
{
  ...
  v6 = ZwQuerySystemInformation(MaxSystemInfoClass, 0LL, 0, &ReturnLength);
  v7 = v6;
  if ( v6 == -1073741789 )
  {
    ...
    if ( PoolWithTag )
    {
      v7 = ZwQuerySystemInformation(MaxSystemInfoClass, PoolWithTag, ReturnLength, &ReturnLength);
      if ( v7 >= 0 )
      {
