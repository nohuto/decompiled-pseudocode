/*
 * XREFs of RtlpPrintErrorInformation @ 0x1800F97EC
 * Callers:
...
ULONG RtlpPrintErrorInformation()
{
  ...
  if ( NtCurrentPeb()->Ldr )
    ...
    DbgPrint("HEAP: ");
  DbgPrint("Heap error detected at %p (heap handle %p)\n", (const void *)qword_18014F6C8, (const void *)qword_18014F6C0);
  if ( dword_18014F6B8 > 7 )
  {
    switch ( dword_18014F6B8 )
    {
      case 8:
  ...
  else if ( dword_18014F6B8 == 7 )
  {
    v0 = "heap_failure_buffer_underrun";
  }
  else if ( dword_18014F6B8 )
  {
    switch ( dword_18014F6B8 )
    {
      case 1:
  ...
  if ( qword_18014F6E0 )
  {
    if ( NtCurrentPeb()->Ldr )
      ...
      DbgPrint("HEAP: ");
    DbgPrint("Parameter3: %p\n", (const void *)qword_18014F6E0);
  }
  if ( qword_18014F6E8 || qword_18014F6F0 )
  {
    if ( NtCurrentPeb()->Ldr )
      DbgPrint("HEAP[%wZ]: ", &NtCurrentPeb()->Ldr->InLoadOrderModuleList.Flink[5].Blink);
    else
    ...
    DbgPrint(
      ...
      (const void *)qword_18014F6E8,
      (const void *)qword_18014F6F0);
  }
  if ( NtCurrentPeb()->Ldr )
    DbgPrint("HEAP[%wZ]: ", &NtCurrentPeb()->Ldr->InLoadOrderModuleList.Flink[5].Blink);
  else
