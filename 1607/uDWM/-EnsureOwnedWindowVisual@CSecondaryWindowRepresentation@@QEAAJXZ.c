/*
 * XREFs of ?EnsureOwnedWindowVisual@CSecondaryWindowRepresentation@@QEAAJXZ @ 0x1800350AC
 * Callers:
...
__int64 __fastcall CSecondaryWindowRepresentation::EnsureOwnedWindowVisual(CSecondaryWindowRepresentation *this)
{
  ...
  if ( !*((_QWORD *)this + 6) )
  {
    ...
    if ( v4 < 0 )
    {
      MilInstrumentationCheckHR(0x14u, 0LL, 0, v4, 0xC9u);
    ...
    else
    {
      ...
      else
      {
        ...
        if ( inserted < 0 )
        {
          MilInstrumentationCheckHR(0x14u, 0LL, 0, inserted, 0xCBu);
        }
        else
        {
          *((_QWORD *)this + 6) = v6;
