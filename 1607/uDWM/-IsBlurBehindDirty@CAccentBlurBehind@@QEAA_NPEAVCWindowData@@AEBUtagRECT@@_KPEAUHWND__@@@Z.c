/*
 * XREFs of ?IsBlurBehindDirty@CAccentBlurBehind@@QEAA_NPEAVCWindowData@@AEBUtagRECT@@_KPEAUHWND__@@@Z @ 0x1800376B8
 * Callers:
 *     ?OnBlurBehindUpdated@CAccent@@QEAAXPEAVCWindowData@@@Z @ 0x1800361E0 (-OnBlurBehindUpdated@CAccent@@QEAAXPEAVCWindowData@@@Z.c)
 * Callees:
 *     ?GetActualWindowRect@CTopLevelWindow@@QEBA?AUtagRECT@@_N00@Z @ 0x180027A88 (-GetActualWindowRect@CTopLevelWindow@@QEBA-AUtagRECT@@_N00@Z.c)
 *     ?GetWindowListForDesktop@CWindowList@@QEAAPEAU_LIST_ENTRY@@_K@Z @ 0x180030360 (-GetWindowListForDesktop@CWindowList@@QEAAPEAU_LIST_ENTRY@@_K@Z.c)
 *     __security_check_cookie @ 0x18004F240 (__security_check_cookie.c)
 */

char __fastcall CAccentBlurBehind::IsBlurBehindDirty(
        CAccentBlurBehind *this,
        struct CWindowData *a2,
        const struct tagRECT *a3,
        __int64 a4,
        HWND a5)
{
  CTopLevelWindow *v5; // r10
  char v6; // bl
  __int64 v11; // r9
  __int64 v12; // rcx
  unsigned int v13; // edx
  unsigned __int8 v14; // cl
  struct _LIST_ENTRY *WindowListForDesktop; // rax
  struct _LIST_ENTRY *i; // rcx
  HWND Blink; // rdx
  struct tagRECT rcDst; // [rsp+30h] [rbp-38h] BYREF

  v5 = (CTopLevelWindow *)*((_QWORD *)a2 + 48);
  v6 = 0;
  if ( v5 )
  {
    v11 = *((_QWORD *)this + 34);
    v12 = 0LL;
    v13 = *((_DWORD *)this + 74);
    v6 = 1;
    if ( v13 )
    {
      while ( *((_QWORD *)a2 + 5) != *(_QWORD *)(v11 + 8 * v12) )
      {
        v12 = (unsigned int)(v12 + 1);
        if ( (unsigned int)v12 >= v13 )
          goto LABEL_3;
      }
    }
    else
    {
LABEL_3:
      CTopLevelWindow::GetActualWindowRect(v5, &rcDst, 0, 1, 1);
      v14 = *((_BYTE *)a2 + 576);
      if ( (v14 & 1) != 0
        && ((unsigned __int8)~*((_BYTE *)a2 + 580) & (unsigned __int8)~(v14 >> 2) & 1) != 0
        && (*((_DWORD *)a2 + 25) & 0x20000000) == 0
        && IntersectRect(&rcDst, &rcDst, a3) )
      {
        WindowListForDesktop = CWindowList::GetWindowListForDesktop(
                                 *((CWindowList **)CDesktopManager::s_pDesktopManagerInstance + 165),
                                 a4);
        for ( i = WindowListForDesktop->Blink; i != WindowListForDesktop; i = i->Blink )
        {
          Blink = (HWND)i[2].Blink;
          if ( Blink == a5 )
            break;
          if ( Blink == *((HWND *)a2 + 5) )
            return 0;
        }
      }
      else
      {
        return 0;
      }
    }
  }
  return v6;
}
