/*
 * XREFs of prfntKillList @ 0x1C0021534
 * Callers:
 *     ?bUnloadWorkhorse@PFTOBJ@@QEAAHPEAVPFF@@PEAPEAV2@K@Z @ 0x1C0022BB8 (-bUnloadWorkhorse@PFTOBJ@@QEAAHPEAVPFF@@PEAPEAV2@K@Z.c)
 *     ?bUnloadAllButPermanentFonts@PFTOBJ@@QEAAHH@Z @ 0x1C0022D74 (-bUnloadAllButPermanentFonts@PFTOBJ@@QEAAHH@Z.c)
 * Callees:
 *     ?prfntActive@PDEVOBJ@@QEAAPEAVRFONT@@XZ @ 0x1C00293B4 (-prfntActive@PDEVOBJ@@QEAAPEAVRFONT@@XZ.c)
 *     ?prfntInactive@PDEVOBJ@@QEAAPEAVRFONT@@XZ @ 0x1C00294A0 (-prfntInactive@PDEVOBJ@@QEAAPEAVRFONT@@XZ.c)
 *     ?cInactive@PDEVOBJ@@QEAAIXZ @ 0x1C00294B8 (-cInactive@PDEVOBJ@@QEAAIXZ.c)
 *     ??1RFONTOBJ@@QEAA@XZ @ 0x1C002D6C4 (--1RFONTOBJ@@QEAA@XZ.c)
 *     ?vRemove@RFONTOBJ@@QEAAXPEAPEAVRFONT@@W4_RFL_TYPE@@@Z @ 0x1C002D748 (-vRemove@RFONTOBJ@@QEAAXPEAPEAVRFONT@@W4_RFL_TYPE@@@Z.c)
 *     ?vInsert@RFONTOBJ@@QEAAXPEAPEAVRFONT@@W4_RFL_TYPE@@@Z @ 0x1C002D7D8 (-vInsert@RFONTOBJ@@QEAAXPEAPEAVRFONT@@W4_RFL_TYPE@@@Z.c)
 *     ?vUnlock@SEMOBJ@@QEAAXXZ @ 0x1C00FDC0C (-vUnlock@SEMOBJ@@QEAAXXZ.c)
 */

__int64 __fastcall prfntKillList(__int64 *a1)
{
  __int64 v2; // rdi
  struct PDEV *v3; // rbx
  struct RFONT *v4; // rsi
  __int64 v5; // rcx
  struct RFONT *v6; // rax
  __int64 v7; // rdx
  struct RFONT *v8; // rsi
  __int64 v9; // rcx
  struct RFONT *v10; // rax
  __int64 v11; // rdx
  unsigned int v12; // ecx
  __int64 v14; // [rsp+20h] [rbp-20h] BYREF
  __int64 v15; // [rsp+28h] [rbp-18h] BYREF
  __int64 v16; // [rsp+30h] [rbp-10h] BYREF
  struct RFONT *v17; // [rsp+78h] [rbp+38h] BYREF
  struct PDEV *v18; // [rsp+80h] [rbp+40h] BYREF
  __int64 v19; // [rsp+88h] [rbp+48h] BYREF

  v2 = 0LL;
  v19 = 0LL;
  v16 = ghsemDriverMgmt;
  GreAcquireSemaphore(ghsemDriverMgmt);
  v15 = ghsemRFONTList;
  GreAcquireSemaphore(ghsemRFONTList);
  GreAcquireHmgrSemaphore();
  v3 = gppdevList;
  if ( gppdevList )
  {
    do
    {
      if ( (*((_DWORD *)v3 + 14) & 1) != 0 )
      {
        v18 = v3;
        v4 = PDEVOBJ::prfntActive((PDEVOBJ *)&v18);
        while ( v4 )
        {
          v5 = *a1;
          v6 = v4;
          v4 = (struct RFONT *)*((_QWORD *)v4 + 63);
          v17 = v6;
          if ( *((_QWORD *)v6 + 15) == v5 && *((int *)v6 + 123) <= 0 )
          {
            v14 = *(_QWORD *)(v5 + 72);
            RFONTOBJ::vRemove(&v17, &v14, 0LL);
            *(_QWORD *)(*a1 + 72) = v14;
            if ( (*((_DWORD *)v3 + 14) & 0x800000) != 0 )
              v7 = *((_QWORD *)v3 + 442) + 1552LL;
            else
              v7 = (__int64)v3 + 1552;
            RFONTOBJ::vRemove(&v17, v7, 1LL);
            RFONTOBJ::vInsert(&v17, &v19, 1LL);
          }
          v17 = 0LL;
          RFONTOBJ::~RFONTOBJ((RFONTOBJ *)&v17);
        }
        v8 = PDEVOBJ::prfntInactive((PDEVOBJ *)&v18);
        while ( v8 )
        {
          v9 = *a1;
          v10 = v8;
          v8 = (struct RFONT *)*((_QWORD *)v8 + 63);
          v17 = v10;
          if ( *((_QWORD *)v10 + 15) == v9 )
          {
            v14 = *(_QWORD *)(v9 + 72);
            RFONTOBJ::vRemove(&v17, &v14, 0LL);
            *(_QWORD *)(*a1 + 72) = v14;
            if ( (*((_DWORD *)v3 + 14) & 0x800000) != 0 )
              v11 = *((_QWORD *)v3 + 442) + 1560LL;
            else
              v11 = (__int64)v3 + 1560;
            RFONTOBJ::vRemove(&v17, v11, 1LL);
            RFONTOBJ::vInsert(&v17, &v19, 1LL);
            v12 = PDEVOBJ::cInactive((PDEVOBJ *)&v18) - 1;
            if ( (*((_DWORD *)v3 + 14) & 0x800000) != 0 )
              *(_DWORD *)(*((_QWORD *)v3 + 442) + 1568LL) = v12;
            else
              *((_DWORD *)v3 + 392) = v12;
          }
          v17 = 0LL;
          RFONTOBJ::~RFONTOBJ((RFONTOBJ *)&v17);
        }
      }
      v3 = (struct PDEV *)*((_QWORD *)v3 + 3);
    }
    while ( v3 );
    v2 = v19;
  }
  GreReleaseHmgrSemaphore();
  SEMOBJ::vUnlock((SEMOBJ *)&v15);
  SEMOBJ::vUnlock((SEMOBJ *)&v16);
  return v2;
}
