/*
 * XREFs of ?xxxInitTerminal@@YAJPEAUtagTERMINAL@@@Z @ 0x1C00EAA84
 * Callers:
...
NTSTATUS __fastcall xxxInitTerminal(struct tagTERMINAL *a1)
{
  ...
  _BYTE v19[52]; // [rsp+88h] [rbp-80h] BYREF
...
LABEL_26:
  ...
  *((_QWORD *)a1 + 5) = Object;
  ...
  if ( v3 < 0 )
    goto LABEL_26;
  v6 = (void *)*((_QWORD *)a1 + 5);
  if ( !(unsigned int)InitCreateSystemThreadsMsg(v19, 1LL, a1) )
    goto LABEL_28;
  v8 = 0;
  if ( gdwInAtomicOperation )
  {
  ...
  v10 = PsGetCurrentProcess(v9) == gpepCSRSS
      ? LpcRequestPort(CsrApiPort, v19)
      : LpcRequestWaitReplyPort(CsrApiPort, v19, v19);
  if ( v10 >= 0 )
  {
    CurrentProcess = PsGetCurrentProcess(v11);
    if ( v20 >= 0 || CurrentProcess == gpepCSRSS )
    {
      KeWaitForSingleObject(v6, WrUserRequest, 0, 0, 0LL);
      v8 = 1;
    }
