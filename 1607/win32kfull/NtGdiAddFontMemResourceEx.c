/*
 * XREFs of NtGdiAddFontMemResourceEx @ 0x1C02A6DC0
 * Callers:
 *     <none>
 * Callees:
 *     __security_check_cookie @ 0x1C0158CD0 (__security_check_cookie.c)
 *     memmove @ 0x1C015A040 (memmove.c)
 *     GreAddFontMemResourceEx @ 0x1C027C95C (GreAddFontMemResourceEx.c)
 *     GreRemoveFontMemResourceEx @ 0x1C027DCCC (GreRemoveFontMemResourceEx.c)
 */

__int64 __fastcall NtGdiAddFontMemResourceEx(
        struct tagDOWNLOADFONTHEADER *a1,
        int a2,
        _BYTE *a3,
        unsigned int a4,
        _DWORD *a5)
{
  __int64 v5; // r15
  _BYTE *v6; // r10
  int v9; // edi
  __int64 v10; // rsi
  _DWORD *v11; // r14
  _DWORD v13[4]; // [rsp+38h] [rbp-C0h] BYREF
  _BYTE *v14; // [rsp+48h] [rbp-B0h]
  unsigned int v15[2]; // [rsp+50h] [rbp-A8h]
  struct tagDOWNLOADFONTHEADER *v16; // [rsp+58h] [rbp-A0h]
  _BYTE v17[4]; // [rsp+60h] [rbp-98h] BYREF
  unsigned int v18; // [rsp+64h] [rbp-94h]

  v5 = a4;
  v6 = a3;
  v16 = a1;
  v13[2] = a2;
  v14 = a3;
  v13[0] = a4;
  v9 = 1;
  v10 = 0LL;
  if ( !a2 )
    return 0LL;
  if ( !a1 )
    return 0LL;
  v11 = a5;
  if ( !a5 )
    return 0LL;
  if ( a4 )
  {
    if ( a4 > 0x48 )
      goto LABEL_9;
    if ( (unsigned __int64)a3 >= W32UserProbeAddress )
      v6 = (_BYTE *)W32UserProbeAddress;
    memmove(v17, v6, a4);
    v6 = v17;
    v14 = v17;
    if ( v5 != 4LL * v18 + 8 )
LABEL_9:
      v9 = 0;
  }
  else
  {
    v6 = 0LL;
    v14 = 0LL;
  }
  if ( v9 )
  {
    v10 = GreAddFontMemResourceEx(a1, a2, (struct tagDESIGNVECTOR *)v6, v5, v13);
    *(_QWORD *)v15 = v10;
    if ( v10 )
    {
      if ( (unsigned __int64)a5 >= W32UserProbeAddress )
        v11 = (_DWORD *)W32UserProbeAddress;
      *v11 = v13[0];
    }
  }
  return v10;
}
