/*
 * XREFs of itrp_SPVTCA_0 @ 0x1C010CAD0
 * Callers:
 *     <none>
 * Callees:
 *     itrp_Check_PF_Proj @ 0x1C010CB9C (itrp_Check_PF_Proj.c)
 */

__int64 __fastcall itrp_SPVTCA_0(__int64 a1)
{
  __int64 v1; // rdx
  __int16 v3; // ax
  __int64 v4; // rcx
  __int64 v5; // rcx
  __int64 v6; // r8
  __int64 result; // rax
  int v8; // r10d
  __int16 v9; // r9

  v1 = qword_1C03294E0;
  dword_1C03294B8 = 0x40000000;
  dword_1C0329510 = 4;
  v3 = *(_WORD *)(qword_1C03294E0 + 452);
  if ( (v3 & 1) != 0 && (v3 & 4) != 0 )
  {
    word_1C0329554 = 1;
    if ( (*(_BYTE *)(qword_1C03294E0 + 120) & 4) != 0 || *(_BYTE *)(qword_1C03294E0 + 333) )
    {
      v4 = 1LL;
      goto LABEL_4;
    }
  }
  else
  {
    word_1C0329554 = 0;
  }
  v4 = 0LL;
LABEL_4:
  v5 = *(unsigned __int16 *)(qword_1C03294E0 + 132) + 8 * v4;
  *(_DWORD *)(qword_1C03294E0 + 128) = itrp_RoundFunctionId[v5];
  word_1C0329558 = -1;
  word_1C0329556 = -1;
  word_1C0329508 = HIWORD(dword_1C03294BC);
  itrp_Check_PF_Proj(v5, v1, a1, 0LL);
  dword_1C0329514 = dword_1C0329510;
  result = v6;
  dword_1C032950C = v8;
  word_1C032952C = v9;
  return result;
}
