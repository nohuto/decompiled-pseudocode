/*
 * XREFs of GetDPIMETRICSForDpiUnsafe @ 0x1C00B5C20
 * Callers:
...
struct tagDpiKMMetricsCacheNode *__fastcall GetDPIMETRICSForDpiUnsafe(int a1)
{
  ...
  if ( a1 == *(unsigned __int16 *)(gpsi + 8678LL) )
    return (struct tagDpiKMMetricsCacheNode *)gdpiSystem;
  if ( a1 == 96 )
    return (struct tagDpiKMMetricsCacheNode *)gdpi96;
  result = EnsureKMDpiMetricsCacheNode(a1);
  v2 = result;
  if ( result )
  {
    RefreshDpiKMMetricsCacheNode(result);
    return (struct tagDpiKMMetricsCacheNode *)((char *)v2 + 120);
  }
  return result;
}
