/*
 * XREFs of lQueryDEVICEMETRICS @ 0x1C0016AB0
 * Callers:
 *     ttfdQueryFontData @ 0x1C00C2348 (ttfdQueryFontData.c)
 * Callees:
 *     bFDXform @ 0x1C00169E4 (bFDXform.c)
 *     bFToL @ 0x1C0018BF4 (bFToL.c)
 *     CJGD @ 0x1C00C296C (CJGD.c)
 *     FixMul @ 0x1C00C947C (FixMul.c)
 *     __security_check_cookie @ 0x1C0158CD0 (__security_check_cookie.c)
 */

__int64 __fastcall lQueryDEVICEMETRICS(__int64 a1, __int64 a2, __int64 a3)
{
  __int64 v3; // rax
  __int64 v6; // rcx
  __int64 v7; // r9
  __int64 v8; // rdx
  int v9; // esi
  __int64 v10; // r8
  __int16 *v11; // rax
  int v12; // r14d
  int v13; // r15d
  int v14; // r11d
  int v15; // r12d
  int v16; // ecx
  int v17; // eax
  unsigned int v18; // r11d
  int v19; // eax
  int v20; // eax
  int v21; // eax
  __int64 v22; // rcx
  int v23; // eax
  __int64 v25; // rcx
  int v26; // r9d
  int v27; // r8d
  int v28; // r8d
  int v29; // r9d
  int v30; // eax
  int v31; // eax
  int v32; // r11d
  int v33; // edx
  __int64 v34; // rcx
  __int64 v35; // r8
  int v36; // r11d
  int v37; // edx
  __int64 v38; // rcx
  __int64 v39; // r8
  __int64 v40; // rcx
  unsigned int v41; // r10d
  __int64 v42; // r8
  int v43; // eax
  int v44; // eax
  int v45; // edx
  char v46; // cl
  int v47; // r8d
  int v48; // ecx
  unsigned int v49; // edx
  int v50; // ecx
  int v51; // ecx
  int v52; // eax
  int v53; // edx
  int v54; // edx
  __int64 v55; // rcx
  int v56; // edx
  __int64 v57; // r8
  float v58; // [rsp+20h] [rbp-50h] BYREF
  _DWORD v59[8]; // [rsp+28h] [rbp-48h] BYREF
  _DWORD v60[8]; // [rsp+48h] [rbp-28h] BYREF

  v3 = *(_QWORD *)(a1 + 8);
  v6 = *(unsigned int *)(v3 + 176);
  v7 = *(_QWORD *)(v3 + 64);
  v8 = v7 + v6;
  if ( !(_DWORD)v6 )
    v8 = 0LL;
  v9 = *(_DWORD *)(a1 + 80);
  if ( v9 < 0 )
    v9 = -v9;
  v10 = *(unsigned int *)(*(_QWORD *)(a1 + 192) + 16LL);
  *(_QWORD *)(a3 + 4) = *(_QWORD *)(a1 + 216);
  *(_QWORD *)(a3 + 12) = *(_QWORD *)(a1 + 256);
  *(_DWORD *)(a3 + 64) = *(_DWORD *)(a1 + 168);
  *(_DWORD *)(a3 + 24) = 16 * *(_DWORD *)(a1 + 144);
  *(_DWORD *)(a3 + 28) = 16 * *(_DWORD *)(a1 + 148);
  v11 = *(__int16 **)(a1 + 8);
  v12 = v11[228];
  v13 = v11[229];
  v14 = v11[226];
  v15 = v11[227];
  *(_DWORD *)a3 = 0;
  *(_DWORD *)(a3 + 20) = 0;
  if ( (*(_DWORD *)(a1 + 116) & 0x40) != 0 )
  {
    *(_QWORD *)(a3 + 108) = 0LL;
  }
  else
  {
    *(_DWORD *)(a3 + 108) = *(__int16 *)(*(_QWORD *)(a1 + 8) + 328LL);
    *(_DWORD *)(a3 + 112) = *(__int16 *)(*(_QWORD *)(a1 + 8) + 330LL);
  }
  if ( (*(_DWORD *)(a1 + 40) & 0x4000) != 0 )
  {
    if ( v8 && (*(_DWORD *)(a1 + 116) & 3) != 0 )
      v40 = (unsigned int)(__int16)__ROR2__(*(_WORD *)(v8 + 76), 8);
    else
      v40 = (unsigned int)-(__int16)__ROR2__(*(_WORD *)(v10 + v7 + 38), 8);
    *(_DWORD *)(a3 + 108) -= FixMul(v40, 22272LL, v10);
    *(_DWORD *)(a3 + 112) += FixMul(v41, 22272LL, v42);
  }
  *(_DWORD *)(a3 + 116) = *(unsigned __int16 *)(*(_QWORD *)(a1 + 8) + 324LL);
  if ( (*(_DWORD *)(a1 + 116) & 1) != 0 )
  {
    v16 = *(_DWORD *)(a1 + 96);
    v17 = v14 * v16;
    v18 = 1;
    v19 = ((v17 >> 15) + 1) >> 1;
    if ( !v19 )
    {
      v19 = -1;
      if ( v16 > 0 )
        v19 = 1;
    }
    *(_DWORD *)(a3 + 52) = v19;
    *(_DWORD *)(a3 + 48) = 0;
    v20 = (((v12 * v16) >> 15) + 1) >> 1;
    if ( !v20 )
    {
      v20 = -1;
      if ( v16 > 0 )
        v20 = 1;
    }
    *(_DWORD *)(a3 + 60) = v20;
    *(_DWORD *)(a3 + 56) = 0;
    *(_DWORD *)(a3 + 32) = 0;
    *(_DWORD *)(a3 + 40) = 0;
    *(_DWORD *)(a3 + 44) = -((((v13 * v16) >> 15) + 1) >> 1);
    v21 = v9 * *(_DWORD *)(a3 + 108);
    *(_DWORD *)(a3 + 36) = -((((v15 * v16) >> 15) + 1) >> 1);
    *(_DWORD *)(a3 + 108) = ((v21 >> 15) + 1) >> 1;
    *(_DWORD *)(a3 + 112) = (((v9 * *(_DWORD *)(a3 + 112)) >> 15) + 1) >> 1;
    *(_DWORD *)(a3 + 116) = (((v9 * *(_DWORD *)(a3 + 116)) >> 15) + 1) >> 1;
  }
  else
  {
    *(_DWORD *)(a3 + 20) = 0;
    v59[3] = -v13;
    v59[7] = -v15;
    v59[0] = 0;
    v59[1] = v12;
    v59[2] = 0;
    v59[4] = 0;
    v59[5] = v14;
    v59[6] = 0;
    bFDXform(a1 + 56, (__int64)v60, (__int64)v59);
    v26 = v60[4];
    v27 = (v60[0] >> 3) + 1;
    *(_DWORD *)(a3 + 60) = ((v60[1] >> 3) + 1) >> 1;
    *(_DWORD *)(a3 + 40) = ((v60[2] >> 3) + 1) >> 1;
    v28 = v27 >> 1;
    *(_DWORD *)(a3 + 44) = ((v60[3] >> 3) + 1) >> 1;
    v29 = ((v26 >> 3) + 1) >> 1;
    *(_DWORD *)(a3 + 52) = ((v60[5] >> 3) + 1) >> 1;
    v30 = (v60[6] >> 3) + 1;
    *(_DWORD *)(a3 + 56) = v28;
    *(_DWORD *)(a3 + 32) = v30 >> 1;
    v31 = (v60[7] >> 3) + 1;
    *(_DWORD *)(a3 + 48) = v29;
    *(_DWORD *)(a3 + 36) = v31 >> 1;
    if ( (*(_DWORD *)(a1 + 116) & 2) != 0 && (!v28 || !v29) )
    {
      v25 = *(unsigned int *)(a1 + 92);
      if ( !v28 )
      {
        v43 = 1;
        if ( (int)v25 > 0 )
          v43 = -1;
        *(_DWORD *)(a3 + 56) = v43;
      }
      if ( !v29 )
      {
        v44 = 1;
        if ( (int)v25 > 0 )
          v44 = -1;
        *(_DWORD *)(a3 + 48) = v44;
      }
    }
    v58 = 0.0;
    bFToL(v25, &v58, 0LL);
    v33 = v32 + (SLODWORD(v58) >> 3);
    v58 = 0.0;
    *(_DWORD *)(a3 + 108) = v33 >> 1;
    bFToL(v34, &v58, v35);
    v37 = v36 + (SLODWORD(v58) >> 3);
    v58 = 0.0;
    *(_DWORD *)(a3 + 112) = v37 >> 1;
    bFToL(v38, &v58, v39);
    *(_DWORD *)(a3 + 116) = (int)(v18 + (SLODWORD(v58) >> 3)) >> 1;
  }
  if ( (*(_DWORD *)(a1 + 116) & 0x21) != 0 )
  {
    v58 = (float)*(int *)(a1 + 80) * 0.000015258789;
    *(float *)(a3 + 76) = v58;
    v58 = (float)*(int *)(a1 + 96) * 0.000015258789;
    *(float *)(a3 + 88) = v58;
    if ( ((unsigned __int8)*(_DWORD *)(a1 + 116) & (unsigned __int8)v18) == 0 )
    {
      v58 = (float)-*(_DWORD *)(a1 + 84) * 0.000015258789;
      *(float *)(a3 + 80) = v58;
      v58 = (float)-*(_DWORD *)(a1 + 92) * 0.000015258789;
      *(float *)(a3 + 84) = v58;
    }
  }
  v22 = *(_QWORD *)(a1 + 8);
  if ( (*(_DWORD *)(v22 + 300) & 2) != 0 )
  {
    v45 = *(_DWORD *)(a1 + 124);
    v46 = *(_BYTE *)(v22 + 397);
    v47 = *(_DWORD *)(a1 + 144) + *(_DWORD *)(a1 + 148) - v45;
    if ( v47 < 0 )
      v47 = 0;
    v48 = v46 & 0xF0;
    if ( v48 == 16 )
    {
      v53 = (*(_DWORD *)(a1 + 48) + 18) / 32;
    }
    else
    {
      if ( v48 == 32 )
      {
        v50 = *(_DWORD *)(a1 + 48);
        if ( (int)(v18 + (*(int *)(a1 + 128) >> 15)) >> 1 >= 14 )
        {
          v51 = v50 + 9;
          v52 = 954437177;
        }
        else
        {
          v51 = v50 + 12;
          v52 = 715827883;
        }
        v49 = (int)((unsigned __int64)(v51 * (__int64)v52) >> 32) >> 2;
      }
      else
      {
        v49 = (int)((unsigned __int64)(53876069772LL * v45) >> 32) >> 6;
      }
      v53 = (v49 >> 31) + v49;
    }
    v54 = 16 * (v53 - v47);
    if ( v54 < 0 )
      v54 = 0;
    *(_DWORD *)(a3 + 92) = v54;
  }
  if ( (*(_DWORD *)(a1 + 40) & 0x2000) != 0 )
  {
    v55 = (unsigned int)*(__int16 *)(*(_QWORD *)(a1 + 8) + 430LL);
    if ( ((unsigned __int8)*(_DWORD *)(a1 + 116) & (unsigned __int8)v18) != 0 )
    {
      *(_DWORD *)(a3 + 100) = ((v9 * (int)v55) >> 12) + 16;
      *(_DWORD *)(a3 + 104) = ((v9 * *(__int16 *)(*(_QWORD *)(a1 + 8) + 428LL)) >> 12) + 16;
    }
    else
    {
      v58 = 0.0;
      bFToL(v55, &v58, 0LL);
      v56 = LODWORD(v58) + 16;
      v58 = 0.0;
      *(_DWORD *)(a3 + 100) = v56;
      bFToL((unsigned int)*(__int16 *)(*(_QWORD *)(a1 + 8) + 428LL), &v58, v57);
      *(_DWORD *)(a3 + 104) = LODWORD(v58) + 16;
    }
  }
  if ( (*(_DWORD *)(a1 + 116) & 8) != 0 )
  {
    *(_DWORD *)(a3 + 68) = v18;
    v23 = CJGD(v18, v18, a1);
  }
  else
  {
    *(_DWORD *)(a3 + 68) = *(_DWORD *)(a1 + 164) - *(_DWORD *)(a1 + 160);
    v23 = *(_DWORD *)(a1 + 172);
  }
  *(_DWORD *)(a3 + 72) = v23;
  return 124LL;
}
