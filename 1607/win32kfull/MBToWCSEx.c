/*
 * XREFs of MBToWCSEx @ 0x1C0245FFC
 * Callers:
...
unsigned __int64 __fastcall MBToWCSEx(__int16 a1, const CHAR *a2, signed int BytesInMultiByteString, PWCH *a4, int a5)
{
  __int64 v5; // rax
  ULONG BytesInUnicodeString; // [rsp+40h] [rbp+8h] BYREF

  LOWORD(BytesInUnicodeString) = a1;
  ...
  if ( a5 >= 0 && RtlMultiByteToUnicodeN(*a4, 2 * a5, &BytesInUnicodeString, a2, BytesInMultiByteString) >= 0 )
    return (unsigned __int64)BytesInUnicodeString >> 1;
  else
    return 0LL;
}
