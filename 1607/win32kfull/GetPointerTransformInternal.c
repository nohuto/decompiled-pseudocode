/*
 * XREFs of GetPointerTransformInternal @ 0x1C01D77D0
 * Callers:
 *     NtUserGetPointerInputTransform @ 0x1C0214420 (NtUserGetPointerInputTransform.c)
 * Callees:
 *     UserSetLastError @ 0x1C00A6F5C (UserSetLastError.c)
 *     ?GetPointerDataQPCTimeList@@YAH_KIPEA_KPEAH@Z @ 0x1C01EF5E8 (-GetPointerDataQPCTimeList@@YAH_KIPEA_KPEAH@Z.c)
 *     ?GetThreadPointerData@@YA_KPEAUtagTHREADINPUTPOINTERLIST@@GPEAKPEAHPEAPEAUHWND__@@@Z @ 0x1C01EFFF4 (-GetThreadPointerData@@YA_KPEAUtagTHREADINPUTPOINTERLIST@@GPEAKPEAHPEAPEAUHWND__@@@Z.c)
 *     ?GetTransformList@InputTransform@@YAHPEAUtagWND@@IPEA_KPEAUtagINPUT_TRANSFORM@@D@Z @ 0x1C022414C (-GetTransformList@InputTransform@@YAHPEAUtagWND@@IPEA_KPEAUtagINPUT_TRANSFORM@@D@Z.c)
 */

__int64 __fastcall GetPointerTransformInternal(__int64 a1, unsigned __int16 a2, unsigned int a3, unsigned __int64 *a4)
{
  __int64 v4; // rbp
  unsigned __int64 ThreadPointerData; // rdi
  __int64 v7; // rcx
  __int64 v9; // rax
  InputTransform *v10; // rsi
  unsigned __int64 *v11; // rbx
  unsigned int PointerDataQPCTimeList; // edi
  __int64 v13; // rcx
  __int64 v14; // rdx
  __int64 v15; // r8
  struct tagINPUT_TRANSFORM *v16; // [rsp+20h] [rbp-38h]
  char v17; // [rsp+28h] [rbp-30h]
  unsigned int v18[2]; // [rsp+30h] [rbp-28h] BYREF
  HWND v19; // [rsp+60h] [rbp+8h] BYREF

  v4 = a3;
  ThreadPointerData = GetThreadPointerData((struct tagTHREADINPUTPOINTERLIST *)(a1 + 968), a2, 0LL, 0LL, &v19);
  if ( !ThreadPointerData || (v9 = ValidateHwnd(v19), (v10 = (InputTransform *)v9) == 0LL) )
  {
    v7 = 87LL;
LABEL_3:
    UserSetLastError(v7);
    return 0LL;
  }
  if ( !*(_QWORD *)(v9 + 336) )
  {
    v7 = 232LL;
    goto LABEL_3;
  }
  if ( (_DWORD)v4 == 1 )
  {
    v11 = (unsigned __int64 *)v18;
  }
  else
  {
    v11 = (unsigned __int64 *)Win32AllocPoolZInit(8 * v4, 2020176725LL);
    if ( !v11 )
    {
      v7 = 8LL;
      goto LABEL_3;
    }
  }
  LODWORD(v19) = 1;
  PointerDataQPCTimeList = GetPointerDataQPCTimeList(ThreadPointerData, v4, v11, (int *)&v19);
  if ( !PointerDataQPCTimeList )
  {
    if ( !(_DWORD)v19 )
    {
      v13 = 232LL;
LABEL_16:
      UserSetLastError(v13);
      goto LABEL_17;
    }
LABEL_15:
    v13 = 87LL;
    goto LABEL_16;
  }
  PointerDataQPCTimeList = InputTransform::GetTransformList(
                             v10,
                             (struct tagWND *)(unsigned int)v4,
                             (unsigned int)v11,
                             a4,
                             v16,
                             v17);
  if ( !PointerDataQPCTimeList )
    goto LABEL_15;
LABEL_17:
  if ( v11 != (unsigned __int64 *)v18 )
    Win32FreePool(v11, v14, v15);
  return PointerDataQPCTimeList;
}
