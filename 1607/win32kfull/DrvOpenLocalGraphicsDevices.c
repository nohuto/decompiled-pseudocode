/*
 * XREFs of DrvOpenLocalGraphicsDevices @ 0x1C0154408
 * Callers:
 *     xxxRemoteReconnect @ 0x1C01537F0 (xxxRemoteReconnect.c)
 * Callees:
 *     memset @ 0x1C015A380 (memset.c)
 */

__int64 DrvOpenLocalGraphicsDevices()
{
  __int64 v0; // rdi
  int v1; // ebp
  struct tagGRAPHICS_DEVICE *v2; // rbx
  PVOID *v3; // rsi
  __int64 v4; // rcx
  int v5; // eax
  NTSTATUS DeviceObjectPointer; // eax
  __int64 v8; // rax
  struct _UNICODE_STRING DestinationString; // [rsp+40h] [rbp-68h] BYREF
  _QWORD v10[6]; // [rsp+50h] [rbp-58h] BYREF
  char v11; // [rsp+B0h] [rbp+8h] BYREF

  LODWORD(v0) = 0;
  v1 = 0;
  if ( !(unsigned int)DrvUpdateGraphicsDeviceList(1LL, 1LL) )
    return 3221225473LL;
  v2 = gpLocalGraphicsDeviceList;
  if ( !gpLocalGraphicsDeviceList )
    return 0LL;
  do
  {
    v3 = (PVOID *)((char *)v2 + 240);
    if ( !*((_QWORD *)v2 + 30) )
    {
      RtlInitUnicodeString(&DestinationString, (PCWSTR)v2);
      DeviceObjectPointer = IoGetDeviceObjectPointer(
                              &DestinationString,
                              0,
                              (PFILE_OBJECT *)v2 + 30,
                              (PDEVICE_OBJECT *)v2 + 17);
      v0 = DeviceObjectPointer;
      if ( DeviceObjectPointer < 0 )
        goto LABEL_12;
      if ( !(unsigned int)bSetDeviceSessionUsage(v2, 1LL) )
      {
        ObfDereferenceObject(*v3);
        *((_QWORD *)v2 + 17) = 0LL;
        LODWORD(v0) = -1073741790;
        *v3 = 0LL;
        v8 = WdLogNewEntry5_WdEvent();
        goto LABEL_13;
      }
    }
    v1 = 1;
    if ( (*((_DWORD *)v2 + 40) & 8) == 0 )
    {
      if ( *v3 )
      {
        memset(&v10[1], 0, 0x20uLL);
        v4 = *((_QWORD *)v2 + 17);
        v10[1] = VideoPortCallout;
        v10[0] = v2;
        v5 = GreDeviceIoControlEx(v4, 2301983LL, v10, 40LL, v10, 40, &v11, 1);
        v0 = v5;
        if ( v5 < 0 )
        {
LABEL_12:
          v8 = WdLogNewEntry5_WdEvent();
          *(_QWORD *)(v8 + 24) = v0;
LABEL_13:
          WdLogEvent5_WdEvent(v8);
        }
      }
    }
    v2 = (struct tagGRAPHICS_DEVICE *)*((_QWORD *)v2 + 16);
  }
  while ( v2 );
  if ( v1 )
    LODWORD(v0) = 0;
  return (unsigned int)v0;
}
