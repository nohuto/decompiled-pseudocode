/*
 * XREFs of NtUserDrawCaption @ 0x1C0211CF0
 * Callers:
 *     <none>
 * Callees:
 *     UserSetLastError @ 0x1C00A6F5C (UserSetLastError.c)
 *     __security_check_cookie @ 0x1C0158CD0 (__security_check_cookie.c)
 *     xxxDrawCaptionTemp @ 0x1C022FFA0 (xxxDrawCaptionTemp.c)
 */

__int64 __fastcall NtUserDrawCaption(__int64 a1, HDC a2, __int128 *a3, unsigned int a4)
{
  __int64 v8; // rax
  __int64 v9; // rdx
  __int64 v10; // rcx
  unsigned int v11; // ebx
  __int64 v12; // rdx
  __int64 v13; // rcx
  _QWORD v15[3]; // [rsp+58h] [rbp-60h] BYREF
  __int128 v16; // [rsp+70h] [rbp-48h]

  EnterCrit(0LL, 1LL);
  v8 = ValidateHwnd(a1);
  v11 = 0;
  if ( v8 )
  {
    v15[0] = *(_QWORD *)(gptiCurrent + 368LL);
    *(_QWORD *)(gptiCurrent + 368LL) = v15;
    v15[1] = v8;
    ++*(_DWORD *)(v8 + 8);
    if ( (unsigned __int64)a3 >= W32UserProbeAddress )
      a3 = (__int128 *)W32UserProbeAddress;
    v16 = *a3;
    v11 = xxxDrawCaptionTemp((struct tagWND *)v8, a2, 0LL, 0LL, a4);
    ThreadUnlock1(v13, v12);
  }
  UserSessionSwitchLeaveCrit(v10, v9);
  return v11;
}
