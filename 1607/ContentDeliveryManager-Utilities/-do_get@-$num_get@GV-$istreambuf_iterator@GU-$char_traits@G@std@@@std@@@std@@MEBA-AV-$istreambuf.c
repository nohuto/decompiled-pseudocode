/*
 * XREFs of ?do_get@?$num_get@GV?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@@std@@MEBA?AV?$istreambuf_iterator@GU?$char_traits@G@std@@@2@V32@0AEAVios_base@2@AEAHAEAPEAX@Z @ 0x1800264C0
 * Callers:
 *     <none>
 * Callees:
 *     ?getloc@ios_base@std@@QEBA?AVlocale@2@XZ @ 0x18001C348 (-getloc@ios_base@std@@QEBA-AVlocale@2@XZ.c)
 *     ?_Getifld@?$num_get@GV?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@@std@@AEBAHPEADAEAV?$istreambuf_iterator@GU?$char_traits@G@std@@@2@1HAEBVlocale@2@@Z @ 0x180028228 (-_Getifld@-$num_get@GV-$istreambuf_iterator@GU-$char_traits@G@std@@@std@@@std@@AEBAHPEADAEAV-$is.c)
 *     ?equal@?$istreambuf_iterator@GU?$char_traits@G@std@@@std@@QEBA_NAEBV12@@Z @ 0x18002A91C (-equal@-$istreambuf_iterator@GU-$char_traits@G@std@@@std@@QEBA_NAEBV12@@Z.c)
 *     _Stoullx @ 0x180054A44 (_Stoullx.c)
 *     __security_check_cookie @ 0x1800589B0 (__security_check_cookie.c)
 *     _guard_dispatch_icall_nop @ 0x1800597C0 (_guard_dispatch_icall_nop.c)
 */

// Hidden C++ exception states: #wind=1
_OWORD *__fastcall std::num_get<unsigned short,std::istreambuf_iterator<unsigned short>>::do_get(
        int a1,
        _OWORD *a2,
        _OWORD *a3,
        __int64 a4,
        __int64 a5,
        _DWORD *a6,
        __int64 *a7)
{
  _QWORD *v11; // rax
  unsigned int v12; // ebx
  void (__fastcall ***v13)(_QWORD, __int64); // rax
  __int64 v14; // rbx
  int v16; // [rsp+30h] [rbp-88h] BYREF
  __int64 v17; // [rsp+38h] [rbp-80h] BYREF
  _QWORD v18[2]; // [rsp+40h] [rbp-78h] BYREF
  _BYTE v19[32]; // [rsp+50h] [rbp-68h] BYREF

  v18[1] = -2LL;
  v16 = 0;
  v11 = std::ios_base::getloc(a5, &v17);
  v12 = std::num_get<unsigned short,std::istreambuf_iterator<unsigned short>>::_Getifld(
          a1,
          (unsigned int)v19,
          (_DWORD)a3,
          a4,
          2048,
          (__int64)v11);
  if ( v17 )
  {
    v13 = (void (__fastcall ***)(_QWORD, __int64))(*(__int64 (__fastcall **)(__int64))(*(_QWORD *)v17 + 16LL))(v17);
    if ( v13 )
      (**v13)(v13, 1LL);
  }
  v14 = Stoullx(v19, v18, v12, &v16);
  if ( (unsigned __int8)std::istreambuf_iterator<unsigned short>::equal(a3, a4) )
    *a6 |= 1u;
  if ( (_BYTE *)v18[0] == v19 || v16 )
    *a6 |= 2u;
  else
    *a7 = v14;
  *a2 = *a3;
  return a2;
}
