/*
 * XREFs of ?PutBackReservedRequest@FxIoQueue@@AEAAXPEAVFxRequest@@@Z @ 0x1C0091864
 * Callers:
...
void __fastcall FxIoQueue::PutBackReservedRequest(FxIoQueue *this, FxRequest *ReservedRequest)
{
  ...
  v6 = v4;
  Blink = p_m_ForwardProgressList->Blink;
  if ( p_m_ForwardProgressList->Flink->Blink != p_m_ForwardProgressList || Blink->Flink != p_m_ForwardProgressList )
    __fastfail(3u);
  Blink->Flink = Flink;
  Flink->Blink = Blink;
  ...
  if ( v9->Flink != p_m_ReservedRequestList )
    __fastfail(3u);
