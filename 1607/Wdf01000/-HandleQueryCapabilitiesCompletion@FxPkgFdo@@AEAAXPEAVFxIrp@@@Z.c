/*
 * XREFs of ?HandleQueryCapabilitiesCompletion@FxPkgFdo@@AEAAXPEAVFxIrp@@@Z @ 0x1C002A030
 * Callers:
...
void __fastcall FxPkgFdo::HandleQueryCapabilitiesCompletion(FxPkgFdo *this, FxIrp *Irp, unsigned __int8 a3)
{
  ...
  int (__fastcall *m_Method)(WDFDEVICE__ *, WDFIORESREQLIST__ *); // rcx
  int (__fastcall *v27)(WDFDEVICE__ *, WDFIORESREQLIST__ *); // rcx
  ...
  if ( SLOBYTE(this->m_ObjectFlags) < 0
    && (m_Method = this[-1].m_DeviceFilterRemoveResourceRequirements.m_Method) != 0LL )
  {
  ...
  if ( SLOBYTE(this->m_ObjectFlags) < 0 && (v27 = this[-1].m_DeviceFilterRemoveResourceRequirements.m_Method) != 0LL )
    FxVerifierLock::Unlock((FxVerifierLock *)v27, v7, v8);
  else
