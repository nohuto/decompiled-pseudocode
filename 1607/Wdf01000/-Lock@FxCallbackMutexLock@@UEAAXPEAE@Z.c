/*
 * XREFs of ?Lock@FxCallbackMutexLock@@UEAAXPEAE@Z @ 0x1C001D040
 * Callers:
 *     <none>
 * Callees:
 *     WPP_IFR_SF_q @ 0x1C001CF1C (WPP_IFR_SF_q.c)
 *     ?Lock@FxVerifierLock@@QEAAXPEAEE@Z @ 0x1C003B900 (-Lock@FxVerifierLock@@QEAAXPEAEE@Z.c)
 *     ?FxVerifierBugCheckWorker@@YAXPEAU_FX_DRIVER_GLOBALS@@W4_WDF_BUGCHECK_CODES@@_K2@Z @ 0x1C007C6D8 (-FxVerifierBugCheckWorker@@YAXPEAU_FX_DRIVER_GLOBALS@@W4_WDF_BUGCHECK_CODES@@_K2@Z.c)
 */

void __fastcall FxCallbackMutexLock::Lock(FxCallbackMutexLock *this, unsigned __int8 *PreviousIrql, unsigned __int8 a3)
{
  struct _KTHREAD *CurrentThread; // rbx
  FxVerifierLock *m_Verifier; // rcx
  _FX_DRIVER_GLOBALS *m_Globals; // rbx

  CurrentThread = KeGetCurrentThread();
  if ( this->m_OwnerThread == CurrentThread )
  {
    m_Globals = this->m_Globals;
    ++this->m_RecursionCount;
    WPP_IFR_SF_q(m_Globals, 1u, 0x12u, 0xAu, WPP_FxCallbackMutexLock_hpp_Traceguids, this);
    FxVerifierBugCheckWorker(m_Globals, WDF_RECURSIVE_LOCK, (unsigned __int64)this, 0LL);
  }
  m_Verifier = this->m_Verifier;
  if ( m_Verifier )
  {
    FxVerifierLock::Lock(m_Verifier, PreviousIrql, a3);
  }
  else
  {
    KeEnterCriticalRegion();
    ExAcquireFastMutexUnsafe(&this->m_Lock.m_Lock);
  }
  this->m_OwnerThread = CurrentThread;
}
