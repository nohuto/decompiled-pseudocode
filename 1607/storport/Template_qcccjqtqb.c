/*
 * XREFs of Template_qcccjqtqb @ 0x1C00425D8
 * Callers:
 *     StorpTelemetrySendUnitSmartAttributes @ 0x1C00415F8 (StorpTelemetrySendUnitSmartAttributes.c)
 * Callees:
 *     __security_check_cookie @ 0x1C001A7E0 (__security_check_cookie.c)
 */

NTSTATUS __fastcall Template_qcccjqtqb(
        __int64 a1,
        __int64 a2,
        __int64 a3,
        int a4,
        char a5,
        char a6,
        char a7,
        __int64 a8,
        char a9,
        char a10,
        __int64 a11,
        __int64 a12)
{
  int v13; // [rsp+30h] [rbp-99h] BYREF
  struct _EVENT_DATA_DESCRIPTOR UserData; // [rsp+40h] [rbp-89h] BYREF
  char *v15; // [rsp+50h] [rbp-79h]
  __int64 v16; // [rsp+58h] [rbp-71h]
  char *v17; // [rsp+60h] [rbp-69h]
  __int64 v18; // [rsp+68h] [rbp-61h]
  char *v19; // [rsp+70h] [rbp-59h]
  __int64 v20; // [rsp+78h] [rbp-51h]
  __int64 v21; // [rsp+80h] [rbp-49h]
  __int64 v22; // [rsp+88h] [rbp-41h]
  char *v23; // [rsp+90h] [rbp-39h]
  __int64 v24; // [rsp+98h] [rbp-31h]
  char *v25; // [rsp+A0h] [rbp-29h]
  __int64 v26; // [rsp+A8h] [rbp-21h]
  int *v27; // [rsp+B0h] [rbp-19h]
  __int64 v28; // [rsp+B8h] [rbp-11h]
  __int64 v29; // [rsp+C0h] [rbp-9h]
  __int64 v30; // [rsp+C8h] [rbp-1h]
  int v31; // [rsp+108h] [rbp+3Fh] BYREF

  v31 = a4;
  UserData.Ptr = (unsigned __int64)&v31;
  *(_QWORD *)&UserData.Size = 4LL;
  v13 = 512;
  v15 = &a5;
  v16 = 1LL;
  v17 = &a6;
  v18 = 1LL;
  v19 = &a7;
  v21 = a8;
  v23 = &a9;
  v25 = &a10;
  v27 = &v13;
  v29 = a12;
  v20 = 1LL;
  v22 = 16LL;
  v24 = 4LL;
  v26 = 4LL;
  v28 = 4LL;
  v30 = 512LL;
  return EtwWrite(Microsoft_Windows_StorPortHandle, &EventUnitPeriodicSmartStatistics, 0LL, 9u, &UserData);
}
