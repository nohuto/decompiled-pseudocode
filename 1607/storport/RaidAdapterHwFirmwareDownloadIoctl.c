/*
 * XREFs of RaidAdapterHwFirmwareDownloadIoctl @ 0x1C0027158
 * Callers:
 *     RaidAdapterDeviceControlIrp @ 0x1C00020D8 (RaidAdapterDeviceControlIrp.c)
 * Callees:
 *     RaBuildHwFirmwareGetInfoBufferForMiniport @ 0x1C00174C8 (RaBuildHwFirmwareGetInfoBufferForMiniport.c)
 *     RaidAdapterHwFirmwareGetInfoFromMiniport @ 0x1C0027290 (RaidAdapterHwFirmwareGetInfoFromMiniport.c)
 *     RaidAdapterSendSrbIoControlSynchronously @ 0x1C00289EC (RaidAdapterSendSrbIoControlSynchronously.c)
 *     RaBuildHwFirmwareDownloadBufferForMiniport @ 0x1C0033960 (RaBuildHwFirmwareDownloadBufferForMiniport.c)
 */

__int64 __fastcall RaidAdapterHwFirmwareDownloadIoctl(__int64 a1, IRP *a2)
{
  PVOID v2; // rdi
  signed int InfoFromMiniport; // ebx
  char v7; // [rsp+28h] [rbp-18h]
  unsigned int v8; // [rsp+60h] [rbp+20h] BYREF
  PVOID P; // [rsp+68h] [rbp+28h] BYREF

  v2 = 0LL;
  v8 = 0;
  P = 0LL;
  if ( !*(_QWORD *)(a1 + 528) || !*(_QWORD *)(a1 + 536) )
  {
    InfoFromMiniport = -1073741823;
    goto LABEL_13;
  }
  if ( a2->Tail.Overlay.CurrentStackLocation->Parameters.Create.Options < 0x28 )
  {
    InfoFromMiniport = -1073741820;
    goto LABEL_15;
  }
  if ( *(_DWORD *)(a1 + 5456) )
    goto LABEL_10;
  RaBuildHwFirmwareGetInfoBufferForMiniport(*(_QWORD *)(a1 + 8), (__int64)a2, &P, &v8);
  v2 = P;
  if ( !P )
  {
LABEL_7:
    InfoFromMiniport = -1073741801;
    goto LABEL_15;
  }
  InfoFromMiniport = RaidAdapterHwFirmwareGetInfoFromMiniport(a1, a2, P, v8);
  if ( InfoFromMiniport >= 0 )
  {
    ExFreePoolWithTag(v2, 0x72536152u);
    P = 0LL;
    v8 = 0;
LABEL_10:
    RaBuildHwFirmwareDownloadBufferForMiniport(*(_QWORD *)(a1 + 8), a2, &P, &v8);
    v2 = P;
    if ( !P )
      goto LABEL_7;
    v7 = 0;
    InfoFromMiniport = RaidAdapterSendSrbIoControlSynchronously(
                         a1,
                         a2,
                         P,
                         v8,
                         PortSrbTranslateFirmwareIoctlStatusToNtStatus,
                         v7,
                         128);
  }
LABEL_13:
  if ( v2 )
    ExFreePoolWithTag(v2, 0x72536152u);
LABEL_15:
  a2->IoStatus.Information = 0LL;
  return RaidCompleteRequestEx(a2, 0, InfoFromMiniport);
}
