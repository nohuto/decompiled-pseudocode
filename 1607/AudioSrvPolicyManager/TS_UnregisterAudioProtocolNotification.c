/*
 * XREFs of TS_UnregisterAudioProtocolNotification @ 0x18001E650
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall TS_UnregisterAudioProtocolNotification(void *a1, struct AUDIOPROTOCOLNOTIFY **a2)
{
  ...
  if ( !*a2 )
    return 87LL;
  result = RpcClientProcessSessionId(a1, &v6, &SessionId);
  if ( !(_DWORD)result )
  {
    result = TsSessionIdDeleteNotify(SessionId, v2);
    if ( !(_DWORD)result )
      *a2 = 0LL;
  }
  return result;
