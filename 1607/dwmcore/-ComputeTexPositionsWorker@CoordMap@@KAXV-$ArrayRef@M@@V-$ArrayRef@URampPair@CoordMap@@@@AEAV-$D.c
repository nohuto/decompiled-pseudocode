/*
 * XREFs of ?ComputeTexPositionsWorker@CoordMap@@KAXV?$ArrayRef@M@@V?$ArrayRef@URampPair@CoordMap@@@@AEAV?$DynArray@M$0A@@@M@Z @ 0x180135D64
 * Callers:
    ...
    if ( *((int *)a1 + 2) > 0 )
    {
      ...
      do
      {
        ...
        if ( v10 < *((_DWORD *)a2 + 2) )
        {
          ...
          if ( !v11
            || a4 >= (float)COERCE_DOUBLE(COERCE_UNSIGNED_INT64((float)(*(float *)(v13 + 4 * v11)
                                                                      - *(float *)(v13 + 4 * v11 - 4))) & _xmm) )
          {
            for ( i = (float *)(v15 + 12 * (v12 + 1)); ; i += 3 )
            {
