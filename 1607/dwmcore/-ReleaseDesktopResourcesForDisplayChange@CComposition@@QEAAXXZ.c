/*
 * XREFs of ?ReleaseDesktopResourcesForDisplayChange@CComposition@@QEAAXXZ @ 0x180116F3C
 * Callers:
 *     ?UpdateDisplaySet@CComposition@@IEAAJPEAJ@Z @ 0x18003CC2C (-UpdateDisplaySet@CComposition@@IEAAJPEAJ@Z.c)
 * Callees:
 *     _guard_dispatch_icall_nop @ 0x1800BF6F0 (_guard_dispatch_icall_nop.c)
 */

void __fastcall CComposition::ReleaseDesktopResourcesForDisplayChange(CComposition *this)
{
  __int64 i; // rbx
  __int64 v3; // rcx

  for ( i = 0LL; (unsigned int)i < *((_DWORD *)this + 138); i = (unsigned int)(i + 1) )
  {
    v3 = *(_QWORD *)(*((_QWORD *)this + 66) + 8 * i);
    (*(void (__fastcall **)(__int64))(*(_QWORD *)v3 + 136LL))(v3);
  }
}
