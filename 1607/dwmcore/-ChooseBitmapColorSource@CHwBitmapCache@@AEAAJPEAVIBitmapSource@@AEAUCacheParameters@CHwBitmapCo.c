/*
 * XREFs of ?ChooseBitmapColorSource@CHwBitmapCache@@AEAAJPEAVIBitmapSource@@AEAUCacheParameters@CHwBitmapColorSource@@AEBUCacheContextParameters@4@AEAPEAV4@3@Z @ 0x1800817E0
 * Callers:
 *     ?GetBitmapColorSource@CHwBitmapCache@@SAJPEAVCD3DDeviceLevel1@@PEAVIBitmapSource@@AEAUCacheParameters@CHwBitmapColorSource@@AEBUCacheContextParameters@5@PEAV1@AEAPEAV5@5@Z @ 0x1800A44E4 (-GetBitmapColorSource@CHwBitmapCache@@SAJPEAVCD3DDeviceLevel1@@PEAVIBitmapSource@@AEAUCacheParam.c)
 * Callees:
 *     ?MilInstrumentationCheckHR@@YAXKQEBJIJI@Z @ 0x180047444 (-MilInstrumentationCheckHR@@YAXKQEBJIJI@Z.c)
 *     ?Release@CMILRefCountBase@@UEAAKXZ @ 0x180079520 (-Release@CMILRefCountBase@@UEAAKXZ.c)
 *     ??_GFormatCacheEntry@CHwBitmapCache@@QEAAPEAXI@Z @ 0x1800A28E0 (--_GFormatCacheEntry@CHwBitmapCache@@QEAAPEAXI@Z.c)
 *     ?GetSetBitmapColorSource@FormatCacheEntry@CHwBitmapCache@@QEAAXAEAUCacheParameters@CHwBitmapColorSource@@AEAPEAV4@PEAPEAV4@@Z @ 0x1800A2914 (-GetSetBitmapColorSource@FormatCacheEntry@CHwBitmapCache@@QEAAXAEAUCacheParameters@CHwBitmapColo.c)
 *     ??0FormatCacheEntry@CHwBitmapCache@@QEAA@XZ @ 0x1800A29C4 (--0FormatCacheEntry@CHwBitmapCache@@QEAA@XZ.c)
 *     ?GetSetBitmapColorSource@CacheEntryList@CHwBitmapCache@@QEAAXAEAUCacheParameters@CHwBitmapColorSource@@AEAPEAV4@PEAPEAV4@@Z @ 0x1800A3B84 (-GetSetBitmapColorSource@CacheEntryList@CHwBitmapCache@@QEAAXAEAUCacheParameters@CHwBitmapColorS.c)
 *     ?Create@CHwBitmapColorSource@@SAJPEAVCD3DDeviceLevel1@@PEAVIMILResourceCache@@AEBUCacheParameters@1@_NPEAPEAV1@@Z @ 0x1800B3B48 (-Create@CHwBitmapColorSource@@SAJPEAVCD3DDeviceLevel1@@PEAVIMILResourceCache@@AEBUCacheParameter.c)
 *     _guard_dispatch_icall_nop @ 0x1800BF6F0 (_guard_dispatch_icall_nop.c)
 *     ?AddDeviceBitmapColorSourcesToReusableList@CHwBitmapCache@@AEBAXAEAPEAVCHwBitmapColorSource@@@Z @ 0x18017C0CC (-AddDeviceBitmapColorSourcesToReusableList@CHwBitmapCache@@AEBAXAEAPEAVCHwBitmapColorSource@@@Z.c)
 */

__int64 __fastcall CHwBitmapCache::ChooseBitmapColorSource(
        CHwBitmapCache *this,
        struct IBitmapSource *a2,
        struct CHwBitmapColorSource::CacheParameters *a3,
        const struct CHwBitmapColorSource::CacheContextParameters *a4,
        struct CHwBitmapColorSource **a5,
        struct CHwBitmapColorSource **a6)
{
  unsigned int v6; // r12d
  struct IBitmapSource *v11; // rax
  _DWORD *v12; // rdi
  __int64 v13; // rdx
  CMILRefCountBase *v14; // rcx
  bool v16; // al
  int v17; // eax
  struct CHwBitmapColorSource *v18; // rcx
  CHwBitmapCache::FormatCacheEntry *v19; // rdi
  CHwBitmapCache::FormatCacheEntry *v20; // rax
  __int64 v21; // rax

  v6 = 0;
  *a6 = 0LL;
  v11 = (struct IBitmapSource *)*((_QWORD *)this + 19);
  *a5 = 0LL;
  if ( v11 != a2 )
  {
    if ( v11 )
    {
      v19 = (CHwBitmapCache *)((char *)this + 160);
      CHwBitmapCache::FormatCacheEntry::`scalar deleting destructor'((CHwBitmapCache *)((char *)this + 160), 0);
      if ( v19 )
        CHwBitmapCache::FormatCacheEntry::FormatCacheEntry(v19);
    }
    *((_QWORD *)this + 19) = a2;
  }
  v12 = (_DWORD *)((char *)this + 160);
  if ( *((_DWORD *)this + 40) == *(_DWORD *)a3 )
    goto LABEL_3;
  if ( !*v12 )
  {
    *v12 = *(_DWORD *)a3;
LABEL_3:
    CHwBitmapCache::CacheEntryList::GetSetBitmapColorSource((CHwBitmapCache *)((char *)this + 176), a3, a5, a6);
    goto LABEL_4;
  }
  if ( *((_QWORD *)this + 21)
    || ((v20 = (CHwBitmapCache::FormatCacheEntry *)(*(__int64 (__fastcall **)(WPF::ProcessHeapImpl *, __int64))(*(_QWORD *)WPF::g_pProcessHeap + 8LL))(
                                                     WPF::g_pProcessHeap,
                                                     336LL)) == 0LL
      ? (v21 = 0LL)
      : (v21 = CHwBitmapCache::FormatCacheEntry::FormatCacheEntry(v20)),
        (*((_QWORD *)this + 21) = v21) != 0LL) )
  {
    CHwBitmapCache::FormatCacheEntry::GetSetBitmapColorSource(
      *((CHwBitmapCache::FormatCacheEntry **)this + 21),
      a3,
      a5,
      a6);
  }
LABEL_4:
  if ( *((_QWORD *)this + 62) )
    CHwBitmapCache::AddDeviceBitmapColorSourcesToReusableList(this, a6);
  v13 = 1LL;
  if ( !*a5 )
  {
    v16 = 0;
    if ( *a6 )
      v16 = (*((_BYTE *)*a6 + 248) & 0x20) != 0;
    v17 = CHwBitmapColorSource::Create(
            *((struct CD3DDeviceLevel1 **)this + 17),
            *((struct IMILResourceCache **)this + 18),
            a3,
            v16,
            a5);
    v6 = v17;
    if ( v17 < 0 )
    {
      MilInstrumentationCheckHR(0x14u, 0LL, 0, v17, 0x3AEu);
      return v6;
    }
    CHwBitmapCache::FormatCacheEntry::GetSetBitmapColorSource((CHwBitmapCache *)((char *)this + 160), a3, a5, 0LL);
  }
  v14 = (CMILRefCountBase *)*((_QWORD *)this + 63);
  if ( v14 == *a5 )
  {
    *((_DWORD *)this + 138) = *((_DWORD *)a4 + 10);
  }
  else
  {
    if ( v14 )
      CMILRefCountBase::Release(v14);
    if ( *((_BYTE *)a4 + 12) && (unsigned int)(*((_DWORD *)a4 + 2) - 4) > 1 )
    {
      *((_QWORD *)this + 63) = 0LL;
    }
    else
    {
      v18 = *a5;
      *((_QWORD *)this + 63) = *a5;
      *((_OWORD *)this + 32) = *(_OWORD *)a4;
      *((_OWORD *)this + 33) = *((_OWORD *)a4 + 1);
      *((_OWORD *)this + 34) = *((_OWORD *)a4 + 2);
      *((_OWORD *)this + 35) = *((_OWORD *)a4 + 3);
      *((_OWORD *)this + 36) = *((_OWORD *)a4 + 4);
      *((_QWORD *)this + 74) = *((_QWORD *)a4 + 10);
      (**(void (__fastcall ***)(struct CHwBitmapColorSource *, __int64))v18)(v18, v13);
    }
  }
  return v6;
}
