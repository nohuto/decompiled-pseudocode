/*
 * XREFs of UsbhResetPort @ 0x1C0009B6C
 * Callers:
 *     UsbhDriverResetPort @ 0x1C0004B20 (UsbhDriverResetPort.c)
 *     UsbhReset1Complete @ 0x1C0009900 (UsbhReset1Complete.c)
 *     UsbhPortConnect @ 0x1C0009DC0 (UsbhPortConnect.c)
 *     UsbhReset2Complete @ 0x1C00183B0 (UsbhReset2Complete.c)
 *     UsbhReset1Timeout @ 0x1C00517E0 (UsbhReset1Timeout.c)
 *     UsbhReset2Timeout @ 0x1C0051D20 (UsbhReset2Timeout.c)
 * Callees:
 *     Usbh_CheckPortHwPendingStatus @ 0x1C0009AB8 (Usbh_CheckPortHwPendingStatus.c)
 *     Usbh_UsbdReadFrameCounter @ 0x1C0009B18 (Usbh_UsbdReadFrameCounter.c)
 *     UsbhSetTimer @ 0x1C0009CFC (UsbhSetTimer.c)
 *     Log @ 0x1C00159D0 (Log.c)
 *     UsbhSyncSendCommand @ 0x1C0015A50 (UsbhSyncSendCommand.c)
 *     FdoExt @ 0x1C0015EE0 (FdoExt.c)
 *     Usb_Disconnected @ 0x1C0027D7C (Usb_Disconnected.c)
 *     WPP_RECORDER_SF_d @ 0x1C0040A3C (WPP_RECORDER_SF_d.c)
 *     UsbhException @ 0x1C004F3A0 (UsbhException.c)
 */

__int64 __fastcall UsbhResetPort(__int64 a1, __int64 a2, __int64 a3)
{
  __int64 v6; // rdi
  unsigned int v7; // edx
  KIRQL v8; // al
  KIRQL v9; // bl
  __int64 v10; // rdx
  __int64 v11; // r8
  __int64 v12; // r9
  unsigned int FrameCounter; // eax
  int v15; // edx
  int v16; // [rsp+20h] [rbp-58h]
  int v17; // [rsp+28h] [rbp-50h]
  int v18; // [rsp+48h] [rbp-30h]
  __int16 v19; // [rsp+88h] [rbp+10h] BYREF
  int v20; // [rsp+90h] [rbp+18h] BYREF
  int v21; // [rsp+98h] [rbp+20h] BYREF
  __int16 v22; // [rsp+9Ch] [rbp+24h]
  __int16 v23; // [rsp+9Eh] [rbp+26h]

  v20 = 0;
  Log(a1, 4, 1919242324, a2, 0LL);
  if ( LOWORD(WPP_GLOBAL_Control->DeviceType) )
    WPP_RECORDER_SF_d(
      WPP_GLOBAL_Control->DeviceExtension,
      0,
      1,
      26,
      (__int64)&WPP_aa88946eb7ae3adf49862ed4f4bccb5e_Traceguids,
      *(_WORD *)(a2 + 4));
  v6 = FdoExt(a1);
  v22 = *(_WORD *)(a2 + 4);
  v19 = 0;
  v21 = 262947;
  v23 = 0;
  v7 = UsbhSyncSendCommand(a1, (unsigned int)&v21, 0, (unsigned int)&v19, v16, (__int64)&v20);
  if ( (v7 & 0xC0000000) == 0xC0000000 && !(unsigned __int8)Usb_Disconnected(v7) )
  {
    LOBYTE(v18) = 0;
    UsbhException(a1, *(unsigned __int16 *)(a2 + 4), 19, 0, 0, v15, v20, usbfile_bus_c, 2486, v18);
  }
  Log(a1, 4, 1919242360, *(unsigned __int16 *)(a2 + 4), *(int *)(a2 + 696));
  Usbh_CheckPortHwPendingStatus(a1, a2);
  v8 = KeAcquireSpinLockRaiseToDpc((PKSPIN_LOCK)(v6 + 4944));
  *(_DWORD *)(a2 + 696) = 1;
  v9 = v8;
  FrameCounter = (unsigned int)Usbh_UsbdReadFrameCounter(a1, v10, v11, v12);
  *(_DWORD *)(a2 + 852) = FrameCounter;
  *(_DWORD *)(a2 + 848) = FrameCounter;
  *(_DWORD *)(a3 + 4) = FrameCounter;
  Log(a1, 4, 1919242580, a2, a3);
  *(_QWORD *)(a2 + 680) = a3;
  KeResetEvent((PRKEVENT)(a2 + 768));
  KeReleaseSpinLock((PKSPIN_LOCK)(v6 + 4944), v9);
  LOBYTE(v17) = 1;
  return UsbhSetTimer(a1, *((unsigned int *)&WPP_MAIN_CB.DeviceQueue.1 + 1), a2 + 552, a2 + 616, 1380799602, v17);
}
