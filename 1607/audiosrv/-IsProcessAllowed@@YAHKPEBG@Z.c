/*
 * XREFs of ?IsProcessAllowed@@YAHKPEBG@Z @ 0x18000D670
 * Callers:
...
__int64 __fastcall IsProcessAllowed(DWORD dwProcessId, const unsigned __int16 *a2)
{
  ...
  if ( v7 )
  {
    ...
    if ( v8 < 0 )
    {
      AudSrvTraceLoggingErrorHelper("IsProcessAllowed", 0x25Cu, v8);
      goto LABEL_16;
    }
  }
  v5 = 1;
  ...
  if ( v8 >= 0 )
  {
    ...
    if ( v8 >= 0 )
    {
      ...
      if ( pSessionId && v6 && pSessionId != v6 )
      {
        ...
        {
          WPP_SF_dD(*((_QWORD *)WPP_GLOBAL_Control + 2), 10LL, &WPP_1922e64f6368316c9ff2859d8f49dc96_Traceguids);
        }
      }
      else
      {
  ...
  if ( v5 )
  {
    ...
    if ( v9 )
    {
      if ( WPP_GLOBAL_Control != (CAudioSession *)&WPP_GLOBAL_Control
        ...
        && *((_BYTE *)WPP_GLOBAL_Control + 25) >= 2u )
      {
        WPP_SF_D(*((_QWORD *)WPP_GLOBAL_Control + 2), 11LL, &WPP_1922e64f6368316c9ff2859d8f49dc96_Traceguids, v9);
      }
    }
  }
  if ( v8 < 0 )
    AudSrvTraceLoggingErrorHelper("IsProcessAllowed", 0x2AEu, v8);
  if ( v12 )
