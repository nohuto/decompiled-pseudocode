/*
 * XREFs of __scrt_dllmain_crt_thread_attach @ 0x1800360EC
 * Callers:
 *     dllmain_crt_dispatch @ 0x180035BC0 (dllmain_crt_dispatch.c)
 * Callees:
 *     ?_Do_call@?$_Func_impl@V_lambda_e798b809e8d9f69eca3e844d88789cba_@@V?$allocator@H@std@@_NPEAUISaDeviceProxy@@@std@@EEAA_N$$QEAPEAUISaDeviceProxy@@@Z @ 0x180038BD0 (-_Do_call@-$_Func_impl@V_lambda_e798b809e8d9f69eca3e844d88789cba_@@V-$allocator@H@std@@_NPEAUISa.c)
 */

char _scrt_dllmain_crt_thread_attach()
{
  if ( !(unsigned __int8)std::_Func_impl<_lambda_e798b809e8d9f69eca3e844d88789cba_,std::allocator<int>,bool,ISaDeviceProxy *>::_Do_call() )
    return 0;
  if ( !(unsigned __int8)std::_Func_impl<_lambda_e798b809e8d9f69eca3e844d88789cba_,std::allocator<int>,bool,ISaDeviceProxy *>::_Do_call() )
  {
    std::_Func_impl<_lambda_e798b809e8d9f69eca3e844d88789cba_,std::allocator<int>,bool,ISaDeviceProxy *>::_Do_call();
    return 0;
  }
  return 1;
}
