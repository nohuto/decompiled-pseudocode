/*
 * XREFs of VidSchiSignalRegisteredEvent @ 0x1C000DEB0
 * Callers:
...
void __fastcall VidSchiSignalRegisteredEvent(__int64 a1, struct _KEVENT **a2)
{
  ...
  v4 = KfRaiseIrql(2u);
  KeAcquireInStackQueuedSpinLockAtDpcLevel((PKSPIN_LOCK)(a1 + 1872), &LockHandle);
  for ( i = *a2; i != (struct _KEVENT *)a2; i = *(struct _KEVENT **)&i->Header.Lock )
  {
    if ( LODWORD(i->Header.WaitListHead.Blink) == 4 )
    {
      if ( *(_DWORD *)(*(_QWORD *)&i[1].Header.Lock + 4LL * LODWORD(i[1].Header.WaitListHead.Blink) + 996) >= *(_DWORD *)(*(_QWORD *)&i[1].Header.Lock + 204LL) )
        continue;
LABEL_9:
      ++i[5].Header.LockNV;
      KeSetEvent(i + 4, 0, 0);
