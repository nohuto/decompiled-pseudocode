/*
 * XREFs of ?DetachFromParent@CCrossChannelParentVisualMarshaler@DirectComposition@@MEAA_NPEAVCApplicationChannel@2@@Z @ 0x1C007F070
 * Callers:
 *     <none>
 * Callees:
 *     <none>
 */

bool __fastcall DirectComposition::CCrossChannelParentVisualMarshaler::DetachFromParent(
        DirectComposition::CCrossChannelParentVisualMarshaler *this,
        struct DirectComposition::CApplicationChannel *a2)
{
  _QWORD *v3; // rcx
  int v4; // ecx
  bool result; // al

  v3 = (_QWORD *)*((_QWORD *)this + 30);
  if ( v3 )
  {
    *v3 = *((_QWORD *)a2 + 50);
    *((_QWORD *)a2 + 50) = v3;
    *((_QWORD *)this + 30) = 0LL;
  }
  v4 = *((_DWORD *)this + 4);
  *((_QWORD *)this + 20) = 0LL;
  result = (v4 & 0x10) != 0;
  *((_DWORD *)this + 4) = v4 & 0xFFFFFFEF;
  return result;
}
