/*
 * XREFs of NtTokenManagerGetAnalogExclusiveTokenEvent @ 0x1C00F4350
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall NtTokenManagerGetAnalogExclusiveTokenEvent(HANDLE *a1)
{
  ...
  if ( UserIsCurrentProcessDwm(v2) )
  {
    ...
    if ( g_pTokenManager )
      AnalogExclusiveTokenEventInternal = CTokenManager::GetAnalogExclusiveTokenEventInternal(v3, &Handle);
  }
  else
  {
