/*
 * XREFs of ?Create@CTargetVisualMarshaler@DirectComposition@@SAJPEBVCSharedSystemResource@2@PEAPEAV12@@Z @ 0x1C0077B84
 * Callers:
 *     CreateTargetVisualMarshaler @ 0x1C0077B50 (CreateTargetVisualMarshaler.c)
 * Callees:
 *     Win32AllocPoolWithQuotaZInit @ 0x1C003A9C0 (Win32AllocPoolWithQuotaZInit.c)
 *     _guard_dispatch_icall_nop @ 0x1C0089D10 (_guard_dispatch_icall_nop.c)
 *     memset @ 0x1C008A080 (memset.c)
 */

__int64 __fastcall DirectComposition::CTargetVisualMarshaler::Create(
        const struct DirectComposition::CSharedSystemResource *a1,
        struct DirectComposition::CTargetVisualMarshaler **a2)
{
  struct DirectComposition::CTargetVisualMarshaler *v4; // rax
  struct DirectComposition::CTargetVisualMarshaler *v5; // rbx
  int v6; // edi

  v4 = (struct DirectComposition::CTargetVisualMarshaler *)Win32AllocPoolWithQuotaZInit(0x40uLL);
  v5 = v4;
  if ( v4 )
  {
    memset(v4, 0, 0x40uLL);
    *((_DWORD *)v5 + 6) = 0;
    *(_QWORD *)v5 = &DirectComposition::CTargetVisualMarshaler::`vftable';
    *((_DWORD *)v5 + 5) = 1;
  }
  else
  {
    v5 = 0LL;
  }
  if ( v5 )
  {
    v6 = (*(__int64 (__fastcall **)(struct DirectComposition::CTargetVisualMarshaler *, const struct DirectComposition::CSharedSystemResource *))(*(_QWORD *)v5 + 232LL))(
           v5,
           a1);
    if ( v6 < 0 )
    {
      (*(void (__fastcall **)(struct DirectComposition::CTargetVisualMarshaler *, __int64))(*(_QWORD *)v5 + 96LL))(
        v5,
        1LL);
      v5 = 0LL;
    }
  }
  else
  {
    v6 = -1073741801;
  }
  *a2 = v5;
  return (unsigned int)v6;
}
