/*
 * XREFs of DrvBuildDevmodeList @ 0x1C0058F50
 * Callers:
...
int __fastcall DrvBuildDevmodeList(struct tagGRAPHICS_DEVICE *a1, int a2, int a3)
{
  ...
  if ( ((unsigned int)v3 & 0x4000000) == 0 )
  {
    if ( ((unsigned int)v3 & 0x800000) != 0 && *((_DWORD *)a1 + 44) && *((_QWORD *)a1 + 23) )
    {
      if ( a3 && !a2 )
        goto LABEL_14;
      LODWORD(v3) = ((__int64 (__fastcall *)(char *, _QWORD, void **))qword_1C01041E8)(
                      (char *)a1 + 264,
                      *((unsigned int *)a1 + 68),
                      &Src);
      if ( (int)v3 >= 0 )
...
LABEL_40:
  ...
  if ( !j )
    LODWORD(v3) = DrvLogDisplayDriverEvent(2LL);
  if ( HIDWORD(WPP_MAIN_CB.DeviceLock.Header.WaitListHead.Blink) && *((_DWORD *)a1 + 44) )
  {
    do
    {
      v29 = *((_QWORD *)a1 + 23) + v4;
