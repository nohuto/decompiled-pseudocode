/*
 * XREFs of ResCRuntimeGetSegmentDataEx @ 0x1800F72E8
 * Callers:
 *     ResCRuntimeGetResourceDataEx @ 0x180094738 (ResCRuntimeGetResourceDataEx.c)
 * Callees:
 *     RtlSetLastWin32Error @ 0x180048BE0 (RtlSetLastWin32Error.c)
 *     DbgPrintEx @ 0x1800492F0 (DbgPrintEx.c)
 *     __security_check_cookie @ 0x180095840 (__security_check_cookie.c)
 *     ResCDirectoryGetSegmentName @ 0x1800F7D4C (ResCDirectoryGetSegmentName.c)
 *     ResCKeSegmentOpenMapping @ 0x1800F8028 (ResCKeSegmentOpenMapping.c)
 *     ResCSegmentCreateMapping @ 0x1800F8090 (ResCSegmentCreateMapping.c)
 *     ResCReleaseInitMutex @ 0x1800F8988 (ResCReleaseInitMutex.c)
 *     ResCRequestInitMutex @ 0x1800F89E0 (ResCRequestInitMutex.c)
 *     _ResCOpenMapping @ 0x1800F90A4 (_ResCOpenMapping.c)
 *     ResCGetSubIndexedName @ 0x1800FAB50 (ResCGetSubIndexedName.c)
 */

__int64 __fastcall ResCRuntimeGetSegmentDataEx(__int64 a1, __int64 a2, unsigned int a3, int a4, char a5)
{
  int v5; // r10d
  __int64 v6; // r9
  __int64 v7; // r15
  __int64 v9; // rbx
  __int64 v10; // r8
  int v11; // r13d
  __int64 SegmentName; // rax
  __int64 v13; // r12
  __int64 v14; // rcx
  __int64 inited; // rsi
  __int64 v16; // rdx
  __int64 v17; // rcx
  wchar_t Buffer[264]; // [rsp+50h] [rbp-248h] BYREF

  v5 = a4;
  v6 = a3;
  v7 = (unsigned int)a2;
  v9 = 0LL;
  if ( !a1 || !v5 )
  {
    if ( !NtCurrentTeb()->LastErrorValue )
      RtlSetLastWin32Error(0x57u);
    return 0LL;
  }
  v10 = *(_QWORD *)(a1 + 16);
  if ( (unsigned int)a2 >= *(_DWORD *)(*(_QWORD *)(v10 + 24) + 68LL)
    || (a5 & 2) == 0 && *(_DWORD *)(32LL * (unsigned int)a2 + *(_QWORD *)(v10 + 32) + 24) )
  {
    return 0LL;
  }
  if ( !*(_QWORD *)(*(_QWORD *)(a1 + 24) + 8LL * (unsigned int)a2) && (a5 & 0x10) == 0 )
  {
    v11 = (*(unsigned __int8 *)(a1 + 4) >> 2) & 1;
    if ( *(char *)a1 < 0 )
    {
      *(_QWORD *)(*(_QWORD *)(a1 + 24) + 8LL * (unsigned int)a2) = ResCKeSegmentOpenMapping(
                                                                     (unsigned int)a2,
                                                                     a2,
                                                                     v10,
                                                                     v6);
LABEL_18:
      v5 = a4;
      LODWORD(v6) = a3;
      goto LABEL_19;
    }
    SegmentName = ResCDirectoryGetSegmentName(v10, (unsigned int)a2);
    v13 = SegmentName;
    if ( SegmentName )
    {
      *(_QWORD *)(*(_QWORD *)(a1 + 24) + 8 * v7) = ResCOpenMapping(SegmentName);
      if ( !*(_QWORD *)(*(_QWORD *)(a1 + 24) + 8 * v7) )
      {
        v14 = *(_QWORD *)(a1 + 32);
        if ( v14 )
        {
          if ( !(unsigned int)ResCGetSubIndexedName(
                                v14,
                                *(_QWORD *)(*(_QWORD *)(a1 + 16) + 24LL),
                                *(_DWORD *)(*(_QWORD *)(*(_QWORD *)(a1 + 16) + 24LL) + 12LL),
                                v7,
                                Buffer) )
            return 0LL;
          inited = ResCRequestInitMutex(v13);
          if ( !inited )
            return 0LL;
          *(_QWORD *)(*(_QWORD *)(a1 + 24) + 8 * v7) = ResCOpenMapping(v13);
          if ( !*(_QWORD *)(*(_QWORD *)(a1 + 24) + 8 * v7) )
            *(_QWORD *)(*(_QWORD *)(a1 + 24) + 8 * v7) = ResCSegmentCreateMapping(Buffer, v11);
          ResCReleaseInitMutex(inited);
        }
      }
      goto LABEL_18;
    }
    return 0LL;
  }
LABEL_19:
  v16 = *(_QWORD *)(*(_QWORD *)(a1 + 24) + 8 * v7);
  if ( !v16 )
    return 0LL;
  v17 = *(_QWORD *)(v16 + 32);
  if ( *(_QWORD *)v17 != 0x47455343534552LL )
  {
    DbgPrintEx(-1, 0, "*** RESCACHE: Segment %u magic field is corrupt!!! ***\n", v7);
    DbgPrintEx(-1, 0, "*** --------> Follow-up: avgarcia;erik;muidev ***\n");
    return 0LL;
  }
  if ( (unsigned int)(v6 + v5) >= *(_DWORD *)(v17 + 28) )
    return 0LL;
  if ( v17 && (unsigned int)v6 < *(_DWORD *)(v17 + 28) )
    return *(_QWORD *)(v16 + 40) + (unsigned int)v6;
  RtlSetLastWin32Error(0x57u);
  return v9;
}
