/*
 * XREFs of RtlpHeapTrkAllocCacheAligned @ 0x1800E6904
 * Callers:
 *     RtlHeapTrkInitialize @ 0x1800E6560 (RtlHeapTrkInitialize.c)
 * Callees:
 *     RtlFreeHeap @ 0x1800207C0 (RtlFreeHeap.c)
 *     RtlAllocateHeap @ 0x1800255D0 (RtlAllocateHeap.c)
 *     NtQuerySystemInformation @ 0x1800A5780 (NtQuerySystemInformation.c)
 */

char __fastcall RtlpHeapTrkAllocCacheAligned(__int64 *a1, unsigned __int64 *a2, int a3, unsigned __int64 a4)
{
  unsigned int v8; // ebx
  __int64 Heap; // rax
  unsigned __int64 v11; // r8
  unsigned int v12; // ebp
  int v13; // ecx
  unsigned __int64 v14; // r9
  int v15; // edx
  __int64 v16; // rcx
  unsigned __int64 v17; // rdx
  bool v18; // zf
  unsigned __int64 v19; // rax
  unsigned __int64 v20; // r8
  __int64 v21; // r10
  unsigned int v22; // r9d
  __int64 v23; // rdx
  _DWORD v24[6]; // [rsp+20h] [rbp-18h] BYREF

  v8 = 0;
  if ( NtQuerySystemInformation((SYSTEM_INFORMATION_CLASS)58, v24, 4u, 0LL) < 0 )
    return 0;
  if ( !v24[0] )
    return 0;
  Heap = RtlAllocateHeap(qword_1801463C0, 0, 8 * a4);
  *a1 = Heap;
  v11 = Heap;
  if ( !Heap )
    return 0;
  v12 = v24[0] * ((unsigned int)(a3 + v24[0] - 1) / v24[0]);
  v13 = -1073741675;
  v14 = v12 * a4;
  if ( is_mul_ok(v12, a4) )
  {
    v15 = 0;
  }
  else
  {
    v14 = -1LL;
    v15 = -1073741675;
  }
  if ( (v15 & 0xC0000000) == 0xC0000000 )
    goto LABEL_9;
  v17 = v24[0] + v14;
  if ( v17 < v14 )
    v17 = -1LL;
  else
    v13 = 0;
  v18 = (v13 & 0xC0000000) == -1073741824;
  v16 = qword_1801463C0;
  if ( v18 )
    goto LABEL_10;
  v19 = RtlAllocateHeap(qword_1801463C0, 8u, v17);
  *a2 = v19;
  if ( !v19 )
  {
    v11 = *a1;
LABEL_9:
    v16 = qword_1801463C0;
LABEL_10:
    RtlFreeHeap(v16, 0, v11);
    *a1 = 0LL;
    return 0;
  }
  v20 = v19 + v24[0] - v19 % v24[0];
  if ( a4 )
  {
    v21 = 0LL;
    v22 = 0;
    do
    {
      ++v8;
      v23 = v22;
      v22 += v12;
      *(_QWORD *)(*a1 + 8 * v21) = v20 + v23;
      v21 = v8;
    }
    while ( v8 < a4 );
  }
  return 1;
}
