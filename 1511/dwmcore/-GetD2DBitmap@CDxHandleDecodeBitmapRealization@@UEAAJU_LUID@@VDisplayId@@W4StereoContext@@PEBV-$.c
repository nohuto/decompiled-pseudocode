/*
 * XREFs of ?GetD2DBitmap@CDxHandleDecodeBitmapRealization@@UEAAJU_LUID@@VDisplayId@@W4StereoContext@@PEBV?$TMilRect@IUMilRectU@@UNotNeeded@RectUniqueness@@@@_N4PEAPEAUID2D1Bitmap1@@@Z @ 0x18015FA20
 * Callers:
 *     <none>
 * Callees:
    ...
    if ( v12 >= 0 )
    {
      ...
      if ( v13 >= 0 )
      {
        ...
        if ( D2DBitmapFromBitmapSource < 0 )
          MilInstrumentationCheckHR(0x14u, 0LL, 0, D2DBitmapFromBitmapSource, 0x77u);
      }
      else
      {
        MilInstrumentationCheckHR(0x14u, 0LL, 0, v13, 0x71u);
      }
    }
    else
    {
      MilInstrumentationCheckHR(0x14u, 0LL, 0, v12, 0x67u);
    }
    if ( v23 )
