/*
 * XREFs of ?Create@CTopLevelWindow@@SAJQEAUMIL_CHANNEL__@@PEAPEAV1@@Z @ 0x18003C580
 * Callers:
...
__int64 __fastcall CTopLevelWindow::Create(struct MIL_CHANNEL__ *const a1, struct CCanvas ***a2)
{
  void *(__fastcall *v4)(WPF::ProcessHeapImpl *, size_t); // rdi
  CTopLevelWindow *v5; // rax
  struct CCanvas **v6; // rbx
  int v7; // eax
  unsigned int v8; // edi
  ...
  if ( !a2 )
  {
    v8 = -2147024809;
LABEL_15:
    MilInstrumentationCheckHR(0x14u, 0LL, 0LL, v8, 0xFFu);
    return v8;
  }
  ...
  if ( v5 )
    v6 = (struct CCanvas **)CTopLevelWindow::CTopLevelWindow(v5);
  else
    v6 = 0LL;
  if ( !v6 )
  {
  ...
  if ( v7 < 0 )
  {
    MilInstrumentationCheckHR(0x14u, 0LL, 0LL, v7, 0xFFu);
  }
  else
  {
    *a2 = v6;
