/*
 * XREFs of ?_UpdateResourcesForMonitorHelper@CLivePreview@@AEAAJPEBVCTopLevelWindow@@PEAULivePreviewResource@@@Z @ 0x180073728
 * Callers:
 ...
 *     ?CreateRoundRectRgnCoerceInvalid@@YAPEAUHRGN__@@HHHHHH@Z @ 0x1800711D8 (-CreateRoundRectRgnCoerceInvalid@@YAPEAUHRGN__@@HHHHHH@Z.c)
        ...
        struct LivePreviewResource *a3)
{
  ...
  if ( v6 && *(char *)(*((_QWORD *)a2 + 93) + 570LL) >= 0 )
    UnionRect((LPRECT)a3, (const RECT *)a3, &rcDst);
  v7 = *((_QWORD *)a2 + 93);
  if ( (*(_BYTE *)(v7 + 570) & 0x40) == 0 && *(_DWORD *)(v7 + 112) == 1 )
  {
    if ( v6 )
    ...
    if ( IntersectRect(&rcSrc2, &rcDst, (const RECT *)a3 + 7) )
    {
      if ( UnionRect(&rc1, (const RECT *)a3 + 5, &rcSrc2) && !EqualRect(&rc1, (const RECT *)a3 + 5) )
      {
        ...
        if ( !RoundRectRgnCoerceInvalid )
        {
          ...
          MilInstrumentationCheckHR(0x14u, 0LL, 0LL, v5, 0x4D4u);
