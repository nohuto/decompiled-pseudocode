/*
 * XREFs of Template_cjqzr2jx @ 0x1C0032FBC
 * Callers:
 *     UsbSleepStudy_CsStateChange @ 0x1C0033660 (UsbSleepStudy_CsStateChange.c)
 * Callees:
 *     __security_check_cookie @ 0x1C0034B70 (__security_check_cookie.c)
 */

NTSTATUS __fastcall Template_cjqzr2jx(
        __int64 a1,
        __int64 a2,
        __int64 a3,
        char a4,
        __int64 a5,
        int a6,
        __int64 a7,
        __int64 a8,
        char a9)
{
  struct _EVENT_DATA_DESCRIPTOR UserData; // [rsp+30h] [rbp-41h] BYREF
  __int64 v11; // [rsp+40h] [rbp-31h]
  __int64 v12; // [rsp+48h] [rbp-29h]
  int *v13; // [rsp+50h] [rbp-21h]
  __int64 v14; // [rsp+58h] [rbp-19h]
  __int64 v15; // [rsp+60h] [rbp-11h]
  int v16; // [rsp+68h] [rbp-9h]
  int v17; // [rsp+6Ch] [rbp-5h]
  __int64 v18; // [rsp+70h] [rbp-1h]
  __int64 v19; // [rsp+78h] [rbp+7h]
  char *v20; // [rsp+80h] [rbp+Fh]
  __int64 v21; // [rsp+88h] [rbp+17h]
  char v22; // [rsp+C8h] [rbp+57h] BYREF

  v22 = a4;
  UserData.Ptr = (unsigned __int64)&v22;
  v11 = a5;
  v13 = &a6;
  v15 = a7;
  *(_QWORD *)&UserData.Size = 1LL;
  v16 = 2 * a6;
  v18 = a8;
  v20 = &a9;
  v12 = 16LL;
  v14 = 4LL;
  v17 = 0;
  v19 = 16LL;
  v21 = 8LL;
  return EtwWrite(Microsoft_Windows_SleepStudyHandle, &SLEEPSTUDY_EVT_SCENARIO_BLOCKER, 0LL, 6u, &UserData);
}
