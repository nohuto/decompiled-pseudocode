/*
 * XREFs of InitEnergyCountersMsrEx @ 0x1C0018FCC
 * Callers:
 *     InitEnergyCounters @ 0x1C0018E20 (InitEnergyCounters.c)
 * Callees:
 *     __security_check_cookie @ 0x1C0002450 (__security_check_cookie.c)
 *     GetCpuIdInfo @ 0x1C0002980 (GetCpuIdInfo.c)
 *     ActivateFixedFunctionCounters @ 0x1C0002DFC (ActivateFixedFunctionCounters.c)
 */

_DWORD *InitEnergyCountersMsrEx()
{
  _DWORD *result; // rax
  int v1; // r9d
  _QWORD *v2; // r8
  __int64 v3; // r10
  unsigned int *v4; // r9
  unsigned __int64 v5; // rax
  _DWORD v6[4]; // [rsp+20h] [rbp-28h] BYREF

  result = ActivateFixedFunctionCounters();
  if ( !v1 )
  {
    GetCpuIdInfo(0xAu, v6);
    qword_1C000DC10 = (1LL << (v6[3] >> 5)) - 1;
    if ( dword_1C000DC64 )
    {
      v2 = &unk_1C000DC28;
      v3 = (unsigned int)dword_1C000DC64;
      v4 = (unsigned int *)&unk_1C000DC68;
      do
      {
        v5 = __readmsr(*v4++);
        *v2++ = v5;
        --v3;
      }
      while ( v3 );
    }
    EnergyModelContext[0] |= 0x40u;
    qword_1C000E2E0[0] = (__int64)SnapEnergyCounters;
    result = ComputeProcessorEnergyMsrEx;
    qword_1C000E2D8[0] = (__int64)ComputeProcessorEnergyMsrEx;
  }
  return result;
}
