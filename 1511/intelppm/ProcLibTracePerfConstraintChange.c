/*
 * XREFs of ProcLibTracePerfConstraintChange @ 0x1C001A614
 * Callers:
...
char __fastcall ProcLibTracePerfConstraintChange(__int64 a1, int a2, int a3)
{
  ...
  struct _PROCESSOR_NUMBER v6[4]; // [rsp+38h] [rbp-9h] BYREF
  ...
  v18 = a3;
  v17 = a2;
  LOBYTE(ProcessorNumber) = EtwEventEnabled(ProcLibEtwHandle, &PPM_ETW_PERF_CONSTRAINT_CHANGE);
  if ( (_BYTE)ProcessorNumber )
  {
    ...
    if ( ProcessorNumber >= 0 )
    {
      ...
      v12 = 4;
      v15 = 4;
      LOBYTE(ProcessorNumber) = EtwWrite(ProcLibEtwHandle, &PPM_ETW_PERF_CONSTRAINT_CHANGE, 0LL, 4u, &UserData);
    }
  }
