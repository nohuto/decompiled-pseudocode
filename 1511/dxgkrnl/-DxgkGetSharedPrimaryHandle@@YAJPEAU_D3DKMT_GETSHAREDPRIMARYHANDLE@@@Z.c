/*
 * XREFs of ?DxgkGetSharedPrimaryHandle@@YAJPEAU_D3DKMT_GETSHAREDPRIMARYHANDLE@@@Z @ 0x1C012BDB0
 * Callers:
 *     <none>
 * Callees:
 *     ?ReleaseReference@DXGADAPTER@@QEAAXXZ @ 0x1C0003184 (-ReleaseReference@DXGADAPTER@@QEAAXXZ.c)
 *     ??1COREADAPTERACCESS@@QEAA@XZ @ 0x1C0003940 (--1COREADAPTERACCESS@@QEAA@XZ.c)
 *     ??0COREADAPTERACCESS@@QEAA@QEAVDXGADAPTER@@0@Z @ 0x1C00039D0 (--0COREADAPTERACCESS@@QEAA@QEAVDXGADAPTER@@0@Z.c)
 *     ?AcquireShared@COREADAPTERACCESS@@QEAAJXZ @ 0x1C0003A40 (-AcquireShared@COREADAPTERACCESS@@QEAAJXZ.c)
 *     ??0DXGADAPTERBYHANDLE@@QEAA@IPEAVDXGPROCESS@@PEAPEAVDXGADAPTER@@@Z @ 0x1C0003E88 (--0DXGADAPTERBYHANDLE@@QEAA@IPEAVDXGPROCESS@@PEAPEAVDXGADAPTER@@@Z.c)
 *     ??1DXGADAPTERBYHANDLE@@QEAA@XZ @ 0x1C0003F64 (--1DXGADAPTERBYHANDLE@@QEAA@XZ.c)
 *     Template_q @ 0x1C0011A2C (Template_q.c)
 *     ?GetCurrent@DXGPROCESS@@SAPEAV1@XZ @ 0x1C00769E0 (-GetCurrent@DXGPROCESS@@SAPEAV1@XZ.c)
 *     ?DxgkpGetPairingAdapters@@YAJPEAVDXGADAPTER@@IPEAPEAV1@1@Z @ 0x1C008A660 (-DxgkpGetPairingAdapters@@YAJPEAVDXGADAPTER@@IPEAPEAV1@1@Z.c)
 *     ?GetCddPrimaryShareResourceHandle@ADAPTER_DISPLAY@@QEBAII@Z @ 0x1C012C700 (-GetCddPrimaryShareResourceHandle@ADAPTER_DISPLAY@@QEBAII@Z.c)
 */

__int64 __fastcall DxgkGetSharedPrimaryHandle(struct _D3DKMT_GETSHAREDPRIMARYHANDLE *a1, __int64 a2, __int64 a3)
{
  unsigned __int64 v3; // rbp
  struct _D3DKMT_GETSHAREDPRIMARYHANDLE *v5; // rax
  struct _KTHREAD **Current; // rax
  __int64 v7; // rcx
  __int64 v8; // r8
  bool v9; // zf
  __int64 v11; // rdx
  __int64 v12; // rcx
  __int64 v13; // r8
  __int64 v14; // r9
  __int64 v15; // r15
  __int64 v16; // rax
  __int64 v17; // rcx
  int PairingAdapters; // esi
  __int64 v19; // rax
  __int64 v20; // r8
  struct DXGADAPTER *v21; // rsi
  __int64 v22; // rax
  __int64 v23; // rcx
  int v24; // r15d
  __int64 v25; // r8
  __int64 v26; // rax
  __int64 v27; // rsi
  __int64 v28; // rax
  ADAPTER_DISPLAY *v29; // rcx
  __int64 v30; // rax
  unsigned int CddPrimaryShareResourceHandle; // eax
  _DWORD *p_hSharedPrimary; // r8
  __int64 v33; // rcx
  __int64 v34; // r8
  _BYTE v35[96]; // [rsp+C0h] [rbp+C0h] BYREF

  v3 = (unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL;
  if ( (qword_1C00467F0 & 2) != 0 && (Microsoft_Windows_DxgKrnlEnableBits & 0x4000) != 0 )
    Template_q((__int64)a1, &EventProfilerEnter, a3, 2021);
  v5 = a1;
  if ( (unsigned __int64)a1 >= MmUserProbeAddress )
    v5 = (struct _D3DKMT_GETSHAREDPRIMARYHANDLE *)MmUserProbeAddress;
  *(_QWORD *)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 0x18) = *(_QWORD *)&v5->hAdapter;
  *(_DWORD *)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 0x20) = v5->hSharedPrimary;
  Current = (struct _KTHREAD **)DXGPROCESS::GetCurrent();
  if ( !Current )
    goto LABEL_7;
  DXGADAPTERBYHANDLE::DXGADAPTERBYHANDLE(
    (DXGADAPTERBYHANDLE *)((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL),
    *(_DWORD *)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 0x18),
    Current,
    (struct DXGADAPTER **)(v3 + 8));
  v15 = *(_QWORD *)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 8);
  if ( !v15 )
  {
    v16 = WdLogNewEntry5_WdWarning(v12, v11, v13, v14);
    *(_QWORD *)(v16 + 24) = *(unsigned int *)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 0x18);
    *(_QWORD *)(v16 + 32) = -1073741811LL;
    WdLogEvent5_WdWarning(v16);
    DXGADAPTERBYHANDLE::~DXGADAPTERBYHANDLE((DXGADAPTER **)((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL));
LABEL_7:
    v9 = (qword_1C00467F0 & 2) == 0;
LABEL_8:
    if ( !v9 && (Microsoft_Windows_DxgKrnlEnableBits & 0x4000) != 0 )
      Template_q(v7, &EventProfilerExit, v8, 2021);
    return 3221225485LL;
  }
  PairingAdapters = DxgkpGetPairingAdapters(
                      *(struct DXGADAPTER **)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 8),
                      *(_DWORD *)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 0x1C),
                      (struct DXGADAPTER **)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 16),
                      (struct DXGADAPTER **)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 8));
  if ( PairingAdapters >= 0 )
  {
    v21 = *(struct DXGADAPTER **)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 0x10);
    if ( !v21 || !*(_QWORD *)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 8) )
    {
      v22 = WdLogNewEntry5_WdAssertion(v17);
      *(_QWORD *)(v22 + 24) = 6538LL;
      WdLogEvent5_WdAssertion(v22);
    }
    COREADAPTERACCESS::COREADAPTERACCESS(
      (COREADAPTERACCESS *)(v3 + 40),
      v21,
      *(struct DXGADAPTER *const *)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 8));
    DXGADAPTER::ReleaseReference(v21);
    DXGADAPTER::ReleaseReference(*(DXGADAPTER **)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 8));
    v24 = COREADAPTERACCESS::AcquireShared((COREADAPTERACCESS *)(v3 + 40));
    if ( v24 >= 0 )
    {
      if ( !*((_QWORD *)v21 + 249) )
      {
        v26 = WdLogNewEntry5_WdAssertion(v23);
        *(_QWORD *)(v26 + 24) = 6553LL;
        WdLogEvent5_WdAssertion(v26);
      }
      v27 = *(_QWORD *)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 8);
      if ( !*(_QWORD *)(v27 + 1984) )
      {
        v28 = WdLogNewEntry5_WdAssertion(v23);
        *(_QWORD *)(v28 + 24) = 6554LL;
        WdLogEvent5_WdAssertion(v28);
      }
      v29 = *(ADAPTER_DISPLAY **)(v27 + 1984);
      if ( *(_DWORD *)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 0x1C) >= *((_DWORD *)v29 + 26) )
      {
        v30 = WdLogNewEntry5_WdError(v29);
        *(_QWORD *)(v30 + 24) = *(unsigned int *)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 0x1C);
        *(_QWORD *)(v30 + 32) = -1073741811LL;
        WdLogEvent5_WdError(v30);
        COREADAPTERACCESS::~COREADAPTERACCESS((COREADAPTERACCESS *)(v3 + 40));
        DXGADAPTERBYHANDLE::~DXGADAPTERBYHANDLE((DXGADAPTER **)((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL));
        v9 = (qword_1C00467F0 & 2) == 0;
        goto LABEL_8;
      }
      CddPrimaryShareResourceHandle = ADAPTER_DISPLAY::GetCddPrimaryShareResourceHandle(
                                        v29,
                                        *(_DWORD *)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 0x1C));
      p_hSharedPrimary = &a1->hSharedPrimary;
      if ( (unsigned __int64)&a1->hSharedPrimary >= MmUserProbeAddress )
        p_hSharedPrimary = (_DWORD *)MmUserProbeAddress;
      *p_hSharedPrimary = CddPrimaryShareResourceHandle;
      COREADAPTERACCESS::~COREADAPTERACCESS((COREADAPTERACCESS *)(v3 + 40));
      DXGADAPTERBYHANDLE::~DXGADAPTERBYHANDLE((DXGADAPTER **)((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL));
      if ( (qword_1C00467F0 & 2) != 0 && (Microsoft_Windows_DxgKrnlEnableBits & 0x4000) != 0 )
        Template_q(v33, &EventProfilerExit, v34, 2021);
      return 0LL;
    }
    else
    {
      COREADAPTERACCESS::~COREADAPTERACCESS((COREADAPTERACCESS *)(v3 + 40));
      DXGADAPTERBYHANDLE::~DXGADAPTERBYHANDLE((DXGADAPTER **)((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL));
      if ( (qword_1C00467F0 & 2) != 0 && (Microsoft_Windows_DxgKrnlEnableBits & 0x4000) != 0 )
        Template_q(qword_1C00467F0, &EventProfilerExit, v25, 2021);
      return (unsigned int)v24;
    }
  }
  else
  {
    v19 = WdLogNewEntry5_WdError(v17);
    *(_QWORD *)(v19 + 24) = v15;
    *(_QWORD *)(v19 + 32) = *(unsigned int *)(((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL) + 0x1C);
    WdLogEvent5_WdError(v19);
    DXGADAPTERBYHANDLE::~DXGADAPTERBYHANDLE((DXGADAPTER **)((unsigned __int64)v35 & 0xFFFFFFFFFFFFFFC0uLL));
    if ( (qword_1C00467F0 & 2) != 0 && (Microsoft_Windows_DxgKrnlEnableBits & 0x4000) != 0 )
      Template_q(qword_1C00467F0, &EventProfilerExit, v20, 2021);
    return (unsigned int)PairingAdapters;
  }
}
