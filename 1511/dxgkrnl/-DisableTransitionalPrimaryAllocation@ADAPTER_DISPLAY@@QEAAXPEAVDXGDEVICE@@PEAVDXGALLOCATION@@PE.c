/*
 * XREFs of ?DisableTransitionalPrimaryAllocation@ADAPTER_DISPLAY@@QEAAXPEAVDXGDEVICE@@PEAVDXGALLOCATION@@PEAVCOREDEVICEACCESS@@@Z @ 0x1C0129B60
 * Callers:
 *     ?DxgkDestroyAllocationInternal@@YAJPEAVDXGPROCESS@@PEAVDXGDEVICE@@PEAIPEAPEAVDXGALLOCATION@@IAEAPEBIIU_D3DDDICB_DESTROYALLOCATION2FLAGS@@PEBI@Z @ 0x1C0063440 (-DxgkDestroyAllocationInternal@@YAJPEAVDXGPROCESS@@PEAVDXGDEVICE@@PEAIPEAPEAVDXGALLOCATION@@IAEA.c)
 * Callees:
 *     ?IsCoreResourceSharedOwner@DXGADAPTER@@QEBAEXZ @ 0x1C00031F4 (-IsCoreResourceSharedOwner@DXGADAPTER@@QEBAEXZ.c)
 *     ?Release@COREDEVICEACCESS@@QEAAXXZ @ 0x1C0003614 (-Release@COREDEVICEACCESS@@QEAAXXZ.c)
 *     ?AcquireSharedUncheck@COREDEVICEACCESS@@QEAAXXZ @ 0x1C0003914 (-AcquireSharedUncheck@COREDEVICEACCESS@@QEAAXXZ.c)
 *     ?GetVidPnSourceOwner@ADAPTER_DISPLAY@@QEBAPEAVDXGDEVICE@@I@Z @ 0x1C0007E78 (-GetVidPnSourceOwner@ADAPTER_DISPLAY@@QEBAPEAVDXGDEVICE@@I@Z.c)
 *     ?GetDisplayedPrimary@DXGDEVICE@@QEAAPEBVDXGALLOCATION@@I@Z @ 0x1C000D2D0 (-GetDisplayedPrimary@DXGDEVICE@@QEAAPEBVDXGALLOCATION@@I@Z.c)
 *     _guard_dispatch_icall_nop @ 0x1C00120A0 (_guard_dispatch_icall_nop.c)
 *     ?VidMmIsAllocationInPresentQueue@VIDMM_EXPORT@@QEAAEPEAVVIDMM_GLOBAL@@PEAU_VIDMM_MULTI_ALLOC@@PEAI@Z @ 0x1C001BCBC (-VidMmIsAllocationInPresentQueue@VIDMM_EXPORT@@QEAAEPEAVVIDMM_GLOBAL@@PEAU_VIDMM_MULTI_ALLOC@@PE.c)
 */

void __fastcall ADAPTER_DISPLAY::DisableTransitionalPrimaryAllocation(
        DXGADAPTER **this,
        struct DXGDEVICE *a2,
        struct DXGALLOCATION *a3,
        struct COREDEVICEACCESS *a4)
{
  __int64 v8; // rax
  __int64 v9; // rax
  __int64 v10; // rcx
  __int64 v11; // rax
  __int64 v12; // rcx
  __int64 v13; // rax
  __int64 v14; // rcx
  __int64 v15; // rax
  unsigned int v16; // edi
  __int64 v17; // rax
  __int64 v18; // rcx
  __int64 v19; // rax
  __int64 v20; // rcx
  __int64 v21; // rax
  __int64 v22; // rcx
  __int64 v23; // rax

  if ( !a2 )
  {
    v8 = WdLogNewEntry5_WdAssertion(this);
    *(_QWORD *)(v8 + 24) = 12920LL;
    WdLogEvent5_WdAssertion(v8);
  }
  if ( !a3 || !*((_QWORD *)a3 + 6) )
  {
    v9 = WdLogNewEntry5_WdAssertion(this);
    *(_QWORD *)(v9 + 24) = 12921LL;
    WdLogEvent5_WdAssertion(v9);
  }
  v10 = *(unsigned int *)(*((_QWORD *)a3 + 6) + 4LL);
  if ( (v10 & 1) == 0 )
  {
    v11 = WdLogNewEntry5_WdAssertion(v10);
    *(_QWORD *)(v11 + 24) = 12922LL;
    WdLogEvent5_WdAssertion(v11);
  }
  if ( !DXGADAPTER::IsCoreResourceSharedOwner(*(DXGADAPTER **)(*((_QWORD *)a2 + 2) + 16LL)) )
  {
    v13 = WdLogNewEntry5_WdAssertion(v12);
    *(_QWORD *)(v13 + 24) = 12923LL;
    WdLogEvent5_WdAssertion(v13);
  }
  if ( !DXGADAPTER::IsCoreResourceSharedOwner(this[2]) )
  {
    v15 = WdLogNewEntry5_WdAssertion(v14);
    *(_QWORD *)(v15 + 24) = 12924LL;
    WdLogEvent5_WdAssertion(v15);
  }
  v16 = (*(_DWORD *)(*((_QWORD *)a3 + 6) + 4LL) >> 6) & 0xF;
  if ( v16 >= *((_DWORD *)this + 26) )
  {
    v17 = WdLogNewEntry5_WdAssertion(v14);
    *(_QWORD *)(v17 + 24) = 12928LL;
    WdLogEvent5_WdAssertion(v17);
  }
  v18 = *(_QWORD *)(*((_QWORD *)a2 + 2) + 16LL);
  if ( *((_QWORD *)a2 + 354) != v18 )
  {
    v19 = WdLogNewEntry5_WdAssertion(v18);
    *(_QWORD *)(v19 + 24) = 12930LL;
    WdLogEvent5_WdAssertion(v19);
  }
  if ( DXGDEVICE::GetDisplayedPrimary(a2, v16) == a3 )
  {
    v21 = WdLogNewEntry5_WdAssertion(v20);
    *(_QWORD *)(v21 + 24) = 12931LL;
    WdLogEvent5_WdAssertion(v21);
  }
  v22 = *(unsigned int *)(*((_QWORD *)a3 + 6) + 4LL);
  if ( (v22 & 0x10) != 0 )
  {
    v23 = WdLogNewEntry5_WdAssertion(v22);
    *(_QWORD *)(v23 + 24) = 12932LL;
    WdLogEvent5_WdAssertion(v23);
  }
  if ( ADAPTER_DISPLAY::GetVidPnSourceOwner(this, v16) == a2
    && VIDMM_EXPORT::VidMmIsAllocationInPresentQueue(
         *(VIDMM_EXPORT **)(*((_QWORD *)a2 + 2) + 424LL),
         *(struct VIDMM_GLOBAL **)(*((_QWORD *)a2 + 2) + 432LL),
         *((struct _VIDMM_MULTI_ALLOC **)a3 + 3),
         0LL) )
  {
    COREDEVICEACCESS::Release(a4);
    (*(void (__fastcall **)(_QWORD, _QWORD, _QWORD))(*(_QWORD *)(*(_QWORD *)(*((_QWORD *)a2 + 2) + 424LL) + 8LL) + 680LL))(
      *(_QWORD *)(*((_QWORD *)a2 + 2) + 432LL),
      *((_QWORD *)a3 + 3),
      v16);
    COREDEVICEACCESS::AcquireSharedUncheck(a4);
  }
  DXGDEVICE::UnpinTransitionalPrimaryAllocation(a2, a3, v16);
}
