/*
 * XREFs of ?ReleaseCoreSync@DXGADAPTER@@QEAAXW4DXGADAPTERCORESYNC_LEVEL@@@Z @ 0x1C007A060
 * Callers:
 *     ?DXGADAPTER_ReleaseCoreSync@@YAXPEAVDXGADAPTER@@W4DXGADAPTERCORESYNC_LEVEL@@@Z @ 0x1C001D1A0 (-DXGADAPTER_ReleaseCoreSync@@YAXPEAVDXGADAPTER@@W4DXGADAPTERCORESYNC_LEVEL@@@Z.c)
 *     DxgkReleaseAdapterCoreSync @ 0x1C00A9F00 (DxgkReleaseAdapterCoreSync.c)
 *     ?DxgkCaptureReleaseSynchronization@@YAJPEBU_DEVICE_OBJECT@@@Z @ 0x1C0135ED0 (-DxgkCaptureReleaseSynchronization@@YAJPEBU_DEVICE_OBJECT@@@Z.c)
 * Callees:
 *     ?ScheduleAdapterActivityCheck@DXGADAPTER@@QEAAXXZ @ 0x1C0002F20 (-ScheduleAdapterActivityCheck@DXGADAPTER@@QEAAXXZ.c)
 *     ?IsCoreResourceSharedOwner@DXGADAPTER@@QEBAEXZ @ 0x1C00031F4 (-IsCoreResourceSharedOwner@DXGADAPTER@@QEBAEXZ.c)
 *     ?UpdateLatencyTolerances@DXGADAPTER@@QEAAXXZ @ 0x1C0018B64 (-UpdateLatencyTolerances@DXGADAPTER@@QEAAXXZ.c)
 *     ?EnableClockCalibration@ADAPTER_RENDER@@QEAAXE@Z @ 0x1C0068A90 (-EnableClockCalibration@ADAPTER_RENDER@@QEAAXE@Z.c)
 *     ?FlushScheduler@ADAPTER_RENDER@@QEAAJW4DXGADAPTER_FLUSHSCHEDULER_REASON@@IH@Z @ 0x1C006A8E4 (-FlushScheduler@ADAPTER_RENDER@@QEAAJW4DXGADAPTER_FLUSHSCHEDULER_REASON@@IH@Z.c)
 *     ?ReleaseCoreResource@DXGADAPTER@@AEAAXXZ @ 0x1C0079E40 (-ReleaseCoreResource@DXGADAPTER@@AEAAXXZ.c)
 *     ?RestoreFromPurgeSegments@ADAPTER_RENDER@@AEAAX_N@Z @ 0x1C00CC2E0 (-RestoreFromPurgeSegments@ADAPTER_RENDER@@AEAAX_N@Z.c)
 *     ?ReleaseLocksForPowerStateD3transition@DXGADAPTER@@QEAAXXZ @ 0x1C012DEF4 (-ReleaseLocksForPowerStateD3transition@DXGADAPTER@@QEAAXXZ.c)
 */

void __fastcall DXGADAPTER::ReleaseCoreSync(__int64 a1, int a2)
{
  __int64 v2; // rdi
  __int64 v4; // rdx
  __int64 v5; // rcx
  __int64 v6; // r8
  ADAPTER_RENDER *v7; // rcx
  __int64 v8; // rcx
  ADAPTER_RENDER *v9; // rcx
  _QWORD *v10; // rax
  __int64 v11; // rax
  struct _KTHREAD *v12; // rdx

  v2 = a2;
  if ( !DXGADAPTER::IsCoreResourceSharedOwner((DXGADAPTER *)a1) )
  {
    v10 = (_QWORD *)WdLogNewEntry5_WdCriticalError(v5, v4, v6);
    v10[3] = 275LL;
    v10[4] = 4LL;
    v10[5] = a1;
    v10[6] = 0LL;
    v10[7] = 0LL;
    WdLogEvent5_WdCriticalError(v10);
  }
  if ( (_DWORD)v2 == 1 )
    goto LABEL_4;
  if ( (_DWORD)v2 == 2 )
    goto LABEL_11;
  if ( (_DWORD)v2 != 3 )
  {
    if ( (_DWORD)v2 != 5 )
    {
      v11 = WdLogNewEntry5_WdError((unsigned int)(v2 - 3));
      *(_QWORD *)(v11 + 24) = v2;
      WdLogEvent5_WdError(v11);
      goto LABEL_7;
    }
    v9 = *(ADAPTER_RENDER **)(a1 + 1992);
    if ( v9 )
      ADAPTER_RENDER::RestoreFromPurgeSegments(v9, 0);
LABEL_11:
    v8 = *(_QWORD *)(a1 + 1992);
    if ( v8 )
      ADAPTER_RENDER::FlushScheduler(v8, 7, 0xFFFFFFFF, 0);
    if ( *(_DWORD *)(a1 + 120) == 1 )
    {
      DXGADAPTER::ReleaseLocksForPowerStateD3transition((DXGADAPTER *)a1);
      *(_DWORD *)(a1 + 120) = 0;
      *(_DWORD *)(a1 + 2468) = 0;
      DXGADAPTER::UpdateLatencyTolerances((DXGADAPTER *)a1);
      if ( *(_QWORD *)(a1 + 2384) )
        DXGADAPTER::ScheduleAdapterActivityCheck((struct _KTHREAD **)a1, v12);
    }
  }
LABEL_4:
  if ( *(_BYTE *)(a1 + 166) )
    *(_BYTE *)(a1 + 166) = 0;
  DXGADAPTER::ReleaseCoreResource((PERESOURCE *)a1);
LABEL_7:
  v7 = *(ADAPTER_RENDER **)(a1 + 1992);
  if ( v7 )
    ADAPTER_RENDER::EnableClockCalibration(v7, 1);
}
