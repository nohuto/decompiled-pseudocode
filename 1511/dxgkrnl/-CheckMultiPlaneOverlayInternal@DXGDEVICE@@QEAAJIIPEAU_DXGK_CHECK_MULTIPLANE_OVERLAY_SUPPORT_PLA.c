/*
 * XREFs of ?CheckMultiPlaneOverlayInternal@DXGDEVICE@@QEAAJIIPEAU_DXGK_CHECK_MULTIPLANE_OVERLAY_SUPPORT_PLANE@@HPEAH@Z @ 0x1C01385B8
 * Callers:
 *     ?CheckMultiPlaneOverlaySupport@DXGDEVICE@@QEAAJIPEAUD3DKMT_CHECK_MULTIPLANE_OVERLAY_PLANE@@_NPEAH@Z @ 0x1C0139024 (-CheckMultiPlaneOverlaySupport@DXGDEVICE@@QEAAJIPEAUD3DKMT_CHECK_MULTIPLANE_OVERLAY_PLANE@@_NPEA.c)
 *     ?CheckMultiPlaneOverlaySupportFromCachedData@DXGCONTEXT@@AEAA_NIIPEAU_D3DKMT_FLIPMODEL_PRESENTHISTORYTOKEN@@@Z @ 0x1C01608C0 (-CheckMultiPlaneOverlaySupportFromCachedData@DXGCONTEXT@@AEAA_NIIPEAU_D3DKMT_FLIPMODEL_PRESENTHI.c)
 * Callees:
 *     ?GetDisplayModeInfo@ADAPTER_DISPLAY@@QEBAQEBU_DXGK_DISPLAYMODE_INFO@@I@Z @ 0x1C0007A48 (-GetDisplayModeInfo@ADAPTER_DISPLAY@@QEBAQEBU_DXGK_DISPLAYMODE_INFO@@I@Z.c)
 *     ?ClearMPOState@ADAPTER_RENDER@@QEAAXXZ @ 0x1C00196C8 (-ClearMPOState@ADAPTER_RENDER@@QEAAXXZ.c)
 *     Template_qqqddddddddddddqqqq @ 0x1C001D67C (Template_qqqddddddddddddqqqq.c)
 *     Template_qqqqq @ 0x1C001D84C (Template_qqqqq.c)
 *     ??_U@YAPEAX_KIW4_POOL_TYPE@@@Z @ 0x1C0065FA0 (--_U@YAPEAX_KIW4_POOL_TYPE@@@Z.c)
 *     ?DdiCheckMultiPlaneOverlaySupport@ADAPTER_DISPLAY@@QEAAJPEAU_DXGKARG_CHECKMULTIPLANEOVERLAYSUPPORT@@@Z @ 0x1C0126978 (-DdiCheckMultiPlaneOverlaySupport@ADAPTER_DISPLAY@@QEAAJPEAU_DXGKARG_CHECKMULTIPLANEOVERLAYSUPPO.c)
 *     ?ClassifyOverlayScenario@DXGDEVICE@@QEAAXIIPEAU_DXGK_CHECK_MULTIPLANE_OVERLAY_SUPPORT_PLANE@@PEAI11@Z @ 0x1C0139844 (-ClassifyOverlayScenario@DXGDEVICE@@QEAAXIIPEAU_DXGK_CHECK_MULTIPLANE_OVERLAY_SUPPORT_PLANE@@PEA.c)
 *     ?IsYUVAllocation@DXGDEVICE@@QEAAHPEAX@Z @ 0x1C013ACAC (-IsYUVAllocation@DXGDEVICE@@QEAAHPEAX@Z.c)
 */

__int64 __fastcall DXGDEVICE::CheckMultiPlaneOverlayInternal(
        DXGDEVICE *this,
        unsigned int a2,
        __int64 a3,
        struct _DXGK_CHECK_MULTIPLANE_OVERLAY_SUPPORT_PLANE *a4,
        int a5,
        int *a6)
{
  int *v6; // rdi
  unsigned __int64 v8; // rsi
  __int64 v11; // rcx
  __int64 v12; // r8
  bool v13; // zf
  unsigned int v14; // edi
  _DWORD *v15; // rax
  unsigned int v16; // edi
  unsigned int v17; // edi
  __int64 v18; // rax
  __int64 v19; // rax
  SIZE_T v20; // rax
  unsigned __int64 v21; // rdi
  _OWORD *v22; // rax
  _OWORD *v23; // rcx
  _OWORD *v24; // r14
  DXGK_MULTIPLANE_OVERLAY_ATTRIBUTES *p_PlaneAttributes; // rdx
  __int128 v26; // xmm0
  __int64 v27; // rdi
  __int64 v28; // rax
  unsigned int v29; // r15d
  int v30; // r14d
  _DWORD *v31; // rax
  unsigned int v32; // edi
  unsigned int v33; // edi
  __int64 v34; // rax
  __int64 v35; // rax
  __int64 v36; // rax
  __int64 v37; // rax
  __int64 v38; // rax
  __int64 v39; // rax
  __int64 v40; // rax
  __int64 v41; // rax
  D3DDDI_VIDEO_PRESENT_SOURCE_ID VidPnSourceId; // edi
  __int64 v43; // rax
  __int64 v44; // rax
  LONG *p_top; // rdi
  void *v46; // rdx
  LONG v47; // r14d
  LONG v48; // r15d
  LONG v49; // r12d
  LONG v50; // r13d
  __int64 IsYUVAllocation; // rdx
  __int64 v52; // r8
  _DXGKARG_CHECKMULTIPLANEOVERLAYSUPPORT v54; // [rsp+B0h] [rbp-9h] BYREF
  unsigned int v55; // [rsp+110h] [rbp+57h] BYREF
  unsigned int v56; // [rsp+120h] [rbp+67h] BYREF
  int v57; // [rsp+128h] [rbp+6Fh]

  v6 = a6;
  *(_QWORD *)&v54.Supported = 0LL;
  v8 = (unsigned int)a3;
  v54.PlaneCount = a3;
  v54.pPlanes = a4;
  *a6 = 0;
  v57 = ADAPTER_DISPLAY::DdiCheckMultiPlaneOverlaySupport(
          *(ADAPTER_DISPLAY **)(*((_QWORD *)this + 354) + 1984LL),
          &v54,
          a3);
  if ( v57 >= 0 )
    *v6 = v54.Supported;
  DXGDEVICE::ClassifyOverlayScenario(this, a2, v8, a4, &v55, (unsigned int *)&a6, &v56);
  v13 = *v6 == 0;
  v14 = v55;
  if ( v13 )
  {
    v29 = v56;
    v30 = (int)a6;
    if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x80u) != 0 )
      Template_qqqqq(v11, &EventCheckMultiPlaneOverlayFail, v12, a4->VidPnSourceId, v8, v55, (char)a6, v56);
    v31 = *(_DWORD **)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
    v32 = v14 - 1;
    if ( v32 )
    {
      v33 = v32 - 1;
      if ( v33 )
      {
        if ( v33 == 1 )
        {
          ++v31[87];
          if ( 6 * v30 >= 10 * v29 )
          {
            v34 = *(_QWORD *)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
            ++*(_DWORD *)(v34 + 352);
          }
        }
        else
        {
          ++v31[96];
        }
      }
      else
      {
        ++v31[93];
        if ( a5 )
        {
          v35 = *(_QWORD *)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
          ++*(_DWORD *)(v35 + 376);
        }
      }
    }
    else
    {
      ++v31[90];
      if ( a5 )
      {
        v36 = *(_QWORD *)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
        ++*(_DWORD *)(v36 + 364);
      }
    }
    if ( (_DWORD)v8 == 2 )
    {
      v37 = *(_QWORD *)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
      ++*(_DWORD *)(v37 + 400);
    }
    else if ( (_DWORD)v8 == 3 )
    {
      v38 = *(_QWORD *)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
      ++*(_DWORD *)(v38 + 404);
    }
    else if ( (unsigned int)v8 >= 4 )
    {
      v39 = *(_QWORD *)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
      ++*(_DWORD *)(v39 + 408);
    }
    if ( 2 * v30 > v29 )
    {
      v41 = *(_QWORD *)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
      if ( 3 * v30 < 4 * v29 )
        ++*(_DWORD *)(v41 + 412);
      else
        ++*(_DWORD *)(v41 + 420);
    }
    else
    {
      v40 = *(_QWORD *)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
      ++*(_DWORD *)(v40 + 416);
    }
    VidPnSourceId = a4->VidPnSourceId;
    if ( *(_DWORD *)ADAPTER_DISPLAY::GetDisplayModeInfo(
                      *(DXGADAPTER ***)(*((_QWORD *)this + 354) + 1984LL),
                      VidPnSourceId) <= 0x780u )
    {
      if ( *(_DWORD *)ADAPTER_DISPLAY::GetDisplayModeInfo(
                        *(DXGADAPTER ***)(*((_QWORD *)this + 354) + 1984LL),
                        VidPnSourceId) < 0x780u )
      {
        v44 = *(_QWORD *)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
        ++*(_DWORD *)(v44 + 396);
      }
    }
    else
    {
      v43 = *(_QWORD *)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
      ++*(_DWORD *)(v43 + 392);
    }
  }
  else
  {
    if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x80u) != 0 )
      Template_qqqqq(v11, &EventCheckMultiPlaneOverlaySuccess, v12, a4->VidPnSourceId, v8, v55, (char)a6, v56);
    v15 = *(_DWORD **)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
    v16 = v14 - 1;
    if ( v16 )
    {
      v17 = v16 - 1;
      if ( v17 )
      {
        if ( v17 == 1 )
          ++v15[86];
        else
          ++v15[95];
      }
      else
      {
        ++v15[92];
        if ( a5 )
        {
          v18 = *(_QWORD *)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
          ++*(_DWORD *)(v18 + 376);
        }
      }
    }
    else
    {
      ++v15[89];
      if ( a5 )
      {
        v19 = *(_QWORD *)(*(_QWORD *)(*((_QWORD *)this + 2) + 16LL) + 3152LL);
        ++*(_DWORD *)(v19 + 364);
      }
    }
    ADAPTER_RENDER::ClearMPOState(*((void ***)this + 2));
    v20 = 88 * v8;
    v21 = v8;
    if ( !is_mul_ok(v8, 0x58uLL) )
      v20 = -1LL;
    v22 = operator new[](v20, 0x4B677844u, PagedPool);
    v24 = v22;
    if ( v22 )
    {
      if ( (_DWORD)v8 )
      {
        v23 = v22;
        p_PlaneAttributes = &a4->PlaneAttributes;
        do
        {
          v26 = *(_OWORD *)&p_PlaneAttributes->Flags.0;
          p_PlaneAttributes = (DXGK_MULTIPLANE_OVERLAY_ATTRIBUTES *)((char *)p_PlaneAttributes + 104);
          *v23 = v26;
          v23 = (_OWORD *)((char *)v23 + 88);
          *(_OWORD *)((char *)v23 - 72) = *(_OWORD *)&p_PlaneAttributes[-1].Flags.0;
          *(_OWORD *)((char *)v23 - 56) = *(_OWORD *)&p_PlaneAttributes[-1].SrcRect.bottom;
          *(_OWORD *)((char *)v23 - 40) = *(_OWORD *)&p_PlaneAttributes[-1].DstRect.bottom;
          *(_OWORD *)((char *)v23 - 24) = *(_OWORD *)&p_PlaneAttributes[-1].ClipRect.bottom;
          *((_QWORD *)v23 - 1) = *(_QWORD *)&p_PlaneAttributes[-1].YCbCrFlags.0;
          --v21;
        }
        while ( v21 );
      }
      v27 = *((_QWORD *)this + 2);
      if ( *(_QWORD *)(v27 + 872) )
      {
        v28 = WdLogNewEntry5_WdAssertion(v23);
        *(_QWORD *)(v28 + 24) = 3025LL;
        WdLogEvent5_WdAssertion(v28);
      }
      *(_DWORD *)(v27 + 868) = a2;
      *(_DWORD *)(v27 + 864) = v8;
      *(_QWORD *)(v27 + 872) = v24;
    }
  }
  if ( (_DWORD)v8 )
  {
    p_top = &a4->PlaneAttributes.ClipRect.top;
    do
    {
      v46 = *(void **)(p_top - 13);
      v47 = p_top[11];
      v48 = p_top[6];
      v49 = p_top[4];
      v50 = p_top[3];
      v56 = *(p_top - 10);
      IsYUVAllocation = (unsigned int)DXGDEVICE::IsYUVAllocation(this, v46);
      if ( (Microsoft_Windows_DxgKrnlEnableBits & 0x80u) != 0 )
        Template_qqqddddddddddddqqqq(
          (unsigned int)p_top[1],
          IsYUVAllocation,
          v52,
          *(p_top - 11),
          IsYUVAllocation,
          v56,
          *(p_top - 9),
          *(p_top - 7),
          *(p_top - 8),
          *(p_top - 6),
          *(p_top - 5),
          *(p_top - 3),
          *(p_top - 4),
          *(p_top - 2),
          *(p_top - 1),
          p_top[1],
          *p_top,
          p_top[2],
          v50,
          v49,
          v48,
          v47);
      p_top += 26;
      --v8;
    }
    while ( v8 );
  }
  return (unsigned int)v57;
}
