/*
 * XREFs of DpiGdiHandleAsyncMonitorEventCallout @ 0x1C016B440
 * Callers:
 *     <none>
 * Callees:
...
void __fastcall DpiGdiHandleAsyncMonitorEventCallout(PVOID IoObject, char *Context, PIO_WORKITEM IoWorkItem)
{
  ...
  if ( !*((_DWORD *)Context + 5) )
  {
    ...
    if ( ((v13 - 1) & 0xFFFFFFFFFFFFFFFCuLL) == 0 && v13 != 2 && (Microsoft_Windows_DxgKrnlEnableBits & 8) != 0 )
      Template_q(v13, &Dxgk_PerfTrack_DMM_Hotplug_Monitor_Set_VideoMode_Stop, v9, *((_DWORD *)Context + 4));
  }
  if ( *Context )
    _InterlockedDecrement(&dword_1C0046E54);
