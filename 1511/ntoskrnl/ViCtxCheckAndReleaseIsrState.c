/*
 * XREFs of ViCtxCheckAndReleaseIsrState @ 0x1406CB3B8
 * Callers:
 *     ViCtxIsr @ 0x1406CB5B4 (ViCtxIsr.c)
 *     ViCtxIsrMessageBased @ 0x1406CB600 (ViCtxIsrMessageBased.c)
 * Callees:
 *     RtlXSave @ 0x140022854 (RtlXSave.c)
 *     VfUtilDbgPrint @ 0x14020AB90 (VfUtilDbgPrint.c)
 *     VfErrorStoreTriageInformation @ 0x1406C2AA0 (VfErrorStoreTriageInformation.c)
 *     ViCtxEqualExtendedState @ 0x1406CB4AC (ViCtxEqualExtendedState.c)
 */

__int64 __fastcall ViCtxCheckAndReleaseIsrState(__int64 a1, const void *a2, __int64 a3)
{
  unsigned __int8 CurrentIrql; // al
  unsigned __int8 v6; // si
  __int64 v7; // r8
  __int64 result; // rax
  unsigned __int8 v9; // al
  __int64 v10; // rax
  __int64 v11; // rdx
  __int64 v12; // r9

  CurrentIrql = KeGetCurrentIrql();
  v6 = *(_BYTE *)(a1 + 8);
  *(_BYTE *)(a1 + 9) = CurrentIrql;
  if ( v6 != CurrentIrql
    || ((*(_DWORD *)(a1 + 4) & 2) == 0
      ? (result = 1LL)
      : (RtlXSave(*(_DWORD **)(a1 + 64), ViCtxXStateEnabledMask, a3),
         v6 = *(_BYTE *)(a1 + 8),
         LOBYTE(v7) = v6 == 0,
         result = (unsigned __int8)ViCtxEqualExtendedState(*(_QWORD *)(a1 + 32), *(_QWORD *)(a1 + 64), v7)),
        !(_DWORD)result) )
  {
    v9 = *(_BYTE *)(a1 + 9);
    if ( v6 == v9 )
    {
      VfUtilDbgPrint(
        "Interrupt Service Routine %p has changed extended thread context.\n"
        "Context saved before executing ISR: 0x%p. Context saved after executing ISR: 0x%p.\n",
        a2,
        *(const void **)(a1 + 32),
        *(const void **)(a1 + 64));
      v10 = *(_QWORD *)(a1 + 64);
      v11 = 272LL;
      v12 = *(_QWORD *)(a1 + 32);
    }
    else
    {
      VfUtilDbgPrint(
        "Interrupt Service Routine %p has changed IRQL.\nIRQL before executing ISR: %d. IRQL after executing ISR: %d.\n",
        a2,
        v6,
        v9);
      v10 = *(unsigned __int8 *)(a1 + 9);
      v11 = 273LL;
      v12 = *(unsigned __int8 *)(a1 + 8);
    }
    result = VfErrorStoreTriageInformation(196LL, v11, (__int64)a2, v12, v10);
    __int2c();
    if ( (_DWORD)result )
      result = (unsigned int)_InterlockedExchange(&VfErrorBugcheckDataReady, 0);
  }
  *(_DWORD *)(a1 + 4) &= ~2u;
  _InterlockedExchange((volatile __int32 *)a1, 0);
  return result;
}
