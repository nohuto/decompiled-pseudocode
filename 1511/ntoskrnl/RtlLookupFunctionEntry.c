/*
 * XREFs of RtlLookupFunctionEntry @ 0x140025E10
 * Callers:
        ...
        PUNWIND_HISTORY_TABLE HistoryTable)
{
  ...
  {
    HistoryTable->Once = 0;
    v31 = *(_QWORD *)&RtlpUnwindHistoryTable[4 * (unsigned __int8)byte_1403815D5 + 6];
    result = *(PRUNTIME_FUNCTION *)&RtlpUnwindHistoryTable[4 * (unsigned __int8)byte_1403815D5 + 8];
    if ( result )
    {
  ...
  if ( !HistoryTable->Search )
    goto LABEL_2;
  if ( ControlPc < qword_1403815D8
    || ControlPc >= qword_1403815E0
    ...
    if ( ControlPc >= HistoryTable->LowAddress && ControlPc < HistoryTable->HighAddress )
    {
      ...
      if ( LocalHint < HistoryTable->Count )
      {
        while ( 1 )
        {
          ...
          if ( ControlPc < v28 + FunctionEntry->EndAddress && ControlPc >= v28 + FunctionEntry->BeginAddress )
            break;
...
LABEL_2:
    if ( ControlPc < *(&xmmword_1402CFDD0 + 1) || ControlPc >= *(&xmmword_1402CFDD0 + 1) + (unsigned int)qword_1402CFDE0 )
    {
      v6 = RtlpxLookupFunctionTable(ControlPc, &v32);
    }
    else
    {
      v32 = *(_OWORD *)&xmmword_1402CFDD0;
      v33 = qword_1402CFDE0;
      v6 = xmmword_1402CFDD0;
    }
    v7 = 0LL;
    if ( v6 )
    {
