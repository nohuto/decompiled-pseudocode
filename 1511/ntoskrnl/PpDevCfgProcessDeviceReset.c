/*
 * XREFs of PpDevCfgProcessDeviceReset @ 0x14060D8F8
 * Callers:
 *     PiConfigureDevice @ 0x140614C4C (PiConfigureDevice.c)
 * Callees:
 *     __security_check_cookie @ 0x140143620 (__security_check_cookie.c)
 *     memset @ 0x140166CC0 (memset.c)
 *     _CmGetDeviceRegProp @ 0x14043AFD0 (_CmGetDeviceRegProp.c)
 *     _PnpOpenObjectRegKey @ 0x14043E904 (_PnpOpenObjectRegKey.c)
 *     RtlFreeAnsiString @ 0x140445624 (RtlFreeAnsiString.c)
 *     RtlStringFromGUIDEx @ 0x140447328 (RtlStringFromGUIDEx.c)
 *     PiDevCfgInitDeviceContext @ 0x14051047C (PiDevCfgInitDeviceContext.c)
 *     PiDevCfgMigrateDevice @ 0x140514B9C (PiDevCfgMigrateDevice.c)
 *     PiDevCfgSetDeviceRegProp @ 0x140514F8C (PiDevCfgSetDeviceRegProp.c)
 *     _PnpCtxRegDeleteTree @ 0x140522444 (_PnpCtxRegDeleteTree.c)
 *     PiDevCfgFreeDeviceContext @ 0x14053B1A8 (PiDevCfgFreeDeviceContext.c)
 *     PiDevCfgQueryObjectProperties @ 0x14053EF3C (PiDevCfgQueryObjectProperties.c)
 *     PiDevCfgResetDeviceDriverSettings @ 0x140609F54 (PiDevCfgResetDeviceDriverSettings.c)
 */

__int64 __fastcall PpDevCfgProcessDeviceReset(__int64 a1)
{
  int v1; // ebx
  int inited; // edi
  __int64 v4; // r14
  __int64 v5; // rdx
  __int64 v6; // rcx
  int v7; // eax
  int v8; // r15d
  __int64 v9; // rdx
  __int64 v10; // rcx
  int v12; // [rsp+48h] [rbp-79h] BYREF
  int v13; // [rsp+4Ch] [rbp-75h] BYREF
  int v14; // [rsp+50h] [rbp-71h] BYREF
  int v15; // [rsp+54h] [rbp-6Dh] BYREF
  UNICODE_STRING UnicodeString; // [rsp+58h] [rbp-69h] BYREF
  __int64 v17; // [rsp+68h] [rbp-59h] BYREF
  _QWORD v18[5]; // [rsp+70h] [rbp-51h] BYREF
  _QWORD v19[10]; // [rsp+98h] [rbp-29h] BYREF
  _QWORD v20[2]; // [rsp+E8h] [rbp+27h] BYREF

  v1 = 0;
  LODWORD(v19[0]) = 0;
  memset(&v19[1], 0, 0x40uLL);
  v15 = 1;
  v17 = 0LL;
  *(_DWORD *)&UnicodeString.Length = 0;
  UnicodeString.Buffer = 0LL;
  v12 = 0;
  v14 = 0;
  v13 = 0;
  if ( !PiDevCfgMode )
  {
    inited = 0;
    goto LABEL_20;
  }
  if ( !*(_QWORD *)(a1 + 48) )
  {
    inited = -1073741808;
    goto LABEL_20;
  }
  inited = PiDevCfgInitDeviceContext(*(_QWORD *)(a1 + 48), 0LL, v19);
  if ( inited >= 0 )
  {
    v4 = v19[2];
    memset(v18, 0, sizeof(v18));
    v5 = *(_QWORD *)(a1 + 48);
    v18[0] = &DEVPKEY_Device_ClassGuid;
    v18[2] = v20;
    LODWORD(v18[1]) = 13;
    LODWORD(v18[3]) = 16;
    inited = PiDevCfgQueryObjectProperties(v6, v5, 1u, (void *)v19[2], (__int64)v18, 1u);
    if ( inited >= 0 )
    {
      if ( SLODWORD(v18[4]) >= 0 )
      {
        inited = RtlStringFromGUIDEx((unsigned int *)v20, (__int64)&UnicodeString, 1);
        if ( inited < 0 )
          goto LABEL_20;
        if ( (int)PnpOpenObjectRegKey(
                    *(__int64 *)&PiPnpRtlCtx,
                    (__int64)UnicodeString.Buffer,
                    2u,
                    131097,
                    0,
                    (__int64)&v17,
                    0LL,
                    0) >= 0 )
          goto LABEL_12;
        RtlFreeAnsiString(&UnicodeString);
      }
      v20[1] = 0LL;
      v20[0] = 0LL;
LABEL_12:
      v7 = PiDevCfgMigrateDevice(
             a1,
             (__int64)v19,
             (unsigned __int16 *)((unsigned __int64)&UnicodeString & -(__int64)(UnicodeString.Buffer != 0LL)),
             0LL,
             &v14,
             0LL);
      v8 = v14;
      if ( v7 < 0 )
        v8 = 0;
      PiDevCfgResetDeviceDriverSettings(
        a1,
        (__int64)v19,
        (_QWORD *)((unsigned __int64)v20 & -(__int64)(UnicodeString.Buffer != 0LL)),
        v17,
        0LL);
      PnpCtxRegDeleteTree(*(__int64 *)&PiPnpRtlCtx, v4, (__int64)L"Devices");
      if ( v8 )
      {
        v9 = *(_QWORD *)(a1 + 48);
        v13 = 4;
        if ( (int)CmGetDeviceRegProp(*(__int64 *)&PiPnpRtlCtx, v9, v4, 11, (__int64)&v15, (__int64)&v12, (__int64)&v13) >= 0
          && v15 == 4
          && v13 == 4 )
        {
          v1 = v12;
        }
        v12 = v8 | v1;
        PiDevCfgSetDeviceRegProp(v10, (__int64)v19, 0xBu, 4u, (__int64)&v12, 4u);
      }
    }
  }
LABEL_20:
  RtlFreeAnsiString(&UnicodeString);
  PiDevCfgFreeDeviceContext((__int64)v19);
  return (unsigned int)inited;
}
