/*
 * XREFs of _PnpValidatePropertyData @ 0x14045C944
 * Callers:
 *     _PnpGetObjectPropertyWorker @ 0x14043DAD8 (_PnpGetObjectPropertyWorker.c)
 *     _PnpSetObjectPropertyWorker @ 0x14045C6A0 (_PnpSetObjectPropertyWorker.c)
 *     ValidFilter @ 0x14045C870 (ValidFilter.c)
 *     PiDqQueryValidateQueryData @ 0x14045F270 (PiDqQueryValidateQueryData.c)
 *     PiSwValidatePropertyArray @ 0x1404E59E8 (PiSwValidatePropertyArray.c)
 * Callees:
 *     _GetBaseTypeSize @ 0x140099AFC (_GetBaseTypeSize.c)
 *     _IsFixedSizeType @ 0x140099B9C (_IsFixedSizeType.c)
 *     RtlUnalignedStringCbLengthW @ 0x140099BBC (RtlUnalignedStringCbLengthW.c)
 *     RtlValidRelativeSecurityDescriptor @ 0x1403E083C (RtlValidRelativeSecurityDescriptor.c)
 *     RtlLengthSecurityDescriptor @ 0x14042E630 (RtlLengthSecurityDescriptor.c)
 */

__int64 __fastcall PnpValidatePropertyData(__int64 *SecurityDescriptor, ULONG SecurityDescriptorLength, int a3)
{
  unsigned int v3; // ebx
  size_t v4; // rsi
  unsigned int v6; // ebp
  int BaseTypeSize; // eax
  __int16 v8; // r8
  int v9; // r10d
  int v10; // r11d
  int v11; // r9d
  unsigned int v13; // esi
  unsigned int v14; // eax
  unsigned int v15; // ecx
  bool v16; // zf
  size_t v17; // rbp
  size_t v18; // rax
  bool v19; // zf
  unsigned int v20; // r9d
  size_t pcbLength; // [rsp+58h] [rbp+20h] BYREF

  v3 = 0;
  v4 = SecurityDescriptorLength;
  if ( (a3 & 0xFFFF0000) != 0 )
    return (unsigned int)-1073741811;
  v6 = a3 & 0xFFF;
  if ( v6 > 0x19 )
    return (unsigned int)-1073741811;
  BaseTypeSize = GetBaseTypeSize(a3 & 0xFFF);
  v9 = v8 & 0xF000;
  v10 = 0x2000;
  if ( !BaseTypeSize )
    goto LABEL_9;
  if ( v9 == 4096 )
  {
    if ( v6 <= 1 || !IsFixedSizeType(v8) || (unsigned int)v4 < v20 )
      return (unsigned int)-1073741811;
    v16 = (unsigned int)v4 % v20 == 0;
  }
  else
  {
    if ( v9 != 0x2000 )
    {
      if ( (v8 & 0xF000) != 0 || IsFixedSizeType(v8) && (_DWORD)v4 != v11 )
        return (unsigned int)-1073741811;
      goto LABEL_9;
    }
    if ( v6 == 18 )
      goto LABEL_9;
    v16 = v6 == 20;
  }
  if ( !v16 )
    return (unsigned int)-1073741811;
LABEL_9:
  if ( v6 <= 1 )
  {
    v19 = (_DWORD)v4 == 0;
    goto LABEL_43;
  }
  if ( v6 != 18 )
  {
    if ( v6 == 16 )
    {
      if ( SecurityDescriptor )
      {
        v13 = (unsigned int)v4 >> 3;
        v14 = 0;
        if ( !v13 )
          return v3;
        while ( *SecurityDescriptor >= 0 )
        {
          ++v14;
          ++SecurityDescriptor;
          if ( v14 >= v13 )
            return v3;
        }
      }
    }
    else
    {
      if ( v6 != 17 )
      {
        if ( v6 != 25 )
        {
          if ( v6 == 19 )
          {
            if ( !SecurityDescriptor || !RtlValidRelativeSecurityDescriptor(SecurityDescriptor, v4, 0) )
              return (unsigned int)-1073741811;
            v19 = RtlLengthSecurityDescriptor(SecurityDescriptor) == (_DWORD)v4;
            goto LABEL_43;
          }
          if ( v6 != 20 )
            return v3;
        }
        goto LABEL_16;
      }
      if ( SecurityDescriptor )
      {
        v15 = 0;
        if ( !(_DWORD)v4 )
          return v3;
        while ( *(_BYTE *)SecurityDescriptor == 0xFF || *(_BYTE *)SecurityDescriptor == 0 )
        {
          ++v15;
          SecurityDescriptor = (__int64 *)((char *)SecurityDescriptor + 1);
          if ( v15 >= (unsigned int)v4 )
            return v3;
        }
      }
    }
    return (unsigned int)-1073741811;
  }
LABEL_16:
  if ( !SecurityDescriptor || (unsigned int)v4 < 2 )
    return (unsigned int)-1073741811;
  if ( (v9 & v10) != 0 )
  {
    v17 = 0LL;
    while ( *(_WORD *)SecurityDescriptor )
    {
      if ( RtlUnalignedStringCbLengthW((STRSAFE_PCUNZWCH)SecurityDescriptor, v4 - v17, &pcbLength) < 0 )
        return (unsigned int)-1073741811;
      if ( (pcbLength & 1) != 0 )
        return (unsigned int)-1073741811;
      v18 = pcbLength + 2;
      pcbLength = v18;
      if ( v18 > 0xFFFE )
        return (unsigned int)-1073741811;
      v17 += v18;
      if ( v17 > v4 )
        return (unsigned int)-1073741811;
      SecurityDescriptor = (__int64 *)((char *)SecurityDescriptor + 2 * (v18 >> 1));
    }
    v19 = v17 + 2 == v4;
LABEL_43:
    if ( v19 )
      return v3;
    return (unsigned int)-1073741811;
  }
  pcbLength = 0LL;
  if ( RtlUnalignedStringCbLengthW((STRSAFE_PCUNZWCH)SecurityDescriptor, v4, &pcbLength) < 0
    || pcbLength + 2 > 0xFFFE
    || pcbLength + 2 != v4 )
  {
    return (unsigned int)-1073741811;
  }
  return v3;
}
