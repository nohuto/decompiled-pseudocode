/*
 * XREFs of IopLiveDumpCaptureMemoryPages @ 0x1403AE820
 * Callers:
 *     IoCaptureLiveDump @ 0x1405FC30C (IoCaptureLiveDump.c)
 * Callees:
 *     _TlgKeywordOn @ 0x1400923CC (_TlgKeywordOn.c)
 *     _TlgWrite @ 0x140092474 (_TlgWrite.c)
 *     VslIsSecureKernelRunning @ 0x1401429E8 (VslIsSecureKernelRunning.c)
 *     __security_check_cookie @ 0x140143620 (__security_check_cookie.c)
 *     memset @ 0x140166CC0 (memset.c)
 *     HvlFinalizeLiveDumpInSk @ 0x1401B3014 (HvlFinalizeLiveDumpInSk.c)
 *     MmDuplicateMemory @ 0x14039E1A0 (MmDuplicateMemory.c)
 *     IopLiveDumpUncorralProcessors @ 0x1403AF83C (IopLiveDumpUncorralProcessors.c)
 */

__int64 __fastcall IopLiveDumpCaptureMemoryPages(__int64 a1)
{
  __int64 v2; // r14
  int v3; // esi
  unsigned __int64 v5; // [rsp+30h] [rbp-29h] BYREF
  _QWORD v6[5]; // [rsp+38h] [rbp-21h] BYREF
  EVENT_DATA_DESCRIPTOR pData; // [rsp+60h] [rbp+7h] BYREF
  unsigned __int64 *v8; // [rsp+80h] [rbp+27h]
  int v9; // [rsp+88h] [rbp+2Fh]
  int v10; // [rsp+8Ch] [rbp+33h]

  memset(&v6[1], 0, 0x20uLL);
  v2 = MEMORY[0xFFFFF78000000008];
  v6[0] = IopLiveDumpStartMirroringCallback;
  v6[1] = IopLiveDumpEndMirroringCallback;
  v6[2] = IopLiveDumpMirrorPhysicalMemoryCallback;
  LODWORD(v6[4]) = 17;
  v3 = MmDuplicateMemory((__int64)v6);
  if ( v3 < 0 )
  {
    if ( (*(_DWORD *)(a1 + 248) & 1) != 0 )
      IopLiveDumpUncorralProcessors(a1 + 240);
  }
  else if ( *(_QWORD *)(a1 + 504) && VslIsSecureKernelRunning() )
  {
    HvlFinalizeLiveDumpInSk((_QWORD *)(a1 + 176), a1 + 128);
  }
  if ( stru_1402D1D70.LevelPlus1 > 5 && TlgKeywordOn(&stru_1402D1D70, 0x400000000000uLL) )
  {
    v9 = 8;
    v10 = 0;
    v5 = (MEMORY[0xFFFFF78000000008] - v2) / 0x2710uLL;
    v8 = &v5;
    TlgWrite(&stru_1402D1D70, &unk_140253BD3, (LPCGUID)(a1 + 624), (LPCGUID)(a1 + 608), 3u, &pData);
  }
  return (unsigned int)v3;
}
