/*
 * XREFs of MiComparePteProtections @ 0x1401D74D0
 * Callers:
 *     MiSecureVad @ 0x140415A20 (MiSecureVad.c)
 *     MiCheckSecuredVad @ 0x14047BB18 (MiCheckSecuredVad.c)
 * Callees:
 *     ExReleaseSpinLockShared @ 0x14000A410 (ExReleaseSpinLockShared.c)
 *     MiQueryAddressState @ 0x14005EA30 (MiQueryAddressState.c)
 *     ExAcquireSpinLockShared @ 0x140079490 (ExAcquireSpinLockShared.c)
 */

__int64 __fastcall MiComparePteProtections(ULONG_PTR a1, unsigned __int64 a2, unsigned __int64 a3, int a4, int a5)
{
  unsigned int v6; // ebx
  _KPROCESS *Process; // r12
  KIRQL v11; // r14
  int v12; // ecx
  bool v14; // zf
  unsigned __int64 v15; // [rsp+40h] [rbp-48h] BYREF
  unsigned __int64 v16[8]; // [rsp+48h] [rbp-40h] BYREF
  int v17; // [rsp+98h] [rbp+10h] BYREF
  int v18; // [rsp+A0h] [rbp+18h] BYREF

  v6 = 0;
  v15 = 0LL;
  Process = KeGetCurrentThread()->ApcState.Process;
  v11 = ExAcquireSpinLockShared((PEX_SPIN_LOCK)&Process[1].IdealNode[12]);
  while ( a2 <= a3 )
  {
    MiQueryAddressState(a2, v11, a1, (__int64)Process, &v15, &v17, (__int64 *)v16, &v18);
    v12 = v17;
    if ( !v17 )
    {
      if ( v18 )
        goto LABEL_8;
      v14 = ((*(_DWORD *)(a1 + 48) >> 3) & 0x1F) == 0;
      v12 = (*(_DWORD *)(a1 + 48) >> 3) & 0x1F;
      v17 = v12;
      if ( v14 )
        goto LABEL_8;
    }
    if ( a5 == 1 )
    {
      if ( v12 != a4 )
        goto LABEL_8;
    }
    else if ( v12 == -1
           || (v12 & 0xFFFFFFF8) == 0x10
           || (a4 != 1 ? (v14 = (v12 & 4) == 0) : (v14 = (v12 & 7) == 0), v14) )
    {
LABEL_8:
      v6 = -1073741755;
      break;
    }
    a2 = v16[0];
  }
  ExReleaseSpinLockShared((PEX_SPIN_LOCK)&Process[1].IdealNode[12], v11);
  return v6;
}
