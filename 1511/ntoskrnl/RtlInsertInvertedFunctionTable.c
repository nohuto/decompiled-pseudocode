/*
 * XREFs of RtlInsertInvertedFunctionTable @ 0x14001A188
 * Callers:
...
__int64 __fastcall RtlInsertInvertedFunctionTable(unsigned __int64 a1, int a2)
{
  ...
  if ( PsInvertedFunctionTable[0] == dword_1402CFDC4 )
  {
    byte_1402CFDCC = 1;
  }
  else
  {
    ...
    if ( PsInvertedFunctionTable[0] != 1 )
    {
      ...
      {
        v6 = (unsigned __int64 *)&unk_1402CFDF0;
        do
        {
          if ( a1 < *v6 )
            break;
      ...
      if ( v5 != PsInvertedFunctionTable[0] )
        memmove(
          &PsInvertedFunctionTable[6 * v5 + 10],
