/*
 * XREFs of PnpProcessTargetDeviceEvent @ 0x1404E0D6C
 * Callers:
...
__int64 __fastcall PnpProcessTargetDeviceEvent(const void **a1)
{
  ...
  if ( (unsigned __int8)PnpCompareGuid(v1 + 13, &GUID_DEVICE_QUERY_AND_REMOVE)
    || (unsigned __int8)PnpCompareGuid(v1 + 13, &GUID_DEVICE_EJECT) )
  {
  ...
  else if ( (unsigned __int8)PnpCompareGuid(v1 + 13, &GUID_DEVICE_ARRIVAL) )
  {
    PiUEventNotifyUserMode((__int64)v1);
  }
  else if ( (unsigned __int8)PnpCompareGuid(v1 + 13, &GUID_DEVICE_NOOP) )
  {
    v3 = 0;
  }
  if ( P )
