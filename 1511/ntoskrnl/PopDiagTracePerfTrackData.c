/*
 * XREFs of PopDiagTracePerfTrackData @ 0x1404F80E0
 * Callers:
 *     PopIssueActionRequest @ 0x1404F7688 (PopIssueActionRequest.c)
 * Callees:
 *     EtwEventEnabled @ 0x140074DA0 (EtwEventEnabled.c)
 *     KeGetPrcb @ 0x14009DB9C (KeGetPrcb.c)
 *     EtwWrite @ 0x14009DEC0 (EtwWrite.c)
 *     __security_check_cookie @ 0x140143620 (__security_check_cookie.c)
 *     PopQpcTimeInMs @ 0x1403A5880 (PopQpcTimeInMs.c)
 *     PopComputeDerivedHiberStats @ 0x1404F8608 (PopComputeDerivedHiberStats.c)
 */

char __fastcall PopDiagTracePerfTrackData(char a1)
{
  _UNKNOWN **v1; // rax
  unsigned __int64 v3; // rdi
  int v4; // ecx
  int v5; // eax
  int v6; // ecx
  int v7; // eax
  int v8; // ecx
  unsigned __int64 v9; // rax
  union _EVENT_DATA_DESCRIPTOR::$535316677C6A15A6ECBA40D88E1D787B *p_Reserved; // rcx
  __int64 v11; // r8
  int *v12; // rdx
  const EVENT_DESCRIPTOR *v13; // rdx
  unsigned int v14; // esi
  int v15; // eax
  int v16; // ecx
  int v17; // eax
  int v18; // ecx
  unsigned __int64 v19; // rax
  union _EVENT_DATA_DESCRIPTOR::$535316677C6A15A6ECBA40D88E1D787B *v20; // rcx
  __int64 v21; // r8
  int *v22; // rdx
  int v24; // [rsp+38h] [rbp-69h] BYREF
  int v25; // [rsp+3Ch] [rbp-65h] BYREF
  int v26; // [rsp+40h] [rbp-61h] BYREF
  int v27; // [rsp+44h] [rbp-5Dh] BYREF
  int v28; // [rsp+48h] [rbp-59h] BYREF
  int v29; // [rsp+4Ch] [rbp-55h] BYREF
  __int64 v30; // [rsp+50h] [rbp-51h] BYREF
  unsigned int v31; // [rsp+5Ch] [rbp-45h]
  int v32; // [rsp+60h] [rbp-41h] BYREF
  int v33; // [rsp+64h] [rbp-3Dh] BYREF
  int v34; // [rsp+68h] [rbp-39h] BYREF
  __int16 v35; // [rsp+6Ch] [rbp-35h]
  __int16 v36; // [rsp+6Eh] [rbp-33h]
  __int16 v37; // [rsp+70h] [rbp-31h]
  __int16 v38; // [rsp+72h] [rbp-2Fh]
  __int16 v39; // [rsp+74h] [rbp-2Dh]
  __int16 v40; // [rsp+76h] [rbp-2Bh]
  int v41; // [rsp+78h] [rbp-29h]
  __int16 v42; // [rsp+7Ch] [rbp-25h]
  __int16 v43; // [rsp+7Eh] [rbp-23h]
  struct _EVENT_DATA_DESCRIPTOR UserData; // [rsp+88h] [rbp-19h] BYREF
  int *v45; // [rsp+98h] [rbp-9h]
  __int64 v46; // [rsp+A0h] [rbp-1h]
  int *v47; // [rsp+A8h] [rbp+7h]
  __int64 v48; // [rsp+B0h] [rbp+Fh]
  int *v49; // [rsp+B8h] [rbp+17h]
  __int64 v50; // [rsp+C0h] [rbp+1Fh]
  int *v51; // [rsp+C8h] [rbp+27h]
  __int64 v52; // [rsp+D0h] [rbp+2Fh]
  _UNKNOWN *retaddr; // [rsp+100h] [rbp+5Fh] BYREF

  v1 = &retaddr;
  if ( PopDiagHandleRegistered )
  {
    LOBYTE(v1) = EtwEventEnabled(PopDiagHandle, &POP_ETW_EVENT_TRANSITIONTIMES);
    if ( (_BYTE)v1 )
    {
      v3 = 1000LL * *(unsigned int *)(KeGetPrcb(0) + 1524);
      v32 = PopQpcTimeInMs(&qword_1402DE610, &qword_1402DE618);
      v29 = PopQpcTimeInMs(&qword_1402DE600, &qword_1402DE608);
      if ( qword_1402DE5F8 )
      {
        v30 = 0LL;
        v26 = PopQpcTimeInMs(&v30, &qword_1402DE5F8);
        v24 = v26 + PopQpcTimeInMs(&qword_1402DE5D8, &qword_1402DE5E0);
        *(_QWORD *)&UserData.Size = 4LL;
        UserData.Ptr = (ULONGLONG)&dword_1402DE798;
        v45 = &v24;
        v47 = &v26;
        v49 = &v29;
        v51 = &v32;
        v46 = 4LL;
        v48 = 4LL;
        v50 = 4LL;
        v52 = 4LL;
        LOBYTE(v1) = EtwWrite(PopDiagHandle, &POP_ETW_EVENT_PERFTRACK_STANDBY, 0LL, 5u, &UserData);
      }
      else
      {
        LOBYTE(v1) = dword_1402DE7E0;
        if ( dword_1402DE7E0 )
        {
          v4 = qword_1402DE660;
          v27 = (4 * dword_1402DE7E0) & 0x3FFFFF;
          v33 = qword_1402DE660;
          v28 = qword_1402DE638 / v3;
          LOBYTE(v1) = qword_1402DE7F0;
          v24 = qword_1402DE7F0;
          if ( (a1 & 8) != 0 )
          {
            if ( !PopShutdownButtonPressTime )
              return (char)v1;
            v25 = PopQpcTimeInMs(&PopShutdownButtonPressTime, &qword_1402DE5C8) + v4;
            UserData.Ptr = (ULONGLONG)&v27;
            *(_QWORD *)&UserData.Size = 4LL;
            v45 = &v25;
            v46 = 4LL;
            v47 = &v28;
            v48 = 4LL;
            EtwWrite(PopDiagHandle, &POP_ETW_EVENT_PERFTRACK_HYBRID_SHUTDOWN, 0LL, 3u, &UserData);
            PopComputeDerivedHiberStats(&qword_1402DE620, v3, &v30);
            v34 = v24;
            v35 = -1;
            if ( (unsigned __int64)qword_1402DE7C8 >> 8 <= 0xFFFF )
              v35 = (unsigned int)qword_1402DE7C8 >> 8;
            v36 = -1;
            if ( (unsigned __int64)qword_1402DE7D0 >> 8 <= 0xFFFF )
              v36 = (unsigned int)qword_1402DE7D0 >> 8;
            v37 = -1;
            if ( qword_1402DE678 / v3 <= 0xFFFF )
              v37 = qword_1402DE678 / v3;
            v38 = -1;
            if ( qword_1402DE758 / v3 <= 0xFFFF )
              v38 = qword_1402DE758 / v3;
            v39 = -1;
            if ( (unsigned int)dword_1402DE668 <= 0xFFFF )
              v39 = dword_1402DE668;
            v40 = -1;
            if ( (unsigned int)dword_1402DE798 <= 0xFFFF )
              v40 = dword_1402DE798;
            v5 = HIDWORD(v30);
            v42 = -1;
            if ( HIDWORD(v30) > 0x3FF )
              v5 = 1023;
            v6 = (v41 ^ v5) & 0x3FF ^ v41;
            v7 = v31;
            if ( v31 > 0x1FF )
              v7 = 511;
            v8 = (v6 ^ (v7 << 10)) & 0x7FC00 ^ v6;
            v9 = (qword_1402DE6F0 - qword_1402DE720) / v3;
            if ( v9 > 0x1FFF )
              LODWORD(v9) = 0x1FFF;
            v41 = v8 & 0x7FFFF | ((_DWORD)v9 << 19);
            if ( (unsigned __int64)qword_1402DE7A8 >> 8 <= 0xFFFF )
              v42 = (unsigned int)qword_1402DE7A8 >> 8;
            v43 = -1;
            if ( (unsigned __int64)qword_1402DE7B0 >> 8 <= 0xFFFF )
              v43 = (unsigned int)qword_1402DE7B0 >> 8;
            p_Reserved = (union _EVENT_DATA_DESCRIPTOR::$535316677C6A15A6ECBA40D88E1D787B *)&UserData.Reserved;
            v11 = 6LL;
            v12 = &v34;
            do
            {
              *(_QWORD *)&p_Reserved[-3].Reserved = v12++;
              *(_QWORD *)&p_Reserved[-1].Reserved = 4LL;
              p_Reserved += 4;
              --v11;
            }
            while ( v11 );
            v13 = (const EVENT_DESCRIPTOR *)&POP_ETW_EVENT_PERFTRACK_HYBRID_RESUME;
          }
          else
          {
            UserData.Ptr = (ULONGLONG)&v27;
            v45 = &v33;
            *(_QWORD *)&UserData.Size = 4LL;
            v47 = &v28;
            v46 = 4LL;
            v48 = 4LL;
            LOBYTE(v1) = EtwWrite(PopDiagHandle, &POP_ETW_EVENT_PERFTRACK_HIBERNATE, 0LL, 3u, &UserData);
            v14 = dword_1402DE668;
            if ( !dword_1402DE668 )
              return (char)v1;
            PopComputeDerivedHiberStats(&qword_1402DE620, v3, &v30);
            v34 = v24;
            v35 = (unsigned int)qword_1402DE7C8 >> 8;
            if ( (unsigned __int64)qword_1402DE7C8 >> 8 >= 0xFFFF )
              v35 = -1;
            v36 = (unsigned int)qword_1402DE7D0 >> 8;
            if ( (unsigned __int64)qword_1402DE7D0 >> 8 >= 0xFFFF )
              v36 = -1;
            v37 = qword_1402DE678 / v3;
            if ( qword_1402DE678 / v3 >= 0xFFFF )
              v37 = -1;
            v38 = qword_1402DE758 / v3;
            if ( qword_1402DE758 / v3 >= 0xFFFF )
              v38 = -1;
            v39 = v14;
            if ( v14 >= 0xFFFF )
              v39 = -1;
            v40 = dword_1402DE798;
            if ( (unsigned int)dword_1402DE798 >= 0xFFFF )
              v40 = -1;
            v15 = HIDWORD(v30);
            if ( HIDWORD(v30) > 0x3FF )
              v15 = 1023;
            v16 = (v41 ^ v15) & 0x3FF ^ v41;
            v17 = v31;
            if ( v31 > 0x1FF )
              v17 = 511;
            v18 = (v16 ^ (v17 << 10)) & 0x7FC00 ^ v16;
            v19 = (qword_1402DE6F0 - qword_1402DE720) / v3;
            if ( v19 > 0x1FFF )
              LODWORD(v19) = 0x1FFF;
            v41 = v18 & 0x7FFFF | ((_DWORD)v19 << 19);
            v42 = (unsigned int)qword_1402DE7A8 >> 8;
            if ( (unsigned __int64)qword_1402DE7A8 >> 8 >= 0xFFFF )
              v42 = -1;
            v43 = (unsigned int)qword_1402DE7B0 >> 8;
            if ( (unsigned __int64)qword_1402DE7B0 >> 8 >= 0xFFFF )
              v43 = -1;
            v20 = (union _EVENT_DATA_DESCRIPTOR::$535316677C6A15A6ECBA40D88E1D787B *)&UserData.Reserved;
            v21 = 6LL;
            v22 = &v34;
            do
            {
              *(_QWORD *)&v20[-3].Reserved = v22++;
              *(_QWORD *)&v20[-1].Reserved = 4LL;
              v20 += 4;
              --v21;
            }
            while ( v21 );
            v13 = &POP_ETW_EVENT_PERFTRACK_RESUME_FROM_HIBERNATE;
          }
          LOBYTE(v1) = EtwWrite(PopDiagHandle, v13, 0LL, 6u, &UserData);
        }
      }
    }
  }
  return (char)v1;
}
