/*
 * XREFs of AlpcpFlushMessagesPort @ 0x14047EE8C
 * Callers:
 *     AlpcpDoPortCleanup @ 0x14047EB1C (AlpcpDoPortCleanup.c)
 * Callees:
 *     ExfAcquirePushLockSharedEx @ 0x140020AB0 (ExfAcquirePushLockSharedEx.c)
 *     ExfReleasePushLockShared @ 0x1400309E0 (ExfReleasePushLockShared.c)
 *     KeAbPostRelease @ 0x140042090 (KeAbPostRelease.c)
 *     ObfDereferenceObject @ 0x140042920 (ObfDereferenceObject.c)
 *     KeAbPreAcquire @ 0x140042DA0 (KeAbPreAcquire.c)
 *     ObReferenceObjectSafe @ 0x1400C8410 (ObReferenceObjectSafe.c)
 *     AlpcpFlushQueue @ 0x14047F078 (AlpcpFlushQueue.c)
 *     AlpcpFlushCancelQueue @ 0x14047F26C (AlpcpFlushCancelQueue.c)
 *     AlpcpFlushMessagesByRequestor @ 0x1404BB744 (AlpcpFlushMessagesByRequestor.c)
 */

void __fastcall AlpcpFlushMessagesPort(__int64 a1)
{
  __int64 *v2; // rsi
  __int64 v3; // rbx
  __int64 v4; // rbx
  __int64 v5; // rdi

  AlpcpFlushQueue(a1, a1 + 144, 1LL);
  AlpcpFlushQueue(a1, a1 + 160, 2LL);
  AlpcpFlushQueue(a1, a1 + 184, 3LL);
  AlpcpFlushQueue(a1, a1 + 208, 4LL);
  AlpcpFlushCancelQueue(a1, 0LL);
  if ( (*(_BYTE *)(a1 + 416) & 6) == 6 )
  {
    v2 = *(__int64 **)(a1 + 16);
    v3 = KeAbPreAcquire((ULONG_PTR)(v2 - 2), 0LL, 0LL);
    if ( _InterlockedCompareExchange64(v2 - 2, 17LL, 0LL) )
      ExfAcquirePushLockSharedEx((unsigned __int64 *)v2 - 2, v3, (ULONG_PTR)(v2 - 2));
    if ( v3 )
      *(_BYTE *)(v3 + 26) |= 1u;
    v4 = *v2;
    v5 = v2[2];
    if ( *v2 )
      v4 &= -(__int64)(ObReferenceObjectSafe(*v2) != 0);
    if ( v5 )
      v5 &= -(__int64)(ObReferenceObjectSafe(v5) != 0);
    if ( _InterlockedCompareExchange64(v2 - 2, 0LL, 17LL) != 17 )
      ExfReleasePushLockShared(v2 - 2);
    KeAbPostRelease((ULONG_PTR)(v2 - 2));
    if ( v4 )
    {
      if ( (*(_DWORD *)(v4 + 416) & 0x40) == 0 && v5 && (*(_DWORD *)(v5 + 416) & 8) == 0 )
      {
        AlpcpFlushMessagesByRequestor(v5, v4, v4 + 144, 1LL);
        AlpcpFlushMessagesByRequestor(v5, v4, v4 + 160, 2LL);
        if ( (*(_DWORD *)(a1 + 416) & 0x80u) == 0 )
          AlpcpFlushMessagesByRequestor(v5, v4, v4 + 184, 3LL);
        AlpcpFlushMessagesByRequestor(v5, v4, v4 + 208, 4LL);
        AlpcpFlushCancelQueue(v4, a1);
      }
      ObfDereferenceObject((PVOID)v4);
    }
    if ( v5 )
      ObfDereferenceObject((PVOID)v5);
  }
}
