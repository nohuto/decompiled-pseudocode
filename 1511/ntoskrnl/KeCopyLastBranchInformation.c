/*
 * XREFs of KeCopyLastBranchInformation @ 0x140028780
 * Callers:
 *     KeContextFromKframes @ 0x140023314 (KeContextFromKframes.c)
 *     PspGetContext @ 0x1403EEB58 (PspGetContext.c)
 *     KiCaptureUmsThreadContext @ 0x14061C814 (KiCaptureUmsThreadContext.c)
 * Callees:
 *     memset @ 0x140166CC0 (memset.c)
 */

unsigned __int64 __fastcall KeCopyLastBranchInformation(__int64 a1, __int64 a2)
{
  unsigned __int64 result; // rax
  _QWORD *v4; // rcx
  __int64 v5; // rdx

  if ( (*(_BYTE *)(a2 + 368) & 1) != 0 )
  {
    result = *(unsigned int *)(a2 + 256);
    if ( (result & 0x100) == 0 )
      return (unsigned __int64)memset((void *)(a1 + 1200), 0, 0x20uLL);
  }
  else
  {
    result = (unsigned __int64)KeGetCurrentPrcb();
    if ( (*(_DWORD *)(result + 136) & 0x100LL) == 0 )
      return (unsigned __int64)memset((void *)(a1 + 1200), 0, 0x20uLL);
  }
  v4 = (_QWORD *)(a1 + 1200);
  *(_OWORD *)(a1 + 1200) = *(_OWORD *)(a2 + 272);
  *(_OWORD *)(a1 + 1216) = *(_OWORD *)(a2 + 288);
  if ( (*(_BYTE *)(a2 + 368) & 1) != 0 )
  {
    v5 = 4LL;
    do
    {
      result = *v4 & 0x7FFFFFFFFFFFFFFFLL;
      if ( result > (unsigned __int64)MmHighestUserAddress )
        *v4 = 0LL;
      ++v4;
      --v5;
    }
    while ( v5 );
  }
  return result;
}
