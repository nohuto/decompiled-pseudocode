/*
 * XREFs of MiLockDynamicMemoryExclusive @ 0x140127060
 * Callers:
...
void __fastcall MiLockDynamicMemoryExclusive(__int64 a1, __int64 a2)
{
  ...
  if ( (KeGetPcr()->Prcb.DpcRequestSummary & 0x10001) != 0 )
    KeBugCheckEx(0x192u, (ULONG_PTR)CurrentThread, a1 + 216, KeGetCurrentIrql(), 0LL);
  ...
  if ( !CurrentThread->AbEntrySummary )
  {
    if ( !CurrentThread->AbOrphanedEntrySummary )
    {
      ...
      if ( (WORD2(PerfGlobalGroupMask) & 0x200) != 0 )
        EtwTraceAutoBoostEntryExhaustion(CurrentThread, a1 + 216);
      goto LABEL_23;
    }
