/*
 * XREFs of EtwpEnableDisableSpecialGuids @ 0x1404CE8C8
 * Callers:
 *     EtwpEnableGuid @ 0x1404CD4B0 (EtwpEnableGuid.c)
 * Callees:
 *     PsIsCurrentThreadInServerSilo @ 0x140079E5C (PsIsCurrentThreadInServerSilo.c)
 *     EtwpCheckGuidAccess @ 0x1404CEC4C (EtwpCheckGuidAccess.c)
 *     WmiTraceRundownNotify @ 0x140509B18 (WmiTraceRundownNotify.c)
 *     EtwpEnableDisableUMGL @ 0x14065FE08 (EtwpEnableDisableUMGL.c)
 *     EtwpCheckGuidAccessAndDoRundown @ 0x1406651C8 (EtwpCheckGuidAccessAndDoRundown.c)
 *     EtwpCheckLoggerAccessAndDoRundown @ 0x140665298 (EtwpCheckLoggerAccessAndDoRundown.c)
 */

__int64 __fastcall EtwpEnableDisableSpecialGuids(
        __int64 *a1,
        unsigned __int16 a2,
        int a3,
        int a4,
        __int64 a5,
        _BYTE *a6)
{
  char v6; // bp
  __int64 v10; // r9
  unsigned int v11; // esi
  char v12; // bl
  unsigned __int8 v13; // r14
  unsigned int v14; // edx
  _UNKNOWN **v15; // r8
  __int64 v16; // rax
  __int64 v17; // rcx
  unsigned __int64 v18; // rcx
  __int64 v20; // r8
  unsigned int v21; // ebx
  __int64 v22; // r8
  int v23; // edi
  int v24; // edi
  int v25; // edi

  v6 = 1;
  v10 = 3221226021LL;
  v11 = 2;
  *a6 = 1;
  if ( a3 == 2 )
  {
    v20 = SystemTraceControlGuid - *a1;
    if ( SystemTraceControlGuid == *a1 )
      v20 = 0x3969A8086000829ALL - a1[1];
    v21 = 0;
    if ( v20 )
    {
LABEL_12:
      *a6 = 0;
      return (unsigned int)v10;
    }
    if ( a4 != 1 )
    {
      if ( a4 != 2 )
        return v21;
      v6 = 0;
    }
    LOBYTE(v10) = v6;
    return (unsigned int)EtwpCheckGuidAccessAndDoRundown(a1, a2, a5, v10);
  }
  v12 = 0;
  if ( a3 == 1 )
  {
    v13 = 1;
  }
  else
  {
    if ( a3 )
      goto LABEL_12;
    v13 = 0;
  }
  v14 = 0;
  v15 = &EtwpUmglProviders;
  do
  {
    v16 = *a1;
    v17 = *(_QWORD *)*v15 - *a1;
    if ( !v17 )
      v17 = *((_QWORD *)*v15 + 1) - a1[1];
    if ( !v17 )
    {
      LOBYTE(v15) = a4;
      return EtwpEnableDisableUMGL(v13, a2, v15, *((unsigned __int8 *)&EtwpUmglProviders + 16 * v14 + 8));
    }
    ++v14;
    v15 += 2;
  }
  while ( v14 < 0xA );
  v18 = KernelRundownGuid - v16;
  if ( KernelRundownGuid == v16 )
    v18 = 0xCDF584518E9C7793uLL - a1[1];
  if ( v18 )
    goto LABEL_12;
  if ( PsIsCurrentThreadInServerSilo() )
  {
    LODWORD(v10) = -1073741727;
    return (unsigned int)v10;
  }
  if ( v13 != 1 )
  {
    LODWORD(v10) = 0;
    return (unsigned int)v10;
  }
  v23 = a4 - 1;
  if ( !v23 )
  {
    v11 = (unsigned __int16)EtwpSystemLogger[0];
    goto LABEL_37;
  }
  v24 = v23 - 1;
  if ( !v24 )
  {
    v11 = (unsigned __int16)EtwpSystemLogger[0];
    goto LABEL_38;
  }
  v25 = v24 - 2;
  if ( !v25 )
  {
LABEL_37:
    v12 = 1;
    goto LABEL_38;
  }
  if ( v25 != 12 )
  {
LABEL_38:
    LOBYTE(v22) = v12;
    LODWORD(v10) = EtwpCheckLoggerAccessAndDoRundown(v11, a2, v22);
    return (unsigned int)v10;
  }
  LODWORD(v10) = EtwpCheckGuidAccess(&FileProvGuid, 128LL, 0LL);
  if ( (int)v10 >= 0 )
  {
    LODWORD(v10) = WmiTraceRundownNotify(a2);
    if ( (int)v10 >= 0 )
      LODWORD(v10) = 0;
  }
  return (unsigned int)v10;
}
