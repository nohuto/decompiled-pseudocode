/*
 * XREFs of ExpLockHandleTableExclusive @ 0x14009646C
 * Callers:
...
void __fastcall ExpLockHandleTableExclusive(__int64 a1, __int64 a2)
{
  ULONG_PTR v2; // rsi
  ...
  if ( (KeGetPcr()->Prcb.DpcRequestSummary & 0x10001) != 0 )
    KeBugCheckEx(0x192u, (ULONG_PTR)CurrentThread, a1 + 56, KeGetCurrentIrql(), 0LL);
  ...
  if ( !CurrentThread->AbEntrySummary )
  {
    if ( !CurrentThread->AbOrphanedEntrySummary )
    {
      ...
      if ( (WORD2(PerfGlobalGroupMask) & 0x200) != 0 )
        EtwTraceAutoBoostEntryExhaustion(CurrentThread, a1 + 56);
      goto LABEL_22;
    }
