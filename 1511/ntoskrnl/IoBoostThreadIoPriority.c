/*
 * XREFs of IoBoostThreadIoPriority @ 0x1401028BC
 * Callers:
 *     ExAcquireResourceExclusiveLite @ 0x14003F890 (ExAcquireResourceExclusiveLite.c)
 *     ExpBoostIoAfterAcquire @ 0x14007967C (ExpBoostIoAfterAcquire.c)
 *     ExpApplyPriorityBoost @ 0x14007EC80 (ExpApplyPriorityBoost.c)
 *     FsRtlpDoBoost @ 0x1400C93A0 (FsRtlpDoBoost.c)
 *     CcBoostLowPriorityWorkerThread @ 0x1400E7D3C (CcBoostLowPriorityWorkerThread.c)
 *     FsRtlpWaitForIoAtEof @ 0x140101EB4 (FsRtlpWaitForIoAtEof.c)
 *     KiAbThreadBoostIoPriority @ 0x1401027C4 (KiAbThreadBoostIoPriority.c)
 *     MiModifiedPageWriter @ 0x1401384BC (MiModifiedPageWriter.c)
 *     IoBoostThreadIo @ 0x1401BABF8 (IoBoostThreadIo.c)
 *     NtSetInformationThread @ 0x140425860 (NtSetInformationThread.c)
 *     NtSetInformationProcess @ 0x1404507B0 (NtSetInformationProcess.c)
 *     CmpBoostActiveHiveWriter @ 0x1404C5F60 (CmpBoostActiveHiveWriter.c)
 *     PspBoostJobIoPriorityCallback @ 0x1404E23A0 (PspBoostJobIoPriorityCallback.c)
 * Callees:
 *     KeReleaseSpinLock @ 0x14002D960 (KeReleaseSpinLock.c)
 *     ExDereferenceCallBackBlock @ 0x1400384D0 (ExDereferenceCallBackBlock.c)
 *     ExReferenceCallBackBlock @ 0x140038510 (ExReferenceCallBackBlock.c)
 *     KeAcquireSpinLockRaiseToDpc @ 0x140038DA0 (KeAcquireSpinLockRaiseToDpc.c)
 *     KxReleaseQueuedSpinLock @ 0x140040BF0 (KxReleaseQueuedSpinLock.c)
 *     ObfReferenceObjectWithTag @ 0x140042AE0 (ObfReferenceObjectWithTag.c)
 *     KeAcquireQueuedSpinLock @ 0x1400B0A70 (KeAcquireQueuedSpinLock.c)
 *     KeReleaseQueuedSpinLock @ 0x1400B0AE0 (KeReleaseQueuedSpinLock.c)
 *     KeAcquireQueuedSpinLockAtDpcLevel @ 0x140102D74 (KeAcquireQueuedSpinLockAtDpcLevel.c)
 *     __security_check_cookie @ 0x140143620 (__security_check_cookie.c)
 *     ExAllocatePoolWithTag @ 0x140238380 (ExAllocatePoolWithTag.c)
 *     ExFreePoolWithTag @ 0x1402391D0 (ExFreePoolWithTag.c)
 */

void __fastcall IoBoostThreadIoPriority(KSPIN_LOCK *a1, int a2, int a3)
{
  char v5; // si
  KSPIN_LOCK *v6; // rdi
  KIRQL v7; // al
  __int64 v8; // r15
  __int64 v9; // rdi
  struct _EX_RUNDOWN_REF *v10; // rax
  unsigned int v11; // r14d
  KIRQL v12; // al
  KSPIN_LOCK v13; // rdi
  char v14; // cl
  unsigned __int64 v15; // rsi
  void *v16; // rsi
  unsigned __int16 v17; // r15
  unsigned __int16 v18; // si
  char *v19; // r15
  __int64 v20; // r12
  unsigned __int16 j; // di
  _QWORD *v22; // rsi
  __int64 v23; // rsi
  __int64 v24; // rdi
  struct _EX_RUNDOWN_REF *v25; // rdx
  KIRQL v26; // al
  __int64 v27; // rdi
  __int64 v28; // rcx
  __int64 i; // rdx
  __int64 v30; // rdx
  _QWORD *v31; // rax
  _QWORD *PoolWithTag; // rax
  KIRQL v33; // [rsp+28h] [rbp-E0h]
  KIRQL v34; // [rsp+28h] [rbp-E0h]
  char v35; // [rsp+29h] [rbp-DFh]
  void *v36; // [rsp+30h] [rbp-D8h]
  char *v37; // [rsp+30h] [rbp-D8h]
  _QWORD v40[10]; // [rsp+48h] [rbp-C0h] BYREF
  _QWORD *v41; // [rsp+98h] [rbp-70h]
  _QWORD v42[16]; // [rsp+A8h] [rbp-60h]

  v5 = 0;
  if ( a3 < 0 )
  {
    v5 = 1;
  }
  else
  {
    v6 = a1 + 229;
    v7 = KeAcquireSpinLockRaiseToDpc(a1 + 229);
    if ( (KSPIN_LOCK *)a1[204] == a1 + 204 )
    {
      if ( (a3 & 0x40000000) == 0 )
      {
        KeReleaseSpinLock(v6, v7);
        return;
      }
      v5 = 1;
    }
    KeReleaseSpinLock(v6, v7);
  }
  v8 = 8LL;
  v9 = 0LL;
  do
  {
    v10 = ExReferenceCallBackBlock((signed __int64 *)((char *)&IopUpdatePriorityCallbackRoutine + v9 * 8));
    v42[v9] = v10;
    if ( v10 )
      v42[v9 + 8] = v10[2].Count;
    ++v9;
    --v8;
  }
  while ( v8 );
  v11 = 0;
  if ( v5 != 1 )
  {
    v36 = 0LL;
    v12 = KeAcquireSpinLockRaiseToDpc(a1 + 229);
    v13 = a1[204];
    v33 = v12;
    if ( (KSPIN_LOCK *)v13 != a1 + 204 )
    {
      while ( 1 )
      {
        v14 = *(_BYTE *)(v13 + 34);
        if ( *(char *)(v13 + 35) > v14 )
          goto LABEL_18;
        v15 = *(_QWORD *)(v13 + 152);
        if ( v15 >= v13 + 8 * (9LL * v14 + 22) )
          goto LABEL_18;
        v16 = *(void **)(v15 + 40);
        if ( !v16 )
          goto LABEL_18;
        v17 = 0;
        while ( 1 )
        {
          if ( v42[v17] )
          {
            v35 = 0;
            KeAcquireQueuedSpinLockAtDpcLevel((char *)KeGetPcr()->NtTib.ArbitraryUserPointer + 160);
            for ( i = *(_QWORD *)(*(_QWORD *)(v42[v17 + 8] + 32LL) + 8LL); i; i = *(_QWORD *)(i + 16) )
            {
              if ( (void *)i == v16 && (*(_DWORD *)(i + 48) & 0x1000000) != 0 )
              {
                v35 = 1;
                break;
              }
            }
            KxReleaseQueuedSpinLock((volatile signed __int64 **)KeGetPcr()->NtTib.ArbitraryUserPointer + 20);
            if ( v35 == 1 && v36 != v16 )
              break;
          }
          if ( ++v17 >= 8u )
            goto LABEL_18;
        }
        v36 = v16;
        if ( v11 < 2 )
          break;
        PoolWithTag = ExAllocatePoolWithTag(NonPagedPoolNx, 0x30uLL, 0x43426F49u);
        if ( PoolWithTag )
        {
          PoolWithTag[4] = v41;
          v41 = PoolWithTag;
          *((_WORD *)PoolWithTag + 20) = v17;
          PoolWithTag[1] = v16;
          PoolWithTag[2] = a1;
          *PoolWithTag = v42[v17];
          *((_DWORD *)PoolWithTag + 6) = a2;
LABEL_55:
          ObfReferenceObjectWithTag(v16, 0x746C6644u);
          ObfReferenceObjectWithTag(a1, 0x746C6644u);
          ++v11;
          if ( (*(_DWORD *)(v13 - 16) & 2) != 0 )
            ++IoBoostedPagingIrpCount;
          else
            ++IoBoostedThreadedIrpCount;
        }
LABEL_18:
        v13 = *(_QWORD *)v13;
        if ( (KSPIN_LOCK *)v13 == a1 + 204 )
        {
          v12 = v33;
          goto LABEL_20;
        }
      }
      v30 = 6LL * v11;
      v40[v30] = v42[v17];
      LODWORD(v40[v30 + 3]) = a2;
      LOWORD(v40[v30 + 5]) = v17;
      v40[v30 + 1] = v16;
      v40[v30 + 2] = a1;
      v40[v30 + 4] = 0LL;
      goto LABEL_55;
    }
LABEL_20:
    KeReleaseSpinLock(a1 + 229, v12);
    if ( v11 )
      goto LABEL_26;
  }
  v18 = 0;
  v37 = 0LL;
  v19 = 0LL;
  do
  {
    v20 = *(_QWORD *)((char *)v42 + (_QWORD)v19);
    if ( !v20 )
      goto LABEL_23;
    v26 = KeAcquireQueuedSpinLock(0xAuLL);
    v34 = v26;
    v27 = *(_QWORD *)(*(_QWORD *)(*(_QWORD *)((char *)&v42[8] + (_QWORD)v19) + 32LL) + 8LL);
    if ( !v27 )
      goto LABEL_44;
    while ( (*(_DWORD *)(v27 + 48) & 0x1000000) == 0 )
    {
LABEL_42:
      v27 = *(_QWORD *)(v27 + 16);
      if ( !v27 )
        goto LABEL_43;
    }
    if ( v11 < 2 )
    {
      v28 = 6LL * v11;
      v40[v28 + 4] = 0LL;
      v40[v28] = v20;
      LOWORD(v40[v28 + 5]) = v18;
      v40[v28 + 1] = v27;
      v40[v28 + 2] = a1;
      LODWORD(v40[v28 + 3]) = a2;
LABEL_41:
      ObfReferenceObjectWithTag((PVOID)v27, 0x746C6644u);
      ObfReferenceObjectWithTag(a1, 0x746C6644u);
      ++v11;
      goto LABEL_42;
    }
    v31 = ExAllocatePoolWithTag(NonPagedPoolNx, 0x30uLL, 0x43426F49u);
    if ( v31 )
    {
      v31[4] = v41;
      v41 = v31;
      *v31 = v20;
      *((_WORD *)v31 + 20) = v18;
      v31[1] = v27;
      v31[2] = a1;
      *((_DWORD *)v31 + 6) = a2;
      goto LABEL_41;
    }
LABEL_43:
    v19 = v37;
    v26 = v34;
LABEL_44:
    KeReleaseQueuedSpinLock(0xAuLL, v26);
LABEL_23:
    v19 += 8;
    ++v18;
    v37 = v19;
  }
  while ( v18 < 8u );
  if ( v11 )
    ++IoBlanketBoostCount;
LABEL_26:
  for ( j = 0; j < v11; ++j )
  {
    if ( j >= 2u )
    {
      v22 = v41;
      v41 = (_QWORD *)v41[4];
    }
    else
    {
      v22 = &v40[6 * j];
    }
    (*(void (__fastcall **)(_QWORD, _QWORD *, _QWORD))(*v22 + 8LL))(*v22, v22 + 1, 0LL);
    if ( j >= 2u )
      ExFreePoolWithTag(v22, 0);
  }
  v23 = 8LL;
  v24 = 0LL;
  do
  {
    v25 = (struct _EX_RUNDOWN_REF *)v42[v24];
    if ( v25 )
      ExDereferenceCallBackBlock((signed __int64 *)((char *)&IopUpdatePriorityCallbackRoutine + v24 * 8), v25);
    ++v24;
    --v23;
  }
  while ( v23 );
}
