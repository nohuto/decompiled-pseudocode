/*
 * XREFs of Control_Transfer_Map @ 0x1C00020E8
 * Callers:
 *     Control_MapTransfer @ 0x1C0001C24 (Control_MapTransfer.c)
 * Callees:
 *     WPP_RECORDER_SF_DDD @ 0x1C0008EC4 (WPP_RECORDER_SF_DDD.c)
 *     Control_Transfer_MapIntoRing @ 0x1C000B950 (Control_Transfer_MapIntoRing.c)
 *     _guard_dispatch_icall_nop @ 0x1C0010C80 (_guard_dispatch_icall_nop.c)
 */

__int64 __fastcall Control_Transfer_Map(__int64 a1)
{
  __int64 v1; // r15
  int v2; // ebx
  __int64 v4; // r14
  _QWORD *v5; // r8
  __int64 v6; // rcx
  __int64 v7; // rax
  __int16 v8; // dx
  __int64 v9; // rsi
  __int64 v10; // rbx
  KIRQL v11; // di
  int v12; // edx
  bool v14; // [rsp+38h] [rbp-30h]

  v1 = *(_QWORD *)(a1 + 296);
  v2 = 0;
  v4 = *(_QWORD *)(v1 + 32);
  if ( *(_WORD *)(v4 + 2) == 50 && *(_DWORD *)(v4 + 56) )
  {
    *(_DWORD *)(v1 + 100) = 1;
    (*(void (__fastcall **)(PWDF_DRIVER_GLOBALS, _QWORD, __int64))(WdfFunctions_01015 + 2552))(
      WdfDriverGlobals,
      *(_QWORD *)(a1 + 288),
      -10000LL * *(unsigned int *)(v4 + 56));
  }
  v5 = *(_QWORD **)(a1 + 40);
  v6 = *(unsigned int *)(v4 + 64);
  v7 = v5[12];
  if ( (unsigned int)v6 >= *(_DWORD *)(v7 + 48) )
    v8 = 0;
  else
    v8 = *(_WORD *)(*(_QWORD *)(v7 + 56) + 2 * v6);
  *(_WORD *)(a1 + 104) = v8;
  if ( *(_DWORD *)(v1 + 48) == 3 )
  {
    v9 = *(_QWORD *)(v5[11] + 80LL);
    v10 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, _QWORD))(WdfFunctions_01015 + 248))(WdfDriverGlobals, *v5);
    v11 = KfRaiseIrql(2u);
    v14 = (*(_BYTE *)(v4 + 32) & 1) == 0;
    v2 = (*(__int64 (__fastcall **)(__int64, __int64, _QWORD, _QWORD, _DWORD, __int64 (__fastcall *)(), __int64, bool))(*(_QWORD *)(v9 + 8) + 88LL))(
           v9,
           v10,
           *(_QWORD *)(v1 + 56),
           *(_QWORD *)(*(_QWORD *)(v1 + 56) + 32LL) + *(unsigned int *)(*(_QWORD *)(v1 + 56) + 44LL),
           *(_DWORD *)(v1 + 88),
           Control_EvtDmaCallback,
           a1,
           v14);
    KeLowerIrql(v11);
    if ( v2 < 0 )
    {
      LOBYTE(v12) = 2;
      WPP_RECORDER_SF_DDD(
        *(_QWORD *)(*(_QWORD *)(a1 + 56) + 80LL),
        v12,
        13,
        36,
        (__int64)&WPP_ee7759932d876114f80ff23edcd4b288_Traceguids,
        *(_BYTE *)(*(_QWORD *)(a1 + 48) + 135LL),
        *(_DWORD *)(*(_QWORD *)(a1 + 56) + 144LL),
        v2);
    }
  }
  else
  {
    Control_Transfer_MapIntoRing(a1);
  }
  return (unsigned int)v2;
}
