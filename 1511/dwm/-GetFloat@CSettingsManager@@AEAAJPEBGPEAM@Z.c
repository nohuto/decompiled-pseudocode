/*
 * XREFs of ?GetFloat@CSettingsManager@@AEAAJPEBGPEAM@Z @ 0x140002580
 * Callers:
 *     ?GetPreferenceFloat@CSettingsManager@@UEAAJPEBGPEAM@Z @ 0x140002920 (-GetPreferenceFloat@CSettingsManager@@UEAAJPEBGPEAM@Z.c)
 * Callees:
 *     __security_check_cookie @ 0x140003130 (__security_check_cookie.c)
 */

__int64 __fastcall CSettingsManager::GetFloat(CSettingsManager *this, const unsigned __int16 *a2, float *a3)
{
  RTL_SRWLOCK *v5; // rdi
  LSTATUS Value; // ebx
  float v9; // xmm1_4
  DWORD cbData[4]; // [rsp+40h] [rbp-88h] BYREF
  wchar_t Data[31]; // [rsp+50h] [rbp-78h] BYREF
  __int16 v12; // [rsp+8Eh] [rbp-3Ah]

  v5 = (RTL_SRWLOCK *)((char *)this + 40);
  cbData[0] = 64;
  AcquireSRWLockShared((PSRWLOCK)this + 5);
  Value = RegQueryValueExW(*((HKEY *)this + 1), a2, 0LL, 0LL, (LPBYTE)Data, cbData);
  if ( !Value
    || (cbData[0] = 64,
        (Value = RegGetValueW(HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\Windows\\DWM", a2, 2u, 0LL, Data, cbData)) == 0) )
  {
    v12 = 0;
    v9 = _wtof(Data);
    *a3 = v9;
  }
  ReleaseSRWLockShared(v5);
  if ( Value <= 0 )
    return (unsigned int)Value;
  else
    return (unsigned __int16)Value | 0x80070000;
}
