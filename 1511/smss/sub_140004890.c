/*
 * XREFs of sub_140004890 @ 0x140004890
 * Callers:
 *     sub_140003500 @ 0x140003500 (sub_140003500.c)
 *     sub_140003D54 @ 0x140003D54 (sub_140003D54.c)
 *     sub_140003E30 @ 0x140003E30 (sub_140003E30.c)
 *     sub_140004800 @ 0x140004800 (sub_140004800.c)
 *     sub_140004D90 @ 0x140004D90 (sub_140004D90.c)
 * Callees:
 *     __security_check_cookie @ 0x14000BFC0 (__security_check_cookie.c)
 */

BOOLEAN __fastcall sub_140004890(char *BaseAddress)
{
  signed __int32 v2; // eax
  bool v3; // cc
  BOOLEAN result; // al
  void *v5; // rdi
  int v6; // eax
  __int128 v7; // xmm1
  _OWORD v8[2]; // [rsp+40h] [rbp-168h] BYREF
  __int64 v9; // [rsp+60h] [rbp-148h]
  int v10; // [rsp+68h] [rbp-140h]
  int v11; // [rsp+6Ch] [rbp-13Ch]
  int v12; // [rsp+70h] [rbp-138h]

  v2 = _InterlockedExchangeAdd((volatile signed __int32 *)BaseAddress + 1, 0xFFFFFFFF);
  v3 = v2 <= 1;
  result = v2 - 1;
  if ( v3 )
  {
    v5 = (void *)*((_QWORD *)BaseAddress + 6);
    if ( v5 )
    {
      if ( (*BaseAddress & 1) == 0 && *((_QWORD *)BaseAddress + 9) != qword_14001FCE8 )
      {
        v6 = *((_DWORD *)BaseAddress + 2);
        v7 = *((_OWORD *)BaseAddress + 5);
        v8[0] = *((_OWORD *)BaseAddress + 4);
        v9 = *((_QWORD *)BaseAddress + 12);
        v8[1] = v7;
        v10 = 5;
        v11 = -1073741823;
        v12 = v6;
        ((void (__fastcall *)(__int64, __int64, _OWORD *, _QWORD, _QWORD, _QWORD, _QWORD, _QWORD))NtAlpcSendWaitReceivePort)(
          qword_14001FCE0,
          0x10000LL,
          v8,
          0LL,
          0LL,
          0LL,
          0LL,
          0LL);
      }
      RtlAcquireSRWLockExclusive(&unk_140020908);
      RtlDeleteNoSplay((PRTL_SPLAY_LINKS)(BaseAddress + 104), &Root);
      RtlReleaseSRWLockExclusive(&unk_140020908);
      NtClose(v5);
    }
    NtClose(*((HANDLE *)BaseAddress + 4));
    NtClose(*((HANDLE *)BaseAddress + 5));
    NtUnmapViewOfSection((HANDLE)0xFFFFFFFFFFFFFFFFLL, *((PVOID *)BaseAddress + 2));
    NtClose(*((HANDLE *)BaseAddress + 3));
    return RtlFreeHeap(*(PVOID *)(*(_QWORD *)&KeGetPcr()->MajorVersion + 48LL), 0, BaseAddress);
  }
  return result;
}
