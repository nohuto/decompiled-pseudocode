/*
 * XREFs of sub_140002310 @ 0x140002310
 * Callers:
 *     sub_140001FB0 @ 0x140001FB0 (sub_140001FB0.c)
 * Callees:
 *     memcpy @ 0x14000C480 (memcpy.c)
 */

__int64 __fastcall sub_140002310(unsigned __int16 *a1, char a2, struct _UNICODE_STRING *a3)
{
  wint_t *v6; // r15
  int v7; // edi
  wint_t *i; // rbx
  __int64 j; // rbp
  WCHAR *Heap; // rax
  unsigned __int16 v11; // cx

  RtlInitUnicodeString(a3, 0LL);
  v6 = (wint_t *)*((_QWORD *)a1 + 1);
  if ( !*a1 )
    return 0LL;
  v7 = 0;
  do
  {
    if ( !iswctype(*v6, 8u) )
      break;
    ++v6;
    v7 += 2;
  }
  while ( (unsigned __int16)v7 < *a1 );
  if ( a2 )
  {
    j = (unsigned int)*a1 - 2 * v7;
    i = (wint_t *)((char *)v6 + j);
  }
  else
  {
    for ( i = v6; (unsigned __int16)v7 < *a1; v7 += 2 )
    {
      if ( iswctype(*i, 8u) )
        break;
      ++i;
    }
    for ( j = (unsigned int)((_DWORD)i - (_DWORD)v6); (unsigned __int16)v7 < *a1; v7 += 2 )
    {
      if ( !iswctype(*i, 8u) )
        break;
      ++i;
    }
  }
  if ( (_DWORD)j )
  {
    Heap = (WCHAR *)RtlAllocateHeap(*(PVOID *)(*(_QWORD *)&KeGetPcr()->MajorVersion + 48LL), Flags, j + 2);
    a3->Buffer = Heap;
    if ( !Heap )
      return 3221225495LL;
    a3->Length = j;
    a3->MaximumLength = j + 2;
    memcpy(Heap, v6, (unsigned int)j);
    a3->Buffer[(unsigned __int64)(unsigned int)j >> 1] = 0;
  }
  v11 = a1[4];
  *((_QWORD *)a1 + 1) = i;
  *a1 += v11 - (_WORD)i;
  a1[1] += v11 - (_WORD)i;
  return 0LL;
}
