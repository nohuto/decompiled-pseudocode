/*
 * XREFs of IcCopyInputStateToDeviceState @ 0x1C0069C64
 * Callers:
 *     IrqArbCommitAllocation @ 0x1C0069DA0 (IrqArbCommitAllocation.c)
 *     AcpiIrqLibSetupSciInterrupt @ 0x1C006D578 (AcpiIrqLibSetupSciInterrupt.c)
 * Callees:
 *     IrqArbGsivFromIrq @ 0x1C0069D80 (IrqArbGsivFromIrq.c)
 */

NTSTATUS __fastcall IcCopyInputStateToDeviceState(__int64 a1, int a2)
{
  struct _RTL_RANGE_LIST *v2; // rcx
  NTSTATUS result; // eax
  PRTL_RANGE i; // rcx
  _DWORD *UserData; // r8
  unsigned int v6; // eax
  __int64 v7; // r8
  __int64 j; // rdx
  __int64 v9; // rcx
  unsigned int v10; // ecx
  struct _RANGE_LIST_ITERATOR Iterator; // [rsp+20h] [rbp-28h] BYREF
  PRTL_RANGE Range; // [rsp+68h] [rbp+20h] BYREF

  if ( a2 == 1 )
    v2 = *(struct _RTL_RANGE_LIST **)(a1 + 48);
  else
    v2 = *(struct _RTL_RANGE_LIST **)(a1 + 40);
  result = RtlGetFirstRange(v2, &Iterator, &Range);
  for ( i = Range; Range; i = Range )
  {
    if ( i->Owner )
    {
      if ( (i->Attributes & 1) == 0 )
      {
        UserData = i->UserData;
        if ( UserData )
        {
          if ( (unsigned int)(UserData[2] - 1) <= 1 )
          {
            v6 = IrqArbGsivFromIrq(i->Start);
            for ( j = IcListHead; &IcListHead != (__int64 *)j; j = *(_QWORD *)j )
            {
              if ( *(int *)(j + 28) >= 0 )
              {
                v10 = *(_DWORD *)(j + 16);
                if ( v10 <= v6 && *(_DWORD *)(j + 20) >= v6 )
                {
                  v9 = 200LL * (v6 - v10);
                  *(_OWORD *)(v7 + 48) = *(_OWORD *)(v9 + j + 136);
                  *(_OWORD *)(v7 + 64) = *(_OWORD *)(v9 + j + 152);
                  *(_OWORD *)(v7 + 80) = *(_OWORD *)(v9 + j + 168);
                  *(_OWORD *)(v7 + 96) = *(_OWORD *)(v9 + j + 184);
                  *(_OWORD *)(v7 + 112) = *(_OWORD *)(v9 + j + 200);
                  *(_QWORD *)(v7 + 128) = *(_QWORD *)(v9 + j + 216);
                }
              }
            }
          }
        }
      }
    }
    result = RtlGetNextRange(&Iterator, &Range, 1u);
  }
  return result;
}
