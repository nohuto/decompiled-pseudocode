/*
 * XREFs of ACPIIoctlUnRegisterOpRegionHandler @ 0x1C003D5F8
 * Callers:
 *     ACPIIrpDispatchDeviceControl @ 0x1C00012D0 (ACPIIrpDispatchDeviceControl.c)
 * Callees:
 *     ACPIInternalGetDeviceExtension @ 0x1C0001538 (ACPIInternalGetDeviceExtension.c)
 *     UnRegisterOperationRegionHandler @ 0x1C007C56C (UnRegisterOperationRegionHandler.c)
 */

__int64 __fastcall ACPIIoctlUnRegisterOpRegionHandler(ULONG_PTR a1, IRP *a2, __int64 a3)
{
  unsigned int v3; // ebx
  __int64 v5; // rcx
  unsigned int v6; // ebx

  v3 = *(_DWORD *)(a3 + 16);
  v5 = *(_QWORD *)(ACPIInternalGetDeviceExtension(a1) + 704);
  if ( v3 >= 0x10 )
    v6 = UnRegisterOperationRegionHandler(v5, a2->AssociatedIrp.MasterIrp->MdlAddress);
  else
    v6 = -1073741820;
  a2->IoStatus.Information = 0LL;
  a2->IoStatus.Status = v6;
  IofCompleteRequest(a2, 0);
  return v6;
}
