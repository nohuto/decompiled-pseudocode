/*
 * XREFs of AcpiDiagTraceActiveCoolingDevicePower @ 0x1C0033D08
 * Callers:
...
void __fastcall AcpiDiagTraceActiveCoolingDevicePower(__int64 a1, __int64 a2, int a3, int a4, char a5)
{
  ...
  struct _UNICODE_STRING v11; // [rsp+40h] [rbp-71h] BYREF
  ...
  unsigned __int16 *Buffer; // [rsp+70h] [rbp-41h]
  ...
  unsigned __int16 *v23; // [rsp+B0h] [rbp-1h]
  ...
  if ( AcpiDiagHandle )
  {
    if ( EtwEventEnabled(AcpiDiagHandle, &ACPI_ETW_EVENT_ACTIVE_COOLING_DEVICE_POWER) )
    {
      if ( (int)ACPIAmliBuildObjectPathnameUnicode(*(__int64 **)(a1 + 704), (__int64)&DestinationString, 1) >= 0 )
      {
        ...
        if ( (int)ACPIAmliBuildObjectPathnameUnicode(v7, (__int64)&v11, 1) >= 0 )
        {
