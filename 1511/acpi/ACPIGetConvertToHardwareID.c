/*
 * XREFs of ACPIGetConvertToHardwareID @ 0x1C003BE08
 * Callers:
 *     ACPIGetWorkerForString @ 0x1C0003910 (ACPIGetWorkerForString.c)
 * Callees:
 *     ACPIGetConvertToHardwareIDWide @ 0x1C0003B88 (ACPIGetConvertToHardwareIDWide.c)
 *     ACPIWideStringToAnsiHelper @ 0x1C001E3C0 (ACPIWideStringToAnsiHelper.c)
 */

__int64 __fastcall ACPIGetConvertToHardwareID(_QWORD *a1, int a2, __int64 a3, int a4, char **a5, unsigned int *a6)
{
  int v6; // ebx
  unsigned int v7; // r11d
  unsigned int v9[6]; // [rsp+30h] [rbp-18h] BYREF

  v9[0] = 0;
  v6 = ACPIGetConvertToHardwareIDWide(a1, a2, a3, a4, a5, (int *)v9);
  if ( v6 >= 0 )
  {
    ACPIWideStringToAnsiHelper(*a5, v9[0]);
    if ( a6 )
      *a6 = v7 >> 1;
  }
  return (unsigned int)v6;
}
