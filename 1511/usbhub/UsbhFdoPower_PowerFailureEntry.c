/*
 * XREFs of UsbhFdoPower_PowerFailureEntry @ 0x1C0045C58
 * Callers:
 *     UsbhFdoS0IoComplete_Action @ 0x1C00012D0 (UsbhFdoS0IoComplete_Action.c)
 *     UsbhFdoD0PoComplete_Action @ 0x1C0001510 (UsbhFdoD0PoComplete_Action.c)
 *     UsbhFdoSxIoComplete_Action @ 0x1C00016B0 (UsbhFdoSxIoComplete_Action.c)
 * Callees:
 *     UsbhFdoUnblockAllPendedPdoD0Irps @ 0x1C0002DFC (UsbhFdoUnblockAllPendedPdoD0Irps.c)
 *     UsbhBusIoInvalidateDeviceRelations @ 0x1C0008EFC (UsbhBusIoInvalidateDeviceRelations.c)
 *     Log @ 0x1C0013250 (Log.c)
 *     FdoExt @ 0x1C00132D0 (FdoExt.c)
 *     UsbhReleaseFdoPwrLock @ 0x1C0020500 (UsbhReleaseFdoPwrLock.c)
 *     UsbhSetFdoPowerState @ 0x1C002058C (UsbhSetFdoPowerState.c)
 *     UsbhReleasePowerContext @ 0x1C00206A0 (UsbhReleasePowerContext.c)
 *     UsbhDisarmHubForWakeDetect @ 0x1C0025B78 (UsbhDisarmHubForWakeDetect.c)
 *     Usb_Disconnected @ 0x1C002765C (Usb_Disconnected.c)
 *     UsbhException @ 0x1C004EC30 (UsbhException.c)
 */

void __fastcall UsbhFdoPower_PowerFailureEntry(__int64 a1, __int64 a2, __int64 a3, __int64 a4)
{
  __int64 v5; // rsi
  _DWORD *v7; // rbx
  bool v8; // al
  __int64 v9; // r8
  __int64 v10; // r8
  __int64 v11; // r9
  __int64 v12; // r8
  __int64 v13; // r9
  int v14; // [rsp+48h] [rbp-10h]
  int v15; // [rsp+68h] [rbp+10h] BYREF

  v15 = a2;
  v5 = (int)a3;
  LOBYTE(v14) = 0;
  v7 = FdoExt(a1, a2, a3, a4);
  UsbhException(a1, 0, 85, (int)&v15, 4, v5, 0, usbfile_fdopwr_c, 1027, v14);
  v7[315] = v5;
  v8 = Usb_Disconnected(v5);
  v9 = 213LL;
  if ( !v8 )
    v9 = 212LL;
  UsbhSetFdoPowerState(a1, v5, v9, (unsigned int)v15);
  Log(a1, 16, 1934645025, v5, 0LL);
  UsbhFdoUnblockAllPendedPdoD0Irps(a1);
  UsbhReleaseFdoPwrLock(a1, *(_QWORD *)(a4 + 72), v10, v11);
  UsbhBusIoInvalidateDeviceRelations(a1, *(_QWORD *)(a4 + 72));
  UsbhReleasePowerContext(a1, a4, v12, v13);
  UsbhDisarmHubForWakeDetect(a1);
}
