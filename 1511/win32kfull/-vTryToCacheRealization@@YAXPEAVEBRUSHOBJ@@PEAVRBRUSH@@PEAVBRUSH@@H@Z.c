/*
 * XREFs of ?vTryToCacheRealization@@YAXPEAVEBRUSHOBJ@@PEAVRBRUSH@@PEAVBRUSH@@H@Z @ 0x1C00E73E0
 * Callers:
...
void __fastcall vTryToCacheRealization(struct EBRUSHOBJ *a1, struct RBRUSH *a2, struct BRUSH *a3, int a4)
{
  ...
  if ( G_fConsole
    || (*(_DWORD *)a3 & 0x800000LL) == 0
    || !*((_DWORD *)a2 + 3)
    || (v8 = 3LL * (unsigned __int16)*(_DWORD *)a3,
        ((*(_DWORD *)(gpentHmgr + 8 * v8 + 8) ^ (unsigned int)PsGetCurrentProcessId() & 0xFFFFFFFD) & 0xFFFFFFFE) == 0) )
  {
    *(_DWORD *)a2 = 1;
