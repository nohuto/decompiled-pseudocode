/*
 * XREFs of GreMakeFontDir @ 0x1C02B2BF0
 * Callers:
...
__int64 __fastcall GreMakeFontDir(int a1, __int64 a2, ULONG_PTR a3)
{
  ...
  _BYTE v19[8]; // [rsp+68h] [rbp-59h] BYREF
  unsigned __int64 v20; // [rsp+70h] [rbp-51h] BYREF
  ...
  v4 = 0;
  v8 = 0;
  if ( !qword_1C032F218 || !UmfdHostLifeTimeManager::EnsureUmfdHost() )
    return 0LL;
  v15 = v3;
  memset(iFile, 0, 120);
  ...
  if ( (unsigned int)EngMapFontFileFDInternal((struct _FILEVIEW *)iFile, &v18, &v22, 0) )
  {
    if ( (unsigned int)UmfdInsertFontFileViewForLookup(&v17, 1u) )
    {
      ...
      if ( v9 )
      {
        if ( v9 != 0xFFFFFFFFLL )
        {
          v11 = PDEVOBJ::QueryFont((PDEVOBJ *)&v15, 0LL, v9, 1u, &v20);
          v12 = v11;
          if ( v11 )
          {
            v8 = GreFillFontDir((__int64)v11, a1, a2);
            if ( *(_QWORD *)(v15 + 3056) )
              PDEVOBJ::Free((PDEVOBJ *)&v15, v12, v20);
          }
        }
        if ( v10 != 0xFFFFFFFFLL )
    ...
    if ( LODWORD(iFile[8]) )
    {
      AutoExclusiveUmfdFileViewLock::AutoExclusiveUmfdFileViewLock((AutoExclusiveUmfdFileViewLock *)v19);
