/*
 * XREFs of xxxDoScrollMenu @ 0x1C023395C
 * Callers:
 *     xxxRealDefWindowProc @ 0x1C006FB88 (xxxRealDefWindowProc.c)
 *     xxxSBWndProc @ 0x1C0234280 (xxxSBWndProc.c)
 * Callees:
 *     CalcSBStuff @ 0x1C0077960 (CalcSBStuff.c)
 *     GetAppCompatFlags @ 0x1C00F4660 (GetAppCompatFlags.c)
 *     xxxTrackPopupMenuEx @ 0x1C0134F0C (xxxTrackPopupMenuEx.c)
 *     GetWndSBDisableFlags @ 0x1C014513C (GetWndSBDisableFlags.c)
 *     __security_check_cookie @ 0x1C01512D0 (__security_check_cookie.c)
 *     safe_cast_fnid_to_PSBWND @ 0x1C01D3A34 (safe_cast_fnid_to_PSBWND.c)
 *     ?xxxDoScroll@@YAXPEAUtagWND@@0HHH@Z @ 0x1C023217C (-xxxDoScroll@@YAXPEAUtagWND@@0HHH@Z.c)
 *     ?xxxGetScrollMenu@@YAPEAUtagMENU@@PEAUtagWND@@H@Z @ 0x1C02323CC (-xxxGetScrollMenu@@YAPEAUtagMENU@@PEAUtagWND@@H@Z.c)
 *     SBCtlSetup @ 0x1C023389C (SBCtlSetup.c)
 *     xxxSBTrackInit @ 0x1C0233E10 (xxxSBTrackInit.c)
 */

void __fastcall xxxDoScrollMenu(struct tagWND *a1, struct tagWND *a2, unsigned int a3, unsigned __int64 a4)
{
  __int64 v8; // r15
  int v9; // ebp
  int v10; // r14d
  int v11; // eax
  struct tagMENU *ScrollMenu; // rax
  int v13; // ebp
  __int64 v14; // rdx
  __int64 v15; // rcx
  struct tagWND *v16; // rcx
  unsigned __int64 v17; // rdx
  unsigned __int16 v18; // [rsp+30h] [rbp-B8h]
  _QWORD v19[3]; // [rsp+38h] [rbp-B0h] BYREF
  _BYTE v20[64]; // [rsp+50h] [rbp-98h] BYREF

  v8 = safe_cast_fnid_to_PSBWND((__int64)a2);
  if ( (int)GetAppCompatFlags(0LL) >= 0 )
  {
    if ( a2 )
    {
      if ( !v8 )
        return;
      SBCtlSetup(v8);
      v9 = (__int16)a4 - *((_DWORD *)a2 + 28);
      v10 = SWORD1(a4) - *((_DWORD *)a2 + 29);
      v11 = *(_DWORD *)(v8 + 380);
    }
    else
    {
      CalcSBStuff((__int64)a1, (__int64)v20, a3);
      LOBYTE(v11) = GetWndSBDisableFlags((__int64)a1, a3);
      v9 = (__int16)a4 - *((_DWORD *)a1 + 28);
      v10 = SWORD1(a4) - *((_DWORD *)a1 + 29);
    }
    v18 = v9;
    if ( (v11 & 3) != 3 )
    {
      ScrollMenu = xxxGetScrollMenu(a1, a3);
      if ( ScrollMenu )
      {
        v19[0] = *(_QWORD *)(gptiCurrent + 368LL);
        *(_QWORD *)(gptiCurrent + 368LL) = v19;
        v19[1] = ScrollMenu;
        ++*((_DWORD *)ScrollMenu + 2);
        v13 = xxxTrackPopupMenuEx((__int64)ScrollMenu, 0x182u, (__int16)a4, SWORD1(a4), (__int64)a1, 0LL);
        ThreadUnlock1(v15, v14);
        if ( v13 )
        {
          if ( (unsigned __int8)v13 == 4 )
          {
            if ( a2 )
            {
              v16 = a2;
              v17 = v18 | (unsigned __int64)((unsigned __int16)v10 << 16);
            }
            else
            {
              v17 = a4;
              v16 = a1;
            }
            xxxSBTrackInit(v16, v17);
          }
          else
          {
            xxxDoScroll(a2, a1, (unsigned __int8)v13, 0, a3);
            xxxDoScroll(a2, a1, 8u, 0, a3);
          }
        }
      }
    }
  }
}
