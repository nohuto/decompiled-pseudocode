/*
 * XREFs of ?bSimpleStrokeAndFill@EPATHOBJ@@QEAAHKPEAVPDEVOBJ@@PEAVSURFACE@@PEAU_CLIPOBJ@@PEAU_XFORMOBJ@@PEAU_BRUSHOBJ@@PEAU_LINEATTRS@@4PEAU_POINTL@@KK@Z @ 0x1C027AEF0
 * Callers:
 *     ?bStrokeAndOrFill@EPATHOBJ@@QEAAHAEAVXDCOBJ@@PEAU_LINEATTRS@@PEAVEXFORMOBJ@@K@Z @ 0x1C00161E0 (-bStrokeAndOrFill@EPATHOBJ@@QEAAHAEAVXDCOBJ@@PEAU_LINEATTRS@@PEAVEXFORMOBJ@@K@Z.c)
 * Callees:
 *     ?bSubOverflow@@YAHJJ@Z @ 0x1C0014178 (-bSubOverflow@@YAHJJ@Z.c)
 *     ?bSimpleFill@EPATHOBJ@@QEAAHKPEAVPDEVOBJ@@PEAVSURFACE@@PEAU_CLIPOBJ@@PEAU_BRUSHOBJ@@PEAU_POINTL@@KK@Z @ 0x1C0016688 (-bSimpleFill@EPATHOBJ@@QEAAHKPEAVPDEVOBJ@@PEAVSURFACE@@PEAU_CLIPOBJ@@PEAU_BRUSHOBJ@@PEAU_POINTL@.c)
 *     ?bSimpleStroke@EPATHOBJ@@QEAAHKPEAVPDEVOBJ@@PEAVSURFACE@@PEAU_CLIPOBJ@@PEAU_XFORMOBJ@@PEAU_BRUSHOBJ@@PEAU_POINTL@@PEAU_LINEATTRS@@K@Z @ 0x1C0130BC4 (-bSimpleStroke@EPATHOBJ@@QEAAHKPEAVPDEVOBJ@@PEAVSURFACE@@PEAU_CLIPOBJ@@PEAU_XFORMOBJ@@PEAU_BRUSH.c)
 *     ??1PRECOMPUTE@@QEAA@XZ @ 0x1C0132714 (--1PRECOMPUTE@@QEAA@XZ.c)
 *     ?vInit@PRECOMPUTE@@QEAAXPEAVSURFACE@@PEAVPDEVOBJ@@PEAVEPATHOBJ@@PEAU_CLIPOBJ@@PEAU_XFORMOBJ@@PEAU_LINEATTRS@@KKW4PRECOMPUTETYPE@@@Z @ 0x1C01327EC (-vInit@PRECOMPUTE@@QEAAXPEAVSURFACE@@PEAVPDEVOBJ@@PEAVEPATHOBJ@@PEAU_CLIPOBJ@@PEAU_XFORMOBJ@@PEA.c)
 *     _guard_dispatch_icall_nop @ 0x1C0152950 (_guard_dispatch_icall_nop.c)
 *     EngStrokeAndFillPath @ 0x1C027D410 (EngStrokeAndFillPath.c)
 */

BOOL __fastcall EPATHOBJ::bSimpleStrokeAndFill(
        PATHOBJ *ppo,
        char a2,
        struct PDEVOBJ *a3,
        struct SURFACE *a4,
        CLIPOBJ *pco,
        XFORMOBJ *pxo,
        BRUSHOBJ *pboStroke,
        LINEATTRS *plineattrs,
        BRUSHOBJ *pbo,
        struct _POINTL *pptlBrushOrg,
        MIX mixFill,
        FLONG flOptions)
{
  __m128i *v12; // r10
  int v13; // ebx
  __m128i *v16; // r10
  PDEVOBJ *v17; // r8
  __int64 v18; // r9
  __m128i *v19; // r10
  char v20; // r11
  int v22; // eax
  int v23; // edi
  MIX v24; // r14d
  int v25; // r13d
  __int64 (__fastcall *v26)(char *, PATHOBJ *, CLIPOBJ *, XFORMOBJ *, BRUSHOBJ *, LINEATTRS *, BRUSHOBJ *, struct _POINTL *, MIX, FLONG); // rax
  __int64 (__fastcall *v27)(char *, PATHOBJ *, CLIPOBJ *, XFORMOBJ *, BRUSHOBJ *, LINEATTRS *, BRUSHOBJ *, struct _POINTL *, MIX, FLONG); // rax
  MIX v28; // ecx
  _QWORD v29[4]; // [rsp+68h] [rbp-49h] BYREF
  _BYTE v30[32]; // [rsp+88h] [rbp-29h] BYREF
  __int64 v31; // [rsp+A8h] [rbp-9h]

  v12 = (__m128i *)ppo[1];
  v13 = 0;
  memset(v29, 0, sizeof(v29));
  if ( bSubOverflow(_mm_cvtsi128_si32(_mm_srli_si128(v12[3], 8)), v12[3].m128i_i32[0])
    || bSubOverflow(_mm_srli_si128(v16[48LL], 8).m128i_i32[1], HIDWORD(v16[3].m128i_i64[0]))
    || _mm_cvtsi128_si32(_mm_srli_si128(v19[3], 8)) - v19[3].m128i_i32[0] < 0
    || (int)(_mm_srli_si128(v19[48LL], 8).m128i_i32[1] - HIDWORD(v19[3].m128i_i64[0])) < 0 )
  {
    return 0;
  }
  if ( !ppo->cCurves )
    return 1;
  v22 = *(_DWORD *)(v18 + 112);
  v23 = 1;
  ++*(_DWORD *)(v18 + 92);
  v24 = mixFill;
  if ( (v22 & 0x80u) != 0 )
  {
    PRECOMPUTE::vInit((__int64)v30, a4, v17, (__int64)ppo, (__int64)pco, pxo, plineattrs, mixFill, flOptions, 2);
    if ( !v31 )
    {
      EngSetLastError(8u);
LABEL_32:
      PRECOMPUTE::~PRECOMPUTE((PRECOMPUTE *)v30);
      return v13;
    }
    if ( (plineattrs->fl & 1) == 0 || (a2 & 2) != 0 )
    {
      v25 = -1;
      if ( (ppo->fl & 1) != 0 )
      {
        if ( (a2 & 1) != 0 )
        {
          if ( (*(_DWORD *)(*(_QWORD *)&ppo[1] + 88LL) & 1) != 0 )
            PushThreadGuardedObject(v29, ppo, vCleanupPathStackObj);
          v26 = *(__int64 (__fastcall **)(char *, PATHOBJ *, CLIPOBJ *, XFORMOBJ *, BRUSHOBJ *, LINEATTRS *, BRUSHOBJ *, struct _POINTL *, MIX, FLONG))(*(_QWORD *)a3 + 2848LL);
          if ( v26 )
            v25 = v26((char *)a4 + 24, ppo, pco, pxo, pboStroke, plineattrs, pbo, pptlBrushOrg, mixFill, flOptions);
          if ( (*(_DWORD *)(*(_QWORD *)&ppo[1] + 88LL) & 1) != 0 )
            PopThreadGuardedObject(v29);
          if ( v25 == 1 )
            goto LABEL_30;
          if ( v25 == -1 )
            goto LABEL_32;
          v25 = -1;
        }
        if ( !EPATHOBJ::bFlatten((EPATHOBJ *)ppo) )
          goto LABEL_32;
      }
      if ( (*(_DWORD *)(*(_QWORD *)&ppo[1] + 88LL) & 1) != 0 )
        PushThreadGuardedObject(v29, ppo, vCleanupPathStackObj);
      v27 = *(__int64 (__fastcall **)(char *, PATHOBJ *, CLIPOBJ *, XFORMOBJ *, BRUSHOBJ *, LINEATTRS *, BRUSHOBJ *, struct _POINTL *, MIX, FLONG))(*(_QWORD *)a3 + 2848LL);
      if ( v27 )
        v25 = v27((char *)a4 + 24, ppo, pco, pxo, pboStroke, plineattrs, pbo, pptlBrushOrg, mixFill, flOptions);
      if ( (*(_DWORD *)(*(_QWORD *)&ppo[1] + 88LL) & 1) != 0 )
        PopThreadGuardedObject(v29);
      if ( v25 == 1 )
      {
LABEL_30:
        v13 = 1;
        goto LABEL_32;
      }
      if ( v25 == -1 )
        goto LABEL_32;
    }
    PRECOMPUTE::~PRECOMPUTE((PRECOMPUTE *)v30);
    v17 = a3;
    v20 = a2;
  }
  if ( (plineattrs->fl & 1) != 0
    && ((_BYTE)mixFill != 13 || (unsigned int)(*(_DWORD *)(*((_QWORD *)a4 + 6) + 2164LL) - 1) > 1) )
  {
    return EngStrokeAndFillPath(
             (SURFOBJ *)((char *)a4 + 24),
             ppo,
             pco,
             pxo,
             pboStroke,
             plineattrs,
             pbo,
             pptlBrushOrg,
             mixFill,
             flOptions);
  }
  v28 = mixFill;
  if ( ((__int64)pbo[5].pvRbrush & 0x8000) == 0 )
    v28 = (unsigned __int8)mixFill | ((unsigned __int8)mixFill << 8);
  if ( ((__int64)pboStroke[5].pvRbrush & 0x8000) == 0 )
    v24 = (unsigned __int8)mixFill | ((unsigned __int8)mixFill << 8);
  if ( !EPATHOBJ::bSimpleFill(ppo, v20, v17, a4, pco, pbo, pptlBrushOrg, v28, flOptions)
    || !EPATHOBJ::bSimpleStroke(ppo, a2, a3, a4, pco, pxo, pboStroke, pptlBrushOrg, plineattrs, v24) )
  {
    return 0;
  }
  return v23;
}
