/*
 * XREFs of ?TabletAdjustBrightness@@YAXH@Z @ 0x1C01D9860
 * Callers:
...
void __fastcall TabletAdjustBrightness(int a1)
{
  ...
  if ( !gfSwitchInProgress && !gPowerTransitionsState[2] && gPowerTransitionsState[0] )
  {
          ...
          if ( IoWMIQueryAllData(DataBlockObject, &InOutBufferSize, v2) >= 0 )
          {
            ...
            if ( a1 )
            {
              ...
              if ( (unsigned int)v7 < v6 )
                gPowerDisplayState[12] = (unsigned __int8)v5[v7 + 8];
            }
            else if ( (unsigned int)v4 > 1 )
            {
              gPowerDisplayState[12] = (unsigned __int8)v5[(unsigned int)(v4 - 1) + 8];
            }
          }
          Win32FreePool(v3);
        }
    ...
    if ( gPowerDisplayState[17] != gPowerDisplayState[12] )
    {
      v11 = 0LL;
      v10 = 4;
