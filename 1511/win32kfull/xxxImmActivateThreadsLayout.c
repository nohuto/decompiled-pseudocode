/*
 * XREFs of xxxImmActivateThreadsLayout @ 0x1C01E3AC0
 * Callers:
 *     xxxInternalActivateKeyboardLayout @ 0x1C0098598 (xxxInternalActivateKeyboardLayout.c)
 *     xxxImmActivateThreadsLayout @ 0x1C01E3AC0 (xxxImmActivateThreadsLayout.c)
 * Callees:
 *     W32GetThreadWin32Thread @ 0x1C005592C (W32GetThreadWin32Thread.c)
 *     PopAndFreeW32ThreadLock @ 0x1C0064370 (PopAndFreeW32ThreadLock.c)
 *     LockW32Thread @ 0x1C008E0B4 (LockW32Thread.c)
 *     xxxImmActivateLayout @ 0x1C01140F4 (xxxImmActivateLayout.c)
 *     __security_check_cookie @ 0x1C01512D0 (__security_check_cookie.c)
 *     xxxImmActivateThreadsLayout @ 0x1C01E3AC0 (xxxImmActivateThreadsLayout.c)
 */

__int64 __fastcall xxxImmActivateThreadsLayout(__int64 a1, __int64 a2, __int64 a3)
{
  __int64 v3; // rbx
  unsigned int v5; // esi
  __int64 v6; // rdx
  __int64 result; // rax
  _QWORD *v8; // rbx
  int i; // edi
  __int64 v10; // rcx
  __int64 v13; // [rsp+30h] [rbp-308h]
  _QWORD v14[82]; // [rsp+80h] [rbp-2B8h] BYREF

  v3 = a3;
  v5 = 0;
  W32GetThreadWin32Thread((__int64)KeGetCurrentThread());
  while ( a1 )
  {
    v6 = *(_QWORD *)(a1 + 392);
    if ( v6 != v3 && (*(_DWORD *)(a1 + 440) & 1) == 0 )
    {
      if ( *(_QWORD *)(a1 + 696) )
      {
        LockW32Thread(a1, &v14[5 * v5 + 2]);
        v14[5 * v5++ + 1] = a1;
        v3 = a3;
        if ( v5 == 16 )
          break;
      }
      else
      {
        *(_QWORD *)(a1 + 712) = *(_QWORD *)(v6 + 40);
        HMAssignmentLock(a1 + 392, v3);
        if ( *(_QWORD *)(a1 + 704) )
          **(_QWORD **)(a1 + 432) |= 0x40uLL;
        *(_QWORD *)(*(_QWORD *)(a1 + 432) + 144LL) = *(_QWORD *)(v3 + 40);
        *(_WORD *)(*(_QWORD *)(a1 + 432) + 152LL) = *(_WORD *)(v3 + 72);
      }
    }
    a1 = *(_QWORD *)(a1 + 584);
  }
  result = a2;
  if ( a2 )
    goto LABEL_14;
  if ( !a1 )
  {
    if ( !v5 )
      return result;
LABEL_14:
    if ( !a1 )
      goto LABEL_17;
  }
  if ( *(_QWORD *)(a1 + 584) )
  {
    v14[0] = a2;
    return xxxImmActivateThreadsLayout(*(_QWORD *)(a1 + 584), v14, v3);
  }
LABEL_17:
  v14[0] = a2;
  v8 = v14;
  while ( v8 )
  {
    for ( i = v5 - 1; i >= 0; --i )
    {
      v10 = v8[5 * i + 1];
      v13 = v10;
      if ( (*(_DWORD *)(v10 + 440) & 1) == 0 )
      {
        xxxImmActivateLayout(v10, a3);
        if ( (*(_DWORD *)(v13 + 440) & 1) == 0 )
        {
          *(_QWORD *)(*(_QWORD *)(v13 + 432) + 144LL) = *(_QWORD *)(a3 + 40);
          *(_WORD *)(*(_QWORD *)(v13 + 432) + 152LL) = *(_WORD *)(a3 + 72);
        }
      }
      PopAndFreeW32ThreadLock((__int64)&v8[4 * i + 2 + i]);
    }
    v8 = (_QWORD *)*v8;
    v5 = 16;
  }
  return 1LL;
}
