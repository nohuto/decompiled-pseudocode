/*
 * XREFs of ?CreateDesktopNotificationEvent@@YAJPEAX@Z @ 0x1C00D7188
 * Callers:
...
__int64 __fastcall CreateDesktopNotificationEvent(void *a1)
{
  ...
  if ( gbRemoteSession )
  {
    result = RtlStringCchPrintfW(SourceString, 0x100uLL, L"\\Sessions\\%ld\\BaseNamedObjects", gSessionId);
    if ( (int)result < 0 )
      return result;
    v3 = SourceString;
  }
