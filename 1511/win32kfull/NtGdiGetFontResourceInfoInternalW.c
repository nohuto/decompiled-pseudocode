/*
 * XREFs of NtGdiGetFontResourceInfoInternalW @ 0x1C02A6910
 * Callers:
 *     <none>
 * Callees:
 *     bCheckAndCapThePath @ 0x1C000E8C8 (bCheckAndCapThePath.c)
 *     PALLOCMEM2 @ 0x1C001A9C0 (PALLOCMEM2.c)
 *     __security_check_cookie @ 0x1C01512D0 (__security_check_cookie.c)
 *     memmove @ 0x1C0152980 (memmove.c)
 *     memset @ 0x1C0152CC0 (memset.c)
 *     GetFontResourceInfoInternalW @ 0x1C02D2860 (GetFontResourceInfoInternalW.c)
 */

__int64 __fastcall NtGdiGetFontResourceInfoInternalW(
        unsigned __int64 a1,
        unsigned int a2,
        unsigned int a3,
        unsigned int a4,
        _DWORD *a5,
        char *a6,
        int a7)
{
  unsigned __int16 *v10; // rdi
  unsigned int FontResourceInfoInternalW; // esi
  unsigned int *v12; // rbx
  _DWORD *v13; // rcx
  char *v14; // rax
  __int64 v16; // [rsp+68h] [rbp-160h] BYREF
  unsigned int v17; // [rsp+70h] [rbp-158h]
  unsigned __int64 v18; // [rsp+78h] [rbp-150h]
  _DWORD v19[24]; // [rsp+80h] [rbp-148h] BYREF
  _BYTE v20[160]; // [rsp+E0h] [rbp-E8h] BYREF

  v18 = a1;
  v17 = a4;
  v16 = (__int64)a6;
  v10 = 0LL;
  FontResourceInfoInternalW = 0;
  memset(v19, 0, 92);
  v12 = 0LL;
  if ( a2 > 1 )
  {
    if ( a2 > 0x50 )
    {
      if ( a2 <= 0x1388000 )
        v10 = (unsigned __int16 *)AllocFreeTmpBuffer(2 * a2);
    }
    else
    {
      v10 = (unsigned __int16 *)v20;
    }
    if ( v10 )
      FontResourceInfoInternalW = bCheckAndCapThePath(v10, v18, a2, a3);
  }
  if ( a4 <= 0x5C )
  {
    v12 = v19;
  }
  else if ( a4 > 0x2710000 || (v12 = (unsigned int *)PALLOCMEM2(a4, 1886221383LL, 1)) == 0LL )
  {
    FontResourceInfoInternalW = 0;
  }
  if ( FontResourceInfoInternalW )
  {
    FontResourceInfoInternalW = GetFontResourceInfoInternalW(v10, a2, a3, a4, (__int64)&v16, v12, a7);
    if ( FontResourceInfoInternalW )
    {
      v13 = a5;
      if ( (unsigned __int64)a5 >= W32UserProbeAddress )
        v13 = (_DWORD *)W32UserProbeAddress;
      *v13 = v16;
      if ( a4 )
      {
        v14 = &a6[a4];
        if ( (unsigned __int64)v14 > W32UserProbeAddress || v14 <= a6 )
          *W32UserProbeAddress = 0;
        memmove(a6, v12, a4);
      }
    }
  }
  if ( v10 && v10 != (unsigned __int16 *)v20 )
    FreeTmpBuffer(v10);
  if ( v12 && v12 != v19 )
    Win32FreePool(v12);
  return FontResourceInfoInternalW;
}
