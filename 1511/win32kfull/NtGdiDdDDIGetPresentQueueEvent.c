/*
 * XREFs of NtGdiDdDDIGetPresentQueueEvent @ 0x1C0263890
 * Callers:
 *     <none>
 * Callees:
 *     <none>
 */

__int64 __fastcall NtGdiDdDDIGetPresentQueueEvent(__int64 a1, HANDLE *a2, __int64 a3)
{
  HANDLE *v3; // rdi
  unsigned int v4; // esi
  int PresentQueueEvent; // ebx
  HANDLE Handle; // [rsp+40h] [rbp+18h] BYREF

  v3 = a2;
  v4 = a1;
  Handle = 0LL;
  PresentQueueEvent = 0;
  if ( !(unsigned int)UserUnsafeIsCurrentProcessDwm(a1, a2, a3) )
    PresentQueueEvent = -1073741790;
  if ( PresentQueueEvent < 0 || (PresentQueueEvent = GreSfmGetPresentQueueEvent(v4, &Handle), PresentQueueEvent < 0) )
  {
    if ( Handle )
      ObCloseHandle(Handle, 1);
  }
  else
  {
    if ( (unsigned __int64)v3 >= W32UserProbeAddress )
      v3 = (HANDLE *)W32UserProbeAddress;
    *v3 = Handle;
  }
  return (unsigned int)PresentQueueEvent;
}
