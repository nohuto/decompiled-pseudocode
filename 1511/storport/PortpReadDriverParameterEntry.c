/*
 * XREFs of PortpReadDriverParameterEntry @ 0x1C0054748
 * Callers:
 *     PortGetDriverParameters @ 0x1C00546DC (PortGetDriverParameters.c)
 * Callees:
...
char __fastcall PortpReadDriverParameterEntry(HANDLE KeyHandle, PVOID *a2)
{
  ...
  struct _STRING v7; // [rsp+30h] [rbp-D0h] BYREF
  ...
  PoolWithTag = (char *)ExAllocatePoolWithTag(NonPagedPoolNx, v5, 0x704D6C50u);
  *a2 = PoolWithTag;
  if ( !PoolWithTag )
    return 0;
  if ( v12 == 1 )
  {
    v7.Buffer = PoolWithTag;
    v7.Length = 0;
    SourceString.Buffer = (unsigned __int16 *)&KeyValueInformation[v13];
    SourceString.Length = NumberOfBytes;
    SourceString.MaximumLength = NumberOfBytes;
    v7.MaximumLength = NumberOfBytes;
    if ( RtlUnicodeStringToAnsiString(&v7, &SourceString, 0) < 0 )
    {
      ExFreePoolWithTag(*a2, 0);
      *a2 = 0LL;
    }
