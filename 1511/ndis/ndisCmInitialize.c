/*
 * XREFs of ndisCmInitialize @ 0x1C00FE024
 * Callers:
...
__int64 ndisCmInitialize()
{
  ...
  if ( (int)result >= 0 )
  {
    ...
    if ( (int)result >= 0 )
    {
      memset(v3, 0, 0x78uLL);
      BYTE2(v3[0]) |= 0xCu;
      v3[9] = ndisCmDeleteNetworkNamespace;
      LOWORD(v3[0]) = 120;
      HIDWORD(v3[3]) = 983103;
      HIDWORD(v3[4]) = 512;
      LODWORD(v3[5]) = 16;
      RtlInitUnicodeString(&DestinationString, L"NetworkNamespace");
      return ObCreateObjectType(&DestinationString, v3, 0LL, &ndisCmNetworkNamespaceObjectType);
    }
  }
  return result;
}
