/*
 * XREFs of NdisAllocateNetBufferListContext @ 0x1C0010EB0
 * Callers:
 ...
 * Callees:
 *     ?ndisIsCurrentNblContextBlockPreallocated@@YAEPEAU_NET_BUFFER_LIST@@@Z @ 0x1C0011030 (-ndisIsCurrentNblContextBlockPreallocated@@YAEPEAU_NET_BUFFER_LIST@@@Z.c)
 */
    ...
    else
    {
      ...
      else
      {
        ...
        if ( LOWORD(NetBufferList->NdisReserved[1]) <= ndisMaxCachedNblContextSize )
        {
          ...
          if ( v15 > ContextSize + ContextBackFill )
            PoolTag = 1668170830;
        }
