/*
 * XREFs of ?OnClose@FxPkgGeneral@@AEAAJPEAVFxIrp@@@Z @ 0x1C000BDA0
 * Callers:
...
LABEL_22:
    ...
    {
      FxVerifierLock::Lock(m_CallbackLockObjectPtr, &PreviousIrql, v9);
      v23 = PreviousIrql;
    }
    else
    {
    ...
    v24->Blink = Blink;
    if ( SLOBYTE(m_DeviceBase->m_ObjectFlags) < 0
      && (v33 = (FxVerifierLock *)m_DeviceBase[-1].m_CallbackLockObjectPtr) != 0LL )
    {
...
$Passthru:
  ...
  {
    m_DeviceObject = v27->m_DeviceObject.m_DeviceObject;
    v3 = 0;
    if ( *(_QWORD *)&v27[3].m_SpinLock.m_DbgFlagIsInitialized && ((__int64)m_DeviceObject[1].CurrentIrp & 1) == 0 )
    {
      _a2 = IoAcquireRemoveLockEx((PIO_REMOVE_LOCK)&m_DeviceObject[1], FxIrp->m_Irp, &id, 1u, 0x20u);
      if ( _a2 < 0 )
      {
        v34 = this->m_Globals;
