/*
 * XREFs of imp_WdfCollectionGetItem @ 0x1C00026C0
 * Callers:
 *     <none>
 * Callees:
        ...
        unsigned int Index)
{
  ...
  if ( SLOBYTE(v4->m_ObjectFlags) < 0 && *(_QWORD *)&v4[-1].m_Count )
  {
    FxVerifierLock::Lock(*(FxVerifierLock **)&v4[-1].m_Count, &irql, Offset);
    v7 = irql;
  }
  else
  {
...
LABEL_21:
  ...
  if ( SLOBYTE(pCollection->m_ObjectFlags) < 0 && *(_QWORD *)&pCollection[-1].m_Count )
    FxVerifierLock::Unlock(*(FxVerifierLock **)&pCollection[-1].m_Count, v7, v6);
  else
