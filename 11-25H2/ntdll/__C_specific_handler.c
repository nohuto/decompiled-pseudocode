/*
 * XREFs of __C_specific_handler @ 0x180123180
 * Callers:
 *     __GSHandlerCheck_SEH @ 0x180162D10 (__GSHandlerCheck_SEH.c)
 * Callees:
 *     RtlUnwindEx @ 0x180077090 (RtlUnwindEx.c)
 *     _NLG_Notify @ 0x180129E60 (_NLG_Notify.c)
 *     __NLG_Return2 @ 0x180129E90 (__NLG_Return2.c)
 *     __except_validate_context_record @ 0x180129E98 (__except_validate_context_record.c)
 */
    ...
    while ( 1 )
    {
      ...
      if ( v11 >= HandlerData[4 * ScopeIndex + 1] && v11 < HandlerData[4 * ScopeIndex + 2] )
      {
        if ( (ExceptionRecord->ExceptionFlags & 0x20) != 0 )
        {
          ...
          {
            if ( v15 >= HandlerData[4 * (unsigned int)i + 1]
              && v15 < HandlerData[4 * (unsigned int)i + 2]
              && HandlerData[4 * (unsigned int)i + 4] == HandlerData[4 * ScopeIndex + 4]
              && HandlerData[4 * (unsigned int)i + 3] == HandlerData[4 * ScopeIndex + 3] )
            {
              break;
            }
          }
          if ( (_DWORD)i != *HandlerData )
            return 1;
        }
        ...
        else
        {
          ...
          LOBYTE(i) = 1;
          ((void (__fastcall *)(__int64, void *))(ImageBase + HandlerData[4 * ScopeIndex + 3]))(i, EstablisherFrame);
        }
      }
      ++ScopeIndex;
    }
