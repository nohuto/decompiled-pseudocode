/*
 * XREFs of RtlpWakeSingle @ 0x18009E500
 * Callers:
 *     RtlSleepConditionVariableCS @ 0x18007ADC0 (RtlSleepConditionVariableCS.c)
 * Callees:
 *     RtlpWakeConditionVariable @ 0x18009E650 (RtlpWakeConditionVariable.c)
 */

bool __fastcall RtlpWakeSingle(volatile signed __int64 *a1, __int64 a2)
{
  signed __int64 v2; // rax
  unsigned __int64 v5; // rdx
  signed __int64 v6; // rtt
  unsigned __int64 v7; // rcx
  _QWORD *v8; // r11
  char v9; // bl
  signed __int64 v10; // rax
  unsigned __int64 v11; // r10
  unsigned __int64 v12; // r8
  signed __int64 v13; // rtt
  signed __int64 v15; // rtt

  v2 = *a1;
  do
  {
    while ( 1 )
    {
      if ( !v2 || (v2 & 7) == 7 )
        return 0;
      if ( (v2 & 8) == 0 )
        break;
      v15 = v2;
      v2 = _InterlockedCompareExchange64(a1, v2 | 7, v2);
      if ( v15 == v2 )
        return 0;
    }
    v5 = v2 + 8;
    v6 = v2;
    v2 = _InterlockedCompareExchange64(a1, v2 + 8, v2);
  }
  while ( v6 != v2 );
  v7 = v5 & 0xFFFFFFFFFFFFFFF0uLL;
  v8 = 0LL;
  v9 = 0;
  v10 = v5;
LABEL_7:
  v11 = v7;
  while ( v7 )
  {
    v12 = *(_QWORD *)v7;
    if ( v7 == a2 )
    {
      if ( !v8 )
      {
        v5 = *(_QWORD *)v7;
        if ( v12 )
          v5 = v10 ^ (v10 ^ v12) & 0xFFFFFFFFFFFFFFF0uLL;
        v13 = v10;
        v10 = _InterlockedCompareExchange64(a1, v5, v10);
        if ( v13 == v10 )
        {
          _interlockedbittestandset((volatile signed __int32 *)(v7 + 36), 2u);
          v9 = 1;
          v10 = v5;
          if ( !v12 )
            return 1;
        }
        else
        {
          v5 = v10;
        }
        v7 = v10 & 0xFFFFFFFFFFFFFFF0uLL;
        v8 = 0LL;
        goto LABEL_7;
      }
      _interlockedbittestandset((volatile signed __int32 *)(v7 + 36), 2u);
      *v8 = v12;
      v9 = 1;
      if ( v12 )
        *(_QWORD *)(v12 + 16) = v8;
      v7 = v12;
    }
    else
    {
      *(_QWORD *)(v7 + 16) = v8;
      v8 = (_QWORD *)v7;
      v7 = v12;
    }
  }
  if ( v11 )
    *(_QWORD *)(v11 + 8) = v8;
  if ( !v9 )
    _InterlockedOr((volatile signed __int32 *)(a2 + 36), 2u);
  RtlpWakeConditionVariable(a1, v5, 0LL);
  if ( !v9 )
    return !_interlockedbittestandreset((volatile signed __int32 *)(a2 + 36), 1u);
  return v9;
}
