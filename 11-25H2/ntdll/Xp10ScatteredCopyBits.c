/*
 * XREFs of Xp10ScatteredCopyBits @ 0x18010B040
 * Callers:
 *     RtlDecompressBuffer2Xp10 @ 0x18010AC98 (RtlDecompressBuffer2Xp10.c)
 * Callees:
 *     Xp10BitCopy @ 0x18010B3B8 (Xp10BitCopy.c)
 *     Xp10ScatteredNextBuffer @ 0x1801215CC (Xp10ScatteredNextBuffer.c)
 */

__int64 __fastcall Xp10ScatteredCopyBits(__int64 a1, int *a2, unsigned int a3)
{
  unsigned int v3; // r14d
  unsigned int v4; // ebp
  unsigned int v5; // r12d
  unsigned __int64 v8; // rdx
  int v9; // ebx
  __int64 v10; // rax
  bool v11; // zf
  unsigned int v12; // r15d
  _QWORD *v13; // r13
  unsigned __int64 v14; // rdx
  int v15; // ebx
  __int64 v16; // rax
  __int64 v17; // rax
  unsigned __int8 *v18; // r8
  unsigned __int64 v19; // rdx
  __int64 result; // rax

  v3 = a3;
  v4 = 0;
  v5 = 0;
  if ( *(_QWORD *)a2 < (unsigned __int64)a3 )
    v3 = *a2;
  if ( a2[4] )
  {
    while ( 1 )
    {
      v8 = 8LL * *(_QWORD *)(a1 + 8) - 8LL * *(_QWORD *)a1 - (v5 & 7);
      if ( v8 >= (unsigned int)a2[4] )
        v8 = (unsigned int)a2[4];
      if ( v3 - v4 <= v8 )
        LODWORD(v8) = v3 - v4;
      v9 = v8;
      Xp10BitCopy(*(void **)a1, (unsigned int)v8);
      v4 += v9;
      v10 = ((v9 + v5) >> 3) - (v5 >> 3);
      v5 += v9;
      *(_QWORD *)a1 += v10;
      *((_QWORD *)a2 + 1) >>= v9;
      v11 = a2[4] == v9;
      a2[4] -= v9;
      if ( v11 )
        break;
      if ( v4 == v3 || !(unsigned int)Xp10ScatteredNextBuffer(a1) )
        goto LABEL_24;
    }
  }
  v12 = 0;
  v13 = a2 + 6;
  do
  {
    v14 = 8LL * *(_QWORD *)(a1 + 8) - 8LL * *(_QWORD *)a1 - (v5 & 7);
    if ( v14 >= 8LL * *((_QWORD *)a2 + 4) - 8LL * *v13 - (unsigned __int64)(v12 & 7) )
      v14 = 8LL * *((_QWORD *)a2 + 4) - 8LL * *v13 - (v12 & 7);
    if ( v3 - v4 <= v14 )
      LODWORD(v14) = v3 - v4;
    v15 = v14;
    Xp10BitCopy(*(void **)a1, (unsigned int)v14);
    v4 += v15;
    v16 = ((v15 + v5) >> 3) - (v5 >> 3);
    v5 += v15;
    *(_QWORD *)a1 += v16;
    v17 = ((v15 + v12) >> 3) - (v12 >> 3);
    v12 += v15;
    *v13 += v17;
  }
  while ( v4 != v3
       && (*(_QWORD *)a1 != *(_QWORD *)(a1 + 8) || (unsigned int)Xp10ScatteredNextBuffer(a1))
       && (*v13 != *((_QWORD *)a2 + 4) || (unsigned int)Xp10ScatteredNextBuffer(a2 + 6)) );
  if ( (v12 & 7) != 0 )
  {
    v18 = (unsigned __int8 *)*((_QWORD *)a2 + 3);
    a2[4] = 8 - (v12 & 7);
    v19 = (unsigned __int64)*v18 >> (v12 & 7);
    *((_QWORD *)a2 + 3) = v18 + 1;
    *((_QWORD *)a2 + 1) = v19;
  }
LABEL_24:
  result = v4;
  *(_QWORD *)a2 -= v4;
  return result;
}
