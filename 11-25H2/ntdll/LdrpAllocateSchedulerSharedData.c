...
__int64 LdrpAllocateSchedulerSharedData()
{
  ...
  if ( v0 >= 0 )
  {
    Heap = (_QWORD *)RtlAllocateHeap((char *)LdrpHeap, (NtdllBaseTag + 0x40000) | 8u, 0x18uLL);
    if ( Heap )
    {
      Heap[2] = Handle;
      RtlAcquireSRWLockExclusive((volatile signed __int32 *)&LdrpSchedulerSharedDataListHeadLock);
