/*
 * XREFs of Controller_LogDiagnosticsOnD0Exit @ 0x140041764
 * Callers:
 *     Controller_WdfEvtDeviceD0Exit @ 0x140038BB0 (Controller_WdfEvtDeviceD0Exit.c)
 * Callees:
 *     WPP_RECORDER_SF_qLLL @ 0x140014BE4 (WPP_RECORDER_SF_qLLL.c)
 *     Controller_GetPreProcessedSystemPowerAction @ 0x140040790 (Controller_GetPreProcessedSystemPowerAction.c)
 *     McTemplateK0pqqhq_EtwWriteTransfer @ 0x140044678 (McTemplateK0pqqhq_EtwWriteTransfer.c)
 *     MicrosoftTelemetryAssertTriggeredArgsMsgKM @ 0x140057938 (MicrosoftTelemetryAssertTriggeredArgsMsgKM.c)
 *     _guard_dispatch_icall @ 0x1400596E0 (_guard_dispatch_icall.c)
 */

_UNKNOWN **__fastcall Controller_LogDiagnosticsOnD0Exit(__int64 a1, int a2)
{
  __int64 v4; // rbx
  int v5; // edi
  int PreProcessedSystemPowerAction; // eax
  __int64 v7; // rdx
  int v8; // r8d
  int v9; // ebp
  _UNKNOWN **result; // rax
  __int64 v11; // rdx
  __int64 v12; // r8
  __int64 v13; // [rsp+30h] [rbp-28h]
  __int64 v14; // [rsp+38h] [rbp-20h]

  v4 = *(_QWORD *)((*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64, void *))(WdfFunctions_01033 + 1616))(
                     WdfDriverGlobals,
                     a1,
                     off_14006B240)
                 + 8);
  v5 = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, __int64))(WdfFunctions_01033 + 3104))(WdfDriverGlobals, a1);
  PreProcessedSystemPowerAction = Controller_GetPreProcessedSystemPowerAction(v4);
  v9 = PreProcessedSystemPowerAction;
  if ( ((__int64)WPP_MAIN_CB.Queue.Wcb.BufferChainingDpc & 8) != 0 )
    McTemplateK0pqqhq_EtwWriteTransfer(
      *(unsigned __int8 *)(v4 + 868),
      (unsigned int)&USBXHCI_ETW_EVENT_CONTROLLER_D0_EXIT_START_V1,
      v8,
      *(_QWORD *)(v4 + 8),
      a2,
      v5,
      *(_BYTE *)(v4 + 868),
      PreProcessedSystemPowerAction);
  result = &WPP_RECORDER_INITIALIZED;
  if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
  {
    LODWORD(v14) = v9;
    LODWORD(v13) = a2;
    result = (_UNKNOWN **)WPP_RECORDER_SF_qLLL(
                            *(_QWORD *)(v4 + 72),
                            v7,
                            4u,
                            0x57u,
                            (__int64)&WPP_e3ec626fc5bc373e46c859b185ef609a_Traceguids,
                            a1,
                            v13,
                            v14,
                            v5);
  }
  v11 = 0LL;
  if ( !v5 )
  {
    v12 = *(unsigned int *)(v4 + 820);
    if ( (_DWORD)v12 != 7 )
    {
      if ( (_DWORD)v12 )
      {
        if ( *(_DWORD *)(v4 + 644) == 1 )
          v11 = *(unsigned __int16 *)(v4 + 652) | (*(unsigned __int16 *)(v4 + 648) << 16);
        return (_UNKNOWN **)MicrosoftTelemetryAssertTriggeredArgsMsgKM(
                              "USBXHCI.SYS",
                              v11,
                              v12,
                              "D0 Exit - WDF and Pre Processed System Power Action are different");
      }
    }
  }
  return result;
}
