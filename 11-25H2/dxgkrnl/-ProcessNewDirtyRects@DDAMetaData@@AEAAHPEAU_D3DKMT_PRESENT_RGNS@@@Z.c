/*
 * XREFs of ?ProcessNewDirtyRects@DDAMetaData@@AEAAHPEAU_D3DKMT_PRESENT_RGNS@@@Z @ 0x14034C218
 * Callers:
...
__int64 __fastcall DDAMetaData::ProcessNewDirtyRects(DDAMetaData *this, struct _D3DKMT_PRESENT_RGNS *a2)
{
  ...
  for ( i = 0; i < a2->DirtyRectCount; ++i )
  {
    if ( !(unsigned int)DDAMetaData::ProcessDirtyRectAgainstMoveList(this, pDirtyRects) )
    {
      ...
      DxgkLogInternalTriageEvent(
        ...
        0x40000,
        0xFFFFFFFFLL,
        L"Failed to process new dirty against move list",
        704LL,
        0LL,
        0LL,
        0LL,
  ...
  DxgkLogInternalTriageEvent(
    ...
    0x40000,
    0xFFFFFFFFLL,
    L"Failed to process dirty rect against dirty list, Idx 0x%I64x, pRect 0x%I64x",
    v7,
    (__int64)v6,
    0LL,
    0LL,
