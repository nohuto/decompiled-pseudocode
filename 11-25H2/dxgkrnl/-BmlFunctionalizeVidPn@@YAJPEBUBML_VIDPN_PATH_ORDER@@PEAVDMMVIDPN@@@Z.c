/*
 * XREFs of ?BmlFunctionalizeVidPn@@YAJPEBUBML_VIDPN_PATH_ORDER@@PEAVDMMVIDPN@@@Z @ 0x1402C5BC0
 * Callers:
  ...
  if ( v7 >= *(unsigned __int8 *)a1 )
  {
    ...
    for ( i = 0; i < *(unsigned __int8 *)a1; ++i )
    {
      ...
      if ( (*(_QWORD *)v25 & 0x20000000000000LL) != 0 )
      {
        ...
        if ( (int)BmlGetRecommendedContentSizeForPath(a1, i, v29, v25) < 0 )
        {
          WdLogSingleEntry0(1LL);
          WdLogGlobalForLineNumber = 4092;
        }
      }
    }
    return 0LL;
