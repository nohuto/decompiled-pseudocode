/*
 * XREFs of ?LogRemoteVsyncTimeout@BLTQUEUE@@AEAAXXZ @ 0x14027B818
 * Callers:
 *     ?BltQueueWorker@BLTQUEUE@@QEAAXXZ @ 0x14027A168 (-BltQueueWorker@BLTQUEUE@@QEAAXXZ.c)
 * Callees:
 *     _tlgWriteTransfer_EtwWriteTransfer @ 0x1400407F4 (_tlgWriteTransfer_EtwWriteTransfer.c)
 *     _tlgKeywordOn @ 0x140047784 (_tlgKeywordOn.c)
 *     _tlgWriteAgg @ 0x14005F950 (_tlgWriteAgg.c)
 *     __security_check_cookie @ 0x14009F860 (__security_check_cookie.c)
 */

void __fastcall BLTQUEUE::LogRemoteVsyncTimeout(BLTQUEUE *this)
{
  unsigned int v1; // r9d
  __int64 v2; // [rsp+30h] [rbp-9h] BYREF
  __int64 v3; // [rsp+38h] [rbp-1h] BYREF
  struct _EVENT_DATA_DESCRIPTOR v4; // [rsp+40h] [rbp+7h] BYREF
  __int64 *v5; // [rsp+60h] [rbp+27h]
  int v6; // [rsp+68h] [rbp+2Fh]
  int v7; // [rsp+6Ch] [rbp+33h]
  __int64 *v8; // [rsp+70h] [rbp+37h]
  int v9; // [rsp+78h] [rbp+3Fh]
  int v10; // [rsp+7Ch] [rbp+43h]

  if ( (unsigned int)dword_14015B5B8 > 5 )
  {
    if ( tlgKeywordOn((__int64)&dword_14015B5B8, 0x400000000800LL) )
    {
      v10 = 0;
      v7 = 0;
      v8 = &v2;
      v9 = 8;
      v5 = &v3;
      v6 = 8;
      v2 = 33556480LL;
      v3 = 1LL;
      tlgWriteAgg((__int64)&dword_14015B5B8, (unsigned __int8 *)dword_14013E151, 0LL, 4u, &v4);
      v1 = dword_14015B5B8;
    }
    if ( v1 > 5 )
    {
      if ( tlgKeywordOn((__int64)&dword_14015B5B8, 2048LL) )
        tlgWriteTransfer_EtwWriteTransfer((__int64)&dword_14015B5B8, byte_14013E12F, 0LL, 0LL, 2u, &v4);
    }
  }
}
