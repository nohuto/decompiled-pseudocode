/*
 * XREFs of ?SaveImmutableMigrationData@ADAPTER_RENDER@@QEAAJW4DXG_VIRTUAL_GPU_TYPE@@PEAU_LUID@@PEAU_DXGKARG_GPUP_SAVE_IMMUTABLE_MIGRATION_DATA@@@Z @ 0x14020898C
 * Callers:
...
__int64 __fastcall ADAPTER_RENDER::SaveImmutableMigrationData(__int64 a1, int a2, struct _LUID *a3, _DWORD *a4)
{
  __int64 v7; // rbx
  ...
  if ( a2 )
  {
    ...
    DxgkLogInternalTriageEvent(
      ...
      0x40000,
      0xFFFFFFFFLL,
      L"SaveImmutableMigrationData called for non GPUP vGPU. Luid: %d %d vGPU Type: %d Adapter: %d",
      HighPart,
      LowPart,
      v7,
      v16,
  ...
  else
  {
    ...
    if ( VirtualGpuByLuid )
    {
      *a4 = *((_DWORD *)VirtualGpuByLuid + 6);
      v11 = (*(__int64 (__fastcall **)(struct DXGK_VIRTUAL_GPU *, _DWORD *))(*(_QWORD *)VirtualGpuByLuid + 192LL))(
              VirtualGpuByLuid,
              a4);
    }
    else
