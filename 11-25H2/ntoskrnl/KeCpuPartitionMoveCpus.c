/*
 * XREFs of KeCpuPartitionMoveCpus @ 0x1405BB870
 * Callers:
 *     KeDeleteCpuPartition @ 0x1407305E8 (KeDeleteCpuPartition.c)
 *     NtSetInformationCpuPartition @ 0x1407664F0 (NtSetInformationCpuPartition.c)
 *     PsCpuPartitionMoveCpus @ 0x140766A4C (PsCpuPartitionMoveCpus.c)
 * Callees:
 *     KxAcquireSpinLock @ 0x140253560 (KxAcquireSpinLock.c)
 *     KiUpdateProcessAvailableCpuState @ 0x140267998 (KiUpdateProcessAvailableCpuState.c)
 *     KeSubtractAffinityEx2 @ 0x140269E70 (KeSubtractAffinityEx2.c)
 *     KeIsEqualAffinityEx @ 0x14026BB80 (KeIsEqualAffinityEx.c)
 *     KeIsSubsetAffinityEx @ 0x1402717F0 (KeIsSubsetAffinityEx.c)
 *     KeOrAffinityEx2 @ 0x1402720D0 (KeOrAffinityEx2.c)
 *     KxReleaseSpinLock @ 0x14034A6E0 (KxReleaseSpinLock.c)
 *     KiAcquireCpuPartitionLock @ 0x1404917D8 (KiAcquireCpuPartitionLock.c)
 *     KiModifySystemAllowedCpuSetsWithLock @ 0x140491940 (KiModifySystemAllowedCpuSetsWithLock.c)
 *     KiLowerIrqlProcessIrqlFlags @ 0x1404F1088 (KiLowerIrqlProcessIrqlFlags.c)
 *     KiNotifyAvailableCpusChangeProcess @ 0x1404F8BC0 (KiNotifyAvailableCpusChangeProcess.c)
 *     KiNotifyAvailableCpusChangeCpuPartition @ 0x1405B4798 (KiNotifyAvailableCpusChangeCpuPartition.c)
 *     KiUpdateSystemAvailableCpuState @ 0x1405B47C4 (KiUpdateSystemAvailableCpuState.c)
 *     KiAdjustProcessCpuSetsAfterCpuPartitionChange @ 0x1405BBFDC (KiAdjustProcessCpuSetsAfterCpuPartitionChange.c)
 *     KiCpuPartitionUpdatePrcbs @ 0x1405BC2D4 (KiCpuPartitionUpdatePrcbs.c)
 *     ExAllocatePool2 @ 0x140B620F0 (ExAllocatePool2.c)
 *     ExFreePoolWithTag @ 0x140B62CD0 (ExFreePoolWithTag.c)
 */

__int64 __fastcall KeCpuPartitionMoveCpus(__int64 a1, __int64 a2, struct _KAFFINITY_EX *a3, char a4)
{
  __int64 v5; // rsi
  unsigned int v7; // edi
  void *Pool2; // r13
  __int64 v10; // rax
  void *v11; // r12
  struct _KAFFINITY_EX *v12; // rbx
  _QWORD *v13; // rbp
  _QWORD *v14; // r15
  unsigned __int16 v15; // si
  __int64 v16; // rbx
  _QWORD *v17; // r15
  unsigned __int16 v18; // si
  __int64 v19; // rbx
  unsigned __int8 v21[4]; // [rsp+30h] [rbp-38h] BYREF
  __int16 v22; // [rsp+34h] [rbp-34h]

  v21[0] = 0;
  v5 = a2;
  v22 = KiActiveGroups;
  v7 = 0;
  Pool2 = (void *)ExAllocatePool2(0x40uLL);
  v10 = ExAllocatePool2(0x40uLL);
  v11 = (void *)v10;
  if ( !Pool2 || !v10 )
  {
    v7 = -1073741670;
    if ( !Pool2 )
      goto LABEL_29;
    goto LABEL_28;
  }
  KiAcquireCpuPartitionLock(v5, v21);
  v12 = *(struct _KAFFINITY_EX **)v5;
  if ( (unsigned int)KeIsSubsetAffinityEx(&a3->Count, *(unsigned __int16 **)v5)
    && (a4 || !(unsigned int)KeIsEqualAffinityEx(&a3->Count, &v12->Count)) )
  {
    if ( v5 != a1 )
    {
      KeSubtractAffinityEx2(v12, a3, v12);
      if ( v5 == KiSystemCpuPartition )
        v7 = KiModifySystemAllowedCpuSetsWithLock(0, 0LL, &a3->Count, 0, 2);
      KxReleaseSpinLock((volatile signed __int64 *)(v5 + 8));
      KxAcquireSpinLock((PKSPIN_LOCK)(a1 + 8));
      KeOrAffinityEx2(*(struct _KAFFINITY_EX **)a1, a3, *(struct _KAFFINITY_EX **)a1);
      KiCpuPartitionUpdatePrcbs(a1, a3);
      if ( a1 == KiSystemCpuPartition )
        v7 = KiModifySystemAllowedCpuSetsWithLock(0, 0LL, &a3->Count, 0, 1);
      KxReleaseSpinLock((volatile signed __int64 *)(a1 + 8));
      KxAcquireSpinLock(&KiCpuPartitionAssignmentLock);
      v13 = (_QWORD *)(v5 + 16);
      v14 = *(_QWORD **)(v5 + 16);
      if ( v14 != (_QWORD *)(v5 + 16) )
      {
        v15 = v22;
        do
        {
          v16 = *(v14 - 2);
          v14 = (_QWORD *)*v14;
          KiAdjustProcessCpuSetsAfterCpuPartitionChange(v16, v11, Pool2, v15, 0LL);
          KiUpdateProcessAvailableCpuState(v16, 1);
          KiNotifyAvailableCpusChangeProcess(v16);
        }
        while ( v14 != v13 );
        v5 = a2;
      }
      v17 = *(_QWORD **)(a1 + 16);
      if ( v17 != (_QWORD *)(a1 + 16) )
      {
        v18 = v22;
        do
        {
          v19 = *(v17 - 2);
          v17 = (_QWORD *)*v17;
          KiAdjustProcessCpuSetsAfterCpuPartitionChange(v19, v11, Pool2, v18, 0LL);
          KiUpdateProcessAvailableCpuState(v19, 1);
          KiNotifyAvailableCpusChangeProcess(v19);
        }
        while ( v17 != (_QWORD *)(a1 + 16) );
        v5 = a2;
      }
      KxReleaseSpinLock((volatile signed __int64 *)&KiCpuPartitionAssignmentLock);
      if ( v5 == KiSystemCpuPartition || a1 == KiSystemCpuPartition )
      {
        KiUpdateSystemAvailableCpuState();
        KiNotifyAvailableCpusChangeCpuPartition(KiSystemCpuPartition);
      }
      goto LABEL_24;
    }
  }
  else
  {
    v7 = -1073741811;
  }
  KxReleaseSpinLock((volatile signed __int64 *)(v5 + 8));
LABEL_24:
  if ( KiIrqlFlags )
    KiLowerIrqlProcessIrqlFlags(KeGetCurrentIrql(), v21[0]);
  __writecr8(v21[0]);
LABEL_28:
  ExFreePoolWithTag(Pool2, 0);
LABEL_29:
  if ( v11 )
    ExFreePoolWithTag(v11, 0);
  return v7;
}
