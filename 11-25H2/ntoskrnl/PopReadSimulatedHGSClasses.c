/*
 * XREFs of PopReadSimulatedHGSClasses @ 0x140C1FF50
 * Callers:
...
char __fastcall PopReadSimulatedHGSClasses(__int64 a1, void *a2, unsigned int a3, ULONG a4)
{
  ...
  HANDLE v12; // rcx
  ULONG v13; // r13d
  char SimulatedProcessorClass; // r9
          ...
          do
          {
            ...
            if ( ZwOpenKey(&v28, 0x20019u, &v43) >= 0 )
            {
              ...
              if ( ZwOpenKey(&v31, 0x20019u, &v44) >= 0 )
              {
                if ( a2 )
                  memset_0(a2, 1, v5);
                SimulatedProcessorClass = PopReadSimulatedProcessorClass(a2, v11, v31, (unsigned int)v5);
                if ( SimulatedProcessorClass )
                {
                  if ( a2 )
                  {
                    if ( a1 )
              ...
              if ( ZwOpenKey(&Handle, 0x20019u, &v45) >= 0 )
              {
                if ( a2 )
                  memset_0(a2, 1, v5);
                v19 = PopReadSimulatedProcessorClass(a2, v11, Handle, (unsigned int)v5);
                if ( v19 )
                {
                  if ( a2 )
                  {
