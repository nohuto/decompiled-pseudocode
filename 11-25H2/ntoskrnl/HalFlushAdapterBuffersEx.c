/*
 * XREFs of HalFlushAdapterBuffersEx @ 0x140332110
 * Callers:
 *     HalPutScatterGatherListV3 @ 0x14033419C (HalPutScatterGatherListV3.c)
 * Callees:
 *     IoFreeMdl @ 0x1402D5960 (IoFreeMdl.c)
 *     MmUnmapLockedPages @ 0x1403285B0 (MmUnmapLockedPages.c)
 *     HalpDmaFlushContiguousTransferV3 @ 0x140331380 (HalpDmaFlushContiguousTransferV3.c)
 *     HalpDmaFlushScatterTransferV3 @ 0x14033239C (HalpDmaFlushScatterTransferV3.c)
 *     HalpDmaControllerFlushChannel @ 0x14033258C (HalpDmaControllerFlushChannel.c)
 *     HalpDmaCheckMdlAccessibility @ 0x140332BD0 (HalpDmaCheckMdlAccessibility.c)
 *     HalpDmaGetAdapterCacheAlignment @ 0x140333410 (HalpDmaGetAdapterCacheAlignment.c)
 *     HalpDmaFlushDriverMdl @ 0x140539DBC (HalpDmaFlushDriverMdl.c)
 *     HalpDmaFreeLa @ 0x140539DFC (HalpDmaFreeLa.c)
 */

__int64 __fastcall HalFlushAdapterBuffersEx(
        __int64 a1,
        __int64 a2,
        __int64 a3,
        unsigned __int64 a4,
        unsigned int a5,
        char a6)
{
  bool v7; // zf
  __int64 v8; // rbp
  __int64 v9; // rbx
  struct _MDL *v11; // rsi
  unsigned int v12; // esi
  unsigned __int64 v14; // rax
  unsigned __int64 v15; // r12
  __int64 v16; // r14
  __int64 v17; // r8
  char v18; // r8
  char v19; // r13
  unsigned int v20; // edx
  unsigned int v21; // ecx
  __int64 v22; // r9
  int AdapterCacheAlignment; // eax
  __int64 v24; // r9
  unsigned __int64 v25; // r10
  int v26; // r11d
  struct _MDL *Next; // r14
  PMDL v28; // r15
  struct _MDL *v29; // rbx
  PMDL Mdl[9]; // [rsp+40h] [rbp-48h] BYREF
  char v31; // [rsp+90h] [rbp+8h] BYREF
  __int64 v32; // [rsp+A0h] [rbp+18h] BYREF

  v7 = *(_DWORD *)(a1 + 520) == 3;
  v8 = a3;
  v9 = a2;
  v31 = 0;
  Mdl[0] = 0LL;
  v32 = 0LL;
  if ( v7 )
    HalpDmaFreeLa(a1, a3 + 24);
  v11 = *(struct _MDL **)(a1 + 552);
  if ( v11 )
  {
    do
    {
      Next = v11->Next;
      if ( (v11->MdlFlags & 1) != 0 )
        MmUnmapLockedPages(v11->MappedSystemVa, v11);
      IoFreeMdl(v11);
      v11 = Next;
    }
    while ( Next );
  }
  v12 = a5;
  *(_QWORD *)(a1 + 552) = 0LL;
  if ( !v12 )
  {
    if ( !*(_BYTE *)(a1 + 441) )
      HalpDmaControllerFlushChannel(*(_QWORD *)(a1 + 368), *(unsigned int *)(a1 + 380));
    return 0LL;
  }
  while ( 1 )
  {
    if ( !v9 )
      return 3221225485LL;
    v14 = *(unsigned int *)(v9 + 40);
    if ( a4 < v14 )
      break;
    v9 = *(_QWORD *)v9;
    a4 -= v14;
  }
  if ( !*(_BYTE *)(a1 + 441) )
    HalpDmaControllerFlushChannel(*(_QWORD *)(a1 + 368), *(unsigned int *)(a1 + 380));
  if ( !*(_BYTE *)(a1 + 440) )
    return 0LL;
  v15 = a4;
  v16 = v9;
  if ( v8 )
  {
    v17 = *(unsigned int *)(v8 + 16);
    if ( *(_QWORD *)v9
      || (AdapterCacheAlignment = HalpDmaGetAdapterCacheAlignment(a1, a2, v17, a4 + *(unsigned int *)(v9 + 44) + v17),
          ((AdapterCacheAlignment - 1) & (unsigned int)v24) != 0)
      && v25 >= (v24 & 0xFFFFFFFFFFFFF000uLL) )
    {
      v12 = v17;
    }
    else
    {
      v12 = (~(AdapterCacheAlignment - 1) & (v25 + AdapterCacheAlignment - 1)) - v26 - a4;
      if ( v12 > (unsigned int)v17 )
        v12 = v17;
    }
  }
  HalpDmaCheckMdlAccessibility(a1, v9, (unsigned int)a4, v12, &v31, Mdl);
  v18 = v31;
  v19 = a6;
  if ( v31 )
  {
    v28 = Mdl[0];
    if ( !a6 )
      HalpDmaFlushDriverMdl((ULONG_PTR)Mdl[0]);
    if ( v28 )
    {
      do
      {
        v29 = v28->Next;
        IoFreeMdl(v28);
        v28 = v29;
      }
      while ( v29 );
    }
    if ( !v8 )
      return 0LL;
    v18 = v31;
LABEL_19:
    *(_QWORD *)(v8 + 56) = v8;
    while ( v12 )
    {
      v20 = v12;
      if ( !v16 )
        break;
      v21 = v12;
      if ( *(_DWORD *)(v16 + 40) - (int)v15 <= v12 )
        v21 = *(_DWORD *)(v16 + 40) - v15;
      v22 = v15 + *(_QWORD *)(v16 + 32) + *(unsigned int *)(v16 + 44);
      v12 -= v21;
      if ( v12 && !*(_QWORD *)v16 )
      {
        v21 = v20;
        v12 = 0;
      }
      if ( *(_BYTE *)(a1 + 442) )
        HalpDmaFlushScatterTransferV3(a1, v16, v8, v22, v21, v19, v18, (__int64)&v32);
      else
        HalpDmaFlushContiguousTransferV3(a1, v16, v8, v22, v21, v19, v18, &v32);
      v8 = v32;
      v15 = 0LL;
      v16 = *(_QWORD *)v16;
      v18 = v31;
    }
    return 0LL;
  }
  if ( v8 )
    goto LABEL_19;
  return 3221225473LL;
}
