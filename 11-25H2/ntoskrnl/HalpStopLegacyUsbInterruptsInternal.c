/*
 * XREFs of HalpStopLegacyUsbInterruptsInternal @ 0x140B405F0
 * Callers:
 *     HalpStopLegacyUsbInterrupts @ 0x140B3B210 (HalpStopLegacyUsbInterrupts.c)
 * Callees:
...
char __fastcall HalpStopLegacyUsbInterruptsInternal(int a1)
{
  ...
  if ( a1 < 5 )
  {
    ...
    while ( v3 != &HalpUsbControllerList )
    {
      ...
      {
        v2 = guard_dispatch_icall_no_overrides(*((unsigned int *)v4 + 4));
        if ( v2 )
        {
          ...
          if ( (unsigned __int16)(v6[0] - 1) <= 0xFFFDu )
          {
            ...
            {
              if ( (_BYTE)v2 == 16 )
                LOBYTE(v2) = HalpUsbLegacyStopOhciInterrupt(65533LL, *((_DWORD *)v4 + 4));
              else
                *((_BYTE *)v4 + 24) = 1;
            }
            else
