/*
 * XREFs of ExpSystemErrorHandler2 @ 0x140B51340
 * Callers:
 *     ExpSystemErrorHandler @ 0x1404FA9A0 (ExpSystemErrorHandler.c)
 * Callees:
...
void __fastcall ExpSystemErrorHandler2(unsigned int a1, __int64 a2, int a3, const char *a4, char a5)
{
  unsigned int v8; // edx
  __int16 v9; // di
  const void *v10; // r9
  ...
  STRING SourceString; // [rsp+88h] [rbp-1E0h] BYREF
  STRING v36; // [rsp+98h] [rbp-1D0h] BYREF
  ULONG_PTR BugCheckParameter1; // [rsp+A8h] [rbp-1C0h]
  const char *v38; // [rsp+B0h] [rbp-1B8h]
  unsigned __int16 *v39; // [rsp+B8h] [rbp-1B0h] BYREF
  ...
  *(_OWORD *)BugCheckParameter2 = 0LL;
  ...
  if ( SystemDllInfo )
  {
    ...
    if ( (int)RtlFindMessage(v19, 0xBu, v9 != 0 ? 0x409 : 0, a1, &v39) < 0 )
    {
      v17 = "Unknown Hard Error";
    ...
    if ( (v39[1] & 1) != 0 )
    {
      RtlInitUnicodeString(&UnicodeString, v39 + 2);
...
LABEL_20:
    ...
    else
    {
      ...
      while ( v24[v25] );
      v26 = v25 + 16;
      v27 = (char *)ExAllocatePool2(0x40uLL);
      v17 = v27;
      if ( !v27 )
      {
        v22 = "Unknown Hard Error";
