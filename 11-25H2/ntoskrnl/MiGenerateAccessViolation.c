/*
 * XREFs of MiGenerateAccessViolation @ 0x1403E297C
 * Callers:
 *     MiRaisedIrqlFault @ 0x1403E1B20 (MiRaisedIrqlFault.c)
 *     MiSystemFault @ 0x1403E1EBC (MiSystemFault.c)
 *     MiCheckSystemPageTables @ 0x1403E2760 (MiCheckSystemPageTables.c)
 * Callees:
 *     ExAcquireSpinLockShared @ 0x1402BBA30 (ExAcquireSpinLockShared.c)
 *     MiDeterminePoolType @ 0x1402FAC30 (MiDeterminePoolType.c)
 *     ExAcquireSpinLockSharedAtDpcLevel @ 0x14031F360 (ExAcquireSpinLockSharedAtDpcLevel.c)
 *     MiReleaseSpinLockShared @ 0x140323610 (MiReleaseSpinLockShared.c)
 */

__int64 __fastcall MiGenerateAccessViolation(unsigned __int64 *a1)
{
  unsigned __int64 v1; // rsi
  unsigned int v2; // edi
  unsigned __int8 CurrentIrql; // al
  _QWORD *v5; // rbx
  KIRQL v6; // bp

  v1 = *a1;
  v2 = 0;
  if ( !MiDeterminePoolType(*a1) )
    return 0LL;
  CurrentIrql = KeGetCurrentIrql();
  if ( CurrentIrql > 2u )
    return 0LL;
  if ( CurrentIrql == 2 )
  {
    v6 = 17;
    ExAcquireSpinLockSharedAtDpcLevel(&dword_140E2C830);
  }
  else
  {
    v6 = ExAcquireSpinLockShared(&dword_140E2C830);
  }
  v5 = (_QWORD *)qword_140E2C838;
  while ( v5 )
  {
    if ( v1 > v5[4] )
    {
      v5 = (_QWORD *)v5[1];
    }
    else
    {
      if ( v1 >= v5[3] )
        break;
      v5 = (_QWORD *)*v5;
    }
  }
  MiReleaseSpinLockShared(&dword_140E2C830, v6);
  LOBYTE(v2) = v5 != 0LL;
  return v2;
}
