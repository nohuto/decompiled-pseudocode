/*
 * XREFs of IoVolumeDeviceNameToGuidPath @ 0x140822A30
 * Callers:
...
{
  int v2; // eax
  LARGE_INTEGER *OutputBuffer; // rsi
  char v6; // r13
  ULONG v7; // edi
  __int64 Pool2; // rax
  void *v9; // r15
      ...
      if ( OutputBuffer )
      {
        ...
        if ( v14 )
        {
          ...
          if ( DeviceObjectPointer >= 0 )
          {
            ...
            for ( i = 0; i < OutputBuffer->HighPart; ++i )
            {
              v16 = (char *)OutputBuffer + OutputBuffer[3 * i + 1].LowPart;
              DeviceObject = (PDEVICE_OBJECT)(3LL * i);
              if ( !wcsnicmp(L"\\??\\Volume", (const wchar_t *)v16, 0xAuLL) )
              {
