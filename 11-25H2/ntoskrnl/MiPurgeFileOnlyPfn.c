/*
 * XREFs of MiPurgeFileOnlyPfn @ 0x1406703DC
 * Callers:
 *     MiPurgeBadFileOnlyPages @ 0x1406701CC (MiPurgeBadFileOnlyPages.c)
 * Callees:
 *     MiUnlockProtoPoolPage @ 0x1402353A0 (MiUnlockProtoPoolPage.c)
 *     MiLockLeafPage @ 0x14033B770 (MiLockLeafPage.c)
 *     MiCheckProtoPtePageState @ 0x14033E110 (MiCheckProtoPtePageState.c)
 *     MiDeleteTransitionPte @ 0x140340900 (MiDeleteTransitionPte.c)
 *     MiSetPfnModified @ 0x1403871E0 (MiSetPfnModified.c)
 *     MiSubsectionProtosCreated @ 0x1404CFB8C (MiSubsectionProtosCreated.c)
 *     KeBugCheckEx @ 0x1404F9280 (KeBugCheckEx.c)
 *     MiChangingSubsectionProtos @ 0x14066EB8C (MiChangingSubsectionProtos.c)
 *     MiPersistPage @ 0x140670158 (MiPersistPage.c)
 */

__int64 __fastcall MiPurgeFileOnlyPfn(__int64 a1)
{
  __int64 v1; // rsi
  int v2; // edi
  __int64 v3; // r14
  __int64 v4; // rsi
  unsigned __int64 *v5; // r14
  ULONG_PTR v6; // rbp
  __int64 v7; // r8
  unsigned __int64 v8; // r9
  __int64 v9; // rbx
  unsigned __int64 v10; // rcx
  unsigned __int64 v12; // rax
  __int64 v13; // rax
  char v14; // al
  _OWORD v15[2]; // [rsp+30h] [rbp-48h] BYREF
  __int64 v16; // [rsp+50h] [rbp-28h]
  unsigned __int8 v17; // [rsp+80h] [rbp+8h] BYREF

  v1 = *(_QWORD *)(a1 + 16);
  v2 = 0;
  memset(v15, 0, sizeof(v15));
  v16 = 0LL;
  if ( qword_140E2D940 && (v1 & 0x10) == 0 )
    v1 &= ~qword_140E2D940;
  v3 = *(_QWORD *)(a1 + 8);
  v4 = v1 >> 16;
  v17 = 17;
  v5 = (unsigned __int64 *)(v3 | 0x8000000000000000uLL);
  MiChangingSubsectionProtos((__int64 *)v4, 32, (__int64)v15);
  v6 = MiCheckProtoPtePageState(*(_QWORD *)v4, (unsigned __int64)v5, 0, &v17);
  if ( v6 )
  {
    v9 = MiLockLeafPage(v5, 0);
    if ( v9 )
    {
      v10 = *v5;
      if ( (*v5 & 1) != 0 )
        goto LABEL_7;
      v12 = *v5;
      if ( qword_140E2D940 )
      {
        if ( (v10 & 0x10) != 0 )
          v12 = v10 & 0xFFFFFFFFFFFFFFEFuLL;
        else
          v12 = v10 & ~qword_140E2D940;
      }
      v7 = 0xFFFFDE0000000000uLL;
      if ( v9 != 48 * ((v12 >> 12) & 0xFFFFFFFFFFLL) - 0x220000000000LL )
        goto LABEL_7;
      v13 = *(_QWORD *)(v9 + 16);
      if ( (v13 & 0x400) == 0
        || (v13 & 1) != 0
        || (unsigned __int64 *)(*(_QWORD *)(v9 + 8) | 0x8000000000000000uLL) != v5 )
      {
        KeBugCheckEx(0xDEu, 2uLL, (ULONG_PTR)v5, *(_QWORD *)(v9 + 8), *v5);
      }
      v14 = *(_BYTE *)(v9 + 34);
      if ( (v14 & 8) != 0 )
      {
LABEL_7:
        _InterlockedAnd64((volatile signed __int64 *)(v9 + 24), 0x7FFFFFFFFFFFFFFFuLL);
      }
      else
      {
        if ( (v14 & 0x10) != 0 )
        {
          if ( *(char *)(v9 + 35) >= 0 )
            MiPersistPage(0xAAAAAAAAAAAAAAABuLL * ((v9 + 0x220000000000LL) >> 4));
          MiSetPfnModified(v9, 0);
        }
        MiDeleteTransitionPte((__int64 *)v5, v9, 2u, 0);
        v2 = 1;
      }
    }
  }
  if ( v17 != 17 )
    MiUnlockProtoPoolPage(v6, v17, v7, v8);
  return MiSubsectionProtosCreated(v4, (__int64)v15, 0, v2);
}
