/*
 * XREFs of SymCryptFdefRawSubUint32 @ 0x14052D06C
 * Callers:
 *     SymCryptFdefIntSubMixedSize @ 0x14052C968 (SymCryptFdefIntSubMixedSize.c)
 *     SymCryptFdefIntSubUint32 @ 0x14052CA78 (SymCryptFdefIntSubUint32.c)
 *     SymCryptFdefModElementSetValueNegUint32 @ 0x140533694 (SymCryptFdefModElementSetValueNegUint32.c)
 *     SymCryptFdefModSetRandomGeneric @ 0x140534120 (SymCryptFdefModSetRandomGeneric.c)
 * Callees:
 *     <none>
 */

__int64 __fastcall SymCryptFdefRawSubUint32(__int64 a1, unsigned int a2, _DWORD *a3, int a4)
{
  unsigned int v4; // r9d
  __int64 v5; // r10
  __int64 v6; // r11
  unsigned __int64 v7; // rcx

  v4 = 16 * a4;
  if ( v4 )
  {
    v5 = a1 - (_QWORD)a3;
    v6 = v4;
    do
    {
      v7 = *(unsigned int *)((char *)a3 + v5) - (unsigned __int64)a2;
      *a3++ = v7;
      a2 = BYTE4(v7) & 1;
      --v6;
    }
    while ( v6 );
  }
  return a2;
}
