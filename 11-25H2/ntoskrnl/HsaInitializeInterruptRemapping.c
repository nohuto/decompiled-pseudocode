/*
 * XREFs of HsaInitializeInterruptRemapping @ 0x140B42B68
 * Callers:
 *     HsaInitializeIommu @ 0x140B42D00 (HsaInitializeIommu.c)
 * Callees:
 *     RtlInitializeBitMap @ 0x140461BB0 (RtlInitializeBitMap.c)
  ...
  if ( v3 )
  {
    if ( v3 == 1 )
    {
      if ( !*((_QWORD *)&HsaSharedRemappingTable + 1) )
      {
        ...
        v4 = guard_dispatch_icall_no_overrides(-1LL);
        *(_QWORD *)&HsaSharedRemappingTable = v4;
        if ( v4
          && (v5 = HalMapIoSpace((LARGE_INTEGER)v4, (unsigned int)NumberOfBytes, MmCached),
              (*((_QWORD *)&HsaSharedRemappingTable + 1) = v5) != 0LL) )
        {
          memset_0(v5, 0, (unsigned int)NumberOfBytes);
