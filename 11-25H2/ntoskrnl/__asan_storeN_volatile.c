/*
 * XREFs of __asan_storeN_volatile @ 0x1405A99E0
 * Callers:
 *     <none>
 * Callees:
...
signed __int8 __fastcall _asan_storeN_volatile(ULONG_PTR BugCheckParameter1, ULONG_PTR BugCheckParameter2)
{
      ...
      if ( BugCheckParameter2 )
      {
        ...
        if ( v7 )
        {
          if ( v7 < 8 )
          {
            ...
            if ( result > v7 )
LABEL_12:
              result = (unsigned __int8)KasaniReport(BugCheckParameter1, BugCheckParameter2, 1u, retaddr, v7);
          }
        }
      }
      if ( KcsaniValidationEnabled )
        return KcsanValidateAddress(BugCheckParameter1, BugCheckParameter2, 1, 1, retaddr);
    }
  }
  return result;
}
