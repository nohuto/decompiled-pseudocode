/*
 * XREFs of PsLeavePriorityRegion @ 0x1402EAB00
 * Callers:
 *     <none>
 * Callees:
...
void PsLeavePriorityRegion()
{
  ...
  _KDPC **p_Dpc; // rsi
  _KDPC *v7; // rcx
  $748129697366ACACF652522F4AB73961 *v8; // rsi
  ...
  if ( LODWORD(CurrentThread[1].Timer.TimerListEntry.Flink) )
  {
    ...
    if ( LODWORD(CurrentThread[1].Timer.TimerListEntry.Flink) )
    {
      if ( v3 )
      {
        ...
        if ( ($748129697366ACACF652522F4AB73961 *)Blink->Flink != &CurrentThread[1].320 )
LABEL_19:
          __fastfail(3u);
    ...
    else
    {
      ...
      while ( 1 )
      {
        ...
        if ( ($748129697366ACACF652522F4AB73961 *)v8->WaitBlock[0].WaitListEntry.Flink == v8 )
          break;
        v10 = Flink->Flink;
        if ( ($748129697366ACACF652522F4AB73961 *)Flink->Blink != v8 || v10->Blink != Flink )
          goto LABEL_19;
