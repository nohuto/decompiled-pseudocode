/*
 * XREFs of RtlGetCompressionWorkSpaceSize @ 0x140393E20
 * Callers:
 *     ?StStart@?$ST_STORE@USM_TRAITS@@@@SAJPEAU1@PEAU_ST_CREATE_PARAMS@@@Z @ 0x1403941C8 (-StStart@-$ST_STORE@USM_TRAITS@@@@SAJPEAU1@PEAU_ST_CREATE_PARAMS@@@Z.c)
 *     ?SmCompressCtxStart@?$SMKM_STORE_MGR@USM_TRAITS@@@@SAJPEAU_SM_COMPRESS_CONTEXT@1@W4_ST_COMPRESSION_FORMAT@@@Z @ 0x1405FDB60 (-SmCompressCtxStart@-$SMKM_STORE_MGR@USM_TRAITS@@@@SAJPEAU_SM_COMPRESS_CONTEXT@1@W4_ST_COMPRESSI.c)
 *     PopHiberInitializeResources @ 0x140746400 (PopHiberInitializeResources.c)
 *     EtwpInitializeCompressedWriter @ 0x1407A04AC (EtwpInitializeCompressedWriter.c)
 *     sub_140BD00B0 @ 0x140BD00B0 (sub_140BD00B0.c)
 * Callees:
 *     _guard_dispatch_icall_no_overrides @ 0x1406A8B20 (_guard_dispatch_icall_no_overrides.c)
 *     RtlCompressWorkSpaceSizeLZNT1 @ 0x140B59950 (RtlCompressWorkSpaceSizeLZNT1.c)
 */

NTSTATUS __stdcall RtlGetCompressionWorkSpaceSize(
        USHORT CompressionFormatAndEngine,
        PULONG CompressBufferWorkSpaceSize,
        PULONG CompressFragmentWorkSpaceSize)
{
  unsigned __int16 v3; // ax
  __int16 v4; // cx

  v3 = (unsigned __int8)CompressionFormatAndEngine;
  if ( (unsigned __int8)CompressionFormatAndEngine < 2u )
    return -1073741811;
  if ( (unsigned __int8)CompressionFormatAndEngine > 8u )
    return -1073741217;
  v4 = CompressionFormatAndEngine & 0xFF00;
  if ( (__int64 (__fastcall *)(_QWORD, _QWORD, _QWORD))RtlWorkSpaceProcs[v3] == RtlCompressWorkSpaceSizeLZNT1 )
    return RtlCompressWorkSpaceSizeLZNT1(v4, CompressBufferWorkSpaceSize, CompressFragmentWorkSpaceSize);
  else
    return guard_dispatch_icall_no_overrides(v4, CompressBufferWorkSpaceSize);
}
