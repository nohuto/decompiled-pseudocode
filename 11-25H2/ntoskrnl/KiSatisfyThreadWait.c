/*
 * XREFs of KiSatisfyThreadWait @ 0x1402F4960
 * Callers:
 *     KeWaitForMultipleObjects @ 0x14027C240 (KeWaitForMultipleObjects.c)
 * Callees:
 *     KiProcessDeferredReadyList @ 0x140285EA0 (KiProcessDeferredReadyList.c)
 *     KiDeliverApc @ 0x140286490 (KiDeliverApc.c)
 *     KeAbPreAcquire @ 0x14029B110 (KeAbPreAcquire.c)
 *     KiComputeThreadPriority @ 0x1402E7250 (KiComputeThreadPriority.c)
 *     KiSetPriorityThread @ 0x1402E7840 (KiSetPriorityThread.c)
 *     KiExitThreadWaitReschedule @ 0x1402F39F4 (KiExitThreadWaitReschedule.c)
 *     HvlNotifyLongSpinWait @ 0x14032DED0 (HvlNotifyLongSpinWait.c)
 *     KiCheckVpBackingLongSpinWaitHypercall @ 0x14032DF00 (KiCheckVpBackingLongSpinWaitHypercall.c)
 *     KiLowerIrqlProcessIrqlFlags @ 0x1404F1088 (KiLowerIrqlProcessIrqlFlags.c)
 *     KeBugCheckEx @ 0x1404F9280 (KeBugCheckEx.c)
 */

__int64 __fastcall KiSatisfyThreadWait(struct _KPRCB *a1, __int64 BugCheckParameter1, char a3, __int64 a4, int a5)
{
  unsigned int v5; // r15d
  struct _KPRCB *v6; // rsi
  __int64 v7; // r12
  __int64 v9; // rcx
  __int64 v12; // rsi
  char v13; // al
  int v14; // ebx
  unsigned __int64 v15; // rdi
  volatile signed __int32 *v17; // r14
  __int64 *v18; // rax
  unsigned int v19; // edi
  ULONG_PTR v20; // rdx
  char v21; // al
  char v22; // al
  unsigned int v23; // eax
  int v24; // r8d
  __int64 *v25; // rax
  _QWORD *v26; // [rsp+38h] [rbp-30h] BYREF

  v5 = 0;
  *(_BYTE *)(BugCheckParameter1 + 388) = 2;
  *(_QWORD *)(BugCheckParameter1 + 64) = 0LL;
  v6 = a1;
  v7 = *(_QWORD *)(BugCheckParameter1 + 200);
  v9 = *(_QWORD *)(BugCheckParameter1 + 976);
  if ( v9 )
  {
    *(_QWORD *)(BugCheckParameter1 + 976) = 0LL;
    v25 = KeAbPreAcquire(v9, 0LL);
    if ( v25 )
      *((_BYTE *)v25 + 10) = 1;
  }
  if ( a5 )
  {
    v12 = a4 + 48LL * (unsigned __int8)a5;
    do
    {
      if ( *(_BYTE *)(a4 + 17) < 5u )
      {
        v17 = *(volatile signed __int32 **)(a4 + 32);
        if ( _interlockedbittestandset(v17, 7u) )
        {
          do
          {
            if ( (++v5 & HvlLongSpinCountMask) == 0
              && (HvlEnlightenments & 0x40) != 0
              && (unsigned __int8)KiCheckVpBackingLongSpinWaitHypercall(v9) )
            {
              HvlNotifyLongSpinWait(v5);
            }
            else
            {
              _mm_pause();
            }
          }
          while ( (*v17 & 0x80u) != 0 || _interlockedbittestandset(v17, 7u) );
        }
        if ( *(_BYTE *)(a4 + 17) == 4 )
        {
          v9 = *(_QWORD *)a4;
          v18 = *(__int64 **)(a4 + 8);
          if ( *(_QWORD *)(*(_QWORD *)a4 + 8LL) != a4 || *v18 != a4 )
            __fastfail(3u);
          *v18 = v9;
          *(_QWORD *)(v9 + 8) = v18;
        }
        _InterlockedAnd(v17, 0xFFFFFF7F);
        v5 = 0;
      }
      a4 += 48LL;
    }
    while ( a4 != v12 );
    v6 = a1;
  }
  v26 = 0LL;
  if ( (a3 & 2) != 0 )
  {
    v19 = 0;
    while ( _interlockedbittestandset64((volatile signed __int32 *)(BugCheckParameter1 + 64), 0LL) )
    {
      do
      {
        if ( (++v19 & HvlLongSpinCountMask) == 0
          && (HvlEnlightenments & 0x40) != 0
          && (unsigned __int8)KiCheckVpBackingLongSpinWaitHypercall(v9) )
        {
          HvlNotifyLongSpinWait(v19);
        }
        else
        {
          _mm_pause();
        }
      }
      while ( *(_QWORD *)(BugCheckParameter1 + 64) );
    }
    v20 = *(char *)(BugCheckParameter1 + 795);
    v21 = *(_BYTE *)(v20 + BugCheckParameter1 + 824);
    if ( !v21 )
      KeBugCheckEx(0x157u, BugCheckParameter1, v20, 2uLL, 0LL);
    v22 = v21 - 1;
    *(_BYTE *)(v20 + BugCheckParameter1 + 824) = v22;
    if ( !v22 )
    {
      v23 = *(_DWORD *)(BugCheckParameter1 + 856) ^ (1 << v20);
      *(_DWORD *)(BugCheckParameter1 + 856) = v23;
      if ( v23 < 1 << v20 && *(char *)(BugCheckParameter1 + 195) <= 31 )
      {
        v24 = KiComputeThreadPriority(BugCheckParameter1, 0);
        if ( v24 < *(char *)(BugCheckParameter1 + 195) )
          KiSetPriorityThread((_BYTE *)BugCheckParameter1, (__int64)&v26, v24);
      }
    }
    *(_BYTE *)(BugCheckParameter1 + 795) = 32;
    *(_QWORD *)(BugCheckParameter1 + 64) = 0LL;
  }
  v13 = *(_BYTE *)(BugCheckParameter1 + 112);
  v14 = a3 & 1;
  if ( (v13 & 0x38) != 0 )
  {
    if ( (v13 & 0x18) != 0 )
    {
      KiExitThreadWaitReschedule(v6, BugCheckParameter1, &v26);
    }
    else
    {
      KiProcessDeferredReadyList(v6, &v26, 1u);
      KiDeliverApc(0, 0LL, 0LL);
      if ( KiIrqlFlags )
        KiLowerIrqlProcessIrqlFlags(KeGetCurrentIrql(), 0LL);
      __writecr8(0LL);
    }
  }
  else
  {
    v15 = *(unsigned __int8 *)(BugCheckParameter1 + 390);
    if ( v14 )
    {
      KiProcessDeferredReadyList(v6, &v26, v15);
    }
    else
    {
      if ( KiIrqlFlags )
        KiLowerIrqlProcessIrqlFlags(KeGetCurrentIrql(), (unsigned __int8)v15);
      __writecr8(v15);
    }
  }
  return v7;
}
