/*
 * XREFs of ViInitPickRandomTargets @ 0x140C2A2B8
 * Callers:
...
__int64 ViInitPickRandomTargets()
{
  ...
  if ( VfRandomVerifiedDrivers )
  {
    ...
    if ( (unsigned int)(ViExpectedDriversCount + 2) > 0x200 )
      v2 = 512;
    qword_140FFCBF8 = (__int64)&VfRandomTargetsBitMap;
    ...
    for ( i = 0; i < VfRandomVerifiedDrivers; ++i )
    {
      ...
      if ( ((*(char *)(v6 + qword_140FFCBF8) >> (Number & 7)) & 1) != 0 )
      {
        if ( v3 )
        {
          ++v1;
        ...
        else
        {
          ...
          while ( 1 )
          {
            ...
            if ( ((*(char *)(v8 + qword_140FFCBF8) >> (v7 & 7)) & 1) == 0 )
              break;
            if ( v7 == Number )
              goto LABEL_17;
          }
          *(_BYTE *)(v8 + qword_140FFCBF8) |= 1 << (v7 & 7);
LABEL_17:
          if ( v7 == Number )
            break;
        }
      ...
      {
        *(_BYTE *)(v6 + qword_140FFCBF8) |= 1 << (Number & 7);
      }
    }
