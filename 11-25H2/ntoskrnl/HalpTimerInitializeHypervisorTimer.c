/*
 * XREFs of HalpTimerInitializeHypervisorTimer @ 0x1405478D4
 * Callers:
            ...
            *(_DWORD *)(v5 + 184) |= 4u;
            ...
            if ( (int)_RAX >= 0 )
            {
              ...
              else
              {
                ...
                if ( (v11 & 0x200) != 0 )
                {
                  LODWORD(_RAX) = HalpInterruptGsiToLine(*(unsigned int *)(v5 + 88), &v21);
                  if ( (int)_RAX < 0 )
                    return _RAX;
                }
                else if ( (v11 & 0x100) != 0 )
              ...
              LODWORD(_RAX) = guard_dispatch_icall_no_overrides(*v7);
              if ( (int)_RAX >= 0 )
              {
                if ( v19[0] )
                {
