/*
 * XREFs of FsRtlIsFatDbcsLegal @ 0x1409D2400
 * Callers:
 *     FsRtlIsFatDbcsLegal @ 0x1409D2400 (FsRtlIsFatDbcsLegal.c)
 * Callees:
 *     FsRtlDissectDbcs @ 0x140701270 (FsRtlDissectDbcs.c)
 *     FsRtlIsFatDbcsLegal @ 0x1409D2400 (FsRtlIsFatDbcsLegal.c)
 *     FsRtlDoesDbcsContainWildCards @ 0x1409D2660 (FsRtlDoesDbcsContainWildCards.c)
 */
        ...
        BOOLEAN LeadingBackslashPermissible)
{
  ...
  char v17; // dl
  STRING v18; // xmm1
  STRING FirstName; // [rsp+20h] [rbp-48h] BYREF
  STRING RemainingName; // [rsp+30h] [rbp-38h] BYREF
  STRING Name; // [rsp+40h] [rbp-28h] BYREF
...
LABEL_4:
    ...
    if ( WildCardsPermissible && FsRtlDoesDbcsContainWildCards(DbcsName) )
    {
      ...
      {
        v16 = (unsigned __int8)DbcsName->Buffer[i];
        if ( (unsigned __int8)v16 >= 0x80u && (_BYTE)NlsMbOemCodePageTag && *((_WORD *)NlsOemLeadByteInfo + v16) )
        {
          ++i;
        }
        else if ( (v16 & 0x80u) == 0LL && (*((_BYTE *)qword_140013DE0 + v16) & 9) == 0 )
        {
          return 0;
        }
      }
    ...
    if ( v10 <= 0xC )
    {
      for ( j = 0; j < v10; ++j )
      {
        ...
        {
          if ( (v8 & 0x80u) == 0LL
            && ((unsigned __int8)(WildCardsPermissible != 0 ? 9 : 1) & *((_BYTE *)qword_140013DE0 + v8)) == 0 )
          {
            return 0;
          }
