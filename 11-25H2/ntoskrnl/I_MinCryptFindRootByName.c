/*
 * XREFs of I_MinCryptFindRootByName @ 0x140814F00
 * Callers:
...
__int64 *__fastcall I_MinCryptFindRootByName(const void **a1, __int64 a2)
{
  ...
  for ( i = 0; i < 0x13; ++i )
  {
    if ( *(_DWORD *)a1 == LODWORD(RootTable[5 * i])
      && RtlCompareMemory(a1[1], (&off_140B2FC58)[5 * i], *(unsigned int *)a1) == *(_DWORD *)a1 )
    {
  ...
  if ( a2 && (*(_DWORD *)(a2 + 4) & 0x80u) != 0 )
  {
    while ( v2 < 2 )
    {
      if ( *(_DWORD *)a1 == *(_DWORD *)&AltRootTable[20 * v2]
        && RtlCompareMemory(a1[1], *(&off_140B2D5E8 + 5 * v2), *(unsigned int *)a1) == *(_DWORD *)a1 )
      {
