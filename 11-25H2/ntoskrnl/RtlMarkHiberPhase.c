/*
 * XREFs of RtlMarkHiberPhase @ 0x1405E5D10
 * Callers:
...
void RtlMarkHiberPhase()
{
  ...
  if ( (KiBugCheckActive & 3) == 0 )
  {
    ...
    if ( v1 == 8 )
    {
      v2 = (unsigned __int64)&byte_1400146AF >> 12;
      for ( i = (unsigned __int64)XpressHashFunction >> 12; i < v2; i += j )
      {
        ...
        for ( j = 1LL; v4 < v2; ++v4 )
        {
          if ( v5.QuadPart - i + v4 != (unsigned __int64)MmGetPhysicalAddress((PVOID)((i << 12) + (j << 12))).QuadPart >> 12 )
            break;
          ++j;
        }
