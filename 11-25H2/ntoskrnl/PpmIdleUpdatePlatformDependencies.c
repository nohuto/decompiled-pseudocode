/*
 * XREFs of PpmIdleUpdatePlatformDependencies @ 0x140A3483C
 * Callers:
 *     PpmInstallPlatformIdleStates @ 0x140A34590 (PpmInstallPlatformIdleStates.c)
 * Callees:
 *     <none>
 */

__int64 __fastcall PpmIdleUpdatePlatformDependencies(__int64 a1)
{
  __int64 result; // rax
  bool v2; // zf
  __int64 v3; // r8
  bool v4; // r11
  __int64 v5; // r10

  result = HvlpFlags;
  if ( (HvlpFlags & 2) != 0 )
  {
    result = (unsigned int)HvlEnlightenments;
    if ( (HvlEnlightenments & 0x10000000) != 0 )
      return result;
    v2 = (HvlEnlightenments & 0x400) == 0;
  }
  else
  {
    v2 = HvlHypervisorConnected == 0;
  }
  if ( !v2 )
  {
    v3 = 0LL;
    result = HvlEnlightenments & 0x200;
    if ( *(_DWORD *)(a1 + 4) )
    {
      v4 = (_DWORD)result != 0;
      do
      {
        v5 = 0LL;
        for ( *(_BYTE *)(a1 + 48 * v3 + 53) = v4;
              (unsigned int)v5 < *(_DWORD *)(a1 + 48 * v3 + 64);
              v5 = (unsigned int)(v5 + 1) )
        {
          result = *(_QWORD *)(a1 + 48 * v3 + 88);
          *(_BYTE *)(result + 8 * v5 + 4) = v4;
        }
        v3 = (unsigned int)(v3 + 1);
      }
      while ( (unsigned int)v3 < *(_DWORD *)(a1 + 4) );
    }
  }
  return result;
}
