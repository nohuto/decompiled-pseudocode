/*
 * XREFs of IopAddMiniDumpPagesToPartialKernelDump @ 0x14059CF28
 * Callers:
  ...
  if ( (int)result >= 0 )
  {
    ...
    if ( (int)result >= 0 )
    {
        ...
        || (result = IopMarkPagesForRunTimeTriageDataBlocks(v15, a6, v13, v13 + a8), (int)result >= 0) )
      {
        ...
        if ( (int)result >= 0 )
        {
          result = MmAddRangeToCrashDump(v15, a7->ApcState.Process, 2112LL);
          if ( (int)result >= 0 )
          {
            result = MmAddRangeToCrashDump(v15, a7, 1944LL);
            if ( (int)result >= 0 )
            {
