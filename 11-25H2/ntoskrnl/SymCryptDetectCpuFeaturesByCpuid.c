/*
 * XREFs of SymCryptDetectCpuFeaturesByCpuid @ 0x14051B288
 * Callers:
 *     SymCryptInitEnvWindowsKernelmodeWin8_1nLater @ 0x1405184E8 (SymCryptInitEnvWindowsKernelmodeWin8_1nLater.c)
 * Callees:
 *     SymCryptCpuidExFunc @ 0x1405ADAE8 (SymCryptCpuidExFunc.c)
 *     __security_check_cookie @ 0x14069A6F0 (__security_check_cookie.c)
 */

__int64 __fastcall SymCryptDetectCpuFeaturesByCpuid(char a1)
{
  int v2; // r14d
  int v3; // ebx
  int v4; // r12d
  __int64 *v5; // rdi
  unsigned int i; // esi
  int v7; // ecx
  char v8; // al
  __int64 result; // rax
  _DWORD v10[2]; // [rsp+20h] [rbp-20h] BYREF
  int v11; // [rsp+28h] [rbp-18h]
  int v12; // [rsp+2Ch] [rbp-14h]

  v2 = 0;
  v3 = -16352;
  SymCryptCpuidExFunc(v10, 0LL, 0LL);
  v4 = v10[0];
  v5 = cpuidBitInfo;
  for ( i = 0; i < 0x14; ++i )
  {
    if ( *(unsigned __int8 *)v5 != v2 )
    {
      v2 = *(unsigned __int8 *)v5;
      SymCryptCpuidExFunc(v10, *(unsigned __int8 *)v5, 0LL);
    }
    if ( *(unsigned __int8 *)v5 > v4
      || (v7 = v10[*((unsigned __int8 *)v5 + 1)], !_bittest(&v7, *((unsigned __int8 *)v5 + 2))) )
    {
      v3 |= *((_DWORD *)v5 + 1);
    }
    ++v5;
  }
  if ( (a1 & 1) != 0 )
  {
    v8 = SymCryptCpuidExFunc(v10, 1LL, 0LL);
    if ( (v11 & 0x8000000) == 0 )
      goto LABEL_13;
    __asm { xgetbv }
    if ( (v8 & 6) == 6 )
    {
      if ( (v8 & 0xE0) == 0xE0 )
        goto LABEL_15;
    }
    else
    {
LABEL_13:
      v3 |= 0x10u;
    }
    v3 |= 0x1000u;
  }
LABEL_15:
  if ( (v3 & 4) == 0 )
  {
    SymCryptCpuidExFunc(v10, 0LL, 0LL);
    if ( v10[1] == 1752462657 && v11 == 1145913699 && v12 == 1769238117 )
    {
      SymCryptCpuidExFunc(v10, 1LL, 0LL);
      if ( ((v10[0] >> 8) & 0xF) != 0xF || (unsigned int)(unsigned __int8)(v10[0] >> 20) + 15 < 0x15 )
        v3 |= 4u;
    }
  }
  result = SymCryptCpuidExFunc(&g_SymCryptCpuid1, 1LL, 0LL);
  g_SymCryptCpuFeaturesNotPresent = v3;
  return result;
}
