/*
 * XREFs of ?KiResortScbQueue@@YAEPEAU_KPRCB@@PEAU_KSCB@@E@Z @ 0x140351E34
 * Callers:
...
LABEL_25:
  ...
  {
    for ( k = a2->QueueNode.ParentValue; ; k = v8->ParentValue )
    {
      v8 = (_RTL_BALANCED_NODE *)(k & 0xFFFFFFFFFFFFFFFCuLL);
  ...
  if ( v8 )
  {
    v13 = a2->Rank - HIDWORD(v8[1].Right);
    if ( !v13 )
    {
      v14 = a2->ReadySummary;
      if ( !(_WORD)v14 )
      {
        if ( !a2->Rank && (_RTL_BALANCED_NODE *)a2->GenerationCycles <= v8[-4].Children[1] )
          return v3;
        goto LABEL_25;
      }
      Right_low = LOWORD(v8[1].Right);
