/*
 * XREFs of PdcTaskClientRegister @ 0x14077EBDC
 * Callers:
 *     PopUserShutdownScenarioInitialize @ 0x140C1DBAC (PopUserShutdownScenarioInitialize.c)
 *     PopDiagInitialize @ 0x140C1DDA4 (PopDiagInitialize.c)
 * Callees:
 *     memset_0 @ 0x1406B4D40 (memset_0.c)
 *     PdcPortOpenCommon @ 0x14077EC84 (PdcPortOpenCommon.c)
 *     PdcAllocate @ 0x140A75538 (PdcAllocate.c)
 *     ExFreePoolWithTag @ 0x140B62CD0 (ExFreePoolWithTag.c)
 */

__int64 __fastcall PdcTaskClientRegister(__int64 a1, _QWORD *a2)
{
  int v3; // edi
  _DWORD *v4; // rax
  _DWORD *v5; // rbx
  unsigned int v6; // edi
  int v8; // [rsp+20h] [rbp-38h]
  int v9; // [rsp+28h] [rbp-30h]
  int v10; // [rsp+38h] [rbp-20h]
  __int64 v11; // [rsp+68h] [rbp+10h] BYREF

  *a2 = 0LL;
  v3 = a1;
  LOBYTE(a1) = 1;
  v4 = (_DWORD *)PdcAllocate(a1, 64LL, 1667458128LL);
  v5 = v4;
  if ( v4 )
  {
    memset_0(v4, 0, 0x40uLL);
    v11 = 0LL;
    *v5 = 1667458128;
    v5[8] = v3;
    v6 = PdcPortOpenCommon(v3, 4, (int)v5, (int)&v11, v8, v9, 0LL, v10, (__int64)(v5 + 10));
    if ( v6 )
      ExFreePoolWithTag(v5, 0);
    else
      *a2 = v5;
  }
  else
  {
    return (unsigned int)-1073741801;
  }
  return v6;
}
