/*
 * XREFs of DrvDbUnloadDatabaseNode @ 0x1409317DC
 * Callers:
 *     DrvDbSetDriverDatabaseMappedProperty @ 0x1408127F4 (DrvDbSetDriverDatabaseMappedProperty.c)
 *     DrvDbDestroyDatabaseNode @ 0x140813F10 (DrvDbDestroyDatabaseNode.c)
 *     DrvDbReleaseDatabaseNodeBaseKey @ 0x140930CDC (DrvDbReleaseDatabaseNodeBaseKey.c)
 *     DrvDbOpenObjectRegKey @ 0x140931000 (DrvDbOpenObjectRegKey.c)
 *     DrvDbLoadDatabaseNode @ 0x140931608 (DrvDbLoadDatabaseNode.c)
 *     DrvDbOpenDriverDatabaseRegKey @ 0x140A84448 (DrvDbOpenDriverDatabaseRegKey.c)
 * Callees:
 *     ZwClose @ 0x14069B320 (ZwClose.c)
 *     _guard_dispatch_icall_no_overrides @ 0x1406A8B20 (_guard_dispatch_icall_no_overrides.c)
 *     PiDrvDbNodeActionCallback @ 0x140931920 (PiDrvDbNodeActionCallback.c)
 */

__int64 __fastcall DrvDbUnloadDatabaseNode(__int64 a1, __int64 a2)
{
  __int64 (__fastcall *v2)(int, int, int, int, __int64, __int64); // rsi
  NTSTATUS v3; // ebx
  __int64 v6; // rax
  __int64 v7; // rdx
  int v8; // eax
  unsigned int v9; // eax
  _QWORD *v11; // r14
  unsigned int v12; // ebp
  HANDLE *v13; // r14
  __int64 v14; // [rsp+78h] [rbp+10h] BYREF

  v2 = *(__int64 (__fastcall **)(int, int, int, int, __int64, __int64))(a2 + 80);
  v3 = 0;
  v14 = 0LL;
  if ( v2 )
  {
    v14 = a2 + 96;
    v6 = *(_QWORD *)(a2 + 88);
    v7 = *(_QWORD *)(a2 + 24);
    if ( v2 == PiDrvDbNodeActionCallback )
      v8 = PiDrvDbNodeActionCallback(a1, v7, 2, 1, (__int64)&v14, v6);
    else
      v8 = guard_dispatch_icall_no_overrides(a1);
    if ( v8 == -1073741822 )
    {
      v2 = 0LL;
    }
    else if ( v8 < 0 )
    {
      v3 = v8;
LABEL_8:
      if ( v2 == PiDrvDbNodeActionCallback )
        v9 = PiDrvDbNodeActionCallback(a1, *(_QWORD *)(a2 + 24), 2, 2, (__int64)&v14, *(_QWORD *)(a2 + 88));
      else
        v9 = guard_dispatch_icall_no_overrides(a1);
      if ( (int)(v9 + 0x80000000) >= 0 && v9 != -1073741822 && !v3 )
        return v9;
      return (unsigned int)v3;
    }
  }
  if ( *(_QWORD *)(a2 + 96) )
  {
    v11 = (_QWORD *)(a2 + 96);
    if ( (*(_DWORD *)(a2 + 64) & 2) != 0 )
    {
      v12 = 0;
      v13 = (HANDLE *)(a2 + 104);
      do
      {
        if ( *v13 )
        {
          v3 = ZwClose(*v13);
          if ( v3 < 0 )
            goto LABEL_7;
          *v13 = 0LL;
        }
        ++v12;
        ++v13;
      }
      while ( v12 < 6 );
      v11 = (_QWORD *)(a2 + 96);
    }
    v3 = ZwClose(*(HANDLE *)(a2 + 96));
    if ( v3 >= 0 )
      *v11 = 0LL;
  }
LABEL_7:
  if ( v2 )
    goto LABEL_8;
  return (unsigned int)v3;
}
