/*
 * XREFs of MmStoreAllocateVirtualMemory @ 0x140A54708
 * Callers:
...
unsigned __int64 __fastcall MmStoreAllocateVirtualMemory(__int64 a1, int a2)
{
  ...
  if ( VirtualMemory < 0 )
    return v2;
  v6 = MiObtainReferencedSecureVad((unsigned __int64)SecureHandle ^ qword_140E2D840 ^ Process, &VirtualMemory);
  v7 = v6;
  if ( !v6 )
    return v2;
  if ( (unsigned int)MiMakeSecureExclusive(v6) )
  {
    ...
    if ( (int)MiLockPageTableRange(v29, v8) < 0 )
    {
      ...
      MiFreeVadRange(v7, (int)&VirtualMemory, v12, v13, Process, 0, 0LL);
    }
    else
    {
