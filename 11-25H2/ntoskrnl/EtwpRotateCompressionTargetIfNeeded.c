/*
 * XREFs of EtwpRotateCompressionTargetIfNeeded @ 0x140645708
 * Callers:
 *     EtwpCompressBuffer @ 0x140644CB4 (EtwpCompressBuffer.c)
 *     EtwpCompressPendingBuffers @ 0x14064501C (EtwpCompressPendingBuffers.c)
 * Callees:
 *     EtwpRotateCompressionTarget @ 0x14064569C (EtwpRotateCompressionTarget.c)
 */

__int64 __fastcall EtwpRotateCompressionTargetIfNeeded(__int64 a1, __int64 a2, __int64 a3, __int64 a4)
{
  __int64 v4; // rax
  __int64 result; // rax

  v4 = *(_QWORD *)(a1 + 1416);
  if ( !v4 )
    return EtwpRotateCompressionTarget(a1, a2, a3, a4);
  result = *(unsigned int *)(v4 + 8);
  a2 = (unsigned int)(*(_DWORD *)(a1 + 4) - result);
  if ( (unsigned int)a2 <= 0x148 )
    return EtwpRotateCompressionTarget(a1, a2, a3, a4);
  return result;
}
