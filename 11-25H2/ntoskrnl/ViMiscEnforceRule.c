/*
 * XREFs of ViMiscEnforceRule @ 0x140B8F4A0
 * Callers:
 *     VfMiscMmAllocateContiguousMemoryEx_Entry @ 0x140B8E0C0 (VfMiscMmAllocateContiguousMemoryEx_Entry.c)
 *     VfMiscMmAllocateContiguousMemory_Entry @ 0x140B8E0F0 (VfMiscMmAllocateContiguousMemory_Entry.c)
 * Callees:
 *     VfUtilIsSignedDriver @ 0x140604348 (VfUtilIsSignedDriver.c)
 *     CarReportRuleViolationFromNt @ 0x140B7D934 (CarReportRuleViolationFromNt.c)
 */

void __fastcall ViMiscEnforceRule(__int64 a1, __int64 a2, char a3)
{
  if ( (VfRuleClasses & 0x400000) != 0 || !VfUtilIsSignedDriver() )
  {
    if ( a3 )
      CarReportRuleViolationFromNt(196, 139LL, 0LL, 0LL, 0LL, 0xBu, a2);
  }
}
