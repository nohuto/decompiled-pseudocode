/*
 * XREFs of ?SmStUpdateMemoryCondition@?$SMKM_STORE@USM_TRAITS@@@@SAXPEAU1@W4_SMP_MEMORY_CONDITION@@@Z @ 0x1403767C8
 * Callers:
...
int __fastcall SMKM_STORE<SM_TRAITS>::SmStUpdateMemoryCondition(__int64 a1, int a2)
{
  ...
  *(_BYTE *)(a1 + 6838) = a2;
  ...
  {
    if ( result != v5 )
      result = KeSetActualBasePriorityThread(*(_QWORD *)(a1 + 7008), v5);
    if ( a2 <= 0 )
      return ST_STORE<SM_TRAITS>::StDmLazyWorkItemQueue(a1 + 80, a1 + 6768);
  }
  else if ( result > v5 )
  {
    return KeSetActualBasePriorityThread(*(_QWORD *)(a1 + 7008), v5);
  }
  return result;
}
