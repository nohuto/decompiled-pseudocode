/*
 * XREFs of CiTaskAllocate @ 0x1C000F7B8
 * Callers:
 *     CiConfigInitializeFromRegistry @ 0x1C0010180 (CiConfigInitializeFromRegistry.c)
 * Callees:
 *     WPP_SF_d @ 0x1C0004960 (WPP_SF_d.c)
 *     CiTaskLocate @ 0x1C000E4E0 (CiTaskLocate.c)
 */

__int64 __fastcall CiTaskAllocate(wchar_t *P, _OWORD *a2, __int64 *a3)
{
  __int64 Pool2; // rax
  __int64 v7; // rcx
  unsigned int v8; // ebx
  __int128 v9; // xmm1
  __int128 v10; // xmm0
  struct _LIST_ENTRY *v11; // rax
  struct _LIST_ENTRY *Blink; // rdx
  __int64 result; // rax
  __int64 v14; // [rsp+48h] [rbp+20h] BYREF

  v14 = 0LL;
  if ( (int)CiTaskLocate(P, &v14) >= 0 )
  {
    v8 = -1073741771;
    goto LABEL_8;
  }
  Pool2 = ExAllocatePool2(64LL, 48LL, 1953658433LL);
  v7 = Pool2;
  if ( !Pool2 )
  {
    v8 = -1073741801;
LABEL_8:
    if ( (HIDWORD(WPP_GLOBAL_Control->Timer) & 1) != 0 && BYTE1(WPP_GLOBAL_Control->Timer) >= 2u )
      WPP_SF_d(
        (__int64)WPP_GLOBAL_Control->AttachedDevice,
        0xAu,
        (__int64)&WPP_62da978b8000316eb1e6fce22b5cf66e_Traceguids,
        v8);
    ExFreePoolWithTag(P, 0);
    return v8;
  }
  v9 = a2[1];
  *(_OWORD *)Pool2 = *a2;
  v10 = a2[2];
  *(_OWORD *)(Pool2 + 16) = v9;
  *(_OWORD *)(Pool2 + 32) = v10;
  *(_QWORD *)(Pool2 + 24) = P;
  v11 = (struct _LIST_ENTRY *)(Pool2 + 8);
  Blink = WPP_MAIN_CB.DeviceLock.Header.WaitListHead.Blink;
  if ( WPP_MAIN_CB.DeviceLock.Header.WaitListHead.Blink->Flink != &WPP_MAIN_CB.DeviceLock.Header.WaitListHead )
    __fastfail(3u);
  v11->Flink = &WPP_MAIN_CB.DeviceLock.Header.WaitListHead;
  v11->Blink = Blink;
  Blink->Flink = v11;
  WPP_MAIN_CB.DeviceLock.Header.WaitListHead.Blink = v11;
  result = 0LL;
  *a3 = v7;
  return result;
}
