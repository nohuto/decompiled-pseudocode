/*
 * XREFs of ?SetWindowCompositionForceActiveWindowAppearance@@YAJPEAUtagWND@@PEBUtagWINDOWCOMPOSITIONATTRIBDATA_UNION@@@Z @ 0x1401E6AF4
 * Callers:
        ...
        const struct tagWINDOWCOMPOSITIONATTRIBDATA_UNION *a2)
{
  ...
  {
    if ( -__CFSHR__(v6[0], 9) == v3 )
    {
      return 0;
    }
    else
    {
      LODWORD(v6[0]) = v6[0] & 0xFFFFFEFF | (v3 << 8);
