/*
 * XREFs of GreSetRedirectionSurfaceSignaling @ 0x1401A675C
 * Callers:
 *     NtGdiHLSurfSetInformation @ 0x1401A6370 (NtGdiHLSurfSetInformation.c)
 * Callees:
 *     ??1SFMALTLOGICALSURFACEREF@@QEAA@XZ @ 0x14007DDEC (--1SFMALTLOGICALSURFACEREF@@QEAA@XZ.c)
 *     ??$GreReleaseSemaphoreCommon@$00P6AXPEAUHSEMAPHORE__@@@Z@@YAXP6AXPEAUHSEMAPHORE__@@@Z0@Z @ 0x14007ED70 (--$GreReleaseSemaphoreCommon@$00P6AXPEAUHSEMAPHORE__@@@Z@@YAXP6AXPEAUHSEMAPHORE__@@@Z0@Z.c)
 *     ??0SFMALTLOGICALSURFACEREF@@QEAA@PEAUHLSURF__@@@Z @ 0x14008D504 (--0SFMALTLOGICALSURFACEREF@@QEAA@PEAUHLSURF__@@@Z.c)
 *     IsDwmActive @ 0x14008D760 (IsDwmActive.c)
 *     ??$GreAcquireSemaphoreCommon@$06P6AXPEAUHSEMAPHORE__@@@Z@@YAXP6AXPEAUHSEMAPHORE__@@@Z0@Z @ 0x14008FE7C (--$GreAcquireSemaphoreCommon@$06P6AXPEAUHSEMAPHORE__@@@Z@@YAXP6AXPEAUHSEMAPHORE__@@@Z0@Z.c)
 *     ??0?$SEMOBJSHARED@$00@@QEAA@AEAUSESSION_GLOBALS@Base@Gre@@@Z @ 0x14009347C (--0-$SEMOBJSHARED@$00@@QEAA@AEAUSESSION_GLOBALS@Base@Gre@@@Z.c)
 *     ??$GreReleaseSemaphoreCommon@$06P6AXPEAUHSEMAPHORE__@@@Z@@YAXP6AXPEAUHSEMAPHORE__@@@Z0@Z @ 0x140098148 (--$GreReleaseSemaphoreCommon@$06P6AXPEAUHSEMAPHORE__@@@Z@@YAXP6AXPEAUHSEMAPHORE__@@@Z0@Z.c)
 *     _guard_dispatch_icall @ 0x140342490 (_guard_dispatch_icall.c)
 */

__int64 __fastcall GreSetRedirectionSurfaceSignaling(Gre::Base *a1, HLSURF a2, __int64 a3, __int64 a4, int a5)
{
  Gre::Base *v9; // rcx
  unsigned int v10; // esi
  struct Gre::Base::SESSION_GLOBALS *v11; // rax
  Gre::Base *v12; // rcx
  __int64 v13; // r13
  Gre::Base *v14; // rcx
  __int64 v15; // r8
  __int64 v16; // r9
  __int64 v17; // rbp
  __int64 v18; // r14
  __int64 v20; // [rsp+30h] [rbp-58h] BYREF
  _BYTE v21[32]; // [rsp+38h] [rbp-50h] BYREF
  __int64 v22; // [rsp+58h] [rbp-30h]

  v10 = 0;
  if ( IsDwmActive(a1) )
  {
    v11 = Gre::Base::Globals(v9);
    SEMOBJSHARED<1>::SEMOBJSHARED<1>((HSEMAPHORE *)&v20, v11);
    v13 = *(_QWORD *)Gre::Base::Globals(v12);
    GreAcquireSemaphoreCommon<7,void (*)(HSEMAPHORE__ *)>(
      (void (__fastcall *)(__int64))GreAcquireSemaphoreSharedInternal,
      v13 + 520);
    if ( IsDwmActive(v14) )
    {
      if ( (unsigned int)UserIsCurrentProcessDwm() )
      {
        if ( *((_QWORD *)a1 + 431) )
        {
          SFMALTLOGICALSURFACEREF::SFMALTLOGICALSURFACEREF((SFMALTLOGICALSURFACEREF *)v21, a2, v15, v16);
          v17 = v22;
          if ( v22 )
          {
            if ( v22 != -256 )
              GreAcquirePushLockExclusive((struct W32_PUSH_LOCK *)(v22 + 256));
            v18 = *(_QWORD *)(v17 + 184);
            if ( v18 && (*(_DWORD *)(v18 + 92) & 1) != 0 )
            {
              if ( (*((unsigned int (__fastcall **)(_QWORD, __int64, HLSURF, _QWORD))a1 + 431))(
                     *(_QWORD *)(v17 + 184),
                     a3,
                     a2,
                     *(_QWORD *)(v18 + 32)) )
              {
                *(_DWORD *)(v17 + 244) = (2 * a5) ^ (*(_DWORD *)(v17 + 244) ^ (2 * a5)) & 0xFFFFFFFD;
                EtwLogicalSurfEnableDirtyNotificationEvent(a2, a3);
              }
              *(_QWORD *)(v17 + 192) = *(_QWORD *)(v18 + 544);
              *(_QWORD *)(v17 + 200) = *(_QWORD *)(v18 + 552);
              *(_DWORD *)(v17 + 208) = *(_DWORD *)(v18 + 100);
              if ( (*(_DWORD *)(v18 + 92) & 0x400) != 0 )
                *(_DWORD *)(v17 + 244) |= 0x100u;
            }
            else
            {
              *(_QWORD *)(v17 + 192) = a3;
              *(_QWORD *)(v17 + 200) = a4;
            }
            if ( v17 != -256 )
              GreReleasePushLockExclusive((struct W32_PUSH_LOCK *)(v17 + 256));
          }
          else
          {
            v10 = -1073741811;
          }
          SFMALTLOGICALSURFACEREF::~SFMALTLOGICALSURFACEREF((SFMALTLOGICALSURFACEREF *)v21);
        }
        else
        {
          v10 = -1073741822;
        }
      }
      else
      {
        v10 = -1073741790;
      }
    }
    else
    {
      v10 = -1071775733;
    }
    GreReleaseSemaphoreCommon<7,void (*)(HSEMAPHORE__ *)>(
      (__int64 (__fastcall *)(__int64))GreReleaseSemaphoreSharedInternal,
      v13 + 520);
    GreReleaseSemaphoreCommon<1,void (*)(HSEMAPHORE__ *)>(
      (__int64 (__fastcall *)(__int64))GreReleaseSemaphoreSharedInternal,
      v20);
  }
  else
  {
    return (unsigned int)-1071775733;
  }
  return v10;
}
