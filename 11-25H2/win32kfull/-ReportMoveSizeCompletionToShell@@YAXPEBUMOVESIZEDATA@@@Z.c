/*
 * XREFs of ?ReportMoveSizeCompletionToShell@@YAXPEBUMOVESIZEDATA@@@Z @ 0x1402A7DC8
 * Callers:
...
LABEL_4:
  ...
  if ( v7 )
  {
    ...
    {
      if ( (*((_DWORD *)a1 + 79) & 2) != 0 )
        NotifyShell::WindowSizingCompleted(*((_QWORD *)a1 + 2), *((unsigned int *)a1 + 78), v5);
    }
    else if ( !v4 && !v5 )
    {
      if ( IsArranged(*((const struct tagWND **)a1 + 2)) )
      {
        NotifyShell::ArrangementCompleted(*((__int64 **)a1 + 2), *((unsigned int *)a1 + 78), 1);
        LastWokenThread::Set(*(_QWORD *)(v7 + 16), 1LL, 0LL);
      }
      else if ( (*((_DWORD *)a1 + 50) & 0x20000000) != 0 )
      {
        NotifyShell::ArrangementCompleted(*((__int64 **)a1 + 2), *((unsigned int *)a1 + 78), 2);
      }
    }
    v8 = 3LL;
    if ( *((_DWORD *)a1 + 44) != 9 )
