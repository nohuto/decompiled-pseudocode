/*
 * XREFs of ?winrt_throw_hresult@wil@@YAXIPEBD0PEAXUhresult@winrt@@@Z @ 0x1800DC320
 * Callers:
 *     <none>
 * Callees:
 *     ??$ReportFailure_Hr@$01@details@wil@@YAXPEAXIPEBD110J@Z @ 0x18007D0EC (--$ReportFailure_Hr@$01@details@wil@@YAXPEAXIPEBD110J@Z.c)
 */

__int64 __fastcall wil::winrt_throw_hresult(int a1, __int64 a2, __int64 a3, __int64 a4, int a5)
{
  wil::details *v6; // [rsp+30h] [rbp-18h]

  LODWORD(v6) = a5;
  return wil::details::ReportFailure_Hr<2>(0LL, a1, a2, a3, 0LL, a4, v6);
}
