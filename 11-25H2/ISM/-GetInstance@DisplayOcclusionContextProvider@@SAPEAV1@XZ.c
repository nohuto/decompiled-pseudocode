/*
 * XREFs of ?GetInstance@DisplayOcclusionContextProvider@@SAPEAV1@XZ @ 0x18019A7D0
 * Callers:
 *     ?OnInserted@OcclusionRectListProxy@@MEAAJI@Z @ 0x180138110 (-OnInserted@OcclusionRectListProxy@@MEAAJI@Z.c)
 *     ?OnRemoved@OcclusionRectListProxy@@MEAAJIAEBUDisplayOcclusionRect@@@Z @ 0x180138200 (-OnRemoved@OcclusionRectListProxy@@MEAAJIAEBUDisplayOcclusionRect@@@Z.c)
...
struct DisplayOcclusionContextProvider *DisplayOcclusionContextProvider::GetInstance(void)
{
  ...
  v1 = 32LL;
  if ( dword_180247ED8 > *(_DWORD *)(*((_QWORD *)NtCurrentTeb()->ThreadLocalStoragePointer + (unsigned int)tls_index)
                                   + 32LL) )
  {
    Init_thread_header(&dword_180247ED8);
    if ( dword_180247ED8 == -1 )
    {
      atexit(DisplayOcclusionContextProvider::GetInstance_::_2_::_dynamic_atexit_destructor_for__s_instance__);
      Init_thread_footer(&dword_180247ED8);
    }
  }
  v6 = 0;
  inited = wil::init_once_nothrow__lambda_2e51b86642a92e3b17e4afda222f3187___(v0, v1, &v6);
  if ( inited < 0 )
