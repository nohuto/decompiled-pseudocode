/*
 * XREFs of ?OnInput@DragNDropProcessor@@UEAAJPEAUInputInfo@@PEAVInputContext@@PEAUContextualProcessorResponse@@@Z @ 0x18014A8E0
 * Callers:
 *     <none>
 * Callees:
 *     ??$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z @ 0x18001AB00 (--$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z.c)
 *     ??0Lock@Bamo@Microsoft@@QEAA@PEAVBaseBamoConnection@12@@Z @ 0x180024D1C (--0Lock@Bamo@Microsoft@@QEAA@PEAVBaseBamoConnection@12@@Z.c)
 *     ??1Lock@Bamo@Microsoft@@QEAA@XZ @ 0x180024E94 (--1Lock@Bamo@Microsoft@@QEAA@XZ.c)
 *     ??$find@X@?$_Hash@V?$_Umap_traits@KUPointerCache@EdgyProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUPointerCache@EdgyProcessor@@@std@@@4@$0A@@std@@@std@@QEAA?AV?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKUPointerCache@EdgyProcessor@@@std@@@std@@@std@@@1@AEBK@Z @ 0x180026F08 (--$find@X@-$_Hash@V-$_Umap_traits@KUPointerCache@EdgyProcessor@@V-$_Uhash_compare@KU-$hash@K@std.c)
 *     ??1?$VariableSizedPayloadStorage@UInputInfo@@@@QEAA@XZ @ 0x180030470 (--1-$VariableSizedPayloadStorage@UInputInfo@@@@QEAA@XZ.c)
 *     ??$count@X@?$_Hash@V?$_Uset_traits@KV?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@K@2@$0A@@std@@@std@@QEBA_KAEBK@Z @ 0x18005B45C (--$count@X@-$_Hash@V-$_Uset_traits@KV-$_Uhash_compare@KU-$hash@K@std@@U-$equal_to@K@2@@std@@V-$a.c)
 *     ??0?$VariableSizedPayloadStorage@UInputInfo@@@@QEAA@PEBUInputInfo@@@Z @ 0x18005BF80 (--0-$VariableSizedPayloadStorage@UInputInfo@@@@QEAA@PEBUInputInfo@@@Z.c)
 *     ??$_Try_emplace@AEBK$$V@?$_Hash@V?$_Umap_traits@KW4ContextualProcessorDecision@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKW4ContextualProcessorDecision@@@std@@@3@$0A@@std@@@std@@IEAA?AU?$pair@PEAU?$_List_node@U?$pair@$$CBKW4ContextualProcessorDecision@@@std@@PEAX@std@@_N@1@AEBK@Z @ 0x18005CCC8 (--$_Try_emplace@AEBK$$V@-$_Hash@V-$_Umap_traits@KW4ContextualProcessorDecision@@V-$_Uhash_compar.c)
 *     ??$_Erase@K@?$_Hash@V?$_Umap_traits@KW4ContextualProcessorDecision@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKW4ContextualProcessorDecision@@@std@@@3@$0A@@std@@@std@@AEAA_KAEBK@Z @ 0x18005CEDC (--$_Erase@K@-$_Hash@V-$_Umap_traits@KW4ContextualProcessorDecision@@V-$_Uhash_compare@KU-$hash@K.c)
 *     ?GetKeyModifiers@PointerManipulationHelper@@SA?AW4VirtualKeyModifiers@System@Windows@@PEAUInputInfo@@@Z @ 0x18009A8E8 (-GetKeyModifiers@PointerManipulationHelper@@SA-AW4VirtualKeyModifiers@System@Windows@@PEAUInputI.c)
 *     __security_check_cookie @ 0x18009D480 (__security_check_cookie.c)
 *     ?RelativePointToScreen@CInputTransform@@SA?AUTransformedPoint@@AEBU2@AEBUtagINPUT_TRANSFORM@@@Z @ 0x1800DFA6C (-RelativePointToScreen@CInputTransform@@SA-AUTransformedPoint@@AEBU2@AEBUtagINPUT_TRANSFORM@@@Z.c)
 *     ?OnManualDragEnded@ManualDragAreaClientProxy@@QEAAXI@Z @ 0x180146794 (-OnManualDragEnded@ManualDragAreaClientProxy@@QEAAXI@Z.c)
 *     ??$_Emplace_reallocate@AEBUDragInfoStruct@@@?$vector@UDragInfoStruct@@V?$allocator@UDragInfoStruct@@@std@@@std@@AEAAPEAUDragInfoStruct@@QEAU2@AEBU2@@Z @ 0x180146EB0 (--$_Emplace_reallocate@AEBUDragInfoStruct@@@-$vector@UDragInfoStruct@@V-$allocator@UDragInfoStru.c)
 *     ??$_Erase@K@?$_Hash@V?$_Umap_traits@KUMessageInfo@DragNDropProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUMessageInfo@DragNDropProcessor@@@std@@@4@$0A@@std@@@std@@AEAA_KAEBK@Z @ 0x180146FEC (--$_Erase@K@-$_Hash@V-$_Umap_traits@KUMessageInfo@DragNDropProcessor@@V-$_Uhash_compare@KU-$hash.c)
 *     ??$_Erase@K@?$_Hash@V?$_Umap_traits@KUPointerCache@DragNDropProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUPointerCache@DragNDropProcessor@@@std@@@4@$0A@@std@@@std@@AEAA_KAEBK@Z @ 0x180147090 (--$_Erase@K@-$_Hash@V-$_Umap_traits@KUPointerCache@DragNDropProcessor@@V-$_Uhash_compare@KU-$has.c)
 *     ??$_Erase@K@?$_Hash@V?$_Umap_traits@KPEAVControllerProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKPEAVControllerProcessor@@@std@@@3@$0A@@std@@@std@@AEAA_KAEBK@Z @ 0x1801471E0 (--$_Erase@K@-$_Hash@V-$_Umap_traits@KPEAVControllerProcessor@@V-$_Uhash_compare@KU-$hash@K@std@@.c)
 *     ??$_Erase@K@?$_Hash@V?$_Umap_traits@KV?$ComPtr@VDataSourcePrincipal@@@WRL@Microsoft@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKV?$ComPtr@VDataSourcePrincipal@@@WRL@Microsoft@@@std@@@5@$0A@@std@@@std@@AEAA_KAEBK@Z @ 0x18014728C (--$_Erase@K@-$_Hash@V-$_Umap_traits@KV-$ComPtr@VDataSourcePrincipal@@@WRL@Microsoft@@V-$_Uhash_c.c)
 *     ??$_Insert_or_assign@AEBIAEA_K@?$unordered_map@I_KU?$hash@I@std@@U?$equal_to@I@2@V?$allocator@U?$pair@$$CBI_K@std@@@2@@std@@AEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBI_K@std@@@std@@@std@@@std@@_N@1@AEBIAEA_K@Z @ 0x180147450 (--$_Insert_or_assign@AEBIAEA_K@-$unordered_map@I_KU-$hash@I@std@@U-$equal_to@I@2@V-$allocator@U-.c)
 *     ??$_Try_emplace@AEBK$$V@?$_Hash@V?$_Umap_traits@K_KV?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBK_K@std@@@2@$0A@@std@@@std@@IEAA?AU?$pair@PEAU?$_List_node@U?$pair@$$CBK_K@std@@PEAX@std@@_N@1@AEBK@Z @ 0x180147570 (--$_Try_emplace@AEBK$$V@-$_Hash@V-$_Umap_traits@K_KV-$_Uhash_compare@KU-$hash@K@std@@U-$equal_to.c)
 *     ??$_Try_emplace@AEBK$$V@?$_Hash@V?$_Umap_traits@KUMessageInfo@DragNDropProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUMessageInfo@DragNDropProcessor@@@std@@@4@$0A@@std@@@std@@IEAA?AU?$pair@PEAU?$_List_node@U?$pair@$$CBKUMessageInfo@DragNDropProcessor@@@std@@PEAX@std@@_N@1@AEBK@Z @ 0x180147678 (--$_Try_emplace@AEBK$$V@-$_Hash@V-$_Umap_traits@KUMessageInfo@DragNDropProcessor@@V-$_Uhash_comp.c)
 *     ??$_Try_emplace@AEBK$$V@?$_Hash@V?$_Umap_traits@KUtagPOINTF@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUtagPOINTF@@@std@@@3@$0A@@std@@@std@@IEAA?AU?$pair@PEAU?$_List_node@U?$pair@$$CBKUtagPOINTF@@@std@@PEAX@std@@_N@1@AEBK@Z @ 0x180147830 (--$_Try_emplace@AEBK$$V@-$_Hash@V-$_Umap_traits@KUtagPOINTF@@V-$_Uhash_compare@KU-$hash@K@std@@U.c)
 *     ??$copy@PEBUtagPOINT@@V?$back_insert_iterator@V?$vector@UtagPOINT@@V?$allocator@UtagPOINT@@@std@@@std@@@std@@@std@@YA?AV?$back_insert_iterator@V?$vector@UtagPOINT@@V?$allocator@UtagPOINT@@@std@@@std@@@0@PEBUtagPOINT@@0V10@@Z @ 0x180147B30 (--$copy@PEBUtagPOINT@@V-$back_insert_iterator@V-$vector@UtagPOINT@@V-$allocator@UtagPOINT@@@std@.c)
 *     ??$emplace@AEAIAEA_K@?$_Hash@V?$_Umap_traits@I_KV?$_Uhash_compare@IU?$hash@I@std@@U?$equal_to@I@2@@std@@V?$allocator@U?$pair@$$CBI_K@std@@@2@$0A@@std@@@std@@QEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBI_K@std@@@std@@@std@@@std@@_N@1@AEAIAEA_K@Z @ 0x180147BA4 (--$emplace@AEAIAEA_K@-$_Hash@V-$_Umap_traits@I_KV-$_Uhash_compare@IU-$hash@I@std@@U-$equal_to@I@.c)
 *     ??$emplace@AEAKUPointerCache@DragNDropProcessor@@@?$_Hash@V?$_Umap_traits@KUPointerCache@DragNDropProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUPointerCache@DragNDropProcessor@@@std@@@4@$0A@@std@@@std@@QEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKUPointerCache@DragNDropProcessor@@@std@@@std@@@std@@@std@@_N@1@AEAK$$QEAUPointerCache@DragNDropProcessor@@@Z @ 0x180147CBC (--$emplace@AEAKUPointerCache@DragNDropProcessor@@@-$_Hash@V-$_Umap_traits@KUPointerCache@DragNDr.c)
 *     ??$emplace@AEAKUResultInfo@DragNDropProcessor@@@?$_Hash@V?$_Umap_traits@KUResultInfo@DragNDropProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKUResultInfo@DragNDropProcessor@@@std@@@4@$0A@@std@@@std@@QEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKUResultInfo@DragNDropProcessor@@@std@@@std@@@std@@@std@@_N@1@AEAK$$QEAUResultInfo@DragNDropProcessor@@@Z @ 0x180147E58 (--$emplace@AEAKUResultInfo@DragNDropProcessor@@@-$_Hash@V-$_Umap_traits@KUResultInfo@DragNDropPr.c)
 *     ?CancelDrag@DragNDropProcessor@@QEAAJKPEAVBamoDragManagerClientProxy@@@Z @ 0x180148950 (-CancelDrag@DragNDropProcessor@@QEAAJKPEAVBamoDragManagerClientProxy@@@Z.c)
 *     ?DragEnded@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Windows@@2@Z @ 0x180148EAC (-DragEnded@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Windows.c)
 *     ?DragInProgress@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Windows@@2@Z @ 0x180148FF0 (-DragInProgress@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Wi.c)
 *     ?DragStarted@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Windows@@2I22@Z @ 0x180149134 (-DragStarted@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Windo.c)
 *     ?DropTargetChanged@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System@Windows@@_K2@Z @ 0x1801492AC (-DropTargetChanged@InputGesture@InputETW@@SAXPEBGKPEAUtagPOINT@@11_KW4VirtualKeyModifiers@System.c)
 *     ?FeedAnimationDataIfNeeded@DragNDropProcessor@@AEAAXKUtagPOINT@@UtagPOINTF@@_N@Z @ 0x180149510 (-FeedAnimationDataIfNeeded@DragNDropProcessor@@AEAAXKUtagPOINT@@UtagPOINTF@@_N@Z.c)
 *     ?GetVelocityForPointerId@DragNDropProcessor@@AEAA?AUtagPOINTF@@K@Z @ 0x180149C50 (-GetVelocityForPointerId@DragNDropProcessor@@AEAA-AUtagPOINTF@@K@Z.c)
 *     ?HitTest@DragNDropProcessor@@AEAAJUtagPOINT@@W4InputType@@U_GUID@@_NPEAUHitTestResult@@PEAU2@PEA_K6PEAPEAXPEAPEAVBamoDragSourceClientProxy@@PEAPEAVBamoDragManagerClientProxy@@PEAPEAVInputSite@@PEAPEAV8@@Z @ 0x180149D7C (-HitTest@DragNDropProcessor@@AEAAJUtagPOINT@@W4InputType@@U_GUID@@_NPEAUHitTestResult@@PEAU2@PEA.c)
 *     ?MakeContextualProcessingDecision@DragNDropProcessor@@AEAAXAEBV?$ComPtr@VManualDragAreaClientProxy@@@WRL@Microsoft@@KPEAVInputContext@@PEAUContextualProcessorResponse@@@Z @ 0x18014A4F0 (-MakeContextualProcessingDecision@DragNDropProcessor@@AEAAXAEBV-$ComPtr@VManualDragAreaClientPro.c)
 *     ?GetInputType@PointerManipulationHelper@@SA?AW4InputType@Input@Internal@UI@Windows@@W42@@Z @ 0x180199380 (-GetInputType@PointerManipulationHelper@@SA-AW4InputType@Input@Internal@UI@Windows@@W42@@Z.c)
 *     ?GetPositionRelativeToSite@PointerManipulationHelper@@SA?AUtagPOINT@@PEAVInputSite@@U2@@Z @ 0x1801993BC (-GetPositionRelativeToSite@PointerManipulationHelper@@SA-AUtagPOINT@@PEAVInputSite@@U2@@Z.c)
 *     ?GetTransformFromRootToSite@PointerManipulationHelper@@SA?AUtagINPUT_TRANSFORM@@PEAVInputSite@@@Z @ 0x180199464 (-GetTransformFromRootToSite@PointerManipulationHelper@@SA-AUtagINPUT_TRANSFORM@@PEAVInputSite@@@.c)
 *     ?OnPointerInfoFrame@DragGestureTracker@@QEAAJAEAUtagPOINTER_INFO@@@Z @ 0x1801994E0 (-OnPointerInfoFrame@DragGestureTracker@@QEAAJAEAUtagPOINTER_INFO@@@Z.c)
 *     _guard_dispatch_icall$thunk$10345483385596137414 @ 0x1801D3010 (_guard_dispatch_icall$thunk$10345483385596137414.c)
 */

// Hidden C++ exception states: #wind=4
__int64 __fastcall DragNDropProcessor::OnInput(
        DragNDropProcessor *this,
        struct InputInfo *a2,
        struct InputContext *a3,
        struct ContextualProcessorResponse *a4)
{
  struct ContextualProcessorResponse *v4; // r13
  int *v5; // r12
  unsigned int KeyModifiers; // ebx
  __int64 v8; // r14
  _QWORD *v9; // rcx
  __int64 v10; // rsi
  struct tagPOINT v11; // rdi
  __int64 v12; // rax
  __int64 v13; // rbx
  __int64 v14; // rcx
  __int128 v15; // xmm8
  __int64 v16; // rax
  __int64 v17; // rax
  DragGestureTracker *v18; // rcx
  __int64 v19; // rax
  int v20; // xmm6_4
  int v21; // xmm7_4
  __int64 v22; // rcx
  _DWORD *v23; // rax
  struct InputSite *v24; // rcx
  struct InputSite *v25; // rcx
  __int64 v26; // r14
  unsigned int v27; // esi
  const WCHAR *v28; // rax
  __int64 v29; // r13
  __int64 v30; // r13
  char v31; // si
  __int64 v32; // rax
  __int64 v33; // rax
  struct Microsoft::Bamo::BaseBamoConnection *v34; // rdx
  __int64 v35; // r14
  __int128 *TransformFromRootToSite; // rax
  float *v37; // rax
  struct tagPOINT v38; // rsi
  __int64 v39; // rax
  struct InputSite *v40; // rcx
  float v41; // xmm1_4
  __int64 v42; // r14
  unsigned int v43; // esi
  const WCHAR *v44; // rax
  __int64 v45; // rsi
  __int64 v46; // r14
  unsigned int v47; // edi
  const WCHAR *v48; // rax
  __int64 v49; // rcx
  unsigned int v50; // esi
  const WCHAR *v51; // rax
  __int64 v52; // r8
  const char *v53; // r9
  ManualDragAreaClientProxy *v54; // rcx
  const struct std::nothrow_t *v55; // rdx
  _QWORD *v56; // rbx
  unsigned int v57; // ebx
  _QWORD *v58; // rax
  __int64 v59; // rcx
  int v61; // [rsp+50h] [rbp-B8h]
  __int64 v62; // [rsp+58h] [rbp-B0h]
  unsigned int v63; // [rsp+78h] [rbp-90h] BYREF
  int v64; // [rsp+7Ch] [rbp-8Ch]
  unsigned int v65; // [rsp+80h] [rbp-88h] BYREF
  unsigned int v66; // [rsp+84h] [rbp-84h]
  struct tagPOINT PositionRelativeToSite; // [rsp+88h] [rbp-80h] BYREF
  int v68; // [rsp+90h] [rbp-78h]
  struct tagPOINT v69; // [rsp+98h] [rbp-70h] BYREF
  struct tagPOINT v70; // [rsp+A0h] [rbp-68h] BYREF
  __int64 v71; // [rsp+A8h] [rbp-60h] BYREF
  unsigned int v72; // [rsp+B0h] [rbp-58h]
  unsigned __int64 v73; // [rsp+B8h] [rbp-50h] BYREF
  __int64 v74; // [rsp+C0h] [rbp-48h] BYREF
  _QWORD *v75; // [rsp+C8h] [rbp-40h] BYREF
  unsigned int InputType; // [rsp+D0h] [rbp-38h]
  struct ContextualProcessorResponse *v77; // [rsp+D8h] [rbp-30h]
  struct InputContext *v78; // [rsp+E0h] [rbp-28h]
  __int128 v79; // [rsp+E8h] [rbp-20h] BYREF
  __int64 v80; // [rsp+F8h] [rbp-10h]
  __int128 v81; // [rsp+100h] [rbp-8h] BYREF
  __int64 v82; // [rsp+110h] [rbp+8h]
  struct tagPOINT v83; // [rsp+118h] [rbp+10h]
  struct tagPOINT v84; // [rsp+120h] [rbp+18h]
  __int64 v85; // [rsp+128h] [rbp+20h] BYREF
  __int64 v86; // [rsp+130h] [rbp+28h] BYREF
  __int64 v87; // [rsp+138h] [rbp+30h] BYREF
  struct InputInfo *v88; // [rsp+140h] [rbp+38h]
  __int128 v89; // [rsp+148h] [rbp+40h] BYREF
  float v90; // [rsp+158h] [rbp+50h] BYREF
  _BYTE v91[8]; // [rsp+160h] [rbp+58h] BYREF
  Microsoft::BamoImpl::BamoImplObject *v92[2]; // [rsp+168h] [rbp+60h] BYREF
  __int128 v93; // [rsp+178h] [rbp+70h] BYREF
  unsigned int v94; // [rsp+188h] [rbp+80h] BYREF
  __int64 v95; // [rsp+190h] [rbp+88h] BYREF
  __int128 v96[4]; // [rsp+198h] [rbp+90h] BYREF
  _BYTE v97[16]; // [rsp+1D8h] [rbp+D0h] BYREF
  _BYTE v98[16]; // [rsp+1E8h] [rbp+E0h] BYREF
  _BYTE v99[16]; // [rsp+1F8h] [rbp+F0h] BYREF
  _BYTE v100[16]; // [rsp+208h] [rbp+100h] BYREF
  _BYTE v101[16]; // [rsp+218h] [rbp+110h] BYREF
  _BYTE v102[16]; // [rsp+228h] [rbp+120h] BYREF
  _BYTE v103[16]; // [rsp+238h] [rbp+130h] BYREF
  _BYTE v104[16]; // [rsp+248h] [rbp+140h] BYREF
  _BYTE v105[16]; // [rsp+258h] [rbp+150h] BYREF
  _BYTE v106[16]; // [rsp+268h] [rbp+160h] BYREF
  _BYTE v107[16]; // [rsp+278h] [rbp+170h] BYREF
  _BYTE v108[16]; // [rsp+288h] [rbp+180h] BYREF
  _BYTE v109[64]; // [rsp+298h] [rbp+190h] BYREF
  float v110[2]; // [rsp+2D8h] [rbp+1D0h] BYREF
  struct tagPOINTF v111[2]; // [rsp+2E0h] [rbp+1D8h]
  __int128 v112; // [rsp+2F0h] [rbp+1E8h] BYREF
  __int128 v113; // [rsp+300h] [rbp+1F8h]
  _QWORD v114[2]; // [rsp+310h] [rbp+208h] BYREF
  unsigned int v115; // [rsp+320h] [rbp+218h]
  unsigned int v116; // [rsp+324h] [rbp+21Ch]

  v4 = a4;
  v77 = a4;
  v78 = a3;
  v5 = (int *)a2;
  v88 = a2;
  *((_DWORD *)a4 + 2) = 4;
  KeyModifiers = PointerManipulationHelper::GetKeyModifiers((__int64)a2);
  v66 = KeyModifiers;
  v72 = *v5;
  InputType = PointerManipulationHelper::GetInputType(v72);
  v8 = 0LL;
  v68 = 0;
  if ( v5[79] )
  {
    v9 = (_QWORD *)((char *)this + 48);
    while ( 1 )
    {
      v10 = 36 * v8;
      v63 = v5[36 * v8 + 81];
      v11 = *(struct tagPOINT *)&v5[36 * v8 + 92];
      v83 = v11;
      if ( (v5[36 * v8 + 83] & 4) != 0 )
      {
        std::_Hash<std::_Umap_traits<unsigned long,EdgyProcessor::PointerCache,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,EdgyProcessor::PointerCache>>,0>>::find<void>(
          (__int64)v9,
          (__int64)&v86,
          &v63);
        v12 = v86;
        if ( v86 == *((_QWORD *)this + 7) )
        {
          v114[0] = v11;
          v114[1] = v11;
          v115 = v72;
          v116 = KeyModifiers;
          std::_Hash<std::_Umap_traits<unsigned long,DragNDropProcessor::PointerCache,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,DragNDropProcessor::PointerCache>>,0>>::emplace<unsigned long &,DragNDropProcessor::PointerCache>(
            (float *)this + 12,
            (__int64)v107,
            &v63,
            (__int64)v114);
        }
        else
        {
          *(struct tagPOINT *)(v86 + 28) = v11;
          *(_DWORD *)(v12 + 40) = KeyModifiers;
        }
      }
      else
      {
        std::_Hash<std::_Umap_traits<unsigned long,DragNDropProcessor::PointerCache,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,DragNDropProcessor::PointerCache>>,0>>::_Erase<unsigned long>(
          v9,
          (__int64)&v63);
      }
      std::_Hash<std::_Umap_traits<unsigned long,EdgyProcessor::PointerCache,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,EdgyProcessor::PointerCache>>,0>>::find<void>(
        (__int64)this + 112,
        (__int64)&v87,
        &v63);
      v13 = v87;
      if ( v87 == *((_QWORD *)this + 15) )
        goto LABEL_67;
      v14 = *(_QWORD *)(v87 + 32);
      if ( !v14 )
        goto LABEL_67;
      v70 = v11;
      v74 = 0LL;
      v71 = 0LL;
      v85 = 0LL;
      v15 = *(_OWORD *)(*(__int64 (__fastcall **)(__int64))(*(_QWORD *)(v14 + 8) + 80LL))(v14 + 8);
      v89 = v15;
      if ( (int)DragNDropProcessor::HitTest(
                  (__int64)this,
                  v11,
                  v72,
                  &v89,
                  0,
                  0LL,
                  &v70,
                  &v74,
                  &v71,
                  &v85,
                  0LL,
                  0LL,
                  0LL,
                  0LL) < 0
        || (v16 = *(_QWORD *)(v13 + 56)) != 0 && !*(_BYTE *)(v16 + 480)
        || (v17 = *(_QWORD *)(v13 + 48)) != 0 && !*(_BYTE *)(v17 + 480) )
      {
        if ( **(_DWORD **)(v13 + 64) == 4 )
          DragNDropProcessor::CancelDrag(this, v63, *(struct BamoDragManagerClientProxy **)(v13 + 32));
        goto LABEL_67;
      }
      v18 = *(DragGestureTracker **)(v13 + 64);
      if ( v18 )
      {
        DragGestureTracker::OnPointerInfoFrame(v18, (struct tagPOINTER_INFO *)&v5[v10 + 80]);
        v19 = *(_QWORD *)(v13 + 64);
        v20 = *(_DWORD *)(v19 + 4);
        v21 = *(_DWORD *)(v19 + 8);
        v22 = *(_QWORD *)std::_Hash<std::_Umap_traits<unsigned long,tagPOINTF,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,tagPOINTF>>,0>>::_Try_emplace<unsigned long const &,>(
                           (float *)this + 76,
                           (__int64)v108,
                           &v63);
        *(_DWORD *)(v22 + 20) = v20;
        *(_DWORD *)(v22 + 24) = v21;
      }
      v23 = *(_DWORD **)(v13 + 64);
      if ( *v23 == 2 )
      {
        if ( (v5[v10 + 83] & 4) != 0 )
        {
          DragGestureTracker::OnPointerInfoFrame(
            *(DragGestureTracker **)(v13 + 64),
            (struct tagPOINTER_INFO *)&v5[v10 + 80]);
          *(_DWORD *)(*(_QWORD *)std::_Hash<std::_Umap_traits<unsigned long,enum ContextualProcessorDecision,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,enum ContextualProcessorDecision>>,0>>::_Try_emplace<unsigned long const &,>(
                                   (_QWORD *)v4 + 2,
                                   (__int64)v97,
                                   &v63)
                    + 20LL) = 1;
        }
        else
        {
          *v23 = 1;
        }
      }
      if ( **(_DWORD **)(v13 + 64) == 1 )
        *(_DWORD *)(*(_QWORD *)std::_Hash<std::_Umap_traits<unsigned long,enum ContextualProcessorDecision,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,enum ContextualProcessorDecision>>,0>>::_Try_emplace<unsigned long const &,>(
                                 (_QWORD *)v4 + 2,
                                 (__int64)v98,
                                 &v63)
                  + 20LL) = 4;
      *(_OWORD *)&v111[0].x = 0LL;
      v110[0] = (float)v11.x;
      v110[1] = (float)v83.y;
      v111[1] = **(struct tagPOINTF **)&DragNDropProcessor::GetVelocityForPointerId(this, &v94, v63);
      PositionRelativeToSite = v11;
      v24 = *(struct InputSite **)(v13 + 56);
      if ( v24 )
        PositionRelativeToSite = PointerManipulationHelper::GetPositionRelativeToSite(v24, v11);
      v69 = 0LL;
      v25 = *(struct InputSite **)(v13 + 48);
      if ( v25 )
        v69 = PointerManipulationHelper::GetPositionRelativeToSite(v25, v11);
      if ( **(_DWORD **)(v13 + 64) == 3
        && !std::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>::count<void>(
              (_QWORD *)v4 + 19,
              &v63) )
      {
        v26 = (*(__int64 (__fastcall **)(__int64))(*(_QWORD *)(*(_QWORD *)(v13 + 24) + 8LL) + 32LL))(*(_QWORD *)(v13 + 24) + 8LL);
        v74 = 0LL;
        v27 = v63;
        v28 = (const WCHAR *)(*(__int64 (__fastcall **)(DragNDropProcessor *))(*(_QWORD *)this + 48LL))(this);
        v29 = v71;
        InputETW::InputGesture::DragStarted(
          v28,
          v27,
          (int *)&PositionRelativeToSite,
          (int *)&v70,
          (int *)&v69,
          0LL,
          v66,
          v71,
          InputType,
          v26,
          v62);
        (*(void (__fastcall **)(__int64, _QWORD, struct tagPOINT *, struct tagPOINT *, struct tagPOINT *, float *, _QWORD, unsigned int, __int64, unsigned int, __int64, _QWORD, _DWORD))(*(_QWORD *)(*(_QWORD *)(v13 + 32) + 8LL) + 24LL))(
          *(_QWORD *)(v13 + 32) + 8LL,
          v63,
          &PositionRelativeToSite,
          &v70,
          &v69,
          v110,
          0LL,
          v66,
          v29,
          InputType,
          v26,
          0LL,
          0);
        std::_Hash<std::_Umap_traits<unsigned int,unsigned __int64,std::_Uhash_compare<unsigned int,std::hash<unsigned int>,std::equal_to<unsigned int>>,std::allocator<std::pair<unsigned int const,unsigned __int64>>,0>>::emplace<unsigned int &,unsigned __int64 &>(
          (_QWORD *)this + 30,
          (__int64)v99,
          &v63,
          &v74);
        **(_DWORD **)(v13 + 64) = 4;
      }
      if ( **(_DWORD **)(v13 + 64) == 4 )
        break;
LABEL_66:
      LODWORD(v8) = v68;
LABEL_67:
      v56 = (_QWORD *)((char *)this + 432);
      if ( std::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>::count<void>(
             (_QWORD *)this + 54,
             &v63) )
      {
        v57 = v63;
        v58 = (_QWORD *)std::_Hash<std::_Umap_traits<unsigned long,DragNDropProcessor::MessageInfo,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,DragNDropProcessor::MessageInfo>>,0>>::_Try_emplace<unsigned long const &,>(
                          (float *)this + 28,
                          (__int64)v106,
                          &v63);
        v4 = v77;
        DragNDropProcessor::MakeContextualProcessingDecision(v59, *v58 + 40LL, v57, (__int64)v78, (__int64)v77);
        v56 = (_QWORD *)((char *)this + 432);
      }
      else
      {
        v4 = v77;
      }
      if ( (v5[36 * (unsigned int)v8 + 83] & 4) == 0 )
      {
        std::_Hash<std::_Umap_traits<unsigned long,enum ContextualProcessorDecision,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,enum ContextualProcessorDecision>>,0>>::_Erase<unsigned long>(
          v56,
          &v63);
        std::_Hash<std::_Umap_traits<unsigned long,Microsoft::WRL::ComPtr<DataSourcePrincipal>,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,Microsoft::WRL::ComPtr<DataSourcePrincipal>>>,0>>::_Erase<unsigned long>(
          (_QWORD *)this + 46,
          (__int64)&v63);
        std::_Hash<std::_Umap_traits<unsigned long,DragNDropProcessor::MessageInfo,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,DragNDropProcessor::MessageInfo>>,0>>::_Erase<unsigned long>(
          (_QWORD *)this + 14,
          (__int64)&v63);
        std::_Hash<std::_Umap_traits<unsigned long,ControllerProcessor *,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,ControllerProcessor *>>,0>>::_Erase<unsigned long>(
          (_QWORD *)this + 38,
          (__int64)&v63);
        std::_Hash<std::_Umap_traits<unsigned long,ControllerProcessor *,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,ControllerProcessor *>>,0>>::_Erase<unsigned long>(
          (_QWORD *)this + 30,
          (__int64)&v63);
      }
      v8 = (unsigned int)(v8 + 1);
      v68 = v8;
      KeyModifiers = v66;
      v9 = (_QWORD *)((char *)this + 48);
      if ( (unsigned int)v8 >= v5[79] )
        return 0LL;
    }
    *(_QWORD *)&v89 = *(_QWORD *)(*(_QWORD *)std::_Hash<std::_Umap_traits<unsigned long,unsigned __int64,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,unsigned __int64>>,0>>::_Try_emplace<unsigned long const &,>(
                                               (_QWORD *)this + 30,
                                               (__int64)v100,
                                               &v63)
                                + 24LL);
    DragNDropProcessor::FeedAnimationDataIfNeeded(this, v63, PositionRelativeToSite, v111[1], 0);
    v81 = 0LL;
    v82 = 0LL;
    v79 = 0LL;
    v30 = 0LL;
    v80 = 0LL;
    v31 = 0;
    LOBYTE(v64) = 0;
    v32 = *(_QWORD *)(v13 + 24);
    if ( !v32 )
      goto LABEL_49;
    v75 = 0LL;
    v65 = 0;
    v33 = *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(v32 + 32) + 24LL) + 32LL);
    v34 = *(int *)(v33 + 8) <= 0 ? 0LL : *(struct Microsoft::Bamo::BaseBamoConnection **)(v33 + 16);
    Microsoft::Bamo::Lock::Lock(v92, v34);
    (*(void (__fastcall **)(__int64, _QWORD **, unsigned int *))(*(_QWORD *)(*(_QWORD *)(v13 + 24) + 8LL) + 48LL))(
      *(_QWORD *)(v13 + 24) + 8LL,
      &v75,
      &v65);
    std::copy<tagPOINT const *,std::back_insert_iterator<std::vector<tagPOINT>>>(&v95, v75, &v75[v65], (__int64)&v81);
    Microsoft::Bamo::Lock::~Lock((Microsoft::Bamo::Lock *)v92);
    v65 = 0;
    if ( (__int64)(*((_QWORD *)&v81 + 1) - v81) >> 3 )
    {
      v35 = *((_QWORD *)&v79 + 1);
      do
      {
        TransformFromRootToSite = (__int128 *)PointerManipulationHelper::GetTransformFromRootToSite(
                                                v109,
                                                *(_QWORD *)(v13 + 48));
        v96[0] = *TransformFromRootToSite;
        v96[1] = TransformFromRootToSite[1];
        v96[2] = TransformFromRootToSite[2];
        v96[3] = TransformFromRootToSite[3];
        v75 = 0LL;
        v73 = _mm_unpacklo_ps(
                (__m128)COERCE_UNSIGNED_INT((float)*(int *)(v81 + 8LL * v65)),
                (__m128)COERCE_UNSIGNED_INT((float)*(int *)(v81 + 8LL * v65 + 4))).m128_u64[0];
        v37 = (float *)CInputTransform::RelativePointToScreen(&v90, (float *)&v73, v96);
        v84.x = v11.x + (int)*v37 - *(_DWORD *)(v13 + 72);
        v84.y = v83.y + (int)v37[1] - *(_DWORD *)(v13 + 76);
        v73 = 0LL;
        if ( *std::_Hash<std::_Umap_traits<unsigned long,EdgyProcessor::PointerCache,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,EdgyProcessor::PointerCache>>,0>>::find<void>(
                v13 + 80,
                (__int64)v91,
                &v65) == *(_QWORD *)(v13 + 88) )
          std::_Hash<std::_Umap_traits<unsigned int,unsigned __int64,std::_Uhash_compare<unsigned int,std::hash<unsigned int>,std::equal_to<unsigned int>>,std::allocator<std::pair<unsigned int const,unsigned __int64>>,0>>::emplace<unsigned int &,unsigned __int64 &>(
            (_QWORD *)(v13 + 80),
            (__int64)v101,
            &v65,
            &v73);
        v93 = v15;
        v38 = v84;
        if ( (int)DragNDropProcessor::HitTest(
                    (__int64)this,
                    v84,
                    v72,
                    &v93,
                    0,
                    0LL,
                    (struct tagPOINT *)&v75,
                    (__int64 *)&v73,
                    &v71,
                    0LL,
                    0LL,
                    0LL,
                    0LL,
                    0LL) >= 0 )
        {
          v39 = std::_Hash<std::_Umap_traits<unsigned long,unsigned __int64,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,unsigned __int64>>,0>>::_Try_emplace<unsigned long const &,>(
                  (_QWORD *)(v13 + 80),
                  (__int64)v102,
                  &v65);
          if ( v73 != *(_QWORD *)(*(_QWORD *)v39 + 24LL) )
          {
            LOBYTE(v64) = 1;
            std::unordered_map<unsigned int,unsigned __int64>::_Insert_or_assign<unsigned int const &,unsigned __int64 &>(
              (_QWORD *)(v13 + 80),
              (__int64)v103,
              &v65,
              &v73);
          }
        }
        v40 = *(struct InputSite **)(v13 + 56);
        if ( v40 )
          v38 = PointerManipulationHelper::GetPositionRelativeToSite(v40, v38);
        *(_QWORD *)&v112 = v73;
        *((float *)&v112 + 2) = (float)v38.x;
        *((float *)&v112 + 3) = (float)v38.y;
        *((float *)&v113 + 2) = (float)(int)v75;
        *((float *)&v113 + 3) = (float)SHIDWORD(v75);
        v41 = (float)*(int *)(v81 + 8LL * v65 + 4);
        *(float *)&v113 = (float)*(int *)(v81 + 8LL * v65);
        *((float *)&v113 + 1) = v41;
        if ( v35 == v30 )
        {
          std::vector<DragInfoStruct>::_Emplace_reallocate<DragInfoStruct const &>((__int64)&v79, (char *)v35, &v112);
          v30 = v80;
          v35 = *((_QWORD *)&v79 + 1);
        }
        else
        {
          *(_OWORD *)v35 = v112;
          *(_OWORD *)(v35 + 16) = v113;
          v35 += 32LL;
          *((_QWORD *)&v79 + 1) = v35;
        }
        ++v65;
      }
      while ( v65 < (unsigned __int64)((__int64)(*((_QWORD *)&v81 + 1) - v81) >> 3) );
      v5 = (int *)v88;
      v31 = v64;
    }
    else
    {
LABEL_49:
      v35 = *((_QWORD *)&v79 + 1);
    }
    if ( v74 == (_QWORD)v89 && !v31 )
    {
      v46 = v74;
      v45 = v71;
    }
    else
    {
      v42 = (v35 - (__int64)v79) >> 5;
      v43 = v63;
      v44 = (const WCHAR *)(*(__int64 (__fastcall **)(DragNDropProcessor *))(*(_QWORD *)this + 48LL))(this);
      InputETW::InputGesture::DropTargetChanged(
        v44,
        v43,
        (int *)&PositionRelativeToSite,
        (int *)&v70,
        (int *)&v69,
        v74,
        v66,
        v42,
        v71);
      v45 = v71;
      v61 = v42;
      v46 = v74;
      (*(void (__fastcall **)(__int64, _QWORD, struct tagPOINT *, struct tagPOINT *, struct tagPOINT *, float *, __int64, unsigned int, _QWORD, int, __int64))(*(_QWORD *)(*(_QWORD *)(v13 + 32) + 8LL) + 56LL))(
        *(_QWORD *)(v13 + 32) + 8LL,
        v63,
        &PositionRelativeToSite,
        &v70,
        &v69,
        v110,
        v74,
        v66,
        v79,
        v61,
        v71);
    }
    *(_QWORD *)(*(_QWORD *)std::_Hash<std::_Umap_traits<unsigned long,unsigned __int64,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,unsigned __int64>>,0>>::_Try_emplace<unsigned long const &,>(
                             (_QWORD *)this + 30,
                             (__int64)v104,
                             &v63)
              + 24LL) = v46;
    if ( (v5[36 * v68 + 83] & 4) != 0 )
    {
      DragNDropProcessor::FeedAnimationDataIfNeeded(this, v63, PositionRelativeToSite, v111[1], 0);
      if ( std::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>::count<void>(
             (_QWORD *)v77 + 19,
             &v63) )
      {
        DragNDropProcessor::CancelDrag(this, v63, *(struct BamoDragManagerClientProxy **)(v13 + 32));
LABEL_62:
        if ( (_QWORD)v79 )
          std::_Deallocate<16,0>((char *)v79, (const struct std::nothrow_t *)((v30 - v79) & 0xFFFFFFFFFFFFFFE0uLL));
        if ( (_QWORD)v81 )
          std::_Deallocate<16,0>((char *)v81, (const struct std::nothrow_t *)((v82 - v81) & 0xFFFFFFFFFFFFFFF8uLL));
        goto LABEL_66;
      }
      v47 = v63;
      v48 = (const WCHAR *)(*(__int64 (__fastcall **)(DragNDropProcessor *))(*(_QWORD *)this + 48LL))(this);
      InputETW::InputGesture::DragInProgress(
        v48,
        v47,
        (int *)&PositionRelativeToSite,
        (int *)&v70,
        (int *)&v69,
        v46,
        v66,
        v45);
      (*(void (__fastcall **)(__int64, _QWORD, struct tagPOINT *, struct tagPOINT *, struct tagPOINT *, float *, __int64, unsigned int, __int64))(*(_QWORD *)(*(_QWORD *)(v13 + 32) + 8LL) + 32LL))(
        *(_QWORD *)(v13 + 32) + 8LL,
        v63,
        &PositionRelativeToSite,
        &v70,
        &v69,
        v110,
        v46,
        v66,
        v45);
    }
    else
    {
      DragNDropProcessor::FeedAnimationDataIfNeeded(this, v63, PositionRelativeToSite, v111[1], 1u);
      v50 = v63;
      v51 = (const WCHAR *)(*(__int64 (__fastcall **)(DragNDropProcessor *))(*(_QWORD *)this + 48LL))(this);
      InputETW::InputGesture::DragEnded(
        v51,
        v50,
        (int *)&PositionRelativeToSite,
        (int *)&v70,
        (int *)&v69,
        v46,
        v66,
        v71);
      (*(void (__fastcall **)(__int64, _QWORD, struct tagPOINT *, struct tagPOINT *, struct tagPOINT *, float *, __int64, unsigned int, __int64))(*(_QWORD *)(*(_QWORD *)(v13 + 32) + 8LL) + 40LL))(
        *(_QWORD *)(v13 + 32) + 8LL,
        v63,
        &PositionRelativeToSite,
        &v70,
        &v69,
        v110,
        v46,
        v66,
        v71);
      v54 = *(ManualDragAreaClientProxy **)(v13 + 40);
      if ( v54 )
        ManualDragAreaClientProxy::OnManualDragEnded(v54);
      VariableSizedPayloadStorage<InputInfo>::VariableSizedPayloadStorage<InputInfo>((__int64)&v112, v5, v52, v53);
      *(_QWORD *)&v113 = v85;
      *((struct tagPOINT *)&v113 + 1) = v11;
      std::_Hash<std::_Umap_traits<unsigned long,DragNDropProcessor::ResultInfo,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,DragNDropProcessor::ResultInfo>>,0>>::emplace<unsigned long &,DragNDropProcessor::ResultInfo>(
        (float *)this + 44,
        (__int64)v105,
        &v63,
        (__int64 *)&v112);
      VariableSizedPayloadStorage<InputInfo>::~VariableSizedPayloadStorage<InputInfo>((void **)&v112, v55);
    }
    DragNDropProcessor::MakeContextualProcessingDecision(v49, v13 + 40, v63, (__int64)v78, (__int64)v77);
    goto LABEL_62;
  }
  return 0LL;
}
