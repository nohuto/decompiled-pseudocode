/*
 * XREFs of ?ResultFromCaughtException_CppWinRt@details@wil@@YAJPEAG_KPEA_N@Z @ 0x1800DC080
 * Callers:
 *     <none>
 * Callees:
        ...
        bool *a4)
{
  ...
  const std::bad_alloc *v11; // [rsp+38h] [rbp-50h] BYREF
  ...
  const std::bad_alloc *v15; // [rsp+58h] [rbp-30h] BYREF
  const std::out_of_range *v16; // [rsp+60h] [rbp-28h] BYREF
  const std::invalid_argument *v17; // [rsp+68h] [rbp-20h] BYREF
  const std::exception *v18; // [rsp+70h] [rbp-18h] BYREF
  const struct wil::ResultException *v19; // [rsp+90h] [rbp+8h]
  const struct winrt::hresult_error *v20; // [rsp+90h] [rbp+8h]
  const struct std::exception *v21; // [rsp+90h] [rbp+8h]
  const struct std::exception *v22; // [rsp+90h] [rbp+8h]
  const struct std::exception *v23; // [rsp+90h] [rbp+8h]
  const struct wil::ResultException *v24; // [rsp+90h] [rbp+8h]
  const struct winrt::hresult_error *v25; // [rsp+90h] [rbp+8h]
  const struct std::exception *v26; // [rsp+90h] [rbp+8h]
  const struct std::exception *v27; // [rsp+90h] [rbp+8h]
  const struct std::exception *v28; // [rsp+90h] [rbp+8h]
  const struct std::exception *v29; // [rsp+90h] [rbp+8h]
  ...
  if ( wil::g_pfnResultFromCaughtException )
  {
    ...
    catch ( const winrt::hresult_error *v8 )
    {
      ...
      winrt::hresult_error::to_abi((__int64)v8, &v43);
      return;
    }
    catch ( const std::bad_alloc *v11 )
    {
  ...
  catch ( const winrt::hresult_error *v9 )
  {
    ...
    winrt::hresult_error::to_abi((__int64)v9, &v43);
    return;
  }
