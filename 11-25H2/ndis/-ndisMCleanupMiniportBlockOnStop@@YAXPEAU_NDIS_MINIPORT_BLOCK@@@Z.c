/*
 * XREFs of ?ndisMCleanupMiniportBlockOnStop@@YAXPEAU_NDIS_MINIPORT_BLOCK@@@Z @ 0x14004EC40
 * Callers:
...
void __fastcall ndisMCleanupMiniportBlockOnStop(struct _NDIS_MINIPORT_BLOCK *a1)
{
  ...
  _X_FILTER *v6; // rcx
  _X_FILTER *EthDB; // rdi
  PNDIS_RW_LOCK_EX *Reserved1; // rdi
  ...
  if ( a1->MediaType || (v6 = a1->EthDB) == 0LL )
  {
    ...
    if ( !EthDB )
      goto LABEL_15;
    NdisFreeRWLock(EthDB->BindListLock);
    ExFreePoolWithTag(EthDB, 0);
  }
  else
  {
    EthFreeFilterBuffers(v6, 1);
  }
  a1->EthDB = 0LL;
LABEL_15:
  Reserved1 = (PNDIS_RW_LOCK_EX *)a1->Reserved1;
  if ( Reserved1 )
  {
    NdisFreeRWLock(Reserved1[36]);
    ExFreePoolWithTag(Reserved1, 0);
    a1->Reserved1 = 0LL;
  }
  OidList = a1->OidList;
  if ( OidList )
