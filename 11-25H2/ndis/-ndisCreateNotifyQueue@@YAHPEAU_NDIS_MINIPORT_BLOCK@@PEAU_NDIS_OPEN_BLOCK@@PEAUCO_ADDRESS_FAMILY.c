/*
 * XREFs of ?ndisCreateNotifyQueue@@YAHPEAU_NDIS_MINIPORT_BLOCK@@PEAU_NDIS_OPEN_BLOCK@@PEAUCO_ADDRESS_FAMILY@@PEAPEAU_NDIS_AF_NOTIFY@@@Z @ 0x140182A70
 * Callers:
  ...
  else
  {
    for ( i = a1->OpenQueue; i; i = i->MiniportNextOpen )
    {
      if ( i->ProtocolHandle->CoAfRegisterNotifyHandler )
      {
        ...
        {
          v16 = ExAllocatePool2(64LL, 72LL, 1868776526);
          if ( !v16 )
          {
            if ( *(int **)&WPP_RECORDER_INITIALIZED != &WPP_RECORDER_INITIALIZED )
            {
