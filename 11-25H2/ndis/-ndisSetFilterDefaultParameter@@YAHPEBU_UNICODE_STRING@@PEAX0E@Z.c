/*
 * XREFs of ?ndisSetFilterDefaultParameter@@YAHPEBU_UNICODE_STRING@@PEAX0E@Z @ 0x1400A3724
 * Callers:
    ...
    if ( !Value )
    {
      ...
      if ( !DestinationString.Buffer )
      {
        if ( String2.Buffer )
        {
          for ( i = 0; i < 6; ++i )
          {
            if ( !RtlCompareUnicodeString((PCUNICODE_STRING)((char *)&unk_1400FDB10 + 24 * i), &String2, 1u) )
            {
              v14 = (const WCHAR *)&word_1400FF1B8;
              if ( (unsigned int)(*((_DWORD *)&unk_1400FDB10 + 6 * i + 4) - 4) <= 1 )
                goto LABEL_18;
              break;
            }
          }
