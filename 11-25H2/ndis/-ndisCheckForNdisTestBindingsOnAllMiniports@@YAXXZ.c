/*
 * XREFs of ?ndisCheckForNdisTestBindingsOnAllMiniports@@YAXXZ @ 0x1400BF220
 * Callers:
...
void ndisCheckForNdisTestBindingsOnAllMiniports(void)
{
  ...
  if ( ndisMiniDriverList )
  {
    do
    {
      ...
      v9 = 0;
      if ( ndisReferenceRefEx(&v1->Ref.SpinLock, 9u, (enum _NDIS_REFERENCE_STATUS *)&v9) )
      {
        ...
        while ( MiniportQueue )
        {
          if ( ndisIsMiniportReady(MiniportQueue) && (unsigned __int8)ndisReferenceMiniport(MiniportQueue, 0x35u) )
          {
            ...
            ndisDereferenceMiniport(MiniportQueue, 0x35u);
          }
