/*
 * XREFs of ?ShouldChainForAxis@CInteractionTracker@@QEBA_NW4ScrollAxis@@PEBVCManipulation@@_N@Z @ 0x18003DDB8
 * Callers:
 *     _lambda_f87cc9cbc192fb07b4c8cbee31a42eb9_::operator() @ 0x18003D720 (_lambda_f87cc9cbc192fb07b4c8cbee31a42eb9_--operator().c)
 *     ?UpdateValueWithChaining@CScrollAnimation@@IEAAMMMW4Boundary@@@Z @ 0x1800F3724 (-UpdateValueWithChaining@CScrollAnimation@@IEAAMMMW4Boundary@@@Z.c)
 *     ?OnManipulationChanged@InteractionSourceManager@@QEAAXW4Flags@NotificationEventArgs@@PEAVCManipulation@@@Z @ 0x1800F650C (-OnManipulationChanged@InteractionSourceManager@@QEAAXW4Flags@NotificationEventArgs@@PEAVCManipu.c)
 *     ?StartInContactWorker@CInteractionTrackerPositionAnimation@@MEAAXXZ @ 0x180231FB0 (-StartInContactWorker@CInteractionTrackerPositionAnimation@@MEAAXXZ.c)
 *     ?UpdateChainingConfigIfBoundaryStateChanges@CScrollKeyframeAnimation@@IEAAXW4ScrollAxis@@W4Boundary@@@Z @ 0x180234354 (-UpdateChainingConfigIfBoundaryStateChanges@CScrollKeyframeAnimation@@IEAAXW4ScrollAxis@@W4Bound.c)
 * Callees:
 *     _lambda_7b30347a8dc9aa6d4f16b9515e4933d5_::operator() @ 0x180217954 (_lambda_7b30347a8dc9aa6d4f16b9515e4933d5_--operator().c)
 *     __security_check_cookie @ 0x18025BF00 (__security_check_cookie.c)
 */

char __fastcall CInteractionTracker::ShouldChainForAxis(__int64 a1, int a2, __int64 a3, unsigned __int8 a4)
{
  char v4; // r10
  __int128 v5; // xmm1
  __int128 v6; // xmm0
  __int128 v7; // xmm1
  __int128 v8; // xmm0
  __int128 v9; // xmm1
  __int128 v10; // xmm0
  __int128 v11; // xmm0
  __int128 v12; // xmm1
  __int64 v13; // rax
  int v14; // edx
  int v15; // ecx
  bool v16; // zf
  int v17; // ecx
  __int64 v19; // rdx
  _QWORD v20[4]; // [rsp+20h] [rbp-E0h] BYREF
  __int128 v21; // [rsp+40h] [rbp-C0h]
  __int128 v22; // [rsp+50h] [rbp-B0h]
  __int128 v23; // [rsp+60h] [rbp-A0h]
  __int128 v24; // [rsp+70h] [rbp-90h]
  __int128 v25; // [rsp+80h] [rbp-80h]
  __int128 v26; // [rsp+90h] [rbp-70h]
  __int128 v27; // [rsp+A0h] [rbp-60h]
  __int128 v28; // [rsp+B0h] [rbp-50h]
  __int128 v29; // [rsp+C0h] [rbp-40h]
  __int128 v30; // [rsp+D0h] [rbp-30h]
  __int128 v31; // [rsp+E0h] [rbp-20h]
  __int64 v32; // [rsp+F0h] [rbp-10h]
  int v33; // [rsp+128h] [rbp+28h] BYREF
  __int64 v34; // [rsp+130h] [rbp+30h] BYREF

  v34 = a3;
  v33 = a2;
  v4 = 0;
  v5 = *(_OWORD *)(a3 + 288);
  v21 = *(_OWORD *)(a3 + 272);
  v6 = *(_OWORD *)(a3 + 304);
  v22 = v5;
  v7 = *(_OWORD *)(a3 + 320);
  v23 = v6;
  v8 = *(_OWORD *)(a3 + 336);
  v24 = v7;
  v9 = *(_OWORD *)(a3 + 352);
  v25 = v8;
  v10 = *(_OWORD *)(a3 + 368);
  v26 = v9;
  v27 = v10;
  v28 = *(_OWORD *)(a3 + 384);
  v11 = *(_OWORD *)(a3 + 416);
  v29 = *(_OWORD *)(a3 + 400);
  v12 = *(_OWORD *)(a3 + 432);
  v13 = *(_QWORD *)(a3 + 448);
  v30 = v11;
  v31 = v12;
  v32 = v13;
  v20[0] = &v34;
  v20[1] = &v33;
  if ( a2 )
  {
    v14 = a2 - 1;
    if ( !v14 )
    {
      if ( !(a4 | ((v32 & 0x200000000LL) == 0)) && (_DWORD)v31 != 2 )
        return v4;
      v15 = *(_DWORD *)(a3 + 480);
      if ( v15 )
      {
        if ( v15 != 1 )
          return v4;
        v16 = HIDWORD(v31) == 0;
      }
      else
      {
        if ( (BYTE12(v31) & 1) != 0 )
          return 1;
        v16 = (BYTE12(v31) & 2) == 0;
      }
      goto LABEL_21;
    }
    if ( v14 == 1 )
    {
      v20[2] = 0x3F8000003F800000LL;
      v19 = *((_QWORD *)&v22 + 1) - 0x3F8000003F800000LL;
      if ( *((_QWORD *)&v22 + 1) == 0x3F8000003F800000LL )
        v19 = (unsigned int)v23
            - (unsigned __int64)(unsigned int)_mm_cvtsi128_si32((__m128i)(unsigned int)`CVisual::SetOpacityInternal'::`2'::sc_defaultValue);
      if ( v19 && (unsigned __int8)lambda_7b30347a8dc9aa6d4f16b9515e4933d5_::operator()(v20, (unsigned int)v32) )
        return 1;
    }
  }
  else if ( a4 | ((v32 & 0x100000000LL) == 0) || (_DWORD)v31 == 1 )
  {
    v17 = *(_DWORD *)(a3 + 472);
    if ( v17 )
    {
      if ( v17 != 1 )
        return v4;
      v16 = DWORD2(v31) == 0;
    }
    else
    {
      if ( (BYTE8(v31) & 1) != 0 )
        return 1;
      v16 = (BYTE8(v31) & 2) == 0;
    }
LABEL_21:
    if ( v16 )
      return v4;
    return 1;
  }
  return v4;
}
