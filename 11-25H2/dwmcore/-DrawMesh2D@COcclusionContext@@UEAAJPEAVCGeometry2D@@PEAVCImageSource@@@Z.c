/*
 * XREFs of ?DrawMesh2D@COcclusionContext@@UEAAJPEAVCGeometry2D@@PEAVCImageSource@@@Z @ 0x180285BD0
 * Callers:
 *     <none>
 * Callees:
 *     __security_check_cookie @ 0x18025BF00 (__security_check_cookie.c)
 *     _guard_dispatch_icall$thunk$10345483385596137414 @ 0x180301010 (_guard_dispatch_icall$thunk$10345483385596137414.c)
 */

__int64 __fastcall COcclusionContext::DrawMesh2D(
        COcclusionContext *this,
        struct CGeometry2D *a2,
        struct CImageSource *a3)
{
  __int64 v5; // rcx
  _QWORD v7[2]; // [rsp+20h] [rbp-38h] BYREF
  __int128 v8; // [rsp+30h] [rbp-28h]
  char v9; // [rsp+40h] [rbp-18h]
  int v10; // [rsp+41h] [rbp-17h]
  __int16 v11; // [rsp+45h] [rbp-13h]
  char v12; // [rsp+47h] [rbp-11h]

  if ( (*(unsigned __int8 (__fastcall **)(struct CImageSource *, __int64))(*(_QWORD *)a3 + 64LL))(a3, 18LL) )
  {
    v5 = *((_QWORD *)this + 1);
    v7[0] = *((_QWORD *)this + 193);
    v10 = 0;
    v11 = 0;
    v12 = 0;
    v7[1] = a3;
    v8 = DirectX::g_XMZero;
    v9 = 0;
    (*(void (__fastcall **)(__int64, _QWORD *))(*(_QWORD *)v5 + 208LL))(v5, v7);
  }
  return 0LL;
}
