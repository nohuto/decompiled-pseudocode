/*
 * XREFs of ?LookupLinkedShader@CShaderCache@@IEBAPEAVCLinkedShader@@ULookupKey@ShaderLinkingConfig@@@Z @ 0x18014B5C0
 * Callers:
...
volatile signed __int32 *__fastcall CShaderCache::LookupLinkedShader(__int64 a1, __int64 a2)
{
  ...
  int v6; // esi
  int v7; // esi
  CReconstructableDrawListEntry_UVx2 *(__fastcall *v8)(CReconstructableDrawListEntry_UVx2 *, char); // rax
  ...
  if ( _InterlockedDecrement(v5 + 2) < -1 )
    wil::details::in1diag3::Log_Hr(
      ...
      (const char *)0x8007029CLL,
      v10);
  v8 = *(CReconstructableDrawListEntry_UVx2 *(__fastcall **)(CReconstructableDrawListEntry_UVx2 *, char))(*(_QWORD *)v5 + 16LL);
  ...
  else
  {
    ...
    {
      v8((CReconstructableDrawListEntry_UVx2 *)v5, 1);
    }
    return v5;
  }
}
