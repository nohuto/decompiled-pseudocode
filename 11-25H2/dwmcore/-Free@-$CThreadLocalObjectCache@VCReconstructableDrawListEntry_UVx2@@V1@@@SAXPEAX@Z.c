/*
 * XREFs of ?Free@?$CThreadLocalObjectCache@VCReconstructableDrawListEntry_UVx2@@V1@@@SAXPEAX@Z @ 0x18014C0DC
 * Callers:
...
int __fastcall CThreadLocalObjectCache<CReconstructableDrawListEntry_UVx2,CReconstructableDrawListEntry_UVx2>::Free(
        _QWORD *lpMem)
{
  _DWORD *Value; // rbx
  __int64 v3; // rax
  ...
  LODWORD(v3) = Value[52];
  if ( Value[53] < (unsigned int)v3 )
  {
    v3 = *((_QWORD *)Value + 27);
    *lpMem = v3;
    ++Value[53];
    *((_QWORD *)Value + 27) = lpMem;
  }
  else if ( lpMem )
  {
