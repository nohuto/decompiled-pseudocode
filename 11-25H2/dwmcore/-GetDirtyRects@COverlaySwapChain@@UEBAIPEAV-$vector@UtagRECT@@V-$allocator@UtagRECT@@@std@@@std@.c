/*
 * XREFs of ?GetDirtyRects@COverlaySwapChain@@UEBAIPEAV?$vector@UtagRECT@@V?$allocator@UtagRECT@@@std@@@std@@@Z @ 0x18006BF70
 * Callers:
 *     <none>
 * Callees:
    ...
    if ( (_DWORD)v6 )
    {
      ...
      if ( v8 > ((_BYTE *)a2[2] - (_BYTE *)*a2) >> 4 )
      {
        if ( v8 > 0xFFFFFFFFFFFFFFFLL )
          std::vector<std::unique_ptr<CBetterCheckMPOCache>>::_Xlength(v2 + 3);
