/*
 * XREFs of ?GetVads@CVirtualAddressAllocator@@QEAAXPEAU_D3DKMT_VIDMM_ESCAPE@@@Z @ 0x140099738
 * Callers:
      ...
      else
      {
        ...
        for ( k = *v34; k != (__int64 *)v34; k = (__int64 *)*k )
        {
          ...
          if ( NtHandle >= *(k - 2) && NtHandle < *(k - 1) )
          {
            ...
            {
              v38 = (_QWORD *)k[PhysicalAdapterIndex + 7];
              v39 = 0;
              m = 0LL;
              while ( v38 )
              {
              ...
              while ( m )
              {
                if ( v39 == a2->GetVads.GetVadRange.VaRangeIndex )
                {
                  v44 = m - 1;
                  if ( v44 )
                  {
