/*
 * XREFs of sub_18000D7F4 @ 0x18000D7F4
 * Callers:
 *     sub_18000E438 @ 0x18000E438 (sub_18000E438.c)
 * Callees:
 *     __security_check_cookie @ 0x18000B2B0 (__security_check_cookie.c)
 *     sub_18000D6B0 @ 0x18000D6B0 (sub_18000D6B0.c)
 *     sub_18000D6D0 @ 0x18000D6D0 (sub_18000D6D0.c)
 *     sub_18000E1C8 @ 0x18000E1C8 (sub_18000E1C8.c)
 *     sub_18000EBFC @ 0x18000EBFC (sub_18000EBFC.c)
 *     sub_18000F0E4 @ 0x18000F0E4 (sub_18000F0E4.c)
 *     sub_18000F688 @ 0x18000F688 (sub_18000F688.c)
 *     sub_18000F7EC @ 0x18000F7EC (sub_18000F7EC.c)
 *     sub_18000FB98 @ 0x18000FB98 (sub_18000FB98.c)
 *     sub_18000FCB4 @ 0x18000FCB4 (sub_18000FCB4.c)
 */

__int64 __fastcall sub_18000D7F4(__int64 a1, _QWORD *a2)
{
  DWORD CurrentProcessId; // eax
  HANDLE Mutex; // rax
  unsigned int v6; // ebx
  __int64 v7; // rdx
  int v8; // eax
  __int64 v9; // r9
  __int64 v10; // rdx
  _DWORD *v11; // rcx
  int v13; // eax
  __int64 v14; // [rsp+30h] [rbp-D0h] BYREF
  __int64 v15; // [rsp+38h] [rbp-C8h] BYREF
  __int64 v16; // [rsp+40h] [rbp-C0h] BYREF
  WCHAR Name[264]; // [rsp+50h] [rbp-B0h] BYREF
  void *retaddr; // [rsp+278h] [rbp+178h]

  *a2 = 0LL;
  CurrentProcessId = GetCurrentProcessId();
  sub_18000F688(Name, 260LL, L"Local\\SM0:%lu:%lu:%hs", CurrentProcessId, 120, a1);
  v14 = 0LL;
  Mutex = CreateMutexExW(0LL, Name, 0, 0x1F0001u);
  sub_18000FCB4(&v14, Mutex);
  if ( v14 )
  {
    sub_18000FB98(&v14, &v15);
    v16 = 0LL;
    v8 = sub_18000F7EC(Name, v7, &v16);
    v6 = v8;
    if ( v8 < 0 )
    {
      sub_18000F0E4(retaddr, 100LL, "wil", (unsigned int)v8);
      sub_18000F0E4(retaddr, 109LL, "wil", v6);
      v9 = v6;
      v10 = 299LL;
LABEL_5:
      sub_18000F0E4(retaddr, v10, "wil", v9);
      sub_18000D6D0(&v15);
      goto LABEL_9;
    }
    v11 = (_DWORD *)(4 * v16);
    if ( 4 * v16 )
    {
      *a2 = v11;
      *(_DWORD *)*a2 = *v11 + 1;
    }
    else
    {
      v13 = sub_18000EBFC(Name, &v14, a2);
      v6 = v13;
      if ( v13 < 0 )
      {
        v9 = (unsigned int)v13;
        v10 = 308LL;
        goto LABEL_5;
      }
    }
    sub_18000D6D0(&v15);
    v6 = 0;
    goto LABEL_9;
  }
  v6 = sub_18000E1C8();
LABEL_9:
  sub_18000D6B0(&v14);
  return v6;
}
