/*
 * XREFs of sub_18006B190 @ 0x18006B190
 * Callers:
 *     sub_18006B350 @ 0x18006B350 (sub_18006B350.c)
 * Callees:
 *     sub_18000B2D4 @ 0x18000B2D4 (sub_18000B2D4.c)
 *     sub_180010134 @ 0x180010134 (sub_180010134.c)
 *     sub_18001050C @ 0x18001050C (sub_18001050C.c)
 *     sub_180012040 @ 0x180012040 (sub_180012040.c)
 */

__int64 __fastcall sub_18006B190(_QWORD *a1)
{
  __int64 v2; // rcx
  __int64 v3; // rcx
  __int64 v4; // rcx
  __int64 v5; // rcx
  __int64 v6; // rcx
  __int64 v7; // rcx
  __int64 v8; // rcx
  __int64 v9; // rcx
  __int64 v10; // rcx
  __int64 v11; // rcx
  __int64 v12; // rcx
  __int64 v13; // rcx
  __int64 v14; // rcx
  __int64 result; // rax
  __int64 v16; // rcx
  __int64 v17; // rcx

  *a1 = &Spectre::Engine::ImageProcessingManager::`vftable';
  v2 = a1[60];
  if ( v2 )
    sub_18001050C(v2);
  v3 = a1[58];
  if ( v3 )
    sub_18001050C(v3);
  v4 = a1[55];
  if ( v4 )
    sub_18001050C(v4);
  v5 = a1[53];
  if ( v5 )
    sub_18001050C(v5);
  v6 = a1[51];
  if ( v6 )
    sub_18001050C(v6);
  v7 = a1[49];
  if ( v7 )
    sub_18001050C(v7);
  v8 = a1[47];
  if ( v8 )
    sub_18001050C(v8);
  v9 = a1[45];
  if ( v9 )
    sub_18001050C(v9);
  v10 = a1[43];
  if ( v10 )
    sub_18001050C(v10);
  v11 = a1[41];
  if ( v11 )
    sub_18001050C(v11);
  v12 = a1[39];
  if ( v12 )
    sub_18001050C(v12);
  v13 = a1[37];
  if ( v13 )
    sub_18001050C(v13);
  v14 = a1[35];
  if ( v14 )
    sub_18001050C(v14);
  sub_18000B2D4(
    (__int64)(a1 + 22),
    16LL,
    6LL,
    (void (__fastcall *)(__int64))Concurrency::details::TaskStack::~TaskStack);
  result = sub_18000B2D4(
             (__int64)(a1 + 10),
             16LL,
             6LL,
             (void (__fastcall *)(__int64))Concurrency::details::TaskStack::~TaskStack);
  v16 = a1[9];
  if ( v16 )
    result = sub_18001050C(v16);
  v17 = a1[5];
  if ( v17 )
  {
    sub_180012040(v17, a1[6]);
    result = sub_180010134((void *)a1[5], (a1[7] - a1[5]) & 0xFFFFFFFFFFFFFFF0uLL);
    a1[5] = 0LL;
    a1[6] = 0LL;
    a1[7] = 0LL;
  }
  return result;
}
