/*
 * XREFs of sub_1800C27A0 @ 0x1800C27A0
 * Callers:
 *     <none>
 * Callees:
 *     sub_18000E854 @ 0x18000E854 (sub_18000E854.c)
 *     sub_18001050C @ 0x18001050C (sub_18001050C.c)
 *     unknown_libname_81 @ 0x180011CB0 (unknown_libname_81.c)
 *     sub_180014FA0 @ 0x180014FA0 (sub_180014FA0.c)
 *     sub_1800C2FB4 @ 0x1800C2FB4 (sub_1800C2FB4.c)
 *     sub_1800C30B0 @ 0x1800C30B0 (sub_1800C30B0.c)
 *     j__guard_dispatch_icall @ 0x1800EA010 (j__guard_dispatch_icall.c)
 */

// Hidden C++ exception states: #wind=3
__int64 __fastcall sub_1800C27A0(__int64 a1, _QWORD *a2, unsigned __int8 a3, char a4)
{
  unsigned int v5; // r15d
  __int64 *v7; // rax
  char v8; // bl
  __int64 v9; // rcx
  __int64 v10; // rdi
  __int64 *v11; // rax
  __int64 v12; // rdx
  __int64 v13; // rcx
  __int64 v14; // r8
  __int64 result; // rax
  __int64 v16; // [rsp+30h] [rbp-30h] BYREF
  __int64 v17; // [rsp+38h] [rbp-28h]
  _QWORD v18[2]; // [rsp+40h] [rbp-20h] BYREF
  __int64 v19; // [rsp+50h] [rbp-10h] BYREF
  __int64 v20; // [rsp+58h] [rbp-8h]
  __int64 v21; // [rsp+90h] [rbp+30h] BYREF
  int v22; // [rsp+A0h] [rbp+40h]

  v5 = a3;
  v22 = 0;
  if ( *a2 )
  {
    v7 = (__int64 *)sub_1800C2FB4(*a2, &v19, *(unsigned int *)(a1 + 88));
    v8 = 1;
    v9 = *v7;
  }
  else
  {
    v17 = 0LL;
    v7 = &v16;
    v8 = 2;
    v9 = 0LL;
  }
  v18[0] = v9;
  v10 = v7[1];
  v18[1] = v10;
  *v7 = 0LL;
  v7[1] = 0LL;
  if ( (v8 & 2) != 0 )
  {
    v8 &= ~2u;
    if ( v17 )
      sub_18001050C(v17);
  }
  if ( (v8 & 1) != 0 && v20 )
    sub_18001050C(v20);
  v11 = unknown_libname_81(&v19, v18);
  sub_180014FA0(&v16, (__int64)v11, *v11);
  v13 = *(_QWORD *)(v12 + 8);
  if ( v13 )
    sub_18001050C(v13);
  sub_1800C30B0(v16, &v21);
  v14 = v5 | 2;
  if ( !a4 )
    v14 = v5;
  (*(void (__fastcall **)(_QWORD, __int64, __int64))(**(_QWORD **)(a1 + 144) + 424LL))(*(_QWORD *)(a1 + 144), v21, v14);
  (*(void (__fastcall **)(__int64))(*(_QWORD *)a1 + 296LL))(a1);
  result = sub_18000E854(&v21);
  if ( v17 )
    result = sub_18001050C(v17);
  if ( v10 )
    return sub_18001050C(v10);
  return result;
}
