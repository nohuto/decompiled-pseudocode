...
ULONG __fastcall DumpObject(__int64 a1, __int64 a2, unsigned int a3)
{
  ...
  if ( v6 > 0x80 )
  {
      ...
      if ( v31 )
      {
        ...
        if ( v32 )
        {
          if ( v32 == 1 )
          {
            ...
            ConPrintf("IndexField(%s:IndexName=%s,DataName=%s)", byte_140075488, pszDest, v37);
          }
        }
        else
        {
          RtlStringCchCopyNA(pszDest, 5uLL, (STRSAFE_PCNZCH)(**(_QWORD **)(a1 + 32) + 40LL), 4uLL);
          ConPrintf("Field(%s:Base=%s)", byte_140075488, pszDest);
        }
      }
      else
      {
    ...
    {
      ConPrintf("DataAlias(%s:Link=%x)", byte_140075488, *(_QWORD *)(a1 + 16));
  ...
  else if ( v6 == 128 )
  {
    ...
    ConPrintf("ObjectAlias(%s:Alias=%s,Type=%s)", byte_140075488, v29, ObjectTypeName);
    if ( ObjectPath )
      ...
      if ( v20 )
      {
        ...
        if ( v21 )
        {
          ...
          if ( v22 )
          {
            ...
            {
              if ( v23 == 1 )
                ConPrintf("DDBHandle(%s:Handle=%x)", byte_140075488, *(_QWORD *)(a1 + 16));
            }
            else
            {
              v24 = *(_QWORD *)(a1 + 32);
          ...
          {
            ConPrintf("ThermalZone(%s)", byte_140075488);
          }
        }
        else
        {
...
LABEL_53:
  ...
  else
  {
    if ( !*(_WORD *)(a1 + 2) )
    {
      ConPrintf("Unknown(%s)", byte_140075488);
      return ConPrintf("\n");
    }
    v7 = v6 - 1;
    if ( !v7 )
    {
      ConPrintf("Integer(%s:Value=0x%I64x[%I64d])", byte_140075488, *(_QWORD *)(a1 + 16), *(_QWORD *)(a1 + 16));
      return ConPrintf("\n");
    }
    v8 = v7 - 1;
    if ( !v8 )
...
LABEL_29:
              ConPrintf(v13, byte_140075488, *(_QWORD *)(a1 + 32));
            }
          }
          else
          {
            ConPrintf("Device(%s)", byte_140075488);
          }
        }
        else
        {
      ...
      {
        ConPrintf("Package(%s:NumElements=%d){", byte_140075488, **(_DWORD **)(a1 + 32));
        ConPrintf("\n");
        v15 = *(_DWORD **)(a1 + 32);
    ...
    {
      ConPrintf("Buffer(%s:Ptr=%x,Len=%d)", byte_140075488, *(_QWORD *)(a1 + 32), *(_DWORD *)(a1 + 24));
