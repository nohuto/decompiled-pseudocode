/*
 * XREFs of ACPIPepInitializePlatformNotificationSupport @ 0x1400A1578
 * Callers:
  ...
  *((_DWORD *)Context + 46) = 1;
  ...
  if ( v14 )
  {
    ...
    if ( (_BYTE)v14 )
    {
      ...
      {
        AcpiGetDriverProxyEndpoint(&v19, (__int64)ACPIPepWnfCallback);
        ExSubscribeWnfStateChange(Context + 336, &WNF_PO_MODERN_STANDBY_STATE_NOTIFICATION, 1LL, 0LL, v19, Context);
      }
      else
      {
        AcpiGetDriverProxyEndpoint(&Callback, (__int64)ACPIPepPowerSettingChangeCallback);
        v15 = Callback;
