/*
 * XREFs of sub_14017A21C @ 0x14017A21C
 * Callers:
 *     sub_14002D1F0 @ 0x14002D1F0 (sub_14002D1F0.c)
 * Callees:
 *     sub_140020090 @ 0x140020090 (sub_140020090.c)
 *     sub_1400296E0 @ 0x1400296E0 (sub_1400296E0.c)
 *     sub_140052E64 @ 0x140052E64 (sub_140052E64.c)
 *     sub_140052F3C @ 0x140052F3C (sub_140052F3C.c)
 *     sub_140056AB0 @ 0x140056AB0 (sub_140056AB0.c)
 *     sub_14006DF7C @ 0x14006DF7C (sub_14006DF7C.c)
 *     __security_check_cookie @ 0x14013C590 (__security_check_cookie.c)
 */

__int64 __fastcall sub_14017A21C(__int64 a1, __int64 a2)
{
  int v3; // ebx
  unsigned int v5; // edi
  _DWORD *v6; // r13
  __int64 *v7; // rsi
  int v8; // eax
  int v9; // ecx
  __int64 v10; // rdi
  bool v11; // zf
  unsigned __int64 v12; // rcx
  __int64 v13; // rdx
  int *v14; // rax
  const EVENT_DESCRIPTOR *v15; // rdx
  unsigned __int64 v16; // rdx
  char v17; // r12
  _BYTE *v18; // r9
  unsigned __int8 v19; // r10
  char v20; // si
  char v21; // r11
  char v22; // r13
  char *v23; // r11
  unsigned int v24; // r8d
  unsigned int *v25; // rax
  __int64 v26; // rax
  unsigned __int64 v27; // r15
  char v28; // cl
  char v29; // r8
  _BYTE *v30; // rax
  unsigned int v31; // eax
  char v32; // al
  char v34; // [rsp+60h] [rbp-29h]
  char v35; // [rsp+61h] [rbp-28h]
  int v36; // [rsp+68h] [rbp-21h]
  unsigned __int64 v37; // [rsp+70h] [rbp-19h]
  GUID v38; // [rsp+98h] [rbp+Fh] BYREF

  v3 = 0;
  v5 = 0;
  v6 = *(_DWORD **)(a2 + 24);
  v7 = (__int64 *)(a2 + 184);
  if ( !v6 )
  {
LABEL_17:
    v5 = -1073741811;
    goto LABEL_18;
  }
  if ( *(_DWORD *)(*v7 + 16) >= 0x1Cu )
  {
    if ( *v6 == 28 && v6[1] == 28 )
    {
      v8 = v6[3];
      if ( v8 )
      {
        if ( v8 < 3 )
        {
          v9 = v6[4];
          if ( v9 < 0 || v9 == 1 )
          {
            if ( v8 == 1 )
            {
              v5 = sub_14006DF7C((__int64)v6, 2LL);
            }
            else if ( v8 == 2 )
            {
              v10 = a1 + 376;
              if ( sub_140020090(a1 + 376, 21) )
                v5 = sub_1400296E0(v10);
              else
                v5 = -1073741637;
              v7 = (__int64 *)(a2 + 184);
            }
            goto LABEL_18;
          }
        }
      }
    }
    goto LABEL_17;
  }
  v5 = -1073741820;
LABEL_18:
  v11 = byte_140168DAA == 0;
  *(_QWORD *)(a2 + 56) = 0LL;
  *(_BYTE *)(a2 + 141) = -84;
  *(_DWORD *)(a2 + 48) = v5;
  if ( v11 )
    goto LABEL_84;
  v38 = 0LL;
  IoGetActivityIdIrp(a2, &v38);
  v13 = *v7;
  if ( *(_BYTE *)*v7 == 14 )
  {
    if ( (byte_1401694F2 & 8) == 0 )
      goto LABEL_84;
    v15 = &stru_140148B18;
    goto LABEL_83;
  }
  if ( *(_BYTE *)*v7 != 15 )
  {
    if ( *(_BYTE *)*v7 != 27 )
      goto LABEL_84;
    if ( *(_BYTE *)(v13 + 1) == 7 && !*(_DWORD *)(v13 + 8) )
    {
      if ( (byte_1401694F2 & 0x40) != 0 )
      {
        v14 = *(int **)(a2 + 56);
        if ( v14 )
          v3 = *v14;
        sub_140056AB0(v12, v13, &v38, a2, v3, *(_DWORD *)(a2 + 48));
      }
      goto LABEL_84;
    }
    if ( (byte_1401694F2 & 0x20) == 0 )
      goto LABEL_84;
    v15 = &stru_140149FE8;
LABEL_83:
    sub_140052F3C(v12, v15, &v38, a2, *(_DWORD *)(a2 + 48));
    goto LABEL_84;
  }
  if ( byte_1401694F1 >= 0 )
    goto LABEL_84;
  v16 = *(_QWORD *)(v13 + 8);
  v17 = 0;
  v18 = 0LL;
  v34 = 0;
  v19 = 0;
  v20 = 0;
  v21 = 0;
  v22 = 0;
  if ( *(_BYTE *)(v16 + 2) != 40 )
  {
    v28 = *(_BYTE *)(v16 + 72);
    v18 = *(_BYTE **)(v16 + 32);
    v19 = *(_BYTE *)(v16 + 11);
    v17 = *(_BYTE *)(v16 + 4);
    if ( !*(_BYTE *)(v16 + 2) )
      goto LABEL_60;
    goto LABEL_84;
  }
  v23 = 0LL;
  v35 = 0;
  if ( *(_DWORD *)(v16 + 20) )
    goto LABEL_84;
  v12 = 0LL;
  v36 = 0;
  if ( !*(_DWORD *)(v16 + 56) )
    goto LABEL_57;
  v24 = *(_DWORD *)(v16 + 56);
  v25 = (unsigned int *)(v16 + 120);
  v37 = v16 + 120;
  while ( 1 )
  {
    v26 = *v25;
    if ( (unsigned int)v26 >= 0x80 )
    {
      v27 = *(unsigned int *)(v16 + 16);
      if ( (unsigned int)v26 < (unsigned int)v27 )
        break;
    }
LABEL_51:
    v12 = (unsigned int)(v12 + 1);
    v25 = (unsigned int *)(v37 + 4);
    v36 = v12;
    v37 += 4LL;
    if ( (unsigned int)v12 >= v24 )
      goto LABEL_57;
  }
  if ( *(_DWORD *)(v26 + v16) != 64 )
  {
    v12 = (unsigned int)(*(_DWORD *)(v26 + v16) - 65);
    if ( *(_DWORD *)(v26 + v16) == 65 )
    {
      v12 = v26 + 56;
      if ( v26 + 56 <= v27 )
      {
        v35 = 1;
        if ( *(_BYTE *)(v26 + v16 + 10) )
          v23 = (char *)(v26 + v16 + 24);
        v17 = *(_BYTE *)(v26 + v16 + 8);
        v18 = *(_BYTE **)(v26 + v16 + 16);
        v19 = *(_BYTE *)(v26 + v16 + 9);
      }
    }
    else if ( *(_DWORD *)(v26 + v16) == 66 )
    {
      v12 = v26 + 40;
      if ( v26 + 40 <= v27 )
      {
        if ( *(_DWORD *)(v26 + v16 + 12) )
          v23 = (char *)(v26 + v16 + 32);
        v18 = *(_BYTE **)(v26 + v16 + 24);
        goto LABEL_56;
      }
    }
    goto LABEL_49;
  }
  v12 = v26 + 40;
  if ( v26 + 40 > v27 )
  {
LABEL_49:
    if ( v35 )
      goto LABEL_57;
    LODWORD(v12) = v36;
    v24 = *(_DWORD *)(v16 + 56);
    goto LABEL_51;
  }
  if ( *(_BYTE *)(v26 + v16 + 10) )
    v23 = (char *)(v26 + v16 + 24);
  v18 = *(_BYTE **)(v26 + v16 + 16);
LABEL_56:
  v19 = *(_BYTE *)(v26 + v16 + 9);
  v17 = *(_BYTE *)(v26 + v16 + 8);
LABEL_57:
  if ( v23 )
  {
    v28 = *v23;
    v21 = 0;
LABEL_60:
    LOBYTE(v12) = v28 - 8;
    if ( (v12 & 0x5D) == 0 )
    {
      v29 = *(_BYTE *)(v16 + 3);
      if ( v29 == 1 || !v18 || !v19 )
        goto LABEL_79;
      LOBYTE(v16) = 0;
      v12 = (unsigned __int64)&v18[v19];
      v30 = v18 + 8;
      if ( (unsigned __int8)((*v18 & 0x7F) - 114) <= 1u )
      {
        if ( (unsigned __int64)v30 <= v12 )
        {
          LOBYTE(v16) = 1;
          v21 = v18[2];
          v20 = v18[1] & 0xF;
          v22 = v18[3];
        }
      }
      else
      {
        if ( (unsigned __int64)v30 <= v12 )
        {
          v20 = v18[2] & 0xF;
          v31 = v19;
          if ( (unsigned int)(unsigned __int8)v18[7] + 8 <= v19 )
            v31 = (unsigned __int8)v18[7] + 8;
          v12 = (unsigned __int64)(v18 + 13);
          v16 = (unsigned __int64)&v18[v31];
          if ( (unsigned __int64)(v18 + 13) > v16 )
            v34 = 0;
          else
            v34 = v18[12];
          if ( (unsigned __int64)(v18 + 14) <= v16 )
            v22 = *(_BYTE *)v12;
          LOBYTE(v16) = 1;
        }
        v21 = v34;
      }
      if ( (_BYTE)v16 )
      {
        LOBYTE(v3) = v21;
        v32 = v22;
      }
      else
      {
LABEL_79:
        v20 = 0;
        v32 = 0;
      }
      sub_140052E64(v12, v16, &v38, a2, *(_DWORD *)(a2 + 48), v29, v17, v20, v3, v32, a2);
    }
  }
LABEL_84:
  IofCompleteRequest((PIRP)a2, 0);
  return v5;
}
