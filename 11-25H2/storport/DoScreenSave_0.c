/*
 * XREFs of DoScreenSave_0 @ 0x1400A0338
 * Callers:
 *     sub_14009F5C4 @ 0x14009F5C4 (sub_14009F5C4.c)
 * Callees:
 *     sub_140014B7C @ 0x140014B7C (sub_140014B7C.c)
 *     sub_1400158A4 @ 0x1400158A4 (sub_1400158A4.c)
 *     sub_140015A14 @ 0x140015A14 (sub_140015A14.c)
 *     sub_140018E30 @ 0x140018E30 (sub_140018E30.c)
 *     sub_140037A5C @ 0x140037A5C (sub_140037A5C.c)
 *     sub_140090464 @ 0x140090464 (sub_140090464.c)
 *     __security_check_cookie @ 0x14013C590 (__security_check_cookie.c)
 */

__int64 __fastcall DoScreenSave_0(__int64 a1)
{
  __int64 v2; // rdi
  _DWORD *v3; // r15
  int v4; // ebx
  __int64 v5; // r15
  __int64 v6; // rax
  __int64 v7; // r8
  ULONG_PTR v8; // rsi
  NTSTATUS Status; // r15d
  __int64 v10; // rcx
  unsigned int LockArray_high; // eax
  __int64 v12; // rdx
  unsigned int v13; // eax
  unsigned int v14; // eax
  IRP *v15; // rax
  IRP *v16; // r13
  __int64 v17; // rdx
  __int64 v18; // r8
  __int64 v19; // rcx
  unsigned __int64 v20; // r9
  char v21; // r10
  unsigned __int8 i; // al
  char v23; // di
  char v24; // r12
  __int64 v25; // rcx
  char v27; // [rsp+48h] [rbp-C0h] BYREF
  __int16 v28; // [rsp+49h] [rbp-BFh] BYREF
  char v29; // [rsp+4Bh] [rbp-BDh] BYREF
  int v30; // [rsp+4Ch] [rbp-BCh]
  __int64 v31; // [rsp+50h] [rbp-B8h] BYREF
  _QWORD v32[4]; // [rsp+58h] [rbp-B0h] BYREF
  struct _EVENT_DATA_DESCRIPTOR v33; // [rsp+78h] [rbp-90h] BYREF
  __int64 v34; // [rsp+98h] [rbp-70h]
  __int64 v35; // [rsp+A0h] [rbp-68h]
  __int64 v36; // [rsp+A8h] [rbp-60h]
  __int64 v37; // [rsp+B0h] [rbp-58h]
  __int64 *v38; // [rsp+B8h] [rbp-50h]
  __int64 v39; // [rsp+C0h] [rbp-48h]
  char *v40; // [rsp+C8h] [rbp-40h]
  __int64 v41; // [rsp+D0h] [rbp-38h]
  __int16 *v42; // [rsp+D8h] [rbp-30h]
  __int64 v43; // [rsp+E0h] [rbp-28h]
  char *v44; // [rsp+E8h] [rbp-20h]
  __int64 v45; // [rsp+F0h] [rbp-18h]
  char *v46; // [rsp+F8h] [rbp-10h]
  __int64 v47; // [rsp+100h] [rbp-8h]
  _QWORD *v48; // [rsp+108h] [rbp+0h]
  __int64 v49; // [rsp+110h] [rbp+8h]

  *(_OWORD *)&v32[1] = 0LL;
  v2 = MEMORY[0xFFFFF78000000014];
  v3 = *(_DWORD **)(a1 + 24);
  v4 = *(_DWORD *)(a1 + 104);
  v30 = v4;
  if ( *v3 == 1314275652 )
  {
    v5 = (__int64)v3 + 274;
  }
  else if ( *v3 == 1094997074 )
  {
    v5 = (__int64)v3 + 482;
  }
  else
  {
    v5 = 98LL;
  }
  v6 = sub_140015A14(*(_QWORD *)(a1 + 8), 0x20u, *(_BYTE *)v5, 0);
  v8 = v6;
  if ( v6 )
  {
    if ( *(_BYTE *)v5 == 1 )
    {
      v10 = *(unsigned int *)(v6 + 52);
      *(_DWORD *)(v6 + 20) = 32;
      *(_BYTE *)(v10 + v6 + 8) = *(_BYTE *)(a1 + 104);
      *(_BYTE *)(v10 + v6 + 9) = *(_BYTE *)(a1 + 105);
      *(_BYTE *)(v10 + v6 + 10) = *(_BYTE *)(a1 + 106);
      *(_DWORD *)(v6 + 24) = 524560;
      v7 = *(_QWORD *)(a1 + 24);
      if ( *(_DWORD *)v7 == 1094997074 && (*(_BYTE *)(v7 + 111) & 4) != 0 )
      {
        LockArray_high = HIDWORD(KeGetPcr()[1].LockArray);
        _InterlockedIncrement(*(volatile signed __int32 **)(*(_QWORD *)(v7 + 6168) + 8LL * LockArray_high));
        v12 = **(_QWORD **)(*(_QWORD *)(v7 + 6168) + 8LL * LockArray_high);
        *(_DWORD *)(v8 + 44) = HIDWORD(v12);
      }
      else
      {
        LODWORD(v12) = -1;
      }
      *(_DWORD *)(v8 + 32) = v12;
      *(_QWORD *)(v8 + 64) = 0LL;
      *(_DWORD *)(v8 + 60) = 0;
      *(_QWORD *)(v8 + 80) = 0LL;
      if ( *(_BYTE *)(a1 + 3368) )
      {
        v13 = *(_DWORD *)(a1 + 3388);
      }
      else
      {
        v13 = *(_DWORD *)(a1 + 1392);
        if ( v13 < 0x1E )
          v13 = 30;
      }
      *(_DWORD *)(v8 + 40) = v13;
    }
    else
    {
      *(_WORD *)v6 = 88;
      *(_BYTE *)(v6 + 2) = 32;
      *(_BYTE *)(v6 + 5) = *(_BYTE *)(a1 + 104);
      *(_BYTE *)(v6 + 6) = *(_BYTE *)(a1 + 105);
      *(_BYTE *)(v6 + 7) = *(_BYTE *)(a1 + 106);
      *(_QWORD *)(v6 + 12) = 524560LL;
      *(_BYTE *)(v6 + 8) = -1;
      *(_QWORD *)(v6 + 24) = 0LL;
      *(_QWORD *)(v6 + 48) = 0LL;
      if ( *(_BYTE *)(a1 + 3368) )
      {
        v14 = *(_DWORD *)(a1 + 3388);
      }
      else
      {
        v14 = *(_DWORD *)(a1 + 1392);
        if ( v14 < 0x1E )
          v14 = 30;
      }
      *(_DWORD *)(v8 + 20) = v14;
    }
    v15 = sub_140014B7C(*(_QWORD *)(a1 + 8), v8, v7, (struct _IO_STATUS_BLOCK *)&v32[1]);
    v16 = v15;
    if ( v15 )
    {
      Status = sub_140018E30(*(PDEVICE_OBJECT *)(a1 + 8), v15);
      if ( Status >= 0 )
        Status = v16->IoStatus.Status;
      sub_1400158A4(v16);
      v19 = *(_QWORD *)(a1 + 24);
      v20 = MEMORY[0xFFFFF78000000014] - v2;
      v21 = *(_BYTE *)(v8 + 3);
      if ( v19 )
      {
        for ( i = 0; i < 5u; ++i )
        {
          v17 = i;
          v18 = (__int64)qword_1401494F0;
          if ( v20 <= qword_1401494F0[i] )
          {
            v18 = 3LL * i;
            ++*(_DWORD *)(v19 + 12LL * i + 5092);
            if ( (v21 & 0x3F) == 1 )
              ++*(_DWORD *)(v19 + 12LL * i + 5096);
            else
              *(_BYTE *)(v19 + 12LL * i + 5100) = v21;
            break;
          }
        }
      }
      v23 = BYTE2(v30);
      v24 = BYTE1(v30);
      if ( (unsigned int)dword_140168178 > 5 )
      {
        v25 = *(_QWORD *)(a1 + 24);
        v35 = 16LL;
        v37 = 16LL;
        v34 = v25 + 5064;
        v36 = a1 + 2104;
        LODWORD(v31) = *(_DWORD *)(v25 + 56);
        v38 = &v31;
        v40 = &v27;
        v42 = &v28;
        v44 = (char *)&v28 + 1;
        v39 = 4LL;
        v27 = v4;
        v41 = 1LL;
        v28 = *(_WORD *)((char *)&v30 + 1);
        v43 = 1LL;
        v45 = 1LL;
        v29 = *(_BYTE *)(v8 + 3);
        v46 = &v29;
        v48 = v32;
        v47 = 1LL;
        v32[0] = v20;
        v49 = 8LL;
        sub_140037A5C(v25, (unsigned __int8 *)dword_1401562D8, v18, v20, 0xAu, &v33);
      }
      if ( (byte_1401694F0 & 1) != 0 )
        sub_140090464(v19, v17, v18, *(_DWORD *)(*(_QWORD *)(a1 + 24) + 56LL), v4, v24, v23);
    }
    else
    {
      Status = -1073741801;
    }
    ExFreePoolWithTag((PVOID)v8, 0x72536152u);
  }
  else
  {
    return (unsigned int)-1073741801;
  }
  return (unsigned int)Status;
}
