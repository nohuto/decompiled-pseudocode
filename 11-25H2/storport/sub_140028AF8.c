/*
 * XREFs of sub_140028AF8 @ 0x140028AF8
 * Callers:
 *     sub_1400287D0 @ 0x1400287D0 (sub_1400287D0.c)
 * Callees:
 *     sub_140028D00 @ 0x140028D00 (sub_140028D00.c)
 *     sub_140055BD4 @ 0x140055BD4 (sub_140055BD4.c)
 */

__int64 __fastcall sub_140028AF8(__int64 a1, __int64 a2)
{
  __int64 v2; // rbx
  unsigned int v4; // ebp
  KIRQL v5; // bl
  union _LARGE_INTEGER Timeout; // [rsp+40h] [rbp+8h] BYREF

  v2 = (unsigned int)a2;
  v4 = 0;
  _InterlockedIncrement((volatile signed __int32 *)(a1 + 40));
  LOBYTE(a2) = 1;
  sub_140028D00(a1, a2);
  if ( (((__int64)*(unsigned int *)(a1 + 88) >> 2) & 0x3FFFFFFF) != 0 )
  {
    if ( (_DWORD)v2 )
    {
      Timeout.QuadPart = -10000000 * v2;
      v4 = KeWaitForSingleObject((PVOID)(a1 + 48), Executive, 0, 0, &Timeout);
      if ( v4 == 258
        && off_140168120 != (PDEVICE_OBJECT)&off_140168120
        && (HIDWORD(off_140168120->Timer) & 4) != 0
        && BYTE1(off_140168120->Timer) >= 3u )
      {
        sub_140055BD4(off_140168120->AttachedDevice, 15LL, &unk_14014B9D8, (unsigned int)v2);
      }
    }
    else
    {
      v4 = KeWaitForSingleObject((PVOID)(a1 + 48), Executive, 0, 0, 0LL);
    }
  }
  if ( _InterlockedExchangeAdd((volatile signed __int32 *)(a1 + 40), 0xFFFFFFFF) == 1 && v4 != 258 )
  {
    v5 = ExAcquireSpinLockExclusive((PEX_SPIN_LOCK)(a1 + 72));
    KeClearEvent((PRKEVENT)(a1 + 48));
    ExReleaseSpinLockExclusive((PEX_SPIN_LOCK)(a1 + 72), v5);
  }
  return v4;
}
