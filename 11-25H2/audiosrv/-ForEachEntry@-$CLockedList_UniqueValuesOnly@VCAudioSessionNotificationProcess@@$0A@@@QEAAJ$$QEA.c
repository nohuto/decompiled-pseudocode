/*
 * XREFs of ?ForEachEntry@?$CLockedList_UniqueValuesOnly@VCAudioSessionNotificationProcess@@$0A@@@QEAAJ$$QEAVCListWorker@1@@Z @ 0x18002B558
 * Callers:
  ...
  try
  {
    ...
    if ( v6 )
    {
      std::_Destroy_range<std::allocator<CLockedList_UniqueValuesOnly<CAudioSessionNotificationProcess,0>::LockedListEntry>>(
        v6,
        *((_QWORD *)&v11 + 1));
      std::_Deallocate<16,0>(v6, (const struct std::nothrow_t *)((v4 - (_QWORD)v6) & 0xFFFFFFFFFFFFFFF0uLL));
    }
    result = 0LL;
  }
  catch ( ... )
