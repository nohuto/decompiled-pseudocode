/*
 * XREFs of ?GetMonitorDpiInfo@@YAXPEAUHDEV__@@HPEAUtagRECT@@PEAU_DPI_INFORMATION@@@Z @ 0x1400CEA1C
 * Callers:
 *     ?SetMonitorData@@YAPEAUtagMONITOR@@PEAU1@K@Z @ 0x1400CDC30 (-SetMonitorData@@YAPEAUtagMONITOR@@PEAU1@K@Z.c)
 * Callees:
 *     DrvGetCurrentDpiInfoFromHDev @ 0x14008A304 (DrvGetCurrentDpiInfoFromHDev.c)
 *     memset @ 0x140243000 (memset.c)
 */

void __fastcall GetMonitorDpiInfo(__int64 a1, char a2, struct tagRECT *a3, struct _DPI_INFORMATION *a4)
{
  int v8; // ecx
  __int128 v9; // xmm1
  __int128 v10; // xmm0
  __int128 v11; // xmm1
  __int128 v12; // xmm0
  __int128 v13; // xmm1
  _OWORD v14[6]; // [rsp+20h] [rbp-60h] BYREF

  memset(v14, 0, sizeof(v14));
  if ( (int)DrvGetCurrentDpiInfoFromHDev(a1, (__int64)v14) < 0 )
  {
    LODWORD(v14[2]) = a3->right - a3->left;
    v8 = a3->bottom - a3->top;
    *(_QWORD *)&v14[0] = 0x6400000064LL;
    *((_QWORD *)&v14[0] + 1) = 0x6400000064LL;
    DWORD1(v14[2]) = v8;
    HIDWORD(v14[5]) = HIDWORD(v14[5]) & 0xFFFFFFDF | (32 * (a2 & 1));
    DWORD2(v14[5]) = 1;
  }
  v9 = v14[1];
  *(_OWORD *)a4 = v14[0];
  v10 = v14[2];
  *((_OWORD *)a4 + 1) = v9;
  v11 = v14[3];
  *((_OWORD *)a4 + 2) = v10;
  v12 = v14[4];
  *((_OWORD *)a4 + 3) = v11;
  v13 = v14[5];
  *((_OWORD *)a4 + 4) = v12;
  *((_OWORD *)a4 + 5) = v13;
}
