/*
 * XREFs of ?GenerateFrameMessages@Pointer@TelemetryDebug@InputTraceLogging@@SAXAEBUTELEMETRY_POINTER_FRAME_TIMES@@_N@Z @ 0x140061230
 * Callers:
 *     ?GenerateMessagesCore@CTouchProcessor@@QEAAHHHHPEBUCPointerInputFrame@@@Z @ 0x140200550 (-GenerateMessagesCore@CTouchProcessor@@QEAAHHHHPEBUCPointerInputFrame@@@Z.c)
 * Callees:
 *     ??$Write@U?$_tlgWrapperByVal@$07@@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U?$_tlgWrapperByVal@$00@@@?$_tlgWriteTemplate@$$A6AJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2IPEAU_EVENT_DATA_DESCRIPTOR@@@Z$1?_tlgWriteTransfer_EtwWriteTransfer@@YAJ0122I3@ZPEBU2@PEBU2@@@SAJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2AEBU?$_tlgWrapperByVal@$07@@33333333333333333AEBU?$_tlgWrapperByVal@$00@@@Z @ 0x140006E20 (--$Write@U-$_tlgWrapperByVal@$07@@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U-$_tlgWrap.c)
 *     ??$Write@U?$_tlgWrapperByVal@$07@@U1@U1@U1@U1@U1@U1@U1@U?$_tlgWrapperByVal@$00@@@?$_tlgWriteTemplate@$$A6AJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2IPEAU_EVENT_DATA_DESCRIPTOR@@@Z$1?_tlgWriteTransfer_EtwWriteTransfer@@YAJ0122I3@ZPEBU2@PEBU2@@@SAJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2AEBU?$_tlgWrapperByVal@$07@@3333333AEBU?$_tlgWrapperByVal@$00@@@Z @ 0x140006FF0 (--$Write@U-$_tlgWrapperByVal@$07@@U1@U1@U1@U1@U1@U1@U1@U-$_tlgWrapperByVal@$00@@@-$_tlgWriteTemp.c)
 *     isChildPartition @ 0x140060A90 (isChildPartition.c)
 *     _tlgKeywordOn @ 0x14009E9B0 (_tlgKeywordOn.c)
 */

void __fastcall InputTraceLogging::TelemetryDebug::Pointer::GenerateFrameMessages(
        const struct TELEMETRY_POINTER_FRAME_TIMES *a1,
        __int64 a2)
{
  char v2; // di
  __int64 v4; // r8
  __int64 v5; // r8
  __int64 v6; // r9
  __int64 v7; // r8
  __int64 v8; // r9
  __int64 v9; // [rsp+C0h] [rbp-80h] BYREF
  __int64 v10; // [rsp+C8h] [rbp-78h] BYREF
  __int64 v11; // [rsp+D0h] [rbp-70h] BYREF
  __int64 v12; // [rsp+D8h] [rbp-68h] BYREF
  __int64 v13; // [rsp+E0h] [rbp-60h] BYREF
  __int64 v14; // [rsp+E8h] [rbp-58h] BYREF
  __int64 v15; // [rsp+F0h] [rbp-50h] BYREF
  __int64 v16; // [rsp+F8h] [rbp-48h] BYREF
  __int64 v17; // [rsp+100h] [rbp-40h] BYREF
  __int64 v18; // [rsp+108h] [rbp-38h] BYREF
  __int64 v19; // [rsp+110h] [rbp-30h] BYREF
  __int64 v20; // [rsp+118h] [rbp-28h] BYREF
  __int64 v21; // [rsp+120h] [rbp-20h] BYREF
  __int64 v22; // [rsp+128h] [rbp-18h] BYREF
  __int64 v23; // [rsp+130h] [rbp-10h] BYREF
  __int64 v24; // [rsp+138h] [rbp-8h] BYREF
  __int64 v25; // [rsp+140h] [rbp+0h] BYREF
  char v26; // [rsp+170h] [rbp+30h] BYREF
  __int64 v27; // [rsp+178h] [rbp+38h] BYREF

  v2 = a2;
  if ( isChildPartition((__int64)a1, a2) )
  {
    if ( (unsigned int)dword_14029EE20 > 7 && (unsigned __int8)tlgKeywordOn(&dword_14029EE20, 0x10000LL, v4) )
    {
      v27 = *((_QWORD *)a1 + 17);
      v15 = *((_QWORD *)a1 + 16);
      v14 = *((_QWORD *)a1 + 15);
      v13 = *((_QWORD *)a1 + 14);
      v12 = *((_QWORD *)a1 + 13);
      v11 = *((_QWORD *)a1 + 12);
      v10 = *((_QWORD *)a1 + 11);
      v9 = *((_QWORD *)a1 + 10);
      v16 = *((_QWORD *)a1 + 9);
      v17 = *((_QWORD *)a1 + 8);
      v18 = *((_QWORD *)a1 + 7);
      v19 = *((_QWORD *)a1 + 6);
      v20 = *((_QWORD *)a1 + 5);
      v21 = *((_QWORD *)a1 + 4);
      v22 = *((_QWORD *)a1 + 3);
      v23 = *((_QWORD *)a1 + 2);
      v24 = *((_QWORD *)a1 + 1);
      v25 = *(_QWORD *)a1;
      v26 = v2;
      _tlgWriteTemplate<long (_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),&long _tlgWriteTransfer_EtwWriteTransfer(_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),_GUID const *,_GUID const *>::Write<_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<1>>(
        (__int64)&dword_14029EE20,
        (__int64)&unk_14027FAD2,
        v5,
        v6,
        (__int64)&v25,
        (__int64)&v24,
        (__int64)&v23,
        (__int64)&v22,
        (__int64)&v21,
        (__int64)&v20,
        (__int64)&v19,
        (__int64)&v18,
        (__int64)&v17,
        (__int64)&v16,
        (__int64)&v9,
        (__int64)&v10,
        (__int64)&v11,
        (__int64)&v12,
        (__int64)&v13,
        (__int64)&v14,
        (__int64)&v15,
        (__int64)&v27,
        (__int64)&v26);
    }
  }
  else if ( (unsigned int)dword_14029EE20 > 7 )
  {
    if ( (unsigned __int8)tlgKeywordOn(&dword_14029EE20, 0x10000LL, v4) )
    {
      v27 = *((_QWORD *)a1 + 7);
      v9 = *((_QWORD *)a1 + 6);
      v10 = *((_QWORD *)a1 + 5);
      v11 = *((_QWORD *)a1 + 4);
      v12 = *((_QWORD *)a1 + 3);
      v13 = *((_QWORD *)a1 + 2);
      v14 = *((_QWORD *)a1 + 1);
      v15 = *(_QWORD *)a1;
      v26 = v2;
      _tlgWriteTemplate<long (_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),&long _tlgWriteTransfer_EtwWriteTransfer(_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),_GUID const *,_GUID const *>::Write<_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapperByVal<1>>(
        (__int64)&dword_14029EE20,
        (__int64)&unk_14027F9D5,
        v7,
        v8,
        (__int64)&v15,
        (__int64)&v14,
        (__int64)&v13,
        (__int64)&v12,
        (__int64)&v11,
        (__int64)&v10,
        (__int64)&v9,
        (__int64)&v27,
        (__int64)&v26);
    }
  }
}
