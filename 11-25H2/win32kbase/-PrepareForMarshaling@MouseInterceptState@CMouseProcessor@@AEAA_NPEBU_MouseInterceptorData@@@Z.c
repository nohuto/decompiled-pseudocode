/*
 * XREFs of ?PrepareForMarshaling@MouseInterceptState@CMouseProcessor@@AEAA_NPEBU_MouseInterceptorData@@@Z @ 0x140221D8C
 * Callers:
        ...
        const struct _MouseInterceptorData *a2)
{
  ...
  if ( *((_DWORD *)this + 7) != (unsigned int)PsGetCurrentThreadId() )
    MicrosoftTelemetryAssertTriggeredArgsKM("IXPTelAssert", 0x20000, 8669);
