/*
 * XREFs of ?TranslateMilCommandBatchHandle@CMilCommandBatchParser@CApplicationChannel@DirectComposition@@AEAAXPEAIW4MIL_RESOURCE_TYPE@@_NPEAJ@Z @ 0x14002194C
 * Callers:
        ...
        _DWORD *a5)
{
  ...
  if ( !a4 || *a2 )
  {
    ...
    if ( v7 )
    {
      for ( i = *((unsigned int *)v7 + 9); (unsigned int)i < 0xC0; LODWORD(i) = dword_140274520[i] )
      {
        if ( (_DWORD)i == a3 )
        {
          *a2 = *((_DWORD *)v7 + 8);
