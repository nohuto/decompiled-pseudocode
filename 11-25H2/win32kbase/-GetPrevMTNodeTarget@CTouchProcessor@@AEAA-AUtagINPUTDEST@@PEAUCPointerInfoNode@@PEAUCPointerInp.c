/*
 * XREFs of ?GetPrevMTNodeTarget@CTouchProcessor@@AEAA?AUtagINPUTDEST@@PEAUCPointerInfoNode@@PEAUCPointerInputFrame@@@Z @ 0x140204180
 * Callers:
        ...
        const struct CPointerInputFrame *a4)
{
  ...
  if ( v8 )
  {
    ValidNodeInFrame = CTouchProcessor::FindValidNodeInFrame((PERESOURCE *)a1, v8, *(_WORD *)(a3 + 172), 0LL);
    v11 = ValidNodeInFrame;
    if ( ValidNodeInFrame )
    {
      if ( *((_WORD *)ValidNodeInFrame + 86) != *(_WORD *)(a3 + 172) )
        MicrosoftTelemetryAssertTriggeredArgsKM("IXPTelAssert", 0x20000, 10810);
