/*
 * XREFs of _SetWatermarkStrings @ 0x1C0099B60
 * Callers:
 *     NtUserSetWatermarkStrings @ 0x1C0099B20 (NtUserSetWatermarkStrings.c)
 * Callees:
...
LABEL_16:
    ...
    if ( (v6 & 1) != 0 )
      goto LABEL_16;
    *((_QWORD *)&WPP_MAIN_CB.Queue.Wcb.DeviceObject + 2 * v2) = (char *)&gWatermarkStrings + 256 * (__int64)v2;
    v9 = (struct _UNICODE_STRING *)(&WPP_MAIN_CB.Queue.Wcb.NumberOfMapRegisters + 4 * v2);
    v9->Length = 0;
    v9->MaximumLength = 256;
    RtlCopyUnicodeString(v9, &SourceString);
    Length = v9->Length;
    if ( Length >= (unsigned __int64)*((unsigned __int16 *)&WPP_MAIN_CB.Queue.Wcb.NumberOfMapRegisters + 4 * v4 + 1) - 2 )
      Length = *((unsigned __int16 *)&WPP_MAIN_CB.Queue.Wcb.NumberOfMapRegisters + 4 * v4 + 1) - 2LL;
    *(_WORD *)(*((_QWORD *)&WPP_MAIN_CB.Queue.Wcb.DeviceObject + v4) + 2 * (Length >> 1)) = 0;
    v2 = v13 + 1;
  }
