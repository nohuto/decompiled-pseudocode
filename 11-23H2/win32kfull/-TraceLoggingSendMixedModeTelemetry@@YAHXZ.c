/*
 * XREFs of ?TraceLoggingSendMixedModeTelemetry@@YAHXZ @ 0x1C00124AC
 * Callers:
 *     NtUserTraceLoggingSendMixedModeTelemetry @ 0x1C0012470 (NtUserTraceLoggingSendMixedModeTelemetry.c)
 * Callees:
 *     _tlgWriteTransfer_EtwWriteTransfer @ 0x1C00D82E4 (_tlgWriteTransfer_EtwWriteTransfer.c)
 *     _tlgKeywordOn @ 0x1C00D9324 (_tlgKeywordOn.c)
 *     __security_check_cookie @ 0x1C01381F0 (__security_check_cookie.c)
 */

__int64 TraceLoggingSendMixedModeTelemetry(void)
{
  __int64 v1; // [rsp+30h] [rbp-48h] BYREF
  struct _EVENT_DATA_DESCRIPTOR v2; // [rsp+38h] [rbp-40h] BYREF
  __int64 *v3; // [rsp+58h] [rbp-20h]
  int v4; // [rsp+60h] [rbp-18h]
  int v5; // [rsp+64h] [rbp-14h]

  if ( (unsigned int)dword_1C0354098 > 5 && (unsigned __int8)tlgKeywordOn(&dword_1C0354098, 0x200000000000LL) )
  {
    v5 = 0;
    v3 = &v1;
    v1 = 0x1000000LL;
    v4 = 8;
    tlgWriteTransfer_EtwWriteTransfer((int)&dword_1C0354098, (int)&dword_1C031C578, 0, 0, 3u, &v2);
  }
  return 1LL;
}
