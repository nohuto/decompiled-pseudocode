/*
 * XREFs of ?BeginApiCallback@ContextSession@WRL2@Microsoft@@AEAAXXZ @ 0x18002CA28
 * Callers:
...
void __fastcall Microsoft::WRL2::ContextSession::BeginApiCallback(struct _RTL_CRITICAL_SECTION *this)
{
  int LockCount; // ecx
  struct _RTL_CRITICAL_SECTION *v3; // r8

  LockCount = this[2].LockCount;
  v3 = this + 2;
  if ( (LockCount > 0 || this[2].RecursionCount != LODWORD(v3->DebugInfo)) && !BYTE1(this[2].OwningThread) )
    Microsoft::WRL2::FailFast::Unexpected("Begin callback preconditions");
  if ( LODWORD(v3->DebugInfo) != ++HIDWORD(this[2].DebugInfo) + LockCount )
