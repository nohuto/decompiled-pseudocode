/*
 * XREFs of ?CreateWARPDevice@SharedD3DDevice@Internal@Composition@UI@Windows@@UEAAJPEAPEAUID3D11Device@@@Z @ 0x1800A28B0
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall Windows::UI::Composition::Internal::SharedD3DDevice::CreateWARPDevice(
        struct IUnknown **this,
        struct ID3D11Device **a2)
{
  int WarpAdapterLUID; // eax
  unsigned int v5; // ebx
  Windows::UI::Composition::Internal::SharedD3DDevicePool *v7; // rax
  wil::details::in1diag3 *retaddr; // [rsp+28h] [rbp+0h]
  struct _LUID v9; // [rsp+40h] [rbp+18h] BYREF

  WarpAdapterLUID = GetWarpAdapterLUID(&v9);
  v5 = WarpAdapterLUID;
  if ( WarpAdapterLUID >= 0 )
  {
  ...
  else
  {
    wil::details::in1diag3::Return_Hr(
      ...
      (const char *)(unsigned int)WarpAdapterLUID);
    return v5;
  }
}
