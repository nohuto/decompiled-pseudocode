/*
 * XREFs of ?ParseExclusiveEndpoints@CConstraintModelXMLParser@@AEAAJPEAUIXmlReader@@PEAV?$CAtlArray@PEAVExclusiveEndpointInfo@@V?$CElementTraits@PEAVExclusiveEndpointInfo@@@ATL@@@ATL@@@Z @ 0x18016179C
 * Callers:
        ...
        _QWORD *a3)
{
  ...
  if ( v3 < 2 )
    LogError(L"ExclusiveEndpoints list must have more than one endpoint (only found %d)", v3);
  return (unsigned int)StartOrEndElement;
}
