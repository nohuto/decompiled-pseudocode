/*
 * XREFs of SmpPathCanBeTrustedIsNotARedirection @ 0x140011C6C
 * Callers:
...
bool __fastcall SmpPathCanBeTrustedIsNotARedirection(HANDLE FileHandle, unsigned __int16 *a2, int a3)
{
  ...
  if ( *a2 >= 0xCu )
  {
    FinalPathNameByHandleW = GetFinalPathNameByHandleW(FileHandle, &pszDest);
    ...
    if ( FinalPathNameByHandleW )
    {
      ...
      if ( v10 > 0x106 )
      {
        ...
        if ( !Heap )
          return v7;
        v9 = GetFinalPathNameByHandleW(FileHandle, Heap);
        if ( !v9 )
        {
LABEL_15:
