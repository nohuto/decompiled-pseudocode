/*
 * XREFs of sub_1800DDDB8 @ 0x1800DDDB8
 * Callers:
 *     sub_1800DD4D4 @ 0x1800DD4D4 (sub_1800DD4D4.c)
 *     sub_1800DDE34 @ 0x1800DDE34 (sub_1800DDE34.c)
 * Callees:
 *     sub_180012440 @ 0x180012440 (sub_180012440.c)
 *     sub_1800DC03C @ 0x1800DC03C (sub_1800DC03C.c)
 */

// Hidden C++ exception states: #wind=2
__int64 __fastcall sub_1800DDDB8(__int64 a1, __int64 a2, __int64 a3)
{
  sub_180012440((_QWORD *)a2, a3);
  *(_DWORD *)(a2 + 32) = *(_DWORD *)(a3 + 32);
  sub_180012440((_QWORD *)(a2 + 40), a3 + 40);
  *(_DWORD *)(a2 + 72) = *(_DWORD *)(a3 + 72);
  *(_QWORD *)(a2 + 80) = *(_QWORD *)(a3 + 80);
  *(_DWORD *)(a2 + 88) = *(_DWORD *)(a3 + 88);
  *(_BYTE *)(a2 + 92) = *(_BYTE *)(a3 + 92);
  return sub_1800DC03C(a2 + 96, a3 + 96);
}
