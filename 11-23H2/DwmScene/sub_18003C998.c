/*
 * XREFs of sub_18003C998 @ 0x18003C998
 * Callers:
 *     sub_180012F60 @ 0x180012F60 (sub_180012F60.c)
 *     sub_180065CC0 @ 0x180065CC0 (sub_180065CC0.c)
 *     sub_180093580 @ 0x180093580 (sub_180093580.c)
 *     sub_180096A80 @ 0x180096A80 (sub_180096A80.c)
 *     sub_180096C00 @ 0x180096C00 (sub_180096C00.c)
 * Callees:
 *     _o_acosf @ 0x18000BFE4 (_o_acosf.c)
 *     sub_18001112C @ 0x18001112C (sub_18001112C.c)
 *     sub_1800189B4 @ 0x1800189B4 (sub_1800189B4.c)
 *     sub_18003BDE0 @ 0x18003BDE0 (sub_18003BDE0.c)
 *     sub_18003C3B4 @ 0x18003C3B4 (sub_18003C3B4.c)
 *     sub_18003CFA0 @ 0x18003CFA0 (sub_18003CFA0.c)
 *     sub_18003DC28 @ 0x18003DC28 (sub_18003DC28.c)
 *     sub_18003E370 @ 0x18003E370 (sub_18003E370.c)
 *     sub_18003E8A4 @ 0x18003E8A4 (sub_18003E8A4.c)
 *     sub_18003EB44 @ 0x18003EB44 (sub_18003EB44.c)
 */

__int64 __fastcall sub_18003C998(__int64 a1, unsigned __int64 *a2, __int64 a3)
{
  __m128 v6; // xmm0
  __m128 v7; // xmm3
  __m128 v8; // xmm8
  __m128 v9; // xmm8
  __m128 v10; // xmm6
  __m128 v11; // xmm4
  __m128 v12; // xmm3
  __m128 v13; // xmm3
  __m128 v14; // xmm1
  unsigned __int32 v15; // eax
  __m128 v16; // xmm7
  __m128 v17; // xmm7
  __m128 v18; // xmm2
  __m128 v19; // xmm4
  __m128 v20; // xmm0
  __m128 v21; // xmm3
  __m128 v22; // xmm1
  __m128 v23; // xmm3
  __m128 v24; // xmm2
  __m128 v25; // xmm3
  __m128 v26; // xmm2
  __m128 v27; // xmm2
  __m128 v28; // xmm1
  __m128 v29; // xmm2
  __int128 v30; // xmm0
  __m128 v31; // xmm3
  __m128 v32; // xmm6
  __m128 v33; // xmm2
  __m128 v34; // xmm3
  __m128 v35; // xmm7
  __m128 v36; // xmm2
  __int128 v37; // xmm0
  __int128 v39; // [rsp+28h] [rbp-89h] BYREF
  unsigned __int64 v40; // [rsp+38h] [rbp-79h] BYREF
  unsigned __int32 v41; // [rsp+40h] [rbp-71h]
  _BYTE v42[64]; // [rsp+48h] [rbp-69h] BYREF
  __int128 v43; // [rsp+88h] [rbp-29h] BYREF

  sub_18003E370();
  sub_18003C3B4(a1, (__int64)&v39);
  v6 = (__m128)*((unsigned int *)a2 + 2);
  v7 = 0LL;
  v8 = (__m128)*a2;
  v40 = 0LL;
  v9 = _mm_movelh_ps(v8, v6);
  v10 = _mm_movelh_ps((__m128)(unsigned __int64)v39, (__m128)DWORD2(v39));
  v11 = _mm_sub_ps(v9, v10);
  v7.m128_u64[0] = v11.m128_u64[0];
  v41 = _mm_shuffle_ps(v11, v11, 170).m128_u32[0];
  v12 = _mm_movelh_ps(v7, (__m128)v41);
  v13 = _mm_mul_ps(v12, v12);
  v14 = _mm_shuffle_ps(v13, v13, 102);
  v13.m128_f32[0] = (float)(v13.m128_f32[0] + v14.m128_f32[0]) + _mm_shuffle_ps(v14, v14, 85).m128_f32[0];
  if ( _mm_sqrt_ps(_mm_shuffle_ps(v13, v13, 0)).m128_f32[0] <= 0.00000059604645 )
  {
    v37 = xmmword_180106A28;
    goto LABEL_6;
  }
  DWORD2(v39) = v41;
  *(_QWORD *)&v39 = v11.m128_u64[0];
  sub_18003CFA0(&v39);
  v15 = *(_DWORD *)(a3 + 8);
  v40 = *(_QWORD *)a3;
  v41 = v15;
  sub_18003CFA0(&v40);
  v16 = (__m128)(unsigned __int64)v39;
  *(_QWORD *)&v39 = 0LL;
  v17 = _mm_movelh_ps(v16, (__m128)DWORD2(v39));
  v18 = _mm_shuffle_ps(v17, v17, 201);
  v19 = _mm_movelh_ps((__m128)v40, (__m128)v41);
  v20 = _mm_shuffle_ps(v18, v18, 201);
  v21 = _mm_shuffle_ps(v19, v19, 210);
  v22 = _mm_shuffle_ps(v21, v21, 210);
  v23 = _mm_mul_ps(v21, v18);
  v24 = 0LL;
  v25 = _mm_and_ps(_mm_sub_ps(v23, _mm_mul_ps(v22, v20)), (__m128)xmmword_180106400);
  v24.m128_u64[0] = v25.m128_u64[0];
  DWORD2(v39) = _mm_shuffle_ps(v25, v25, 170).m128_u32[0];
  v26 = _mm_movelh_ps(v24, (__m128)DWORD2(v39));
  v27 = _mm_mul_ps(v26, v26);
  v28 = _mm_shuffle_ps(v27, v27, 102);
  v27.m128_f32[0] = (float)(v27.m128_f32[0] + v28.m128_f32[0]) + _mm_shuffle_ps(v28, v28, 85).m128_f32[0];
  if ( _mm_sqrt_ps(_mm_shuffle_ps(v27, v27, 0)).m128_f32[0] <= 0.00000059604645 )
  {
    v31 = _mm_shuffle_ps(v17, v17, 210);
    DWORD2(v39) = -1082130432;
    v32 = _mm_movelh_ps((__m128)0LL, (__m128)0xBF800000);
    v33 = _mm_shuffle_ps(v32, v32, 201);
    v34 = _mm_and_ps(
            _mm_sub_ps(_mm_mul_ps(v31, v33), _mm_mul_ps(_mm_shuffle_ps(v31, v31, 210), _mm_shuffle_ps(v33, v33, 201))),
            (__m128)xmmword_180106400);
    v40 = v34.m128_u64[0];
    v41 = _mm_shuffle_ps(v34, v34, 170).m128_u32[0];
    sub_18003CFA0(&v40);
    v35 = _mm_mul_ps(v17, v32);
    v36 = _mm_shuffle_ps(v35, v35, 153);
    sub_18001112C((float)(v35.m128_f32[0] + v36.m128_f32[0]) + _mm_shuffle_ps(v36, v36, 85).m128_f32[0], -1.0, 1.0);
    o_acosf();
    sub_18003BDE0((__int64)&v39, &v40);
    v37 = v39;
LABEL_6:
    v39 = v37;
    return sub_18003DC28(a1, &v39);
  }
  *(_OWORD *)&v42[4] = 0LL;
  *(_OWORD *)&v42[44] = 0LL;
  v29 = v19;
  *((_QWORD *)&v30 + 1) = v10.m128_u64[1];
  *(_OWORD *)&v42[24] = 0LL;
  *(double *)&v30 = sub_18003E8A4();
  *(_OWORD *)v42 = v30;
  *(__m128 *)&v42[16] = _mm_sub_ps(v10, v9);
  *(__m128 *)&v42[32] = v29;
  *(_OWORD *)&v42[48] = 0LL;
  sub_1800189B4((__int128 *)v42, (__int64)&v43);
  *((_QWORD *)&v30 + 1) = *((_QWORD *)&v43 + 1);
  *(double *)&v30 = sub_18003EB44();
  v39 = v30;
  return sub_18003DC28(a1, &v39);
}
