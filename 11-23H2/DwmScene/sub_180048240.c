/*
 * XREFs of sub_180048240 @ 0x180048240
 * Callers:
 *     sub_180048440 @ 0x180048440 (sub_180048440.c)
 * Callees:
 *     sub_180010530 @ 0x180010530 (sub_180010530.c)
 *     sub_180047930 @ 0x180047930 (sub_180047930.c)
 *     sub_180048974 @ 0x180048974 (sub_180048974.c)
 *     sub_180048980 @ 0x180048980 (sub_180048980.c)
 *     sub_180048990 @ 0x180048990 (sub_180048990.c)
 *     sub_180048A00 @ 0x180048A00 (sub_180048A00.c)
 *     sub_180048A38 @ 0x180048A38 (sub_180048A38.c)
 *     sub_180048A48 @ 0x180048A48 (sub_180048A48.c)
 *     sub_180048A80 @ 0x180048A80 (sub_180048A80.c)
 *     sub_1800E2990 @ 0x1800E2990 (sub_1800E2990.c)
 */

// Hidden C++ exception states: #wind=9
__int64 __fastcall sub_180048240(__int64 a1, __int64 a2, char a3)
{
  __int64 result; // rax
  char v5; // bl
  BOOL v6; // r15d
  __int64 v7; // rcx
  char v8; // r12
  __int64 *v9; // rax
  __int64 v10; // rdi
  __int64 *v11; // rax
  __int64 *v12; // rax
  __int64 v13; // r13
  int v14; // esi
  int v15; // edi
  __int64 v16; // rcx
  int v17; // ebx
  int v18; // eax
  __int64 v19; // [rsp+60h] [rbp-A0h] BYREF
  __int64 v20; // [rsp+68h] [rbp-98h]
  __int64 v21; // [rsp+70h] [rbp-90h] BYREF
  __int64 v22; // [rsp+78h] [rbp-88h]
  __int128 v23; // [rsp+80h] [rbp-80h] BYREF
  _BYTE v24[8]; // [rsp+90h] [rbp-70h] BYREF
  __int64 v25; // [rsp+98h] [rbp-68h]
  _BYTE v26[16]; // [rsp+A0h] [rbp-60h] BYREF
  _QWORD v27[7]; // [rsp+B0h] [rbp-50h] BYREF
  _QWORD *v28; // [rsp+E8h] [rbp-18h]
  __int128 v29; // [rsp+F0h] [rbp-10h]

  result = *(unsigned int *)(a1 + 300);
  v5 = 3;
  if ( !(_DWORD)result || (_DWORD)result == 3 )
  {
    v6 = sub_180048A38() == 3;
    if ( (unsigned int)sub_180048980() == 1 )
    {
      v8 = 1;
      v9 = (__int64 *)sub_180048990(v7, v24, 0LL);
      v10 = *v9;
      v21 = *v9;
      v22 = v9[1];
      *v9 = 0LL;
      v9[1] = 0LL;
      v11 = &v21;
    }
    else
    {
      v8 = 0;
      v12 = (__int64 *)sub_180048990(v7, &v23, 0LL);
      v10 = *v12;
      v20 = v12[1];
      *v12 = 0LL;
      v12[1] = 0LL;
      v11 = &v19;
      v5 = 12;
    }
    v13 = v11[1];
    *v11 = 0LL;
    v11[1] = 0LL;
    if ( (v5 & 8) != 0 )
    {
      v5 &= ~8u;
      if ( v20 )
        sub_180010530(v20);
    }
    if ( (v5 & 4) != 0 )
    {
      v5 &= ~4u;
      if ( *((_QWORD *)&v23 + 1) )
        sub_180010530(*((__int64 *)&v23 + 1));
    }
    if ( (v5 & 2) != 0 )
    {
      v5 &= ~2u;
      if ( v22 )
        sub_180010530(v22);
    }
    if ( (v5 & 1) != 0 && v25 )
      sub_180010530(v25);
    v23 = 0LL;
    v28 = 0LL;
    if ( sub_180047930() )
    {
      v27[0] = &std::_Func_impl_no_alloc<_lambda_0c82bcc1497f3afd643663d7c3bef72a_,void,std::shared_ptr<Spectre::Utils::Math::Vector4> const &,std::shared_ptr<Spectre::Utils::Math::Vector3> const &,void *>::`vftable';
      v27[1] = a1;
      v28 = v27;
    }
    v14 = sub_180048974(a1);
    v19 = v10;
    v20 = v13;
    v29 = 0LL;
    v15 = sub_180048A80();
    v17 = sub_180048A48(v16, v24, 0LL);
    v18 = sub_180048A00(a1, v26, 0LL);
    result = sub_1800E2990(v18, v17, v15, (unsigned int)&v19, v14, v8, v6, a3, (__int64)v27);
    if ( *((_QWORD *)&v23 + 1) )
      return sub_180010530(*((__int64 *)&v23 + 1));
  }
  return result;
}
