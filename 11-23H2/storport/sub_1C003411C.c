/*
 * XREFs of sub_1C003411C @ 0x1C003411C
 * Callers:
 *     sub_1C0007400 @ 0x1C0007400 (sub_1C0007400.c)
 * Callees:
 *     sub_1C0003440 @ 0x1C0003440 (sub_1C0003440.c)
 *     sub_1C001F308 @ 0x1C001F308 (sub_1C001F308.c)
 *     sub_1C00573B0 @ 0x1C00573B0 (sub_1C00573B0.c)
 */

__int64 __fastcall sub_1C003411C(_QWORD *a1, IRP *a2)
{
  PVOID v2; // rbx
  unsigned int v5; // edi
  struct _IRP *MasterIrp; // rcx
  __int16 MdlAddress; // ax
  int v8; // eax
  ULONG v10; // [rsp+50h] [rbp+8h] BYREF
  PVOID P; // [rsp+58h] [rbp+10h] BYREF

  v2 = 0LL;
  v10 = 0;
  P = 0LL;
  if ( a1[71] && a1[72] )
  {
    if ( a2->Tail.Overlay.CurrentStackLocation->Parameters.Create.Options < 0x10 )
    {
      v5 = -1073741820;
      goto LABEL_14;
    }
    MasterIrp = a2->AssociatedIrp.MasterIrp;
    MdlAddress = (__int16)MasterIrp->MdlAddress;
    if ( (MdlAddress & 1) == 0 )
      LOWORD(MasterIrp->MdlAddress) = MdlAddress | 1;
    v8 = sub_1C00573B0(a1[1], a2, &P, &v10);
    v2 = P;
    if ( v8 >= 0 && P )
      v5 = sub_1C001F308((__int64)a1, a2, (unsigned int *)P, v10, 0LL, 0, 128);
    else
      v5 = -1073741801;
  }
  else
  {
    v5 = -1073741823;
  }
  if ( v2 )
    ExFreePoolWithTag(v2, 0x72536152u);
LABEL_14:
  a2->IoStatus.Information = 0LL;
  return sub_1C0003440(a2, 0, v5);
}
