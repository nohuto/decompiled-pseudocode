/*
 * XREFs of ?GetFinalMinRect@CTopLevelWindow3D@@SAXPEAUtagRECT@@MPEAUMilPointAndSizeF@@@Z @ 0x180008D40
 * Callers:
...
void __fastcall CTopLevelWindow3D::GetFinalMinRect(struct tagRECT *a1, float a2, struct MilPointAndSizeF *a3)
{
  ...
  if ( a1->right - a1->left >= 0 )
    v3 = a1->right - a1->left;
  v5 = (float)v3 * *((float *)CDesktopManager::s_pDesktopManagerInstance + 169);
  *((float *)a3 + 2) = v5;
  *((float *)a3 + 3) = v5 * a2;
  *(float *)a3 = (float)(v5 * *((float *)v4 + 170)) + (float)a1->left;
