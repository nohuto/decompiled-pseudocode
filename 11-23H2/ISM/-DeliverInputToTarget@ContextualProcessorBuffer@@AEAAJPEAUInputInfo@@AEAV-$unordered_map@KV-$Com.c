/*
 * XREFs of ?DeliverInputToTarget@ContextualProcessorBuffer@@AEAAJPEAUInputInfo@@AEAV?$unordered_map@KV?$ComPtr@UIInputTarget@@@WRL@Microsoft@@U?$hash@K@std@@U?$equal_to@K@5@V?$allocator@U?$pair@$$CBKV?$ComPtr@UIInputTarget@@@WRL@Microsoft@@@std@@@5@@std@@1AEAV?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@4@@Z @ 0x1801C0928
 * Callers:
 *     ?DeliverInput@ContextualProcessorBuffer@@UEAAJPEAUInputInfo@@@Z @ 0x1801C03A0 (-DeliverInput@ContextualProcessorBuffer@@UEAAJPEAUInputInfo@@@Z.c)
 * Callees:
 *     ?InternalAddRef@?$ComPtr@UIAsyncActionCompletedHandler@Foundation@Windows@@@WRL@Microsoft@@IEBAXXZ @ 0x180002D48 (-InternalAddRef@-$ComPtr@UIAsyncActionCompletedHandler@Foundation@Windows@@@WRL@Microsoft@@IEBAX.c)
 *     ?InternalRelease@?$ComPtr@VHotkeyRegistrationForwarder@@@WRL@Microsoft@@IEAAKXZ @ 0x180015974 (-InternalRelease@-$ComPtr@VHotkeyRegistrationForwarder@@@WRL@Microsoft@@IEAAKXZ.c)
 *     ??$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z @ 0x18001A124 (--$_Deallocate@$0BA@$0A@@std@@YAXPEAX_K@Z.c)
 *     ?Return_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z @ 0x180031C9C (-Return_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z.c)
 *     ??4?$ComPtr@UIMessageSession@@@WRL@Microsoft@@QEAAAEAV012@AEBV012@@Z @ 0x180044CFC (--4-$ComPtr@UIMessageSession@@@WRL@Microsoft@@QEAAAEAV012@AEBV012@@Z.c)
 *     ??1?$unordered_map@KW4ContextualProcessorDecision@@U?$hash@K@std@@U?$equal_to@K@3@V?$allocator@U?$pair@$$CBKW4ContextualProcessorDecision@@@std@@@3@@std@@QEAA@XZ @ 0x1800554A8 (--1-$unordered_map@KW4ContextualProcessorDecision@@U-$hash@K@std@@U-$equal_to@K@3@V-$allocator@U.c)
 *     ??2@YAPEAX_K@Z @ 0x180056A1C (--2@YAPEAX_K@Z.c)
 *     ??_V@YAXPEAX@Z @ 0x180056D48 (--_V@YAXPEAX@Z.c)
 *     _guard_xfg_dispatch_icall_nop @ 0x180065970 (_guard_xfg_dispatch_icall_nop.c)
 *     ??$_Free_non_head@V?$allocator@U?$_List_node@U?$pair@$$CBKW4ContextualProcessorState@@@std@@PEAX@std@@@std@@@?$_List_node@U?$pair@$$CBKW4ContextualProcessorState@@@std@@PEAX@std@@SAXAEAV?$allocator@U?$_List_node@U?$pair@$$CBKW4ContextualProcessorState@@@std@@PEAX@std@@@1@PEAU01@@Z @ 0x180098338 (--$_Free_non_head@V-$allocator@U-$_List_node@U-$pair@$$CBKW4ContextualProcessorState@@@std@@PEAX.c)
 *     ??$_Free_non_head@V?$allocator@U?$_List_node@U?$pair@QEAUIInputTarget@@V?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@std@@@std@@PEAX@std@@@std@@@?$_List_node@U?$pair@QEAUIInputTarget@@V?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@std@@@std@@PEAX@std@@SAXAEAV?$allocator@U?$_List_node@U?$pair@QEAUIInputTarget@@V?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@std@@@std@@PEAX@std@@@1@PEAU01@@Z @ 0x1800A0968 (--$_Free_non_head@V-$allocator@U-$_List_node@U-$pair@QEAUIInputTarget@@V-$unordered_set@KU-$hash.c)
 *     ??1?$unordered_map@IV?$unordered_set@_KU?$hash@_K@std@@U?$equal_to@_K@2@V?$allocator@_K@2@@std@@U?$hash@I@2@U?$equal_to@I@2@V?$allocator@U?$pair@$$CBIV?$unordered_set@_KU?$hash@_K@std@@U?$equal_to@_K@2@V?$allocator@_K@2@@std@@@std@@@2@@std@@QEAA@XZ @ 0x1800A10C4 (--1-$unordered_map@IV-$unordered_set@_KU-$hash@_K@std@@U-$equal_to@_K@2@V-$allocator@_K@2@@std@@.c)
 *     ??1?$VariableSizedPayloadStorage@UInputInfo@@@@QEAA@XZ @ 0x1800BA8EC (--1-$VariableSizedPayloadStorage@UInputInfo@@@@QEAA@XZ.c)
 *     ?_Assign_grow@?$_Hash_vec@V?$allocator@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKUMessageInfo@ResizeProcessor@@@std@@@std@@@std@@@std@@@std@@@std@@QEAAX_KV?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKUMessageInfo@ResizeProcessor@@@std@@@std@@@std@@@2@@Z @ 0x1800D282C (-_Assign_grow@-$_Hash_vec@V-$allocator@V-$_List_unchecked_iterator@V-$_List_val@U-$_ea_1800D282C.c)
 *     ??0?$VariableSizedPayloadStorage@UInputInfo@@@@QEAA@PEBUInputInfo@@@Z @ 0x1800ED0C4 (--0-$VariableSizedPayloadStorage@UInputInfo@@@@QEAA@PEBUInputInfo@@@Z.c)
 *     ??$count@X@?$_Hash@V?$_Uset_traits@KV?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@K@2@$0A@@std@@@std@@QEBA_KAEBK@Z @ 0x1800F76E8 (--$count@X@-$_Hash@V-$_Uset_traits@KV-$_Uhash_compare@KU-$hash@K@std@@U-$equal_to@K@2@@std@@V-$a.c)
 *     ??0?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@std@@QEAA@XZ @ 0x1800F79D0 (--0-$unordered_set@KU-$hash@K@std@@U-$equal_to@K@2@V-$allocator@K@2@@std@@QEAA@XZ.c)
 *     ??$_Try_emplace@AEBK$$V@?$_Hash@V?$_Umap_traits@KV?$ComPtr@UIInputTarget@@@WRL@Microsoft@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKV?$ComPtr@UIInputTarget@@@WRL@Microsoft@@@std@@@5@$0A@@std@@@std@@IEAA?AU?$pair@PEAU?$_List_node@U?$pair@$$CBKV?$ComPtr@UIInputTarget@@@WRL@Microsoft@@@std@@PEAX@std@@_N@1@AEBK@Z @ 0x18015B41C (--$_Try_emplace@AEBK$$V@-$_Hash@V-$_Umap_traits@KV-$ComPtr@UIInputTarget@@@WRL@Microsoft@@V-$_Uh.c)
 *     ??$emplace@AEBK@?$_Hash@V?$_Uset_traits@KV?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@K@2@$0A@@std@@@std@@QEAA?AU?$pair@V?$_List_const_iterator@V?$_List_val@U?$_List_simple_types@K@std@@@std@@@std@@_N@1@AEBK@Z @ 0x18015BE84 (--$emplace@AEBK@-$_Hash@V-$_Uset_traits@KV-$_Uhash_compare@KU-$hash@K@std@@U-$equal_to@K@2@@std@.c)
 *     ??$_Try_emplace@K$$V@?$_Hash@V?$_Umap_traits@KV?$ComPtr@UIInputTarget@@@WRL@Microsoft@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKV?$ComPtr@UIInputTarget@@@WRL@Microsoft@@@std@@@5@$0A@@std@@@std@@IEAA?AU?$pair@PEAU?$_List_node@U?$pair@$$CBKV?$ComPtr@UIInputTarget@@@WRL@Microsoft@@@std@@PEAX@std@@_N@1@$$QEAK@Z @ 0x1801BEDB8 (--$_Try_emplace@K$$V@-$_Hash@V-$_Umap_traits@KV-$ComPtr@UIInputTarget@@@WRL@Microsoft@@V-$_Uhash.c)
 *     ??$_Try_emplace@PEAUIInputTarget@@$$V@?$_Hash@V?$_Umap_traits@PEAUIInputTarget@@V?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@std@@V?$_Uhash_compare@PEAUIInputTarget@@U?$hash@PEAUIInputTarget@@@std@@U?$equal_to@PEAUIInputTarget@@@3@@3@V?$allocator@U?$pair@QEAUIInputTarget@@V?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@std@@@std@@@3@$0A@@std@@@std@@IEAA?AU?$pair@PEAU?$_List_node@U?$pair@QEAUIInputTarget@@V?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@std@@@std@@PEAX@std@@_N@1@$$QEAPEAUIInputTarget@@@Z @ 0x1801BEFFC (--$_Try_emplace@PEAUIInputTarget@@$$V@-$_Hash@V-$_Umap_traits@PEAUIInputTarget@@V-$unordered_set.c)
 *     ??R?$_Func_class@XPEAUInputInfo@@AEAV?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@std@@@std@@QEBAXPEAUInputInfo@@AEAV?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@1@@Z @ 0x1801BFF6C (--R-$_Func_class@XPEAUInputInfo@@AEAV-$unordered_set@KU-$hash@K@std@@U-$equal_to@K@2@V-$allocato.c)
 *     ??R?$_Func_class@XPEAUInputInfo@@AEAV?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@std@@PEAU1@@std@@QEBAXPEAUInputInfo@@AEAV?$unordered_set@KU?$hash@K@std@@U?$equal_to@K@2@V?$allocator@K@2@@1@0@Z @ 0x1801BFFB4 (--R-$_Func_class@XPEAUInputInfo@@AEAV-$unordered_set@KU-$hash@K@std@@U-$equal_to@K@_ea_1801BFFB4.c)
 *     ??R?$_Func_class@XPEAUInputInfo@@PEAK@std@@QEBAXPEAUInputInfo@@PEAK@Z @ 0x1801C0008 (--R-$_Func_class@XPEAUInputInfo@@PEAK@std@@QEBAXPEAUInputInfo@@PEAK@Z.c)
 *     ?OnlyObserve@@YA_NPEAUInputInfo@@@Z @ 0x1801C1B40 (-OnlyObserve@@YA_NPEAUInputInfo@@@Z.c)
 *     ?Get@InfoMetadata@@SAPEAV1@XZ @ 0x1801C32EC (-Get@InfoMetadata@@SAPEAV1@XZ.c)
 *     ?GetInput@InfoMetadata@@QEAAPEBUInputInfoMetadata@1@W4InputType@@@Z @ 0x1801C3324 (-GetInput@InfoMetadata@@QEAAPEBUInputInfoMetadata@1@W4InputType@@@Z.c)
 */

// Hidden C++ exception states: #wind=7
__int64 __fastcall ContextualProcessorBuffer::DeliverInputToTarget(
        _QWORD *a1,
        struct InputInfo *a2,
        _QWORD *a3,
        __int64 a4,
        __int64 a5)
{
  _QWORD *v7; // r13
  __int64 **v9; // rdi
  __int64 *i; // rbx
  __int64 v11; // rax
  __int64 v12; // rbx
  __int64 v13; // rax
  struct InfoMetadata *v14; // rax
  __int64 Input; // r12
  _QWORD *v16; // rax
  __int64 v17; // r8
  const char *v18; // r9
  unsigned __int8 *v19; // rsi
  unsigned __int8 *j; // rdi
  __int64 v21; // rax
  _QWORD *v22; // r14
  _QWORD *k; // rdi
  __int64 v24; // r12
  __int64 v25; // rsi
  unsigned __int8 *v26; // r13
  unsigned __int8 *m; // rsi
  void *v28; // r12
  __int64 v29; // rcx
  int v30; // eax
  unsigned int v31; // esi
  __int64 v32; // rcx
  int v33; // eax
  __int64 v34; // rcx
  __int64 v35; // rcx
  __int64 v36; // rcx
  __int64 v37; // rcx
  unsigned __int64 v38; // rdx
  __int64 v39; // rcx
  __int64 v40; // rcx
  __int64 v41; // r11
  unsigned int v42; // eax
  unsigned __int8 v43; // [rsp+38h] [rbp-D0h]
  int v44; // [rsp+3Ch] [rbp-CCh] BYREF
  __int64 v45; // [rsp+40h] [rbp-C8h] BYREF
  __int64 v46; // [rsp+48h] [rbp-C0h]
  void *v47; // [rsp+50h] [rbp-B8h] BYREF
  void *v48; // [rsp+58h] [rbp-B0h] BYREF
  __int64 v49; // [rsp+68h] [rbp-A0h] BYREF
  _QWORD *v50; // [rsp+70h] [rbp-98h]
  __int64 v51; // [rsp+78h] [rbp-90h]
  void *v52; // [rsp+80h] [rbp-88h] BYREF
  __int128 v53; // [rsp+88h] [rbp-80h]
  __int64 v54; // [rsp+98h] [rbp-70h]
  __int64 v55; // [rsp+A0h] [rbp-68h]
  char v56[8]; // [rsp+A8h] [rbp-60h] BYREF
  unsigned __int8 *v57; // [rsp+B0h] [rbp-58h]
  void *v58; // [rsp+C0h] [rbp-48h]
  __int128 v59; // [rsp+C8h] [rbp-40h]
  __int64 v60; // [rsp+E8h] [rbp-20h] BYREF
  _QWORD **v61; // [rsp+F0h] [rbp-18h]
  void *v62; // [rsp+100h] [rbp-8h]
  __int128 v63; // [rsp+108h] [rbp+0h]
  _BYTE v64[64]; // [rsp+128h] [rbp+20h] BYREF
  wil::details::in1diag3 *retaddr; // [rsp+170h] [rbp+68h]

  v7 = a1;
  if ( OnlyObserve(a2) )
    return 0LL;
  v9 = (__int64 **)v7[12];
  for ( i = *v9; i != (__int64 *)v9; i = (__int64 *)*i )
  {
    if ( !std::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>::count<void>(
            (__int64)a3,
            (const unsigned __int8 *)i + 16) )
    {
      v11 = std::_Hash<std::_Umap_traits<unsigned long,Microsoft::WRL::ComPtr<IInputTarget>,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,Microsoft::WRL::ComPtr<IInputTarget>>>,0>>::_Try_emplace<unsigned long const &,>(
              a3,
              (__int64)&v48,
              (unsigned __int8 *)i + 16);
      Microsoft::WRL::ComPtr<IMessageSession>::operator=((__int64 *)(*(_QWORD *)v11 + 24LL), i + 3);
    }
  }
  v43 = 0;
  v12 = *(_QWORD *)(v7[19] + 16LL);
  v45 = v12;
  Microsoft::WRL::ComPtr<Windows::Foundation::IAsyncActionCompletedHandler>::InternalAddRef(&v45);
  v44 = 0;
  if ( std::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>::count<void>(
         (__int64)a3,
         (const unsigned __int8 *)&v44) )
  {
    v44 = 0;
    v13 = std::_Hash<std::_Umap_traits<unsigned long,Microsoft::WRL::ComPtr<IInputTarget>,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,Microsoft::WRL::ComPtr<IInputTarget>>>,0>>::_Try_emplace<unsigned long,>(
            a3,
            (__int64)&v48,
            (unsigned __int8 *)&v44);
    Microsoft::WRL::ComPtr<IMessageSession>::operator=(&v45, (__int64 *)(*(_QWORD *)v13 + 24LL));
    v43 = 1;
    v12 = v45;
  }
  v14 = InfoMetadata::Get();
  Input = InfoMetadata::GetInput(v14, *(unsigned int *)a2);
  v46 = Input;
  if ( !Input )
  {
LABEL_43:
    v41 = v7[20];
    if ( v41 )
    {
      if ( v43 || !*(_QWORD *)(a4 + 16) )
        v42 = (*(__int64 (__fastcall **)(__int64, struct InputInfo *, __int64, __int64, _DWORD))(*(_QWORD *)v41 + 24LL))(
                v41,
                a2,
                a5,
                v12,
                v43);
      else
        v42 = (*(__int64 (__fastcall **)(_QWORD, struct InputInfo *, __int64))(*(_QWORD *)v41 + 32LL))(v7[20], a2, a4);
    }
    else
    {
      if ( !v12 )
        goto LABEL_40;
      v42 = (*(__int64 (__fastcall **)(__int64, struct InputInfo *))(*(_QWORD *)v12 + 24LL))(v12, a2);
    }
    v31 = v42;
    goto LABEL_41;
  }
  LODWORD(v49) = 0;
  v50 = 0LL;
  v51 = 0LL;
  v16 = operator new(0x58uLL);
  *v16 = v16;
  v16[1] = v16;
  v50 = v16;
  v52 = 0LL;
  v53 = 0LL;
  v54 = 7LL;
  v55 = 8LL;
  *(float *)&v49 = FLOAT_1_0;
  std::_Hash_vec<std::allocator<std::_List_unchecked_iterator<std::_List_val<std::_List_simple_types<std::pair<unsigned long const,ResizeProcessor::MessageInfo>>>>>>::_Assign_grow(
    (__int64)&v52,
    0x10uLL,
    (unsigned __int64)v16);
  v19 = (unsigned __int8 *)a3[1];
  for ( j = *(unsigned __int8 **)v19; j != v19; j = *(unsigned __int8 **)j )
  {
    if ( *((_DWORD *)j + 4) && *((_QWORD *)j + 3) != v12 )
    {
      v47 = (void *)*((_QWORD *)j + 3);
      v21 = std::_Hash<std::_Umap_traits<IInputTarget *,std::unordered_set<unsigned long>,std::_Uhash_compare<IInputTarget *,std::hash<IInputTarget *>,std::equal_to<IInputTarget *>>,std::allocator<std::pair<IInputTarget * const,std::unordered_set<unsigned long>>>,0>>::_Try_emplace<IInputTarget *,>(
              (float *)&v49,
              (__int64)&v48,
              (unsigned __int8 *)&v47);
      std::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>::emplace<unsigned long const &>(
        (_QWORD *)(*(_QWORD *)v21 + 24LL),
        (__int64)v64,
        j + 16);
    }
  }
  if ( v51 )
  {
    v22 = v50;
    for ( k = (_QWORD *)*v50; k != v22; k = (_QWORD *)*k )
    {
      VariableSizedPayloadStorage<InputInfo>::VariableSizedPayloadStorage<InputInfo>((__int64)&v48, (int *)a2, v17, v18);
      v24 = (__int64)v48;
      v47 = v48;
      v25 = v46;
      std::_Func_class<void,InputInfo *,std::unordered_set<unsigned long> &,InputInfo *>::operator()(
        v46,
        (__int64)a2,
        (__int64)(k + 3),
        v48);
      std::unordered_set<unsigned long>::unordered_set<unsigned long>((__int64)&v60);
      std::unordered_set<unsigned long>::unordered_set<unsigned long>((__int64)v56);
      std::_Func_class<void,InputInfo *,std::unordered_set<unsigned long> &>::operator()(v25 + 192, v24);
      v26 = v57;
      for ( m = *(unsigned __int8 **)v57; m != v26; m = *(unsigned __int8 **)m )
      {
        if ( std::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>::count<void>(
               a5,
               m + 16) )
        {
          std::_Hash<std::_Uset_traits<unsigned long,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<unsigned long>,0>>::emplace<unsigned long const &>(
            &v60,
            (__int64)v64,
            m + 16);
        }
      }
      v28 = v47;
      std::_Func_class<void,InputInfo *,unsigned long *>::operator()(v46 + 128, (__int64)v47, (__int64)&v44);
      v7 = a1;
      if ( v44 )
      {
        v29 = a1[20];
        if ( v29 )
        {
          v30 = (*(__int64 (__fastcall **)(__int64, void *, __int64 *, _QWORD, int))(*(_QWORD *)v29 + 24LL))(
                  v29,
                  v28,
                  &v60,
                  k[2],
                  1);
          v31 = v30;
          if ( v30 < 0 )
          {
            wil::details::in1diag3::Return_Hr(
              retaddr,
              (void *)0x44D,
              (__int64)"onecoreuap\\windows\\moderncore\\inputv2\\contextualprocessors\\manager\\lib\\contextualprocessorbuffer.cpp",
              (const char *)(unsigned int)v30);
            std::unordered_map<unsigned long,enum ContextualProcessorDecision>::~unordered_map<unsigned long,enum ContextualProcessorDecision>((__int64)v56);
            std::unordered_map<unsigned long,enum ContextualProcessorDecision>::~unordered_map<unsigned long,enum ContextualProcessorDecision>((__int64)&v60);
            VariableSizedPayloadStorage<InputInfo>::~VariableSizedPayloadStorage<InputInfo>(&v48);
LABEL_36:
            std::unordered_map<unsigned int,std::unordered_set<unsigned __int64>>::~unordered_map<unsigned int,std::unordered_set<unsigned __int64>>((__int64)&v49);
            goto LABEL_41;
          }
        }
        else
        {
          v32 = k[2];
          if ( v32 )
          {
            v33 = (*(__int64 (__fastcall **)(__int64, void *))(*(_QWORD *)v32 + 24LL))(v32, v28);
            v31 = v33;
            if ( v33 < 0 )
            {
              wil::details::in1diag3::Return_Hr(
                retaddr,
                (void *)0x451,
                (__int64)"onecoreuap\\windows\\moderncore\\inputv2\\contextualprocessors\\manager\\lib\\contextualprocessorbuffer.cpp",
                (const char *)(unsigned int)v33);
              std::_Deallocate<16,0>(v58, (v59 - (_QWORD)v58) & 0xFFFFFFFFFFFFFFF8uLL);
              v58 = 0LL;
              v59 = 0LL;
              std::_List_node<std::pair<unsigned long const,enum ContextualProcessorState>,void *>::_Free_non_head<std::allocator<std::_List_node<std::pair<unsigned long const,enum ContextualProcessorState>,void *>>>(
                v36,
                (_QWORD **)v57);
              std::_Deallocate<16,0>(v57, 0x18uLL);
              std::_Deallocate<16,0>(v62, (v63 - (_QWORD)v62) & 0xFFFFFFFFFFFFFFF8uLL);
              v62 = 0LL;
              v63 = 0LL;
              std::_List_node<std::pair<unsigned long const,enum ContextualProcessorState>,void *>::_Free_non_head<std::allocator<std::_List_node<std::pair<unsigned long const,enum ContextualProcessorState>,void *>>>(
                v37,
                v61);
              std::_Deallocate<16,0>(v61, 0x18uLL);
              if ( v48 )
                operator delete[](v48);
              goto LABEL_36;
            }
          }
        }
      }
      std::_Deallocate<16,0>(v58, (v59 - (_QWORD)v58) & 0xFFFFFFFFFFFFFFF8uLL);
      v58 = 0LL;
      v59 = 0LL;
      std::_List_node<std::pair<unsigned long const,enum ContextualProcessorState>,void *>::_Free_non_head<std::allocator<std::_List_node<std::pair<unsigned long const,enum ContextualProcessorState>,void *>>>(
        v34,
        (_QWORD **)v57);
      std::_Deallocate<16,0>(v57, 0x18uLL);
      std::_Deallocate<16,0>(v62, (v63 - (_QWORD)v62) & 0xFFFFFFFFFFFFFFF8uLL);
      v62 = 0LL;
      v63 = 0LL;
      std::_List_node<std::pair<unsigned long const,enum ContextualProcessorState>,void *>::_Free_non_head<std::allocator<std::_List_node<std::pair<unsigned long const,enum ContextualProcessorState>,void *>>>(
        v35,
        v61);
      std::_Deallocate<16,0>(v61, 0x18uLL);
      if ( v48 )
        operator delete[](v48);
    }
    Input = v46;
  }
  std::_Func_class<void,InputInfo *,unsigned long *>::operator()(Input + 128, (__int64)a2, (__int64)&v44);
  v38 = (v53 - (_QWORD)v52) & 0xFFFFFFFFFFFFFFF8uLL;
  if ( v44 )
  {
    std::_Deallocate<16,0>(v52, v38);
    v52 = 0LL;
    v53 = 0LL;
    std::_List_node<std::pair<IInputTarget * const,std::unordered_set<unsigned long>>,void *>::_Free_non_head<std::allocator<std::_List_node<std::pair<IInputTarget * const,std::unordered_set<unsigned long>>,void *>>>(
      v40,
      (__int64)v50);
    std::_Deallocate<16,0>(v50, 0x58uLL);
    goto LABEL_43;
  }
  std::_Deallocate<16,0>(v52, v38);
  v52 = 0LL;
  v53 = 0LL;
  std::_List_node<std::pair<IInputTarget * const,std::unordered_set<unsigned long>>,void *>::_Free_non_head<std::allocator<std::_List_node<std::pair<IInputTarget * const,std::unordered_set<unsigned long>>,void *>>>(
    v39,
    (__int64)v50);
  std::_Deallocate<16,0>(v50, 0x58uLL);
LABEL_40:
  v31 = 0;
LABEL_41:
  Microsoft::WRL::ComPtr<HotkeyRegistrationForwarder>::InternalRelease(&v45);
  return v31;
}
