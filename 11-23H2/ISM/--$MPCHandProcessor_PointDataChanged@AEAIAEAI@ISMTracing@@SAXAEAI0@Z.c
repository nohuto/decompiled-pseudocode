/*
 * XREFs of ??$MPCHandProcessor_PointDataChanged@AEAIAEAI@ISMTracing@@SAXAEAI0@Z @ 0x1800BDCC8
 * Callers:
...
void __fastcall ISMTracing::MPCHandProcessor_PointDataChanged<unsigned int &,unsigned int &>(
        unsigned int *a1,
        unsigned int *a2)
{
  ISMTracing *v4; // rcx
  ...
  if ( ISMTracing::IsEnabled((char)a1) )
  {
    ...
    ISMTracing::MPCHandProcessor_PointDataChanged_(v4, *a1, *a2);
  }
}
