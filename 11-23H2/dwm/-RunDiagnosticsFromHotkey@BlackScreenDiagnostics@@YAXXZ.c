/*
 * XREFs of ?RunDiagnosticsFromHotkey@BlackScreenDiagnostics@@YAXXZ @ 0x14000EA28
 * Callers:
 ...
 *     ?now@steady_clock@chrono@std@@SA?AV?$time_point@Usteady_clock@chrono@std@@V?$duration@_JU?$ratio@$00$0DLJKMKAA@@std@@@23@@23@XZ @ 0x14000EDD4 (-now@steady_clock@chrono@std@@SA-AV-$time_point@Usteady_clock@chrono@std@@V-$duration@_JU-$ratio.c)
 */
...
void __fastcall BlackScreenDiagnostics::RunDiagnosticsFromHotkey(BlackScreenDiagnostics *this)
{
  ...
  Uuid = 0LL;
  UuidCreate(&Uuid);
  if ( *(_QWORD *)std::chrono::steady_clock::now(v1) - qword_140017E78 < 30000000000LL )
  {
  ...
  else
  {
    ...
    qword_140017E78 = *(_QWORD *)std::chrono::steady_clock::now(v1);
  }
}
