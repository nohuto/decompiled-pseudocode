/*
 * XREFs of ?GetEndpointNotificationHandler@CAPOProcessingHost@@AEAAJPEAUIMMDevice@@PEAPEAVCAPOEndpointNotificationsHandler@@@Z @ 0x14006CC7C
 * Callers:
        ...
        struct CAPOEndpointNotificationsHandler **a3)
{
  ...
  struct CAPOEndpointNotificationsHandler *v13; // rcx
  LPVOID pv; // [rsp+20h] [rbp-78h] BYREF
  struct IMMDevice *v15; // [rsp+28h] [rbp-70h] BYREF
  _QWORD v16[3]; // [rsp+30h] [rbp-68h] BYREF
  void *v17[3]; // [rsp+48h] [rbp-50h] BYREF
  ...
  std::wstring::wstring(v17, (__int64)pv);
  std::_Hash<std::_Umap_traits<std::wstring,wil::com_ptr_t<CAPOEndpointNotificationsHandler,wil::err_returncode_policy>,std::_Uhash_compare<std::wstring,std::hash<std::wstring>,std::equal_to<std::wstring>>,std::allocator<std::pair<std::wstring const,wil::com_ptr_t<CAPOEndpointNotificationsHandler,wil::err_returncode_policy>>>,0>>::find<void>(
    ...
    (__int64)v17);
  if ( v18 >= 8 )
    std::_Deallocate<16,0>(v17[0], 2 * v18 + 2);
  if ( v16[0] == *((_QWORD *)this + 11) )
  {
    v11 = Microsoft::WRL::Details::MakeAndInitialize<CAPOEndpointNotificationsHandler,CAPOEndpointNotificationsHandler,IMMDevice * &>(
            a3,
            &v15);
    v12 = v11;
    if ( v11 < 0 )
  ...
  {
    v13 = *(struct CAPOEndpointNotificationsHandler **)(v16[0] + 48LL);
    if ( v13 )
    {
      *a3 = v13;
      (*(void (__fastcall **)(struct CAPOEndpointNotificationsHandler *))(*(_QWORD *)v13 + 8LL))(v13);
    }
    else
    {
      *a3 = 0LL;
