/*
 * XREFs of ?ndisSetTaskOffloadValidate@@YAHPEAU_NDIS_FILTER_BLOCK@@PEAU_NDIS_MINIPORT_BLOCK@@PEAU_NDIS_OID_REQUEST@@@Z @ 0x1C00A6F9C
 * Callers:
        ...
        struct _NDIS_OID_REQUEST *a3)
{
    ...
    && (v7 = *((unsigned int *)InformationBuffer + 3), (unsigned int)v7 <= (unsigned int)InformationBufferLength) )
  {
    if ( (_DWORD)v7 )
    {
      v8 = &InformationBuffer[v7];
      if ( (unsigned int)v7 < 0xFFFFFFE8 )
      {
        while ( (unsigned __int64)(unsigned int)v7 + 24 <= InformationBufferLength )
        {
          v9 = *((_DWORD *)v8 + 4);
          v10 = v9 + v7;
          ...
          if ( *((_DWORD *)v8 + 2) == 2 )
          {
            ...
            if ( v9 == 16 && *((_DWORD *)v8 + 6) <= Offload->TopCapabilities.LsoV1.IPv4.MaxOffLoadSize )
            {
              if ( *((_DWORD *)v8 + 7) < Offload->TopCapabilities.LsoV1.IPv4.MinSegmentCount )
                v4 = -1073676267;
            }
            else
            {
