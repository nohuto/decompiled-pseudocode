/*
 * XREFs of ?ndisVerifierInitialization@@YAEXZ @ 0x1C0035EE0
 * Callers:
  ...
  if ( MmIsVerifierEnabled(&VerifierFlags) >= 0 )
  {
    ...
    if ( ndisVerifierWdmDispatch )
    {
      ...
      if ( ndisVerifierNdisDispatch )
      {
        ...
        if ( MmAddVerifierThunks(&off_1C00D78E0, 0x190u) >= 0 )
        {
          qword_1C00F7810 = (__int64)&ndisMiniportTrackAllocList;
          ndisMiniportTrackAllocList.Flink = &ndisMiniportTrackAllocList;
          qword_1C00F7800 = (__int64)&ndisDriverTrackAllocList;
