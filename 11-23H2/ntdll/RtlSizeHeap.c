/*
 * XREFs of RtlSizeHeap @ 0x18003AA50
 * Callers:
 *     RtlSetEnvironmentVar @ 0x180014C10 (RtlSetEnvironmentVar.c)
 *     RtlpReAllocateHeapInternal @ 0x180042250 (RtlpReAllocateHeapInternal.c)
 *     RtlSetCurrentEnvironment @ 0x18008E750 (RtlSetCurrentEnvironment.c)
 *     RtlSetEnvironmentStrings @ 0x1800D8D40 (RtlSetEnvironmentStrings.c)
 *     RtlpScanHeapAllocBlocks @ 0x180104D48 (RtlpScanHeapAllocBlocks.c)
 *     RtlDebugSizeHeap @ 0x180107838 (RtlDebugSizeHeap.c)
 *     RtlpHeapTrkDumpOutstandingAllocs @ 0x18010C2D4 (RtlpHeapTrkDumpOutstandingAllocs.c)
 * Callees:
 *     RtlpHpSizeHeap @ 0x18003ACD0 (RtlpHpSizeHeap.c)
 *     RtlpHpConvertFlagsToSegmentFlags @ 0x18003AFAC (RtlpHpConvertFlagsToSegmentFlags.c)
 *     RtlNtStatusToDosError @ 0x1800536B0 (RtlNtStatusToDosError.c)
 *     RtlpProbeUserBufferSafe @ 0x1800768C4 (RtlpProbeUserBufferSafe.c)
 *     RtlDebugSizeHeap @ 0x180107838 (RtlDebugSizeHeap.c)
 *     RtlpLogHeapFailure @ 0x1801229F0 (RtlpLogHeapFailure.c)
 */

__int64 __fastcall RtlSizeHeap(__int64 a1, unsigned int a2, unsigned __int64 a3)
{
  __int64 v3; // r14
  unsigned __int64 v4; // rbp
  __int64 v7; // rsi
  unsigned int v8; // eax
  __int64 v9; // r10
  __int64 v10; // rax
  __int64 v11; // rbx
  unsigned int v13; // ebx
  int v14; // ecx
  char v15; // al
  unsigned __int64 v16; // rdx
  __int64 v17; // rax
  unsigned __int16 v18; // r8
  char v19; // cl
  unsigned __int64 v20; // rdx
  int v21; // eax
  unsigned int v22; // edx
  unsigned __int16 v23; // ax
  int v24; // eax
  int v25; // eax
  unsigned __int16 v26; // ax
  struct _TEB *v27; // rbx
  int v28; // [rsp+38h] [rbp-10h]
  int v29; // [rsp+38h] [rbp-10h]
  int v30; // [rsp+38h] [rbp-10h]

  v3 = 0LL;
  v4 = a3;
  if ( !a1 )
    RtlpLogHeapFailure(19, 0, a3, 0, 0LL, 0LL);
  if ( *(_DWORD *)(a1 + 16) == -571548178 )
  {
    if ( (RtlpHpAppCompatFlags & 2) != 0 && v4 )
      v7 = *(_QWORD *)(v4 - 16);
    else
      v7 = 0LL;
    v8 = RtlpHpConvertFlagsToSegmentFlags(a2);
    v10 = RtlpHpSizeHeap(a1, v9, v8);
    v11 = v10;
    if ( v10 != -1 )
      return v10 - v7;
    RtlpLogHeapFailure(9, a1, v4, 0, 0LL, 0LL);
    return v11;
  }
  v13 = *(_DWORD *)(a1 + 116) | a2;
  if ( (v13 & 0x61000000) != 0 && (v13 & 0x10000000) == 0 )
    return RtlDebugSizeHeap(a1, v13, v4);
  if ( (*(_BYTE *)(a1 + 120) & 1) != 0 )
  {
    v4 = RtlpProbeUserBufferSafe(a1, v4);
  }
  else
  {
    if ( (v4 & 0xF) != 0 )
    {
      v14 = 9;
LABEL_13:
      RtlpLogHeapFailure(v14, a1, v4, 0, 0LL, 0LL);
      v4 = 0LL;
      goto LABEL_14;
    }
    v4 -= 16LL;
    _m_prefetchw((const void *)v4);
    if ( *(_BYTE *)(v4 + 15) == 5 )
      v4 -= 16LL * *(unsigned __int8 *)(v4 + 14);
    if ( (*(_BYTE *)(v4 + 15) & 0x3F) == 0 )
    {
      v14 = 8;
      goto LABEL_13;
    }
  }
LABEL_14:
  if ( !v4 )
  {
    NtCurrentTeb()->LastStatusValue = -1073741811;
    v27 = NtCurrentTeb();
    v27->LastErrorValue = RtlNtStatusToDosError(-1073741811);
    return -1LL;
  }
  v15 = *(_BYTE *)(v4 + 15);
  if ( v15 != 4 )
  {
    if ( v15 >= 0 )
    {
      if ( *(_DWORD *)(a1 + 124) )
      {
        v21 = *(_DWORD *)(v4 + 8);
        LOWORD(v28) = v21;
        if ( (v21 & *(_DWORD *)(a1 + 124)) != 0 )
          v28 = *(_DWORD *)(a1 + 136) ^ v21;
        v18 = v28;
        v16 = v4 >> 4;
      }
      else
      {
        v18 = *(_WORD *)(v4 + 8);
        v16 = v4 >> 4;
      }
    }
    else
    {
      v16 = v4 >> 4;
      if ( *(_WORD *)(v4 + 8) ^ (unsigned __int16)(a1 ^ RtlpLFHKey ^ (v4 >> 4)) )
        v17 = 0LL;
      else
        v17 = *(_QWORD *)(v4
                        - ((unsigned __int64)(*(_DWORD *)(v4 + 8) ^ (unsigned int)a1 ^ (unsigned int)RtlpLFHKey ^ (unsigned int)(v4 >> 4)) >> 12));
      v18 = *(_WORD *)(v17 + 36);
    }
    v19 = *(_BYTE *)(v4 + 15);
    if ( v19 == 5 )
    {
      v20 = *(unsigned __int16 *)(a1 + 140) ^ (unsigned __int64)*(unsigned __int16 *)(v4 + 12);
    }
    else if ( (v19 & 0x40) != 0 )
    {
      v20 = *(unsigned __int16 *)(v4 + 16LL * (v19 & 0x3F) + 12);
    }
    else if ( (v19 & 0x3F) == 0x3F )
    {
      if ( v19 >= 0 )
      {
        if ( *(_DWORD *)(a1 + 124) )
        {
          v24 = *(_DWORD *)(v4 + 8);
          LOWORD(v29) = v24;
          if ( (v24 & *(_DWORD *)(a1 + 124)) != 0 )
            v29 = *(_DWORD *)(a1 + 136) ^ v24;
          v20 = *(_QWORD *)(v4 + 16LL * (unsigned __int16)v29);
          return 16LL * v18 - v20;
        }
        v23 = *(_WORD *)(v4 + 8);
      }
      else
      {
        v22 = RtlpLFHKey ^ a1 ^ *(_DWORD *)(v4 + 8) ^ v16;
        if ( !(_WORD)v22 )
          v3 = *(_QWORD *)(v4 - ((unsigned __int64)v22 >> 12));
        v23 = *(_WORD *)(v3 + 36);
      }
      v20 = *(_QWORD *)(v4 + 16LL * v23);
    }
    else
    {
      v20 = v19 & 0x3F;
    }
    return 16LL * v18 - v20;
  }
  if ( *(_DWORD *)(a1 + 124) )
  {
    v25 = *(_DWORD *)(v4 + 8);
    LOWORD(v30) = v25;
    if ( (v25 & *(_DWORD *)(a1 + 124)) != 0 )
      v30 = *(_DWORD *)(a1 + 136) ^ v25;
    v26 = v30;
  }
  else
  {
    v26 = *(_WORD *)(v4 + 8);
  }
  return *(_QWORD *)(v4 - 16) - v26;
}
