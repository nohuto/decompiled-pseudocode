/*
 * XREFs of CountUTF8ToUnicode @ 0x180055BE0
 * Callers:
 *     RtlUTF8StringToUnicodeString @ 0x180055750 (RtlUTF8StringToUnicodeString.c)
 *     RtlUTF8ToUnicodeN @ 0x180055830 (RtlUTF8ToUnicodeN.c)
 * Callees:
 *     <none>
 */

__int64 __fastcall CountUTF8ToUnicode(unsigned __int64 a1, unsigned int a2, _DWORD *a3)
{
  int v3; // r9d
  unsigned __int64 v4; // r10
  unsigned int v5; // r11d
  unsigned int v6; // ebx
  unsigned __int64 v8; // r8
  unsigned int v9; // edx
  unsigned int v10; // r9d
  unsigned int v11; // eax
  __int64 result; // rax
  unsigned __int64 v13; // r8
  unsigned int v14; // edx
  char v15; // r9
  int v16; // eax
  __int64 v17; // rax
  unsigned int v18; // r9d
  char v19; // dl

  v3 = 0;
  v4 = a1 + a2;
  v5 = a2;
  v6 = 0;
  while ( 1 )
  {
LABEL_2:
    v8 = a1;
    if ( a1 >= v4 )
    {
      if ( v3 )
      {
        v5 += (v3 >> 30) + 1;
        v6 = 263;
      }
      goto LABEL_14;
    }
    v9 = *(char *)a1++;
    if ( v3 )
    {
      if ( (v9 & 0xC0) != 0x80 )
      {
        a1 = v8;
        v5 += v3 >> 30;
        goto LABEL_47;
      }
      v3 = (v3 << 6) | v9 & 0x3F;
      if ( (v3 & 0x20000000) == 0 )
      {
        if ( (v3 & 0x10000000) != 0 )
        {
          if ( (v3 & 0x800000) != 0 || (v3 & 0x1F0u) - 16 <= 0xF0 )
            continue;
        }
        else if ( (v3 & 0x3E0) != 0 && (v3 & 0x3E0) != 0x360 )
        {
          continue;
        }
LABEL_47:
        v6 = 263;
        ++v5;
        goto LABEL_39;
      }
    }
    else
    {
      LOBYTE(v10) = v9;
      if ( v9 > 0x7F )
      {
LABEL_8:
        --v5;
        if ( (v10 & 0x40) != 0 )
        {
          if ( (v10 & 0x20) != 0 )
          {
            v11 = v10 & 0xF;
            if ( (v10 & 0x10) == 0 )
            {
              v3 = v11 | 0x48228000;
              --v5;
              continue;
            }
            if ( v11 <= 4 )
            {
              v3 = v11 | 0x504D0C00;
              --v5;
              continue;
            }
          }
          else
          {
            v18 = v10 & 0x1F;
            if ( v18 > 1 )
            {
              v3 = v18 | 0x800000;
              continue;
            }
          }
        }
        goto LABEL_47;
      }
    }
    if ( v4 - a1 <= 0xD )
      break;
    v13 = v4 - 7;
    if ( a1 >= v4 - 7 )
      goto LABEL_39;
    while ( 1 )
    {
      v14 = *(char *)a1++;
      if ( v14 <= 0x7F )
      {
        if ( (a1 & 1) == 0 )
          break;
        v14 = *(char *)a1++;
        if ( v14 <= 0x7F )
          break;
      }
LABEL_27:
      v15 = *(_BYTE *)a1++;
      if ( (v14 & 0x40) == 0 || (v15 & 0xC0) != 0x80 )
      {
LABEL_60:
        a1 -= 2LL;
        v3 = 0;
        goto LABEL_2;
      }
      if ( (v14 & 0x20) != 0 )
      {
        if ( (v14 & 0x10) != 0 )
        {
          if ( ((v15 & 0x3F | ((v14 & 0xF) << 6)) >> 4) - 1 > 0xF )
            goto LABEL_60;
          v19 = *(_BYTE *)a1 & 0xC0;
          if ( v19 != (char)0x80 || (*(_BYTE *)(a1 + 1) & 0xC0) != v19 )
            goto LABEL_60;
          v17 = 2LL;
        }
        else
        {
          v16 = v15 & 0x20 | ((v14 & 0xF) << 6) & 0x3E0;
          if ( !v16 || v16 == 864 || (*(_BYTE *)a1 & 0xC0) != 0x80 )
            goto LABEL_60;
          v17 = 1LL;
        }
        --v5;
        a1 += v17;
      }
      else if ( (v14 & 0x1E) == 0 )
      {
        goto LABEL_60;
      }
      --v5;
LABEL_38:
      if ( a1 >= v13 )
        goto LABEL_39;
    }
    if ( (a1 & 2) == 0 )
      goto LABEL_21;
    LOWORD(v14) = *(_WORD *)a1;
    if ( (*(_WORD *)a1 & 0x8080) != 0 )
    {
LABEL_37:
      ++a1;
      if ( (unsigned __int8)v14 <= 0x7Fu )
        goto LABEL_38;
      goto LABEL_27;
    }
    a1 += 2LL;
LABEL_21:
    while ( a1 < v13 )
    {
      v14 = *(_DWORD *)a1;
      if ( ((*(_DWORD *)(a1 + 4) | *(_DWORD *)a1) & 0x80808080) != 0 )
        goto LABEL_37;
      a1 += 8LL;
      if ( a1 >= v13 )
        break;
      v14 = *(_DWORD *)a1;
      if ( ((*(_DWORD *)(a1 + 4) | *(_DWORD *)a1) & 0x80808080) != 0 )
        goto LABEL_37;
      a1 += 8LL;
    }
LABEL_39:
    v3 = 0;
  }
  while ( a1 < v4 )
  {
    v10 = *(char *)a1++;
    if ( v10 > 0x7F )
      goto LABEL_8;
  }
LABEL_14:
  result = v6;
  *a3 = 2 * v5;
  return result;
}
