/*
 * XREFs of RtlpLookupSafeCurDirList @ 0x1800EAB00
 * Callers:
  ...
  *(_DWORD *)&ValueName.Length = 0x2000000;
  if ( qword_180184870 == -1 )
  {
    ...
    if ( Heap )
    {
      Heap[1] = Heap;
      *Heap = Heap;
      KeyHandle = qword_180188428;
      v3 = qword_180188428;
      if ( !qword_180188428 )
      {
        v4 = NtOpenKey(&KeyHandle, 1u, (POBJECT_ATTRIBUTES)&stru_1801347A8);
        v5 = (signed __int64)KeyHandle;
        if ( v4 < 0 )
          v5 = -1LL;
        KeyHandle = (HANDLE)v5;
        v3 = (HANDLE)_InterlockedCompareExchange64((volatile signed __int64 *)&qword_180188428, v5, 0LL);
        if ( v3 )
        {
          if ( v4 >= 0 )
            NtClose(KeyHandle);
      ...
      if ( v3 != (HANDLE)-1LL
        && NtQueryValueKey(
             v3,
             (PUNICODE_STRING)&stru_180134788,
        ...
        && ResultLength[0] == 16
        && v21 == 1
        || RtlQueryEnvironmentVariable_U(0LL, (PUNICODE_STRING)&stru_180134798, &Value) >= 0
...
LABEL_29:
    ...
    v11 = _InterlockedCompareExchange64(&qword_180184870, (signed __int64)v2, -1LL);
    if ( v11 == -1 )
    {
      return (__int64)v2;
    }
