/*
 * XREFs of HalpQueryProfileSource @ 0x14050806C
 * Callers:
 *     HalpQueryProfileInformation @ 0x140865678 (HalpQueryProfileInformation.c)
 * Callees:
 *     KxReleaseSpinLock @ 0x1402505D0 (KxReleaseSpinLock.c)
 *     KeIsEmptyAffinityEx @ 0x140255230 (KeIsEmptyAffinityEx.c)
 *     HalpGetProfileDescriptor @ 0x14037B6E0 (HalpGetProfileDescriptor.c)
 *     HalpAcquireHighLevelLock @ 0x14037CD18 (HalpAcquireHighLevelLock.c)
 *     Feature_Servicing_DefaultProfileIntervalQueryErrorETLVMPlatforms__private_IsEnabledDeviceUsage @ 0x14040FA04 (Feature_Servicing_DefaultProfileIntervalQueryErrorETLVMPlatforms__private_IsEnabledDeviceUsage.c)
 *     KiRemoveSystemWorkPriorityKick @ 0x14041057C (KiRemoveSystemWorkPriorityKick.c)
 *     HalpTimerGetProfilingTarget @ 0x14050AE28 (HalpTimerGetProfilingTarget.c)
 */

__int64 __fastcall HalpQueryProfileSource(unsigned int a1, __int64 a2, _DWORD *a3)
{
  unsigned int v3; // ebx
  unsigned __int64 v6; // rdi
  __int64 v7; // rdx
  unsigned __int8 v8; // cl
  struct _KPRCB *v9; // r9
  _DWORD *v10; // r8
  int v11; // eax
  bool v12; // zf
  __int64 v14; // rsi
  __int64 v15; // rdx
  __int64 v16; // rcx
  int ProfilingTarget; // eax
  _DWORD *v18; // rdx
  unsigned __int8 CurrentIrql; // al
  struct _KPRCB *CurrentPrcb; // r11
  _DWORD *SchedulerAssist; // r9
  int v22; // edx
  __int64 v23; // [rsp+58h] [rbp+20h] BYREF

  v3 = 0;
  v23 = 0LL;
  if ( a1 < 0x18 )
  {
    v3 = -1073741820;
LABEL_29:
    *a3 = 24;
    return v3;
  }
  v6 = HalpAcquireHighLevelLock(&HalpProfileSourceDescriptorListLock);
  if ( (int)HalpGetProfileDescriptor(*(_DWORD *)a2, v7, &v23) >= 0 )
  {
    v14 = v23;
    *(_BYTE *)(a2 + 4) = (unsigned int)KeIsEmptyAffinityEx((_WORD *)(v23 + 32)) == 0;
    if ( (unsigned int)Feature_Servicing_DefaultProfileIntervalQueryErrorETLVMPlatforms__private_IsEnabledDeviceUsage() )
    {
      if ( *(_DWORD *)v14 || HalpProfileInterface == &DefaultProfileInterface )
        ProfilingTarget = *(_DWORD *)(v14 + 24);
      else
        ProfilingTarget = HalpTimerGetProfilingTarget(v16, v15);
      v18 = (_DWORD *)(a2 + 8);
    }
    else
    {
      v18 = (_DWORD *)(a2 + 8);
      if ( *(_DWORD *)v14 )
        ProfilingTarget = *(_DWORD *)(v14 + 24);
      else
        ProfilingTarget = HalpTimerGetProfilingTarget(v16, v18);
    }
    *v18 = ProfilingTarget;
    *(_QWORD *)(a2 + 16) = *(_QWORD *)(v14 + 296);
    KxReleaseSpinLock((volatile signed __int64 *)&HalpProfileSourceDescriptorListLock);
    if ( (_DWORD)KiIrqlFlags )
    {
      CurrentIrql = KeGetCurrentIrql();
      if ( ((unsigned __int8)KiIrqlFlags & 1) != 0
        && CurrentIrql <= 0xFu
        && (unsigned __int8)v6 <= 0xFu
        && CurrentIrql >= 2u )
      {
        CurrentPrcb = KeGetCurrentPrcb();
        SchedulerAssist = CurrentPrcb->SchedulerAssist;
        v22 = ~(unsigned __int16)(-1LL << ((unsigned __int8)v6 + 1));
        v12 = (v22 & SchedulerAssist[5]) == 0;
        SchedulerAssist[5] &= v22;
        if ( v12 )
          KiRemoveSystemWorkPriorityKick((__int64)CurrentPrcb);
      }
    }
    __writecr8(v6);
    goto LABEL_29;
  }
  KxReleaseSpinLock((volatile signed __int64 *)&HalpProfileSourceDescriptorListLock);
  if ( (_DWORD)KiIrqlFlags )
  {
    v8 = KeGetCurrentIrql();
    if ( ((unsigned __int8)KiIrqlFlags & 1) != 0 && v8 <= 0xFu && (unsigned __int8)v6 <= 0xFu && v8 >= 2u )
    {
      v9 = KeGetCurrentPrcb();
      v10 = v9->SchedulerAssist;
      v11 = ~(unsigned __int16)(-1LL << ((unsigned __int8)v6 + 1));
      v12 = (v11 & v10[5]) == 0;
      v10[5] &= v11;
      if ( v12 )
        KiRemoveSystemWorkPriorityKick((__int64)v9);
    }
  }
  __writecr8(v6);
  *a3 = 0;
  return 3221225659LL;
}
