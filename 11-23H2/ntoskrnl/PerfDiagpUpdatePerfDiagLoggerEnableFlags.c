/*
 * XREFs of PerfDiagpUpdatePerfDiagLoggerEnableFlags @ 0x14083BBD8
 * Callers:
...
NTSTATUS __fastcall PerfDiagpUpdatePerfDiagLoggerEnableFlags(__int64 a1, __int64 a2)
{
  ...
  LODWORD(v6[4]) = 3;
  LODWORD(Src[0]) = 48;
  result = RtlpQueryRegistryValues(2LL, L"Diagnostics\\Performance\\BootCKCLSettings", v6, 0LL);
  if ( result >= 0 )
  {
    if ( DWORD1(Src[0]) == 3 )
    {
      ...
      {
        memmove(&unk_140C327C8, (char *)Src + 8, LODWORD(Src[0]));
