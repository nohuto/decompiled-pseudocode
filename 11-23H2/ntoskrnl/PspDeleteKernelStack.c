/*
 * XREFs of PspDeleteKernelStack @ 0x14035AAB0
 * Callers:
 *     <none>
 * Callees:
 *     MmDeleteKernelStackEx @ 0x14026FB60 (MmDeleteKernelStackEx.c)
 *     KeResetKernelShadowStackRstorssp @ 0x140974500 (KeResetKernelShadowStackRstorssp.c)
 *     KiDeleteKernelShadowStack @ 0x14097458C (KiDeleteKernelShadowStack.c)
 */

signed __int64 __fastcall PspDeleteKernelStack(__int64 a1, __int64 a2)
{
  unsigned int v3; // edx
  signed __int64 result; // rax
  __int64 v5; // r9
  bool v6; // zf
  unsigned int v7; // r10d
  __int64 v8; // [rsp+30h] [rbp+8h] BYREF

  if ( (*(_DWORD *)(a1 + 116) & 0x800000) == 0 || (v6 = *(_QWORD *)(a2 + 16) == *(_QWORD *)(a1 + 96), v3 = 8, !v6) )
    v3 = 2;
  result = MmDeleteKernelStackEx(*(_QWORD *)a2, v3, a1);
  v5 = *(_QWORD *)(a2 + 48);
  if ( v5 )
  {
    v7 = *(_DWORD *)(a2 + 40) & 7;
    v8 = *(_QWORD *)(a2 + 48);
    if ( v7 == 2 )
    {
      KeResetKernelShadowStackRstorssp(&v8, *(_QWORD *)(a2 + 56));
      v5 = v8;
    }
    return KiDeleteKernelShadowStack(*(_QWORD *)(a2 + 32), v7, 0LL, v5);
  }
  return result;
}
