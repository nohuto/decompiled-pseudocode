/*
 * XREFs of MmDoesFileHaveUserWritableReferences @ 0x140287B70
 * Callers:
...
ULONG __stdcall MmDoesFileHaveUserWritableReferences(PSECTION_OBJECT_POINTERS SectionPointer)
{
  ...
  while ( 1 )
  {
    ...
    {
      ExpAcquireSpinLockExclusiveAtDpcLevelInstrumented(&dword_140C65540, CurrentIrql);
    }
    else
    {
      v22 = 0;
      if ( _interlockedbittestandset(&dword_140C65540, 0x1Fu) )
        v22 = ExpWaitForSpinLockExclusiveAndAcquire(&dword_140C65540, CurrentIrql);
      while ( (dword_140C65540 & 0xBFFFFFFF) != 0x80000000 )
      {
        if ( (dword_140C65540 & 0x40000000) == 0 )
          _InterlockedOr(&dword_140C65540, 0x40000000u);
    ...
    if ( !SectionPointer->DataSectionObject )
    {
      if ( (BYTE6(PerfGlobalGroupMask) & 1) != 0 )
        ExpReleaseSpinLockExclusiveFromDpcLevelInstrumented(&dword_140C65540, retaddr);
      else
        dword_140C65540 = 0;
