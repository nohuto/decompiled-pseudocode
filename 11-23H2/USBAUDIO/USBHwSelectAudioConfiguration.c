/*
 * XREFs of USBHwSelectAudioConfiguration @ 0x1C0032868
 * Callers:
...
__int64 __fastcall USBHwSelectAudioConfiguration(__int64 a1)
{
  __int64 v1; // rax
  _QWORD *v3; // r14
  struct _USB_CONFIGURATION_DESCRIPTOR *v4; // r15
  int v5; // ebx
      ...
      if ( v8->UrbHeader.Status >= 0 )
      {
        if ( v9 >= 0 )
        {
          ...
          do
          {
            ...
            v5 = USBHwAllocateAndBag(InterfaceList, *(unsigned __int16 *)InterfaceList->InterfaceDescriptor, 64LL, v11);
            if ( v5 >= 0 )
              memmove(
