/*
 * XREFs of ?SetApplicationId@CChannelContext@@QEAAJPEBG@Z @ 0x18009240C
 * Callers:
 *     ?Channel_SetApplicationId@CComposition@@AEAAJPEAVCChannelContext@@PEAVCResourceTable@@PEBUtagMILCMD_CHANNEL_SETAPPLICATIONID@@PEBXI@Z @ 0x1800923E0 (-Channel_SetApplicationId@CComposition@@AEAAJPEAVCChannelContext@@PEAVCResourceTable@@PEBUtagMIL.c)
 * Callees:
  ...
  if ( v7 )
  {
    ...
    if ( v8 )
    {
      std::lower_bound<std::_Vector_iterator<std::_Vector_val<std::_Simple_types<CProcessAttributionManager::Record *>>>,unsigned __int64,bool (*)(CProcessAttributionManager::ProcessAttributionRecord const *,__int64)>(
        &v10,
        *(_QWORD *)qword_1803E5C98,
        *((_QWORD *)qword_1803E5C98 + 1),
        v7 + 16);
      do
        ++v4;
      while ( v8[v4] );
