/*
 * XREFs of PerformSample @ 0x1800BA674
 * Callers:
 *     PrepareSamplers @ 0x1800BAF88 (PrepareSamplers.c)
 *     AppendShaderBody @ 0x1800BB27C (AppendShaderBody.c)
 * Callees:
 *     ?MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJIPEAX@Z @ 0x18009DA0C (-MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJIPEAX@Z.c)
 *     ?AppendNode@CShaderLinkingGraphBuilder@@QEAAJW4ShaderLinkingArgument@@PEBDPEAUID3D11Module@@1V?$span@$$CBW4ShaderLinkingArgument@@$0?0@gsl@@@Z @ 0x1800B457C (-AppendNode@CShaderLinkingGraphBuilder@@QEAAJW4ShaderLinkingArgument@@PEBDPEAUID3D11Module@@1V-$.c)
 *     ?CopyNode@CShaderLinkingGraphBuilder@@QEAAJW4ShaderLinkingArgument@@0@Z @ 0x1800BA5CC (-CopyNode@CShaderLinkingGraphBuilder@@QEAAJW4ShaderLinkingArgument@@0@Z.c)
 *     ApplyTexcoordTransformations @ 0x1800BBB44 (ApplyTexcoordTransformations.c)
 *     AppendColorModifications @ 0x1800BBBDC (AppendColorModifications.c)
 *     GetExtendModeDecoration @ 0x1800F65C0 (GetExtendModeDecoration.c)
 *     ??0?$extent_type@$0?0@details@gsl@@QEAA@_K@Z @ 0x180103C6C (--0-$extent_type@$0-0@details@gsl@@QEAA@_K@Z.c)
 *     __security_check_cookie @ 0x18010EE30 (__security_check_cookie.c)
 *     ?terminate@details@gsl@@YAXXZ @ 0x1801B1AE0 (-terminate@details@gsl@@YAXXZ.c)
 */

__int64 __fastcall PerformSample(
        unsigned __int64 a1,
        __int64 a2,
        unsigned int a3,
        __int16 a4,
        unsigned __int16 a5,
        void *a6)
{
  __int16 v7; // r13
  _BYTE *v9; // rdi
  int v11; // r15d
  unsigned __int16 v12; // r15
  int appended; // eax
  __int64 v14; // rcx
  unsigned int v15; // ebx
  int v16; // eax
  __int64 v17; // rcx
  unsigned __int8 v18; // si
  __int64 v19; // rdx
  gsl::details *v20; // rcx
  void *v21; // r12
  __int128 v23; // xmm0
  char ExtendModeDecoration; // al
  __int64 v25; // rcx
  __int64 v26; // rdx
  __int64 v27; // r8
  char v28; // r8
  char v29; // al
  __int64 v30; // xmm1_8
  int v31; // r9d
  unsigned int v32; // [rsp+20h] [rbp-69h]
  __int16 v33; // [rsp+30h] [rbp-59h]
  __int128 v34; // [rsp+40h] [rbp-49h] BYREF
  _BYTE v35[24]; // [rsp+50h] [rbp-39h] BYREF
  char v36; // [rsp+68h] [rbp-21h]
  unsigned __int8 v37; // [rsp+69h] [rbp-20h]
  char v38; // [rsp+6Ah] [rbp-1Fh]
  void *v39; // [rsp+70h] [rbp-19h] BYREF
  _WORD v40[4]; // [rsp+78h] [rbp-11h] BYREF

  v39 = a6;
  v7 = a3 | 0x300;
  v9 = (_BYTE *)(a2 + 32LL * a3);
  v33 = a3 | 0x400;
  if ( !v9[23] )
  {
    v11 = *(_DWORD *)(a1 + 80);
    *(_DWORD *)(a1 + 80) = v11 + 1;
    v12 = v11 | 0x600;
    appended = CShaderLinkingGraphBuilder::CopyNode(a1, v12, a4);
    v15 = appended;
    if ( appended < 0 )
    {
      v32 = 683;
      goto LABEL_35;
    }
    v16 = ApplyTexcoordTransformations(a1, a2, a3, v12, v39);
    v15 = v16;
    if ( v16 < 0 )
    {
      MilInstrumentationCheckHR_MaybeFailFast(v17, 0LL, 0, v16, 0x2B2u, 0LL);
      return v15;
    }
    v18 = a3 + 48;
    v40[0] = v12;
    v40[1] = v7;
    v40[2] = v33;
    if ( *(_BYTE *)(a2 + 156) )
    {
      qmemcpy(v35, "SampleCubeTexture", 17);
      *(_WORD *)&v35[17] = v18;
      goto LABEL_8;
    }
    if ( v9[21] )
    {
      v19 = 1LL;
      *(_WORD *)&v35[18] = v18;
      qmemcpy(v35, "MultiSampleTexture", 18);
      if ( v9[18] )
      {
        LOBYTE(v17) = v9[16];
        if ( (_BYTE)v17 )
        {
          ExtendModeDecoration = GetExtendModeDecoration(v17, 1LL, 0LL);
          LOBYTE(v25) = v9[17];
          v35[19] = ExtendModeDecoration;
          v35[20] = GetExtendModeDecoration(v25, v26, v27);
          v19 = 2LL;
          v35[21] = v28;
        }
      }
LABEL_9:
      gsl::details::extent_type<-1>::extent_type<-1>(&v34, v19);
      *((_QWORD *)&v34 + 1) = v40;
      if ( (_QWORD)v34 == -1LL )
      {
        gsl::details::terminate(v20);
        JUMPOUT(0x1800BA8F8LL);
      }
      v21 = v39;
      appended = CShaderLinkingGraphBuilder::AppendNode(
                   a1,
                   a5,
                   (__int64)word_18033C470,
                   (__int64)v39,
                   (__int64)v35,
                   &v34);
      v15 = appended;
      if ( appended < 0 )
      {
        v32 = 744;
        goto LABEL_35;
      }
      v15 = AppendColorModifications(a1, v9, a5, v21);
      if ( (v15 & 0x80000000) != 0 )
      {
        v31 = v15;
        v32 = 745;
LABEL_36:
        MilInstrumentationCheckHR_MaybeFailFast(v14, 0LL, 0, v31, v32, 0LL);
        return v15;
      }
      if ( v9[18] )
      {
        if ( !v9[16] )
        {
          *(_QWORD *)&v34 = 3LL;
          *((_QWORD *)&v34 + 1) = &v39;
          LOWORD(v39) = a5;
          WORD1(v39) = v12;
          WORD2(v39) = v7;
          appended = CShaderLinkingGraphBuilder::AppendNode(
                       a1,
                       a5,
                       (__int64)word_18033C470,
                       (__int64)v21,
                       (__int64)"ClampTransparent",
                       &v34);
          v15 = appended;
          if ( appended < 0 )
          {
            v32 = 755;
            goto LABEL_35;
          }
        }
      }
      return v15;
    }
    if ( !v9[20] )
    {
      qmemcpy(v35, "SampleTexture", 13);
      *(_WORD *)&v35[13] = v18;
LABEL_8:
      v19 = 1LL;
      goto LABEL_9;
    }
    if ( v9[17] == 2 )
    {
      if ( v9[16] == 2 )
      {
        v23 = *(_OWORD *)"SampleWrappedTexture";
        *(_DWORD *)&v35[16] = *(_DWORD *)"ture";
        *(_WORD *)&v35[20] = v18;
LABEL_20:
        *(_OWORD *)v35 = v23;
        v19 = 3LL;
        goto LABEL_9;
      }
      v23 = *(_OWORD *)"SampleWrappedTextureVOnly";
      v29 = aSamplewrappedt[24];
      v30 = *(_QWORD *)"tureVOnly";
    }
    else
    {
      v23 = *(_OWORD *)"SampleWrappedTextureUOnly";
      v29 = aSamplewrappedt_0[24];
      v30 = *(_QWORD *)"tureUOnly";
    }
    *(_QWORD *)&v35[16] = v30;
    v36 = v29;
    v37 = v18;
    v38 = 0;
    goto LABEL_20;
  }
  appended = CShaderLinkingGraphBuilder::CopyNode(a1, a5, v7);
  v15 = appended;
  if ( appended < 0 )
  {
    v32 = 673;
LABEL_35:
    v31 = appended;
    goto LABEL_36;
  }
  return v15;
}
