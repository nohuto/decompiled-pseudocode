/*
 * XREFs of ACPIThermalStartDevice @ 0x1C00895C0
 * Callers:
 *     <none>
 * Callees:
 *     ACPIInternalGetDeviceExtension @ 0x1C000169C (ACPIInternalGetDeviceExtension.c)
 *     AMLIGetNamedChild @ 0x1C0002B24 (AMLIGetNamedChild.c)
 *     __security_check_cookie @ 0x1C0003F30 (__security_check_cookie.c)
 *     memset @ 0x1C0004540 (memset.c)
 *     WPP_RECORDER_SF_qqss @ 0x1C0008CB0 (WPP_RECORDER_SF_qqss.c)
 *     WPP_RECORDER_SF_Dqss @ 0x1C00173F8 (WPP_RECORDER_SF_Dqss.c)
 *     ACPIDeviceInternalDeviceRequest @ 0x1C001A984 (ACPIDeviceInternalDeviceRequest.c)
 *     ACPIInternalSetDeviceInterface @ 0x1C002A4D4 (ACPIInternalSetDeviceInterface.c)
 *     ACPIRegisterForDeviceNotifications @ 0x1C0035630 (ACPIRegisterForDeviceNotifications.c)
 *     ACPIThermalGetSensorDevice @ 0x1C003C9E4 (ACPIThermalGetSensorDevice.c)
 *     ACPIThermalLoopEx @ 0x1C003CE8C (ACPIThermalLoopEx.c)
 *     AMLIFreeDataBuffs @ 0x1C0043994 (AMLIFreeDataBuffs.c)
 *     ACPIAmliEvaluateDsm @ 0x1C00747A8 (ACPIAmliEvaluateDsm.c)
 */

__int64 __fastcall ACPIThermalStartDevice(PDEVICE_OBJECT PhysicalDeviceObject, PIRP Irp)
{
  const char *v4; // rbp
  __int64 DeviceExtension; // rbx
  const char *v6; // rdx
  const char *v7; // rcx
  __int64 v8; // rax
  __int64 v9; // rsi
  _QWORD *v10; // rcx
  __int64 v11; // rcx
  int SensorDevice; // edi
  __int64 v13; // rcx
  const char *v14; // rax
  unsigned __int16 v15; // r9
  PDEVICE_OBJECT AttachedDeviceReference; // rax
  PIRP v17; // rax
  PVOID PoolWithTag; // rax
  _QWORD *v19; // rdi
  struct _DEVICE_OBJECT *v20; // rcx
  __int64 v21; // rcx
  void *v22; // rcx
  IRP *v23; // rcx
  void *v24; // rcx
  __int64 v26; // [rsp+50h] [rbp-98h] BYREF
  __int128 v27; // [rsp+60h] [rbp-88h] BYREF
  _BYTE v28[48]; // [rsp+70h] [rbp-78h] BYREF

  v4 = byte_1C005B1F0;
  DeviceExtension = ACPIInternalGetDeviceExtension((ULONG_PTR)PhysicalDeviceObject);
  v6 = byte_1C005B1F0;
  *(_DWORD *)(*(_QWORD *)(DeviceExtension + 200) + 96LL) = 100;
  v7 = byte_1C005B1F0;
  *(_DWORD *)(DeviceExtension + 192) |= 0x8000000u;
  v8 = *(_QWORD *)(DeviceExtension + 8);
  if ( (v8 & 0x200000000000LL) != 0 )
  {
    v6 = *(const char **)(DeviceExtension + 560);
    if ( (v8 & 0x400000000000LL) != 0 )
      v7 = *(const char **)(DeviceExtension + 568);
  }
  WPP_RECORDER_SF_qqss(
    (__int64)WPP_GLOBAL_Control->DeviceExtension,
    4u,
    5u,
    0x14u,
    (__int64)&WPP_1b1165dd198a34919fb1e2a61779dbfc_Traceguids,
    (char)Irp,
    DeviceExtension,
    v6,
    v7);
  v9 = *(_QWORD *)(DeviceExtension + 200);
  KeClearEvent((PRKEVENT)(v9 + 280));
  *(_QWORD *)(v9 + 112) = AMLIGetNamedChild(*(_QWORD **)(DeviceExtension + 712), 1347245151);
  v26 = 0LL;
  memset(v28, 0, sizeof(v28));
  v10 = *(_QWORD **)(DeviceExtension + 712);
  v27 = THRM_EXTENSIONS_DSM_UUID;
  if ( (int)ACPIAmliEvaluateDsm(v10, (__int64)&v27, 0, 0, (__int64)v28, &v26) < 0 )
  {
    *(_DWORD *)(v9 + 128) = 0;
  }
  else
  {
    v11 = v26;
    if ( *(_WORD *)(v26 + 2) == 3 && *(_DWORD *)(v26 + 24) )
      *(_DWORD *)(v9 + 128) = **(unsigned __int8 **)(v26 + 32);
    AMLIFreeDataBuffs(v11);
  }
  SensorDevice = ACPIInternalSetDeviceInterface(PhysicalDeviceObject, &GUID_DEVICE_THERMAL_ZONE);
  if ( SensorDevice < 0 )
  {
    v13 = *(_QWORD *)(DeviceExtension + 8);
    v14 = byte_1C005B1F0;
    if ( (v13 & 0x200000000000LL) != 0 )
    {
      v4 = *(const char **)(DeviceExtension + 560);
      if ( (v13 & 0x400000000000LL) != 0 )
        v14 = *(const char **)(DeviceExtension + 568);
    }
    v15 = 21;
    goto LABEL_15;
  }
  AttachedDeviceReference = IoGetAttachedDeviceReference(*(PDEVICE_OBJECT *)(DeviceExtension + 720));
  *(_QWORD *)(v9 + 200) = AttachedDeviceReference;
  v17 = IoAllocateIrp(AttachedDeviceReference->StackSize, 0);
  *(_QWORD *)(v9 + 192) = v17;
  if ( v17
    && (ACPIRegisterForDeviceNotifications(
          *(_QWORD *)(DeviceExtension + 720),
          (__int64)ACPIThermalEvent,
          *(_QWORD *)(DeviceExtension + 720)),
        PoolWithTag = ExAllocatePoolWithTag(PagedPool, 0x40uLL, 0x54706341u),
        (v19 = PoolWithTag) != 0LL) )
  {
    memset(PoolWithTag, 0, 0x40uLL);
    *(_DWORD *)v19 = 1;
    v19[1] = &ACPIThermalGuidList;
    v19[2] = ACPIThermalQueryWmiRegInfo;
    v19[3] = ACPIThermalQueryWmiDataBlock;
    v20 = *(struct _DEVICE_OBJECT **)(DeviceExtension + 720);
    *(_QWORD *)(DeviceExtension + 224) = v19;
    SensorDevice = IoWMIRegistrationControl(v20, 1u);
    if ( SensorDevice >= 0 )
    {
      SensorDevice = ACPIThermalGetSensorDevice((_QWORD *)DeviceExtension);
      if ( SensorDevice >= 0 )
      {
        *(_DWORD *)(DeviceExtension + 320) = 2;
        SensorDevice = ACPIDeviceInternalDeviceRequest((_QWORD *)DeviceExtension, 1LL, 0LL, 0LL, 0);
        if ( SensorDevice == 259 )
          SensorDevice = 0;
        ACPIThermalLoopEx(DeviceExtension, 0x8000000, 0x10000000);
LABEL_28:
        if ( SensorDevice >= 0 )
          goto LABEL_36;
        goto LABEL_29;
      }
      IoWMIRegistrationControl(*(PDEVICE_OBJECT *)(DeviceExtension + 720), 2u);
      v21 = *(_QWORD *)(DeviceExtension + 8);
      v14 = byte_1C005B1F0;
      if ( (v21 & 0x200000000000LL) != 0 )
      {
        v4 = *(const char **)(DeviceExtension + 560);
        if ( (v21 & 0x400000000000LL) != 0 )
          v14 = *(const char **)(DeviceExtension + 568);
      }
      v15 = 22;
LABEL_15:
      WPP_RECORDER_SF_Dqss(
        (__int64)WPP_GLOBAL_Control->DeviceExtension,
        2u,
        0x15u,
        v15,
        (__int64)&WPP_1b1165dd198a34919fb1e2a61779dbfc_Traceguids,
        SensorDevice,
        DeviceExtension,
        v4,
        v14);
      goto LABEL_28;
    }
  }
  else
  {
    SensorDevice = -1073741670;
  }
LABEL_29:
  v22 = *(void **)(v9 + 200);
  if ( v22 )
  {
    ObfDereferenceObject(v22);
    *(_QWORD *)(v9 + 200) = 0LL;
  }
  v23 = *(IRP **)(v9 + 192);
  if ( v23 )
  {
    IoFreeIrp(v23);
    *(_QWORD *)(v9 + 192) = 0LL;
  }
  v24 = *(void **)(DeviceExtension + 224);
  if ( v24 )
  {
    ExFreePoolWithTag(v24, 0x54706341u);
    *(_QWORD *)(DeviceExtension + 224) = 0LL;
  }
  KeSetEvent((PRKEVENT)(v9 + 280), 0, 0);
LABEL_36:
  Irp->IoStatus.Status = SensorDevice;
  IofCompleteRequest(Irp, 0);
  return (unsigned int)SensorDevice;
}
