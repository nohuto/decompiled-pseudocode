/*
 * XREFs of ProcessorpReserveIdtEntriesApic @ 0x1C008F320
 * Callers:
 *     <none>
 * Callees:
 *     _guard_dispatch_icall_nop @ 0x1C00041D0 (_guard_dispatch_icall_nop.c)
 */

__int64 __fastcall ProcessorpReserveIdtEntriesApic(
        PVOID Owner,
        __int64 a2,
        unsigned int a3,
        __int64 a4,
        unsigned int a5,
        int a6,
        int a7,
        int a8,
        _OWORD *a9)
{
  _QWORD *UserData; // rdi
  int v15; // r15d
  int v16; // esi
  int v17; // ecx
  __int16 v18; // ax
  __int64 v19; // rsi
  UCHAR v20; // bl
  unsigned __int8 i; // bp
  ULONG ProcessorIndexFromNumber; // eax
  struct _RTL_RANGE_LIST *v23; // r10
  struct _RTL_RANGE_LIST *v24; // rcx
  NTSTATUS v25; // r15d
  struct _PROCESSOR_NUMBER v26; // [rsp+50h] [rbp-48h]
  struct _PROCESSOR_NUMBER ProcNumber; // [rsp+58h] [rbp-40h] BYREF
  ULONGLONG Start; // [rsp+60h] [rbp-38h]

  UserData = ExAllocatePoolWithTag(PagedPool, 0x20uLL, 0x49706341u);
  if ( !UserData )
    return 3221225626LL;
  v15 = a8;
  v16 = ((__int64 (__fastcall *)(PVOID, __int64, __int64, _QWORD, unsigned int, int, int, int))ProcessorFindIdtEntries)(
          Owner,
          a2,
          a4,
          a5,
          a3,
          a6,
          a7,
          a8);
  if ( v16 < 0 )
  {
    ExFreePoolWithTag(UserData, 0);
    return (unsigned int)v16;
  }
  v17 = *(_DWORD *)(a4 + 32);
  *UserData = 0LL;
  UserData[1] = 0LL;
  v18 = *(_WORD *)(a4 + 24);
  *UserData = 0LL;
  *((_WORD *)UserData + 4) = v18;
  *((_DWORD *)UserData + 4) = a3;
  *((_DWORD *)UserData + 5) = v17;
  *((_DWORD *)UserData + 6) = a5;
  v19 = *(_QWORD *)(a4 + 16);
  v20 = (a3 >= 0xFFF00000 ? 4 : 0) | ((a6 & 1) + 1);
  LODWORD(Start) = v17;
  for ( i = 0; ; ++i )
  {
    if ( !v19 )
    {
      *a9 = *(_OWORD *)UserData;
      a9[1] = *((_OWORD *)UserData + 1);
      return 0LL;
    }
    if ( _bittest64(&v19, i) )
    {
      v26.Group = *(_WORD *)(a4 + 24);
      *(_WORD *)&v26.Number = i;
      ProcNumber = v26;
      ProcessorIndexFromNumber = KeGetProcessorIndexFromNumber(&ProcNumber);
      if ( ProcessorIndexFromNumber == -1 || ProcessorIndexFromNumber >= ProcessorInstanceCount )
        v23 = 0LL;
      else
        v23 = (struct _RTL_RANGE_LIST *)*((_QWORD *)ProcessorByNtNumber + ProcessorIndexFromNumber);
      if ( v23 )
        break;
    }
LABEL_16:
    v19 &= ~(1LL << i);
  }
  v24 = v23 + 1;
  if ( v15 != 1 )
    v24 = v23;
  v25 = RtlAddRange(v24, (unsigned int)Start, a5 + (unsigned int)Start - 1LL, v20, 2 * (a7 & 1) + 1, UserData, Owner);
  if ( v25 >= 0 )
  {
    v15 = a8;
    *UserData |= 1LL << i;
    goto LABEL_16;
  }
  ExFreePoolWithTag(UserData, 0);
  return (unsigned int)v25;
}
