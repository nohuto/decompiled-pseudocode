/*
 * XREFs of DumpObject @ 0x1C004C888
 * Callers:
 ...
 *     GetRegionSpaceName @ 0x1C0046DB0 (GetRegionSpaceName.c)
...
ULONG __fastcall DumpObject(__int64 a1, __int64 a2, unsigned int a3)
{
  ...
  if ( v6 > 0x80 )
  {
      ...
      if ( v31 )
      {
        ...
        if ( v32 )
        {
          if ( v32 == 1 )
          {
            ...
            ConPrintf("IndexField(%s:IndexName=%s,DataName=%s)", byte_1C005B1F0, pszDest, v35);
          }
        }
        else
        {
          RtlStringCchCopyNA(pszDest, 5uLL, (STRSAFE_PCNZCH)(**(_QWORD **)(a1 + 32) + 40LL), 4uLL);
          ConPrintf("Field(%s:Base=%s)", byte_1C005B1F0, pszDest);
        }
      }
      else
      {
    ...
    {
      ConPrintf("DataAlias(%s:Link=%x)", byte_1C005B1F0, *(_QWORD *)(a1 + 16));
  ...
  else if ( v6 == 128 )
  {
    ...
    ConPrintf("ObjectAlias(%s:Alias=%s,Type=%s)", byte_1C005B1F0, v29, ObjectTypeName);
    if ( ObjectPath )
      ...
      if ( v20 )
      {
        ...
        if ( v21 )
        {
          ...
          if ( v22 )
          {
            ...
            {
              if ( v23 == 1 )
                ConPrintf("DDBHandle(%s:Handle=%x)", byte_1C005B1F0, *(_QWORD *)(a1 + 16));
            }
            else
            {
              v24 = *(_QWORD *)(a1 + 32);
          ...
          {
            ConPrintf("ThermalZone(%s)", byte_1C005B1F0);
          }
        }
        else
        {
  ...
  else
  {
    if ( !*(_WORD *)(a1 + 2) )
    {
      ConPrintf("Unknown(%s)", byte_1C005B1F0);
      return ConPrintf("\n");
    }
    v7 = v6 - 1;
    if ( !v7 )
    {
      ConPrintf("Integer(%s:Value=0x%I64x[%I64d])", byte_1C005B1F0, *(_QWORD *)(a1 + 16), *(_QWORD *)(a1 + 16));
      return ConPrintf("\n");
    }
    v8 = v7 - 1;
    if ( !v8 )
...
LABEL_29:
              ConPrintf(v13, byte_1C005B1F0, *(_QWORD *)(a1 + 32));
            }
          }
          else
          {
            ConPrintf("Device(%s)", byte_1C005B1F0);
          }
        }
        else
        {
      ...
      {
        ConPrintf("Package(%s:NumElements=%d){", byte_1C005B1F0, **(_DWORD **)(a1 + 32));
        ConPrintf("\n");
        v15 = *(_DWORD **)(a1 + 32);
    ...
    {
      ConPrintf("Buffer(%s:Ptr=%x,Len=%d)", byte_1C005B1F0, *(_QWORD *)(a1 + 32), *(_DWORD *)(a1 + 24));
