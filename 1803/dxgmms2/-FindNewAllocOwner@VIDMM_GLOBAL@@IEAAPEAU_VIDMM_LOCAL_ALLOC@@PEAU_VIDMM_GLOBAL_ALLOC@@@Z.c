/*
 * XREFs of ?FindNewAllocOwner@VIDMM_GLOBAL@@IEAAPEAU_VIDMM_LOCAL_ALLOC@@PEAU_VIDMM_GLOBAL_ALLOC@@@Z @ 0x1C006C018
 * Callers:
 *     ?ProcessDeferredCommand@VIDMM_GLOBAL@@QEAAJPEAU_VIDMM_DEFERRED_COMMAND@@PEA_N_N_KPEAU_VIDSCH_SYNC_OBJECT@@2PEAPEAUVIDMM_ALLOC@@@Z @ 0x1C0063D00 (-ProcessDeferredCommand@VIDMM_GLOBAL@@QEAAJPEAU_VIDMM_DEFERRED_COMMAND@@PEA_N_N_KPEAU_VIDSCH_SYN.c)
 * Callees:
 *     ?VidMmRecordAlloc@@YAXPEAVVIDMM_GLOBAL@@PEAU_VIDMM_GLOBAL_ALLOC@@PEAU_VIDMM_LOCAL_ALLOC@@PEAVVIDMM_SEGMENT@@_KW4VIDMM_ALLOC_RECORD_TYPE@@@Z @ 0x1C0005984 (-VidMmRecordAlloc@@YAXPEAVVIDMM_GLOBAL@@PEAU_VIDMM_GLOBAL_ALLOC@@PEAU_VIDMM_LOCAL_ALLOC@@PEAVVID.c)
 */

struct _VIDMM_LOCAL_ALLOC *__fastcall VIDMM_GLOBAL::FindNewAllocOwner(
        VIDMM_GLOBAL *this,
        struct _VIDMM_GLOBAL_ALLOC *a2)
{
  _QWORD **v4; // rdx
  _QWORD *v5; // r8
  __int64 v6; // rdi
  __int64 v7; // r9

  v4 = (_QWORD **)((char *)a2 + 296);
  v5 = *v4;
  do
  {
    if ( v5 == v4 )
      return 0LL;
    v6 = (__int64)(v5 - 7);
    v5 = (_QWORD *)*v5;
  }
  while ( (*(_BYTE *)(v6 + 32) & 2) != 0 );
  v7 = *((_QWORD *)a2 + 17);
  if ( v7 )
  {
    VidMmRecordAlloc(this, (__int64)a2, *((_QWORD *)a2 + 13), v7, *((_QWORD *)a2 + 2), 1);
    VidMmRecordAlloc(this, (__int64)a2, v6, *((_QWORD *)a2 + 17), *((_QWORD *)a2 + 2), 0);
  }
  return (struct _VIDMM_LOCAL_ALLOC *)v6;
}
