/*
 * XREFs of ??0MONITOR_MGR@@QEAA@PEAVADAPTER_DISPLAY@@@Z @ 0x1C0046C58
 * Callers:
 *     MonitorCreateMonitorManager @ 0x1C022D548 (MonitorCreateMonitorManager.c)
 * Callees:
 *     ??_U@YAPEAX_KIW4_POOL_TYPE@@@Z @ 0x1C0015040 (--_U@YAPEAX_KIW4_POOL_TYPE@@@Z.c)
 */

MONITOR_MGR *__fastcall MONITOR_MGR::MONITOR_MGR(MONITOR_MGR *this, struct ADAPTER_DISPLAY *a2, __int64 a3, __int64 a4)
{
  __int64 v6; // rax
  __int64 v7; // rax
  _DWORD *v8; // rax
  KIRQL v9; // al
  _QWORD *v10; // rdx
  _QWORD *v11; // rcx

  *((_WORD *)this + 2) = 0;
  *((_DWORD *)this + 30) = 0;
  *(_DWORD *)this = 16777217;
  *((_QWORD *)this + 1) = a2;
  *((_QWORD *)this + 17) = (char *)this + 128;
  *((_QWORD *)this + 16) = (char *)this + 128;
  *((_QWORD *)this + 19) = (char *)this + 144;
  *((_QWORD *)this + 18) = (char *)this + 144;
  *((_DWORD *)this + 40) = -1;
  *((_DWORD *)this + 70) = 0;
  *((_BYTE *)this + 164) = 1;
  if ( !a2 )
  {
    v6 = WdLogNewEntry5_WdAssertion(this);
    WdLogEvent5_WdAssertion(v6);
  }
  v7 = WdLogNewEntry5_WdTrace(this, a2, a3, a4);
  *(_QWORD *)(v7 + 24) = this;
  *(_QWORD *)(v7 + 32) = *((_QWORD *)a2 + 2);
  ExInitializeResourceLite((PERESOURCE)((char *)this + 16));
  KeInitializeGuardedMutex((PKGUARDED_MUTEX)this + 3);
  KeInitializeGuardedMutex((PKGUARDED_MUTEX)this + 4);
  v8 = operator new[](0x30uLL, 0x4D677844u, (POOL_TYPE)512);
  *((_QWORD *)this + 96) = v8;
  if ( v8 )
  {
    v8[4] = 1;
    *(_DWORD *)(*((_QWORD *)this + 96) + 20LL) = 0;
    KeInitializeEvent((PRKEVENT)(*((_QWORD *)this + 96) + 24LL), NotificationEvent, 1u);
    v9 = KeAcquireSpinLockRaiseToDpc(&MONITOR_MGR::_MonitorPendingEventTraceLock);
    v10 = (_QWORD *)qword_1C007BB20;
    v11 = (_QWORD *)*((_QWORD *)this + 96);
    if ( *(struct _LIST_ENTRY **)qword_1C007BB20 != &MONITOR_MGR::_MonitorPendingEventTraceHead )
      __fastfail(3u);
    v11[1] = qword_1C007BB20;
    *v11 = &MONITOR_MGR::_MonitorPendingEventTraceHead;
    *v10 = v11;
    qword_1C007BB20 = (__int64)v11;
    KeReleaseSpinLock(&MONITOR_MGR::_MonitorPendingEventTraceLock, v9);
  }
  return this;
}
