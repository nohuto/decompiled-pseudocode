/*
 * XREFs of ?CreateVmBusGuestSubscribers@DXGVAILGUESTOBJECT@@IEAAJXZ @ 0x1C01E6324
 * Callers:
 *     ?Initialize@DXGVAILGUESTOBJECT@@QEAAJXZ @ 0x1C01E6A58 (-Initialize@DXGVAILGUESTOBJECT@@QEAAJXZ.c)
 * Callees:
 *     ??2@YAPEAX_KIHW4_POOL_TYPE@@@Z @ 0x1C000DC30 (--2@YAPEAX_KIHW4_POOL_TYPE@@@Z.c)
 *     ??0DXGAUTOMUTEX@@QEAA@QEAVDXGFASTMUTEX@@E@Z @ 0x1C00167D8 (--0DXGAUTOMUTEX@@QEAA@QEAVDXGFASTMUTEX@@E@Z.c)
 *     ?Release@DXGAUTOMUTEX@@QEAAXXZ @ 0x1C0016828 (-Release@DXGAUTOMUTEX@@QEAAXXZ.c)
 *     ?Acquire@DXGAUTOMUTEX@@QEAAXXZ @ 0x1C001688C (-Acquire@DXGAUTOMUTEX@@QEAAXXZ.c)
 *     _guard_dispatch_icall_nop @ 0x1C001C5F0 (_guard_dispatch_icall_nop.c)
 *     ?RegisterSubscriber@DXGVMBUSCHANNEL@@QEAAJIPEAUIDXGCHANNELSUBSCRIBER@@PEAPEAUIDXGCHANNEL@@@Z @ 0x1C01E55D4 (-RegisterSubscriber@DXGVMBUSCHANNEL@@QEAAJIPEAUIDXGCHANNELSUBSCRIBER@@PEAPEAUIDXGCHANNEL@@@Z.c)
 */

__int64 __fastcall DXGVAILGUESTOBJECT::CreateVmBusGuestSubscribers(DXGVAILGUESTOBJECT *this)
{
  PVOID v2; // rax
  __int64 v3; // rdx
  void **v4; // rcx
  __int64 v5; // r8
  __int64 v6; // rdi
  __int64 v7; // rax
  int v8; // eax
  __int64 v9; // rdx
  __int64 v10; // rcx
  __int64 v11; // r8
  _QWORD *v12; // rax
  __int64 v13; // rdx
  void **v14; // rcx
  __int64 v15; // r8
  __int64 v16; // rax
  DXGVMBUSCHANNEL *v17; // rdi
  __int64 v18; // rsi
  __int64 v19; // rax
  int v20; // eax
  __int64 v21; // rcx
  __int64 v22; // rcx
  _BYTE v24[24]; // [rsp+20h] [rbp-18h] BYREF

  DXGAUTOMUTEX::DXGAUTOMUTEX((DXGAUTOMUTEX *)v24, (DXGVAILGUESTOBJECT *)((char *)this + 16), 1);
  DXGAUTOMUTEX::Acquire((DXGAUTOMUTEX *)v24);
  if ( *(_OWORD *)((char *)this + 72) != 0LL )
  {
    LODWORD(v6) = -1073740528;
    goto LABEL_22;
  }
  v2 = operator new(0x18uLL, 0x4B677844u, 1, PagedPool);
  if ( v2 )
  {
    *((_QWORD *)v2 + 1) = 0LL;
    v4 = &DXG_GUEST_REMOTEOBJECTCHANNEL::`vftable';
    *((_WORD *)v2 + 8) = 0;
    *(_QWORD *)v2 = &DXG_GUEST_REMOTEOBJECTCHANNEL::`vftable';
  }
  *((_QWORD *)this + 9) = v2;
  if ( !v2 )
  {
    LODWORD(v6) = -1073741801;
    v7 = WdLogNewEntry5_WdWarning(v4, v3, v5);
    *(_QWORD *)(v7 + 24) = -1073741801LL;
LABEL_16:
    WdLogEvent5_WdWarning(v7);
LABEL_17:
    v21 = *((_QWORD *)this + 9);
    if ( v21 )
    {
      (*(void (__fastcall **)(__int64))(*(_QWORD *)v21 + 48LL))(v21);
      *((_QWORD *)this + 9) = 0LL;
    }
    v22 = *((_QWORD *)this + 10);
    if ( v22 )
    {
      (*(void (__fastcall **)(__int64))(*(_QWORD *)v22 + 48LL))(v22);
      *((_QWORD *)this + 10) = 0LL;
    }
    goto LABEL_22;
  }
  v8 = DXGVMBUSCHANNEL::RegisterSubscriber(
         *((DXGVMBUSCHANNEL **)this + 8),
         1869901170,
         (struct IDXGCHANNELSUBSCRIBER *)v2,
         (struct IDXGCHANNEL **)v2 + 1);
  v6 = v8;
  if ( v8 < 0 )
    goto LABEL_15;
  v12 = operator new(0x18uLL, 0x4B677844u, 1, PagedPool);
  if ( v12 )
  {
    v12[1] = 0LL;
    v14 = &DXG_GUEST_COMPOSITIONOBJECTCHANNEL::`vftable';
    *((_WORD *)v12 + 8) = 0;
    *v12 = &DXG_GUEST_COMPOSITIONOBJECTCHANNEL::`vftable';
  }
  *((_QWORD *)this + 10) = v12;
  if ( !v12 )
  {
    LODWORD(v6) = -1073741801;
    v16 = WdLogNewEntry5_WdWarning(v14, v13, v15);
    *(_QWORD *)(v16 + 24) = -1073741801LL;
    WdLogEvent5_WdWarning(v16);
  }
  if ( (int)v6 < 0 )
    goto LABEL_17;
  v17 = (DXGVMBUSCHANNEL *)*((_QWORD *)this + 8);
  v18 = *((_QWORD *)this + 10);
  if ( !v17 )
  {
    v19 = WdLogNewEntry5_WdAssertion(v14);
    *(_QWORD *)(v19 + 24) = 538LL;
    WdLogEvent5_WdAssertion(v19);
  }
  v20 = DXGVMBUSCHANNEL::RegisterSubscriber(
          v17,
          1886220131,
          (struct IDXGCHANNELSUBSCRIBER *)v18,
          (struct IDXGCHANNEL **)(v18 + 8));
  v6 = v20;
  if ( v20 < 0 )
  {
LABEL_15:
    v7 = WdLogNewEntry5_WdWarning(v10, v9, v11);
    *(_QWORD *)(v7 + 24) = v6;
    goto LABEL_16;
  }
LABEL_22:
  if ( v24[8] )
    DXGAUTOMUTEX::Release((DXGAUTOMUTEX *)v24);
  return (unsigned int)v6;
}
