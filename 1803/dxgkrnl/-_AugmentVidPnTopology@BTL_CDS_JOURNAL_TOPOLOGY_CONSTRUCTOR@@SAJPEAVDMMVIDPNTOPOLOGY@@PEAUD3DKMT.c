/*
 * XREFs of ?_AugmentVidPnTopology@BTL_CDS_JOURNAL_TOPOLOGY_CONSTRUCTOR@@SAJPEAVDMMVIDPNTOPOLOGY@@PEAUD3DKMT_AUGMENT_CDSJ@@@Z @ 0x1C0229A7C
 * Callers:
 *     ?AugmentTopology@BTL_CDS_JOURNAL_TOPOLOGY_CONSTRUCTOR@@SAJPEAVCCD_TOPOLOGY@@PEAUD3DKMT_AUGMENT_CDSJ@@_N@Z @ 0x1C00D7B0C (-AugmentTopology@BTL_CDS_JOURNAL_TOPOLOGY_CONSTRUCTOR@@SAJPEAVCCD_TOPOLOGY@@PEAUD3DKMT_AUGMENT_C.c)
 * Callees:
 *     ?GetContainer@?$ContainedBy@VDMMVIDPN@@@@QEBAPEAVDMMVIDPN@@XZ @ 0x1C000A86C (-GetContainer@-$ContainedBy@VDMMVIDPN@@@@QEBAPEAVDMMVIDPN@@XZ.c)
 *     ?AddPathToVidPnTopology@VIDPN_MGR@@QEBAJQEAVDMMVIDPNTOPOLOGY@@IIW4_D3DKMDT_VIDPN_PRESENT_PATH_IMPORTANCE@@GEW4_D3DKMDT_MONITOR_CONNECTIVITY_CHECKS@@@Z @ 0x1C00B9938 (-AddPathToVidPnTopology@VIDPN_MGR@@QEBAJQEAVDMMVIDPNTOPOLOGY@@IIW4_D3DKMDT_VIDPN_PRESENT_PATH_IM.c)
 *     ?FindFirstAvailableTarget@DMMVIDPNTOPOLOGY@@QEBAJIW4_D3DKMDT_VIDEO_PRESENT_TARGET_SUBSET_TYPE@@PEAIPEAW4_D3DKMDT_VIDEO_OUTPUT_TECHNOLOGY@@2PEAW4_D3DKMT_DISPLAY_TARGET_USAGE@@@Z @ 0x1C021ECFC (-FindFirstAvailableTarget@DMMVIDPNTOPOLOGY@@QEBAJIW4_D3DKMDT_VIDEO_PRESENT_TARGET_SUBSET_TYPE@@P.c)
 */

__int64 __fastcall BTL_CDS_JOURNAL_TOPOLOGY_CONSTRUCTOR::_AugmentVidPnTopology(
        struct DMMVIDPNTOPOLOGY *a1,
        struct D3DKMT_AUGMENT_CDSJ *a2)
{
  __int64 v3; // r15
  __int64 Container; // rax
  VIDPN_MGR *v6; // rbx
  int FirstAvailableTarget; // eax
  __int64 v8; // rcx
  __int64 v9; // r14
  int v10; // eax
  __int64 v11; // rcx
  __int64 v12; // rcx
  _QWORD *v13; // rsi
  __int64 v14; // rax
  __int64 v15; // rax
  __int64 v16; // rcx
  __int64 v17; // rax
  __int64 v18; // rax
  __int64 v19; // rcx
  _QWORD *v20; // rsi
  __int64 v21; // rax
  __int64 v22; // rax
  __int64 v23; // rcx
  __int64 v24; // rax
  __int64 v25; // rax
  __int64 v26; // rax
  __int64 v27; // rcx
  __int64 v28; // rax
  __int64 v29; // rax
  __int64 v30; // rcx
  __int64 v31; // rax
  __int64 v32; // rax
  __int64 v34; // rcx
  _QWORD *v35; // rsi
  __int64 v36; // rax
  __int64 v37; // rax
  __int64 v38; // rcx
  __int64 v39; // rax
  unsigned int v40; // [rsp+80h] [rbp+40h] BYREF
  int v41; // [rsp+88h] [rbp+48h] BYREF
  int v42; // [rsp+90h] [rbp+50h] BYREF
  int v43; // [rsp+98h] [rbp+58h] BYREF

  LODWORD(v3) = -1;
  *((_DWORD *)a2 + 5) = -2;
  *((_DWORD *)a2 + 4) = -1;
  *((_DWORD *)a2 + 6) = -2;
  Container = ContainedBy<DMMVIDPN>::GetContainer((__int64)a1 + 160);
  v41 = 0;
  v40 = -1;
  v42 = -2;
  v6 = *(VIDPN_MGR **)(Container + 48);
  v43 = -2;
  while ( 1 )
  {
    FirstAvailableTarget = DMMVIDPNTOPOLOGY::FindFirstAvailableTarget(a1, v3, 1, &v40, &v42, &v43, &v41);
    v9 = FirstAvailableTarget;
    if ( FirstAvailableTarget == -1071774925 )
      break;
    if ( FirstAvailableTarget < 0 )
    {
      v20 = (_QWORD *)WdLogNewEntry5_WdError(v8);
      v20[3] = v9;
      v28 = *((_QWORD *)v6 + 1);
      if ( !v28 )
      {
        v29 = WdLogNewEntry5_WdAssertion(v27);
        WdLogEvent5_WdAssertion(v29);
        v28 = *((_QWORD *)v6 + 1);
      }
      v30 = *(int *)(*(_QWORD *)(v28 + 16) + 272LL);
      v20[4] = v30;
      v31 = *((_QWORD *)v6 + 1);
      if ( !v31 )
      {
        v32 = WdLogNewEntry5_WdAssertion(v30);
        WdLogEvent5_WdAssertion(v32);
        v31 = *((_QWORD *)v6 + 1);
      }
      v20[5] = *(unsigned int *)(*(_QWORD *)(v31 + 16) + 268LL);
      v26 = *((unsigned int *)a2 + 3);
LABEL_24:
      v20[6] = v26;
      WdLogEvent5_WdError(v20);
      return (unsigned int)v9;
    }
    if ( v41 )
    {
      LODWORD(v3) = v40;
    }
    else
    {
      v3 = v40;
      v10 = VIDPN_MGR::AddPathToVidPnTopology(
              v6,
              a1,
              *((_DWORD *)a2 + 3),
              v40,
              (enum _D3DKMDT_VIDPN_PRESENT_PATH_IMPORTANCE)255,
              0xFFFFu,
              0,
              D3DKMDT_MCC_ENFORCE);
      v9 = v10;
      if ( v10 != -1071774975 )
      {
        if ( v10 < 0 )
        {
          v20 = (_QWORD *)WdLogNewEntry5_WdError(v11);
          v20[3] = v9;
          v21 = *((_QWORD *)v6 + 1);
          if ( !v21 )
          {
            v22 = WdLogNewEntry5_WdAssertion(v19);
            WdLogEvent5_WdAssertion(v22);
            v21 = *((_QWORD *)v6 + 1);
          }
          v23 = *(int *)(*(_QWORD *)(v21 + 16) + 272LL);
          v20[4] = v23;
          v24 = *((_QWORD *)v6 + 1);
          if ( !v24 )
          {
            v25 = WdLogNewEntry5_WdAssertion(v23);
            WdLogEvent5_WdAssertion(v25);
            v24 = *((_QWORD *)v6 + 1);
          }
          v20[5] = *(unsigned int *)(*(_QWORD *)(v24 + 16) + 268LL);
          v26 = *((unsigned int *)a2 + 3);
          v20[7] = v3;
          goto LABEL_24;
        }
        *((_DWORD *)a2 + 5) = v42;
        *((_DWORD *)a2 + 6) = v43;
        *((_DWORD *)a2 + 4) = v3;
        return (unsigned int)v9;
      }
      v13 = (_QWORD *)WdLogNewEntry5_WdDmmEvent(v11);
      v14 = *((_QWORD *)v6 + 1);
      if ( !v14 )
      {
        v15 = WdLogNewEntry5_WdAssertion(v12);
        WdLogEvent5_WdAssertion(v15);
        v14 = *((_QWORD *)v6 + 1);
      }
      v16 = *(int *)(*(_QWORD *)(v14 + 16) + 272LL);
      v13[3] = v16;
      v17 = *((_QWORD *)v6 + 1);
      if ( !v17 )
      {
        v18 = WdLogNewEntry5_WdAssertion(v16);
        WdLogEvent5_WdAssertion(v18);
        v17 = *((_QWORD *)v6 + 1);
      }
      v13[4] = *(unsigned int *)(*(_QWORD *)(v17 + 16) + 268LL);
      v13[5] = *((unsigned int *)a2 + 3);
      v13[6] = v3;
      WdLogEvent5_WdDmmEvent(v13);
    }
  }
  v35 = (_QWORD *)WdLogNewEntry5_WdDmmEvent(v8);
  v36 = *((_QWORD *)v6 + 1);
  if ( !v36 )
  {
    v37 = WdLogNewEntry5_WdAssertion(v34);
    WdLogEvent5_WdAssertion(v37);
    v36 = *((_QWORD *)v6 + 1);
  }
  v35[3] = *(int *)(*(_QWORD *)(v36 + 16) + 272LL);
  v38 = *((_QWORD *)v6 + 1);
  if ( !v38 )
  {
    v39 = WdLogNewEntry5_WdAssertion(0LL);
    WdLogEvent5_WdAssertion(v39);
    v38 = *((_QWORD *)v6 + 1);
  }
  v35[4] = *(unsigned int *)(*(_QWORD *)(v38 + 16) + 268LL);
  v35[5] = *((unsigned int *)a2 + 3);
  WdLogEvent5_WdDmmEvent(v35);
  return 3223192371LL;
}
