/*
 * XREFs of ??0PATHSTACKOBJ@@QEAA@AEAVXDCOBJ@@H@Z @ 0x1C007BC4C
 * Callers:
 *     GrePolyPolygonInternal @ 0x1C007BA9C (GrePolyPolygonInternal.c)
 *     GreRectangle @ 0x1C00F6F20 (GreRectangle.c)
 *     GrePolylineTo @ 0x1C011B6E0 (GrePolylineTo.c)
 *     ?GreLineTo@@YAHPEAUHDC__@@HH@Z @ 0x1C011B9EC (-GreLineTo@@YAHPEAUHDC__@@HH@Z.c)
 *     GrePolyBezierTo @ 0x1C011C660 (GrePolyBezierTo.c)
 *     ?GrePolyBezierInternal@@YAHPEAUHDC__@@PEAUtagPOINT@@K@Z @ 0x1C026EF74 (-GrePolyBezierInternal@@YAHPEAUHDC__@@PEAUtagPOINT@@K@Z.c)
 *     GreAngleArc @ 0x1C026F194 (GreAngleArc.c)
 *     GrePolyDraw @ 0x1C026F8F8 (GrePolyDraw.c)
 *     GrePolyPolylineInternal @ 0x1C026FE74 (GrePolyPolylineInternal.c)
 *     NtGdiEllipse @ 0x1C0270080 (NtGdiEllipse.c)
 *     NtGdiRoundRect @ 0x1C02702C0 (NtGdiRoundRect.c)
 *     NtGdiArcInternal @ 0x1C02818C0 (NtGdiArcInternal.c)
 * Callees:
 *     ?vQuickInit@EXFORMOBJ@@QEAAXAEAVXDCOBJ@@K@Z @ 0x1C009C134 (-vQuickInit@EXFORMOBJ@@QEAAXAEAVXDCOBJ@@K@Z.c)
 *     ??1XEPATHOBJ@@QEAA@XZ @ 0x1C011CF20 (--1XEPATHOBJ@@QEAA@XZ.c)
 *     ?ptfxGetCurrent@EPATHOBJ@@QEBA?AU_POINTFIX@@XZ @ 0x1C011D14C (-ptfxGetCurrent@EPATHOBJ@@QEBA-AU_POINTFIX@@XZ.c)
 *     ?vLock@EPATHOBJ@@QEAAXPEAUHPATH__@@@Z @ 0x1C011D4B4 (-vLock@EPATHOBJ@@QEAAXPEAUHPATH__@@@Z.c)
 *     ??0XEPATHOBJ@@QEAA@PEAUHPATH__@@@Z @ 0x1C0257180 (--0XEPATHOBJ@@QEAA@PEAUHPATH__@@@Z.c)
 */

PATHSTACKOBJ *__fastcall PATHSTACKOBJ::PATHSTACKOBJ(PATHSTACKOBJ *this, DC **a2, int a3)
{
  int v6; // eax
  __int64 v7; // r8
  DC *v9; // rdx
  __int64 v10; // rcx
  int v11; // eax
  struct HPATH__ *v12; // rdx
  _BYTE v13[16]; // [rsp+20h] [rbp-E0h] BYREF
  _BYTE v14[8]; // [rsp+30h] [rbp-D0h] BYREF
  struct HPATH__ **v15; // [rsp+38h] [rbp-C8h]
  _BYTE v16[8]; // [rsp+B0h] [rbp-50h] BYREF
  __int64 v17; // [rsp+B8h] [rbp-48h]
  int v18; // [rsp+160h] [rbp+60h]
  int v19; // [rsp+164h] [rbp+64h]

  *((_QWORD *)this + 9) = 0LL;
  *((_DWORD *)this + 28) = 0;
  *((_QWORD *)this + 2) = 0LL;
  *((_QWORD *)this + 3) = 0LL;
  *((_QWORD *)this + 4) = 0LL;
  *((_QWORD *)this + 5) = 0LL;
  *((_QWORD *)this + 7) = 0LL;
  *((_QWORD *)this + 8) = 0LL;
  *((_QWORD *)this + 6) = 0LL;
  v6 = *((_DWORD *)*a2 + 64);
  if ( (v6 & 1) != 0 )
  {
    if ( (v6 & 2) != 0 )
    {
      *((_DWORD *)*a2 + 64) = v6 & 0xFFFFFFFD;
      XEPATHOBJ::XEPATHOBJ((XEPATHOBJ *)v16, *((struct HPATH__ **)*a2 + 26));
      PATHMEMOBJ::PATHMEMOBJ((PATHMEMOBJ *)v14);
      if ( v15 && v17 && EPATHOBJ::bClone((EPATHOBJ *)v14, (struct EPATHOBJ *)v16) )
        v12 = *v15;
      else
        v12 = 0LL;
      DC::hpath(*a2, v12);
      PATHMEMOBJ::~PATHMEMOBJ((PATHMEMOBJ *)v14);
      XEPATHOBJ::~XEPATHOBJ((XEPATHOBJ *)v16);
    }
    *((_QWORD *)this + 1) = 0LL;
    if ( !*((_DWORD *)this + 28) )
    {
      PushThreadGuardedObject((char *)this + 80, this, XEPATHOBJ_vDestructWrap);
      *((_DWORD *)this + 28) = 1;
    }
    EPATHOBJ::vLock(this, *((struct HPATH__ **)*a2 + 26));
    if ( *((_QWORD *)this + 1) && a3 )
    {
      if ( (*(_DWORD *)(*((_QWORD *)*a2 + 10) + 8LL) & 0x200) != 0 )
      {
        EXFORMOBJ::vQuickInit((EXFORMOBJ *)v13, (struct XDCOBJ *)a2, 0x204u);
        EXFORMOBJ::bXformRound(
          (EXFORMOBJ *)v13,
          (struct _POINTL *)(*((_QWORD *)*a2 + 10) + 76LL),
          (struct _POINTFIX *)(*((_QWORD *)*a2 + 10) + 84LL),
          1uLL);
        *(_DWORD *)(*((_QWORD *)*a2 + 10) + 8LL) &= ~0x200u;
        v7 = *((_QWORD *)*a2 + 10);
LABEL_9:
        EPATHOBJ::bMoveTo(this, 0LL, (struct _POINTL *)(v7 + 84));
        return this;
      }
      EPATHOBJ::ptfxGetCurrent(this);
      if ( *(_DWORD *)(v7 + 84) != v18 || *(_DWORD *)(v7 + 88) != v19 )
        goto LABEL_9;
    }
  }
  else
  {
    *(_QWORD *)this = 0LL;
    *((_QWORD *)this + 1) = (char *)this + 120;
    *((_QWORD *)this + 18) = (char *)this + 440;
    *((_QWORD *)this + 55) = 0LL;
    *((_QWORD *)this + 56) = (char *)this + 464;
    *((_DWORD *)this + 50) = 1;
    *((_QWORD *)this + 19) = 0LL;
    *((_QWORD *)this + 20) = 0LL;
    *((_QWORD *)this + 21) = 0LL;
    *((_QWORD *)this + 22) = 0LL;
    *((_DWORD *)this + 51) = 1;
    *((_DWORD *)this + 114) = 256;
    if ( a3 )
    {
      v9 = *a2;
      if ( (*(_DWORD *)(*((_QWORD *)v9 + 10) + 8LL) & 0x200) != 0 )
      {
        EXFORMOBJ::vQuickInit((EXFORMOBJ *)v13, (struct XDCOBJ *)a2, 0x204u);
        EXFORMOBJ::bXformRound(
          (EXFORMOBJ *)v13,
          (struct _POINTL *)(*((_QWORD *)*a2 + 10) + 76LL),
          (struct _POINTFIX *)(*((_QWORD *)*a2 + 10) + 84LL),
          1uLL);
        *(_DWORD *)(*((_QWORD *)*a2 + 10) + 8LL) &= ~0x200u;
        v9 = *a2;
      }
      *((_QWORD *)this + 23) = *(_QWORD *)(*((_QWORD *)v9 + 10) + 84LL);
      v10 = *((_QWORD *)*a2 + 10);
      v11 = *(_DWORD *)(v10 + 8);
      if ( (v11 & 0x400) != 0 )
      {
        *(_DWORD *)(v10 + 8) = v11 & 0xFFFFFBFF;
        *((_DWORD *)*a2 + 62) = 0;
      }
    }
  }
  return this;
}
