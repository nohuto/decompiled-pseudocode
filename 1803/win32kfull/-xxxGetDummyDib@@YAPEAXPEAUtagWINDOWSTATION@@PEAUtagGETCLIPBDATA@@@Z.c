/*
 * XREFs of ?xxxGetDummyDib@@YAPEAXPEAUtagWINDOWSTATION@@PEAUtagGETCLIPBDATA@@@Z @ 0x1C0130EE8
 * Callers:
...
void *__fastcall xxxGetDummyDib(struct tagWINDOWSTATION *a1, struct tagGETCLIPBDATA *a2)
{
  ...
  {
    ClipboardData = (HPALETTE)xxxGetClipboardData(a1, 9LL, a2);
    if ( !ClipboardData )
      return 0LL;
  }
  ...
  if ( ClipFormat )
  {
    ...
    {
      result = xxxGetClipboardData(a1, 17LL, a2);
      if ( result )
      {
        *(_DWORD *)a2 = 17;
  ...
  if ( *(_QWORD *)(v6 + 8) == 2LL )
    return 0LL;
  v7 = (HBITMAP)xxxGetClipboardData(a1, 2LL, a2);
  if ( !v7 )
    return 0LL;
  v8 = 0LL;
