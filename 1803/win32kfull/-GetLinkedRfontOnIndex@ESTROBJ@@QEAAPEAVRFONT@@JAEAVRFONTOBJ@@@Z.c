/*
 * XREFs of ?GetLinkedRfontOnIndex@ESTROBJ@@QEAAPEAVRFONT@@JAEAVRFONTOBJ@@@Z @ 0x1C028BAD8
 * Callers:
 *     ?bExtraRectsToPath@ESTROBJ@@QEAAHAEAVEPATHOBJ@@@Z @ 0x1C028BD5C (-bExtraRectsToPath@ESTROBJ@@QEAAHAEAVEPATHOBJ@@@Z.c)
 *     ?vInitSimple@ESTROBJ@@QEAAXPEAGJAEAVXDCOBJ@@AEAVRFONTOBJ@@JJPEAX@Z @ 0x1C028D504 (-vInitSimple@ESTROBJ@@QEAAXPEAGJAEAVXDCOBJ@@AEAVRFONTOBJ@@JJPEAX@Z.c)
 * Callees:
 *     <none>
 */

struct RFONT *__fastcall ESTROBJ::GetLinkedRfontOnIndex(ESTROBJ *this, int a2, struct RFONTOBJ *a3)
{
  int v3; // r9d
  _QWORD *v4; // rdx

  if ( (*((_DWORD *)this + 58) & 0x1400) == 0 )
    return *(struct RFONT **)a3;
  v3 = *(_DWORD *)(*((_QWORD *)this + 26) + 4LL * a2);
  v4 = *(_QWORD **)a3;
  if ( v3 )
  {
    switch ( v3 )
    {
      case 1:
        return (struct RFONT *)v4[88];
      case 2:
        return (struct RFONT *)v4[89];
      case 3:
        return (struct RFONT *)v4[90];
      default:
        return *(struct RFONT **)(v4[91] + 8LL * (unsigned int)(v3 - 4));
    }
  }
  return (struct RFONT *)v4;
}
