/*
 * XREFs of Display_PSS @ 0x1C0006910
 * Callers:
...
__int64 __fastcall Display_PSS(unsigned int *a1, __int64 a2, __int64 a3)
{
  ...
  if ( a1 )
  {
    ...
    for ( i = 0; i < *a1; ++i )
    {
      ...
      {
        v7 = *(_QWORD *)&a1[12 * i + 6];
        result = WPP_RECORDER_SF_x((__int64)WPP_GLOBAL_Control->DeviceExtension, a2, a3, 0x5Au, v6);
      }
      if ( LOWORD(WPP_GLOBAL_Control->DeviceType) )
      {
        v7 = *(_QWORD *)&a1[12 * i + 8];
        result = WPP_RECORDER_SF_x((__int64)WPP_GLOBAL_Control->DeviceExtension, a2, a3, 0x5Bu, v6);
      }
      if ( LOWORD(WPP_GLOBAL_Control->DeviceType) )
      {
        v7 = *(_QWORD *)&a1[12 * i + 10];
        result = WPP_RECORDER_SF_x((__int64)WPP_GLOBAL_Control->DeviceExtension, a2, a3, 0x5Cu, v6);
      }
      if ( LOWORD(WPP_GLOBAL_Control->DeviceType) )
      {
        v7 = *(_QWORD *)&a1[12 * i + 12];
        result = WPP_RECORDER_SF_x((__int64)WPP_GLOBAL_Control->DeviceExtension, a2, a3, 0x5Du, v6);
      }
      if ( LOWORD(WPP_GLOBAL_Control->DeviceType) )
        result = WPP_RECORDER_SF_(
                   (__int64)WPP_GLOBAL_Control->DeviceExtension,
