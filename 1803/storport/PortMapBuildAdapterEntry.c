/*
 * XREFs of PortMapBuildAdapterEntry @ 0x1C00480C8
 * Callers:
 *     RaidAdapterCreateDevmapEntry @ 0x1C0025A6C (RaidAdapterCreateDevmapEntry.c)
 * Callees:
 *     PortRegistrySetValueKey @ 0x1C0016D48 (PortRegistrySetValueKey.c)
 *     PortRegistryCreateKeyEx @ 0x1C0016F18 (PortRegistryCreateKeyEx.c)
 */

__int64 __fastcall PortMapBuildAdapterEntry(
        void *a1,
        int a2,
        int a3,
        int a4,
        int a5,
        char **a6,
        HANDLE Handle,
        _QWORD *a8)
{
  __int64 result; // rax
  HANDLE v9; // rbx
  int v10; // [rsp+30h] [rbp-18h]
  int v11; // [rsp+60h] [rbp+18h] BYREF
  int v12; // [rsp+68h] [rbp+20h] BYREF

  v12 = a4;
  v11 = a3;
  v10 = 0;
  result = PortRegistryCreateKeyEx(a1, 1u, &Handle, L"Scsi Port %d", a2);
  if ( (int)result >= 0 )
  {
    v9 = Handle;
    if ( v11 )
      PortRegistrySetValueKey(Handle, L"Interrupt", 4u, (char *)&v11, 4u);
    v12 = 1;
    PortRegistrySetValueKey(v9, L"Dma64BitAddresses", 4u, (char *)&v12, 4u);
    if ( a6 )
      PortRegistrySetValueKey(v9, L"Driver", 1u, a6[1], *(unsigned __int16 *)a6 + 2);
    if ( a8 )
      *a8 = v9;
    else
      ZwClose(v9);
    return 0LL;
  }
  return result;
}
