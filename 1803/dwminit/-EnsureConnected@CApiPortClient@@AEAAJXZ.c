/*
 * XREFs of ?EnsureConnected@CApiPortClient@@AEAAJXZ @ 0x18000466C
 * Callers:
 ...
 *     ??0CPortClient@@QEAA@IPEAX@Z @ 0x180004900 (--0CPortClient@@QEAA@IPEAX@Z.c)
...
__int64 __fastcall CApiPortClient::EnsureConnected(CApiPortClient *this)
{
  ...
  else
  {
    ...
    if ( v5 )
    {
      ...
      {
        MilInstrumentationCheckHR_MaybeFailFast(4u, &CApiPortClient::MILINSTRUMENTATIONHRESULTLIST, 1LL, v6, 0x110u);
      }
      else
      {
        ...
        if ( v7 < 0 )
        {
          MilInstrumentationCheckHR_MaybeFailFast(4u, &CApiPortClient::MILINSTRUMENTATIONHRESULTLIST, 1LL, v7, 0x117u);
        }
        else
        {
