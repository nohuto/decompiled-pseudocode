/*
 * XREFs of ?AdjustTargetRect@CAnimatedGlassSheet@@AEAAJXZ @ 0x180086DD0
 * Callers:
...
__int64 __fastcall CAnimatedGlassSheet::AdjustTargetRect(RECT *this)
{
  ...
  if ( v4 )
  {
    ...
    if ( GetMonitorInfoW(v4, &mi) )
    {
      ...
      {
        left -= (int)(*((double *)CDesktopManager::s_pDesktopManagerInstance + 55) * -8.0);
        this[26].left = left;
      }
      right = this[26].right;
      if ( mi.rcWork.right == right )
      {
        right -= (int)(*((double *)v7 + 55) * 8.0);
        this[26].right = right;
      }
      top = this[26].top;
      if ( mi.rcWork.top == top )
      {
        top -= (int)(*((double *)v7 + 55) * -8.0);
        this[26].top = top;
      }
      bottom = this[26].bottom;
      if ( mi.rcWork.bottom == bottom )
      {
        bottom -= (int)(*((double *)v7 + 55) * 8.0);
        this[26].bottom = bottom;
      }
    ...
    else
    {
      ...
      v3 = LastError;
      MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, LastError, 0x232u);
    }
  }
  return v3;
}
