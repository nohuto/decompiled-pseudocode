/*
 * XREFs of ??0CDWMDisplay@@AEAA@PEAVCDWMDXGIAdapter@@AEBUDXGIOutputInfo@@@Z @ 0x180034EF4
 * Callers:
 *     ?EnumerateOutputs@CDWMDisplaySet@@AEAAJXZ @ 0x180033FD8 (-EnumerateOutputs@CDWMDisplaySet@@AEAAJXZ.c)
 * Callees:
 *     ?ReportUsageToService@?$Feature@U__WilFeatureTraits_Feature_RgbaSwapChains@@@wil@@CAX_NW4ReportingKind@2@_K@Z @ 0x180035208 (-ReportUsageToService@-$Feature@U__WilFeatureTraits_Feature_RgbaSwapChains@@@wil@@CAX_NW4Reporti.c)
 *     ?ForceHighColor@@YAKXZ @ 0x180035570 (-ForceHighColor@@YAKXZ.c)
 *     __security_check_cookie @ 0x180048EF0 (__security_check_cookie.c)
 *     memset_0 @ 0x18004B360 (memset_0.c)
 *     _guard_dispatch_icall_nop @ 0x18004B390 (_guard_dispatch_icall_nop.c)
 */

CDWMDisplay *__fastcall CDWMDisplay::CDWMDisplay(
        CDWMDisplay *this,
        struct CDWMDXGIAdapter *a2,
        const struct DXGIOutputInfo *a3)
{
  _WORD *v6; // rcx
  __int64 v7; // rdx
  __int16 v8; // ax
  _WORD *v9; // rax
  int v10; // eax
  int v11; // esi
  int v12; // r14d
  int v13; // eax
  int v14; // eax
  CDWMDisplay *result; // rax
  __int128 v16; // [rsp+20h] [rbp-89h] BYREF
  _BYTE v17[96]; // [rsp+30h] [rbp-79h] BYREF
  __int64 v18; // [rsp+90h] [rbp-19h]

  *(_QWORD *)&v16 = 0LL;
  memset_0(v17, 0, 0x98uLL);
  *(_DWORD *)this = 1;
  v6 = (_WORD *)((char *)this + 96);
  v7 = 32LL;
  *((_BYTE *)this + 164) = (*((_DWORD *)a3 + 50) & 2) != 0;
  *((_QWORD *)this + 21) = *((_QWORD *)a2 + 42);
  *((_DWORD *)this + 44) = *((_DWORD *)a3 + 32);
  *((_QWORD *)this + 2) = *((_QWORD *)a3 + 13);
  *((_QWORD *)this + 11) = 0LL;
  *((_BYTE *)this + 220) = (*((_DWORD *)a3 + 50) & 4) != 0;
  *((_BYTE *)this + 221) = (*((_DWORD *)a3 + 50) & 8) != 0;
  do
  {
    v8 = *(_WORD *)((char *)v6 + a3 - (CDWMDisplay *)((char *)this + 96) + 204);
    if ( !v8 )
      break;
    *v6++ = v8;
    --v7;
  }
  while ( v7 );
  v9 = v6 - 1;
  if ( v7 )
    v9 = v6;
  *v9 = 0;
  if ( ((*((_DWORD *)a3 + 40) - 2) & 0xFFFFFFFD) != 0 )
  {
    *((_DWORD *)this + 45) = *((_DWORD *)a3 + 35);
    v10 = *((_DWORD *)a3 + 36);
  }
  else
  {
    *((_DWORD *)this + 45) = *((_DWORD *)a3 + 36);
    v10 = *((_DWORD *)a3 + 35);
  }
  *((_DWORD *)this + 46) = v10;
  if ( (***(int (__fastcall ****)(_QWORD, GUID *, __int128 *))a3)(
         *(_QWORD *)a3,
         &GUID_068346e8_aaec_4b84_add7_137f513f77a1,
         &v16) < 0
    || (*(int (__fastcall **)(_QWORD, _BYTE *))(*(_QWORD *)v16 + 216LL))(v16, v17) < 0 )
  {
    v18 = 8LL;
  }
  if ( (_QWORD)v16 )
  {
    (*(void (__fastcall **)(_QWORD))(*(_QWORD *)v16 + 16LL))(v16);
    *(_QWORD *)&v16 = 0LL;
  }
  v11 = 12;
  v12 = 10;
  if ( ForceHighColor() )
  {
    v13 = 12;
    v18 = 0xC0000000ALL;
  }
  else
  {
    v13 = HIDWORD(v18);
  }
  if ( v13 >= 12 && (v13 <= 14 || v13 == 16) )
  {
    if ( *((_DWORD *)a2 + 74) == 32902 && ForceHighColor() != 3 || ForceHighColor() == 2 )
      v12 = 24;
    else
      v11 = 1;
    *((_DWORD *)this + 49) = v12;
  }
  else
  {
    wil::Feature<__WilFeatureTraits_Feature_RgbaSwapChains>::ReportUsageToService();
    *((_DWORD *)this + 49) = 87;
    v11 = 0;
  }
  *((_DWORD *)this + 53) = v11;
  *((_DWORD *)this + 47) = *((_DWORD *)a3 + 38);
  *((_DWORD *)this + 48) = *((_DWORD *)a3 + 39);
  *((_DWORD *)this + 52) = *((_DWORD *)a3 + 40);
  *((_DWORD *)this + 54) = *((_DWORD *)a3 + 67);
  *((_DWORD *)this + 50) = *((_DWORD *)a3 + 41);
  *((_DWORD *)this + 51) = 0;
  *((_QWORD *)this + 3) = 0LL;
  *((_DWORD *)this + 8) = *((_DWORD *)a3 + 35);
  *((_DWORD *)this + 9) = *((_DWORD *)a3 + 36);
  v14 = *((_DWORD *)a3 + 46);
  HIDWORD(v16) = *((_DWORD *)a3 + 49);
  LODWORD(v16) = v14;
  *(_QWORD *)((char *)&v16 + 4) = *(_QWORD *)((char *)a3 + 188);
  result = this;
  *(_OWORD *)((char *)this + 56) = v16;
  v16 = *(_OWORD *)((char *)a3 + 168);
  *(_OWORD *)((char *)this + 40) = v16;
  return result;
}
