/*
 * XREFs of ?_ScheduleStoryboardsForAnimationClock@CAnimationScheduler@@AEAAJU_GUID@@@Z @ 0x18000B994
 * Callers:
      ...
      if ( (unsigned int)v11 >= *((_DWORD *)this + 10) )
      {
        if ( v3 >= 0 && v10 )
        {
          ...
          else
          {
            ...
            v3 = started;
            if ( started < 0 )
              MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0, started, 0x276u);
            else
              CWindowList::UpdateScene(*((CWindowList **)CDesktopManager::s_pDesktopManagerInstance + 56));
          }
        }
