/*
 * XREFs of ?OnEndTransitionRequest@CGrowPanel@@UEAAJXZ @ 0x18009C7A0
 * Callers:
 *     <none>
 * Callees:
...
__int64 __fastcall CGrowPanel::OnEndTransitionRequest(CGrowPanel *this)
{
  ...
  if ( v2 >= 0 )
  {
    ...
    v3 = v4;
    if ( v4 < 0 )
      MilInstrumentationCheckHR_MaybeFailFast(0x14u, &CStoryboard::MILINSTRUMENTATIONHRESULTLIST, 1LL, v4, 0x59Du);
  }
  else
  {
    MilInstrumentationCheckHR_MaybeFailFast(0x14u, &CStoryboard::MILINSTRUMENTATIONHRESULTLIST, 1LL, v2, 0x59Cu);
  }
  return v3;
}
