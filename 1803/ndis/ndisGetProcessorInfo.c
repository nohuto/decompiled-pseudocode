/*
 * XREFs of ndisGetProcessorInfo @ 0x1C011E710
 * Callers:
...
__int64 ndisGetProcessorInfo()
{
  ...
  v0 = 20 * (ndisMaxNumberOfProcessors + 2LL + 2LL * HIDWORD(WPP_MAIN_CB.Queue.Wcb.DeviceContext));
  ...
  LODWORD(v2->Blink) = 0;
LABEL_6:
  WPP_MAIN_CB.Queue.Wcb.DeviceRoutine = (_IO_ALLOCATION_ACTION (__fastcall *)(_DEVICE_OBJECT *, _IRP *, void *, void *))ExAllocatePoolWithTag(NonPagedPoolNx, 4LL * HIDWORD(WPP_MAIN_CB.Queue.Wcb.DeviceContext), 0x2020444Eu);
  ...
  *(_QWORD *)&WPP_MAIN_CB.DeviceType = (char *)Flink + 40;
  ...
  if ( LogicalProcessorInformation < 0 )
  {
    v15 = ndisRssBaseCpu;
    for ( i = 0LL;
          v15 < ndisNumberOfActiveProcessorsAtBoot && (unsigned int)i < HIDWORD(WPP_MAIN_CB.Queue.Wcb.DeviceContext);
          i = (unsigned int)(i + 1) )
    {
      KeGetProcessorNumberFromIndex(v15++, (PPROCESSOR_NUMBER)WPP_MAIN_CB.Queue.Wcb.DeviceRoutine + i);
    }
    *((_DWORD *)&WPP_MAIN_CB.Queue.Wcb.1 + 5) = i;
    return 0;
  }
  else
  {
    v10 = ndisRssBaseCpu;
    v11 = 0LL;
    while ( v10 < ndisNumberOfActiveProcessorsAtBoot && (unsigned int)v11 < HIDWORD(WPP_MAIN_CB.Queue.Wcb.DeviceContext) )
    {
      for ( j = 0LL; (unsigned int)j < (unsigned int)v11; j = (unsigned int)(j + 1) )
      {
        ProcessorIndexFromNumber = KeGetProcessorIndexFromNumber((PPROCESSOR_NUMBER)WPP_MAIN_CB.Queue.Wcb.DeviceRoutine + j);
      ...
      if ( (_DWORD)j == (_DWORD)v11 )
      {
        LogicalProcessorInformation = KeGetProcessorNumberFromIndex(
                                        v10,
                                        (PPROCESSOR_NUMBER)WPP_MAIN_CB.Queue.Wcb.DeviceRoutine + v11);
        v11 = (unsigned int)(v11 + 1);
      }
      ++v10;
    }
    *((_DWORD *)&WPP_MAIN_CB.Queue.Wcb.1 + 5) = v11;
  }
