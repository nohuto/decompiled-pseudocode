/*
 * XREFs of ndisReturnQueuedLowPowerNbls @ 0x1C000F744
 * Callers:
 *     ndisPnPIrpSurpriseRemovalInner @ 0x1C00EE3E8 (ndisPnPIrpSurpriseRemovalInner.c)
 *     ndisDevicePowerOn @ 0x1C0100560 (ndisDevicePowerOn.c)
 * Callees:
 *     ?ndisNblTrackerTransferOwnershipInternal@@YAXPEAU_NET_BUFFER_LIST@@PEAUNDIS_NBL_TRACKER_HANDLE__@@1W4_NDIS_NBL_TRACKER_OWNERSHIP_EVENT@@K@Z @ 0x1C0002064 (-ndisNblTrackerTransferOwnershipInternal@@YAXPEAU_NET_BUFFER_LIST@@PEAUNDIS_NBL_TRACKER_HANDLE__.c)
 *     _guard_dispatch_icall_nop @ 0x1C0025E10 (_guard_dispatch_icall_nop.c)
 */

void __fastcall ndisReturnQueuedLowPowerNbls(__int64 a1, char a2)
{
  KSPIN_LOCK *v2; // rsi
  KIRQL v5; // bp
  struct _NET_BUFFER_LIST *v6; // rcx

  v2 = (KSPIN_LOCK *)(a1 + 3896);
  v5 = KeAcquireSpinLockRaiseToDpc((PKSPIN_LOCK)(a1 + 3896));
  if ( a2 )
    *(_BYTE *)(a1 + 3924) = 1;
  v6 = *(struct _NET_BUFFER_LIST **)(a1 + 3912);
  if ( v6 )
  {
    if ( ndisNblTrackerMode )
    {
      ndisNblTrackerTransferOwnershipInternal(v6, 0LL, *(_QWORD *)(a1 + 4104), 141LL, 1u);
      v6 = *(struct _NET_BUFFER_LIST **)(a1 + 3912);
    }
    (*(void (__fastcall **)(_QWORD, struct _NET_BUFFER_LIST *, __int64))(*(_QWORD *)(a1 + 3784) + 192LL))(
      *(_QWORD *)(a1 + 24),
      v6,
      1LL);
    *(_QWORD *)(a1 + 3912) = 0LL;
  }
  KeReleaseSpinLock(v2, v5);
}
