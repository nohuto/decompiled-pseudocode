/*
 * XREFs of ndisOidPostRcvFilterMoveFilter @ 0x1C003B8F0
 * Callers:
 *     <none>
 * Callees:
 *     ndisFindReceiveFilterByFilterId @ 0x1C003B4D8 (ndisFindReceiveFilterByFilterId.c)
 *     ndisIovFindVPortByVPortId @ 0x1C006C8D8 (ndisIovFindVPortByVPortId.c)
 */

__int64 __fastcall ndisOidPostRcvFilterMoveFilter(__int64 a1)
{
  __int64 result; // rax
  __int64 v3; // rsi
  __int64 v4; // rbp
  unsigned int v5; // r14d
  __int64 *ReceiveFilterByFilterId; // rax
  __int64 *v7; // r14
  KIRQL v8; // r10
  __int64 v9; // rbx
  _QWORD *v10; // rcx
  __int64 v11; // r8
  __int64 *v12; // rdx
  __int64 v13; // rdx
  _QWORD *v14; // rdx
  __int64 v15; // r8
  __int64 v16; // rcx

  result = *(_QWORD *)(a1 + 32);
  v3 = *(_QWORD *)a1;
  v4 = *(_QWORD *)(result + 40);
  v5 = *(_DWORD *)(v4 + 4);
  if ( *(_QWORD *)a1 )
  {
    result = *(unsigned int *)(a1 + 40);
    if ( !(_DWORD)result || (_DWORD)result == 65539 && (result = *(unsigned int *)(v3 + 120), (result & 4) != 0) )
    {
      KeAcquireSpinLockRaiseToDpc((PKSPIN_LOCK)(v3 + 96));
      *(_QWORD *)(v3 + 520) = KeGetCurrentThread();
      *(_DWORD *)(v3 + 1856) = 2690850;
      ReceiveFilterByFilterId = ndisFindReceiveFilterByFilterId(v3, v5);
      *(_QWORD *)(v3 + 520) = 0LL;
      *(_DWORD *)(v3 + 1856) = 0;
      v7 = ReceiveFilterByFilterId;
      KeReleaseSpinLock((PKSPIN_LOCK)(v3 + 96), v8);
      v9 = v7[22];
      result = ndisIovFindVPortByVPortId(v3, *(unsigned int *)(v4 + 20));
      v10 = v7 + 4;
      v11 = v7[4];
      if ( *(__int64 **)(v11 + 8) != v7 + 4 || (v12 = (__int64 *)v7[5], (_QWORD *)*v12 != v10) )
        __fastfail(3u);
      *v12 = v11;
      *(_QWORD *)(v11 + 8) = v12;
      --*(_DWORD *)(v9 + 864);
      v13 = *(_QWORD *)(v9 + 88);
      if ( *(_DWORD *)(v9 + 124) )
        --*(_DWORD *)(v13 + 656);
      else
        --*(_DWORD *)(v13 + 652);
      v14 = (_QWORD *)(result + 872);
      v15 = *(_QWORD *)(result + 872);
      if ( *(_QWORD *)(v15 + 8) != result + 872 )
        __fastfail(3u);
      *v10 = v15;
      v7[5] = (__int64)v14;
      *(_QWORD *)(v15 + 8) = v10;
      *v14 = v10;
      ++*(_DWORD *)(result + 864);
      v16 = *(_QWORD *)(result + 88);
      if ( *(_DWORD *)(result + 124) )
        ++*(_DWORD *)(v16 + 656);
      else
        ++*(_DWORD *)(v16 + 652);
      v7[22] = result;
      *(_DWORD *)(a1 + 40) = 0;
    }
  }
  return result;
}
