/*
 * XREFs of ??$_Insert@AEAU?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@@std@@@std@@@2@@?$_Hash@V?$_Umap_traits@KVMessageInfo@DragNDropProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@@4@$0A@@std@@@std@@IEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@@std@@@std@@@std@@_N@1@AEAU?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@1@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@@std@@@std@@@1@@Z @ 0x180065C38
 * Callers:
 *     ??A?$unordered_map@KVMessageInfo@DragNDropProcessor@@U?$hash@K@std@@U?$equal_to@K@4@V?$allocator@U?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@@4@@std@@QEAAAEAVMessageInfo@DragNDropProcessor@@AEBK@Z @ 0x1800657D0 (--A-$unordered_map@KVMessageInfo@DragNDropProcessor@@U-$hash@K@std@@U-$equal_to@K@4@V-$allocator.c)
 *     ??$_Insert@AEAU?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@@std@@@std@@@2@@?$_Hash@V?$_Umap_traits@KVMessageInfo@DragNDropProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@@4@$0A@@std@@@std@@IEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@@std@@@std@@@std@@_N@1@AEAU?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@1@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@@std@@@std@@@1@@Z @ 0x180065C38 (--$_Insert@AEAU-$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@V-$_List_unchecked_iterator@V-.c)
 * Callees:
 *     ?_Init@?$_Hash@V?$_Umap_traits@K_KV?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBK_K@std@@@2@$0A@@std@@@std@@IEAAX_K@Z @ 0x18000A408 (-_Init@-$_Hash@V-$_Umap_traits@K_KV-$_Uhash_compare@KU-$hash@K@std@@U-$equal_to@K@2@@std@@V-$all.c)
 *     ??1MessageInfo@DragNDropProcessor@@QEAA@XZ @ 0x1800638B8 (--1MessageInfo@DragNDropProcessor@@QEAA@XZ.c)
 *     ??$_Insert@AEAU?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@@std@@@std@@@2@@?$_Hash@V?$_Umap_traits@KVMessageInfo@DragNDropProcessor@@V?$_Uhash_compare@KU?$hash@K@std@@U?$equal_to@K@2@@std@@V?$allocator@U?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@@4@$0A@@std@@@std@@IEAA?AU?$pair@V?$_List_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@@std@@@std@@@std@@_N@1@AEAU?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@1@V?$_List_unchecked_iterator@V?$_List_val@U?$_List_simple_types@U?$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@@std@@@std@@@1@@Z @ 0x180065C38 (--$_Insert@AEAU-$pair@$$CBKVMessageInfo@DragNDropProcessor@@@std@@V-$_List_unchecked_iterator@V-.c)
 *     ??3@YAXPEAX@Z @ 0x1800E1CE8 (--3@YAXPEAX@Z.c)
 */

__int64 __fastcall std::_Hash<std::_Umap_traits<unsigned long,DragNDropProcessor::MessageInfo,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,DragNDropProcessor::MessageInfo>>,0>>::_Insert<std::pair<unsigned long const,DragNDropProcessor::MessageInfo> &,std::_List_unchecked_iterator<std::_List_val<std::_List_simple_types<std::pair<unsigned long const,DragNDropProcessor::MessageInfo>>>>>(
        float *a1,
        __int64 a2,
        unsigned int *a3,
        __int64 *a4)
{
  __int64 v6; // r10
  __int64 v8; // rcx
  unsigned __int64 i; // r9
  __int64 v10; // rdi
  __int64 v11; // r8
  __int64 v12; // rax
  __int64 result; // rax
  __int64 v14; // rax
  _QWORD *v15; // rdx
  __int64 v16; // rdx
  __int64 v17; // rax
  __int64 *v18; // rax
  __int64 v19; // rdx
  __int64 v20; // rax
  __int64 v21; // rdx
  float v22; // xmm0_4
  float v23; // xmm2_4
  _QWORD *v24; // rax
  __int64 v25; // rdi
  _QWORD *iter; // rax
  __int64 v27; // [rsp+20h] [rbp-48h] BYREF
  __int64 v28[2]; // [rsp+28h] [rbp-40h] BYREF
  _BYTE v29[48]; // [rsp+38h] [rbp-30h] BYREF

  v28[1] = -2LL;
  v6 = a2;
  try
  {
    v8 = 0xCBF29CE484222325uLL;
    for ( i = 0LL; i < 4; ++i )
      v8 = 0x100000001B3LL * (*((unsigned __int8 *)a3 + i) ^ (unsigned __int64)v8);
  }
  catch ( ... )
  {
    std::_Hash<std::_Umap_traits<unsigned long,DragNDropProcessor::MessageInfo,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,DragNDropProcessor::MessageInfo>>,0>>::_Destroy_if_node(
      a1,
      a4);
    throw;
  }
  v10 = *((_QWORD *)a1 + 1);
  v11 = 2 * (v8 & *((_QWORD *)a1 + 6));
  v12 = *((_QWORD *)a1 + 3);
  if ( v10 == *(_QWORD *)(v12 + 16 * (v8 & *((_QWORD *)a1 + 6)))
    || (v10 = **(_QWORD **)(v12 + 16 * (v8 & *((_QWORD *)a1 + 6)) + 8),
        v10 == *(_QWORD *)(v12 + 16 * (v8 & *((_QWORD *)a1 + 6)))) )
  {
LABEL_11:
    v28[0] = (__int64)a4;
    v14 = *a4;
    if ( v10 != *a4 )
    {
      *(_QWORD *)a4[1] = v14;
      **(_QWORD **)(v14 + 8) = v10;
      **(_QWORD **)(v10 + 8) = a4;
      v15 = *(_QWORD **)(v10 + 8);
      *(_QWORD *)(v10 + 8) = *(_QWORD *)(v14 + 8);
      v8 = a4[1];
      *(_QWORD *)(v14 + 8) = v8;
      a4[1] = (__int64)v15;
    }
    v16 = *((_QWORD *)a1 + 3);
    v17 = *(_QWORD *)(v16 + 8 * v11);
    if ( v17 == *((_QWORD *)a1 + 1) )
    {
      *(_QWORD *)(v16 + 8 * v11) = a4;
      *(_QWORD *)(*((_QWORD *)a1 + 3) + 8 * v11 + 8) = a4;
    }
    else if ( v17 == v10 )
    {
      *(_QWORD *)(v16 + 8 * v11) = a4;
    }
    else
    {
      v18 = *(__int64 **)(v16 + 8 * v11 + 8);
      v8 = *v18;
      *(_QWORD *)(v16 + 8 * v11 + 8) = *v18;
      if ( (__int64 *)v8 != a4 )
      {
        v19 = *((_QWORD *)a1 + 3);
        v8 = *(_QWORD *)(*(_QWORD *)(v19 + 8 * v11 + 8) + 8LL);
        *(_QWORD *)(v19 + 8 * v11 + 8) = v8;
      }
    }
    try
    {
      v20 = *((_QWORD *)a1 + 2);
      v21 = *((_QWORD *)a1 + 7);
      v22 = (float)(int)v20;
      if ( v20 < 0 )
        v22 = v22 + 1.8446744e19;
      v23 = (float)(int)v21;
      if ( v21 < 0 )
        v23 = v23 + 1.8446744e19;
      if ( (float)(v22 / v23) > *a1 )
      {
        if ( (unsigned __int64)v21 >= 0x200 )
        {
          if ( (unsigned __int64)v21 < 0xFFFFFFFFFFFFFFFLL )
            v21 *= 2LL;
        }
        else
        {
          v21 *= 8LL;
        }
        std::_Hash<std::_Umap_traits<unsigned long,unsigned __int64,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,unsigned __int64>>,0>>::_Init(
          a1,
          v21);
        v24 = (_QWORD *)*((_QWORD *)a1 + 1);
        if ( (_QWORD *)*v24 != v24 )
        {
          v27 = v24[1];
          do
          {
            v25 = **((_QWORD **)a1 + 1);
            std::_Hash<std::_Umap_traits<unsigned long,DragNDropProcessor::MessageInfo,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,DragNDropProcessor::MessageInfo>>,0>>::_Insert<std::pair<unsigned long const,DragNDropProcessor::MessageInfo> &,std::_List_unchecked_iterator<std::_List_val<std::_List_simple_types<std::pair<unsigned long const,DragNDropProcessor::MessageInfo>>>>>(
              a1,
              v29,
              v25 + 16);
          }
          while ( v25 != v27 );
        }
        v6 = a2;
      }
    }
    catch ( ... )
    {
      iter = std::_Hash<std::_Umap_traits<unsigned long,Microsoft::WRL::ComPtr<PointerInfoAdapter>,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,Microsoft::WRL::ComPtr<PointerInfoAdapter>>>,0>>::_Make_iter(
               v8,
               v28,
               v28[0]);
      std::_Hash<std::_Umap_traits<unsigned long,DragNDropProcessor::MessageInfo,std::_Uhash_compare<unsigned long,std::hash<unsigned long>,std::equal_to<unsigned long>>,std::allocator<std::pair<unsigned long const,DragNDropProcessor::MessageInfo>>,0>>::erase<std::_List_iterator<std::_List_val<std::_List_simple_types<std::pair<unsigned long const,DragNDropProcessor::MessageInfo>>>>,void>(
        a1,
        &v27,
        *iter);
      throw;
    }
    *(_QWORD *)v6 = a4;
    *(_BYTE *)(v6 + 8) = 1;
    return v6;
  }
  else
  {
    v8 = *a3;
    while ( 1 )
    {
      v10 = *(_QWORD *)(v10 + 8);
      if ( (_DWORD)v8 == *(_DWORD *)(v10 + 16) )
        break;
      if ( v10 == *(_QWORD *)(v12 + 8 * v11) )
        goto LABEL_11;
    }
    *(_QWORD *)a4[1] = *a4;
    *(_QWORD *)(*a4 + 8) = a4[1];
    --*((_QWORD *)a1 + 2);
    DragNDropProcessor::MessageInfo::~MessageInfo((DragNDropProcessor::MessageInfo *)(a4 + 3));
    operator delete(a4);
    result = a2;
    *(_QWORD *)a2 = v10;
    *(_BYTE *)(a2 + 8) = 0;
  }
  return result;
}
