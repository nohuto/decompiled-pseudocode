/*
 * XREFs of ?InitEditionOnceCallback@@YAHPEAT_RTL_RUN_ONCE@@PEAXPEAPEAX@Z @ 0x1800017D0
 * Callers:
 *     <none>
 * Callees:
 *     <none>
...
__int64 __fastcall InitEditionOnceCallback(PINIT_ONCE InitOnce, PVOID Parameter, PVOID *Context)
{
  ...
  if ( gdwDeviceFamily == 6 )
    gdwDeviceFamily = (unsigned __int8)RtlIsMultiSessionSku() != 0 ? 6 : 0;
  hKey = 0LL;
  if ( !RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"Software\\Microsoft\\CoreShell\\ViewEventDispatcher", 0, 1u, &hKey) )
  {
    ...
    {
      cbData = 4;
      if ( !RegQueryValueExW(hKey, L"RunAsServiceHostComponent", 0LL, &Type, Data, &cbData) && Type == 4 && cbData == 4 )
        gbIsRunningWithCShell = *(_DWORD *)Data == 1;
    }
  }
