/*
 * XREFs of ?LogGamepadTaskSwitcherInvoked@ControllerProcessorTelemetry@@SAXI@Z @ 0x1800369B4
 * Callers:
 *     _lambda_1ee5a9c53eca02d00e8b9ffa9f88f873_::_lambda_invoker_cdecl_ @ 0x180036FE0 (_lambda_1ee5a9c53eca02d00e8b9ffa9f88f873_--_lambda_invoker_cdecl_.c)
 * Callees:
 *     _TlgWrite @ 0x180001118 (_TlgWrite.c)
 *     ?Provider@ControllerProcessorTelemetry@@SAQEBU_TlgProvider_t@@XZ @ 0x18003689C (-Provider@ControllerProcessorTelemetry@@SAQEBU_TlgProvider_t@@XZ.c)
 *     __security_check_cookie @ 0x1800E1B10 (__security_check_cookie.c)
 */

void __fastcall ControllerProcessorTelemetry::LogGamepadTaskSwitcherInvoked(int a1)
{
  const struct _TlgProvider_t *v1; // rax
  EVENT_DATA_DESCRIPTOR pData; // [rsp+30h] [rbp-48h] BYREF
  int *v3; // [rsp+50h] [rbp-28h]
  int v4; // [rsp+58h] [rbp-20h]
  int v5; // [rsp+5Ch] [rbp-1Ch]
  int v6; // [rsp+80h] [rbp+8h] BYREF

  v6 = a1;
  v1 = ControllerProcessorTelemetry::Provider();
  if ( *(_DWORD *)v1 > 5u
    && (*((_QWORD *)v1 + 2) & 0x400000000000LL) != 0
    && (*((_QWORD *)v1 + 3) & 0x400000000000LL) == *((_QWORD *)v1 + 3) )
  {
    v5 = 0;
    v3 = &v6;
    v4 = 4;
    TlgWrite(v1, &unk_18010D3C8, 0LL, 0LL, 3u, &pData);
  }
}
