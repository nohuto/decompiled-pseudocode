/*
 * XREFs of ?erase@?$_Tree@V?$_Tmap_traits@USPATIAL_NODE_ID@@V?$vector@V?$unique_ptr@VV2PropertyRequest@Holographic@Internal@Windows@@U?$default_delete@VV2PropertyRequest@Holographic@Internal@Windows@@@std@@@std@@V?$allocator@V?$unique_ptr@VV2PropertyRequest@Holographic@Internal@Windows@@U?$default_delete@VV2PropertyRequest@Holographic@Internal@Windows@@@std@@@std@@@2@@std@@USpatialNodeIdComp@Holographic@Internal@Windows@@V?$allocator@U?$pair@$$CBUSPATIAL_NODE_ID@@V?$vector@V?$unique_ptr@VV2PropertyRequest@Holographic@Internal@Windows@@U?$default_delete@VV2PropertyRequest@Holographic@Internal@Windows@@@std@@@std@@V?$allocator@V?$unique_ptr@VV2PropertyRequest@Holographic@Internal@Windows@@U?$default_delete@VV2PropertyRequest@Holographic@Internal@Windows@@@std@@@std@@@2@@std@@@std@@@3@$0A@@std@@@std@@QEAA?AV?$_Tree_iterator@V?$_Tree_val@U?$_Tree_simple_types@U?$pair@$$CBUSPATIAL_NODE_ID@@V?$vector@V?$unique_ptr@VV2PropertyRequest@Holographic@Internal@Windows@@U?$default_delete@VV2PropertyRequest@Holographic@Internal@Windows@@@std@@@std@@V?$allocator@V?$unique_ptr@VV2PropertyRequest@Holographic@Internal@Windows@@U?$default_delete@VV2PropertyRequest@Holographic@Internal@Windows@@@std@@@std@@@2@@std@@@std@@@std@@@std@@@2@V?$_Tree_const_iterator@V?$_Tree_val@U?$_Tree_simple_types@U?$pair@$$CBUSPATIAL_NODE_ID@@V?$vector@V?$unique_ptr@VV2PropertyRequest@Holographic@Internal@Windows@@U?$default_delete@VV2PropertyRequest@Holographic@Internal@Windows@@@std@@@std@@V?$allocator@V?$unique_ptr@VV2PropertyRequest@Holographic@Internal@Windows@@U?$default_delete@VV2PropertyRequest@Holographic@Internal@Windows@@@std@@@std@@@2@@std@@@std@@@std@@@std@@@2@@Z @ 0x1800CBB80
 * Callers:
 *     ?PropertyUpdatedListenerThreadProcV2@SpatialGraphDriverClient@Holographic@Internal@Windows@@AEAAJXZ @ 0x1800C2EFC (-PropertyUpdatedListenerThreadProcV2@SpatialGraphDriverClient@Holographic@Internal@Windows@@AEAA.c)
 * Callees:
 *     ??1?$vector@V?$unique_ptr@VV2PropertyRequest@Holographic@Internal@Windows@@U?$default_delete@VV2PropertyRequest@Holographic@Internal@Windows@@@std@@@std@@V?$allocator@V?$unique_ptr@VV2PropertyRequest@Holographic@Internal@Windows@@U?$default_delete@VV2PropertyRequest@Holographic@Internal@Windows@@@std@@@std@@@2@@std@@QEAA@XZ @ 0x1800C9244 (--1-$vector@V-$unique_ptr@VV2PropertyRequest@Holographic@Internal@Windows@@U-$default_delete@VV2.c)
 *     ??3@YAXPEAX@Z @ 0x1800E1CE8 (--3@YAXPEAX@Z.c)
 */

_QWORD *__fastcall std::_Tree<std::_Tmap_traits<SPATIAL_NODE_ID,std::vector<std::unique_ptr<Windows::Internal::Holographic::V2PropertyRequest>>,Windows::Internal::Holographic::SpatialNodeIdComp,std::allocator<std::pair<SPATIAL_NODE_ID const,std::vector<std::unique_ptr<Windows::Internal::Holographic::V2PropertyRequest>>>>,0>>::erase(
        __int64 **a1,
        _QWORD *a2,
        _QWORD *a3)
{
  _QWORD *j; // rbx
  _BYTE *v7; // rax
  _BYTE *v8; // r8
  __int64 i; // rax
  __int64 v10; // r8
  __int64 v11; // rdx
  __int64 *v12; // r10
  __int64 k; // r9
  __int64 v14; // rcx
  __int64 v15; // rax
  __int64 v16; // rcx
  void **v17; // rax
  char v18; // cl
  bool m; // zf
  __int64 v20; // r10
  __int64 *v21; // rcx
  __int64 *v22; // rcx
  __int64 **v23; // rax
  __int64 v24; // r9
  __int64 v25; // rax
  __int64 *v26; // rcx
  __int64 v27; // rax
  __int64 **v28; // rax
  __int64 v29; // r9
  __int64 *v30; // rax
  __int64 *v31; // rcx
  __int64 **v32; // rax
  __int64 **v33; // r9
  __int64 **v34; // rax
  __int64 v35; // rax
  __int64 **v36; // rax
  __int64 *v37; // rax
  _QWORD *result; // rax

  j = a3;
  if ( !*((_BYTE *)a3 + 25) )
  {
    v7 = (_BYTE *)a3[2];
    if ( v7[25] )
    {
      for ( i = a3[1]; !*(_BYTE *)(i + 25) && j == *(_QWORD **)(i + 16); i = *(_QWORD *)(i + 8) )
        j = (_QWORD *)i;
      j = (_QWORD *)i;
    }
    else
    {
      v8 = *(_BYTE **)v7;
      for ( j = v7; !v8[25]; v8 = *(_BYTE **)v8 )
        j = v8;
    }
  }
  v10 = a3[2];
  if ( !*(_BYTE *)(*a3 + 25LL) )
  {
    if ( *(_BYTE *)(v10 + 25) )
    {
      v10 = *a3;
    }
    else
    {
      v10 = j[2];
      if ( j != a3 )
      {
        *(_QWORD *)(*a3 + 8LL) = j;
        *j = *a3;
        if ( j == (_QWORD *)a3[2] )
        {
          v11 = (__int64)j;
        }
        else
        {
          v11 = j[1];
          if ( !*(_BYTE *)(v10 + 25) )
            *(_QWORD *)(v10 + 8) = v11;
          *(_QWORD *)v11 = v10;
          j[2] = a3[2];
          *(_QWORD *)(a3[2] + 8LL) = j;
        }
        if ( (_QWORD *)(*a1)[1] == a3 )
        {
          (*a1)[1] = (__int64)j;
        }
        else
        {
          v17 = (void **)a3[1];
          if ( *v17 == a3 )
            *v17 = j;
          else
            v17[2] = j;
        }
        j[1] = a3[1];
        v18 = *((_BYTE *)j + 24);
        *((_BYTE *)j + 24) = *((_BYTE *)a3 + 24);
        *((_BYTE *)a3 + 24) = v18;
        goto LABEL_46;
      }
    }
  }
  v11 = a3[1];
  if ( !*(_BYTE *)(v10 + 25) )
    *(_QWORD *)(v10 + 8) = v11;
  if ( (_QWORD *)(*a1)[1] == a3 )
  {
    (*a1)[1] = v10;
  }
  else if ( *(_QWORD **)v11 == a3 )
  {
    *(_QWORD *)v11 = v10;
  }
  else
  {
    *(_QWORD *)(v11 + 16) = v10;
  }
  v12 = *a1;
  if ( (_QWORD *)**a1 == a3 )
  {
    if ( *(_BYTE *)(v10 + 25) )
    {
      k = v11;
    }
    else
    {
      v14 = *(_QWORD *)v10;
      for ( k = v10; !*(_BYTE *)(v14 + 25); v14 = *(_QWORD *)v14 )
        k = v14;
    }
    *v12 = k;
  }
  if ( (_QWORD *)(*a1)[2] == a3 )
  {
    if ( *(_BYTE *)(v10 + 25) )
    {
      v15 = v11;
    }
    else
    {
      v16 = *(_QWORD *)(v10 + 16);
      v15 = v10;
      while ( !*(_BYTE *)(v16 + 25) )
      {
        v15 = v16;
        v16 = *(_QWORD *)(v16 + 16);
      }
    }
    (*a1)[2] = v15;
  }
LABEL_46:
  if ( *((_BYTE *)a3 + 24) != 1 )
    goto LABEL_113;
  for ( m = v10 == (*a1)[1]; !m; m = v20 == (*a1)[1] )
  {
    v20 = v11;
    if ( *(_BYTE *)(v10 + 24) != 1 )
      break;
    v21 = *(__int64 **)v11;
    if ( v10 == *(_QWORD *)v11 )
    {
      v21 = *(__int64 **)(v11 + 16);
      if ( !*((_BYTE *)v21 + 24) )
      {
        *((_BYTE *)v21 + 24) = 1;
        v22 = *(__int64 **)(v11 + 16);
        *(_BYTE *)(v11 + 24) = 0;
        *(_QWORD *)(v11 + 16) = *v22;
        if ( !*(_BYTE *)(*v22 + 25) )
          *(_QWORD *)(*v22 + 8) = v11;
        v22[1] = *(_QWORD *)(v11 + 8);
        if ( v11 == (*a1)[1] )
        {
          (*a1)[1] = (__int64)v22;
        }
        else
        {
          v23 = *(__int64 ***)(v11 + 8);
          if ( (__int64 *)v11 == *v23 )
            *v23 = v22;
          else
            v23[2] = v22;
        }
        *v22 = v11;
        *(_QWORD *)(v11 + 8) = v22;
        v21 = *(__int64 **)(v11 + 16);
      }
      if ( !*((_BYTE *)v21 + 25) )
      {
        if ( *(_BYTE *)(*v21 + 24) != 1 || *(_BYTE *)(v21[2] + 24) != 1 )
        {
          if ( *(_BYTE *)(v21[2] + 24) == 1 )
          {
            *(_BYTE *)(*v21 + 24) = 1;
            v24 = *v21;
            *((_BYTE *)v21 + 24) = 0;
            *v21 = *(_QWORD *)(v24 + 16);
            v25 = *(_QWORD *)(v24 + 16);
            if ( !*(_BYTE *)(v25 + 25) )
              *(_QWORD *)(v25 + 8) = v21;
            *(_QWORD *)(v24 + 8) = v21[1];
            if ( v21 == (__int64 *)(*a1)[1] )
            {
              (*a1)[1] = v24;
            }
            else
            {
              v30 = (__int64 *)v21[1];
              if ( v21 == (__int64 *)v30[2] )
                v30[2] = v24;
              else
                *v30 = v24;
            }
            *(_QWORD *)(v24 + 16) = v21;
            v21[1] = v24;
            v21 = *(__int64 **)(v11 + 16);
          }
          *((_BYTE *)v21 + 24) = *(_BYTE *)(v11 + 24);
          *(_BYTE *)(v11 + 24) = 1;
          *(_BYTE *)(v21[2] + 24) = 1;
          v31 = *(__int64 **)(v11 + 16);
          *(_QWORD *)(v11 + 16) = *v31;
          if ( !*(_BYTE *)(*v31 + 25) )
            *(_QWORD *)(*v31 + 8) = v11;
          v31[1] = *(_QWORD *)(v11 + 8);
          if ( v11 == (*a1)[1] )
          {
            (*a1)[1] = (__int64)v31;
          }
          else
          {
            v32 = *(__int64 ***)(v11 + 8);
            if ( (__int64 *)v11 == *v32 )
              *v32 = v31;
            else
              v32[2] = v31;
          }
          *v31 = v11;
LABEL_111:
          *(_QWORD *)(v11 + 8) = v31;
          break;
        }
LABEL_80:
        *((_BYTE *)v21 + 24) = 0;
      }
    }
    else
    {
      if ( !*((_BYTE *)v21 + 24) )
      {
        *((_BYTE *)v21 + 24) = 1;
        v26 = *(__int64 **)v11;
        *(_BYTE *)(v11 + 24) = 0;
        *(_QWORD *)v11 = v26[2];
        v27 = v26[2];
        if ( !*(_BYTE *)(v27 + 25) )
          *(_QWORD *)(v27 + 8) = v11;
        v26[1] = *(_QWORD *)(v11 + 8);
        if ( v11 == (*a1)[1] )
        {
          (*a1)[1] = (__int64)v26;
        }
        else
        {
          v28 = *(__int64 ***)(v11 + 8);
          if ( (__int64 *)v11 == v28[2] )
            v28[2] = v26;
          else
            *v28 = v26;
        }
        v26[2] = v11;
        *(_QWORD *)(v11 + 8) = v26;
        v21 = *(__int64 **)v11;
      }
      if ( !*((_BYTE *)v21 + 25) )
      {
        v29 = v21[2];
        if ( *(_BYTE *)(v29 + 24) != 1 || *(_BYTE *)(*v21 + 24) != 1 )
        {
          if ( *(_BYTE *)(*v21 + 24) == 1 )
          {
            *(_BYTE *)(v29 + 24) = 1;
            v33 = (__int64 **)v21[2];
            *((_BYTE *)v21 + 24) = 0;
            v21[2] = (__int64)*v33;
            if ( !*((_BYTE *)*v33 + 25) )
              (*v33)[1] = (__int64)v21;
            v33[1] = (__int64 *)v21[1];
            if ( v21 == (__int64 *)(*a1)[1] )
            {
              (*a1)[1] = (__int64)v33;
            }
            else
            {
              v34 = (__int64 **)v21[1];
              if ( v21 == *v34 )
                *v34 = (__int64 *)v33;
              else
                v34[2] = (__int64 *)v33;
            }
            *v33 = v21;
            v21[1] = (__int64)v33;
            v21 = *(__int64 **)v11;
          }
          *((_BYTE *)v21 + 24) = *(_BYTE *)(v11 + 24);
          *(_BYTE *)(v11 + 24) = 1;
          *(_BYTE *)(*v21 + 24) = 1;
          v31 = *(__int64 **)v11;
          *(_QWORD *)v11 = *(_QWORD *)(*(_QWORD *)v11 + 16LL);
          v35 = v31[2];
          if ( !*(_BYTE *)(v35 + 25) )
            *(_QWORD *)(v35 + 8) = v11;
          v31[1] = *(_QWORD *)(v11 + 8);
          if ( v11 == (*a1)[1] )
          {
            (*a1)[1] = (__int64)v31;
          }
          else
          {
            v36 = *(__int64 ***)(v11 + 8);
            if ( (__int64 *)v11 == v36[2] )
              v36[2] = v31;
            else
              *v36 = v31;
          }
          v31[2] = v11;
          goto LABEL_111;
        }
        goto LABEL_80;
      }
    }
    v10 = v11;
    v11 = *(_QWORD *)(v11 + 8);
  }
  *(_BYTE *)(v10 + 24) = 1;
LABEL_113:
  std::vector<std::unique_ptr<Windows::Internal::Holographic::V2PropertyRequest>>::~vector<std::unique_ptr<Windows::Internal::Holographic::V2PropertyRequest>>(
    (__int64)(a3 + 6),
    v11);
  operator delete(a3);
  v37 = a1[1];
  if ( v37 )
    a1[1] = (__int64 *)((char *)v37 - 1);
  result = a2;
  *a2 = j;
  return result;
}
