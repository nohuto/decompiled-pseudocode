/*
 * XREFs of ?OpenStoreKey@CAudioSessionStore@@AEAAJXZ @ 0x180042320
 * Callers:
 ...
 *     WPP_SF_qSSq @ 0x1800C0264 (WPP_SF_qSSq.c)
 */
...
LABEL_16:
  if ( hKey )
    RegCloseKey(hKey);
  if ( ThreadUserStringSid < 0 )
    AudSrvTraceLoggingErrorHelper("CAudioSessionStore::OpenStoreKey", 0x172Fu, ThreadUserStringSid);
  return (unsigned int)ThreadUserStringSid;
}
