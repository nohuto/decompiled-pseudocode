/*
 * XREFs of ?Initialize@CVolumeControlBase@@UEAAJPEAUIMMDevice@@@Z @ 0x18004E0B0
 * Callers:
 *     ?Initialize@CVolumeHardware@@QEAAJPEAUIMMDevice@@PEAUIPartsList@@@Z @ 0x18004D7BC (-Initialize@CVolumeHardware@@QEAAJPEAUIMMDevice@@PEAUIPartsList@@@Z.c)
 *     ?Initialize@CVolumeSoftware@@UEAAJPEAUIMMDevice@@@Z @ 0x1800A25E0 (-Initialize@CVolumeSoftware@@UEAAJPEAUIMMDevice@@@Z.c)
 * Callees:
 *     ?AtlComQIPtrAssign@ATL@@YAPEAUIUnknown@@PEAPEAU2@PEAU2@AEBU_GUID@@@Z @ 0x1800184EC (-AtlComQIPtrAssign@ATL@@YAPEAUIUnknown@@PEAPEAU2@PEAU2@AEBU_GUID@@@Z.c)
 *     ?AudSrvTraceLoggingErrorHelper@@YAXPEBDIJ@Z @ 0x18004A8B0 (-AudSrvTraceLoggingErrorHelper@@YAXPEBDIJ@Z.c)
 *     ?reset@?$unique_storage@U?$resource_policy@PEAGP6AXPEAX@Z$1?CoTaskMemFree@@YAX0@ZU?$integral_constant@_K$0A@@wistd@@PEAG$0A@$$T@details@wil@@@details@wil@@QEAAXPEAG@Z @ 0x18004B238 (-reset@-$unique_storage@U-$resource_policy@PEAGP6AXPEAX@Z$1-CoTaskMemFree@@YAX0@ZU-$integral_con.c)
 *     ?GetLevelData@CVolumeHardware@@MEAAJXZ @ 0x18004DA80 (-GetLevelData@CVolumeHardware@@MEAAJXZ.c)
 *     _guard_dispatch_icall_nop @ 0x1800657D0 (_guard_dispatch_icall_nop.c)
 */

// Hidden C++ exception states: #wind=1
__int64 __fastcall CVolumeControlBase::Initialize(void **this, struct IUnknown *a2)
{
  _QWORD *v4; // r14
  int v5; // ebx
  struct IUnknown *v6; // rcx
  __int64 (__fastcall *v7)(CVolumeHardware *); // rax
  int LevelData; // eax
  int v9; // ecx
  struct IUnknown *v11; // [rsp+40h] [rbp+8h] BYREF

  v11 = 0LL;
  v4 = this + 8;
  v5 = ((__int64 (__fastcall *)(struct IUnknown *, void **))a2->lpVtbl[1].Release)(a2, this + 8);
  if ( v5 < 0 )
    goto LABEL_12;
  wil::details::unique_storage<wil::details::resource_policy<unsigned short *,void (*)(void *),&void CoTaskMemFree(void *),wistd::integral_constant<unsigned __int64,0>,unsigned short *,0,std::nullptr_t>>::reset(
    this + 9,
    0LL);
  mmdDevGetInstanceIdFromMMDeviceId(*v4, this + 9);
  v6 = v11;
  if ( v11 != a2 )
  {
    ATL::AtlComQIPtrAssign(&v11, a2, &GUID_1be09788_6894_4089_8586_9a2a6c265ac5);
    v6 = v11;
  }
  v5 = ((__int64 (__fastcall *)(struct IUnknown *, char *))v6->lpVtbl[1].QueryInterface)(v6, (char *)this + 80);
  if ( v5 < 0
    || ((v7 = (__int64 (__fastcall *)(CVolumeHardware *))*((_QWORD *)*this + 28), v7 != CVolumeHardware::GetLevelData)
      ? (LevelData = v7((CVolumeHardware *)this))
      : (LevelData = CVolumeHardware::GetLevelData((CVolumeHardware *)this)),
        v5 = LevelData,
        v9 = *((_DWORD *)this + 20),
        this[29] = (void *)*v4,
        *((_DWORD *)this + 60) = v9,
        LevelData < 0) )
  {
LABEL_12:
    AudSrvTraceLoggingErrorHelper("CVolumeControlBase::Initialize", 133, v5);
  }
  if ( v11 )
    ((void (__fastcall *)(struct IUnknown *))v11->lpVtbl->Release)(v11);
  return (unsigned int)v5;
}
