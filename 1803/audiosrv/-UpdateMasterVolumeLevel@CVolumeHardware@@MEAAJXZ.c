/*
 * XREFs of ?UpdateMasterVolumeLevel@CVolumeHardware@@MEAAJXZ @ 0x1800A49B0
 * Callers:
 *     <none>
 * Callees:
 *     ?GetDB@CVolumeUnit@@QEAAMXZ @ 0x18004E374 (-GetDB@CVolumeUnit@@QEAAMXZ.c)
 *     _guard_dispatch_icall_nop @ 0x1800657D0 (_guard_dispatch_icall_nop.c)
 *     ?AtlThrowImpl@ATL@@YAXJ@Z @ 0x18009BB24 (-AtlThrowImpl@ATL@@YAXJ@Z.c)
 *     ?Allocate@?$CHeapPtr@MVCCRTAllocator@ATL@@@ATL@@QEAA_N_K@Z @ 0x1800A1DE4 (-Allocate@-$CHeapPtr@MVCCRTAllocator@ATL@@@ATL@@QEAA_N_K@Z.c)
 *     WPP_SF_Pg @ 0x1800A4F18 (WPP_SF_Pg.c)
 */

// Hidden C++ exception states: #wind=1
__int64 __fastcall CVolumeHardware::UpdateMasterVolumeLevel(CVolumeHardware *this)
{
  unsigned __int64 v2; // rdi
  float *v3; // rbx
  unsigned int v4; // edi
  float *v5; // rcx
  __int64 v6; // r14
  __int64 v7; // rdx
  __int64 v8; // r8
  float DB; // xmm0_4
  void *Block; // [rsp+50h] [rbp+8h] BYREF

  v2 = 0LL;
  Block = 0LL;
  ATL::CHeapPtr<float,ATL::CCRTAllocator>::Allocate(&Block, *((unsigned int *)this + 31));
  v3 = (float *)Block;
  if ( Block )
  {
    if ( *((_QWORD *)this + 12) )
    {
      v6 = 0LL;
      do
      {
        DB = CVolumeUnit::GetDB((CVolumeUnit *)(v6 + *((_QWORD *)this + 11)));
        v3[v2] = DB;
        if ( WPP_GLOBAL_Control != (CAudioSessionManager *)&WPP_GLOBAL_Control
          && (*((_DWORD *)WPP_GLOBAL_Control + 7) & 0x10000) != 0
          && *((_BYTE *)WPP_GLOBAL_Control + 25) >= 4u )
        {
          WPP_SF_Pg(*((_QWORD *)WPP_GLOBAL_Control + 2), v7, v8, v2, DB);
        }
        ++v2;
        v6 += 56LL;
      }
      while ( v2 < *((_QWORD *)this + 12) );
    }
    v4 = (*(__int64 (__fastcall **)(_QWORD, float *, _QWORD, char *))(**((_QWORD **)this + 32) + 64LL))(
           *((_QWORD *)this + 32),
           v3,
           *((unsigned int *)this + 31),
           (char *)this + 264);
    v5 = v3;
  }
  else
  {
    v4 = -2147024882;
    v5 = 0LL;
  }
  free(v5);
  return v4;
}
