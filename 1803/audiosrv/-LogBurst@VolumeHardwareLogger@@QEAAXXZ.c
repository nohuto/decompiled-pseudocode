/*
 * XREFs of ?LogBurst@VolumeHardwareLogger@@QEAAXXZ @ 0x1800A302C
 * Callers:
 *     ?ProviderFinalRelease@CVolumeHardware@@UEAAXXZ @ 0x1800A36F0 (-ProviderFinalRelease@CVolumeHardware@@UEAAXXZ.c)
 *     ?TimerCallback@VolumeHardwareLogger@@CAXPEAU_TP_CALLBACK_INSTANCE@@PEAXPEAU_TP_TIMER@@@Z @ 0x1800A4870 (-TimerCallback@VolumeHardwareLogger@@CAXPEAU_TP_CALLBACK_INSTANCE@@PEAXPEAU_TP_TIMER@@@Z.c)
 * Callees:
 *     ?get@?$static_lazy@VAudioSrvTelemetryProvider@@@details@wil@@QEAAPEAVAudioSrvTelemetryProvider@@P6AXXZ@Z @ 0x180008688 (-get@-$static_lazy@VAudioSrvTelemetryProvider@@@details@wil@@QEAAPEAVAudioSrvTelemetryProvider@@.c)
 *     _TlgWrite @ 0x180043BEC (_TlgWrite.c)
 *     _TlgCreateWsz @ 0x180043DA4 (_TlgCreateWsz.c)
 *     _TlgKeywordOn @ 0x180043F18 (_TlgKeywordOn.c)
 *     ?CurrentVolumeLevel@VolumeHardwareLogger@@AEAA?AUVolumeLevel@1@XZ @ 0x18004BC88 (-CurrentVolumeLevel@VolumeHardwareLogger@@AEAA-AUVolumeLevel@1@XZ.c)
 *     __security_check_cookie @ 0x1800608E0 (__security_check_cookie.c)
 *     ?ScheduleTimer@VolumeHardwareLogger@@AEAAXXZ @ 0x1800A3AAC (-ScheduleTimer@VolumeHardwareLogger@@AEAAXXZ.c)
 */

// Hidden C++ exception states: #wind=1
void __fastcall VolumeHardwareLogger::LogBurst(struct _RTL_CRITICAL_SECTION *this)
{
  HANDLE OwningThread; // rcx
  double DebugInfo; // xmm1_8
  __int64 v4; // rcx
  LPVOID v5; // rax
  __int64 v6; // r11
  LPCGUID v7; // r8
  LPCGUID v8; // r9
  TraceLoggingHProvider v9; // r10
  __int64 v10; // [rsp+38h] [rbp-D0h] BYREF
  __int64 v11; // [rsp+40h] [rbp-C8h] BYREF
  struct _RTL_CRITICAL_SECTION *v12; // [rsp+48h] [rbp-C0h] BYREF
  LARGE_INTEGER PerformanceCount; // [rsp+50h] [rbp-B8h] BYREF
  double v14[2]; // [rsp+58h] [rbp-B0h] BYREF
  EVENT_DATA_DESCRIPTOR pData; // [rsp+68h] [rbp-A0h] BYREF
  struct _EVENT_DATA_DESCRIPTOR pDesc; // [rsp+88h] [rbp-80h] BYREF
  struct _EVENT_DATA_DESCRIPTOR v17; // [rsp+98h] [rbp-70h] BYREF
  struct _RTL_CRITICAL_SECTION **v18; // [rsp+A8h] [rbp-60h]
  __int64 v19; // [rsp+B0h] [rbp-58h]
  ULONG_PTR *p_SpinCount; // [rsp+B8h] [rbp-50h]
  __int64 v21; // [rsp+C0h] [rbp-48h]
  char *v22; // [rsp+C8h] [rbp-40h]
  __int64 v23; // [rsp+D0h] [rbp-38h]
  struct _RTL_CRITICAL_SECTION *v24; // [rsp+D8h] [rbp-30h]
  __int64 v25; // [rsp+E0h] [rbp-28h]
  double *v26; // [rsp+E8h] [rbp-20h]
  __int64 v27; // [rsp+F0h] [rbp-18h]
  char *v28; // [rsp+F8h] [rbp-10h]
  __int64 v29; // [rsp+100h] [rbp-8h]
  __int64 *v30; // [rsp+108h] [rbp+0h]
  __int64 v31; // [rsp+110h] [rbp+8h]
  char *v32; // [rsp+118h] [rbp+10h]
  __int64 v33; // [rsp+120h] [rbp+18h]
  __int64 *v34; // [rsp+128h] [rbp+20h]
  __int64 v35; // [rsp+130h] [rbp+28h]
  LONG *p_LockCount; // [rsp+138h] [rbp+30h]
  __int64 v37; // [rsp+140h] [rbp+38h]
  LONG *p_RecursionCount; // [rsp+148h] [rbp+40h]
  __int64 v39; // [rsp+150h] [rbp+48h]
  HANDLE *p_OwningThread; // [rsp+158h] [rbp+50h]
  __int64 v41; // [rsp+160h] [rbp+58h]
  ULONG_PTR *v42; // [rsp+168h] [rbp+60h]
  __int64 v43; // [rsp+170h] [rbp+68h]
  char *v44; // [rsp+178h] [rbp+70h]
  __int64 v45; // [rsp+180h] [rbp+78h]
  struct _RTL_CRITICAL_SECTION *v46; // [rsp+188h] [rbp+80h]
  __int64 v47; // [rsp+190h] [rbp+88h]
  char *v48; // [rsp+198h] [rbp+90h]
  __int64 v49; // [rsp+1A0h] [rbp+98h]
  LONG *v50; // [rsp+1A8h] [rbp+A0h]
  __int64 v51; // [rsp+1B0h] [rbp+A8h]
  LONG *v52; // [rsp+1B8h] [rbp+B0h]
  __int64 v53; // [rsp+1C0h] [rbp+B8h]

  v14[1] = NAN;
  EnterCriticalSection(this);
  v12 = this;
  if ( this[1].LockSemaphore && (__int64)this[1].DebugInfo > 0 && SHIDWORD(this[2].OwningThread) > 0 )
  {
    QueryPerformanceCounter(&PerformanceCount);
    OwningThread = this[3].OwningThread;
    DebugInfo = (double)(int)this[1].DebugInfo;
    if ( LOBYTE(this[1].OwningThread) || (double)(PerformanceCount.LowPart - (int)OwningThread) / DebugInfo >= 30.0 )
    {
      v14[0] = (double)((int)OwningThread - LODWORD(this[2].LockSemaphore)) / DebugInfo;
      VolumeHardwareLogger::CurrentVolumeLevel((__int64)this, (__int64)&v10);
      v5 = wil::details::static_lazy<AudioSrvTelemetryProvider>::get(
             v4,
             lambda_b4cb8953ddd9016186cd5ae31274160b_::_lambda_invoker_cdecl_);
      if ( **((_DWORD **)v5 + 1) > 4u && TlgKeywordOn(*((TraceLoggingHProvider *)v5 + 1), 0x400000000000uLL) )
      {
        TlgCreateWsz(&pDesc, *((LPCWSTR *)this[1].LockSemaphore + 8));
        TlgCreateWsz(&v17, *((LPCWSTR *)this[1].LockSemaphore + 9));
        LODWORD(v12) = *((_DWORD *)this[1].LockSemaphore + 20);
        v18 = &v12;
        v19 = v6;
        p_SpinCount = &this[1].SpinCount;
        v21 = v6;
        v22 = (char *)&this[1].SpinCount + 4;
        v23 = v6;
        v24 = this + 2;
        v25 = v6;
        v26 = v14;
        v27 = 8LL;
        v28 = (char *)&this[2].OwningThread + 4;
        v29 = v6;
        v30 = &v10;
        v31 = v6;
        v32 = (char *)&v10 + 4;
        v33 = v6;
        v34 = &v11;
        v35 = v6;
        p_LockCount = &this[2].LockCount;
        v37 = v6;
        p_RecursionCount = &this[2].RecursionCount;
        v39 = v6;
        p_OwningThread = &this[2].OwningThread;
        v41 = v6;
        v42 = &this[2].SpinCount;
        v43 = v6;
        v44 = (char *)&this[2].SpinCount + 4;
        v45 = v6;
        v46 = this + 3;
        v47 = v6;
        v48 = (char *)&this[3].DebugInfo + 4;
        v49 = v6;
        v50 = &this[3].LockCount;
        v51 = v6;
        v52 = &this[3].RecursionCount;
        v53 = v6;
        TlgWrite(v9, &unk_18014510C, v7, v8, 0x16u, &pData);
      }
      HIDWORD(this[2].OwningThread) = 0;
      *(_QWORD *)&this[2].LockCount = v10;
      LODWORD(this[2].OwningThread) = v11;
    }
    else
    {
      VolumeHardwareLogger::ScheduleTimer((VolumeHardwareLogger *)this);
    }
  }
  if ( this )
    LeaveCriticalSection(this);
}
