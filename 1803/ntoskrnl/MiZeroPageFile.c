/*
 * XREFs of MiZeroPageFile @ 0x140252420
 * Callers:
 *     MiZeroAllPageFiles @ 0x140485208 (MiZeroAllPageFiles.c)
 * Callees:
 *     KeReleaseInStackQueuedSpinLockFromDpcLevel @ 0x1400888A0 (KeReleaseInStackQueuedSpinLockFromDpcLevel.c)
 *     MmZeroPageWrite @ 0x140099068 (MmZeroPageWrite.c)
 *     KeAcquireInStackQueuedSpinLock @ 0x1400E05E0 (KeAcquireInStackQueuedSpinLock.c)
 *     KeSetEvent @ 0x1400FB1C0 (KeSetEvent.c)
 *     ExFreePoolWithTag @ 0x1402EA410 (ExFreePoolWithTag.c)
 */

LONG __fastcall MiZeroPageFile(_QWORD *a1)
{
  unsigned __int64 *v1; // r14
  struct _KEVENT *v2; // r15
  unsigned int v3; // edi
  int v4; // ebx
  unsigned int v5; // ebp
  unsigned __int64 v6; // rdx
  unsigned int v7; // esi
  unsigned __int64 v8; // r8
  unsigned int v9; // eax
  __int64 v10; // rcx
  struct _KLOCK_QUEUE_HANDLE LockHandle; // [rsp+20h] [rbp-58h] BYREF
  unsigned __int64 v13; // [rsp+80h] [rbp+8h] BYREF
  unsigned __int64 v14; // [rsp+88h] [rbp+10h] BYREF

  v1 = (unsigned __int64 *)a1[4];
  v2 = (struct _KEVENT *)a1[5];
  ExFreePoolWithTag(a1, 0);
  v3 = 0;
  v4 = 0;
  v5 = 0;
  KeAcquireInStackQueuedSpinLock(v1 + 29, &LockHandle);
  v6 = *v1;
  v7 = 1;
  if ( *v1 > 1 )
  {
    v8 = 1LL;
    do
    {
      if ( _bittest64(*(const signed __int64 **)(v1[14] + 16), v8) )
      {
        if ( v3 )
          v4 = 1;
      }
      else
      {
        v9 = v7;
        if ( v3 )
          v9 = v5;
        ++v3;
        v5 = v9;
        if ( v3 == 16 || v8 == v6 - 1 )
          v4 = 1;
      }
      if ( v4 )
      {
        KeReleaseInStackQueuedSpinLockFromDpcLevel(&LockHandle);
        __writecr8(LockHandle.OldIrql);
        v10 = v1[7];
        v14 = (unsigned __int64)v5 << 12;
        v13 = (unsigned __int64)v3 << 12;
        MmZeroPageWrite(v10, (__int64)&v14, (__int64 *)&v13, 0);
        v4 = 0;
        KeAcquireInStackQueuedSpinLock(v1 + 29, &LockHandle);
        v6 = *v1;
        v3 = 0;
      }
      v8 = ++v7;
    }
    while ( v7 < v6 );
  }
  KeReleaseInStackQueuedSpinLockFromDpcLevel(&LockHandle);
  __writecr8(LockHandle.OldIrql);
  return KeSetEvent(v2, 0, 0);
}
