/*
 * XREFs of MiImageRvaRawEnumFirst @ 0x14056E7D4
 * Callers:
 *     RtlpCompressRvaList @ 0x1404B3F20 (RtlpCompressRvaList.c)
 * Callees:
 *     _guard_dispatch_icall @ 0x1401B3560 (_guard_dispatch_icall.c)
 *     MiImageGetRawRvaState @ 0x14056E8A4 (MiImageGetRawRvaState.c)
 */

__int64 __fastcall MiImageRvaRawEnumFirst(__int64 a1, _DWORD *a2)
{
  unsigned int v2; // ebx
  __int64 v3; // rsi
  unsigned int i; // ebp
  __int64 (__fastcall *v7)(__int64, __int64, __int64); // rax
  unsigned int v8; // eax
  __int64 result; // rax

  v2 = 0;
  v3 = a1 + 104;
  for ( i = 0; i < *(_DWORD *)(a1 + 100); v3 += 40LL )
  {
    v7 = *(__int64 (__fastcall **)(__int64, __int64, __int64))(v3 + 8);
    if ( v7 )
    {
      v8 = v7(a1, v3 + 24, v3 + 4);
      *(_DWORD *)v3 = v8;
      if ( v8 )
      {
        if ( v2 )
        {
          if ( v2 >= v8 )
            v2 = v8;
        }
        else
        {
          v2 = v8;
        }
      }
    }
    ++i;
  }
  if ( a2 && v2 )
    *a2 = MiImageGetRawRvaState(a1, v2);
  result = v2;
  *(_DWORD *)(a1 + 224) = v2;
  return result;
}
