/*
 * XREFs of AslpFileGetHeaderAttributesPE @ 0x1407DD0F4
 * Callers:
 *     AslFileAllocAndGetAttributes @ 0x1407DC024 (AslFileAllocAndGetAttributes.c)
 * Callees:
 *     AslLogCallPrintf @ 0x1407D92C4 (AslLogCallPrintf.c)
 *     AslFileMappingEnsure @ 0x1407D969C (AslFileMappingEnsure.c)
 *     AslpFileGetNtHeaderAttributes @ 0x1407DD3DC (AslpFileGetNtHeaderAttributes.c)
 */

__int64 __fastcall AslpFileGetHeaderAttributesPE(__int64 a1, __int64 a2)
{
  int v4; // eax
  unsigned int v5; // edi
  int NtHeaderAttributes; // eax
  int v7; // r9d
  int v8; // r10d
  int v9; // r11d
  int v10; // esi
  int v11; // r14d
  int v12; // ecx
  int v13; // r9d
  int v14; // r10d
  int v15; // r11d
  int v16; // esi
  int v17; // r14d
  int v19; // [rsp+20h] [rbp-40h]
  _WORD v20[2]; // [rsp+40h] [rbp-20h] BYREF
  unsigned int v21; // [rsp+44h] [rbp-1Ch] BYREF
  unsigned int v22; // [rsp+48h] [rbp-18h] BYREF
  unsigned int v23; // [rsp+4Ch] [rbp-14h] BYREF
  unsigned int v24; // [rsp+50h] [rbp-10h] BYREF
  unsigned __int16 v25; // [rsp+A0h] [rbp+40h] BYREF
  unsigned __int16 v26; // [rsp+A8h] [rbp+48h] BYREF

  v21 = 0;
  v22 = 0;
  v23 = 0;
  v24 = 0;
  v4 = AslFileMappingEnsure(a2);
  v5 = v4;
  if ( v4 >= 0 )
  {
    if ( *(_DWORD *)(a2 + 64) == 6 )
    {
      *(_DWORD *)(a1 + 664) |= 2u;
      *(_DWORD *)(a1 + 696) |= 2u;
      NtHeaderAttributes = AslpFileGetNtHeaderAttributes(
                             (unsigned int)&v21,
                             (unsigned int)&v22,
                             (unsigned int)&v23,
                             (unsigned int)&v24,
                             (__int64)&v25,
                             (__int64)&v26,
                             (__int64)v20,
                             a2);
      v7 = *(_DWORD *)(a1 + 600);
      v8 = *(_DWORD *)(a1 + 632);
      v9 = *(_DWORD *)(a1 + 728);
      v10 = *(_DWORD *)(a1 + 56);
      v11 = *(_DWORD *)(a1 + 952);
      if ( NtHeaderAttributes < 0 )
      {
        v13 = v7 | 2;
        v14 = v8 | 2;
        v15 = v9 | 2;
        v16 = v10 | 2;
        v17 = v11 | 2;
        v12 = 2;
      }
      else
      {
        *(_QWORD *)(a1 + 592) = v21;
        *(_QWORD *)(a1 + 624) = v22;
        v12 = 1;
        *(_QWORD *)(a1 + 720) = v23;
        v13 = v7 | 1;
        v14 = v8 | 1;
        *(_QWORD *)(a1 + 48) = v24;
        v15 = v9 | 1;
        v16 = v10 | 1;
        *(_QWORD *)(a1 + 944) = v25;
        v17 = v11 | 1;
        *(_QWORD *)(a1 + 976) = v26;
        *(_QWORD *)(a1 + 1008) = v20[0];
        *(_DWORD *)(a1 + 576) = 2;
        *(_QWORD *)(a1 + 584) = 4LL;
        *(_DWORD *)(a1 + 608) = 2;
        *(_QWORD *)(a1 + 616) = 4LL;
        *(_DWORD *)(a1 + 704) = 2;
        *(_QWORD *)(a1 + 712) = 4LL;
        *(_DWORD *)(a1 + 32) = 2;
        *(_QWORD *)(a1 + 40) = 4LL;
        *(_DWORD *)(a1 + 928) = 1;
        *(_QWORD *)(a1 + 936) = 2LL;
        *(_DWORD *)(a1 + 960) = 1;
        *(_QWORD *)(a1 + 968) = 2LL;
        *(_DWORD *)(a1 + 992) = 1;
        *(_QWORD *)(a1 + 1000) = 2LL;
      }
      *(_DWORD *)(a1 + 984) |= v12;
      *(_DWORD *)(a1 + 1016) |= v12;
      *(_DWORD *)(a1 + 600) = v13;
      *(_DWORD *)(a1 + 952) = v17;
      *(_DWORD *)(a1 + 56) = v16;
      *(_DWORD *)(a1 + 728) = v15;
      *(_DWORD *)(a1 + 632) = v14;
    }
    else
    {
      *(_DWORD *)(a1 + 600) |= 2u;
      *(_DWORD *)(a1 + 632) |= 2u;
      *(_DWORD *)(a1 + 728) |= 2u;
      *(_DWORD *)(a1 + 56) |= 2u;
      *(_DWORD *)(a1 + 1016) |= 2u;
      *(_DWORD *)(a1 + 952) |= 2u;
      *(_DWORD *)(a1 + 984) |= 2u;
    }
    return 0;
  }
  else
  {
    v19 = v4;
    AslLogCallPrintf(
      1LL,
      (unsigned int)"AslpFileGetHeaderAttributesPE",
      2474,
      (unsigned int)"AslFileMappingEnsure failed [%x]",
      v19);
  }
  return v5;
}
