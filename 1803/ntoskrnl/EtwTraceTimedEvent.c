/*
 * XREFs of EtwTraceTimedEvent @ 0x1400B76E4
 * Callers:
 *     KeDisableTimer2 @ 0x140087498 (KeDisableTimer2.c)
 *     KiFinalizeTimer2Disablement @ 0x1400876FC (KiFinalizeTimer2Disablement.c)
 *     IopTimerDispatch @ 0x1400D1F10 (IopTimerDispatch.c)
 *     KiExpireTimer2 @ 0x1400EA230 (KiExpireTimer2.c)
 *     KiProcessExpiredTimerList @ 0x140108660 (KiProcessExpiredTimerList.c)
 *     EtwTraceCpuCacheFlush @ 0x1402AE9B8 (EtwTraceCpuCacheFlush.c)
 *     EtwTraceSiloTimedEvent @ 0x1402AF354 (EtwTraceSiloTimedEvent.c)
 *     EtwpTraceFltTimedIo @ 0x1402B0690 (EtwpTraceFltTimedIo.c)
 *     PerfInfoLogIpi @ 0x1402B0DEC (PerfInfoLogIpi.c)
 * Callees:
 *     EtwpLogKernelEvent @ 0x1401080D0 (EtwpLogKernelEvent.c)
 *     __security_check_cookie @ 0x140187410 (__security_check_cookie.c)
 */

__int64 __fastcall EtwTraceTimedEvent(__int16 a1, unsigned int a2, __int64 a3, int a4, int a5, __int64 a6)
{
  __int64 result; // rax
  unsigned int v8; // ebx
  bool v9; // zf
  unsigned __int64 i; // rbp
  __int64 v11; // rcx
  __int64 v12; // rax
  __int64 v13; // [rsp+30h] [rbp-48h] BYREF
  int v14; // [rsp+38h] [rbp-40h]
  int v15; // [rsp+3Ch] [rbp-3Ch]
  __int64 v16; // [rsp+40h] [rbp-38h]
  int v17; // [rsp+48h] [rbp-30h]
  int v18; // [rsp+4Ch] [rbp-2Ch]

  result = EtwpHostSiloState;
  v16 = a3;
  v17 = a4;
  v18 = 0;
  v8 = *(_DWORD *)(EtwpHostSiloState + 4188);
  v9 = !_BitScanForward((unsigned int *)&a3, v8);
  for ( i = a2; !v9; v9 = !_BitScanForward((unsigned int *)&a3, v8) )
  {
    result = v8 - 1;
    v8 &= result;
    v11 = EtwpHostSiloState + 32 * (a3 + 132);
    if ( v11 )
    {
      result = i >> 29;
      if ( ((unsigned int)i & *(_DWORD *)(v11 + 4 * (i >> 29)) & 0x1FFFFFFF) != 0 )
      {
        v12 = *(unsigned __int8 *)(EtwpHostSiloState + 2 * a3 + 4173);
        v15 = 0;
        v14 = 8;
        v13 = a6 + 8 * (v12 - 1);
        result = EtwpLogKernelEvent(
                   (unsigned int)&v13,
                   EtwpHostSiloState,
                   *(unsigned __int8 *)(EtwpHostSiloState + 2 * a3 + 4172),
                   2,
                   a1,
                   a5);
      }
    }
  }
  return result;
}
