/*
 * XREFs of PiAuGetStateDirectorySecurityObject @ 0x140729E54
 * Callers:
...
__int64 __fastcall PiAuGetStateDirectorySecurityObject(_QWORD *a1)
{
  ...
  v6 = RtlLengthSid(SeTrustedInstallerSid) + 64 + 2 * (v3 + v5 + v4);
  ...
  if ( PoolWithTag )
  {
    ...
    if ( Acl >= 0 )
    {
      Acl = RtlpAddKnownAce(v8, 2u, 3, 0x10000000, (unsigned __int8 *)SeTrustedInstallerSid, 0);
      if ( Acl >= 0 )
      {
        Acl = RtlpAddKnownAce(v8, 2u, 3, -1073676288, (unsigned __int8 *)SeLocalSystemSid, 0);
        if ( Acl >= 0 )
        {
          Acl = RtlpAddKnownAce(v8, 2u, 2, 32, (unsigned __int8 *)SeLocalSystemSid, 0);
          if ( Acl >= 0 )
          {
            Acl = RtlpAddKnownAce(v8, 2u, 3, 0x80000000, (unsigned __int8 *)SeExports->SeUserModeDriversSid, 0);
            if ( Acl >= 0 )
            {
              Acl = RtlpAddKnownAce(v8, 2u, 2, 32, (unsigned __int8 *)SeExports->SeUserModeDriversSid, 0);
              if ( Acl >= 0 )
              {
                Acl = RtlpAddKnownAce(v8, 2u, 3, 0x80000000, (unsigned __int8 *)SeAliasAdminsSid, 0);
                if ( Acl >= 0 )
                {
                  Acl = RtlpAddKnownAce(v8, 2u, 2, 32, (unsigned __int8 *)SeAliasAdminsSid, 0);
                  if ( Acl >= 0 )
                  {
