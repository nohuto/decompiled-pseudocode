/*
 * XREFs of EtwGetKernelTraceTimestampSilo @ 0x1400B7A08
 * Callers:
 *     PfHardFaultRecord @ 0x1400B797C (PfHardFaultRecord.c)
 * Callees:
 *     RtlGetSystemTimePrecise @ 0x140132A80 (RtlGetSystemTimePrecise.c)
 *     _guard_dispatch_icall @ 0x1401B3560 (_guard_dispatch_icall.c)
 */

LARGE_INTEGER __fastcall EtwGetKernelTraceTimestampSilo(LARGE_INTEGER *a1, unsigned int a2, __int64 a3)
{
  int v3; // ebx
  unsigned __int64 v4; // r11
  _QWORD *v6; // r8
  __int64 v7; // r8
  unsigned int v8; // r9d
  __int64 v10; // rcx
  __int64 v11; // rdx
  LARGE_INTEGER PerformanceCounter; // rax
  LARGE_INTEGER result; // rax
  LARGE_INTEGER v14; // [rsp+40h] [rbp+18h] BYREF

  v3 = 0;
  v4 = a2;
  if ( a3 )
    v6 = *(_QWORD **)(a3 + 1256);
  else
    v6 = &PspHostSiloGlobals;
  v7 = v6[108];
  if ( v7 )
  {
    v8 = *(_DWORD *)(v7 + 4188);
    while ( _BitScanForward((unsigned int *)&v10, v8) )
    {
      v8 &= v8 - 1;
      v11 = v7 + 32 * (v10 + 132);
      if ( v11 )
      {
        if ( ((unsigned int)v4 & *(_DWORD *)(v11 + 4 * (v4 >> 29)) & 0x1FFFFFFF) != 0 )
          v3 |= 1 << *(_BYTE *)(v7 + 2 * v10 + 4173);
      }
    }
  }
  else
  {
    LOBYTE(v3) = 30;
  }
  if ( (v3 & 2) != 0 )
    PerformanceCounter = KeQueryPerformanceCounter(0LL);
  else
    PerformanceCounter.QuadPart = 0LL;
  *a1 = PerformanceCounter;
  if ( (v3 & 4) != 0 )
    result = RtlGetSystemTimePrecise();
  else
    result.QuadPart = 0LL;
  a1[1] = result;
  if ( (v3 & 8) != 0 )
  {
    result.QuadPart = __rdtsc();
    a1[2] = result;
  }
  else
  {
    a1[2].QuadPart = 0LL;
  }
  if ( (v3 & 0x10) != 0 )
  {
    v14.QuadPart = 0LL;
    ((void (__fastcall *)(LARGE_INTEGER *))off_140398AC0)(&v14);
    result = v14;
    a1[3] = v14;
  }
  else
  {
    a1[3].QuadPart = 0LL;
  }
  return result;
}
