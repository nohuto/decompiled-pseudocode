/*
 * XREFs of IoSteerInterrupt @ 0x140723210
 * Callers:
 *     <none>
 * Callees:
 *     IoAddTriageDumpDataBlock @ 0x140185870 (IoAddTriageDumpDataBlock.c)
 *     KeBugCheckEx @ 0x1401AAED0 (KeBugCheckEx.c)
 *     KeIntSteerAssignCpuSet @ 0x140249DC8 (KeIntSteerAssignCpuSet.c)
 */

__int64 __fastcall IoSteerInterrupt(__int64 a1, __int64 a2)
{
  unsigned int v3; // r9d
  int v4; // r10d
  ULONG_PTR v5; // r8
  __int64 v6; // rcx
  __int64 v7; // rcx
  __int64 v8; // r8
  unsigned __int16 v9; // dx
  __int64 v11; // rcx
  _WORD *v12; // rcx
  __int64 v13; // rcx
  __int64 v14; // rdi
  __int64 v15; // rcx
  __int64 v16; // rcx
  __int64 v17; // rcx
  __int64 v18; // rcx
  __int64 v19; // rdx
  __int64 v20; // rcx
  __int64 v21; // rcx
  __int64 v22; // rax
  __int64 v23; // rcx
  __int64 v24; // rcx
  __int64 v25; // rdx
  __int64 v26; // rcx

  v3 = -1073741823;
  if ( KeGetCurrentIrql() )
    KeBugCheckEx(0x121u, 1uLL, KeGetCurrentIrql(), 0LL, 0LL);
  if ( !a1 || !a2 )
    __fastfail(5u);
  if ( !*(_DWORD *)a1 )
  {
    v4 = *(_DWORD *)a2;
    if ( *(int *)a2 < 2 && (v4 != 1 || *(_QWORD *)(a2 + 16)) )
    {
      v5 = *(_QWORD *)(a1 + 16);
      if ( v5 )
      {
        v6 = *(_QWORD *)(*(_QWORD *)(v5 + 312) + 40LL);
        if ( v6 && (*(_DWORD *)(v6 + 396) & 0x20000) == 0 )
        {
          if ( *(_DWORD *)(a1 + 8) != 1 )
          {
            if ( *(_DWORD *)(a1 + 8) == 2 )
            {
              v7 = *(_QWORD *)(*(_QWORD *)(a1 + 24) + 264LL) + 112LL;
              goto LABEL_21;
            }
            if ( *(_DWORD *)(a1 + 8) == 3 )
            {
              v7 = *(_QWORD *)(*(_QWORD *)(a1 + 24) + 24LL);
              goto LABEL_21;
            }
            if ( *(_DWORD *)(a1 + 8) != 4 )
              return (unsigned int)-1073741637;
          }
          v7 = *(_QWORD *)(a1 + 24);
LABEL_21:
          if ( *(_DWORD *)(*(_QWORD *)(v7 + 352) + 88LL) != *(_DWORD *)(v7 + 88) )
            __fastfail(5u);
          if ( v4 == 1 )
          {
            v8 = *(_QWORD *)(a2 + 16);
            v9 = *(_WORD *)(a2 + 8);
          }
          else
          {
            if ( v4 )
              return v3;
            v9 = 0;
            v8 = 0LL;
          }
          return (unsigned int)KeIntSteerAssignCpuSet(*(_QWORD *)(v7 + 352), v9, v8);
        }
        IoAddTriageDumpDataBlock(v5, *(unsigned __int16 *)(v5 + 2));
        v5 = *(_QWORD *)(a1 + 16);
        v11 = *(_QWORD *)(v5 + 8);
        if ( v11 )
        {
          IoAddTriageDumpDataBlock(v11, *(__int16 *)(v11 + 2));
          v5 = *(_QWORD *)(a1 + 16);
          v12 = (_WORD *)(*(_QWORD *)(v5 + 8) + 56LL);
          if ( *v12 )
          {
            IoAddTriageDumpDataBlock((int)v12, 2);
            IoAddTriageDumpDataBlock(
              *(_QWORD *)(*(_QWORD *)(*(_QWORD *)(a1 + 16) + 8LL) + 64LL),
              *(unsigned __int16 *)(*(_QWORD *)(*(_QWORD *)(a1 + 16) + 8LL) + 56LL));
            v5 = *(_QWORD *)(a1 + 16);
          }
        }
        if ( v5 )
          v13 = *(_QWORD *)(*(_QWORD *)(v5 + 312) + 40LL);
        else
          v13 = 0LL;
        if ( v13 )
        {
          if ( v5 )
            v14 = *(_QWORD *)(*(_QWORD *)(v5 + 312) + 40LL);
          else
            v14 = 0LL;
          if ( v5 )
            v15 = *(_QWORD *)(*(_QWORD *)(v5 + 312) + 40LL);
          else
            LODWORD(v15) = 0;
          IoAddTriageDumpDataBlock(v15, 728);
          if ( *(_WORD *)(v14 + 40) )
          {
            IoAddTriageDumpDataBlock(v14 + 40, 2);
            IoAddTriageDumpDataBlock(*(_QWORD *)(v14 + 48), *(unsigned __int16 *)(v14 + 40));
          }
          v5 = *(_QWORD *)(a1 + 16);
          if ( v5 )
            v16 = *(_QWORD *)(*(_QWORD *)(v5 + 312) + 40LL);
          else
            v16 = 0LL;
          if ( *(_WORD *)(v16 + 56) )
          {
            if ( v5 )
              v17 = *(_QWORD *)(*(_QWORD *)(v5 + 312) + 40LL);
            else
              LODWORD(v17) = 0;
            IoAddTriageDumpDataBlock(v17 + 56, 2);
            v18 = *(_QWORD *)(a1 + 16);
            if ( v18 )
              v19 = *(_QWORD *)(*(_QWORD *)(v18 + 312) + 40LL);
            else
              v19 = 0LL;
            if ( v18 )
              v20 = *(_QWORD *)(*(_QWORD *)(v18 + 312) + 40LL);
            else
              v20 = 0LL;
            IoAddTriageDumpDataBlock(*(_QWORD *)(v20 + 64), *(unsigned __int16 *)(v19 + 56));
            v5 = *(_QWORD *)(a1 + 16);
          }
          if ( v5 )
          {
            v21 = *(_QWORD *)(*(_QWORD *)(v5 + 312) + 40LL);
          }
          else
          {
            v21 = 0LL;
            v5 = 0LL;
          }
          if ( *(_QWORD *)(v21 + 16) )
          {
            if ( v5 )
            {
              v22 = *(_QWORD *)(*(_QWORD *)(v5 + 312) + 40LL);
            }
            else
            {
              v22 = 0LL;
              v5 = 0LL;
            }
            if ( *(_WORD *)(*(_QWORD *)(v22 + 16) + 56LL) )
            {
              if ( v5 )
                v23 = *(_QWORD *)(*(_QWORD *)(v5 + 312) + 40LL);
              else
                v23 = 0LL;
              IoAddTriageDumpDataBlock(*(_QWORD *)(v23 + 16) + 56, 2);
              v24 = *(_QWORD *)(a1 + 16);
              if ( v24 )
                v25 = *(_QWORD *)(*(_QWORD *)(v24 + 312) + 40LL);
              else
                v25 = 0LL;
              if ( v24 )
                v26 = *(_QWORD *)(*(_QWORD *)(v24 + 312) + 40LL);
              else
                v26 = 0LL;
              IoAddTriageDumpDataBlock(
                *(_QWORD *)(*(_QWORD *)(v26 + 16) + 64LL),
                *(unsigned __int16 *)(*(_QWORD *)(v25 + 16) + 56LL));
              v5 = *(_QWORD *)(a1 + 16);
            }
          }
        }
      }
      KeBugCheckEx(0xCAu, 2uLL, v5, 0LL, 0LL);
    }
  }
  return (unsigned int)-1073741811;
}
