/*
 * XREFs of BgpFwAcquireLock @ 0x140164F8C
 * Callers:
 ...
 *     BgAcquireSpinLock @ 0x14083DCA8 (BgAcquireSpinLock.c)
...
void BgpFwAcquireLock()
{
  ...
  if ( (dword_14039D7F0 & 0xC00) != 0xC00 )
  {
    ...
    if ( CurrentIrql <= 2u )
    {
      while ( qword_1403E4B10 )
        _mm_pause();
      CurrentIrql = KeGetCurrentIrql();
      __writecr8(2uLL);
    }
    KxAcquireSpinLock(&qword_1403E4B10);
    byte_1403D1310 = CurrentIrql;
  }
}
