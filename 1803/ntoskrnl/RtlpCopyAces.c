/*
 * XREFs of RtlpCopyAces @ 0x1404C6710
 * Callers:
 *     RtlpInheritAcl2 @ 0x1404C5AE0 (RtlpInheritAcl2.c)
 *     RtlpComputeMergedAcl2 @ 0x140785814 (RtlpComputeMergedAcl2.c)
 * Callees:
 *     RtlFindAceByType @ 0x14005C9A0 (RtlFindAceByType.c)
 *     __security_check_cookie @ 0x140187410 (__security_check_cookie.c)
 *     memmove @ 0x1401BC900 (memmove.c)
 *     RtlpCopyEffectiveAce @ 0x1404C8940 (RtlpCopyEffectiveAce.c)
 */

__int64 __fastcall RtlpCopyAces(
        __int64 a1,
        GENERIC_MAPPING *a2,
        int a3,
        char a4,
        char a5,
        void *a6,
        __int64 a7,
        __int64 a8,
        __int64 a9,
        char a10,
        __int64 a11,
        int a12,
        unsigned int *a13,
        __int64 a14)
{
  int v14; // esi
  GENERIC_MAPPING *GenericMapping; // r10
  __int64 v16; // rbp
  unsigned int *v17; // r14
  unsigned __int8 v18; // al
  char v19; // r9
  unsigned __int64 v20; // rcx
  unsigned int v21; // r8d
  int *v22; // rbx
  __int64 v23; // rdi
  unsigned int v24; // r12d
  unsigned int v25; // r13d
  char v26; // r14
  char v27; // cl
  signed __int64 v28; // rsi
  int v29; // ecx
  GENERIC_MAPPING *v30; // rdx
  int v31; // ecx
  unsigned int v32; // eax
  int v33; // r8d
  int GenericAll; // eax
  int v36; // ecx
  char v37; // al
  int *v38; // rbp
  bool v39; // r14
  char v40; // r8
  int v41; // edx
  char v43; // [rsp+81h] [rbp-97h]
  _WORD v44[7]; // [rsp+82h] [rbp-96h] BYREF
  __int64 v45; // [rsp+90h] [rbp-88h] BYREF
  GENERIC_MAPPING *v46; // [rsp+98h] [rbp-80h]
  int v47; // [rsp+A0h] [rbp-78h]
  void *v48[2]; // [rsp+A8h] [rbp-70h] BYREF
  __int64 v49; // [rsp+B8h] [rbp-60h]
  __int64 v50; // [rsp+C0h] [rbp-58h]
  __int64 v51; // [rsp+C8h] [rbp-50h]
  unsigned int *v52; // [rsp+D0h] [rbp-48h]
  int v53; // [rsp+D8h] [rbp-40h]
  unsigned __int16 v54; // [rsp+DCh] [rbp-3Ch]

  v14 = a3;
  GenericMapping = a2;
  v16 = a1;
  v17 = a13;
  v51 = a7;
  v50 = a8;
  v49 = a9;
  v18 = *(_BYTE *)a14 - 2;
  v19 = 0;
  v47 = a3;
  v46 = a2;
  *(_QWORD *)&v44[3] = a1;
  v48[1] = a6;
  v52 = a13;
  v43 = 0;
  if ( v18 > 2u )
    return 3221225560LL;
  v20 = a14 + 8;
  v21 = 0;
  v22 = 0LL;
  if ( *(_WORD *)(a14 + 4) )
  {
    do
    {
      if ( v20 >= a14 + (unsigned __int64)*(unsigned __int16 *)(a14 + 2) )
        return 3221225597LL;
      ++v21;
      v20 += *(unsigned __int16 *)(v20 + 2);
    }
    while ( v21 < *(unsigned __int16 *)(a14 + 4) );
    v19 = 0;
  }
  if ( v20 <= a14 + (unsigned __int64)*(unsigned __int16 *)(a14 + 2) )
    v22 = (int *)v20;
  v23 = v16 + 8;
  v24 = 0;
  v25 = 0;
  if ( !*(_WORD *)(v16 + 4) )
  {
LABEL_37:
    *v17 = v24;
    return v19 != 0 ? 0xC0000023 : 0;
  }
  v26 = a4;
  while ( 1 )
  {
    v27 = a5;
    if ( *(_BYTE *)v23 == 17 )
      break;
    if ( a12 == 3 )
      goto LABEL_34;
LABEL_9:
    if ( v14 )
    {
      if ( v14 != 1 )
      {
        if ( v14 != 2 )
          goto LABEL_34;
LABEL_12:
        if ( !v27 )
        {
          v28 = *(unsigned __int16 *)(v23 + 2);
          if ( !v22 || v28 > a14 + *(unsigned __int16 *)(a14 + 2) - (_QWORD)v22 )
          {
            v19 = 1;
            v43 = 1;
            v22 = (int *)(a14 + *(unsigned __int16 *)(a14 + 2));
            goto LABEL_33;
          }
          if ( !v19 )
          {
            memmove(v22, (const void *)v23, *(unsigned __int16 *)(v23 + 2));
            if ( (*(_BYTE *)v22 < 0xBu || (unsigned __int8)(*(_BYTE *)v22 - 13) <= 1u) && (*((_BYTE *)v22 + 1) & 8) == 0 )
            {
              v29 = v22[1];
              v30 = v46;
              if ( v29 < 0 )
              {
                v29 |= v46->GenericRead;
                v22[1] = v29;
              }
              if ( (v29 & 0x40000000) != 0 )
              {
                v29 |= v30->GenericWrite;
                v22[1] = v29;
              }
              if ( (v29 & 0x20000000) != 0 )
              {
                v29 |= v30->GenericExecute;
                v22[1] = v29;
              }
              if ( (v29 & 0x10000000) != 0 )
              {
                v29 |= v30->GenericAll;
                v22[1] = v29;
              }
              v31 = v29 & 0xFFFFFFF;
              v22[1] = v31;
              v32 = *(unsigned __int8 *)v22;
              if ( (unsigned __int8)v32 <= 0xAu && (v33 = 1651, _bittest(&v33, v32)) )
                GenericAll = v30->GenericAll;
              else
                GenericAll = v30->GenericAll | 0x1000000;
              v22[1] = v31 & GenericAll;
            }
            *((_BYTE *)v22 + 1) &= ~v26;
            goto LABEL_31;
          }
LABEL_62:
          v22 = (int *)(a14 + *(unsigned __int16 *)(a14 + 2));
          goto LABEL_33;
        }
        v36 = 0;
        v37 = *(_BYTE *)(v23 + 1);
        v38 = v22;
        LODWORD(v45) = 0;
        LODWORD(v28) = 0;
        LOBYTE(v44[0]) = 0;
        v48[0] = v22;
        v39 = a10 && (v37 & 3) != 0;
        if ( (v37 & 8) == 0 )
        {
          if ( !(unsigned __int8)RtlpCopyEffectiveAce(
                                   (void *)v23,
                                   v51,
                                   v50,
                                   v49,
                                   GenericMapping,
                                   0LL,
                                   0,
                                   (__int64)v48,
                                   (__int64)&v45,
                                   a14,
                                   0LL,
                                   (__int64)v44,
                                   (__int64)v44 + 1) )
            return 3221225597LL;
          if ( HIBYTE(v44[0]) )
          {
            v19 = 1;
            v43 = 1;
          }
          else
          {
            v19 = v43;
          }
          v36 = v45;
          LODWORD(v28) = v45;
          if ( !v19 && (_DWORD)v45 )
          {
            v40 = a4;
            v38 = (int *)v48[0];
            *((_BYTE *)v22 + 1) &= ~a4;
LABEL_48:
            if ( !v39 )
              goto LABEL_49;
            v41 = *(_DWORD *)(v23 + 10);
            v53 = 0;
            v54 = 768;
            if ( !v41 )
              v41 = *(unsigned __int16 *)(v23 + 14) - v54;
            if ( v36 && !LOBYTE(v44[0]) )
            {
              v16 = *(_QWORD *)&v44[3];
              v26 = a4;
              if ( !v19 )
              {
                *((_BYTE *)v22 + 1) = ~v40 & (*((_BYTE *)v22 + 1) | *(_BYTE *)(v23 + 1) & 0x1F);
                goto LABEL_32;
              }
              goto LABEL_62;
            }
            if ( *(_BYTE *)v23 > 8u || *(_DWORD *)(v23 + 4) || !v41 )
            {
              LODWORD(v28) = *(unsigned __int16 *)(v23 + 2) + (_DWORD)v28;
              if ( (unsigned int)v28 > 0xFFFF )
                return 3221225597LL;
              if ( *(unsigned __int16 *)(v23 + 2) > a14 + *(unsigned __int16 *)(a14 + 2) - (_QWORD)v38 )
              {
                v19 = 1;
                v43 = 1;
              }
              else if ( !v19 )
              {
                memmove(v38, (const void *)v23, *(unsigned __int16 *)(v23 + 2));
                v26 = a4;
                *((_BYTE *)v38 + 1) = ~a4 & (*((_BYTE *)v38 + 1) | 8);
                v16 = *(_QWORD *)&v44[3];
LABEL_31:
                v19 = v43;
                ++*(_WORD *)(a14 + 4);
LABEL_32:
                v22 = (int *)((char *)v22 + (unsigned int)v28);
LABEL_33:
                v24 += v28;
                v14 = v47;
                goto LABEL_34;
              }
            }
            else
            {
LABEL_49:
              if ( !v19 )
              {
                v16 = *(_QWORD *)&v44[3];
                v26 = a4;
                goto LABEL_32;
              }
            }
            v16 = *(_QWORD *)&v44[3];
            v26 = a4;
            goto LABEL_62;
          }
          v38 = (int *)v48[0];
        }
        v40 = a4;
        goto LABEL_48;
      }
      if ( (*(_BYTE *)(v23 + 1) & 0x10) == 0 )
        goto LABEL_12;
    }
    else if ( (*(_BYTE *)(v23 + 1) & 0x10) != 0 )
    {
      goto LABEL_12;
    }
LABEL_34:
    ++v25;
    v23 += *(unsigned __int16 *)(v23 + 2);
    if ( v25 >= *(unsigned __int16 *)(v16 + 4) )
      goto LABEL_35;
    GenericMapping = v46;
  }
  if ( a12 != 3 )
    goto LABEL_34;
  if ( !RtlFindAceByType(a14, 17, 0LL) )
  {
    v19 = v43;
    v27 = a5;
    GenericMapping = v46;
    goto LABEL_9;
  }
LABEL_35:
  if ( v24 <= 0xFFFF )
  {
    v19 = v43;
    v17 = v52;
    goto LABEL_37;
  }
  return 3221225597LL;
}
