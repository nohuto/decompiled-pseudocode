/*
 * XREFs of KiTimerExpiration @ 0x1401543F4
 * Callers:
 *     KiTimerExpirationDpc @ 0x140154320 (KiTimerExpirationDpc.c)
 * Callees:
 *     EtwTraceKernelEvent @ 0x1400C1C38 (EtwTraceKernelEvent.c)
 *     KiExpireTimerTable @ 0x140154548 (KiExpireTimerTable.c)
 *     KiSelectActiveTimerTable @ 0x140168150 (KiSelectActiveTimerTable.c)
 *     __security_check_cookie @ 0x140187410 (__security_check_cookie.c)
 */

__int64 __fastcall KiTimerExpiration(__int64 a1, __int64 a2, unsigned __int64 a3, __int64 a4)
{
  int v4; // r14d
  __int64 result; // rax
  __int64 v9; // rcx
  __int64 v10; // r8
  int v11; // r12d
  unsigned int v12; // ecx
  unsigned __int64 v13; // rbp
  __int64 v14; // rsi
  unsigned int v15; // ebx
  int v16; // ebp
  int v17; // eax
  unsigned __int64 *v18; // [rsp+40h] [rbp-78h] BYREF
  int v19; // [rsp+48h] [rbp-70h]
  int v20; // [rsp+4Ch] [rbp-6Ch]
  unsigned __int64 v21; // [rsp+50h] [rbp-68h] BYREF
  char v22; // [rsp+58h] [rbp-60h]

  v4 = a2;
  LOBYTE(a2) = 1;
  result = KiSelectActiveTimerTable(a1, a2);
  v11 = result;
  if ( result )
  {
    v12 = *(_DWORD *)(v9 + 26624);
    *(_DWORD *)(a1 + 26624) = ((_BYTE)v12 + 1) & 0xF;
    *(_QWORD *)(a1 + 16LL * v12 + 26632) = v10;
    *(LARGE_INTEGER *)(a1 + 16LL * v12 + 26640) = KeQueryPerformanceCounter(0LL);
    v13 = a3 >> 18;
    if ( (DWORD2(PerfGlobalGroupMask) & 0x20000) != 0 )
    {
      v20 = 0;
      v18 = &v21;
      v21 = a3;
      v22 = 0;
      v19 = 16;
      EtwTraceKernelEvent((int)&v18, 1, 0x40020000u, 3920, 1538);
    }
    v14 = 0LL;
    v15 = v13 - v4 + 1;
    if ( v15 > 0x100 )
      v14 = (v13 - v15 + 1) << 18;
    do
    {
      if ( v15 <= 0x100 )
      {
        v16 = v15;
        v17 = 256 - v15;
        v14 = a3;
        if ( 256 - v15 > 0x18 )
          v17 = 24;
      }
      else
      {
        v14 += 0x4000000LL;
        v16 = 256;
        v17 = 0;
      }
      result = KiExpireTimerTable(a1, v11, v4, v16, v17, v14, a4);
      v4 += v16;
      v15 -= v16;
    }
    while ( v15 );
    if ( (*(_BYTE *)(a1 + 11884) & 8) == 0 )
    {
      *(_DWORD *)(a1 + 23460) = 0;
      result = *(unsigned int *)(a1 + 23464);
      if ( (unsigned int)result >= KeTimeIncrement )
      {
        result = (unsigned int)(result - KeTimeIncrement);
        *(_DWORD *)(a1 + 23464) = result;
      }
      else
      {
        *(_DWORD *)(a1 + 23464) = 0;
      }
    }
  }
  return result;
}
