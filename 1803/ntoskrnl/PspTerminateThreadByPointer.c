/*
 * XREFs of PspTerminateThreadByPointer @ 0x1404BB3D0
 * Callers:
 *     PspSystemThreadStartup @ 0x140098F60 (PspSystemThreadStartup.c)
 *     PsDispatchIumService @ 0x14015CBE4 (PsDispatchIumService.c)
 *     PspUserThreadStartup @ 0x1404B8600 (PspUserThreadStartup.c)
 *     NtTerminateThread @ 0x1404BB47C (NtTerminateThread.c)
 *     PspTerminateAllThreads @ 0x1404E89FC (PspTerminateAllThreads.c)
 *     NtTerminateProcess @ 0x14056E088 (NtTerminateProcess.c)
 *     PsTerminateSystemThread @ 0x140582610 (PsTerminateSystemThread.c)
 * Callees:
 *     KeRequestTerminationThread @ 0x140043C6C (KeRequestTerminationThread.c)
 *     PsGetProcessServerSilo @ 0x14006CFF0 (PsGetProcessServerSilo.c)
 *     PspExitThread @ 0x1404B8D70 (PspExitThread.c)
 *     PspCatchCriticalBreak @ 0x14077D014 (PspCatchCriticalBreak.c)
 */

__int64 __fastcall PspTerminateThreadByPointer(__int64 a1, __int64 a2, char a3)
{
  __int64 v4; // rdi
  unsigned int v5; // esi

  v4 = *(_QWORD *)(a1 + 544);
  v5 = a2;
  if ( (*(_DWORD *)(a1 + 1744) & 0x20) != 0 && (*(_DWORD *)(v4 + 772) & 0x40000008) == 0 )
  {
    PsGetProcessServerSilo(v4);
    PspCatchCriticalBreak("Terminating critical thread 0x%p (in %s)\n", (const void *)a1, (const char *)(v4 + 1104));
  }
  if ( a3 && (struct _KTHREAD *)a1 == KeGetCurrentThread() )
  {
    _InterlockedOr((volatile signed __int32 *)(a1 + 1744), 1u);
    PspExitThread(v5);
    __debugbreak();
  }
  if ( (*(_DWORD *)(a1 + 116) & 0x400) != 0 )
    return 3221225506LL;
  if ( (*(_DWORD *)(v4 + 1740) & 1) != 0 )
    return 3221225659LL;
  if ( !_interlockedbittestandset((volatile signed __int32 *)(a1 + 1744), 0) )
    *(_DWORD *)(a1 + 1792) = v5;
  KeRequestTerminationThread(a1, a2);
  return 0LL;
}
