/*
 * XREFs of ?WilApiImpl_GetFeatureEnabledState@details@wil@@YA?AW4FEATURE_ENABLED_STATE@@IW4FEATURE_CHANGE_TIME@@@Z @ 0x1800BB2C0
 * Callers:
 *     <none>
 * Callees:
 *     ?IsFeatureConfigured@details@wil@@YA_NPEAUwil_FeatureState@@I_NW4wil_FeatureStore@@@Z @ 0x1800BB32C (-IsFeatureConfigured@details@wil@@YA_NPEAUwil_FeatureState@@I_NW4wil_FeatureStore@@@Z.c)
 */

__int64 __fastcall wil::details::WilApiImpl_GetFeatureEnabledState(wil::details *this, unsigned int a2, __int64 a3)
{
  char v3; // al
  unsigned int v4; // ecx
  unsigned int v5; // edx
  __int64 result; // rax
  _QWORD v7[2]; // [rsp+20h] [rbp-28h] BYREF
  __int64 v8; // [rsp+30h] [rbp-18h]

  LOBYTE(a3) = (a2 & 0xFFFFFF7F) - 2 <= 1;
  v7[0] = 0LL;
  v7[1] = 0LL;
  v8 = 0LL;
  v3 = ((__int64 (__fastcall *)(_QWORD *, _QWORD, __int64, _QWORD))wil::details::IsFeatureConfigured)(
         v7,
         (unsigned int)this,
         a3,
         (a2 >> 7) & 1);
  v4 = 0;
  if ( v3 )
    v4 = v7[0];
  v5 = v4 | 0x80;
  if ( !(_DWORD)v8 )
    v5 = v4;
  result = v5 | 0x40;
  if ( !HIDWORD(v8) )
    return v5;
  return result;
}
