/*
 * XREFs of ?DepartBufferedOutputPointer@CInteractionProcessor@@QEAA_NIW4InputType@@@Z @ 0x1801D4DCC
 * Callers:
 *     ?DepartBufferedOutputPointer@CInteraction@@UEAA_NI@Z @ 0x180191C20 (-DepartBufferedOutputPointer@CInteraction@@UEAA_NI@Z.c)
 *     ?DepartBufferedOutputPointer@CInteractionRoot@@UEAA_NI@Z @ 0x1801DA4E0 (-DepartBufferedOutputPointer@CInteractionRoot@@UEAA_NI@Z.c)
 * Callees:
 *     ?InternalRelease@?$ComPtr@UIInteractionContextWrapper@@@WRL@Microsoft@@IEAAKXZ @ 0x180025008 (-InternalRelease@-$ComPtr@UIInteractionContextWrapper@@@WRL@Microsoft@@IEAAKXZ.c)
 *     _guard_dispatch_icall_nop @ 0x1800DD420 (_guard_dispatch_icall_nop.c)
 *     ?GetInteractionContext@CInteractionProcessor@@QEBAJW4InputType@@PEAPEAUIInteractionContextWrapper@@@Z @ 0x1801D4EE8 (-GetInteractionContext@CInteractionProcessor@@QEBAJW4InputType@@PEAPEAUIInteractionContextWrappe.c)
 */

char __fastcall CInteractionProcessor::DepartBufferedOutputPointer(__int64 a1, unsigned int a2)
{
  char v3; // si
  CNaturalAnimationScalarForceAdapater *v6; // [rsp+48h] [rbp+20h] BYREF

  v3 = 0;
  v6 = 0LL;
  Microsoft::WRL::ComPtr<IInteractionContextWrapper>::InternalRelease(&v6);
  CInteractionProcessor::GetInteractionContext(a1);
  if ( v6 )
    v3 = (*(__int64 (__fastcall **)(CNaturalAnimationScalarForceAdapater *, _QWORD))(*(_QWORD *)v6 + 104LL))(v6, a2);
  Microsoft::WRL::ComPtr<IInteractionContextWrapper>::InternalRelease(&v6);
  return v3;
}
