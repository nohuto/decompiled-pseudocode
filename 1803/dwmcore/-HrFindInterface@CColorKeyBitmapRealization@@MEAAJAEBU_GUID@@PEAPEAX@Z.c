/*
 * XREFs of ?HrFindInterface@CColorKeyBitmapRealization@@MEAAJAEBU_GUID@@PEAPEAX@Z @ 0x180213880
 * Callers:
 *     <none>
 * Callees:
 *     ?HrFindInterface@CD2DBitmapCache@@MEAAJAEBU_GUID@@PEAPEAX@Z @ 0x18013DAA0 (-HrFindInterface@CD2DBitmapCache@@MEAAJAEBU_GUID@@PEAPEAX@Z.c)
 */

__int64 __fastcall CColorKeyBitmapRealization::HrFindInterface(
        CColorKeyBitmapRealization *this,
        const struct _GUID *a2,
        void **a3)
{
  __int64 v5; // rcx
  unsigned int v6; // r8d
  unsigned __int64 v7; // rcx
  __int64 v8; // rcx

  v5 = *(_QWORD *)&a2->Data1 - *(_QWORD *)&GUID_db178a3c_d733_4570_9513_5392d717955f.Data1;
  if ( *(_QWORD *)&a2->Data1 == *(_QWORD *)&GUID_db178a3c_d733_4570_9513_5392d717955f.Data1 )
    v5 = *(_QWORD *)a2->Data4 - *(_QWORD *)GUID_db178a3c_d733_4570_9513_5392d717955f.Data4;
  v6 = 0;
  if ( v5 )
  {
    v8 = *(_QWORD *)&a2->Data1 - *(_QWORD *)&GUID_475af409_d8b1_4ca5_8177_4562f6260b68.Data1;
    if ( *(_QWORD *)&a2->Data1 == *(_QWORD *)&GUID_475af409_d8b1_4ca5_8177_4562f6260b68.Data1 )
      v8 = *(_QWORD *)a2->Data4 - *(_QWORD *)GUID_475af409_d8b1_4ca5_8177_4562f6260b68.Data4;
    if ( v8 )
      return (unsigned int)CD2DBitmapCache::HrFindInterface(this, a2, a3);
    v7 = (unsigned __int64)this + 88;
  }
  else
  {
    v7 = (unsigned __int64)this + 144;
  }
  *a3 = (void *)(v7 & -(__int64)(this != 0LL));
  return v6;
}
