/*
 * XREFs of RtlpConvertCultureNamesToLCIDs @ 0x1800E8C70
 * Callers:
 *     <none>
 * Callees:
 *     RtlFreeHeap @ 0x180016DF0 (RtlFreeHeap.c)
 *     RtlAllocateHeap @ 0x1800190C0 (RtlAllocateHeap.c)
 *     sub_1800310D8 @ 0x1800310D8 (sub_1800310D8.c)
 *     RtlCultureNameToLCID @ 0x1800367F0 (RtlCultureNameToLCID.c)
 *     RtlInitUnicodeString @ 0x1800369B0 (RtlInitUnicodeString.c)
 *     __security_check_cookie @ 0x18008B0F0 (__security_check_cookie.c)
 *     wcsnlen @ 0x180090E20 (wcsnlen.c)
 *     RtlConvertLCIDToString @ 0x1800E7AB0 (RtlConvertLCIDToString.c)
 */

__int64 __fastcall RtlpConvertCultureNamesToLCIDs(__int16 *SourceString, unsigned __int64 *a2)
{
  unsigned int v2; // ebx
  const wchar_t *v4; // rdi
  unsigned __int64 v5; // rbp
  unsigned int v6; // esi
  int v7; // r14d
  unsigned int v8; // eax
  unsigned int v9; // r15d
  unsigned int v10; // ecx
  _DWORD *v11; // rax
  __int64 v12; // r12
  __int64 Heap; // rax
  _WORD *v14; // rdi
  unsigned int v15; // r14d
  unsigned int *v16; // r15
  __int64 v17; // rcx
  __int64 v18; // rax
  int v20; // [rsp+30h] [rbp-68h] BYREF
  UNICODE_STRING DestinationString; // [rsp+38h] [rbp-60h] BYREF
  _DWORD v22[4]; // [rsp+48h] [rbp-50h] BYREF

  v2 = 0;
  v4 = (const wchar_t *)SourceString;
  v5 = 0LL;
  v6 = 0;
  if ( !a2 || !SourceString || *a2 || (int)sub_1800310D8(SourceString, (__int64)a2, &v20) < 0 )
    return 3221225485LL;
  v7 = 2 * v20;
  v8 = 2 * v20;
  while ( *v4 && v8 )
  {
    if ( v6 >= 4 )
      goto LABEL_17;
    v9 = 2 * wcsnlen(v4, (unsigned __int64)v8 >> 1);
    RtlInitUnicodeString(&DestinationString, v4);
    if ( !RtlCultureNameToLCID(&DestinationString.Length, &v22[v6]) )
      goto LABEL_17;
    v10 = 0;
    if ( v6 )
    {
      v11 = v22;
      while ( v22[v6] != *v11 )
      {
        ++v10;
        ++v11;
        if ( v10 >= v6 )
          goto LABEL_14;
      }
LABEL_17:
      v2 = -1073741811;
LABEL_27:
      *a2 = v5;
      return v2;
    }
LABEL_14:
    v4 = (const wchar_t *)((char *)v4 + v9 + 2);
    ++v6;
    v7 += -2 - v9;
    v8 = v7;
    if ( !v4 )
      break;
  }
  v12 = (unsigned __int16)(((_WORD)v6 << 6) + 4);
  Heap = RtlAllocateHeap((__int64)NtCurrentPeb()->ProcessHeap, 0, v12);
  v5 = Heap;
  if ( !Heap )
  {
    v2 = -1073741801;
    goto LABEL_27;
  }
  v14 = (_WORD *)Heap;
  v15 = 0;
  if ( !v6 )
  {
LABEL_26:
    *(_DWORD *)v14 = 0;
    goto LABEL_27;
  }
  v16 = v22;
  while ( 1 )
  {
    RtlConvertLCIDToString(*v16, 0x10u, 4u, v14, 0x20u);
    v17 = -1LL;
    do
      ++v17;
    while ( v14[v17] );
    LODWORD(v12) = -2 - v17 + v12;
    v18 = -1LL;
    do
      ++v18;
    while ( v14[v18] );
    v14 += v18 + 1;
    if ( (unsigned int)v12 < 4 )
      break;
    ++v15;
    ++v16;
    if ( v15 >= v6 )
      goto LABEL_26;
  }
  v2 = -1073741595;
  RtlFreeHeap((__int64)NtCurrentPeb()->ProcessHeap, 0, v5);
  return v2;
}
