/*
 * XREFs of sub_18008CDCC @ 0x18008CDCC
 * Callers:
 *     _wcstoi64 @ 0x18008CD70 (_wcstoi64.c)
 *     _wcstoui64 @ 0x18008CDA0 (_wcstoui64.c)
 * Callees:
 *     _errno @ 0x180083380 (_errno.c)
 *     sub_18008B020 @ 0x18008B020 (sub_18008B020.c)
 *     sub_18008EA4C @ 0x18008EA4C (sub_18008EA4C.c)
 *     sub_180092A04 @ 0x180092A04 (sub_180092A04.c)
 */

unsigned __int64 __fastcall sub_18008CDCC(
        __int64 a1,
        unsigned __int16 *a2,
        unsigned __int16 **a3,
        int a4,
        int a5,
        int *a6)
{
  int v6; // r14d
  unsigned __int64 result; // rax
  unsigned __int16 v10; // bp
  unsigned __int16 *v11; // rbx
  unsigned __int64 v12; // rsi
  __int64 i; // rcx
  int v14; // edi
  unsigned __int64 v15; // r13
  unsigned int v16; // ecx
  int v17; // ecx
  __int64 v18; // rdx
  unsigned __int16 *v19; // rbx
  int *v20; // rax

  v6 = a4;
  if ( a3 )
    *a3 = a2;
  if ( !a2 || a4 && (unsigned int)(a4 - 2) > 0x22 )
  {
    sub_18008B020();
    return 0LL;
  }
  v10 = *a2;
  v11 = a2 + 1;
  v12 = 0LL;
  for ( i = *a2; (unsigned int)sub_18008EA4C(i, 8LL, 0LL); v10 = i )
  {
    do
      i = *v11++;
    while ( (_WORD)i == v10 );
  }
  v14 = a5;
  if ( v10 == 45 )
  {
    v14 = a5 | 2;
LABEL_15:
    v10 = *v11++;
    goto LABEL_16;
  }
  if ( v10 == 43 )
    goto LABEL_15;
LABEL_16:
  if ( v6 && (unsigned int)(v6 - 2) > 0x22 )
  {
    if ( a3 )
      *a3 = a2;
    return 0LL;
  }
  if ( v6 )
    goto LABEL_26;
  if ( !(unsigned int)sub_180092A04(v10) )
  {
    if ( ((*v11 - 88) & 0xFFDF) != 0 )
    {
      v6 = 8;
      goto LABEL_30;
    }
    v6 = 16;
LABEL_26:
    if ( v6 == 16 && !(unsigned int)sub_180092A04(v10) && ((*v11 - 88) & 0xFFDF) == 0 )
    {
      v10 = v11[1];
      v11 += 2;
    }
    goto LABEL_30;
  }
  v6 = 10;
LABEL_30:
  v15 = 0xFFFFFFFFFFFFFFFFuLL / v6;
  while ( 1 )
  {
    v16 = sub_180092A04(v10);
    if ( v16 != -1 )
      goto LABEL_37;
    if ( (unsigned __int16)(v10 - 65) > 0x19u && (unsigned __int16)(v10 - 97) > 0x19u )
      break;
    v17 = v10 - 32;
    if ( (unsigned __int16)(v10 - 97) > 0x19u )
      v17 = v10;
    v16 = v17 - 55;
LABEL_37:
    if ( v16 >= v6 )
      break;
    v14 |= 8u;
    if ( v12 < v15 )
    {
      v18 = v16;
LABEL_47:
      v12 = v18 + v6 * v12;
      goto LABEL_48;
    }
    if ( v12 == v15 )
    {
      v18 = v16;
      if ( v16 <= 0xFFFFFFFFFFFFFFFFuLL % v6 )
        goto LABEL_47;
    }
    v14 |= 4u;
    if ( !a3 )
      break;
LABEL_48:
    v10 = *v11++;
  }
  v19 = v11 - 1;
  if ( (v14 & 8) != 0 )
  {
    if ( (v14 & 4) == 0 )
    {
      if ( (v14 & 1) != 0 )
        goto LABEL_60;
      if ( (v14 & 2) != 0 )
      {
        if ( v12 <= 0x8000000000000000uLL )
          goto LABEL_60;
      }
      else if ( v12 <= 0x7FFFFFFFFFFFFFFFLL )
      {
        goto LABEL_60;
      }
    }
    v20 = a6;
    if ( !a6 )
      v20 = errno();
    *v20 = 34;
    if ( (v14 & 1) != 0 )
      v12 = -1LL;
    else
      v12 = ((v14 & 2) != 0) + 0x7FFFFFFFFFFFFFFFLL;
  }
  else
  {
    if ( a3 )
      v19 = a2;
    v12 = 0LL;
  }
LABEL_60:
  if ( a3 )
    *a3 = v19;
  result = -(__int64)v12;
  if ( (v14 & 2) == 0 )
    return v12;
  return result;
}
