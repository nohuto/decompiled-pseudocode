...
NTSTATUS __cdecl TpAllocCleanupGroup(PTP_CLEANUP_GROUP *CleanupGroupReturn)
{
  ...
  {
    sub_1801086C8(CleanupGroupReturn, Ldr, v2);
    return -1073741811;
  }
  else
  {
    *CleanupGroupReturn = 0LL;
    Heap = RtlAllocateHeap(NtCurrentPeb()->ProcessHeap, dword_18015D050 | 8, 0x50uLL);
    ...
    if ( result < 0 )
    {
      if ( v5 )
      {
        RtlFreeHeap(NtCurrentPeb()->ProcessHeap, dword_18015D050, BaseAddress);
        return v8;
      }
    }
  }
