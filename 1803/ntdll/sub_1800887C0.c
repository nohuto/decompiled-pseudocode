/*
 * XREFs of sub_1800887C0 @ 0x1800887C0
 * Callers:
 *     <none>
 * Callees:
 *     sub_18009F9D0 @ 0x18009F9D0 (sub_18009F9D0.c)
 *     memmove @ 0x1800A1380 (memmove.c)
 *     memset @ 0x1800A16C0 (memset.c)
 */

__int64 __fastcall sub_1800887C0(char *a1, int a2, __int16 *a3, unsigned int a4, int a5, int *a6)
{
  __int16 *v6; // rsi
  char *v7; // r15
  int v8; // edi
  char *v9; // rbp
  __int16 v10; // bx
  unsigned __int64 v11; // r13
  __int16 i; // r14
  unsigned int v13; // r14d
  unsigned int v14; // r9d
  int v15; // r8d
  __int64 v16; // rbx
  unsigned int v17; // edx
  int *v18; // rax
  unsigned int v20; // ecx
  size_t v21; // r8
  char *v22; // r14
  int v23; // [rsp+70h] [rbp+8h]
  int v24; // [rsp+78h] [rbp+10h] BYREF
  char v25; // [rsp+88h] [rbp+20h] BYREF

  v23 = (int)a1;
  v6 = a3;
  v7 = &a1[a2];
  v24 = 0;
  v8 = 0;
  v9 = a1;
  v10 = *a3;
  v11 = (unsigned __int64)a3 + a4;
  for ( i = *a3; ; i = v10 )
  {
    v13 = (i & 0xFFF) + 3;
    if ( (unsigned __int64)v6 + v13 > v11 )
    {
LABEL_26:
      v18 = a6;
LABEL_15:
      v8 = -1073741246;
      *v18 = (int)v6;
      return (unsigned int)v8;
    }
    if ( v10 >= 0 )
    {
      if ( v13 < 2 )
        goto LABEL_26;
      v20 = v13 - 2;
      v24 = v13 - 2;
      if ( &v9[v13 - 2] > v7 )
      {
        v20 = (_DWORD)v7 - (_DWORD)v9;
        v24 = (_DWORD)v7 - (_DWORD)v9;
      }
      if ( (unsigned __int64)v6 + v20 + 2 > v11 )
        goto LABEL_26;
      memmove(v9, v6 + 1, v20);
      v16 = v13;
      goto LABEL_6;
    }
    v14 = v13 + (_DWORD)v6;
    v15 = (_DWORD)v6 + 2;
    v16 = v13;
    if ( a5 )
      break;
    v8 = sub_18009F9D0((_DWORD)v9, (_DWORD)v7, v15, v14, (__int64)&v24);
    if ( v8 < 0 )
      goto LABEL_25;
LABEL_6:
    v17 = v24;
LABEL_7:
    v6 = (__int16 *)((char *)v6 + v16);
    v9 += v17;
    if ( v9 == v7 || (unsigned __int64)v6 > v11 - 2 || (v10 = *v6) == 0 )
    {
LABEL_8:
      v18 = a6;
      if ( (unsigned __int64)v6 <= v11 )
      {
        *a6 = (_DWORD)v9 - v23;
        return (unsigned int)v8;
      }
      goto LABEL_15;
    }
    if ( v17 < 0x1000 )
    {
      v21 = (int)(4096 - v17);
      v22 = &v9[v21];
      if ( &v9[v21] >= v7 )
        goto LABEL_8;
      memset(v9, 0, v21);
      v9 = v22;
    }
  }
  v8 = sub_18009F9D0((_DWORD)v9, (_DWORD)v7, v15, v14, (__int64)&v25);
  if ( v8 >= 0 )
  {
    v17 = a5;
    v24 = a5;
    goto LABEL_7;
  }
LABEL_25:
  *a6 = v24;
  return (unsigned int)v8;
}
