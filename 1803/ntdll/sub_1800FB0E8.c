/*
 * XREFs of sub_1800FB0E8 @ 0x1800FB0E8
 * Callers:
 *     sub_1800FAD5C @ 0x1800FAD5C (sub_1800FAD5C.c)
 * Callees:
 *     RtlCompareUnicodeStrings @ 0x180022BB0 (RtlCompareUnicodeStrings.c)
 *     RtlLCIDToCultureName @ 0x1800361F0 (RtlLCIDToCultureName.c)
 *     RtlInitUnicodeString @ 0x1800369B0 (RtlInitUnicodeString.c)
 *     sub_18006F7EC @ 0x18006F7EC (sub_18006F7EC.c)
 *     sub_18008A9A4 @ 0x18008A9A4 (sub_18008A9A4.c)
 *     __security_check_cookie @ 0x18008B0F0 (__security_check_cookie.c)
 *     ZwEnumerateValueKey @ 0x18009AD20 (ZwEnumerateValueKey.c)
 *     ZwQueryValueKey @ 0x18009ADA0 (ZwQueryValueKey.c)
 *     ZwIsUILanguageComitted @ 0x18009CA30 (ZwIsUILanguageComitted.c)
 *     ZwQueryInstallUILanguage @ 0x18009D3B0 (ZwQueryInstallUILanguage.c)
 *     memset @ 0x1800A16C0 (memset.c)
 *     sub_1800FA6F0 @ 0x1800FA6F0 (sub_1800FA6F0.c)
 *     sub_1800FB548 @ 0x1800FB548 (sub_1800FB548.c)
 */

__int64 __fastcall sub_1800FB0E8(_WORD *a1, __int64 a2, __int64 a3, __int64 a4)
{
  unsigned __int64 v4; // rbp
  __int64 v8; // r12
  __int16 v9; // ax
  int v10; // r13d
  unsigned int v11; // ebx
  __int64 result; // rax
  unsigned __int16 v13; // di
  unsigned __int16 v14; // r14
  bool v15; // sf
  __int64 v16; // r15
  int v17; // eax
  _WORD *v18; // rdi
  __int64 v19; // r14
  int i; // r13d
  unsigned __int64 v21; // rdi
  __int16 v22; // r14
  unsigned int *v23; // rax
  _WORD v24[2]; // [rsp+50h] [rbp+0h] BYREF

  v4 = (unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL;
  *(_QWORD *)(v4 + 40) = a2;
  *(_QWORD *)(v4 + 64) = a4;
  *(_QWORD *)(v4 + 8) = a3;
  *(_DWORD *)(v4 + 36) = 0;
  memset((void *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 80), 0, 0xAAuLL);
  memset((void *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 768), 0, 0xAAuLL);
  memset((void *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 944), 0, 0xAAuLL);
  *(_DWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x18) = 0;
  LODWORD(v8) = 0;
  *(_WORD *)v4 = 0;
  *(_DWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 4) = 0;
  if ( !a1 || !a3 || !a2 )
    return 3221225485LL;
  v9 = a1[2];
  v10 = 0;
  *(_DWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x20) = 0;
  v11 = 0;
  if ( v9 )
  {
    v13 = a1[3];
    v14 = a1[4];
    *(_WORD *)v4 = v9;
  }
  else
  {
    result = ZwQueryInstallUILanguage();
    v10 = result;
    if ( (int)result < 0 )
      return result;
    if ( (int)sub_18008A9A4((__int64)a1, (_WORD *)(v4 + 16), (_WORD *)(v4 + 20)) >= 0 )
    {
      v14 = *(_WORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x14);
      v13 = *(_WORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x10);
    }
    else
    {
      v13 = 0;
      v14 = 0;
    }
    v15 = (int)ZwIsUILanguageComitted() < 0;
    v9 = *(_WORD *)v4;
    if ( !v15 )
    {
      a1[4] = v14;
      a1[3] = v13;
      a1[2] = v9;
    }
  }
  v16 = -1LL;
  if ( *(_WORD *)(*(_QWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 8) + 4LL) == v9 )
  {
    if ( v13
      && (*(_QWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x38) = ((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL)
                                                                              + 768,
          *(_DWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 4) = 512,
          *(_WORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x32) = 170,
          RtlLCIDToCultureName(v13, v4 + 48))
      && (int)ZwQueryValueKey() >= 0 )
    {
      v17 = sub_1800FB548(a1, v4 + 256, v14, v4 + 944, 512, v4 + 4);
      v18 = *(_WORD **)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 8);
      if ( v17 >= 0 && (int)sub_1800FA6F0((__int64)a1, v18, 0, v4 + 256, (wchar_t *)(v4 + 944)) >= 0 )
      {
        v11 = 1;
        v8 = -1LL;
        do
          ++v8;
        while ( *(_WORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x300 + 2 * v8) );
      }
    }
    else
    {
      v18 = *(_WORD **)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 8);
    }
  }
  else
  {
    v18 = *(_WORD **)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 8);
  }
  RtlInitUnicodeString((PUNICODE_STRING)(v4 + 48), L"DefaultFallback");
  v19 = *(_QWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x28);
  *(_DWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x10) = 1;
  *(_DWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 4) = 170;
  if ( (int)sub_18006F7EC(v19, v4 + 48, (_DWORD *)(v4 + 16), (void *)(v4 + 80), (unsigned int *)(v4 + 4)) >= 0
    && *(_DWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x10) == 1
    && (!(_DWORD)v8
     || (unsigned int)RtlCompareUnicodeStrings(
                        (unsigned __int16 *)(v4 + 80),
                        (unsigned __int64)*(unsigned int *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 4) >> 1,
                        v4 + 768,
                        (unsigned int)v8,
                        1))
    && (RtlInitUnicodeString((PUNICODE_STRING)(v4 + 48), (PCWSTR)(v4 + 80)),
        *(_DWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 4) = 512,
        (int)ZwQueryValueKey() >= 0)
    && *(_DWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x104) == 7
    && (int)sub_1800FA6F0((__int64)a1, v18, v11, v4 + 256, 0LL) >= 0 )
  {
    ++v11;
    do
      ++v16;
    while ( *(_WORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x50 + 2 * v16) );
  }
  else
  {
    v16 = *(_QWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x18);
  }
  if ( v10 != -2147483622 )
  {
    for ( i = *(_DWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x20);
          v11 < 4 && (int)ZwEnumerateValueKey() >= 0;
          ++i )
    {
      if ( *(_DWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x104) == 7
        && *(_DWORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x110) <= 0x200u )
      {
        if ( (_DWORD)v16 || (_DWORD)v8 )
        {
          v21 = (unsigned __int64)*(unsigned int *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x110) >> 1;
          v22 = *(_WORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x114 + 2 * v21);
          *(_WORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x114 + 2 * v21) = 0;
          if ( (_DWORD)v8
            && !(unsigned int)RtlCompareUnicodeStrings(
                                (unsigned __int16 *)(v4 + 276),
                                (unsigned __int64)*(unsigned int *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL)
                                                                  + 0x110) >> 1,
                                v4 + 768,
                                (unsigned int)v8,
                                1)
            || (_DWORD)v16
            && !(unsigned int)RtlCompareUnicodeStrings(
                                (unsigned __int16 *)(v4 + 276),
                                (unsigned __int64)*(unsigned int *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL)
                                                                  + 0x110) >> 1,
                                v4 + 80,
                                (unsigned int)v16,
                                1) )
          {
            v18 = *(_WORD **)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 8);
            continue;
          }
          *(_WORD *)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x114 + 2 * v21) = v22;
          v18 = *(_WORD **)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 8);
        }
        if ( (int)sub_1800FA6F0((__int64)a1, v18, v11, v4 + 256, 0LL) >= 0 )
          ++v11;
      }
    }
  }
  v23 = *(unsigned int **)(((unsigned __int64)v24 & 0xFFFFFFFFFFFFFFE0uLL) + 0x40);
  if ( v23 )
    *v23 = v11;
  return 0LL;
}
