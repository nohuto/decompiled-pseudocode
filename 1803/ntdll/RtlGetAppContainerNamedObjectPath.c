...
LABEL_11:
  if ( RelativePath )
  {
    ...
    if ( v10 )
    {
      result = ZwQueryInformationToken((HANDLE)0xFFFFFFFFFFFFFFFCLL, 0x2Au, &v14, 4u, &ReturnLength);
      if ( result < 0 )
        return result;
      if ( v14 )
      {
        result = ZwQueryInformationToken((HANDLE)v4, 0x2Au, &v15, 4u, &ReturnLength);
        if ( result < 0 )
          return result;
        if ( !(_DWORD)v15 )
          return -1073741637;
  ...
  if ( !v8 )
    v12 = v9;
  return sub_180042B30((HANDLE)v4, AppContainerSid, v12, ObjectPath);
}
