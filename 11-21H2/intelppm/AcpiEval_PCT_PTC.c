/*
 * XREFs of AcpiEval_PCT_PTC @ 0x1C0039D20
 * Callers:
 *     InitAcpi3ThrottleStates @ 0x1C003FE64 (InitAcpi3ThrottleStates.c)
 *     InitAcpiPerfStates @ 0x1C003FFA8 (InitAcpiPerfStates.c)
 * Callees:
 *     WPP_RECORDER_SF_s @ 0x1C0002544 (WPP_RECORDER_SF_s.c)
 *     __security_check_cookie @ 0x1C0003340 (__security_check_cookie.c)
 *     AcpiTranslateAccessSize @ 0x1C00264E0 (AcpiTranslateAccessSize.c)
 *     AcpiParseCore @ 0x1C0026814 (AcpiParseCore.c)
 *     AcpiEvaluateMethod @ 0x1C00282E8 (AcpiEvaluateMethod.c)
 */

__int64 __fastcall AcpiEval_PCT_PTC(__int64 a1, int a2, _BYTE *a3)
{
  unsigned int v4; // eax
  int v6; // eax
  PVOID v7; // rsi
  int v8; // ebx
  int v10; // [rsp+40h] [rbp-39h] BYREF
  int v11; // [rsp+48h] [rbp-31h] BYREF
  PVOID P; // [rsp+50h] [rbp-29h] BYREF
  unsigned __int8 v13[2]; // [rsp+58h] [rbp-21h] BYREF
  int v14; // [rsp+5Ah] [rbp-1Fh]
  const char *v15; // [rsp+60h] [rbp-19h]
  __int64 (__fastcall *v16)(__int64, __int64, unsigned int); // [rsp+68h] [rbp-11h]
  char v17; // [rsp+70h] [rbp-9h]
  int v18; // [rsp+72h] [rbp-7h]
  const char *v19; // [rsp+78h] [rbp-1h]
  __int64 (__fastcall *v20)(__int64, __int64, unsigned int); // [rsp+80h] [rbp+7h]
  int v21; // [rsp+88h] [rbp+Fh] BYREF
  char v22; // [rsp+8Ch] [rbp+13h]

  v10 = a2;
  v11 = 0;
  v15 = "Control";
  v13[0] = 0;
  v16 = AcpiParseRegister;
  v19 = "Status";
  v20 = AcpiParseRegister;
  v4 = 32;
  v14 = 2;
  v17 = 1;
  if ( a2 != 1413697631 )
    v4 = 1024;
  v18 = 786434;
  v21 = a2;
  v22 = 0;
  P = 0LL;
  _InterlockedOr((volatile signed __int32 *)(a1 + 1108), v4);
  v6 = AcpiEvaluateMethod(a1, v10, 0LL, (unsigned int **)&P, &v11);
  v7 = P;
  v8 = v6;
  if ( v6 >= 0 )
  {
    if ( v11 )
    {
      v8 = AcpiParseCore(
             v13,
             2LL,
             (unsigned __int64)P + 12,
             *((_DWORD *)P + 1) - 12,
             (__int64)a3,
             24,
             (const char *)&v21,
             0);
      if ( v8 >= 0 )
      {
        AcpiTranslateAccessSize(a3);
        AcpiTranslateAccessSize(a3 + 12);
      }
      else if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
      {
        WPP_RECORDER_SF_s(
          (__int64)WPP_GLOBAL_Control->DeviceExtension,
          2u,
          1u,
          0x13u,
          (__int64)&WPP_ea9a346b7756351237689efb62ec9115_Traceguids,
          (const char *)&v21);
      }
    }
    else
    {
      if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
        WPP_RECORDER_SF_s(
          (__int64)WPP_GLOBAL_Control->DeviceExtension,
          2u,
          1u,
          0x12u,
          (__int64)&WPP_ea9a346b7756351237689efb62ec9115_Traceguids,
          (const char *)&v10);
      v8 = -1073741275;
    }
  }
  if ( v7 )
    ExFreePoolWithTag(v7, (ULONG)0);
  if ( v8 < 0 )
  {
    if ( v10 == 1413697631 )
      _InterlockedOr((volatile signed __int32 *)(a1 + 1112), 0x20u);
    else
      _InterlockedOr((volatile signed __int32 *)(a1 + 1112), 0x400u);
  }
  return (unsigned int)v8;
}
