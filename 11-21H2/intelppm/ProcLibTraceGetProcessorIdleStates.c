/*
 * XREFs of ProcLibTraceGetProcessorIdleStates @ 0x1C0034470
 * Callers:
 *     ProcLibTraceControlCallback @ 0x1C002DCD0 (ProcLibTraceControlCallback.c)
 *     InitPepIdleStates @ 0x1C004048C (InitPepIdleStates.c)
 * Callees:
 *     __security_check_cookie @ 0x1C0003340 (__security_check_cookie.c)
 *     _guard_dispatch_icall_nop @ 0x1C0003700 (_guard_dispatch_icall_nop.c)
 *     ProcLibGetProcessorNumber @ 0x1C0006CA4 (ProcLibGetProcessorNumber.c)
 */

char __fastcall ProcLibTraceGetProcessorIdleStates(__int64 a1, char a2)
{
  const EVENT_DESCRIPTOR *v2; // rsi
  NTSTATUS ProcessorNumber; // eax
  __int64 v5; // r14
  __int64 Pool2; // rax
  _BYTE *v7; // rdi
  __int64 v8; // r9
  __int64 v9; // rcx
  __int64 v10; // r8
  unsigned int v11; // eax
  int v12; // ecx
  struct _PROCESSOR_NUMBER v14; // [rsp+30h] [rbp-29h] BYREF
  unsigned int v15; // [rsp+34h] [rbp-25h] BYREF
  int v16; // [rsp+38h] [rbp-21h] BYREF
  struct _EVENT_DATA_DESCRIPTOR UserData; // [rsp+40h] [rbp-19h] BYREF
  unsigned __int8 *p_Number; // [rsp+50h] [rbp-9h]
  int v19; // [rsp+58h] [rbp-1h]
  int v20; // [rsp+5Ch] [rbp+3h]
  int *v21; // [rsp+60h] [rbp+7h]
  int v22; // [rsp+68h] [rbp+Fh]
  int v23; // [rsp+6Ch] [rbp+13h]
  unsigned int *v24; // [rsp+70h] [rbp+17h]
  int v25; // [rsp+78h] [rbp+1Fh]
  int v26; // [rsp+7Ch] [rbp+23h]
  __int64 v27; // [rsp+80h] [rbp+27h]
  int i; // [rsp+88h] [rbp+2Fh]
  int v29; // [rsp+8Ch] [rbp+33h]

  v16 = 0;
  v14 = 0;
  v2 = (const EVENT_DESCRIPTOR *)&PPM_ETW_GET_IDLE_STATES;
  if ( a2 )
    v2 = &PPM_ETW_GET_IDLE_STATES_RUNDOWN;
  LOBYTE(ProcessorNumber) = EtwEventEnabled((REGHANDLE)WPP_MAIN_CB.Queue.ListEntry.Blink, v2);
  if ( (_BYTE)ProcessorNumber )
  {
    if ( *(_QWORD *)(a1 + 1144) )
    {
      ProcessorNumber = ProcLibGetProcessorNumber(a1, &v14);
      if ( ProcessorNumber >= 0 )
      {
        (*(void (__fastcall **)(PWDF_DRIVER_GLOBALS, _QWORD, _QWORD))(WdfFunctions_01015 + 2504))(
          WdfDriverGlobals,
          *(_QWORD *)(a1 + 208),
          0LL);
        v5 = (unsigned int)(13 * *(_DWORD *)(a1 + 1132));
        v15 = *(_DWORD *)(a1 + 1132);
        Pool2 = ExAllocatePool2(256LL, v5, 1919119952LL);
        v7 = (_BYTE *)Pool2;
        if ( Pool2 )
        {
          v29 = 0;
          v8 = 0LL;
          v27 = Pool2;
          for ( i = v5; (unsigned int)v8 < v15; *(_DWORD *)&v7[v10 + 8] = (v11 >> 2) & 1 )
          {
            v9 = *(_QWORD *)(a1 + 1144);
            v10 = 13LL * (unsigned int)v8;
            v7[v10 + 12] = (*(_DWORD *)(v9 + 4 * v8 + 8) >> 3) & 0xF;
            *(_DWORD *)&v7[v10 + 4] = (*(_DWORD *)(v9 + 4 * v8 + 8) >> 1) & 1;
            *(_DWORD *)&v7[v10] = *(_BYTE *)(v9 + 4 * v8 + 8) & 1;
            v11 = *(_DWORD *)(v9 + 4 * v8 + 8);
            v8 = (unsigned int)(v8 + 1);
          }
          v12 = *(_DWORD *)(*(_QWORD *)(a1 + 1144) + 4LL);
          UserData.Reserved = 0;
          v20 = 0;
          v23 = 0;
          v26 = 0;
          UserData.Ptr = (unsigned __int64)&v14;
          p_Number = &v14.Number;
          v16 = v12;
          v21 = &v16;
          v24 = &v15;
          v22 = 4;
          v25 = 4;
          UserData.Size = 2;
          v19 = 1;
          EtwWrite((REGHANDLE)WPP_MAIN_CB.Queue.ListEntry.Blink, v2, 0LL, (ULONG)5, &UserData);
          ExFreePoolWithTag(v7, (ULONG)1919119952);
        }
        LOBYTE(ProcessorNumber) = (*(__int64 (__fastcall **)(PWDF_DRIVER_GLOBALS, _QWORD))(WdfFunctions_01015 + 2512))(
                                    WdfDriverGlobals,
                                    *(_QWORD *)(a1 + 208));
      }
    }
  }
  return ProcessorNumber;
}
