/*
 * XREFs of xxxHelpLoop @ 0x1C024A384
 * Callers:
 *     xxxSysCommand @ 0x1C011BA1C (xxxSysCommand.c)
 * Callees:
 *     ?zzzSetCursor@@YAPEAUtagCURSOR@@PEAU1@@Z @ 0x1C0023BF4 (-zzzSetCursor@@YAPEAUtagCURSOR@@PEAU1@@Z.c)
 *     W32GetThreadWin32Thread @ 0x1C0041904 (W32GetThreadWin32Thread.c)
 *     xxxDispatchMessage @ 0x1C00429F0 (xxxDispatchMessage.c)
 *     ThreadLock @ 0x1C0068634 (ThreadLock.c)
 *     SetOrClrWF @ 0x1C0069680 (SetOrClrWF.c)
 *     xxxWindowEvent @ 0x1C0073AB0 (xxxWindowEvent.c)
 *     PtInRect @ 0x1C0077984 (PtInRect.c)
 *     xxxInternalGetMessage @ 0x1C00A4B4C (xxxInternalGetMessage.c)
 *     xxxRedrawTitle @ 0x1C00A6034 (xxxRedrawTitle.c)
 *     xxxReleaseCapture @ 0x1C00AA35C (xxxReleaseCapture.c)
 *     xxxCapture @ 0x1C00AA7F8 (xxxCapture.c)
 *     GetWindowBorders @ 0x1C00BB5A4 (GetWindowBorders.c)
 *     _GetParent @ 0x1C00DE714 (_GetParent.c)
 *     xxxTranslateMessage @ 0x1C010081C (xxxTranslateMessage.c)
 *     ?xxxWindowHitTest@@YAPEAUHWND__@@PEAUtagWND@@UtagPOINT@@PEAHK@Z @ 0x1C0101270 (-xxxWindowHitTest@@YAPEAUHWND__@@PEAUtagWND@@UtagPOINT@@PEAHK@Z.c)
 *     __security_check_cookie @ 0x1C01593A0 (__security_check_cookie.c)
 *     xxxInternalEnumWindow @ 0x1C01CE524 (xxxInternalEnumWindow.c)
 *     xxxWaitMessageEx @ 0x1C01DD5A8 (xxxWaitMessageEx.c)
 *     xxxSendHelpMessage @ 0x1C024A834 (xxxSendHelpMessage.c)
 *     ?IsHelpParent@@YAHPEAUtagWND@@@Z @ 0x1C025D3AC (-IsHelpParent@@YAHPEAUtagWND@@@Z.c)
 *     GetContextHelpId @ 0x1C025D408 (GetContextHelpId.c)
 */

void __fastcall xxxHelpLoop(struct tagWND *a1)
{
  int WindowBorders; // eax
  _DWORD *v3; // r8
  int v4; // ecx
  int v5; // edx
  int v6; // ecx
  int v7; // edx
  struct tagCURSOR *v8; // rcx
  HWND v9; // rax
  ShellWindowManagement *v10; // rsi
  __int64 v11; // rdx
  __int64 v12; // rcx
  __int64 v13; // r8
  struct tagWND *v14; // rbx
  __int64 ThreadWin32Thread; // rax
  __int64 v16; // rdx
  __int64 v17; // rcx
  ULONG_PTR Parent; // rsi
  __int64 v19; // r8
  __int64 v20; // rdx
  __int64 v21; // rcx
  __int64 v22; // r8
  __int64 v23; // rax
  int ContextHelpId; // eax
  __int64 v25; // rdx
  __int64 v26; // rcx
  __int64 v27; // r8
  __int64 v28; // [rsp+38h] [rbp-49h] BYREF
  struct tagWND *v29; // [rsp+40h] [rbp-41h]
  __int64 v30; // [rsp+48h] [rbp-39h]
  __int128 v31; // [rsp+50h] [rbp-31h] BYREF
  __int128 v32; // [rsp+60h] [rbp-21h]
  struct tagPOINT v33[2]; // [rsp+70h] [rbp-11h]
  __int128 v34; // [rsp+80h] [rbp-1h] BYREF
  __int64 v35; // [rsp+90h] [rbp+Fh]
  _QWORD v36[3]; // [rsp+98h] [rbp+17h] BYREF
  __int128 v37; // [rsp+B0h] [rbp+2Fh] BYREF
  __int64 v38; // [rsp+C0h] [rbp+3Fh]

  v31 = 0LL;
  v35 = 0LL;
  v38 = 0LL;
  v32 = 0LL;
  *(_OWORD *)&v33[0].x = 0LL;
  v34 = 0LL;
  v37 = 0LL;
  xxxWindowEvent(0xCu, a1, 0, 0, 0);
  zzzSetCursor(*(struct tagCURSOR **)(gasyscur[0] + 6632LL));
  xxxCapture(gptiCurrent, a1, 4);
  WindowBorders = GetWindowBorders(
                    *(unsigned int *)(*((_QWORD *)a1 + 5) + 28LL),
                    *(unsigned int *)(*((_QWORD *)a1 + 5) + 24LL));
  v3 = (_DWORD *)*((_QWORD *)a1 + 5);
  v4 = v3[24] - WindowBorders;
  LODWORD(v28) = WindowBorders + v3[22];
  v5 = v3[23];
  LODWORD(v29) = v4;
  v6 = v3[25] - WindowBorders;
  HIDWORD(v28) = WindowBorders + v5;
  HIDWORD(v29) = v6;
  while ( 1 )
  {
    while ( 1 )
    {
      if ( *(struct tagWND **)(*(_QWORD *)(gptiCurrent + 432LL) + 104LL) != a1 )
        goto LABEL_33;
      if ( (unsigned int)xxxInternalGetMessage(&v31, 0LL, 0, 0, 2, 0) )
        break;
      xxxWaitMessageEx(0x3CFFu, 0, 0LL);
    }
    if ( DWORD2(v31) == 161 )
      goto LABEL_33;
    if ( DWORD2(v31) == 513 )
    {
      if ( !PtInRect(&v28, *(unsigned __int64 *)&v33[0].y) )
        goto LABEL_33;
      v9 = xxxWindowHitTest(a1, *(struct tagPOINT *)&v33[0].y, 0LL, 0);
      v10 = (ShellWindowManagement *)ValidateHwnd(v9);
      ThreadLock((__int64)v10, (__int64 *)&v37);
      if ( (unsigned int)IsHelpParent(v10) )
      {
        v35 = *(_QWORD *)&v33[0].y;
        v34 = (unsigned __int64)v10;
        xxxInternalEnumWindow(v10, (__int64 (__fastcall *)(__int64, __int64))EnumPwndDlgChildProc, (__int64)&v34, 1u);
        v10 = (ShellWindowManagement *)*((_QWORD *)&v34 + 1);
      }
      v14 = 0LL;
      if ( v10 != a1 )
        v14 = v10;
      if ( !v14 && (!*(_DWORD *)(*((_QWORD *)a1 + 5) + 280LL) || (v14 = a1) == 0LL) )
      {
        ThreadUnlock1(v12, v11, v13);
        goto LABEL_33;
      }
      v36[2] = 0LL;
      v30 = 0LL;
      ThreadWin32Thread = W32GetThreadWin32Thread((__int64)KeGetCurrentThread());
      v28 = *(_QWORD *)(ThreadWin32Thread + 416);
      *(_QWORD *)(ThreadWin32Thread + 416) = &v28;
      v29 = v14;
      HMLockObject(v14);
      zzzSetCursor((struct tagCURSOR *)gasyscur[1]);
      xxxReleaseCapture();
      xxxRedrawTitle((unsigned __int64 *)a1, 0);
      SetOrClrWF(0, a1, 0x580u, 1);
      xxxInternalGetMessage(&v31, 0LL, 0, 0, 1, 1u);
      xxxWindowEvent(0x800Au, a1, -2, 4u, 0);
      xxxWindowEvent(0xDu, a1, 0, 0, 0);
      if ( (*(_BYTE *)(*((_QWORD *)v14 + 5) + 31LL) & 8) != 0 )
      {
        Parent = GetParent(v14);
        if ( !Parent )
        {
LABEL_29:
          ThreadUnlock1(v17, v16, v19);
          ThreadUnlock1(v21, v20, v22);
          return;
        }
      }
      else
      {
        Parent = (ULONG_PTR)v14;
      }
      v23 = W32GetThreadWin32Thread((__int64)KeGetCurrentThread());
      v36[0] = *(_QWORD *)(v23 + 416);
      *(_QWORD *)(v23 + 416) = v36;
      v36[1] = Parent;
      HMLockObject(Parent);
      ContextHelpId = GetContextHelpId(v14);
      xxxSendHelpMessage(Parent, ContextHelpId);
      ThreadUnlock1(v26, v25, v27);
      goto LABEL_29;
    }
    if ( (unsigned int)(DWORD2(v31) - 516) <= 7 )
    {
      v7 = 137;
      if ( _bittest(&v7, DWORD2(v31) - 516) )
        goto LABEL_33;
    }
    if ( DWORD2(v31) != 512 )
      break;
    if ( PtInRect(&v28, *(unsigned __int64 *)&v33[0].y) )
      v8 = *(struct tagCURSOR **)(gasyscur[0] + 6632LL);
    else
      v8 = (struct tagCURSOR *)gasyscur[1];
    zzzSetCursor(v8);
LABEL_15:
    xxxInternalGetMessage(&v31, 0LL, 0, 0, 1, 1u);
    xxxTranslateMessage((__int64)&v31, 0);
    xxxDispatchMessage((__int64)&v31);
  }
  if ( DWORD2(v31) != 256 || (_QWORD)v32 != 27LL )
    goto LABEL_15;
  xxxInternalGetMessage(&v31, 0LL, 0, 0, 1, 1u);
LABEL_33:
  xxxReleaseCapture();
  zzzSetCursor((struct tagCURSOR *)gasyscur[1]);
  xxxRedrawTitle((unsigned __int64 *)a1, 0);
  SetOrClrWF(0, a1, 0x580u, 1);
  xxxWindowEvent(0x800Au, a1, -2, 4u, 0);
  xxxWindowEvent(0xDu, a1, 0, 0, 0);
}
