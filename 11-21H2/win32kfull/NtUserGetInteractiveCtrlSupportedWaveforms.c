/*
 * XREFs of NtUserGetInteractiveCtrlSupportedWaveforms @ 0x1C01F4DD0
 * Callers:
 *     <none>
 * Callees:
 *     UserSetLastError @ 0x1C007274C (UserSetLastError.c)
 *     ?Instance@InteractiveControlManager@@SAPEAV1@XZ @ 0x1C010C048 (-Instance@InteractiveControlManager@@SAPEAV1@XZ.c)
 *     memmove @ 0x1C0160280 (memmove.c)
 *     ?GetDeviceSupportedWaveforms@InteractiveControlManager@@QEAAJKPEAPEAU_WAVEFORM_INFO@@PEAJ@Z @ 0x1C024EF4C (-GetDeviceSupportedWaveforms@InteractiveControlManager@@QEAAJKPEAPEAU_WAVEFORM_INFO@@PEAJ@Z.c)
 */

__int64 __fastcall NtUserGetInteractiveCtrlSupportedWaveforms(__int64 a1, volatile void *a2, int *a3)
{
  unsigned __int16 v5; // r15
  __int64 v6; // rdx
  int v7; // ebx
  __int64 v8; // rcx
  ULONG64 v9; // rcx
  __int64 v10; // rcx
  SIZE_T v11; // rsi
  __int64 CurrentProcessWow64Process; // rax
  InteractiveControlManager *v13; // rax
  int v14; // r8d
  _DWORD *v15; // rdx
  void *v16; // rdx
  int v18; // [rsp+20h] [rbp-48h]
  int v19; // [rsp+80h] [rbp+18h] BYREF
  void *Src; // [rsp+88h] [rbp+20h] BYREF

  v5 = a1;
  EnterSharedCrit(a1, a2, a3);
  v7 = 0;
  v19 = 0;
  Src = 0LL;
  if ( !a3 )
  {
    v8 = 87LL;
LABEL_3:
    UserSetLastError(v8, v6);
    goto LABEL_21;
  }
  ProbeForRead(a3, 4uLL, 4u);
  v11 = *a3;
  v18 = *a3;
  if ( a2 && (int)v11 > 0 )
  {
    CurrentProcessWow64Process = PsGetCurrentProcessWow64Process(v10);
    ProbeForWrite(a2, v11, CurrentProcessWow64Process != 0 ? 1 : 4);
  }
  v13 = InteractiveControlManager::Instance();
  if ( (int)InteractiveControlManager::GetDeviceSupportedWaveforms(v13, v5, (struct _WAVEFORM_INFO **)&Src, &v19) >= 0 )
  {
    if ( (unsigned __int64)v19 >= 0xFFFFFFF )
    {
      v8 = 534LL;
      goto LABEL_3;
    }
    v14 = 8 * v19;
    if ( (int)v11 > 0 && (int)v11 < v14 )
    {
      v8 = 122LL;
      goto LABEL_3;
    }
    v9 = MmUserProbeAddress;
    v15 = a3;
    if ( (unsigned __int64)a3 >= MmUserProbeAddress )
      v15 = (_DWORD *)MmUserProbeAddress;
    *v15 = *v15;
    if ( a2 && (v16 = Src) != 0LL && v18 >= v14 )
    {
      *a3 = v14;
      memmove((void *)a2, v16, v14);
    }
    else
    {
      *a3 = v14;
    }
    v7 = 1;
  }
LABEL_21:
  UserSessionSwitchLeaveCrit(v9);
  return v7;
}
