/*
 * XREFs of ?ndisQueuedCheckForHang@@YAXPEAX@Z @ 0x1C00091D0
 * Callers:
 *     <none>
 * Callees:
 ...
 *     ??1MiniportSpinLock@@QEAA@XZ @ 0x1C003922C (--1MiniportSpinLock@@QEAA@XZ.c)
...
LABEL_23:
  ...
  if ( RefCountTracker && (unsigned __int64)RefCountTracker - 2 > 1 )
  {
    ...
    if ( *((_BYTE *)RefCountTracker + 2) <= 0x53u )
      ndisBugCheckEx(0x1EuLL, 2uLL, (ULONG_PTR)a1->RefCountTracker, 0x53uLL);
    if ( *((_BYTE *)RefCountTracker + 1) )
    {
      if ( *((_BYTE *)RefCountTracker + 1) == 1 )
      {
        ...
        {
          if ( v30 == 0 && (v29 & 1) == 0 )
            ndisBugCheckEx(0x1EuLL, 0LL, (ULONG_PTR)a1->RefCountTracker, 0x53uLL);
