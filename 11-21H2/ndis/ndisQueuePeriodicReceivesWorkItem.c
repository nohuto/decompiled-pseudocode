/*
 * XREFs of ndisQueuePeriodicReceivesWorkItem @ 0x1C0039A36
 * Callers:
  ...
  if ( !_InterlockedExchange(&a1->PeriodicReceiveQueue.WorkItemQueued, 1) )
  {
    if ( HIBYTE(word_1C00EE5EC) )
    {
      ...
      *(GUID *)&v6[6] = EtwGuidNdisReceive;
      BYTE2(v5) = 24;
      *(_QWORD *)((char *)&v5 + 6) = qword_1C00EE5E0;
      IoWMIWriteEvent(&v4);
    }
    ndisReferenceMiniportNoCheck(a1, MPREF_IO_PERIODICRCV);
    ndisInsertInWorkQueue(
      (struct _KSEMAPHORE *)qword_1C00EC280 + 2 * (unsigned __int64)KeGetPcr()->Prcb.Number,
