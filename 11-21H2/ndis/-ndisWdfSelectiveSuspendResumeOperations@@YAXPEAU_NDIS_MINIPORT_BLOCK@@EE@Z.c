/*
 * XREFs of ?ndisWdfSelectiveSuspendResumeOperations@@YAXPEAU_NDIS_MINIPORT_BLOCK@@EE@Z @ 0x1C00A91C0
 * Callers:
 ...
 *     NdisWdfAsyncPowerReferenceCompleteNotification @ 0x1C012D3F0 (NdisWdfAsyncPowerReferenceCompleteNotification.c)
 * Callees:
...
void __fastcall ndisWdfSelectiveSuspendResumeOperations(struct _NDIS_MINIPORT_BLOCK *a1, __int64 a2, char a3)
{
  ...
  *((_QWORD *)SelectiveSuspend + 71) = (char *)SelectiveSuspend + 560;
  ...
  if ( v8 )
  {
    if ( a3 )
    {
      if ( byte_1C00EC210 && (*((_DWORD *)&a1->PktMonComp + 13) & 2) != 0 )
