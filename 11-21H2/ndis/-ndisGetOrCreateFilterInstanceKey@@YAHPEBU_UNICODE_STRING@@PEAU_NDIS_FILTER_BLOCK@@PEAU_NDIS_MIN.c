/*
 * XREFs of ?ndisGetOrCreateFilterInstanceKey@@YAHPEBU_UNICODE_STRING@@PEAU_NDIS_FILTER_BLOCK@@PEAU_NDIS_MINIPORT_BLOCK@@EPEAEPEAU1@@Z @ 0x1C0060AA4
 * Callers:
 *     NdisOpenConfigurationEx @ 0x1C001B190 (NdisOpenConfigurationEx.c)
 *     ?ndisCreateFilterInstanceRegistry@@YAHPEBU_UNICODE_STRING@@PEAU_NDIS_FILTER_BLOCK@@PEAU_NDIS_MINIPORT_BLOCK@@@Z @ 0x1C005FBBC (-ndisCreateFilterInstanceRegistry@@YAHPEBU_UNICODE_STRING@@PEAU_NDIS_FILTER_BLOCK@@PEAU_NDIS_MIN.c)
 * Callees:
 *     WPP_RECORDER_SF_qqqL @ 0x1C000D5C0 (WPP_RECORDER_SF_qqqL.c)
 *     WPP_RECORDER_SF_qqq @ 0x1C000D6C0 (WPP_RECORDER_SF_qqq.c)
 *     WPP_RECORDER_SF_dd @ 0x1C0023120 (WPP_RECORDER_SF_dd_ea_1C0023120.c)
 *     ?RtlStringCchPrintfW@@YAJPEA_W_KPEB_WZZ @ 0x1C002C654 (-RtlStringCchPrintfW@@YAJPEA_W_KPEB_WZZ.c)
 *     __security_check_cookie @ 0x1C0034520 (__security_check_cookie.c)
 *     memmove @ 0x1C0036080 (memmove.c)
 *     memset @ 0x1C0036340 (memset.c)
 */

__int64 __fastcall ndisGetOrCreateFilterInstanceKey(
        const struct _UNICODE_STRING *a1,
        struct _NDIS_FILTER_BLOCK *a2,
        struct _NDIS_MINIPORT_BLOCK *a3,
        char a4,
        unsigned __int8 *a5,
        struct _UNICODE_STRING *Destination)
{
  unsigned __int16 *Pool2; // r15
  int v9; // edx
  struct _NDIS_MINIPORT_BLOCK *v10; // rbx
  unsigned int v11; // edi
  char v12; // si
  __int16 v13; // ax
  __int64 v14; // rbx
  NTSTATUS v16; // ebx
  ULONG i; // esi
  NTSTATUS v18; // eax
  __int64 v19; // rcx
  int v20; // eax
  NTSTATUS RegistryValues; // ebx
  wchar_t *v22; // rax
  unsigned __int16 v23; // ax
  wchar_t *v24; // rax
  NTSTATUS RegistryKey; // eax
  wchar_t *Buffer; // rcx
  char v27[8]; // [rsp+30h] [rbp-D0h]
  char v28; // [rsp+50h] [rbp-B0h]
  char v29; // [rsp+51h] [rbp-AFh]
  int v32; // [rsp+60h] [rbp-A0h]
  _UNICODE_STRING String1; // [rsp+68h] [rbp-98h] BYREF
  UNICODE_STRING v34; // [rsp+78h] [rbp-88h] BYREF
  _UNICODE_STRING DestinationString; // [rsp+88h] [rbp-78h] BYREF
  ULONG ResultLength; // [rsp+98h] [rbp-68h] BYREF
  unsigned int v37; // [rsp+9Ch] [rbp-64h]
  HANDLE KeyHandle; // [rsp+A0h] [rbp-60h] BYREF
  _UNICODE_STRING UnicodeString; // [rsp+A8h] [rbp-58h] BYREF
  _UNICODE_STRING GuidString; // [rsp+B8h] [rbp-48h] BYREF
  UNICODE_STRING String2; // [rsp+C8h] [rbp-38h] BYREF
  char v42[8]; // [rsp+D8h] [rbp-28h]
  UNICODE_STRING SourceString; // [rsp+E0h] [rbp-20h] BYREF
  UNICODE_STRING Source; // [rsp+F0h] [rbp-10h] BYREF
  _OBJECT_ATTRIBUTES ObjectAttributes; // [rsp+100h] [rbp+0h] BYREF
  _RTL_QUERY_REGISTRY_TABLE QueryTable[3]; // [rsp+130h] [rbp+30h] BYREF

  Pool2 = 0LL;
  *(_QWORD *)v42 = a2;
  SourceString.Buffer = (wchar_t *)L"\\Registry\\Machine\\System\\CurrentControlSet\\Services\\";
  *(_QWORD *)&GuidString.Length = 0LL;
  v9 = 0;
  GuidString.Buffer = 0LL;
  *(_QWORD *)&DestinationString.Length = 0LL;
  v10 = a3;
  Source.Buffer = (wchar_t *)L"\\";
  v11 = 0;
  DestinationString.Buffer = 0LL;
  v12 = 0;
  *(_QWORD *)&SourceString.Length = 6946920LL;
  *(_QWORD *)&Source.Length = 262146LL;
  *(_QWORD *)&String1.Length = 0LL;
  String1.Buffer = 0LL;
  *(_QWORD *)&UnicodeString.Length = 0LL;
  UnicodeString.Buffer = 0LL;
  v28 = 0;
  v29 = 0;
  memset(&ObjectAttributes, 0, sizeof(ObjectAttributes));
  KeyHandle = 0LL;
  ResultLength = 0;
  *(_QWORD *)&v34.Length = 0LL;
  v34.Buffer = 0LL;
  *(_QWORD *)&String2.Length = 0LL;
  String2.Buffer = 0LL;
  v37 = 0;
  v32 = 0;
  if ( *(unsigned int **)&WPP_RECORDER_INITIALIZED != &WPP_RECORDER_INITIALIZED )
  {
    WPP_RECORDER_SF_qqq(
      *((_QWORD *)WPP_GLOBAL_Control + 8),
      4u,
      1u,
      0x57u,
      (struct _GUID *)&WPP_63db700a80b438f6540a523b9430c1eb_Traceguids,
      (char)a1,
      (char)a3,
      a2);
    v9 = 0;
  }
  if ( !v10->BaseMiniport )
  {
    v11 = -1073741811;
    goto LABEL_17;
  }
  *a5 = 0;
  *Destination = 0LL;
  if ( RtlStringFromGUID(&v10->BaseMiniport->InterfaceGuid, &GuidString) )
    goto LABEL_16;
  v13 = a1->Length + 2;
  DestinationString.Length = 0;
  DestinationString.MaximumLength = GuidString.Length
                                  + SourceString.Length
                                  + ndisParameterStr.Length
                                  + ndisFilterAdapterStr.Length
                                  + v13;
  v28 = 1;
  DestinationString.Buffer = (wchar_t *)ExAllocatePool2(64LL, DestinationString.MaximumLength, 538985550);
  if ( !DestinationString.Buffer )
  {
    v11 = -1073741670;
    goto LABEL_17;
  }
  RtlCopyUnicodeString(&DestinationString, &SourceString);
  RtlAppendUnicodeStringToString(&DestinationString, a1);
  RtlAppendUnicodeStringToString(&DestinationString, &ndisParameterStr);
  RtlAppendUnicodeStringToString(&DestinationString, &ndisFilterAdapterStr);
  RtlAppendUnicodeStringToString(&DestinationString, &GuidString);
  ObjectAttributes.RootDirectory = 0LL;
  ObjectAttributes.ObjectName = &DestinationString;
  ObjectAttributes.Length = 48;
  ObjectAttributes.Attributes = 576;
  *(_OWORD *)&ObjectAttributes.SecurityDescriptor = 0LL;
  if ( ZwOpenKey(&KeyHandle, 0x40000000u, &ObjectAttributes) )
  {
    v11 = -1073741823;
    KeyHandle = 0LL;
LABEL_20:
    RtlFreeUnicodeString(&GuidString);
    goto LABEL_21;
  }
  v14 = *(_QWORD *)(*(_QWORD *)v42 + 112LL);
  if ( v14 )
  {
    while ( (*(_DWORD *)(*(_QWORD *)(*(_QWORD *)(v14 + 16) + 336LL) + 40LL) & 2) != 0 )
    {
      v14 = *(_QWORD *)(v14 + 112);
      if ( !v14 )
        goto LABEL_13;
    }
    String1.Length = *(_WORD *)(*(_QWORD *)(v14 + 16) + 128LL);
    String1.MaximumLength = String1.Length + 2;
    String1.Buffer = (wchar_t *)ExAllocatePool2(64LL, (unsigned __int16)(String1.Length + 2), 538985550);
    if ( !String1.Buffer )
      goto LABEL_35;
    memmove(String1.Buffer, *(const void **)(*(_QWORD *)(v14 + 16) + 136LL), String1.Length);
    goto LABEL_38;
  }
LABEL_13:
  LOBYTE(v10) = (_BYTE)a3;
  if ( RtlStringFromGUID(&a3->InterfaceGuid, &UnicodeString) )
  {
LABEL_16:
    v11 = -1073741823;
    goto LABEL_17;
  }
  String1.Length = UnicodeString.Length;
  String1.MaximumLength = UnicodeString.MaximumLength;
  String1.Buffer = (wchar_t *)ExAllocatePool2(64LL, UnicodeString.MaximumLength, 538985550);
  if ( !String1.Buffer )
  {
    RtlFreeUnicodeString(&UnicodeString);
    goto LABEL_16;
  }
  v12 = 1;
  v16 = RtlUpcaseUnicodeString(&String1, &UnicodeString, 0);
  RtlFreeUnicodeString(&UnicodeString);
  if ( v16 )
  {
LABEL_41:
    LOBYTE(v10) = (_BYTE)a3;
    goto LABEL_16;
  }
LABEL_38:
  Pool2 = (unsigned __int16 *)ExAllocatePool2(66LL, 536LL, 538985550);
  if ( Pool2 )
  {
    for ( i = 0; i <= 0x270F; ++i )
    {
      memset(Pool2, 0, 0x218uLL);
      ResultLength = 0;
      v18 = ZwEnumerateKey(KeyHandle, i, KeyBasicInformation, Pool2, 0x216u, &ResultLength);
      if ( v18 == -2147483622 )
        break;
      if ( v18 )
        goto LABEL_67;
      v34.Length = Pool2[6];
      v34.Buffer = Pool2 + 8;
      v34.MaximumLength = v34.Length + 2;
      memset(QueryTable, 0, sizeof(QueryTable));
      QueryTable[0].Flags = 1;
      QueryTable[0].Name = Pool2 + 8;
      v19 = *(_QWORD *)(*(_QWORD *)v42 + 16LL);
      if ( *(_BYTE *)(v19 + 100) > 6u || *(_BYTE *)(v19 + 100) == 6 && *(_BYTE *)(v19 + 101) >= 0x28u )
      {
        v20 = 256;
        v37 = 0x1000000;
        v32 = 256;
      }
      else
      {
        v20 = v32;
      }
      QueryTable[1].Flags = v20 | 0x24;
      QueryTable[1].QueryRoutine = 0LL;
      QueryTable[1].Name = (wchar_t *)L"LowerComponent";
      QueryTable[1].EntryContext = &String2;
      QueryTable[2].Name = 0LL;
      QueryTable[1].DefaultType = v37;
      RegistryValues = RtlQueryRegistryValuesEx(0x40000000u, (PCWSTR)KeyHandle, QueryTable, 0LL, 0LL);
      if ( *(unsigned int **)&WPP_RECORDER_INITIALIZED != &WPP_RECORDER_INITIALIZED )
      {
        *(_DWORD *)v27 = v32;
        WPP_RECORDER_SF_dd(
          *((_QWORD *)WPP_GLOBAL_Control + 8),
          v9,
          1u,
          0x58u,
          (struct _GUID *)&WPP_63db700a80b438f6540a523b9430c1eb_Traceguids,
          RegistryValues,
          *(_QWORD *)v27);
      }
      if ( RegistryValues )
        goto LABEL_67;
      if ( !RtlCompareUnicodeString(&String1, &String2, 1u) )
      {
        *a5 = 1;
        break;
      }
      if ( String2.Buffer )
      {
        ExFreePoolWithTag(String2.Buffer, 0);
        String2.Buffer = 0LL;
      }
    }
    if ( *a5 )
      goto LABEL_62;
    if ( a4 )
    {
      if ( i > 0x270F )
        goto LABEL_39;
      *(_DWORD *)&v34.Length = 655368;
      v22 = (wchar_t *)ExAllocatePool2(64LL, 10LL, 538985550);
      v34.Buffer = v22;
      if ( !v22 )
        goto LABEL_39;
      v29 = 1;
      if ( !RtlStringCchPrintfW(v22, (unsigned __int64)v34.MaximumLength >> 1, (wchar_t *)L"%04d", i) )
      {
LABEL_62:
        v23 = GuidString.Length
            + Source.Length
            + ndisParameterStr.Length
            + ndisFilterAdapterStr.Length
            + v34.Length
            + a1->Length
            + 2;
        Destination->MaximumLength = v23;
        v24 = (wchar_t *)ExAllocatePool2(64LL, v23, 538985550);
        Destination->Buffer = v24;
        if ( !v24 )
          goto LABEL_39;
        RtlCopyUnicodeString(Destination, a1);
        RtlAppendUnicodeStringToString(Destination, &ndisParameterStr);
        RtlAppendUnicodeStringToString(Destination, &ndisFilterAdapterStr);
        RtlAppendUnicodeStringToString(Destination, &GuidString);
        RtlAppendUnicodeStringToString(Destination, &Source);
        RtlAppendUnicodeStringToString(Destination, &v34);
        v12 = 1;
        if ( *a5 )
          goto LABEL_36;
        v12 = 1;
        RegistryKey = RtlCreateRegistryKey(1u, (PWSTR)Destination->Buffer);
        Buffer = Destination->Buffer;
        if ( !RegistryKey )
        {
          if ( !RtlWriteRegistryValue(
                  1u,
                  (PCWSTR)Destination->Buffer,
                  L"LowerComponent",
                  1u,
                  String1.Buffer,
                  String1.Length + 2) )
            goto LABEL_36;
          Buffer = Destination->Buffer;
        }
        ExFreePoolWithTag(Buffer, 0);
        *Destination = 0LL;
        goto LABEL_41;
      }
    }
LABEL_67:
    v12 = 1;
    goto LABEL_41;
  }
LABEL_39:
  v12 = 1;
LABEL_35:
  v11 = -1073741670;
LABEL_36:
  LOBYTE(v10) = (_BYTE)a3;
LABEL_17:
  if ( KeyHandle )
    ZwClose(KeyHandle);
  if ( v28 )
    goto LABEL_20;
LABEL_21:
  if ( DestinationString.Buffer )
    ExFreePoolWithTag(DestinationString.Buffer, 0);
  if ( v12 )
    ExFreePoolWithTag(String1.Buffer, 0);
  if ( String2.Buffer )
    ExFreePoolWithTag(String2.Buffer, 0);
  if ( Pool2 )
    ExFreePoolWithTag(Pool2, 0);
  if ( v29 )
    ExFreePoolWithTag(v34.Buffer, 0);
  if ( *(unsigned int **)&WPP_RECORDER_INITIALIZED != &WPP_RECORDER_INITIALIZED )
    WPP_RECORDER_SF_qqqL(
      *((_QWORD *)WPP_GLOBAL_Control + 8),
      v9,
      1u,
      0x59u,
      (struct _GUID *)&WPP_63db700a80b438f6540a523b9430c1eb_Traceguids,
      (char)a1,
      (char)v10,
      v42[0],
      v11);
  return v11;
}
