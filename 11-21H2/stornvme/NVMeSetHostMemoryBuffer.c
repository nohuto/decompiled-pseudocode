/*
 * XREFs of NVMeSetHostMemoryBuffer @ 0x1C0024584
 * Callers:
 *     NVMeInitHostMemoryBuffer @ 0x1C0006EB0 (NVMeInitHostMemoryBuffer.c)
 *     NVMeControllerPowerDown @ 0x1C0018BDC (NVMeControllerPowerDown.c)
 *     NVMeFreeHostMemoryBuffer @ 0x1C00196F4 (NVMeFreeHostMemoryBuffer.c)
 * Callees:
 *     ProcessCommand @ 0x1C00039C8 (ProcessCommand.c)
 *     SrbAssignQueueId @ 0x1C0005238 (SrbAssignQueueId.c)
 *     WaitForCommandCompleteWithCustomTimeout @ 0x1C000A1DC (WaitForCommandCompleteWithCustomTimeout.c)
 *     memmove @ 0x1C0010700 (memmove.c)
 *     memset @ 0x1C00109C0 (memset.c)
 */

__int64 __fastcall NVMeSetHostMemoryBuffer(
        __int64 a1,
        unsigned __int8 a2,
        unsigned __int8 a3,
        unsigned int a4,
        void *Src,
        size_t Size)
{
  int v6; // edi
  int v8; // ebx
  __int64 v10; // rax
  __int64 v11; // rax

  v6 = a3;
  v8 = a2;
  *(_BYTE *)(a1 + 867) = 0;
  memset(*(void **)(a1 + 952), 0, 0x10A0uLL);
  *(_QWORD *)(*(_QWORD *)(a1 + 952) + 4232LL) = 0LL;
  v10 = *(_QWORD *)(a1 + 952);
  *(_QWORD *)(a1 + 920) = v10;
  *(_DWORD *)(a1 + 856) = 1;
  *(_BYTE *)(v10 + 4253) |= 1u;
  *(_BYTE *)(*(_QWORD *)(a1 + 952) + 4253LL) &= ~2u;
  SrbAssignQueueId(a1, a1 + 864);
  *(_BYTE *)(*(_QWORD *)(a1 + 952) + 4096LL) = 9;
  *(_BYTE *)(*(_QWORD *)(a1 + 952) + 4136LL) = 13;
  *(_DWORD *)(*(_QWORD *)(a1 + 952) + 4140LL) ^= (*(_DWORD *)(*(_QWORD *)(a1 + 952) + 4140LL) ^ v8) & 1;
  *(_DWORD *)(*(_QWORD *)(a1 + 952) + 4140LL) ^= (*(_DWORD *)(*(_QWORD *)(a1 + 952) + 4140LL) ^ (2 * v6)) & 2;
  *(_DWORD *)(*(_QWORD *)(a1 + 952) + 4144LL) = a4
                                              / (1 << (((*(_DWORD *)(*(_QWORD *)(a1 + 176) + 20LL) >> 7) & 0xF) + 12));
  memmove(*(void **)(a1 + 1736), Src, (unsigned int)Size);
  v11 = HIDWORD(*(_QWORD *)(a1 + 1744));
  *(_DWORD *)(*(_QWORD *)(a1 + 952) + 4148LL) = *(_DWORD *)(a1 + 1744) & 0xFFFFFFF0;
  *(_DWORD *)(*(_QWORD *)(a1 + 952) + 4152LL) = v11;
  *(_DWORD *)(*(_QWORD *)(a1 + 952) + 4156LL) = (unsigned int)Size >> 4;
  ProcessCommand(a1, a1 + 864);
  WaitForCommandCompleteWithCustomTimeout(a1, a1 + 864, 1, 10000000);
  return *(_BYTE *)(a1 + 867) != 1 ? 0xC1000001 : 0;
}
