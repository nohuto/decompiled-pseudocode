/*
 * XREFs of LdrpLogVsmEnclaveLdrInitializeEnclaveTelemetry @ 0x1800DC328
 * Callers:
 *     LdrInitializeEnclave @ 0x1800D8410 (LdrInitializeEnclave.c)
 * Callees:
 *     _tlgKeywordOn @ 0x180002E84 (_tlgKeywordOn.c)
 *     _tlgWriteTransfer_EtwEventWriteTransfer @ 0x180004EA0 (_tlgWriteTransfer_EtwEventWriteTransfer.c)
 *     RtlRunOnceExecuteOnce @ 0x1800163A0 (RtlRunOnceExecuteOnce.c)
 *     __security_check_cookie @ 0x180093840 (__security_check_cookie.c)
 */

char __fastcall LdrpLogVsmEnclaveLdrInitializeEnclaveTelemetry(int a1)
{
  struct _PEB *v1; // rax
  __int64 v3; // rcx
  __int64 v4; // r8
  __int64 v5; // r9
  int v7; // [rsp+30h] [rbp-68h] BYREF
  _BYTE v8[32]; // [rsp+40h] [rbp-58h] BYREF
  const char *v9; // [rsp+60h] [rbp-38h]
  int v10; // [rsp+68h] [rbp-30h]
  int v11; // [rsp+6Ch] [rbp-2Ch]
  int *v12; // [rsp+70h] [rbp-28h]
  int v13; // [rsp+78h] [rbp-20h]
  int v14; // [rsp+7Ch] [rbp-1Ch]

  v1 = NtCurrentPeb();
  if ( v1->ProcessHeap )
  {
    LOBYTE(v1) = RtlRunOnceExecuteOnce(
                   &VsmEnclaveTelemetryInitRunOnce,
                   (unsigned int (__fastcall *)(volatile signed __int64 *, __int64, __int64 *))VsmEnclaveTelemetryInitOnce,
                   0LL,
                   0LL);
    if ( (unsigned int)dword_180174930 > 4 )
    {
      LOBYTE(v1) = tlgKeywordOn((__int64)&dword_180174930, 0x400000000000LL);
      if ( (_BYTE)v1 )
      {
        v11 = 0;
        v14 = 0;
        v9 = "LdrInitializeEnclave";
        v12 = &v7;
        v10 = 21;
        v7 = a1;
        v13 = v5;
        LOBYTE(v1) = tlgWriteTransfer_EtwEventWriteTransfer(v3, byte_180141C71, v4, v5, v5, (__int64)v8);
      }
    }
  }
  return (char)v1;
}
