/*
 * XREFs of RtlpPrintErrorInformation @ 0x1801176E0
 * Callers:
...
ULONG RtlpPrintErrorInformation()
{
  ...
  if ( NtCurrentPeb()->Ldr )
    ...
    DbgPrint("HEAP: ");
  DbgPrint("Heap error detected at %p (heap handle %p)\n", (const void *)qword_180177798, (const void *)qword_180177790);
  if ( dword_180177788 > 8 )
  {
    switch ( dword_180177788 )
    {
      case 9:
  ...
  else if ( dword_180177788 == 8 )
  {
    v0 = "heap_failure_block_not_busy";
  }
  else if ( dword_180177788 )
  {
    switch ( dword_180177788 )
    {
      case 1:
  ...
  if ( qword_1801777B0 )
  {
    if ( NtCurrentPeb()->Ldr )
      ...
      DbgPrint("HEAP: ");
    DbgPrint("Parameter3: %p\n", (const void *)qword_1801777B0);
  }
  if ( qword_1801777B8 || qword_1801777C0 )
  {
    if ( NtCurrentPeb()->Ldr )
      DbgPrint("HEAP[%wZ]: ", &NtCurrentPeb()->Ldr->InLoadOrderModuleList.Flink[5].Blink);
    else
    ...
    DbgPrint(
      ...
      (const void *)qword_1801777B8,
      (const void *)qword_1801777C0);
  }
  if ( NtCurrentPeb()->Ldr )
    DbgPrint("HEAP[%wZ]: ", &NtCurrentPeb()->Ldr->InLoadOrderModuleList.Flink[5].Blink);
  else
