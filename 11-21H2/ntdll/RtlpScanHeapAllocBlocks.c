/*
 * XREFs of RtlpScanHeapAllocBlocks @ 0x180103C4C
 * Callers:
 *     RtlpScanProcessVirtualMemory @ 0x180103EA4 (RtlpScanProcessVirtualMemory.c)
 * Callees:
 *     DbgPrint @ 0x1800069C0 (DbgPrint.c)
 *     RtlSizeHeap @ 0x1800270A0 (RtlSizeHeap.c)
 *     _guard_xfg_dispatch_icall_nop @ 0x1800AAAD0 (_guard_xfg_dispatch_icall_nop.c)
 *     RtlpGetBlockInfo @ 0x1801036A8 (RtlpGetBlockInfo.c)
 *     RtlpGetHeapBlock @ 0x1801036F4 (RtlpGetHeapBlock.c)
 */

char RtlpScanHeapAllocBlocks()
{
  __int64 *v0; // rbx
  _QWORD *v1; // r14
  unsigned __int64 *v2; // rcx
  __int64 *v3; // rsi
  __int64 v4; // rdx
  unsigned __int64 *v5; // rdi
  __int64 HeapBlock; // rax
  __int64 v7; // rcx
  int v8; // eax
  __int64 v9; // rax
  _QWORD *v10; // rdx
  __int64 *v11; // rax
  bool v12; // zf
  __int64 i; // rdi
  __int64 BlockInfo; // rax
  int v15; // r9d
  unsigned __int64 v16; // r10
  __int64 v17; // rbx
  __int64 v18; // rax
  __int64 v19; // rsi
  unsigned __int8 *v20; // rbx
  int v21; // eax

  v0 = (__int64 *)RtlpBusyList;
  while ( v0 != &RtlpBusyList )
  {
    v1 = v0 + 2;
    v2 = (unsigned __int64 *)v0[2];
    v3 = v0;
    v4 = v0[3];
    v0 = (__int64 *)*v0;
    v5 = v2 + 1;
    if ( v2 + 1 <= (unsigned __int64 *)((char *)v2 + v4) )
    {
      do
      {
        HeapBlock = RtlpGetHeapBlock(*v2);
        v7 = HeapBlock;
        if ( HeapBlock )
        {
          v8 = *(_DWORD *)(HeapBlock + 32);
          if ( !v8 )
          {
            v9 = *(_QWORD *)v7;
            if ( *(_QWORD *)(*(_QWORD *)v7 + 8LL) != v7
              || (v10 = *(_QWORD **)(v7 + 8), *v10 != v7)
              || (*v10 = v9,
                  *(_QWORD *)(v9 + 8) = v10,
                  v11 = (__int64 *)qword_180178CD8,
                  *(__int64 **)qword_180178CD8 != &RtlpBusyList) )
            {
              __fastfail(3u);
            }
            *(_QWORD *)v7 = &RtlpBusyList;
            *(_QWORD *)(v7 + 8) = v11;
            *v11 = v7;
            qword_180178CD8 = v7;
            v8 = *(_DWORD *)(v7 + 32);
          }
          v12 = *(_QWORD *)(v7 + 16) == 0LL;
          *(_DWORD *)(v7 + 32) = v8 + 1;
          if ( v12 )
            __debugbreak();
        }
        v2 = v5++;
      }
      while ( (unsigned __int64)v5 <= *v1 + v3[3] );
    }
  }
  for ( i = RtlpLeakList; (__int64 *)i != &RtlpLeakList; i = *(_QWORD *)i )
  {
    BlockInfo = RtlpGetBlockInfo((_QWORD *)RtlpProcessMemoryMap, *(_QWORD *)(i + 16));
    v17 = BlockInfo;
    if ( BlockInfo )
    {
      if ( *((_QWORD *)&xmmword_180178E40 + 1) )
      {
        v18 = RtlSizeHeap(*(_QWORD *)(BlockInfo + 8), 0, v16);
        (*((void (__fastcall **)(_QWORD, _QWORD, _QWORD, __int64, _DWORD, _QWORD))&xmmword_180178E40 + 1))(
          0LL,
          *(_QWORD *)(v17 + 8),
          *(_QWORD *)(i + 16),
          v18,
          0,
          0LL);
      }
      else
      {
        if ( !v15 )
        {
          DbgPrint("Below is a list of potentially leaked heap entries \nuse !heap -i Entry -h Heap for more information\n\n");
          DbgPrint("Entry     Heap              Size       \n");
          DbgPrint("---------------------------------------\n");
          v16 = *(_QWORD *)(i + 16);
        }
        v19 = *(_QWORD *)(v17 + 8);
        if ( *(_DWORD *)(v19 + 16) == -571548178 )
        {
          v20 = (unsigned __int8 *)v16;
        }
        else
        {
          v20 = (unsigned __int8 *)(v16 - 16);
          _m_prefetchw((const void *)(v16 - 16));
          if ( *(_BYTE *)(v16 - 16 + 15) == 5 )
            v20 -= 16 * v20[14];
        }
        v21 = RtlSizeHeap(v19, 0, v16);
        DbgPrint("%p  %-16Ix  %Id", v20, v19, v21);
        DbgPrint("\n");
      }
      ++RtlpLeaksCount;
    }
  }
  if ( *((_QWORD *)&xmmword_180178E40 + 1) )
    (*((void (__fastcall **)(_QWORD, _QWORD, _QWORD, _QWORD, _DWORD, _QWORD))&xmmword_180178E40 + 1))(
      0LL,
      0LL,
      0LL,
      0LL,
      0,
      0LL);
  return 1;
}
