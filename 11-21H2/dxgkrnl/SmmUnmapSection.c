/*
 * XREFs of SmmUnmapSection @ 0x1C03C1888
 * Callers:
 *     ?SysMmUnmapPhysicalObject@@YAXPEAUSYSMM_PHYSICAL_OBJECT@@PEAX_K@Z @ 0x1C0070B10 (-SysMmUnmapPhysicalObject@@YAXPEAUSYSMM_PHYSICAL_OBJECT@@PEAX_K@Z.c)
 * Callees:
 *     DxgkLogInternalTriageEvent @ 0x1C0008E10 (DxgkLogInternalTriageEvent.c)
 */

char __fastcall SmmUnmapSection(PVOID a1, __int64 a2, __int64 a3, __int64 a4)
{
  __int64 CurrentProcess; // rax
  NTSTATUS v5; // eax

  if ( a1 > MmHighestUserAddress )
  {
    v5 = MmUnmapViewInSystemSpace(a1);
  }
  else
  {
    CurrentProcess = PsGetCurrentProcess(a1, a2, a3, a4);
    v5 = MmUnmapViewOfSection(CurrentProcess);
  }
  if ( v5 < 0 )
  {
    WdLogSingleEntry1(1LL, 184LL);
    LOBYTE(v5) = DxgkLogInternalTriageEvent(
                   0LL,
                   262146,
                   -1,
                   (__int64)L"(((NTSTATUS)(Status)) >= 0)",
                   184LL,
                   0LL,
                   0LL,
                   0LL,
                   0LL);
  }
  return v5;
}
