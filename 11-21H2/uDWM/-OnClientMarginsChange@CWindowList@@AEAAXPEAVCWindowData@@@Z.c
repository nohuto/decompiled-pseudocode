/*
 * XREFs of ?OnClientMarginsChange@CWindowList@@AEAAXPEAVCWindowData@@@Z @ 0x180021188
 * Callers:
 *     ?ClientMarginsChange@CWindowList@@UEAAJPEAUIDwmWindow@@@Z @ 0x180021790 (-ClientMarginsChange@CWindowList@@UEAAJPEAUIDwmWindow@@@Z.c)
 *     ?UpdateWindowScale@CWindowList@@QEAAJPEAVCWindowData@@H@Z @ 0x180030C00 (-UpdateWindowScale@CWindowList@@QEAAJPEAVCWindowData@@H@Z.c)
 * Callees:
 *     ?SetDirtyFlags@CThumbnailVisual@@UEAAXK@Z @ 0x18001CE70 (-SetDirtyFlags@CThumbnailVisual@@UEAAXK@Z.c)
 *     ?SetDirtyFlags@CVisual@@UEAAXK@Z @ 0x18001DB80 (-SetDirtyFlags@CVisual@@UEAAXK@Z.c)
 *     ?UpdateSizeOrMargins@CWindowIconic@@AEAAJ_N@Z @ 0x18004ABB8 (-UpdateSizeOrMargins@CWindowIconic@@AEAAJ_N@Z.c)
 *     _o_ceilf_0 @ 0x180060EB0 (_o_ceilf_0.c)
 *     _guard_xfg_dispatch_icall_nop @ 0x180063740 (_guard_xfg_dispatch_icall_nop.c)
 *     ?_Log_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z @ 0x18009870C (-_Log_Hr@in1diag3@details@wil@@YAXPEAXIPEBDJ@Z.c)
 */

void __fastcall CWindowList::OnClientMarginsChange(CWindowList *this, struct CWindowData *a2)
{
  float v3; // xmm6_4
  int v4; // r14d
  int v5; // ebp
  int v6; // esi
  int v7; // eax
  CVisual *v8; // rcx
  CWindowIconic *v9; // rcx
  unsigned int i; // ebx
  unsigned int j; // ebx
  int v12; // eax
  CThumbnailVisual *v13; // rcx
  __int64 v14; // rax
  CThumbnailVisual *v15; // rcx
  int v16[4]; // [rsp+20h] [rbp-28h]
  wil::details::in1diag3 *retaddr; // [rsp+48h] [rbp+0h]

  v3 = *((float *)a2 + 83);
  v16[0] = (int)o_ceilf_0((float)*((int *)a2 + 66) * v3);
  v4 = (int)o_ceilf_0((float)*((int *)a2 + 67) * v3);
  v16[1] = v4;
  v5 = (int)o_ceilf_0((float)*((int *)a2 + 68) * v3);
  v16[2] = v5;
  v6 = (int)o_ceilf_0((float)*((int *)a2 + 69) * v3);
  v16[3] = v6;
  v7 = (int)o_ceilf_0((float)*((int *)a2 + 74) * v3);
  if ( *((_QWORD *)a2 + 8) != __PAIR64__(v4, v16[0])
    || *((_DWORD *)a2 + 18) != v5
    || *((_DWORD *)a2 + 19) != v6
    || *((_DWORD *)a2 + 28) != v7 )
  {
    v8 = (CVisual *)*((_QWORD *)a2 + 55);
    *((_DWORD *)a2 + 28) = v7;
    *((_OWORD *)a2 + 4) = *(_OWORD *)v16;
    if ( v8 )
      CVisual::SetDirtyFlags(v8, 0x2000);
    v9 = (CWindowIconic *)*((_QWORD *)a2 + 61);
    if ( v9 )
      CWindowIconic::UpdateSizeOrMargins(v9, 0);
    for ( i = 0; i < *((_DWORD *)a2 + 148); ++i )
    {
      v13 = *(CThumbnailVisual **)(*(_QWORD *)(*((_QWORD *)a2 + 71) + 8LL * i) + 88LL);
      if ( v13 )
      {
        v14 = *((_QWORD *)v13 + 47);
        if ( v14 )
        {
          if ( *(_DWORD *)(v14 + 77) )
            CThumbnailVisual::SetDirtyFlags(v13, 0x4000);
        }
      }
    }
    for ( j = 0; j < *((_DWORD *)a2 + 140); ++j )
    {
      v15 = *(CThumbnailVisual **)(*(_QWORD *)(*((_QWORD *)a2 + 67) + 8LL * j) + 88LL);
      if ( v15 )
        CThumbnailVisual::SetDirtyFlags(v15, 4096);
    }
    v12 = (*(__int64 (__fastcall **)(_QWORD, struct CWindowData *))(**((_QWORD **)CDesktopManager::s_pDesktopManagerInstance
                                                                     + 56)
                                                                  + 112LL))(
            *((_QWORD *)CDesktopManager::s_pDesktopManagerInstance + 56),
            a2);
    if ( v12 < 0 )
      wil::details::in1diag3::_Log_Hr(
        retaddr,
        (void *)0xFFD,
        (unsigned int)"clientcore\\windows\\dwm\\udwm\\windowlist.cpp",
        (const char *)(unsigned int)v12,
        v16[0]);
  }
}
