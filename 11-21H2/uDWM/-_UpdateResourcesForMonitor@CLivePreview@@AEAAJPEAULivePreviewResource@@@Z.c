/*
 * XREFs of ?_UpdateResourcesForMonitor@CLivePreview@@AEAAJPEAULivePreviewResource@@@Z @ 0x1800C4E64
 * Callers:
 *     ?_UpdateResources@CLivePreview@@AEAAJXZ @ 0x1800C4D74 (-_UpdateResources@CLivePreview@@AEAAJXZ.c)
 *     ?s_UpdateResourcesForMonitor@CLivePreview@@CAHPEAUHMONITOR__@@PEAUHDC__@@PEAUtagRECT@@_J@Z @ 0x1800C5740 (-s_UpdateResourcesForMonitor@CLivePreview@@CAHPEAUHMONITOR__@@PEAUHDC__@@PEAUtagRECT@@_J@Z.c)
 * Callees:
 *     ?MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJIPEAX@Z @ 0x1800045F8 (-MilInstrumentationCheckHR_MaybeFailFast@@YAXKQEBJIJIPEAX@Z.c)
 *     ??$CreateProxy@VCCachedVisualImageProxy@@@CCompositor@@IEAAJPEAPEAVCCachedVisualImageProxy@@@Z @ 0x18000D86C (--$CreateProxy@VCCachedVisualImageProxy@@@CCompositor@@IEAAJPEAPEAVCCachedVisualImageProxy@@@Z.c)
 *     ??$CreateProxy@VCImageLegacyMilBrushProxy@@@CCompositor@@IEAAJPEAPEAVCImageLegacyMilBrushProxy@@@Z @ 0x18000DF0C (--$CreateProxy@VCImageLegacyMilBrushProxy@@@CCompositor@@IEAAJPEAPEAVCImageLegacyMilBrushProxy@@.c)
 *     ?CreateRectangleGeometry@ResourceHelper@@SAJHHHHPEAPEAVCRectangleGeometryProxy@@@Z @ 0x18000FAB8 (-CreateRectangleGeometry@ResourceHelper@@SAJHHHHPEAPEAVCRectangleGeometryProxy@@@Z.c)
 *     ?Update@CImageLegacyMilBrushProxy@@QEAAJNAEBUMilRectF@@0PEBVCDoubleResourceProxy@@W4Enum@MilBrushMappingMode@@2PEBVCRectResourceProxy@@3W44MilStretch@@W44MilTileMode@@W44MilHorizontalAlignment@@W44MilVerticalAlignment@@PEBVCBaseImageProxy@@@Z @ 0x18001B0D4 (-Update@CImageLegacyMilBrushProxy@@QEAAJNAEBUMilRectF@@0PEBVCDoubleResourceProxy@@W4Enum@MilBrus.c)
 *     ??$ReleaseGDIObject@PEAUHRGN__@@@@YAXAEAPEAUHRGN__@@@Z @ 0x180029520 (--$ReleaseGDIObject@PEAUHRGN__@@@@YAXAEAPEAUHRGN__@@@Z.c)
 *     ?CreateGeometryFromHRGN@ResourceHelper@@SAJPEAUHRGN__@@PEAPEAVCRgnGeometryProxy@@@Z @ 0x180029550 (-CreateGeometryFromHRGN@ResourceHelper@@SAJPEAUHRGN__@@PEAPEAVCRgnGeometryProxy@@@Z.c)
 *     ?Update@CCachedVisualImageProxy@@QEAAJAEBUMilRectF@@AEBUMilSizeD@@PEBVCRectResourceProxy@@PEBVCSizeResourceProxy@@PEAVCVisualProxy@@W4Enum@MilBrushMappingMode@@@Z @ 0x1800385B8 (-Update@CCachedVisualImageProxy@@QEAAJAEBUMilRectF@@AEBUMilSizeD@@PEBVCRectResourceProxy@@PEBVCS.c)
 *     ?Freeze@CCachedVisualImageProxy@@QEAAJXZ @ 0x180046EDC (-Freeze@CCachedVisualImageProxy@@QEAAJXZ.c)
 *     __security_check_cookie @ 0x180060050 (__security_check_cookie.c)
 *     ?_UpdateResourcesForMonitorHelper@CLivePreview@@AEAAJPEBVCTopLevelWindow@@PEAULivePreviewResource@@@Z @ 0x1800C5364 (-_UpdateResourcesForMonitorHelper@CLivePreview@@AEAAJPEBVCTopLevelWindow@@PEAULivePreviewResourc.c)
 */

__int64 __fastcall CLivePreview::_UpdateResourcesForMonitor(CLivePreview *this, struct LivePreviewResource *a2)
{
  unsigned int v3; // ebx
  unsigned int v5; // esi
  CLivePreview *v6; // rcx
  const struct CTopLevelWindow *v7; // rdx
  int updated; // eax
  char v9; // al
  int v10; // r9d
  int v11; // edx
  int v12; // r8d
  int RectangleGeometry; // eax
  CCachedVisualImageProxy **v14; // rsi
  int v15; // eax
  __int64 i; // rax
  __int64 v17; // rax
  CCachedVisualImageProxy *v18; // rcx
  int v19; // eax
  int v20; // eax
  CBaseObject **v21; // rdx
  __int64 v22; // rcx
  int v23; // eax
  int v24; // eax
  int v25; // r9d
  int v26; // edx
  int v27; // r8d
  int v28; // eax
  CCachedVisualImageProxy **v29; // rsi
  int v30; // eax
  __int64 j; // rax
  __int64 v32; // rax
  CCachedVisualImageProxy *v33; // rcx
  int v34; // eax
  int v35; // eax
  CBaseObject **v36; // rdx
  __int64 v37; // rcx
  int v38; // eax
  int v39; // eax
  int GeometryFromHRGN; // eax
  struct CRectangleGeometryProxy **v42; // [rsp+20h] [rbp-69h]
  struct CRectangleGeometryProxy **v43; // [rsp+20h] [rbp-69h]
  __int64 v44; // [rsp+50h] [rbp-39h]
  CCachedVisualImageProxy *v45; // [rsp+68h] [rbp-21h]
  CCachedVisualImageProxy *v46; // [rsp+68h] [rbp-21h]
  __int128 v47; // [rsp+70h] [rbp-19h] BYREF
  __int128 v48; // [rsp+80h] [rbp-9h] BYREF
  __int128 v49; // [rsp+90h] [rbp+7h] BYREF

  v3 = 0;
  SetRectEmpty((LPRECT)a2);
  SetRectEmpty((LPRECT)((char *)a2 + 40));
  SetRectEmpty((LPRECT)a2 + 5);
  *((_QWORD *)a2 + 12) = CreateRectRgn(0, 0, 0, 0);
  v5 = 0;
  if ( !*((_DWORD *)this + 84) )
  {
LABEL_5:
    *((_BYTE *)a2 + 128) = !IsRectEmpty((const RECT *)a2);
    v9 = !IsRectEmpty((const RECT *)((char *)a2 + 40));
    *((_BYTE *)a2 + 129) = v9;
    if ( *((_BYTE *)a2 + 128) )
    {
      v10 = 0;
      v11 = *((_DWORD *)a2 + 1);
      if ( *((_DWORD *)a2 + 3) - v11 >= 0 )
        v10 = *((_DWORD *)a2 + 3) - v11;
      v12 = 0;
      if ( *((_DWORD *)a2 + 2) - *(_DWORD *)a2 >= 0 )
        v12 = *((_DWORD *)a2 + 2) - *(_DWORD *)a2;
      RectangleGeometry = ResourceHelper::CreateRectangleGeometry(
                            *(_DWORD *)a2,
                            v11,
                            v12,
                            v10,
                            (struct CRectangleGeometryProxy **)a2 + 2);
      v3 = RectangleGeometry;
      if ( RectangleGeometry < 0 )
      {
        MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, RectangleGeometry, 0x529u);
        return v3;
      }
      v14 = (CCachedVisualImageProxy **)((char *)a2 + 24);
      if ( !*((_QWORD *)a2 + 3) )
      {
        v15 = CCompositor::CreateProxy<CCachedVisualImageProxy>(
                *((_QWORD *)CDesktopManager::s_pDesktopManagerInstance + 5),
                (CBaseObject **)a2 + 3);
        v3 = v15;
        if ( v15 < 0 )
        {
          MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v15, 0x52Du);
          return v3;
        }
      }
      for ( i = 0LL; i < 4; ++i )
        *((float *)&v47 + i) = (float)*((int *)a2 + i);
      v17 = *((_QWORD *)this + 63);
      v18 = *v14;
      v48 = 0LL;
      v19 = CCachedVisualImageProxy::Update(
              (__int64)v18,
              (__int64)&v47,
              (__int64)&v48,
              0LL,
              0LL,
              *(_QWORD *)(v17 + 16),
              0);
      v3 = v19;
      if ( v19 < 0 )
      {
        MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v19, 0x53Cu);
        return v3;
      }
      v20 = CCachedVisualImageProxy::Freeze(*v14);
      v3 = v20;
      if ( v20 < 0 )
      {
        MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v20, 0x53Eu);
        return v3;
      }
      v21 = (CBaseObject **)((char *)a2 + 32);
      v22 = *((_QWORD *)a2 + 4);
      if ( !v22 )
      {
        v23 = CCompositor::CreateProxy<CImageLegacyMilBrushProxy>(
                *((_QWORD *)CDesktopManager::s_pDesktopManagerInstance + 5),
                v21);
        v3 = v23;
        if ( v23 < 0 )
        {
          MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v23, 0x542u);
          return v3;
        }
        v22 = *((_QWORD *)a2 + 4);
      }
      v45 = *v14;
      v42 = (struct CRectangleGeometryProxy **)*((_QWORD *)this + 60);
      v49 = _xmm;
      v47 = _xmm;
      v24 = CImageLegacyMilBrushProxy::Update(
              v22,
              (__int64)v21,
              (__int64)&v49,
              (__int64)&v47,
              (__int64)v42,
              1u,
              1,
              0LL,
              0LL,
              0,
              v44,
              0,
              0,
              (__int64)v45);
      v3 = v24;
      if ( v24 < 0 )
      {
        MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v24, 0x55Du);
        return v3;
      }
      v9 = *((_BYTE *)a2 + 129);
    }
    if ( v9 )
    {
      v25 = 0;
      v26 = *((_DWORD *)a2 + 11);
      if ( *((_DWORD *)a2 + 13) - v26 >= 0 )
        v25 = *((_DWORD *)a2 + 13) - v26;
      v27 = 0;
      if ( *((_DWORD *)a2 + 12) - *((_DWORD *)a2 + 10) >= 0 )
        v27 = *((_DWORD *)a2 + 12) - *((_DWORD *)a2 + 10);
      v28 = ResourceHelper::CreateRectangleGeometry(
              *((_DWORD *)a2 + 10),
              v26,
              v27,
              v25,
              (struct CRectangleGeometryProxy **)a2 + 7);
      v3 = v28;
      if ( v28 < 0 )
      {
        MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v28, 0x567u);
        return v3;
      }
      v29 = (CCachedVisualImageProxy **)((char *)a2 + 64);
      if ( !*((_QWORD *)a2 + 8) )
      {
        v30 = CCompositor::CreateProxy<CCachedVisualImageProxy>(
                *((_QWORD *)CDesktopManager::s_pDesktopManagerInstance + 5),
                (CBaseObject **)a2 + 8);
        v3 = v30;
        if ( v30 < 0 )
        {
          MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v30, 0x56Bu);
          return v3;
        }
      }
      for ( j = 0LL; j < 4; ++j )
        *((float *)&v47 + j) = (float)*((int *)a2 + j + 10);
      v32 = *((_QWORD *)this + 65);
      v33 = *v29;
      v49 = 0LL;
      v34 = CCachedVisualImageProxy::Update(
              (__int64)v33,
              (__int64)&v47,
              (__int64)&v49,
              0LL,
              0LL,
              *(_QWORD *)(v32 + 16),
              0);
      v3 = v34;
      if ( v34 < 0 )
      {
        MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v34, 0x57Au);
        return v3;
      }
      v35 = CCachedVisualImageProxy::Freeze(*v29);
      v3 = v35;
      if ( v35 < 0 )
      {
        MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v35, 0x57Cu);
        return v3;
      }
      v36 = (CBaseObject **)((char *)a2 + 72);
      v37 = *((_QWORD *)a2 + 9);
      if ( !v37 )
      {
        v38 = CCompositor::CreateProxy<CImageLegacyMilBrushProxy>(
                *((_QWORD *)CDesktopManager::s_pDesktopManagerInstance + 5),
                v36);
        v3 = v38;
        if ( v38 < 0 )
        {
          MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v38, 0x580u);
          return v3;
        }
        v37 = *((_QWORD *)a2 + 9);
      }
      v46 = *v29;
      v43 = (struct CRectangleGeometryProxy **)*((_QWORD *)this + 61);
      v48 = _xmm;
      v47 = _xmm;
      v39 = CImageLegacyMilBrushProxy::Update(
              v37,
              (__int64)v36,
              (__int64)&v48,
              (__int64)&v47,
              (__int64)v43,
              1u,
              1,
              0LL,
              0LL,
              0,
              v44,
              0,
              0,
              (__int64)v46);
      v3 = v39;
      if ( v39 < 0 )
      {
        MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v39, 0x59Bu);
        return v3;
      }
      GeometryFromHRGN = ResourceHelper::CreateGeometryFromHRGN(
                           *((HRGN *)a2 + 12),
                           (struct CRgnGeometryProxy **)a2 + 13);
      v3 = GeometryFromHRGN;
      if ( GeometryFromHRGN < 0 )
      {
        MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, GeometryFromHRGN, 0x59Du);
        return v3;
      }
    }
    ReleaseGDIObject<HRGN__ *>((void **)a2 + 12);
    return v3;
  }
  while ( 1 )
  {
    v6 = *(CLivePreview **)(*((_QWORD *)this + 39) + 40LL * v5);
    v7 = (const struct CTopLevelWindow *)*((_QWORD *)v6 + 55);
    if ( v7 )
    {
      updated = CLivePreview::_UpdateResourcesForMonitorHelper(v6, v7, a2);
      v3 = updated;
      if ( updated < 0 )
        break;
    }
    if ( ++v5 >= *((_DWORD *)this + 84) )
      goto LABEL_5;
  }
  MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, updated, 0x51Au);
  return v3;
}
