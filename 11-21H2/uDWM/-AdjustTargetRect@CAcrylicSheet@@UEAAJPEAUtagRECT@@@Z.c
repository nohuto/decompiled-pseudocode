/*
 * XREFs of ?AdjustTargetRect@CAcrylicSheet@@UEAAJPEAUtagRECT@@@Z @ 0x180099B60
 * Callers:
 *     <none>
 * Callees:
  ...
  if ( v4 )
  {
    ...
    if ( GetMonitorInfoW(v4, &mi) )
    {
      ...
      if ( mi.rcWork.left == a2->left )
        a2->left -= (int)(*((double *)CDesktopManager::s_pDesktopManagerInstance + 51) * -8.0);
      right = a2->right;
      if ( mi.rcWork.right == right )
        a2->right = right - (int)(*((double *)v6 + 51) * 8.0);
      top = a2->top;
      if ( mi.rcWork.top == top )
        a2->top = top - (int)(*((double *)v6 + 51) * -8.0);
      bottom = a2->bottom;
      if ( mi.rcWork.bottom == bottom )
        a2->bottom = bottom - (int)(*((double *)v6 + 51) * 8.0);
    }
    else
    {
      ...
      if ( v3 >= 0 )
        v3 = -2003304445;
      MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v3, 0x1B4u);
    }
  }
