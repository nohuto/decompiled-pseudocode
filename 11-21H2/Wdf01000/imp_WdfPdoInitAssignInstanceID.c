/*
 * XREFs of imp_WdfPdoInitAssignInstanceID @ 0x1C0030930
 * Callers:
 *     <none>
 * Callees:
        ...
        const _UNICODE_STRING *InstanceID)
{
  ...
  if ( (int)result >= 0 )
  {
    ...
    if ( (int)result >= 0 )
    {
      ...
      {
        v8 = DeviceInit->Pdo.InstanceID;
        if ( v8 )
          return FxDuplicateUnicodeString(v8->m_Globals, InstanceID, &v8->m_UnicodeString);
