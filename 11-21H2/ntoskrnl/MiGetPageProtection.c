/*
 * XREFs of MiGetPageProtection @ 0x140319160
 * Callers:
 *     MiQueryAddressState @ 0x140318330 (MiQueryAddressState.c)
 *     MiCommitExistingVad @ 0x14032C1B0 (MiCommitExistingVad.c)
 *     MiProtectPrivateMemory @ 0x14032EA60 (MiProtectPrivateMemory.c)
 * Callees:
 *     MiLocateCloneAddress @ 0x140234F14 (MiLocateCloneAddress.c)
 *     MiGetImageProtoProtection @ 0x14024129C (MiGetImageProtoProtection.c)
 *     MiRotatedToFrameBuffer @ 0x1402433D0 (MiRotatedToFrameBuffer.c)
 *     MiTryLockProtoPoolPageAtDpc @ 0x14026FA0C (MiTryLockProtoPoolPageAtDpc.c)
 *     MI_PROTO_FORMAT_COMBINED @ 0x1402802DC (MI_PROTO_FORMAT_COMBINED.c)
 *     MiIsPrototypePteVadLookup @ 0x1402806A0 (MiIsPrototypePteVadLookup.c)
 *     MiLockTransitionLeafPageEx @ 0x140315D60 (MiLockTransitionLeafPageEx.c)
 *     MiGetProtoPteAddress @ 0x140319600 (MiGetProtoPteAddress.c)
 *     MiUnlockProtoPoolPage @ 0x140334790 (MiUnlockProtoPoolPage.c)
 *     MiCaptureProtectionFromLockedProto @ 0x140375150 (MiCaptureProtectionFromLockedProto.c)
 *     KeBugCheckEx @ 0x14041F3D0 (KeBugCheckEx.c)
 *     MiGetProtectionFromPte @ 0x1405A5FCC (MiGetProtectionFromPte.c)
 *     MiGetValidAwePartitionId @ 0x1405AB064 (MiGetValidAwePartitionId.c)
 *     MiGetValidAweProtection @ 0x1405AB0FC (MiGetValidAweProtection.c)
 */

unsigned __int64 __fastcall MiGetPageProtection(
        ULONG_PTR BugCheckParameter2,
        unsigned __int64 a2,
        __int64 **a3,
        _DWORD *a4,
        _WORD *a5)
{
  _WORD *v5; // r14
  unsigned __int64 v6; // rdi
  unsigned int v11; // esi
  __int64 v12; // r8
  unsigned __int64 v13; // rax
  __int64 v14; // r15
  __int64 v15; // rcx
  unsigned __int64 result; // rax
  unsigned __int64 v17; // r8
  unsigned __int64 v18; // rax
  __int64 v19; // r8
  _KPROCESS *Process; // rcx
  __int64 v21; // rax
  __int64 v22; // r8
  __int64 v23; // rdi
  unsigned __int64 v24; // rbx
  __int64 *ProtoPteAddress; // rsi
  _QWORD *v26; // rdx
  __int64 v27; // rdx
  __int64 v28; // rbx
  int v29; // r8d
  unsigned __int64 v30; // r9
  _KPROCESS *v31; // rcx
  _WORD *v32; // rcx
  struct _LIST_ENTRY *Flink; // rdx
  __int64 v34; // rax
  __int64 v35; // rdx
  unsigned int v36; // esi
  unsigned int v37; // r8d
  int v38; // edx
  struct _LIST_ENTRY *v39; // r9
  __int64 v40; // rcx
  _QWORD *CloneAddress; // rax
  __int64 v42; // [rsp+60h] [rbp+8h] BYREF

  v5 = a5;
  v6 = *(_QWORD *)a2;
  v42 = 0LL;
  a5 = 0LL;
  *v5 = 0;
  *a3 = 0LL;
  *a4 = 0;
  if ( a2 >= 0xFFFFF6FB7DBED000uLL
    && a2 <= 0xFFFFF6FB7DBED7F8uLL
    && (MiFlags & 0xC00000) != 0
    && KeGetCurrentThread()->ApcState.Process->AddressPolicy != 1
    && (v6 & 1) != 0
    && ((v6 & 0x20) == 0 || (v6 & 0x42) == 0) )
  {
    Flink = KeGetCurrentThread()->ApcState.Process[1].ProcessListEntry.Flink;
    if ( Flink )
    {
      v34 = *((_QWORD *)&Flink->Flink + ((a2 >> 3) & 0x1FF));
      v35 = v6 | 0x20;
      if ( (v34 & 0x20) == 0 )
        v35 = v6;
      v6 = v35;
      if ( (v34 & 0x42) != 0 )
        v6 = v35 | 0x42;
    }
  }
  v11 = *(_DWORD *)(BugCheckParameter2 + 48);
  v12 = v11 & 0x70;
  if ( (_DWORD)v12 == 48 && (v11 & 0x6200000) != 0x4200000 )
  {
    *v5 = MiGetValidAwePartitionId(v6, 0xFFFFF6FB7DBED000uLL, v12, 0xFFFFF6FB7DBED7F8uLL);
    return MiGetValidAweProtection(BugCheckParameter2, v6);
  }
  if ( (v6 & 1) != 0 )
  {
    v13 = (v6 >> 12) & 0xFFFFFFFFFFLL;
    v14 = 6 * v13;
    if ( (_DWORD)v12 == 16 )
    {
      if ( v13 <= qword_140C50840 && ((*(_QWORD *)(48 * v13 - 0x21FFFFFFFFD8LL) >> 54) & 1) != 0 )
        *v5 = (*(_QWORD *)(48 * v13 - 0x220000000000LL + 40) >> 43) & 0x3FF;
      v36 = (v11 >> 7) & 0x1F;
      if ( (v6 & 0x18) == 8 )
      {
        v36 |= 0x18u;
      }
      else if ( (v6 & 0x10) != 0 )
      {
        v36 |= 8u;
      }
      return v36;
    }
    else if ( (_DWORD)v12 == 64 && MiRotatedToFrameBuffer(a2) )
    {
      v37 = 4;
      if ( (v6 & 0x800) == 0 )
        v37 = 1;
      v38 = v11 & 0xC00;
      if ( (v11 & 0x380) != 0 && v38 == 3072 )
      {
        v37 |= 0x18u;
      }
      else if ( v38 == 1024 )
      {
        v37 |= 8u;
      }
      return v37;
    }
    else
    {
      v15 = *(_QWORD *)(8 * v14 - 0x220000000000LL + 40);
      *v5 = ((unsigned __int64)v15 >> 43) & 0x3FF;
      if ( (v15 & 0x10000000000LL) != 0 )
      {
        result = MiGetProtectionFromPte(BugCheckParameter2, v6);
        if ( (*(_DWORD *)(BugCheckParameter2 + 48) & 0x2200000) == 0x2200000 )
          *a4 = 0;
      }
      else if ( v15 < 0 )
      {
        v17 = ((((__int64)(a2 << 25) >> 16) & 0xFFFFFFFFF000uLL) >> 9) - 0x98000000000LL;
        v18 = *(_QWORD *)v17;
        if ( v17 >= 0xFFFFF6FB7DBED000uLL
          && v17 <= 0xFFFFF6FB7DBED7F8uLL
          && (MiFlags & 0xC00000) != 0
          && KeGetCurrentThread()->ApcState.Process->AddressPolicy != 1
          && (v18 & 1) != 0
          && ((v18 & 0x20) == 0 || (v18 & 0x42) == 0) )
        {
          v39 = KeGetCurrentThread()->ApcState.Process[1].ProcessListEntry.Flink;
          if ( v39 )
          {
            v40 = *((_QWORD *)&v39->Flink + ((v17 >> 3) & 0x1FF));
            if ( (v40 & 0x20) != 0 )
              v18 |= 0x20uLL;
            if ( (v40 & 0x42) != 0 )
              v18 |= 0x42uLL;
          }
        }
        result = (v18 >> 60) & 7;
        if ( (_DWORD)result )
        {
          if ( (*(_QWORD *)v17 & 0x18) == 8 )
          {
            return (unsigned int)result | 0x18;
          }
          else if ( (*(_QWORD *)v17 & 0x10) != 0 )
          {
            return (unsigned int)result | 8;
          }
        }
        else
        {
          v19 = (*(_DWORD *)(8 * v14 - 0x220000000000LL + 16) >> 5) & 0x1F;
          if ( *(__int64 *)(8 * v14 - 0x220000000000LL + 8) > 0 )
          {
            return (unsigned int)MmMakeProtectNotWriteCopy[v19];
          }
          else
          {
            Process = KeGetCurrentThread()->ApcState.Process;
            if ( Process[1].Affinity.StaticBitmap[12] )
            {
              if ( MiLocateCloneAddress(
                     (__int64)Process,
                     *(_QWORD *)(8 * v14 - 0x220000000000LL + 8) | 0x8000000000000000uLL) )
              {
                LODWORD(v19) = MmMakeProtectNotWriteCopy[v19];
              }
            }
            return (unsigned int)v19;
          }
        }
      }
      else
      {
        return (*(_DWORD *)(8 * v14 - 0x220000000000LL + 16) >> 5) & 0x1F;
      }
    }
    return result;
  }
  if ( (v6 & 0x400) == 0 )
  {
    if ( (v6 & 0x800) != 0 && (v21 = MiLockTransitionLeafPageEx(a2, 0LL, 2)) != 0 )
    {
      v22 = *(_QWORD *)(qword_140C51F48 + 8 * ((*(_QWORD *)(v21 + 40) >> 43) & 0x3FFLL));
      if ( (*(_DWORD *)(v22 + 4) & 0x100) != 0 )
        v22 = **(_QWORD **)(v22 + 16624);
      *v5 = *(_WORD *)v22;
      _InterlockedAnd64((volatile signed __int64 *)(v21 + 24), 0x7FFFFFFFFFFFFFFFuLL);
    }
    else
    {
      *v5 = **(_WORD **)(qword_140C51F48 + 8LL * KeGetCurrentThread()->ApcState.Process[1].IdealProcessor[25]);
    }
    v23 = (v6 >> 5) & 0x1F;
    if ( (*(_DWORD *)(BugCheckParameter2 + 48) & 0x2200000) == 0x2200000 )
      *a4 = 0;
    return (unsigned int)v23;
  }
  if ( MiIsPrototypePteVadLookup(v6) )
  {
    v23 = (v6 >> 5) & 0x1F;
    if ( (v11 & 0x200000) == 0 )
    {
      *v5 = **(_WORD **)(qword_140C51F48 + 8LL * (*(_WORD *)(**(_QWORD **)(BugCheckParameter2 + 72) + 60LL) & 0x3FF));
      v24 = (unsigned __int64)((__int64)(a2 << 25) >> 16) >> 12;
      ProtoPteAddress = (__int64 *)MiGetProtoPteAddress(BugCheckParameter2, v24, 4LL, &a5);
      if ( !ProtoPteAddress )
        return 0LL;
      v26 = *(_QWORD **)(BugCheckParameter2 + 120);
      if ( (__int64)v26 < 0
        && v24
         - (*(unsigned int *)(BugCheckParameter2 + 24) | ((unsigned __int64)*(unsigned __int8 *)(BugCheckParameter2 + 32) << 32)) > (unsigned __int64)(*v26 - 1LL) >> 12 )
      {
        return 0LL;
      }
      if ( !*(_QWORD *)(*(_QWORD *)a5 + 64LL) && (*(_DWORD *)(*(_QWORD *)a5 + 56LL) & 0x2000) == 0 )
      {
        if ( (int)MiTryLockProtoPoolPageAtDpc((unsigned __int64)ProtoPteAddress, (__int64)v26, &v42, 0LL) < 0 )
        {
          *a3 = ProtoPteAddress;
          return 256LL;
        }
        LOBYTE(v27) = 2;
        v28 = *ProtoPteAddress;
        MiUnlockProtoPoolPage(v42, v27);
        if ( !v28 )
          return 0LL;
      }
    }
    return (unsigned int)v23;
  }
  if ( MI_PROTO_FORMAT_COMBINED(v6) )
  {
    v32 = **(_WORD ***)(((v30 - 48) & 0xFFFFFFFFFFFFF000uLL) + 0x10);
LABEL_47:
    *v5 = *v32;
    return (unsigned int)MmMakeProtectNotWriteCopy[(unsigned int)MiCaptureProtectionFromLockedProto(v30)];
  }
  v31 = KeGetCurrentThread()->ApcState.Process;
  if ( v31[1].Affinity.StaticBitmap[12] )
  {
    CloneAddress = MiLocateCloneAddress((__int64)v31, v30);
    if ( CloneAddress )
    {
      v32 = *(_WORD **)(CloneAddress[7] + 24LL);
      goto LABEL_47;
    }
  }
  if ( v29 != 32 || (v11 & 0xF80) != 0x380 )
    KeBugCheckEx(0x1Au, 0x41202uLL, a2, v6, BugCheckParameter2);
  *v5 = **(_WORD **)(qword_140C51F48 + 8LL * (*(_WORD *)(**(_QWORD **)(BugCheckParameter2 + 72) + 60LL) & 0x3FF));
  return MiGetImageProtoProtection(BugCheckParameter2, v30);
}
