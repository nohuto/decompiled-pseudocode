/*
 * XREFs of sub_14035A264 @ 0x14035A264
 * Callers:
 *     sub_1402B4EC0 @ 0x1402B4EC0 (sub_1402B4EC0.c)
 *     sub_1403591D8 @ 0x1403591D8 (sub_1403591D8.c)
 *     sub_14039C4BC @ 0x14039C4BC (sub_14039C4BC.c)
 * Callees:
 *     sub_14035A424 @ 0x14035A424 (sub_14035A424.c)
 */

_QWORD **__fastcall sub_14035A264(__int64 a1, char a2, int a3, int a4, __int64 a5)
{
  _QWORD **result; // rax
  unsigned __int64 v8; // rbx
  __int64 v10; // r12
  unsigned __int64 v12; // rcx
  _QWORD *v13; // rcx
  _UNKNOWN *retaddr; // [rsp+58h] [rbp+0h] BYREF
  int v15; // [rsp+60h] [rbp+8h] BYREF
  int v16; // [rsp+68h] [rbp+10h] BYREF

  result = (_QWORD **)&retaddr;
  v15 = 0;
  v16 = 0;
  v8 = *(_QWORD *)(a1 + 56);
  v10 = a5;
  LOBYTE(v16) = a2;
  while ( v8 && *(char *)(v8 + 24) < a2 )
  {
    sub_14035A424(v8 - 24, (unsigned int)&v16, a3, a4, v10, (__int64)&v15);
    *(_BYTE *)(v8 + 24) = a2;
    v12 = v8;
    result = *(_QWORD ***)(v8 + 8);
    if ( result )
    {
      v13 = *result;
      v8 = *(_QWORD *)(v8 + 8);
      if ( *result )
      {
        do
        {
          result = (_QWORD **)*v13;
          v8 = (unsigned __int64)v13;
          v13 = result;
        }
        while ( result );
      }
    }
    else
    {
      while ( 1 )
      {
        v8 = *(_QWORD *)(v8 + 16) & 0xFFFFFFFFFFFFFFFCuLL;
        if ( !v8 || *(_QWORD *)v8 == v12 )
          break;
        v12 = v8;
      }
    }
  }
  if ( !*(_BYTE *)(a1 + 17) )
    return (_QWORD **)sub_14035A424(a1, (unsigned int)&v16, a3, a4, v10, (__int64)&v15);
  return result;
}
