/*
 * XREFs of sub_140A33D28 @ 0x140A33D28
 * Callers:
 *     sub_140A32B9C @ 0x140A32B9C (sub_140A32B9C.c)
 *     sub_140A32D28 @ 0x140A32D28 (sub_140A32D28.c)
 *     sub_140A32E98 @ 0x140A32E98 (sub_140A32E98.c)
 *     sub_140A32F88 @ 0x140A32F88 (sub_140A32F88.c)
 *     sub_140A33A58 @ 0x140A33A58 (sub_140A33A58.c)
 *     sub_140A33E74 @ 0x140A33E74 (sub_140A33E74.c)
 * Callees:
 *     sub_140A339DC @ 0x140A339DC (sub_140A339DC.c)
 */

__int64 __fastcall sub_140A33D28(_BYTE *a1, int a2, unsigned int *a3, _QWORD *a4)
{
  unsigned int v6; // ebx
  unsigned __int8 *v7; // rsi
  int v8; // edi
  char v9; // al
  unsigned int v10; // ecx
  int v12; // eax
  unsigned int v13; // [rsp+48h] [rbp+10h] BYREF

  v13 = 0;
  v6 = a2 - 1;
  if ( !a2 )
    return (unsigned int)-2;
  v7 = a1 + 1;
  if ( (*a1 & 0x1F) == 0x1F )
  {
    v8 = 2;
    while ( v6 )
    {
      v9 = *v7;
      --v6;
      ++v7;
      if ( v9 >= 0 )
        goto LABEL_10;
      ++v8;
    }
    return (unsigned int)-2;
  }
  v8 = 1;
LABEL_10:
  v12 = sub_140A339DC(&v13, v7, v6);
  v10 = v12;
  if ( v12 >= 0 )
  {
    if ( v13 <= v6 - v12 )
    {
      *a3 = v13;
      *a4 = &v7[v12];
      return (unsigned int)(v8 + v12);
    }
    return (unsigned int)-2;
  }
  return v10;
}
