/*
 * XREFs of sub_1405DE9D4 @ 0x1405DE9D4
 * Callers:
 *     sub_140751B78 @ 0x140751B78 (sub_140751B78.c)
 * Callees:
 *     sub_14024A9B0 @ 0x14024A9B0 (sub_14024A9B0.c)
 *     __security_check_cookie @ 0x1403DF760 (__security_check_cookie.c)
 *     sub_1405D5D28 @ 0x1405D5D28 (sub_1405D5D28.c)
 *     sub_1405DEB88 @ 0x1405DEB88 (sub_1405DEB88.c)
 *     sub_14099FFFC @ 0x14099FFFC (sub_14099FFFC.c)
 */

__int64 __fastcall sub_1405DE9D4(__int64 a1, __int64 a2)
{
  int v3; // edi
  __int64 v4; // r9
  __int64 v5; // rbx
  __int64 v6; // r9
  __int64 v7; // r9
  __int16 v9; // [rsp+40h] [rbp-39h] BYREF
  __int64 v10; // [rsp+48h] [rbp-31h] BYREF
  __int64 v11; // [rsp+50h] [rbp-29h] BYREF
  __int64 v12; // [rsp+58h] [rbp-21h]
  struct _EVENT_DATA_DESCRIPTOR v13; // [rsp+60h] [rbp-19h] BYREF
  __int16 *v14; // [rsp+80h] [rbp+7h]
  __int64 v15; // [rsp+88h] [rbp+Fh]
  __int64 *v16; // [rsp+90h] [rbp+17h]
  __int64 v17; // [rsp+98h] [rbp+1Fh]

  v10 = 0LL;
  v11 = 0LL;
  LODWORD(v12) = 0;
  if ( qword_140C1C948 )
  {
    v3 = -1073741768;
  }
  else if ( *(_DWORD *)(a1 + 4) )
  {
    v3 = -1073741735;
  }
  else if ( *(_QWORD *)(a1 + 16) )
  {
    v3 = sub_14099FFFC(a1 + 24, a1, a2 + a1, &v10);
    if ( v3 >= 0 )
    {
      v5 = v10;
      *(_QWORD *)(v10 + 32) = *(_QWORD *)(a1 + 8);
      *(_QWORD *)(v5 + 40) = *(_QWORD *)(a1 + 16);
      v12 = v5;
      sub_1405D5D28(0LL, (__int64)sub_1405DE2A0, 0LL, v4, 1, 0, (__int64)&v11);
      sub_1405D5D28((__int64)sub_1405DE3B0, (__int64)sub_1405DE840, (__int64)sub_1405DE550, v6, 0, 0, (__int64)&v11);
      sub_1405DEB88(v5);
      sub_1405D5D28(0LL, (__int64)sub_1405DE3F0, 0LL, v7, 0, 1, (__int64)&v11);
    }
  }
  else
  {
    v3 = -1073741811;
  }
  if ( (unsigned int)dword_140C03A00 > 5 )
  {
    v9 = 1;
    v17 = 4LL;
    v14 = &v9;
    v15 = 2LL;
    v16 = &v10;
    LODWORD(v10) = v3;
    sub_14024A9B0((__int64)&dword_140C03A00, (unsigned __int8 *)&dword_1400323C4, 0LL, 1u, 0, 0, 4u, &v13);
  }
  return (unsigned int)v3;
}
