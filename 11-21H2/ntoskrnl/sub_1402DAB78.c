/*
 * XREFs of sub_1402DAB78 @ 0x1402DAB78
 * Callers:
 *     sub_140A6FA44 @ 0x140A6FA44 (sub_140A6FA44.c)
 *     sub_140A7328C @ 0x140A7328C (sub_140A7328C.c)
 *     sub_140A73424 @ 0x140A73424 (sub_140A73424.c)
 * Callees:
 *     KeQueryActiveProcessorCountEx @ 0x140348830 (KeQueryActiveProcessorCountEx.c)
 *     sub_140A6F124 @ 0x140A6F124 (sub_140A6F124.c)
 *     sub_140A70018 @ 0x140A70018 (sub_140A70018.c)
 */

__int64 __fastcall sub_1402DAB78(int a1, __int64 a2, __int64 a3)
{
  __int16 v4; // ax
  ULONG ActiveProcessorCount; // eax
  ULONG v7; // ecx
  __int64 v8; // rdi
  __int64 v9; // rsi
  __int64 result; // rax
  unsigned int v11; // [rsp+40h] [rbp+8h] BYREF

  v11 = 0;
  v4 = word_140D069A4;
  *(_DWORD *)a3 = a1;
  *(_WORD *)(a3 + 4) = v4;
  *(_WORD *)(a3 + 6) = HIDWORD(KeGetPcr()[1].LockArray);
  ActiveProcessorCount = KeQueryActiveProcessorCountEx(0xFFFFu);
  v7 = *(unsigned __int16 *)(a3 + 6);
  *(_DWORD *)(a3 + 8) = ActiveProcessorCount;
  if ( ActiveProcessorCount <= v7 )
    *(_DWORD *)(a3 + 8) = v7 + 1;
  *(_QWORD *)(a3 + 16) = KeGetCurrentThread();
  v8 = *(_QWORD *)(a2 + 248);
  *(_QWORD *)(a3 + 24) = v8;
  *(_OWORD *)(a3 + 192) = 0LL;
  *(_OWORD *)(a3 + 208) = 0LL;
  *(_OWORD *)(a3 + 224) = 0LL;
  sub_140A6F124(v8, a3 + 216, 16, 0, 4, (__int64)&v11);
  v9 = v11;
  *(_WORD *)(a3 + 212) = v11;
  result = sub_140A70018(v8, v8 + v9 - 1);
  if ( (_BYTE)result )
    return sub_140A6F124(v8, (int)a3 + 216, v9, 0, 4, (__int64)&v11);
  return result;
}
