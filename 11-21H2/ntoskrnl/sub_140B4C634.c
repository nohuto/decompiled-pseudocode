/*
 * XREFs of sub_140B4C634 @ 0x140B4C634
 * Callers:
 *     sub_140AF968C @ 0x140AF968C (sub_140AF968C.c)
 * Callees:
 *     sub_1403DEA70 @ 0x1403DEA70 (sub_1403DEA70.c)
 *     qsort @ 0x1403E1E70 (qsort.c)
 *     memset @ 0x140435E00 (memset.c)
 *     sub_140B4CB64 @ 0x140B4CB64 (sub_140B4CB64.c)
 */

__int64 __fastcall sub_140B4C634(__int64 a1, _QWORD *a2, unsigned int a3, unsigned int *a4)
{
  __int64 v4; // rbx
  size_t v6; // rdi
  _QWORD *v8; // rax
  unsigned int v9; // ecx
  unsigned int v10; // ebx
  unsigned int v11; // r12d
  unsigned __int64 v12; // rbp
  __int64 v13; // rbx
  int *v14; // r13
  __int64 v15; // rsi
  _QWORD *v17; // [rsp+20h] [rbp-48h]
  void *Base; // [rsp+28h] [rbp-40h]
  int v19; // [rsp+70h] [rbp+8h]
  unsigned __int16 v20; // [rsp+80h] [rbp+18h] BYREF

  v4 = qword_140C4ACF8;
  v6 = a3;
  v20 = 0;
  Base = 0LL;
  v8 = 0LL;
  v9 = 8;
  v19 = 8;
  if ( a3 )
  {
    if ( a3 < 8 )
    {
LABEL_3:
      v10 = -1073741789;
LABEL_25:
      memset(a2, 0, v6);
      if ( a4 )
        *a4 = v6;
      return v10;
    }
    *a2 = 0LL;
    v8 = a2 + 1;
    Base = a2 + 1;
  }
  v11 = 0;
  v12 = v4 + *(unsigned int *)(v4 + 4);
  v13 = v4 + 40;
  v14 = (int *)(v13 + 8);
  if ( v13 + 8 <= v12 )
  {
    v17 = v8 + 1;
    do
    {
      v15 = *(unsigned int *)(v13 + 4);
      if ( (unsigned int)v15 < 8 || v13 + v15 > v12 )
        break;
      if ( !(unsigned __int8)sub_140B4CB64(v13) )
        goto LABEL_24;
      if ( *(_WORD *)v13 == 2 )
      {
        if ( (int)sub_1403DEA70(*v14, &v20) < 0 )
        {
LABEL_24:
          v10 = -1073741216;
          if ( !(_DWORD)v6 )
            return v10;
          goto LABEL_25;
        }
        v9 = v19 + 40;
        v19 += 40;
        if ( (_DWORD)v6 )
        {
          if ( (unsigned int)v6 < v9 )
            goto LABEL_3;
          ++v11;
          *(_OWORD *)(v17 - 1) = 0LL;
          *(_OWORD *)(v17 + 1) = 0LL;
          v17[3] = 0LL;
          *((_DWORD *)v17 - 2) = v20;
          *((_DWORD *)v17 - 1) = *v14;
          *v17 = *(_QWORD *)(v13 + 16);
          *((_DWORD *)v17 + 2) = *(_DWORD *)(v13 + 24) & 0xF;
          *((_DWORD *)v17 + 3) = (*(_DWORD *)(v13 + 24) >> 4) & 0xF;
          *((_DWORD *)v17 + 4) = (*(_DWORD *)(v13 + 24) >> 8) & 0xF;
          *((_DWORD *)v17 + 5) = (unsigned __int8)HIBYTE(*(_WORD *)(v13 + 24)) >> 4;
          *((_DWORD *)v17 + 6) = *(unsigned __int16 *)(v13 + 26);
          LODWORD(v15) = *(_DWORD *)(v13 + 4);
          v17 += 5;
        }
      }
      else
      {
        v9 = v19;
      }
      v13 += (unsigned int)v15;
      v14 = (int *)(v13 + 8);
    }
    while ( v13 + 8 <= v12 );
  }
  if ( a4 )
    *a4 = v9;
  if ( (_DWORD)v6 )
  {
    *(_DWORD *)a2 = v11;
    if ( v11 > 1 )
      qsort(Base, v11, 0x28uLL, (int (__cdecl *)(const void *, const void *))sub_140B4C610);
  }
  return 0;
}
