/*
 * XREFs of sub_140774F24 @ 0x140774F24
 * Callers:
 *     sub_140774EC0 @ 0x140774EC0 (sub_140774EC0.c)
 *     sub_14081F570 @ 0x14081F570 (sub_14081F570.c)
 *     sub_14081F69C @ 0x14081F69C (sub_14081F69C.c)
 * Callees:
 *     ExAcquireFastMutex @ 0x14028A160 (ExAcquireFastMutex.c)
 *     ObfDereferenceObject @ 0x1402AD3E0 (ObfDereferenceObject.c)
 *     KeReleaseGuardedMutex @ 0x1402AF9B0 (KeReleaseGuardedMutex.c)
 *     sub_1403D99B4 @ 0x1403D99B4 (sub_1403D99B4.c)
 *     KeBugCheckEx @ 0x14041F3D0 (KeBugCheckEx.c)
 *     sub_1406DFD98 @ 0x1406DFD98 (sub_1406DFD98.c)
 *     sub_1407490C0 @ 0x1407490C0 (sub_1407490C0.c)
 *     sub_14081F570 @ 0x14081F570 (sub_14081F570.c)
 *     ExFreePoolWithTag @ 0x140A6E010 (ExFreePoolWithTag.c)
 */

void __fastcall sub_140774F24(char *P)
{
  __int64 v2; // r8
  void *v3; // rcx
  void *v4; // rcx
  void **v5; // rsi
  void *v6; // rdi
  void *v7; // rcx
  unsigned __int16 *v8; // rsi
  __int64 v9; // rax
  unsigned __int16 *v10; // rsi
  __int64 v11; // rax
  void **v12; // rax

  if ( P )
  {
    if ( *((_QWORD *)P + 90) )
    {
      v8 = (unsigned __int16 *)(P + 40);
      sub_1403D99B4((ULONG)P, (PVOID)0x310);
      if ( *v8 )
      {
        sub_1403D99B4((ULONG)v8, (PVOID)2);
        sub_1403D99B4(*((_QWORD *)v8 + 1), (PVOID)*v8);
      }
      if ( *((_WORD *)P + 28) )
      {
        sub_1403D99B4((_DWORD)P + 56, (PVOID)2);
        sub_1403D99B4(*((_QWORD *)P + 8), (PVOID)*((unsigned __int16 *)P + 28));
      }
      v9 = *((_QWORD *)P + 2);
      if ( v9 && *(_WORD *)(v9 + 56) )
      {
        sub_1403D99B4(v9 + 56, (PVOID)2);
        sub_1403D99B4(*(_QWORD *)(*((_QWORD *)P + 2) + 64LL), (PVOID)*(unsigned __int16 *)(*((_QWORD *)P + 2) + 56LL));
      }
      KeBugCheckEx(0xCAu, 5uLL, *((_QWORD *)P + 4), *((_QWORD *)P + 90), 0x11uLL);
    }
    v2 = *((_QWORD *)P + 4);
    if ( (*(_DWORD *)(v2 + 48) & 0x1000) != 0 && *((_QWORD *)P + 2) )
    {
      v10 = (unsigned __int16 *)(P + 40);
      sub_1403D99B4((ULONG)P, (PVOID)0x310);
      if ( *v10 )
      {
        sub_1403D99B4((ULONG)v10, (PVOID)2);
        sub_1403D99B4(*((_QWORD *)v10 + 1), (PVOID)*v10);
      }
      if ( *((_WORD *)P + 28) )
      {
        sub_1403D99B4((_DWORD)P + 56, (PVOID)2);
        sub_1403D99B4(*((_QWORD *)P + 8), (PVOID)*((unsigned __int16 *)P + 28));
      }
      v11 = *((_QWORD *)P + 2);
      if ( v11 && *(_WORD *)(v11 + 56) )
      {
        sub_1403D99B4(v11 + 56, (PVOID)2);
        sub_1403D99B4(*(_QWORD *)(*((_QWORD *)P + 2) + 64LL), (PVOID)*(unsigned __int16 *)(*((_QWORD *)P + 2) + 56LL));
      }
      KeBugCheckEx(0xCAu, 5uLL, *((_QWORD *)P + 4), 0LL, 0LL);
    }
    if ( (*((_DWORD *)P + 99) & 0x20000) != 0 )
    {
      sub_14081F570(-1, (_DWORD)qword_140C46250, v2, 0, 0LL);
    }
    else
    {
      v3 = (void *)*((_QWORD *)P + 54);
      if ( v3 )
        ObfDereferenceObject(v3);
      if ( *((_WORD *)P + 28) )
        ExFreePoolWithTag(*((PVOID *)P + 8), 0);
      sub_1406DFD98((__int64)P);
      ExAcquireFastMutex(&stru_140C461A0);
      v4 = (void *)*((_QWORD *)P + 55);
      if ( v4 )
        ExFreePoolWithTag(v4, 0);
      KeReleaseGuardedMutex(&stru_140C461A0);
      sub_1407490C0(*((_QWORD *)P + 4), 0LL);
      v5 = (void **)(P + 608);
      while ( 1 )
      {
        v6 = *v5;
        if ( *v5 == v5 )
          break;
        if ( *((void ***)v6 + 1) != v5 || (v12 = *(void ***)v6, *(void **)(*(_QWORD *)v6 + 8LL) != v6) )
          __fastfail(3u);
        *v5 = v12;
        v12[1] = v5;
        ExFreePoolWithTag(*((PVOID *)v6 + 3), 0);
        ExFreePoolWithTag(v6, 0);
      }
      v7 = (void *)*((_QWORD *)P + 89);
      if ( v7 )
        ExFreePoolWithTag(v7, 0x62655250u);
      *(_QWORD *)(*(_QWORD *)(*((_QWORD *)P + 4) + 312LL) + 40LL) = 0LL;
      ExFreePoolWithTag(P, 0);
      _InterlockedDecrement(&dword_140C46010);
    }
  }
}
