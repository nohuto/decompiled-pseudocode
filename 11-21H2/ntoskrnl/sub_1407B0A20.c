/*
 * XREFs of sub_1407B0A20 @ 0x1407B0A20
 * Callers:
 *     sub_140965FC0 @ 0x140965FC0 (sub_140965FC0.c)
 * Callees:
 *     ObfDereferenceObject @ 0x1402AD3E0 (ObfDereferenceObject.c)
 *     sub_1402F9540 @ 0x1402F9540 (sub_1402F9540.c)
 *     sub_14069B58C @ 0x14069B58C (sub_14069B58C.c)
 *     SeImpersonateClientEx @ 0x1406BFE80 (SeImpersonateClientEx.c)
 *     ObReferenceObjectByHandle @ 0x140732D00 (ObReferenceObjectByHandle.c)
 *     sub_1407ABD80 @ 0x1407ABD80 (sub_1407ABD80.c)
 *     sub_1407B0C70 @ 0x1407B0C70 (sub_1407B0C70.c)
 *     sub_1407B0EB0 @ 0x1407B0EB0 (sub_1407B0EB0.c)
 *     sub_1407B0F40 @ 0x1407B0F40 (sub_1407B0F40.c)
 *     sub_140966A84 @ 0x140966A84 (sub_140966A84.c)
 */

__int64 __fastcall sub_1407B0A20(void *a1, __int64 a2, unsigned __int64 a3)
{
  void *v5; // r10
  PVOID v6; // rbx
  struct _KTHREAD *CurrentThread; // rax
  void *v8; // r13
  KPROCESSOR_MODE v9; // dl
  int v10; // r14d
  unsigned __int64 v11; // rdi
  int v12; // r15d
  BOOL v13; // r12d
  int v14; // esi
  __int64 v15; // r9
  ULONG_PTR v16; // rdi
  __int64 v18; // rax
  int Object; // [rsp+20h] [rbp-B8h]
  int v20; // [rsp+30h] [rbp-A8h] BYREF
  ULONG_PTR BugCheckParameter2[3]; // [rsp+38h] [rbp-A0h] BYREF
  struct _SECURITY_CLIENT_CONTEXT ClientContext; // [rsp+50h] [rbp-88h] BYREF
  PVOID v24; // [rsp+E8h] [rbp+10h] BYREF
  int v25; // [rsp+F8h] [rbp+20h] BYREF

  v5 = a1;
  v6 = 0LL;
  BugCheckParameter2[0] = 0LL;
  CurrentThread = KeGetCurrentThread();
  --*((_WORD *)CurrentThread + 242);
  v25 = 0;
  v20 = 0;
  BugCheckParameter2[1] = 0LL;
  v8 = 0LL;
  v9 = *((_BYTE *)KeGetCurrentThread() + 562);
  LOBYTE(v24) = v9;
  if ( a2 )
  {
    sub_1407B0EB0(a2, &v25, &v20);
    v10 = v25;
    if ( !v25 )
    {
LABEL_28:
      v14 = -1073741811;
      goto LABEL_11;
    }
    v5 = a1;
    v9 = (char)v24;
  }
  else
  {
    v10 = v25;
  }
  v11 = a3 >> 2;
  if ( (unsigned int)(a3 >> 2) > 3 )
    goto LABEL_28;
  v12 = a3 & 1;
  v13 = (((4 * (_DWORD)v11) | 2) & (unsigned int)a3) != 0LL;
  v24 = 0LL;
  v14 = ObReferenceObjectByHandle(v5, 1u, qword_140D069D8, v9, &v24, 0LL);
  v6 = v24;
  if ( v14 < 0 )
    goto LABEL_11;
  if ( !v10 )
  {
    v18 = sub_14069B58C((__int64)v24);
    v8 = (void *)v18;
    if ( !v18 )
    {
      v14 = -1073741790;
      goto LABEL_11;
    }
    if ( (*(_DWORD *)(v18 + 256) & 0x10000) == 0 )
    {
      v14 = -1073741790;
      goto LABEL_11;
    }
    if ( (*(_DWORD *)(v18 + 416) & 0x400) != 0 )
    {
      v14 = -1073741790;
      goto LABEL_11;
    }
    ClientContext = *(struct _SECURITY_CLIENT_CONTEXT *)(v18 + 64);
    if ( v13 )
    {
      if ( (int)v11 > ClientContext.SecurityQos.ImpersonationLevel )
      {
        v14 = -1073741790;
        goto LABEL_11;
      }
      ClientContext.SecurityQos.ImpersonationLevel = v11;
    }
    v14 = SeImpersonateClientEx(&ClientContext, 0LL);
    goto LABEL_11;
  }
  v14 = sub_1407ABD80((__int64)v24, v10, v20, v15, BugCheckParameter2);
  if ( v14 >= 0 )
  {
    Object = v11;
    v16 = BugCheckParameter2[0];
    v14 = sub_1407B0C70((_DWORD)v6, BugCheckParameter2[0], v12, v13, Object);
    if ( dword_140C2A158 )
      sub_140966A84(v16);
    sub_1407B0F40(v16);
  }
LABEL_11:
  if ( v8 )
    ObfDereferenceObject(v8);
  if ( v6 )
    ObfDereferenceObject(v6);
  sub_1402F9540((__int64)KeGetCurrentThread());
  return (unsigned int)v14;
}
