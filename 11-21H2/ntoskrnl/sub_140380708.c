/*
 * XREFs of sub_140380708 @ 0x140380708
 * Callers:
 *     sub_140237D58 @ 0x140237D58 (sub_140237D58.c)
 *     sub_14037ED14 @ 0x14037ED14 (sub_14037ED14.c)
 *     sub_14037F4B4 @ 0x14037F4B4 (sub_14037F4B4.c)
 *     sub_14037F958 @ 0x14037F958 (sub_14037F958.c)
 *     sub_1403800F0 @ 0x1403800F0 (sub_1403800F0.c)
 *     sub_1405F5F44 @ 0x1405F5F44 (sub_1405F5F44.c)
 * Callees:
 *     sub_140237D58 @ 0x140237D58 (sub_140237D58.c)
 *     sub_140380840 @ 0x140380840 (sub_140380840.c)
 *     sub_140383620 @ 0x140383620 (sub_140383620.c)
 *     sub_1403838F0 @ 0x1403838F0 (sub_1403838F0.c)
 *     sub_1405F67D0 @ 0x1405F67D0 (sub_1405F67D0.c)
 */

__int64 __fastcall sub_140380708(__int64 a1, unsigned int a2, __int64 a3)
{
  __int64 v6; // rdx
  unsigned int v7; // ebx
  int v8; // ebp
  __int64 v9; // rcx
  __int64 *v10; // rdi
  __int64 v11; // rsi
  unsigned int v12; // eax
  int *v13; // r8
  int v14; // eax
  __int64 *v16; // rcx
  unsigned int v17; // [rsp+60h] [rbp+18h] BYREF
  int v18; // [rsp+68h] [rbp+20h] BYREF

  sub_140237D58((__int64 *)a1, a3);
  v7 = 0;
  if ( *(_DWORD *)(a3 + 24) == -1 )
  {
    v8 = 0;
    v10 = (__int64 *)(a3 + 8);
  }
  else
  {
    v8 = 1;
    v9 = 0LL;
    if ( *(_QWORD *)a1 )
      v9 = *(unsigned __int8 *)(*(_QWORD *)a1 + 2LL);
    if ( *(_DWORD *)(a3 + 28) < (unsigned int)v9 && !(unsigned int)sub_140383620(v9, v6, a3 + 28, a3) )
      return (unsigned int)-1073741670;
    v10 = *(__int64 **)a3;
  }
  v11 = *(_QWORD *)a1;
  if ( !*(_QWORD *)a1 )
  {
    if ( !v8 )
    {
      *v10 = 0LL;
      v10[1] = 0LL;
    }
    return (unsigned int)-1073741275;
  }
  while ( 1 )
  {
    v12 = sub_140380840(a1, v11, a2);
    if ( *(_BYTE *)(v11 + 3) )
      break;
    if ( v8 )
    {
      *v10 = v11;
      v10[1] = v11 + 16 * (v12 + 1LL);
      v10 += 2;
    }
    if ( v12 )
      v16 = (__int64 *)(v11 + 16LL * (v12 - 1) + 24);
    else
      v16 = (__int64 *)(v11 + 8);
    if ( **(_DWORD **)(a1 + 16) == -1 || *(_BYTE *)(v11 + 2) != 2 )
    {
      v11 = *v16;
    }
    else
    {
      v11 = sub_1405F67D0(a1 + 16, v16, 2 * (*(_DWORD *)(a3 + 32) & 1u));
      if ( !v11 )
        return (unsigned int)-1073741818;
    }
  }
  *v10 = v11;
  v13 = (int *)(v11 + 4 * (v12 + 4LL));
  v10[1] = (__int64)v13;
  if ( v8 )
    *(_DWORD *)(a3 + 24) = (((__int64)v10 - *(_QWORD *)a3) >> 4) + 1;
  if ( v12 >= (unsigned __int16)*(_DWORD *)v11 )
    return (unsigned int)-1073741275;
  v14 = *v13;
  v17 = a2;
  v18 = v14;
  if ( (unsigned int)sub_1403838F0(a1, &v18, &v17) )
    return (unsigned int)-1073741275;
  return v7;
}
