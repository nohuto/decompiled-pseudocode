/*
 * XREFs of sub_1403528A4 @ 0x1403528A4
 * Callers:
 *     sub_1402CFF00 @ 0x1402CFF00 (sub_1402CFF00.c)
 *     sub_1403525F0 @ 0x1403525F0 (sub_1403525F0.c)
 * Callees:
 *     sub_14020D8D0 @ 0x14020D8D0 (sub_14020D8D0.c)
 *     sub_14027D6E0 @ 0x14027D6E0 (sub_14027D6E0.c)
 *     sub_140317A10 @ 0x140317A10 (sub_140317A10.c)
 *     sub_14031D9B0 @ 0x14031D9B0 (sub_14031D9B0.c)
 *     sub_140352E50 @ 0x140352E50 (sub_140352E50.c)
 *     KeBugCheckEx @ 0x14041F3D0 (KeBugCheckEx.c)
 */

__int64 __fastcall sub_1403528A4(__int64 a1, unsigned __int64 a2)
{
  __int64 v4; // r15
  unsigned __int64 v5; // rbp
  int v6; // eax
  __int64 v7; // rbx
  unsigned __int64 v8; // r8
  unsigned __int64 v9; // rcx
  unsigned __int64 v10; // rdi
  unsigned __int64 v12; // rax
  int v13; // esi
  char v14; // al
  _OWORD v15[2]; // [rsp+30h] [rbp-48h] BYREF
  unsigned __int64 v16; // [rsp+80h] [rbp+8h] BYREF

  v16 = 0LL;
  v4 = *(_QWORD *)(a1 + 48);
  v5 = (__int64)(a2 << 25) >> 16;
  v6 = sub_14031D9B0(v4, v5, 0);
  v7 = v6;
  if ( v6
    && (memset(v15, 0, sizeof(v15)),
        sub_140352E50(v5, v15),
        v8 = *((_QWORD *)v15 + v7),
        v9 = *((_QWORD *)v15 + (int)v7 - 1),
        v16 = v9,
        v8) )
  {
    v10 = v8;
    if ( (_DWORD)v7 == 1 )
      goto LABEL_4;
    v13 = v7;
    if ( (_DWORD)v7 != 2 )
    {
LABEL_12:
      LODWORD(v7) = 0;
LABEL_13:
      if ( v10 )
      {
        sub_14020D8D0(v4, v10);
        v10 = 0LL;
      }
      goto LABEL_5;
    }
  }
  else
  {
    v12 = sub_14027D6E0(v4, a2, &v16);
    v9 = v16;
    v10 = v12;
    if ( v16 == a2 )
    {
      LODWORD(v7) = 1;
LABEL_4:
      if ( (*(_BYTE *)(a1 + 78) & 4) != 0 )
        KeBugCheckEx(0x1Au, 0x5301uLL, v5, 0LL, 0LL);
      goto LABEL_5;
    }
    if ( v16 != ((a2 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL )
      goto LABEL_12;
    v13 = 2;
  }
  v14 = sub_140317A10(v9);
  if ( (v14 & 1) != 0 )
  {
    if ( v14 < 0 )
      KeBugCheckEx(0x1Au, 0x5302uLL, v5, 0LL, 0LL);
    if ( (*(_BYTE *)(a1 + 78) & 4) != 0 )
      KeBugCheckEx(0x1Au, 0x5303uLL, v5, 0LL, 0LL);
  }
  LODWORD(v7) = (*(_BYTE *)(a1 + 78) & 4) != 0 ? v13 : 0;
  if ( !(_DWORD)v7 )
    goto LABEL_13;
LABEL_5:
  *(_QWORD *)(a1 + 64) = v10;
  return (unsigned int)v7;
}
