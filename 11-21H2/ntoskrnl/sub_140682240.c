/*
 * XREFs of sub_140682240 @ 0x140682240
 * Callers:
 *     sub_1406821D4 @ 0x1406821D4 (sub_1406821D4.c)
 * Callees:
 *     sub_14025863C @ 0x14025863C (sub_14025863C.c)
 *     sub_1409B0030 @ 0x1409B0030 (sub_1409B0030.c)
 */

__int64 __fastcall sub_140682240(__int64 a1, __int64 a2, int a3)
{
  int v6; // edx
  int v7; // eax
  __int64 result; // rax

  if ( (*(_DWORD *)(a2 + 1124) & 0x4000000) == 0
    || !*(_QWORD *)(a1 + 528)
    || (result = sub_1409B0030(), (int)result >= 0) )
  {
    if ( *(_DWORD *)(a1 + 1416) == -1 )
      return 3221225540LL;
    v6 = *(_DWORD *)(a1 + 256);
    if ( (v6 & 8) != 0 && (unsigned int)(a3 + *(_DWORD *)(a1 + 216) - *(_DWORD *)(a1 + 1420)) > *(_DWORD *)(a1 + 260) )
    {
      _InterlockedAnd((volatile signed __int32 *)(a2 + 1120), 0xFFFFFFDF);
      if ( *(_QWORD *)(a1 + 552) && (*(_DWORD *)(a1 + 1068) & 8) != 0 )
        sub_14025863C(a1, 3LL, 0, 1);
      return 3221225540LL;
    }
    if ( (v6 & 4) != 0 && *(_DWORD *)(a1 + 4) )
      return 3221225540LL;
    v7 = *(_DWORD *)(a1 + 1512);
    if ( (v7 & 0x20000000) != 0 )
      return 3221225485LL;
    else
      return (v7 & 0x80u) != 0 ? 0xC000010A : 0;
  }
  return result;
}
