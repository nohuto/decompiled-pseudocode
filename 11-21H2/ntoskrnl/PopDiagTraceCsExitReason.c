/*
 * XREFs of PopDiagTraceCsExitReason @ 0x140805320
 * Callers:
  ...
  if ( Pool2 )
  {
    ...
    *(_QWORD *)&Pool2[46].Size = 4LL;
    if ( !v11 )
    {
      ...
      {
        if ( PopPlatformAoAc )
          EtwWrite(v12, &POP_ETW_EVENT_CS_EXIT_REASON, 0LL, 0x2Fu, UserData);
      }
    }
    if ( PopDiagSleepStudyHandleRegistered )
    {
      ...
      {
        if ( PopPlatformAoAc )
          EtwWrite(v13, &SLEEPSTUDY_EVT_SCENARIO_EXIT_REASON, 0LL, 0x2Fu, UserData);
      }
    }
  ...
  if ( (unsigned int)dword_140C03A00 > 5 )
  {
    if ( tlgKeywordOn((__int64)&dword_140C03A00, 0x400000000000LL) )
    {
      ...
      tlgWriteTransfer_EtwWriteTransfer(
        ...
        (unsigned __int8 *)byte_14002FD6D,
        0LL,
        0LL,
        0x21u,
        &v78);
