/*
 * XREFs of sub_140B0C87C @ 0x140B0C87C
 * Callers:
 *     sub_140B0B9A8 @ 0x140B0B9A8 (sub_140B0B9A8.c)
 * Callees:
 *     sub_1402DCBFC @ 0x1402DCBFC (sub_1402DCBFC.c)
 *     sub_1403C4824 @ 0x1403C4824 (sub_1403C4824.c)
 *     __security_check_cookie @ 0x1403DF760 (__security_check_cookie.c)
 *     sub_1403E08C8 @ 0x1403E08C8 (sub_1403E08C8.c)
 *     ExFreePoolWithTag @ 0x140A6E010 (ExFreePoolWithTag.c)
 *     ExAllocatePool2 @ 0x140A6E430 (ExAllocatePool2.c)
 *     sub_140B0C394 @ 0x140B0C394 (sub_140B0C394.c)
 *     sub_140B0C830 @ 0x140B0C830 (sub_140B0C830.c)
 *     sub_140B0CCAC @ 0x140B0CCAC (sub_140B0CCAC.c)
 *     sub_140B0CD58 @ 0x140B0CD58 (sub_140B0CD58.c)
 *     sub_140B0D2E8 @ 0x140B0D2E8 (sub_140B0D2E8.c)
 *     sub_140B0D408 @ 0x140B0D408 (sub_140B0D408.c)
 *     sub_140B0D634 @ 0x140B0D634 (sub_140B0D634.c)
 */

__int64 __fastcall sub_140B0C87C(__int64 a1)
{
  unsigned int v2; // ebp
  int v3; // edi
  unsigned int v4; // r13d
  __int64 v5; // r12
  __int64 Pool2; // rax
  __int64 v7; // rbx
  __int64 v8; // r9
  _QWORD *v9; // rax
  void *v10; // rcx
  unsigned __int8 *v11; // rax
  unsigned __int8 *v12; // rax
  unsigned int v13; // eax
  unsigned int v14; // esi
  bool v15; // zf
  unsigned int v16; // eax
  __int64 v18; // rax
  unsigned int v19; // edi
  __int64 v20; // rax
  _QWORD *v21; // rax
  _QWORD *v22; // r14
  _QWORD *v23; // rax
  __int64 i; // rdi
  void *v25; // rcx
  unsigned int v26; // eax
  __int64 v27; // rdi
  __int64 v28; // rsi
  __int64 v29; // rdx
  _QWORD *v30; // rax
  __int64 v31; // rdx
  _QWORD *v32; // rax
  __int128 v33; // [rsp+20h] [rbp-48h] BYREF

  v33 = 0LL;
  v2 = 0;
  v3 = 0;
  v4 = sub_140B0CD58(a1, "RuleDef");
  if ( v4 )
  {
    do
    {
      v5 = sub_140B0D408(a1, "RuleDef", v2);
      if ( !v5 )
        return 0;
      Pool2 = ExAllocatePool2(256LL, 0x68uLL, 0x74694D45u);
      v7 = Pool2;
      if ( !Pool2 )
        return (unsigned int)-1073741670;
      *(_QWORD *)(Pool2 + 56) = 0LL;
      *(_QWORD *)(Pool2 + 64) = 0LL;
      v8 = Pool2;
      *(_QWORD *)(Pool2 + 72) = 0LL;
      *(_QWORD *)(Pool2 + 32) = 0LL;
      *(_DWORD *)(Pool2 + 16) = 1;
      *(_BYTE *)(Pool2 + 20) = 0;
      v9 = (_QWORD *)(Pool2 + 80);
      v9[1] = v9;
      *v9 = v9;
      *(_DWORD *)(v7 + 48) = 0;
      *(_DWORD *)(v7 + 40) = 0;
      v3 = sub_140B0CCAC(a1, "RuleNameGuidDef", v5, v8);
      v10 = (void *)v7;
      if ( v3 < 0 || sub_1402DCBFC((_QWORD *)v7) )
      {
        ExFreePoolWithTag(v10, 0x74694D45u);
      }
      else
      {
        v11 = (unsigned __int8 *)sub_140B0D2E8(a1, "RuleDef", v2, 0LL);
        if ( !v11 )
          goto LABEL_27;
        *(_DWORD *)(v7 + 40) = sub_1403E08C8(v11, 0LL, 0xAu);
        v12 = (unsigned __int8 *)sub_140B0D2E8(a1, "RuleDef", v2, 1LL);
        if ( !v12 )
          goto LABEL_27;
        *(_DWORD *)(v7 + 44) = sub_1403E08C8(v12, 0LL, 0xAu);
        v13 = sub_140B0C830(a1, (__int64)"RuleDef", v2);
        v14 = v13;
        if ( v13 < 2 )
          goto LABEL_27;
        v15 = v13 == 2;
        v16 = v13 - 2;
        *(_DWORD *)(v7 + 48) = v16;
        if ( !v15 )
        {
          v18 = ExAllocatePool2(256LL, 8LL * v16, 0x74694D45u);
          *(_QWORD *)(v7 + 56) = v18;
          if ( !v18 )
          {
            ExFreePoolWithTag((PVOID)v7, 0x74694D45u);
            return (unsigned int)-1073741670;
          }
          v19 = 2;
          if ( v14 > 2 )
          {
            while ( 1 )
            {
              v20 = sub_140B0D2E8(a1, "RuleDef", v2, v19);
              if ( (int)sub_140B0CCAC(a1, "EntryTypeGuidDef", v20, &v33) < 0 )
                break;
              v21 = sub_1403C4824(&v33);
              if ( !v21 )
                break;
              v22 = v21 + 9;
              *(_QWORD *)(*(_QWORD *)(v7 + 56) + 8LL * (v19 - 2)) = v21;
              if ( !(unsigned __int8)sub_140B0D634(v21 + 9, v7) )
              {
                v23 = (_QWORD *)ExAllocatePool2(256LL, 0x18uLL, 0x74694D45u);
                if ( !v23 )
                  break;
                *v23 = v7;
                v23[1] = *v22;
                *v22 = v23 + 1;
              }
              if ( ++v19 >= v14 )
                goto LABEL_10;
            }
            v26 = v19 - 2;
            if ( v19 != 2 )
            {
              v27 = 0LL;
              v28 = v26;
              do
              {
                v29 = *(_QWORD *)(v27 + *(_QWORD *)(v7 + 56));
                v30 = *(_QWORD **)(v29 + 72);
                if ( *(v30 - 1) == v7 )
                {
                  if ( v30 )
                    *(_QWORD *)(v29 + 72) = *v30;
                  ExFreePoolWithTag(v30 - 1, 0x74694D45u);
                }
                v27 += 8LL;
                --v28;
              }
              while ( v28 );
            }
            goto LABEL_25;
          }
        }
LABEL_10:
        v3 = sub_140B0C394(a1, v5, v7);
        if ( v3 < 0 )
        {
          for ( i = 0LL; (unsigned int)i < *(_DWORD *)(v7 + 48); i = (unsigned int)(i + 1) )
          {
            v31 = *(_QWORD *)(*(_QWORD *)(v7 + 56) + 8 * i);
            v32 = *(_QWORD **)(v31 + 72);
            if ( *(v32 - 1) == v7 )
            {
              if ( v32 )
                *(_QWORD *)(v31 + 72) = *v32;
              ExFreePoolWithTag(v32 - 1, 0x74694D45u);
            }
          }
LABEL_25:
          v25 = *(void **)(v7 + 56);
          if ( v25 )
            ExFreePoolWithTag(v25, 0x74694D45u);
LABEL_27:
          ExFreePoolWithTag((PVOID)v7, 0x74694D45u);
          v3 = 0;
          goto LABEL_12;
        }
        ++dword_140C54CEC;
        *(_QWORD *)(v7 + 24) = qword_140C48C28;
        qword_140C48C28 = v7 + 24;
      }
LABEL_12:
      ++v2;
    }
    while ( v2 < v4 );
  }
  return (unsigned int)v3;
}
