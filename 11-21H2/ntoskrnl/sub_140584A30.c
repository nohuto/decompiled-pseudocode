/*
 * XREFs of sub_140584A30 @ 0x140584A30
 * Callers:
 *     <none>
 * Callees:
 *     sub_1402341A4 @ 0x1402341A4 (sub_1402341A4.c)
 *     sub_1402B7320 @ 0x1402B7320 (sub_1402B7320.c)
 *     sub_1402CFEB0 @ 0x1402CFEB0 (sub_1402CFEB0.c)
 *     sub_1402E5B0C @ 0x1402E5B0C (sub_1402E5B0C.c)
 *     sub_140317A10 @ 0x140317A10 (sub_140317A10.c)
 *     KeBugCheckEx @ 0x14041F3D0 (KeBugCheckEx.c)
 */

__int64 __fastcall sub_140584A30(_QWORD *a1, unsigned __int64 a2, int a3)
{
  __int64 v6; // r14
  __int64 v7; // rax
  char v8; // di
  __int64 v9; // rsi
  unsigned __int64 v10; // r9
  unsigned __int64 v11; // r10
  unsigned __int64 v12; // r8
  __int64 v13; // rdx
  unsigned __int64 v14; // rcx
  unsigned __int64 v15; // rdx
  unsigned __int64 v16; // rsi
  unsigned __int64 v18; // r9
  int v19; // edi
  __int64 v20; // rbx
  __int64 v21; // r15
  __int64 v22; // [rsp+80h] [rbp+8h] BYREF
  unsigned __int64 v23; // [rsp+88h] [rbp+10h]
  ULONG_PTR BugCheckParameter4; // [rsp+98h] [rbp+20h] BYREF

  v23 = a2;
  v6 = a1[21];
  v7 = sub_140317A10(a2);
  v8 = v7;
  v22 = v7;
  v9 = 1LL;
  if ( (v7 & 1) == 0 )
  {
    if ( (v7 & 0x400) != 0 )
    {
      sub_1402341A4((__int64)a1);
      v22 = (__int64)(a2 << 25) >> 16;
      return 2LL;
    }
    v18 = v7;
    if ( qword_140C50780 )
    {
      if ( (v7 & 0x10) != 0 )
        v18 = v7 & 0xFFFFFFFFFFFFFFEFuLL;
      else
        v18 = v7 & ~qword_140C50780;
    }
    v10 = (v18 >> 12) & 0xFFFFFFFFFFLL;
    goto LABEL_18;
  }
  v10 = ((unsigned __int64)sub_140317A10((unsigned __int64)&v22) >> 12) & 0xFFFFFFFFFFLL;
  if ( a3 < 1 )
  {
LABEL_18:
    v19 = *(_DWORD *)v6 != 0 ? 0x80 : 0;
    if ( v9 )
    {
      v20 = 48 * v10 - 0x220000000000LL;
      v21 = v9;
      do
      {
        LODWORD(v22) = 1;
        BugCheckParameter4 = 0x3FFFFFFFFFLL;
        sub_1402B7320(v20, v19 + 256, (__int64 *)&BugCheckParameter4, (int *)&v22);
        if ( BugCheckParameter4 != 0x3FFFFFFFFFLL )
          sub_1402E5B0C(48 * BugCheckParameter4 - 0x220000000000LL, v22);
        v20 += 48LL;
        --v21;
      }
      while ( v21 );
    }
    *(_QWORD *)(v6 + 8) += v9;
    return 0LL;
  }
  if ( v8 < 0 )
  {
    v11 = sub_1402CFEB0(a2);
    v12 = v11;
    if ( a1[4] > v11 )
      v12 = a1[4];
    v13 = 1LL;
    v14 = v12 >> 12;
    do
    {
      v10 += v13 * (v14 & 0x1FF);
      v14 >>= 9;
      v13 <<= 9;
      --a3;
    }
    while ( a3 );
    v15 = v11 + (v13 << 12);
    v16 = a1[5] + 1LL - v12;
    if ( a1[5] + 1LL >= v15 )
      v16 = v15 - v12;
    v9 = v16 >> 12;
    goto LABEL_18;
  }
  return 0LL;
}
