/*
 * XREFs of sub_14064AFB8 @ 0x14064AFB8
 * Callers:
 *     sub_140219130 @ 0x140219130 (sub_140219130.c)
 * Callees:
 *     sub_140218EAC @ 0x140218EAC (sub_140218EAC.c)
 *     sub_1402FD65C @ 0x1402FD65C (sub_1402FD65C.c)
 *     __security_check_cookie @ 0x1403DF760 (__security_check_cookie.c)
 *     memmove @ 0x140435B40 (memmove.c)
 */

__int64 __fastcall sub_14064AFB8(__int64 a1, __int64 a2, char a3, char a4, char a5, _BYTE *a6)
{
  char v7; // di
  int v11; // eax
  unsigned int v12; // ebx
  size_t v13; // r8
  __int64 v14; // rdx
  bool v15; // al
  __int128 v17; // [rsp+30h] [rbp-B8h] BYREF
  __int128 v18; // [rsp+40h] [rbp-A8h]
  void *Src; // [rsp+50h] [rbp-98h]
  _BYTE v20[80]; // [rsp+60h] [rbp-88h] BYREF

  LODWORD(Src) = 0;
  v7 = a5 != 0;
  WORD2(Src) = 0;
  *a6 = 0;
  v17 = 0LL;
  v18 = 0LL;
  while ( 1 )
  {
    v11 = sub_140218EAC(a1, (__int64)&v17);
    v12 = v11;
    if ( v11 == -2147483622 )
      break;
    if ( v11 < 0 )
      return v12;
    v13 = DWORD2(v18);
    if ( DWORD2(v18) >= 0x44 )
      v13 = 68LL;
    memmove(v20, Src, v13);
    v14 = *(_QWORD *)(a2 + 1096);
    if ( v14 )
      v15 = sub_1402FD65C((PSID_AND_ATTRIBUTES_HASH)(v14 + 32 + (a4 != 0 ? 0x110 : 0)), 0LL, v20, a3, a4, 0);
    else
      v15 = 0;
    if ( a5 )
    {
      v7 &= v15;
      if ( !v7 )
        goto LABEL_15;
    }
    else
    {
      v7 |= v15;
      if ( v7 )
        goto LABEL_15;
    }
  }
  v12 = 0;
LABEL_15:
  *a6 = v7;
  return v12;
}
