/*
 * XREFs of sub_140A87EFC @ 0x140A87EFC
 * Callers:
 *     sub_140A853A0 @ 0x140A853A0 (sub_140A853A0.c)
 *     sub_140A86C30 @ 0x140A86C30 (sub_140A86C30.c)
 * Callees:
 *     MmMapLockedPages @ 0x140585870 (MmMapLockedPages.c)
 *     sub_1405FDF9C @ 0x1405FDF9C (sub_1405FDF9C.c)
 *     sub_1405FFA20 @ 0x1405FFA20 (sub_1405FFA20.c)
 *     sub_140A87C8C @ 0x140A87C8C (sub_140A87C8C.c)
 *     sub_140A87EC4 @ 0x140A87EC4 (sub_140A87EC4.c)
 *     sub_140A88268 @ 0x140A88268 (sub_140A88268.c)
 *     sub_140A88624 @ 0x140A88624 (sub_140A88624.c)
 *     sub_140A88948 @ 0x140A88948 (sub_140A88948.c)
 */

__int64 __fastcall sub_140A87EFC(_QWORD *a1, __int64 a2, int a3, unsigned int a4, char a5)
{
  ULONG_PTR v6; // rbx
  __int64 v9; // rax
  ULONG_PTR v10; // r14
  __int64 v12; // rcx
  ULONG_PTR v13; // rdi
  char *v14; // r15
  __int64 v15; // rax
  unsigned int v16; // ebp
  __int64 v17; // rcx
  ULONG_PTR v18[2]; // [rsp+30h] [rbp-38h] BYREF

  v6 = a4;
  LODWORD(v18[0]) = 0;
  v9 = sub_140A88624(a2, a3);
  v10 = v9;
  if ( !v9 )
    return 0LL;
  if ( !sub_140A87EC4((__int64)a1, v9, v18) )
  {
    sub_140A88948(byte_140C0D94C, "Cannot flush buffers that aren't mapped (Addr %p).", (const void *)0x16);
    sub_1405FFA20(0xE6u, 0x16uLL, v10, (ULONG_PTR)a1, 0LL, byte_140C0D94C);
    return 0LL;
  }
  v12 = a1[7];
  v13 = (unsigned int)(LODWORD(v18[0]) << 12) + (v10 & 0xFFF);
  v14 = (char *)(v13 + a1[8]);
  v18[0] = v13;
  if ( (*(_BYTE *)(v12 + 10) & 5) != 0 )
  {
    v15 = *(_QWORD *)(v12 + 24);
  }
  else
  {
    LODWORD(v15) = (unsigned int)MmMapLockedPages((PMDL)v12, 0);
    v12 = a1[7];
  }
  v16 = v15 + *(_DWORD *)(v12 + 40) - (_DWORD)v14;
  if ( (unsigned int)v6 > v16 )
  {
    sub_140A88948(
      byte_140C0D950,
      "FLUSH: Can only flush %x bytes to end of map register file (%x attempted)",
      0x10000000,
      1);
    sub_1405FFA20(0xE6u, 0LL, 1uLL, v16, v6, byte_140C0D950);
    v13 = v18[0];
    LODWORD(v6) = v16;
  }
  if ( !a5 )
  {
    v17 = a1[9];
    if ( v17 )
      sub_140A87C8C(a2, a3, v14, (char *)(v13 + v17), (unsigned int)v6);
  }
  if ( !(unsigned int)sub_140A88268(a1, v10, (unsigned int)v6) )
    sub_1405FDF9C("Flushing too many map registers\n");
  return 1LL;
}
