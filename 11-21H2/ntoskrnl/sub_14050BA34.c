/*
 * XREFs of sub_14050BA34 @ 0x14050BA34
 * Callers:
 *     sub_140521670 @ 0x140521670 (sub_140521670.c)
 *     sub_14052C660 @ 0x14052C660 (sub_14052C660.c)
 * Callees:
 *     sub_1402FEDA0 @ 0x1402FEDA0 (sub_1402FEDA0.c)
 *     sub_14038A10C @ 0x14038A10C (sub_14038A10C.c)
 */

char __fastcall sub_14050BA34(int *a1, __int64 a2, _QWORD *a3)
{
  int v3; // ecx
  __int64 *v4; // r10
  __int64 v5; // r11
  __int64 v6; // rbx
  unsigned __int16 v7; // r9
  __int64 v8; // rdx
  _QWORD *v9; // rcx
  char result; // al
  __int64 v11; // [rsp+38h] [rbp+10h] BYREF

  v11 = a2;
  *a3 = 0LL;
  v3 = *a1;
  v11 = 0LL;
  if ( (int)sub_14038A10C(v3, a2, &v11) < 0 )
    return 0;
  v6 = v11;
  v7 = 0;
  *v4 = v11;
  if ( v6 != -32 )
    v7 = *(_WORD *)(v6 + 34);
  if ( (unsigned int)sub_1402FEDA0((_WORD *)(v6 + 32), (char *)(v5 + 8), (_BYTE *)(v6 + 32), v7) )
    return 0;
  v8 = *(_QWORD *)(v6 + 8);
  if ( *(_QWORD *)(v8 + 8) != v6 + 8 || (v9 = *(_QWORD **)(v6 + 16), *v9 != v6 + 8) )
    __fastfail(3u);
  *v9 = v8;
  result = 1;
  *(_QWORD *)(v8 + 8) = v9;
  return result;
}
