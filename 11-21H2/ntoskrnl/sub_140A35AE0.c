/*
 * XREFs of sub_140A35AE0 @ 0x140A35AE0
 * Callers:
 *     sub_140A3579C @ 0x140A3579C (sub_140A3579C.c)
 * Callees:
 *     memmove @ 0x140435B40 (memmove.c)
 *     sub_14082FFE4 @ 0x14082FFE4 (sub_14082FFE4.c)
 *     ExFreePoolWithTag @ 0x140A6E010 (ExFreePoolWithTag.c)
 */

_QWORD *__fastcall sub_140A35AE0(const void **P, __int16 a2, __int16 a3, char a4)
{
  _QWORD *v4; // rdi
  __int16 v7; // ax
  __int16 v8; // dx
  size_t v9; // rsi
  size_t v10; // rbp
  _QWORD *v11; // rax

  v4 = 0LL;
  v7 = 4;
  if ( a2 >= 1 )
    v7 = a2;
  v8 = 40;
  if ( a3 >= 1 )
    v8 = a3;
  if ( P )
  {
    if ( v7 >= (int)*((unsigned __int16 *)P + 3) && v8 >= (int)*((unsigned __int16 *)P + 5) )
    {
      v9 = 2LL * *((unsigned __int16 *)P + 2);
      v10 = 2LL * *((unsigned __int16 *)P + 4);
      v11 = sub_14082FFE4(v7, v8);
      v4 = v11;
      if ( v11 )
      {
        memmove((void *)v11[2], P[2], v9);
        memmove((void *)v4[3], P[3], v10);
        *((_WORD *)v4 + 3) = *((_WORD *)P + 3);
        *((_WORD *)v4 + 5) = *((_WORD *)P + 5);
        if ( !a4 )
          ExFreePoolWithTag(P, 0);
      }
    }
  }
  return v4;
}
