/*
 * XREFs of sub_140A9FA80 @ 0x140A9FA80
 * Callers:
 *     <none>
 * Callees:
 *     sub_14042A5E0 @ 0x14042A5E0 (sub_14042A5E0.c)
 *     sub_140A8B064 @ 0x140A8B064 (sub_140A8B064.c)
 *     sub_140A8C924 @ 0x140A8C924 (sub_140A8C924.c)
 *     sub_140A96B0C @ 0x140A96B0C (sub_140A96B0C.c)
 *     sub_140AA0838 @ 0x140AA0838 (sub_140AA0838.c)
 *     sub_140AA09D0 @ 0x140AA09D0 (sub_140AA09D0.c)
 */

struct _MDL *__fastcall sub_140A9FA80(__int64 a1, ULONG_PTR a2, __int64 a3, ULONG_PTR a4)
{
  __int64 v6; // rcx
  struct _MDL *v9; // rax
  struct _MDL *v10; // rbx
  __int64 retaddr; // [rsp+58h] [rbp+0h]

  v6 = (unsigned int)dword_140C29FC0 >> 17;
  LOBYTE(v6) = (dword_140C29FC0 & 0x20000) != 0;
  sub_140AA09D0(v6);
  if ( (unsigned int)sub_140A96B0C(0) )
    return 0LL;
  if ( (dword_140C29FC0 & 0x4000000) != 0 && a2 < 0x100000000LL )
    sub_140A8C924(0xC4u, 0x141uLL, a2, a4, 0LL);
  v9 = (struct _MDL *)sub_14042A5E0(a1, a2);
  v10 = v9;
  if ( v9 )
  {
    sub_140AA0838(v9);
    if ( (dword_140C29FC0 & 0x1000) != 0 )
      sub_140A8B064(retaddr, 232LL, 0xF0u, v10->ByteCount);
  }
  return v10;
}
