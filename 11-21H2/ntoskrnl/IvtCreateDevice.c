/*
 * XREFs of IvtCreateDevice @ 0x140537770
 * Callers:
 *     <none>
 * Callees:
 *     ExtEnvAllocateMemory @ 0x1403CE24C (ExtEnvAllocateMemory.c)
 *     ExtEnvFreeMemory @ 0x14052268C (ExtEnvFreeMemory.c)
 *     HalpIvtCheckIdPathMatch @ 0x140536FB4 (HalpIvtCheckIdPathMatch.c)
 *     IvtpGetNextInternalDeviceScope @ 0x1405378F4 (IvtpGetNextInternalDeviceScope.c)
 */

__int64 __fastcall IvtCreateDevice(__int64 a1, __int64 a2, char a3, unsigned __int64 *a4)
{
  __int64 v6; // rdi
  char *v7; // rdx
  char *NextInternalDeviceScope; // rax
  char *v9; // rbx
  int Memory; // ebx
  int v12; // eax
  __int64 v13; // rcx
  unsigned __int64 v14; // rdi
  unsigned __int64 v15; // rdx
  unsigned __int64 v16; // [rsp+40h] [rbp+8h] BYREF

  v16 = 0LL;
  v6 = a1;
  if ( *(_DWORD *)a2 != 1 || *(_DWORD *)(a1 + 252) != *(unsigned __int16 *)(a2 + 8) )
    return (unsigned int)-1073741275;
  if ( (a3 & 1) == 0 || !*(_BYTE *)(a1 + 268) )
  {
    v7 = 0LL;
    while ( 1 )
    {
      NextInternalDeviceScope = (char *)IvtpGetNextInternalDeviceScope(a1, v7);
      v9 = NextInternalDeviceScope;
      if ( !NextInternalDeviceScope )
        return (unsigned int)-1073741275;
      if ( HalpIvtCheckIdPathMatch(a2, NextInternalDeviceScope) )
        break;
      v7 = v9;
      a1 = v6;
    }
  }
  v12 = ExtEnvAllocateMemory(a1, 0x30u, &v16);
  v14 = v16;
  Memory = v12;
  if ( v12 < 0 || (Memory = ExtEnvAllocateMemory(v13, 4u, (_QWORD *)(v16 + 32)), Memory < 0) )
  {
    if ( v14 )
    {
      v15 = *(_QWORD *)(v14 + 32);
      if ( v15 )
        ExtEnvFreeMemory(v13, v15);
      ExtEnvFreeMemory(v13, v14);
    }
  }
  else
  {
    Memory = 0;
    **(_DWORD **)(v14 + 32) = *(unsigned __int16 *)(a2 + 12);
    *a4 = v14;
  }
  return (unsigned int)Memory;
}
