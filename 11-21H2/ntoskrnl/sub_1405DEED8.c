/*
 * XREFs of sub_1405DEED8 @ 0x1405DEED8
 * Callers:
 *     sub_1403B9260 @ 0x1403B9260 (sub_1403B9260.c)
 * Callees:
 *     sub_14020A9C4 @ 0x14020A9C4 (sub_14020A9C4.c)
 *     __security_check_cookie @ 0x1403DF760 (__security_check_cookie.c)
 */

NTSTATUS __fastcall sub_1405DEED8(__int64 a1)
{
  NTSTATUS result; // eax
  __int16 v3; // [rsp+30h] [rbp-68h] BYREF
  struct _EVENT_DATA_DESCRIPTOR v4; // [rsp+40h] [rbp-58h] BYREF
  __int16 *v5; // [rsp+60h] [rbp-38h]
  int v6; // [rsp+68h] [rbp-30h]
  int v7; // [rsp+6Ch] [rbp-2Ch]
  __int64 v8; // [rsp+70h] [rbp-28h]
  int v9; // [rsp+78h] [rbp-20h]
  int v10; // [rsp+7Ch] [rbp-1Ch]

  if ( (unsigned int)dword_140C03A00 > 5 )
  {
    v3 = 1;
    v5 = &v3;
    v7 = 0;
    v6 = 2;
    v8 = a1;
    v10 = 0;
    v9 = 16;
    result = sub_14020A9C4((__int64)&dword_140C03A00, (unsigned __int8 *)byte_140032141, 0LL, 0LL, 4u, &v4);
  }
  *(_QWORD *)(a1 + 432) = 0LL;
  return result;
}
