/*
 * XREFs of sub_1406CCD8C @ 0x1406CCD8C
 * Callers:
 *     sub_14075FC44 @ 0x14075FC44 (sub_14075FC44.c)
 * Callees:
 *     sub_14024B47C @ 0x14024B47C (sub_14024B47C.c)
 *     sub_14027E128 @ 0x14027E128 (sub_14027E128.c)
 *     sub_140287970 @ 0x140287970 (sub_140287970.c)
 *     RtlFindExportedRoutineByName @ 0x140757F00 (RtlFindExportedRoutineByName.c)
 */

__int64 __fastcall sub_1406CCD8C(__int64 a1)
{
  PVOID ExportedRoutineByName; // rdi
  unsigned int *v3; // r14
  unsigned __int8 *v4; // rbp
  PVOID v5; // rsi
  unsigned int *v6; // r15
  unsigned __int8 *v7; // r12
  int v8; // r13d
  int v10; // [rsp+60h] [rbp+8h]

  ExportedRoutineByName = RtlFindExportedRoutineByName(*(PVOID *)(a1 + 48), "W32pServiceTable");
  if ( !ExportedRoutineByName )
    return 3221225594LL;
  v3 = (unsigned int *)RtlFindExportedRoutineByName(*(PVOID *)(a1 + 48), "W32pServiceLimit");
  if ( !v3 )
    return 3221225594LL;
  v4 = (unsigned __int8 *)RtlFindExportedRoutineByName(*(PVOID *)(a1 + 48), "W32pArgumentTable");
  if ( !v4 )
    return 3221225594LL;
  v5 = RtlFindExportedRoutineByName(*(PVOID *)(a1 + 48), "W32pServiceTableFilter");
  if ( !v5 )
    return 3221225594LL;
  v6 = (unsigned int *)RtlFindExportedRoutineByName(*(PVOID *)(a1 + 48), "W32pServiceLimitFilter");
  if ( !v6 )
    return 3221225594LL;
  v7 = (unsigned __int8 *)RtlFindExportedRoutineByName(*(PVOID *)(a1 + 48), "W32pArgumentTableFilter");
  if ( !v7 )
    return 3221225594LL;
  v8 = 4 * *v3;
  sub_14027E128(a1, (unsigned __int64)ExportedRoutineByName, v8, 4LL);
  v10 = 4 * *v6;
  sub_14027E128(a1, (unsigned __int64)v5, v10, 4LL);
  sub_14024B47C((ULONG_PTR)v5, v7, *v6, 3LL, *(_QWORD *)(a1 + 48));
  sub_14024B47C((ULONG_PTR)ExportedRoutineByName, v4, *v3, 1LL, *(_QWORD *)(a1 + 48));
  if ( *(_QWORD *)(sub_140287970(*(_QWORD *)(a1 + 112)) + 144) )
  {
    sub_14027E128(a1, (unsigned __int64)ExportedRoutineByName, v8, 256LL);
    sub_14027E128(a1, (unsigned __int64)v5, v10, 256LL);
  }
  return 0LL;
}
