/*
 * XREFs of sub_1406A6D60 @ 0x1406A6D60
 * Callers:
 *     sub_1406A6AF8 @ 0x1406A6AF8 (sub_1406A6AF8.c)
 *     sub_1406A6C04 @ 0x1406A6C04 (sub_1406A6C04.c)
 * Callees:
 *     ExAcquirePushLockExclusiveEx @ 0x1402AC910 (ExAcquirePushLockExclusiveEx.c)
 *     sub_1402AFC00 @ 0x1402AFC00 (sub_1402AFC00.c)
 *     ExfTryToWakePushLock @ 0x140359F40 (ExfTryToWakePushLock.c)
 */

__int64 __fastcall sub_1406A6D60(volatile signed __int32 *a1, _QWORD *a2, ULONG_PTR a3, char a4)
{
  __m128i v5; // xmm2
  unsigned int v7; // r8d
  _QWORD *v8; // rdx
  unsigned __int64 v9; // r10
  __m128i v10; // xmm3
  unsigned int v11; // eax
  __m128i v12; // xmm1
  _QWORD *v13; // rsi
  __int64 result; // rax
  _QWORD *v15; // rdi
  __m128i v16; // [rsp+20h] [rbp-18h]

  v5 = _mm_unpacklo_epi64((__m128i)(unsigned __int64)a2, (__m128i)(unsigned __int64)a2);
  if ( a4 )
  {
    v7 = 1;
    v8 = a2 + 3;
    v9 = _mm_srli_si128((__m128i)0LL, 8).m128i_u64[0];
    v10 = _mm_cvtsi32_si128(4u);
    do
    {
      v11 = v7 + 1;
      *(v8 - 1) = 0LL;
      v7 += 2;
      v16.m128i_i64[0] = 0x100000000LL;
      v8[1] = v9;
      v12 = _mm_add_epi64(
              _mm_sll_epi64(
                _mm_unpacklo_epi32(_mm_add_epi32(_mm_shuffle_epi32(_mm_cvtsi32_si128(v11), 0), v16), (__m128i)0LL),
                v10),
              v5);
      *v8 = v12.m128i_i64[0];
      v8 += 4;
      *(v8 - 2) = _mm_srli_si128(v12, 8).m128i_u64[0];
    }
    while ( v7 < 0xFF );
    v13 = a2 + 510;
    a2[510] = 0LL;
    a2[511] = 0LL;
  }
  else
  {
    v13 = a2 + 510;
  }
  result = (unsigned int)_InterlockedExchangeAdd(a1, 0x400u);
  if ( a4 )
  {
    ExAcquirePushLockExclusiveEx(a3, 0LL);
    v15 = a2 + 2;
    if ( *(_QWORD *)(a3 + 8) )
      *(_QWORD *)(*(_QWORD *)(a3 + 16) + 8LL) = v15;
    else
      *(_QWORD *)(a3 + 8) = v15;
    *(_QWORD *)(a3 + 16) = v13;
    if ( (_InterlockedExchangeAdd64((volatile signed __int64 *)a3, 0xFFFFFFFFFFFFFFFFuLL) & 6) == 2 )
      ExfTryToWakePushLock(a3);
    return sub_1402AFC00(a3);
  }
  return result;
}
