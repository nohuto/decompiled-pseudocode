/*
 * XREFs of sub_14068E2EC @ 0x14068E2EC
 * Callers:
 *     sub_14068C238 @ 0x14068C238 (sub_14068C238.c)
 *     sub_14091A344 @ 0x14091A344 (sub_14091A344.c)
 * Callees:
 *     sub_14042A5E0 @ 0x14042A5E0 (sub_14042A5E0.c)
 *     memset @ 0x140435E00 (memset.c)
 *     sub_140AB41B0 @ 0x140AB41B0 (sub_140AB41B0.c)
 */

__int64 __fastcall sub_14068E2EC(__int64 a1, _QWORD *a2, _QWORD *a3)
{
  _QWORD *v4; // r15
  char *v6; // rax
  char *v7; // rbx
  __int64 v8; // rdx
  __int64 v9; // r8
  int v10; // ecx
  unsigned int v11; // eax
  unsigned int v12; // ecx
  unsigned int v13; // edi
  int v14; // eax
  int v16; // eax

  *a2 = 0LL;
  v4 = a2;
  LOBYTE(a2) = 1;
  v6 = (char *)sub_14042A5E0(*(unsigned int *)(a1 + 132), a2);
  v7 = v6;
  if ( v6 )
  {
    memset(v6, 0, 0x1000uLL);
    if ( (int)sub_14042A5E0(a1, 0LL) >= 0 )
    {
      if ( *(_DWORD *)v7 == 1718052210 && *((_DWORD *)v7 + 127) == (unsigned int)sub_140AB41B0(v7, v8, v9) )
      {
        v10 = *((_DWORD *)v7 + 10);
        if ( (unsigned int)(v10 - 1) <= 0x7FFFDFFF && (v10 & 0xFFF) == 0 )
        {
          if ( !*((_DWORD *)v7 + 7) )
          {
            v11 = *((_DWORD *)v7 + 5);
            if ( v11 <= 1 )
            {
              v12 = *((_DWORD *)v7 + 6);
              v13 = 3;
              if ( v12 >= 3 && (v12 <= 6 || v11 != 1) && *((_DWORD *)v7 + 8) == 1 )
              {
                *a3 = *(_QWORD *)(v7 + 12);
                v14 = *((_DWORD *)v7 + 2);
                *v4 = v7;
                if ( *((_DWORD *)v7 + 1) == v14 )
                  *((_DWORD *)v7 + 11) = 1;
                else
                  return 5;
                return v13;
              }
            }
          }
          goto LABEL_16;
        }
      }
      v16 = sub_14042A5E0(a1, 0LL);
      if ( v16 == -1073741807 )
      {
LABEL_16:
        v13 = 0;
LABEL_26:
        sub_14042A5E0(v7, *(unsigned int *)(a1 + 132));
        return v13;
      }
      if ( v16 >= 0 )
      {
        if ( *(_DWORD *)v7 == 1852400232 && !*((_DWORD *)v7 + 1) )
        {
          v13 = 4;
          *a3 = *(_QWORD *)(v7 + 20);
          goto LABEL_26;
        }
        goto LABEL_16;
      }
    }
    v13 = 7;
    goto LABEL_26;
  }
  return 2LL;
}
