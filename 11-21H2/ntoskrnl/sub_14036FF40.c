/*
 * XREFs of sub_14036FF40 @ 0x14036FF40
 * Callers:
 *     sub_14036F620 @ 0x14036F620 (sub_14036F620.c)
 *     sub_14036FD8C @ 0x14036FD8C (sub_14036FD8C.c)
 * Callees:
 *     InitializeSListHead @ 0x1402A05A0 (InitializeSListHead.c)
 *     memset @ 0x140435E00 (memset.c)
 */

void __fastcall sub_14036FF40(unsigned __int64 a1, __int64 a2, __int128 *a3, int a4, int a5, _DWORD *a6)
{
  __int128 v10; // xmm0
  __int128 v11; // xmm1
  __int64 i; // rcx
  __int64 v13; // rax

  memset((void *)a1, 0, 0xC0uLL);
  v10 = *a3;
  v11 = a3[1];
  *(_DWORD *)(a1 + 180) = a5;
  *(_OWORD *)(a1 + 136) = v10;
  *(_QWORD *)(a1 + 128) = a2 ^ a1;
  *(_QWORD *)&v10 = *((_QWORD *)a3 + 4);
  *(_OWORD *)(a1 + 152) = v11;
  *(_DWORD *)(a1 + 8) = a4;
  *(_QWORD *)(a1 + 168) = v10;
  *(_DWORD *)(a1 + 176) = *a6;
  for ( i = 0LL; i < 4; ++i )
    *(_QWORD *)(a1 + 8 * i + 136) ^= a1 ^ qword_140C5A5C0;
  v13 = *(_QWORD *)(a1 + 168);
  if ( v13 )
    *(_QWORD *)(a1 + 168) = a1 ^ qword_140C5A5C0 ^ v13;
  *(_QWORD *)a1 = 0LL;
  *(_QWORD *)(a1 + 16) = 0LL;
  *(_QWORD *)(a1 + 24) = 0LL;
  if ( (a5 & 0x8000000) != 0 )
    *(_BYTE *)(a1 + 24) = 1;
  *(_QWORD *)(a1 + 40) = 0LL;
  *(_QWORD *)(a1 + 32) = 0LL;
  *(_OWORD *)(a1 + 64) = 0LL;
  InitializeSListHead((PSLIST_HEADER)(a1 + 64));
}
