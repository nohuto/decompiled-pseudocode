/*
 * XREFs of sub_140840DE0 @ 0x140840DE0
 * Callers:
 *     sub_140840CD8 @ 0x140840CD8 (sub_140840CD8.c)
 * Callees:
 *     sub_140840E70 @ 0x140840E70 (sub_140840E70.c)
 *     sub_140840F30 @ 0x140840F30 (sub_140840F30.c)
 *     sub_140840FC4 @ 0x140840FC4 (sub_140840FC4.c)
 */

__int64 __fastcall sub_140840DE0(__int64 a1, __int64 a2, __int64 a3)
{
  int v5; // r9d
  __int64 v6; // rdi
  __int64 v7; // rdx
  __int64 i; // rdi

  if ( a1 && a2 )
  {
    sub_140840FC4(a1, a2, a3, 0LL);
    v6 = 0LL;
    if ( *(_DWORD *)(v7 + 48) )
    {
      while ( 1 )
      {
        v5 = sub_140840E70(a1, *(_QWORD *)(a2 + 8 * v6 + 56));
        if ( v5 < 0 )
          break;
        v6 = (unsigned int)(v6 + 1);
        if ( (unsigned int)v6 >= *(_DWORD *)(a2 + 48) )
          goto LABEL_6;
      }
    }
    else
    {
LABEL_6:
      for ( i = 0LL; (unsigned int)i < *(_DWORD *)(a2 + 52); i = (unsigned int)(i + 1) )
      {
        v5 = sub_140840F30(a1, *(_QWORD *)(a2 + 8 * i + 1048));
        if ( v5 < 0 )
          break;
      }
    }
  }
  else
  {
    return (unsigned int)-1073741811;
  }
  return (unsigned int)v5;
}
