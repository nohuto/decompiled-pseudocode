/*
 * XREFs of sub_1406172F0 @ 0x1406172F0
 * Callers:
 *     <none>
 * Callees:
 *     MmMapVideoDisplay @ 0x140215300 (MmMapVideoDisplay.c)
 *     sub_14042A5E0 @ 0x14042A5E0 (sub_14042A5E0.c)
 *     sub_140604844 @ 0x140604844 (sub_140604844.c)
 *     sub_140628C98 @ 0x140628C98 (sub_140628C98.c)
 */

PVOID __fastcall sub_1406172F0(PHYSICAL_ADDRESS PhysicalAddress, SIZE_T NumberOfBytes, MEMORY_CACHING_TYPE CacheType)
{
  __int64 v6; // rdx
  __int64 v7; // rcx
  __int64 *v8; // rsi
  __int64 v9; // r8
  __int64 v10; // r9
  int v11; // eax
  __int64 v12; // rax
  __int64 *i; // rdi
  PVOID result; // rax
  __int64 v15; // rdx
  __int64 *v16; // rbx
  __int128 v17; // [rsp+20h] [rbp-30h] BYREF
  __int128 v18; // [rsp+30h] [rbp-20h]
  PVOID v19; // [rsp+40h] [rbp-10h]
  __int64 retaddr; // [rsp+68h] [rbp+18h]

  v19 = 0LL;
  v17 = 0LL;
  v18 = 0LL;
  v8 = sub_140604844(333);
  if ( !v8 )
    goto LABEL_18;
  if ( dword_140C1B2A0
    && ((qword_140D01450 & 0xFFA9F6E6) != 0 || (v7 = HIDWORD(qword_140D01450), (qword_140D01450 & 0x200000000LL) != 0))
    || (qword_140D01450 & 0x800000000LL) == 0 )
  {
    if ( (*((_DWORD *)v8 + 3) & 0x20) != 0 )
      goto LABEL_13;
    goto LABEL_11;
  }
  v11 = *((_DWORD *)v8 + 3);
  if ( (v11 & 0x18) == 0 )
  {
    if ( (v11 & 4) == 0 )
      goto LABEL_13;
LABEL_11:
    v12 = sub_140628C98(v7, v6, v9, v10);
    goto LABEL_12;
  }
  v12 = retaddr;
LABEL_12:
  *(_QWORD *)&v17 = v12;
LABEL_13:
  *((PHYSICAL_ADDRESS *)&v18 + 1) = PhysicalAddress;
  *(_QWORD *)&v18 = NumberOfBytes;
  DWORD2(v17) = CacheType;
  for ( i = (__int64 *)v8[4]; i != v8 + 4; i = (__int64 *)*i )
  {
    if ( i != (__int64 *)16 )
      sub_14042A5E0(&v17, v6);
  }
LABEL_18:
  result = MmMapVideoDisplay(PhysicalAddress, NumberOfBytes, CacheType);
  v19 = result;
  if ( v8 )
  {
    v16 = (__int64 *)v8[6];
    if ( v16 != v8 + 6 )
    {
      do
      {
        if ( v16 != (__int64 *)16 )
          sub_14042A5E0(&v17, v15);
        v16 = (__int64 *)*v16;
      }
      while ( v16 != v8 + 6 );
      return v19;
    }
  }
  return result;
}
