/*
 * XREFs of sub_1406C57AC @ 0x1406C57AC
 * Callers:
 *     sub_1402FA820 @ 0x1402FA820 (sub_1402FA820.c)
 *     sub_1403841DC @ 0x1403841DC (sub_1403841DC.c)
 * Callees:
 *     ExAllocatePool2 @ 0x140A6E430 (ExAllocatePool2.c)
 */

void __fastcall sub_1406C57AC(int a1, char a2, char a3, char a4, LUID **a5)
{
  LUID *Pool2; // rax
  LUID *v9; // rdx
  __int64 LowPart; // rax

  if ( a5 )
  {
    if ( a1 )
    {
      Pool2 = (LUID *)ExAllocatePool2(256LL, (unsigned int)(12 * (a1 - 1) + 20), 1917871443LL);
      v9 = Pool2;
      if ( Pool2 )
      {
        Pool2->LowPart = 0;
        Pool2->HighPart = 0;
        LowPart = 0LL;
        if ( a2 )
        {
          v9[1] = stru_140D3CA98;
          v9[2].LowPart = 0x80000000;
          LowPart = ++v9->LowPart;
        }
        if ( a3 )
        {
          *(LUID *)((char *)&v9[1] + 12 * LowPart) = (LUID)qword_140D3CAC0;
          *(&v9[2].LowPart + 3 * v9->LowPart++) = 0x80000000;
          LowPart = v9->LowPart;
        }
        if ( a4 )
        {
          *(LUID *)((char *)&v9[1] + 12 * LowPart) = (LUID)qword_140D3CAA0;
          *(&v9[2].LowPart + 3 * v9->LowPart++) = 0x80000000;
        }
        *a5 = v9;
      }
    }
  }
}
