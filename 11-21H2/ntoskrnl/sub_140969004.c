/*
 * XREFs of sub_140969004 @ 0x140969004
 * Callers:
 *     sub_140968388 @ 0x140968388 (sub_140968388.c)
 * Callees:
 *     sub_1402828F0 @ 0x1402828F0 (sub_1402828F0.c)
 *     memmove @ 0x140435B40 (memmove.c)
 */

__int64 __fastcall sub_140969004(_QWORD *a1, unsigned int *a2, __int64 a3)
{
  unsigned __int64 v3; // rax
  unsigned __int64 v5; // r13
  unsigned __int64 v7; // r11
  __int64 v8; // rax
  unsigned int *v9; // rbx
  unsigned __int64 v10; // r15
  __int64 v11; // rcx
  __int64 v12; // rdx
  __int64 v13; // rcx
  __int64 v14; // rdi
  _QWORD *v15; // r10
  __int64 v16; // rsi
  unsigned __int64 v17; // rax
  unsigned __int64 v18; // r9
  SIZE_T v20; // rdx
  _QWORD *v21; // rax
  _QWORD *v22; // rdx
  char *v23; // r14
  _DWORD *v24; // rsi
  unsigned __int64 *v25; // rcx
  __int64 v26; // rbp
  __int64 v27; // r15
  __int64 v28; // rdi
  __int64 v29; // rcx
  __int64 v30; // rax
  __int64 v31; // r8
  int v32; // r9d
  __int64 v34; // [rsp+80h] [rbp+18h]
  unsigned __int64 v35; // [rsp+88h] [rbp+20h]

  v3 = *(_QWORD *)(a3 + 24);
  v5 = *(_QWORD *)(a3 + 16);
  *a1 = 0LL;
  v35 = v3;
  v7 = v3 + v5;
  v8 = *a2;
  if ( *(_QWORD *)a3 )
    v9 = &a2[4 * v8 + 4];
  else
    v9 = 0LL;
  if ( (int)v8 + 1 < (unsigned int)v8 )
    return 3221225626LL;
  v10 = *a2;
  v11 = 16LL * (unsigned int)(v8 - 1);
  v12 = v11 + 48;
  if ( v9 )
    v12 = v11 + 8 * ((unsigned int)(v8 + 1) + 6LL);
  v13 = 0LL;
  v34 = 1LL;
  v14 = 0LL;
  v15 = a2 + 6;
  v16 = 1LL;
  while ( 1 )
  {
    v17 = *(v15 - 1);
    if ( *v15 )
    {
      v18 = v17 + *v15;
      if ( v5 >= v17 )
      {
        if ( v5 < v18 )
          return 3221225496LL;
      }
      else if ( v7 > v17 )
      {
        return 3221225496LL;
      }
      if ( (v5 == v18 || v7 == v17)
        && (!v9 || v9[2 * v13] == *(_DWORD *)(a3 + 32) && v9[2 * v13 + 1] == *(_DWORD *)(a3 + 36)) )
      {
        if ( v16 == 1 )
        {
          v14 = v13;
          v16 = 0LL;
        }
        else
        {
          v16 = -1LL;
        }
        v34 = v16;
      }
    }
    if ( ++v13 >= v10 )
      break;
    v15 += 2;
  }
  v20 = 16 * v16 + v12;
  if ( v9 )
    v20 += 8 * (v10 + v16);
  v21 = sub_1402828F0(64, v20, 0x20206D4Du);
  if ( !v21 )
    return 3221225626LL;
  *v21 = &StartContext;
  v22 = a2 + 4;
  v21[1] = 1LL;
  v23 = (char *)(v21 + 2);
  *((_DWORD *)v21 + 4) = v16 + v10;
  v24 = &v21[2 * (unsigned int)(v16 + v10) + 4];
  v21[3] = v35 + *((_QWORD *)a2 + 1);
  v25 = v21 + 4;
  if ( v34 == -1 )
  {
    v26 = 4 * v14;
    memmove(v25, v22, 16 * v14 + 16);
    if ( v9 )
      memmove(v24, v9, 8 * v14 + 8);
    *(_QWORD *)&v23[v26 * 4 + 24] += v35 + *(_QWORD *)&a2[v26 + 10];
    if ( v14 + 2 != v10 )
    {
      v27 = v10 - v14;
      memmove(&v23[v26 * 4 + 32], &a2[v26 + 12], 16 * v27 - 32);
      if ( v9 )
        memmove(&v24[2 * v14 + 2], &v9[2 * v14 + 4], 8 * v27 - 16);
    }
  }
  else if ( v34 )
  {
    v31 = 0LL;
    v32 = 0;
    do
    {
      if ( !v32 && v35 + v5 <= *v22 )
      {
        *v25 = v5;
        v25[1] = v35;
        v25 += 2;
        if ( v9 )
        {
          v24[1] = *(_DWORD *)(a3 + 36);
          *v24 = *(_DWORD *)(a3 + 32);
          v24 += 2;
        }
        v32 = 1;
      }
      *(_OWORD *)v25 = *(_OWORD *)v22;
      v25 += 2;
      if ( v9 )
      {
        *(_QWORD *)v24 = *(_QWORD *)&v9[2 * v31];
        v24 += 2;
      }
      ++v31;
      v22 += 2;
    }
    while ( v31 != *a2 );
    if ( !v32 )
    {
      *v25 = v5;
      v25[1] = v35;
      if ( v9 )
      {
        v24[1] = *(_DWORD *)(a3 + 36);
        *v24 = *(_DWORD *)(a3 + 32);
      }
    }
  }
  else
  {
    memmove(v25, v22, 16 * v10);
    if ( v9 )
      memmove(v24, v9, 8 * v10);
    v28 = 2 * v14;
    v29 = *(_QWORD *)&a2[2 * v28 + 4];
    v30 = *(_QWORD *)&a2[2 * v28 + 6];
    *(_QWORD *)&v23[8 * v28 + 24] += v35;
    if ( v5 != v29 + v30 )
      *(_QWORD *)&v23[8 * v28 + 16] = v5;
  }
  *a1 = v23;
  return 0LL;
}
