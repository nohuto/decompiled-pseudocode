/*
 * XREFs of ?GetClsidsFromVar@@YAXAEBUtagPROPVARIANT@@AEAV?$vector@U_GUID@@V?$allocator@U_GUID@@@std@@@std@@AEAG@Z @ 0x140060F8C
 * Callers:
  ...
  for ( i = 0LL; (unsigned int)i < *(_DWORD *)(v5 + 8); i = (unsigned int)(i + 1) )
  {
    ...
    if ( CLSIDFromString(*(LPCOLESTR *)(*(_QWORD *)(v5 + 16) + 8 * i), &pclsid) >= 0 )
    {
      ...
      if ( v7 == *(GUID **)(v4 + 16) )
      {
        try
        {
          std::vector<_GUID>::_Emplace_reallocate<_GUID const &>((const void **)v4, v7, &pclsid);
        }
        catch ( ... )
        {
