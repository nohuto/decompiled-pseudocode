/*
 * XREFs of ?TraverseToTerminalForDataFlowInPin@CSidebandDevice@@AEAAJPEAU_KSFILTER_DESCRIPTOR@@PEBUKSTOPOLOGY_CONNECTION@@PEAK@Z @ 0x1C0010A34
 * Callers:
...
LABEL_40:
      ...
      v11->Blink = &P;
      if ( LODWORD(Flink[1].Blink) == -1 )
      {
        v6 = 0;
        *a4 = HIDWORD(Flink[1].Blink);
        while ( 1 )
        {
          ExFreePool(Flink);
          Flink = P.Flink;
LABEL_36:
      ...
      while ( 1 )
      {
        ...
        if ( *(ULONG *)((char *)&Connections->FromNode + (_QWORD)v15) == LODWORD(Flink[1].Blink) )
          break;
LABEL_23:
