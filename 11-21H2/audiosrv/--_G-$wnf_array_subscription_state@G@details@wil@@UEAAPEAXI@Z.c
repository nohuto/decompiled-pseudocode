/*
 * XREFs of ??_G?$wnf_array_subscription_state@G@details@wil@@UEAAPEAXI@Z @ 0x1800E3D10
 * Callers:
 *     <none>
 * Callees:
 *     ??3@YAXPEAX_K@Z @ 0x18005EFB8 (--3@YAXPEAX_K@Z.c)
 *     ??1?$wnf_subscription_state@Uempty_wnf_state@details@wil@@@details@wil@@UEAA@XZ @ 0x1800E3C00 (--1-$wnf_subscription_state@Uempty_wnf_state@details@wil@@@details@wil@@UEAA@XZ.c)
 */

_QWORD *__fastcall wil::details::wnf_array_subscription_state<unsigned short>::`scalar deleting destructor'(
        _QWORD *a1,
        char a2)
{
  wil::details::wnf_subscription_state<wil::details::empty_wnf_state>::~wnf_subscription_state<wil::details::empty_wnf_state>(a1);
  if ( (a2 & 1) != 0 )
    operator delete(a1);
  return a1;
}
