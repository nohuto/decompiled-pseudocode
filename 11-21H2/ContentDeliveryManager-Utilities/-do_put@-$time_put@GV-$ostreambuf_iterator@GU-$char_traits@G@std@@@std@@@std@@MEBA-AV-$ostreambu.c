/*
 * XREFs of ?do_put@?$time_put@GV?$ostreambuf_iterator@GU?$char_traits@G@std@@@std@@@std@@MEBA?AV?$ostreambuf_iterator@GU?$char_traits@G@std@@@2@V32@AEAVios_base@2@GPEBUtm@@DD@Z @ 0x18001B190
 * Callers:
 *     <none>
 * Callees:
 *     ??4?$ostreambuf_iterator@_WU?$char_traits@_W@std@@@std@@QEAAAEAV01@_W@Z @ 0x18000FC40 (--4-$ostreambuf_iterator@_WU-$char_traits@_W@std@@@std@@QEAAAEAV01@_W@Z.c)
 *     __security_check_cookie @ 0x180021DB0 (__security_check_cookie.c)
 *     ??3@YAXPEAX@Z_0 @ 0x180022171 (--3@YAXPEAX@Z_0.c)
 *     ?append@?$basic_string@_WU?$char_traits@_W@std@@V?$allocator@_W@2@@std@@QEAAAEAV12@_K_W@Z @ 0x1800CCA7C (-append@-$basic_string@_WU-$char_traits@_W@std@@V-$allocator@_W@2@@std@@QEAAAEAV12@_K_W@Z.c)
 */

// Hidden C++ exception states: #wind=1
__int128 *__fastcall std::time_put<unsigned short,std::ostreambuf_iterator<unsigned short>>::do_put(
        __int64 a1,
        __int128 *a2,
        __int128 *a3,
        __int64 a4,
        int a5,
        struct tm *Timeptr,
        char a7,
        char a8)
{
  __int64 i; // rbx
  wchar_t *v12; // rcx
  size_t v13; // r8
  wchar_t **v14; // rax
  wchar_t *v15; // rcx
  unsigned __int64 v16; // rdx
  unsigned __int16 *v17; // rsi
  wchar_t **v18; // rbx
  unsigned __int16 *v19; // rbx
  __int128 v21; // [rsp+40h] [rbp-49h] BYREF
  wchar_t *Buffer[2]; // [rsp+50h] [rbp-39h] BYREF
  size_t Max_size; // [rsp+60h] [rbp-29h]
  unsigned __int64 v24; // [rsp+68h] [rbp-21h]
  wchar_t Format[4]; // [rsp+70h] [rbp-19h] BYREF
  __int16 v26; // [rsp+78h] [rbp-11h]

  wcscpy(Format, L"!%x");
  v26 = 0;
  v24 = 7LL;
  Max_size = 0LL;
  LOWORD(Buffer[0]) = 0;
  if ( a8 )
  {
    Format[2] = a8;
    Format[3] = a7;
  }
  else
  {
    Format[2] = a7;
  }
  for ( i = 16LL; ; i *= 2LL )
  {
    std::wstring::append(Buffer, i, 0LL);
    v12 = (wchar_t *)Buffer;
    if ( v24 >= 8 )
      v12 = Buffer[0];
    v13 = _Wcsftime(v12, Max_size, Format, Timeptr, *(void **)(a1 + 16));
    if ( v13 )
      break;
  }
  v14 = Buffer;
  v15 = Buffer[0];
  v16 = v24;
  if ( v24 >= 8 )
    v14 = (wchar_t **)Buffer[0];
  v17 = (unsigned __int16 *)v14 + v13;
  v18 = Buffer;
  if ( v24 >= 8 )
    v18 = (wchar_t **)Buffer[0];
  v19 = (unsigned __int16 *)v18 + 1;
  v21 = *a3;
  if ( v19 != v17 )
  {
    do
      std::ostreambuf_iterator<wchar_t>::operator=((__int64)&v21, *v19++);
    while ( v19 != v17 );
    v16 = v24;
    v15 = Buffer[0];
  }
  *a2 = *a3;
  if ( v16 >= 8 )
    operator delete(v15);
  return a2;
}
