/*
 * XREFs of ?GetDesktopUniqueName@@YAXPEB_WPEA_WK@Z @ 0x18019F07C
 * Callers:
 ...
 *     ?StringCchCopyW@@YAJPEA_W_KPEB_W@Z @ 0x18003102C (-StringCchCopyW@@YAJPEA_W_KPEB_W@Z.c)
...
void __fastcall GetDesktopUniqueName(const wchar_t *a1, wchar_t *a2)
{
  __int64 v4; // rdx
  int SessionID; // esi
  int v6; // [rsp+28h] [rbp-240h]
  wchar_t v7[264]; // [rsp+30h] [rbp-238h] BYREF

  v7[0] = 0;
  SessionID = GetSessionID();
  if ( SessionID == -1 || !(unsigned int)GetThreadDesktopName(v7) )
  {
  ...
  {
    v6 = SessionID;
    StringCchPrintfW(a2, 260LL, L"%s%s%d", a1, v7, v6);
  }
}
