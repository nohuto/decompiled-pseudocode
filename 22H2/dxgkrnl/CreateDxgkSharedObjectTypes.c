/*
 * XREFs of CreateDxgkSharedObjectTypes @ 0x1C022146C
 * Callers:
 *     ?Initialize@DXGGLOBAL@@QEAAJXZ @ 0x1C02143F8 (-Initialize@DXGGLOBAL@@QEAAJXZ.c)
 * Callees:
 *     ?GenericMapping@DxgkCompositionObject@@SA?AU_GENERIC_MAPPING@@XZ @ 0x1C001727C (-GenericMapping@DxgkCompositionObject@@SA-AU_GENERIC_MAPPING@@XZ.c)
...
__int64 CreateDxgkSharedObjectTypes()
{
  ...
  if ( g_pDxgkSharedAllocationObjectType
    || (v2 = (struct _GENERIC_MAPPING)xmmword_1C00A46F0,
        ...
        (int)result >= 0) )
  {
    if ( g_pDxgkSharedKeyedMutexObjectType
      || (v2 = (struct _GENERIC_MAPPING)xmmword_1C00A4700,
          result = CreateSharedObject(
          ...
          (int)result >= 0) )
    {
      if ( g_pDxgkSharedSyncObjectType
        || (v2 = (struct _GENERIC_MAPPING)xmmword_1C00A4710,
            result = CreateSharedObject(
                ...
                (int)result >= 0) )
          {
                  ...
                  (int)result >= 0) )
            {
              if ( g_pDxgkSharedBundleObjectType
                || (v2 = (struct _GENERIC_MAPPING)xmmword_1C00A4720,
                    result = CreateSharedObject(
