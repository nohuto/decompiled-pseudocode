/*
 * XREFs of RtlIpv6StringToAddressA @ 0x18007D720
 * Callers:
 *     RtlIpv6StringToAddressExA @ 0x18007D680 (RtlIpv6StringToAddressExA.c)
 * Callees:
 *     __isascii @ 0x18008CF80 (__isascii.c)
 *     isdigit @ 0x18008D0C0 (isdigit.c)
 *     isxdigit @ 0x18008D210 (isxdigit.c)
 *     strtol @ 0x180091D90 (strtol.c)
 *     memmove @ 0x1800A4340 (memmove.c)
 *     memset @ 0x1800A4600 (memset.c)
 */

LONG __stdcall RtlIpv6StringToAddressA(PCSTR S, PCSTR *Terminator, struct in6_addr *Addr)
{
  struct in6_addr *v4; // rbp
  int v5; // r15d
  const char *v6; // rax
  unsigned int v7; // esi
  unsigned int v8; // edi
  unsigned int v9; // r12d
  unsigned int v10; // r14d
  int v11; // edx
  CHAR v12; // bl
  unsigned int v14; // eax
  int v15; // eax
  unsigned int v16; // eax
  PCSTR v17; // [rsp+20h] [rbp-58h]
  char v18; // [rsp+80h] [rbp+8h]
  int v21; // [rsp+98h] [rbp+20h]

  v4 = Addr;
  v5 = 0;
  v17 = 0LL;
  v6 = 0LL;
  v18 = 0;
  v7 = 0;
  v21 = 0;
  v8 = 0;
  v9 = 0;
  v10 = 0;
  v11 = 0;
  while ( 1 )
  {
    v12 = *S;
    if ( !*S )
      break;
    if ( v5 != 1 )
    {
      if ( v12 == 58 )
      {
        if ( v8 || v7 || S[1] != 58 )
          goto LABEL_11;
        v7 = 2;
        v10 = 1;
        v5 = 2;
        v21 = v11 + 1;
        ++S;
        v4->u.Word[v11] = 0;
        v6 = v17;
        goto LABEL_36;
      }
      if ( v7 > 7 )
        goto LABEL_11;
      if ( !_isascii(v12) || !isdigit(v12) )
      {
        if ( !_isascii(v12) || !isxdigit(v12) || v8 )
          goto LABEL_10;
        v6 = S;
        v18 = 1;
        v5 = 1;
        v17 = S;
        v9 = 1;
        goto LABEL_25;
      }
      v6 = S;
      v18 = 0;
      v5 = 1;
      v17 = S;
      v9 = 1;
LABEL_42:
      v11 = v21;
      goto LABEL_26;
    }
    if ( _isascii(v12) && isdigit(v12) )
    {
      v4 = Addr;
      ++v9;
      goto LABEL_28;
    }
    if ( !_isascii(v12) || !isxdigit(v12) )
    {
      if ( v12 != 58 )
      {
        if ( v12 != 46 || v18 || v8 > 2 )
          goto LABEL_22;
        v6 = v17;
        if ( v7 > 6 )
        {
          v4 = Addr;
          v11 = v21;
          break;
        }
        ++v8;
        v5 = 0;
        goto LABEL_36;
      }
      if ( v8 || v7 > 6 )
      {
LABEL_22:
        v4 = Addr;
LABEL_10:
        v11 = v21;
LABEL_11:
        v6 = v17;
        break;
      }
      if ( S[1] == 58 )
      {
        if ( v10 )
          goto LABEL_22;
        v15 = 2;
        ++S;
        v5 = 2;
        v10 = v7 + 1;
      }
      else
      {
        v5 = 0;
        v15 = 1;
      }
      v4 = Addr;
      v7 += v15;
      v6 = v17;
LABEL_25:
      v11 = v21;
      if ( v5 == 1 )
        goto LABEL_26;
LABEL_36:
      if ( v6 )
      {
        if ( v8 )
        {
          if ( v9 > 3 )
            return -1073741811;
          v14 = strtol(v6, 0LL, 10);
          if ( v14 > 0xFF )
            return -1073741811;
          v4 = Addr;
          Addr->u.Byte[2 * v21 - 1 + v8] = v14;
LABEL_28:
          v11 = v21;
        }
        else
        {
          if ( v9 > 4 )
            return -1073741811;
          v4 = Addr;
          Addr->u.Word[v21] = __ROR2__(strtol(v6, 0LL, 16), 8);
          v11 = ++v21;
        }
        v6 = v17;
        goto LABEL_26;
      }
      v4 = Addr;
      goto LABEL_42;
    }
    v4 = Addr;
    ++v9;
    v11 = v21;
    v6 = v17;
    if ( v8 )
      break;
    v18 = 1;
LABEL_26:
    ++S;
  }
  *Terminator = S;
  if ( v8 )
  {
    if ( v8 == 3 )
    {
      ++v7;
      goto LABEL_13;
    }
    return -1073741811;
  }
LABEL_13:
  if ( !v10 && v7 != 7 )
    return -1073741811;
  if ( v5 != 1 )
  {
    if ( v5 == 2 )
    {
      v4->u.Word[v11] = 0;
      goto LABEL_69;
    }
    return -1073741811;
  }
  if ( v8 )
  {
    if ( v9 <= 3 )
    {
      v16 = strtol(v6, 0LL, 10);
      if ( v16 <= 0xFF )
      {
        v4->u.Byte[2 * v21 + v8] = v16;
        goto LABEL_69;
      }
    }
    return -1073741811;
  }
  if ( v9 > 4 )
    return -1073741811;
  v4->u.Word[v21] = __ROR2__(strtol(v6, 0LL, 16), 8);
LABEL_69:
  if ( v10 )
  {
    memmove((char *)&v4[1] + 2 * (v10 - v7), (char *)v4 + 2 * v10, 2LL * (v7 - v10));
    memset((char *)v4 + 2 * v10, 0, 2LL * (8 - v7));
  }
  return 0;
}
