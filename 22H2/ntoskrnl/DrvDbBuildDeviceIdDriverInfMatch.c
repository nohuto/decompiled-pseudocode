/*
 * XREFs of DrvDbBuildDeviceIdDriverInfMatch @ 0x1406F9D24
 * Callers:
...
int __fastcall DrvDbBuildDeviceIdDriverInfMatch(__int64 a1, unsigned __int8 *a2, wchar_t *a3, unsigned int a4)
{
  ...
  v5 = *a2;
  if ( !*a2 )
    return RtlStringCchCopyExW(a3, a4, &word_1407D7A40, 0LL, 0LL, 0x800u);
  if ( v5 > 2u )
  {
    if ( v5 == 3 )
      return RtlStringCchPrintfExW(a3, a4, &ppszDestEnd, &pcchRemaining, 0x800u, L"%ws\\*", &word_1407D7A40);
    else
      return -1073741811;
  }
  else
  {
    ...
    v9 = v6;
    result = RtlStringCchPrintfExW(a3, a4, &ppszDestEnd, &pcchRemaining, 0x800u, L"%ws\\%X", &word_1407D7A40, v9);
