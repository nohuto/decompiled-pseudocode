/*
 * XREFs of MiMapWithLargePages @ 0x1403B848C
 * Callers:
 *     MiInsertInSystemSpace @ 0x14027B460 (MiInsertInSystemSpace.c)
 *     MiMapContiguousMemoryLarge @ 0x1403B81F4 (MiMapContiguousMemoryLarge.c)
 *     MiMapSystemImageWithLargePage @ 0x1408DA420 (MiMapSystemImageWithLargePage.c)
 * Callees:
 *     MiLockPageTableInternal @ 0x14020EAB0 (MiLockPageTableInternal.c)
 *     MiUnlockWorkingSetShared @ 0x14020F750 (MiUnlockWorkingSetShared.c)
 *     MiLockWorkingSetShared @ 0x140219C70 (MiLockWorkingSetShared.c)
 *     MiGetSharedVm @ 0x14021AF10 (MiGetSharedVm.c)
 *     MiUnlockWorkingSetExclusive @ 0x14021CAA0 (MiUnlockWorkingSetExclusive.c)
 *     ExAcquireSpinLockExclusive @ 0x14021D020 (ExAcquireSpinLockExclusive.c)
 *     MiMakeValidPte @ 0x1402AEDC0 (MiMakeValidPte.c)
 *     MiPteInShadowRange @ 0x1402C9180 (MiPteInShadowRange.c)
 *     MiUnlockPageTableInternal @ 0x1402DB460 (MiUnlockPageTableInternal.c)
 *     MiWritePteShadow @ 0x14030E10C (MiWritePteShadow.c)
 *     MiPteHasShadow @ 0x14030E16C (MiPteHasShadow.c)
 */

void __fastcall MiMapWithLargePages(
        __int64 a1,
        unsigned __int64 a2,
        unsigned __int64 a3,
        unsigned __int64 a4,
        int a5,
        char a6,
        int a7)
{
  unsigned __int64 v8; // r12
  unsigned __int64 v9; // r9
  __int64 v10; // r15
  int v11; // ebp
  unsigned __int64 v12; // rbx
  unsigned __int64 v13; // r8
  unsigned __int64 v14; // rsi
  unsigned int v16; // ebp
  unsigned __int64 v17; // rsi
  unsigned __int64 v18; // rax
  __int64 v19; // rdx
  __int64 v20; // r8
  _DWORD *v21; // r9
  unsigned __int64 v22; // rbp
  unsigned __int64 v23; // r13
  unsigned __int64 v24; // rdi
  unsigned __int8 v25; // al
  unsigned __int8 v26; // r14
  unsigned __int64 v27; // r14
  unsigned __int64 v28; // rbx
  int v29; // r12d
  __int64 v30; // r8
  unsigned __int64 *v31; // r14
  unsigned __int64 v32; // r13
  unsigned __int64 ValidPte; // rbx
  unsigned __int64 v34; // rdi
  int v35; // r12d
  __int64 v36; // r8
  bool v37; // zf
  __int64 v38; // rcx
  LONG *SharedVm; // rbx
  bool v40; // zf
  unsigned __int64 v42; // [rsp+78h] [rbp+10h]
  unsigned __int64 v44; // [rsp+88h] [rbp+20h]
  unsigned __int8 v45[8]; // [rsp+98h] [rbp+30h]

  v8 = MiLargePageSizes[a5];
  v9 = a3;
  v42 = v8;
  v10 = a1;
  v11 = a6 & 7;
  v44 = a4 / v8;
  v12 = a4 / v8;
  v13 = v8 * (a4 / v8);
  v14 = ((a2 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL;
  if ( v9 <= 0xFFFFFFFFFLL && ((*(_QWORD *)(48 * v9 - 0x57FFFFFFFD8LL) >> 50) & 1) != 0 )
    a7 = *(unsigned __int8 *)(48 * v9 - 0x57FFFFFFFDELL) >> 6;
  if ( a7 )
  {
    if ( a7 == 2 )
      v11 |= 0x18u;
  }
  else
  {
    v11 |= 8u;
  }
  v16 = v11 | 0xA0000000;
  if ( !(a4 % v8) )
    goto LABEL_6;
  v31 = (unsigned __int64 *)(v14 + 8 * v13);
  v32 = v14 + 8 * a4;
  ValidPte = MiMakeValidPte(0LL, v13 + v9, v16, v9);
  if ( (unsigned __int64)v31 >= v32 )
    goto LABEL_32;
  do
  {
    v34 = ValidPte;
    v35 = 0;
    if ( !MiPteInShadowRange((unsigned __int64)v31) )
      goto LABEL_28;
    if ( (unsigned int)MiPteHasShadow() )
    {
      v35 = 1;
      if ( HIBYTE(word_140C4E008) )
        goto LABEL_28;
      v37 = (ValidPte & 1) == 0;
    }
    else
    {
      if ( (HIDWORD(KeGetCurrentThread()->ApcState.Process[2].Header.WaitListHead.Flink) & 0x1000) == 0 )
        goto LABEL_28;
      v37 = (ValidPte & 1) == 0;
    }
    if ( !v37 )
      v34 = ValidPte | 0x8000000000000000uLL;
LABEL_28:
    *v31 = v34;
    if ( v35 )
      MiWritePteShadow((__int64)v31, v34, v36);
    ++v31;
    ValidPte ^= (ValidPte ^ (ValidPte + 4096)) & 0xFFFFFFFFF000LL;
  }
  while ( (unsigned __int64)v31 < v32 );
  v10 = a1;
  v8 = v42;
LABEL_32:
  v12 = v44;
  v9 = a3;
LABEL_6:
  v17 = ((v14 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL;
  if ( !a5 )
  {
    v38 = 1LL;
    do
    {
      v17 = ((v17 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL;
      --v38;
    }
    while ( v38 );
  }
  v18 = MiMakeValidPte(0LL, v9, v16 | 0x4000000, v9);
  v22 = 0LL;
  v23 = v17 + 8 * v12;
  v24 = v18;
  if ( a5 )
  {
    v25 = MiLockWorkingSetShared(v10, v19, v20, v21);
  }
  else
  {
    SharedVm = MiGetSharedVm(v10);
    v25 = ExAcquireSpinLockExclusive(SharedVm);
    SharedVm[1] = 0;
  }
  v45[0] = v25;
  v26 = v25;
  if ( v17 < v23 )
  {
    v27 = v8 << 12;
    while ( 1 )
    {
      if ( !a5 )
        goto LABEL_14;
      if ( v22 )
      {
        if ( (v17 & 0xFFF) != 0 )
          goto LABEL_14;
        MiUnlockPageTableInternal(v10, v22);
      }
      v22 = ((v17 >> 9) & 0x7FFFFFFFF8LL) - 0x98000000000LL;
      MiLockPageTableInternal(v10, v22, 0LL);
LABEL_14:
      v28 = v24;
      v29 = 0;
      if ( MiPteInShadowRange(v17) )
      {
        if ( (unsigned int)MiPteHasShadow() )
        {
          v29 = 1;
          if ( !HIBYTE(word_140C4E008) )
          {
            v40 = (v24 & 1) == 0;
            goto LABEL_51;
          }
        }
        else if ( (HIDWORD(KeGetCurrentThread()->ApcState.Process[2].Header.WaitListHead.Flink) & 0x1000) != 0 )
        {
          v40 = (v24 & 1) == 0;
LABEL_51:
          if ( !v40 )
            v28 = v24 | 0x8000000000000000uLL;
        }
      }
      *(_QWORD *)v17 = v28;
      if ( v29 )
        MiWritePteShadow(v17, v28, v30);
      v17 += 8LL;
      v24 ^= (v24 ^ (v27 + v24)) & 0xFFFFFFFFF000LL;
      if ( v17 >= v23 )
      {
        v26 = v45[0];
        break;
      }
    }
  }
  if ( a5 )
  {
    if ( v22 )
      MiUnlockPageTableInternal(v10, v22);
    MiUnlockWorkingSetShared(v10, v26);
  }
  else
  {
    MiUnlockWorkingSetExclusive(v10, v26);
  }
}
