/*
 * XREFs of SepIsImpersonationAllowedDueToCapability @ 0x140681634
 * Callers:
...
BOOLEAN __fastcall SepIsImpersonationAllowedDueToCapability(char *Token, __int64 a2)
{
  ...
  v7 = SepSidInToken((__int64)Token, 0LL, SeDefaultAccountAliasSid, 0, 0, 0, 0);
  if ( v7 )
  {
    if ( SeTokenIsRestricted(Token) )
      v7 = SepSidInToken((__int64)Token, 0LL, v6, 0, 1, 0, 0);
    if ( v7 )
      v4 = 1;
  }
  if ( SepAllowSessionImpersonationCap && (*((_DWORD *)Token + 50) & 0x4000) == 0 )
  {
    ...
    v9 = SepSidInToken((__int64)Token, 0LL, SeSessionImpersonationCapabilityGroupSid, 0, 0, 0, 0);
    if ( v9 )
    {
      if ( SeTokenIsRestricted(Token) )
        v9 = SepSidInToken((__int64)Token, 0LL, v8, 0, 1, 0, 0);
      if ( v9 )
        return 1;
    }
    v10 = v9;
  ...
  v12 = SepSidInToken((__int64)Token, 0LL, SeConstrainedImpersonationCapabilityGroupSid, 0, 0, 0, 0);
  if ( v12 )
  {
    if ( SeTokenIsRestricted(Token) )
      v12 = SepSidInToken((__int64)Token, 0LL, v11, 0, 1, 0, 0);
    if ( v12 )
      goto LABEL_28;
  }
  v13 = v12;
