/*
 * XREFs of sub_18012B380 @ 0x18012B380
 * Callers:
 *     <none>
 * Callees:
 *     sub_180109778 @ 0x180109778 (sub_180109778.c)
 */

__int64 __fastcall sub_18012B380(__int64 a1, __int64 a2)
{
  struct _RTL_CRITICAL_SECTION *v2; // rsi
  unsigned int v5; // edi
  __int64 v6; // r8
  __int64 v7; // r9
  __int64 v9; // [rsp+60h] [rbp+8h] BYREF
  __int64 v10; // [rsp+68h] [rbp+10h] BYREF
  __int64 v11; // [rsp+70h] [rbp+18h] BYREF

  v2 = (struct _RTL_CRITICAL_SECTION *)(a1 + 56);
  v5 = 0;
  EnterCriticalSection((LPCRITICAL_SECTION)(a1 + 56));
  if ( v2 )
    LeaveCriticalSection(v2);
  if ( a2 )
  {
    *(_OWORD *)a2 = *(_OWORD *)(a1 + 96);
    *(_OWORD *)(a2 + 16) = *(_OWORD *)(a1 + 112);
    *(_OWORD *)(a2 + 32) = *(_OWORD *)(a1 + 128);
    *(_OWORD *)(a2 + 48) = *(_OWORD *)(a1 + 144);
    *(_OWORD *)(a2 + 64) = *(_OWORD *)(a1 + 160);
    *(_OWORD *)(a2 + 80) = *(_OWORD *)(a1 + 176);
    *(_OWORD *)(a2 + 96) = *(_OWORD *)(a1 + 192);
    *(_OWORD *)(a2 + 112) = *(_OWORD *)(a1 + 208);
    *(_OWORD *)(a2 + 128) = *(_OWORD *)(a1 + 224);
    *(_WORD *)(a2 + 144) = *(_WORD *)(a1 + 240);
  }
  else
  {
    if ( (unsigned int)dword_18019C480 > 2 )
    {
      LODWORD(v9) = -2147024809;
      v11 = (__int64)"SpatialAudioEncoderProperties::GetProperties";
      LODWORD(v10) = 449;
      sub_180109778((__int64)&dword_18019C480, byte_18016C12C, v6, v7, (const CHAR **)&v11, (__int64)&v10, (__int64)&v9);
    }
    return (unsigned int)-2147024809;
  }
  return v5;
}
