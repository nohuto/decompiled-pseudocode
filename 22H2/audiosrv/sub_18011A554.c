/*
 * XREFs of sub_18011A554 @ 0x18011A554
 * Callers:
 *     sub_18011B0C8 @ 0x18011B0C8 (sub_18011B0C8.c)
 * Callees:
 *     _guard_dispatch_icall_nop @ 0x180074470 (_guard_dispatch_icall_nop.c)
 */

// Hidden C++ exception states: #wind=5
__int64 __fastcall sub_18011A554(__int64 a1, int a2, __int64 a3, int a4, unsigned __int64 a5, _DWORD *a6)
{
  __int64 (__fastcall *v7)(__int64, void *, __int64, __int16 *, __int64 *); // rax
  int v8; // ebx
  void (*v9)(void); // rax
  __int64 v10; // rcx
  double v11; // xmm1_8
  double v12; // xmm0_8
  int v13; // eax
  __int64 v15; // [rsp+30h] [rbp-30h] BYREF
  __int64 v16; // [rsp+38h] [rbp-28h] BYREF
  __int16 v17; // [rsp+40h] [rbp-20h] BYREF
  int v18; // [rsp+48h] [rbp-18h]
  __int64 v19; // [rsp+50h] [rbp-10h]
  int v20; // [rsp+88h] [rbp+28h] BYREF
  __int64 v21; // [rsp+90h] [rbp+30h] BYREF
  int v22; // [rsp+98h] [rbp+38h] BYREF

  v16 = 0LL;
  v17 = 65;
  v18 = a4;
  v19 = a3;
  v7 = *(__int64 (__fastcall **)(__int64, void *, __int64, __int16 *, __int64 *))(*(_QWORD *)a1 + 24LL);
  v21 = 0LL;
  if ( a2 )
  {
    v8 = v7(a1, &unk_180173750, 1LL, &v17, &v21);
    if ( v8 < 0 )
    {
      v10 = v21;
      goto LABEL_7;
    }
  }
  else
  {
    v8 = v7(a1, &unk_180173760, 1LL, &v17, &v21);
    if ( v8 < 0 )
    {
      if ( !v21 )
        goto LABEL_32;
      v9 = *(void (**)(void))(*(_QWORD *)v21 + 16LL);
      goto LABEL_5;
    }
  }
  v8 = (**(__int64 (__fastcall ***)(__int64, void *, __int64 *))v21)(v21, &unk_18015EC78, &v16);
  v10 = v21;
  if ( v8 < 0 )
    goto LABEL_7;
  if ( v21 )
    (*(void (**)(void))(*(_QWORD *)v21 + 16LL))();
  v8 = (*(__int64 (__fastcall **)(__int64, int *))(*(_QWORD *)v16 + 40LL))(v16, &v20);
  if ( v8 < 0 )
    goto LABEL_32;
  if ( !v20 )
  {
    v11 = (double)*(int *)(a3 + 48);
    v12 = (double)(int)a5;
LABEL_31:
    *a6 = (int)(v11 * v12 / 10000000.0 + 0.5);
    goto LABEL_32;
  }
  v8 = (*(__int64 (__fastcall **)(__int64, int *))(*(_QWORD *)v16 + 32LL))(v16, &v22);
  if ( v8 < 0 )
    goto LABEL_32;
  if ( !v22 )
  {
    v13 = 20000;
    if ( a5 >= 0x4E20 )
      v13 = a5;
    v11 = (double)v13;
    v12 = (double)*(int *)(a3 + 48);
    goto LABEL_31;
  }
  v15 = 0LL;
  v21 = 0LL;
  v8 = (**(__int64 (__fastcall ***)(__int64, void *, __int64 *))v16)(v16, &unk_18015EC88, &v21);
  if ( v8 >= 0 )
  {
    v8 = (**(__int64 (__fastcall ***)(__int64, void *, __int64 *))v16)(v16, &unk_18015EC68, &v15);
    if ( v8 >= 0 )
    {
      v8 = (*(__int64 (__fastcall **)(__int64, __int64))(*(_QWORD *)v21 + 48LL))(v21, 0x40000LL);
      if ( v8 >= 0 )
      {
        v8 = (*(__int64 (__fastcall **)(__int64, unsigned __int64, _QWORD))(*(_QWORD *)v16 + 24LL))(v16, a5, 0LL);
        if ( (int)(v8 + 0x80000000) < 0 || v8 == -2005139372 )
          v8 = (*(__int64 (__fastcall **)(__int64, _DWORD *))(*(_QWORD *)v15 + 32LL))(v15, a6);
      }
    }
  }
  if ( v21 )
    (*(void (__fastcall **)(__int64))(*(_QWORD *)v21 + 16LL))(v21);
  v10 = v15;
LABEL_7:
  if ( v10 )
  {
    v9 = *(void (**)(void))(*(_QWORD *)v10 + 16LL);
LABEL_5:
    v9();
  }
LABEL_32:
  if ( v16 )
    (*(void (__fastcall **)(__int64))(*(_QWORD *)v16 + 16LL))(v16);
  return (unsigned int)v8;
}
