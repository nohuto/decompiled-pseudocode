/*
 * XREFs of ?ndisVerifierInitialization@@YAEXZ @ 0x1C003A1C4
 * Callers:
  ...
  if ( MmIsVerifierEnabled(&VerifierFlags) >= 0 )
  {
    ...
    if ( ndisVerifierWdmDispatch )
    {
      ...
      if ( ndisVerifierNdisDispatch )
      {
        ...
        if ( MmAddVerifierThunks(&off_1C00C9D40, 0x190u) >= 0 )
        {
          qword_1C00E72D8 = (__int64)&ndisMiniportTrackAllocList;
          ndisMiniportTrackAllocList.Flink = &ndisMiniportTrackAllocList;
          qword_1C00E72C8 = (__int64)&ndisDriverTrackAllocList;
