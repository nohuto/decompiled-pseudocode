/*
 * XREFs of ?NdisMTransferDataComplete@@YAXPEAXPEAU_NDIS_PACKET@@HI@Z @ 0x1C00C0C40
 * Callers:
 *     <none>
 * Callees:
...
void __fastcall NdisMTransferDataComplete(_DWORD *a1, struct _NDIS_PACKET *a2, unsigned int a3, unsigned int a4)
{
  unsigned int v5; // ecx
  KIRQL v6; // di
  PNDIS_PACKET v9; // r10
  ...
  else
  {
    ...
    *(_DWORD *)((char *)&Packet->Private.Tail + Packet->Private.NdisPacketOobOffset) = 14;
    (*((void (__fastcall **)(_DWORD *, PNDIS_PACKET *, __int64))a1 + 54))(a1, &Packet, 1LL);
    if ( (a1[30] & 0x40000) == 0
