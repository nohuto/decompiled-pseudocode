/*
 * XREFs of ??1VIDMM_PROCESS_ADAPTER_INFO@@QEAA@XZ @ 0x1C009143C
 * Callers:
 *     ??_GVIDMM_PROCESS_ADAPTER_INFO@@QEAAPEAXI@Z @ 0x1C0004C40 (--_GVIDMM_PROCESS_ADAPTER_INFO@@QEAAPEAXI@Z.c)
 * Callees:
 *     ??$Write@U?$_tlgWrapperByVal@$07@@U?$_tlgWrapperByVal@$01@@U?$_tlgWrapperByRef@$0BA@@@U?$_tlgWrapBuffer@U_UNICODE_STRING@@@@U_tlgWrapperPtrSize@@U5@@?$_tlgWriteTemplate@$$A6AJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2IPEAU_EVENT_DATA_DESCRIPTOR@@@Z$1?_tlgWriteTransfer_EtwWriteTransfer@@YAJ0122I3@ZPEBU2@PEBU2@@@SAJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2AEBU?$_tlgWrapperByVal@$07@@AEBU?$_tlgWrapperByVal@$01@@AEBU?$_tlgWrapperByRef@$0BA@@@AEBU?$_tlgWrapBuffer@U_UNICODE_STRING@@@@AEBU_tlgWrapperPtrSize@@7@Z @ 0x1C0004CAC (--$Write@U-$_tlgWrapperByVal@$07@@U-$_tlgWrapperByVal@$01@@U-$_tlgWrapperByRef@$0BA@@@U-$_tlgWra.c)
 *     _tlgKeywordOn @ 0x1C0004E2C (_tlgKeywordOn.c)
 *     ??3@YAXPEAX@Z @ 0x1C0005500 (--3@YAXPEAX@Z.c)
 *     __security_check_cookie @ 0x1C0019900 (__security_check_cookie.c)
 *     ??$Write@U?$_tlgWrapperByVal@$07@@U?$_tlgWrapperByVal@$01@@U?$_tlgWrapperByRef@$0BA@@@U?$_tlgWrapBuffer@U_UNICODE_STRING@@@@U?$_tlgWrapperByVal@$03@@U5@U5@U5@U5@@?$_tlgWriteTemplate@$$A6AJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2IPEAU_EVENT_DATA_DESCRIPTOR@@@Z$1?_tlgWriteTransfer_EtwWriteTransfer@@YAJ0122I3@ZPEBU2@PEBU2@@@SAJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2AEBU?$_tlgWrapperByVal@$07@@AEBU?$_tlgWrapperByVal@$01@@AEBU?$_tlgWrapperByRef@$0BA@@@AEBU?$_tlgWrapBuffer@U_UNICODE_STRING@@@@AEBU?$_tlgWrapperByVal@$03@@7777@Z @ 0x1C00309B0 (--$Write@U-$_tlgWrapperByVal@$07@@U-$_tlgWrapperByVal@$01@@U-$_tlgWrapperByRef@$0BA_ea_1C00309B0.c)
 *     ??$Write@U?$_tlgWrapperByVal@$07@@U?$_tlgWrapperByVal@$01@@U?$_tlgWrapperByRef@$0BA@@@U?$_tlgWrapBuffer@U_UNICODE_STRING@@@@U?$_tlgWrapperByVal@$03@@U5@U_tlgWrapperPtrSize@@U6@U6@U6@U6@U6@U6@@?$_tlgWriteTemplate@$$A6AJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2IPEAU_EVENT_DATA_DESCRIPTOR@@@Z$1?_tlgWriteTransfer_EtwWriteTransfer@@YAJ0122I3@ZPEBU2@PEBU2@@@SAJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2AEBU?$_tlgWrapperByVal@$07@@AEBU?$_tlgWrapperByVal@$01@@AEBU?$_tlgWrapperByRef@$0BA@@@AEBU?$_tlgWrapBuffer@U_UNICODE_STRING@@@@AEBU?$_tlgWrapperByVal@$03@@7AEBU_tlgWrapperPtrSize@@888888@Z @ 0x1C0030AC0 (--$Write@U-$_tlgWrapperByVal@$07@@U-$_tlgWrapperByVal@$01@@U-$_tlgWrapperByRef@$0BA_ea_1C0030AC0.c)
 *     ??$Write@U?$_tlgWrapperByVal@$07@@U?$_tlgWrapperByVal@$01@@U?$_tlgWrapperByRef@$0BA@@@U?$_tlgWrapBuffer@U_UNICODE_STRING@@@@U_tlgWrapperPtrSize@@U5@U5@U5@@?$_tlgWriteTemplate@$$A6AJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2IPEAU_EVENT_DATA_DESCRIPTOR@@@Z$1?_tlgWriteTransfer_EtwWriteTransfer@@YAJ0122I3@ZPEBU2@PEBU2@@@SAJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2AEBU?$_tlgWrapperByVal@$07@@AEBU?$_tlgWrapperByVal@$01@@AEBU?$_tlgWrapperByRef@$0BA@@@AEBU?$_tlgWrapBuffer@U_UNICODE_STRING@@@@AEBU_tlgWrapperPtrSize@@777@Z @ 0x1C0030C50 (--$Write@U-$_tlgWrapperByVal@$07@@U-$_tlgWrapperByVal@$01@@U-$_tlgWrapperByRef@$0BA_ea_1C0030C50.c)
 *     ?TotalDemotedCommitment@VIDMM_PROCESS_BUDGET_STATE@@QEBA_KXZ @ 0x1C0091614 (-TotalDemotedCommitment@VIDMM_PROCESS_BUDGET_STATE@@QEBA_KXZ.c)
 *     ?LogData@VIDMM_TELEMETRY_PROCESS_ALLOCATED_CONTENT@@QEAAXPEAVDXGPROCESS@@PEAVDXGADAPTER@@@Z @ 0x1C0091638 (-LogData@VIDMM_TELEMETRY_PROCESS_ALLOCATED_CONTENT@@QEAAXPEAVDXGPROCESS@@PEAVDXGADAPTER@@@Z.c)
 */

// write access to const memory has been detected, the output may be wrong!
void __fastcall VIDMM_PROCESS_ADAPTER_INFO::~VIDMM_PROCESS_ADAPTER_INFO(VIDMM_PROCESS_ADAPTER_INFO *this)
{
  void *v2; // rcx
  __int64 v3; // rcx
  unsigned int v4; // r14d
  __int64 v5; // r15
  __int64 *v6; // rdi
  __int64 *v7; // rsi
  struct DXGADAPTER *v8; // r14
  __int64 v9; // rsi
  __int64 v10; // r15
  char *v11; // r9
  char *v12; // rcx
  __int64 v13; // rcx
  unsigned __int64 v14; // rax
  __int64 v15; // rcx
  __int64 v16; // rcx
  __int64 v17; // r8
  unsigned __int16 *v18; // r9
  __int128 v19; // xmm0
  __int64 v20; // rcx
  __int64 v21; // r8
  unsigned __int16 *v22; // r9
  __int128 v23; // xmm0
  unsigned __int64 *v24; // rcx
  __int64 v25; // r10
  __int64 v26; // rcx
  __int64 v27; // r8
  __int64 v28; // r9
  int v29; // r10d
  int v30; // r11d
  __int128 v31; // xmm0
  __int64 v32; // rcx
  __int64 v33; // r8
  __int64 v34; // r9
  __int128 v35; // xmm0
  _QWORD *v36; // r10
  unsigned __int64 v37; // rax
  __int64 v38; // r8
  _DWORD *v39; // r9
  __int128 v40; // xmm0
  __int16 v41; // [rsp+98h] [rbp-80h] BYREF
  __int128 *v42; // [rsp+A0h] [rbp-78h] BYREF
  unsigned __int16 *v43; // [rsp+A8h] [rbp-70h] BYREF
  __int64 v44; // [rsp+B0h] [rbp-68h] BYREF
  int v45; // [rsp+B8h] [rbp-60h] BYREF
  int v46; // [rsp+BCh] [rbp-5Ch] BYREF
  __int64 v47; // [rsp+C0h] [rbp-58h] BYREF
  int v48; // [rsp+C8h] [rbp-50h]
  __int128 *v49; // [rsp+D0h] [rbp-48h] BYREF
  int v50; // [rsp+D8h] [rbp-40h]
  __int64 v51; // [rsp+E0h] [rbp-38h] BYREF
  int v52; // [rsp+E8h] [rbp-30h]
  unsigned __int16 *v53; // [rsp+F0h] [rbp-28h] BYREF
  int v54; // [rsp+F8h] [rbp-20h]
  _BYTE *v55; // [rsp+100h] [rbp-18h] BYREF
  int v56; // [rsp+108h] [rbp-10h]
  __int64 v57; // [rsp+110h] [rbp-8h] BYREF
  int v58; // [rsp+118h] [rbp+0h]
  __int64 v59; // [rsp+120h] [rbp+8h] BYREF
  int v60; // [rsp+128h] [rbp+10h]
  __int64 v61; // [rsp+130h] [rbp+18h] BYREF
  int v62; // [rsp+138h] [rbp+20h]
  _BYTE *v63; // [rsp+140h] [rbp+28h] BYREF
  int v64; // [rsp+148h] [rbp+30h]
  _BYTE *v65; // [rsp+150h] [rbp+38h] BYREF
  int v66; // [rsp+158h] [rbp+40h]
  _DWORD *v67; // [rsp+160h] [rbp+48h] BYREF
  int v68; // [rsp+168h] [rbp+50h]
  _DWORD *v69; // [rsp+170h] [rbp+58h] BYREF
  int v70; // [rsp+178h] [rbp+60h]
  _DWORD *v71; // [rsp+180h] [rbp+68h] BYREF
  int v72; // [rsp+188h] [rbp+70h]
  _DWORD *v73; // [rsp+190h] [rbp+78h] BYREF
  int v74; // [rsp+198h] [rbp+80h]
  _DWORD *v75; // [rsp+1A0h] [rbp+88h] BYREF
  int v76; // [rsp+1A8h] [rbp+90h]
  __int128 v77; // [rsp+1B0h] [rbp+98h] BYREF
  _BYTE v78[176]; // [rsp+1C8h] [rbp+B0h] BYREF
  _BYTE v79[176]; // [rsp+278h] [rbp+160h] BYREF
  _BYTE v80[176]; // [rsp+328h] [rbp+210h] BYREF

  v2 = (void *)*((_QWORD *)this + 2);
  if ( v2 )
    operator delete(v2);
  v3 = *(_QWORD *)this;
  if ( !*(_QWORD *)this )
  {
LABEL_16:
    if ( !v3 )
      goto LABEL_22;
    goto LABEL_17;
  }
  if ( *((_DWORD *)this + 88) )
  {
    v4 = 0;
    if ( *(_DWORD *)(*(_QWORD *)this + 7016LL) )
    {
      do
      {
        v5 = 2LL;
        v6 = (__int64 *)(*((_QWORD *)this + 6) + 296LL * v4);
        v7 = v6 + 25;
        do
        {
          if ( *(v7 - 8) || *(v7 - 16) || *v7 )
          {
            v13 = *v7;
            g_DxgMmsBugcheckExportIndex = 1;
            WdLogSingleEntry5(0LL, 270LL, 51LL, this, v13, 0LL);
          }
          ++v7;
          --v5;
        }
        while ( v5 );
        if ( VIDMM_PROCESS_BUDGET_STATE::TotalDemotedCommitment((VIDMM_PROCESS_BUDGET_STATE *)v6) )
        {
          v14 = VIDMM_PROCESS_BUDGET_STATE::TotalDemotedCommitment((VIDMM_PROCESS_BUDGET_STATE *)v6);
          g_DxgMmsBugcheckExportIndex = 1;
          WdLogSingleEntry5(0LL, 270LL, 51LL, this, v14, 0LL);
        }
        if ( v6[27] )
        {
          v15 = v6[27];
          g_DxgMmsBugcheckExportIndex = 1;
          WdLogSingleEntry5(0LL, 270LL, 51LL, this, v15, 0LL);
        }
        v3 = *(_QWORD *)this;
        ++v4;
      }
      while ( v4 < *(_DWORD *)(*(_QWORD *)this + 7016LL) );
      goto LABEL_16;
    }
  }
LABEL_17:
  v8 = *(struct DXGADAPTER **)(v3 + 24);
  v9 = *(_QWORD *)(*((_QWORD *)this + 5) + 32LL);
  VIDMM_TELEMETRY_PROCESS_ALLOCATED_CONTENT::LogData(
    (VIDMM_PROCESS_ADAPTER_INFO *)((char *)this + 528),
    (struct DXGPROCESS *)v9,
    v8);
  if ( *((_BYTE *)this + 1148)
    && (unsigned int)dword_1C0076048 > 5
    && tlgKeywordOn((__int64)&dword_1C0076048, 0x400000000008LL) )
  {
    v19 = *(_OWORD *)(v9 + 360);
    v51 = (__int64)v18;
    v49 = (__int128 *)(v18 + 116);
    v43 = (unsigned __int16 *)*((_QWORD *)v8 + 218);
    v42 = &v77;
    v52 = 232;
    v50 = 116;
    v77 = v19;
    v41 = 1;
    v44 = 0x1000000LL;
    _tlgWriteTemplate<long (_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),&long _tlgWriteTransfer_EtwWriteTransfer(_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),_GUID const *,_GUID const *>::Write<_tlgWrapperByVal<8>,_tlgWrapperByVal<2>,_tlgWrapperByRef<16>,_tlgWrapBuffer<_UNICODE_STRING>,_tlgWrapperPtrSize,_tlgWrapperPtrSize>(
      v16,
      (int)&unk_1C0055FD3,
      v17,
      (__int64)v18,
      (__int64)&v44,
      (__int64)&v41,
      (__int64 *)&v42,
      &v43,
      (__int64 *)&v49,
      &v51);
  }
  if ( *((_BYTE *)this + 1212)
    && (unsigned int)dword_1C0076048 > 5
    && tlgKeywordOn((__int64)&dword_1C0076048, 0x400000000008LL) )
  {
    v23 = *(_OWORD *)(v9 + 360);
    v47 = (__int64)v22;
    v53 = v22 + 20;
    v43 = (unsigned __int16 *)*((_QWORD *)v8 + 218);
    v42 = &v77;
    v48 = 40;
    v54 = 20;
    v77 = v23;
    v41 = 1;
    v44 = 0x1000000LL;
    _tlgWriteTemplate<long (_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),&long _tlgWriteTransfer_EtwWriteTransfer(_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),_GUID const *,_GUID const *>::Write<_tlgWrapperByVal<8>,_tlgWrapperByVal<2>,_tlgWrapperByRef<16>,_tlgWrapBuffer<_UNICODE_STRING>,_tlgWrapperPtrSize,_tlgWrapperPtrSize>(
      v20,
      (int)&unk_1C0055F45,
      v21,
      (__int64)v22,
      (__int64)&v44,
      (__int64)&v41,
      (__int64 *)&v42,
      &v43,
      (__int64 *)&v53,
      &v47);
  }
  v10 = 22LL;
  if ( *((_BYTE *)this + 1744) )
  {
    v24 = (unsigned __int64 *)v78;
    v25 = 22LL;
    do
    {
      *v24 = 10000000
           * *(unsigned __int64 *)((char *)v24
                                 + (VIDMM_PROCESS_ADAPTER_INFO *)((char *)this + 1216)
                                 - (VIDMM_PROCESS_ADAPTER_INFO *)v78
                                 + 352)
           / VIDMM_GLOBAL::_QpcFrequency.QuadPart;
      ++v24;
      --v25;
    }
    while ( v25 );
    if ( (unsigned int)dword_1C0076048 > 5 && tlgKeywordOn((__int64)&dword_1C0076048, 0x400000000008LL) )
    {
      v31 = *(_OWORD *)(v9 + 360);
      v56 = v30;
      v55 = v78;
      v58 = v30;
      v57 = v28 + 176;
      v59 = v28 + 88;
      v60 = v29 + 88;
      v62 = v29 + 88;
      v43 = (unsigned __int16 *)*((_QWORD *)v8 + 218);
      v42 = &v77;
      v61 = v28;
      v77 = v31;
      v41 = 0;
      v44 = 0x1000000LL;
      _tlgWriteTemplate<long (_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),&long _tlgWriteTransfer_EtwWriteTransfer(_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),_GUID const *,_GUID const *>::Write<_tlgWrapperByVal<8>,_tlgWrapperByVal<2>,_tlgWrapperByRef<16>,_tlgWrapBuffer<_UNICODE_STRING>,_tlgWrapperPtrSize,_tlgWrapperPtrSize,_tlgWrapperPtrSize,_tlgWrapperPtrSize>(
        v26,
        byte_1C0055EA6,
        v27,
        v28,
        (__int64)&v44,
        (__int64)&v41,
        (__int64 *)&v42,
        &v43,
        &v61,
        &v59,
        &v57,
        (__int64 *)&v55);
    }
  }
  if ( *((_BYTE *)this + 1772)
    && (unsigned int)dword_1C0076048 > 5
    && tlgKeywordOn((__int64)&dword_1C0076048, 0x400000000008LL) )
  {
    v35 = *(_OWORD *)(v9 + 360);
    v45 = *((_DWORD *)this + 442);
    v46 = *((_DWORD *)this + 441);
    LODWORD(v44) = *((_DWORD *)this + 440);
    LODWORD(v42) = *((_DWORD *)this + 439);
    LODWORD(v43) = *((_DWORD *)this + 438);
    v47 = *((_QWORD *)v8 + 218);
    v49 = &v77;
    v77 = v35;
    v41 = 0;
    v51 = 0x1000000LL;
    _tlgWriteTemplate<long (_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),&long _tlgWriteTransfer_EtwWriteTransfer(_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),_GUID const *,_GUID const *>::Write<_tlgWrapperByVal<8>,_tlgWrapperByVal<2>,_tlgWrapperByRef<16>,_tlgWrapBuffer<_UNICODE_STRING>,_tlgWrapperByVal<4>,_tlgWrapperByVal<4>,_tlgWrapperByVal<4>,_tlgWrapperByVal<4>,_tlgWrapperByVal<4>>(
      v32,
      byte_1C0055A88,
      v33,
      v34,
      (__int64)&v51,
      (__int64)&v41,
      (__int64 *)&v49,
      (unsigned __int16 **)&v47,
      (__int64)&v43,
      (__int64)&v42,
      (__int64)&v44,
      (__int64)&v46,
      (__int64)&v45);
  }
  v11 = (char *)this + 1776;
  if ( *((_BYTE *)this + 2752) )
  {
    v36 = (_QWORD *)((char *)this + 2048);
    do
    {
      *(_QWORD *)((char *)v36 + v80 - v11 - 272) = (unsigned __int64)(10000000LL * *(v36 - 22))
                                                 / VIDMM_GLOBAL::_QpcFrequency.QuadPart;
      v37 = 10000000LL * *v36++;
      *(_QWORD *)((char *)v36 + v79 - v11 - 280) = v37 / VIDMM_GLOBAL::_QpcFrequency.QuadPart;
      --v10;
    }
    while ( v10 );
    if ( (unsigned int)dword_1C0076048 > 5 && tlgKeywordOn((__int64)&dword_1C0076048, 0x400000000008LL) )
    {
      v40 = *(_OWORD *)(v9 + 360);
      v41 = 1;
      v64 = 176;
      v63 = v79;
      v65 = v80;
      v67 = v39 + 222;
      v68 = 88;
      v69 = v39 + 200;
      v71 = v39 + 156;
      v73 = v39 + 112;
      v75 = v39 + 2;
      LODWORD(v43) = v39[1];
      LODWORD(v42) = *v39;
      v51 = *((_QWORD *)v8 + 218);
      v49 = &v77;
      v70 = 88;
      v76 = 88;
      v66 = 176;
      v72 = 176;
      v74 = 176;
      v77 = v40;
      v47 = 0x1000000LL;
      _tlgWriteTemplate<long (_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),&long _tlgWriteTransfer_EtwWriteTransfer(_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),_GUID const *,_GUID const *>::Write<_tlgWrapperByVal<8>,_tlgWrapperByVal<2>,_tlgWrapperByRef<16>,_tlgWrapBuffer<_UNICODE_STRING>,_tlgWrapperByVal<4>,_tlgWrapperByVal<4>,_tlgWrapperPtrSize,_tlgWrapperPtrSize,_tlgWrapperPtrSize,_tlgWrapperPtrSize,_tlgWrapperPtrSize,_tlgWrapperPtrSize,_tlgWrapperPtrSize>(
        176LL,
        byte_1C0055949,
        v38,
        (__int64)v39,
        (__int64)&v47,
        (__int64)&v41,
        (__int64 *)&v49,
        (unsigned __int16 **)&v51,
        (__int64)&v42,
        (__int64)&v43,
        (__int64 *)&v75,
        (__int64 *)&v73,
        (__int64 *)&v71,
        (__int64 *)&v69,
        (__int64 *)&v67,
        (__int64 *)&v65,
        (__int64 *)&v63);
    }
  }
LABEL_22:
  v12 = (char *)*((_QWORD *)this + 6);
  if ( v12 != (char *)this + 56 && v12 )
    ExFreePoolWithTag(v12, 0);
  *((_QWORD *)this + 6) = 0LL;
  *((_DWORD *)this + 88) = 0;
}
