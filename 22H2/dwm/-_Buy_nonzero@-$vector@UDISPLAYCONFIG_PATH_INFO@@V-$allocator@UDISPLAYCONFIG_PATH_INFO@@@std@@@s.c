/*
 * XREFs of ?_Buy_nonzero@?$vector@UDISPLAYCONFIG_PATH_INFO@@V?$allocator@UDISPLAYCONFIG_PATH_INFO@@@std@@@std@@AEAAX_K@Z @ 0x140009774
 * Callers:
 *     ??0?$vector@UDISPLAYCONFIG_PATH_INFO@@V?$allocator@UDISPLAYCONFIG_PATH_INFO@@@std@@@std@@QEAA@_KAEBV?$allocator@UDISPLAYCONFIG_PATH_INFO@@@1@@Z @ 0x140008710 (--0-$vector@UDISPLAYCONFIG_PATH_INFO@@V-$allocator@UDISPLAYCONFIG_PATH_INFO@@@std@@@std@@QEAA@_K.c)
 * Callees:
 *     ??$_Allocate@$0BA@U_Default_allocate_traits@std@@$0A@@std@@YAPEAX_K@Z @ 0x1400083F4 (--$_Allocate@$0BA@U_Default_allocate_traits@std@@$0A@@std@@YAPEAX_K@Z.c)
 */

_QWORD *__fastcall std::vector<DISPLAYCONFIG_PATH_INFO>::_Buy_nonzero(_QWORD *a1, unsigned __int64 a2)
{
  __int64 v3; // rbx
  _QWORD *v4; // rax
  _QWORD *result; // rax

  if ( a2 > 0x38E38E38E38E38ELL )
    std::_Xlength_error("vector<T> too long");
  v3 = 9 * a2;
  v4 = std::_Allocate<16,std::_Default_allocate_traits,0>(72 * a2);
  *a1 = v4;
  a1[1] = v4;
  result = &v4[v3];
  a1[2] = result;
  return result;
}
