/*
 * XREFs of ?TabletAdjustBrightness@@YAXH@Z @ 0x1C01D8A00
 * Callers:
 *     ?xxxTabletButtonExecuteAction@@YAXPEAUtagTABLET_BUTTON_ACTION@@HPEAU_devicemodeW@@@Z @ 0x1C01D909C (-xxxTabletButtonExecuteAction@@YAXPEAUtagTABLET_BUTTON_ACTION@@HPEAU_devicemodeW@@@Z.c)
 * Callees:
...
void __fastcall TabletAdjustBrightness(int a1)
{
  ...
  _DWORD v10[2]; // [rsp+30h] [rbp-20h] BYREF
  ...
  if ( !gfSwitchInProgress && !gPowerTransitionsState[2] && gPowerTransitionsState[0] )
  {
    if ( IoWMIOpenBlock(&WmiMonitorBrightness_GUID, 1u, &DataBlockObject) >= 0 )
    {
      ...
      {
        v2 = (unsigned int *)Win32AllocPoolNonPaged(InOutBufferSize, 1869640533LL);
        v3 = v2;
        if ( v2 )
        {
