/*
 * XREFs of ?xxxDrawMenuItemText@@YAXAEBV?$SmartObjStackRef@UtagMENU@@@@PEAUtagITEM@@PEAUHDC__@@HHPEAGHH@Z @ 0x1C0233FEC
 * Callers:
...
const WCHAR *__fastcall xxxDrawMenuItemText(
        __int64 **a1,
        __int64 a2,
        Gre::Base *a3,
        __int64 a4,
        ...
        int a8)
{
  ...
  if ( a7 < 255 )
  {
    SourceString = (const WCHAR *)v23;
  }
  else
  {
    result = (const WCHAR *)Win32AllocPoolZInit(2LL * (a7 + 1), 1953657685LL);
    SourceString = result;
    if ( !result )
      return result;
...
LABEL_21:
  result = (const WCHAR *)v23;
  if ( SourceString != (const WCHAR *)v23 )
    return (const WCHAR *)PopAndFreeAlwaysW32ThreadLock((__int64)&v21);
  return result;
}
