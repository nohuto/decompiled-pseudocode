/*
 * XREFs of _lambda_beb7e60510a54e82b7f39bd19dd130e9_::_lambda_invoker_cdecl_ @ 0x1C0027D90
 * Callers:
 *     <none>
 * Callees:
 *     ?IsWindowActivateable@@YA_NPEBUtagWND@@@Z @ 0x1C0028248 (-IsWindowActivateable@@YA_NPEBUtagWND@@@Z.c)
 */

bool __fastcall lambda_beb7e60510a54e82b7f39bd19dd130e9_::_lambda_invoker_cdecl_(_QWORD *a1, _OWORD *a2)
{
  int v4; // edx
  const struct tagWND *v5; // rdi
  __int128 v6; // [rsp+20h] [rbp-18h]

  *a2 = 0LL;
  if ( gpqForeground == *(_QWORD *)(a1[2] + 432LL) )
  {
    v4 = *(_DWORD *)(a1[5] + 236LL);
    if ( v4 != 2 && v4 != 16 )
    {
      v5 = *(const struct tagWND **)(a1[3] + 296LL);
      if ( IsWindowActivateable(v5) )
      {
        *(_QWORD *)&v6 = v5;
        DWORD2(v6) = 5;
        *a2 = v6;
      }
    }
  }
  return *(_QWORD *)a2 != 0LL;
}
