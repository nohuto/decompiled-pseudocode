/*
 * XREFs of ?Initialize@CButton@@MEAAJXZ @ 0x180026B90
 * Callers:
...
__int64 __fastcall CButton::Initialize(CButton *this)
{
  ...
  if ( v2 < 0 )
  {
    MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v2, 0x2Du);
  }
  else
  {
    ...
    if ( v4 < 0 )
    {
      MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, v4, 0x2Fu);
    }
    else
    {
      ...
      else
      {
        ...
        if ( inserted < 0 )
        {
          MilInstrumentationCheckHR_MaybeFailFast(0x14u, 0LL, 0LL, inserted, 0x34u);
        }
        else
        {
