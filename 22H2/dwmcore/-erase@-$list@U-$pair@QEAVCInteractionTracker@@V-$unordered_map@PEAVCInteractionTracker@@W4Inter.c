...
_QWORD *__fastcall std::list<std::pair<CInteractionTracker * const,std::unordered_map<CInteractionTracker *,enum InteractionTrackerBindingModeFlags>>>::erase(
        __int64 a1,
        _QWORD *a2,
        void *a3)
{
  ...
  *(_QWORD *)(*(_QWORD *)a3 + 8LL) = *((_QWORD *)a3 + 1);
  --*(_QWORD *)(a1 + 8);
  std::vector<std::_List_unchecked_iterator<std::_List_val<std::_List_simple_types<std::pair<CInteractionTracker * const,enum InteractionTrackerBindingModeFlags>>>>>::_Tidy((__int64)a3 + 48);
  ...
  *v6[1] = 0LL;
  ...
  std::_Deallocate<16,0>(*((void **)a3 + 4), 0x20uLL);
