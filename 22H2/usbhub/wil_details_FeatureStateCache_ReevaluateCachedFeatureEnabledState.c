/*
 * XREFs of wil_details_FeatureStateCache_ReevaluateCachedFeatureEnabledState @ 0x1C001D4C8
 * Callers:
 *     wil_details_GetCurrentFeatureEnabledState @ 0x1C001D60C (wil_details_GetCurrentFeatureEnabledState.c)
 *     wil_details_IsEnabledFallback @ 0x1C001D790 (wil_details_IsEnabledFallback.c)
 * Callees:
 *     wil_details_GetCurrentFeatureEnabledState @ 0x1C001D60C (wil_details_GetCurrentFeatureEnabledState.c)
 *     _guard_dispatch_icall_nop @ 0x1C001DE80 (_guard_dispatch_icall_nop.c)
 */

__int64 __fastcall wil_details_FeatureStateCache_ReevaluateCachedFeatureEnabledState(
        volatile signed __int32 *a1,
        __int64 a2,
        __int64 a3)
{
  unsigned int v3; // r14d
  signed __int32 v5; // edi
  unsigned __int16 CurrentFeatureEnabledState; // bx
  signed __int32 v8; // esi
  signed __int32 v9; // eax
  int v11; // [rsp+50h] [rbp+30h] BYREF
  __int64 v12; // [rsp+58h] [rbp+38h]

  v3 = 0;
  v11 = 0;
  v12 = a2;
  v5 = a2;
  if ( WPP_MAIN_CB.Reserved )
    v3 = ((__int64 (*)(void))WPP_MAIN_CB.Reserved)();
  CurrentFeatureEnabledState = wil_details_GetCurrentFeatureEnabledState(a3, &v11);
  if ( !*(_BYTE *)(a3 + 28) )
    v11 = v3 != 0 ? v11 : 0;
  while ( 1 )
  {
    v8 = v5;
    LODWORD(v12) = v5;
    if ( v11 && (v5 & 2) == 0 )
    {
      v8 = v5 ^ (CurrentFeatureEnabledState ^ (unsigned __int16)v5) & 0x9C1 | 2;
      LODWORD(v12) = v8;
    }
    if ( (v5 & 4) == 0 )
    {
      v8 = ((unsigned __int16)v8 ^ CurrentFeatureEnabledState) & 0x400 ^ v8 | 4;
      LODWORD(v12) = v8;
    }
    v9 = _InterlockedCompareExchange(a1, v8, v5);
    if ( v5 == v9 )
      break;
    v5 = v9;
  }
  if ( (v5 & 4) == 0 && *(_QWORD *)&WPP_MAIN_CB.DeviceLock.Header.Lock )
    (*(void (__fastcall **)(volatile signed __int32 *, _QWORD, _QWORD))&WPP_MAIN_CB.DeviceLock.Header.Lock)(
      a1,
      *(unsigned __int8 *)(a3 + 28),
      v3);
  if ( !v11 )
    LODWORD(v12) = ((unsigned __int16)v8 ^ CurrentFeatureEnabledState) & 0x9C1 ^ v8;
  return v12;
}
