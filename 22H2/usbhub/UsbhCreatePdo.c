/*
 * XREFs of UsbhCreatePdo @ 0x1C00542B4
 * Callers:
...
__int64 __fastcall UsbhCreatePdo(__int64 a1, __int64 a2)
{
  ...
  do
  {
    ...
    if ( PdoName >= 0 )
    {
      PdoName = IoCreateDevice(UsbhDriverObject, 0xBF8u, &DeviceName, 0x22u, 0, 0, &DeviceObject);
      if ( PdoName >= 0 )
        break;
  ...
  *(_DWORD *)(v15 + 72) = 1145980532;
  PoolWithTag = (const WCHAR *)ExAllocatePoolWithTag(ProcessorHistory_high, 0x10uLL, 0x42554855u);
          ...
          else
          {
            ...
            else
            {
              ...
              *((_DWORD *)DeviceExtension + 684) = 0;
              ...
              LOBYTE(NewIrql) = KeAcquireSpinLockRaiseToDpc(&HubG);
              HIDWORD(NewIrql) = 1;
