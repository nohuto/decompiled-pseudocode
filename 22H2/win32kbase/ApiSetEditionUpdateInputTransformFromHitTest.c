/*
 * XREFs of ApiSetEditionUpdateInputTransformFromHitTest @ 0x1C00C9C00
 * Callers:
 *     ?ResolveDcompHitTestResultToInputDest@CSpatialProcessor@@CA?AVCInputDest@@AEBU_InputHitTestResult@@K_K@Z @ 0x1C0058BF4 (-ResolveDcompHitTestResultToInputDest@CSpatialProcessor@@CA-AVCInputDest@@AEBU_InputHitTestResul.c)
 * Callees:
 *     _guard_dispatch_icall_nop @ 0x1C00D6980 (_guard_dispatch_icall_nop.c)
 */

__int64 (*__fastcall ApiSetEditionUpdateInputTransformFromHitTest(
        __int64 a1,
        __int64 a2,
        __int64 a3,
        unsigned int a4,
        __int64 a5,
        __int64 a6))(void)
{
  __int64 (*result)(void); // rax

  result = qword_1C0296640;
  if ( qword_1C0296640 )
  {
    result = (__int64 (*)(void))qword_1C0296640();
    if ( (int)result >= 0 )
    {
      result = (__int64 (*)(void))qword_1C0296648;
      if ( qword_1C0296648 )
        return (__int64 (*)(void))qword_1C0296648(a1, a2, a3, a4, a5, a6);
    }
  }
  return result;
}
