void __fastcall sub_14003AC20(__int64 a1)
{
  char v1; // di
  __int64 *v3; // rbp
  _QWORD *Pool2; // rax
  __int64 *v5; // rax
  int v6; // esi
  void *v7; // r15
  void *v8; // rdi
  void *v9; // rax
  __int64 v10; // rax
  int v11; // edx
  void *v12; // rax
  volatile signed __int32 *v13; // rsi
  volatile signed __int32 *v14; // rdi
  void *v15; // r15
  void *v16; // rdi
  void *v17; // rax
  __int64 v18; // rax
  int v19; // edx
  void *v20; // rax
  volatile signed __int32 *v21; // rsi
  volatile signed __int32 *v22; // rdi
  _BYTE v23[8]; // [rsp+50h] [rbp-58h] BYREF
  volatile signed __int32 *v24; // [rsp+58h] [rbp-50h]
  _BYTE v25[8]; // [rsp+60h] [rbp-48h] BYREF
  volatile signed __int32 *v26; // [rsp+68h] [rbp-40h]
  _BYTE v27[8]; // [rsp+70h] [rbp-38h] BYREF
  volatile signed __int32 *v28; // [rsp+78h] [rbp-30h]
  _BYTE v29[8]; // [rsp+80h] [rbp-28h] BYREF
  volatile signed __int32 *v30; // [rsp+88h] [rbp-20h]

  v1 = 0;
  v3 = 0LL;
  if ( (HIDWORD(DeviceObject->Timer) & 0x200) != 0 && BYTE1(DeviceObject->Timer) >= 8u )
  {
    Pool2 = (_QWORD *)ExAllocatePool2(64LL, 40LL, 1970304114LL);
    if ( Pool2 )
    {
      v5 = sub_14000BBDC(Pool2, (__int64)L"product_e2fn::ProductE2fnMacPassthrough::EvtFirmwareInterruptAsserted", a1);
      if ( v5 )
        v3 = v5;
    }
  }
  v6 = *(_DWORD *)(*(_QWORD *)(*(_QWORD *)(a1 + 376) + 8LL) + 23380LL);
  if ( v6 == -1 )
  {
    if ( off_1400DF038 != (_UNKNOWN *)&off_1400DF038 )
    {
      v7 = &unk_1400D44E0;
      v8 = &unk_1400D44E0;
      v9 = *(void **)(*(_QWORD *)(*(__int64 (__fastcall **)(__int64, _BYTE *))(*(_QWORD *)(a1 + 56) + 16LL))(
                                   a1 + 56,
                                   v25)
                    + 16LL);
      if ( v9 )
        v8 = v9;
      v10 = (*(__int64 (__fastcall **)(__int64, _BYTE *))(*(_QWORD *)(a1 + 56) + 24LL))(a1 + 56, v23);
      LOBYTE(v11) = 2;
      v12 = *(void **)(*(_QWORD *)v10 + 16LL);
      if ( v12 )
        v7 = v12;
      sub_1400A3320(
        *(_QWORD *)(*(_QWORD *)(a1 + 176) + 16LL),
        v11,
        10,
        13,
        (__int64)&unk_1400D7110,
        (__int64)v7,
        (__int64)v8,
        (__int64)qword_1400B7720);
      v1 = 3;
    }
    if ( (v1 & 2) != 0 )
    {
      v13 = v24;
      v1 &= ~2u;
      if ( v24 )
      {
        if ( !_InterlockedDecrement(v24 + 2) )
        {
          (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v13 + 8LL))(v13);
          if ( !_InterlockedDecrement(v13 + 3) )
            (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v13 + 16LL))(v13);
        }
      }
    }
    if ( (v1 & 1) != 0 )
    {
      v14 = v26;
      if ( v26 )
      {
        if ( !_InterlockedDecrement(v26 + 2) )
        {
          (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v14 + 8LL))(v14);
          if ( !_InterlockedDecrement(v14 + 3) )
            (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v14 + 16LL))(v14);
        }
      }
    }
  }
  else if ( (v6 & 0x1F80000) == 0x1380000 )
  {
    if ( off_1400DF038 != (_UNKNOWN *)&off_1400DF038 )
    {
      v15 = &unk_1400D44E0;
      v16 = &unk_1400D44E0;
      v17 = *(void **)(*(_QWORD *)(*(__int64 (__fastcall **)(__int64, _BYTE *))(*(_QWORD *)(a1 + 56) + 16LL))(
                                    a1 + 56,
                                    v29)
                     + 16LL);
      if ( v17 )
        v16 = v17;
      v18 = (*(__int64 (__fastcall **)(__int64, _BYTE *))(*(_QWORD *)(a1 + 56) + 24LL))(a1 + 56, v27);
      LOBYTE(v19) = 4;
      v20 = *(void **)(*(_QWORD *)v18 + 16LL);
      if ( v20 )
        v15 = v20;
      sub_1400A5E80(
        *(_QWORD *)(*(_QWORD *)(a1 + 176) + 16LL),
        v19,
        10,
        14,
        (__int64)&unk_1400D7110,
        (__int64)v15,
        (__int64)v16,
        (__int64)qword_1400B7720,
        v6);
      v1 = 12;
    }
    if ( (v1 & 8) != 0 )
    {
      v21 = v28;
      v1 &= ~8u;
      if ( v28 )
      {
        if ( _InterlockedExchangeAdd(v28 + 2, 0xFFFFFFFF) == 1 )
        {
          (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v21 + 8LL))(v21);
          if ( _InterlockedExchangeAdd(v21 + 3, 0xFFFFFFFF) == 1 )
            (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v21 + 16LL))(v21);
        }
      }
    }
    if ( (v1 & 4) != 0 )
    {
      v22 = v30;
      if ( v30 )
      {
        if ( _InterlockedExchangeAdd(v30 + 2, 0xFFFFFFFF) == 1 )
        {
          (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v22 + 8LL))(v22);
          if ( _InterlockedExchangeAdd(v22 + 3, 0xFFFFFFFF) == 1 )
            (*(void (__fastcall **)(volatile signed __int32 *))(*(_QWORD *)v22 + 16LL))(v22);
        }
      }
    }
    (*(void (__fastcall **)(_QWORD))(**(_QWORD **)(a1 + 360) + 8LL))(*(_QWORD *)(a1 + 360));
  }
  if ( v3 )
  {
    sub_14000BFA4(v3);
    ExFreePool(v3);
  }
}
