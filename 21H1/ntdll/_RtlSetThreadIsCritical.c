...
NTSTATUS __cdecl RtlSetThreadIsCritical(unsigned __int8 a1, _BYTE *a2, char a3)
{
  NTSTATUS result; // eax
  ...
  if ( a2 )
  {
    result = NtQueryInformationThread((HANDLE)0xFFFFFFFE, (THREADINFOCLASS)18, &ThreadInformation, 4u, 0);
    if ( result < 0 )
      return result;
    *a2 = ThreadInformation;
  }
  ThreadInformation = a1;
  return ZwSetInformationThread(-2, 18, (int)&ThreadInformation, 4);
}
