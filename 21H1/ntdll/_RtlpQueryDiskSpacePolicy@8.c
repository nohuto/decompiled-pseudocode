...
NTSTATUS __fastcall RtlpQueryDiskSpacePolicy(int a1, _DWORD *a2)
{
  NTSTATUS DiskSpacePolicyByHandle; // esi
  int v5; // [esp-8h] [ebp-250h]
  struct _IO_STATUS_BLOCK IoStatusBlock; // [esp+Ch] [ebp-23Ch] BYREF
  _BYTE v8[8]; // [esp+14h] [ebp-234h] BYREF
  OBJECT_ATTRIBUTES ObjectAttributes; // [esp+1Ch] [ebp-22Ch] BYREF
  ...
  {
    StringCbPrintfW(pszDest, 0x208u, L"\\??\\%s");
    RtlUnicodeStringInitWorker((int)v8, pszDest, v5, a1);
