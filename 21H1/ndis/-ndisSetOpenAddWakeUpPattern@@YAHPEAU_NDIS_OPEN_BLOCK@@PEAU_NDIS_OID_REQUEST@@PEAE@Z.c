/*
 * XREFs of ?ndisSetOpenAddWakeUpPattern@@YAHPEAU_NDIS_OPEN_BLOCK@@PEAU_NDIS_OID_REQUEST@@PEAE@Z @ 0x1C00945E0
 * Callers:
  ...
  else
  {
    ...
    if ( (unsigned int)a2 >= 0x18 )
    {
      ...
      if ( !(_DWORD)MaskSize
        ...
        || (v12 = PatternOffset + InformationBuffer->PatternSize, v12 < (unsigned int)PatternOffset) )
      {
LABEL_8:
        v6 = -1073676267;
        goto LABEL_28;
      ...
      if ( (unsigned int)a2 >= v12 )
      {
        ...
        {
          if ( InformationBuffer->PatternSize == LODWORD(i[8].Next) && InformationBuffer->MaskSize == LODWORD(i[7].Next) )
          {
      ...
      v8 = PatternOffset + InformationBuffer->PatternSize;
    }
