/*
 * XREFs of ndisQueuePeriodicReceivesWorkItem @ 0x1C00A3764
 * Callers:
  ...
  if ( !_InterlockedExchange(&a1->PeriodicReceiveQueue.WorkItemQueued, 1) )
  {
    if ( HIBYTE(word_1C00E6054) )
    {
      ...
      *(GUID *)&v6[6] = EtwGuidNdisReceive;
      BYTE2(v5) = 24;
      *(_QWORD *)((char *)&v5 + 6) = qword_1C00E6048;
      IoWMIWriteEvent(&v4);
    }
    ndisReferenceMiniportNoCheck(a1, 0x4Cu);
    ndisInsertInWorkQueue(
      (struct _NDIS_WORK_QUEUE *)((char *)qword_1C00E4000 + 64 * (unsigned __int64)KeGetPcr()->Prcb.Number),
