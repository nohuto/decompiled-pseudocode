/*
 * XREFs of ?ndisVerifierInitialization@@YAEXZ @ 0x1C0039D74
 * Callers:
  ...
  if ( MmIsVerifierEnabled(&VerifierFlags) >= 0 )
  {
    ...
    if ( ndisVerifierWdmDispatch )
    {
      ...
      if ( ndisVerifierNdisDispatch )
      {
        ...
        if ( MmAddVerifierThunks(&off_1C00C8D20, 0x190u) >= 0 )
        {
          qword_1C00E6190 = (__int64)&ndisMiniportTrackAllocList;
          ndisMiniportTrackAllocList.Flink = &ndisMiniportTrackAllocList;
          qword_1C00E6180 = (__int64)&ndisDriverTrackAllocList;
