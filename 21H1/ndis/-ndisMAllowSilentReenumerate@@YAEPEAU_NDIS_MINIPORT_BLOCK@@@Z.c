/*
 * XREFs of ?ndisMAllowSilentReenumerate@@YAEPEAU_NDIS_MINIPORT_BLOCK@@@Z @ 0x1C011DD40
 * Callers:
...
bool __fastcall ndisMAllowSilentReenumerate(struct _NDIS_MINIPORT_BLOCK *a1)
{
  ...
  if ( IoOpenDeviceRegistryKey(PhysicalDeviceObject, 2u, 0xC2000000, &DeviceRegKey) >= 0 )
  {
    ...
    ValueUlong = KRegKey::QueryValueUlong(
                   (KRegKey *)&KeyHandle,
                   (struct _UNICODE_STRING *)&stru_1C00C92B0,
                   (unsigned int *)&Data);
    v2 = KeyHandle;
    if ( ValueUlong >= 0 )
    {
      if ( (_DWORD)Data )
      {
        ...
        if ( ZwSetValueKey(KeyHandle, (PUNICODE_STRING)&stru_1C00C92B0, 0, 4u, &Data, 4u) >= 0 )
          v4 = 1;
      }
    }
  }
