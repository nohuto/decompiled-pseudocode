...
void WdmlibProcgrpInitialize(void)
{
  ...
  if ( _InterlockedCompareExchange(&WdmlibProcgrpInitState, 1, 0) <= 0 )
  {
    ...
    WPP_GLOBAL_WDF_Control.Dpc.DpcData = MmGetSystemRoutineAddress(&ApiName);
    if ( !WPP_GLOBAL_WDF_Control.Dpc.DpcData )
      WPP_GLOBAL_WDF_Control.Dpc.DpcData = ProcgrpQueryMaximumProcessorCount;
    ...
    if ( !WPP_GLOBAL_WDF_Control.SecurityDescriptor )
      WPP_GLOBAL_WDF_Control.SecurityDescriptor = ProcgrpSetSystemAffinityThreadEx;
    ...
    *(_QWORD *)&WPP_GLOBAL_WDF_Control.ActiveThreadCount = MmGetSystemRoutineAddress(&ApiName);
    if ( !*(_QWORD *)&WPP_GLOBAL_WDF_Control.ActiveThreadCount )
      *(_QWORD *)&WPP_GLOBAL_WDF_Control.ActiveThreadCount = ProcgrpRevertToUserAffinityThreadEx;
