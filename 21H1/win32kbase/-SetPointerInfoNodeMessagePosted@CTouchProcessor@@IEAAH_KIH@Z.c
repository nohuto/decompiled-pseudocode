/*
 * XREFs of ?SetPointerInfoNodeMessagePosted@CTouchProcessor@@IEAAH_KIH@Z @ 0x1C01A1398
 * Callers:
 *     ?MarkPointerMessageGenerated@CTouchProcessor@@QEAAXI_JHPEAX@Z @ 0x1C019B3C0 (-MarkPointerMessageGenerated@CTouchProcessor@@QEAAXI_JHPEAX@Z.c)
 * Callees:
 *     WPP_RECORDER_SF_ @ 0x1C00678DC (WPP_RECORDER_SF_.c)
 *     ?FindFrameById@CTouchProcessor@@AEAAPEAUCPointerInputFrame@@K@Z @ 0x1C01938C8 (-FindFrameById@CTouchProcessor@@AEAAPEAUCPointerInputFrame@@K@Z.c)
 *     ?LookupNode@CTouchProcessor@@AEAAPEAUCPointerInfoNode@@PEBUCPointerInputFrame@@K@Z @ 0x1C019B35C (-LookupNode@CTouchProcessor@@AEAAPEAUCPointerInfoNode@@PEBUCPointerInputFrame@@K@Z.c)
 *     IsPointerInputMessageWithState @ 0x1C01A42B8 (IsPointerInputMessageWithState.c)
 *     MicrosoftTelemetryAssertTriggeredNoArgsKM @ 0x1C02015EC (MicrosoftTelemetryAssertTriggeredNoArgsKM.c)
 */

__int64 __fastcall CTouchProcessor::SetPointerInfoNodeMessagePosted(
        struct _KTHREAD **this,
        __int64 a2,
        unsigned int a3,
        int a4)
{
  __int64 v6; // rsi
  int v8; // edx
  __int64 v9; // rcx
  PDEVICE_OBJECT v10; // rcx
  int v11; // r9d
  struct CPointerInputFrame *FrameById; // rax
  CTouchProcessor *v13; // rcx
  struct CPointerInputFrame *v14; // rbp
  struct CPointerInfoNode *v16; // rax
  int v17; // edx
  __int64 v18; // rcx
  int *v19; // rbx
  __int64 v20; // rcx
  __int64 v21; // rcx
  int v22; // esi
  int v23; // edi
  int v24; // eax
  int v25; // eax
  int v26; // eax
  unsigned int v27; // eax
  __int64 v28; // rcx
  __int64 v29; // rcx

  v6 = a2;
  if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED && LOWORD(WPP_GLOBAL_Control->DeviceType) )
  {
    LOBYTE(a2) = 5;
    WPP_RECORDER_SF_(
      WPP_GLOBAL_Control->DeviceExtension,
      a2,
      7,
      234,
      (__int64)&WPP_6cc13e1a12c13a444721acaa10d1d35b_Traceguids);
  }
  if ( !(unsigned int)IsPointerInputMessageWithState(a3) )
    MicrosoftTelemetryAssertTriggeredNoArgsKM(v9);
  if ( !v6 )
  {
    if ( WPP_RECORDER_INITIALIZED == (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
      return 0LL;
    v10 = WPP_GLOBAL_Control;
    if ( !LOWORD(WPP_GLOBAL_Control->DeviceType) )
      return 0LL;
    v11 = 235;
LABEL_14:
    LOBYTE(v8) = 5;
    WPP_RECORDER_SF_(v10->DeviceExtension, v8, 7, v11, (__int64)&WPP_6cc13e1a12c13a444721acaa10d1d35b_Traceguids);
    return 0LL;
  }
  FrameById = CTouchProcessor::FindFrameById(this, *(_DWORD *)(v6 + 28));
  v14 = FrameById;
  if ( !FrameById )
  {
    if ( WPP_RECORDER_INITIALIZED == (_UNKNOWN *)&WPP_RECORDER_INITIALIZED )
      return 0LL;
    v10 = WPP_GLOBAL_Control;
    if ( !LOWORD(WPP_GLOBAL_Control->DeviceType) )
      return 0LL;
    v11 = 236;
    goto LABEL_14;
  }
  v16 = CTouchProcessor::LookupNode(v13, FrameById, *(_DWORD *)(v6 + 32));
  v18 = *(unsigned __int16 *)(v6 + 16);
  v19 = (int *)v16;
  if ( *((_WORD *)v16 + 86) != (_WORD)v18 )
    MicrosoftTelemetryAssertTriggeredNoArgsKM(v18);
  v20 = *((unsigned int *)v14 + 12);
  if ( v19[2] >= (unsigned int)v20 )
    MicrosoftTelemetryAssertTriggeredNoArgsKM(v20);
  switch ( a3 )
  {
    case 0x249u:
      v17 = *v19;
      v21 = (unsigned int)*v19;
      if ( (*v19 & 8) == 0 && (v17 & 0x40) == 0 && (v19[45] & 1) == 0 )
      {
        MicrosoftTelemetryAssertTriggeredNoArgsKM(v21);
        v21 = (unsigned int)*v19;
      }
      v22 = 0x800000;
      v23 = 0x1000000;
      if ( (v21 & 0x800000) != 0 || (v21 & 0x1000000) != 0 || (v21 & 0x4000000) != 0 || (v21 & 0x2000000) != 0 )
      {
        MicrosoftTelemetryAssertTriggeredNoArgsKM(v21);
        LODWORD(v21) = *v19;
      }
      v24 = a4 << 24;
LABEL_31:
      v25 = v22 | v21 ^ v23 & (v21 ^ v24);
LABEL_61:
      *v19 = v25;
      goto LABEL_62;
    case 0x24Au:
      v21 = (unsigned int)*v19;
      v22 = 0x8000000;
      v23 = 0x10000000;
      if ( (v21 & 0x8000000) != 0 || (v21 & 0x10000000) != 0 || (v21 & 0x40000000) != 0 || (v21 & 0x20000000) != 0 )
      {
        MicrosoftTelemetryAssertTriggeredNoArgsKM(v21);
        LODWORD(v21) = *v19;
      }
      v24 = a4 << 28;
      goto LABEL_31;
    case 0x251u:
      v26 = *v19;
      if ( (*v19 & 0x40) == 0 )
      {
        MicrosoftTelemetryAssertTriggeredNoArgsKM(v20);
        v26 = *v19;
      }
      if ( v26 < 0 || (v20 = (unsigned int)v19[1], (v20 & 1) != 0) || (v20 & 4) != 0 || (v20 & 2) != 0 )
      {
        MicrosoftTelemetryAssertTriggeredNoArgsKM(v20);
        v26 = *v19;
        LODWORD(v20) = v19[1];
      }
      *v19 = v26 | 0x80000000;
      v27 = v20 ^ ((unsigned __int8)a4 ^ (unsigned __int8)v20) & 1;
      break;
    case 0x252u:
      v28 = (unsigned int)v19[1];
      if ( (v28 & 8) != 0 || (v28 & 0x10) != 0 || (v28 & 0x40) != 0 || (v28 & 0x20) != 0 )
      {
        MicrosoftTelemetryAssertTriggeredNoArgsKM(v28);
        LODWORD(v28) = v19[1];
      }
      v27 = v28 ^ ((unsigned __int8)v28 ^ (unsigned __int8)(16 * a4)) & 0x10 | 8;
      break;
    default:
      v29 = (unsigned int)*v19;
      if ( (v29 & 0x80000) != 0 || (v29 & 0x400000) != 0 || (v29 & 0x200000) != 0 )
      {
        MicrosoftTelemetryAssertTriggeredNoArgsKM(v29);
        LODWORD(v29) = *v19;
      }
      v25 = v29 ^ (v29 ^ (a4 << 20)) & 0x100000 | 0x80000;
      goto LABEL_61;
  }
  v19[1] = v27;
LABEL_62:
  if ( a4 )
    *(_DWORD *)(160LL * (unsigned int)v19[2] + *((_QWORD *)v14 + 18) + 140) |= 8u;
  if ( WPP_RECORDER_INITIALIZED != (_UNKNOWN *)&WPP_RECORDER_INITIALIZED && LOWORD(WPP_GLOBAL_Control->DeviceType) )
  {
    LOBYTE(v17) = 5;
    WPP_RECORDER_SF_(
      WPP_GLOBAL_Control->DeviceExtension,
      v17,
      7,
      237,
      (__int64)&WPP_6cc13e1a12c13a444721acaa10d1d35b_Traceguids);
  }
  return 1LL;
}
