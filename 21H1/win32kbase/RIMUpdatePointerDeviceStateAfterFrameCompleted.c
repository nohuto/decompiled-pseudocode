/*
 * XREFs of RIMUpdatePointerDeviceStateAfterFrameCompleted @ 0x1C017FAE8
 * Callers:
 *     RIMAbandonPointerDeviceFrame @ 0x1C01793D0 (RIMAbandonPointerDeviceFrame.c)
 *     RIMCompletePointerDeviceFrame @ 0x1C01794B0 (RIMCompletePointerDeviceFrame.c)
 * Callees:
 *     RIMRemoveFromActiveDevices @ 0x1C0167FE0 (RIMRemoveFromActiveDevices.c)
 *     RIMUpdatePrimaryDevice @ 0x1C0168794 (RIMUpdatePrimaryDevice.c)
 *     RIMResetPointerDevicePrimaryContact @ 0x1C017F974 (RIMResetPointerDevicePrimaryContact.c)
 *     RIMCmActiveContactsBegin @ 0x1C01864D8 (RIMCmActiveContactsBegin.c)
 *     RIMCmActiveContactsEnd @ 0x1C01865F0 (RIMCmActiveContactsEnd.c)
 *     RIMCmActiveContactsNext @ 0x1C0186644 (RIMCmActiveContactsNext.c)
 *     RIMCmDeactivateContact @ 0x1C018693C (RIMCmDeactivateContact.c)
 *     RIMCmIsContactDeliveringPointerData @ 0x1C0186B1C (RIMCmIsContactDeliveringPointerData.c)
 *     RIMCmResetContactFrameState @ 0x1C0186C84 (RIMCmResetContactFrameState.c)
 *     ApiSetDwmAsyncNotifyDigitizerActiveContacts @ 0x1C01CCCDC (ApiSetDwmAsyncNotifyDigitizerActiveContacts.c)
 *     MicrosoftTelemetryAssertTriggeredNoArgsKM @ 0x1C02015EC (MicrosoftTelemetryAssertTriggeredNoArgsKM.c)
 */

__int64 __fastcall RIMUpdatePointerDeviceStateAfterFrameCompleted(__int64 a1, __int64 a2, int a3, int a4)
{
  __int64 v4; // rdi
  __int64 v8; // rbx
  __int64 v9; // rax
  __int64 active; // rax
  __int64 v11; // xmm1_8
  __int64 v12; // rax
  __int128 v13; // xmm1
  __int64 v14; // xmm0_8
  __int64 v15; // rax
  __int64 result; // rax
  __int64 v17; // rbx
  __int64 v18; // rcx
  int v19; // edx
  __int64 v20; // rcx
  __int64 v21; // rdx
  int v22; // eax
  __int128 v23; // xmm1
  __int128 v24; // xmm0
  __int128 v25; // xmm1
  __int128 v26; // xmm0
  __int128 v27; // xmm1
  __int128 v28; // xmm0
  __int128 v29; // xmm1
  __int128 v30; // xmm1
  __int128 v31; // xmm0
  __int128 v32; // xmm1
  __int128 v33; // xmm0
  __int128 v34; // xmm1
  __int128 v35; // xmm0
  __int64 v36; // rcx
  __int128 v37; // [rsp+20h] [rbp-50h] BYREF
  __int64 v38; // [rsp+30h] [rbp-40h]
  __int128 v39; // [rsp+38h] [rbp-38h] BYREF
  __int64 v40; // [rsp+48h] [rbp-28h]
  _BYTE v41[32]; // [rsp+50h] [rbp-20h] BYREF

  v4 = *(_QWORD *)(a2 + 480);
  v8 = *(_QWORD *)(v4 + 712);
  v9 = v8;
  if ( a3 && a4 )
  {
    MicrosoftTelemetryAssertTriggeredNoArgsKM(a1);
    v9 = *(_QWORD *)(v4 + 712);
  }
  if ( v9 )
    *(_DWORD *)(v8 + 36) = 0;
  active = RIMCmActiveContactsBegin(&v39, v4);
  v11 = *(_QWORD *)(active + 16);
  v37 = *(_OWORD *)active;
  v38 = v11;
  while ( 1 )
  {
    v12 = RIMCmActiveContactsEnd(v41, v4);
    v13 = *(_OWORD *)v12;
    v14 = *(_QWORD *)(v12 + 16);
    v15 = *(_QWORD *)v12;
    v40 = v14;
    v39 = v13;
    if ( (_QWORD)v37 == v15 )
    {
      result = DWORD2(v39);
      if ( DWORD2(v37) == DWORD2(v39) && v38 == v40 )
        break;
    }
    v17 = v38 - 16;
    if ( (*(_DWORD *)(v38 + 16) & 2) == 0 )
      __int2c();
    RIMCmActiveContactsNext(v4, &v37);
    if ( a3 && (*(_DWORD *)(v17 + 2340) & 1) != 0 )
    {
      if ( (*(_DWORD *)(v17 + 2660) & 4) == 0 )
      {
        v19 = *(_DWORD *)(v17 + 2420);
        if ( (v19 & 4) != 0 )
        {
          v20 = *(unsigned int *)(v17 + 32);
          if ( (v20 & 4) == 0 && (v19 & 0x10000) == 0 )
          {
            MicrosoftTelemetryAssertTriggeredNoArgsKM(v20);
            v20 = *(unsigned int *)(v17 + 32);
          }
          v21 = (unsigned int)(*(_DWORD *)(a1 + 468) + 1);
          *(_DWORD *)(a1 + 468) = v21;
          if ( (v20 & 4) == 0 )
            ++*(_DWORD *)(a1 + 472);
          goto LABEL_28;
        }
      }
      if ( (*(_DWORD *)(v17 + 2660) & 4) != 0 )
      {
        v22 = *(_DWORD *)(v17 + 2420);
        if ( (v22 & 4) == 0 )
        {
          v20 = *(unsigned int *)(v17 + 32);
          if ( (v20 & 4) == 0 && (v22 & 0x40000) == 0 )
          {
            MicrosoftTelemetryAssertTriggeredNoArgsKM(v20);
            v20 = *(unsigned int *)(v17 + 32);
          }
          v21 = (unsigned int)(*(_DWORD *)(a1 + 468) - 1);
          *(_DWORD *)(a1 + 468) = v21;
          if ( (v20 & 4) == 0 )
            --*(_DWORD *)(a1 + 472);
LABEL_28:
          ApiSetDwmAsyncNotifyDigitizerActiveContacts(v20, v21);
        }
      }
      if ( (*(_DWORD *)(v17 + 32) & 8) != 0 && (*(_DWORD *)(v17 + 2660) & 2) != 0 && (*(_DWORD *)(v17 + 2420) & 2) == 0 )
      {
        if ( *(_QWORD *)(v4 + 960) != v17 )
          MicrosoftTelemetryAssertTriggeredNoArgsKM(v18);
        RIMResetPointerDevicePrimaryContact(v4);
      }
      v23 = *(_OWORD *)(v17 + 2360);
      *(_OWORD *)(v17 + 2584) = *(_OWORD *)(v17 + 2344);
      v24 = *(_OWORD *)(v17 + 2376);
      *(_OWORD *)(v17 + 2600) = v23;
      v25 = *(_OWORD *)(v17 + 2392);
      *(_OWORD *)(v17 + 2616) = v24;
      v26 = *(_OWORD *)(v17 + 2408);
      *(_OWORD *)(v17 + 2632) = v25;
      v27 = *(_OWORD *)(v17 + 2424);
      *(_OWORD *)(v17 + 2648) = v26;
      v28 = *(_OWORD *)(v17 + 2440);
      *(_OWORD *)(v17 + 2664) = v27;
      v29 = *(_OWORD *)(v17 + 2456);
      *(_OWORD *)(v17 + 2680) = v28;
      v18 = v17 + 2712;
      *(_OWORD *)(v17 + 2696) = v29;
      v30 = *(_OWORD *)(v17 + 2488);
      *(_OWORD *)(v17 + 2712) = *(_OWORD *)(v17 + 2472);
      v31 = *(_OWORD *)(v17 + 2504);
      *(_OWORD *)(v17 + 2728) = v30;
      v32 = *(_OWORD *)(v17 + 2520);
      *(_OWORD *)(v17 + 2744) = v31;
      v33 = *(_OWORD *)(v17 + 2536);
      *(_OWORD *)(v17 + 2760) = v32;
      v34 = *(_OWORD *)(v17 + 2552);
      *(_OWORD *)(v17 + 2776) = v33;
      v35 = *(_OWORD *)(v17 + 2568);
      *(_OWORD *)(v17 + 2792) = v34;
      *(_OWORD *)(v17 + 2808) = v35;
    }
    if ( a4 || (*(_DWORD *)(v17 + 2340) & 4) == 0 )
    {
      RIMCmResetContactFrameState(v17);
    }
    else
    {
      if ( (*(_DWORD *)(v17 + 32) & 8) != 0 )
      {
        if ( *(_QWORD *)(v4 + 960) != v17 )
          MicrosoftTelemetryAssertTriggeredNoArgsKM(v18);
        RIMResetPointerDevicePrimaryContact(v4);
      }
      if ( (unsigned int)RIMCmIsContactDeliveringPointerData(v17) )
      {
        if ( (*(_DWORD *)(v17 + 32) & 4) == 0 )
          MicrosoftTelemetryAssertTriggeredNoArgsKM(v36);
      }
      RIMCmDeactivateContact(v4, v17);
    }
  }
  if ( !a4 )
  {
    if ( v4 == *(_QWORD *)(a1 + 480) && !*(_DWORD *)(v4 + 956) )
      result = RIMUpdatePrimaryDevice(a1, 0LL);
    if ( !*(_DWORD *)(v4 + 952) )
    {
      RIMRemoveFromActiveDevices(a1, v4, 0LL, 0LL);
      result = 0LL;
      *(_OWORD *)(v4 + 784) = 0LL;
      *(_OWORD *)(v4 + 800) = 0LL;
      *(_OWORD *)(v4 + 816) = 0LL;
      *(_QWORD *)(v4 + 832) = 0LL;
    }
  }
  return result;
}
