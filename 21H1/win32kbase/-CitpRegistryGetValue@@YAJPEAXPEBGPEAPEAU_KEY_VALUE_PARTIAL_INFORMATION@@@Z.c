/*
 * XREFs of ?CitpRegistryGetValue@@YAJPEAXPEBGPEAPEAU_KEY_VALUE_PARTIAL_INFORMATION@@@Z @ 0x1C00290F4
 * Callers:
 *     ?CitpParameterGetString@@YAJPEAXPEBGPEAPEBG@Z @ 0x1C0028AEC (-CitpParameterGetString@@YAJPEAXPEBGPEAPEBG@Z.c)
 * Callees:
 *     Win32FreePool @ 0x1C008C9A0 (Win32FreePool.c)
 *     Win32AllocPool @ 0x1C008D7C0 (Win32AllocPool.c)
 *     ?CitpLogFailureWorker@@YAXJPEBDI@Z @ 0x1C01FF578 (-CitpLogFailureWorker@@YAXJPEBDI@Z.c)
 */

__int64 __fastcall CitpRegistryGetValue(
        HANDLE KeyHandle,
        const unsigned __int16 *a2,
        struct _KEY_VALUE_PARTIAL_INFORMATION **a3)
{
  struct _KEY_VALUE_PARTIAL_INFORMATION *v5; // rdi
  NTSTATUS v6; // eax
  unsigned int v7; // ebx
  struct _KEY_VALUE_PARTIAL_INFORMATION *v8; // rax
  const char *v9; // rdx
  NTSTATUS v10; // eax
  const char *v11; // rdx
  struct _UNICODE_STRING ValueName; // [rsp+30h] [rbp-18h] BYREF
  const unsigned __int16 *ResultLength; // [rsp+58h] [rbp+10h] BYREF

  ResultLength = a2;
  ValueName = 0LL;
  v5 = 0LL;
  RtlInitUnicodeString(&ValueName, L"BuildLabEx");
  LODWORD(ResultLength) = 0;
  v6 = ZwQueryValueKey(KeyHandle, &ValueName, KeyValuePartialInformation, 0LL, 0, (PULONG)&ResultLength);
  v7 = v6;
  if ( (v6 == -1073741789 || v6 == -2147483643) && (_DWORD)ResultLength )
  {
    v8 = (struct _KEY_VALUE_PARTIAL_INFORMATION *)Win32AllocPool((unsigned int)ResultLength, 1231254357LL);
    v5 = v8;
    if ( !v8 )
    {
      v7 = -1073741670;
      CitpLogFailureWorker(-1073741670, v9, 0x8ABu);
      return v7;
    }
    v10 = ZwQueryValueKey(
            KeyHandle,
            &ValueName,
            KeyValuePartialInformation,
            v8,
            (ULONG)ResultLength,
            (PULONG)&ResultLength);
    v7 = v10;
    if ( v10 >= 0 )
    {
      *a3 = v5;
      return 0;
    }
    CitpLogFailureWorker(v10, v11, 0x8B7u);
  }
  else
  {
    if ( v6 < 0 )
      return v7;
    v7 = -1073741823;
  }
  if ( v5 )
    Win32FreePool(v5);
  return v7;
}
