...
__int64 __fastcall GreGetDeviceCaps(HDC a1, int a2)
{
  ...
  if ( (unsigned int)PsGetWin32KFilterSet(a1) == 5 && a1 == (HDC)-589410304LL )
  {
    ...
    if ( v19 )
    {
      DeviceCapsInternal = GreGetDeviceCapsInternal((struct PDEVOBJ *)&v19, a2);
  ...
  else
  {
    ...
    if ( v18[0] )
    {
      ...
      if ( v20 )
      {
        DeviceCapsInternal = GreGetDeviceCapsInternal((struct PDEVOBJ *)&v20, a2);
        NEEDDYNAMICMODECHANGESHARELOCK::~NEEDDYNAMICMODECHANGESHARELOCK(
          (NEEDDYNAMICMODECHANGESHARELOCK *)&v19,
          v16,
          v17);
