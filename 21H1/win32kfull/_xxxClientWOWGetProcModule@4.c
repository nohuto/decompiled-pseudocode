/*
 * XREFs of _xxxClientWOWGetProcModule@4 @ 0xA1B00
 * Callers:
...
int __thiscall xxxClientWOWGetProcModule(void *this)
{
  int v1; // esi
  int *v2; // ecx
  int result; // eax
  ...
  EtwTraceBeginCallback(90);
  v1 = KeUserModeCallback(90, &v4, 4, &v5, &v6);
  EtwTraceEndCallback(90);
  LeaveEnterCritProperDisposition::~LeaveEnterCritProperDisposition((LeaveEnterCritProperDisposition *)&v8);
  ReleaseAndReacquirePerObjectLocks::~ReleaseAndReacquirePerObjectLocks((ReleaseAndReacquirePerObjectLocks *)&v7);
  if ( v1 < 0 || v6 != 12 )
    return 0;
