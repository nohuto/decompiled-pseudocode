/*
 * XREFs of _xxxClientBroadcastThemeChange@8 @ 0xE79A4
 * Callers:
...
int __fastcall xxxClientBroadcastThemeChange(int a1, int a2)
{
  int v2; // esi
  int *v3; // ecx
  int result; // eax
  ...
  EtwTraceBeginCallback(121);
  v2 = KeUserModeCallback(121, v5, 8, &v6, &v7);
  EtwTraceEndCallback(121);
  LeaveEnterCritProperDisposition::~LeaveEnterCritProperDisposition((LeaveEnterCritProperDisposition *)&v9);
  ReleaseAndReacquirePerObjectLocks::~ReleaseAndReacquirePerObjectLocks((ReleaseAndReacquirePerObjectLocks *)&v8);
  if ( v2 < 0 || v7 != 12 )
    return 0;
