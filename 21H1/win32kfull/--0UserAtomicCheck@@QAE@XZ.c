/*
 * XREFs of ??0UserAtomicCheck@@QAE@XZ @ 0x2C3CC
 * Callers:
...
UserAtomicCheck *__thiscall UserAtomicCheck::UserAtomicCheck(UserAtomicCheck *this)
{
  UserAtomicCheck *v1; // edi
  int v3; // ecx
  _DWORD *i; // eax
  int v5; // ebx
  int v6; // edx
  ...
  *(_BYTE *)this = 0;
  if ( _gpresUser )
  {
    if ( UserIsUserCritSecInExclusive() )
    {
      ...
      {
        v3 = 0;
        *((_DWORD *)v1 + 1) = ++_gdwAtomicCheckSerial;
        if ( _gdwAtomicCheckLogSize )
        {
          for ( i = (_DWORD *)_gpAtomickCheckStacks; *i; i += 9 )
          {
            if ( (unsigned int)++v3 >= _gdwAtomicCheckLogSize )
              return v1;
          }
          v5 = 36 * v3;
          *(_DWORD *)(v5 + _gpAtomickCheckStacks) = _gdwAtomicCheckSerial;
          *(_DWORD *)(_gpAtomickCheckStacks + v5 + 4) = PsGetCurrentThreadId();
          ...
          if ( (unsigned int)MEMORY[0xFFDF0004] < 0x1000000 )
          {
            ...
            v7 = MEMORY[0xFFDF0004] * (v8 << 8) + (((unsigned int)v6 * (unsigned __int64)v9) >> 24);
          }
          else
          {
            v7 = ((unsigned int)MEMORY[0xFFDF0004] * (unsigned __int64)MEMORY[0xFFDF0320]) >> 24;
          }
          *(_DWORD *)(_gpAtomickCheckStacks + v5 + 8) = v7;
          RtlWalkFrameChain((PVOID *)(v5 + _gpAtomickCheckStacks + 12), 6u, 0x200u);
        }
      }
    }
  }
