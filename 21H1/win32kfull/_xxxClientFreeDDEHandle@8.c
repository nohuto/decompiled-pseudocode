/*
 * XREFs of _xxxClientFreeDDEHandle@8 @ 0x19428F
 * Callers:
...
int __fastcall xxxClientFreeDDEHandle(int a1, int a2)
{
  int v2; // esi
  int *v3; // ecx
  int result; // eax
  ...
  EtwTraceBeginCallback(67);
  v2 = KeUserModeCallback(67, v5, 8, &v6, &v7);
  EtwTraceEndCallback(67);
  LeaveEnterCritProperDisposition::~LeaveEnterCritProperDisposition((LeaveEnterCritProperDisposition *)&v9);
  ReleaseAndReacquirePerObjectLocks::~ReleaseAndReacquirePerObjectLocks((ReleaseAndReacquirePerObjectLocks *)&v8);
  if ( v2 < 0 || v7 != 12 )
    return 0;
