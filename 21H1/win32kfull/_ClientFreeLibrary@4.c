/*
 * XREFs of _ClientFreeLibrary@4 @ 0xD2F16
 * Callers:
...
int __thiscall ClientFreeLibrary(void *this)
{
  int v1; // esi
  int *v2; // ecx
  int result; // eax
  ...
  EtwTraceBeginCallback(68);
  v1 = KeUserModeCallback(68, &v4, 4, &v5, &v6);
  EtwTraceEndCallback(68);
  LeaveEnterCritProperDisposition::~LeaveEnterCritProperDisposition((LeaveEnterCritProperDisposition *)&v8);
  ReleaseAndReacquirePerObjectLocks::~ReleaseAndReacquirePerObjectLocks((ReleaseAndReacquirePerObjectLocks *)&v7);
  if ( v1 < 0 || v6 != 12 )
    return 0;
