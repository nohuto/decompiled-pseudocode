/*
 * XREFs of ?Free@?$CSectionBitmapAllocator@$0FAAA@$0EI@@NSInstrumentation@@QAEXPAX@Z @ 0x1A2914
 * Callers:
 *     ?Free@?$CTypeIsolation@$0FAAA@$0EI@@NSInstrumentation@@IAEXPAX@Z @ 0x1A2979 (-Free@-$CTypeIsolation@$0FAAA@$0EI@@NSInstrumentation@@IAEXPAX@Z.c)
 * Callees:
 *     _memset @ 0xF92A7 (_memset.c)
 *     ?VirtualAddressToSlot@?$CSectionBitmapAllocator@$0FAAA@$0EI@@NSInstrumentation@@AAEIPBX@Z @ 0x1A2D77 (-VirtualAddressToSlot@-$CSectionBitmapAllocator@$0FAAA@$0EI@@NSInstrumentation@@AAEIPBX@Z.c)
 */

void __thiscall NSInstrumentation::CSectionBitmapAllocator<20480,72>::Free(int *this, void *a2)
{
  int v3; // esi
  ULONG v4; // edi

  if ( a2 )
  {
    v3 = *this;
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusiveEx(v3, 0);
    v4 = NSInstrumentation::CSectionBitmapAllocator<20480,72>::VirtualAddressToSlot(a2);
    RtlClearBit((PRTL_BITMAP)(this[2] ^ this[3]), v4);
    memset(a2, 0, 0x48u);
    this[4] = v4;
    ExReleasePushLockExclusiveEx(v3, 0);
    KeLeaveCriticalRegion();
  }
}
