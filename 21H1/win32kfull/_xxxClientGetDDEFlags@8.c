/*
 * XREFs of _xxxClientGetDDEFlags@8 @ 0x194375
 * Callers:
...
int __fastcall xxxClientGetDDEFlags(int a1, int a2)
{
  int v2; // esi
  int *v3; // ecx
  int result; // eax
  ...
  EtwTraceBeginCallback(70);
  v2 = KeUserModeCallback(70, v5, 8, &v6, &v7);
  EtwTraceEndCallback(70);
  LeaveEnterCritProperDisposition::~LeaveEnterCritProperDisposition((LeaveEnterCritProperDisposition *)&v9);
  ReleaseAndReacquirePerObjectLocks::~ReleaseAndReacquirePerObjectLocks((ReleaseAndReacquirePerObjectLocks *)&v8);
  if ( v2 < 0 || v7 != 12 )
    return 0;
