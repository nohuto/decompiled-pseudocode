/*
 * XREFs of VfPutDmaAdapter @ 0x1409C9C10
 * Callers:
 *     <none>
 * Callees:
...
__int64 (__fastcall *__fastcall VfPutDmaAdapter(ULONG_PTR a1))(ULONG_PTR)
{
  ...
  if ( result )
  {
      ...
      if ( (struct _LIST_ENTRY *)a1 == Flink[1].Flink )
      {
        ...
        if ( v8 < 0 )
        {
          ViHalPreprocessOptions(
            ...
            (const void *)0x18);
          VfReportIssueWithOptions(0xE6u, 0x18uLL, a1, (ULONG_PTR)Flink, 0LL, byte_140C12D08);
        }
        ...
        if ( LODWORD(Flink[10].Flink) )
        {
          ViHalPreprocessOptions(
            ...
            (const void *)9,
            a1);
          VfReportIssueWithOptions(0xE6u, 9uLL, a1, SLODWORD(Flink[10].Flink), (ULONG_PTR)Flink, byte_140C12CFC);
        }
        if ( LODWORD(Flink[10].Blink) )
        {
          ViHalPreprocessOptions(
            ...
            (const void *)0xA,
            a1);
          VfReportIssueWithOptions(0xE6u, 0xAuLL, a1, SLODWORD(Flink[10].Blink), (ULONG_PTR)Flink, byte_140C12D00);
        }
    ...
    KeReleaseSpinLock(&qword_140D47088, v7);
    if ( Flink && !v6 )
