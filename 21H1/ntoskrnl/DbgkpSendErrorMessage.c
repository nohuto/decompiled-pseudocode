/*
 * XREFs of DbgkpSendErrorMessage @ 0x140882F60
 * Callers:
 *     PsDispatchIumService @ 0x14057D274 (PsDispatchIumService.c)
 *     DbgkForwardException @ 0x14071E1F0 (DbgkForwardException.c)
 * Callees:
 *     PsGetProcessServerSilo @ 0x140204560 (PsGetProcessServerSilo.c)
 *     KeLeaveCriticalRegionThread @ 0x1402486B0 (KeLeaveCriticalRegionThread.c)
 *     ObfReferenceObjectWithTag @ 0x14025A2D0 (ObfReferenceObjectWithTag.c)
 *     ExAcquirePushLockSharedEx @ 0x140260BA0 (ExAcquirePushLockSharedEx.c)
 *     MmGetSessionIdEx @ 0x140260EB0 (MmGetSessionIdEx.c)
 *     ObfDereferenceObjectWithTag @ 0x1402611B0 (ObfDereferenceObjectWithTag.c)
 *     KeLeaveCriticalRegion @ 0x140261420 (KeLeaveCriticalRegion.c)
 *     KeAbPostRelease @ 0x140263C10 (KeAbPostRelease.c)
 *     KeWaitForSingleObject @ 0x1402643F0 (KeWaitForSingleObject.c)
 *     PsGetServerSiloGlobals @ 0x1402D70B4 (PsGetServerSiloGlobals.c)
 *     PsGetServerSiloServiceSessionId @ 0x1402E5A50 (PsGetServerSiloServiceSessionId.c)
 *     KeCopyExceptionRecord @ 0x140323088 (KeCopyExceptionRecord.c)
 *     KeTestAlertThread @ 0x14032FC90 (KeTestAlertThread.c)
 *     ExfReleasePushLockShared @ 0x14035A580 (ExfReleasePushLockShared.c)
 *     AlpcGetMessageAttribute @ 0x14035CC60 (AlpcGetMessageAttribute.c)
 *     AlpcInitializeMessageAttribute @ 0x14035CCA0 (AlpcInitializeMessageAttribute.c)
 *     __security_check_cookie @ 0x1403CC020 (__security_check_cookie.c)
 *     ZwAlpcSendWaitReceivePort @ 0x1403F34B0 (ZwAlpcSendWaitReceivePort.c)
 *     memset @ 0x140408F80 (memset.c)
 *     MmGetSectionInformation @ 0x1405DB7B0 (MmGetSectionInformation.c)
 *     PsTestProtectedProcessIncompatibility @ 0x14064CEC8 (PsTestProtectedProcessIncompatibility.c)
 *     PsThawProcess @ 0x1406CF790 (PsThawProcess.c)
 *     DbgkForwardException @ 0x14071E1F0 (DbgkForwardException.c)
 *     DbgkpDeleteErrorPort @ 0x140882C38 (DbgkpDeleteErrorPort.c)
 *     DbgkpRemoveErrorPort @ 0x140882C64 (DbgkpRemoveErrorPort.c)
 *     DbgkpStartSystemErrorHandler @ 0x140883440 (DbgkpStartSystemErrorHandler.c)
 *     DbgkpSuspendProcess @ 0x1408838D0 (DbgkpSuspendProcess.c)
 */

__int64 __fastcall DbgkpSendErrorMessage(__int64 a1, char a2, __int64 a3)
{
  struct _KTHREAD *CurrentThread; // r13
  __int64 Process; // r15
  __int64 ProcessServerSilo; // rbx
  PVOID *v8; // rsi
  signed int started; // edi
  int ServerSiloServiceSessionId; // ebx
  LARGE_INTEGER *Timeout; // rax
  NTSTATUS v12; // eax
  PVOID v13; // r14
  PVOID v14; // rax
  __int64 v15; // rdx
  __int64 v16; // r8
  __int64 v17; // r9
  __int64 v18; // rdx
  __int128 v19; // xmm0
  __int128 v20; // xmm1
  __int128 v21; // xmm0
  __int128 v22; // xmm1
  __int64 v23; // rcx
  __int64 v24; // rax
  __int16 v25; // dx
  char v26; // r13
  int v27; // ebx
  char *MessageAttribute; // rax
  __int64 v29; // rdx
  __int64 v30; // r8
  _DWORD *v31; // r9
  __int64 v32; // rcx
  int v33; // eax
  char v34; // bl
  char v36; // [rsp+40h] [rbp-C0h]
  __int64 v38; // [rsp+48h] [rbp-B8h] BYREF
  PVOID Object; // [rsp+50h] [rbp-B0h]
  LARGE_INTEGER v40; // [rsp+58h] [rbp-A8h] BYREF
  __int64 v41; // [rsp+60h] [rbp-A0h]
  struct _KTHREAD *v42; // [rsp+68h] [rbp-98h]
  _OWORD v43[10]; // [rsp+80h] [rbp-80h] BYREF

  v41 = a1;
  memset(&v43[4], 0, 0x60uLL);
  v38 = 0LL;
  CurrentThread = KeGetCurrentThread();
  v40.QuadPart = 0LL;
  v36 = 0;
  v42 = CurrentThread;
  Process = (__int64)CurrentThread->Process;
  ProcessServerSilo = PsGetProcessServerSilo(Process);
  v8 = (PVOID *)((char *)PsGetServerSiloGlobals(ProcessServerSilo) + 968);
  memset(v43, 0, 0x40uLL);
  if ( (*(_DWORD *)(Process + 2172) & 1) != 0 )
    return (unsigned int)-1073741637;
  MmGetSectionInformation(*(_QWORD *)(Process + 1304), 1, (__int64)v43);
  ServerSiloServiceSessionId = PsGetServerSiloServiceSessionId(ProcessServerSilo);
  if ( (unsigned int)MmGetSessionIdEx(Process) != ServerSiloServiceSessionId && LODWORD(v43[2]) != 1 && (a2 & 2) != 0 )
    v36 = DbgkpSuspendProcess(Process);
  started = DbgkpStartSystemErrorHandler();
  if ( started >= 0 )
  {
    if ( DbgkErrorPortStartTimeout == -1 )
    {
      Timeout = 0LL;
    }
    else
    {
      Timeout = &v40;
      v40.QuadPart = -10000LL * DbgkErrorPortStartTimeout;
    }
    v12 = KeWaitForSingleObject(v8[3], Executive, 1, 0, Timeout);
    if ( v12 == 258 || !v8[1] )
    {
      started = -1073740973;
    }
    else if ( v12 == 257 || v12 == 192 )
    {
      started = -1073741749;
    }
    else
    {
      Object = 0LL;
      --CurrentThread->KernelApcDisable;
      started = 0;
      v13 = 0LL;
      ExAcquirePushLockSharedEx((ULONG_PTR)v8, 0LL);
      v14 = v8[2];
      if ( (PVOID)Process == v14 )
      {
        started = -1073741420;
      }
      else
      {
        v13 = v8[1];
        if ( v13 )
        {
          Object = v8[2];
          ObfReferenceObjectWithTag(v14, 0x50676244u);
          _InterlockedIncrement((volatile signed __int32 *)v13);
        }
        else
        {
          started = -1073740973;
        }
      }
      if ( _InterlockedCompareExchange64((volatile signed __int64 *)v8, 0LL, 17LL) != 17 )
        ExfReleasePushLockShared((signed __int64 *)v8);
      KeAbPostRelease((ULONG_PTR)v8);
      KeLeaveCriticalRegionThread((__int64)CurrentThread, v15, v16, v17);
      if ( started >= 0 )
      {
        v18 = v41;
        *(_QWORD *)(a3 + 8) = 0LL;
        *(_QWORD *)(a3 + 16) = 0LL;
        *(_QWORD *)(a3 + 24) = 0LL;
        *(_QWORD *)(a3 + 32) = 0LL;
        *(_DWORD *)(a3 + 4) = 8;
        *(_WORD *)(a3 + 4) = -32761;
        *(_DWORD *)a3 = 17826024;
        *(_DWORD *)(a3 + 40) = 7;
        *(_DWORD *)(a3 + 44) = -2147418111;
        KeCopyExceptionRecord((_OWORD *)(a3 + 48), v18);
        v19 = v43[0];
        v20 = v43[1];
        *(_DWORD *)(a3 + 264) = 0;
        *(_OWORD *)(a3 + 200) = v19;
        v21 = v43[2];
        *(_OWORD *)(a3 + 216) = v20;
        v22 = v43[3];
        *(_OWORD *)(a3 + 232) = v21;
        *(_OWORD *)(a3 + 248) = v22;
        if ( (*(_BYTE *)(Process + 2170) & 7) != 0 )
        {
          *(_DWORD *)(a3 + 264) = 1;
          v23 = 1LL;
        }
        else
        {
          v23 = 0LL;
        }
        if ( (*(_BYTE *)(Process + 992) & 1) != 0 )
        {
          v23 = (unsigned int)v23 | 8;
          *(_DWORD *)(a3 + 264) = v23;
        }
        v24 = *(_QWORD *)(Process + 1408);
        if ( v24 )
        {
          v25 = *(_WORD *)(v24 + 8);
          if ( v25 == 332 || v25 == 452 )
          {
            v23 = (unsigned int)v23 | 2;
            *(_DWORD *)(a3 + 264) = v23;
          }
        }
        if ( (a2 & 1) != 0 )
        {
          v23 = (unsigned int)v23 | 4;
          v26 = 1;
          *(_DWORD *)(a3 + 264) = v23;
        }
        else
        {
          v26 = 0;
        }
        LOBYTE(v23) = 1;
        v27 = PsTestProtectedProcessIncompatibility(v23, (__int64)Object, Process) ? 1055744 : 0x1FFFFF;
        AlpcInitializeMessageAttribute(0x10000000LL, v43, 0xA0uLL, &v38);
        DWORD1(v43[0]) = 0x10000000;
        MessageAttribute = AlpcGetMessageAttribute(v43, 0x10000000);
        *(_DWORD *)MessageAttribute = 0;
        *((_DWORD *)MessageAttribute + 5) = v27;
        *((_DWORD *)MessageAttribute + 4) = 4;
        *((_QWORD *)MessageAttribute + 1) = -2LL;
        v38 = 272LL;
        KeTestAlertThread(0, v29, v30, v31);
        v32 = (__int64)v42;
        if ( (*(_DWORD *)(&v42[1].SwapListEntry + 1) & 1) != 0 )
          started = -1073741749;
        if ( started >= 0 )
        {
          v33 = ZwAlpcSendWaitReceivePort(*((_QWORD *)v13 + 1), 2228224LL);
          started = v33;
          if ( v33 >= 0 )
          {
            if ( v33 == 257 )
            {
              started = -1073741749;
            }
            else
            {
              if ( (*(_WORD *)(a3 + 4) & 0x2000) != 0 )
                ZwAlpcSendWaitReceivePort(*((_QWORD *)v13 + 1), 0x10000LL);
              started = *(_DWORD *)(a3 + 44);
              if ( started >= 0 && !v26 && started == 65538 )
              {
                v34 = v36;
                if ( v36 )
                {
                  PsThawProcess(Process, 0);
                  KeLeaveCriticalRegion();
                  v34 = 0;
                }
                started = !DbgkForwardException(v41, 1, 1) ? 0xC0000144 : 0;
                goto LABEL_53;
              }
            }
LABEL_52:
            v34 = v36;
LABEL_53:
            ObfDereferenceObjectWithTag(Object, 0x50676244u);
            if ( _InterlockedExchangeAdd((volatile signed __int32 *)v13, 0xFFFFFFFF) == 1 )
              DbgkpDeleteErrorPort((HANDLE *)v13);
            goto LABEL_58;
          }
          v32 = (__int64)v42;
        }
        if ( started == -1073741769 )
          DbgkpRemoveErrorPort(v32, (ULONG_PTR)v8, (volatile signed __int32 *)v13);
        goto LABEL_52;
      }
    }
  }
  v34 = v36;
LABEL_58:
  if ( v34 )
  {
    PsThawProcess(Process, 0);
    KeLeaveCriticalRegion();
  }
  return (unsigned int)started;
}
