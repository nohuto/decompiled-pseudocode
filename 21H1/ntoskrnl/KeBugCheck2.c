/*
 * XREFs of KeBugCheck2 @ 0x140511010
 * Callers:
 *     KeBugCheckEx @ 0x1403F5E40 (KeBugCheckEx.c)
 * Callees:
 *     KeIsEmptyAffinityEx @ 0x140276350 (KeIsEmptyAffinityEx.c)
 *     KeRemoveProcessorAffinityEx @ 0x1402778F0 (KeRemoveProcessorAffinityEx.c)
 *     KeCopyAffinityEx @ 0x140277930 (KeCopyAffinityEx.c)
 *     KeStallExecutionProcessor @ 0x140278400 (KeStallExecutionProcessor.c)
 *     KeQueryCurrentStackInformation @ 0x14032BCA0 (KeQueryCurrentStackInformation.c)
 *     MmIsSessionAddress @ 0x14035ADE0 (MmIsSessionAddress.c)
 *     DbgPrintEx @ 0x14037C640 (DbgPrintEx.c)
 *     IoAddTriageDumpDataBlock @ 0x1403C8408 (IoAddTriageDumpDataBlock.c)
 *     KdRefreshDebuggerNotPresent @ 0x1403CBAF0 (KdRefreshDebuggerNotPresent.c)
 *     KiFilterBugCheckInfo @ 0x1403D79FC (KiFilterBugCheckInfo.c)
 *     KiMarkBugCheckRegions @ 0x1403D8398 (KiMarkBugCheckRegions.c)
 *     _guard_dispatch_icall @ 0x1403FE9E0 (_guard_dispatch_icall.c)
 *     memmove @ 0x140408CC0 (memmove.c)
 *     memset @ 0x140408F80 (memset.c)
 *     HalReturnToFirmware @ 0x1404B8BE0 (HalReturnToFirmware.c)
 *     HvlLogGuestCrashInformation @ 0x1404EC16C (HvlLogGuestCrashInformation.c)
 *     HvlNotifyRootCrashdump @ 0x1404F0434 (HvlNotifyRootCrashdump.c)
 *     HvlPrepareForRootCrashdump @ 0x1404F04FC (HvlPrepareForRootCrashdump.c)
 *     IoInitializeBugCheckProgress @ 0x1404FCA68 (IoInitializeBugCheckProgress.c)
 *     IoSaveBugCheckProgress @ 0x1404FCCE0 (IoSaveBugCheckProgress.c)
 *     IoSetBugCheckProgressFlag @ 0x1404FCF08 (IoSetBugCheckProgressFlag.c)
 *     IoWriteCrashDump @ 0x1404FD230 (IoWriteCrashDump.c)
 *     KdDecodeDataBlock @ 0x14050AD54 (KdDecodeDataBlock.c)
 *     KdEnableDebuggerWithLock @ 0x14050B2CC (KdEnableDebuggerWithLock.c)
 *     KiCheckForFreezeExecution @ 0x14050CB60 (KiCheckForFreezeExecution.c)
 *     KiBugCheckDebugBreak @ 0x140512360 (KiBugCheckDebugBreak.c)
 *     KiBugCheckUnicodeToAnsi @ 0x140512580 (KiBugCheckUnicodeToAnsi.c)
 *     KiBugcheckUnloadDebugSymbols @ 0x1405125C4 (KiBugcheckUnloadDebugSymbols.c)
 *     KiDisplayBlueScreen @ 0x140512600 (KiDisplayBlueScreen.c)
 *     KiDumpParameterImages @ 0x140512994 (KiDumpParameterImages.c)
 *     KiInvokeBugCheckAddTriageDumpDataCallbacks @ 0x140512B2C (KiInvokeBugCheckAddTriageDumpDataCallbacks.c)
 *     KiInvokeBugCheckEntryCallbacks @ 0x140512CE8 (KiInvokeBugCheckEntryCallbacks.c)
 *     KiPcToFileHeader @ 0x140512DB0 (KiPcToFileHeader.c)
 *     KiSaveCurrentEtwTraceBuffer @ 0x140512E80 (KiSaveCurrentEtwTraceBuffer.c)
 *     KiScanBugCheckCallbackList @ 0x140512FAC (KiScanBugCheckCallbackList.c)
 *     KeSaveSupervisorState @ 0x140514438 (KeSaveSupervisorState.c)
 *     KiSendFreeze @ 0x140518680 (KiSendFreeze.c)
 *     KiSendThawExecution @ 0x140518750 (KiSendThawExecution.c)
 *     KiSetDebuggerOwner @ 0x140518914 (KiSetDebuggerOwner.c)
 *     KiAttemptBugcheckRecovery @ 0x14051F284 (KiAttemptBugcheckRecovery.c)
 *     MmLocateUnloadedDriver @ 0x14052FDAC (MmLocateUnloadedDriver.c)
 *     MmIsAddressValid @ 0x140531050 (MmIsAddressValid.c)
 *     MmIsSpecialPoolAddress @ 0x1405319D0 (MmIsSpecialPoolAddress.c)
 *     PoAddPowerTriageData @ 0x140568FCC (PoAddPowerTriageData.c)
 *     VfNotifyVerifierOfEvent @ 0x1409C3030 (VfNotifyVerifierOfEvent.c)
 */

void __fastcall __noreturn KeBugCheck2(
        int a1,
        unsigned __int64 a2,
        unsigned __int64 a3,
        __int64 a4,
        unsigned __int64 a5,
        __int64 a6)
{
  const CHAR *v6; // rsi
  __int64 v8; // r14
  __int64 v11; // r8
  unsigned __int8 CurrentIrql; // cl
  _DWORD *SchedulerAssist; // r9
  signed __int32 v14; // eax
  unsigned int v15; // edx
  signed __int32 v16; // ecx
  char v17; // bl
  signed __int32 v18; // eax
  unsigned int v19; // edx
  signed __int32 v20; // ecx
  int v21; // ecx
  size_t v22; // r8
  _OWORD *v23; // rcx
  __int64 v24; // rdx
  _CONTEXT *Context; // rax
  __int64 v26; // r8
  __int128 v27; // xmm1
  char v28; // r10
  unsigned int v29; // ecx
  int v30; // eax
  __int64 v31; // rbx
  int v32; // eax
  __int64 v33; // rax
  int v34; // eax
  struct _KTHREAD *v35; // r15
  __int64 UnloadedDriver; // rax
  const CHAR *v37; // r14
  _QWORD *v38; // rcx
  unsigned __int64 v39; // r15
  __int64 v40; // rax
  struct _KTHREAD *v41; // rcx
  _KPROCESS *Process; // rcx
  int v43; // edx
  int IsEmptyAffinity; // eax
  struct _KPRCB *v45; // rdi
  __int64 v46; // rcx
  _DWORD *v47; // r9
  bool v48; // si
  int v49; // ecx
  bool v50; // r14
  unsigned int v51; // eax
  __int64 v52; // rcx
  _CONTEXT *v53; // rax
  _OWORD *v54; // rcx
  __int64 v55; // rdx
  __int64 v56; // r8
  __int128 v57; // xmm1
  __int64 v58; // rbx
  char v59; // bl
  int v60; // ecx
  char v61; // [rsp+50h] [rbp-B0h]
  char v62; // [rsp+51h] [rbp-AFh]
  char v63; // [rsp+52h] [rbp-AEh] BYREF
  bool v64; // [rsp+53h] [rbp-ADh]
  bool v65; // [rsp+54h] [rbp-ACh]
  bool v66; // [rsp+55h] [rbp-ABh]
  char v67; // [rsp+56h] [rbp-AAh]
  struct _KTHREAD *CurrentThread; // [rsp+58h] [rbp-A8h]
  unsigned int v69; // [rsp+60h] [rbp-A0h] BYREF
  PCSTR Format; // [rsp+68h] [rbp-98h]
  int v71; // [rsp+78h] [rbp-88h]
  unsigned __int64 v72; // [rsp+80h] [rbp-80h]
  int v73; // [rsp+88h] [rbp-78h]
  struct _KPRCB *CurrentPrcb; // [rsp+90h] [rbp-70h]
  unsigned int v75; // [rsp+98h] [rbp-68h] BYREF
  unsigned __int64 v76; // [rsp+A0h] [rbp-60h] BYREF
  unsigned int Number; // [rsp+A8h] [rbp-58h]
  unsigned __int64 v78; // [rsp+B0h] [rbp-50h] BYREF
  void *Src; // [rsp+B8h] [rbp-48h] BYREF
  __int64 (__fastcall *v80)(); // [rsp+C0h] [rbp-40h]
  unsigned __int16 v81[88]; // [rsp+D0h] [rbp-30h] BYREF
  _OWORD v82[77]; // [rsp+180h] [rbp+80h] BYREF
  char pszDest[176]; // [rsp+650h] [rbp+550h] BYREF

  v6 = (const CHAR *)a5;
  v8 = a6;
  v69 = a1;
  memset(v81, 0, 0xA8uLL);
  v64 = 1;
  pszDest[0] = 0;
  CurrentThread = KeGetCurrentThread();
  v80 = KiBugCheckProgress;
  v66 = IopAutoReboot != 0;
  v76 = 0LL;
  v63 = 0;
  LOBYTE(v71) = 0;
  v67 = 0;
  Format = 0LL;
  v72 = 0LL;
  v65 = 1;
  v73 = 0;
  v78 = 0LL;
  Src = 0LL;
  v75 = 0;
  v11 = 15LL;
  if ( KeGetCurrentIrql() < 2u )
  {
    CurrentIrql = KeGetCurrentIrql();
    __writecr8(2uLL);
    if ( KiIrqlFlags )
    {
      if ( (KiIrqlFlags & 1) != 0 && CurrentIrql <= 0xFu )
      {
        SchedulerAssist = KeGetCurrentPrcb()->SchedulerAssist;
        v11 = (-1 << (CurrentIrql + 1)) & 4u | SchedulerAssist[5];
        SchedulerAssist[5] = v11;
      }
    }
  }
  if ( KeGetCurrentThread()->InitialStack )
  {
    LOBYTE(v11) = KeQueryCurrentStackInformation(&v75, (char **)&Src, &v78);
    v18 = KiBugCheckActive;
    v19 = (16 * KeGetCurrentPrcb()->Number) | 3;
    while ( (v18 & 3) != 3 )
    {
      v20 = v18;
      v18 = _InterlockedCompareExchange(&KiBugCheckActive, v19, v18);
      if ( v18 == v20 )
      {
        if ( (_BYTE)v11 )
        {
          if ( v75 > 9 || (v21 = 929, !_bittest(&v21, v75)) )
          {
            v22 = v78 - (_QWORD)Src;
            if ( v78 - (unsigned __int64)Src > 0x6000 )
              v22 = 24576LL;
            memmove(&KiPreBugcheckStackSaveArea, Src, v22);
          }
        }
LABEL_25:
        v61 = 1;
        goto LABEL_11;
      }
    }
  }
  else
  {
    v14 = KiBugCheckActive;
    v15 = (16 * KeGetCurrentPrcb()->Number) | 3;
    while ( (v14 & 3) != 3 )
    {
      v16 = v14;
      v14 = _InterlockedCompareExchange(&KiBugCheckActive, v15, v14);
      if ( v14 == v16 )
        goto LABEL_25;
    }
  }
  v61 = 0;
LABEL_11:
  if ( KeSmapEnabled )
    __asm { stac }
  CurrentPrcb = KeGetCurrentPrcb();
  Number = CurrentPrcb->Number;
  KeSaveSupervisorState(CurrentPrcb->ExtendedSupervisorState, MEMORY[0xFFFFF780000005F0] | 0x100LL, v11);
  if ( !qword_140C23440 )
    goto LABEL_14;
  if ( PopSimulateHiberBugcheck )
    PoPowerDownActionInProgress = 0;
  if ( *(_BYTE *)(qword_140C23440 + 3) )
  {
    if ( v61 )
    {
      DbgPrintEx(
        0x65u,
        0,
        "\n"
        "A bugcheck occurred during the late stages of hibernate suspend or resume.\n"
        "Due to verification temporarily enabled by Po during this time,\n"
        "regular bugcheck processing may not work.\n"
        "\n");
      if ( v69 == 10 )
        DbgPrintEx(
          0x65u,
          0,
          "Memory was accessed during this time that was not properly marked\n"
          "for the boot phase of hibernate! Check the callstack and parameters\n"
          "to find the pages that need to be marked.\n"
          "\n");
    }
    v80 = 0LL;
    v17 = 1;
  }
  else
  {
LABEL_14:
    v17 = 0;
  }
  v23 = v82;
  v24 = 9LL;
  v62 = v17;
  Context = CurrentPrcb->Context;
  v26 = 128LL;
  do
  {
    *v23 = *(_OWORD *)&Context->P1Home;
    v23[1] = *(_OWORD *)&Context->P3Home;
    v23[2] = *(_OWORD *)&Context->P5Home;
    v23[3] = *(_OWORD *)&Context->ContextFlags;
    v23[4] = *(_OWORD *)&Context->SegGs;
    v23[5] = *(_OWORD *)&Context->Dr1;
    v23[6] = *(_OWORD *)&Context->Dr3;
    v23 += 8;
    v27 = *(_OWORD *)&Context->Dr7;
    Context = (_CONTEXT *)((char *)Context + 128);
    *(v23 - 1) = v27;
    --v24;
  }
  while ( v24 );
  v28 = v61;
  *v23 = *(_OWORD *)&Context->P1Home;
  v23[1] = *(_OWORD *)&Context->P3Home;
  v23[2] = *(_OWORD *)&Context->P5Home;
  v23[3] = *(_OWORD *)&Context->ContextFlags;
  v23[4] = *(_OWORD *)&Context->SegGs;
  if ( !v61 )
  {
    v35 = CurrentThread;
    goto LABEL_143;
  }
  if ( ViVerifierEnabled )
    VfNotifyVerifierOfEvent(2LL);
  if ( !v17 )
    KiSaveCurrentEtwTraceBuffer(v23, v24, v26);
  IoAddTriageDumpDataBlock((ULONG)&KseEngine, (PVOID)0x60);
  v29 = v69;
  if ( v69 == 229 )
  {
    KiScanBugCheckCallbackList();
    ((void (__fastcall *)(_QWORD))off_140C008A8[0])(0LL);
    HalReturnToFirmware(3);
  }
  *((_QWORD *)&KiBugCheckData + 1) = a2;
  *(_QWORD *)&xmmword_140C2B4B0 = a3;
  if ( v69 == -1073741103 )
    v29 = 195;
  *((_QWORD *)&xmmword_140C2B4B0 + 1) = a4;
  *(_QWORD *)&KiBugCheckData = v29;
  v69 = v29;
  qword_140C2B4C0 = a5;
  if ( v29 > 0xCB )
  {
    if ( v29 != 216 )
    {
      if ( v29 == 234 )
      {
        KiBugCheckDriver = a4;
        goto LABEL_113;
      }
      if ( v29 == 239 )
        goto LABEL_60;
      if ( v29 != 252 )
      {
        if ( v29 == 317 )
        {
          v73 = 8;
        }
        else if ( v29 == 335 )
        {
          if ( a3 < 0x100 && a5 )
          {
            v41 = CurrentThread;
            if ( *(_QWORD *)(a5 + 8) )
              v41 = *(struct _KTHREAD **)(a5 + 8);
            CurrentThread = v41;
          }
          Process = KeGetCurrentThread()->ApcState.Process;
          v43 = HIDWORD(Process[2].Header.WaitListHead.Flink) >> 12;
          LOBYTE(v43) = (HIDWORD(Process[2].Header.WaitListHead.Flink) & 0x1000) == 0;
          v71 = v43;
        }
        goto LABEL_113;
      }
LABEL_52:
      if ( !a6 )
      {
        if ( !a4 || (a4 & 3) != 0 )
          goto LABEL_113;
        v8 = a4;
      }
      if ( v29 == 142
        || (v72 = *(_QWORD *)(v8 + 360), KeGetCurrentThread()->ApcStateIndex != 1)
        || (LOBYTE(v30) = MmIsSessionAddress(v72), !v30)
        || (HIDWORD(KeGetCurrentThread()->ApcState.Process[2].Header.WaitListHead.Flink) & 0x1000) != 0 )
      {
LABEL_113:
        v35 = CurrentThread;
        goto LABEL_114;
      }
LABEL_60:
      LOBYTE(v71) = 1;
      goto LABEL_113;
    }
    v76 = a2;
    v40 = a2 + 88;
LABEL_112:
    KiBugCheckDriver = v40;
    goto LABEL_113;
  }
  switch ( v29 )
  {
    case 0xCBu:
      v72 = a2;
      goto LABEL_113;
    case 0xAu:
      if ( a5 >= ExPoolCodeStart && a5 < ExPoolCodeEnd )
      {
        *(_QWORD *)&KiBugCheckData = 197LL;
        goto LABEL_113;
      }
      KiPcToFileHeader(a5, &v76, 0LL, &v63);
      if ( v63 != 1 )
      {
        *(_QWORD *)&KiBugCheckData = 209LL;
        goto LABEL_113;
      }
      if ( !KiPcToFileHeader(a2, &v76, 1LL, &v63) )
      {
        KiBugCheckDriver = MmLocateUnloadedDriver(a2);
        if ( KiBugCheckDriver )
          *(_QWORD *)&KiBugCheckData = 212LL;
        goto LABEL_113;
      }
      v40 = v76 + 88;
      *(_QWORD *)&KiBugCheckData = 211LL;
      goto LABEL_112;
    case 0x4Cu:
      v38 = (_QWORD *)&KiBugCheckData + 1;
      *(_QWORD *)&KiBugCheckData = (unsigned int)a2;
      v39 = a3 - ((_QWORD)&KiBugCheckData + 8);
      LOBYTE(v71) = 1;
      v37 = (const CHAR *)a4;
      v67 = 1;
      do
      {
        *v38 = *(_QWORD *)((char *)v38 + v39);
        ++v38;
      }
      while ( (__int64)v38 < (__int64)qword_140C2B4C8 );
      v35 = CurrentThread;
      goto LABEL_115;
  }
  if ( v29 != 80 )
  {
    if ( v29 == 123 )
    {
      v64 = (a4 & 1) == 0;
      v65 = (a4 & 2) == 0;
      goto LABEL_113;
    }
    if ( v29 != 142 && v29 != 190 )
      goto LABEL_113;
    goto LABEL_52;
  }
  v31 = 0LL;
  if ( a6 )
    goto LABEL_66;
  if ( a4 && (a4 & 3) == 0 )
  {
    v8 = a4;
LABEL_66:
    v72 = *(_QWORD *)(v8 + 360);
    *((_QWORD *)&xmmword_140C2B4B0 + 1) = v72;
    v31 = KiPcToFileHeader(v72, &v76, 0LL, &v63);
    if ( KeGetCurrentThread()->ApcStateIndex == 1 )
    {
      LOBYTE(v32) = MmIsSessionAddress(v72);
      if ( v32 )
      {
        if ( (HIDWORD(KeGetCurrentThread()->ApcState.Process[2].Header.WaitListHead.Flink) & 0x1000) == 0 )
          LOBYTE(v71) = 1;
      }
    }
    goto LABEL_70;
  }
  v63 = 1;
LABEL_70:
  if ( (unsigned int)MmIsSpecialPoolAddress(a2) == 1 )
  {
    v33 = 213LL;
    v17 = v62;
    if ( v63 == 1 )
      v33 = 204LL;
    *(_QWORD *)&KiBugCheckData = v33;
    goto LABEL_113;
  }
  if ( v72 == a2 )
  {
    LOBYTE(v34) = MmIsSessionAddress(a2);
    v35 = CurrentThread;
    if ( v34 == 1 && (unsigned __int64)CurrentThread->Teb - 1 > 0xFFFF7FFFFFFFFFFEuLL )
    {
      *(_QWORD *)&KiBugCheckData = 207LL;
LABEL_79:
      v17 = v62;
LABEL_114:
      v37 = Format;
      v6 = Format;
      goto LABEL_115;
    }
  }
  else
  {
    v35 = CurrentThread;
  }
  if ( v31 )
    goto LABEL_79;
  UnloadedDriver = MmLocateUnloadedDriver(a2);
  v37 = Format;
  v17 = v62;
  v6 = Format;
  KiBugCheckDriver = UnloadedDriver;
  if ( UnloadedDriver )
    *(_QWORD *)&KiBugCheckData = 206LL;
LABEL_115:
  if ( v66 )
    KiAttemptBugcheckRecovery();
  off_140C008C8[0]();
  HvlEnlightenments &= 0x2000u;
  IoSaveBugCheckProgress(96);
  IsEmptyAffinity = KeIsEmptyAffinityEx(KiNmiInProgress);
  ((void (__fastcall *)(bool))off_140C00698[0])(IsEmptyAffinity == 0);
  KiFilterBugCheckInfo(&v69, (__int64)&KiBugCheckData);
  HvlLogGuestCrashInformation(
    KiBugCheckData,
    *((__int64 *)&KiBugCheckData + 1),
    xmmword_140C2B4B0,
    *((__int64 *)&xmmword_140C2B4B0 + 1),
    qword_140C2B4C0);
  if ( KiBugCheckDriver )
  {
    KiBugCheckUnicodeToAnsi(KiBugCheckDriver, pszDest);
  }
  else if ( v72 )
  {
    KiDumpParameterImages(pszDest);
  }
  if ( !KdPitchDebugger )
    qword_140C00B48 = (__int64)v82;
  if ( v69 == 226
    || !(_BYTE)KdDebuggerEnabled && !KdEventLoggingEnabled
    || KiHypervisorInitiatedCrashDump
    || KdRefreshDebuggerNotPresent() && !KdEventLoggingPresent )
  {
    v28 = v61;
LABEL_143:
    v45 = CurrentPrcb;
    goto LABEL_144;
  }
  v45 = CurrentPrcb;
  if ( !CurrentPrcb->NmiActive )
  {
    DbgPrintEx(
      0x65u,
      0,
      "\n*** Fatal System Error: 0x%08lx\n                       (0x%p,0x%p,0x%p,0x%p)\n\n",
      (_DWORD)KiBugCheckData,
      *((const void **)&KiBugCheckData + 1),
      (const void *)xmmword_140C2B4B0,
      *((const void **)&xmmword_140C2B4B0 + 1),
      (const void *)qword_140C2B4C0);
    if ( KiBugCheckDriver )
      DbgPrintEx(0x65u, 0, "Driver at fault: %s.\n", pszDest);
    if ( v67 )
    {
      if ( v37 )
        DbgPrintEx(0x65u, 0, v37);
      if ( v6 )
        DbgPrintEx(0x65u, 0, v6);
    }
  }
  if ( (_BYTE)KdDebuggerEnabled && !(_BYTE)KdDebuggerNotPresent )
    KiBugCheckDebugBreak(3u);
  v28 = v61;
LABEL_144:
  _disable();
  v46 = KeGetCurrentIrql();
  __writecr8(0xFuLL);
  if ( KiIrqlFlags && (KiIrqlFlags & 1) != 0 && (unsigned __int8)v46 <= 0xFu )
  {
    v46 = (unsigned int)(unsigned __int8)v46 + 1;
    v47 = KeGetCurrentPrcb()->SchedulerAssist;
    v47[5] |= ~((unsigned __int16)(1LL << v46) - 1) & 0xFFFC;
  }
  if ( v28 )
  {
    if ( (unsigned int)KeNumberProcessors_0 > 1 && !KiHypervisorInitiatedCrashDump )
    {
      KiSetDebuggerOwner(v45);
      KeCopyAffinityEx((__int64)v81, (unsigned __int16 *)KeActiveProcessors);
      KeRemoveProcessorAffinityEx(v81, v45->Number);
      KiSendFreeze(v81, 0LL);
      KeStallExecutionProcessor(0xF4240u);
    }
    IoInitializeBugCheckProgress(v69, a2);
    IoSaveBugCheckProgress(1);
    v48 = v65;
    if ( v17 )
    {
      v50 = v64;
    }
    else
    {
      if ( CrashdmpDumpBlock && v65 )
        v49 = v73;
      else
        v49 = v73 | 4;
      v50 = v64;
      v51 = v49 | 2;
      if ( v66 )
        v51 = v49;
      v52 = v51 | 1;
      if ( v64 )
        v52 = v51;
      KiDisplayBlueScreen(v52);
    }
    HvlPrepareForRootCrashdump();
    if ( !v17 )
    {
      KiInvokeBugCheckEntryCallbacks(1LL, 0LL, 0LL);
      IoSaveBugCheckProgress(2);
      KiInvokeBugCheckAddTriageDumpDataCallbacks();
      IoSaveBugCheckProgress(5);
    }
    if ( !(_BYTE)KdDebuggerEnabled && !KdPitchDebugger )
      KdEnableDebuggerWithLock(0);
    v53 = v45->Context;
    v54 = v82;
    v55 = 9LL;
    v56 = 128LL;
    do
    {
      *(_OWORD *)&v53->P1Home = *v54;
      *(_OWORD *)&v53->P3Home = v54[1];
      *(_OWORD *)&v53->P5Home = v54[2];
      *(_OWORD *)&v53->ContextFlags = v54[3];
      *(_OWORD *)&v53->SegGs = v54[4];
      *(_OWORD *)&v53->Dr1 = v54[5];
      *(_OWORD *)&v53->Dr3 = v54[6];
      v53 = (_CONTEXT *)((char *)v53 + 128);
      v57 = v54[7];
      v54 += 8;
      *(_OWORD *)&v53[-1].LastExceptionToRip = v57;
      --v55;
    }
    while ( v55 );
    *(_OWORD *)&v53->P1Home = *v54;
    *(_OWORD *)&v53->P3Home = v54[1];
    *(_OWORD *)&v53->P5Home = v54[2];
    *(_OWORD *)&v53->ContextFlags = v54[3];
    *(_OWORD *)&v53->SegGs = v54[4];
    if ( v48 )
    {
      if ( v69 == 265 )
      {
        KiMarkBugCheckRegions(
          *((__int64 *)&KiBugCheckData + 1),
          xmmword_140C2B4B0,
          *((unsigned __int64 *)&xmmword_140C2B4B0 + 1),
          qword_140C2B4C0);
        if ( qword_140C2B4C0 == 47 )
        {
          v58 = *((_QWORD *)&xmmword_140C2B4B0 + 1);
          if ( MmIsAddressValid((PVOID)(*((_QWORD *)&xmmword_140C2B4B0 + 1) + 1928LL)) )
            IoAddTriageDumpDataBlock(*(_QWORD *)(v58 + 1928), (PVOID)0x1000);
        }
      }
      KdDecodeDataBlock();
      if ( v69 == 395 )
      {
        if ( *((_QWORD *)&KiBugCheckData + 1) != 396LL )
        {
LABEL_185:
          IoWriteCrashDump(
            KiBugCheckData,
            *((__int64 *)&KiBugCheckData + 1),
            xmmword_140C2B4B0,
            *((__int64 *)&xmmword_140C2B4B0 + 1),
            qword_140C2B4C0,
            v82,
            v35,
            (__int64)v80);
          IoSaveBugCheckProgress(3);
          goto LABEL_193;
        }
        if ( KdpBreakpointChangeCount )
          IoAddTriageDumpDataBlock((ULONG)&KdpBreakpointChangeCount, (PVOID)4);
        IoAddTriageDumpDataBlock((ULONG)&VslpTraceLog, (PVOID)0xA08);
        if ( *(_QWORD *)&VslpHotpatchLog )
        {
          IoAddTriageDumpDataBlock((ULONG)&VslpHotpatchLog, (PVOID)8);
          IoAddTriageDumpDataBlock(VslpHotpatchLog, (PVOID)0x50008);
        }
      }
      if ( v69 == 239 )
      {
        PoAddPowerTriageData();
        if ( CriticalProcessExceptionData )
        {
          IoAddTriageDumpDataBlock((ULONG)&CriticalProcessExceptionData, (PVOID)0x30);
          if ( (_WORD)xmmword_140C19770 )
            IoAddTriageDumpDataBlock(DWORD2(xmmword_140C19770), (PVOID)(unsigned __int16)xmmword_140C19770);
        }
      }
      goto LABEL_185;
    }
  }
  else
  {
    v59 = KiBugCheckActive;
    if ( Number != (unsigned int)KiBugCheckActive >> 4 )
    {
      while ( 1 )
      {
        KiCheckForFreezeExecution(0LL);
        _mm_pause();
      }
    }
    if ( KiHypervisorInitiatedCrashDump || (KiBugCheckActive & 0xCu) >= 8 )
    {
      while ( 1 )
        ((void (__fastcall *)(__int64))off_140C005D8)(v46);
    }
    IoSetBugCheckProgressFlag(0x20000);
    _InterlockedExchangeAdd(&KiBugCheckActive, 4u);
    if ( (v59 & 0xC) != 0 )
      KiBugCheckDebugBreak(4u);
    v50 = v64;
  }
LABEL_193:
  if ( !VslVsmEnabled )
  {
    if ( (HvlpFlags & 2) != 0 )
      HvlNotifyRootCrashdump(2);
    HvlEnlightenments = HvlpEnlightenments;
    ((void (__fastcall *)(_OWORD *, __int64, __int64))off_140C007A8[0])(v54, v55, v56);
  }
  IoSaveBugCheckProgress(99);
  if ( !v62 )
    KiScanBugCheckCallbackList();
  off_140C008B8[0]();
  IoSaveBugCheckProgress(4);
  if ( v66 )
  {
    KiResumeForReboot = 1;
    KiSendThawExecution(0LL);
    KiBugcheckUnloadDebugSymbols();
    ((void (__fastcall *)(_QWORD))off_140C008A8[0])(0LL);
    if ( PoPowerDownActionInProgress && !PoPowerResetActionInProgress
      || PoModernStandbyActionInProgress
      || (v60 = 3, !v50) )
    {
      v60 = 1;
    }
    HalReturnToFirmware(v60);
  }
  KiBugCheckDebugBreak(4u);
}
