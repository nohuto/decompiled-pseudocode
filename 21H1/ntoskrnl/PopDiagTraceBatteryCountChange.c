/*
 * XREFs of PopDiagTraceBatteryCountChange @ 0x1408E3F60
 * Callers:
...
char __fastcall PopDiagTraceBatteryCountChange(int a1, int a2, int a3)
{
  _UNKNOWN **v3; // rax
  REGHANDLE v4; // rbx
  ...
  {
    v4 = PopDiagHandle;
    LOBYTE(v3) = EtwEventEnabled(PopDiagHandle, &POP_ETW_EVENT_BATTERY_COUNT_CHANGE);
    if ( (_BYTE)v3 )
    {
      UserData.Reserved = 0;
      v9 = 0;
      ...
      v8 = 4;
      v11 = 4;
      LOBYTE(v3) = EtwWrite(v4, &POP_ETW_EVENT_BATTERY_COUNT_CHANGE, 0LL, 3u, &UserData);
    }
  }
  return (char)v3;
}
