/*
 * XREFs of KiIpiEnlightenedGuestPriorityKick @ 0x140359624
 * Callers:
            ...
            if ( CurrentPrcb->SchedulerAssist )
            {
              if ( KiVpThreadSystemWorkPriority >= 8 )
              {
                ...
                if ( (v7 & 0x100000) != 0 || v8 && (unsigned __int8)v7 < KiVpThreadSystemWorkPriority )
                {
                  ...
                  if ( v9 )
                    LODWORD(v11) = (unsigned __int8)byte_140D00681[2 * v11] | ((unsigned __int8)HvlpVirtualProcessorMapping[2 * v11] << 6);
