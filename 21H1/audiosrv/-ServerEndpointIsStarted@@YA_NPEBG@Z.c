/*
 * XREFs of ?ServerEndpointIsStarted@@YA_NPEBG@Z @ 0x1800643D0
 * Callers:
...
char __fastcall ServerEndpointIsStarted(const unsigned __int16 *a1)
{
  ...
  if ( !v3 || v3 == 1718 )
  {
          ...
          if ( !RpcStringBindingParseW(StringBinding, 0LL, &Protseq, 0LL, String, 0LL) )
          {
            if ( !(unsigned int)_o__wcsicmp(Protseq, L"ncalrpc") )
            {
              if ( WPP_GLOBAL_Control != (CAudioSessionManager *)&WPP_GLOBAL_Control
                ...
                && *((_BYTE *)WPP_GLOBAL_Control + 25) >= 4u )
              {
                WPP_SF_(*((_QWORD *)WPP_GLOBAL_Control + 2), 11LL, &WPP_d589406c240c3254166a1ed404a43ff9_Traceguids);
              }
    ...
    if ( WPP_GLOBAL_Control != (CAudioSessionManager *)&WPP_GLOBAL_Control
      ...
      && *((_BYTE *)WPP_GLOBAL_Control + 25) >= 4u )
    {
      WPP_SF_S(*((_QWORD *)WPP_GLOBAL_Control + 2), 13LL, &WPP_d589406c240c3254166a1ed404a43ff9_Traceguids, a1);
    }
LABEL_15:
    if ( BindingVector )
