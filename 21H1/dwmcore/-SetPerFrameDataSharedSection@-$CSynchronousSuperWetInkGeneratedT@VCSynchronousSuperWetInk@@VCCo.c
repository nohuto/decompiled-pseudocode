/*
 * XREFs of ?SetPerFrameDataSharedSection@?$CSynchronousSuperWetInkGeneratedT@VCSynchronousSuperWetInk@@VCContent@@@@QEAAJPEAVCSharedSectionBase@@@Z @ 0x1801766B4
 * Callers:
...
__int64 __fastcall CSynchronousSuperWetInkGeneratedT<CSynchronousSuperWetInk,CContent>::SetPerFrameDataSharedSection(
        ...
        struct CResource *a2)
{
  ...
  if ( a2 != this[13] )
  {
    ...
    if ( v5 < 0 )
    {
      MilInstrumentationCheckHR_MaybeFailFast(v6, 0LL, 0, v5, 0x105Bu, 0LL);
    }
    else
    {
