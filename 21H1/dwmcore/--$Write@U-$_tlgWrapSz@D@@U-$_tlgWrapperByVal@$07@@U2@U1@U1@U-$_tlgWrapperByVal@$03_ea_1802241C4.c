/*
 * XREFs of ??$Write@U?$_tlgWrapSz@D@@U?$_tlgWrapperByVal@$07@@U2@U1@U1@U?$_tlgWrapperByVal@$03@@U2@U3@U3@U3@U3@U1@U1@U3@U3@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U1@U3@U3@@?$_tlgWriteTemplate@$$A6AJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2IPEAU_EVENT_DATA_DESCRIPTOR@@@Z$1?_tlgWriteTransfer_EventWriteTransfer@@YAJ0122I3@ZPEBU2@PEBU2@@@SAJPEBU_tlgProvider_t@@PEBXPEBU_GUID@@2AEBU?$_tlgWrapSz@D@@AEBU?$_tlgWrapperByVal@$07@@433AEBU?$_tlgWrapperByVal@$03@@4555533553333333333333355@Z @ 0x1802241C4
 * Callers:
 *     ?InteractionConfigurationUpdate@GestureTargeting@InputTraceLogging@@SAXPEBUInteractionUpdate@CManipulationManager@@@Z @ 0x180032E04 (-InteractionConfigurationUpdate@GestureTargeting@InputTraceLogging@@SAXPEBUInteractionUpdate@CMa.c)
 * Callees:
 *     __security_check_cookie @ 0x1800E1200 (__security_check_cookie.c)
 *     _tlgWriteTransfer_EventWriteTransfer @ 0x1801560E0 (_tlgWriteTransfer_EventWriteTransfer.c)
 */

ULONG __fastcall _tlgWriteTemplate<long (_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),&long _tlgWriteTransfer_EventWriteTransfer(_tlgProvider_t const *,void const *,_GUID const *,_GUID const *,unsigned int,_EVENT_DATA_DESCRIPTOR *),_GUID const *,_GUID const *>::Write<_tlgWrapSz<char>,_tlgWrapperByVal<8>,_tlgWrapperByVal<8>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapperByVal<4>,_tlgWrapperByVal<8>,_tlgWrapperByVal<4>,_tlgWrapperByVal<4>,_tlgWrapperByVal<4>,_tlgWrapperByVal<4>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapperByVal<4>,_tlgWrapperByVal<4>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapSz<char>,_tlgWrapperByVal<4>,_tlgWrapperByVal<4>>(
        __int64 a1,
        unsigned __int8 *a2,
        __int64 a3,
        __int64 a4,
        const unsigned __int16 **a5,
        __int64 a6,
        __int64 a7,
        const unsigned __int16 **a8,
        const unsigned __int16 **a9,
        __int64 a10,
        __int64 a11,
        __int64 a12,
        __int64 a13,
        __int64 a14,
        __int64 a15,
        const unsigned __int16 **a16,
        const unsigned __int16 **a17,
        __int64 a18,
        __int64 a19,
        const unsigned __int16 **a20,
        const unsigned __int16 **a21,
        const unsigned __int16 **a22,
        const unsigned __int16 **a23,
        const unsigned __int16 **a24,
        const unsigned __int16 **a25,
        const unsigned __int16 **a26,
        const unsigned __int16 **a27,
        const unsigned __int16 **a28,
        const unsigned __int16 **a29,
        const unsigned __int16 **a30,
        const unsigned __int16 **a31,
        const unsigned __int16 **a32,
        const unsigned __int16 **a33,
        __int64 a34,
        __int64 a35)
{
  __int64 v36; // rdx
  const unsigned __int16 *v37; // r8
  __int64 v38; // rax
  int v39; // eax
  const unsigned __int16 *v40; // r8
  __int64 v41; // rax
  int v42; // eax
  const unsigned __int16 *v43; // r8
  __int64 v44; // rax
  int v45; // eax
  const unsigned __int16 *v46; // r8
  __int64 v47; // rax
  int v48; // eax
  const unsigned __int16 *v49; // r8
  __int64 v50; // rax
  int v51; // eax
  const unsigned __int16 *v52; // r8
  __int64 v53; // rax
  int v54; // eax
  const unsigned __int16 *v55; // r8
  __int64 v56; // rax
  int v57; // eax
  const unsigned __int16 *v58; // r8
  __int64 v59; // rax
  int v60; // eax
  const unsigned __int16 *v61; // r8
  __int64 v62; // rax
  int v63; // eax
  const unsigned __int16 *v64; // r8
  __int64 v65; // rax
  int v66; // eax
  const unsigned __int16 *v67; // r8
  __int64 v68; // rax
  int v69; // eax
  const unsigned __int16 *v70; // r8
  __int64 v71; // rax
  int v72; // eax
  const unsigned __int16 *v73; // r8
  __int64 v74; // rax
  int v75; // eax
  const unsigned __int16 *v76; // r8
  __int64 v77; // rax
  int v78; // eax
  const unsigned __int16 *v79; // r8
  __int64 v80; // rax
  int v81; // eax
  const unsigned __int16 *v82; // r8
  __int64 v83; // rax
  int v84; // eax
  const unsigned __int16 *v85; // r8
  __int64 v86; // rax
  int v87; // eax
  const unsigned __int16 *v88; // r8
  __int64 v89; // rax
  int v90; // eax
  const unsigned __int16 *v91; // r8
  int v92; // edx
  struct _EVENT_DATA_DESCRIPTOR v94; // [rsp+30h] [rbp-D0h] BYREF
  const unsigned __int16 *v95; // [rsp+50h] [rbp-B0h]
  int v96; // [rsp+58h] [rbp-A8h]
  int v97; // [rsp+5Ch] [rbp-A4h]
  __int64 v98; // [rsp+60h] [rbp-A0h]
  __int64 v99; // [rsp+68h] [rbp-98h]
  __int64 v100; // [rsp+70h] [rbp-90h]
  __int64 v101; // [rsp+78h] [rbp-88h]
  const unsigned __int16 *v102; // [rsp+80h] [rbp-80h]
  int v103; // [rsp+88h] [rbp-78h]
  int v104; // [rsp+8Ch] [rbp-74h]
  const unsigned __int16 *v105; // [rsp+90h] [rbp-70h]
  int v106; // [rsp+98h] [rbp-68h]
  int v107; // [rsp+9Ch] [rbp-64h]
  __int64 v108; // [rsp+A0h] [rbp-60h]
  __int64 v109; // [rsp+A8h] [rbp-58h]
  __int64 v110; // [rsp+B0h] [rbp-50h]
  __int64 v111; // [rsp+B8h] [rbp-48h]
  __int64 v112; // [rsp+C0h] [rbp-40h]
  __int64 v113; // [rsp+C8h] [rbp-38h]
  __int64 v114; // [rsp+D0h] [rbp-30h]
  __int64 v115; // [rsp+D8h] [rbp-28h]
  __int64 v116; // [rsp+E0h] [rbp-20h]
  __int64 v117; // [rsp+E8h] [rbp-18h]
  __int64 v118; // [rsp+F0h] [rbp-10h]
  __int64 v119; // [rsp+F8h] [rbp-8h]
  const unsigned __int16 *v120; // [rsp+100h] [rbp+0h]
  int v121; // [rsp+108h] [rbp+8h]
  int v122; // [rsp+10Ch] [rbp+Ch]
  const unsigned __int16 *v123; // [rsp+110h] [rbp+10h]
  int v124; // [rsp+118h] [rbp+18h]
  int v125; // [rsp+11Ch] [rbp+1Ch]
  __int64 v126; // [rsp+120h] [rbp+20h]
  __int64 v127; // [rsp+128h] [rbp+28h]
  __int64 v128; // [rsp+130h] [rbp+30h]
  __int64 v129; // [rsp+138h] [rbp+38h]
  const unsigned __int16 *v130; // [rsp+140h] [rbp+40h]
  int v131; // [rsp+148h] [rbp+48h]
  int v132; // [rsp+14Ch] [rbp+4Ch]
  const unsigned __int16 *v133; // [rsp+150h] [rbp+50h]
  int v134; // [rsp+158h] [rbp+58h]
  int v135; // [rsp+15Ch] [rbp+5Ch]
  const unsigned __int16 *v136; // [rsp+160h] [rbp+60h]
  int v137; // [rsp+168h] [rbp+68h]
  int v138; // [rsp+16Ch] [rbp+6Ch]
  const unsigned __int16 *v139; // [rsp+170h] [rbp+70h]
  int v140; // [rsp+178h] [rbp+78h]
  int v141; // [rsp+17Ch] [rbp+7Ch]
  const unsigned __int16 *v142; // [rsp+180h] [rbp+80h]
  int v143; // [rsp+188h] [rbp+88h]
  int v144; // [rsp+18Ch] [rbp+8Ch]
  const unsigned __int16 *v145; // [rsp+190h] [rbp+90h]
  int v146; // [rsp+198h] [rbp+98h]
  int v147; // [rsp+19Ch] [rbp+9Ch]
  const unsigned __int16 *v148; // [rsp+1A0h] [rbp+A0h]
  int v149; // [rsp+1A8h] [rbp+A8h]
  int v150; // [rsp+1ACh] [rbp+ACh]
  const unsigned __int16 *v151; // [rsp+1B0h] [rbp+B0h]
  int v152; // [rsp+1B8h] [rbp+B8h]
  int v153; // [rsp+1BCh] [rbp+BCh]
  const unsigned __int16 *v154; // [rsp+1C0h] [rbp+C0h]
  int v155; // [rsp+1C8h] [rbp+C8h]
  int v156; // [rsp+1CCh] [rbp+CCh]
  const unsigned __int16 *v157; // [rsp+1D0h] [rbp+D0h]
  int v158; // [rsp+1D8h] [rbp+D8h]
  int v159; // [rsp+1DCh] [rbp+DCh]
  const unsigned __int16 *v160; // [rsp+1E0h] [rbp+E0h]
  int v161; // [rsp+1E8h] [rbp+E8h]
  int v162; // [rsp+1ECh] [rbp+ECh]
  const unsigned __int16 *v163; // [rsp+1F0h] [rbp+F0h]
  int v164; // [rsp+1F8h] [rbp+F8h]
  int v165; // [rsp+1FCh] [rbp+FCh]
  const unsigned __int16 *v166; // [rsp+200h] [rbp+100h]
  int v167; // [rsp+208h] [rbp+108h]
  int v168; // [rsp+20Ch] [rbp+10Ch]
  const unsigned __int16 *v169; // [rsp+210h] [rbp+110h]
  int v170; // [rsp+218h] [rbp+118h]
  int v171; // [rsp+21Ch] [rbp+11Ch]
  __int64 v172; // [rsp+220h] [rbp+120h]
  __int64 v173; // [rsp+228h] [rbp+128h]
  __int64 v174; // [rsp+230h] [rbp+130h]
  __int64 v175; // [rsp+238h] [rbp+138h]

  v174 = a35;
  v172 = a34;
  v36 = -1LL;
  v175 = 4LL;
  v173 = 4LL;
  v37 = *a33;
  if ( *a33 )
  {
    v38 = -1LL;
    do
      ++v38;
    while ( *((_BYTE *)v37 + v38) );
    v39 = v38 + 1;
  }
  else
  {
    v37 = &word_1802CFB96;
    v39 = 1;
  }
  v170 = v39;
  v169 = v37;
  v171 = 0;
  v40 = *a32;
  if ( *a32 )
  {
    v41 = -1LL;
    do
      ++v41;
    while ( *((_BYTE *)v40 + v41) );
    v42 = v41 + 1;
  }
  else
  {
    v40 = &word_1802CFB96;
    v42 = 1;
  }
  v167 = v42;
  v166 = v40;
  v168 = 0;
  v43 = *a31;
  if ( *a31 )
  {
    v44 = -1LL;
    do
      ++v44;
    while ( *((_BYTE *)v43 + v44) );
    v45 = v44 + 1;
  }
  else
  {
    v43 = &word_1802CFB96;
    v45 = 1;
  }
  v164 = v45;
  v163 = v43;
  v165 = 0;
  v46 = *a30;
  if ( *a30 )
  {
    v47 = -1LL;
    do
      ++v47;
    while ( *((_BYTE *)v46 + v47) );
    v48 = v47 + 1;
  }
  else
  {
    v46 = &word_1802CFB96;
    v48 = 1;
  }
  v161 = v48;
  v160 = v46;
  v162 = 0;
  v49 = *a29;
  if ( *a29 )
  {
    v50 = -1LL;
    do
      ++v50;
    while ( *((_BYTE *)v49 + v50) );
    v51 = v50 + 1;
  }
  else
  {
    v49 = &word_1802CFB96;
    v51 = 1;
  }
  v158 = v51;
  v157 = v49;
  v159 = 0;
  v52 = *a28;
  if ( *a28 )
  {
    v53 = -1LL;
    do
      ++v53;
    while ( *((_BYTE *)v52 + v53) );
    v54 = v53 + 1;
  }
  else
  {
    v52 = &word_1802CFB96;
    v54 = 1;
  }
  v155 = v54;
  v154 = v52;
  v156 = 0;
  v55 = *a27;
  if ( *a27 )
  {
    v56 = -1LL;
    do
      ++v56;
    while ( *((_BYTE *)v55 + v56) );
    v57 = v56 + 1;
  }
  else
  {
    v55 = &word_1802CFB96;
    v57 = 1;
  }
  v152 = v57;
  v151 = v55;
  v153 = 0;
  v58 = *a26;
  if ( *a26 )
  {
    v59 = -1LL;
    do
      ++v59;
    while ( *((_BYTE *)v58 + v59) );
    v60 = v59 + 1;
  }
  else
  {
    v58 = &word_1802CFB96;
    v60 = 1;
  }
  v149 = v60;
  v148 = v58;
  v150 = 0;
  v61 = *a25;
  if ( *a25 )
  {
    v62 = -1LL;
    do
      ++v62;
    while ( *((_BYTE *)v61 + v62) );
    v63 = v62 + 1;
  }
  else
  {
    v61 = &word_1802CFB96;
    v63 = 1;
  }
  v146 = v63;
  v145 = v61;
  v147 = 0;
  v64 = *a24;
  if ( *a24 )
  {
    v65 = -1LL;
    do
      ++v65;
    while ( *((_BYTE *)v64 + v65) );
    v66 = v65 + 1;
  }
  else
  {
    v64 = &word_1802CFB96;
    v66 = 1;
  }
  v143 = v66;
  v142 = v64;
  v144 = 0;
  v67 = *a23;
  if ( *a23 )
  {
    v68 = -1LL;
    do
      ++v68;
    while ( *((_BYTE *)v67 + v68) );
    v69 = v68 + 1;
  }
  else
  {
    v67 = &word_1802CFB96;
    v69 = 1;
  }
  v140 = v69;
  v139 = v67;
  v141 = 0;
  v70 = *a22;
  if ( *a22 )
  {
    v71 = -1LL;
    do
      ++v71;
    while ( *((_BYTE *)v70 + v71) );
    v72 = v71 + 1;
  }
  else
  {
    v70 = &word_1802CFB96;
    v72 = 1;
  }
  v137 = v72;
  v136 = v70;
  v138 = 0;
  v73 = *a21;
  if ( *a21 )
  {
    v74 = -1LL;
    do
      ++v74;
    while ( *((_BYTE *)v73 + v74) );
    v75 = v74 + 1;
  }
  else
  {
    v73 = &word_1802CFB96;
    v75 = 1;
  }
  v134 = v75;
  v133 = v73;
  v135 = 0;
  v76 = *a20;
  if ( *a20 )
  {
    v77 = -1LL;
    do
      ++v77;
    while ( *((_BYTE *)v76 + v77) );
    v78 = v77 + 1;
  }
  else
  {
    v76 = &word_1802CFB96;
    v78 = 1;
  }
  v131 = v78;
  v128 = a19;
  v126 = a18;
  v130 = v76;
  v132 = 0;
  v129 = 4LL;
  v79 = *a17;
  v127 = 4LL;
  if ( v79 )
  {
    v80 = -1LL;
    do
      ++v80;
    while ( *((_BYTE *)v79 + v80) );
    v81 = v80 + 1;
  }
  else
  {
    v79 = &word_1802CFB96;
    v81 = 1;
  }
  v124 = v81;
  v123 = v79;
  v125 = 0;
  v82 = *a16;
  if ( *a16 )
  {
    v83 = -1LL;
    do
      ++v83;
    while ( *((_BYTE *)v82 + v83) );
    v84 = v83 + 1;
  }
  else
  {
    v82 = &word_1802CFB96;
    v84 = 1;
  }
  v121 = v84;
  v118 = a15;
  v116 = a14;
  v114 = a13;
  v112 = a12;
  v110 = a11;
  v108 = a10;
  v120 = v82;
  v122 = 0;
  v119 = 4LL;
  v85 = *a9;
  v117 = 4LL;
  v115 = 4LL;
  v113 = 4LL;
  v111 = 8LL;
  v109 = 4LL;
  if ( v85 )
  {
    v86 = -1LL;
    do
      ++v86;
    while ( *((_BYTE *)v85 + v86) );
    v87 = v86 + 1;
  }
  else
  {
    v85 = &word_1802CFB96;
    v87 = 1;
  }
  v106 = v87;
  v105 = v85;
  v107 = 0;
  v88 = *a8;
  if ( *a8 )
  {
    v89 = -1LL;
    do
      ++v89;
    while ( *((_BYTE *)v88 + v89) );
    v90 = v89 + 1;
  }
  else
  {
    v88 = &word_1802CFB96;
    v90 = 1;
  }
  v103 = v90;
  v100 = a7;
  v98 = a6;
  v102 = v88;
  v104 = 0;
  v101 = 8LL;
  v91 = *a5;
  v99 = 8LL;
  if ( v91 )
  {
    do
      ++v36;
    while ( *((_BYTE *)v91 + v36) );
    v92 = v36 + 1;
  }
  else
  {
    v91 = &word_1802CFB96;
    v92 = 1;
  }
  v95 = v91;
  v96 = v92;
  v97 = 0;
  return tlgWriteTransfer_EventWriteTransfer(a1, a2, 0LL, 0LL, 0x21u, &v94);
}
