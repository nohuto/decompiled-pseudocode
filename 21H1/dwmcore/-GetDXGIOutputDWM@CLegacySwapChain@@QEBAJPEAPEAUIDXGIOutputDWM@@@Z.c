/*
 * XREFs of ?GetDXGIOutputDWM@CLegacySwapChain@@QEBAJPEAPEAUIDXGIOutputDWM@@@Z @ 0x18024A9CC
 * Callers:
...
__int64 __fastcall CLegacySwapChain::GetDXGIOutputDWM(CLegacySwapChain *this, struct IDXGIOutputDWM **a2)
{
  ...
  *a2 = 0LL;
  ...
  if ( v3 )
  {
    ...
    if ( v6 < 0 )
    {
      MilInstrumentationCheckHR_MaybeFailFast(v7, 0LL, 0, v6, 0x44Fu, 0LL);
    }
    else
    {
      ...
      v4 = v8;
      if ( v8 < 0 )
        MilInstrumentationCheckHR_MaybeFailFast(v9, 0LL, 0, v8, 0x451u, 0LL);
    }
